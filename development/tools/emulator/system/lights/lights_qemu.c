...
set_light_notifications( struct light_device_t* dev, struct light_state_t const* state )
{
    ...
    char buffer[64];
    snprintf( buffer, sizeof(buffer), "led:%d,%d,%d", state->flashOnMS, state->flashOffMS, state->color);
    D( "%s: led notification command: %s", __FUNCTION__, buffer );

    if (qemud_channel_send( fd, buffer, -1 ) < 0) {
        E( "%s: could not query led notification: %s", __FUNCTION__, strerror(errno) );
        close( fd );
        return -1;
    }
