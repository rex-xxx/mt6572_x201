...
data__poll(struct sensors_poll_device_t *dev, sensors_event_t* values)
{
    ...
    while (1) {
        ...
        if (sscanf(buff, "gyroscope:%g:%g:%g", params+0, params+1, params+2) == 3) {
            new_sensors |= SENSORS_GYROSCOPE;
            data->sensors[ID_GYROSCOPE].acceleration.x = params[0];
            data->sensors[ID_GYROSCOPE].acceleration.y = params[1];
            data->sensors[ID_GYROSCOPE].acceleration.z = params[2];
            continue;
        }
