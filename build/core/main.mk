...
ifdef FULL_BUILD
  ...
  # Work around for BUILD_TINY_ANDROID
  # Google remove user TAG, so install modules control by product config
  # make files, but BUILD_TINY_ANDROID not install any PRODUCT_PACKAGES module.
  # Google may use low-level product package instead tiny android.
  product_MODULES := $(PRODUCTS.$(INTERNAL_PRODUCT).PRODUCT_PACKAGES)
  product_MODULES := $(filter-out $(foreach p, $(product_MODULES), \
      $(PACKAGES.$(p).OVERRIDES)), $(product_MODULES))
  $(call expand-required-modules,product_MODULES,$(product_MODULES))
