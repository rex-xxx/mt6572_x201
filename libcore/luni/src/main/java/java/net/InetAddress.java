...
public class InetAddress implements Serializable {
    ...
    private static InetAddress[] lookupHostByName(String host) throws UnknownHostException {
        ...
        try {
            ...
            if (preferIPv6Addresses()) {
                Arrays.sort(addresses, LONGEST_FIRST);
            }else{
                Arrays.sort(addresses, SHORTEST_FIRST);
            }
            /// @}
            ...
            /// M: Debug logging for DNS query
            System.out.println("[CDS][DNS]" + detailMessage);
    ...
    private static InetAddress getHostByAddrImpl(InetAddress address) throws UnknownHostException {
        ...
        try {
            ...
            /// M: Disable reverse host name query for private IP address @{
            String hostname = "";
            
            if(isPrivateIp(address.ipaddress)){
               hostname = Libcore.os.getnameinfo(address, NI_NUMERICHOST);
            }else{            
               hostname = Libcore.os.getnameinfo(address, NI_NAMEREQD);
            }
            /// @}
    ...
    /// M: utility function for inet address sorting
    private static final Comparator<InetAddress> SHORTEST_FIRST = new Comparator<InetAddress>() {
        public int compare(InetAddress a1, InetAddress a2) {
            return a1.ipaddress.length - a2.ipaddress.length;
        }
    };

    /// M: utility function for inet address sorting
    private static final Comparator<InetAddress> LONGEST_FIRST = new Comparator<InetAddress>() {
        public int compare(InetAddress a1, InetAddress a2) {
            return a2.ipaddress.length - a1.ipaddress.length;
        }
    };

    /// M: check the byte address is private address or not
    private static boolean isPrivateIp(byte[] addr){
        
        if((addr[0] == 10) || ((addr[0] == (byte) 172) && (addr[1] >= 16 && addr[1] <= 31)) || (addr[0] == (byte) 192 && addr[1] == (byte) 168)){
            return true;
        }
