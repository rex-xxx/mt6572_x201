...
public class Socket {
    ...
    public synchronized void setSendBufferSize(int size) throws SocketException {
        ...
        System.out.println("[CDS]SO_SNDBUF:" + size);
    ...
    public synchronized void setReceiveBufferSize(int size) throws SocketException {
        ...
        System.out.println("[CDS]SO_RCVBUF:" + size);
    ...
    public synchronized void setSoTimeout(int timeout) throws SocketException {
        ...
        /// M: debug logging        
        System.out.println("[CDS]rx timeout:" + timeout);
    ...
    public void connect(SocketAddress remoteAddr, int timeout) throws IOException {
        ...
        synchronized (connectLock) {
            try {
                ...
                /// M: debug logging @{                
                System.out.println("[socket][" + inx + "] connection " + remoteAddr +";LocalPort="+getLocalPort()+ "("+ timeout+")");
                inx = inx+1;
                /// @}
                ...
                /// M: debug logging
                System.out.println("[socket][" + inx + ":"+ getLocalPort()+"] exception");
                impl.close();
                throw e;
            }
        }
    ...
    /**
     * Returns this socket's {@link SocketOptions#SO_SND_TIMEOUT send timeout}.
     * @hide
     */
    public synchronized int getSoSndTimeout() throws SocketException {
        checkOpenAndCreate(true);
        return (Integer) impl.getOption(SocketOptions.SO_SND_TIMEOUT);
    }
    
    /**
     * Sets this socket's {@link SocketOptions#SO_SND_TIMEOUT write timeout} in milliseconds.
     * Use 0 for no timeout.
     * To take effect, this option must be set before the blocking method was called.
     * @hide
     */
    public synchronized void setSoSndTimeout(int timeout) throws SocketException {
        ...
        impl.setOption(SocketOptions.SO_SND_TIMEOUT, Integer.valueOf(timeout));
    }

    /**
     * Returns TCP's RTO value
     * @hide
     */
    public synchronized int getTcpMaxRto() throws SocketException {
        checkOpenAndCreate(true);
        return (Integer) impl.getOption(SocketOptions.TCP_MAX_RTO);
    }
    
    /**
     * Configure TCP's RTO value
     * @hide   
     * To take effect, this option must be set before the blocking method was called.
     */
    public synchronized void setTcpMaxRto(int rto) throws SocketException {
        checkOpenAndCreate(true);
        ...
        if (rto < 5) {
            System.err.println("rto value is too small:" + rto);
            return;
        }
        System.out.println("rto value is " + rto);
        try{
            impl.setOption(SocketOptions.TCP_MAX_RTO, Integer.valueOf(rto));
        }catch(Exception e){
            e.printStackTrace();
        }
    }
