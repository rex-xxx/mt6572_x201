...
public class PlainSocketImpl extends SocketImpl {
    ...
    private int read(byte[] buffer, int offset, int byteCount) throws IOException {
        ...
        if (shutdownInput) {
            /// M: debug logging
            System.out.println("[CDS]shutdownInput in read");
            return -1;
        }
        ...
        if (readCount == 0) {
            /// M: debug logging
            System.out.println("[CDS]read byte is 0");
    ...
    private void write(byte[] buffer, int offset, int byteCount) throws IOException {
        ...
        if (streaming) {
            while (byteCount > 0) {
				...
				// / M: support snd timeout socket option for socket write
				// function @{
                if(bytesWritten == 0){//Add by MTK03594 for ALPS00076216
					System.out.println("[CDS]write is:" + bytesWritten + "/"
							+ byteCount);
                   throw new SocketTimeoutException("write timeout");
				} else if (bytesWritten != byteCount) {// Add by MTK04247 for
														// ALPS00239330
					// Modify by MTK80881 for especial application which has
					// been set SO_SND_TIMEOUT property
					System.out.println("[CDS]write is:" + bytesWritten + "/"
							+ byteCount);
					int exceptionCount = Integer.parseInt(System.getProperty(
							"SO_SND_TIMEOUT", "0"));
					System.out.println("[CDS]write exceptionCount:"
							+ exceptionCount);
					if (exceptionCount == 0)
						;
					else if (--exceptionCount == 0)
						throw new SocketException("write byteCount err");
					else
						System.setProperty("SO_SND_TIMEOUT", Integer
								.toString(exceptionCount));
                }
                /// @}
