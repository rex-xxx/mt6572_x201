...
class HttpURLConnectionImpl extends HttpURLConnection {
    ...
    @Override public final void connect() throws IOException {
        ...
        try {
            ...
            httpEngineFailure = e;
            throw e;
        /** M: Thrown IOException instead of NULL Exception to comply with SDK API*/
        } catch (Exception ex) {
            IOException exIo = new IOException(ex.getMessage());
            ex.printStackTrace();
            System.out.println("[CDS] fix other exception in HttpUrlConnection");
            httpEngineFailure = exIo;
            throw exIo;
        }
        /** @} */
    }
        ...
        try {
            if (doOutput) {
                if (method == HttpEngine.GET) {
                    ...
                    /// M: debug logging
                    System.out.println("initHttpEngine: implies a POST method");
    ...
    /// M: support setReadTimeout fucntion for HttpURLConnection
    @Override public void setReadTimeout(int timeout) {
        super.setReadTimeout(timeout);
        
        try{
            if (httpEngine != null) {
                HttpConnection httpConnect = httpEngine.getConnection();
                if(httpConnect != null){
                   System.out.println("[CDS] setReadTimeout in HttpUrlConnectionImpl");
                   httpConnect.setSoTimeout(timeout);
                }
            }
        }catch(Exception e){
            e.printStackTrace();
