...
//add by ping-hsun
HAVE_MADVISE              default: 1 if LCA_RAM_OPTIMIZE, else 0
  True if this system supports MADV_DONTNEED or an emulation of it.  If so,
  MADVISE is used instead of memset in calloc to clear large blocks.
...
//add by ping-hsun
DEFAULT_MADV_THRESHOLD      default: 16K
  The request size threshold for using MADVISE to service a clear
  request. Clear requests of at least this size will be serviced via
  madvise. The advantage of madvise is the potential gain on
  performance comparing to plain memset. The default is a measured
  value in a particularly profiled system. You can disable madvise by
  setting to MAX_SIZE_T.
...
#endif  /* linux */
#endif  /* HAVE_MREMAP */
#ifndef HAVE_MADVISE    //add by ping-hsun
...
#ifndef DEFAULT_MADV_THRESHOLD   //add by ping-hsun
#if HAVE_MADVISE
#define DEFAULT_MADV_THRESHOLD ((size_t)16U * (size_t)1024U)
#else   /* HAVE_MADVISE */
#define DEFAULT_MADV_THRESHOLD MAX_SIZE_T
#endif  /* HAVE_MADVISE */
#endif  /* DEFAULT_MADV_THRESHOLD */
...
#endif /* LACKS_FCNTL_H */
#endif /* HAVE_MMAP */
#if HAVE_MADVISE         //add by ping-hsun
#ifndef LACKS_SYS_MMAN_H
...
#ifndef WIN32
#ifdef _MTK_ENG_
#define MUNMAP_DEFAULT(a, s)    munmap_no_record((a), (s))
#else
#define MUNMAP_DEFAULT(a, s)  munmap((a), (s))
#endif
#define MMAP_PROT            (PROT_READ|PROT_WRITE|PROT_MALLOCFROMBIONIC)
...
#if HAVE_MADVISE
#ifndef WIN32
#define MADVISE_DEFAULT(a, s, b)    madvise((a), (s), (b))
#endif /* WIN32 */
#endif /* HAVE_MADVISE */
 ...
 */
#if HAVE_MADVISE
    #define USE_MADVISE                 (SIZE_T_ONE)

    #ifdef MADVISE
        #define CALL_MADVISE(a, s, b)   MADVISE((a), (s), (b))
    #else /* MADVISE */
        #define CALL_MADVISE(a, s, b)   MADVISE_DEFAULT((a), (s), (b))
    #endif /* MADVISE */
#else  /* HAVE_MADVISE */
    #define USE_MADVISE                 (SIZE_T_ZERO)

    #define CALL_MADVISE(a, s, b)       MFAIL
#endif /* HAVE_MADVISE */
...
//add by ping-hsun
#define use_madvise()         USE_MADVISE
...
//add by ping-hsun
#define madvise_align(A)    page_align((size_t)(A))
#define madvise_end(A, S)   (((size_t)(A) + (S)) & ~(mparams.page_size - SIZE_T_ONE))
...
//add by ping-hsun
static inline void *memset0(void *s, int c, size_t n) {
    if (/*__builtin_constant_p(c) &&*/ c == 0) {
        if (use_madvise() && n >= DEFAULT_MADV_THRESHOLD) {
            size_t start = madvise_align(s);
            size_t end   = madvise_end(s, n);
            if (end > start) {
                if (CALL_MADVISE((void*)(start), end - start, MADV_DONTNEED) == 0) {
                    size_t n0 = start - (size_t)(s);
                    size_t n1 = ((size_t)(s) + n) - end;
                    if (n0 != 0)
                        memset(s, 0, n0);
                    if (n1 != 0)
                        memset((void*)(end), 0, n1);
                    return s;
                } else {
                    error_log("memset0 madvise failed on %p %u: %s (%d)\n", s, n, strerror(errno), errno);
                }
            }
        }
    }
...
static void** ialloc(mstate m,
  ...
  */
  ...
  if (opts & 0x2) {       /* optionally clear the elements */
    memset0((size_t*)mem, 0, remainder_size - SIZE_T_SIZE - array_size);    //ping-hsun
  }
