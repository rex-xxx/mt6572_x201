...
# Mmap Debug  select different source files for eng and user build
# =========================================================
  ifeq ($(TARGET_BUILD_VARIANT),eng)
	...
	#remove exist file
	libc_common_src_files := $(filter-out arch-arm/syscalls/__munmap.S, $(libc_common_src_files))
	#add new file
	libc_common_src_files += \
	  arch-arm/syscalls/munmap.S
	endif
  else
  #remove exist file
  libc_common_src_files := $(filter-out arch-arm/syscalls/__munmap.S, $(libc_common_src_files))
...
# FD leakage select different source files for eng and user build
# =========================================================
  ifeq ($(TARGET_BUILD_VARIANT),eng)
    ifeq ($(filter banyan_addon banyan_addon_x86,$(TARGET_PRODUCT)),)
      libc_common_src_files += \
        unistd/fdleak_debug_func.c
    else
    #remove exist file
    libc_common_src_files := $(filter-out arch-arm/syscalls/__close.S, $(libc_common_src_files))
    libc_common_src_files := $(filter-out arch-arm/syscalls/__dup.S, $(libc_common_src_files))
    libc_common_src_files := $(filter-out arch-arm/syscalls/__pipe.S, $(libc_common_src_files))
    libc_common_src_files := $(filter-out arch-arm/syscalls/__pipe2.S, $(libc_common_src_files))
    libc_common_src_files := $(filter-out arch-arm/syscalls/__dup2.S, $(libc_common_src_files))
    libc_common_src_files := $(filter-out arch-arm/syscalls/__socket.S, $(libc_common_src_files))
    libc_common_src_files := $(filter-out arch-arm/syscalls/__eventfd.S, $(libc_common_src_files))
    libc_common_src_files := $(filter-out arch-arm/syscalls/__accept.S, $(libc_common_src_files))
    #add new file    
    libc_common_src_files += \
      arch-arm/syscalls/close.S \
      arch-arm/syscalls/dup.S \
      arch-arm/syscalls/pipe.S \
      arch-arm/syscalls/pipe2.S \
      arch-arm/syscalls/dup2.S \
      arch-arm/syscalls/socket.S \
      arch-arm/syscalls/eventfd.S \
      arch-arm/syscalls/accept.S
    endif
  else
  #remove exist file
  libc_common_src_files := $(filter-out arch-arm/syscalls/__close.S, $(libc_common_src_files))
...
# mtk80851: malloc/mspace_malloc debug {
# Use frame pointer register and APCS(Arm Procedure Call Standard) in MTK internal projects
# Notice: these also need force arm build
ifeq ($(TARGET_BUILD_VARIANT),eng)
  ifeq ($(filter banyan_addon banyan_addon_x86,$(TARGET_PRODUCT)),)
    ifeq ($(MTK_INTERNAL),yes)
      libc_common_cflags += \
		...
		-mapcs
    endif # ifeq ($(MTK_INTERNAL),yes)
      libc_common_cflags += \
      -D_FDLEAK_DEBUG_
  endif # ifeq ($(filter banyan_addon banyan_addon_x86,$(TARGET_PRODUCT)),)
  
  ifeq ($(MTK_LCA_RAM_OPTIMIZE),yes)
      libc_common_cflags += \
	-DDISABLE_MALLOC_DEBUG
  endif # ifeq ($(MTK_LCA_RAM_OPTIMIZE),yes)

  libc_common_cflags += \
	-DHAVE_MALLOC_DEBUG_FEATURE \
	-DHAVE_MSPACE_DEBUG_FEATURE
else
  libc_common_cflags += -DIS_USER_BUILD
endif
# } mtk80851: malloc/mspace_malloc debug
...
# Cortex-A7 have 64 byte data cache line
ifeq ($(strip $(MTK_CPU)), arm_cortexa7)
        libc_common_cflags += -DHAVE_64_BYTE_CACHE_LINE=1
endif
