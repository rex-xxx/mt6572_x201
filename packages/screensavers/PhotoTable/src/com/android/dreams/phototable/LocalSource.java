...
public class LocalSource extends PhotoSource {
    ...
    public Collection<AlbumData> findAlbums() {
        ...
        Cursor cursor = null;
        try {
            cursor = mResolver.query(MediaStore.Images.Media.EXTERNAL_CONTENT_URI,
            ...
            if (cursor != null) {
                ...
                /** M: close cursor in finally block */
                // cursor.close();
        ...
        try {
            ...
            if (cursor != null) {
                if (cursor.getCount() > howMany && mNextPosition == -1) {
                    /** M: ALPS00504572, Random.nextInt() maybe return negative value */
                    mNextPosition = Math.abs(mRNG.nextInt()) % (cursor.getCount() - howMany);
                }
                if (mNextPosition == -1) {
                    mNextPosition = 0;
                }
