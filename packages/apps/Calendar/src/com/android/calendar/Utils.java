...
public class Utils {
     ...
     * @return
     */
    public static boolean canUseProviderByUri(ContentResolver resolver, Uri uri) {
    ...
    /**
     * M: Get the first day that could be displayed in our calendar, which means this is the very
    ...
    public static Time getFirstDisplayTimeInCalendar(Context context) {
        Time t = new Time(getTimeZone(context, null));
        int firstDayOfWeek = getFirstDayOfWeek(context);
    ...
    public static Time getLastDisplayTimeInCalendar(Context context) {
        Time t = new Time(getTimeZone(context, null));
        int firstDayOfWeek = getFirstDayOfWeek(context);
        int weekNo = Time.getWeeksSinceEpochFromJulianDay(LAST_CALENDAR_DAY, firstDayOfWeek);
        // the julian day of Monday in the last week displayed
    ...
    public static Time getValidTimeInCalendar(Context context, Time t) {
        ...
        long first = getFirstDisplayTimeInCalendar(context).toMillis(false);
        long last = getLastDisplayTimeInCalendar(context).toMillis(false);
        // if time is invalid, then set to now
     ...
     * @return
     */
    public static Time getValidTimeInCalendar(Context context, long timeMillis) {
        Time t = new Time(getTimeZone(context, null));
        t.set(timeMillis);
        long first = getFirstDisplayTimeInCalendar(context).toMillis(false);
        long last = getLastDisplayTimeInCalendar(context).toMillis(false);
        // if time is invalid, then set to now
        if (timeMillis < first || timeMillis > last) {
            t.setToNow();
        }
        return t;
    }
     ...
     * @return
     */
    public static long setJulianDayInGeneral(Time time, int julianDay) {
        int dif = julianDay - Time.EPOCH_JULIAN_DAY;
        Time julianTime = new Time(time.timezone);
        if (dif <= 0) {
            LogUtil.d(TAG, "Julian day before epoch day, adjust by epoch day");
            julianTime.monthDay += dif;
            time.set(julianTime);
        } else {
     ...
     * @return
     */
    public static int getJulianDayInGeneral(Time time, boolean ignoreDst) {
        Time t = new Time(time);
        t.hour = 0;
        t.minute = 0;
        t.second = 0;
        t.normalize(ignoreDst);
        long millis = t.toMillis(ignoreDst);
        Time timeJulian = new Time(time.timezone); // 1970-1-1 00:00:00
        long millisJulian = timeJulian.toMillis(ignoreDst);
        long dif = millis - millisJulian;
