    ...
    public DeleteEventHelper(Context context, Activity parentActivity, boolean exitWhenDone) {
        ...
        mService = new AsyncQueryService(mContext) {
            ...
            protected void onQueryComplete(int token, Object cookie, Cursor cursor) {
                ///M:add error handling the condition that no data in cursor.
                if (cursor == null || cursor.getCount() != 1/**M:only 1 event*/) {
                    if(cursor != null) {
                        cursor.close();
                    }
                    return;
            ...
            protected void onUpdateComplete(int token, Object cookie, int result) {
                LogUtil.d(TAG, "onUpdateComplete, in DeleteEventHelper.");
            }
        };
    ...
    public void delete(long begin, long end, CalendarEventModel model, int which) {
        ...
        if (TextUtils.isEmpty(rRule)) {
            ...
            if (mParent != null && mParent.isFinishing()) {
                LogUtil.w(TAG, "delete failed, parent is finishing.");
                return;
            }
            dialog.show();
            mAlertDialog = dialog;
