...
public class SelectCalendarsSimpleAdapter extends BaseAdapter implements ListAdapter {
    ...
    ///M: Porting changelist 954652 from ICS.@{      
    ...
    private void initData(Cursor c) {
        ...
        int visibleColumn = c.getColumnIndexOrThrow(Calendars.VISIBLE);
        int ownerAccountColumn = c.getColumnIndexOrThrow(Calendars.OWNER_ACCOUNT);
        if (idColumn < 0 || nameColumn < 0 || colorColumn < 0 || visibleColumn < 0 || ownerAccountColumn < 0) {
            LogUtil.w(TAG, "getColumIndex failed, return like cursor == null");
            mRowCount = 0;
            mData = null;
            return;
        }
        ...
        mVisibleColumn = visibleColumn;
        mOwnerAccountColumn = ownerAccountColumn;
        /// @}
