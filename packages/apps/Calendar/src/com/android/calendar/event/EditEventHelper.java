...
import com.mediatek.calendar.extension.ExtConsts;
import com.mediatek.calendar.extension.ExtConsts.PCSync;
...
public class EditEventHelper {
    ...
    public static final String[] EVENT_PROJECTION = new String[] {
            ...
            Events.STATUS, // 21
            ///M:#Lunar#
            ExtConsts.LunarEvent.IS_LUNAR, ///M:22
    };
    ...
    ///M:#Lunar#
    protected static final int EVENT_INDEX_IS_LUNAR = 22;
    ...
    /**
     * M: update the modifyTime column after edit event, used for PCSync.
     */
    public static boolean updateModifyTime(ArrayList<ContentProviderOperation> ops, Uri eventUri) {
        if (eventUri == null || ops == null) {
            return false;
        }
        ContentProviderOperation.Builder b;
        ContentValues values = new ContentValues();
        values.clear();
        values.put(ExtConsts.PCSync.MODIFY_TIME, System.currentTimeMillis());
    ...
    public static void setModelFromCursor(CalendarEventModel model, Cursor cursor) {
        ...
        ///M:#Lunar# @{
        model.mIsLunar = cursor.getInt(EVENT_INDEX_IS_LUNAR) != 0;
        ///@}
    ...
    public static boolean setModelFromCalendarCursor(CalendarEventModel model, Cursor cursor) {
        ...
        while (cursor.moveToNext()) {
            ...
            ///M: #PC Sync# @{
            model.mAccountType = cursor.getString(CALENDARS_INDEX_ACCOUNT_TYPE);
            ///@}
            return true;
       }
       return false;
    ...
    public static boolean canRespond(CalendarEventModel model) {
        ...
        ///M:#PC Sync#, we do not allow Local account's event to do response.@{
        if (CalendarContract.ACCOUNT_TYPE_LOCAL.equals(model.mAccountType)) {
            return false;
        }
        ///@}
    ...
    ContentValues getContentValuesFromModel(CalendarEventModel model) {
        ...
        ///M:#Lunar# @{
        values.put(ExtConsts.LunarEvent.IS_LUNAR, model.mIsLunar ? 1 : 0);
        ///@}
        ///M:#PC Sync# if model's account type is local, do not allow it save attendees info.@{
        if (CalendarContract.ACCOUNT_TYPE_LOCAL.equals(model.mAccountType)) {
