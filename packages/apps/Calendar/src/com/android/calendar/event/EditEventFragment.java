...
public class EditEventFragment extends Fragment implements EventHandler {
    ...
    private void setModelIfDone(int queryType) {
        ...
        if (mOutstandingQueries == 0) {
            ...
            /// M: format the rRule of originalModel after loading UI. @{
            ...
            ///M: Send a message to UI thread to request view focus if the view
...
///TODO: extract these code out
    ...
    EditEventView getEditEventView() {
        return mView;
    }
    ///@}

    /**
     * M: format the rRule of originalModel, unify the rule style. The rule of mModel is created
     * by updateRecurrenceRule function, so should use it format the rule of originalModel,
     * otherwise the two model would not same when onDestroy, and brought many problems.
     */
    private void formatOriginalRrule() {
        int position = mView.mRepeatsSpinner.getSelectedItemPosition();
        int selection = mView.getRecurrenceIndexes().get(position);
        EditEventHelper.updateRecurrenceRule(selection, mOriginalModel, Utils
                .getFirstDayOfWeek(mContext) + 1);
    }
}
