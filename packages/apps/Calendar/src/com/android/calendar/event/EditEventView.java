...
public class EditEventView implements View.OnClickListener, DialogInterface.OnCancelListener,
        DialogInterface.OnClickListener, OnItemSelectedListener, IEditEventView {
    ...
    /// M: Repeate mode values @{
    private static final long MAX_MONTH_IN_MILLS = DateUtils.DAY_IN_MILLIS * 31;
    private static final int REPEATS_DAILY = 1;
    private static final int REPEATS_WEEKLY = 2;
    private static final int REPEATS_MONTHLY = 3;
    private static final int REPEATS_YEARLY = 4;
    private static final int REPEATS_NEVER = 5;
    ///@}
    ...
    private class TimeListener implements OnTimeSetListener {
        ...
        public void onTimeSet(TimePicker view, int hourOfDay, int minute) {
            ...
            if (mView == mStartTimeButton) {
                ...
                /// M: find out the last repeat mode, originalMinRepeatMode is used to check whether
                // should update repeatSpinner later. @{
                long originalDuration = endTime.toMillis(true) - startTime.toMillis(true);
                int originalMinRepeatMode = findMinReapeatMode(originalDuration);
                /// @}
                ...
                /// M: find out current minimum repeat mode, if is different wit
                // originalMinRepeatMode, means we should update the repeatSpinner, to add
                // or remove some options from mRepeatsSpinner.
                long currentDuration = endTime.toMillis(true) - startTime.toMillis(true);
                int currentMinRepeatMode = findMinReapeatMode(currentDuration);
                if (originalMinRepeatMode != currentMinRepeatMode) {
                    populateRepeats();
                }
                /// @}
    ...
    private class TimeClickListener implements View.OnClickListener {
        ...
        public void onClick(View v) {
            ...
            if (v.getId() == R.id.start_time) {
                Log.d(TAG, "TimeClickListener, mStartTimeButton is clicked.");
                setDateTimeViewId(ID_START_TIME);
            }
            if (v.getId() == R.id.end_time) {
                Log.d(TAG, "TimeClickListener, mEndTimeButton is clicked.");
                setDateTimeViewId(ID_END_TIME);
            }
            ///@}

            ///M: Multi-dialog issue @{
            if (mDialogManager.isAnyDialogShown()) {
                LogUtil.d(TAG, "There is a dialog shown, abort creating dialog: " + v);
                return;
            }
            ...
            ///M: keep DatePickerDialog and TimePickerDialog when rotate device @{
            EditEventTimePickerDialogFragment f = EditEventTimePickerDialogFragment
                    .newInstance(mTime.hour, mTime.minute,
                            DateFormat.is24HourFormat(mActivity), true, 0);
            f.show(mActivity.getFragmentManager(), EDIT_EVENT_TIME_DIALOG_FRAG);
            ///@}
        }
    }
    ...
    private void populateTimezone(long eventStartTime) {
        ///M: @{
        LogUtil.v(TAG, "populateTimezone, eventStartTime = " + eventStartTime + ", mTimezone = " + mTimezone);
        ///@}
        ...
        mTimezoneButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ///M: multi-dialog issue @{
                if (mDialogManager.isAnyDialogShown()) {
                    LogUtil.d(TAG, "There is dailog shown, abort showing timezone dialog");
                    return;
                }
                /// @}
                showTimezoneDialog();
    ...
    private void showTimezoneDialog() {
        ...
        ///M:Fix timezoneFooterView still show when the all timezones are showing @{
        if (!mTimezoneAdapter.isAllTimezonesShown()) {
    ...
    private void populateRepeats() {
        ...
        /// M: use minRepeatMode to check whether we can show out one repeat mode.
        // like: minRepeatMode is REPEATS_DAILY, every mode can be shown.
        // minRepeatMode is REPEATS_WEEKLY, daily repeat mode can't be shown. @{
        long duration  = mEndTime.toMillis(true) - mStartTime.toMillis(true);
        int minRepeatMode = findMinReapeatMode(duration);

        repeatArray.add(r.getString(R.string.does_not_repeat));
        recurrenceIndexes.add(EditEventHelper.DOES_NOT_REPEAT);

        if (minRepeatMode <= REPEATS_DAILY) {
        ...
        String format;
        if (minRepeatMode <= REPEATS_WEEKLY) {
            format = r.getString(R.string.weekly);
        ...
        // of the given day.
        if (minRepeatMode <= REPEATS_MONTHLY) {
        ...
        if (minRepeatMode <= REPEATS_YEARLY) {
            repeatArray
                    .add(String.format(format, DateUtils.formatDateTime(mActivity, when, flags)));
        ...
        /// M: find out last Repeat Mode, if it also can be shown now, do not change UI, just
        // keep select it. @{
        int lastRepeatMode = -1;
        // get the last Repeat Mode before change mRecurrenceIndexes.
        if (mRecurrenceIndexes.size() > 0) {
            lastRepeatMode = mRecurrenceIndexes.get(mRepeatsSpinner.getSelectedItemPosition());
        }

        mRecurrenceIndexes = recurrenceIndexes;

        // after change mRecurrenceIndexes, check whether also contains the old repeat mode.
        int position;
        if (lastRepeatMode != -1 && recurrenceIndexes.contains(lastRepeatMode)) {
            position = recurrenceIndexes.indexOf(lastRepeatMode);
        } else {
            position = recurrenceIndexes.indexOf(EditEventHelper.DOES_NOT_REPEAT);
        }
        /// @}
        ...
        /// M: if the duration is larger than one year, disable the mRepeatsSpinner.
        // else if the event has not original event, enable mRepeatsSpinner.
        if (mModel.mOriginalSyncId != null || minRepeatMode == REPEATS_NEVER) {
    ...
    private class DateClickListener implements View.OnClickListener {
        ...
        public void onClick(View v) {
            ...
            ///M: MTK extends here the google default code are skipped @{
            if (mExt.isExtensionEnabled()) {
                mExt.onDateClicked(v, mTime);
                return;
            }
            ///@}
            ...
            ///M: multi-dialog issue
            if (mDialogManager.isAnyDialogShown()) {
                Log.d(TAG, "Google default: one DataPicker dialog shown, return.");
                return;
            } else {
                Log.d(TAG, "Google default: no DataPicker dialog shown, show one.");
                mDialogManager.dialogShown();
                dpd.show();
            }
        }
    ...
    public void requestFocus() {
        if (!(mTitleTextView.isFocused() || mLocationTextView.isFocused() || mDescriptionTextView.isFocused() || mAttendeesList.isFocused())) {
...
///M: MTK code below
    ...
    private void setTimeZoneHint() {
        LogUtil.w(TAG, "timezone is not permitted, Default string is set to TimeZone button");
        mTimezoneTextView.setHint(R.string.timezone_label);
        mTimezoneButton.setHint(R.string.timezone_label);

    }

    /**
     * M: use time duration to find the minimum repeat cycle we can use.
     * like:
     * duration is 2 days, then return REPEATS_WEEKLY.
     * duration is 10 days, then return REPEATS_MONTHLY.
     * @param duration
     * @return
     */
    private int findMinReapeatMode(long duration) {
        if (duration < 0 || duration > DateUtils.YEAR_IN_MILLIS) {
            return REPEATS_NEVER;
        } else if (duration > MAX_MONTH_IN_MILLS) {
    ...
    private class DialogManager implements DialogInterface.OnDismissListener,
        ...
        public void dialogShown() {
            if (isAnyDialogShown()) {
                LogUtil.w(TAG, "There is already a dialog shown, but another dialog is " + "going to show.");
            }
        ...
        private void dialogDismissed() {
            if (!isAnyDialogShown()) {
                LogUtil.w(TAG, "There is no dialog shown, but some dialog dismissed.");
            }
    ...
    OnTimeSetListener getDateTimeOnTimeSetListener(View v) {
    ...
    /**
     * M: use to get mRecurrenceIndexes.
     */
    public ArrayList<Integer> getRecurrenceIndexes() {
        return mRecurrenceIndexes;
    }
}
