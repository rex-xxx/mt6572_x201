...
public class DayView extends View implements View.OnCreateContextMenuListener,
    ...
    */
    ...
    private void init(Context context) {
        ...
        if (mCalendarThemeExt.isThemeManagerEnable()) {
            ...
            mClickedColor = mResources
                    .getColor(R.color.day_event_clicked_background_color);
        }
        ///@}
    ...
    protected void onSizeChanged(int width, int height, int oldw, int oldh) {
        ...
        */
        mHoursWidth = mViewWidth - ((mViewWidth - mHoursWidth) / mNumDays * mNumDays);
        /** @} */
        mViewHeight = height;
...
//        mContext.startProgressSpinner();
    ...
    private void drawDayHeader(String dayStr, int day, int cell, Canvas canvas, Paint p) {
        ...
        if (mNumDays > 1) {
            ///M: if the string text width is larger than cell width, set the smaller font size to Paint.@{
            float dayHeaderFontSize = DAY_HEADER_FONT_SIZE;
            float dateHeaderFontSize = DATE_HEADER_FONT_SIZE;
            if (mDateStrWidth >= mCellWidth && mDateStrWidth2letter >= mCellWidth) {
            ...
            /** M: @{ */
            int textWidth = (int) p.measureText(dayStr + " " + dateNumStr);
            int margin = (mCellWidth > textWidth) ? (mCellWidth - textWidth) / 2 : 0;
            // Move compute x to here.
            x = computeDayLeftPosition(day + 1) - margin/* DAY_HEADER_RIGHT_MARGIN */;
            /**@}*/
