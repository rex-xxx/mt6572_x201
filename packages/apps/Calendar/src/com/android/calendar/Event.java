...
public class Event implements Cloneable {
    ...
    ///M:@{
    public static final int DECLINED_EVENT_ALPHA = 0x88;
    ///@}
    ...
    private static Event generateEventFromCursor(Cursor cEvents) {
        ...
        ///M: if the self attendee status is declined. reset the color.@{
        if (e.selfAttendeeStatus == Attendees.ATTENDEE_STATUS_DECLINED) {
            // not call Utils.getDeclinedColorFromColor(),because sometimes its
            // color seems like background color.
            int bg = 0xffffffff;// TODO:change the bg color value.
            e.color = Utils.getDeclinedColorFromColor(e.color, DECLINED_EVENT_ALPHA, bg);
        }
        ///@}
        return e;
    }
