...
public class EventInfoFragment extends DialogFragment implements OnCheckedChangeListener,
    ...
    /** M: For save state when rotate phone @{ */
    protected static final String BUNDLE_KEY_ORIGINAL_REMINDERS = "key_original_reminders";
    protected static final String BUNDLE_KEY_REMINDERS = "key_reminders";
    protected static final String BUNDLE_KEY_WHICH_EVENTS = "key_which_events";
    /** @} */
    ...
    /// M: Insert token for create exception event @{
    private static final int TOKEN_INSERT_EXCEPTION_EVENT = 1 << 6;
    /// @}
    ...
    /** M: Use for save event in DB while user exit this activity, while don't save when
     * rotate phone, however, we save some states like "reminders" "response" to bundle @{
     */
    private boolean mNeedSaveEvent = false;
    private ArrayList<ReminderEntry> mSavedReminders;
    /** @} */
    ...
    private class QueryHandler extends AsyncQueryService {
        ...
        protected void onBatchComplete(int token, Object cookie, ContentProviderResult[] results) {
            ...
            /// M: we save reminders for single update repeating event here, cause we can only
            // get right event id here
            if (token == TOKEN_INSERT_EXCEPTION_EVENT) {
                if (results == null || results.length == 0) {
                    Log.e(TAG, "Create exception event fail: no results returned");
                    return;
                }
                Uri uri = results[0].uri;
                if (uri == null) {
                    Log.e(TAG, "Create exception event fail: uri = null");
                    return;
                }
                Log.d(TAG, "Create exception event success: " + uri);
                try {
                    long eventId = Long.parseLong(uri.getLastPathSegment());
                    saveRemindersForEvent(eventId);
        ...
        protected void onQueryComplete(int token, Object cookie, Cursor cursor) {
            ...
            case TOKEN_QUERY_CALENDARS:
                ...
                /** M: We restore reminders from saved reminders preferentially @{ */
                if (mSavedReminders != null) {
                    restoreReminders(mSavedReminders);
                    mSavedReminders = null;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        if (savedInstanceState != null) {
            ...
            /** M: read the saved state @{ */
            mUserSetResponse = savedInstanceState.getInt(BUNDLE_KEY_ATTENDEE_RESPONSE,
                    Attendees.ATTENDEE_STATUS_NONE);
            mEditResponseHelper.setWhichEvents(savedInstanceState.getInt(BUNDLE_KEY_WHICH_EVENTS, UPDATE_ALL));
            mOriginalReminders = (ArrayList<ReminderEntry>) savedInstanceState.getSerializable(
                    BUNDLE_KEY_ORIGINAL_REMINDERS);
            mSavedReminders = (ArrayList<ReminderEntry>) savedInstanceState.getSerializable(BUNDLE_KEY_REMINDERS);
            /** @} */
            /**M:**/
            mWhichDelete = savedInstanceState.getInt(BUNDLE_KEY_WHICH_DELETE, -1);
        }
        ...
        mAddReminderBtn = (Button)mView.findViewById(R.id.reminder_add);
    ...
    public void onSaveInstanceState(Bundle outState) {
        ...
        /** Read states from UI and save instance @{ */
        int response;
        ...
        outState.putInt(BUNDLE_KEY_WHICH_EVENTS, mEditResponseHelper.getWhichEvents());
        outState.putSerializable(BUNDLE_KEY_ORIGINAL_REMINDERS, mOriginalReminders);
        mReminders = EventViewUtils.reminderItemsToReminders(mReminderViews,
                mReminderMinuteValues, mReminderMethodValues);
        outState.putSerializable(BUNDLE_KEY_REMINDERS, mReminders);
        /** @} */
        /** M: **/
        outState.putInt(BUNDLE_KEY_WHICH_DELETE, mWhichDelete);
        /**M: call super at last*/
    ...
    public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
        ...
        if (!mIsDialog && !mIsTabletConfig || mWindowStyle == EventInfoFragment.FULL_WINDOW_STYLE) {
            ...
            updateMenu();
            ///M: #extension# @{
            mOptionsMenuExt = ExtensionFactory.getEventInfoOptionsMenuExt(getActivity(), mEventId);
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (itemId == android.R.id.home) {
            ...
            mDeleteHelper.delete(mStartMillis, mEndMillis, mEventId, -1, onDeleteRunnable);
        /**
         * M: #extension# Show edit/delete buttons only in non-dialog configuration @{
         */
        } else if (!mIsDialog && !mIsTabletConfig || mWindowStyle == EventInfoFragment.FULL_WINDOW_STYLE){
            mOptionsMenuExt.onOptionsItemSelected(item.getItemId());
        /** @} */
        }
    ...
    public void onDestroyView() {
        /** M: only save event when it's really necessary @{ */
        if (!mEventDeletionStarted && mNeedSaveEvent) {
            mNeedSaveEvent = false;
            boolean responseSaved = saveResponse();
            /// M: If save response for repeating events is udpate_single, we have to
            // try to save reminders for new exception event in onInsertComplete, and toast,
            // cause saveReminders() will use mEventId
            if (responseSaved && mIsRepeating
                    && mEditResponseHelper.getWhichEvents() == UPDATE_SINGLE) {
                // Read reminders from UI
                mReminders = EventViewUtils.reminderItemsToReminders(mReminderViews,
                        mReminderMinuteValues, mReminderMethodValues);
                Toast.makeText(getActivity(), R.string.saving_event, Toast.LENGTH_SHORT).show();
            } else {
                boolean remindersSaved = saveReminders();
                if (remindersSaved || responseSaved) {
                    Toast.makeText(getActivity(), R.string.saving_event, Toast.LENGTH_SHORT).show();
                }
            }
        }
    ...
    private void createExceptionResponse(long eventId, int status) {
        ...
        /// M: use special token @{
        mHandler.startBatch(TOKEN_INSERT_EXCEPTION_EVENT, null, CalendarContract.AUTHORITY, ops,
                Utils.UNDO_DELAY);
        /// @}
   }
    ...
    void updateResponse(View view) {
        ...
        if (!mCanModifyCalendar || (mHasAttendeeData && mIsOrganizer && mNumOfAttendees <= 1) ||
                (mIsOrganizer && !mOwnerCanRespond) || isLocalAccount/*M:is a local account*/ ||
                (mCalendarOwnerAttendeeId == EditEventHelper.ATTENDEE_ID_NONE)) {
    ...
    private void setTextCommon(View view, int id, CharSequence text) {
        ...
        ///M: for SNS plugin @{
        /*
         * First, it must be a all-day event, then we will judge if it is a SNS
         * account supported by SNS plugin, we will put the SNS icon and
         * birthday string into the title TextView if yes.
         */
        if (mAllDay && id == R.id.title) {
            setSNSData(view, textView);
        }
        ///@}
    }
    ...
    ///M: for SNS plugin @{
    /*
     * Set SNS data into the TextView, and add a SNS logo.
     */
    private void setSNSData(View view, TextView tv) {
        // we MUST initialize the helper first
        SNSCalendarDataHelper.initialize(mContext);

        // get account type and user id from cursor
        final String accountType = mEventCursor
                .getString(EVENT_INDEX_ACCOUNT_TYPE);
        final String userId = mEventCursor.getString(EVENT_INDEX_SYNC_ID);
        // see if the account if supported by SNS plugin
        if (SNSCalendarDataHelper.isAccountTypeSupported(accountType)) {
            Drawable icon = SNSCalendarDataHelper.getAccountIcon(mContext,
                    accountType);

            if (icon != null) {
                // get the birthday string to display
                final int calendarId = (int) mEventCursor
                        .getLong(EVENT_INDEX_CALENDAR_ID);
                String birthday = SNSCalendarDataHelper.getBirthdayString(
                        mContext, calendarId);
                if (birthday != null) {
                    // add a space to look better
                    tv.append(" ");
                    tv.append(birthday);
                }
                ...
                // pad to its parent
                final int pad = 1;
                // logo's size is adjusted according to title's TextView
                final int size = tv.getHeight() - pad;

                // get the SNS logo's view
                ImageView im = (ImageView) view.findViewById(R.id.snslogo);
                if (im != null) {
                    im.setMinimumWidth(size);
                    im.setMinimumHeight(size);
                    im.setImageDrawable(icon);
                    im.setOnClickListener(new OnClickListener() {
                        @Override
                        public void onClick(View v) {
                            Intent i = SNSCalendarDataHelper
                                    .getPostActionIntent(mContext, accountType,
                                            userId);
                            if (i != null) {
                                Log.d(TAG, "sns icon clicked, start to post.");
                                mActivity.startActivity(i);
                            } else {
                                Log.d(TAG, "sns icon clicked, intent is null, can not post.");
                            }
                        }
                    });
                }
    ...
    public void onPause() {
        ...
        if (mDeleteDialogVisible && mDeleteHelper != null) {
            /**M:*/
            mWhichDelete = mDeleteHelper.getWhichDelete();
    ...
    public void onResume() {
        ...
        if (mDeleteDialogVisible) {
            ...
            mDeleteHelper.setOnDismissListener(createDeleteOnDismissListener());
            mDeleteHelper.delete(mStartMillis, mEndMillis, mEventId, mWhichDelete, onDeleteRunnable);
        }
    }
    ...
    private boolean saveReminders() {
        ...
        mReminders = EventViewUtils.reminderItemsToReminders(mReminderViews,
                mReminderMinuteValues, mReminderMethodValues);
        return saveRemindersForEvent(mEventId);
    }

    /**
     * M: Save reminders for event with eventId, this function use mReminders to save
     * @param eventId
     * @return
     */
    private boolean saveRemindersForEvent(long eventId) {
        Log.d(TAG, "Save reminders for event: " + eventId);
        ...
        boolean changed = EditEventHelper.saveReminders(ops, eventId, mReminders,
        ...
        /** M: Use mHandler to do DB operations @{ */
        // save new reminders
        mHandler.startBatch(0, null, Calendars.CONTENT_URI.getAuthority(), ops, 0);
        ...
        if (hasAlarm != mHasAlarm) {
            ...
            mHandler.startUpdate(0, null, uri, values, null, null, 0);
        }
        /** @} */
   ...
   ///M: add removeDuplicate() function. @{
    ...
    private Button mAddReminderBtn = null;
    ...
    /**
     * M: Set need save event
     * @param need
     */
    public void setNeedSaveEvent(boolean need) {
        mNeedSaveEvent = need;
    }

    /**
     * M: Restore reminders on UI from an ArrayList
     * @param reminders
     */
    public void restoreReminders(ArrayList<ReminderEntry> reminders) {
        // Temporary reminders used to be processed to add on UI
        ArrayList<ReminderEntry> tempReminders = new ArrayList<ReminderEntry>();
        mUnsupportedReminders.clear();
        ...
        for (int i = 0; i < len; ++i) {
            int minutes = reminders.get(i).getMinutes();
            int method = reminders.get(i).getMethod();
            ...
            if (method != Reminders.METHOD_DEFAULT && !mReminderMethodValues.contains(method)) {
                ...
                tempReminders.add(ReminderEntry.valueOf(minutes, method));
            }
        }
        // Sort appropriately for display (by time, then type)
        Collections.sort(tempReminders);
        ...
        // Insert any minute values that aren't represented in the minutes list.
        for (ReminderEntry re : tempReminders) {
            EventViewUtils.addMinutesToList(
                    mActivity, mReminderMinuteValues, mReminderMinuteLabels, re.getMinutes());
        }
        // Create a UI element for each reminder.  We display all of the reminders we get
    ...
    /**M: @{*/
    /**key used to save which delete status*/
    protected static final String BUNDLE_KEY_WHICH_DELETE = "key_which_delete";
