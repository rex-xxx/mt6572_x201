...
public class AgendaWindowAdapter extends BaseAdapter
    ...
    private static int[] sTopDeviationInfo = new int[2]; // Save the top deviation to make switch screen smoothly.
    ...
    /// M: Set the top deviation when AgendaFragment onCreatView.
    public static void setTopDeviation(int[] topDeviationInfo) {
        sTopDeviationInfo = topDeviationInfo.clone();
    }
    /// @}

    /// M: Save the top deviation from AgendaFragment onSaveInstanceState, this
    //  will be used to make the
    //  switch screen action looks smoothly. @{
    public int[] saveTopDeviation(Time time) {
        /// M: Get the first visible position, if it is 0, it is the list
        //  header; if getItemViewType(firstVisiblePosition - 1) is 0, it is a DAYTYPE
        //  item; else it is a MEETINGTYPE item. Then calculate the deviation
        //  from the first same time instance position.
        int firstVisiblePosition = mAgendaListView.getFirstVisiblePosition();
        View firstChildView = mAgendaListView.getChildAt(0);
        boolean isTopIsHeader = firstVisiblePosition == 0;
        boolean isDayType = getItemViewType(firstVisiblePosition - 1) == 0;
        int topDeviation = (firstChildView == null) ? 0 : firstChildView.getTop();
        if (isDayType || isTopIsHeader) {
            int dayTypeItemHeight;
            /// M: Get the height of dayType item.
            if (isDayType) {
                dayTypeItemHeight = (firstChildView == null) ? 0 : firstChildView.getHeight();
            } else {
                View firstDayTypeView = mAgendaListView.getChildAt(1);
                dayTypeItemHeight = (firstDayTypeView == null) ? 0 : firstDayTypeView.getHeight();
                int headerTypeItemHeight = (firstChildView == null) ? 0 : firstChildView
                        .getHeight();
                topDeviation += headerTypeItemHeight;
            }
            /// M: add one dayType item height to result.
            topDeviation += dayTypeItemHeight;
        }

        int itemNum = 0;
        if (!isDayType && !isTopIsHeader) {
            int meetingTypeItemHeight = (firstChildView == null) ? 0 : firstChildView.getHeight();
            /// M: This is a MEETING TYPE item, add the items number above it.
            int firstItemOnSameTime = findEventPositionNearestTime(time, -1);
            for (int i = firstVisiblePosition; i > firstItemOnSameTime
                    && getItemViewType(i - 1) == 1; i--) {
                itemNum++;
            }
        } else {
            itemNum = 1;
        }
        sTopDeviationInfo[0] = itemNum;
        sTopDeviationInfo[1] = topDeviation;
    ...
    private class QueryHandler extends AsyncQueryHandler {
        ...
        protected void onQueryComplete(int token, Object cookie, Cursor cursor) {
            ///M: @{
            if (cursor == null) {
                LogUtil.e(TAG, "onQueryComplete, cursor is null.");
                if (mHeaderView != null) {
                    mHeaderView.setText(R.string.loading_failed);
                }
                return;
            }
            ///@}
            ...
            /// M: Update header and footer before set the selected position, because add footer
            //  view will change the list view items, setSelectionFromTop function will not work. @{
            if (!mDoneSettingUpHeaderFooter) {
                ...
                /// M: Add OnTouchListener to header / footer view, to support color feedback. @{
                mFooterView.setOnTouchListener(headerFooterOnTouchListener);
                mHeaderView.setOnTouchListener(headerFooterOnTouchListener);
                /// @}
            ...
            if (cursorSize > 0 || mAdapterInfos.isEmpty() || data.queryType == QUERY_TYPE_CLEAN) {
                ...
                if (data.goToTime == null) { // Typical Scrolling type query
                    ...
                    if (newPosition >= 0) {
                        ...
                        /// M: Set the top deviation to remain the same ListView place.
                        mAgendaListView.setSelectionFromTop(newPosition + OFF_BY_ONE_BUG + sTopDeviationInfo[0],
                                sTopDeviationInfo[1]);
                        Log.d(TAG, "Setting listview to " + (sTopDeviationInfo[0] + OFF_BY_ONE_BUG) +
                                "   add top deviation height: " + sTopDeviationInfo[1]);
                        Time actualTime = new Time(mTimeZone);
                        actualTime.set(goToTime);
            ...
            synchronized (SLOCK) {
                ...
                /// M: If is refresh called, Clear the top deviation data when 3 times query are
                //  all completed, means refresh action finished. @{
                if (data.goToTime != null && mQueryQueue.size() == 0) {
                    sTopDeviationInfo[0] = 0;
                    sTopDeviationInfo[1] = 0;
                    if (DEBUGLOG) {
                        Log.e(TAG, "Compelete query time: " + System.currentTimeMillis());
                    }
                }
                /// @}
            }
    ...
    /// M: Add this listener to HEADER / FOOTER VIEW to support touch background color feedback. @{
    OnTouchListener headerFooterOnTouchListener = new OnTouchListener() {
        public boolean onTouch(View v, MotionEvent event) {
            switch (event.getAction()) {
                case MotionEvent.ACTION_DOWN:
                    ICalendarThemeExt calendarThemeExt = ExtensionFactory
                            .getCalendarTheme(mContext);
                    if (calendarThemeExt.isThemeManagerEnable()) {
                        v.setBackgroundColor(calendarThemeExt.getThemeColor());
                    } else {
                        v.setBackgroundColor(mResources.getColor(R.color.pressed));
                    }
                    break;
    ...
    /**
     * M:check the query range.
     * if the data has been queried, do not need do it again.
     */
    private boolean checkQueryRange(QuerySpec queryData) {
        if (!mAdapterInfos.isEmpty()) {
            int start = mAdapterInfos.getFirst().start;
            int end = mAdapterInfos.getLast().end;
            if ((queryData.queryType == QUERY_TYPE_OLDER && start <= Utils.FIRST_CALENDAR_DAY)
                    || (queryData.queryType == QUERY_TYPE_NEWER && end >= Utils.LAST_CALENDAR_DAY)) {
                /**
                 * if the previous newer(or older) query's date is out of range.
                 * do nothing for this newer(or older) query.
                 */
                Log.d(TAG, "preHandleQuery: out of range, do nothing");
                return false;
            }
        }
        return true;
    ...
    /**
     * M: adjust the query range.
     * If it is out of range, correct the end or start for the query data.
     */
    private void adjustQueryRange(QuerySpec queryData) {
        if ((queryData.queryType == QUERY_TYPE_CLEAN || queryData.queryType == QUERY_TYPE_NEWER)
                && queryData.end > Utils.LAST_CALENDAR_DAY) {
            queryData.end = Utils.LAST_CALENDAR_DAY;
            Log.w(TAG, "limitQueryData, reset end");
        }
        if ((queryData.queryType == QUERY_TYPE_CLEAN || queryData.queryType == QUERY_TYPE_OLDER)
                && queryData.start < Utils.FIRST_CALENDAR_DAY) {
            queryData.start = Utils.FIRST_CALENDAR_DAY;
            Log.w(TAG, "limitQueryData, reset start");
        }
    }
}
