...
public class TimelineHorizontalScrollView extends HorizontalScrollView {
    ...
    /* M: User's scroll action will remove app's scroll action,
     * then, App will show user's final scroll position,
     * but not move to user's position for that onScrollEnd()'s mAppScroll is true.
     * mAppScroll = true means: position is correct, just need to set view.
     * mAppScroll = false means: view is right, just need to set position.
     * @{
     */
    private int mLastOnTouchEvent;
    
    private void setLastEvent(MotionEvent ev) {
        if (LOG) MtkLog.v(TAG, "setLastEvent(" + ev + ")");
    ...
    private void consumeScrollEndedIfNeed() {
        ...
        case MotionEvent.ACTION_MOVE: {
            if (mAppScroll) {
                ...
                mAppScroll = false;
                if (LOG) MtkLog.v(TAG, "consumeScrollEndedIfNeed() app scroll is stoped by user touch event.");
            }
            break;
        }
