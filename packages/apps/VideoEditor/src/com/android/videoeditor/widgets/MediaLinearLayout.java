 ...
 */
public class MediaLinearLayout extends LinearLayout implements ProjectStatePuller {
    // Logging
            ...
            public boolean onSingleTapConfirmed(View view, int area, MotionEvent e) {
                ...
                switch (area) {
                    case ItemSimpleGestureListener.LEFT_AREA: {
                        if (view.isSelected() && canPickTransition()) {
    ...
    public void invalidateActionBar() {
        ...
        //invalid current select view to refresh add transition icon state
        if (mSelectedView != null) {
    ...
    public boolean onDragEvent(DragEvent event) {
        ...
        switch (event.getAction()) {
            case DragEvent.ACTION_DRAG_STARTED: {
                ...
                mLeftHandle.saveVisibility();
                mRightHandle.saveVisibility();
            ...
            case DragEvent.ACTION_DRAG_ENDED: {
                ...
                //Restore the handles visibility
                mLeftHandle.restoreVisibility();
                mRightHandle.restoreVisibility();
    ...
    private void select(View selectedView) {
        ...
        if (tag instanceof MovieMediaItem) {
            ...
            if (LOG) MtkLog.v(TAG, "select() videoClip=" + videoClip + ", appBoundaryBeginTime=" + mi.getAppBoundaryBeginTime()
                    + ", appTimelineDuration=" + mi.getAppTimelineDuration());
            if (videoClip) {
                mLeftHandle.setVisibility(View.VISIBLE);
                ...
                mLeftHandle.setListener(new HandleView.MoveListener() {
                    ...
                    public boolean onMove(HandleView view, int left, int delta) {
                        ...
                        long leftWidth = getWidth() - (2 * mHalfParentWidth);
                        if (leftWidth <= 0) {
                            if (LOG) MtkLog.w(TAG, "mLeftHandle.onMove() leftWidth=" + leftWidth + ", do nothing.");
                            return false;
                        }
            ...
            mRightHandle.setListener(new HandleView.MoveListener() {
                ...
                public boolean onMove(HandleView view, int left, int delta) {
                    ...
                    long leftWidth = getWidth() - (2 * mHalfParentWidth);
                    if (leftWidth <= 0) {
                        if (LOG) MtkLog.w(TAG, "mRightHandle.onMove() leftWidth=" + leftWidth + ", do nothing.");
                        return false;
                    }
                    // Compute the new duration
    ...
    public void enableOrDisableAddMediaItemButtons(boolean isEnable) {
        mLeftAddClipButton.setAlpha(!isEnable ? 100 : 255);
        mLeftAddClipButton.setEnabled(isEnable);
        
        mRightAddClipButton.setAlpha(!isEnable ? 100 : 255);
        mRightAddClipButton.setEnabled(isEnable);
    }
    
    private static final boolean LOG = true;

    @Override
    public boolean canPickTransition() {
        //same as enabled in onPrepareActionMode
        boolean canPick = !ApiService.isProjectBeingEdited(mProject.getPath()) && !mPlaybackInProgress;
        if (LOG) MtkLog.v(TAG, "canPickTransition() return " + canPick);
        return canPick;
    }
}

interface ProjectStatePuller {
    boolean canPickTransition();
}
