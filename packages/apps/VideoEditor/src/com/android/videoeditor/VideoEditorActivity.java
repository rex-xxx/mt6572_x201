...
public class VideoEditorActivity extends VideoEditorBaseActivity
        ...
        switch (item.getItemId()) {
            ...
            case R.id.menu_item_capture_video: {
                ...
                try {
                    ...
                    intent.putExtra(MediaStore.EXTRA_VIDEO_QUALITY, CamcorderProfile.QUALITY_MTK_HIGH);//need check for JB
    ...
    public void onClickHandler(View target) {
        if (LOG) MtkLog.v(TAG, "onClickHandler() mPendingMoveCount=" + mPendingMoveCount);
    ...
    protected void setAspectRatio(final int aspectRatio) {
        ...
        /// We change the behavier to stop it until set aspec and recreate transition both finished
        logd("mProject: " + mProject + ", mPreviewThread: " + mPreviewThread);
        if (mProject != null && mPreviewThread != null) {
            if (mPreviewThread.isPlaying()) {
                // Save the pos before change 
                long stopTimeMs = mProject.getPlayheadPos();
                // This method will change playhead pos to 0, because it has already been stopped by ApiService.
                mPreviewThread.previewStopped(false);
                movePlayhead(stopTimeMs);
                logd("isPlaying: true " + ", stopTimeMs" + stopTimeMs);
            }
        }
        /// @}
    }
