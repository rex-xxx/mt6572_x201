...
public abstract class VideoEditorBaseActivity extends Activity {
    ...
    private class ServiceListener extends ApiServiceListener {
        ...
        public void onVideoEditorCreated(String projectPath, VideoEditorProject project,
            ...
            if (LOG) MtkLog.v(TAG, "onVideoEditorCreated(" + projectPath + ", " + project + ") mProjectPath="
                    + mProjectPath + ", mProject=" + mProject);
            // Check if the VideoEditor is the one we are expecting
            if (!projectPath.equals(mProjectPath)) {
                return;
            }
        ...
        public void onVideoEditorLoaded(String projectPath, VideoEditorProject project,
                List<MediaItem> mediaItems, List<AudioTrack> audioTracks, Exception exception) {
            if (LOG) MtkLog.v(TAG, "onVideoEditorLoaded(" + projectPath + ", " + project + ") mProjectPath="
                    + mProjectPath + ", mProject=" + mProject);
            if (!projectPath.equals(mProjectPath)) {
                return;
            }
            ...
            if (exception != null || project == null) {
                ...
                /* M: when rotating quickly, this callback will be delayed.
                 * Then, A activity's onLoaded will be delayed to B activity.
                 * In this case, we should update mProject.
                 */
                if (mProject == null || mProject != project) {
                    if (LOG && mProject != null) MtkLog.w(TAG, "onVideoEditorLoaded() delayed actions!!");
                    mProject = project;
                    initializeFromProject(true);
                }
            }
        ...
        public void onVideoEditorExportComplete(String projectPath, String filename,
            ...
            /// M: clear it first, so onExportComplete() can render frame.
    ...
    public void onDestroy() {
        ...
        if (mProjectPath != null) {
            if (!isChangingConfigurations() || (MtkFeatureSwitcher.isCancelGeneratingEnabled() && !isExporting())) {
    ...
    protected abstract boolean isExporting();
}
