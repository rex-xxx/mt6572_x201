...
public class CellBroadcastConfigService extends IntentService {
            ...
            for (String channelRange : ranges.split(",")) {
                ...
                if (dashIndex != -1) {
                    ...
                    if (enable) {
                        ...
                        GeminiSmsManager.enableCellBroadcastRangeGemini(startId, endId, CellBroadcastReceiver.sSlotId);
                    } else {
                        if (DBG) {
                            log("disabling emergency IDs " + startId + '-' + endId);
                        }
                        //manager.disableCellBroadcastRange(startId, endId);
                        GeminiSmsManager.disableCellBroadcastRangeGemini(startId, endId, CellBroadcastReceiver.sSlotId);
                    }
                } else {
                    ...
                    if (enable) {
                        ...
                        GeminiSmsManager.enableCellBroadcastGemini(messageId, CellBroadcastReceiver.sSlotId);
                    } else {
                        if (DBG) {
                            log("disabling emergency message ID " + messageId);
                        }
                        //manager.disableCellBroadcast(messageId);
                        GeminiSmsManager.disableCellBroadcastGemini(messageId, CellBroadcastReceiver.sSlotId);
                    }
                }
            }
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        if (ACTION_ENABLE_CHANNELS.equals(intent.getAction())) {
            try {
                ...
                if (enableEmergencyAlerts) {
                    if (!TextUtils.isEmpty(emergencyIdRange)) {
                        ...
                        GeminiSmsManager.enableCellBroadcastRangeGemini(
                                SmsCbConstants.MESSAGE_ID_CMAS_FIRST_IDENTIFIER,
                                SmsCbConstants.MESSAGE_ID_CMAS_ALERT_EXERCISE, 
                    ...
                    if (!TextUtils.isEmpty(emergencyIdRange)) {
                        ...
                        //        SmsCbConstants.MESSAGE_ID_PWS_FIRST_IDENTIFIER,
                        //        SmsCbConstants.MESSAGE_ID_PWS_LAST_IDENTIFIER);
                        GeminiSmsManager.disableCellBroadcastRangeGemini(
                                SmsCbConstants.MESSAGE_ID_CMAS_FIRST_IDENTIFIER,
                                SmsCbConstants.MESSAGE_ID_CMAS_ALERT_EXERCISE,
                                CellBroadcastReceiver.sSlotId);
                    }
            ...
            if ( enableCB ) {
                if (GeminiSmsManager.activateCellBroadcastSms(true,
                        CellBroadcastMainSettings.sReadySlotId)) {
