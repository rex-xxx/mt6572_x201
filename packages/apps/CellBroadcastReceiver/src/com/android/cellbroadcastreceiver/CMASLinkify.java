...
import com.android.cellbroadcastreceiver.CMASAlertFullWindow.IAutoLinkClick;
...
public class CMASLinkify {
    ...
    public static final boolean addLinks(Spannable text, int mask, IAutoLinkClick autoLinkclicked) {
        if (mask == 0) {
            return false;
        }
        ...
        for (LinkSpec link: links) {
            applyLink(link.url, link.start, link.end, text, autoLinkclicked);
        }
    ...
    private static final void applyLink(String url, int start, int end, Spannable text) {
        URLSpan span = new UrlSpannable(url);
        
        text.setSpan(span, start, end, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
    }

    private static final void applyLink(String url, int start, int end, Spannable text, IAutoLinkClick autoLinkClick) {
        URLSpan span = new UrlSpannable(url, autoLinkClick);
...
class UrlSpannable extends URLSpan {

    private IAutoLinkClick mAutoLinkClick = null;

    public UrlSpannable(String url) {
        super(url);
    }

    public UrlSpannable(String url, IAutoLinkClick autoLinkClick) {
        super(url);
        mAutoLinkClick = autoLinkClick;
    }

    @Override
    public void onClick(View widget) {
        Log.i("Linkify", "enter onclick");
        ...
        try {
            if (widget instanceof TextView) {
                /*
                 * TextView textView = (TextView)widget; boolean
                 * isWebURL = false; String scheme = uri.getScheme();
                 */

                context.startActivity(intent);
                if (mAutoLinkClick != null) {
                    mAutoLinkClick.onAutoLinkClicked();
                }
