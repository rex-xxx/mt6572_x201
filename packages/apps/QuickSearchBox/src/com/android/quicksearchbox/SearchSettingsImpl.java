...
public class SearchSettingsImpl implements SearchSettings {
    ...
    public String getSavedSearchEngineFavicon() {
        String favicon = getSearchPreferences().getString(
                        SearchEngineItemsController.SEARCH_ENGINE_PREF, "");
        if ("".equals(favicon)) {
            favicon = getDefaultSearchEngineFavicon();
        }
        return favicon;
    }
    ...
    public String getDefaultSearchEngineFavicon() {
        ...
        String favicon = searchEngineManager.getDefaultSearchEngine().getFaviconUri();
        try {
            PluginManager<IPreferenceSetting> pluginManager;
            pluginManager = PluginManager.<IPreferenceSetting>create(
                    mContext, IPreferenceSetting.class.getName());

            Plugin<IPreferenceSetting> plugin = pluginManager.getPlugin(0);
            IPreferenceSetting handler = plugin.createObject();

            SearchEngineInfo engine = handler.getDefaultSearchEngine(mContext);
            if(engine != null) {
                favicon = engine.getFaviconUri();
            }
    ...
    public String getSearchEngineFaviconByName(String name) {
        ISearchEngineManager searchEngineManager = (ISearchEngineManager) mContext
        .getSystemService(Context.SEARCH_ENGINE_SERVICE);
        return searchEngineManager.getSearchEngineByName(name) != null
                ? searchEngineManager.getSearchEngineByName(name).getFaviconUri()
                : getDefaultSearchEngineFavicon();
    }
    ...
    public String getSearchEngineNameByFavicon(String favicon) {
        ...
        SearchEngineInfo searchEngineInfo
                = searchEngineManager.getBestMatchSearchEngine("", favicon) != null
                        ? searchEngineManager.getBestMatchSearchEngine("", favicon)
                        : searchEngineManager.getBestMatchSearchEngine("",
                                getDefaultSearchEngineFavicon());

        return searchEngineInfo.getName();
    }

    /**
     * M: Whether should synchornize SearchEngine with Browser or not.
     * @return if should sync return true, else return false
     */
    public boolean shouldSyncSearchEngineWithBrowser() {
        return getSearchPreferences().getBoolean(
                SearchEngineItemsController.PREF_SYNC_SEARCH_ENGINE, true);
    }
