...
public class SearchableSource extends AbstractSource {
    ...
    public CharSequence getLabel() {
        return mActivityInfo.loadLabel(getContext().getPackageManager());
    }
    ...
    public SuggestionCursor refreshShortcut(String shortcutId, String extraData) {
        ...
        try {
            ...
            if (cursor != null && cursor.getCount() > 0) {
                cursor.moveToFirst();
            /// M: If the provider is ongoing, return null. @{
            } else if (cursor != null && cursor.getCount() == 0
                    && cursor.getColumnIndex("ongoing") != -1) {
                cursor.close();
                return null;
            /// @}
            }
    ...
    public String getSuggestAuthority() {
        if (mSearchable != null) {
            return mSearchable.getSuggestAuthority();
        } else {
            return null;
        }
    }
    ...
    /**
     * M: Whether some voice search applications can receive the voice search intent.
     * @return If the source can launch voice search, but no activity can get the intent,
     *         return false.
     */
    public boolean canReceiveVoiceSearchIntent() {
        if (mSearchable.getVoiceSearchLaunchWebSearch()) {
            return true;
        } else if (mSearchable.getVoiceSearchLaunchRecognizer()) {
