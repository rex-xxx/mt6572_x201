...
public class VoiceSearch {
    ...
    /**
     * M: If the corpus can create voice search intent, but no activity can show, return false.
     * @param corpus the corpus to check whether it can create intent
     * @return if corpus is null or corpus can support voice search and has app can receive this
     *         intent return true, else return false.
     */
    protected boolean corpusSupportsVoiceSearch(Corpus corpus) {
        if (corpus == null) {
            return true;
        }
        else if (corpus.voiceSearchEnabled() && corpus.canReceiveVoiceSearchIntent()) {
            return true;
        }
        return false;
    }
