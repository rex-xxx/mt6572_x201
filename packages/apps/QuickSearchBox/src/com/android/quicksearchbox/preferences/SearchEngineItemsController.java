...
public class SearchEngineItemsController implements PreferenceController, 
    ...
    private void populateSourcePreference() {
        ...
        if (len <= 0) {
            Log.w(TAG, "SearchEngineManager.getAvailableSearchEngines return 0 search engine");
            return;
        }
        mEntryValues = null;
        mEntries = null;
    ...
    public boolean onPreferenceClick(Preference preference) {
        ...
        /// M: If mix mode is false, save data first and create search engine. @{
        if (QsbApplication.isMixMode()) {
            return true;
        } else {
            editor.commit();
            QsbApplication.get(mContext).createSearchEngineInfo();
            return false;
        }
        /// @}
    }
