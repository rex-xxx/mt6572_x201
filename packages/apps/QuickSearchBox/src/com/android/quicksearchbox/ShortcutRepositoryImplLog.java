...
public class ShortcutRepositoryImplLog implements ShortcutRepository {
    ...
    /// M: Query shortcut when only show shortcuts
    private String mShortcutNoQuery;
    ...
    private void buildShortcutQueries() {
        ...
        /// M: init mShortcutNoQuery, order by click times and latest hit time @{
        String ordering_by_times = "COUNT(" + ClickLog.hit_time.fullName + ")";
        orderBy = ordering_by_times + " DESC, " + preferLatest + " DESC";
        mShortcutNoQuery = SQLiteQueryBuilder.buildQueryString(
                false, TABLES, SHORTCUT_QUERY_COLUMNS, where, GROUP_BY, having, orderBy, null);
        Log.d(TAG, "Shortcut no query:\n" + mShortcutNoQuery);
        /// @}
...
// --------------------- Interface ShortcutRepository ---------------------
    ...
    /**
     * M: If show suggestions and shortcuts separately, 
     * get shortcuts differently when query is null.
     * @param allowedCorpora allowed corpora to provide data
     * @param allowWebSearchShortcuts allow web search shortcuts or not
     * @param consumer consumer to acquire data
     */
    public void getShortcutsNoQuery(final Collection<Corpus> allowedCorpora,
            final boolean allowWebSearchShortcuts, final Consumer<ShortcutCursor> consumer) {
        final long now = System.currentTimeMillis();
        mLogExecutor.execute(new Runnable() {
            public void run() {
                ShortcutCursor shortcuts = getShortcutsNoQuery(allowedCorpora,
                        allowWebSearchShortcuts, now);
                Consumers.consumeCloseable(consumer, shortcuts);
            }
        });
...
// -------------------------- end ShortcutRepository --------------------------
    ...
    /*
     * M: If change shortcut mechanism, only get shortcuts when query is null.
     */
    @VisibleForTesting
    ShortcutCursor getShortcutsNoQuery(Collection<Corpus> allowedCorpora,
            boolean allowWebSearchShortcuts, long now) {
        if (DBG) Log.d(TAG, "getShortcutsNoQuery(" + allowedCorpora + ")");
        String sql = mShortcutNoQuery;
        String[] params = buildShortcutQueryParams("", now);
        ...
        return new ShortcutCursor(new SuggestionCursorImpl(allowedSources, "", cursor),
                allowWebSearchShortcuts, mUiThread, mRefresher, this);
    }
