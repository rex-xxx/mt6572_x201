...
public class CallController extends Handler {
    ...
    public void placeCall(Intent intent) {
        ...
        //Before send the call request to FW, first check if there is normal call exist,
        //if there is/are normal call exist, disconnect.
        boolean isEccCall = PhoneNumberUtils.isEmergencyNumber(number) 
            && !PhoneNumberUtilsEx.isSpecialEmergencyNumber(number);
        ...
        switch (status) {
            ...
            case DROP_VOICECALL:
                inCallUiState.setPendingCallStatusCode(CallStatusCode.DROP_VOICECALL);
                ...
                inCallUiState.setNumberSlot(slot, realNumber != null ? realNumber : number);
                break;
            /** @} */
        ...
        // - if necessary, though, maybe PhoneGlobals.displayCallScreen()
    ...
    private CallStatusCode placeCallInternal(Intent intent) {
        //Profiler.trace(Profiler.CallControllerEnterPlaceCallInternal);
        ...
        slot = intent.getIntExtra(Constants.EXTRA_SLOT_ID, PhoneConstants.GEMINI_SIM_1);
        if (DBG) {
            log("placeCallInternal()...  slot = " + slot);
        }
         ...
         * @{
         */
        boolean isEmergencyNumber = PhoneNumberUtils.isEmergencyNumber(number);    /** M: Change Feature performance*/
        /** @} */
        ...
        */
        ...
        if (GeminiUtils.isGeminiSupport() && isEmergencyNumber) {
            slot = GeminiRegister.pickBestSlotForEmergencyCall(PhoneGlobals.getPhone(), number);
        }
        ...
        if (phone instanceof SipPhone) {
            ...
            okToCallStatus = checkIfOkToInitiateOutgoingCall(
                    phone.getServiceState().getState(), slot);
        }
        ...
        // TODO : use a common API to placeCall !!!
        int callStatus;
        ...
        switch (callStatus) {
            case PhoneUtils.CALL_STATUS_DIALED:
                ...
                /** M: Modify for CR: XXXX */
                boolean voicemailUriSpecified = PhoneUtils.isVoicemailNumber(number, slot, phone);
        ...
        if (GeminiUtils.isGeminiSupport() && GeminiUtils.isValidSlot(slot)) {
            ...
            if (!DualTalkUtils.isSupportDualTalk()) {
                ...
                for (int gs : geminiSlots) {
                    if (gs != slot && phone.getStateGemini(gs) != PhoneConstants.State.IDLE) {
                        return CallStatusCode.CALL_FAILED;
                    }
                }
                /// @}
