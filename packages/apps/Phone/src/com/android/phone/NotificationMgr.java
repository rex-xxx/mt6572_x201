...
public class NotificationMgr implements CallerInfoAsyncQuery.OnQueryCompleteListener {
    ...
    private static final String[] CALL_LOG_PROJECTION = new String[] { Calls._ID, Calls.NUMBER,
            Calls.DATE, Calls.DURATION, Calls.TYPE, Calls.VTCALL, };
    ...
    /**
     * Initialize the singleton NotificationMgr instance. This is only done once, at startup, from
     * PhoneGlobals.onCreate(). From then on, the NotificationMgr instance is available via the
     * PhoneGlobals's public "notificationMgr" field, which is why there's no getInstance() method
     * here.
     */
    ...
    /**
     * Helper class that's a wrapper around the framework's StatusBarManager.disable() API. This
     * class is used to control features like: - Disabling the status bar "notification windowshade"
     * while the in-call UI is up - Disabling notification alerts (audible or vibrating) while a
     * phone call is active - Disabling navigation via the system bar (the "soft buttons" at the
     * bottom of the screen on devices with no hard buttons) We control these features through a
     * single point of control to make sure that the various StatusBarManager.disable() calls don't
    ...
    public class StatusBarHelper {
        ...
        /**
         * Enables or disables auditory / vibrational alerts. (We disable these any time a voice
         * call is active, regardless of whether or not the in-call UI is visible.)
         */
        ...
        /**
         * Enables or disables the expanded view of the status bar (i.e. the ability to pull down
         * the "notification windowshade"). (This feature is disabled by the InCallScreen while the
         * in-call UI is active.)
         */
        ...
        /**
         * Enables or disables the navigation via the system bar (the "soft buttons" at the bottom
         * of the screen) (This feature is disabled while an incoming call is ringing, because it's
         * easy to accidentally touch the system bar while pulling the phone out of your pocket.)
         */
        ...
        private void updateStatusBar() {
            ...
            if (DBG)
                log("updateStatusBar: state = 0x" + Integer.toHexString(state));
            mStatusBarManager.disable(state);
        }
    }

    /**
     * Makes sure phone-related notifications are up to date on a freshly-booted device.
     */
    ...
    static final String[] PHONES_PROJECTION = new String[] { PhoneLookup.NUMBER,
            PhoneLookup.DISPLAY_NAME, PhoneLookup._ID };

    /**
     * Class used to run asynchronous queries to re-populate the notifications we care about. There
     * are really 3 steps to this: 1. Find the list of missed calls 2. For each call, run a query to
     * retrieve the caller's name. 3. For each caller, try obtaining photo.
     */
    private class QueryHandler extends AsyncQueryHandler implements
        ...
        public void onImageLoadComplete(int token, Drawable photo, Bitmap photoIcon, Object cookie) {
            if (DBG)
                log("Finished loading image: " + photo);
            NotificationInfo n = (NotificationInfo) cookie;
            notifyMissedCall(n.name, n.number, n.type, photo, photoIcon, n.date, n.callVideo);
        }

        /**
         * Factory method to generate a NotificationInfo object given a cursor from the call log
         * table.
         */
    ...
    /**
     * Configures a Notification to emit the blinky green message-waiting/ missed-call signal.
     */
     ...
     *            the contact name.
     * @param number
     *            the phone number. Note that this may be a non-callable String like "Unknown", or
     *            "Private Number", which possibly come from methods like
     *            {@link PhoneUtils#modifyForSpecialCnapCases(Context, CallerInfo, String, int)}.
     * @param type
     *            the type of the call. {@link android.provider.CallLog.Calls#INCOMING_TYPE}
     *            {@link android.provider.CallLog.Calls#OUTGOING_TYPE}, or
     *            {@link android.provider.CallLog.Calls#MISSED_TYPE}
     * @param photo
     *            picture which may be used for the notification (when photoIcon is null). This also
     *            can be null when the picture itself isn't available. If photoIcon is available it
     *            should be prioritized (because this may be too huge for notification). See also
     *            {@link ContactsAsyncHelper}.
     * @param photoIcon
     *            picture which should be used for the notification. Can be null. This is the most
     *            suitable for {@link android.app.Notification.Builder#setLargeIcon(Bitmap)}, this
     *            should be used when non-null.
     * @param date
     *            the time when the missed call happened
     */
    /* package */void notifyMissedCall(String name, String number, String type, Drawable photo,
        ...
        if (DBG) {
            log("notifyMissedCall(). name: " + name + ", number: " + number + ", label: " + type
                    + ", photo: " + photo + ", photoIcon: " + photoIcon + ", date: " + date);
        }
        ...
        // Support to notify user when full screen (for example, play the video)
        PendingIntent calllogIntentPlus = PendingIntent.getBroadcast(mContext, 0, callLogIntent, 0);
        NotificationPlus notiPlus = new NotificationPlus.Builder(mContext).setTitle(
                mContext.getString(titleResId)).setMessage(
                mContext.getString(R.string.notification_missedCallTicker, callName))
                .setPositiveButton(mContext.getString(R.string.ok), calllogIntentPlus).create();
    ...
    /**
     * Shows or hides the "speakerphone" notification in the status bar, based on the actual current
     * state of the speaker. If you already know the current speaker state (e.g. if you just called
     * AudioManager.setSpeakerphoneOn() yourself) then you should directly call
     * {@link #updateSpeakerNotification(boolean)} instead. (But note that the status bar icon is
     * *never* shown while the in-call UI is active; it only appears if you bail out to some other
     * activity.)
     */
    public void updateSpeakerNotification() {
        ...
        if (DBG)
            log(showNotification ? "updateSpeakerNotification: speaker ON"
     ...
     *            if true, call notifySpeakerphone(); if false, call cancelSpeakerphone(). Use
     *            {@link updateSpeakerNotification()} to update the status bar based on the actual
     *            current state of the speaker. (But note that the status bar icon is *never* shown
     *            while the in-call UI is active; it only appears if you bail out to some other
     *            activity.)
     */
    ...
    private void notifyMute() {
        if (!mShowingMuteIcon) {
            mStatusBarManager.setIcon("mute", android.R.drawable.stat_notify_call_mute, 0, mContext
    ...
    /**
     * Shows or hides the "mute" notification in the status bar, based on the current mute state of
     * the Phone. (But note that the status bar icon is *never* shown while the in-call UI is
     * active; it only appears if you bail out to some other activity.)
     */
    void updateMuteNotification() {
        ...
        PhoneConstants.State state = mCM.getState(); // IDLE, RINGING, or OFFHOOK

        if ((state == PhoneConstants.State.OFFHOOK) && PhoneUtils.getMute()) {
            if (DBG)
    ...
    /**
     * Updates the phone app's status bar notification based on the current telephony state, or
     * cancels the notification if the phone is totally idle. This method will never actually launch
     * the incoming-call UI. (Use updateNotificationAndLaunchIncomingCallUi() for that.)
     */
    ...
    /**
     * Updates the phone app's status bar notification *and* launches the incoming call UI in
     * response to a new incoming call. This is just like updateInCallNotification(), with one
     * exception: If an incoming call is ringing (or call-waiting), the notification will also
     * include a "fullScreenIntent" that will cause the InCallScreen to be launched immediately,
     * unless the current foreground activity is marked as "immersive". (This is the mechanism that
     * actually brings up the incoming call UI when we receive a "new ringing connection" event from
     * the telephony layer.) Watch out: this method should ONLY be called directly from the code
     * path in CallNotifier that handles the "new ringing connection" event from the telephony
     * layer. All other places that update the in-call notification (like for phone state changes)
     * should call updateInCallNotification() instead. (This ensures that we don't end up launching
     * the InCallScreen multiple times for a single incoming call, which could cause slow
     * responsiveness and/or visible glitches.) Also note that this method is safe to call even if
     * the phone isn't actually ringing (or, more likely, if an incoming call *was* ringing briefly
     * but then disconnected). In that case, we'll simply update or cancel the in-call notification
     * based on the current phone state.
     * 
    ...
    /**
     * Helper method for updateInCallNotification() and updateNotificationAndLaunchIncomingCallUi():
     * Update the phone app's status bar notification based on the current telephony state, or
     * cancels the notification if the phone is totally idle.
     * 
     * @param allowFullScreenIntent
     *            If true, *and* an incoming call is ringing, the notification will include a
     *            "fullScreenIntent" pointing at the InCallScreen (which will cause the InCallScreen
     *            to be launched.) Watch out: This should be set to true *only* when directly
     *            handling the "new ringing connection" event from the telephony layer (see
     *            updateNotificationAndLaunchIncomingCallUi().)
     */
    private void updateInCallNotification(boolean allowFullScreenIntent) {
        int resId;
        if (DBG)
            log("updateInCallNotification(allowFullScreenIntent = " + allowFullScreenIntent
                    + ")...");
        ...
        InCallScreen screen = mApp.getInCallScreenInstance();
        if (screen != null) {
            suppressNotification = !screen.isNeedToUpdateInCallNotification();
        }
        ...
        if (DBG)
            log("- Updating status bar icon: resId = " + resId);
        mInCallResId = resId;
        ...
        Intent inCallIntent = PhoneGlobals.createInCallIntent();
        inCallIntent.putExtra(Constants.EXTRA_IS_NOTIFICATION, true);
        PendingIntent inCallPendingIntent = PendingIntent.getActivity(mContext, 0, PhoneGlobals
                .createInCallIntent(), 0);
                ...
                if (callerInfo.cachedPhotoIcon != null) {
                    ...
                    if (DBG) {
                        log("- Failed to fetch icon from CallerInfo's cached photo."
                                ...
                                + ", cachedPhoto: " + callerInfo.cachedPhoto + ")." + " Ignore it.");
                    }
                }
            }
        ...
        if (currentConn != null) {
            ...
            if (hasRingingCall) {
                ...
                if (DBG)
                    log("callDurationMsec: " + callDurationMsec);
                if (FeatureOption.MTK_VT3G324M_SUPPORT && currentConn.isVideo()) {
                    if ((VTCallUtils.VTTimingMode.VT_TIMING_NONE == VTCallUtils
                            .checkVTTimingMode(currentCall.getLatestConnection().getAddress()))
                ...
                if (TelephonyCapabilities.canDistinguishDialingAndConnected(call.getPhone()
                        .getPhoneType())
        ...
        if (null != currentCall && PhoneUtils.isConferenceCall(currentCall)) {
            ...
            PhoneUtils.CallerInfoToken cit = PhoneUtils.startGetCallerInfo(mContext, currentCall,
                    this, this);
        ...
        if (DBG)
            log("- Updating expanded view: line 2 '" + /* expandedViewLine2 */"xxxxxxx" + "'");
        ...
        if (hasRingingCall) {
            ...
            if (allowFullScreenIntent) {
                ...
                if (DBG)
                    log("- Setting fullScreenIntent: " + inCallPendingIntent);
                ...
                // /M: Add for the case that a call reach in the process of last phone hung up. @{
                // /Call ringingCall = mCM.getFirstActiveRingingCall();
                // /Original:if ((ringingCall.getState() == Call.State.WAITING) &&
                ...
                mApp.startActivity(inCallIntent);
            }
            ...
            builder.addAction(R.drawable.stat_sys_phone_call_end, mContext
                    .getText(R.string.notification_action_end_call), PhoneGlobals
    ...
    /**
     * Implemented for CallerInfoAsyncQuery.OnQueryCompleteListener interface. refreshes the
     * contentView when called.
     */
    @Override
    public void onQueryComplete(int token, Object cookie, CallerInfo ci) {
        if (DBG)
            log("CallerInfo query complete (for NotificationMgr), "
        ...
        if (cookie == this) {
            ...
            Log.w(LOG_TAG, "onQueryComplete: caller-id query from unknown source! " + "cookie = "
                    + cookie);
        }
    }
    ...
    /**
     * Completely take down the in-call notification *and* the mute/speaker notifications as well,
     * to indicate that the phone is now idle.
     */
    /* package */void cancelCallInProgressNotifications() {
        ...
        if (DBG)
            log("cancelCallInProgressNotifications: " + mInCallResId);
        cancelInCall();
        cancelMute();
        cancelSpeakerphone();
    }
    ...
    /* package */void updateMwi(boolean visible, int slotId) {
        ...
        if (DBG)
            log("updateMwi(): " + visible + "simId:" + slotId);
        ...
        if (visible) {
            ...
            String vmNumber = GeminiUtils.getVoiceMailNumber(mPhone, slotId);
            ...
            boolean iccRecordloaded = GeminiUtils.getIccRecordsLoaded(mPhone, slotId);
            ...
            if ((vmNumber == null) && !iccRecordloaded) {
                ...
                if (mVmNumberRetriesRemaining-- > 0) {
                    if (DBG)
                        log("  - Retrying in " + VM_NUMBER_RETRY_DELAY_MILLIS + " msec...");
         ...
         // !!!!! Need consider again, both google and MTK modify below
            ...
            notification.simId = getSimId(slotId);
    ...
    /* package */void updateCfi(boolean visible, int slotId) {
        if (DBG)
            log("updateCfi(): " + visible + "slotId:" + slotId);
        // / M:Gemini+
        ...
        if (visible) {
            ...
            SIMInfo simInfo = com.mediatek.phone.SIMInfoWrapper.getDefault().getSimInfoBySlot(slotId);
            ...
            if (showExpandedNotification) {
                ...
                notification = new Notification(
                        /*android.R.drawable.stat_sys_phone_call_forward*/resId,  // icon
                        null, // tickerText
    ...
    /* package */void showDataDisconnectedRoaming(int simId) {
        ...
        Notification notification = new Notification(android.R.drawable.stat_sys_warning, // icon
                null, // tickerText
        ...
        mNotificationManager.notify(DATA_DISCONNECTED_ROAMING_NOTIFICATION, notification);
    }
    ...
    void updateNetworkSelection(int serviceState, int simId) {
        if (mPhone.getPhoneType() == PhoneConstants.PHONE_TYPE_GSM) {
            ...
            if (serviceState == ServiceState.STATE_OUT_OF_SERVICE
                ...
                IccCard iccCard = GeminiUtils.getIccCard(mPhone, simId);
    ...
    /* package */public void postTransientNotification(int notifyId, CharSequence msg) {
        if (mToast != null) {
            mToast.cancel();
        }
    ...
    private long getSimId(int slot) {
        SIMInfo info = SIMInfoWrapper.getDefault().getSimInfoBySlot(slot);
        if (info != null) {
            return info.mSimId;
        }
    ...
    public void showVoiceCommandNotification(String firstVoiceCommand, String secondVoiceCommand) {
        Notification notification = new Notification.Builder(mContext).setContentTitle(
                mContext.getString(R.string.voice_command_notification_title)).setTicker(
                mContext.getString(R.string.voice_command_notification_ticker, firstVoiceCommand,
                        secondVoiceCommand)).setContentText(
                mContext.getString(R.string.voice_command_notification_message)).setSmallIcon(
                com.mediatek.internal.R.drawable.stat_voice).build();
