...
public class RespondViaSmsManager {
    ...
    public void showRespondViaSmsPopup(Call ringingCall) {
        ...
        RespondViaSmsCancelListener listener = new RespondViaSmsCancelListener();
    ...
    public class RespondViaSmsItemClickListener implements AdapterView.OnItemClickListener {
        ...
        public void onItemClick(AdapterView<?> parent,  // The ListView
                                ...
                                long id) {
            if (DBG) {
                log("RespondViaSmsItemClickListener.onItemClick(" + position + ")...");
            }
     ...
     */
    public class RespondViaSmsCancelListener implements DialogInterface.OnCancelListener, DialogInterface.OnDismissListener {
        public RespondViaSmsCancelListener() {
        }

        public void onDismiss(DialogInterface dialog) {
            if (DBG) {
                log("RespondViaSmsCancelListener.onDismiss()...");
            }
            if (PhoneGlobals.getInstance().mCM.getState() == PhoneConstants.State.IDLE) {
                PhoneGlobals.getInstance().mCM.clearDisconnected();
                log("- onDismiss: clear disconnected.");
                if (mInCallScreen != null) {
                    //Fix issue: ALPS00420693
                    mInCallScreen.finish();
                    log("- onDismiss: finish...");
                }
            }
        }
        /**
        ...
        public void onCancel(DialogInterface dialog) {
            if (DBG) {
                log("RespondViaSmsCancelListener.onCancel()...");
            }
    ...
    private void sendText(String phoneNumber, String message) {
         ...
         * description : put the SIM id into the extra
         */
        if (GeminiUtils.isGeminiSupport()) {
            if (-1 != mOldSoltId) {
                intent.putExtra(GeminiConstants.SLOT_ID_KEY, mOldSoltId);
            } else {
                intent.putExtra(GeminiConstants.SLOT_ID_KEY, getSendTextSimId());
            }
        }
        /**
    ...
    private void launchSmsCompose(String phoneNumber) {
        if (VDBG) {
            log("launchSmsCompose: number " + phoneNumber);
        }
        ...
        if (VDBG) {
            log("- Launching SMS compose UI: " + intent);
        }
        mInCallScreen.startService(intent);
    }
    ...
    public static boolean allowRespondViaSmsForCall(Context context, Call ringingCall) {
        if (DBG) {
            log("allowRespondViaSmsForCall(" + ringingCall + ")...");
        }
        ...
        if (PhoneUtils.isDMLocked()) {
            Log.i(TAG, "allowRespondViaSmsForCall: isDMLocked.");
            return false;
        }
    ...
    protected int getSendTextSimId() {
        ...
        log("getSendTextSimId, slot = " + slot + " id = " + id);
        return id;
    }

    protected boolean shouldDismissInCallScreen() {
        CallManager cm = PhoneGlobals.getInstance().mCM;
        // For dualtalk case, maybe two incoming call
        return !(cm.hasActiveFgCall() || cm.hasActiveBgCall() || cm.getState() == PhoneConstants.State.RINGING);
    }
}
