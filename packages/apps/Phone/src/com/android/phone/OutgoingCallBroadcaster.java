...
import com.mediatek.phone.OutgoingCallReceiver;
...
public class OutgoingCallBroadcaster extends Activity
        implements DialogInterface.OnClickListener, DialogInterface.OnCancelListener,
                   CallOptionBaseHandler.ICallOptionResultHandle {
    ...
    public class OutgoingCallReceiver extends BroadcastReceiver {
        ...
        public void doReceive(Context context, Intent intent) {
            ...
            if (number == null) {
                ...
                return;
                /** M: Change feature: move the PhoneNumberUtils.isPotentialLocalEmergencyNumber
                ...
                /** M: Change feature: use isEmergencyNumber to avoid performance issues @{ */
                //final boolean isPotentialLocalEmergencyNumber = 
                //      PhoneNumberUtils.isPotentialLocalEmergencyNumber(number, context);
    ...
    private void startSipCallOptionHandler(Context context, Intent intent,
        ...
        /** M: Change feature: using CallOptionHandler to handle newIntent directly @{ */
        //if (PhoneGlobals.sGemini) {
    ...
    private void processIntent(Intent intent) {
        ...
        /** M: Change feature: Modify since that isExactEmergencyNumber is no used and
               use isEmergencyNumber to avoid performance issues @{ */
        ...
        if (Intent.ACTION_CALL.equals(action)) {
            if (isPotentialEmergencyNumber) {
                ...
                try {
                    ...
                    Log.e(TAG, "Activity for DialtactsActivity isn't found.");
                }
                finish();
                return;
            }
        ...
        if (TextUtils.isEmpty(number)) {
            if (intent.getBooleanExtra(EXTRA_SEND_EMPTY_FLASH, false)) {
                ...
                /** M: Change feature: when dialing voicemail number, the data of intent
        ...
        //if (isVoiceMailNumber || Constants.SCHEME_SIP.equals(scheme) || PhoneNumberUtils.isUriNumber(number)) {
        //    Log.i(TAG, "The requested number was detected as SIP call.");
            /** M: Change feature: do not finish here for gemini @{ */
            //if (PhoneGlobals.sGemini) {
        ...
        if (!callNow) {
            ...
            //For callNow case, we finish ourself directly instead of sending an intent.
            finish();
        }
        /** }@ */
    }
    ...
    public void onContinueCallProcess(Intent intent) {
        ...
        if (intent.hasExtra(Constants.EXTRA_ACTUAL_NUMBER_TO_DIAL)) {
            number = intent.getStringExtra(Constants.EXTRA_ACTUAL_NUMBER_TO_DIAL);
        } else {
    ...
    public static void sendNewCallBroadcast(Context context, Intent intent, String number,
        ...
        // timeout.
        //Remove this, we think the timeout isn't acceptable
...
//                OUTGOING_CALL_TIMEOUT_THRESHOLD);
    ...
    private void sendNewCallBroadcast(Intent intent, String number, boolean callNow) {
        ...
        sendNewCallBroadcast(this, intent, number, callNow, new OutgoingCallReceiver());
        /*Intent broadcastIntent = new Intent(Intent.ACTION_NEW_OUTGOING_CALL);
        if (number != null) {
            broadcastIntent.putExtra(Intent.EXTRA_PHONE_NUMBER, number);
        }
...
//                OUTGOING_CALL_TIMEOUT_THRESHOLD);
    ...
    private void doCallOptionHandle(Intent intent) {
        PhoneLog.d(TAG, "doCallOptionHandle, intent is " + intent);
        ...
        mCallOptionHandler.doCallOptionHandle(this, PhoneGlobals.getInstance(), intent,
                                              this, PhoneGlobals.getInstance().cellConnMgr,
                                              PhoneGlobals.getInstance().phoneMgr,
                                              GeminiUtils.isGeminiSupport(),
                                              FeatureOption.MTK_GEMINI_3G_SWITCH);
    }

    /*public CallOptionHandler getCallOptionHandlerOnlyForTest() {
        PhoneLog.d(TAG, "getCallOptionHandlerOnlyForTest");
        return mCallOptionHandler;
    }

    public void setCallOptionHandlerOnlyForTest(CallOptionHandler newCallOptionHandler) {
        PhoneLog.d(TAG, "setCallOptionHandlerOnlyForTest");
        mCallOptionHandler = newCallOptionHandler;
    }

    public boolean getmCallOptionHandledOnlyForTest() {
        PhoneLog.d(TAG, "getmCallOptionHandledOnlyForTest");
        return mCallOptionHandled;
    }

    public void setmCallOptionHandledOnlyForTest(boolean newCallOptionHandled) {
        PhoneLog.d(TAG, "setmCallOptionHandledOnlyForTest");
        mCallOptionHandled = newCallOptionHandled;
    }*/
}
