...
/**
 * Note: The purpose of this feature is the use of VT loopback mode in the Engineer Mode.
 *       For this purpose, these code are exactly copied from phone app(VTIncallScreen.java),
 *       and therefore, we comment out many of unused code.
 *       It shall be remove or revise afterwards.
 */
...
//    private VTCallBanner mCallBanner;
     ...
     */
    public VTInCallScreenLoopback(Context context, AttributeSet attrs) {
        super(context, attrs);
     ...
     */
    public VTInCallScreenLoopback(Context context, AttributeSet attrs, int defStyle) {
    ...
    public void onDestroy() {
        ...
        if (getVTInControlRes()) {
            ...
            mActivity.sendBroadcast(new Intent(VTCallUtils.VT_CALL_END));
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case VTManager.VT_MSG_READY:
                    ...
                    log ("finish to set onConnect,mState="+VTManager.getInstance().getState());
                    break;
                ...
                case VTManager.VT_ERROR_CALL_DISCONNECT:
                    ...
                    if ((!VTInCallScreenFlagsLoopback.getInstance().mVTInEndingCall)
    ...
    public void updateVTScreen(Constants.VTScreenMode mode) {
        ...
        if (DBG) {
            log("updateVTScreen : VTInCallScreenFlagsLoopback.getInstance().mVTHideMeNow - "
                    + VTInCallScreenFlagsLoopback.getInstance().mVTHideMeNow);
    ...
    private void updateVTInCallButtons() {
        ...
        mVTSwapVideo.setChecked(!VTInCallScreenFlagsLoopback.getInstance().mVTPeerBigger);
    ...
    public void initVTInCallScreen() {
        ...
        //mVTPowerManager = (PowerManager) mInCallScreen.getSystemService(Context.POWER_SERVICE);//loopbackActivity
        mVTPowerManager = (PowerManager) mActivity.getSystemService(Context.POWER_SERVICE);//loopbackActivity
        ...
        //mVTCallBannerController = new VTCallBannerController(mCallBanner, mInCallScreen);//loopbackActivity
        //mVTCallBannerController = new VTCallBannerController(mCallBanner, mActivity);//loopbackActivity
        ...
        //if (ViewConfiguration.get(mInCallScreen).hasPermanentMenuKey()) {//loopbackActivity
        if (ViewConfiguration.get(mActivity).hasPermanentMenuKey()) {//loopbackActivity
        ...
        //if (ViewConfiguration.get(mInCallScreen).hasPermanentMenuKey()) {//activity
        if (ViewConfiguration.get(mActivity).hasPermanentMenuKey()) {//activity        
        ...
        //if (ViewConfiguration.get(mInCallScreen).hasPermanentMenuKey()) {//acvitiy
        if (ViewConfiguration.get(mActivity).hasPermanentMenuKey()) {//acvitiy
    ...
    private void updateLocalZoomOrBrightness() {
        ...
        if (VTInCallScreenFlagsLoopback.getInstance().mVTInLocalZoomSetting) {
            mVTLowUp.setEnabled(VTManager.getInstance().canIncZoom());
            mVTLowDown.setEnabled(VTManager.getInstance().canDecZoom());
        } else if (VTInCallScreenFlagsLoopback.getInstance().mVTInLocalBrightnessSetting) {
            mVTLowUp.setEnabled(VTManager.getInstance().canIncBrightness());
            mVTLowDown.setEnabled(VTManager.getInstance().canDecBrightness());
        } else if (VTInCallScreenFlagsLoopback.getInstance().mVTInLocalContrastSetting) {
            mVTHighUp.setEnabled(VTManager.getInstance().canIncContrast());
            mVTHighDown.setEnabled(VTManager.getInstance().canDecContrast());
        }
    }
    ...
    public void internalAnswerVTCallPre() {
        ...
        if (VTCallUtils.isVTActive()) {
            ...
            if (DBG) {
                log("internalAnswerVTCallPre:"
                    + " set VTInCallScreenFlagsLoopback.getInstance().mVTShouldCloseVTManager = false");
            }
        ...
        if (DBG) {
            log("Incallscreen, before incomingVTCall");
        }
        VTManager.getInstance().incomingVTCall(1);
        if (DBG) {
            log("Incallscreen, after incomingVTCall");
        }*/
        ...
        //VTInCallScreenFlagsLoopback.getInstance().mVTIsMT = true;
        //VTInCallScreenFlagsLoopback.getInstance().mVTPeerBigger = VTSettingUtils.getInstance().mPeerBigger;

        /*Call ringingCall = mCM.getFirstActiveRingingCall();
        SIMInfo simInfo = PhoneUtils.getSimInfoByCall(ringingCall);
        if (null != simInfo) {
            VTInCallScreenFlagsLoopback.getInstance().mVTSlotId = simInfo.mSlot;
        } else {
        ...
        /*if (null != mBkgBitmapHandler) {
            ...
            if (null != mBkgBitmapHandler.getBitmap()
                    && !VTInCallScreenFlagsLoopback.getInstance().mVTHasReceiveFirstFrame) {
        ...
        */
        ...
        if (!getVTInControlRes()) {
            ...
            mActivity.sendBroadcast(new Intent(VTCallUtils.VT_CALL_START));
        ...
        if (FeatureOption.MTK_GEMINI_SUPPORT) {
            VTManager.getInstance().setVTOpen(PhoneGlobals.getInstance().getBaseContext(), mCMGemini, 0);//simInfo.mSlot
        } else {
            VTManager.getInstance().setVTOpen(PhoneGlobals.getInstance().getBaseContext(), mCM, 0);
        }
        if (VDBG) {
        ...
        if (VTInCallScreenFlagsLoopback.getInstance().mVTSurfaceChangedH
                && VTInCallScreenFlagsLoopback.getInstance().mVTSurfaceChangedL) {
            if (VDBG) {
        ...
        if (VTInCallScreenFlagsLoopback.getInstance().mVTHideMeNow) {
            ...
            if (VTSettingUtils.getInstance().mPicToReplaceLocal.equals("0")) {
                ...
                VTManager.getInstance().setLocalVideoType(1,
                        VTAdvancedSetting.getPicPathUserselect(VTInCallScreenFlagsLoopback.getInstance().mVTSlotId));
            }
        }

        VTInCallScreenFlagsLoopback.getInstance().mVTHideMeNow = !VTInCallScreenFlagsLoopback.getInstance().mVTHideMeNow;
        updateVTScreen(getVTScreenMode());
    }
    ...
    private void onVTSwitchCameraClick() {
        ...
        VTInCallScreenFlagsLoopback.getInstance().mVTFrontCameraNow = !VTInCallScreenFlagsLoopback.getInstance().mVTFrontCameraNow;
        updateVTScreen(getVTScreenMode());
    ...
    public void surfaceChanged(SurfaceHolder holder, int format, int width, int height) {
        ...
        if (VTInCallScreenFlagsLoopback.getInstance().mVTSurfaceChangedH
            ...
            log("surfacedChanged_afterReady_State= "+VTManager.getInstance().getState());
            updateVTScreen(getVTScreenMode());
        }
    }
    ...
    private void onVTSwapVideos() {
        ...
        if (VTInCallScreenFlagsLoopback.getInstance().mVTInLocalZoomSetting
                || VTInCallScreenFlagsLoopback.getInstance().mVTInLocalBrightnessSetting
                || VTInCallScreenFlagsLoopback.getInstance().mVTInLocalContrastSetting) {
            hideLocalZoomOrBrightness();
        }

        VTInCallScreenFlagsLoopback.getInstance().mVTPeerBigger
            = !VTInCallScreenFlagsLoopback.getInstance().mVTPeerBigger;
        VTManager.getInstance().setVTVisible(false);
        updateVTLocalPeerDisplay();
        VTManager.getInstance().setVTVisible(true);
...
/*            if (mCM.getActiveFgCall().isIdle() && mCM.getFirstActiveBgCall().isIdle()
    ...
    private void setVTInControlRes(boolean value) {
        VTInCallScreenFlagsLoopback.getInstance().mVTInControlRes = value;
    }
    ...
    public void onStop() {
        ...
        closeVTManager();        
        /*if (getVTInControlRes()) {
            log("VTInCallScreenLoopback:onStop:sentIntent:VTCallUtils.VT_CALL_END");
            mActivity.sendBroadcast(new Intent(VTCallUtils.VT_CALL_END));
            setVTInControlRes(false);
        } */       
    }
    ...
    private void makeVoiceReCall(final String number, final int slot) {
        ...
        //mInCallScreen.finish();//activity
        mActivity.finish();//activity
    }
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                if (FeatureOption.MTK_GEMINI_SUPPORT) {
                    //mInCallScreen.delayedCleanupAfterDisconnect(
                            //mInCallScreen.DELAYED_CLEANUP_AFTER_DISCONNECT2);
                }
                //mInCallScreen.delayedCleanupAfterDisconnect(
                  //      mInCallScreen.DELAYED_CLEANUP_AFTER_DISCONNECT);
            }
        };
...
//        mVTVoiceReCallDialog.setOnShowListener(mInCallScreen);
    ...
    public void setupMenuItems(Menu menu) {
        ...
        takePeerPhotoMenu.setEnabled(!isDialerOpened() && 
                VTInCallScreenFlagsLoopback.getInstance().mVTVideoConnected);
...
//        swapVideosMenu.setVisible(!ViewConfiguration.get(mInCallScreen).hasPermanentMenuKey());
        swapVideosMenu.setEnabled(VTInCallScreenFlagsLoopback.getInstance().mVTHasReceiveFirstFrame);
        ...
        videoSettingMenu.setEnabled(VTInCallScreenFlagsLoopback.getInstance().mVTVideoConnected);
    }
...
//        InCallControlState inCallControlState
  //              = mInCallScreen.getUpdatedInCallControlState();
    ...
    public void onReceiveVTManagerStartCounter() {
        ...
        if (VTInCallScreenFlagsLoopback.getInstance().mVTConnectionStarttime.mStarttime < 0) {
            Call call = mCM.getActiveFgCall();
            ...
            if (null != mCM.getActiveFgCall()) {
                if (mCM.getActiveFgCall().getLatestConnection() != null) {
                        VTInCallScreenFlagsLoopback.getInstance().mVTConnectionStarttime.mStarttime
                                = SystemClock.elapsedRealtime();
                        VTInCallScreenFlagsLoopback.getInstance().mVTConnectionStarttime.mConnection
    ...
    private void onReceiveVTManagerReady() {
        ...
        if (!PhoneUtils.isDMLocked()) {
            ...
            if (VTSettingUtils.getInstance().mShowLocalMT.equals("1")
                    && VTInCallScreenFlagsLoopback.getInstance().mVTIsMT) {
                if (DBG) {
