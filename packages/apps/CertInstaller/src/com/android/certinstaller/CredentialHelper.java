...
class CredentialHelper {
    ...
    private boolean mEcc;
    private boolean mHasPkey;
    /// @}
    ...
    private void parseCert(byte[] bytes) {
        ...
        try {
            ...
            if (isCa(cert)) {
                ...
                Log.i(TAG, "getAlgorithm()=" + alg);
                if (alg.equals("RSA"))
                    Log.i(TAG, "User certificate of RSA");
                else if (alg.equals("1.2.840.10045.2.1")) {
                    Log.i(TAG, "User certificate of ECC");
                    mEcc = true;
                    mWapiUserCert = bytes; 
                    String wapiUserCert = new String(bytes);
                    if (wapiUserCert.contains("-----BEGIN EC PRIVATE KEY-----") &&
                        wapiUserCert.contains("-----END EC PRIVATE KEY-----")) {
                        mHasPkey = true;
                        Log.i(TAG, "User certificate of ECC with private key");
                    }
                }
                else
                    Log.w(TAG, "User certificate with unknown public key algorithm");
                /// @}
            }
    ...
    Intent createSystemInstallIntent() {
        ...
        try {
            ...
            if (mUserCert != null) {
                ...
                if (mEcc) {
                    if (mHasPkey) {
                        keyPrefix = Credentials.WAPI_USER_CERTIFICATE;
                        extraName = Credentials.EXTRA_WAPI_USER_CERTIFICATE_NAME;
                        extraData = Credentials.EXTRA_WAPI_USER_CERTIFICATE_DATA;
                    } else {
                        keyPrefix = Credentials.WAPI_AS_CERTIFICATE;
                        extraName = Credentials.EXTRA_WAPI_AS_CERTIFICATE_NAME;
                        extraData = Credentials.EXTRA_WAPI_AS_CERTIFICATE_DATA;
                    }
                    intent.putExtra(extraName,keyPrefix + mName);
                    intent.putExtra(extraData,mWapiUserCert);
                } else {
