...
public class ExtensionHelper {
    public static IQualityLevel getExtension(Context context) {
        ...
        PluginManager<IQualityLevel> pm = PluginManager.<IQualityLevel>create(context, IQualityLevel.class.getName());
            if((null != pm) && (0 != pm.getPluginCount())){
                Plugin<IQualityLevel> plugin = pm.getPlugin(0);
                if (null != plugin) {
                    try {
                        qaulityLevel = plugin.createObject();
                    } catch(ObjectCreationException e) {
                        qaulityLevel = new DefaultQualityLevel();                
                    }
                    return qaulityLevel;
                } else {
                    qaulityLevel = new DefaultQualityLevel();    
