...
public class LauncherProvider extends ContentProvider {
    ...
    public Uri insert(Uri uri, ContentValues initialValues) {
        ...
        SQLiteDatabase db = sOpenHelper.getWritableDatabase();
        final long rowId = dbInsertAndCheck(sOpenHelper, db, args.table, null, initialValues);
    ...
    public int bulkInsert(Uri uri, ContentValues[] values) {
        ...
        try {
            ...
            for (int i = 0; i < numValues; i++) {
                if (dbInsertAndCheck(sOpenHelper, db, args.table, null, values[i]) < 0) {
                    return 0;
                }
            }
    ...
    synchronized public void loadDefaultFavoritesIfNecessary(int origWorkspaceResId) {
        ...
        if (sp.getBoolean(DB_CREATED_BUT_DEFAULT_WORKSPACE_NOT_LOADED, false)) {
            ...
            /// M: added for scene feature, load scene from a xml file.
            sOpenHelper.updateSceneField(sOpenHelper.getWritableDatabase(),
                    getContext().getString(R.string.scene_name_default));
            editor.commit();
        }
    }
    ...
    static class DatabaseHelper extends SQLiteOpenHelper {
        ...
        /// M: Added for mota update.{@
        private static final String TAG_MOTAUPDATE = "mota-update";
        private static final ArrayList<MotaUpdate> MOTA_UPDATE_APPS = new ArrayList<MotaUpdate>();

        class MotaUpdate {
            public MotaUpdate(String oldPkgName, String oldClsName, String newPkgName, String newClsName) {
            	mOldComponent = new ComponentName(oldPkgName, oldClsName);
            	mNewComponent = new ComponentName(newPkgName, newClsName);
            }
        ...
        public void onCreate(SQLiteDatabase db) {
            ...
            db.execSQL("CREATE TABLE favorites (" +
                    ...
                    "uri TEXT," +
                    "displayMode INTEGER," +
                    "scene TEXT" +	/// M: Add for scene feature, add the new column "scnee".
                    ");");
        ...
        private int copyFromCursor(SQLiteDatabase db, Cursor c) {
            ...
            /// M: added for scene feature, get the scene index.
            ...
            while (c.moveToNext()) {
                ...
                /// M: added for scene feature, get the scene value.
        ...
        public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
            ...
            /// M: added for scene feature, insert new column "scene" for db of the lower version
            /// which has no the "scene" column.
            if (version < 10) {          
            	db.beginTransaction();
                try {
                    // Insert new column "scene"
                    db.execSQL("ALTER TABLE favorites " +
                ...
                /// M: added for scene feature, update scene field of the database.
                updateSceneField(db, mContext.getString(R.string.scene_name_default));
            }
        ...
        /*
         * M: Load mota_update.xml
         */
        private void loadMotaUpdate() {
            long start = System.currentTimeMillis();
            if (LauncherLog.DEBUG) {
                LauncherLog.d(TAG, "loadMotaUpdate begin: start = " + start);
            }
            ...
            try {
                ...
                XmlUtils.beginDocument(parser, TAG_MOTAUPDATE);
                ...
                while (((type = parser.next()) != XmlPullParser.END_TAG || parser.getDepth() > depth)
                    ...
                    TypedArray a = mContext.obtainStyledAttributes(attrs, R.styleable.MotaUpdate);
    				MOTA_UPDATE_APPS.add(new MotaUpdate(
    						a.getString(R.styleable.MotaUpdate_oldPackageName),
    						a.getString(R.styleable.MotaUpdate_oldClassName),
    						a.getString(R.styleable.MotaUpdate_newPackageName),
    						a.getString(R.styleable.MotaUpdate_newClassName)));
                    a.recycle();

                }
            } catch (XmlPullParserException e) {
                LauncherLog.w(TAG, "Got XmlPullParserException while parsing mota update apps.", e);
            } catch (IOException e) {
                LauncherLog.w(TAG, "Got IOException while parsing mota update apps.", e);
            }
            if (LauncherLog.DEBUG) {
                LauncherLog.d(TAG, "loadMotaUpdate end: time used = "
                        + (System.currentTimeMillis() - start));
            }
        }
        
        /*
         * M: Update some apps after mota updae.
         */
        private void motaUpdate(SQLiteDatabase db) {
        	if (LauncherLog.DEBUG) {
        		LauncherLog.d(TAG, "motaUpdate");
        	}

        	loadMotaUpdate();
        	
        	if (MOTA_UPDATE_APPS.size() <=0 ) {
        		return;
        	}
        	
        	final int size = MOTA_UPDATE_APPS.size();
        	Cursor c = null;
            db.beginTransaction();
            try {
                // Select and iterate through each matching widget
                ...
                while (c.moveToNext()) {
                    long favoriteId = c.getLong(idIndex);
                    final String intentDescription = c.getString(intentIndex);
                    if (intentDescription != null) {
                        try {
                            final Intent intent = Intent.parseUri(intentDescription, 0);
                            final ComponentName cmpName = intent.getComponent();
                            for(int i=0; i<this.MOTA_UPDATE_APPS.size(); i++) {
                            	final ComponentName tmpName = MOTA_UPDATE_APPS.get(i).mOldComponent;
                            	if (cmpName.equals(tmpName)) {
                            		intent.setComponent(MOTA_UPDATE_APPS.get(i).mNewComponent);
                            		final ContentValues values = new ContentValues();
                                    values.put(LauncherSettings.Favorites.INTENT,
                                            intent.toUri(0));
                            ...
                            Log.e(TAG, "Problem mota update", ex);
                        } catch (URISyntaxException e) {
                            Log.e(TAG, "Problem mota update", e);
                        }
                    }
                }
                ...
                Log.w(TAG, "Problem while mota update", ex);
                return;
            } finally {
                db.endTransaction();
                if (c != null) {
        ...
        /**
         * M: Loads the scene name and the corresponding wallpaper of favorite packages from an xml file.
         * change the private modifier to the public.
         *
         * @param db The database to write the values into
         * @param sceneName The specific container id of items to load
         */
        public void updateSceneField(SQLiteDatabase db, String sceneName) {
            if (LauncherLog.DEBUG) {
                LauncherLog.d(TAG, "updateSceneField: sceneName = " + sceneName);
            }
            ...
            try {
                ...
                LauncherLog.w(TAG, "Got SQLException when update favorites.", e);
            } finally {
                db.endTransaction();
            }
        }
        ...
        /**
         * M: loads the default set of favorite packages from an xml file.
         * change the private modifier to the public.
         *
         * @param db The database to write the values into
