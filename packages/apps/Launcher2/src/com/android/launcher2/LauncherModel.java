...
public class LauncherModel extends BroadcastReceiver {
    ...
    static void moveItemInDatabase(Context context, final ItemInfo item, final long container,
        ...
        if (LauncherLog.DEBUG) {
            LauncherLog.d(TAG, "moveItemInDatabase: item = " + item + ", container = " + container + ", screen = " + screen
        ...
        /// M: added for scene feature, save the current scene value of the item.
        values.put(LauncherSettings.Favorites.SCENE, getCurrentSceneName(context));
    ...
    static void modifyItemInDatabase(Context context, final ItemInfo item, final long container,
        ...
        if (LauncherLog.DEBUG) {
            LauncherLog.d(TAG, "modifyItemInDatabase: item = " + item + ", container = " + container + ", screen = " + screen
    ...
    static void addItemToDatabase(Context context, final ItemInfo item, final long container,
        ...
        if (LauncherLog.DEBUG) {
            LauncherLog.d(TAG, "addItemToDatabase item = " + item + ", container = " + container + ", screen = " + screen
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (Intent.ACTION_PACKAGE_CHANGED.equals(action)
            ...
            if (mPreviousConfigMcc != currentConfig.mcc || mPreviousConfigMnc != currentConfig.mnc) {
                LauncherLog.d(TAG, "Reload apps on config change. curr_mcc:" + currentConfig.mcc + ", prevmcc:"
        ...
        private void loadWorkspace() {
            ...
            synchronized (sBgLock) {
                ...
                final Cursor c = contentResolver.query(LauncherSettings.Favorites.CONTENT_URI,
                		null, "scene = '" + getCurrentSceneName(context) + "'", null, null);
        ...
        public void run() {
            ...
            if (LauncherLog.DEBUG) {
                LauncherLog.d(TAG, "PackageUpdatedTask: added = " + added + ",modified = "
                        + modified + ",removedPackageNames = " + removedPackageNames
     ...
     * @return
     */
    private static String getCurrentSceneName(final Context context) {
        LauncherApplication app = (LauncherApplication) context.getApplicationContext();
        return app.getCurrentSceneName();
    }
    
    /**
     * M: update database and set wallpaper.
     */
    private void updateDatabaseAndSetWallpaper() {
        SceneChooser.SceneMetaData newScene = mApp.getCurrentScene();
        final String sceneName = mApp.getResources().getString(newScene.sceneNameResId);

        if (LauncherLog.DEBUG) {
            LauncherLog.d(TAG, "Switch to new scene: " + newScene);
        }

        if (LauncherModel.exists(mApp, sceneName)) {
            LauncherModel.resetScene(mApp, sceneName);
        }

        LauncherProvider.DatabaseHelper dbHelper = LauncherProvider.getOpenHelper();
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        dbHelper.loadFavorites(db, newScene.workspaceResId);
        dbHelper.updateSceneField(db, sceneName);
        dbHelper.close();

        SceneChooser.setWallpaper(mApp, newScene.sceneWallpaper);
        forceReload();
    }
}
