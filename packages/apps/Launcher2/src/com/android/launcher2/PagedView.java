...
public abstract class PagedView extends ViewGroup implements ViewGroup.OnHierarchyChangeListener, ScreenEffect {
    ...
    protected void getVisiblePages(int[] range) {
        ...
        if (pageCount > 0) {
        	/// M: modify to cycle sliding screen.
        	if (isSupportCycleSlidingScreen() && (mScrollX < 0 || mScrollX > mMaxScrollX)) {
        		range[0] = pageCount - 1;
        		range[1] = 0;
        	} else {
				...
				while (leftScreen < pageCount - 1
						&& currPage.getX() + currPage.getWidth() - currPage.getPaddingRight() < getScrollX()) {
					leftScreen++;
                ...
                if (Math.abs(deltaX) >= 1.0f) {
                	...
                	if (isSupportCycleSlidingScreen()) {
						if (deltaX < 0) {
							...
							final float availableToScroll = getPageAt(getChildCount() - 1).getRight() - mTouchX;
        ...
        case MotionEvent.ACTION_UP:
            if (mTouchState == TOUCH_STATE_SCROLLING) {
                ...
                if (((isSignificantMove && deltaX > 0 && !isFling) ||
                        (isFling && velocityX > 0)) && (mCurrentPage > 0 || isSupportCycleSlidingScreen())) {
                	/// M: modify to cycle sliding screen.
                	if (mCurrentPage > 0) {
                    ...
                    snapToPageWithVelocity(finalPage, velocityX);
                        ...
                        (isFling && velocityX < 0)) && (mCurrentPage < getChildCount() - 1 || isSupportCycleSlidingScreen())) {
                	/// M: modify to cycle sliding screen.
                    ...
                    if (LauncherLog.DEBUG) {
                        LauncherLog.d(TAG, "Case 3: mCurrentPage = " + mCurrentPage + ", mScrollX = " + mScrollX);
                    }
    ...
    protected void snapToPageWithVelocity(int whichPage, int velocity) {
        ...
        if (newX == mUnboundedScrollX) {
            LauncherLog.d(TAG, "snapToPageWithVelocity: do nothing because mUnboundedScrollX equals with newX.");
            return;
        }

    	/// M: modify to cycle sliding screen.
        if (isSupportCycleSlidingScreen()) {
			if (mScrollX <= 0 && mCurrentPage == 0 && whichPage == getChildCount() - 1) {
				mUnboundedScrollX = getChildCount() * getWidth() + mScrollX;
			} else if (mScrollX >= mMaxScrollX && mCurrentPage == getChildCount() - 1 && whichPage == 0) {
    ...
    protected void snapToPage(int whichPage, int duration) {
        ...
        // mUnboundedScrollX already equals to newX.
        if (newX == mUnboundedScrollX) {
            LauncherLog.d(TAG, "snapToPage: do nothing because mUnboundedScrollX equals with newX.");
            return;
        }
    ...
    public void scrollRight() {
        if (mScroller.isFinished()) {
            ...
            if (mNextPage < getChildCount() - 1) {
            	snapToPage(mNextPage + 1);
            } else if(isSupportCycleSlidingScreen() && mNextPage == getChildCount() - 1) {
            	/// M: modify to cycle sliding screen.
            	snapToPage(0);
            }
        }
