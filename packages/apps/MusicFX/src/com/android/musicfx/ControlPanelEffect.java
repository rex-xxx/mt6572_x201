...
public class ControlPanelEffect {
    ...
    /**
     * M: Map the effect parameter and save them to share preference after activity has been in backgaround.
     */
    private static final HashMap<String, String> mBooleanParameter = new HashMap<String, String>();
    private static final HashMap<String, String> mIntParameter = new HashMap<String, String>();
    
    /**
     * M: Map all the effect status when MusicFX activity is on create and use them to instead share 
     * preference to avoid too many IO.
     */
    private static final HashMap<String, Boolean> mBooleanStatus = new HashMap<String, Boolean>();
    private static final HashMap<String, Integer> mIntStatus = new HashMap<String, Integer>();
    private static List<String> mNeedSaved = new ArrayList<String>(Key.values().length);
    ...
    /// M: Add to save flat equalizer to be default
    private static final int PRESET_EQUALIZER_CURRENT_PRESET_DEFAULT = 3; // flat
    ...
    public static void initEffectsPreferences(final Context context, final String packageName,
        ...
        try {
            ...
            /// M: init all settings to a hash map from share preference,then we can get value
            /// from it to avoid writing share preference too much
            mBooleanStatus.put(Key.global_enabled.toString(), isGlobalEnabled);
            ...
            /// M: init all settings to a hash map from share preference,then we can get value
            /// from it to avoid writing share preference too much
            mBooleanStatus.put(Key.virt_enabled.toString(), isVIEnabled);
            mIntStatus.put(Key.virt_strength.toString(), vIStrength);
            ...
            /// M: init all settings to a hash map from share preference,then we can get value
            /// from it to avoid writing share preference too much
            mBooleanStatus.put(Key.bb_enabled.toString(), isBBEnabled);
            mIntStatus.put(Key.bb_strength.toString(), bBStrength);
            ...
            synchronized (mEQInitLock) {
                ...
                /// M: init all settings to a hash map from share preference when init effect,then
                /// we can get value from it to avoid writing share preference too much @{
                mIntStatus.put(Key.eq_level_range.toString() + 0, (int)mEQBandLevelRange[0]);
                mIntStatus.put(Key.eq_level_range.toString() + 1, (int)mEQBandLevelRange[1]);
                mIntStatus.put(Key.eq_num_bands.toString(), (int)mEQNumBands);
                mIntStatus.put(Key.eq_num_presets.toString(), (int)mEQNumPresets);
                /// @}
                ...
                // final short eQPreset = (short) prefs.getInt(Key.eq_current_preset.toString(), mEQNumPresets);
                // When we first launch MusicFX,the 5 Nand-Equalizer is all on 0db,so we should set the default preset
                // equalizer to be flat = 3.
                final short eQPreset = (short) prefs.getInt(Key.eq_current_preset.toString(), PRESET_EQUALIZER_CURRENT_PRESET_DEFAULT);
                editor.putInt(Key.eq_current_preset.toString(), eQPreset);
                
                /// M: init all settings to a hash map from share preference,then we can get value
                /// from it to avoid writing share preference too much
                mIntStatus.put(Key.eq_current_preset.toString(), (int)eQPreset);
                ...
                for (short band = 0; band < mEQNumBands; band++) {
                    if (controlMode == ControlMode.CONTROL_PREFERENCES) {
                        ...
                        /// M: init all settings to a hash map from share preference,then we can get value
                        /// from it to avoid writing share preference too much
                        mIntStatus.put(Key.eq_band_level.toString() + band, (int)bandLevel[band]);
                    }
                    ...
                    /// M: init all settings to a hash map from share preference,then we can get value
                    /// from it to avoid writing share preference too much
                    mIntStatus.put(Key.eq_center_freq.toString() + band, mEQCenterFreq[band]);
                    mIntStatus.put(Key.eq_preset_ci_extreme_band_level.toString() + band,
                            (int)eQPresetCIExtremeBandLevel[band]);
                    mIntStatus.put(Key.eq_preset_user_band_level_default.toString() + band,
                           (int)eQPresetUserBandLevelDefault[band]);
                    mIntStatus.put(Key.eq_preset_user_band_level.toString() + band, prefs.getInt(
                            Key.eq_preset_user_band_level.toString() + band,
                            eQPresetUserBandLevelDefault[band]));
                    mIntStatus.put(Key.eq_band_level.toString() + band, prefs.getInt(
                            Key.eq_band_level.toString() + band,eQPresetUserBandLevelDefault[band]));
                ...
                for (short preset = 0; preset < mEQNumPresets; preset++) {
                    ...
                    for (short band = 0; band < mEQNumBands; band++) {
                        ...
                        /// M: init all settings to a hash map from share preference,then we can get value
                        /// from it to avoid writing share preference too much
                        mIntStatus.put(Key.eq_preset_opensl_es_band_level.toString() + preset + "_"
                                + band, (int)mEQPresetOpenSLESBandLevel[preset][band]);
                    }
                }
            }
            ...
            /// M: init all settings to a hash map from share preference,then we can get value
            /// from it to avoid writing share preference too much
            mBooleanStatus.put(Key.eq_enabled.toString(), isEQEnabled);
            ...
            /// M: init all settings to a hash map from share preference,then we can get value
            /// from it to avoid writing share preference too much
            mBooleanStatus.put(Key.pr_enabled.toString(), isEnabledPR);
            mIntStatus.put(Key.pr_current_preset.toString(), (int) presetPR);
     ...
     * <p>M: Save the parameter settings to hashmap, so that we can get values from hashmap instead
     * of sharepreferences to avoid ANR.
     * 
     * @param context
     * @param packageName
     * @param audioSession
    ...
    public static void setParameterBoolean(final Context context, final String packageName,
        ...
        try {
            ...
            Log.v(TAG, "setParameterBoolean: audioSession = " + audioSession + ", key = " + key + "value = " + value);
            if (!mNeedSaved.contains(key.toString())) {
                mNeedSaved.add(key.toString());
            }
            // Global on/off
            if (key == Key.global_enabled) {
                ...
                if (value == true) {
                    ...
                    if (controlMode == ControlMode.CONTROL_EFFECTS) {
                        ...
                        if (virtualizerEffect != null) {
                            ...
                            final int vIStrength = prefs.getInt(Key.virt_strength.toString(),
                                    VIRTUALIZER_STRENGTH_DEFAULT);*/
                            virtualizerEffect.setEnabled(mBooleanStatus.get(Key.virt_enabled.toString()));
                            final int vIStrength = mIntStatus.get(Key.virt_strength.toString());
                        ...
                        if (bassBoostEffect != null) {
                            ...
                            final int bBStrength = prefs.getInt(Key.bb_strength.toString(),
                                    BASS_BOOST_STRENGTH_DEFAULT);*/
                            bassBoostEffect.setEnabled(mBooleanStatus.get(Key.bb_enabled.toString()));
                            final int bBStrength = mIntStatus.get(Key.bb_strength.toString());
                            setParameterInt(context, packageName,
                        ...
                        if (equalizerEffect != null) {
                            /*equalizerEffect.setEnabled(prefs.getBoolean(Key.eq_enabled.toString(),
                                    EQUALIZER_ENABLED_DEFAULT));*/
                            equalizerEffect.setEnabled(mBooleanStatus.get(Key.eq_enabled.toString()));
                            ...
                            /// M: Because every time set global enabled, it will set current
                            /// parameter to eq_band_level and this will cause MusicFX copy eq_band_level
                            /// to eq_preset_user_band_level, so we should restore it after changed.
                            final int[] userBandLevels = getParameterIntArray(context,
                                    packageName, audioSession, Key.eq_preset_user_band_level);
                            ...
                            for (short band = 0; band < len; band++) {
                                mIntParameter.put(Key.eq_preset_user_band_level.toString() + band,
                                        String.valueOf(userBandLevels[band]));
                                mIntStatus.put(Key.eq_preset_user_band_level.toString() + band, userBandLevels[band]);
                            }
                        }
                        ...
                        // creation to not use MIPS
                         ...
                         if (presetReverbEffect != null) {
                             ...
                             presetReverbEffect.setEnabled(mBooleanStatus.get(Key.pr_enabled.toString()));
                         }
                    }
                ...
                /*final boolean isGlobalEnabled = prefs.getBoolean(Key.global_enabled.toString(),
                        GLOBAL_ENABLED_DEFAULT);*/
                final boolean isGlobalEnabled = mBooleanStatus.get(Key.global_enabled.toString());
            ...
            editor.commit();*/
            mBooleanParameter.put(key.toString(), String.valueOf(enabled));
            mBooleanStatus.put(key.toString(), enabled);

            Log.v(TAG, "setParameterBoolean<<< key = " + key + "value = " + value);
        } catch (final RuntimeException e) {
     ...
     * <p>M: get values from hashmap instead of sharepreferences.
     * 
     * @param context
     * @param packageName
     * @param audioSession
    ...
    public static Boolean getParameterBoolean(final Context context, final String packageName,
        ...
        try {
            ...
            value = mBooleanStatus.get(key.toString());
        } catch (final RuntimeException e) {
    ...
    public static void setParameterInt(final Context context, final String packageName,
        ...
        try {
            ...
            Log.v(TAG, "setParameterInt: audioSession = " + audioSession + ", key = " + key + arg1 + ", value = " + arg0);
            // Set effect parameters
            if (controlMode == ControlMode.CONTROL_EFFECTS) {
                ...
                case eq_band_level: {
                    ...
                    if (equalizerEffect != null) {
                        ...
                        if (!mNeedSaved.contains(Key.eq_preset_user_band_level.toString() + band)) {
                            mNeedSaved.add(Key.eq_preset_user_band_level.toString() + band);
                        }
                        mIntParameter.put(Key.eq_preset_user_band_level.toString() + band, String.valueOf(value));
                        mIntStatus.put(Key.eq_preset_user_band_level.toString() + band, value);
                    }
                    break;
                }
                case eq_current_preset: {
                    ...
                    if (equalizerEffect != null) {
                        ...
                        final int numBands = mIntStatus.get(Key.eq_num_bands.toString());
                        final int numPresets = mIntStatus.get(Key.eq_num_presets.toString());
                        if (preset < numPresets) {
                            ...
                            for (short band = 0; band < numBands; band++) {
                                ...
                                if (preset == numPresets) {
                                    ...
                                    bandLevel = mIntStatus.get(Key.eq_preset_ci_extreme_band_level.toString() + band).shortValue();
                                } else {
                                    // User
                                    /*bandLevel = (short) prefs.getInt(
                                            Key.eq_preset_user_band_level.toString() + band,
                                            eQPresetUserBandLevelDefault[band]);*/
                                    bandLevel = mIntStatus.get(Key.eq_preset_user_band_level.toString()
                                            + band).shortValue();
                                }
                        ...
                        for (short band = 0; band < numBands; band++) {
                            ...
                            if (!mNeedSaved.contains(Key.eq_band_level.toString() + band)) {
                                mNeedSaved.add(Key.eq_band_level.toString() + band);
                            }
                            mIntParameter.put(Key.eq_band_level.toString() + band, String.valueOf(level));
                            mIntStatus.put(Key.eq_band_level.toString() + band, (int)level);
                            Log.d(TAG, "setParameterInt: key " + Key.eq_band_level.toString() + band + ", level" + level
                                    + ", numBands = " + numBands + ",numPresets =" + numPresets);
                        }
                    }
                    break;
                }
                ...
                case eq_current_preset: {
                    ...
                    for (short band = 0; band < numBands; band++) {
                        ...
                        if (preset < numPresets) {
                            ...
                            bandLevel = mIntStatus.get(Key.eq_preset_opensl_es_band_level.toString() + preset + "_"
                                            + band).shortValue();
                            ...
                            /*bandLevel = (short) prefs.getInt(
                                    Key.eq_preset_user_band_level.toString() + band,
                                    eQPresetUserBandLevelDefault[band]);*/
                            bandLevel = mIntStatus.get(Key.eq_preset_user_band_level.toString() + band).shortValue();
                        }
                        // editor.putInt(Key.eq_band_level.toString() + band, bandLevel);
                        if (!mNeedSaved.contains(Key.eq_band_level.toString() + band)) {
                            mNeedSaved.add(Key.eq_band_level.toString() + band);
                        }
                        mIntParameter.put(Key.eq_band_level.toString() + band, String.valueOf(bandLevel));
                        mIntStatus.put(Key.eq_band_level.toString() + band, (int)bandLevel);
                    }
                    break;
                }
                case eq_preset_user_band_level:
            ...
            //editor.apply();
            if (!mNeedSaved.contains(strKey.toString())) {
                mNeedSaved.add(strKey.toString());
            }
            mIntParameter.put(strKey.toString(), String.valueOf(value));
            mIntStatus.put(strKey.toString(), value);

            Log.v(TAG, "setParameterInt<<<: key = " + strKey.toString() + ", value = " + value);
        } catch (final RuntimeException e) {
    ...
    public static int getParameterInt(final Context context, final String packageName,
        ...
        try {
            /// M: get values from hashmap
            /*final SharedPreferences prefs = context.getSharedPreferences(packageName,
                    Context.MODE_PRIVATE);
            value = prefs.getInt(key, value);*/
            value = mIntStatus.get(key);
            Log.d(TAG, "getParameterInt: key = " + key + ", value = " + value);
        } catch (final RuntimeException e) {
    ...
    public static int[] getParameterIntArray(final Context context, final String packageName,
        ...
        try {
            ...
            for (int i = 0; i < intArray.length; i++) {
                ...
                intArray[i] = mIntStatus.get(key.toString() + i);
                Log.d(TAG, "getParameterIntArray: " + (key.toString() + i) + " = " + intArray[i]);
            }
    ...
    public static void openSession(final Context context, final String packageName,
        ...
        final Virtualizer virtualizerEffect = getVirtualizerEffect(audioSession);
        {
            ...
            try {
                ...
                /// M: init all settings to a hash map from share preference,then we can get value
                /// from it to avoid writing share preference too much
                mBooleanStatus.put(Key.virt_enabled.toString(), isEnabled);
                mIntStatus.put(Key.virt_strength.toString(), (int) settings.strength);
            } catch (final RuntimeException e) {
        ...
        final BassBoost bassBoostEffect = getBassBoostEffect(audioSession);
        {
            ...
            try {
                ...
                /// M: init all settings to a hash map from share preference,then we can get value
                /// from it to avoid writing share preference too much
                mBooleanStatus.put(Key.bb_enabled.toString(), isEnabled);
                mIntStatus.put(Key.bb_strength.toString(), (int) settings.strength);
            } catch (final RuntimeException e) {
                Log.e(TAG, errorTag + e);
            }
        }
        ...
        final Equalizer equalizerEffect = getEqualizerEffect(audioSession);
        {
            ...
            try {
                ...
                synchronized (mEQInitLock) {
                    ...
                    /// M: init all settings to a hash map from share preference when create 
                    /// equalizer,then we can get value from it to avoid writing share preference too much
                    mIntStatus.put(Key.eq_level_range.toString() + 0, (int)mEQBandLevelRange[0]);
                    mIntStatus.put(Key.eq_level_range.toString() + 1, (int)mEQBandLevelRange[1]);
                    mIntStatus.put(Key.eq_num_bands.toString(), (int)mEQNumBands);
                    mIntStatus.put(Key.eq_num_presets.toString(), (int)mEQNumPresets);
                ...
                /// M: init all settings to a hash map from share preference,then we can get value
                /// from it to avoid writing share preference too much      
                mBooleanStatus.put(Key.eq_enabled.toString(), isEnabled);
                if (isGlobalEnabled == true) {
        ...
        // create effect
         final PresetReverb presetReverbEffect = getPresetReverbEffect(audioSession);
         {
             ...
             /// M: init all settings to a hash map from share preference,then we can get value
             /// from it to avoid writing share preference too much      
             mBooleanStatus.put(Key.pr_enabled.toString(), isEnabled);
             mIntStatus.put(Key.pr_current_preset.toString(), (int) settings.preset);
             } catch (final RuntimeException e) {
                 Log.e(TAG, errorTag + e);
             }
         }
     ...
     */
     private static PresetReverb getPresetReverbEffect(final int audioSession) {
         ...
         if (presetReverbEffect == null) {
             try {
                 ...
                 /// M: PresetReverb is an auxiliary effect, so attach this effect directly to
                 /// session 0, i.e. the main audio output mix
                 final PresetReverb newPresetReverbEffect = new PresetReverb(0, 0);
     ...
     /**
      * M: Gets the auxiliary effect id with audio session.
      *
      * @param audioSession
      * System wide unique audio session identifier.
      * @return auxiliary effect id
      */
      public static int getAuxiliaryEffectId(final int audioSession) {
          PresetReverb presetReverb = getPresetReverbEffect(audioSession);
          int id = -1;
          if (null == presetReverb) {
              return -1;
          }
          try {
              ...
              id = -1;
              Log.e(TAG, "getAuxiliaryEffectId throw IllegalStateException: " + ex);
          }
          Log.d(TAG, "getAuxiliaryEffectId: id = " + id);
          return id;
      }
      
      /**
       * M: Gets if the reverb effect is enabled with audio session.
       *
       * @param audioSession
       * System wide unique audio session identifier.
       * @return if reverb enabled ,return true,else return false
       */
      public static boolean getReverbEnabled(final int audioSession) {
          PresetReverb presetReverb = getPresetReverbEffect(audioSession);
          ...
          try {
              enabled = presetReverb.getEnabled();
          } catch (IllegalStateException ex) {
              enabled = false;
              Log.e(TAG, "getReverbEnabled throw IllegalStateException: " + ex);
          }
          Log.d(TAG, "getReverbEnabled: enabled = " + enabled);
          return enabled;
      }
      
      /**
       * M: Save setting change in MusicFX to share preference.
       *
       * @param context
       * @param packageName
       * 
       */
      public static void saveToSharePreference(final Context context, final String packageName) {
          if (mBooleanParameter.isEmpty() && mIntParameter.isEmpty()) {
              Log.d(TAG, "The MusicFX settings is not changed, return");
              return;
          }
          Log.d(TAG, "Save The MusicFX settings changed to share preference when MusicFX onPause!");
          final SharedPreferences prefs = context.getSharedPreferences(packageName, Context.MODE_PRIVATE);
          final SharedPreferences.Editor editor = prefs.edit();
          int size = mNeedSaved.size();
          for (int i = 0; i < size; i++) {
              String name = mNeedSaved.get(i);
              if (name != null) {
                  String value = mBooleanParameter.get(name);
                  if (value != null) {
                      editor.putBoolean(name, Boolean.parseBoolean(value));
                  } else {
                      value = mIntParameter.get(name);
                      editor.putInt(name, Integer.parseInt(value));
                  }
                  Log.d(TAG, "saveToSharePreference: name = " + name + ", value = " + value);
              }
          }
          editor.commit();
          Log.d(TAG, "saveToSharePreference: mNeedSaved = " + mNeedSaved);
          mNeedSaved.clear();
          mBooleanParameter.clear();
          mIntParameter.clear();
          Log.d(TAG, "saveToSharePreference finsh and clear all save hashmap to restore for next time!");
      }
      
      /**
       * M: Reset preset reverb instances in Java so that MusicFX can new a new preset reverb.
       * @param audioSession Audio session id.
       */
      public static void resetPresetReverbInstances(int audioSession) {
          final PresetReverb presetReverb = mPresetReverbInstances.remove(audioSession);
          if (presetReverb != null) {
              presetReverb.release();
          }
          Log.d(TAG, "resetPresetReverbInstances with audioSession " + audioSession);
      }
}
