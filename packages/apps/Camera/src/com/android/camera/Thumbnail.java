...
public class Thumbnail {
    ...
    public void deleteFrom(File filesDir) {
        synchronized (sLock) {
            File file = new File(filesDir, LAST_THUMB_FILENAME);
            file.delete();
        }
    ...
    private static class Media {
        public Media(long id, int orientation, long dateTaken, Uri uri, int mediaType) {
            this.id = id;
    ...
    public static Bitmap decodeLastPictureThumb(String filePath,int inSampleSize) {
        ...
        if (LOG) {
            Log.v(TAG, "lastPictureThumb = " + lastPictureThumb + "!!!; file path" + filePath);
        }
    ...
    public static int getLastThumbnailFromContentResolver(ContentResolver resolver, Thumbnail[] result) {
        if (LOG) {
            Log.v(TAG, "getLastThumbnailFromContentResolver() begin.");
        }
        ...
        if (LOG) {
            Log.v(TAG, "getLastThumbnailFromContentResolver() media=" + media);
        }
        if (media == null) {
        ...
        if (LOG) {
            Log.v(TAG, "Quit getLastThumbnail");
        }
