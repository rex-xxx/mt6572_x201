...
public class CameraHolder {
    ...
    private CameraHolder() {
        ...
        if (mMockCameraInfo != null) {
            ...
            mNumberOfCameras = FrameworksClassFactory.getNumberOfCameras();
            mInfo = new CameraInfo[mNumberOfCameras];
            for (int i = 0; i < mNumberOfCameras; i++) {
                mInfo[i] = new CameraInfo();
                FrameworksClassFactory.getCameraInfo(i, mInfo[i]);
            }
        }
    ...
    public synchronized CameraProxy open(int cameraId)
        ...
        if (mCameraProxy != null && mCameraId != cameraId) {
        ...
        if (mCameraProxy == null) {
            try {
                ...
                if (mMockCameraInfo == null) {
                    mCameraProxy = CameraManager.instance().cameraOpen(cameraId);
                } else {
                    if (mMockCamera == null) {
                        throw new RuntimeException();
                    }
                    mCameraProxy = mMockCamera[cameraId];
                }
                mCameraId = cameraId;
