...
/**
 * Encapsulates the mobile filter framework components needed to record video with effects applied. Modeled after
 * MediaRecorder.
 */
public class EffectsRecorder {
    ...
    public interface OnSurfaceStateChangeListener {
        /**
         * Callback method to be called when the surface is disconnected(EGL context)
         * 
         * @param surfaceApiReady
         *            will be true if the surface is disconnected(EGL context).
         */
        void onStateChange(boolean surfaceApiReady);
    }
    ...
    private OnSurfaceStateChangeListener mSurfaceStateListener;

    /**
     * Determine if a given effect is supported at runtime Some effects require libraries not available on all devices
     */
    ...
    /**
     * Sets the maximum filesize (in bytes) of the recording session. This will be passed on to the MediaEncoderFilter and
     * then to the MediaRecorder ultimately. If zero or negative, the MediaRecorder will disable the limit
     */
    ...
    /**
     * Sets the maximum recording duration (in ms) for the next recording session Setting it to zero (the default) disables
     * the limit.
     */
    ...
    public void setCaptureRate(double fps) {
        ...
        if (LOG) {
            Log.v(TAG, "Setting time lapse capture rate to " + fps + " fps");
        }
        mCaptureRate = fps;
    }

    public void setPreviewSurfaceTexture(SurfaceTexture previewSurfaceTexture, int previewWidth, int previewHeight) {
        if (LOG) {
            Log.v(TAG, "setPreviewSurfaceTexture(" + this + ")" + " width = " + previewWidth
                    + " height = " + previewHeight + ",mState = " + mState);
        }
            switch (mState) {
        case STATE_RECORD:
    ...
    public void setEffect(int effect, Object effectParameter) {
        if (LOG) {
            Log.v(TAG, "setEffect: effect ID " + effect + ", parameter " + effectParameter.toString()
                    + "mState = " + mState);
        }
            switch (mState) {
        case STATE_RECORD:
        ...
        if (mState == STATE_PREVIEW || mState == STATE_STARTING_PREVIEW) {
            initializeEffect(false);
        }
    }

    public interface EffectsListener {
        void onEffectsUpdate(int effectId, int effectMsg);

        void onEffectsError(Exception exception, String filePath);
    }
    ...
    public void setOrientationHint(int degrees) {
        ...
        if (LOG) {
            Log.v(TAG, "Setting orientation hint to: " + degrees + ",mState = " + mState);
        }
    ...
    // M:
    public void setOnCameraReleasedListener(MediaRecorder.OnInfoListener infoListener) {
        switch (mState) {
        case STATE_RECORD:
            throw new RuntimeException("setOnCameraReleasedListener cannot be called while recording!");
        case STATE_RELEASED:
            throw new RuntimeException("setOnCameraReleasedListener called on an already released recorder!");
        default:
            break;
        }
        mCameraReleasedListener = infoListener;
    }
    ...
    private void initializeFilterFramework() {
        ...
        if (mCameraDisplayOrientation == 90 || mCameraDisplayOrientation == 270) {
            // Consider width is bigger than height in Origin profile, if not, mean w h has swapped.
            if (videoFrameWidth > videoFrameHeight) {
                int temp = videoFrameWidth;
                videoFrameWidth = videoFrameHeight;
                videoFrameHeight = temp;
            }
        }
        if (LOG) {
            Log.v(TAG, "frameWidth = " + videoFrameWidth + " frameHeight = " + videoFrameHeight);
        }
    ...
    private synchronized void initializeEffect(boolean forceReset) {
        if (LOG) {
            Log.v(TAG,"forceReset is " + forceReset + ",mCurrentEffect = " + mCurrentEffect
                    + ",mEffect = " + mEffect + ",mState = " + mState);
        }
        if (forceReset || mCurrentEffect != mEffect || mCurrentEffect == EFFECT_BACKDROPPER) {
            ...
            Log.v("VideoActor", "EffectsRecorder--initializeEffect--" + ",mState==" + mState);
            if (mState == STATE_PREVIEW || mState == STATE_STARTING_PREVIEW) {
            ...
            if (LOG) {
                Log.v(TAG, "New runner: " + mRunner + ". Old runner: " + mOldRunner);
            }
            if (mState == STATE_PREVIEW || mState == STATE_STARTING_PREVIEW) {
                ...
                try {
                    if (LOG) {
                        Log.v(TAG, "initializeEffect,mCameraDevice.setPreviewTexture(null)");
                    }
        ...
        case EFFECT_GOOFY_FACE:
            ...
            goofyFilter.setInputValue("currentEffect", ((Integer) mEffectParameter).intValue());
            break;
        case EFFECT_BACKDROPPER:
            ...
            if (mCameraFacing == Camera.CameraInfo.CAMERA_FACING_FRONT) {
                ...
                if (LOG) {
                    Log.v(TAG, "Setting the background to be mirrored");
                }
            }

            // for performance improvment start
            int maskWidthExp = 7;
            int maskHeightExp = 7;
            Filter replacer = mRunner.getGraph().getFilter("replacer");
            replacer.setInputValue("maskWidthExp", maskWidthExp);
            replacer.setInputValue("maskHeightExp", maskHeightExp);
            // for performance improvment end
            break;
        default:
            break;
        }
    ...
    public synchronized void startPreview() {
        if (LOG) {
            Log.v(TAG, "start preview (" + this + ")" + ",mState= " + mState + ",mEffect= "
                + mEffect + ",mEffectParameter= " + mEffectParameter + ",mProfile= " + mProfile
                + ",mPreviewSurfaceTexture= " + mPreviewSurfaceTexture + ",mCameraDevice= " + mCameraDevice);
        }
        switch (mState) {
        case STATE_STARTING_PREVIEW:
        case STATE_PREVIEW:
            ...
            if (LOG) {
                Log.v(TAG, "startPreview called when already running preview");
            }
            return;
        case STATE_RECORD:
    ...
    private SurfaceTextureSourceListener mSourceReadyCallback = new SurfaceTextureSourceListener() {
        ...
        public void onSurfaceTextureSourceReady(SurfaceTexture source) {
            if (LOG) {
                Log.v(TAG, "SurfaceTexture ready callback received,mState = " + mState);
            }
            synchronized (EffectsRecorder.this) {
                ...
                try {
                    if (LOG) {
                        Log.v(TAG, "setPreviewTexture = " + mTextureSource);
                    }
    ...
    private LearningDoneListener mLearningDoneListener = new LearningDoneListener() {
        @Override
        public void onLearningDone(BackDropperFilter filter) {
            if (LOG) {
                Log.v(TAG, "Learning done callback triggered");
            }
            // Called in a processing thread, so have to post message back to UI thread
    ...
    private OnRecordingDoneListener mRecordingDoneListener = new OnRecordingDoneListener() {
    ...
    public synchronized void startRecording() {
        ...
        recorder.setInputValue("muteAudio", mMuteAudio);
        if (!mMuteAudio) {
        ...
        if (captureTimeLapse) {
            ...
            recorder.setInputValue("timelapseRecordingIntervalUs", (long) (1000000 * timeBetweenFrameCapture));
        } else {
        ...
        if (mCameraReleasedListener != null) {
            recorder.setInputValue("releasedListener", mCameraReleasedListener);
        }
    ...
    public synchronized void stopRecording() {
        ...
        case STATE_PREVIEW:
            if (LOG) {
                Log.v(TAG, "StopRecording called when recording not active!");
            }
            return;
        case STATE_RELEASED:
        ...
        mHandler.removeCallbacks(mPlayStopSound);
        mHandler.postDelayed(mPlayStopSound, 300);
        mState = STATE_PREVIEW;
    }
    ...
    public synchronized void disconnectDisplay() {
        ...
        SurfaceTextureTarget display = (SurfaceTextureTarget) mRunner.getGraph().getFilter("display");
    ...
    public synchronized void stopCameraPreview() {
        ...
        if (mCameraDevice == null) {
            if (LOG) {
                Log.v(TAG, "Camera already null. Nothing to disconnect");
            }
            return;
        }
        mCameraDevice.stopPreview();
        try {
            if (LOG) {
                Log.v(TAG, "mCameraDevice.setPreviewTexture(null)");
            }
            mCameraDevice.setPreviewTexture(null);
        } catch (IOException e) {
    ...
    public synchronized void stopPreview() {
        if (LOG) {
            Log.v(TAG, "Stopping preview (" + this + ")" + ",mState = " + mState);
        }
        switch (mState) {
        case STATE_CONFIGURE:
            if (LOG) {
                Log.v(TAG, "StopPreview called when preview not active!");
            }
            return;
        case STATE_RELEASED:
        ...
        mOldRunner = mRunner;
        if (mSurfaceStateListener != null) {
            mSurfaceStateListener.onStateChange(false);
        }
        mRunner.stop();
        mRunner = null;
    ...
    boolean tryEnableVideoStabilization(boolean toggle) {
        ...
        if (mCameraDevice == null) {
            if (LOG) {
                Log.v(TAG, "Camera already null. Not enabling video stabilization.");
            }
            return false;
        }
        ...
        if ("true".equals(vstabSupported)) {
            if (LOG) {
                Log.v(TAG, "Setting video stabilization to " + toggle);
            }
    ...
    synchronized boolean tryEnable3ALocks(boolean toggle) {
        if (LOG) {
            Log.v(TAG, "tryEnable3ALocks ,toggle = " + toggle);
        }
        if (mCameraDevice == null) {
            if (LOG) {
                Log.v(TAG, "Camera already null. Not tryenabling 3A locks.");
            }
            return false;
        }
        Camera.Parameters params = mCameraDevice.getParameters();
    ...
    synchronized void enable3ALocks(boolean toggle) {
        ...
        if (mCameraDevice == null) {
            if (LOG) {
                Log.v(TAG, "Camera already null. Not enabling 3A locks.");
            }
            return;
        }
        Camera.Parameters params = mCameraDevice.getParameters();
    ...
    private OnRunnerDoneListener mRunnerDoneCallback = new OnRunnerDoneListener() {
        @Override
        public void onRunnerDone(int result) {
            synchronized (EffectsRecorder.this) {
                if (mSurfaceStateListener != null) {
                    mSurfaceStateListener.onStateChange(true);
                }
                if (LOG) {
                    Log.v(TAG, "Graph runner done (" + EffectsRecorder.this + ", mRunner " + mRunner + ", mOldRunner "
                            + mOldRunner + ",mState = " + mState + ")");
                }
                ...
                if (mState == STATE_PREVIEW || mState == STATE_STARTING_PREVIEW) {
                    ...
                    if (result == GraphRunner.RESULT_ERROR && mCurrentEffect == EFFECT_BACKDROPPER) {
                        sendMessage(EFFECT_BACKDROPPER, EFFECT_MSG_STARTED_LEARNING);
                    }
                    mRunner.run();
                    ...
                    if (LOG) {
                        Log.v(TAG, " camera will be/has been released");
    ...
    public synchronized void release() {
        if (LOG) {
            Log.v(TAG, "Releasing (" + this + ")" + ",mState = " + mState);
        }
    ...
    private Runnable mPlayStopSound = new Runnable() {
        public void run() {
            if (mState != STATE_CONFIGURE && mCameraSound != null) {
                mCameraSound.play(MediaActionSound.STOP_VIDEO_RECORDING);
            }
        }
    };

    public void setSurfaceStateListener(OnSurfaceStateChangeListener listener) {
        mSurfaceStateListener = listener;
    }
