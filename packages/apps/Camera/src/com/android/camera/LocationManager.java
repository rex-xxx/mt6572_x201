...
public class LocationManager {
    ...
    public Location getCurrentLocation() {
        ...
        if (mRecordLocation) {
            ...
            for (int i = 0; i < mLocationListeners.length; i++) {
                location = mLocationListeners[i].current();
                if (location != null) {
                    break;
                }
            }
        ...
        if (LOG) {
            Log.v(TAG, "getCurrentLocation() mRecordLocation=" + mRecordLocation + ", return " + location);
        }
        return location;
    }
    ...
    public void recordLocation(boolean recordLocation) {
        if (LOG) {
            Log.v(TAG, "recordLocation(" + recordLocation + ") mRecordLocation=" + mRecordLocation);
        }
    ...
    private class LocationListener
        ...
        public void onLocationChanged(Location newLocation) {
            if (LOG) {
                Log.v(TAG, "onLocationChanged(" + newLocation + ") mRecordLocation=" + mRecordLocation
                        + ", mListener=" + mListener + ", mProvider=" + mProvider);
            }
        ...
        public void onStatusChanged(String provider, int status, Bundle extras) {
            if (LOG) {
                Log.v(TAG, "onStatusChanged(" + provider + ", " + status + ") mRecordLocation="
                        + mRecordLocation + ", mListener=" + mListener);
            }
            switch(status) {
