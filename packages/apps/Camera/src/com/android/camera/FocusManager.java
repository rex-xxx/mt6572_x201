...
public class FocusManager implements Camera.OnOrientationListener , Camera.OnParametersReadyListener {
    ...
    public void setFaceView(FaceView faceView) {
        mFaceView = faceView;
        mFaceView.setFocusIndicatorRotateLayout(mFocusIndicatorRotateLayout);
    ...
    public void onShutterUp() {
        ...
        if (mLockAeAwbNeeded && (mAeLock || mAwbLock) && (mState != STATE_FOCUSING_SNAP_ON_FINISH)) {
            mAeLock = false;
            mAwbLock = false;
            mListener.setFocusParameters();
        }
    ...
    public void doSnap() {
        if (LOG) {
            Log.v(TAG, "mInitialized =" + mInitialized + " mState=" + mState);
        }
        if (!mInitialized) { return; }
    ...
    public String getFocusMode() {
        if (LOG) {
            Log.v(TAG, "getFocusMode() mOverrideFocusMode=" + mOverrideFocusMode
        ...
        if (LOG) {
            Log.v(TAG, "getFocusMode() return " + mFocusMode);
        }
        return mFocusMode;
    }
    ...
    public String getCurrentFocusMode(Camera context) {
        return context.getParameters().getFocusMode();
    }

    public FocusManager(Camera context, ComboPreferences preferences, String[] defaultFocusModes,
               View focusIndicatorRotate, Parameters parameters, Listener listener,
               boolean mirror, Looper looper, String continous) {
        mContext = context;
        mHandler = new MainHandler(looper);
        mMatrix = new Matrix();
        ...
        if (LOG) {
            Log.v(TAG, "FocusManager(" + continous + ")");
            if (defaultFocusModes != null) {
                for (int i = 0, len = defaultFocusModes.length; i < len; i++) {
                    Log.v(TAG, "FocusManager() defaultFocusModes[" + i + "]=" + defaultFocusModes[i]);
                }
            }
        }
    }
