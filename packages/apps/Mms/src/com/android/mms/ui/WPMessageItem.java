...
//            sBuilder.append("<br>");
        ...
        if (0 != mExpirationLong) {
            mExpiration = String.format(context.getString(R.string.wp_msg_expiration_label), MessageUtils
                    .formatTimeStampString(context, mExpirationLong));
        }
...
/*    public boolean isSelected() {
    ...
    public void markAsRead() {
        final Uri wpMsgUri = ContentUris.withAppendedId(
                WapPush.CONTENT_URI, mMsgId);
        new Thread(new Runnable() {
            public void run() {
                if (wpMsgUri != null) {
                    ...
                    Cursor c = mContext.getContentResolver().query(wpMsgUri, UNREAD_PROJECTION,
                    ...
                    if (needUpdate) {
                        mContext.getContentResolver().update(wpMsgUri, sReadContentValues, UNREAD_SELECTION, null);
                        mIsUnread = false;
                    }
                }
