...
//    private boolean mIsLandscape; // Whether we're in landscape mode
    ...
    private static String getWPMessageDetails(Context context, WPMessageItem msgItem) {
        ...
        if (expiredDate != 0) {
            details.append('\n');
            details.append(String.format(context.getString(R.string.wp_msg_expiration_label), MessageUtils
                    .formatTimeStampString(context, expiredDate, true)));
        }
...
//                R.plurals.confirm_delete_conversation, cnt, cnt));
    ...
    private final WPMessageListAdapter.OnDataSetChangedListener mDataSetChangedListener = new WPMessageListAdapter.OnDataSetChangedListener() {
...
//            deleteitem = menu.findItem(R.id.delete);
        ...
        public boolean onPrepareActionMode(ActionMode mode, Menu menu) {
            if (mMultiSelectActionBarView == null) {
                ViewGroup v = (ViewGroup) LayoutInflater.from(WPMessageActivity.this).inflate(
                        R.layout.conversation_list_multi_select_actionbar2, null);
    ...
    public void markAllMessageAsSeen() {
        Thread thread = new Thread(new Runnable() {
            public void run() {
                    ...
                    WapPushMessagingNotification.blockingUpdateNewMessageIndicator(WPMessageActivity.this,
    ...
    private static void blockingMarkAllWapPushMessagesAsSeen(final Context context) {
        ...
        Cursor cursor = resolver.query(WapPush.CONTENT_URI, SEEN_PROJECTION, UNSEEN_SELECTION, null, null);
        int count = 0;
        if (cursor != null) {
            try {
                count = cursor.getCount();
