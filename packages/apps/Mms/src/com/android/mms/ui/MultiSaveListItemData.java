...
public class MultiSaveListItemData {
    ...
    ///M: @{ add for attachment enhance, for save text/plain, text/html attachment files
    public MultiSaveListItemData(Context context, PduPart part, long msgid, int flag) {
        mPduPart = part;
        mMessageId = msgid;
        mFallbackName = Long.toHexString(msgid);
        mName = getNameFromPart(part);
        mSize = getSizeFromData(part.getData().length);
        final float density = context.getResources().getDisplayMetrics().density;
        mThumbnail = getThumbnailFromPart(part, context,
                getDesiredThumbnailWidth(density), getDesiredThumbnailHeight(density));
    }

    private String getSizeFromData(int size) {
    ...
    public String toString() {
        return "[MultiSaveListItemData from:" + getName() + " subject:" + getSize() + ",selected " + mItemSelected + "]";
    }
    ...
    private Bitmap getThumbnailFromPart(PduPart part, Context context, int width, int height) {
        ...
        if (EncapsulatedContentType.isImageType(type)) {
            ...
            return thumb;
        } else if (EncapsulatedContentType.isVideoType(type)) {
