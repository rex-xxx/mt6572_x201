...
import com.mediatek.encapsulation.com.android.ex.chips.EncapsulatedMTKRecipientEditTextView;

/**
 * Provide UI for editing the recipients of multi-media messages.
 */
public class RecipientsEditor extends EncapsulatedMTKRecipientEditTextView {
 ...
 /// M:       setDropDownWidth(LayoutParams.MATCH_PARENT);
    ...
    public static CharSequence contactToToken(Contact c) {
        /// M: Just one log
        MmsLog.v(TAG, "RecipientsEditor:contactToToken, contact.getNameAndNumber=" + c.getmNameAndNumberProtosomatic());
        /// M: BUGFIX:559376; change the contactName's formatter @{
        String contactName = c.getName();
        String contactNum = c.getNumberProtosomatic();
        if (!TextUtils.isEmpty(contactNum)) {
            ...
            if (contactName.matches(".*[\\(\\)<>@,;:\\\\\".\\[\\]].*")) {
                if (!contactName.matches("^\".*\"$")) {
                    MmsLog.v(TAG, "add \" for " + contactName);
                    contactName = "\"" + contactName + "\"";
                }
            }
        }
        String tokenData = contactName + "<" + contactNum + ">";
        /// @}
        SpannableString s = new SpannableString(tokenData);
    ...
    public void populate(ContactList list) {
        ...
        MmsLog.v(TAG,"RecipientsEditor:populate, list.size = " + list.size());
        if (list.size() == 0) {
            ...
            for (Contact c : list) {
                ...
                MmsLog.v(TAG,"addRecipient, name = " + displayName + ", number = " + c.getNumberProtosomatic());
    ...
    private class RecipientsEditorTokenizer
        ...
        public List<String> getNumbers() {
            ...
            /// M: BugFix:ALPS00538922 @{
            MmsLog.v(TAG, "getNumbers sp:" + sp.toString() + "|END");
            List<String> list = new ArrayList<String>();
            /// M: get numbers through using the rfc822Token tool.
            Rfc822Token[] tokens = Rfc822Tokenizer.tokenize(sp);
            if (tokens.length == 0 && (sp != null && sp.length() > 0)) {
                list.add(sp.toString());
            }
            for (Rfc822Token token : tokens) {
                MmsLog.v(TAG, "getNumbers:" + token.getAddress());
                list.add(token.getAddress());
            }
            /// @}
            return list;
        }
    ...
    protected void updatePressedChip(UpdatePressedChipType type) {
        updatePressedChip(mChipX, mChipY, type);
    }
    /// @}
}
