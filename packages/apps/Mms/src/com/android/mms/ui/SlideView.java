...
public class SlideView extends AbsoluteLayout implements
        AdaptableSlideViewInterface, IMmsSlideShowHost {
    ...
    /// M: Code analyze 002, fix bug ALPS00099267, set MTKImageView to paly gif @{
    //private MTKImageView mImageView;
      private EncapsulatedMTKImageView mImageView;
    /// @}
    ...
    private IMmsSlideShow mMmsSlideShowPlugin;
    /// @}

    /// M: fix bug ALPS00416014, parser text smiley in playing slideshow
    ...
    public void setImage(String name, Bitmap bitmap) {
        if (mImageView == null) {
            /// M: Code analyze 002, fix bug ALPS00099267, @{
          	mImageView = new EncapsulatedMTKImageView(mContext);
    ...
    public void setVideo(String name, Uri video) {
        ...
        /// M: Code analyze 004, new feature, Slideshow plugin (unknown) @{
        mMmsSlideShowPlugin.configVideoView(mVideoView);
        /// @}
    }
    ...
    public void setText(String name, String text) {
        if (!mConformanceMode) {
            ...
            if (null == mTextView) {
                ...
                /// @}
                /// M: Code analyze 004, new feature, Slideshow plugin (unknown) @{
                mMmsSlideShowPlugin.configTextView(mActivity,mTextView);
                /// @}
        ...
        /// @}
        /// M: fix bug ALPS00416014, parser text smiley in playing slideshow @{
        mTextView.setText(mParser.addSmileySpans(text));
        /// @}
        ...
        MmsConfig.setExtendUrlSpan(mTextView);
        /// @}
        /// @}
    }
    ...
    public void stopAudio() {
        if ((mAudioPlayer != null) && mIsPrepared) {
            try {
                ...
                Log.w(TAG, "Present audio exception");
            }
            mAudioPlayer = null;
            hideAudioInfo();
        } else {
    ...
    public void enableMMSConformanceMode(int textLeft, int textTop, int textWidth, int textHeight,
        ...
        if (imageLeft >=0 && imageTop >=0) {
            /// M: Code analyze 002, fix bug ALPS00099267, setPadding for MTKImageView gif(unknown) @{
            mImageView = new EncapsulatedMTKImageView(mContext);
            //mImageView = new MTKImageView(mContext);
    ...
    /// M: Code analyze 004, new feature, Slideshow plugin (unknown) @{
    public void setMmsSlideShowPlugin(IMmsSlideShow plugin) {
        mMmsSlideShowPlugin = plugin;
        mMmsSlideShowPlugin.init(this);
    }
    /// @}
