...
public class MessageListAdapter extends MessageCursorAdapter {
    ...
    private static final String SMS_IP_MESSAGE_ID = Sms.IPMSG_ID;
    private static final String SMS_IMPOIRTANT = Sms.LOCKED;
    ...
    static final String[] PROJECTION = new String[] {
        ...
        EncapsulatedTelephony.Mms.SIM_ID,
        Sms.SERVICE_CENTER,
        EncapsulatedTelephony.Mms.SERVICE_CENTER,
        /// @}
        /// M: add for ipmessage
        SMS_IP_MESSAGE_ID,
        SMS_IMPOIRTANT,
    ...
    static final int COLUMN_SMS_SIMID           = 24;
    static final int COLUMN_MMS_SIMID           = 25;
    ...
    static final int COLUMN_SMS_IMPOIRTANT      = 29;
    static final int COLUMN_SMS_SPAM            = 30;
    ...
    /// @}
    /// M: Operator Plugin
    private IMmsMultiDeleteAndForward mDeleteAndForwardPlugin  = null;
    ...
    public MessageListAdapter(
            Context context, Cursor c, ListView listView,
            boolean useDefaultColumnsMap, Pattern highlight,IMmsMultiDeleteAndForward DeleteAndForwardPlugin) {
            ...
            if (!mIsScrolling || mIsDeleteMode) {
                ...
                if (msgItem != null) {
                    ...
                    if (mIsDeleteMode) {
                        ...
                        if (msgItem.isSimMsg() && cursor.getColumnIndex("index_on_icc") != -1) {
...
//                            }
    ...
    public View newView(Context context, Cursor cursor, ViewGroup parent) {
        ...
        case INCOMING_ITEM_TYPE:
            return mInflater.inflate(R.layout.message_list_item_recv, parent, false);
        case OUTGOING_ITEM_TYPE:
            return mInflater.inflate(R.layout.message_list_item_send, parent, false);
        case INCOMING_ITEM_TYPE_IPMSG:
            return mInflater.inflate(R.layout.message_list_item_recv_ipmsg, parent, false);
        case OUTGOING_ITEM_TYPE_IPMSG:
            return mInflater.inflate(R.layout.message_list_item_send_ipmsg, parent, false);
        default:
            return mInflater.inflate(R.layout.message_list_item_send, parent, false);
        }
    }
    ...
    public MessageItem getCachedMessageItem(String type, long msgId, Cursor c) {
        ...
        if (item == null && c != null && isCursorValid(c)) {
            ...
            final boolean isDrawLoadAllMessagesButton = mIsImportantThread ? isMessageItemShowLoadAllButton(c) : false;
            if (type.equals("mms")) {
                ...
                final int simId = c.getInt(mColumnsMap.mColumnSmsSimId);
                ...
                final int mmsStatus = c.getInt(mColumnsMap.mColumnMmsStatus);
                        ...
                        try {
                            ...
                            if (backgroundItem == null) {
                                backgroundItem = new MessageItem(mContext, boxId, messageType, simId, errorType,
                                        ...
                                        isDrawLoadAllMessagesButton, indDate, mmsStatus);
                                MmsLog.d(CACHE_TAG,
                ...
                try {
                    item = new MessageItem(mContext, type, c, mColumnsMap, mHighlight, isDrawTimeDivider,
                            isDrawUnreadDivider, unreadCount, isDrawLoadAllMessagesButton, 0L);
    ...
    private int getItemViewType(Cursor cursor) {
        ...
        if ("sms".equals(type)) {
            ...
            //boxId = cursor.getInt(mColumnsMap.mColumnSmsType);
            /// M: add for ipmessage
            int ipMessageId = cursor.getInt(mColumnsMap.mColumnSmsIpMessageId);
            MmsLog.d(IPMSG_TAG, "getItemViewType(): ipMessageId = " + ipMessageId);
    ...
    public static class ColumnsMap {
        ...
        public int mColumnSmsImportant;
        ...
        public ColumnsMap() {
            ...
            mColumnSmsImportant       = COLUMN_SMS_IMPOIRTANT;
        ...
        public ColumnsMap(Cursor cursor) {
            ...
            try {
                mColumnSmsSimId = cursor.getColumnIndexOrThrow(EncapsulatedTelephony.Sms.SIM_ID);
            } catch (IllegalArgumentException e) {
                MmsLog.w(TAG, e.getMessage());
            }

            try {
                mColumnMmsSimId = cursor.getColumnIndexOrThrow(EncapsulatedTelephony.Mms.SIM_ID);
            } catch (IllegalArgumentException e) {
                MmsLog.w(TAG, e.getMessage());
            }
            ...
            try {
                mColumnMmsServiceCenter = cursor.getColumnIndexOrThrow(EncapsulatedTelephony.Mms.SERVICE_CENTER);
            } catch (IllegalArgumentException e) {
                MmsLog.w(TAG, e.getMessage());
            }
            ...
            try {
                mColumnSmsImportant = cursor.getColumnIndexOrThrow(SMS_IMPOIRTANT);
            } catch (IllegalArgumentException e) {
                MmsLog.w(TAG, e.getMessage());
            }
...
//                    }
                    ...
                    mDeleteAndForwardPlugin.setBodyandAddress(cursor,mColumnsMap.mColumnSmsAddress,
                        mColumnsMap.mColumnSmsBody,type,msgId);
                }
            }
        }  
    }
    ...
    private boolean getMessageImportatStatusByCursor(Cursor cursor) {
        MmsLog.w(IPMSG_TAG, "getMessageImportatStatusByCursor()");
        if (null == cursor) {
            MmsLog.w(IPMSG_TAG, "getMessageImportatStatusByCursor(): cursor is null!");
            return false;
        }
        if (cursor.isBeforeFirst() || cursor.isAfterLast()) {
            MmsLog.w(IPMSG_TAG, "getMessageImportatStatusByCursor(): cursor is at before first or after last!");
            return false;
        }
        String type = cursor.getString(mColumnsMap.mColumnMsgType);
        if (type.equals("mms")) {
            ...
            return cursor.getLong(mColumnsMap.mColumnSmsLocked) != 0;
        }
    }
...
//        }
        if (cursor.moveToNext()) {
            ...
            if (mLastImportantTime == 0L && isUnread) {
                mLastImportantTime = currentTime;
            }
    ...
    private boolean isMessageItemShowLoadAllButton(Cursor cursor) {
        MmsLog.d(IPMSG_TAG, "isMessageItemShowLoadAllButton(): cursor = " + cursor);
        MmsLog.d(TAG_DIVIDER, "adapter.isMessageItemShowLoadAllButton(): cursor = " + cursor);
        if (null == cursor) {
            MmsLog.w(IPMSG_TAG, "isMessageItemShowLoadAllButton(): cursor is null!");
            MmsLog.w(TAG_DIVIDER, "adapter.isMessageItemShowLoadAllButton(): cursor is null!");
            return false;
        }
        if (cursor.isBeforeFirst() || cursor.isAfterLast()) {
            MmsLog.w(IPMSG_TAG, "isMessageItemShowLoadAllButton(): cursor is at before first or after last!");
            MmsLog.w(TAG_DIVIDER, "adapter.isMessageItemShowLoadAllButton(): cursor is at before first or after last!");
            return false;
        }

        if (cursor.getCount() == 1) {
            mLastImportantTime = getMessageTimeByCursor(cursor);
            return true;
        }

        if (mLastImportantTime != 0L && mLastImportantTime == getMessageTimeByCursor(cursor)) {
            return true;
        }
        if (!getMessageImportatStatusByCursor(cursor)) {
            return false;
        }
        ...
        if (getNextMessageUnreadStatusByCursor(cursor)) {
            return true;
        }
    ...
    private long mEnterImportantThreadTime = 0L;
    private long mLastImportantTime = 0L;
    ...
    public void setIsImportantThread(boolean isImportantThread) {
        this.mIsImportantThread = isImportantThread;
        if (!mIsImportantThread) {
            mMessageItemCache.evictAll();
        }
    }

    public void setEnterImportantThreadTime(long enterImportantThreadTime) {
        this.mEnterImportantThreadTime = enterImportantThreadTime;
    }
    /// @}
    ...
    public int getSimIdFromCursor(Cursor c) {
        int simId = -1;
        try {
            String type = c.getString(mColumnsMap.mColumnMsgType);
            if (type.equals("mms")) {
                simId = c.getInt(mColumnsMap.mColumnMmsSimId);
            } else if (type.equals("sms")) {
                simId = c.getInt(mColumnsMap.mColumnSmsSimId);
            }
        } catch (Exception e) {
