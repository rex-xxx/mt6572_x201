...
public class TempFileProvider extends ContentProvider {
    ...
    public ParcelFileDescriptor openFile(Uri uri, String mode) throws FileNotFoundException {
        ...
        switch (match) {
            case MMS_SCRAP_SPACE:
                /// M: Code analyze 003,fix bug ALPS00240011,the video reached
                // size limit can't attached if replace the attachment with
                // video @{
                if (mode.contains("w")) {
    ...
    public static String getScrapPath(Context context, String fileName) {
        ...
        // MMS with image. @{
        File mFile = EncapsulatedStorageManager.getExternalCacheDir(context.getPackageName());
        if (mFile != null) {
