...
/// M: Code analyze 001, new feature, It is used for collecting log. @{
...
public class SuggestionsProvider extends android.content.ContentProvider {
    ...
    /// M: change google default to next line
    //final static int MODE = DATABASE_MODE_QUERIES + DATABASE_MODE_2LINES;
    /// M: Code analyze 001, new feature, It is used for collecting log. @{
    final static String LOG_TAG = "SuggestionsProvider";
    /// @}
    private static final UriMatcher URI_MATCHER =
        new UriMatcher(UriMatcher.NO_MATCH);
    private static final int SUGGEST_URI_PATH_SHORTCUT             = 4;
    /// M: fix bug ALPS00542953, cut out the Snippet length 2048. @{
    private static final int SNIPPET_MAX_LENGTH = 2048;
    /// @}
    static{
    ...
    public Cursor query(Uri uri, String[] projection, String selection,
        ...
        Uri u = null;
        if (null != uri && URI_MATCHER.match(uri) == SUGGEST_URI_PATH_SHORTCUT) {
            String id = uri.getLastPathSegment();
            u = Uri.parse(String.format("content://mms-sms/search_suggest_shortcut/" + id));
        } else {
            u = Uri.parse(String.format("content://mms-sms/searchSuggest?pattern=%s",
                    selectionArgs[0]));
        }
        /// M: Code analyze 002, new feature, When we query a cursor in the
        // database, we should consider exception. @{
        Cursor c = null;
        try {        
            ...
            MmsLog.e(LOG_TAG, "got exception: " + ex.toString());
            if (c != null) {
                c.close();
            }
            return null;
    ...
    private class SuggestionsCursor implements CrossProcessCursor {
        ...
        private void computeRows() {
            ...
            for (int i = 0; i < count; i++) {
                ...
                /// M: Code analyze 003, fix bug alps00231834, same words
                // needn't display twice on atuoCompleteView when searching. @{
                if (snippet != null && snippet.trim().length() > 0 &&
                        !TextUtils.equals(previousSnippet, snippet.trim())) {
                    /// M: fix bug ALPS00542953, cut out the Snippet length 2048. @{
                    if (snippet.length() > SNIPPET_MAX_LENGTH) {
                        snippet = snippet.substring(0, SNIPPET_MAX_LENGTH - 1);
                    }
                    /// @}
                    mRows.add(new Row(i, snippet));
                    MmsLog.d(LOG_TAG, "computeRows snippet = " + snippet + " size = " + snippet.length());
                    previousSnippet = snippet.trim();
                }
                /// @}
            }
        }
