...
//    };
    ...
    public static String formatNameAndNumber(String name, String number, String numberE164) {
         ...
         * }
         */
        Log.i(TAG,"formatNameAndNumber, name=" + name + ", number=" + number);
    ...
    public synchronized Drawable getAvatar(final Context context, Drawable defaultValue, final long threadId) {
        if (mAvatar == null) {
            if (mAvatarData != null) {
        ...
        if (mAvatar == null && IpMessageUtils.getIpMessagePlugin(context).isActualPlugin()) {
            if (!mIpMessageAvatarFetched) {
                if (threadId > 0 && IpMessageUtils.getContactManager(context).isIpMessageNumber(mNumber)) {
                    final Object lock = new Object();
                    mIpMessageAvatarBitmap = null;
                    sContactCache.pushTask(new Runnable() {
                        public void run() {
                            mIpMessageAvatarBitmap = IpMessageUtils.getContactManager(context).getAvatarByThreadId(threadId);
                            synchronized (lock) {
                                lock.notifyAll();
                            }
                        }
    ...
    private static class ContactsCache {
        ...
        private boolean mSelectTask = true;
        ...
        private void updateContact(final Contact c) {
            if (c == null) {
                /// M:
                MmsLog.d(M_TAG, "Contact.updateContact(): Contact for query is null.");
                return;
            }
            ...
            synchronized (c) {
                if (contactChanged(c, entry)) {
                    ...
                    /// M: add for ipmessage
                    if (c.mThreadId > 0) {
                        MmsLog.d("avatar", "Contact.updateContact(): c.mThreadId > 0, c.mName = " + c.mName);
                        if (TextUtils.isEmpty(c.mName) || c.mNumber.equals(c.mName)) {
                            c.mName = IpMessageUtils.getContactManager(mContext).getNameByThreadId(c.mThreadId);
                            MmsLog.d("avatar", "Contact.updateContact(): group name = " + c.mName);
                        }
                    } else {
                        c.mName = entry.mName;
                    }
        ...
        private Contact getContactInfo(Contact c) {
            ...
            if (c.mIsMe) {
                ...
                contact = getContactInfoForEmailAddress(c.mNumber);
                /// M: Some shortcodes are saved as Phone number. So we should find it as phone number,
...
//
//                //MmsLog.d(M_TAG, "getContactInfoForPhoneNumber(): exact query number=" + normalizedNumber);
//                Cursor cursor = mContext.getContentResolver().query(
//                        PHONES_WITH_PRESENCE_URI, CALLER_ID_PROJECTION, CALLER_ID_SELECTION_EXACT_MATCH, args, null);
//
//                /// M: fix bug ALPS00473248, WHERE min_match = ? not normalizedNumbe
//                /// when number isn't E164 @{
//                if (cursor == null || (cursor != null && cursor.getCount() == 0)) {
//                    String numberE164 = PhoneNumberUtils.formatNumberToE164(number,
//                            MmsApp.getApplication().getCurrentCountryIso());
//                    if (TextUtils.isEmpty(numberE164)) {
//                        MmsLog.d(M_TAG, "It isn't a E164 number : " + number);
//                        if (cursor != null) {
//                            cursor.close();
//                        }
//                        args = new String[] {matchNumber, numberLen, normalizedNumber, numberLen};
//                        cursor = mContext.getContentResolver().query(PHONES_WITH_PRESENCE_URI,
//                                CALLER_ID_PROJECTION, CALLER_ID_SELECTION_WITHOUT_E164, args, null);
//                    }
//                }
//                /// @}
//
//                /// M: Fix ALPS00440015, JB2 don't support 7 number match.
////                if (cursor == null || (cursor != null && cursor.getCount() == 0)) {
////                    if (cursor != null) {
////                        cursor.close();
////                    }
////                    // MmsLog.d(M_TAG,
////                    // "getContactInfoForPhoneNumber(): need query twice with minMatch! number=" +
////                    // normalizedNumber);
////                   cursor = mContext.getContentResolver().query(PHONES_WITH_PRESENCE_URI,
////                            CALLER_ID_PROJECTION, CALLER_ID_SELECTION, new String[] {matchNumber}, null);
////                }
//                /// @}
            /// M: Fix bug ALPS00527791 for smart contact matching
            String normalizedNumber = PhoneNumberUtils.normalizeNumber(number);
            String matchNumber = PhoneNumberUtils.toCallerIDMinMatch(normalizedNumber);
            if (!TextUtils.isEmpty(normalizedNumber) && !TextUtils.isEmpty(matchNumber)) {
                Cursor cursor = mContext.getContentResolver().query(
                ...
                try {
                    if (cursor.moveToFirst()) {
                        /// M:
                        MmsLog.d(M_TAG,
                            "getContactInfoForPhoneNumber(): ready to fill contact with query result. number="
                                + normalizedNumber);
                        fillPhoneTypeContact(entry, cursor);
                    }
                } finally {
        ...
        private void fillPhoneTypeContact(final Contact contact, final Cursor cursor) {
            ...
            if (MmsConfig.getIpMessagServiceId(mContext) == IpMessageServiceId.ISMS_SERVICE
                    && contact.getNumber().startsWith(IpMessageConsts.GROUP_START)) {
                return;
            }
        ...
        private byte[] loadAvatarData(Contact entry) {
            ...
            if ((!entry.mIsMe && entry.mPersonId == 0) || entry.mAvatar != null) {
                /// M:
                MmsLog.d(M_TAG, "loadAvatarData(): return null");
                return null;
            }
            ...
            /// M:
            MmsLog.d(M_TAG, "loadAvatarData(): contactUri=" + contactUri);
            InputStream avatarDataStream = Contacts.openContactPhotoInputStream(
                        mContext.getContentResolver(),
                        contactUri);
            try {
                /// M:
                MmsLog.d(M_TAG, "loadAvatarData(): avatarDataStream is null?=" + (avatarDataStream == null));
                ...
                /// M:
                MmsLog.w(M_TAG, "loadAvatarData(): IOException!");
            } finally {
                try {
                    if (avatarDataStream != null) {
        ...
        private Contact getContactInfoForEmailAddress(String email) {
            /// M:
            MmsLog.d(M_TAG, "getContactInfoForEmailAddress(): email=" + email);
            ...
            if (cursor != null) {
                /// M:
                MmsLog.d(M_TAG, "getContactInfoForEmailAddress(): cursor != null, cursor.getCount()=" + cursor.getCount());
                try {
                    while (cursor.moveToNext()) {
                        boolean found = false;
                        synchronized (entry) {
                        ...
                        /// M:
                        MmsLog.d(M_TAG, "getContactInfoForEmailAddress(): found=" + found);
                        if (found) {
        ...
        //synchronized (ContactsCache.this) {
                ...
                //MmsLog.d(M_TAG, "Contact.internalGet(): workingNumberOrEmail=" + workingNumberOrEmail);
                String key = "";
                /// @}
                ...
                if (isNotRegularPhoneNumber) {
                    ...
                    //MmsLog.d(M_TAG, "Contact.internalGet(): Number without space and '-' is a well-formed number for sending sms.");
                    if (workingNumberOrEmail.length() > NORMAL_NUMBER_MAX_LENGTH) {
                        MmsLog.d(M_TAG, "Contact.internalGet(): Long number.");
                    ...
                    MmsLog.d(M_TAG, "Contact.internalGet(): Unknown formed number.");
                    ...
                    if (numberOrEmail.equals(workingNumberOrEmail)) {
                        ...
                        MmsLog.d(M_TAG, "Contact.internalGet(): Bad number.");
                        key = numberOrEmail;
                    }
                }
        ...
        private String getKey(String numberOrEmail, boolean isMe) {
            ...
            MmsLog.d(M_TAG, "getKey(" + numberOrEmail + ", " + isMe + ") return:" + key);
            return key;
        }
                ...
                for (ArrayList<Contact> alc : mContactsHash.values()) {
                    for (Contact c : alc) {
                        synchronized (c) {
                            ...
                            c.mIpMessageAvatarFetched = false;
    ...
    /// M:
    protected Contact(String number, String label, String name, String nameAndNumber, long personId, int presence,
            String presenceText) {
        setNumber(number);
        mLabel = label;
        mName = name;
    ...
    public static String getValidNumber(String numberOrEmail) {
        ...
        /// M: add for ipmessage
        if (MmsConfig.getIpMessagServiceId(MmsApp.getApplication()) == IpMessageServiceId.ISMS_SERVICE &&
            numberOrEmail.startsWith(IpMessageConsts.GROUP_START)) {
        ...
        if (numberOrEmail.equals(SELF_ITEM_KEY) || Mms.isEmailAddress(numberOrEmail)) {
            ...
            // MmsLog.d(M_TAG,
            // "Contact.getValidNumber(): Number without space and '-' is a well-formed number for sending sms.");
            ...
            if (numberOrEmail.equals(workingNumberOrEmail)) {
                // MmsLog.d(M_TAG,
                // "Contact.getValidNumber(): Unknown formed number, but the number without local number formatting is a well-formed number.");
    ...
    public synchronized Drawable getGroupAvatar(final Context context, final long threadId) {
        MmsLog.d(TAG, "Contact.getGroupAvatar(): mAvatar is null ?= " + (mAvatar == null));
        if (mAvatar == null) {
            final Object lock = new Object();
            mIpMessageAvatarBitmap = null;
            sContactCache.pushTask(new Runnable() {
            ...
            MmsLog.d(TAG, "Contact.getGroupAvatar(): mIpMessageAvatarBitmap is null ?= " + (mIpMessageAvatarBitmap == null));
            if (null != mIpMessageAvatarBitmap) {
