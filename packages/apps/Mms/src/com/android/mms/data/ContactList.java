...
public class ContactList extends ArrayList<Contact>  {
    ...
    public static ContactList getByNumbers(String semiSepNumbers,
        ...
        if (semiSepNumbers != null) {
            for (String number : semiSepNumbers.split(";")) {
                if (!TextUtils.isEmpty(number)) {
                    MmsLog.d(TAG, "ContactList.getByNumbers(): before Contact.get(), number=" + number);
                    Contact contact = Contact.get(number, canBlock);
                    MmsLog.d(TAG, "ContactList.getByNumbers(): after Contact.get(), number=" + contact.getNumber());
                    list.add(contact);
                }
            }
        }
    ...
    public static ContactList getByNumbers(Context mContext, String semiSepNumbers, boolean canBlock,
        ...
        if (semiSepNumbers.contains(";")) {
            ...
            MmsLog.d(TAG, "ContactList.getByNumbers()--selection = \"" + selection + "\"");
            ...
            Cursor cursor = mContext.getContentResolver().query(PHONES_WITH_PRESENCE_URI, CALLER_ID_PROJECTION,
                selection, null, null);
            if (cursor == null) {
                MmsLog.w(TAG, "ContactList.getByNumbers(" + semiSepNumbers + ") returned NULL cursor! contact uri used "
    ...
    public static void removeNumberFromList(LinkedList<String> list, String number) {
        ...
        while (go) {
            ...
            MmsLog.d(TAG, "removeNumberFromList(): i=" + i + ", j=" + j + ", start=" + start + ", end=" + end);
            if (j == 0) {
                MmsLog.d(TAG, "removeNumberFromList(): remove number: " + list.get(i));
                list.remove(i);
                break;
            } else if (j > 0) {
                start = i + 1;
