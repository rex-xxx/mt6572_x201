...
public class Conversation {
    ...
    public static final String[] ALL_THREADS_PROJECTION = {
        ...
        /// M:
        , Threads.TYPE ,EncapsulatedTelephony.Threads.READCOUNT ,EncapsulatedTelephony.Threads.STATUS,
        ThreadSettings._ID /// M: add for common
    };
    ...
    public static final String[] ALL_THREADS_PROJECTION_EXTEND = {
        ...
        , Threads.TYPE ,EncapsulatedTelephony.Threads.READCOUNT ,EncapsulatedTelephony.Threads.STATUS,
        ThreadSettings._ID, /// M: add for common
        ThreadSettings.NOTIFICATION_ENABLE, ThreadSettings.SPAM, ThreadSettings.MUTE,
        Threads.LATEST_IMPORTANT_DATE, Threads.LATEST_IMPORTANT_SNIPPET, Threads.LATEST_IMPORTANT_SNIPPET_CHARSET,
...
//    private static ContentValues mReadContentValues;
    ...
    public static final int IMP_DATE       = 16;
    public static final int IMP_SNIPPET    = 17;
    public static final int IMP_SNIPPET_CS = 18;
    public static final int MUTE_START     = 19;

    /// M: add for ipmessage
    private boolean mNotificationEnable;
    private boolean mSpamStatus;
    private boolean mMuteStatus;
    private long mImpDate;// the latest important/lock messages's date
    private String mImpSnippet;// the latest important/lock messages's text
    ...
    public synchronized int hashCode() {   
        ...
        if (mType == EncapsulatedTelephony.Threads.WAPPUSH_THREAD) {
     ...
     */
    public static void startQueryForAll(AsyncQueryHandler handler, int token, long delay) {
        /// M: Google default
        ...
        startQuery(handler, token, null, delay);
    }
     ...
     */
    public static void startQuery(AsyncQueryHandler handler, int token, String selection, long delay) {
        /// M: Code analyze 068, For bug ALPS00043722, to resolve MS enter Message slower than HTC . @{
        final int queryToken = token;
        final AsyncQueryHandler queryHandler = handler;
        final String localSelection = selection;
        /// @}
        ...
        Runnable queryRunnable = new Runnable() {
            public void run() {
                queryHandler.startQuery(queryToken, null, sAllThreadsUriExtend,
                    ALL_THREADS_PROJECTION_EXTEND, localSelection, null, Conversations.DEFAULT_SORT_ORDER);
            }
        };
    ...
    public static void startDeleteAll(AsyncQueryHandler handler, int token, boolean deleteAll, int maxMmsId, int maxSmsId) {
        ...
        uri = MmsConfig.appendExtraQueryParameterForConversationDeleteAll(uri);
        /// @}
...
//                }
    ...
    private static void fillFromCursor(Context context, Conversation conv,
        ...
        synchronized (conv) {
            ...
            // get unread count
            if (conv.mType == Threads.IP_MESSAGE_GUIDE_THREAD) {
                conv.mUnreadMessageCount = 3;
            } else {
            ...
            if (c.getColumnCount() > SPAM && sIsActivated) {
                ...
                conv.mSpamStatus = (spam == 1) ? true : false;

                // update field needed by important
                conv.mImpDate = c.getLong(IMP_DATE);
                // Replace the snippet with a default value if it's empty.
                String snippet2 = MessageUtils.extractEncStrFromCursor(c, IMP_SNIPPET, IMP_SNIPPET_CS);
                if (TextUtils.isEmpty(snippet2)) {
                    snippet2 = context.getString(R.string.no_subject_view);
                }
                conv.mImpSnippet = snippet2;
            }
        }
        ...
        synchronized (conv) {
            ...
            /// M: update message count if it is a group message, performance ok?
            if (sIsActivated && recipients.size() != 0 &&
                recipients.get(0).getNumber().startsWith(IpMessageConsts.GROUP_START)) {
                conv.mMessageCount = IpMessageUtils.getGroupManager(context).getGroupIpMessageCount(conv.mThreadId);
                conv.mType = TYPE_GROUP;
            }
        }
    ...
    private static class Cache {
        ...
        static void keepOnly(Set<Long> threads) {
            synchronized (sInstance) {
//                Iterator<Conversation> iter = sInstance.mCache.values().iterator();
//                Conversation c = null;
//                while (iter.hasNext()) {
//                    c = iter.next();
...
//                }
    ...
    */
    ...
    private boolean loadFromThreadId(long threadId, boolean allowQuery) {
        ...
        Cursor c = null;
        c = mContext.getContentResolver().query(sAllThreadsUriExtend, ALL_THREADS_PROJECTION_EXTEND,
    ...
    private static void blockingMarkAllCellBroadcastMessagesAsSeen(final Context context) {
        ContentResolver resolver = context.getContentResolver();
        Cursor cursor = resolver.query(EncapsulatedTelephony.SmsCb.CONTENT_URI,
                SEEN_PROJECTION,
                "seen=0",
                null,
                null);
        ...
        resolver.update(EncapsulatedTelephony.SmsCb.CONTENT_URI,
                values,
                "seen=0",
                null);
    }
    ...
    public synchronized Uri getThreadSettingsUri() {
        ...
        return ContentUris.withAppendedId(EncapsulatedTelephony.ThreadSettings.CONTENT_URI, mThreadSettingsId);
    }
    ...
    /**
     * Returns the time of the last important message time of this conversation in milliseconds,
     * on the {@link System#currentTimeMillis} timebase.
     */
    public synchronized long getImpDate() {
        return mImpDate;
    }

    /**
     * Returns a snippet of text from the most recent important message in the conversation.
     */
    public synchronized String getImpSnippet() {
        return mImpSnippet;
    }

    /**
     * Returns conversation's typing status.
     */
    public synchronized boolean getTyping() {
        return mIsTyping;
    }

    /**
     * Set conversation's typing status.
     */
