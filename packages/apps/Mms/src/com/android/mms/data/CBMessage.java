...
public class CBMessage {
    ...
    // Query all URI
    private static final Uri ALL_MESSAGE_URI = EncapsulatedTelephony.SmsCb.CONTENT_URI;
    ...
    private static final Uri CHANNEL_URI_SLOT_0 = Uri.parse("content://cb/channel");
    private static final Uri CHANNEL_URI_SLOT_1 = Uri.parse("content://cb/channel1");
    private static final Uri CHANNEL_URI_SLOT_2 = Uri.parse("content://cb/channel2");
    private static final Uri CHANNEL_URI_SLOT_3 = Uri.parse("content://cb/channel3");

    private static final String[] ALL_MESSAGE_PROJECTION = { "_id",
            EncapsulatedTelephony.SmsCb.SIM_ID, EncapsulatedTelephony.SmsCb.CHANNEL_ID,
            EncapsulatedTelephony.SmsCb.BODY, EncapsulatedTelephony.SmsCb.READ,
            EncapsulatedTelephony.SmsCb.DATE, EncapsulatedTelephony.SmsCb.THREAD_ID };
    private static final String[] ALL_CHANNEL_PROJECTION = { "_id",
            EncapsulatedTelephony.SmsCb.CbChannel.NAME,
            EncapsulatedTelephony.SmsCb.CbChannel.NUMBER };
    ...
    public synchronized String getChannelName() {
        ...
        if (mChannelName == null) {
            if (EncapsulatedFeatureOption.MTK_GEMINI_SUPPORT) {
                mChannelName = CBMessage.getCBChannelNameGemini(mChannelId, mSimId);
            } else {
                mChannelName = CBMessage.getCBChannelName(mChannelId);
            }
        }
        return mChannelName;
    }
    ...
    public static synchronized String getCBChannelName(int channelId) {
        return getCBChannelNameGemini(channelId, 0);
    }

    public static synchronized String getCBChannelNameGemini(int channelId, int simId) {
        Uri uri = null;
        if (0 != simId) {
            int slotId = SIMInfo.getSlotById(MmsApp.getApplication().getApplicationContext(), simId);
            switch (slotId) {
                case 0:
                     uri = CHANNEL_URI_SLOT_0;
                     break;
                case 1:
                     uri = CHANNEL_URI_SLOT_1;
                     break;
                case 2:
                     uri = CHANNEL_URI_SLOT_2;
                     break;
                case 3:
                     uri = CHANNEL_URI_SLOT_3;
                     break;
                default:
                     LogTag.debug("invalid soltId: " + slotId);
                     return MmsApp.getApplication().getApplicationContext().getString(R.string.cb_default_channel_name);
            }
        } else {
            uri = CHANNEL_URI_SLOT_0;
        }
    ...
    public static void startQueryForAll(AsyncQueryHandler handler, int token) {
        ...
        handler.startQuery(token, null, ALL_MESSAGE_URI, ALL_MESSAGE_PROJECTION,
                null, null, EncapsulatedTelephony.SmsCb.DEFAULT_SORT_ORDER);
    }
    ...
    private static void fillFromCursor(Context context, CBMessage message,
        ...
        synchronized (message) {
            ...
            message.mSimId = c.getInt(SIM_ID);
