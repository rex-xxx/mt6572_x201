...
public class MediaModelFactory {
    ...
    private static MediaModel createEmptyTextModel(Context context,  RegionModel regionModel)
            throws IOException {
        return new TextModel(context, EncapsulatedContentType.TEXT_PLAIN, null, regionModel);
    }
    ...
    private static MediaModel getGenericMediaModel(Context context,
        ...
        if (tag.equals(SmilHelper.ELEMENT_TAG_TEXT)) {
            ...
            if (contentType.equals("application/oct-stream") || contentType.equals("application/octet-stream")) {					
                if (src!=null) {
                    ...
                    if (suffix.equals("")) {
                        ...
                        contentType = EncapsulatedContentType.IMAGE_BMP;
                    } else if (suffix.equals(".jpg")) {
                        contentType = EncapsulatedContentType.IMAGE_JPG;
                    } else if (suffix.equals(".wbmp")) {
                        contentType = EncapsulatedContentType.IMAGE_WBMP;						
                    } else if (suffix.equals(".gif")) {
                        contentType = EncapsulatedContentType.IMAGE_GIF;
                    } else if (suffix.equals(".png")) {
                        contentType = EncapsulatedContentType.IMAGE_PNG;
                    } else if (suffix.equals(".jpeg")) {
                        contentType = EncapsulatedContentType.IMAGE_JPEG;
                    } else {
            ...
            if (EncapsulatedContentType.isTextType(contentType)) {
                media = new TextModel(context, contentType, src,
                        part.getCharset(), part.getData(), regionModel);
            } else if (EncapsulatedContentType.isImageType(contentType)) {
                media = new ImageModel(context, contentType, src,
                        part.getDataUri(), regionModel);
            } else if (EncapsulatedContentType.isVideoType(contentType)) {
                media = new VideoModel(context, contentType, src,
                        part.getDataUri(), regionModel);
            } else if (EncapsulatedContentType.isAudioType(contentType)) {
                media = new AudioModel(context, contentType, src,
                        part.getDataUri());
            } else {
