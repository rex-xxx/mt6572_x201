...
public class SmilHelper {
    ...
    private static PduPart findSmilPart(PduBody body) {
        ...
        for(int i = 0; i < partNum; i++) {
            ...
            if (Arrays.equals(part.getContentType(),
                    EncapsulatedContentType.APP_SMIL.getBytes())) {
    ...
    private static SMILDocument createSmilDocument(Context context, PduBody pb) {
        ...
        EncapsulatedDrmManagerClient drmManagerClient = MmsApp.getApplication().getDrmManagerClient();
        ...
        for (int i = 0; i < partsNum; i++) {
            ...
            if (EncapsulatedContentType.isDrmType(contentType)) {
                contentType = drmManagerClient.getOriginalMimeType(part.getDataUri());
            }

            if (contentType.equals(EncapsulatedContentType.TEXT_PLAIN)
                    || contentType.equalsIgnoreCase(EncapsulatedContentType.APP_WAP_XHTML)
                    || contentType.equals(EncapsulatedContentType.TEXT_HTML)) {
