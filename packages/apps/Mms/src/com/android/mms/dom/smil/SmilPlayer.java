...
public class SmilPlayer implements Runnable {
    ...
    /// M: Code analyze 002, fix bug ALPS00246974, new SmilPlayer for updating state when playing slideshow @{
    public static SmilPlayer getPlayer() {
        /** M: mtk81083 use the singlton pattern will come out a bug when there is  animated gif file.
         * the onStop of Activity is invoked late when there is animated gif in the mms.
         * if open the mms again just after out of it, the state is in a mess, the previous state is still keeping.
         */
        //if (sPlayer == null) {
    ...
    public synchronized void pause() {
        if (isPlayingState()) {
            ...
            /// M: Code analyze 003, new feature, must change PlayerState when change PlayerAction @{
    ...
    private synchronized void reloadActiveSlide() {
        ...
        for (int i = mCurrentSlide; i < mCurrentElement; i++) {
            /// M: Code analyze 004, new feature, Exception handle @{
            try{
            ...
            /// M: @{
                ...
                Log.e(TAG, "Unexpected IndexOutOfBoundsException.", e);
            }
            /// @}
        }
        seekActiveMedia();
    ...
    private synchronized void waitForEntry(long interval)
        ...
        /// M: Code analyze 005, unknown, fix mCurrentTime Line @{
        long offset = interval + mCurrentTime;
        /// @}
        long overhead = 0;
        ...
        while (interval > 0) {
            ...
            if (overhead < sleep) {
                ...
                sleep = 0;
                if(mCurrentTime + overhead < offset){
    ...
    /// M: Code analyze 006, new feature, set player action(start) 
    /// for automatically resuming the paused player @{
    public synchronized void setStateStart() {
        mAction = SmilPlayerAction.START;
        notifyAll();
    }
    /// @}

    /// M: Code analyze 007, new feature, for setting prevButton,
    /// should return when currentSlide <= 1 @{
    public int getCurrentSlide(){
    	return mCurrentSlide;
    }
    /// @}

    /// M: Code analyze 008, fix bug ALPS00111374, (CMCC feature)
    /// pause the playing but need prepareToPlay when enter the slideshow activity @{
    public synchronized void prepareToPlay() {
        if (!isPlayingState()) {
            mCurrentTime = 0;
            mCurrentElement = 0;
            mCurrentSlide = 0;
