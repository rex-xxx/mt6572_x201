...
public class MmsMessageSender implements MessageSender {
    ...
    /// M:Code analyze 001,new added members,add for gemini @{
    private int mSimId;
    /// @}
    ...
    public boolean sendMessage(long token) throws MmsException {
        ...
        /// M:Code analyze 003, add for gemini,save corresponding simID into database @{
            // set sim id 
        ...
        if (cr != null) {
            try {
                if ((cr.getCount() == 1) && cr.moveToFirst()) {
                    ...
                    valuesforPending.put(EncapsulatedTelephony.MmsSms.PendingMessages.SIM_ID,
                            mSimId);
                    int columnIndex = cr.getColumnIndexOrThrow(PendingMessages._ID);
        ...
        if (EncapsulatedFeatureOption.MTK_GEMINI_SUPPORT) {        
            transactionIntent.putExtra(EncapsulatedPhone.GEMINI_SIM_ID_KEY, mSimId);
        }
    ...
    private void updatePreferencesHeaders(SendReq sendReq) throws MmsException {
        ...
        // Delivery report.
        /// M:Code analyze 006, change to if else for gemini,if in gemini mode,get info from
        /// multicardpreferences.xml,but at present,delivery report is not useful for mms @{
        boolean dr;
        if (EncapsulatedFeatureOption.MTK_GEMINI_SUPPORT) {
            dr = prefs.getBoolean(mSimId + "_" + MmsPreferenceActivity.MMS_DELIVERY_REPORT_MODE,
                    DEFAULT_DELIVERY_REPORT_MODE);
        } else {
            dr = prefs.getBoolean(MmsPreferenceActivity.MMS_DELIVERY_REPORT_MODE,
                    DEFAULT_DELIVERY_REPORT_MODE);
        }
        /// @}
        ...
        // Read report.
        /// M:Code analyze 007, change to if else for gemini,but at present,read report
        /// is not useful for mms @{
        boolean rr;
        if (EncapsulatedFeatureOption.MTK_GEMINI_SUPPORT) {
            rr = prefs.getBoolean(mSimId + "_" + MmsPreferenceActivity.READ_REPORT_MODE,
                    DEFAULT_READ_REPORT_MODE);
        } else {
            rr = prefs.getBoolean(MmsPreferenceActivity.READ_REPORT_MODE,
                    DEFAULT_READ_REPORT_MODE);
        }
        /// @}
    ...
    /// M:Code analyze 008, add for gemini,new methods
