...
/**
* Handle when the time is set
* The expired message should be rechecked!
*/
public class WapPushTimeChangedReceiver extends BroadcastReceiver {

    static final String TAG = "Mms/WapPush";

    private static WapPushTimeChangedReceiver sInstance;

    public static WapPushTimeChangedReceiver getInstance() {
        if (sInstance == null) {
            sInstance = new WapPushTimeChangedReceiver();
        }
        return sInstance;
    }
    ...
    protected void onReceiveTimeChange(final Context context, Intent intent) {
        ...
        if (action.equals(Intent.ACTION_TIME_CHANGED) || action.equals(Intent.ACTION_DATE_CHANGED)) {
            MmsLog.i(TAG,"onReceiveTimeChange");
            ...
            new Thread() {
                public void run() {
                    try {
                        ...
                        MmsLog.e(TAG,"SiExpiredCheck.onTimeChanged Error!");
                        return;
                    }
                }
            }.start();
