...
public abstract class Transaction extends Observable {
    ...
    public boolean isEquivalent(Transaction transaction) {
        /** M: @{ */
        MmsLog.d(MmsApp.TXN_TAG, "Transaction: isEquivalent mId=" + mId 
        ...
        MmsLog.d(MmsApp.TXN_TAG, "Transaction: isEquivalent id1=" + id1 + "\t id2=" + id2);
        boolean bClassEqual = false;
        /// M: fix bug ALPS00415570
        boolean autoDownload = DownloadManager.getInstance().isAuto(transaction.mSimId);
        
        if (getClass().equals(transaction.getClass())) {
            bClassEqual = true;
        } else if (autoDownload && ((this.getType() == RETRIEVE_TRANSACTION
                && transaction.getType() == NOTIFICATION_TRANSACTION)
                || (this.getType() == NOTIFICATION_TRANSACTION
                && transaction.getType() == RETRIEVE_TRANSACTION))) {
            bClassEqual = true;
        }
        
        return bClassEqual && (Uri.parse(mId).getLastPathSegment()
                .equals(Uri.parse(transaction.mId).getLastPathSegment()));
        /** @} */
    }
