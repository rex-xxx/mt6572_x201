...
public class WapPushMessagingNotification {
    ...
    private static final String[] WAPPUSH_THREAD_ID_PROJECTION = new String[] { WapPush.THREAD_ID };
    ...
    public static void nonBlockingUpdateNewMessageIndicator(final Context context, final long newMsgThreadId) {
    ...
    private static void updateNotification(
        ...
        if (isNew) {
            SharedPreferences sp = PreferenceManager.getDefaultSharedPreferences(context);
            /** M: comment this, not use
            String vibrateWhen;
            if (sp.contains(NotificationPreferenceActivity.NOTIFICATION_VIBRATE_WHEN)) {
                vibrateWhen =
                    sp.getString(NotificationPreferenceActivity.NOTIFICATION_VIBRATE_WHEN, null);
            } else if (sp.contains(NotificationPreferenceActivity.NOTIFICATION_VIBRATE)) {
                vibrateWhen =
                        sp.getBoolean(NotificationPreferenceActivity.NOTIFICATION_VIBRATE, false) ?
            ...
            */
            ...
            String ringtoneStr = sp.getString(NotificationPreferenceActivity.NOTIFICATION_RINGTONE,
                    null);
            /** M: for brazil request, when calling still have sound @{ */
            if (EncapsulatedFeatureOption.MTK_BRAZIL_CUSTOMIZATION_CLARO) {
        ...
        if (messageCount == 1) {
            ...
            if (uniqueThreadCount == 1) {
                ...
                WapPushNotificationInfo infos[] =
                        sNotificationSet.toArray(new WapPushNotificationInfo[sNotificationSet.size()]);
    ...
    public static long getWapPushThreadId(Context context, Uri uri) {
        ...
        try {
            if (cursor.moveToFirst()) {
                return cursor.getLong(cursor.getColumnIndex(WapPush.THREAD_ID));
            } else {
                return THREAD_NONE;
            }
        } finally {
    ...
    public static boolean notifySlAutoLanuchMessage(Context context, String url) {
        ...
        MmsLog.d(TAG, "notifySlAutoLanuchMessage, enabled = " + enabled);        
        if (!enabled) {
            return false;
        }
