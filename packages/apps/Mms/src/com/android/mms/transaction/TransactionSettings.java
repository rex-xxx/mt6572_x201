...
public class TransactionSettings {
    ...
    public TransactionSettings(Context context, String apnName) {
        ...
        try {
            while (cursor.moveToNext() && TextUtils.isEmpty(mServiceCenter)) {
                ...
                if (isValidApnType(cursor.getString(COLUMN_TYPE), EncapsulatedPhone.APN_TYPE_MMS)) {
                    sawValidApn = true;
                    /// M:Code analyze 001, check null,if yes,set service center to null @{
                    mServiceCenter = cursor.getString(COLUMN_MMSC) != null
                            ? NetworkUtils.trimV4AddrZeros(cursor.getString(COLUMN_MMSC).trim()) : null;
                    /// @}
    ...
    static private boolean isValidApnType(String types, String requestType) {
        ...
        for (String t : types.split(",")) {
            if (t.equals(requestType) || t.equals(EncapsulatedPhone.APN_TYPE_ALL)) {
                return true;
            }
        }
        return false;
    ...
    /// M:Code analyze 002, a new cunstructor methods,add for gemini @{
    /**
     * Constructor that uses the default settings of the MMS Client.
     *
     * @param context The context of the MMS Client
    ...
    public TransactionSettings(Context context, String apnName, int slotId) {
        ...
        if ( slotId >= EncapsulatedPhone.GEMINI_SIM_1) {
            cursor = SqliteWrapper.query(context, context.getContentResolver(), Uri
                    .withAppendedPath(Uri.parse("content://telephony/carriers_sim" + (slotId + 1)),
                            "current"), APN_PROJECTION, selection, null, null);
        } else {
            MmsLog.e(MmsApp.TXN_TAG, "Invalide slot id:" + slotId);
        }

        if (cursor == null) {
            MmsLog.e(MmsApp.TXN_TAG, "Apn is not found in Database!");
            return;
        }
        ...
        try {
            while (cursor.moveToNext() && TextUtils.isEmpty(mServiceCenter)) {
                ...
                if (isValidApnType(cursor.getString(COLUMN_TYPE), EncapsulatedPhone.APN_TYPE_MMS)) {
                    ...
                    MmsLog.d(MmsApp.TXN_TAG, "Service Center=" + mServiceCenter);
                    mProxyAddress = cursor.getString(COLUMN_MMSPROXY);
                    MmsLog.d(MmsApp.TXN_TAG, "Proxy=" + mProxyAddress);
                    if (isProxySet()) {
                        String portString = cursor.getString(COLUMN_MMSPORT);
                        MmsLog.d(MmsApp.TXN_TAG, "Port=" + portString);
                        try {
                            mProxyPort = Integer.parseInt(portString);
                        } catch (NumberFormatException e) {
                            if (TextUtils.isEmpty(portString)) {
