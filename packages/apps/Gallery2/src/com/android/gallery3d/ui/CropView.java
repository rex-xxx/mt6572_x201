...
public class CropView extends GLView {
    ...
    private static final String TAG = "Gallery2/CropView";
    ...
    private boolean mStereoWallpaperMode = false;
    ...
    public CropView(AbstractGalleryActivity activity) {
        ...
        /// M: change the border color acording theme change @{
        if (MediatekFeature.isThemeManagerSupported() &&  MediatekFeature.themeMainColor() != 0) {
            mPaint.setColor(MediatekFeature.themeMainColor());
        } else {
    ...
    private class FaceHighlightView extends GLView {
        ...
        protected void renderBackground(GLCanvas canvas) {
            //when setting stereo wallpaper, we don't show any box
            if (mStereoWallpaperMode) return;
    ...
    private class HighlightRectangle extends GLView {
        ...
        protected boolean onTouch(MotionEvent event) {
            //when setting stereo wallpaper, we don't show any box
            if (mStereoWallpaperMode) return true;
    ...
    public void setStereoWallpaperMode(boolean mode) {
        mStereoWallpaperMode = mode;
    }

    public void setStereoMode(boolean stereoMode) {
        if (mStereoMode == stereoMode) return;

        Log.i(TAG,"setStereoMode(stereoMode="+stereoMode+")");
        mStereoMode = stereoMode;
        //we should synchronize ?
        getGLRoot().setStereoMode(mStereoMode, false);
    }
