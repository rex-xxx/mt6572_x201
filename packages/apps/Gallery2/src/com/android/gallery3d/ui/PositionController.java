...
public class PositionController {
    ...
    boolean mPopFromTop;
    /// M: if mBoxes.get(0) == null ,should return mLastScale as mCurrentScale @{
    ...
    public void setViewSize(int viewW, int viewH) {
        Log.d(TAG, "invoke setViewSize");
        Log.d(TAG, "the viewW is: " + viewW + ", the viewH is: " + viewH);
    ...
    public void setImageSize(int index, Size s, Rect cFrame) {
        ...
        //if (cFrameChanged) {
            // M: if camera frame changed (most probably due to preview size change)
    ...
    private boolean setBoxSize(int i, int width, int height, boolean isViewSize) {
        ...
        if (width == b.mImageW && height == b.mImageH) {
            if (!b.mSubTypeChanged) {
                return false;
            } else {
                b.mSubTypeChanged = false;
            }
        }
    ...
    private boolean startAnimation(int targetX, int targetY, float targetScale,
        ...
        // M: for Mediatek feature
        mtkPostStartAnimation(targetX, targetY, targetScale, kind);
    ...
    private void convertBoxToRect(int i) {
        ...
        if (i == 0) {
            ...
            // M: possible loss of precision in "w / 2"
            //r.left = x - w / 2;
            r.left = x - Math.round((float) w / 2f);
            r.right = 2 * x - r.left;
        } else if (i > 0) {
        ...
        // M: possible loss of precision in "h/2"
        //r.top = y - h / 2;
        r.top = y - Math.round((float) h / 2f);
        r.bottom = r.top + h;
    }
    ...
    public void moveBox(int fromIndex[], boolean hasPrev, boolean hasNext,
            boolean constrained, Size[] sizes, int[] types) {
        //debugMoveBox(fromIndex);
        mHasPrev = hasPrev;
        mHasNext = hasNext;

        RangeIntArray from = new RangeIntArray(fromIndex, -BOX_MAX, BOX_MAX);
        // M: used to avoid read null value when run moveBox
        ...
        for (int i = -BOX_MAX; i <= BOX_MAX; i++) {
            ...
            if (boxesBackup.get(i) != null) continue;
            ...
            setImageSubType(i, types[i + BOX_MAX]);
    ...
    /// M: Modify for JE:
    //{@ After Thread(UI thread or child thread) update the ScreenNails(p.reload()), should wake up RenderThread to draw.
    //and after that The thread should update Movebox.
    //while do mBoxes.put(i, null), RenderThread maybe do getImageScale(), should got JE.
    public float getImageScale() {
        Box b = mBoxes.get(0);
        if (b != null) {
            mLastScale = b.mCurrentScale;
        }
        return mLastScale;
    }
    ///@}
    ...
    private float getMaximalScale(Box b) {
        ...
        if (MediatekFeature.doesMaxEqualMin(b.mSubType)) return getMinimalScale(b);
        return SCALE_LIMIT;
    }
    ...
    private class Platform extends Animatable {
        ...
        private boolean interpolateLinear(float progress) {
            ...
            if (progress >= 1) {
                ...
                mCurrentX = (int) (mFromX + progress * (mToX - mFromX) + 0.5f);
                mCurrentY = (int) (mFromY + progress * (mToY - mFromY) + 0.5f);
                if (mAnimationKind == ANIM_KIND_CAPTURE) {
                    return false;
                } else {
    ...
    private class Box extends Animatable {
        ...
        // The sub-type indicate what type rendering the box should be displayed
        // This variable is defined for Mediatek added features.
        public int mSubType = 0;

        // The following varible indicate if the sub-type has change since last
        // time we recalculate the position of the box.
        public boolean mSubTypeChanged = false;
    ...
    public void setImageSubType(int index, int subType) {
        Box b = mBoxes.get(index);

        if (subType == b.mSubType) return;

        b.mSubType = subType;
        b.mSubTypeChanged = true;
    }
    ...
    public static long sLastBoxAnimationTime = 0;

    private void mtkPostStartAnimation(int targetX, int targetY, float targetScale,
            int kind) {
        if (kind == ANIM_KIND_ZOOM) {
            sLastBoxAnimationTime = 0;
        }
    }

    private void mtkPostAdvanceAnimation() {
        Box currentBox = mBoxes.get(0);
        if (null == currentBox) {
            Log.e(TAG, "mtkPostAdvanceAnimation:why we got a null current box!");
            return;
        }
        if (currentBox.mAnimationStartTime == NO_ANIMATION) {
            // M: for performance auto test: the current box has finished animation
            sLastBoxAnimationTime = SystemClock.uptimeMillis();
        }

        //only when we are not in "film mode", and we are not in zoomed state,
        //we enter 3d display mode
        if (currentBox.mScaleMax == currentBox.mScaleMin) {
            if (mFilmMode || MediatekFeature.doesMaxEqualMin(currentBox.mSubType)) {
                mStereoMode = false;
            } else {
                mStereoMode = true;
            }
        } else if (currentBox.mCurrentScale > currentBox.mScaleMin) {
            if ((currentBox.mCurrentScale - currentBox.mScaleMin) /
                currentBox.mScaleMin > 0.01) {
                mStereoMode = false;
            }
        } else {
            mStereoMode = true;
    ...
    // M: hit test to ignore top and bottom;
    ...
    ///M: added to judge whether first box is ready @{
