...
public class TileImageView extends GLView {
    ...
    public TileImageView(GalleryContext context) {
        ...
        if (TILE_SIZE == 0) {
            // M: change tile size to 248 to improve tiling image quality.
            // 1 / (254 + 1 + 1) has long tail, causing float number round-off error
            // and may make special patterned image looks very bad (such as 1 pixel
            // white strip and 1 pixel black strip interwined).
            // 1 / (248 + 1 + 1) = 0.004, which is accurate.
            // This patch is potentially less performance and more memory usage.
    ...
    public void notifyModelInvalidated() {
        ...
        if (mModel == null) {
            ...
            mStereoScreenNail[0] = null;
            mStereoScreenNail[1] = null;
            mStereoScreenNail[2] = null;
        } else {
            ...
            // M: added for stereo feature
            setScreenNail(1, mModel.getStereoScreenNail(1));
            setScreenNail(2, mModel.getStereoScreenNail(2));
        }
    ...
    private void layoutTiles(float centerX, float centerY, float scale, int rotation) {
        ...
        Log.d(TAG, "current level is: " + mLevel + ", level count is: " + mLevelCount);
    ...
    protected void render(GLCanvas canvas) {
        ...
        try {
            if (renderStereo(canvas)) {
                //stereo screen has been drawn, we make some operation and return
                if (mRenderComplete) {
    ...
    private boolean renderStereo(GLCanvas canvas) {
        if (!isStereoMode()) {
            return false;
        }

        //in stereo mode, wo do not draw with tile fashion
        ScreenNail stereoScreenNail = findStereoScreenNail();
        //Utils.assertTrue(null != stereoScreenNail);
        if (null == stereoScreenNail) {
            Log.e(TAG, "renderStereo: got null stereo screen nail!");
            return false;
        }

        //draw the correct stereo screen nail
        stereoScreenNail.draw(canvas, mOffsetX, mOffsetY,
                              Math.round(mImageWidth * mScale),
                              Math.round(mImageHeight * mScale));
        return true;
    }

    private ScreenNail findStereoScreenNail() {
        Utils.assertTrue(getGLRoot().getStereoMode() &&
                MediatekFeature.isStereoDisplaySupported());

        if (getGLRoot().getStereoPassId() == 
            MediatekFeature.STEREO_DISPLAY_RIGHT_PASS) {
            return mStereoScreenNail[2];
        } else {
            if (null != mStereoScreenNail[1]) {
                return mStereoScreenNail[1];
            } else {
                return mScreenNail;
            }
        }
