...
// M: change to BitmapTexture from TiledTexture: upload the whole bitmap takes 
// a long time, especially big image, so we often look a PLACEHOLDER_COLOR bitmap when 
// image display and so on, in order to improve the usability, we decide to change it
public class TiledTexture implements Texture {
    ...
    public static class Uploader implements OnGLIdleListener {
        ...
        public synchronized void clear() {
            mTextures.clear();
            ///M: if not set mIsQueued as true here, 
    ...
    private static class Tile extends UploadedTexture {
        ...
        protected Bitmap onGetBitmap() {
            if (bitmap == null) {
                Log.i(TAG, "onGetBitmap(): bitmap is null");
    ...
    public static void prepareResources() {
        /// M:catch out of memory error when create bitmap fail.@{
        try {
            sUploadBitmap = Bitmap.createBitmap(TILE_SIZE, TILE_SIZE, Config.ARGB_8888);
        } catch (OutOfMemoryError e) {
            MtkLog.i(TAG, "TileTexture prepareResource createBitmap outofmemory error");
            sUploadBitmap = null;
        }
        /// @}
