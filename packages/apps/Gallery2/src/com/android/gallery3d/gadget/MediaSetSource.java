...
public class MediaSetSource implements WidgetSource, ContentListener {
    ...
    private static final String TAG = "Gallery2/MediaSetSource";
    ...
    public synchronized Uri getContentUri(int index) {
        ...
        if (index < mCacheStart || index >= mCacheEnd) {
            if (MtkLog.DBG) {
                MtkLog.e(TAG, "getContentUri: index out of range: " + index + ", start=" + mCacheStart + ", end=" + mCacheEnd);
            }
            return null;
        }
    ...
    public synchronized Bitmap getImage(int index) {
        ...
        if (index < mCacheStart || index >= mCacheEnd) {
            if (MtkLog.DBG) {
                MtkLog.e(TAG, "getImage: index out of range: " + index + ", start=" + mCacheStart + ", end=" + mCacheEnd);
            }
            return null;
        }
        if (MtkLog.DBG) {
            MediaItem item = mCache[index - mCacheStart];
            MtkLog.i(TAG, "getImage: mediaitem=" + (item == null ? "null" : item.getName()));
        }
    ...
    public void reload() {
        ...
        if (mSourceVersion != version) {
            MtkLog.i(TAG, "reload: new data version!");
