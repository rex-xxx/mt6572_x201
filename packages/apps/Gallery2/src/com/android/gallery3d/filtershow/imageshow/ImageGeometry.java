...
public abstract class ImageGeometry extends ImageSlave {
    ...
    /// M: use to save previous geometry data and bitmap @{
    private GeometryMetadata mPreLocalGeometry = new GeometryMetadata();
    ...
    private static final String LOGTAG = "Gallery2/ImageGeometry";
    ...
    protected void syncLocalToMasterGeometry() {
        ...
        calculateLocalScalingFactorAndOffset();
        /// M: save previous geometry and bitmap @{
        mPreLocalGeometry.set(mLocalGeometry);
        Bitmap bitmap = getMaster().getFilteredImage();
        if(null != bitmap) {
            try {
                mPreBitmap = bitmap.copy(Bitmap.Config.ARGB_8888, true);
            } catch (Exception e) {
                mPreBitmap = null;
            }
    ...
    protected void onVisibilityChanged(View changedView, int visibility) {
        ...
        if (visibility == View.VISIBLE) {
            ...
            /// M: while the view is invisible or gone, should set backUpImage as null.
            backUpImage = null;
    ...
    ///M: use to update utility panel data @{
    ...
    public void saveAndSetPreset() {
        ...
        if (lastHistoryItem != null && lastHistoryItem.historyName().equalsIgnoreCase(getName())) {
            ...
            if (mLocalGeometry.hasModifications()) {
                ...
                /// M: imagePreset has save in history
                getPanelController().haveSavePreset(true);
            }
        }
        invalidate();
    }
    ...
    public void onDraw(Canvas canvas) {
        ...
        /// M: while image != null, mean has got the newest image.
        if (image != null) {
            backUpImage = image;
            mHasDrawn = true;
        }
        /// M: use backUpImage to draw. The backUpImage is old image or newest image.
        if (backUpImage != null) {
            drawShape(canvas, backUpImage);
        }
        /// M: if image is null, should refresh the view again.
        if (image == null) {
            invalidate();
        }
    }
    ...
    /// M: recover geometry and reset cache
    /// set current image with previous bitmap @{
    public void showPreGeometry() {
        if(mLocalGeometry.equals(mPreLocalGeometry)) {
            return;
        }
        mLocalGeometry.set(mPreLocalGeometry);
        getImagePreset().setGeometry(mLocalGeometry);
        resetImageCaches(this);
        getMaster().updateFilteredImage(mPreBitmap);
        mPreBitmap = null;
    }
    /// }@
