...
public class ImageLoader {
    ...
    private static final String LOGTAG = "Gallery2/ImageLoader";
    ...
    public void recycleBitmaps() {
        ...
        // release cache
        mCache.recycleBitmaps();
        mHiresCache.recycleBitmaps();
    }
    /// @}
        ...
        try {
            ...
            if (uri != null && ContentResolver.SCHEME_CONTENT.equals(uri.getScheme())) {
                try {
                    ...
                    Log.d(LOGTAG, "come here,oritation:" + orientation);
                } catch (FileNotFoundException e1) {
                    e1.printStackTrace();
                }
            } else {
                // Ignore error for no orientation column; just use the default orientation value 0.
                    MtkLog.e(LOGTAG, "getOrientation: uri=" + uri + ", cannot fetch orientation!");
            }
            return orientation;
        } finally {
    ...
    private Bitmap loadScaledBitmap(Uri uri, int size) {
        ...
        try {
            ...
            closeStream(is);
            is = mContext.getContentResolver().openInputStream(uri);
            Bitmap mbitmap = BitmapFactory.decodeStream(is, null, o2);
            /// M: wbmp format should decode index8 format ,and index8 format do not support BitmapFactory resize;
            // So We should resize the bitmap for avoid out of memory.
            if (mbitmap.getConfig() == null) {
                mbitmap = BitmapUtils.resizeBitmapByScale(mbitmap, (float) (1/Math.pow(2, scale)), true);
            }
            return mbitmap;
        } catch (OutOfMemoryError e) {
            Log.e(LOGTAG, "[loadScaledBitmap] out of memory: " + uri);
        } catch (FileNotFoundException e) {
            Log.e(LOGTAG, "FileNotFoundException: " + uri);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public void saveImage(ImagePreset preset, final FilterShowActivity filterShowActivity,
        ...
        new SaveCopyTask(mContext, mUri, destination, new SaveCopyTask.Callback() {
            ...
            @Override
            public void onComplete(Uri result, int failedType) {
                filterShowActivity.completeSaveImage(result, failedType);
            }
