...
public class DirectPresetCache implements Cache {
    ...
    private static final String LOGTAG = "Gallery2/DirectPresetCache";
    ...
    private final Vector<Bitmap> mBitmapCache = new Vector<Bitmap>();
    ...
    protected void compute(CachedPreset cache) {
        ...
        // cache.mBitmap = null;
        /// M:
        if (cache.mBitmap != null) {
            mBitmapCache.add(cache.mBitmap);
        }
        if (mOriginalBitmap == null) {
            return;
        }
        cache.mBitmap = mOriginalBitmap.copy(mBitmapConfig, true);
        ///M: out of memory will return null  @{
        if(cache.mBitmap == null) {
            return;
        }
        ///}@
        ...
        /// M:
        Bitmap temp = cache.mBitmap;
        cache.mBitmap = cache.mPreset.apply(cache.mBitmap);
        if (temp != cache.mBitmap) {
            mBitmapCache.add(temp);
        }
    ...
    public void recycleBitmaps() {
        for (int i = 0; i < mBitmapCache.size(); i++) {
            ...
            mBitmapCache.set(i, null);
        }
        mBitmapCache.clear();

        for (int i = 0; i < mCache.size(); i++) {
            Bitmap bm = ((CachedPreset)mCache.get(i)).mBitmap;
            if (bm != null) {
                bm.recycle();
                bm = null;
            }
