...
public class SlideshowPage extends ActivityState {
    private static final String TAG = "Gallery2/SlideshowPage";
    ...
    private void showPendingBitmap() {
        ...
        /// M: pass media type inside in order to identify videos @{
        // mSlideshowView.next(slide.bitmap, slide.item.getRotation(), slide.subType);
        mSlideshowView.next(slide.bitmap, slide.item.getRotation(), slide.subType, slide.item.getMediaType());
        /// @}
    ...
    private void initializeData(Bundle data) {
        ...
        if (mIsDrmSupported) {
            mDrmInclusion = data.getInt(DrmHelper.DRM_INCLUSION, 
        ...
        /// M: slide show video thumbnail as well @{
        // never sildeshow camera short cut
        ...
        if (mIsDrmSupported) {
            mediaSet = mActivity.getDataManager().getMediaSet(mediaPath,
                                       mDrmInclusion);
        } else {
            mediaSet = mActivity.getDataManager().getMediaSet(mediaPath);
        }
        ...
        if (random) {
            ...
            Path path = null;
            if (mIsDrmSupported) {
                int mtkInclusion = data.getInt(DrmHelper.DRM_INCLUSION);
                path = itemPath != null ? Path.fromString(itemPath, mtkInclusion) : null;
            } else {
                path = itemPath != null ? Path.fromString(itemPath) : null;
            }
    ...
    private void initializeViews() {
        /// M: pass mActivity inside to obtain a Context to retrieve resource files
    ...
    private static class ShuffleSource implements SlideshowDataAdapter.SlideshowSource {
        ...
        public long reload() {
            ...
            long version = mMediaSet.reloadForSlideShow();
