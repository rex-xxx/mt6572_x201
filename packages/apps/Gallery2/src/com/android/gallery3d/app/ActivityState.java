...
abstract public class ActivityState {
    ...
    void resume() {
        ...
        if (actionBar != null) {
            if ((mFlags & FLAG_HIDE_ACTION_BAR) != 0) {
                actionBar.hide();
            } else if (!mShouldKeepLightsOutWhenResume) {
                // M: do not show action bar automatically if lights were out
                actionBar.show();
            }
    ...
    // M: remember lights out conditions for PhotoPage
    public boolean mShouldKeepLightsOutWhenResume = false;
}
