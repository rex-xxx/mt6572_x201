...
class LocalSource extends MediaSource {
    ...
    public MediaObject createMediaObject(Path path) {
        ...
        switch (mMatcher.match(path)) {
            ...
            case LOCAL_ALL_ALBUM: {
                ...
                if (mIsDrmSupported) {
                    imagePath = LocalAlbumSet.getPathImage(mtkInclusion).getChild(bucketId,
                                                         mtkInclusion);
                    videoPath = LocalAlbumSet.getPathVideo(mtkInclusion).getChild(bucketId,
                                                         mtkInclusion);
                } else {
                    imagePath = LocalAlbumSet.PATH_IMAGE.getChild(bucketId);
                    videoPath = LocalAlbumSet.PATH_VIDEO.getChild(bucketId);
                }
        ...
        try {
            switch (mUriMatcher.match(uri)) {
                case LOCAL_IMAGE_ITEM: {
                    ...
                    String mtkInclusion = uri.getQueryParameter("mtkInclusion");
                    if (null != mtkInclusion) {
                        Path path = id < 0 ? null : 
                            LocalImage.getItemPath(Integer.valueOf(mtkInclusion))
                                      .getChild(id,Integer.valueOf(mtkInclusion));
                        return path;
                    }
                ...
                case LOCAL_VIDEO_ITEM: {
                    ...
                    if (null != mtkInclusion) {
                        Path path = id < 0 ? null : 
                            LocalVideo.getItemPath(Integer.valueOf(mtkInclusion))
                                      .getChild(id,Integer.valueOf(mtkInclusion));
                        return path;
                    }
    ...
    public void mapMediaItems(ArrayList<PathId> list, ItemConsumer consumer) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (mIsDrmSupported) {
                if (parent == LocalImage.getItemPath(parent.getMtkInclusion())) {
                    imageList.add(pid);
                } else if (parent == LocalVideo.getItemPath(parent.getMtkInclusion())) {
                    videoList.add(pid);
                }
                continue;
            }
    ...
    private void processMapMediaItems(ArrayList<PathId> list,
        ...
        for (int i = 0; i < n; ) {
            ...
            MediaItem[] items = LocalAlbum.getMediaItemById(
                    mApplication, isImage, ids, pid.path.getMtkInclusion());
            for(int k = i ; k < j; k++) {
                PathId pid2 = list.get(k);
                // M: for null item, we do NOT consume it
                if (items[k - i] != null) {
