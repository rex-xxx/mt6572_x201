...
public class MtpDeviceSet extends MediaSet
    ...
    private static final String TAG = "Gallery2/MtpDeviceSet";
    ...
    public synchronized long synchronizedAlbumData(){
        if (mNotifier.isDirty()) {
            if (mDeviceSet != null ) {
                Iterator <MediaSet> iter = mDeviceSet.iterator();
                MediaSet device;
                while (iter.hasNext()) {
                    device = iter.next();
                    device.reload();
                    // If album.getTotalMediaItemCount() == 0, means there is not a photo is this folder,
                    //so should delete album.
                    if (device.getTotalMediaItemCount() == 0) {
                        iter.remove();
                    }
                }
                mDataVersion = nextVersionNumber();
