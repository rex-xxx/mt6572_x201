...
public abstract class MediaSet extends MediaObject {
    ...
    protected int enumerateTotalMediaItems(
        ...
        for (int i = 0; i < m; i++) {
            if (skipMediaSet(getSubMediaSet(i))) {
                continue;
            }
    ...
    protected boolean skipMediaSet(MediaSet mediaSet) {
        //skip media set that has 0 bucket id.
        //In StereoDisplay feature, there is a virtual folder named
        //"3D Media" containing all 3D media on local storage.
        //But when enumerating by clustering, the virtual folder
        //should be skipped, or there will be duplicating items,
        //resulting illigal status in Gallery.
        //whether we should do the same thing in getTotalMediaItemCount()?
        if ((mediaSet instanceof LocalAlbum || 
             mediaSet instanceof LocalMergeAlbum) &&
            mediaSet.getPath().toString().endsWith(
                StereoHelper.INVALID_LOCAL_PATH_END)) {
            Log.d(TAG,"skipMediaSet:skia folder with 0 bucket_id");
            return true;
        }
        return false;
    }
    ...
    /// M: while reload in ClusterAlbum ,should assign this object to ClusterAlbumSet.
    public MediaSet mCurrentClusterAlbum;
    /// M: We think  recursive calls in ClusterAlbum and ClusterAlbumSet as a Stack , and record offset to the Stack top.
