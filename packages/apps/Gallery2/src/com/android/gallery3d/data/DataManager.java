...
public class DataManager implements StitchingChangeListener {
    ...
    private static final String TAG = "Gallery2/DataManager";
    ...
    public MediaObject getMediaObject(String s, int mtkInclusion) {
        Path path = Path.fromString(s, mtkInclusion);
        path.setMtkInclusion(mtkInclusion);
    ...
    //added by Mediatek to support DRM feature
    public MediaSet getMediaSet(String s, int mtkInclusion) {
        return (MediaSet) getMediaObject(s, mtkInclusion);
    }
    ...
    public MediaSet[] getMediaSetsFromString(String segment,
        ...
        MediaSet[] sets = new MediaSet[n];
        for (int i = 0; i < n; i++) {
            sets[i] = getMediaSet(seq[i], mtkInclusion);
        }
        return sets;
    }
    ...
    /// M: for refresh all set
    public void forceRefreshAll() {
        MtkLog.d(TAG, "forceRefreshAll");
