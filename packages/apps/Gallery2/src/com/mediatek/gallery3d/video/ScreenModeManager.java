...
public class ScreenModeManager {
    private static final String TAG = "Gallery2/VideoPlayer/ScreenModeManager";
    ...
    public void setScreenModes(final int screenModes) {
        ...
        if (LOG) {
            MtkLog.v(TAG, "enableScreenMode(" + screenModes + ") mScreenModes=" + mScreenModes);
        }
    }
    ...
    public void setScreenMode(final int curScreenMode) {
        if (LOG) {
            MtkLog.v(TAG, "setScreenMode(" + curScreenMode + ")");
        }
    ...
    public int getScreenMode() {
        if (LOG) {
            MtkLog.v(TAG, "getScreenMode() return " + mScreenMode);
        }
        return mScreenMode;
    }
    ...
    public int getNextScreenMode() {
        ...
        if (LOG) {
            MtkLog.v(TAG, "getNextScreenMode() = " + mode);
        }
        return mode;
    }
