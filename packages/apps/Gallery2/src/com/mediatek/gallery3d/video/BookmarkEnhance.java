...
public class BookmarkEnhance {
    ...
    public Uri insert(final String title, final String uri, final String mimeType,final long position) {
        ...
        if (LOG) {
            MtkLog.v(TAG, "insert(" + title + "," + uri + ", " + position + ") return " + insertUri);
        }
        return insertUri;
    }
    ...
    public int delete(final long id) {
        ...
        if (LOG) {
            MtkLog.v(TAG, "delete(" + id + ") return " + count);
        }
        return count;
    }
    ...
    public int deleteAll() {
        ...
        if (LOG) {
            MtkLog.v(TAG, "deleteAll() return " + count);
        }
        return count;
    }
    ...
    public boolean exists(final String uri) {
        ...
        if (LOG) {
            MtkLog.v(TAG, "exists(" + uri + ") return " + exist);
        }
        return exist;
    }
    ...
    public Cursor query() {
        ...
        if (LOG) {
            MtkLog.v(TAG, "query() return cursor=" + (cursor == null ? -1 : cursor.getCount()));
        }
        return cursor;
    }
    ...
    public int update(final long id, final String title, final String uri, final int position) {
        ...
        if (LOG) {
            MtkLog.v(TAG, "update(" + id + ", " + title + ", " + uri + ", " + position + ")" +
                    " return " + count);
        }
        return count;
    }
