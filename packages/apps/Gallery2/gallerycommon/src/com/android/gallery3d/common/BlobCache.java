...
public class BlobCache implements Closeable {
    ...
    private void closeAll() {
        mIndexBuffer = null;
        Log.d(TAG, "mapped buffer closed");
        closeSilently(mIndexChannel);
        Log.d(TAG, "index file channel closed");
        closeSilently(mIndexFile);
        Log.d(TAG, "index file closed");
        closeSilently(mDataFile0);
        Log.d(TAG, "data file 0 closed");
        closeSilently(mDataFile1);
        Log.d(TAG, "data file 1 closed");
    }
    ...
    private boolean loadIndex() {
        try {
            ...
            Log.i(TAG, "<loadIndex> mIndexChannel.map");
    ...
    static void closeSilently(Closeable c) {
        ...
        try {
            ...
            // do nothing
            Log.e(TAG, "throwable in closeSilently(" + c + "): ", t);
        }
    }
