...
public class MovieListLoader implements IMovieListLoader {  
    private static final String TAG = "Gallery2/MovieListLoader";
    ...
    public void fillVideoList(Context context, Intent intent, LoaderListener l, IMovieItem item) {
        ...
        if (LOG) {
            MtkLog.v(TAG, "fillVideoList() fetechAll=" + fetechAll + ", orderBy=" + orderBy);
        }
    }
    ...
    public boolean isEnabledVideoList(Intent intent) {
        ...
        if (LOG) {
            MtkLog.v(TAG, "isEnabledVideoList() return " + enable);
        }
        return enable;
    }
    ...
    private class MovieListFetcherTask extends AsyncTask<IMovieItem, Void, IMovieList> {
        ...
        public MovieListFetcherTask(Context context, boolean fetechAll, LoaderListener l, String orderBy) {
            ...
            mOrderBy = orderBy;
            if (LOG) {
                MtkLog.v(TAG, "MovieListFetcherTask() fetechAll=" + fetechAll + ", orderBy=" + orderBy);
            }
        }
        ...
        protected void onPostExecute(IMovieList params) {
            if (LOG) {
                MtkLog.v(TAG, "onPostExecute() isCancelled()=" + isCancelled());
            }
            if (isCancelled()) {
                return;
            }
        ...
        protected IMovieList doInBackground(IMovieItem... params) {
            if (LOG) {
                MtkLog.v(TAG, "doInBackground() begin");
            }
            if (params[0] == null) {
                return null;
            }
            ...
            if (LOG) {
                MtkLog.v(TAG, "doInBackground() done return " + movieList);
            }
            return movieList;
        }
        ...
        private IMovieList fillUriList(String where, String[] whereArgs, long curId, IMovieItem current) {
            ...
            try {
                cursor = mCr.query(MediaStore.Video.Media.EXTERNAL_CONTENT_URI,
                        new String[]{"_id", "mime_type", OpenableColumns.DISPLAY_NAME, COLUMN_STEREO_TYPE},
                        where,
                        whereArgs,
                        mOrderBy);
                boolean find = false;
                if (cursor != null && cursor.getCount() > 0) {
                    ...
                    while (cursor.moveToNext()) {
                        ...
                        int stereoType = cursor.getInt(3);
                        movieList.add(new MovieItem(uri, mimeType, title, stereoType));
                    }
                }
            ...
            if (LOG) {
                MtkLog.v(TAG, "fillUriList() cursor=" + cursor + ", return " + movieList);
            }
            return movieList;
        }
    }
