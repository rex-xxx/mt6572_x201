...
/**
 * M: An extend Base64 InputStream that can decode the
 * extra data behind the standard end flag '='.
...
public class ExtendBase64InputStream extends FilterInputStream {
    private final ExtendBase64.Coder coder;
    ...
    public ExtendBase64InputStream(InputStream in, int flags, boolean encode) {
        ...
        if (encode) {
            coder = new ExtendBase64.Encoder(flags, null);
        } else {
            coder = new ExtendBase64.Decoder(flags, null);
        }
