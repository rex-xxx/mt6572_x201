...
public class MimeMessage extends Message {
    ...
    private static final SimpleDateFormat SIMPLE_DATE_FORMAT_3 = new SimpleDateFormat(
            "EEE,dd MMM yyyy HH:mm ss Z", Locale.US);
    ...
    public Date getSentDate() throws MessagingException {
        if (mSentDate == null) {
            String dateString = getFirstHeader("Date");
            try {
                DateTimeField field = (DateTimeField)Field.parse("Date: "
                        + MimeUtility.unfoldAndDecode(dateString));
                mSentDate = field.getDate();
                if (mSentDate == null) {
                    throw new RuntimeException("try to parse the date in exception block");
                }
            } catch (Exception e) {
                Log.d("MimeMessage", "1st  :" + dateString + " failed");
                try {
                    mSentDate = SIMPLE_DATE_FORMAT_1.parse(dateString);
                    return mSentDate;
                } catch (Exception e1) {
                    Log.d("MimeMessage", "2nd round parse date string:" + dateString + " failed");
                }
                try {
                    mSentDate = SIMPLE_DATE_FORMAT_2.parse(dateString);
                    return mSentDate;
                } catch (Exception e1) {
                    Log.d("MimeMessage", "3nd round parse date string:" + dateString + " failed");
                }
                try {
                    mSentDate = SIMPLE_DATE_FORMAT_3.parse(dateString);
                } catch (Exception e1) {
                    Log.d("MimeMessage", "3nd round parse date string:" + dateString + " failed");
                }
            }
