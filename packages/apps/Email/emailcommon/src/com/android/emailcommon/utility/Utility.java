...
public class Utility {
    ...
    /**
     * M: Modified host name to protocol in order to avoid adding duplicated
     * account which will be caused when server's address is changed.eg:hotmail @{
     */
    private final static String HOSTAUTH_WHERE_CREDENTIALS = HostAuthColumns.PROTOCOL + " like ?"
     ...
     * @param hostProtocol the protocol of server's address
    ...
    public static Account findExistingAccount(Context context, long allowAccountId,
            String hostProtocol, String userLogin, String serverAddr) {
        ContentResolver resolver = context.getContentResolver();
        if (userLogin == null) {
            return null;
        }
        String userName = userLogin.trim().replace("_", "$_");
        Cursor c = resolver.query(HostAuth.CONTENT_URI, HostAuth.CONTENT_PROJECTION,
                HOSTAUTH_WHERE_CREDENTIALS, new String[] {
                        hostProtocol, userName
                }, null);
        if (c == null) {
        ...
        try {
            while (c.moveToNext()) {
                ...
                String qryUsrNam = c.getString(HostAuth.CONTENT_LOGIN_COLUMN);
                if (!userLogin.trim().equals(qryUsrNam)) {
                    continue;
                }
                ...
                String qryServerAddr = c.getString(HostAuth.CONTENT_ADDRESS_COLUMN);
                if (serverAddr == null) {
                    isExistingAccount = true;
                } else {
                    isSameServer = (serverAddr != null && qryServerAddr != null)
                            && (serverAddr.toLowerCase()
                                    .trim().equals(qryServerAddr.toLowerCase().trim()));
                    if (isSameServer) {
                        isExistingAccount = true;
                    }
                }
    ...
    public static boolean attachmentExists(Context context, Attachment attachment) {
        ...
        try {
            ...
            try {
                ...
                /** M: try to read the stream to decide whether the attachment exist really. @} */
                try {
                    inStream.read();
                } catch (IOException e) {
                    Log.w(Logging.LOG_TAG, "attachmentExists IOException=" + e);
                    return false;
                }finally {
                    try {
                        inStream.close();
            ...
            if (!attachmentExists(context, att)) {
                ...
                if ((att.mFlags & (Attachment.FLAG_DOWNLOAD_FORWARD |
                    ...
                    if (att.mContentUri != null) {
                        /**
                         * M: In this case, the attachment file is gone from the cache; let's clear the
                         * contentUri; this may be caused by AUTO CLEAR CACHE, so we should update uri
                         * to NULL, and return true to wait for unloaded attachments before send the
                         * message @{
                         */
                    ...
                    /** M: In this case, the message is a forwarded message, return true
                     * to wait for unloaded attachments before send the message. @{ */
                    Logging.d("Send Email wait for unloaded attachment " + att.mFileName +
                            ", #" + att.mId);
                    return true;
                    /** @} */
                }
    ...
    /**
     * M: Check whether the message with a given id has lost some attachments.
     * This can happened when user add attachment from SD card and then delete it or
     * unmount the SD card.
     * @param context
     * @param messageId
     * @return true, if some attachments manual added can't find now.
     */
    public static boolean isSomeAttachmentsLost(Context context, long messageId) {
        Message msg = Message.restoreMessageWithId(context, messageId);
        if (msg == null)
            return false;
        Attachment[] atts = Attachment.restoreAttachmentsWithMessageId(context, messageId);
        for (Attachment att : atts) {
            if (((att.mFlags & (Attachment.FLAG_SMART_FORWARD
                    | Attachment.FLAG_DOWNLOAD_FORWARD | Attachment.FLAG_DOWNLOAD_USER_REQUEST)) == 0)
                    && !attachmentExists(context, att)) {
                Log.d(Logging.LOG_TAG, "The attachment lost: " + att.mFileName
                        + ", #" + att.mId);
                return true;
            }
        }
    ...
    public static EmailAsyncTask<Void, Void, Void> updateLastSeenMessageKey(final Context context,
        ...
        return EmailAsyncTask.runAsyncParallel(new Runnable() {
            ...
            @Override
            public void run() {
                EmailAsyncTask.printStartLog("Utility#updateLastSeenMessageKey");
                updateLastSeenMessageKeyForAccount(accountId);
                EmailAsyncTask.printStopLog("Utility#updateLastSeenMessageKey");
            }
        });
    }
    ...
    public static String getContentFileName(Context context, Uri contentUri) {
        String name = null;
        try {
            name = getFirstRowString(context, contentUri, ATTACHMENT_META_NAME_PROJECTION, null, 
                    null, null, ATTACHMENT_META_NAME_COLUMN_DISPLAY_NAME);
        } catch (SQLiteException e) {
            Logging.e("Query FileName from contentUri  " + contentUri + " failed : " + e.toString());
        } catch (RuntimeException re){
            Logging.e("Have no Permission Query FileName from contentUri  " + contentUri
                    + " failed : " + re.toString());
        }
        // When the name or size are not provided, we need to generate them locally.
        if (name == null) {
            if(contentUri.toString().startsWith(CONTACT_URI_PREFIX)) {
                name = VCARD_UNKNOWN;
            } else {
    ...
    public static boolean isAttchmentInlineStrictly(String htmlBody, Attachment attachment) {
        ...
        Pattern patternImg = Pattern.compile(contentIdRe);
        Matcher matcher = patternImg.matcher(htmlBody);
        if (matcher.find()) {
            /** M: if the mimetype isn't image , it may not be a real image (RTF
             * treat all inline attachment as inline image<img src = "cid:id" @{ */
            String imgContentIdRe = AttachmentUtilities.getImgContentIdStr(attachment.mContentId);
            boolean notInlineImgFile = (attachment.mMimeType != null)
                    && !(attachment.mMimeType.toLowerCase().startsWith(
                            "image") || attachment.mMimeType.toLowerCase().startsWith(
                            AttachmentUtilities.GENERAL_MIME_TYPE));
            boolean loadAsImg = Pattern.compile(imgContentIdRe,
                    Pattern.CASE_INSENSITIVE)
                    .matcher(htmlBody)
                    .find();
            if (notInlineImgFile && loadAsImg) {
                return false;
            }
            return true;
            /** @} */
        } else {
            Pattern patternBkg = Pattern.compile(backgroundRe);
            Matcher matcherBkg = patternBkg.matcher(htmlBody);
            if (matcherBkg.find()) {
                return true;
            }
        }
