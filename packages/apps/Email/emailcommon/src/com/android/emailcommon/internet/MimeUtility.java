...
import com.android.emailcommon.utility.ExtendBase64InputStream;
...
public class MimeUtility {
    /**
     * M: A global suggestion to max limitation of body due to android's cursor window size limit(2M)
     * and process OOM risk(1.5M bytes).
     */
    public static final int FETCH_BODY_SIZE_LIMIT = (1 * 1024 * 1024 + 512 * 1024);

    public static final String MIME_TYPE_RFC822 = "message/rfc822";
    /** M: Because of dismissing a condition that Email address contains the sign "\t",
    add "\t" in the Pattern.compile() */
    private final static Pattern PATTERN_CR_OR_LF_OR_HTAB = Pattern.compile("\r|\n|\t");
    /// M: message/rfc822 suffix
    private static final String MESSAGE_RFC822_SUFFIX = " (msg)";
    ...
    public static String unfold(String s) {
        ...
        Matcher patternMatcher = PATTERN_CR_OR_LF_OR_HTAB.matcher(s);
    ...
    public static String getHeaderParameter(String header, String name) {
        ...
        parts = mergeIncorrectlySplit(parts, ";");
        if (name == null) {
            return parts[0].trim();
        }
        ...
        for (String part : parts) {
            if (part.trim().toLowerCase().startsWith(lowerCaseName)) {
                // "2" means spliting the string into two parts by "="
     ...
     /**
     * Merge the string parts which were split incorrectly.
     * Such as 'filename=";io;.mid"; size=13' split with ';',
     * the ";io;.mid" should not be split.
      */
    public static String[] mergeIncorrectlySplit(String[] parts, String splitKey) {
        ...
        for (int i = 0; i < parts.length; i++) {
            String s = parts[i];
            int count = 0;
            // How many '/"' this part contains
            for (int j = 0; j < s.length(); j++) {
                if ('\"'== s.charAt(j)) {
                    count ++;
                }
            }
            // If count is odd, which means that the adjacent two parts are mean to be one part in 
            // a pair of '\"', so this part should not be split, although ';' exists between these 
            // two parts, just merge it with next part
            if (count % 2 == 1) {
                // shouldMerge indicates that the first '\"' has appeared and the following parts 
                // should be merged together until the the second '\"' appears, "i + 1 
                // == parts.length" means that the first '\"' part is the last one part
                if (shouldMerge || i + 1 == parts.length) {
                    currentString.append(splitKey).append(parts[i]);
                    mergedStrings.add(currentString.toString());
                ...
                if (shouldMerge) {
                    ...
                    mergedStrings.add(parts[i]);
                }
            }
        }

        return mergedStrings.toArray(new String[]{});
    }
        ...
        try {
            if (part != null && part.getBody() != null) {
                ...
                if (mimeType != null && MimeUtility.mimeTypeMatches(mimeType, "text/*")) {
                    ...
                    /// M: for POP3, we must fetch whole message content to parse the body's size.
                    /// So we have to limit the body size here.
                    IOUtils.copy(in, out, MimeUtility.FETCH_BODY_SIZE_LIMIT);
                    in.close();
                     ...
                     */
                    if (charset == null) {
                        charset = DecoderUtil.emailCharsetDetect(new ByteArrayInputStream(out.toByteArray()));
                        Log.d(Logging.LOG_TAG, "Body Stream charset is: " + charset);
                        if (charset == null) {
                            charset = "ASCII";
                        }
                    }
    ...
    public static InputStream getInputStreamForContentTransferEncoding(InputStream in,
        ...
        if (contentTransferEncoding != null) {
            ...
            if ("quoted-printable".equalsIgnoreCase(contentTransferEncoding)) {
                ...
                /// M: Use ExtendBase64InputStream to replace Base64InputStream to
                /// decode the extra data behind the standard end flag '='.
                in = new ExtendBase64InputStream(in, Base64.DEFAULT);
            }
        }
        return in;
    }
    ...
    public static Body decodeBody(InputStream in, String contentTransferEncoding)
        ...
        try {
            ...
            Logging.e("decodeBody Base64DataException Exception: " + bde);
        } catch (IOException ioe) {
            Logging.e("decodeBody Exception: " + ioe);
            tempBody.deleteFile();
            throw ioe;
        } finally {
            out.close();
    ...
    public static void collectParts(Part part, ArrayList<Part> viewables,
        ...
        if (part.getBody() instanceof Multipart) {
            ...
            int mCountOfmp = mp.getCount();
            if (mp.getSubTypeForTest().equals("alternative")) {
                for (int i = 0; i < mCountOfmp; i++) {
            ...
            viewables.add(part);
                ...
                /** M: rtf822 message as attachment to display @{ */
                || part.isMimeType(MimeUtility.MIME_TYPE_RFC822)) {
            /** @} */
            // Finally, if it's an attachment or an inline or rtf822 message we
            // will include it as an attachment.
            attachments.add(part);
        }
    }

    /** M: Add (msg) behind all of message/rfc822 attachment to avoid confuse  @{ */
    public static String generateAttName(String name, String mimeType) {
        String attName = name;
        if (MimeUtility.mimeTypeMatches(mimeType, MimeUtility.MIME_TYPE_RFC822)) {
            attName += MESSAGE_RFC822_SUFFIX;
        }
        return attName;
    }
    /** @} */
