...
public abstract class EmailContent {
    ...
    /// M: The exchange process name
    private static String EXCHANGE_PROCESS_NAME = "com.android.exchange";
    ...
    public static <T extends EmailContent> T restoreContentWithId(Context context,
        ...
        try {
            ...
            /** M: The provider may be has died, just wait for 1s and try again... @{ */
            if (c == null) {
                Logging.d("restoreContentWithId ruturn null,  wait for 1s and try again.");
                try {
                    Thread.sleep(1000);
                } catch (InterruptedException ex) {
                    Logging.w("restoreContentWithId sleep 1s was Interrupted!");
                }
                c = context.getContentResolver().query(u, contentProjection, null, null, null);
                if (c == null) {
                    Logging.w("!!!Warning: restoreContentWithId ruturn null again. Kill self if needed.",
                            new ProviderUnavailableException());
                    killSelfIfRunningInExchangeProcess(context);
                    return null;
                }
            }
            /** @} */
            ...
            Log.w(Logging.LOG_TAG, "EmailContent#restoreContentWithId throw out IllegalStateException", e);
        } catch (SQLiteException e) {
            Log.w(Logging.LOG_TAG, "EmailContent#restoreContentWithId throw out SQLiteException", e);
        } catch (IllegalMonitorStateException e) {
            Log.w(Logging.LOG_TAG, "EmailContent#restoreContentWithId throw out IllegalMonitorStateException", e);
        } finally {
    ...
    /**
     * M: Kill the process if it was Exchange process.
     * This method may be used to stop Exchange process while the Email provider is unavailable.
     * @param context the context
     */
    private static void killSelfIfRunningInExchangeProcess(Context context) {
        ActivityManager am = (ActivityManager)context.getSystemService(Context.ACTIVITY_SERVICE);
        int currendPid = android.os.Process.myPid();
        String processName = null;
        List<ActivityManager.RunningAppProcessInfo> processes = am.getRunningAppProcesses();
        for (ActivityManager.RunningAppProcessInfo process : processes) {
            if (process.pid == currendPid) {
                processName = process.processName;
                break;
            }
        }
        if (EXCHANGE_PROCESS_NAME.equals(processName)) {
            Logging.d("It is running in exchange process, kill Self...");
    ...
    public static final class Body extends EmailContent implements BodyColumns {
        ...
        public static final Uri CONTENT_LARGE_URI = Uri.parse(EmailContent.CONTENT_URI + "/bodyLarge");
    ...
    public interface MessageColumns {
        ...
        // Indicate whether the message is a dirty one (used by bad sync key recovery)
    ...
    public static final class Message extends EmailContent implements SyncColumns, MessageColumns {
        ...
        public static final Uri SYNCED_CONTENT_NOTIFIER_URI =
            Uri.parse(EmailContent.CONTENT_NOTIFIER_URI + "/message/synced");
        ...
        public static final int CONTENT_SIZE_COLUMN = 24;
        public static final int CONTENT_DIRTY_COLUMN = 25;
        ...
        public static final String[] CONTENT_PROJECTION = new String[] {
            ...
            MessageColumns.THREAD_TOPIC, MessageColumns.SIZE,
        ...
        public static final String FLAG_LOADED_SELECTION =
            ...
            +     Message.FLAG_LOADED_ENVELOPE + ")";

        /// M: The all VIPs selection, used for VIP folder
        ...
        /** M: Selection to retrieve all messages in "non-(out/sent/draft)box" for any account @{*/
        public static final String ALL_NON_OUTBOX_DRAFT_SENT_SELECTION = "(("
                + MessageColumns.MAILBOX_KEY + " NOT IN ("
                + "SELECT " + MailboxColumns.ID + " FROM " + Mailbox.TABLE_NAME
                + " WHERE " + MailboxColumns.TYPE + " IN (" + Mailbox.TYPE_OUTBOX + ", "
                + Mailbox.TYPE_DRAFTS +", " + Mailbox.TYPE_SENT + "))) AND ("
        ...
        public static final int FLAG_LOADED_ENVELOPE = 4;
        ...
        /// M: message body's size was too large, and was truncated to avoid OOM.
        public static final int FLAG_BODY_TOO_LARGE = 1 << 22;
        /** a pseudo ID for "no message". */
        public static final long NO_MESSAGE = -1L;

        /** M: flag to indicate whether a new local message search is initiated */
        ...
        public ContentValues toContentValues() {
            ...
            values.put(MessageColumns.SIZE, mSize);
        ...
        public Uri save(Context context) {
            ...
            if (mText == null && mHtml == null && mTextReply == null && mHtmlReply == null &&
                ...
                if (doSave) {
                    ...
                    logMessageReceiving(context, new Message[]{this});
                    return uri;
                } else {
        ...
        public void addSaveOps(ArrayList<ContentProviderOperation> ops) {
            ...
            if (mText != null) {
                ...
                cv.put(Body.TEXT_CONTENT, Utility.HtmlToText(mHtml));
            }
            if (mHtml != null) {
         ...
         * @param messages
         */
        public static void logMessageReceiving(Context context,Message[] messages){
            if(Logging.DEBUG) {
                for(Message message : messages){
                    Uri uri = ContentUris.withAppendedId(Mailbox.CONTENT_URI, message.mMailboxKey);
                    Cursor c = context.getContentResolver().query(uri, new String[]{MailboxColumns.TYPE},
                            null, null, null);
                    try {
                        if (c.moveToFirst()) {
                            int mType=c.getInt(0);
                            if(mType == Mailbox.TYPE_INBOX){
                                Logging.d(Logging.EmailReceive_TAG, (new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")).format(new Date())
                                        + " Message received. " + "messageId: "+ message.mId + ", subject: " + message.mSubject
                                        + ", From: " + message.mFrom + ", To: " + message.mTo+", Cc: " + message.mCc
                                        + ", Bcc: " + message.mBcc);
                            }
                        }
                    } finally {
        ...
        public static String buildMessageListSelection(
            ...
            /** M: Support VIP folder. Query all messages of the account,
             *  and then filter the VIP messages @{*/
            if (mailboxId == Mailbox.QUERY_ALL_VIPS) {
                if (accountId == Account.ACCOUNT_ID_COMBINED_VIEW) {
                    return Message.ALL_VIP_SELECTION;
                }
                final StringBuilder selection = new StringBuilder();
                selection.append(MessageColumns.ACCOUNT_KEY).append('=').append(accountId)
                        .append(" AND ")
                        .append(Message.ALL_VIP_SELECTION);
                return selection.toString();
            }
            /** @} */
        ...
        /**
         * M: Build the SQL selection clause for searching "subject", "sender" or "receiver"
        ...
        private static String buildSelectionClause(String[] queryTerms, String queryField) {
            ...
            if (!TextUtils.isEmpty(queryField)) {  // subject, sender
                ...
                for (int i = 0; i < queryTerms.length; i++) {
                    ...
                    .append(queryTerms[i].toLowerCase()).append("%\' ESCAPE \'\\'").append(" OR ").append("LOWER(")
                    .append(MessageColumns.CC_LIST).append(") like \'%").append(queryTerms[i].toLowerCase())
                    .append("%\' ESCAPE \'\\') AND ");
                }
            }
            ...
            if (queryField.contains(SearchParams.SEARCH_FIELD_BODY) || isAll) {
                ...
                try {
                    c = context.getContentResolver().query(Message.CONTENT_URI, 
                            new String[]{MessageColumns.ID}, messageListSelection, null, null);
                        messageIds = new String[c.getCount()];
                        int i = 0;
                        while (c.moveToNext()) {
                            messageIds[i++] = c.getString(0);
                        }
                }catch (Exception e) {
                    Logging.d("buildLocalSearchSelection query exception: " + e);
                } finally {
                    if (c != null) {
                        c.close();
                    }
                ...
                try {
                    c = context.getContentResolver().query(Body.CONTENT_URI, new String[]{
                            BodyColumns.MESSAGE_KEY, BodyColumns.TEXT_CONTENT}, bodySelection.toString()
                                , null, null);
                    ...
                    while (c.moveToNext()) {
                        ...
                        if (!TextUtils.isEmpty(messageContent = c.getString(1))) {
                            flag = true;
                            for (int i = 0; i < queryTermsForBody.length; i++) {
                                if (!Pattern.compile(queryTermsForBody[i],Pattern.CASE_INSENSITIVE).matcher(
                                        messageContent).find()) {
                                    flag = false;
                                    break;
                                }
                            ...
                            if (flag) {
                                resultMessageIds.add(c.getString(0));
                            }
                        } 
                    }
                } catch (PatternSyntaxException e) {
                    // Do nothing for the intermediate regex compile error
                } finally {
                    if (c != null) {
                        c.close();
                    }
                ...
                first = true;
                selection.append(MessageColumns.ID + " IN (");
    ...
    public static final class Attachment extends EmailContent
         ...
         * M: For clear cache @{
         */
        public static final String[] ID_CONTENTURI_ACCOUNT_PROJECTION =
                new String[] {RECORD_ID,
                        AttachmentColumns.CONTENT_URI,
                        AttachmentColumns.ACCOUNT_KEY};
        public static final int ID_CONTENTURI_ACCOUNT_PROJECTION_COLUMN_CONTENTURI = 1;
        public static final int ID_CONTENTURI_ACCOUNT_PROJECTION_COLUMN_ACCOUNT = 2;
        /** @} */
    ...
    public interface AccountColumns {
        ...
        /// M: VIP Ringtone
        public static final String VIP_RINGTONE_URI = "vipRingtoneUri";
    }
