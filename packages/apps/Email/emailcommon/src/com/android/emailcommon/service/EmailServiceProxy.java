...
public class EmailServiceProxy extends ServiceProxy implements IEmailService {
     ...
     */
    public void cancelLoadAttachment(final long attachmentId) throws RemoteException {
        setTask(new ProxyTask() {
            public void run() throws RemoteException {
                try {
                    mService.cancelLoadAttachment(attachmentId);
                } catch (RemoteException e) {
                }
            }
        }, "cancelLoadAttachment");
    }
    ...
    /**
     * Remove the global callback object used by the service
     *
     * @param cb a callback object through which all service callbacks are executed
     */
    ...
    /**
     * Request the sync of a partial downloaded message; the service MUST send the syncMailboxStatus callback
     * indicating "starting" and "finished" (or error), regardless of whether the mailbox is
     * actually syncable.
     *
     * @param messageId the id of the mailbox record
     * @param userRequest whether or not the user specifically asked for the sync
     */
    public void fetchMessage(final long messageId) throws RemoteException {
        setTask(new ProxyTask() {
            public void run() throws RemoteException {
                if (mCallback != null) mService.setCallback(mCallback);
                mService.fetchMessage(messageId);
            }
        }, "fetchMessage");
    }
    ...
    */
    ...
    /**
     * Support BT-MAP to send message, use EmailExternalCalls to return the send result.
     */
    public void sendMessageForBT(final long accountId, final long mailboxId, final Uri uri,
            final EmailExternalCalls callback, final boolean saveToSent) throws RemoteException {
        setTask(new ProxyTask() {
            public void run() throws RemoteException{
                if (mCallback != null) mService.setCallback(mCallback);
                mService.sendMessageForBT(accountId, mailboxId, uri, callback, saveToSent);
            }
        }, "sendMessageForBT");
    }

    /**
     * Support BT-MAP to sync MailBox, use EmailExternalCalls to return the send result.
     */
    public void startSyncForBT(final long mailboxId, final EmailExternalCalls callback) throws RemoteException {
        setTask(new ProxyTask() {
            public void run() throws RemoteException{
                if (mCallback != null) mService.setCallback(mCallback);
                mService.startSyncForBT(mailboxId, callback);
            }
        }, "startSyncForBT");
    }
     ...
     */
    public OofParams syncOof(final long accountId, final OofParams oofParams,
            final boolean isGet) throws RemoteException {
        if (Configuration.mIsRunTestcase) {
            mService = sCallback.getMock();
            mReturn = mService.syncOof(accountId, oofParams, isGet);
        } else {
            setTask(new ProxyTask() {
                @Override
                public void run() throws RemoteException{
        ...
        OofParams op = (OofParams)mReturn;
        return op;
    }
     ...
     */
    public void stopOof(final long accountId) throws RemoteException {
        setTask(new ProxyTask() {
            @Override
            public void run() throws RemoteException{
                if (mCallback != null) mService.setCallback(mCallback);
                mService.stopOof(accountId);
            }
        }, "stopOof");
    }
