...
interface IEmailService {
    ...
    oneway void cancelLoadAttachment(long attachmentId);
    ...
    oneway void sendMessageForBT(long accountId, long mailboxId, in Uri uri,
                in EmailExternalCalls callback, boolean saveToSent);
                
    oneway void startSyncForBT(long mailboxId, EmailExternalCalls callback);
    ...
    int getApiLevel();
    oneway void fetchMessage(long messageId);
    ...
    OofParams syncOof(long accountId, in OofParams oofParams, boolean isGet);

    void stopOof(long accountId);
}
