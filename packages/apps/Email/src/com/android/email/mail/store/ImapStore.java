...
public class ImapStore extends Store {
    ...
    private ImapFolder addMailbox(Context context, long accountId, String mailboxPath,
        ...
        } else if ((mailbox = Mailbox.restoreMailboxOfType(context, accountId, mailboxType))
                    == null) {
            mailbox = new Mailbox();
        }
    ...
    public Folder[] updateFolders() throws MessagingException {
        ...
        try {
            ...
            for (ImapResponse response : responses) {
                ...
                if (response.isDataResponse(0, xlistSupported ? ImapConstants.XLIST
                        : ImapConstants.LIST)) {
                    // Get folder name.
                    ...
                    if (!xlistSupported && ImapConstants.INBOX.equalsIgnoreCase(folderName)) {
                        continue;
                    }
                    ...
                    if (xlistSupported) {
                        int mailboxType = getMailboxTypeFromAttributes(response.getListOrEmpty(1)
                                .toString());
                        if (mailboxType == Mailbox.TYPE_INBOX) {
                            continue;
                        }
                        folder = addMailbox(mContext, mAccount.mId, folderName, delimiterChar,
                                selectable, mailboxType);
                    } else {
                        folder = addMailbox(mContext, mAccount.mId, folderName, delimiterChar,
                                selectable, Mailbox.TYPE_NONE);
                    }
            ...
            Folder newFolder = addMailbox(mContext, mAccount.mId, ImapConstants.INBOX, '\0',
                    true /* selectable */, Mailbox.TYPE_NONE);
