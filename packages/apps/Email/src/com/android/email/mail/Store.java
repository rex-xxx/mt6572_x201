...
public abstract class Store {
    ...
    public static final HashMap<String, Class<? extends Store>> sStoreClasses =
    ...
    /**
     * M: Get an instance of a mail store for the given account. The account must be valid (i.e. has
    ...
    public synchronized static Store getInstance(Account account, Context context)
        ...
        return getInstance(account, context, true);
    }
     ...
     *
     * @param account The account of the store.
     * @param useCache is use the cache store if existed
     * @return an initialized store of the appropriate class
     * @throws MessagingException If the store cannot be obtained or if the account is invalid.
     */
    public synchronized static Store getInstance(Account account, Context context, boolean useCache)
            throws MessagingException {
        HostAuth hostAuth = account.getOrCreateHostAuthRecv(context);
        // An existing account might have been deleted
        if (hostAuth == null) {
            Logging.w(Logging.LOG_TAG, "Instantiate Store for " + account.mDisplayName
                    + " failed due to NULL hostAuth");
            throw new MessagingException("Instantiate Store for " + account.mDisplayName
                    + " failed due to NULL hostAuth");
        }
        Store store = null;
        if (useCache) {
            store = sStores.get(hostAuth);
        }
        if (store == null) {
            ...
            if (useCache && hostAuth.mId != EmailContent.NOT_SAVED) {
    ...
    protected static void updateMailbox(Mailbox mailbox, long accountId, String mailboxPath,
        ...
        mailbox.mType = type;
        //box.mUnreadCount;
        // M: reset mailbox.mVisibleLimit only when it's invalid
