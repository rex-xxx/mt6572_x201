...
public class ImapFolder extends Folder {
    ...
    /** 
     * The ratio of original data size to encoded data size, 0.73 is an  
     * empirical value based on practice and the algorithm of base64 encoding
     */
    private static final double BASE64_RATIO = 0.73;

    private static int sFetchSize = 0;
    ...
    @VisibleForTesting
    public String[] searchForUids(String searchCriteria) throws MessagingException {
        checkOpen();
        try {
            try {
    ...
    public String[] searchMimeFieldsForUids(String searchCriteria) throws MessagingException {
        checkOpen();
        try {
            try {
                String command = ImapConstants.UID_SEARCH + " " + searchCriteria;
    ...
    public Message[] getMessages(SearchParams params, MessageRetrievalListener listener)
        ...
        /**
         * M:Search remote message by filter and field.
         * ALL = TO+FROM+SUBJECT+BODY
         * @{                             
         */
        if(field.equalsIgnoreCase(SearchParams.SEARCH_FIELD_ALL)){
            // Break the command up into pieces ending with the string literal length
            commands.add(ImapConstants.UID_SEARCH + " CHARSET " + charset + " OR FROM "
                    + octetLength);
            ...
            commands.add(ImapConstants.UID_SEARCH + " CHARSET " + charset + " "
                    + field.toUpperCase() +" " + octetLength);
            commands.add(filter);
        }
        checkOpen();
    ...
    public Message[] getMessages(int start, int end, MessageRetrievalListener listener)
        ...
        return getMessagesInternal(
                /// M: add US as locale to avoid send bad command to server.
    ...
    /**
     * Supports FetchProfile.Item.BODY and FetchProfile.Item.STRUCTURE with EmailSize
     */
    @Override
        ...
        if (fetchPart != null) {
            ...
            if (partIds != null) {
                ...
                fetchFields.add(ImapConstants.FETCH_FIELD_BODY_PEEK_BARE
                        + "[" + partIds[0] + "]" + (sFetchSize > 0 ? String.format(Locale.US, "<0.%d>", sFetchSize) : ""));
            }
        }
            ...
            do {
                ...
                try {
                    ...
                    if (fp.contains(FetchProfile.Item.ENVELOPE)) {
                        ...
                        final InputStream header = fetchList.getKeyedStringOrEmpty(
                    ...
                    if (fetchPart != null && fetchPart.getSize() > 0) {
                        /// M: change for redecoding when catch Base64DataException
                        ImapString imapString = fetchList.getKeyedStringOrEmpty("BODY[", true);
                        String contentType = fetchPart.getContentType();
                        String[] encodingHeader = fetchPart.getHeader(
                        ...
                        fetchPart.setBody(decodeBody(imapString, contentTransferEncoding, listener));
                    }
     ...
     * @param ImapString M:use for redecoding when catch Base64DataException
     */
    private Body decodeBody(ImapString imapString,
            String contentTransferEncoding, MessageRetrievalListener listener)
            throws IOException {
        // Get a properly wrapped input stream
        InputStream in = imapString.getAsStream();
        /**
         * M: Converts "\r" bytes not followed by "\n" and "\n" not preceded by "\r" to "\r\n".
         * Special for the case which QP encode data have only "\n" but no "\r"
         * If this is a FileInputStream, compose a BufferedInputStream
        ...
        try {
            ...
            /**
             * M: This attachment have decoded finished. Send finished callback,
             * to ask UI to update progress status.@{
             */
            if (listener != null) {
                Logging.d("attachment copy finished, update loadAttachmentProgress 100");
                listener.loadAttachmentProgress(100);
            }
            /**@}*/
        } catch (Base64DataException bde) {
            String warning = "\n\n" + Email.getMessageDecodeErrorString();
            out.write(warning.getBytes());
            /// M: redecoding when catch Base64DataException
            return decodeBody(imapString, null, listener);
        } finally {
            out.close();
        }
        return tempBody;
    ...
    /**
     * M: Support to parse attachment or embedded message in message/rfc822
    ...
    private static void parseBodyStructure(ImapList bs, Part part, String id)
        ...
        if (bs.getElementOrNone(0).isList()) {
            ...
            /** M: get start to support message/rfc822   @{ */
...
//                        + " not yet supported.");
                Logging.w("BODYSTRUCTURE " + MimeUtility.MIME_TYPE_RFC822
                        + " now supported but still not so complete");
                isRfc822Msg = true;
                //return;
            }
            /** @} */
            ...
            /// M: bodyDisposition may not included in message/rfc822 part
            // Extension items
            ...
            /// M: bodyDisposition may be null in message/rfc822 part, just for safe
            if (bodyDisposition != null && bodyDisposition.size() > 0) {
                ...
                /** M: Parse and set required content-disposition for message/rfc822 part  @{ */
            ...
            if ((size > 0)
                    && (MimeUtility.getHeaderParameter(contentDisposition.toString(), "size")
                            == null)) {
                // If the encoding is base64 and no Content-Disposition size fetched, then 
                // recounting the actual size: the encoded size multiplied by a fixed ratio
