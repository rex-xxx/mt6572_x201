...
public class ImapResponseParser {
    ...
    private MessageRetrievalListener mListener;
    ...
    public ImapResponse readResponse() throws IOException, MessagingException {
        ...
        try {
            ...
            /** M: optimize code @{ */
            synchronized (MailService.SYNCHRONIZE_LOCK) {
                WakeLock wakeLock = MailService.getWakeLock();
                if (wakeLock != null) {
                    if (!wakeLock.isHeld()) {
                        wakeLock.acquire();
    ...
    /** M:in case of that server may use GBK or other encoding instead of unicode. @{*/
    private byte[] charStr2bytes(String str) {
        int size = str.length();
    ...
    String readBytesTilEol() throws IOException {
        String result = null;
        ByteArrayInputStream byteStream = null;
        String encoding = null;
        boolean isDefault = true;
        String charStr = readUntilEol();// ISO-8859-1
        byte[] strBytes = charStr2bytes(charStr);
        // M: improve the accurate of the ICU
        String regStr = "[^\\p{ASCII}]+";
        Matcher match = Pattern.compile(regStr, Pattern.CASE_INSENSITIVE)
                .matcher(charStr);
        ByteArrayOutputStream byteOutput = new ByteArrayOutputStream();
        try {
            while (match.find()) {
                byte[] matchBytes;
                matchBytes = charStr2bytes(match.group());
                if (matchBytes.length > 0) {
                    byteOutput.write(matchBytes);
                }
            }
            byte[] checkBytes = byteOutput.toByteArray();
            if (checkBytes.length != 0) {
                byteStream = new ByteArrayInputStream(checkBytes);
                encoding = DecoderUtil.emailCharsetDetect(byteStream);
                if (encoding != null) {
            ...
            if (Email.DEBUG) {
                Log.d(Logging.LOG_TAG, "readBytesTilEol#Error " + e.getMessage());
            }
        }
        if (isDefault) {
            result = charStr;// using ISO-8859-1 Encoding
        }
        return result;
    }
    /** }@*/
            ...
            if (ch == '+') { // Continuation request
                ...
                if (peek() == ' ') {
                    ...
                    if (responseToDestroy.isStatusResponse()) { // It's a status response
                        ...
                        /**M: server may use other encoding instead of unicode @{*/
                        String rest = null;
                        if (responseToDestroy.getResponseCodeOrEmpty().is(ImapConstants.ALERT)) {
                            rest = readBytesTilEol();
                        } else {
                            rest = readUntilEol();
                        }
                        /**}@*/
    ...
    private ImapString parseLiteral() throws IOException, MessagingException {
        ...
        if (size > mLiteralKeepInMemoryThreshold) {
            return new ImapTempFileLiteral(in, mListener);
        } else {
            return new ImapMemoryLiteral(in, mListener);
        }
    }

    public void setUpdateUICallback(MessageRetrievalListener listener){
        mListener = listener;
    }

    public void removeUpdateUICallback(){
        mListener = null;
    }
}
