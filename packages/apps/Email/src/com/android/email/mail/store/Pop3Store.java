...
//    private int mThroughputKbS = Integer.MAX_VALUE;
    ...
    private Pop3Store(Context context, Account account) throws MessagingException {
        ...
        int defaultPort = Configuration.POP3_DEFAULT_PORT;
        ...
        if ((recvAuth.mFlags & HostAuth.FLAG_SSL) != 0) {
            ...
            defaultPort = Configuration.POP3_DEFAULT_SSL_PORT;
    ...
    public class Pop3Folder extends Folder {
        ...
        /// M: fetch body size limitation.
        private int mFetchSize = -1;
            ...
            for (int i = 0, count = messages.length; i < count; i++) {
                ...
                try {
                    if (fp.contains(FetchProfile.Item.BODY)) {
                        /// M : limit the size of body fetch.
                        if (mFetchSize > 0) {
                            fetchBody(pop3Message, mFetchSize / 76);
                        } else {
        ...
        private void fetchEnvelope(Message[] messages,
            ...
            if (unsizedMessages < 50 && mMessageCount > 5000) {
                ...
                for (int i = 0, count = messages.length; i < count; i++) {
                    ...
                    String response = executeSimpleCommand(String.format(Locale.US, "LIST %d",
        ...
        private void fetchBody(Pop3Message message, int lines)
            ...
            if (lines == -1) {
                ...
                try {
                    /// M: add US as locale to avoid send bad command to server.
                    response = executeSimpleCommand(String.format(Locale.US, "TOP %d %d",
                            messageId, lines));
                } catch (MessagingException me) {
                    /// M: add US as locale to avoid send bad command to server.
                    response = executeSimpleCommand(String.format(Locale.US, "RETR %d", messageId));
        ...
        public void setFlags(Message[] messages, Flag[] flags, boolean value)
            ...
            try {
                for (Message message : messages) {
                    /// M: add US as locale to avoid send bad command to server.
                    executeSimpleCommand(String.format(Locale.US, "DELE %s",
...
//        }
        ...
        private String executeSensitiveCommand(String command, String sensitiveReplacement)
            ...
            try {
                ...
                Log.e(Logging.LOG_TAG, "==writeLine==>["+command+"],exception"+e.getMessage());
                throw e;
            }
            try {
                String response = mTransport.readLine();
                if (response.length() > 1 && response.charAt(0) == '-') {
                    Log.w(Logging.LOG_TAG, "Email send command [" + command
                            + "] receive Server response [" + response + "]");
                    throw new MessagingException(MessagingException.UNSPECIFIED_EXCEPTION,response);
                } else if ( null == response ){
                    Log.w(Logging.LOG_TAG, "Email send command [" + command
                            + "] receive Server response is null");
                } else if (null != response && 0 == response.length()){
                    Log.w(Logging.LOG_TAG, "Email send command [" + command
                            + "] receive Server response is 0 byte");
                    throw new MessagingException(MessagingException.UNSPECIFIED_EXCEPTION,
                            response);
                }
                return response;
            } catch(IOException e) {
                Log.e(Logging.LOG_TAG, "==readLine==>["+command+"],exception"+e.getMessage());
                throw e;
            }
        }
    ...
    class Pop3ResponseInputStream extends InputStream {
        ...
        public int read() throws IOException {
            ...
            int d = mIn.read();
            if (d == -1 && !mFinished) {
                Logging.e("Response ended prematurely. -1 received but not finished.");
                throw new IOException("Response ended prematurely. -1 received but not finished.");
            }
            if (mStartOfLine && d == '.') {
                d = mIn.read();
                if (d == '\r') {
