...
public class FolderProperties {
    ...
    private final Drawable mVipMailboxDrawable;
    ...
    private FolderProperties(Context context) {
        ...
        mVipMailboxDrawable =
            context.getResources().getDrawable(R.drawable.ic_folder_vip_holo_light);
    }
    ...
    /**
     * @return message count to show for the UI for a account's inbox.
     *
     * Note this method doesn't use mContext so we can inject a mock context for provider
     * access.  So it's static.
     */
    public static int getMessageCountForCombinedMailbox(Context context, long accountId,
            long mailboxId) {
    ...
    public Drawable getIcon(int type, long mailboxId, int mailboxFlags) {
        ...
        } else if (mailboxId == Mailbox.QUERY_ALL_VIPS) {
            return mVipMailboxDrawable;
        }
