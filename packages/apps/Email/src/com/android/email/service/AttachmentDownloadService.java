...
public class AttachmentDownloadService extends Service implements Runnable {
    ...
    /// M: The network read timeout is 60s, so, we should wait for at least 60s.
    private static final int CALLBACK_TIMEOUT = 60 * ((int)DateUtils.SECOND_IN_MILLIS);
    ...
    private final Object mConnectivityManagerLock = new Object();
    ...
    public static class DownloadRequest {
        ...
        private DownloadRequest(Context context, Attachment attachment, boolean auto) {
    ...
    /*package*/ class DownloadSet extends TreeSet<DownloadRequest> {
        ...
        public synchronized void onChange(Context context, Attachment att) {
            ...
            if (priority == PRIORITY_NONE) {
                if (Email.DEBUG) {
                    Log.d(TAG, "== Attachment changed: " + att.mId + " of message " + 
                            att.mMessageKey);
                }
                ...
                if (req != null) {
                    ...
                    if (Email.DEBUG) {
                        Log.d(TAG, "== Attachment " + att.mId  + " of message " + 
                                att.mMessageKey + " was in queue, removing");
                    }
                    remove(req);
                    if (mDownloadsInProgress.containsKey(req.attachmentId)) {
                        cancelDownload(req);
                    }
                }
            } else {
                ...
                if (req == null) {
                    req = new DownloadRequest(context, att, false);
                    add(req);
                }
                // If the request already existed, we'll update the priority (so that the time is
                // up-to-date); otherwise, we create a new request
                if (Email.DEBUG) {
                    Log.d(TAG, "== Download queued for attachment " + att.mId + ", message " + 
                            att.mMessageKey + ", priority " + req.priority + ", time " + req.time);
                }
            }
                        ...
                        if (account == null) {
                            ...
                            if (info.isEligibleForDownload()) {
                                ...
                                if (att != null) {
                                    ...
                                    DownloadRequest req = new DownloadRequest(mContext, att, true);
                                    mDownloadSet.tryStartDownload(req);
                                    break;
                                }
                            }
        ...
        private void cancelWatchdogAlarm() {
            Logging.d("cancelWatchdogAlarm in AttachmentDownloadService");
        ...
        private void onWatchdogAlarm() {
            Logging.d("onWatchdogAlarm in AttachmentDownloadService with mStop: " + mStop);
        ...
        /*package*/ synchronized boolean tryStartDownload(DownloadRequest req) {
            ...
            try {
                Logging.d(TAG, ">> Starting download for attachment #" + req.attachmentId
        ...
        private void startDownload(Intent intent, DownloadRequest req)
            ...
            Logging.d(TAG, "startDownload EmailServiceProxy callback " +mServiceCallback);
            ...
            Logging.d("startDownload in AttachmentDownloadService and Setup repeat Watchdog alarm");
            // Set the alarm
        ...
        private void cancelDownload(DownloadRequest req) {
            ...
            try {
                new EmailServiceProxy(mContext, intent, null)
                        .cancelLoadAttachment(req.attachmentId);
            } catch (RemoteException e) {
                Logging.d("RemoteException happend in AttachmentDownloadService cancelDownload");
                return;
            }
        ...
        /*package*/ synchronized void endDownload(long attachmentId, int statusCode) {
            ...
            if (statusCode == EmailServiceStatus.CONNECTION_ERROR) {
                ...
                if (req != null) {
                    req.inProgress = false;
                    /** M: For UI request download, try 5 times for the connection error @{ */
                    Integer tryCount;
                    tryCount = mAttachmentFailureMap.get(attachmentId);
                    if (tryCount != null && tryCount > MAX_DOWNLOAD_RETRIES) {
                        Logging.d(TAG, "attachment " + attachmentId
                                + " download failed due to connection error");
                        mAttachmentFailureMap.remove(attachmentId);
                        // Reset the download flag, then onChange() will be invoked
                        // and the attachment will be removed from the download set
                        // but here we can remove it in advance
                        remove(req);
                        int flags =
            ...
            if (Email.DEBUG) {
                ...
                String status = (statusCode == EmailServiceStatus.SUCCESS) ? "Success" :
                    "Error " + statusCode;
                Logging.d(TAG, "<< Download finished for attachment #" + attachmentId + "; " + secs
                        + " seconds from request, status: " + status);
            }

            if (statusCode == EmailServiceStatus.MESSAGE_NOT_FOUND) {
                Log.d(TAG, "remove all attachment download requests of this "
                        + "message duo to message not found");
                //remove all attachment download requests of this message
                Iterator<DownloadRequest> iterator = iterator();
                while(iterator.hasNext()) {
                    DownloadRequest it = iterator.next();
                    if (it.messageId == req.messageId) {
                        mAttachmentFailureMap.remove(it.attachmentId);
                        iterator.remove();
                    }
                }
                mAttachmentFailureMap.remove(attachmentId);
            ...
            if (attachment != null) {
                ...
                if (statusCode == EmailServiceStatus.MESSAGE_NOT_FOUND) {
                    ...
                    int flags =
                        Attachment.FLAG_DOWNLOAD_FORWARD | Attachment.FLAG_DOWNLOAD_USER_REQUEST;
                    if(statusCode == EmailServiceStatus.SUCCESS && 
                            attachment.mFlags == (attachment.mFlags &= ~flags)){
                        //The attachment flags has been the expected value, not update it again.
                        Logging.d(TAG, "endDownload not changed attachment flag to "
                                + attachment.mFlags + "attachment Id:" + attachmentId);
                        return;
                    }
                    ContentValues cv = new ContentValues();
                    cv.put(Attachment.FLAGS, attachment.mFlags &= ~flags);
                    Logging.d(TAG, "endDownload update attachment mFlags : "
                            + attachment.mFlags + " attachment Id:" + attachmentId 
    ...
    /**
     * We use an EmailServiceCallback to keep track of the progress of
     * downloads. These callbacks come from either Controller (IMAP) or
     * ExchangeService (EAS). Note that we only implement the single callback
     * that's defined by the EmailServiceCallback interface.
     */
    private class ServiceCallback extends IEmailServiceCallback.Stub {
        ...
        public void loadAttachmentStatus(long messageId, long attachmentId, int statusCode,
            ...
            if (req != null) {
                ...
                Logging.d(TAG, "loadAttachmentStatus Attachment " + attachmentId + ": " + progress
                        + "%" + " statusCode " + statusCode);
                switch (statusCode) {
                ...
                /*
                 * By default it will continually handle the call back, even the
                 * DownloadRequest can't be found. It will cause the
                 * mDownloadSet receive some duplicate endDownload call back. We
                 * change this unreasonable feature.
                 */
                Logging.d(TAG, "loadAttachmentStatus req for " + attachmentId
                        + " can not be found, ignored it");
            }
        }
    ...
    /**
     * Ask the service whether a particular attachment is in auto-downloading
     * @param attachmentId the id of the Attachment (as stored by EmailProvider)
     * @return whether or not the attachment is in auto-downloading
     */
    public static boolean isAutoDownload(long attachmentId) {
        AttachmentDownloadService service = sRunningService;
        if (service != null) {
            DownloadRequest req = service.mDownloadSet.mDownloadsInProgress.get(attachmentId);
            if (req != null) {
                return req.isAutoDownload;
            }
        }
        return false;
    }
    ...
    public static void watchdogAlarm() {
        Logging.d("watchdogAlarm in AttachmentDownloadService with sRunningService: "
                + sRunningService);
        AttachmentDownloadService service = sRunningService;
        if (service != null) {
    ...
    public static void attachmentChanged(final Context context, final long id, final int flags) {
        Logging.d(TAG, ">>>>> Utility.runAsync attachmentChanged");
        // because this is called from EmailProvider, so no need to run it in background thread.
        // Or this may lead to AsyncTask reject execution bug.
        ...
        if (attachment != null) {
            ...
            // Testcases may deliver mockContext to this service, then it will throw out 
            // UnsupportedOperationException, so here use try catch to avoid this exception.
            try {
                Intent intent = new Intent(context, AttachmentDownloadService.class);
                intent.putExtra(EXTRA_ATTACHMENT, attachment);
                context.startService(intent);
            } catch (UnsupportedOperationException e) {
                Log.w(TAG, "attachmentChanged throw out UnsupportedOperationException");
            }
        }
    }
    ...
    public void run() {
        ...
        synchronized(mConnectivityManagerLock) {
    ...
    public void onDestroy() {
        ...
        Logging.d("onDesetroy in AttachmentDownloadService");
        mStop = true;
        if (sRunningService != null) {
            kick();
            mDownloadSet.cancelWatchdogAlarm();
    ...
    // Remove serviceCallbacks that has been registered to ControlerService or ExchangeService
    private void removeServiceCallback() {
        for (Intent intent : mAccountServiceMap.values()) {
            if (intent == null) {
                continue;
            }
            try {
                EmailServiceProxy proxy = new EmailServiceProxy(mContext, intent, null);
                proxy.removeCallback(mServiceCallback);
                // Wait 1s to complete remove callback
                proxy.setTimeout(1).waitForCompletion();
                Logging.d("removeServiceCallback: " + mServiceCallback);
            } catch (RemoteException e) {
                Logging.d("RemoteException happend in removeServiceCallback");
                continue;
            }
        }
        mAccountServiceMap.clear();
    }
    ...
    /**
     * M: Just used for get if attachment may download failed due
     * to connection error. Note that the right scenerio to call
     * this method is when IOERROR was detected.
     * @param attachmentId
     * @return true if the attachment is about to fail in downloading
     */
    public static boolean mayDownloadFailed(long attachmentId) {
        AttachmentDownloadService service = sRunningService;
        if (service != null) {
            // MessageViewFragmentBase::loadAttachmentCallback() may be called earlier than
            // AttachmentDownloadSerivce::loadAttachmentStatus(), so we use MAX_DOWNLOAD_RETRIES - 1
            // instead of MAX_DOWNLOAD_RETRIES to make sure MessageViewFragmentBase can update the
            // attachment view UI properly all the time. Note that when calling this method the
            // attachment may had been removed from download set due to connection error
            synchronized(service.mDownloadSet) {
                Integer tryTimes = service.mAttachmentFailureMap.get(attachmentId);
                if ((tryTimes != null && tryTimes >= (MAX_DOWNLOAD_RETRIES - 1))
                        || !AttachmentDownloadService.isAttachmentQueued(attachmentId)) {
                    return true;
                }
            }
        }
