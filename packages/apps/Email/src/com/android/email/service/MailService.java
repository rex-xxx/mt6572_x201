...
public class MailService extends Service {
    ...
    /**
     * M: Actions for auto internal storage control @{
     */
    private static final String ACTION_CLEAR_OLD_ATTACHMENT =
            "com.android.email.intent.action.MAIL_SERVICE_CLEAR_OLD_ATTACHMENT";
    private static final String ACTION_RESCHEDULE_CLEAR_OLD_ATTACHMENT =
            "com.android.email.intent.action.MAIL_SERVICE_RESCHEDULE_CLEAR_OLD_ATTACHMENT";
    private static final String ACTION_CANCEL_CLEAR_OLD_ATTACHMENT =
    ...
    public static final Object SYNCHRONIZE_LOCK = new Object();
    ...
    /**
     * M: cancel clear old attachment files(internal storage)
     * @param context
     */
    public static void actionCancelClearOldAttachment(Context context) {
        Intent i = new Intent();
        i.setClass(context, MailService.class);
        i.setAction(MailService.ACTION_CANCEL_CLEAR_OLD_ATTACHMENT);
        context.startService(i);
    }

    /**
     * M: action: clear old attachment files(internal storage)
     * @param context
     */
    public static void actionClearOldAttachment(Context context) {
        // If clear cache is enabled
        if (Preferences.getPreferences(context).getAutoClearCache()) {
            Intent i = new Intent();
            i.setClass(context, MailService.class);
            i.setAction(MailService.ACTION_CLEAR_OLD_ATTACHMENT);
            context.startService(i);
        } else {
            actionCancelClearOldAttachment(context);
        }
    }

    /**
     * M: Reschedule the action: clear old attachment files(internal storage)
     * @param context
     */
    public static void actionRescheduleClearOldAttachment(Context context) {
        // If clear cache is enabled
        if (Preferences.getPreferences(context).getAutoClearCache()) {
            Intent i = new Intent();
            i.setClass(context, MailService.class);
            i.setAction(MailService.ACTION_RESCHEDULE_CLEAR_OLD_ATTACHMENT);
            context.startService(i);
        } else {
            actionCancelClearOldAttachment(context);
        }
    ...
    /** M: use for testing @{ */
    public static void actionCheckMail(Context context) {
        Intent i = new Intent();
        i.setClass(context, MailService.class);
    ...
    private void acquireWakeLock(String action) {
        synchronized (SYNCHRONIZE_LOCK) {
            if (sWakeLock == null && ACTION_CHECK_MAIL.equals(action)) {
                PowerManager pm = (PowerManager) getSystemService(Context.POWER_SERVICE);
                sWakeLock = pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "CHECK_MAIL_SERVICE");
                sWakeLock.setReferenceCounted(false);
                sWakeLock.acquire();
                Log.d(LOG_TAG, "!!! MailService, will start and acquire WAKE_LOCK");
            }
        }
    }
    ...
    private void releaseWakeLock() {
        synchronized (SYNCHRONIZE_LOCK) {
            if (sWakeLock != null) {
                sWakeLock.release();
                sWakeLock = null;
                Log.d(LOG_TAG, "!!! MailService, will stop and release WAKE_LOCK");
            }
        }
    }
    ...
    public int onStartCommand(final Intent intent, int flags, final int startId) {
        ...
        // Check if current status is low storage, if so, don't start MailService service
        final Preferences pref = Preferences.getPreferences(this);
        if (pref.getLowStorage()) {
            Log.i(LOG_TAG, "!!! MailService, will not start due to low storage");
            stopSelf(startId);
        ...
        EmailAsyncTask.runAsyncParallel(new Runnable() {
            @Override
            public void run() {
                EmailAsyncTask.printStartLog("MailService#onStartCommand");
                reconcilePopImapAccountsSync(MailService.this);
                EmailAsyncTask.printStopLog("MailService#onStartCommand");
            }
        });
        ...
        if (ACTION_CHECK_MAIL.equals(action)) {
            ...
            EmailAsyncTask.runAsyncParallel(new Runnable() {
                ...
                public void run() {
                    ...
                    Account account = Account.restoreAccountWithId(MailService.this, accountId);
                    if (account != null) {
                        android.accounts.Account accountManagerAccount = new android.accounts.
                        Account(account.mEmailAddress, AccountManagerTypes.TYPE_POP_IMAP);
                        if (accountId != -1
        ...
        /**
         * M: For auto internal storage controller, delete some old attachments @{
         */
        } else if (ACTION_RESCHEDULE_CLEAR_OLD_ATTACHMENT.equals(action)) {
            if (Email.DEBUG) {
                Log.d(LOG_TAG, "action: reschedule clear old attachments");
            }
            rescheduleClearOldAttachment(alarmManager);
            stopSelf(startId);
        } else if (ACTION_CLEAR_OLD_ATTACHMENT.equals(action)) {
            // @NOTE: we should set watch dog for clear old attachment in UI thread before we run the
            // asyncTask, otherwise, the periodic clear action won't work if the AsyncTask is killed
            // (due to memory pressure). This is different from the Mail_Check action, which set
            // the watch dog in the asyncTask, cause user can reschedule Mail_Check action by press
            // refresh button on the UI, although the watch dog may not be set successfully in the
            // AsyncTask
            setWatchdogForClearOldAttachment(alarmManager);
            EmailAsyncTask.runAsyncParallel(new Runnable() {
                ...
                public void run() {
                    ...
                    if (Utility.mayLowStorage()) {
                        ...
                        mController.deleteCachedAttachmentsDaysBefore(3);
                        rescheduleClearOldAttachment(alarmManager);
                        stopSelf(startId);
                        // release wake lock for clear cache
                        //releaseWakeLockForClearCache();
                    }
                }
            });
        } else if (ACTION_CANCEL_CLEAR_OLD_ATTACHMENT.equals(action)) {
            cancelClearOldAttachment();
            stopSelf(startId);
        }
        /** @} */
    ...
    /**
     * M: cancel the alarm for clear old attachment
     */
    private void cancelClearOldAttachment() {
        AlarmManager alarmMgr = (AlarmManager)getSystemService(Context.ALARM_SERVICE);
        PendingIntent pi = this.createAlarmIntentForClearOldAttachment(false);
        alarmMgr.cancel(pi);
    }
    ...
    /**
     * M: reschedule action: clear old attachment
     * @param alarmMgr
     */
    private void rescheduleClearOldAttachment(AlarmManager alarmMgr) {
        Log.d(LOG_TAG, "MailService reschedule clear old attachments");
        PendingIntent pi = createAlarmIntentForClearOldAttachment(false);
        long timeNow = SystemClock.elapsedRealtime();
        alarmMgr.set(AlarmManager.ELAPSED_REALTIME_WAKEUP, timeNow + CLEAR_CACHE_PERIOD, pi);
        if (Email.DEBUG) {
            Log.d(LOG_TAG, "reschedule clear old attchment: alarm set at "
    ...
    private void reschedule(AlarmManager alarmMgr) {
        Log.d(LOG_TAG, "MaiService reschedule!!!");
    ...
    * @param accountId the account we were trying to check
    ...
    /**
     * M: Create a watchdog alarm and set it.  This is used in case a clear attachment fails
     * (e.g. we are killed by the system due to memory pressure.)  Normally, a clear attachment
     * will complete and the watchdog will be replaced by the call to
     * {@link #rescheduleClearOldAttachment(AlarmManager)}.
     * @param alarmMgr system alarm manager
     */
    private void setWatchdogForClearOldAttachment(AlarmManager alarmMgr) {
        PendingIntent pi = createAlarmIntentForClearOldAttachment(true);
        long timeNow = SystemClock.elapsedRealtime();
        long nextCheckTime = timeNow + CLEAR_CACHE_PERIOD;
        alarmMgr.set(AlarmManager.ELAPSED_REALTIME_WAKEUP, nextCheckTime, pi);
    }

    /**
     * M: Return a pending intent for starting service to clear old attachments
     * @param isWatchdog
     * @return
     */
    private PendingIntent createAlarmIntentForClearOldAttachment(boolean isWatchdog) {
        Intent i = new Intent();
    ...
    class ControllerResults extends Controller.Result {
        ...
        public void serviceCheckMailCallback(MessagingException result, long accountId,
            ...
            if (result != null || progress == 100) {
                ...
                // Release wakelock and Callback here, because OnDestroy would
                // be trigger by Exchange Notify.
                new Thread() {
                    @Override
                    public void run() {
                        Controller.getInstance(getApplication()).removeResultCallback(
