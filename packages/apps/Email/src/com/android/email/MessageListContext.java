...
public class MessageListContext implements Parcelable {
     ...
     * in the {@link Intent}.
     * Note: This method is only used to support the remote search intent.
     */
    public static MessageListContext forIntent(Context context, Intent intent) {
        ...
        if (EmailActivity.ACTION_REMOTE_SEARCH.equals(intent.getAction())) {
            final String queryTerm = intent.getStringExtra(EmailActivity.EXTRA_QUERY_STRING);
            final String queryField = intent.getStringExtra(EmailActivity.EXTRA_QUERY_FIELD);
            /** M: Always delete the old search mailbox before create a new one @{ */
            final long searchMailboxId =
                    Controller.getInstance(context).getNewSearchMailbox(accountId).mId;
            /** }@ */
            ...
            if(accountId == Account.NO_ACCOUNT) {
                Logging.d("MessageListContext forIntent is failed. Account not found. ");
                return null;
            }
            ...
            return forMailboxFromEmailActivity(accountId, mailboxId);
        }
    }
    ...
    public static MessageListContext forMailboxFromEmailActivity(long accountId, long mailboxId) {
    ...
    public boolean isLocalSearch() {
        return isSearch() && mSearchParams.mIsLocal;
    }
