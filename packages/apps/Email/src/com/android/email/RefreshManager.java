...
public class RefreshManager {
            ...
            if (exception == null && progress == 0) {
                ...
                if (Logging.LOG_PERFORMANCE) {
                    Logging.d(Logging.LOG_TAG,
                            "[Exchange Download Speed] MessageList:updateMailbox:finished ["
    ...
    public boolean checkIsLowStorage(Context context) {
        // Check if low storage at the every beginning
        Preferences pref = Preferences.getPreferences(context);
        pref.checkLowStorage();
        if (pref.getLowStorage()) {
            Toast.makeText(context, R.string.low_storage_service_stop, Toast.LENGTH_SHORT).show();
            Toast.makeText(context, R.string.low_storage_hint_delete_mail, Toast.LENGTH_SHORT)
                    .show();
            Logging.d("checkIsLowStorage canceled refresh due to low storage");
            return true;
        }
        return false;
    }
    ...
    private class ControllerResult extends Controller.Result {
        ...
        public void synchronizeEnvelopeFinished(long accountId, long mailboxId) {
            Logging.d("RefreshManager # mListeners.synchronizeEnvelopeFinished()" );
            for (Listener l : mListeners) {
                l.onStopRefreshIcon(accountId, mailboxId);
            }
        }
    ...
    /**
     * M: Support to stop refresh if network is unavailable.
     */
    public void stopRefresh(long accountId,long mailboxId) {
        Status currentStatus = mMessageListStatus.get(mailboxId);
        if (null != currentStatus && currentStatus.isRefreshing()) {
            Logging.d("stopRefresh for accountId " + accountId + " mailboxId "
                    + mailboxId);
            currentStatus.mIsRefreshing = false;
            currentStatus.mIsRefreshRequested = false;
            currentStatus.mLastRefreshTime = mClock.getTime();
            notifyRefreshStatusChanged(accountId, mailboxId);
        }
    }
}
