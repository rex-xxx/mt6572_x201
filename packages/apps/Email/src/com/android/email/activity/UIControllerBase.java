...
abstract class UIControllerBase implements MailboxListFragment.Callback,
    ...
    static final String TAG = "UIControllerBase";
    static final String KEY_LIST_CONTEXT = "UIControllerBase.listContext";
    ///M: add  KEY_IN_EAS_REMOTE_MODE for not showing unsupported function in Eas remote search
    // mode.
    static final String KEY_IN_EAS_REMOTE_MODE = "UIControllerBase.in_eas_remote_mode";
    ...
    protected MessageViewFragment mMessageViewFragment;
    ...
    /// M: Indicate whether the remote search is on-going
    public boolean mIsRemoteSearching = false;

    ///M: use mIsEasRemoteMode for not showing unsupported function in Eas remote search mode.
    private boolean mIsEasRemoteMode = false;
    ...
    public void onMessagesLoadFinish(boolean isEasRemoteMode) {
        mIsEasRemoteMode = isEasRemoteMode;
    }
    /**
     * M: Support local search call back, register in MessageListFragment and used to update 
     * search result count.
     */
    private MessageListFragment.LocalSearchCallback mLocalSearchCallback 
            = new MessageListFragment.LocalSearchCallback() {
        @Override
        public void onLocalSearchFinished(int count) {
            mActionBarController.updateSearchCount(count);
        }
        /**
         * M: Support local search, update the MessageListContext
         */
        @Override
        public void onMessageListContextChanged(MessageListContext newMessageListContext) {
            setListContext(newMessageListContext);
            Logging.d("onMessageListContextChanged replace to a new messageListContext: " +
                    newMessageListContext);
        };
    };

    /**
     * M: Support remote search call back, register in MessageListFragment and used to update 
     * search result count.
     */
    private final MessageListFragment.RemoteSearchCallback mRemoveSearchCallback 
            = new MessageListFragment.RemoteSearchCallback() {
        @Override
    ...
    private class RefreshListener implements RefreshManager.Listener {
        ...
        public void onStopRefreshIcon(long accountId, long mailboxId) {
            Logging.d("UIControllerBase.onStopRefreshIcon()");
            stopRefreshIcon();
        }
        ...
        public void updateRefreshIcon() {
            ...
            if (isRefreshInProgress() || mIsRemoteSearching) {
    ...
    public void onActivityResume() {
        ...
        if (accountId > 0) {
            NotificationController.getInstance(mActivity).cancelSendFailedNotification(accountId);
        }
    }
    ...
    public void onSaveInstanceState(Bundle outState) {
        ...
        ///M: save mIsEasRemoteMode for not showing unsupported function in Eas remote search mode.
        outState.putBoolean(KEY_IN_EAS_REMOTE_MODE, mIsEasRemoteMode);
    }
    ...
    public void onRestoreInstanceState(Bundle savedInstanceState) {
        ...
        ///M: restore mIsEasRemoteMode for not showing unsupported function in Eas remote search
        // mode.
        mIsEasRemoteMode = savedInstanceState.getBoolean(KEY_IN_EAS_REMOTE_MODE);
    }
    ...
    // MessageViewFragment$Callback
    @Override
    public boolean onCheckIsEasRemoteMessage() {
        return mIsEasRemoteMode;
    }
    ...
    protected void installMessageListFragment(MessageListFragment fragment) {
        ...
        //M: init remote search call back@{
        MessageListContext listContext = mMessageListFragment.getListContext();
        if (null != listContext && listContext.isRemoteSearch()) {
            mMessageListFragment.setRemoteSearchCallBack(mRemoveSearchCallback);
        }
        //@}
        refreshActionBar();
    }
    ...
    /**
     * M : Remove the fragment wheich refrenced by mRemovedFragments.
     *     Must be caleld when Fragment was removed from activity, not detach.
     */
    public final void onRemoveFragment(Fragment fragment) {
        if (Logging.DEBUG_LIFECYCLE && Email.DEBUG) {
            Log.d(Logging.LOG_TAG, this + " onRemoveFragment  fragment=" + fragment);
        }
    ...
    protected void uninstallMessageListFragment() {
        ...
        mMessageListFragment.setRemoteSearchCallBack(null);
    ...
    public final void switchAccount(long accountId, boolean forceShowInbox) {
        ...
        /** M: switch the account for recording and start a new duration recording @{ */
        mActivity.setRecordAccount(accountId);
        DataCollectUtils.stopRecord(mActivity);
        DataCollectUtils.startRecord(mActivity, accountId, true);
        /** @} */
        ...
        /* M: If the account which we switch to didn't done it's folder list sync,
            we should not change the last used account id to escape from the folder
            sync loop.*/ 
        if (Mailbox.findMailboxOfType(mActivity, accountId, Mailbox.TYPE_INBOX)
                != Mailbox.NO_MAILBOX) {
            Preferences.getPreferences(mActivity).setLastUsedAccountId(accountId);
        }
        showAccountSpecificWarning(accountId);
    ...
    /**
     * M: Shortcut for {@link #open} with {@link Message#NO_MESSAGE}.
     * If now is localSearch, open the mailbox with mListContext, else
     * create a new listConext from forMailbox() to open Mailbox.
     */
    protected final void openMailbox(long accountId, long mailboxId) {
        MessageListContext listConext = null;
        if (mListContext != null && mListContext.isLocalSearch()) {
            listConext = mListContext;
        } else {
            listConext = MessageListContext.forMailbox(accountId, mailboxId);
        }
        Logging.d(TAG, "openMailbox ListContext: " + listConext);
        open(listConext, Message.NO_MESSAGE);
    }
    ...
    public final void open(final MessageListContext listContext, final long messageId) {
        ...
        // M: just open a mail,not change action bar status. @{
        if (messageId != Message.NO_MESSAGE) {
            return;
        }
        // @}

        if (listContext.isRemoteSearch()) {
            Logging.d(TAG, "UIControllerBase " + this.hashCode() + " enter remote search..");
            mIsRemoteSearching = true;
            mRefreshListener.updateRefreshIcon();
            mActionBarController.enterRemoteSearchMode(listContext.getSearchParams().mFilter);
        } else if (mActionBarController.isLocalSearchMode() && messageId < 0) {
            Logging.d(TAG, "isLocalSearchMode");
            String searchHint = getSearchHint(listContext.mAccountId, listContext.getMailboxId());
            mActionBarController.setSearchHint(searchHint);
        }
    }
    ...
    public void onSearchRequested(String query) {
        /** M: Enter local search mode only if the mailbox
            has server counter part @{ */
        if (isMessageListInstalled() && !mActionBarController.isLocalSearchMode()
                && !mActionBarController.isRemoteSearchMode()
                && !getMessageListFragment().isInSelectionMode()) {
            ...
            if (mailboxHasServerCounterpart || query != null) {
                mActionBarController.enterLocalSearchMode(query);
            }
        }
        /** @} */
    }
    ...
    protected String getSearchHint(long accountId, long mailboxId) {
        Logging.d(TAG, "getSearchHint(" + accountId + ", " + mailboxId + ")");
        Account account = Account.restoreAccountWithId(mActivity, accountId);
        Mailbox mailbox = Mailbox.restoreMailboxWithId(mActivity, mailboxId);
     ...
     */
    protected void onSearchSubmit(final String queryTerm, final String queryField) {
        /// M: If remote search is on-going, ignore this search request @{
        if (mIsRemoteSearching) {
            return;
        }
        /// @}
    ...
    /**
     * M: Kicks off a local search, if the UI is in a state where a search is possible.
     * @param queryTerm the keyword for filtering
     * @param queryField by which field to filter
     */
    protected void onLocalSearchSubmit(final String queryTerm, final String queryField) {
        MessageListFragment listFragment = getMessageListFragment();
        listFragment.startLocalSearch(queryTerm, queryField, mLocalSearchCallback);
        listFragment.enterLocalSearchMode();
    }

    /**
     * Handles exiting of search entry mode.
     * Note: This is method only support local search.
     */
    protected void onSearchExit() {
        if ((mListContext != null) && mListContext.isRemoteSearch()) {
            mActivity.finish();
            //TODO: For local search, should back to message list 
        } else {
            // Re show the search icon.
            mActivity.invalidateOptionsMenu();
            /// M: Restore the message list cursor
            MessageListFragment listFragment = getMessageListFragment();
            /// M: Add a judgment to avoid the listFragment is null in some case.
            if (listFragment != null) {
                listFragment.startLocalSearch(null, null, mLocalSearchCallback);
                listFragment.exitLocalSearchMode();
            }
        }
    }
    ...
    public boolean onPrepareOptionsMenu(MenuInflater inflater, Menu menu) {
        ...
        // M: Not show search menu since it has in search mode (local Search or Remote Search). 
        //    Not show search menu if current folder is not syncable.@{
        boolean showSearchIcon = !mActionBarController.isLocalSearchMode()
                && !mActionBarController.isRemoteSearchMode()
                && mailboxHasServerCounterpart;
        // @}
        ...
        /** M: Only show this menu item in eng mode and EAS account @{ */
        MenuItem habit = menu.findItem(R.id.generate_habitdata);
        if (habit != null) {
            // Make this menu item invisible temporarily
            habit.setVisible(false);
            // For enigneer, we can enable below code for debugging
            // We can remove this menu item when smart push feature is stable enough
//            Account acct = Account.restoreAccountWithId(mActivity,getUIAccountId());
//            habit.setVisible(Build.TYPE.equals("eng") &&
//                    acct != null ? acct.isEasAccount(mActivity) : false);
        }
        return true;
        /** @} */
    }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            ...
            case R.id.mailbox_settings:
                ...
                return true;
            /// M: Generate simulated user habit data for testing
            case R.id.generate_habitdata:
                generateHabitData();
                return true;
        }
        return false;
    ...
    protected abstract boolean canStopRefreshIcon(boolean isSynchronizeEnvelopeFinished);
    ...
    public void onMailboxNotFound(boolean isFirstLoad) {
        ...
        /// M: Moditify the default workflow, not jump to Welcome activity since run testcase. @{
        if (!Configuration.mIsRunTestcase) {
            long accountId = getUIAccountId();
            if (accountId != Account.NO_ACCOUNT) {
                /** M: if account is valid, just forget this problem in remote search mode,
                 * because we can sure this is caused by the Id of this mailbox changed to one new
                 * mailboxId by us, used to avoid the messages belong to old search command show on
                 * UI. @{ */
                if (mListContext.isRemoteSearch() && Account.isValidId(mActivity, accountId)) {
                    Logging.d(TAG, "onMailboxNotFound, the mailbox deleted in remote search mode");
                    return;
                }
                /** @} */
                mActivity.startActivity(Welcome.createOpenAccountInboxIntent(
                        mActivity, accountId));
            } else {
                Welcome.actionStart(mActivity);
            }
        } else {
            Logging.d(TAG, "MailboxNotFound but not start Welcome since running testcase");
        }
        /// @}
        mActivity.finish();
    }
    ...
    protected final boolean moveToOlder() {
        if (!mMessageViewFragment.isMessageLoading() && (mOrderManager != null)
                && mOrderManager.moveToOlder()) {
            mMessageViewFragment.stopLoading();
            mMessageViewFragment.showWaitingDialogIfNeeded();
    ...
    private void updateMessageOrderManager() {
        ...
        if (mOrderManager == null || !mOrderManager.getListContext().equals(mListContext)) {
            stopMessageOrderManager();
            Logging.d(TAG, "updateMessageOrderManager update mOrderManager with mlistContext:"
                    + mListContext);
    ...
    public String onGetQueryTerm() {
        if (mActionBarController.isLocalSearchMode()) {
            return mActionBarController.getQueryTermIfSearchBody();
        } else if (mActionBarController.isRemoteSearchMode()) {
            String field = mListContext.getSearchParams().mField;
            if (field.contains(SearchParams.SEARCH_FIELD_BODY)
                    || field.contains(SearchParams.SEARCH_FIELD_ALL)) {
                return mListContext.getSearchParams().mFilter;
            }
        }
    ...
    /**
     * M: Update the refresh icon status (currently used by remote search)
     */
   public void updateRefreshIcon() {
       mRefreshListener.updateRefreshIcon();
   }
   ...
   /**
    * M: Used to simulate habit data generation, just used for testing
    */
   private void generateHabitData() {
       ...
       final int SPAN = 300000; //300s
       final int MINSPAN = 300000; // 300s

       long accountId = getUIAccountId();
       // Delete all the existed habit data of the account before generating new
       mActivity.getContentResolver().delete(SmartPush.CONTENT_URI, SmartPush.ACCOUNT_KEY + "=?", new String[]{String.valueOf(accountId)});
       // The habit data date would start from 3 days ago
       long current = System.currentTimeMillis();
       long days = current / DAY;
       // Get the start time of today (GMT)
       long todayStartTime = days * DAY;
       long base = todayStartTime - 3 * DAY;

       Random random = new Random();

       // Insert 100-400 data items
       int recordNum = 100 + random.nextInt(300);
       SmartPush sp = null;
       Log.i("SmartPushService", "insert " + recordNum + "+ habit data---current time: " + current);

       for (int i = 0; i < recordNum; i++) {
           // The timespan between any 2 events would be 300---600s,
           // so the max possible total timespan can be 600s*400 = 2.77 days, which less than 3 days
           base += (MINSPAN + random.nextInt(SPAN));
           int eventType = 1 + random.nextInt(3);

           if (eventType == SmartPush.TYPE_DURATION) {
               // The duration value would be 2-202s
               sp = SmartPush.addEvent(mActivity, base, accountId, eventType, 2000 + random.nextInt(200000));
           } else {
               sp = SmartPush.addEvent(mActivity, base, accountId, eventType, 1);
           }
           if (sp != null) {
               sp.save(mActivity);
           }

           // There always be a duration event come after an open event, the timestamp is same as the open's
           if (eventType == SmartPush.TYPE_OPEN) {
               sp = SmartPush.addEvent(mActivity, base, accountId, SmartPush.TYPE_DURATION, 2000 + random.nextInt(200000));
               sp.save(mActivity);
           }
       }
   }
