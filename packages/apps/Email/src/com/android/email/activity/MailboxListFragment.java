...
public class MailboxListFragment extends ListFragment implements OnItemClickListener,
    ...
    /** M: For save the state of mail count of VIP folder @{*/
    private static final String BUNDLE_VIP_MAIL_COUNT = "MailboxListFragment.state.VipMailCount";
    ...
    private void setInitialParentAndHighlight() {
        ...
        if (getAccountId() == Account.ACCOUNT_ID_COMBINED_VIEW) {
            ...
            final long inboxId = Mailbox.findMailboxOfType(getActivity(), getAccountId(),
                    Mailbox.TYPE_INBOX);
            /// M: we should treat "QUERY_ALL_VIPS" as "INBOX", cause they don't have parent folders
            if (initialMailboxId == inboxId || initialMailboxId == Mailbox.QUERY_ALL_VIPS) {
    ...
    public void onSaveInstanceState(Bundle outState) {
        ...
        /// M: Save the state of mail count of VIP folder
        outState.putInt(BUNDLE_VIP_MAIL_COUNT, mVipMailCount);
    }
    ...
    private void restoreInstanceState(Bundle savedInstanceState) {
        ...
        /** M: Restore the state of mail count of VIP folder @{*/
        mVipMailCount = savedInstanceState.getInt(BUNDLE_VIP_MAIL_COUNT);
        mListAdapter.updateVipMessageCount(mVipMailCount);
        /** @} */
    }
    ...
    private class MailboxListLoaderCallbacks implements LoaderCallbacks<Cursor> {
        ...
        public void onLoadFinished(Loader<Cursor> loader, Cursor cursor) {
            ...
            if (cursor.getCount() == 0) {
                ...
                /// M: Load the mail count of VIP folder
                new LoadVipFolderMessageCountTask(mTaskTracker, mActivity)
                        .executeParallel(getAccountId());
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long doNotUse) {
        ...
        if (mListAdapter.isAccountRow(position)) {
            ...
            if ((view instanceof MailboxListItem) 
                    && ((MailboxListItem) view).isNavigable() 
                    && (id != mParentMailboxId)) {
    ...
    /**
     * M: Load task for loading the mail count of VIP folder
     */
    class LoadVipFolderMessageCountTask extends EmailAsyncTask<Long, Void, Integer> {
        ...
        protected void onSuccess(Integer result) {
            mVipMailCount = result;
            mListAdapter.updateVipMessageCount(result);
        }
    }
    ...
    private boolean onDrop(DragEvent event) {
        ...
        if (mDropTargetView.mMailboxType == Mailbox.TYPE_TRASH) {
            controller.deleteMessages(Utility.toLongSet(messageIds));
        } else {
