...
public class MessageOrderManager {
    ...
    private class LoadMessageListTask extends EmailAsyncTask<Void, Void, Cursor> {
        ...
        protected Cursor doInBackground(Void... params) {
            EmailAsyncTask.printStartLog("LoadMessageListTask#doInBackground");
            Cursor c = openNewCursor();
            EmailAsyncTask.printStopLog("LoadMessageListTask#doInBackground");
            return c;
        }
    ...
    /**
     * M: Open a new cursor for a message list. If is localSearch, open a new cursor
     * by query the localSearch Mailbox,else query the whole mailbox.
     *
     * This method is called on a worker thread by LoadMessageListTask.
     */
    private Cursor openNewCursor() {
        /** M: Support for VIP folder @{ */
        if (mListContext.getMailboxId() == Mailbox.QUERY_ALL_VIPS) {
            return VipMemberCache.getVipMessagesIds(mContext, mListContext.mAccountId);
        }
        /** @} */
        Cursor cursor = null;
        if (mListContext.isLocalSearch()) {
            cursor = mContentResolver.query(EmailContent.Message.CONTENT_URI,
                    EmailContent.ID_PROJECTION,
                    Message.buildLocalSearchSelection(mContext, mListContext.mAccountId,
                            mListContext.getSearchedMailbox(),
                            mListContext.getSearchParams().mFilter,
    ...
    /* package */ void onCursorOpenDone(Cursor cursor) {
        try {
            ...
            Logging.d("onCursorOpenDone update cursor with count: " + mTotalMessageCount);
