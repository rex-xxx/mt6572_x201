...
class UIControllerOnePane extends UIControllerBase {
    ...
    private class ActionBarControllerCallback implements ActionBarController.Callback {
        ...
        public void onMailboxSelected(long accountId, long mailboxId) {
            if (mailboxId == getMailboxId()) {
                //Not trigger mail box to refresh if it not changed.
                return ;
            }
        ...
        @Override
        public void onAccountSelected(long accountId) {
            switchAccount(accountId, false);  // Do not force refresh mail box.
        }

        @Override
        public void onNoAccountsFound() {
            // M: No need jump to AccountSetupBasic UI when running testcase @{
        ...
        @Override
        public void onSearchSubmit(final String queryTerm, final String queryField) {
            if (!isMessageListInstalled()) {
                return;
            }
            UIControllerOnePane.this.onSearchSubmit(queryTerm, queryField);
        }

        @Override
        public void onLocalSearchSubmit(final String queryTerm, final String queryField) {
            if (!isMessageListInstalled()) {
                return;
            }
            UIControllerOnePane.this.onLocalSearchSubmit(queryTerm, queryField);
        }
    ...
    public boolean onBackPressed(boolean isSystemBackKey) {
        /// M : Current is running testcase, no need do complete operation again.@{
        if (Configuration.mIsRunTestcase) {
            Logging.d(" Run Testcase, not handle back key event.");
            return false;
        }
        /// @}
        if (Email.DEBUG) {
        ...
        // M: Remote search Home key and isMessageListInstalled 
        // should finish current EmailActivity.@{
        if (!isSystemBackKey && isMessageListInstalled()
        ...
        if (isMessageViewInstalled()) {
            ...
            return true;
                ...
                && !mActionBarController.isRemoteSearchMode()) {
            // Non-inbox list. Go to inbox, except the RemoteSearch activity
    ...
    private void showFragment(Fragment fragment) {
        ...
        if ((installed instanceof MessageViewFragment)
            ...
            if (installed != null) {
                if (installed instanceof MessageViewFragment) {
                    // reuse the existing MessageViewFragment
                    if (DEBUG_FRAGMENTS) {
                        Log.d(Logging.LOG_TAG, "MessageViewFragment reload");
                    }
                    MessageViewFragment mf = (MessageViewFragment) installed;
                    mf.setMessageId(fragment.getArguments());
                    mf.resetPartialLoading();
                    mf.loadMessage();
                    return;
                }
            }
    ...
    private boolean shouldPopFromBackStack(boolean isSystemBackKey) {
        ...
        // M: Support open a remote search mail or folder.
        // We have 2 fragments: MessageList and the opened MessageView in back stack.
        // For back press, should un-attach MessageView and re-attach MessageList. @{
        if(mPreviousFragment instanceof MessageListFragment){
            MessageListContext listContext = ((MessageListFragment) mPreviousFragment)
                    .getListContext();
    ...
    public boolean onPrepareOptionsMenu(MenuInflater inflater, Menu menu) {
        ...
        MenuItem item = menu.findItem(R.id.show_all_mailboxes);
        if (item != null) {
            item.setVisible(!mActionBarController.isLocalSearchMode()
                    && !mActionBarController.isRemoteSearchMode());
        }
        ...
        if (messageViewVisible) {
            ...
            mActionBarController.setSwitchNewerEnable((om != null) && om.canMoveToNewer());
            mActionBarController.setSwitchOlderEnable((om != null) && om.canMoveToOlder());
            /** @} */
        }
        return true;
    }
    ...
    protected void onRefresh() {
        ...
        if (isMessageListInstalled()) {
            if (mListContext != null && mListContext.isRemoteSearch()) {
                UIControllerOnePane.this.onSearchSubmit(mListContext.getSearchParams().mFilter,
                        mListContext.getSearchParams().mField);
            } else {
    ...
    @Override
    protected boolean canStopRefreshIcon(boolean isSynchronizeEnvelopeFinished) {
        if (!isRefreshEnabled()) {
            return false;
        }
        if (isMessageListInstalled()) {
