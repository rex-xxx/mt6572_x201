...
import com.mediatek.email.emailvip.activity.VipPreference;
...
public class AccountSettingsFragment extends PreferenceFragment {
    ...
    /// M: Key to get the oof preference
    ...
    private static final String PREFERENCE_VIBRATE_WHEN = "ics_account_settings_vibrate_when";
    ...
    //Constants for save return check status
    private static final int OOF_SETTING_ORDER = 4;
    ...
    private CheckBoxPreference mAccountVibrateWhen;
    ...
    public static OofParams sOofParams;

    // Async Tasks
    private AsyncTask<?,?,?> mLoadAccountTask;

    /// M: The preference order of the VIP settings
    private static final int VIPSETTINGS_ORDER = 6;
    private VipPreference mVipPreference;
    ...
    private void updateSyncOptionsPreference() {
        if (mAccount == null) {
            return;
        }
        String currentProtocol = mAccount.mHostAuthRecv.mProtocol;
        if (HostAuth.SCHEME_EAS.equals(currentProtocol)) {
            ...
            mSyncContacts.setOnPreferenceChangeListener(mSyncOptionsPreferenceChangeListener);
            mSyncCalendar.setChecked(ContentResolver.getSyncAutomatically(acct,
                    CalendarProviderStub.AUTHORITY));
            mSyncCalendar.setOnPreferenceChangeListener(mSyncOptionsPreferenceChangeListener);
            mSyncEmail.setChecked(ContentResolver
                    .getSyncAutomatically(acct, EmailContent.AUTHORITY));
            mSyncEmail.setOnPreferenceChangeListener(mSyncOptionsPreferenceChangeListener);
            mCheckFrequency.setEnabled(ContentResolver
                    .getSyncAutomatically(acct, EmailContent.AUTHORITY));
        } else {
            android.accounts.Account account = new android.accounts.Account(mAccount.mEmailAddress,
            ...
            boolean syncAuto = ContentResolver
                    .getSyncAutomatically(account, EmailContent.AUTHORITY);
            mSyncEmail.setChecked(syncAuto);
            mCheckFrequency.setEnabled(syncAuto);
            mSyncEmail.setOnPreferenceChangeListener(mSyncOptionsPreferenceChangeListener);
        }
    }
    ...
    private void loadSettings() {
        ...
        mAccountDescription.getEditText().addTextChangedListener(
                new TextNotNullWatcher(mAccountDescription));
        ...
        mAccountName.getEditText().addTextChangedListener(new TextNotNullWatcher(mAccountName));
        ...
        boolean showOof = false;
        showOof = mAccount.isEasAccount(mContext);
        if (showOof) {
            PreferenceScreen preferenceScreen = getPreferenceScreen();
            Preference prefOof = new Preference(mContext);
            prefOof.setTitle(R.string.account_settings_oof_label);
            prefOof.setOnPreferenceClickListener(new Preference.OnPreferenceClickListener() {
                        @Override
            ...
            prefOof.setOrder(OOF_SETTING_ORDER);
            preferenceScreen.addPreference(prefOof);
        }
        /** @} */
...
*/
        ...
        mAccountVibrateWhen = (CheckBoxPreference) findPreference(PREFERENCE_VIBRATE_WHEN);
        ...
        if (vibrator.hasVibrator()) {
            ...
            mAccountVibrateWhen.setChecked(vibrateAlways);
        ...
        /// M: Update the checkFrequency enable status with synEmail
        mSyncEmail.setOnPreferenceClickListener(new Preference.OnPreferenceClickListener() {
            @Override
            public boolean onPreferenceClick(Preference preference) {
                mCheckFrequency.setEnabled(mSyncEmail.isChecked());
                return false;
            }
        });
        updateSyncOptionsPreference();
    ...
    private final Preference.OnPreferenceChangeListener mSyncOptionsPreferenceChangeListener
                        = new Preference.OnPreferenceChangeListener() {
        public boolean onPreferenceChange(Preference preference, Object newValue) {
            String key = preference.getKey();
            android.accounts.Account acct = null;
            boolean isEasAccount = false;
            if (HostAuth.SCHEME_EAS.equals(mAccount.mHostAuthRecv.mProtocol)) {
                acct = new android.accounts.Account(mAccount.mEmailAddress,
                        AccountManagerTypes.TYPE_EXCHANGE);
                isEasAccount = true;
            } else {
                acct = new android.accounts.Account(mAccount.mEmailAddress,
            ...
            if (AccountSettingsFragment.PREFERENCE_SYNC_EMAIL.equals(key)) {
                ContentResolver.setSyncAutomatically(acct, EmailContent.AUTHORITY,
                        (Boolean) newValue);
            } else if (AccountSettingsFragment.PREFERENCE_SYNC_CALENDAR.equals(key)) {
                ContentResolver.setSyncAutomatically(acct, CalendarProviderStub.AUTHORITY,
                        (Boolean) newValue);
            } else if (AccountSettingsFragment.PREFERENCE_SYNC_CONTACTS.equals(key)) {
    ...
    private void saveSettings() {
        ...
        int newFlags = mAccount.getFlags() &
                ~(Account.FLAGS_NOTIFY_NEW_MAIL |
                        ...
                        Account.FLAGS_BACKGROUND_ATTACHMENTS | Account.FLAGS_VIP_VIBRATE_ALWAYS |
                        Account.FLAGS_NOTIFY_VIP_NEW_MAIL);
        ...
        if (mAccountVibrateWhen.isChecked()) {
    ...
    /**
     * TextNotNullWatcher to catch whether the input text is null in EditTextPreference dialog.
     * if input text is null, it will disable the positive button.
     */
    private static class TextNotNullWatcher implements TextWatcher {
        private EditTextPreference mEditText = null;
        public TextNotNullWatcher(EditTextPreference editText) {
            mEditText = editText;
        }
        @Override
   ...
   private void initLengthFilter() {
        UiUtilities.setupLengthFilter(mAccountName.getEditText(), mContext,
                Email.EDITVIEW_MAX_LENGTH_1, true);
        UiUtilities.setupLengthFilter(mAccountDescription.getEditText(),
                mContext, Email.EDITVIEW_MAX_LENGTH_1, true);
        UiUtilities.setupLengthFilter(mAccountSignature.getEditText(),
                mContext, Email.EDITVIEW_MAX_LENGTH_2, true);
    }
