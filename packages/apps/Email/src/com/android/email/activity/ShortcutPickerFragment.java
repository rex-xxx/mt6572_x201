...
public abstract class ShortcutPickerFragment extends ListFragment
    ...
    public void onAttach(Activity activity) {
        ...
        mAdapter = new SimpleCursorAdapter(activity,
            R.layout.account_shortcutpicker_list_item1, null, fromColumns, TO_VIEWS, 0);
    ...
    public static class AccountShortcutPickerFragment extends ShortcutPickerFragment {
        private volatile Boolean mLoadFinished = Boolean.valueOf(false);
        private static final String[] ACCOUNT_FROM_COLUMNS = new String[] {
    ...
    private static final class MailboxPickerLoader extends CursorLoader {
        ...
        public Cursor loadInBackground() {
            ...
            /** M: support multi-language  @{ */
            Cursor mailboxCursor = super.loadInBackground();
            mailboxCursor = updateMailboxNames(context,mailboxCursor);
            return new MergeCursor(new Cursor[] { mailboxCursor, unreadCursor });
            /** @} */
        }

        /**
         * M: Update the display name of mailbox to support multi-language
         * @param context the context
         * @param cursor the original cursor
         * @return
         */
        private Cursor updateMailboxNames(Context context, Cursor cursor) {
            if (cursor == null) {
                return null;
            }
            FolderProperties.removeInstance();
            FolderProperties fp = FolderProperties.getInstance(context);
            MatrixCursor matrixCursor = new ClosingMatrixCursor(cursor.getColumnNames(), cursor);
            cursor.moveToPosition(-1);
            while (cursor.moveToNext()) {
                RowBuilder row = matrixCursor.newRow();
                int columnCount = cursor.getColumnCount();
                int nameColumnIndex = cursor.getColumnIndex(MailboxColumns.DISPLAY_NAME);
                Object columnValue;
                for (int i = 0; i < columnCount; i++) {
                    if (nameColumnIndex == i) {
                        columnValue = fp.getDisplayName(cursor);
                    } else {
                        columnValue = getValue(cursor, i);
                    }
                    row.add(columnValue);
                }
            }
        ...
        /**
         * M: Retrieve the object value in the current row and the column index of the cursor
         * @param c the current cursor
         * @param index the index of the column
         * @return the object value in the current row and the column index of the cursor
         */
        private Object getValue(Cursor c,int index) {
    ...
    public static class MailboxShortcutPickerFragment extends ShortcutPickerFragment {
        ...
        public static final int FILTER_ALLOW_ALL    = 0;
        /** Only allow an account's INBOX */
        public static final int FILTER_INBOX_ONLY   = 1 << 0;
        /** Allow an "unread" mailbox; this is not affected by {@link #FILTER_INBOX_ONLY} */
        public static final int FILTER_ALLOW_UNREAD = 1 << 1;
        ...
        private static final String[] MAILBOX_FROM_COLUMNS = new String[] {
            MailboxColumns.DISPLAY_NAME,
        };
        /** Loader projection used for IMAP & POP3 accounts */
        /// M: Mailbox type is also needed
        private static final String[] IMAP_PROJECTION = new String [] {
            MailboxColumns.ID, MailboxColumns.ID + " as " + REAL_ID,
            MailboxColumns.SERVER_ID + " as " + MailboxColumns.DISPLAY_NAME, MailboxColumns.TYPE
        };
        /** Loader projection used for EAS accounts */
        /// M: Mailbox type is also needed
        private static final String[] EAS_PROJECTION = new String [] {
            MailboxColumns.ID, MailboxColumns.ID + " as " + REAL_ID,
            MailboxColumns.DISPLAY_NAME, MailboxColumns.TYPE
        };
        ...
        private static final String INBOX_ONLY_SELECTION = ALL_MAILBOX_SELECTION +
