...
public class AccountSettingsUtils {
    ...
    public static ContentValues getAccountContentValues(Account account) {
        ...
        cv.put(AccountColumns.VIP_RINGTONE_URI, account.mVipRingtoneUri);
        return cv;
    }
    ...
    public static ArrayList<String> CollectAutoSetupDomain(Context context) {
        ArrayList<String> serverName = new ArrayList<String>();
         try{
            // Parse R.xml.manual_setup_helper
            XmlResourceParser xml = context.getResources().getXml(R.xml.manual_setup_helper);
            int xmlEventType;
            ...
            Log.e(Logging.LOG_TAG, "Error while trying to load manual_setup_helper settings.", e);
        }
    return serverName;
    }

    /**
     * Search the default servername definition when user setup account manually.
     * 
     * @param context The application context.
     * @param domain The domain portion of the user's email address.
     * @param protocal The protocal that user selected.
     * @return suitable server name definition, or null if no match found.
     */
    public static String findManualInfoForDomain(Context context, String domain, String protocal) {
            ...
            while ((xmlEventType = xml.next()) != XmlResourceParser.END_DOCUMENT) {
                if (xmlEventType == XmlResourceParser.START_TAG
                    ...
                    try {
                        ...
                        && "pop3support".equals(xml.getName())
                        && isMatched && HostAuth.SCHEME_POP3.equals(protocal)) {
                    serverName = getXmlAttribute(context, xml, "servername");
                } else if (xmlEventType == XmlResourceParser.START_TAG
                        && "imapsupport".equals(xml.getName())
                        && isMatched && HostAuth.SCHEME_IMAP.equals(protocal)) {
                    serverName = getXmlAttribute(context, xml, "servername");
                } else if (xmlEventType == XmlResourceParser.START_TAG
                        && "eassupport".equals(xml.getName())
                        && isMatched && HostAuth.SCHEME_EAS.equals(protocal)) {
                    serverName = getXmlAttribute(context, xml, "servername");
                } else if (xmlEventType == XmlResourceParser.START_TAG
                        && "smtpsupport".equals(xml.getName())
                        && isMatched && HostAuth.SCHEME_SMTP.equals(protocal)) {
                    serverName = getXmlAttribute(context, xml, "servername");
