...
import com.android.email.activity.SafeFragment;
 ...
 * conditions.
 */
public class AccountCheckSettingsFragment extends SafeFragment {

    public static final String TAG = "AccountCheckSettingsFragment";
    public static final String BUNDLE_KEY = "AccountCheckSettingFragment.checkMode";
    /// M: add for save the checking state
    public static final String STATE_KEY = "AccountCheckSettingFragment.state";
    ...
    private static final int STATE_AUTODISCOVER_RESULT = 8;         // terminal
    /// M: use to decide whether the check account progress has reported result.
    private static final int STATE_REPORTED = 4;
    ...
    /** Check settings abort due to user cancel */
    public static final int CHECK_SETTINGS_USER_CANCEL = 4;
    ...
    public void onSaveInstanceState(Bundle outState) {
        ...
        outState.putInt(BUNDLE_KEY, mCheckMode);
        /// M: save the state to avoid restart checkAccountTask when onActivityCreated.
        /// this case happens when email been killed and restarted again.
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        if (mAccountCheckTask == null) {
            if (savedInstanceState != null
                    && savedInstanceState.containsKey(BUNDLE_KEY)) {
                mCheckMode = savedInstanceState.getInt(BUNDLE_KEY, 0);
            } else {
                mCheckMode = getTargetRequestCode();
            }
            /// M: restore the state to avoid restart checkAccountTask.
            /// this case happens when email been killed and restarted again.
            if (savedInstanceState != null && savedInstanceState.containsKey(STATE_KEY)) {
                mState = savedInstanceState.getInt(STATE_KEY, 0);
            }
            /// M: if the check account progress has reported result and the result code is OK,
            // don't restart mAccountCheckTask.
            // this case happens when error happened in checking account, email been killed
            // and restarted again. Should just restart mAccountCheckTask.
            if (mState != STATE_CHECK_OK) {
                mState = STATE_START;
                Account checkAccount = SetupData.getAccount();
                mAccountCheckTask = (AccountCheckTask) new AccountCheckTask(mCheckMode,
                        checkAccount).executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
                Logging.d(this + "onActivityCreated  new AccountCheckTask ");
            }
            Logging.d(this + " ### onActivityCreated  #### checkMode = " + mCheckMode + "  "
                    + mAccountCheckTask + "  mState= " + mState);
        }
    }

    /**
     * Check if the instance account is available.
     */
    private boolean isAccountAvailable(Account checkAccount) {
        return null != checkAccount
                && null != checkAccount.mHostAuthRecv
                && null != checkAccount.mEmailAddress
                && null != checkAccount.mHostAuthRecv.mAddress
                && null != checkAccount.mHostAuthRecv.mPassword;
    }
            ...
            switch (newState) {
                case STATE_CHECK_OK:
                    ...
                    if (isResumed()) {
                        // 2. exit self
                        fm.popBackStack();
                        Logging.d("Exit curren fragment, state: ok");
                    } else {
                        addResumeRunnable(new Runnable(){

                            @Override
                            public void run() {
                                if (isResumed()) {
                                    fm.popBackStack();
                                    Logging.d("Exit curren fragment in thread, state: ok");
                                }
                            }});
                    }
                    ...
                    if (isResumed()) {
                        Logging.d("Send onCheckSettingsComplete call back ");
                        getCallbackTarget().onCheckSettingsComplete(CHECK_SETTINGS_OK);
                    } else {
                        Logging.d("The curren fragment is in backgroud, " +
                                    "the call back will be send on Resume");
                    }
                    break;
                case STATE_CHECK_SHOW_SECURITY:
                    ...
                    if (fm.findFragmentByTag(SecurityRequiredDialog.TAG) == null) {
                        ...
                        securityRequiredDialog.setCancelable(false);
                        fm.beginTransaction()
                ...
                case STATE_AUTODISCOVER_AUTH_DIALOG:
                    ...
                    if (fm.findFragmentByTag(ErrorDialog.TAG) == null) {
                        ...
                        Logging.d("launch the error dialog");
                        fm.beginTransaction()
                ...
                case STATE_AUTODISCOVER_RESULT:
                    ...
                    if (isResumed()) {
                        // 2. exit self
                        Logging.d("Exit curren fragment, state: AUTODISCOVER");
                        fm.popBackStack();
                    } else {
                        addResumeRunnable(new Runnable(){
                            ...
                            @Override
                            public void run() {
                                Logging.d("Exit curren fragment in thread," +
                                            " state:AUTODISCOVER, isResume:" + isResumed());
                                fm.popBackStack();
                            }});
                    }
                    if (ex != null) {
                        ...
                        getCallbackTarget().onAutoDiscoverComplete(
                                (autoDiscoverResult != null) ? AUTODISCOVER_OK
                                        : AUTODISCOVER_NO_DATA, autoDiscoverResult);
                    }
                    break;
                default:
    ...
    private Callbacks getCallbackTarget() {
        ...
        Log.d(Logging.LOG_TAG, "getCallbackTarget activity = "+ activity);
        if (activity instanceof Callbacks) {
            return (Callbacks) activity;
        }

        // Return null indicate that the getActivity() return null, when this
        // fragment has been removed form this activity.
        return null;
    }
    ...
    private void onCheckingDialogCancel() {
        ...
        Log.d(Logging.LOG_TAG, "onCheckingDialogCancel");

        Callbacks callbackTarget = getCallbackTarget();
        if (null == callbackTarget) {
            return;
        }
        // report check settings failure to target fragment or activity
        callbackTarget.onCheckSettingsComplete(CHECK_SETTINGS_USER_CANCEL);

        finish();
    }

    private void onEditCertificateOk() {
        Callbacks callback = getCallbackTarget();
        if (null == callback) {
            return;
        }
        callback.onCheckSettingsComplete(CHECK_SETTINGS_CLIENT_CERTIFICATE_NEEDED);
        finish();
    }
    ...
    private void finish() {
        ...
        if (fm != null) {
            Logging.d("Kill current fragment if not already killed (finish)");
    ...
    private void onSecurityRequiredDialogResultOk(boolean okPressed) {
        ...
        FragmentManager fm = getFragmentManager();
        if (fm != null) {
            Logging.d("Kill current fragment if not already killed" +
                        " (onSecurityRequiredDialogResultOk)");
            fm.popBackStack();
        }
    }
    ...
    private class AccountCheckTask extends AsyncTask<Void, Integer, MessagingException> {
        ...
        protected MessagingException doInBackground(Void... params) {
            ...
            try {
                if ((mMode & SetupData.CHECK_AUTODISCOVER) != 0) {
                    ...
                    if (errorCode != MessagingException.NO_ERROR) {
                        return new AutoDiscoverResults(false, null);
                    } else {
                ...
                if ((mMode & SetupData.CHECK_INCOMING) != 0) {
                    ...
                    /// M: Check incoming settings should use a new store instance not the cache
                    Store store = Store.getInstance(mAccount, mContext, false);
                    ...
                    if (bundle != null) {
                        ...
                        String protocolVerion = bundle.getString(
                                EmailServiceProxy.VALIDATE_BUNDLE_PROTOCOL_VERSION, null);
                        if (protocolVerion != null) {
                            mAccount.mProtocolVersion = protocolVerion;
                        }
                    }
                    ...
                    if (resultCode == MessagingException.SECURITY_POLICIES_UNSUPPORTED) {
                ...
                if ((mMode & SetupData.CHECK_OUTGOING) != 0) {
                    ...
                    sender.close();
                    /** M: Check the authentication strictly while checking SMTP settings @{*/
                    if (sender instanceof SmtpSender) {
                        ((SmtpSender)sender).open(true);
                    } else {
                        sender.open();
                    }
                    /** @} */
        ...
        private MessagingException fakeChecker() {
            ...
            if ((mMode & SetupData.CHECK_AUTODISCOVER) != 0) {
                ...
                Logging.d("fakeChecker occur InterruptedException : " + e.getMessage());
                }
    ...
    public static class CheckingDialog extends DialogFragment {
        ...
        public void updateProgress(int progress) {
            ...
            if (dialog == null) {
                Logging.e(TAG, "getDialog return a null object. This should never happen.");
                return;
            }
    ...
    public static class ErrorDialog extends DialogFragment {
        ...
        private static final String ARGS_EXCEPTION_MESSAGE = "ErrorDialog.ExceptionMessage";
        private static final String ARGS_EXCEPTION_DATA = "ErrorDialog.ExceptionData";
        ...
        public static ErrorDialog newInstance(Context context, AccountCheckSettingsFragment target,
            ...
            arguments.putString(ARGS_EXCEPTION_MESSAGE, ex.getMessage());
            arguments.putInt(ARGS_EXCEPTION_ID, ex.getExceptionType());
            arguments.putStringArray(ARGS_EXCEPTION_DATA, (String[])ex.getExceptionData());
        ...
        public Dialog onCreateDialog(Bundle savedInstanceState) {
            ...
            final String exceptionMessage = arguments.getString(ARGS_EXCEPTION_MESSAGE);
            final int exceptionId = arguments.getInt(ARGS_EXCEPTION_ID);
            final String[] exceptionData = arguments.getStringArray(ARGS_EXCEPTION_DATA);
            final AccountCheckSettingsFragment target =
                    (AccountCheckSettingsFragment) getTargetFragment();

            MessagingException exception = new MessagingException(
                    exceptionId, exceptionMessage, exceptionData);
            String message = getErrorString(context, exception);
            ...
            Log.d(Logging.LOG_TAG, "ErrorDialog onCreateDialog");
        ...
        public void onCancel(DialogInterface dialog) {
            AccountCheckSettingsFragment target =
                (AccountCheckSettingsFragment) getTargetFragment();
            Log.d(Logging.LOG_TAG, "ErrorDialog onCancel");
            Log.d(Logging.LOG_TAG, "target is removing = " + target.isRemoving()
                    + " target.isAdded() = " + target.isAdded());
            target.onErrorDialogEditButton();
            super.onCancel(dialog);
        }
