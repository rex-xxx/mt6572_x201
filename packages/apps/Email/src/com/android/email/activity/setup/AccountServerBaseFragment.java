...
public abstract class AccountServerBaseFragment extends Fragment
    ...
    protected static Bundle sSetupModeArgs = null;
    protected static URI sDefaultUri;

    private static final String BUNDLE_KEY_SETTINGS = "AccountServerBaseFragment.settings";
    private static final String BUNDLE_KEY_SETTINGS_ACCOUNTID =
                                        "AccountServerBaseFragment.settings.accountId";
    private static final String BUNDLE_KEY_ACTIVITY_TITLE = "AccountServerBaseFragment.title";
    public static final String TAG = "AccountServerBaseFragment";
    ...
    // It is only used for Account Setting to save current account id . 
    protected Long mAccountID = -1L;
     ...
     * @param accountID: setting accountID
     */
    public static synchronized Bundle getSettingsModeArgs(Long accountID) {
        if (sSetupModeArgs == null) {
            sSetupModeArgs = new Bundle();
        }
        sSetupModeArgs.putBoolean(BUNDLE_KEY_SETTINGS, true);
        sSetupModeArgs.putLong(BUNDLE_KEY_SETTINGS_ACCOUNTID, accountID);
        Logging.d(TAG, "getSettingsModeArgs account: " + accountID);
    ...
    /**
     * Get a reasonable account. 
     * The SetupData is a common data for the account setup fragment, the SetupData may be changed 
     * if one fragment editing it while another reading it.
     * 1) If the SetupData Account Not changed just get it.
     * 2) else restore it from DB.
     * @return Current Setting account
     */
    public Account getCurrentAccount(){
        Account account = SetupData.getAccount();
        if (mAccountID > 0 && mAccountID != account.mId) {
            // From Setting but not the same account
            account = Account.restoreAccountWithId(mContext, mAccountID);
            ...
            Logging.d(TAG, "From Settting restoreAccountWithId " + mAccountID);
        } else {
            Logging.d(TAG, "Normal Setup or SetupData Account is OK  ");
        }
        return account;
    }
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (getArguments() != null) {
            ...
            mAccountID = getArguments().getLong(BUNDLE_KEY_SETTINGS_ACCOUNTID, -1L);
            Logging.d(TAG, "onCreate from Setting CurrentAccountID = " + mAccountID);
        }
        // Restore accountID on Orientation changed
        if (savedInstanceState != null && mAccountID == -1L) {
            mAccountID = savedInstanceState.getLong(BUNDLE_KEY_SETTINGS_ACCOUNTID, -1L);
            Logging.d(TAG, "onCreate from savedInstanceState CurrentAccountID = " + mAccountID);
        }
    }
    ...
    public void onSaveInstanceState(Bundle outState) {
        ...
        outState.putLong(BUNDLE_KEY_SETTINGS_ACCOUNTID, mAccountID);
    }
     ...
     */
    protected void startDuplicateTaskCheck(long accountId, String checkProtocol, String checkLogin,
            String serverAddr, int checkSettingsMode) {
        new DuplicateCheckTask(accountId, checkProtocol, checkLogin, serverAddr, checkSettingsMode)
    ...
    private class DuplicateCheckTask extends AsyncTask<Void, Void, Account> {
        ...
        public DuplicateCheckTask(long accountId, String checkProtocol, String checkLogin,
                String serverAddr, int checkSettingsMode) {
        ...
        new AsyncTask<Void, Void, Void>() {
            ...
            protected Void doInBackground(Void... params) {
                if (settingsResult == AccountCheckSettingsFragment.CHECK_SETTINGS_OK) {
                    int mode = SetupData.getFlowMode();
                    if (mode == SetupData.FLOW_MODE_EDIT
                            || mode == SetupData.FLOW_MODE_CHANGE_PASSWORD) {
