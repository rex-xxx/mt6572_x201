...
public class AccountSettings extends PreferenceActivity {
    ...
    // This boolean variable is for the purpose: 
    // Do not enable the menu "add account" before
    // LoadAccountListTask.onPostExecute->invalidateHeaders()
    private boolean mFinishedLoadAccount;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (savedInstanceState == null) {
            ...
            if (loginWarningAccount != null) {
                ...
                LoginWarningDialog dialog = LoginWarningDialog.newInstance(loginWarningAccount,
                        mRequestedAccountId);
                dialog.setCallback(new LoginFailedCallback() {
                    @Override
                    public void onChangePwd(long accountId) {
                        AccountSettings.this.onChangePwd(accountId);
                    }
                });
        ...
        if (title != null) {
            ...
            // launch of a specific settings screen.
            setTitle(R.string.settings_action);
        }
    ...
    public void onResume() {
        super.onResume();
        mIsBackGround = false;
        if (!onIsMultiPane() || mFinishedLoadAccount) {
            invalidateHeaders();
        }
    ...
    public void onBackPressed() {
        ...
        /** M: Stop the oof operation when backKey pressed @{ */
        if (mCurrentFragment instanceof AccountSettingsFragment) {
            Controller controller = Controller.getInstance(this);
            controller.stopOof(mRequestedAccountId);
        }
        /** @} */
        super.onBackPressed();
    }
    ...
    private void enableDebugMenu() {
        if (AccountSetupBasics.ENTER_DEBUG_SCREEN) {
            mShowDebugMenu = true;
            invalidateHeaders();
        }
    }
    ...
    private class LoadAccountListTask extends AsyncTask<Long, Void, Object[]> {
        ...
        protected void onPostExecute(Object[] result) {
            ...
            mFinishedLoadAccount = true;
            invalidateOptionsMenu();
            if (!deletingAccountFound) {
                mDeletingAccountId = -1;
            }
            // Go to account setup if no account
            if (headers == null || headers.length <= 0) {
                ...
                /** M: Switch to AccountSetupBasics only when AccountSettings activity is in
                 * the foreground state. @{ **/
                Utility.getMainThreadHandler().post(new Runnable() {
                    public void run() {
                        if (!mIsBackGround) {
                            Logging.d("There are no account left, go to account setup!");
                            SetupData.init(SetupData.FLOW_MODE_NORMAL);
                            Intent intent = new Intent(AccountSettings.this,
    ...
    private class AccountServerSettingsFragmentCallback
        ...
        public void onProceedNext(int checkMode, AccountServerBaseFragment target) {
            ...
            startCheckingFragment(checkMode, target);
        }

        private void startCheckingFragment(int checkMode, AccountServerBaseFragment target){
            FragmentManager fm = getFragmentManager();
            // Just return because another FragmentTransaction has been invoked just now
            if (fm.getBackStackEntryCount() > 0
                    && fm.findFragmentByTag(AccountCheckSettingsFragment.TAG) != null) {
                return;
            }
            AccountCheckSettingsFragment checkerFragment =
                AccountCheckSettingsFragment.newInstance(checkMode, target);
    ...
    public void onIncomingSettings(Account account) {
        try {
            ...
            Store store = Store.getInstance(account, getApplication(), false);
            if (store != null) {
                ...
                if (setting != null) {
                    ...
                    if (setting.equals(AccountSetupIncoming.class)) {
                        startPreferencePanel(AccountSetupIncomingFragment.class.getName(),
                                AccountSetupIncomingFragment.getSettingsModeArgs(account.mId),
                        ...
                        startPreferencePanel(AccountSetupExchangeFragment.class.getName(),
                                AccountSetupExchangeFragment.getSettingsModeArgs(account.mId),
                                R.string.account_settings_incoming_label, null, null, 0);
                    }
                }
            }
    ...
    public void onChangePwd(long accountId) {
        try {
            ...
            nc.cancelLoginFailedNotification(accountId);
            ...
            if (store != null) {
                ...
                if (setting != null) {
                    if (setting.equals(AccountSetupIncoming.class)) {
                        AccountSetupIncoming.actionIncomingSettings(this,
                                SetupData.FLOW_MODE_CHANGE_PASSWORD, account);
                    } else if (setting.equals(AccountSetupExchange.class)) {
                        SetupData.init(SetupData.FLOW_MODE_EDIT, account);
                        startPreferencePanel(AccountSetupExchangeFragment.class.getName(),
                                AccountSetupExchangeFragment.getSettingsModeArgs(account.mId),
                ...
                if (setting != null) {
                    ...
                    if (setting.equals(AccountSetupOutgoing.class)) {
                        startPreferencePanel(AccountSetupOutgoingFragment.class.getName(),
                                AccountSetupOutgoingFragment.getSettingsModeArgs(account.mId),
     ...
     */
    public void deleteAccount(Account account) {
        mProgressDialog = WaitingRemoveAccountDialog.newInstance();
        mProgressDialog.setCancelable(false);
        mProgressDialog.show(getFragmentManager(), WaitingRemoveAccountDialog.TAG);
        // Kick off the work to actually delete the account
        // Delete the account (note, this is async.  Would be nice to get a callback.
        Controller.getInstance(this).deleteAccount(account.mId, new RemoveAccountCallback());
    }
    
    /**
     * The call back of dismiss the WaitingRemoveAccountDialog.
     */
    public class RemoveAccountCallback {
        public void finishRemoveAccount(final long accountId) {
            AccountSettings.this.runOnUiThread(new Runnable() {
                public void run() {
                    ...
                    if (onIsMultiPane()) {
                        // Not need switch the header when activity is finishing now.
                        if(AccountSettings.this.isFinishing()){
                            Logging.d(TAG,
                                    "no more account, not switch the header just finsh itself.");
                            return;
                        }
                        Header prefsHeader = getAppPreferencesHeader();
                        AccountSettings.this.switchToHeader(prefsHeader.fragment,
                                prefsHeader.fragmentArguments);
                        mDeletingAccountId = accountId;
                        updateAccounts();
                    } else {
                        // We should only be calling this while showing AccountSettingsFragment,
                        // so a finish() should bring us back to headers.
                        // No point hiding the deleted account.
                        finish();
                    }
                }
            });
     ...
     */
    /* package */ public static class UnsavedChangesDialogFragment extends DialogFragment {
    ...
    /**
     * Dialog waiting remove account.
     */
    public static class WaitingRemoveAccountDialog extends DialogFragment {
        public static final String TAG = "WaitingRemoveDialog";

        public static WaitingRemoveAccountDialog newInstance() {
            final WaitingRemoveAccountDialog dialog = new WaitingRemoveAccountDialog();
            return dialog;
        }
        ...
        public Dialog onCreateDialog(Bundle savedInstanceState) {
            ...
            progressDialog.setMessage(context
                    .getString(R.string.account_settings_delete_account_label)
                    + "...");
    ...
    public static class LoginWarningDialog extends DialogFragment
        ...
        private LoginFailedCallback mLoginFailedCallback;

        /**
         * Create a new dialog.
         */
        public static LoginWarningDialog newInstance(String accountName, long accountId) {
            final LoginWarningDialog dialog = new LoginWarningDialog();
            Bundle b = new Bundle();
            b.putString(BUNDLE_KEY_ACCOUNT_NAME, accountName);
            b.putLong(ACCOUNT_ID, accountId);
            dialog.setArguments(b);
            return dialog;
        }

        public void setCallback(LoginFailedCallback callback) {
            mLoginFailedCallback = callback;
        }
        ...
        public void onClick(DialogInterface dialog, int which) {
            ...
            if (which == DialogInterface.BUTTON_NEGATIVE) {
                ...
                mLoginFailedCallback.onChangePwd(getArguments().getLong(ACCOUNT_ID));
            }
        }
    }

    private interface LoginFailedCallback {
        
        void onChangePwd(long accountId);
