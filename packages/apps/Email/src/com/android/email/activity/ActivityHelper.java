...
public final class ActivityHelper {
    ...
    public static void openCalendar(Activity activity, long epochEventStartTime) {
        ...
        UiUtilities.startRemoteActivity(activity, intent, true);
    }

    public static void deleteMessage(Context context, long messageId) {
        Controller.getInstance(context).deleteMessage(messageId);
        // Use short toast instead of long.
        Utility.showToastShortTime(context,
    ...
    public static void moveMessages(final Context context, final long newMailboxId,
        ...
        EmailAsyncTask.runAsyncSerial(new Runnable() {
            @Override
            public void run() {
                /// M: Get display name of mailbox to support i18n
                Mailbox newMailbox = Mailbox.restoreMailboxWithId(context, newMailboxId);
                String mailboxName = FolderProperties.getInstance(context).getDisplayName(newMailbox);
                if (mailboxName == null) {
                    return; // Mailbox gone??
                }
     ...
     *
     * M: This method has been deprecated. And the acceleration was decided in the manifest.
     */
