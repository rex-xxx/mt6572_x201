...
public class EmailActivity extends Activity implements View.OnClickListener, FragmentInstallable {
    /// M: add this action for email remote search instead of system Intent.ACTION_SEARCH.
    public static final String ACTION_REMOTE_SEARCH = "com.android.email.action.REMOTE_SEARCH";
    /// M: add local search action.
    public static final String ACTION_LOCAL_SEARCH = "com.android.email.action.LOCAL_SEARCH";
    ...
    public static final String EXTRA_QUERY_FIELD = "QUERY_FIELD";
    ...
    // Id of the account being recorded
    private long mRecordAccountId = Account.NO_ACCOUNT;
    ...
    /**
     * M : Create an intent to launch all account's inbox local search activity.
     *
     ...
     */
    public static Intent createLocalSearchIntent(Activity fromActivity, long accountId,
            long mailboxId, String query, String field) {
        ...
        i.putExtra(EXTRA_QUERY_FIELD, field);
        i.setAction(ACTION_LOCAL_SEARCH);
        return i;
    }

    /**
     * Create an intent to launch remote search activity.
     *
     * @param accountId ID of the account for the mailbox.  Must not be {@link Account#NO_ACCOUNT}.
     * @param mailboxId ID of the mailbox to search, or {@link Mailbox#NO_MAILBOX} to perform
     *     global search.
     * @param query query string.
     */
    public static Intent createRemoteSearchIntent(Activity fromActivity, long accountId,
            long mailboxId, String query, String field) {
        ...
        i.putExtra(EXTRA_QUERY_STRING, query);
        i.putExtra(EXTRA_QUERY_FIELD, field);
        i.setAction(ACTION_REMOTE_SEARCH);
        return i;
    }

    /**
     * Initialize {@link #mUIController}.
     * TODO: Check Local Search
     */
    private void initUIController() {
        if (UiUtilities.useTwoPane(this)) {
            if (getIntent().getAction() != null
                    && ACTION_REMOTE_SEARCH.equals(getIntent().getAction())
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // clear static instance to reload application resources.
        FolderProperties.removeInstance();
    ...
    private void initFromIntent() {
        ...
        /// M: Get the account Id for recording
        mRecordAccountId = intent.getLongExtra(EXTRA_ACCOUNT_ID, Account.NO_ACCOUNT);
    ...
    protected void onResume() {
        ...
        DataCollectUtils.startRecord(this, mRecordAccountId, sRecordOpening);
        sRecordOpening = true;
        ...
        /** M: start local search @{ */
        Intent i = getIntent();
        if (i != null && ACTION_LOCAL_SEARCH.equals(i.getAction())) {
            mUIController.onSearchRequested(getIntent().getStringExtra(EXTRA_QUERY_STRING));
            // Clean the search action to avoid re-search after rotating phone.
            i.setAction("");
            setIntent(i);
        }
        /** @} */
    ...
    protected void onPause() {
        ...
        if (sRecordOpening) {
            Intent i = getIntent();
            if (i == null || !ACTION_REMOTE_SEARCH.equalsIgnoreCase(i.getAction())) {
    ...
    public boolean onSearchRequested() {
        ...
        mUIController.onSearchRequested(null);
    ...
    /**
     * When EmailAsyncTask is busy, should call this function to show a dialog to let user don't
     * do action too frequently.
     */
    ...
    static class WaitingDialog extends DialogFragment {
    ...
    /**
     * M: Set account which will be recorded
     * @param accountId
     */
    public void setRecordAccount(long accountId) {
        mRecordAccountId = accountId;
    }
}
