...
public class MessageListFragment extends ListFragment implements
        ...
        MoveMessageToDialog.Callback, DeleteMessageConfirmationDialog.Callback,
    ...
    private static final int LIST_FOOTER_MODE_LOCAL_SEARCH = 2;
    ...
    /** Indicate whether we are waiting for the first load */
    private boolean mWaitLoading = false;

    /** For FPS test log*/
    private boolean mIsPerformanceLogged = true;
    
    /** M: Support for message local search @{ */
    private MessageListContext mLocalSearchListContext = null;
    private LocalSearchCallback mLocalSearchCallback = null;
    private RemoteSearchCallback mRemoteSearchCallback = null;
    private boolean mPendingLocalSearch = false;
    private String mPendingQueryTerm;
    private String mPendingQueryField;
    /** @} */

    /** M: Record if the the loader be restarted @{ */
    private boolean mHasRestartLoader = false;
    /** @} */
    ...
    public interface Callback {
         ...
         */
        void onMessageOpen(long messageId, long messageMailboxId, long listMailboxId,
                int type);
        ...
        /**
         * M: when load finish, set the {@link UIControllerBase}mIsEasRemoteMode.
         * @param isEasRemoteMode
         */
        void onMessagesLoadFinish(boolean isEasRemoteMode);
    }
    ...
    private static final class EmptyCallback implements Callback {
        ...
        /**
         * M: onMessagesLoadFinish.
         */
        @Override
        public void onMessagesLoadFinish(boolean isEasRemoteMode) {
        }
    ...
    public static void clearDeletedSet(Set<Long> messageIds) {
        synchronized (MessagesAdapter.sDeletedSet) {
            if (MessagesAdapter.sDeletedSet.size() > 0) {
                MessagesAdapter.sDeletedSet.removeAll(messageIds);
            }
        }
    }
    ...
    private void initializeArgCache() {
        ...
        Logging.d(TAG, " initializeArgCache mListContext " + mListContext);
    }
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
         ...
         * and The below two values are just experience values. {@ */
        lv.setVelocityScale(0.4f);
        lv.setFriction(0.005f);
        /** @} */

        mListFooterView = getActivity().getLayoutInflater().inflate(
                R.layout.message_list_item_footer, lv, false);
        /** M: Get the views before use it to avoid NullPointException. @{*/
    ...
    public void onDestroy() {
        ...
        finishSelectionMode();
        /** M: restartLoader would re-create a new loader in LoaderManager.
         * If the loader created too frequently, the new loader will be pending.
         * The pending loader may be re-used in the new fragment while the device rotated.
         * But it may cause crash because the pending loader associated with
         * LoaderCallbacks of previous Fragment. So, if we has restarted the loader
         *  and the the device rotated, we should destroy the loader. @{ */
        if (mHasRestartLoader && getActivity().isChangingConfigurations()) {
            getLoaderManager().destroyLoader(LOADER_ID_MESSAGES_LOADER);
        }
        /** @} */
    ...
    public boolean onTouch(View v, MotionEvent event) {
        if (event.getAction() == MotionEvent.ACTION_DOWN) {
            ...
            /// M: hide the soft keyboard while touch local search mail list
            if (((EmailActivity)mActivity).getUIController()
                    .mActionBarController.isLocalSearchMode()) {
                hideSoftKeyboard();
            }
        }
    ...
    public boolean onItemLongClick(AdapterView<?> parent, View view, int position, long id) {
        ///M: if mMailbox is a search box, cancel this calling because don't support some
        // function in Eas remote search mode, like delete message, mark as unread and so on. @{
        if (mListAdapter.mIsEasRemoteMode) {
            return false;
        }
        /// @}
        if (view != mListFooterView) {
    ...
    private void onLoadMoreMessages() {
        ...
        if (!Utility.hasConnectivity(mActivity)) {
            ///M: stop refresh since network unavailable. recovery to 'Load more'
            ///if currently is refreshing @{
            Logging.d("Stop refresh since network unavailable");
            mRefreshManager.stopRefresh(accountId, mailBoxId);
            ///@}
            ConnectionAlertDialog.newInstance().show(
                    mActivity.getFragmentManager(), "connectionalertdialog");
            return;
        }

        if (mIsRefreshable) {
            mRefreshManager.loadMoreMessages(accountId, mailBoxId);
        }
    }
    ...
    private void toggleRead(Set<Long> selectedSet) {
        toggleMultiple(selectedSet, new MultiToggleHelper() {
            ...
            @Override
            public void setFields(ArrayList<Long> messageIds, boolean newValue) {
                mController.setMessageReadAsync(messageIds, !newValue);
            }
        });
    }
    ...
    private void toggleFavorite(Set<Long> selectedSet) {
        toggleMultiple(selectedSet, new MultiToggleHelper() {
            ...
            @Override
            public void setFields(ArrayList<Long> messageIds, boolean newValue) {
                mController.setMessageFavoriteAsync(messageIds, newValue);
            }
        });
    }

    private void deleteMessages(final Set<Long> selectedSet) {
        final HashSet<Long> messageIds = new HashSet<Long>(selectedSet);
        if (selectedSet == null || selectedSet.size() == 0) {
            throw new IllegalArgumentException();
        }

        mController.deleteMessages(messageIds);
        mListAdapter.updateDeletedMessageList(messageIds);

        Toast.makeText(mActivity, mActivity.getResources().getQuantityString(
                R.plurals.message_deleted_toast, selectedSet.size()), Toast.LENGTH_SHORT).show();
        selectedSet.clear();
        updateSelectionMode();
    }
    ...
    private void toggleMultiple(final Set<Long> selectedSet, final MultiToggleHelper helper) {
        ...
        if (!setValues.isEmpty()) {
            ...
            EmailAsyncTask.runAsyncParallel(new Runnable() {
               @Override
                public void run() {
                   EmailAsyncTask.printStartLog("MessageListFragment#toggleMultiple ==== start at ["
                           + System.currentTimeMillis() + "]");
                   ArrayList<Long> messageIds = new ArrayList<Long>();
                   for (long id : messageResort) {
                       if (setValues.get(id) != newValue) {
                           messageIds.add(id);
                       }
                   }
                   helper.setFields(messageIds, newValue);
                   EmailAsyncTask.printStopLog("MessageListFragment#toggleMultiple ==== end at ["
                           + System.currentTimeMillis() + "]");
                }});
        }
        // Clear selected status here to return to Non-select mode
        selectedSet.clear();
        updateSelectionMode();
    }
    ...
    private void autoRefreshStaleMailbox() {
        ...
        if (null != mMailbox && mMailbox.mType == Mailbox.TYPE_SEARCH) {
            return;
        }
        // If it's not the first load, we need to judge the switch "sync Email" in 
        // account settings, this switch is consistent with the option "sync Email"
        // in phone "account & sync" of this account.
        if (!mIsFirstLoad && mAccount != null) {
            android.accounts.Account acct = new android.accounts.Account(mAccount.mEmailAddress,
                    mAccount.isEasAccount(mActivity) ? AccountManagerTypes.TYPE_EXCHANGE
                            : AccountManagerTypes.TYPE_POP_IMAP);
            if (!ContentResolver.getSyncAutomatically(acct, EmailContent.AUTHORITY)) {
                return;
            }
        }
        onRefresh(false);
    ...
    public void onAdapterFavoriteChanged(MessageListItem itemView, boolean newFavorite) {
        MessageListContext messageListContext = getListContext();
        if (messageListContext != null && 
                messageListContext.getMailboxId() == Mailbox.QUERY_ALL_FAVORITES) {
            if (mListAdapter.isSelected(itemView)) {
                mListAdapter.toggleSelected(itemView);
            }
        }
    ...
    private void updateSearchHeader(Cursor cursor) {
        /// M: update the local search result count on UI @{
        if (mLocalSearchListContext != null && mLocalSearchCallback != null
                && cursor != null) {
            mLocalSearchCallback.onLocalSearchFinished(cursor.getCount());
            return;
        }
        /// @}

        MessageListContext listContext = getListContext();
        if (!listContext.isRemoteSearch() || cursor == null) {
            UiUtilities.setVisibilitySafe(mSearchHeader, View.GONE);
            return;
        }

        /// M: update the remote search result count on UI @{
        SearchResultsCursor searchCursor = (SearchResultsCursor) cursor;
        int resultCount = searchCursor.getResultsCount();
        if(null != mRemoteSearchCallback && resultCount >=0){
            mRemoteSearchCallback.onRemoteSearchFinished(resultCount);
        }
        /// @}
        ...
        if (resultCount < 0) {
            ...
            mSearchHeaderText.setText(R.string.remote_search_failed);
        }
    }
    ...
    private int determineFooterMode() {
        ...
        if (null != mLocalSearchListContext) {
            return LIST_FOOTER_MODE_LOCAL_SEARCH;
        }
        ...
        Logging.d(TAG, "determineFooterMode result " + result);
        return result;
    }
    ...
    private void updateFooterView() {
        ...
        // TODO: The default workflow should be check since add local search and remote search.
        if (mListFooterMode == mode) {
            Logging.d(TAG, "updateFooterView mListFooterMode already is " + mListFooterMode);
            return;
        }
        mListFooterMode = mode;
        ListView lv = getListView();
        if (mListFooterMode != LIST_FOOTER_MODE_NONE ) {
            // M: Should erasure "load more" firstly then add "Search on server".
            // @{
            lv.removeFooterView(mListFooterView); 
            // @}
            
            // Not show "Search on Server" if account not support remote search
            // TODO: To avoid ANR, better move 'supportRemoteSearch' to background.
            if(mListFooterMode == LIST_FOOTER_MODE_MORE
                    || (mListFooterMode == LIST_FOOTER_MODE_LOCAL_SEARCH && supportRemoteSearch())){
    ...
    private void updateListFooter() {
        if (mListFooterMode != LIST_FOOTER_MODE_NONE) {
            Logging.d(TAG, "updateListFooter mListFooterMode " + mListFooterMode);
            int footerTextId = 0;
            switch (mListFooterMode) {
                case LIST_FOOTER_MODE_MORE:
                    ...
                    break;
                case LIST_FOOTER_MODE_LOCAL_SEARCH:
                    footerTextId = mIsEasAccount
                            ? R.string.searching_all_fields_on_sever : R.string.searching_on_server;
    ...
    /**
     * M: The entrance of remote search function.
     * It will start a new activity to perform remote search.
     */
    private void startRemoteSearch(){
        // 1) Check if low storage at the very beginning
        if (RefreshManager.getInstance(mActivity).checkIsLowStorage(mActivity)) {
            return;
        }
        ...
        // 2) Check Account and MailBox
        final long accountId = getAccountId();
        if (!Account.isNormalAccount(accountId)) {
            return; // Invalid account to search from.
        }

        Mailbox searchableMailbox = getMailbox();
        if (searchableMailbox == null) {
            return;
        }

        // 3) Rebuild a new search params.
        final long mailboxId = searchableMailbox.mId;
        SearchParams searchParams = mLocalSearchListContext.getSearchParams();
        if(null == searchParams || null == searchParams.mFilter 
                || null == searchParams.mField){
            Logging.e(TAG, "Can't do remote search for the searchParams is null");
            return;
        }
        String queryTerm = searchParams.mFilter;
        String queryField = mIsEasAccount ? SearchParams.SEARCH_FIELD_ALL : searchParams.mField;
        Logging.d(TAG, "Submitting search: [" + queryTerm + "]" +
                       "Fileds "+ queryField +" in mailboxId = " + mailboxId);

        //4) Start remote search activity.
        mActivity.startActivity(EmailActivity.createRemoteSearchIntent(
                mActivity, accountId, mailboxId, queryTerm, queryField));
    }
    ...
    /**
     * M: if the current mailbox is OutBox, the send menu should shown dynamically.
     *    e.g. if all the message has send successfully, the send menu should be invisible.
     */
    private void updateSendMenu() {
        ...
        showSendCommand(isOutbox && (mListAdapter != null)
    ...
    private void adjustMessageNotification(boolean updateLastSeenKey) {
        final long accountId = getAccountId();
        final long mailboxId = getMailboxId();
        /** M: If we were in the VIP folder, update the last seen key too. @{ */
        if (mailboxId == Mailbox.QUERY_ALL_INBOXES || mailboxId > 0
                || mailboxId == Mailbox.QUERY_ALL_VIPS) {
    ...
    private final LoaderManager.LoaderCallbacks<Cursor> LOADER_CALLBACKS =
        ...
        public Loader<Cursor> onCreateLoader(int id, Bundle args) {
            ...
            /// M: Create the loader for local search @{
            if (mLocalSearchListContext != null) {
                return MessagesAdapter.createLoader(getActivity(), mLocalSearchListContext);
            }
            /// @}
            ...
            if (mListContext.isRemoteSearch()) {
        ...
        public void onLoadFinished(Loader<Cursor> loader, Cursor c) {
            ...
            /// M: always swapCursor to get the latest data. @{
            mListAdapter.swapCursor(cursor);
            /// @}

            /// M: update the refresh icon after remote search finished @{
            if (mListContext.isRemoteSearch()) {
                UIControllerBase uiController = ((EmailActivity)mActivity).getUIController();
                if (uiController != null) {
                    uiController.mIsRemoteSearching = false;
                    uiController.updateRefreshIcon();
                }
            }
            /// @}

            //M: Need check if the loaded cursor is null @{
            if (null == cursor) {
                Logging.w(TAG, "MessageList onLoadFinished MessagesCursor is null");
                return ;
            }
            // @}

            /** M: return early before swapCursor. @{ */
            if (!cursor.mIsFound) {
            ...
            if((getListAdapter() != null) && cursor.getCount() > 0) {
                if (Logging.LOG_PERFORMANCE) {
                    Logging.d(Logging.LOG_TAG,
                            "[Exchange Download Speed] MessageList:ShowMessages ["
                            + System.currentTimeMillis() + "]");
                }
            }

            if (!mListAdapter.getSelectedSet().isEmpty()
                    && cursor.getCount() > 0) {
                HashSet<Long> messageIds = new HashSet<Long>();
                cursor.moveToPosition(-1);
                while (cursor.moveToNext()) {
                    messageIds.add(cursor.getLong(MessagesAdapter.COLUMN_ID));
                }
                Long[] selected = mListAdapter.getSelectedSet().toArray(new Long[]{});
                for (long selectedId : selected) {
                    if (!messageIds.contains(selectedId)) {
                        mListAdapter.getSelectedSet().remove(selectedId);
                    }
                }
            } else {
                mListAdapter.clearSelection();
            ...
            /// M: when load finish, set the IsEasRemoteMode flag of every file. @{
            mListAdapter.mIsEasRemoteMode = mIsEasAccount && mListContext.isRemoteSearch();
            mCallback.onMessagesLoadFinish(mListAdapter.mIsEasRemoteMode);
            /// @}
            ...
            if (mIsFirstLoad) {
                if (mMailbox != null && mMailbox.mType == Mailbox.TYPE_SEARCH) {
                    mListAdapter.setQuery(getListContext().getSearchParams().mFilter, 
                            getListContext().getSearchParams().mField);
                    mSearchedMailbox = ((SearchResultsCursor) c).getSearchedMailbox();
                } else {
                    mListAdapter.setQuery(null, null);
            ...
            /// M: always update the mailbox specific action. @{
            updateSendMenu();
            /// @}
            ...
            if (mIsFirstLoad) {
                ...
                if (!isEmptyAndLoading(cursor)) {
                    ...
                    if (Logging.LOG_PERFORMANCE) {
                        Logging.d(Logging.LOG_TAG,
                                "[Exchange Download Speed] MessageList:ShowLoading ["
                                + System.currentTimeMillis() + "]");
                    }
                }
                ...
                if (Logging.LOG_PERFORMANCE) {
                    Logging.d(Logging.LOG_TAG,
                            "[Exchange Download Speed] MessageList:FisrtShowMessages ["
                            + System.currentTimeMillis() + "]");
                }
            }
            ...
            if (mIsPerformanceLogged) {
                Logging.d("[Performance test][Email] MessageList FPS test end time ["
                        + System.currentTimeMillis() + "]");
                mIsPerformanceLogged = false;
            }

            mIsFirstLoad = false;

            /** M: this is just for the screen rotation with not-empty 
                search terms in the search view @{ */
            if (mPendingLocalSearch) {
                Logging.d(TAG, "startLocalSearch by onLoadFinished mPendingLocalSearch");
                startLocalSearch(mPendingQueryTerm, mPendingQueryField, mLocalSearchCallback);
            }
            /** @} */
        }
        ...
        private boolean isEmptyAndLoading(Cursor cursor) {
            if (cursor == null || mRefreshManager == null || mMailbox == null) {
                Logging.e(TAG, "isEmptyAndLoading have null parameter " + cursor + ":"
                        + mRefreshManager + ":" + mMailbox);
                return false;
            }

            boolean b = false;
            try {
                b = mRefreshManager.isMessageListRefreshing(mMailbox.mId);
            } catch (NullPointerException e) {
                Logging.e(TAG, "isEmptyAndLoading throw out NPE with mailbox id " + mMailbox.mId);
            }
            return (cursor.getCount() == 0) && b;
        }
    ...
    private class SelectionModeCallback implements ActionMode.Callback {
        ...
        /**
         * M: Initialize the selection menu to support select all
         * @param actionMode the action mode of the action bar
         */
        private void startSelectionMode(final ActionMode actionMode) {
            ...
            mSelectionMenu = new PopupMenu(activity, mSelectionButton);
        ...
        /**
         * M: Update the selection menu
         */
        private void updateSelectionMenu() {
            // update title
            int num = getSelectedCount();
            // Set title -- "# selected"
            mSelectionButton.setText(getActivity().getResources().getQuantityString(
        ...
        public boolean onActionItemClicked(ActionMode mode, MenuItem item) {
            ...
            switch (item.getItemId()) {
                ...
                case R.id.remove_star:
                    ...
                    break;
                case R.id.delete:
                    /// M: Popup confirm dialog if needed,when do delete operation. @{
                    showConfirmIfNeeded(selectedConversations);
                    /// @}
                    break;
                case R.id.move:
                    showMoveMessagesDialog(selectedConversations);
                    break;
                /** M: Deal with the select all menu @{ */
    ...
    private class RefreshListener implements RefreshManager.Listener {
        ...
        //Only for IMAP account.
        @Override
        public void onStopRefreshIcon(long accountId, long mailboxId) {
        }
        ...
        public void onRefreshStatusChanged(long accountId, long mailboxId) {
            updateListFooter();
            if (mailboxId == -1) {
                Logging.d("MessagelistFragment:onRefreshStatusChanged(), "
                        + "mailboxId = -1, so we just return");
                return;
            }
            // In case of retrieving no mails during the first load, the message content observer
            // of MessagesCursorLoader(CursorLoader) would not detect any changes, thus no another 
            // force load will do and then the routine onLoadFinished in the loader callbacks would
            // not be invoked that result in missing call setListAdapter() at last. 
            if (mWaitLoading && !mRefreshManager.isMessageListRefreshing(mailboxId) 
                    && getListAdapter() == null) {
                if (mMailbox != null && mMailbox.mId == mailboxId) {
                    mWaitLoading = false;
                    Log.d(Logging.LOG_TAG,
                            "MessageListFragment: onRefreshStatusChanged: setListAdapter : count = "
                                    + mListAdapter.getCount());
                    new CheckMessageCountTask(accountId, mailboxId)
                            .cancelPreviousAndExecuteParallel();
                }
            }
        }
    }
    private class CheckMessageCountTask extends EmailAsyncTask<Void, Void, Long> {
         ...
         */
        public CheckMessageCountTask(long accountId, long mailboxId) {
            super(mTaskTracker);
        ...
        protected Long doInBackground(Void... params) {
            long messageCount;
            // Check if there was really no message.
            Cursor c = mActivity.getContentResolver().query(
                    Message.CONTENT_URI,
                    new String[] { MessageColumns.ID },
                    Message.buildMessageListSelection(mActivity, mAccountId,
                            mMailboxId), null, EmailContent.MessageColumns.TIMESTAMP + " DESC");
            if (c != null) {
                try {
        ...
        @Override
        protected void onSuccess(Long messageCount) {
            // If messageCount == 0, we display no message, or we waiting for
            // onLoadFinished setListAdapter;
            if (messageCount == 0) {
                setListAdapter(mListAdapter);
            }
        }
    ...
    /**
     * M: To check is the local search request same as previous
     * @param queryTerm the local search term
     * @param queryField the local search field
     * @return true if the local search request same as previous
     */
    private boolean isSearchRequestSameAsPrevious(String queryTerm, String queryField) {
        if (TextUtilities.stringOrNullEquals(mPendingQueryField, queryField) &&
                TextUtilities.stringOrNullEquals(mPendingQueryTerm, queryTerm)) {
            return true;
        }
        return false;
    }
    ...
    /**
     * M: try to start LocalSearchCursorLoader to do a local search,
     *    in some special cases, we just ought to reset the result count UI
     *    or pending the search request for executing later
     */
    public void startLocalSearch(final String queryTerm, final String queryField,
            LocalSearchCallback cb) {
         /** M: If the search request same as previous and it is not pending local search,
         *  just return to avoid duplicate local search.
         *  This case would happen when rotate device. @{
         */
        if (isSearchRequestSameAsPrevious(queryTerm, queryField) && !mPendingLocalSearch) {
            Logging.d(TAG, this + " startLocalSearch same as privous just return");
            //M: If the mLocalSearchListContext is not null, update the MessageListContext
            if (mLocalSearchListContext != null) {
                cb.onMessageListContextChanged(mLocalSearchListContext);
            }
            return;
        }

        if (mPendingLocalSearch) {
            mPendingLocalSearch = false;
        }
        // Store the queryTerm and queryField
        mPendingQueryTerm = queryTerm;
        mPendingQueryField = queryField;
        /** @} */
        if (cb != null) {
            mLocalSearchCallback = cb;
        }
        // Reset the UI result count to 0 after user clearing
        // the search terms in the search view. And we should
        // to restart the loader to restore the full message data.
        if (queryTerm == null || TextUtils.isEmpty(queryTerm.trim())) {
            mLocalSearchListContext = null;
            if (mLocalSearchCallback != null) {
                cb.onLocalSearchFinished(0);
            }
            /// M: if queryTerm is null, update the local search ListContext
            cb.onMessageListContextChanged(mListContext);
            Logging.d(TAG, "start local Search.with null queryTerm.");
        } else if (mListAdapter.getCursor() != null) {   // about to start the local search
            SearchParams params = new SearchParams(mListContext.getMailboxId(), queryTerm, queryField, true);
            mLocalSearchListContext = MessageListContext.forSearch(mListContext.mAccountId,
                    mListContext.getMailboxId(), params);
            cb.onMessageListContextChanged(mLocalSearchListContext);
            Logging.d(TAG, " startLocalSearch mListContext " + mLocalSearchListContext);
        } else {
            // rotate the device with not-empty search terms, the mListAdapter will be null at the
            // moment since the messages loading by MessageCursorLoader has not finished yet
            Logging.d(TAG, " start pending local Search..");
            mPendingLocalSearch = true;
            return;
        }

        // for highlighting the queryTerm in the content of the queryField 
        mListAdapter.setQuery(queryTerm, queryField);
        final LoaderManager lm = getLoaderManager();
        lm.restartLoader(LOADER_ID_MESSAGES_LOADER, null, LOADER_CALLBACKS);
        /** M: Record that the loader has been restarted, so we can destroy it in onDestroy() @{ */
        mHasRestartLoader = true;
        /** @} */
    }

    public interface LocalSearchCallback {
        public void onLocalSearchFinished(int count);
        public void onMessageListContextChanged(MessageListContext newMessageListContext);
    }

    /**
     * TODO: Integrate local search and remote search in a common interface, can we do it ?
     */
    public interface RemoteSearchCallback {
        public void onRemoteSearchFinished(int count);
    }

    public void setRemoteSearchCallBack(RemoteSearchCallback remoteSearchCallback) {
        mRemoteSearchCallback = remoteSearchCallback;
    }

    /**
     * M: check the current account and mailbox is searchable. 
     */
    private boolean supportRemoteSearch() {
        boolean accountSearchable = false;
        Account account = getAccount();
        if (null == account) {
            long accountId = getAccountId();
            if (accountId == Account.ACCOUNT_ID_COMBINED_VIEW) {
                // Not do remote search in account Combined view
                return false;
            }
            if (accountId > 0) {
                account = Account.restoreAccountWithId(mActivity, accountId);
            }
        }
        accountSearchable = (null != account)
        ...
        if (null == mailbox) {
            long mailBoxId = getMailboxId();
            if (mailBoxId > 0) {
                mailbox = Mailbox.restoreMailboxWithId(mActivity, mailBoxId);
            }
        }
        mailboxHasServerCounterpart = (mailbox != null) && mailbox.loadsFromServer(protocol);
        return mailboxHasServerCounterpart && accountSearchable;
    }

    // M: workaround for not show footer when the listview is empty.
    // set the emptyview of listview as null. 
    public void enterLocalSearchMode() {
        // Set EmptyText and EmptyView null, so that when the local search
        // number is 0, the footer view "Search on Server" can be shown.
        // By default the footer view only show when the listview count is not 0.
        setEmptyText(null);
        getListView().setEmptyView(null);
    }

    // M: workaround for not show footer when the listview is empty.
    // recovery the emptyview of listview.
    public void exitLocalSearchMode() {
    ...
    /// M: Support to run testcase.
    MessagesAdapter getMessagesAdapter() {
        return mListAdapter;
    }

    /// M: add ask before deleting new feature, show confirm dialog if needed.
    private void showConfirmIfNeeded(Set<Long> selected) {
        boolean askBeforeDeleting = Preferences.getPreferences(mActivity)
                .isAskBeforeDelete();
        if (askBeforeDeleting) {
            DeleteMessageConfirmationDialog.newInstance(selected.size(), this)
                    .show(getFragmentManager(),
                            DeleteMessageConfirmationDialog.TAG);
        } else {
            onDelete();
        }
    }

    /// M: delete selected message
    ...
    /// M: call back from delete message confirm dialog.
    @Override
