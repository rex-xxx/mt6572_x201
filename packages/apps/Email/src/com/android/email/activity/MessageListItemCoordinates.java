...
public class MessageListItemCoordinates {
    ...
    // Static threshold.
    private static int sMinimumWidthWideMode = -1;
    private static int sMsgUseWideMode = -1;
    private static int[] sSubjectLengths;
    ...
    int subjectFontSize;
    int subjectAscent;
    /** M: Subject line height and subject text "top" distance. {@ */
    ...
    private static SparseArray<MessageListItemCoordinates> sCache =
    ...
    public static int getMode(Context context, int width, boolean isSearch) {
        ...
        if (sMinimumWidthWideMode <= 0) {
            sMinimumWidthWideMode = res.getDimensionPixelSize(R.dimen.minimum_width_wide_mode);
        }
        if (sMsgUseWideMode < 0) {
            sMsgUseWideMode = res.getInteger(R.integer.message_use_wide_header_mode);
        }
        // Choose the correct mode based on view width.
        int mode = NORMAL_MODE;
        if (sMsgUseWideMode != 0 && width > sMinimumWidthWideMode) {
            mode = WIDE_MODE;
        }
        return mode;
    }
    ...
    public static int getHeight(Context context, int mode) {
        float itemHeight = context.getResources().getDimensionPixelSize(
                (mode == WIDE_MODE) ? R.dimen.message_list_item_height_wide
                        : R.dimen.message_list_item_height_normal);
        Log.d(Logging.LOG_TAG, "Email messagelist itemHeight = " + itemHeight);
    ...
    public static int getSubjectLength(Context context, int mode) {
        Resources res = context.getResources();
        if (sSubjectLengths == null) {
            sSubjectLengths = res.getIntArray(R.array.subject_lengths);
        }
        return sSubjectLengths[mode];
    }
    ...
    public static MessageListItemCoordinates forWidth(Context context, int width,
        ...
        MessageListItemCoordinates coordinates = sCache.get(width);
        if (coordinates == null) {
            coordinates = new MessageListItemCoordinates();
            sCache.put(width, coordinates);
            ...
            View unreadState = view.findViewById(R.id.unread_state);
            coordinates.unreadStateX = getX(unreadState);
            coordinates.unreadStateY = getY(unreadState);
            ...
            /** M: Get the subject text line height and the text top distance. {@ */
            coordinates.subjectLineHeight = subject.getLineHeight();
            coordinates.subjectTop = subject.getPaint().getFontMetricsInt().top;
            /** @} */
