...
public class MailboxFinder {
    ...
    /**
     * Creates an instance for {@code accountId} and {@code mailboxType} with
     * executing delay time. (But won't start yet)
     * 
     * Must be called on the UI thread.
     */
    public MailboxFinder(Context context, long accountId, int mailboxType, Callback callback, int delayTime) {
        if (accountId == -1) {
    ...
    public void startLookup() {
        ...
        mStarted = true;
        mTask = new FindMailboxTask(true, mDelayTime);
    ...
    private class ControllerResults extends Controller.Result {
        ...
        public void updateMailboxListCallback(MessagingException result, long accountId,
            ...
            Log.i(Logging.LOG_TAG, "MailboxFinder: updateMailboxListCallback accountId = " + accountId
            ...
            if (result != null) {
                ...
                mTask = new FindMailboxTask(false, 0);
                mTask.executeParallel();
            }
        }
    }
    ...
    private class FindMailboxTask extends EmailAsyncTask<Void, Void, Long> {
         ...
         */
        public FindMailboxTask(boolean okToRecurse, int delayTime) {
            super(null);
        ...
        protected Long doInBackground(Void... params) {
            try {
                Thread.sleep(mDelyTime);
            } catch (InterruptedException e) {
                e.printStackTrace();
            }
            EmailAsyncTask.printStartLog("FindMailboxTask#doInBackground");
            ...
            if (Account.isSecurityHold(mContext, mAccountId)) {
                ...
                EmailAsyncTask.printStopLog("FindMailboxTask#doInBackground");
