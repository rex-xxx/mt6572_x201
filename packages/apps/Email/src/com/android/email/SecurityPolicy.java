...
public class SecurityPolicy {
    ...
    public boolean isSupported(Policy policy) {
        ...
        if (policy.mRequireEncryption) {
            ...
            if (encryptionStatus == DevicePolicyManager.ENCRYPTION_STATUS_UNSUPPORTED
                  || !FeatureOption.MTK_EMMC_SUPPORT 
                  || FeatureOption.MTK_CACHE_MERGE_SUPPORT) {
                Log.d(TAG, "Do not support encryption");
                return false;
            }
        }
    ...
    public Policy clearUnsupportedPolicies(Policy policy) {
        ...
        if (policy.mRequireEncryption) {
            ...
            if (encryptionStatus == DevicePolicyManager.ENCRYPTION_STATUS_UNSUPPORTED
                  || !FeatureOption.MTK_EMMC_SUPPORT 
                  || FeatureOption.MTK_CACHE_MERGE_SUPPORT) {
                Log.d(TAG, "clear require encryption flag from policy");
    ...
    public int getInactiveReasons(Policy policy) {
        ...
        if (policy == null) {
            /// M: If the policy is null, we should check the aggregate stored policies.
            /// Only clear the cache can the getAggregatePolicy() get the newest policies.
            mAggregatePolicy = null;
    ...
    public void setAccountPolicy(long accountId, Policy policy, String securityKey) {
        ...
        /** M: Need to check whether this account was deleted. {@ */
        if (account == null) {
            return;
        }
        /** @} */
    ...
    public static void onDeviceAdminReceiverMessage(Context context, int message) {
        ...
        switch (message) {
            ...
            case DEVICE_ADMIN_MESSAGE_PASSWORD_EXPIRING:
                /// M: use for testing. Make sure that the context is the same as the SecurityPolicyTest
