...
public class EmailWidget implements RemoteViewsService.RemoteViewsFactory,
    ...
    public EmailWidget(Context context, int _widgetId) {
        ...
        mWidgetId = _widgetId;
        /// M: Also make the related loader to get the widget id,
         // because the loader may need to save the new preference
         // values for the wigdet
        mLoader = new EmailWidgetLoader(mContext, mWidgetId);
    ...
    public void start() {
        ...
        // Legacy support; if preferences haven't been saved for this widget,
        // load something
        ...
        Logging.d(EmailWidget.TAG, "EmailWidget start() mAccountId: " + mAccountId
                + ", mWidgetId: " + mWidgetId);
        synchronized (this) {
            mLoader.init();
     ...
     */
    public void reset() {
        Logging.d(TAG, "#reset(); widgetId: " + mWidgetId);
        start();
    }
    ...
    public void onLoadComplete(Loader<Cursor> loader, Cursor cursor) {
        Logging.d(EmailWidget.TAG,"EmailWidget onLoadComplete()");
        synchronized(this) {
        mCursor = (EmailWidgetLoader.WidgetCursor) cursor;   // Save away the cursor
        }

        /** M: The mailbox id of the selection in EmailWidgetLoader was changed
            and the loader will reload the messages list @{ */
        if (mCursor.getNeedReload()) {
            WidgetManager.getInstance().getOrCreateWidget(mContext, mWidgetId).start();
            return;
        }
        /** @} */
     ...
     */
    private void setFillInIntent(RemoteViews views, int viewId, Uri baseUri, String messageId, String mailboxId) {
        Intent intent = null;
        try {
            intent = getOpenMessageIntent(mContext, Long.parseLong(messageId), Long.parseLong(mailboxId));
        } catch (NumberFormatException e) {
            if (Logging.DEBUG_LIFECYCLE && Email.DEBUG) {
                Log.d(TAG, "#setFillInIntent(); invalid messageId: " + messageId + " or mailboxId: " + mailboxId);
            }
        }
        views.setOnClickFillInIntent(viewId, intent);
    }

    private Intent getOpenMessageIntent(final Context context, final long messageId, final long mailboxId) {
        Mailbox mailbox = Mailbox.restoreMailboxWithId(context, mailboxId);
        return Welcome.createOpenMessageIntent(context, mailbox.mAccountKey, mailboxId, messageId);
    }

    private void setTextViewTextAndDesc(RemoteViews views, final int id, String text) {
        views.setTextViewText(id, text);
        //TODO wait for the method implements in framework
    ...
    private void updateHeader() {
        ...
        if (isCursorValid()) {
            ...
            intent = MessageCompose.getWidgetIntent(mContext, mAccountId);
    ...
    private CharSequence getStyledSubjectSnippet(String subject, String snippet, boolean read) {
        ...
        if (!TextUtils.isEmpty(subject)) {
            ...
            ss.setSpan(new StyleSpan(read ? Typeface.NORMAL : Typeface.BOLD), 0, ss.length(),
                    Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
            ss.setSpan(new ForegroundColorSpan(mResource
                    .getColor(R.color.widget_default_text_color)), 0, ss.length(),
                    Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
            ssb.append(ss);
            hasSubject = true;
        }
        if (!TextUtils.isEmpty(snippet)) {
            if (hasSubject) {
                ssb.append(mResource.getString(R.string.message_list_subject_snippet_divider));
            }
            SpannableString ss = new SpannableString(snippet);
            ss.setSpan(
                    new ForegroundColorSpan(mResource.getColor(R.color.widget_light_text_color)),
                    0, snippet.length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
            ssb.append(ss);
        }
        return addStyle(ssb, mResource.getDimensionPixelSize(R.dimen.widget_subject_font_size), 0);
    }
    ...
    public synchronized RemoteViews getViewAt(int position) {
        ...
        if (isUnread) {
            views.setViewVisibility(R.id.widget_unread_state, View.VISIBLE);
            drawableId = R.drawable.conversation_unread_selector;
        } else {
            views.setViewVisibility(R.id.widget_unread_state, View.INVISIBLE);
        }
        ...
        CharSequence styledFrom = addStyle(from, mResource
                .getDimensionPixelSize(R.dimen.widget_senders_font_size),
                mResource.getColor(R.color.widget_default_text_color));
        ...
        // Add style to date
        CharSequence styledDate = addStyle(date, mResource
                .getDimensionPixelSize(R.dimen.widget_date_font_size),
                mResource.getColor(R.color.widget_default_text_color));
