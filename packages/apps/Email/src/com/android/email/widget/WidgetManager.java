...
public class WidgetManager {
    ...
    private static final String MAILBOX_NAME_PREFIX = "mailboxName_";

    private final static WidgetManager sInstance = new WidgetManager();

    private static final String[] MAILBOXNAME_PROJECTION = new String[] {
        MailboxColumns.DISPLAY_NAME};
    private static final String MAILBOXID_SELECTION = MailboxColumns.ID + "=?";
    ...
    // Update all widgets of email
    public synchronized void updateAllEmailWidgets(Context context) {
        ArrayList<Integer> widgetIds = getEmailWidgetIdsFromPrefs(context);
        if (widgetIds == null) {
            return;
        }
        int[] intWidgetIds = new int[widgetIds.size()];
        for (int index = 0; index < widgetIds.size(); index++) {
            intWidgetIds[index] = widgetIds.get(index);
        }
        Logging.d(EmailWidget.TAG, "WidgetManager.updateAllEmailWidgets() widgetIds: "
                + widgetIds.toString());
        updateWidgets(context, intWidgetIds);
        for (int widgetId : widgetIds) {
            AppWidgetManager.getInstance(context).notifyAppWidgetViewDataChanged(widgetId,
                    R.id.message_list);
        }
    }
    ...
    static void saveWidgetPrefs(Context context, int appWidgetId, long accountId, long mailboxId) {
        ...
        /** M: Also save the mailbox name, for recover the widget loader after Exchange account mailbox
            (_eas) arise bad sync key @{ */
        Cursor cursor = context.getContentResolver().query(Mailbox.CONTENT_URI, MAILBOXNAME_PROJECTION,
                MAILBOXID_SELECTION, new String[]{String.valueOf(mailboxId)}, null);
        if (cursor != null) {
            try {
                if (cursor.moveToFirst()) {
                    prefs.edit()
                    .putString(MAILBOX_NAME_PREFIX + appWidgetId, cursor.getString(0)).commit();
                }
            } finally {
                cursor.close();
            }
    ...
    // Get all email widget ids from preference file
    static ArrayList<Integer> getEmailWidgetIdsFromPrefs(Context context) {
        SharedPreferences prefs = context.getSharedPreferences(PREFS_NAME, 0);
        ...
        for (String key : prefs.getAll().keySet()) {
            if (key.startsWith(ACCOUNT_ID_PREFIX)) {
                try {
                    String id = key.substring(ACCOUNT_ID_PREFIX.length());
    ...
    /**
     * M: Returns the saved mailbox name for the given widget. Otherwise, null if
     * the mailbox name was not previously saved.
     */
    static String loadMailboxNamePref(Context context, int appWidgetId) {
        SharedPreferences prefs = context.getSharedPreferences(PREFS_NAME, 0);
        String mailboxName = prefs.getString(MAILBOX_NAME_PREFIX + appWidgetId, null);
        return mailboxName;
    }
}
