...
public class CertificateSelector extends RelativeLayout implements OnClickListener {
    ...
    public void onClick(View target) {
        if (target == mSelectButton && mHost != null) {
            if (hasCertificate()) {
                ...
                setCertificate(null);
                /// M: Restore the SslSecurityView enable state after Certificate remove
                mHost.onCertificateRemoved();
            } else {
    ...
    /** M:Only using for test @{ */
    public Button getButtonFun() {
        return mSelectButton;
    }
    public View initTest(Context context) {
        LinearLayout layout = new LinearLayout(context);
        int layoutId = R.layout.account_settings_exchange_fragment;
        LayoutInflater inflater = (LayoutInflater) context
                .getSystemService(Context.LAYOUT_INFLATER_SERVICE);
        View view = inflater.inflate(layoutId, layout, false);
        return UiUtilities.getView(view, R.id.client_certificate_selector);
    }
    /** @} */
