...
public class AttachmentInfo {
    ...
    public static final int COLUMN_CONTENT_URI = 6;
    ...
    public static final int DENY_DELETED = 0x40;

    /** Action for view MPO picture */
    public static final String MPO_VIEW_ACTION = "com.mediatek.action.VIEW_MPO";

    public final long mId;
    public final long mSize;
    /// M: File name can be renamed
    public String mName;
    public final String mContentType;
    public final long mAccountKey;
    public final int mFlags;
    public final String mContentURI;
    ...
    public AttachmentInfo(Context context, Attachment attachment) {
        this(context, attachment.mId, attachment.mSize, attachment.mFileName, attachment.mMimeType,
                attachment.mAccountKey, attachment.mFlags, attachment.mContentUri);
    }
    ...
    public AttachmentInfo(Context context, Cursor c) {
        this(context, c.getLong(COLUMN_ID), c.getLong(COLUMN_SIZE), c.getString(COLUMN_FILENAME),
                ...
                c.getInt(COLUMN_FLAGS), c.getString(COLUMN_CONTENT_URI));
    }
    ...
    public AttachmentInfo(Context context, long id, long size, String fileName, String mimeType,
        ...
        /// M: MTK dependency. For DRM sd file, we allow it to be loaded and save
        if (activityList.isEmpty() && (!FeatureOption.MTK_DRM_APP ||
                !MIME_DCF.equalsIgnoreCase(mContentType))) {
            canView = false;
            canSave = false;
            denyFlags |= DENY_NOINTENT;
        }

        // Disable user to save attachments which in fact are local files 
        if (!TextUtils.isEmpty(mContentURI)
                && !mContentURI.contains(AttachmentUtilities.AUTHORITY)) {
            isLocal = true;
            canSave = false;
            /// M: If the attachment is a local .vcf file, we do not enable
             // user to view it because import a local vcf can produce duplicated
             // contacts items and senseless, and may lead to ANR (for multi vcard)
            if (mContentURI.contains(CONTACT_URI_PREFIX)) {
                canView = false;
            }
        }
    ...
    public Intent getAttachmentIntent(Context context, long accountId) {
        ...
        // Roll back changes for MPO_VIEW_ACTION special intent
        //if (AttachmentUtilities.MPO_MIME_TYPE.equals(mContentType)) {
        //    intent.setAction(MPO_VIEW_ACTION);
        //    Logging.d("getAttachmentIntent setAction MPO_VIEW_ACTION");
        //}
