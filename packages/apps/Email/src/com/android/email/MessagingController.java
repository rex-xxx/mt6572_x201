...
public class MessagingController implements Runnable {
    ...
    /// M: save unsynced messages as global to access it in multi-thread
    public ArrayList<Message> mUnsyncedMessages =new ArrayList<Message>();
    /// M: save MessagingException in thread to send it out
    private MessagingException mMessagingException;
    /** M: Save the count of running sync threads */
    ...
    private final PriorityBlockingQueue<Command> mHighPriorityCommands =
            new PriorityBlockingQueue<Command>(11, new Comparator<Command>() {
                @Override
                public int compare(Command cmd1, Command cmd2) {
                    return cmd1.priorityNumber - cmd2.priorityNumber;
                }
            });
    private Thread mHighPriorityThread;
    private static int sPriorityNumber = 100;
    
    private Command mRunningCommand;
    ...
    // The list of attachment IDs whose downloading would be canceled by user's request
    // Note that from current UI design, an identical ID may be repeatedly add into
    // the list several times before handling
    public final ArrayList<Long> mCancelledAttachments = new ArrayList<Long>();
    
    // IMAP remote folder to which current downloading attachment belongs
    private Folder mAttachmentFolder = null;
    
    // The id of the downloading attachment
    private long mDownloadingAttachmentId = -1;
    ...
    public void run() {
        ...
        while (true) {
            ...
            try {
                ...
                Logging.d("There are still "+ size +" tasks left Commands List");
            ...
            if (command.listener == null || isActiveListener(command.listener)) {
                mBusy = true;
                mRunningCommand = command;
                long time = System.currentTimeMillis();
                Logging.d("Command [" + command.description + "] start");
                command.runnable.run();
                Logging.d("Command [" + command.description +"] time ["
                        + (System.currentTimeMillis() - time)+"ms], left " + mCommands.size()
                            +" tasks");
    ...
    /**
     * Support BT-MAP send email.Changed the default code
     * @param account
     * @param folder
     * @param listener
     * @param callback
    ...
    public void synchronizeMailbox(final Account account,
            final Mailbox folder, MessagingListener listener,
            final EmailExternalCalls callback) {
        put("synchronizeMailbox", listener, new Runnable() {
            public void run() {
                try {
                    //register callback reference
                    ...
                    if (0 == mSendStatus) {
                        try {
                            ...
                            Logging.w(Logging.LOG_TAG, "synchronizeMailbox Failed");
                        }
                    } else {
                        try {
                            callback.updateCallback(EmailExternalConstants.RESULT_SUCCESS,
                                    account.mId, folder.mId);
                        } catch (RemoteException e) {
                            Logging.w(Logging.LOG_TAG, "synchronizeMailbox success");
                        }
                    }
                }
            }
    ...
    public void stopSynchronize(Context context) {
        Logging.d(" *** MessagingController stopSynchronize *** ");
        mCommands.clear();

        if (mRunningCommand != null) {
            if (mRunningCommand.runnable instanceof CancelableRunnable) {
                ((CancelableRunnable)mRunningCommand.runnable).cancel();
            }
        }
    }
    /**
    ...
    private boolean imapPartialFetchMessage(Message message, Folder remoteFolder)
        ...
        if (!htmlTexts.isEmpty()) {
            if (htmlTextsSize <= MAX_SMALL_MESSAGE_SIZE) {
                for (Part part : htmlTexts) {
                    fp.clear();
                    fp.add(part);
            ...
            if (plainTextsSize <= MAX_SMALL_MESSAGE_SIZE) {
                for (Part part : plainTexts) {
                    fp.clear();
                    fp.add(part);
                    remoteFolder.fetch(new Message[] { message }, fp, null);
                }
     ...
     * @param unsyncedMessage an Message's we've got headers for
     * @param toMailbox the destination mailbox we're syncing
     * @throws MessagingException
     */
    void loadUnsyncedMessage(final Account account, Folder remoteFolder,
            Message unsyncedMessage, final Mailbox toMailbox)
            throws MessagingException {

        // 1. Divide the unsynced messages into small & large (by size)
        FetchProfile smallfp = new FetchProfile();
        smallfp.add(FetchProfile.Item.BODY);
        FetchProfile largefp = new FetchProfile();

        /** M: Check whether this Message have been downloaded complete before and skip it */
        Message message = unsyncedMessage;
        int flagLoaded = -1;
        Cursor c = null;
        c = mContext.getContentResolver().query(EmailContent.Message.CONTENT_URI,
                new String[]{EmailContent.Message.FLAG_LOADED},
                EmailContent.SyncColumns.SERVER_ID + "=?", new String[] { message.getUid() },
                null);
        try {
            if (c!=null && c.moveToFirst()) {
                int index = c.getColumnIndex(EmailContent.Message.FLAG_LOADED);
                flagLoaded = c.getInt(index);
            }
        } finally {
            if (c != null) {
                c.close();
        ...
        if (flagLoaded == EmailContent.Message.FLAG_LOADED_COMPLETE) {
            Logging.d("The message with uId " + message.getUid()
                    + " has been downloaded by the previous behavior of user! So we jump it!");
            return;
        }
        ...
        if (message.getSize() > (MAX_SMALL_MESSAGE_SIZE) || message.getSize() == 0) {
            // 2. Download large messages.  We ask the server to give us the message structure,
            // but not all of the attachments.
            largefp.clear();
            largefp.add(FetchProfile.Item.STRUCTURE);
            remoteFolder.fetch(new Message[]{message}, largefp, null);
            ...
            if (message.getBody() == null) {
                ...
                // further download.
                largefp.clear();
                largefp.add(FetchProfile.Item.BODY_SANE);
            ...
            // 3. Download small messages

            // TODO Problems with this implementation.
            // 1. For IMAP, where we get a real envelope, this is going to be inefficient and
            // duplicate work we've already done.  2.  It's going back to the DB for a local
            // message that we already had (and discarded).
            // For small messages, we specify "body", which returns everything
            // (incl. attachments)
            remoteFolder.fetch(new Message[]{message}, smallfp,
     ...
     * @throws MessagingException
     */
    void loadUnsyncedMessages(final Account account, Store remoteStore, Folder remoteFolder,
        ...
        if (remoteStore instanceof ImapStore) {
            ...
            /** M: Set MAX number for IMAP sync task threads */
            final int maxSyncThreads = 10;
            /** M: Start {@link MAX_SYNC_THREADS} threads to synchronize Messages concurrently */
            try {
                synchronized (mUnsyncedMessages) {
                    ...
                    while (unsyncedMessagesCount > mRunningSyncThreadCount
                            && mRunningSyncThreadCount < maxSyncThreads) {
                        Logging.v("unsyncedMessages size: " + unsyncedMessagesCount
                                + " threadIndex: " + mRunningSyncThreadCount);
                        Thread syncTaskThread = new Thread(new LoadUnsyncMessageTask(account,
            ...
            /** M: For POP3 account, run the old way to download, MAYBE Multi-Threading later */
            for (Message message : unsyncedMessages) {
                loadUnsyncedMessage(account, remoteFolder, message, toMailbox);
            }
        }
        /** @} */
    }
    ...
    public void downloadFlagAndEnvelope(final Account account, final Mailbox mailbox,
            ...
            final boolean isImapAccount) throws MessagingException {
        FetchProfile fp = new FetchProfile();
        ...
        final ArrayList<EmailContent.Message> batchMessages = new ArrayList<EmailContent.Message>();
        MessageRetrievalListener listener = new MessageRetrievalListener() {
            @Override
            public void messageRetrieved(Message message) {
                try {
                    // Determine if the new message was already known (e.g. partial)
                    // And create or reload the full message info
                    LocalMessageInfo localMessageInfo = localMapCopy.get(message.getUid());
                    ...
                    if (localMessage != null) {
                        try {
                            ...
                            LegacyConversions.updateMessageFields(localMessage, message,
                                    account.mId, mailbox.mId);
                            if (isImapAccount && localMessage.mFlagLoaded
                                        == EmailContent.Message.FLAG_LOADED_PARTIAL) {
                                localMessage.mFlagLoaded = EmailContent.Message.FLAG_LOADED_ENVELOPE;
                            }
                            batchMessages.add(localMessage);
...
//        // for Imap mailbox, we fetch mail's envelop one by one from latest to older.
//        // otherwise, for pop3, we fetch all information in a bulk request.
//        if (isImapAccount) {
//            for (Message message : unsyncedMessages) {
//                remoteFolder.fetch(new Message[] { message }, fp, listener);
//            }
//        } else {
//            remoteFolder.fetch(unsyncedMessages.toArray(new Message[0]), fp, listener);
//        }
//            Comparator<EmailContent.Message> comparator = new Comparator<EmailContent.Message>() {
//                @Override
//                public int compare(EmailContent.Message message1,
//                        EmailContent.Message message2) {
//                    long serverId1 = Long.parseLong(message1.mServerId);
//                    long serverId2 = Long.parseLong(message2.mServerId);
...
//                }
//            };
//            Collections.sort(batchMessages, comparator);
//
        remoteFolder.fetch(unsyncedMessages.toArray(new Message[0]), fp, listener);
        // Use a batch operation to handle the changes
        ArrayList<ContentProviderOperation> ops = new ArrayList<ContentProviderOperation>();
        for (EmailContent.Message message : batchMessages) {
            if (message.isSaved()) {
                ops.add(ContentProviderOperation.newUpdate(message.getUri()).withValues(
                        message.toContentValues()).build());
            } else {
                ops.add(ContentProviderOperation.newInsert(message.mBaseUri).withValues(
                        message.toContentValues()).build());
            }
        }
        try {
            ContentProviderResult[] results = mContext.getContentResolver().applyBatch(
                    EmailContent.AUTHORITY, ops);
        } catch (RemoteException exception) {
            Logging.e("RemoteException catched: " + exception.getMessage());
        } catch (android.content.OperationApplicationException e) {
            Logging.e("OperationApplicationException catched: " + e.getMessage());
        }
    }
    ...
    private int searchMailboxImpl(long accountId, SearchParams searchParams,
        ...
        Message[] searchingMessages = new Message[messageList.size()];
        messageList.toArray(searchingMessages);
        remoteFolder.fetch(searchingMessages, fp, null);
        for (Message message : searchingMessages) {
            try {
                boolean isPartial = false;
                if (message.getSize() > (MAX_SMALL_MESSAGE_SIZE) || message.getSize() == 0) {
                    if (message.getBody() == null) {
                        // POP doesn't support STRUCTURE mode, but it should not go into search 
                        // mode.
                        Log.w(Logging.LOG_TAG, "POP doesn't support search mode.");
                    } else {
                        isPartial = imapPartialFetchMessage(message, remoteFolder);
                    }
                } else {
                    FetchProfile smallfp = new FetchProfile();
                    smallfp.add(FetchProfile.Item.BODY);
                    ...
                    // For small messages, we specify "body", which returns everything
                    // (incl. attachments)
                    remoteFolder.fetch(new Message[]{message}, smallfp, null);
                }
                // Determine if the new message was already known (e.g. partial)
                // And create or reload the full message info
                ...
                try {
                    ...
                    // Commit the message to the local store
                    ///M: move downward to save right mMailboxKey to DB, it must is destMailboxId.
                    saveOrUpdate(localMessage, mContext);
                    // We load 5k or so; maybe it's complete, maybe not...
                ...
                Log.e(Logging.LOG_TAG,
                        "Error while downloaded searching messages." + e.toString());
                // Delete existing contents of search mailbox
                ContentResolver resolver = mContext.getContentResolver();
                resolver.delete(EmailContent.Message.CONTENT_URI,
                        EmailContent.Message.MAILBOX_KEY + "=" + destMailboxId, null);
                return -1;
            }
        }
    ...
    private SyncResults synchronizeMailboxGeneric(final Account account, final Mailbox mailbox)
        ...
        if (remoteMessageCount > 0) {
            ...
            for (int i = (remoteMessages.length - 1); i >= 0; i--) {
                LocalMessageInfo localMessage = localMessageMap.get(remoteMessages[i].getUid());
                if (localMessage == null) {
                    newMessageCount++;
                }
                ...
                if (localMessage == null ||
                    ...
                    unsyncedMessages.add(remoteMessages[i]);
                }
            }
        }
        ...
        /** M Load unsynced messages for IMAP and POP in different ways */
        loadUnsyncedMessages(account, remoteStore, remoteFolder, unsyncedMessages, mailbox);
    ...
    public void copyOneMessageToProvider(Message message, EmailContent.Message localMessage,
        ...
        try {
            ...
            try {
                ...
                /** M: If a POP message has not load completely, the last attachment is not
                 * download completely in most case, so delete it. It will be download again
                 * and completely when user click the mRemainBtnView. @{ */
                if ((loadStatus != EmailContent.Message.FLAG_LOADED_COMPLETE && "pop3"
                        .equals(Account.getProtocol(mContext, localMessage.mAccountKey)))) {
                ...
                ConversionUtilities.updateAttachments(context, localMessage, attachments);
    ...
    private void processPendingDataChange(Store remoteStore, Mailbox mailbox, boolean changeRead,
            ...
            throws MessagingException {
        /** M: Pop3 account would not sync read, flagged and mailbox changes to server @{ */
        if (remoteStore instanceof Pop3Store) {
            return;
        }
        /** @} */
    ...
    private boolean processPendingAppend(Store remoteStore, Account account,
        ...
        // 2. If possible, load a remote message
        Message remoteMessage = null;
        if (message.mServerId != null && message.mServerId.length() > 0) {

            // 2a. Load a remote message with the matching UID
            remoteMessage = remoteFolder.getMessage(message.mServerId);
        } else if (remoteFolder instanceof ImapFolder) {

            // 2b. Load a remote message with the matching MessageId, and get the UID
            String searchString = "HEADER Message-Id \"" + message.mMessageId + "\"";
            String[] newIdArray = ((ImapFolder) remoteFolder).searchForUids(searchString);
            if (newIdArray.length != 0) {
                message.mServerId = newIdArray[0];
                remoteMessage = remoteFolder.getMessage(message.mServerId);
            }
        }
        ...
        if (remoteMessage == null) {
            ...
            if (remoteDate != null && remoteDate.compareTo(localDate) > 0) {
                ...
                Logging.d("processPendingAppend remote message is newer than ours");
                updateInternalDate = true;
                updateMessage = true;
            } else {
        ...
        if (updateInternalDate && message.mServerId != null) {
            try {
                ...
                Logging.e("MessagingException catched: " + me.getMessage());
            }
    ...
    public void loadMessageForView(final long messageId, MessagingListener listener) {
                ...
                try {
                    ...
                    if (remoteFolder instanceof ImapFolder) {
                        ...
                        for (Part part : viewables) {
                             fp.clear();
                             fp.add(part);
                            /// M: for IMAP, we should fetch limited message body to avoid OOM.
                             Log.d(Logging.LOG_TAG,
                            ...
                            if ( part.getSize() > MimeUtility.FETCH_BODY_SIZE_LIMIT) {
                                bodyTruncated = true;
                                remoteFolder.fetch(new Message[] { remoteMessage }, fp, null,
                                        MimeUtility.FETCH_BODY_SIZE_LIMIT);
                            } else {
                                remoteFolder.fetch(new Message[] { remoteMessage }, fp, null);
                            }
                        }
                        ...
                        fp.add(FetchProfile.Item.BODY);
                        /// M: for POP3, we must fetch whole message content to parse the body or attachment part.
                        remoteFolder.fetch(new Message[] { remoteMessage }, fp, null);
                        /// M: The mockFolder should not run into this case.
                        if (remoteFolder instanceof Pop3Folder) {
                            ArrayList<Part> viewables = new ArrayList<Part>();
                            ArrayList<Part> attachments = new ArrayList<Part>();
                            MimeUtility.collectParts(remoteMessage, viewables, attachments);
                            for (Part part : viewables) {
                                BinaryTempFileBodyInputStream in = (BinaryTempFileBodyInputStream) part.getBody()
                                        .getInputStream();
                                Log.d(Logging.LOG_TAG,
                                        " MessagingController : loadMessageForView : POP3 fetch body size = "
                                                + in.getLength());
                    ...
                    if (bodyTruncated) {
                        ...
                        flags |= EmailContent.Message.FLAG_BODY_TOO_LARGE;
                        cv.put(EmailContent.MessageColumns.FLAGS, flags);
                        Uri uri = ContentUris.withAppendedId(EmailContent.Message.CONTENT_URI,
                                message.mId);
                        mContext.getContentResolver().update(uri, cv, null, null);
                    }
                    ...
                    // 4. Write to provider
                    copyOneMessageToProvider(remoteMessage, account, mailbox, EmailContent.Message.FLAG_LOADED_COMPLETE);
        ...
        if ("pop3".equals(Account.getProtocol(mContext, message.mAccountKey))) {
            put("loadMessageForViewRemote", listener, runnable);
            return;
        }
        ...
        try {
            Command command = new Command();
            synchronized (this) {
                sPriorityNumber = sPriorityNumber - 1;
                command.priorityNumber = sPriorityNumber;
            }
            command.listener = listener;
            command.runnable = runnable;
            command.description = "loadMessageForViewRemote";
            mHighPriorityCommands.offer(command);
            Logging.d("Add a new command and priorityNumber is: " + command.priorityNumber);
        } catch (IllegalStateException ie) {
            throw new Error(ie);
        }
        
        if (mHighPriorityThread == null || !mHighPriorityThread.isAlive()) {
            Logging.d("###Create a new thread for loadMessageForView");
            mHighPriorityThread = null;
            mHighPriorityThread = new Thread(new Runnable() {
                @Override
                 public void run() {
                    // We do not synchronized this, because we expect the method poll() can get the
                    // newest command every time. As well as the efficiency will be better.
                    while (!mHighPriorityCommands.isEmpty()) {
                        int size = mHighPriorityCommands.size();
                        Logging.d("mImapCommands size is: " + size);
                        Command command;
                        command = mHighPriorityCommands.poll();
                        Logging.d(String.format("Running command's priorityNumber is: %d",
                                command != null ? command.priorityNumber : "command is null!"));
                        if (command == null) {
                            continue;
                        }
                        if (command.listener == null || isActiveListener(command.listener)) {
            ...
            mHighPriorityThread.start();
        } 
    }

    /**
     * Stop downloading the specific attachment 
     * @param attachmentId the attachment id
     */
    public void cancelLoadAttachment(final long attachmentId) {
        synchronized(mCancelledAttachments) {
            Logging.d(ATTACHMENT_TAG, "Add canceled attachment id to the canceled list");
            mCancelledAttachments.add(attachmentId);

            // Try to break the downloading course if the data had already transferring from the
            // remote side.
            // i.e. close the network connection.
            if (mRunningCommand != null) {
                if (mRunningCommand.runnable instanceof CancelableRunnable 
                        && mDownloadingAttachmentId == attachmentId) {
                    Logging.d(ATTACHMENT_TAG, "Try to close the attachment downloading connection");
                    ((CancelableRunnable)mRunningCommand.runnable).cancel();
                }
            }
        }
    ...
    public void loadAttachment(final long accountId, final long messageId, final long mailboxId,
        ...
        put("loadAttachment", listener, new CancelableRunnable() {
            private boolean mIsCancelled = false;

            public void run() {
                try {
                    // Check if the attachment downloading has already been canceled
                    if (mCancelledAttachments.contains(attachmentId)) {
                        Logging.d(ATTACHMENT_TAG,
                                "Attachment downloading canceled before really starting");
                        return;
                    }
                    ...
                    synchronized(mCancelledAttachments) {
                        if (mCancelledAttachments.contains(attachmentId)) {
                            Logging.d(ATTACHMENT_TAG,
                                    "Attachment downloading canceled before opening remote folder");
                            return;
                        }
                        // After this place, we should try to shut down the remote folder connection 
                        // for canceling the downloading 
                        remoteFolder.open(OpenMode.READ_WRITE);
                        mAttachmentFolder = remoteFolder;
                        mDownloadingAttachmentId = attachmentId;
                    }
                    ...
                    // set encoding type.
                    Logging.d("loadAttachment  attachment : " + attachment);
                    ...
                    // Check the cancellation before updating the content URI.
                    // If the cancellation was detected here, abandon the 
                    // fully downloaded data for the consistent UI
                    if (mCancelledAttachments.contains(attachmentId)) {
                        Logging.d(ATTACHMENT_TAG, "Abandon downloaded attachment data");
                        return;
                    }
                    ...
                    // The exception is caused by the user cancellation
                    // No need to do the callback.
                    if (mIsCancelled) {
                        return;
                    }
                    if (Logging.LOGD) {
                        Log.v(Logging.LOG_TAG, "", me);
                    }
                    ...
                    mListeners.loadAttachmentFailed(accountId, messageId, attachmentId,
                            new MessagingException(MessagingException.IOERROR, ioe.getMessage()),
                                background);
                } catch (NullPointerException e) {
                    // The poor multi-threads supporting in ImapFolder could cause this NPE when
                    // closing the network connection, just ignore it.
                    if (!mIsCancelled) {
                        throw e;
                    }
                } finally {
                    synchronized(mCancelledAttachments) {
                        mAttachmentFolder = null;
                        mDownloadingAttachmentId = -1;
                        mCancelledAttachments.remove(attachmentId);
                    }
                }
            }
            public void cancel() {
                Logging.v("cancel loadAttachment");
                try {
                    if (mAttachmentFolder != null && mAttachmentFolder.isOpen()) {
                        Logging.d(ATTACHMENT_TAG, "Close the connection");
                        mIsCancelled = true;
                        mAttachmentFolder.closeConnection();
                        mAttachmentFolder.close(false);
                        mAttachmentFolder = null;
                    }
                } catch (MessagingException e) {
                    Logging.v("loadAttachment cancel failed", e);
                }
            }
        });
    }
    ...
    public void sendPendingMessagesSynchronous(final Account account,
        ...
        try {
            ...
            if (requireMoveMessageToSentFolder) {
                ...
                moveToSentValues.put(MessageColumns.DISPLAY_NAME, account.getSenderName());
            }

            /// M: Cancel the login failed notification only when has no login failure
            ...
            while (c.moveToNext()) {
                ...
                if (requireMoveMessageToSentFolder) {
                    ...
                    if (msg != null &&
                        ...
                        AttachmentUtilities.deleteAllAttachmentFiles(mContext, account.mId,
                                messageId);
                        /// M: clear FLAG_TYPE_FORWARD, otherwise when user download the message in
                        /// sent mailbox.It will fetch the original message's attachment.
                        /// So the downloading progress would not be update. @{
                        moveToSentValues.put(EmailContent.Message.FLAGS, msg.mFlags
     ...
     */
    public boolean haveUnsyncedMessages() {
        synchronized (mUnsyncedMessages) {
            return !mUnsyncedMessages.isEmpty();
        }
    }
    ...
    public class LoadUnsyncMessageTask implements Runnable {
        ...
        public LoadUnsyncMessageTask(final Account account, Store remoteStore,
        ...
        public void run() {
            try {
                ...
                remoteFolder.open(OpenMode.READ_WRITE);
                while (haveUnsyncedMessages()) {
                    synchronized (mUnsyncedMessages) {
                        mUnsyncedMessageInner = mUnsyncedMessages.get(HEADER);
                        mUnsyncedMessages.remove(mUnsyncedMessageInner);
                    }
                    loadUnsyncedMessage(mAccountInner, remoteFolder,
                            mUnsyncedMessageInner, mToMailboxInner);
                }
                remoteFolder.close(false);
            } catch (MessagingException me) {
                Logging.v("LoadUnsyncMessageAsyncTask", me);
