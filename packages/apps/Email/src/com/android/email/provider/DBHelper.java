...
public final class DBHelper {
    ...
    private static final String TRIGGER_ACCOUNT_DELETE =
        ...
        "; delete from " + VipMember.TABLE_NAME +
        " where " + VipMember.ACCOUNT_KEY + "=old." + EmailContent.RECORD_ID +
        "; delete from " + SmartPush.TABLE_NAME +
        /// M: Delete the smart push data of this account meantime
        " where " + SmartPush.ACCOUNT_KEY + "=old." + EmailContent.RECORD_ID +
        "; end";
    ...
    // Version 40: Add dirty flag field to message (for Bad sync key recovery)
    // Version 42: Add vip member table to support vip settings
    ...
    static void createMessageTable(SQLiteDatabase db) {
        String messageColumns = MessageColumns.DISPLAY_NAME + " text, "
            ...
            + MessageColumns.SIZE + " integer, "
            + MessageColumns.DIRTY + " text"
            + ");";
    ...
    static void createAccountTable(SQLiteDatabase db) {
        String s = " (" + EmailContent.RECORD_ID + " integer primary key autoincrement, "
            ...
            + AccountColumns.VIP_RINGTONE_URI + " text"
            + ");";
    ...
    static void createVipMemberTable(SQLiteDatabase db) {
        String s = " (" + EmailContent.RECORD_ID + " integer primary key autoincrement, "
                ...
                + ");";
        db.execSQL("create table " + VipMember.TABLE_NAME + s);
    }

    static void createSmartPushTable(SQLiteDatabase db) {
        String s = " (" + EmailContent.RECORD_ID + " integer primary key autoincrement, "
                + SmartPush.ACCOUNT_KEY + " integer, "
                + SmartPush.EVENT_TYPE + " text, "
                + SmartPush.TIMESTAMP + " integer, "
                + SmartPush.VALUE + " integer"
                + ");";
        db.execSQL("create table " + SmartPush.TABLE_NAME + s);
    }
    ...
    protected static class DatabaseHelper extends SQLiteOpenHelper {
        ...
        public void onCreate(SQLiteDatabase db) {
            ...
            /// M: add vip member table
            createVipMemberTable(db);
            /// M: add smart push table
            createSmartPushTable(db);
        }

        @Override
        @SuppressWarnings("deprecation")
        public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
            Logging.d(TAG, "DBHelper upgrading EmailProvider.db from " + oldVersion + " to " + newVersion);
            ...
            if (oldVersion == 28) {
                // Message: add message size column into Message tables
                try {
                    db.execSQL("alter table " + Message.TABLE_NAME
                            + " add column " + MessageColumns.SIZE + " integer" + ";");
                    db.execSQL("alter table " + Message.UPDATED_TABLE_NAME
                            + " add column " + MessageColumns.SIZE + " integer" + ";");
                    db.execSQL("alter table " + Message.DELETED_TABLE_NAME
                            + " add column " + MessageColumns.SIZE + " integer" + ";");
            ...
            if (oldVersion == 30) {
                upgradeFromVersion30ToVersion31(db);
                oldVersion = 31;
            }
            if (oldVersion == 31) {
                try {
            ...
            if (oldVersion == 36 || oldVersion == 37) {
                oldVersion = 38;
            }
            if (oldVersion == 38) {
                try {
            ...
            if (oldVersion == 40) {
                try {
                    db.execSQL("alter table " + Message.TABLE_NAME
                            + " add column " + MessageColumns.DIRTY + " text;");
                    db.execSQL("alter table " + Message.DELETED_TABLE_NAME
                            + " add column " + MessageColumns.DIRTY + " text;");
                    db.execSQL("alter table " + Message.UPDATED_TABLE_NAME
                            + " add column " + MessageColumns.DIRTY + " text;");
                    ...
                    // Shouldn't be needed unless we're debugging and interrupt the process
                    Log.w(TAG, "Exception upgrading EmailProvider.db from 40 to 41 " + e);
                }
                oldVersion = 41;
            }
            /** M: Add vip member table to support vip settings @{ */
            if (oldVersion == 41) {
                try {
                    db.execSQL("alter table " + Account.TABLE_NAME
                            + " add column " + AccountColumns.VIP_RINGTONE_URI + " text;");
                    createVipMemberTable(db);
                    db.execSQL("drop trigger account_delete;");
                    db.execSQL(TRIGGER_ACCOUNT_DELETE);
                } catch (SQLException e) {
                    Log.w(TAG, "Exception upgrading EmailProvider.db from 41 to 42 " + e);
                }
                oldVersion = 42;
            }
            if (oldVersion == 42) {
                try {
                    ...
                    Log.w(TAG, "Exception upgrading EmailProvider.db from 42 to 43 " + e);
                }
                oldVersion = 43;
            }
            /** @} */
    ...
    /** Upgrades the database from v30 to v31 by updating all address fields in Message */
    ...
    private static void upgradeFromVersion30ToVersion31(SQLiteDatabase db) {
        try {
