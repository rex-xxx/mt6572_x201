...
public class EmailProvider extends ContentProvider {
    ...
    public static final boolean DEBUG_PROVIDER = Build.TYPE.equals("eng") ? true : false;
    ...
    private final ThreadLocal<HashMap<Uri, ContentObserver>> mNotifyMap = 
                                             new ThreadLocal<HashMap<Uri, ContentObserver>>();

    private final ThreadLocal<Integer> mApplyingNotifyCount = new ThreadLocal<Integer>();
    ...
    /** M: Support VIP Settings @{ */
    private static final int VIPMEMBER_BASE = 0xA000;
    ...
    private static final int SMARTPUSH_BASE = 0xB000;
    ...
    private static final int LAST_EMAIL_PROVIDER_DB_BASE = SMARTPUSH_BASE;
    /** @} */

    // DO NOT CHANGE BODY_BASE!!
    private static final int BODY_BASE = LAST_EMAIL_PROVIDER_DB_BASE + MAILBOX_BASE;
    ...
    private static final String[] TABLE_NAMES = {
        ...
        null,  // UI
        VipMember.TABLE_NAME,
        SmartPush.TABLE_NAME,
        Body.TABLE_NAME,
    };
    ...
    private static final String QUERY_ORPHAN_BODIES = BodyColumns.MESSAGE_KEY + " in (select "
        ...
        + EmailContent.RECORD_ID + " from " + Message.TABLE_NAME+ ")";
    ...
    /// M: Indicate the provider was created by instrumentation.
    private boolean mIsMockContext = false;
    ...
    static {
        ...
        // A specific mail body (size is bigger than 1M)
        matcher.addURI(EmailContent.AUTHORITY, "bodyLarge", BODY_LARGE);
        ...
        /** M: Support VIP Settings @{ */
        matcher.addURI(EmailContent.AUTHORITY, "vipmember", VIPMEMBER);
        matcher.addURI(EmailContent.AUTHORITY, "vipmember/#", VIPMEMBER_ID);
        /** @} */

        /** M: Support Smart push @{ */
        matcher.addURI(EmailContent.AUTHORITY, "smartpush", SMARTPUSH);
        matcher.addURI(EmailContent.AUTHORITY, "smartpush/#", SMARTPUSH_ID);
        /** @} */
    }
    ...
    synchronized SQLiteDatabase getDatabase(Context context) {
        ...
        Logging.d(TAG, " EmailProvider : getDatabase : checkDatabases " + this);
        // Whenever we create or re-cache the databases, make sure that we haven't lost one
        // to corruption
        checkDatabases();
        Logging.d(TAG, " EmailProvider : getDatabase : create EmailProvider " + this);
        DBHelper.DatabaseHelper helper = new DBHelper.DatabaseHelper(context, DATABASE_NAME);
        mDatabase = helper.getWritableDatabase();
        Logging.d(TAG, " EmailProvider : getDatabase : create EmailBodyProvider " + this);
    ...
    private HashMap<Integer, Long> getOrCreateAccountMailboxTypeMap(long accountId) {
        synchronized(mMailboxTypeMap) {
            Logging.d(Logging.LOG_TAG, " EmailProvider : getOrCreateAccountMailboxTypeMap : mMailboxTypeMap size = "
                    + mMailboxTypeMap.size() + " Current accountId = " + accountId);
    ...
    /** M : Only get mailboxTypeMap specified accountId.
     * @param accountId
     * @return null or mailboxTypeMap
     */
    private HashMap<Integer, Long> getAccountMailboxTypeMap(long accountId) {
        synchronized(mMailboxTypeMap) {
            return mMailboxTypeMap.get(accountId);
        }
    }

    private void addToMailboxTypeMap(Cursor c) {
        long accountId = c.getLong(Mailbox.CONTENT_ACCOUNT_KEY_COLUMN);
        /** M: Do not cache the mailbox type of mailbox with GLOBAL_MAILBOX_ACCOUNT_KEY,
         *  because it was a virtual account key. Otherwise the "count(*) of accounts"
         *  would not correct because the count number come from  of mMailboxTypeMap.size(). @{ */
        if (accountId == Controller.GLOBAL_MAILBOX_ACCOUNT_KEY) {
            return;
        }
        /** @} */
        int type = c.getInt(Mailbox.CONTENT_TYPE_COLUMN);
        synchronized(mMailboxTypeMap) {
            /**  M: only get the mailboxTypeMap, don't create a new one if the account not exist. @{ */
            HashMap<Integer, Long> accountMailboxTypeMap =
                getAccountMailboxTypeMap(accountId);
            if (accountMailboxTypeMap != null) {
    ...
    private void preCacheData() {
        synchronized(mMailboxTypeMap) {
            ...
            Logging.d(Logging.LOG_TAG, " EmailProvider : preCacheData : mMailboxTypeMap clear ");
    ...
    public void shutdown() {
        ...
        if (sBackupDatabase != null) {
            sBackupDatabase.close();
            sBackupDatabase = null;
        }
    }
            ...
            if (messageDeletion) {
                if (match == MESSAGE_ID) {
                    ...
                    if (hasEnoughDisk()) {
                        ...
                        Logging.d("EmailProvider do bulk deletion");
    ...
    /**
     * This function targets for delete mails when phone is not in enough space
    ...
    private void doDeleteMailPieceByPiece(SQLiteDatabase db) {
        Logging.d("EmailProvider delete mail piece by piece");
        ...
        Cursor c = null;
        try {
            c = db.query(Body.TABLE_NAME, new String[]{Body.ID},
        ...
        Logging.d("EmailProvider#doDeleteMailPieceByPiece has to delete "
                + idList.size() + " mails");
        ...
        while (idList.size() != 0) {
            try {
                ...
                if (hasEnoughDisk()) {
                    ...
                    Logging.d("EmailProvider delete " + loopTime + " mails");
                    ...
                    try {
                        int result = db.delete(Body.TABLE_NAME, Body.ID + " in " + s + ")", null);
                        if (result == 0) {
                            Logging.e("EmailProvider cannot even delete one mail");
                            db.endTransaction();
                            break;
                        }
                    } catch (SQLiteException e) {
                        Logging.e("EmailProvider sqlite exception", e);
                        db.endTransaction();
                        break;
                    }
                }
        ...
        db.beginTransaction();
        Logging.d("Finish EmailProvider delete mail piece by piece");

    }

    /**
     * Judge if the disk has enough space to do bulk deletion.
     */
                ...
                case SMARTPUSH:
                    ...
                    switch(match) {
                        ...
                        case POLICY:
                            ...
                            if (c != null) {
                                try {
                                    if (match == MAILBOX && c.moveToFirst()) {
                                        addToMailboxTypeMap(c);
    ...
    private HashMap<Uri, ContentObserver> getNotifyMap() {
        HashMap<Uri, ContentObserver> notifyMap = mNotifyMap.get();
        if (notifyMap == null) {
            notifyMap = new HashMap<Uri, ContentObserver>();
            mNotifyMap.set(notifyMap);
        }
        return notifyMap;
    }
    private void clearNotifyMap() {
        HashMap<Uri, ContentObserver> notifyMap = mNotifyMap.get();
        if (notifyMap != null) {
            notifyMap.clear();
        }
    ...
    private void notifyChange(Uri uri, ContentObserver observer) {
        if (applyingBatch()) {
            Integer i = mApplyingNotifyCount.get();
            i++;
            mApplyingNotifyCount.set(i);

            HashMap<Uri, ContentObserver> notifyMap = getNotifyMap();
            notifyMap.put(uri, observer);
        } else {
            Context context = getContext();
            ContentResolver resolver = context.getContentResolver();
    ...
    private void notifyAllChange() {
        ...
        Context context = getContext();
        ContentResolver resolver = context.getContentResolver();
        for (Uri u : notifyMap.keySet()) {
            resolver.notifyChange(u, notifyMap.get(u));
        }
        notifyMap.clear();
        if (Logging.LOG_PERFORMANCE) {
            Logging.d("[Exchange Download Speed] EmailProvider notifyAllChange() notify request: "
                    + mApplyingNotifyCount.get() + ", actual notify: " + size + ", spent: "
    ...
    private void grantUriPermission(Context context) {
        if (!mIsMockContext) {
            ...
            context.grantUriPermission(PACKAGE_QUICKSEARCHBOX, EmailContent.Message.CONTENT_URI,
                    Intent.FLAG_GRANT_READ_URI_PERMISSION);
            // grant qsb the body table query permission.
            context.grantUriPermission(PACKAGE_QUICKSEARCHBOX, EmailContent.Body.CONTENT_URI,
                    Intent.FLAG_GRANT_READ_URI_PERMISSION);
        }
    }
    ...
    public boolean onCreate() {
        Logging.d(TAG, " EmailProvider : onCreate()" + this);
        checkMockContext();
    ...
    /**
     * M: Check if it is a MockContext, for MockContext it will throws UnsupportedOperationException
     * when do getPackageManager(). This case only occur in test environment
     * and workaround to avoid testcase crash.
     */
    private void checkMockContext(){
        Context context = getContext();
        try{
            // For mockContext , all methods are non-functional 
            // and throw  {@link java.lang.UnsupportedOperationException}. 
            context.getPackageManager();
        } catch (UnsupportedOperationException upe){
            Logging.w(TAG, "This is a MockContext, not call setServicesEnabledAsync method.");
            mIsMockContext = true;
        }
    }
    ...
    public void checkDatabases() {
        Logging.d(TAG, " EmailProvider : checkDatabases() " + this);
            ...
            switch (match) {
                ...
                case SMARTPUSH:
                    ...
                    if (match == ACCOUNT && Arrays.equals(projection, EmailContent.COUNT_COLUMNS) &&
                        ...
                        /// M: add synchronized lock to avoid got dirty data when concurrent operation, such as
                        /// query() and delete() occur at the same time in different thread.
                        synchronized (mMailboxTypeMap) {
    ...
    private static SQLiteDatabase sBackupDatabase;
    private static SQLiteDatabase getBackupDatabase(Context context) {
        ...
        sBackupDatabase = helper.getWritableDatabase();
        return sBackupDatabase;
    }
    ...
    private String decompressBodyData(byte[] body) {
        int readCount;
        int originSize;
        int compressedSize;
        ...
        InflaterInputStream compressedStr = new InflaterInputStream(new ByteArrayInputStream(body));
        ...
        try {
            ...
            Logging.d(Logging.LOG_TAG,
                    "Decompressing message body: originSize = "
                            + originSize + " decompressedBytes = "
                            + decompressedBytes.length);
            outStream.close();
            compressedStr.close();
        } catch(Exception e) {
            Logging.w(Logging.LOG_TAG,
                    "Decompressing message body failed!");
        }
...
outer:
            switch (match) {
                ...
                case VIPMEMBER:
                case SMARTPUSH:
                    // Decompress body data when this field size > 1M bytes.
                    if (match == BODY_LARGE) {
                        byte[] compressedBody;
                        compressedBody = values.getAsByteArray(Body.TEXT_CONTENT);
                        if (compressedBody != null) {
                            values.put(Body.TEXT_CONTENT, decompressBodyData(compressedBody));
                        }
                        compressedBody = values.getAsByteArray(Body.HTML_CONTENT);
                        if (compressedBody != null) {
                            values.put(Body.HTML_CONTENT, decompressBodyData(compressedBody));
                        }
                    }
                    switch(match) {
        ...
        notifyChange(notificationUri, null);
        return result;
    }
    ...
    private Uri getBaseNotificationUri(int match) {
        ...
        switch (match) {
            ...
            case MESSAGE_ID:
                ...
                break;
            case SYNCED_MESSAGE_ID:
                baseUri = Message.SYNCED_CONTENT_NOTIFIER_URI;
                break;
            case ACCOUNT:
            case ACCOUNT_ID:
    ...
    private void sendNotifierChange(Uri baseUri, String op, String id) {
        ...
        /// M: If it was applying batch, do not append the id into the notifying uri. Because the
        /// applying batch may contains lots of database operations, and all the notifying uri are
        /// different if it contains id. it will issue out lots of notifications. If the id was
        /// not appended into the notifying uri. Most of the notifying uri would be same, and the
        /// notifyChange() method can filter the same uri. So, it only sent a few different
        /// notifications. This can reduce the duplicate sending and handling notification and
        /// improve the performance. e.g. bad case: marks 1000 mails to read/unread, it will send
        /// 1000 notifications, and the SyncedMessageObserver of Exchange will handle the
        /// notifications 1000 times.
        if (!applyingBatch() && longId > 0) {
            notifyChange(ContentUris.withAppendedId(baseUri, longId), null);
        } else {
            notifyChange(baseUri, null);
        }
/*
...
*/
    ...
    public ContentProviderResult[] applyBatch(ArrayList<ContentProviderOperation> operations)
        ...
        if (Logging.LOG_PERFORMANCE) {
            Logging.d(Logging.LOG_TAG,
                    "[Exchange Download Speed] EmailProvider:applyBatch:beginTransaction ["
                            + (beginTrans - dbstart) + "] ms");
        }
        try {
            mApplyingBatch.set(true);
            mApplyingNotifyCount.set(0);
            ContentProviderResult[] results = super.applyBatch(operations);
            long endapplyBatch = System.currentTimeMillis();
            if (Logging.LOG_PERFORMANCE) {
                Logging.d(Logging.LOG_TAG,
                        "[Exchange Download Speed] EmailProvider:applyBatch spend ["
                                + (endapplyBatch - beginTrans) + "] ms");
            }
            db.setTransactionSuccessful();
            if (Logging.LOG_PERFORMANCE) {
                Logging.d(Logging.LOG_TAG,
                    "[Exchange Download Speed] EmailProvider:applyBatch:setTransactionSuccessful ["
                              + (System.currentTimeMillis() - endapplyBatch) + "] ms");
            }
            notifyAllChange();
            return results;
        } finally {
            ...
            if (Logging.LOG_PERFORMANCE) {
                Logging.d(Logging.LOG_TAG,
                        "[Exchange Download Speed] EmailProvider:applyBatch:endTransaction ["
                                + (System.currentTimeMillis() - endTrans) + "] ms");
            }
        }
    }
