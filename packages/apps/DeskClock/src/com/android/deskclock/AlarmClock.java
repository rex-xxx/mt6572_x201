    ...
    public class AlarmItemAdapter extends CursorAdapter {
        ...
        private void bindExpandArea(final ItemHolder itemHolder, final Alarm alarm) {
            ...
            if (alarm.alert != null) {
                ...
                if (alarm.alert.toString().contains(Settings.System.ALARM_ALERT)) {
        ...
        private String usingDefaultRingtone() {
            ...
            Uri defaultUri = RingtoneManager.getActualDefaultRingtoneUri(
                    mContext, RingtoneManager.TYPE_ALARM);
            if (AlarmPreference.isRingtoneExisted(mContext, defaultUri)) {
            ...
            if (r != null) {
                ...
                //saveRingtone(AlarmPreference.DEFAULT_RINGTONE_URI);
                r = RingtoneManager.getRingtone(mContext, AlarmPreference.DEFAULT_RINGTONE_URI);
                title = r.getTitle(mContext);
            }
            return title;
        }
        ...
        mUndoBar.show(new ActionableToastBar.ActionClickedListener() {
            ...
            public void onActionClicked() {
                ...
                if (AlarmKlaxon.mCurrentPlayingAlarmId != -1
                        && mDeletedAlarm.id == AlarmKlaxon.mCurrentPlayingAlarmId) {
                    Intent enableSnoozeNotification = new Intent(Alarms.DISABLE_OR_ENABLE_SNOOZE_NOTIFICATION);
                    enableSnoozeNotification.putExtra(Alarms.ALARM_INTENT_EXTRA, mDeletedAlarm);
                    enableSnoozeNotification.putExtra("enable", true);
                    enableSnoozeNotification.putExtra("alarmID", mDeletedAlarm.id);
                    enableSnoozeNotification.putExtra("alarm", mDeletedAlarm);
                    sendBroadcast(enableSnoozeNotification);
                }
                mDeletedAlarm = null;
                mUndoShowing = false;
            }
