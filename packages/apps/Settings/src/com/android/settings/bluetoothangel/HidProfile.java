...
final class HidProfile implements LocalBluetoothProfile {
        ...
        public void onServiceConnected(int profile, BluetoothProfile proxy) {
            ...
            if (!deviceList.isEmpty()) {
                ...
                for(int i = 0; i < size; i++) {
                    ...
                    CachedBluetoothDevice device = mDeviceManager.findDevice(bluetoothDevice);
                    // we may add a new device here, but generally this should not happen
                    if (device == null) {
                        Log.w(TAG, "HidProfile found new device: " + bluetoothDevice);
                        device = mDeviceManager.addDevice(mLocalAdapter, mProfileManager, bluetoothDevice);
                    }
                    Log.d(TAG, "HID, onServiceConnected, set device " + device.getName() + " state to connected");
                    device.onProfileStateChanged(HidProfile.this,
    ...
    public int getConnectionStatus(BluetoothDevice device) {
        if (mService == null) {
            Log.d(TAG, "Service is null, return disconnected");
        ...
        // MTK BT support one devices connects with multiple HID devices,
        // do not just get the first device connection status
