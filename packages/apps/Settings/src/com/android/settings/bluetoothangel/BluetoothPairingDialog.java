...
public final class BluetoothPairingDialog extends AlertActivity implements
    ...
    private void onCancel() {
        /// M: when pairing with keyboard device, will set passkey directly, so use cancelBondProcess() instead @{
        if (mType == BluetoothDevice.PAIRING_VARIANT_DISPLAY_PASSKEY || mType == BluetoothDevice.PAIRING_VARIANT_DISPLAY_PIN) {
    ...
    /// M: when press back key, catch the IllegalStateException @{
    @Override
