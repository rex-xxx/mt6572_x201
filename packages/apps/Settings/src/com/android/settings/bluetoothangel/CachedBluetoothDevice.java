...
final class CachedBluetoothDevice implements Comparable<CachedBluetoothDevice> {
    ...
    // Following constants indicate the user's choices of Phone book access
    // settings
    ...
    CachedBluetoothDevice(Context context, LocalBluetoothAdapter adapter,
    ...
    int getProfileConnectionState(LocalBluetoothProfile profile) {
        if (mProfileConnectionState == null
            ...
            Log.d(TAG, "getProfileConnectionState from binder call, state is " + state);
        }
        Log.d(TAG, "return the profile state from map " + mProfileConnectionState.get(profile));
    ...
    private void fetchBtClass() {
        Log.d(TAG, "fetchClass");
        mBtClass = mDevice.getBluetoothClass();
        if (mBtClass == null) {
            Log.d(TAG, "mBtClass is null");
        } else {
            int mClass = mBtClass.getMajorDeviceClass();
            Log.d(TAG, "mBtClass is " + mClass);
        }
    }
    ...
    private boolean updateProfiles() {
        ...
        mProfileManager.updateProfiles(uuids, localUuids, mProfiles, 
                              mRemovedProfiles, mLocalNapRoleConnected);
    ...
    void onBondingStateChanged(int bondState) {
        ...
        Log.d(TAG, "onBondingStateChanged : refresh");
        refresh();

        if (bondState == BluetoothDevice.BOND_BONDED) {
            Log.d(TAG, "Bond state changed to bonded");
            if (mProfileManager != null) {
                ParcelUuid[] uuids = mLocalAdapter.getUuids();
                Log.d(TAG, "getLocalAdapterUuid" + uuids);
                if (uuids != null) {
                    Log.d(TAG, "update LocalBluetoothProfileManager");
    ...
    List<LocalBluetoothProfile> getConnectableProfiles() {
        Log.d(TAG, "mprofile size :" + mProfiles.size());
        ...
        Log.d(TAG, "Conectable profile size :" + connectableProfiles.size());
