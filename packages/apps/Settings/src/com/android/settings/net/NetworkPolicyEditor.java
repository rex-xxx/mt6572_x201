...
public class NetworkPolicyEditor {
    ...
    public void setPolicyMetered(NetworkTemplate template, boolean metered) {
        ...
        NetworkPolicy policy = getPolicy(template);
        /// M: clear active , and reset it as true {@
        Xlog.d(TAG , "setPolicyMetered " + policy + " ,checkbox state = " + metered);
        if (template == null || NetworkTemplate.MATCH_WIFI == template.getMatchRule()) {
            if (policy != null) {
                policy.active = false;
            }
        } else {
        ...
        if (metered) {
            if (policy == null) {
                ...
                policy.metered = true;
                policy.inferred = false;
                ///M : the active attr of policy set true {@ 
                policy.active = true;
                /// @}
                mPolicies.add(policy);
                modified = true;
        ...
        if (modified)  {
            /// M: if wifi policy , call new API , CR:ALPS00741267 {@
           if (template != null && NetworkTemplate.MATCH_WIFI == template.getMatchRule()) {
               Xlog.d(TAG,"modify wifi policy to update all the policies");
               writeAsyncForWifi();
           } else 
           /// @}
           {
    ...
    /**
     * M: call a new API mPolicyManager.setNetworkPoliciesForWifi(NetworkPolicy[] policies)
     * to deal with wifi policy. Avoid to lead to open mobile data when setting wifi policy 
     * CR:ALPS00741267
     * */
    public void writeAsyncForWifi() {
        // TODO: consider making more robust by passing through service
        final NetworkPolicy[] policies = mPolicies.toArray(new NetworkPolicy[mPolicies.size()]);
        new AsyncTask<Void, Void, Void>() {
            @Override
            protected Void doInBackground(Void... params) {
                mPolicyManager.setNetworkPoliciesForWifi(policies);
                return null;
            }
        }.execute();
    }
