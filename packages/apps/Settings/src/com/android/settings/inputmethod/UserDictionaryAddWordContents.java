...
public class UserDictionaryAddWordContents {
    ...
    /* package */ UserDictionaryAddWordContents(final View view, final Bundle args) {
        ...
        ///M: [CRALPS00348594] when word is too long, the JE pop

        if (null != word) {
            int wordMaxLength = view.getContext().getResources().getInteger(R.integer.maximum_user_dictionary_word_length);

            mWordEditText.setText(word);
            mWordEditText.setSelection(word.length() > wordMaxLength ? wordMaxLength: word.length());
        }
