...
public class InputMethodAndLanguageSettings extends SettingsPreferenceFragment
    ...
    private class VoiceCommandCallback extends VoiceCommandListener {
        ...
        public void onVoiceCommandNotified(int mainAction, int subAction,Bundle extraData) {
            ...
            if (subAction == VoiceCommandListener.ACTION_VOICE_SETTING_PROCESSLIST) {
                handleVoiceUiApps(result, extraData);
            }
        }
    }

    private class VoiceControlListener implements VoiceUiSwitchPreference.OnCheckedChangeListener {
        @Override
        public void onCheckedChanged(boolean isChecked) {
            Xlog.d(TAG, "onCheckedChanged isChecked = " + isChecked);
            Xlog.d(TAG, "onCheckedChanged mVoiceUiEnaber = " + mVoiceUiEnaber.isChecked());

            Secure.putInt(getActivity().getContentResolver(),
                    VOICE_CONTROL_ENABLED, isChecked ? 1 : 0);
            boolean isAllOff = isAllDisabled(mRawData);
            Xlog.d(TAG, "isAllOff=" + isAllOff);
            ...
            if (!isChecked) {
                ...
                for (String key : mRawData.keySet()) {
                    if (mRawData.get(key) == 1) {
                        Xlog.d(TAG, "status is unchecked, disable process name: " + key);
                        mRawData.put(key, 0);
                    }
                }
            } else {
                //if Voice Control is off, disabled all;
                for (String key : mVoiceUiAppStatus.keySet()) {
                    if (mVoiceUiAppStatus.get(key) == 1) {
                        Xlog.d(TAG, "status is checked, enable process name: " + key);
                        mRawData.put(key, 1);
                    }
                }
            }
            ...
            Xlog.d(TAG, "update values " + (isChecked ? "on " : "off ") + procs.toString());

        }

        @Override
        public boolean onBeforeCheckedChanged(boolean isChecked) {
            Xlog.d(TAG, "onBeforeCheckedChanged isChecked = " + isChecked);
            //judge if all is disable, if all disabed, the switch can't open.
            boolean isRemoteDataOff = isAllDisabled(mRawData);
            boolean isLocaleDataOff = isAllDisabled(mVoiceUiAppStatus);
            if (isRemoteDataOff && isLocaleDataOff) {
                Xlog.d(TAG, "can't open switch, because all is off");
                /**
                 * if false, handle the switch change,
                 * if true, don't handle this changed
                 */
                return !isChecked;
            }
            return true;
    ...
    public void onCreate(Bundle icicle) {
        ...
        //For voice control, these should be changed according process name changed.
        //For voice control
        mVoiceCategory = (PreferenceCategory)findPreference(KEY_VOICE_CATEGORY);
        if (mVoiceCategory != null) {
            mVoiceUiEnaber = (VoiceUiSwitchPreference)mVoiceCategory.findPreference(KEY_VOICE_UI_ENABLER);
        }
        ...
        if (!mIsOnlyImeSettings && FeatureOption.MTK_VOICE_UI_SUPPORT) {
            ...
            if (mVoiceUiEnaber != null) {
                mVoiceUiEnaber.setOnCheckedChangeListener(new VoiceControlListener());
            }
    ...
    public void onResume() {
        ...
        //Voice control
        if (!mIsOnlyImeSettings && FeatureOption.MTK_VOICE_UI_SUPPORT) {
            SharedPreferences processSharedPres = getActivity().getSharedPreferences(
                    "voice_ui_settings", Context.MODE_PRIVATE);
            for (String key : mVoiceUiAppStatus.keySet()) {
            ...
            Xlog.d(TAG, "Voice Control is " + (mVoiceControlEnable ? "on" : "off"));
            mVoiceUiEnaber.setChecked(mVoiceControlEnable, false);

            if (mVoiceCmdMgr != null) {
                try {
                    Xlog.d(TAG, "register voice command listener");
    ...
    /**
     * judge the current status of the switch status.
     * if all disabled, disable the switch.
     * @param appsStatus
     * @return
     */
    private boolean isAllDisabled(HashMap<String, Integer> appsStatus) {
        ...
        Xlog.d(TAG, "all app " + (allAppsDisabled ? "is" : "is not") + " disabled");
