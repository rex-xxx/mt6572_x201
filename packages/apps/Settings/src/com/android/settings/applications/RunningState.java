    ...
    private boolean update(Context context, ActivityManager am) {
        ...
        try {
            ...
            long[] pss = ActivityManagerNative.getDefault()
                    .getProcessPss(pids);
            /// M: get swap usage @{
            long[] pswap = ActivityManagerNative.getDefault()
                    .getProcessPswap(pids);
            MemInfoReader mMemInfoReader = new MemInfoReader();
            mMemInfoReader.readMemInfo();
            long totalSize = mMemInfoReader.getTotalSize();
            float zramAdjust = (float)(totalSize) / (float)(totalSize + Process.getZramExtraTotalSize());
            /// @}
            int bgIndex = 0;
            for (int i=0; i<pids.length; i++) {
                ...
                changed |= proc.updateSize(context, procSize, mSequence);
                /// @}
                if (proc.mCurSeq == mSequence) {
        ...
        //Log.i("RunningState", "Swap Memory: "
        //        + serviceProcessSwapMemory + " + " + backgroundProcessSwapMemory);
