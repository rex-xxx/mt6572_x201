...
public class VpnSettings extends SettingsPreferenceFragment implements
    ...
    public void onClick(DialogInterface dialog, int button) {
        if (button == DialogInterface.BUTTON_POSITIVE) {
            ...
            final String lockdownKey = LockdownConfigFragment.getStringOrNull(keyStore, Credentials.LOCKDOWN_VPN);
            ...
            if (preference != null) {
                if (lockdownKey == null) {
                    ...
                    Log.d(TAG, "lockdown vpn is enable for connect, do not disconnect.");
                }
            ...
            if (!mDialog.isEditing()) {
                ...
                try {
                    ...
                    Log.d(TAG, "lockdown vpn is enable for connect, do not connect to other profile.");
                    Toast.makeText(getActivity(), R.string.lockdown_vpn_already_connected, Toast.LENGTH_LONG).show();
                }
            }
    ...
    public boolean onContextItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
            ...
            case R.string.vpn_menu_delete:
                ...
                ///M: delete lockdown vpn profile@{
                final KeyStore keyStore = KeyStore.getInstance();
                final String lockdownKey = LockdownConfigFragment.getStringOrNull(keyStore, Credentials.LOCKDOWN_VPN);
                Log.v(TAG, "lockdownKey = " + lockdownKey + ",mSelectedKey" + mSelectedKey);
                if (TextUtils.equals(mSelectedKey, lockdownKey)) {
                    Log.v(TAG, "delete lockdown vpn profile");
    ...
    public boolean onPreferenceClick(Preference preference) {
        ...
        if (preference instanceof VpnPreference) {
            ...
            if (mInfo != null && profile.key.equals(mInfo.key) &&
                ...
                try {
                    if (mInfo.intent == null) {
                        Log.v(TAG, "mInfo.intent == null");
                        Toast.makeText(getActivity(), R.string.lockdown_vpn_already_connected, Toast.LENGTH_LONG).show();
                    } else {
                        Log.v(TAG, "mInfo.intent = " + mInfo.intent);
                        mInfo.intent.send();
                    }
                    return true;
                } catch (Exception e) {
