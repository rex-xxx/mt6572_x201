...
public final class CredentialStorage extends Activity {
    ...
    private void handleUnlockOrInstall() {
        ...
        Xlog.d(TAG, "keystore state " + mKeyStore.state());
        Xlog.d(TAG, "KeyGuardQuality " + (!checkKeyGuardQuality()));
    ...
    private void installIfAvailable() {
        if (mInstallBundle != null && !mInstallBundle.isEmpty()) {
            ...
            if (bundle.containsKey(Credentials.EXTRA_WAPI_USER_CERTIFICATE_NAME)) {
                ...
                if (caListName != null && !mKeyStore.put(caListName, caListData)) {
                    Log.e(TAG, "Failed to install " + caListName);
                    return;
                }
            }

            if (bundle.containsKey(Credentials.EXTRA_WAPI_AS_CERTIFICATE_NAME)) {
                String caListName = bundle.getString(Credentials.EXTRA_WAPI_AS_CERTIFICATE_NAME);
                byte[] caListData = bundle.getByteArray(Credentials.EXTRA_WAPI_AS_CERTIFICATE_DATA);
            ...
            implements DialogInterface.OnClickListener, DialogInterface.OnDismissListener
    {
        ...
        private ResetDialog() {
            // /M: modify the android default issue
            if (sResetDialog == null) {
                AlertDialog dialog = new AlertDialog.Builder(
                        CredentialStorage.this).setTitle(
                        android.R.string.dialog_alert_title).setIcon(
                        android.R.drawable.ic_dialog_alert).setMessage(
                        R.string.credentials_reset_hint).setPositiveButton(
    ...
    private class ResetKeyStoreAndKeyChain extends AsyncTask<Void, Void, Boolean> {
        ...
        @Override protected Boolean doInBackground(Void... unused) {
            Xlog.d(TAG, "AsyncTask: ResetKeyStoreAndKeyChain");
            synchronized (CredentialStorage.this) {
                mKeyStore.reset();
                try {
        ...
        @Override protected void onPostExecute(Boolean success) {
            Xlog.d(TAG, "AsyncTask: reset " + success);
            if (success) {
            ...
            implements DialogInterface.OnClickListener, DialogInterface.OnDismissListener
    {
        ...
        private ConfigureKeyGuardDialog() {
            ///M: solve android default issue
            if (sConfigureKeyGuardDialog == null) {
                AlertDialog dialog = new AlertDialog.Builder(
                        CredentialStorage.this).setTitle(
                        android.R.string.dialog_alert_title).setIcon(
                        android.R.drawable.ic_dialog_alert).setMessage(
                        R.string.credentials_configure_lock_screen_hint)
    ...
    private class UnlockDialog implements TextWatcher,
        ...
        private UnlockDialog() {
            ...
            // /M: solve android issue
            if (sUnlockDialog == null) {
                AlertDialog dialog = new AlertDialog.Builder(
                        CredentialStorage.this).setView(view).setTitle(
                        R.string.credentials_unlock).setPositiveButton(
                        android.R.string.ok, this).setNegativeButton(
                        android.R.string.cancel, this).create();
