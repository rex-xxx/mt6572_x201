...
public class BookmarkUtils {
    ...
    /**
     * M: Show a confirmation dialog to remove a folder.
     * @param id Id of the folder to remove
     * @param title Title of the folder, to be displayed in the confirmation method.
     * @param context Package Context for strings, dialog, ContentResolver
     * @param msg Message to send if the folder is deleted.
     */
    static void displayRemoveFolderDialog(final long id, final String title,
            final Context context, final Message msg) {
        new AlertDialog.Builder(context)
        .setIcon(android.R.drawable.ic_dialog_alert)
        .setMessage(context.getString(R.string.delete_folder_warning,
                title))
                .setPositiveButton(R.string.ok,
                        new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int whichButton) {
                        ...
                        Runnable runnable = new Runnable() {
                            @Override
                            public void run() {
                                deleteFoldBookmarks(id);
                            }

                            private void deleteFoldBookmarks(long id) {
                                ContentResolver cr = context.getContentResolver();
                                Uri uri = BookmarkUtils.getBookmarksUri(context);
                                Cursor cursor = cr.query(uri,
                                        new String[] { BrowserContract.Bookmarks._ID },
                                        BrowserContract.Bookmarks.PARENT + " = ? AND " +
                                                BrowserContract.Bookmarks.IS_DELETED + " = ?",
                                        new String[] { id + "", 0 + "" },
                                        null);

                                deleteBookmarkById(id);
                                while (cursor.moveToNext()) {
                                    deleteFoldBookmarks(cursor.getInt(BookmarksLoader.COLUMN_INDEX_ID));
                                }
                                cursor.close();
                            }
