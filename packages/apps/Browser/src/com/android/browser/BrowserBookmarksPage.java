...
public class BrowserBookmarksPage extends Fragment implements View.OnCreateContextMenuListener,
    ...
    public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
        ...
        /// M: Operator Feature create BookmarksOptionMenu. @{
        mBrowserSmallFeatureEx = Extensions.getSmallFeaturePlugin(getActivity());
        if (mBrowserSmallFeatureEx.shouldCreateBookmarksOptionMenu(menu, inflater)) {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        /// M: Operator Feature change the manner of the menu. @{
        mBrowserSmallFeatureEx = Extensions.getSmallFeaturePlugin(getActivity());
        if (mBrowserSmallFeatureEx.shouldChangeBookmarkMenuManner()) {
            changeOptionItemSelectedManner(item, getActivity());
        }
        ///@}
        return super.onOptionsItemSelected(item);
    }
    
    private void changeOptionItemSelectedManner(MenuItem item, Activity activity) {
        if (item.getItemId() == R.id.add_bookmark_menu_id) {
            ...
            Xlog.i(LOGTAG, "CurrentFolderId: " + mCurrentFolderId);
            activity.startActivity(intent);
            /// @}
        }
        if (item.getItemId() == R.id.new_bmfolder_menu_id) {
            Intent intent = new Intent(activity, AddBookmarkFolderForOP01Menu.class);
            intent.putExtra(BrowserContract.Bookmarks.PARENT, mCurrentFolderId);
            activity.startActivity(intent);
        }
    }
    ...
    public boolean onChildClick(ExpandableListView parent, View v,
        ...
        /// M: add for rtsp:// @ {	
        String url = BrowserBookmarksPage.getUrl(cursor);
        if(url != null && url.startsWith("rtsp://") && mCallbacks != null){
                Intent i = new Intent();
        ...
        if (isFolder) {
            ...
            if (crumbs != null) {
                ...
                mCurrentFolderId = (data != null ? ContentUris.parseId((Uri) data) : -1);
            }
    ...
    private void loadUrl(BrowserBookmarksAdapter adapter, int position) {
        if (mCallbacks != null && adapter != null) {
            /// M: add for rtsp:// @ {
            String url = BrowserBookmarksPage.getUrl(adapter.getItem(position));
    ...
    private void openInNewWindow(BrowserBookmarksAdapter adapter, int position) {
        if (mCallbacks != null) {
            ...
            if (isFolder) {
                ...
                String url = BrowserBookmarksPage.getUrl(c);
                /// M: add for rtsp:// @ {
                if(url.startsWith("rtsp://")){
                    Intent i = new Intent();
                    i.setAction(Intent.ACTION_VIEW);
    ...
    class OpenAllInTabsTask extends AsyncTask<Void, Void, Cursor> {
        ...
        protected Cursor doInBackground(Void... params) {
            ...
            return c.getContentResolver().query(BookmarkUtils.getBookmarksUri(c),
                    BookmarksLoader.PROJECTION, BrowserContract.Bookmarks.PARENT + "=? AND "
        ...
        protected void onPostExecute(Cursor result) {
            if (result != null && result.getCount() == 0) {
                ...
                Toast.makeText(ctx, ctx.getString(R.string.contextheader_folder_empty),
                        Toast.LENGTH_LONG).show();
            } else if (mCallbacks != null && result.getCount() > 0) {
        ...
        protected Integer doInBackground(Long... params) {
            ...
            try {
                c = mContext.getContentResolver().query(uri,
                        null, BrowserContract.Bookmarks.PARENT + "=? AND " + BrowserContract.Bookmarks.IS_FOLDER + " ==0",
