...
public abstract class BaseUi implements UI {
    ...
    public BaseUi(Activity browser, UiController controller) {
        ...
        mTitleBar = new TitleBar(mActivity, mUiController, this,
                mContentView);
        /// M: add bottom bar @{
        mNeedBottomBar = BrowserFeatureOption.BROWSER_BOTTOM_BAR_SUPPORT && !BrowserActivity.isTablet(mActivity);
        if (mNeedBottomBar) {
    ...
    protected void showTitleBar() {
        ...
        /// M: show bottom bar when title bar show
        showBottomBarForDuration(BaseUi.HIDE_TITLEBAR_DELAY * 2);
    }
...
//        mNavigationBar.setDisplayTitle(url);
        ...
        /// M: Operator Feature Set NavigationBarTitle. @{
        mBrowserSmallFeatureEx = Extensions.getSmallFeaturePlugin(mActivity);
        if (mBrowserSmallFeatureEx.shouldSetNavigationBarTitle()) {
            mNavigationBar.setDisplayTitle(title);
        } else {
            mNavigationBar.setDisplayTitle(url);
        }
    ...
    protected void setUrlTitle(Tab tab) {
        String url = tab.getUrl();
        String title = tab.getTitle();
        Xlog.i(LOGTAG, "Load Progress: " + tab.getLoadProgress() + "inPageLoad: " + tab.inPageLoad());
        if (TextUtils.isEmpty(title) || 
                (!tab.inPageLoad() && title.equals(mActivity.getString(R.string.title_bar_loading)))) {
            title = url;
        }
        if (tab.inForeground()) {
            /// M: Operator Feature Set NavigationBarTitle. @{
    ...
    protected final void showBottomBarForDuration(long duration) {
        if (getWebView()!=null/* && !(getWebView().canScrollVertically(-1) || getWebView().canScrollVertically(1))*/) {
