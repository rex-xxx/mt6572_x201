...
public class Controller
        implements WebViewController, UiController, ActivityController, IBrowserControllerEx {
    ...
    private static final int RELEASE_MEMORY = 301;
    ...
    private NotificationManagerPlus mNm;
    private static IBrowserProcessNetworkEx sBrowserProcessNetworkEx = null;
    ...
    public Controller(Activity browser) {
        ...
        mNm = new NotificationManagerPlus.ManagerBuilder(mActivity).create();
        startHandler();
        ...
        mSiteNavigationObserver = new ContentObserver(mHandler) {
            @Override
            public void onChange(boolean selfChange) {
                Xlog.d(LOGTAG, "SiteNavigation.SITE_NAVIGATION_URI changed");
    ...
    public void onPause() {
        ...
        if (mNm != null && mSettings.useFullscreen()) {
            mNm.stopListening();
        }
        mHandler.removeMessages(RELEASE_MEMORY);
    }
    ...
    public void onPageStarted(Tab tab, WebView view, Bitmap favicon) {
        ...
        /// M: Operator Feature handle the network status message. @{
        sBrowserProcessNetworkEx = Extensions.getProcessNetworkPlugin(mActivity);
        if (sBrowserProcessNetworkEx.shouldProcessNetworkNotify(mNetworkHandler.isNetworkUp())) {
            sBrowserProcessNetworkEx.processNetworkNotify(view, mActivity, mNetworkHandler.isNetworkUp(), this);
        } else if (!mNetworkHandler.isNetworkUp()) {
                            ...
                            if (isSiteNavigationAboutBlankUrl(itemUrl)) {
                                ...
                                menu.findItem(R.id.delete_sn_context_menu_id)
                                        .setOnMenuItemClickListener(new OnMenuItemClickListener() {
                                            @Override
                                            public boolean onMenuItemClick(MenuItem item) {
                                                showSiteNavigationDeleteDialog(Uri
                                                        .decode(imageAnchorUrlExtra));
                                                return false;
                                            }
                                        });
                            }
...
//                    mCrashRecoveryHandler.clearState();
    ...
    private void updateScreenshot(Tab tab) {
        ...
        Log.d(LOGTAG, "need update: " + mNeedUpdateThumbnail
                + " update bookmark url: " + mUrlNeedUpdateThumbnail
                + " originalUrl: " + originalUrl + " url: " + url);
        if (url != null && mUrlNeedUpdateThumbnail != null 
                && Patterns.WEB_URL.matcher(url).matches()
                && Patterns.WEB_URL.matcher(mUrlNeedUpdateThumbnail).matches()) {
            String urlHost = (new WebAddress(url)).getHost();
            String bookmarkHost = (new WebAddress(mUrlNeedUpdateThumbnail)).getHost();
			...
			/*
	            ...
	            Log.d(LOGTAG, "addressUrl domain: " + urlDomain);
	            Log.d(LOGTAG, "bookmarkUrl domain: " + bookmarkDomain);
        ...
        new AsyncTask<Void, Void, Void>() {
            ...
            protected Void doInBackground(Void... unused) {
                ...
                try {
                    // TODO: Clean this up
                    /// M: update the thumbnail after load finish the URL @{
    ...
    private Tab createNewTab(boolean incognito, boolean setActive,
        ...
        if (mTabControl.canCreateNewTab()) {
            ...
            if (setActive) {
             ...
             /// M: post delay to destroy webview: onLowMemory();
            if (!canAllocateMemory()) {
                mHandler.sendMessageDelayed(mHandler
                        .obtainMessage(RELEASE_MEMORY), 1000);
            }
        } else {
            if (useCurrent) {
...
//          case KeyEvent.KEYCODE_Z:    // unused
    ...
    public void showNetworkUnavilableDialog() {
        Intent dlg = new Intent(mActivity, DataConnectionDialog.class);
        mActivity.startActivity(dlg);
    }
    ...
    /// M: add to check system memory @{
    public boolean canAllocateMemory() {
        ActivityManager.MemoryInfo memInfo = new ActivityManager.MemoryInfo();
        ActivityManager mgr = (ActivityManager)mActivity.getSystemService(Context.ACTIVITY_SERVICE);
        mgr.getMemoryInfo(memInfo);
        if (memInfo.lowMemory) {
            Xlog.i(LOGTAG, "Low memory checked: " + memInfo.lowMemory);
            return false;
        }
        return true;
    }
