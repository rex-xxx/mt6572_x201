...
public class BrowserProvider2 extends SQLiteContentProvider implements IBrowserProvider2Ex {
    ...
    private static final String XLOGTAG = "browser/BrowserProvider";
    ...
    private static final String TABLE_BOOKMARKS_JOIN_HISTORY =
        "history LEFT OUTER JOIN bookmarks ON (history.url = bookmarks.url AND bookmarks.deleted=0 AND bookmarks.folder=0)";
    ...
    //Old bookmarks table column name
    private static final String BOOKMARK_COLUMN_FOLDER_ID = "folder_id";
    private static final String BOOKMARK_COLUMN_VISITS = "visits";
    //Old bookmark_folders table which will be used on upgrade from version 24.
    public interface BookmarkFoldersColumns {
        String ID = "_id";
        String PARENT_ID = "parent_id";
        String FOLDER_LEVEL = "folder_level";
        String NAME = "name";
        String DATE = "date";
        String VISITS = "visits";
    }
    public static final String[] BOOKMARK_FOLDERS_PROJECTION = new String[] {
        BookmarkFoldersColumns.ID, BookmarkFoldersColumns.PARENT_ID, 
        BookmarkFoldersColumns.FOLDER_LEVEL, BookmarkFoldersColumns.NAME,
        BookmarkFoldersColumns.DATE, BookmarkFoldersColumns.VISITS}; 

    /* these indices dependent on BOOKMARK_FOLDERS_PROJECTION */
    ...
    static {
        ...
        // For DM
        URI_MATCHER.addURI(authority, "homepage", HOMEPAGE);
    ...
    @Override
    public int addDefaultBookmarksHost(SQLiteDatabase db, long parentId, 
            CharSequence[] bookmarks, int operatorId, int position) {
        Xlog.i(XLOGTAG, "parentId: " + parentId + "position: " + position);
        TypedArray preloads = null;
        switch(operatorId) {
        case 1:
            preloads = getContext().getResources().obtainTypedArray(R.array.bookmark_preloads_for_op01);
            position = mOpenHelper.addDefaultBookmarks(db, parentId, bookmarks, preloads, position);
            break;
        case 2:
            preloads = getContext().getResources().obtainTypedArray(R.array.bookmark_preloads_for_op02);
            position = mOpenHelper.addDefaultBookmarks(db, parentId, bookmarks, preloads, position);
            
            //reserve position 1 for google url homepage.
            position += position > 0 ? 1 : 2;
            if (BrowserFeatureOption.BROWSER_YAHOO_BOOKMARKS_SUPPORT) {
                position = mOpenHelper.addDefaultBookmarksForYahoo(db, FIXED_ID_ROOT, position);
            }
            position = mOpenHelper.addDefaultBookmarks(db, FIXED_ID_ROOT, position);
            break;
        default:
            break;
        }
    ...
    final class DatabaseHelper extends SQLiteOpenHelper {
        ...
        boolean importFromBrowserProvider(SQLiteDatabase db) {
            ...
            if (oldDbFile.exists()) {
                ...
                Cursor c = null;
                try {
                    /// M: add for feature bookmark folders @{
                    int position = 0;
                    // create default Bookmarks folder
                    {
                        ContentValues values = new ContentValues();
                    ...
                    String tableFolder = BrowserProvider.TABLE_NAMES[BrowserProvider.URI_MATCH_BOOKMARK_FOLDERS];
                    //check the bookmark_folders exist or not.
                    //if don't exist then create empty table for avoiding no table exception.
                    oldDb.execSQL("CREATE TABLE IF NOT EXISTS bookmark_folders (" +
                    ...
                    // Import bookmark folders
                    c = oldDb.query(tableFolder,
                            BOOKMARK_FOLDERS_PROJECTION, 
                            null, null, null, null, BookmarkFoldersColumns.VISITS + " DESC");
                    if (c != null) {
                        while (c.moveToNext()) {
                            position++;
                            ContentValues values = new ContentValues();
                            //In MTK bookmark folder feature, root folder id is 0. However, it is 1 in google android4.0
                            //So we have increase 1 to match them.
                            values.put(Bookmarks._ID, c.getInt(BOOKMARK_FOLDERS_PROJECTION_ID_INDEX) + 1);
                            values.put(Bookmarks.TITLE, c.getString(BOOKMARK_FOLDERS_PROJECTION_NAME_INDEX));
                            values.put(Bookmarks.DATE_CREATED, c.getInt(BOOKMARK_FOLDERS_PROJECTION_DATE_INDEX));
                            values.put(Bookmarks.POSITION, position);
                            values.put(Bookmarks.IS_FOLDER, true);
                            values.put(Bookmarks.PARENT, c.getInt(BOOKMARK_FOLDERS_PROJECTION_PARENT_ID_INDEX) + 1);
                    ...
                    c = oldDb.query(table,
                            ...
                            /// M: add for feature bookmark folders
                            BOOKMARK_COLUMN_FOLDER_ID, //5
                            }, BookmarkColumns.BOOKMARK + "!=0", null,
                            /// M: add for feature bookmark folders
                            null, null, BOOKMARK_COLUMN_VISITS + " DESC");
                    if (c != null) {
                        while (c.moveToNext()) {
                            ...
                            /// M: add for feature bookmark folders
                            values.put(Bookmarks.POSITION, position);
                            //when import folder, folder id has increase 1, do it here too for matching them .
                            values.put(Bookmarks.PARENT, c.getInt(5) + 1);
        ...
        private void createDefaultBookmarks(SQLiteDatabase db) {
            ...
            /// M: Operator Feature for add default bookmarks. @{
            mBrowserSmallFeatureEx = Extensions.getSmallFeaturePlugin(getContext());
            if ((position = mBrowserSmallFeatureEx.addDefaultBookmarksForCustomer(BrowserProvider2.this, 
                    db, FIXED_ID_ROOT, position)) == 0) {
                //reserve position 1 for google url homepage.
                position += position > 0 ? 1 : 2;
                if (BrowserFeatureOption.BROWSER_YAHOO_BOOKMARKS_SUPPORT) {
        ...
        private void addDefaultBookmarks(SQLiteDatabase db, long parentId) {
            ...
            addDefaultBookmarks(db, parentId, bookmarks, preloads, 0);
        }

        /**
         * M: add for feature default bookmarks
         */
         ...
         * M: add for feature default bookmarks
         */
        private int addDefaultBookmarks(SQLiteDatabase db, long parentId, 
                                            CharSequence[] bookmarks, TypedArray preloads, int position) {
            Resources res = getContext().getResources();
            int size = bookmarks.length;
            try {
                String parent = Long.toString(parentId);
                String now = Long.toString(System.currentTimeMillis());
                boolean isGoogleHomepage = false;
                for (int i = 0; i < size; i = i + 2) {
                    ...
                    db.execSQL("INSERT INTO bookmarks (" +
                        ...
                        ") VALUES (" +
                            ...
                            "0," +
                            parent + "," +
                            (isGoogleHomepage ? 1 : Integer.toString(position + i)) + "," +
                            now +
                            ");");
    ...
    /// M: modification for site navigation
    //Bring it out from above to make it accessible to other class and packages
    ...
    public Uri insertInTransaction(Uri uri, ContentValues values, boolean callerIsSyncAdapter) {
        ...
        switch (match) {
            case BOOKMARKS: {
                ...
                /// M: If add a folder to bookmark, make sure parent folder does not contains a same folder. @{
                if (values.containsKey(Bookmarks.IS_FOLDER)) {
                    if (values.getAsBoolean(Bookmarks.IS_FOLDER) && values.containsKey(Bookmarks.PARENT)
                            && values.containsKey(Bookmarks.TITLE)) {
                        boolean isValid = isValidAccountName(values.getAsLong(Bookmarks.PARENT),
                                values.getAsString(Bookmarks.TITLE));
                        if (!isValid) {
                            return null;
                        }
                    }
    ...
    private boolean isValidAccountName(long parentId, String title) {
        ...
        try {
            ...
            while (cursor.moveToNext()) {
             ...
             android.util.Log.e(XLOGTAG, e.getMessage());
        } finally {
            if (cursor != null) {
                cursor.close();
            }
    ...
    public int updateInTransaction(Uri uri, ContentValues values, String selection,
        ...
        switch (match) {
            ...
            /// M: add for feature Homepage @{
            case HOMEPAGE:
                //Note: in this case, homepage is stored in 
                if (null != values) {
                    ...
                    if (setHomePage(getContext(), homepage)) {
                        return 1; // success
                    }
                }
                return 0; // fail
    ...
    /**
     * M: Modified homepage in preferences.
     */
    ...
    private String getHomePage(Context context) {
        ...
        SharedPreferences p = PreferenceManager.getDefaultSharedPreferences(context);
        String url = p.getString(BrowserSettings.PREF_HOMEPAGE, factoryResetUrl);
        return url;
    }
