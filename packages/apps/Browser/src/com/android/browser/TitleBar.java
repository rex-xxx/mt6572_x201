...
public class TitleBar extends RelativeLayout {
    ...
    public TitleBar(Context context, UiController controller, BaseUi ui,
        ...
        ViewConfiguration config = ViewConfiguration.get(mUiController.getActivity());
        /// M: slop used to show title bar when scroll near to top
    ...
    public void onScrollChanged() {
        if (!mShowing && !mIsFixedTitleBar) {
            ...
            /// M: when screen scroll, redraw bottom bar if necessary. @{
            if (unVisibleHeight > -mSlop) {
                show();
                mBaseUi.showBottomBarForDuration(BaseUi.HIDE_TITLEBAR_DELAY);
            } else if (unVisibleHeight < -mSlop) {
                mBaseUi.hideBottomBar();
            }
            /// @}
        }
    }
