...
import android.webkit.WebSettings.NotificationState;
...
public class BrowserSettings implements OnSharedPreferenceChangeListener,
    ...
    public static final String GAMELOFT_UA_STRING = "Mozilla/5.0 " +
    		"(Linux; U; Android 4.1.1; zh-cn; 158; mt6575; 480*800; 4G+4G; Build/JRO03C) " +
    ...
    private void syncSetting(WebSettingsClassic settings) {
        ...
        if (FeatureOption.MTK_WEB_NOTIFICATION_SUPPORT) {
            settings.setNotificationState(getNotificationState());
        }
        /// @ }
...
//          String operatorUA = mBrowserSmallFeatureEx.getOperatorUA(settings.getUserAgentString());
            ...
            if (getUserAgent() == 0 && operatorUA != null) {
                ...
                if (BrowserFeatureOption.BROWSER_GAMELOFT_SUPPORT == false) {
                    settings.setUserAgentString(null);
                } else {
                    settings.setUserAgentString(GAMELOFT_UA_STRING);
                }      
            } else {
    ...
    private void syncStaticSettings(WebSettingsClassic settings) {
        ...
        /// M : HTML5 web notification @ {
        if (FeatureOption.MTK_WEB_NOTIFICATION_SUPPORT) {
            settings.setWebNotificationDatabasePath(mContext.getDir("webnotification", 0).getPath());
        }
        /// @ }
    }
    ...
    /// M : HTML5 web notification @{
    public WebSettings.NotificationState getNotificationState() {
        String notificationState = mPrefs.getString(PreferenceKeys.KEY_NOTIFICATION_STATE, "ON");
        Xlog.d(XLOGTAG, "getNotificationState()" + notificationState);
        return NotificationState.valueOf(notificationState);
    }

    public void clearWebNotifications() {
        Xlog.d(XLOGTAG, "clearWebNotifications()");
        NotificationPermissions.getInstance().clearAll();
    }
    /// @}
    ...
    public String getDefaultDownloadPathWithMultiSDcard() {
        ...
        if (null != defaultStorage) {
            defaultDownloadPath = defaultStorage + DEFAULT_MY_FAVORITE_FOLDER_OP01;
        }
