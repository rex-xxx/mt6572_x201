...
public class BrowserActivity extends Activity {
    ...
    protected void onNewIntent(Intent intent) {
        ...
        if (mController == NullController.INSTANCE) {
            Xlog.w(XLOGTAG, "onNewIntent for Action_Search Intent reached before finish(), " +
        ...
        if (FeatureOption.MTK_WEB_NOTIFICATION_SUPPORT
                && ACTION_NOTIFICATIONS.equals(intent.getAction())) {
            Xlog.w(XLOGTAG, "ACTION_NOTIFICATIONS.equals(intent.getAction())");
            SharedPreferences.Editor localEditor = PreferenceManager.getDefaultSharedPreferences(this).edit();
            localEditor.putInt(EXTRA_SHOWN, 0);
            localEditor.apply();
            ...
            if (mController != null) {
                TabControl localTabControl = getController().getTabControl();
                if (localTabControl != null) {
                    Tab localTab = localTabControl.getTab(position);
                    if (localTab == null) {
                        return;
                    }
                    getController().switchToTab(localTab);
                    localTab.onTest(title, body, counter);
                }
            }
        }
        /// @}
