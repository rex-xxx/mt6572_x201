...
class Tab implements PictureListener {
    ...
    /// M : HTML5 web notification @{
    private NotificationPermissions.Callback mNotificationPermissions;
    private NotificationPermissionsPrompt mNotificationPermissionsPrompt;
    ...
    private static HashMap<Tab, List<Integer>> mTabPosition = new HashMap();
    /// @}
    ...
    private final WebViewClientClassicExt mWebViewClient = new WebViewClientClassicExt() {
        ...
        public void onPageStarted(WebView view, String url, Bitmap favicon) {
            ...
            if (mIsDebuggable) {
                ...
                Xlog.i(LOGTAG, "[" + tc.getTabPosition(Tab.this) + "/" + tc.getTabCount()
                        + "] onPageStarted url=" + url);

                /// M: dump memory info
                new OutputMemoryInfo().execute(tc,null);
            }
            /// @}
    ...
    private final WebChromeClient mWebChromeClient = new WebChromeClient() {
        ...
        public boolean onCreateWindow(WebView view, final boolean dialog,
            ...
            mWebViewController.onShowPopupWindowAttempt(Tab.this, dialog, resultMsg);
            return true;
        }
        ...
        public int getNextNotificationId() {
            ...
            int i = 1 + p.getInt(PreferenceKeys.KEY_NOTIFICATION_ID, 0);

            SharedPreferences.Editor editor = p.edit();
            editor.putInt(PreferenceKeys.KEY_NOTIFICATION_ID, i);
            editor.apply();
            Log.v(LOGTAG, "getNextNotificationId: " + i);
            return i;
        }

        @Override
        public void onNotificationManagerCancelPrompt(int id) {
            SharedPreferences p = PreferenceManager.getDefaultSharedPreferences(mContext);
            if (p.getInt("Shown", 0) == 1) {
                Log.v(LOGTAG, "Inside onNotificationManagerCancelPrompt cancel from NM notify id is " + id);
                mManager.cancel(id);
            }
            Log.v(LOGTAG, "Inside onNotificationManagerCancelPrompt cancel prompt from display, id = " + id);
        }

        @Override
        public void onNotificationManagerShowPrompt(String icon, String title, String body, int id, NotificationPermissions.Callback callback) {
            NotificationPermissions.getInstance();

            mNotificationPermissions = callback;
            mNotificationID = id;
            ...
            mNotification.icon = R.drawable.ic_web_holo_dark;
            ...
            TabControl tabControl = mWebViewController.getTabControl();
            Tab localTab = tabControl.getCurrentTab();
            mNotificationID = getNextNotificationId();
            Log.v(LOGTAG, "notificationId is" + mNotificationID);
            Log.v(LOGTAG, "position is" + tabControl.getCurrentPosition());

            Object localObject = (List)mTabPosition.get(localTab);
            if (localObject == null) {
                localObject = new ArrayList();
            }
            ((List)localObject).add(Integer.valueOf(mNotificationID));
            mTabPosition.put(localTab, (java.util.List<java.lang.Integer>)localObject);

            Log.v(LOGTAG, "SIZE  is " + mTabPosition.size());

            Intent intent = new Intent(BrowserActivity.ACTION_NOTIFICATIONS, null, mContext, BrowserActivity.class);
            intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
            intent.setType(Integer.toString(mNotificationID));
            intent.putExtra(BrowserActivity.EXTRA_TITLE, title);
            intent.putExtra(BrowserActivity.EXTRA_BODY, body);
            intent.putExtra(BrowserActivity.EXTRA_POSITION, tabControl.getCurrentPosition());
            intent.putExtra(BrowserActivity.EXTRA_COUNTER, id);
            intent.putExtra(Browser.EXTRA_APPLICATION_ID, mContext.getPackageName());
            PendingIntent localPendingIntent = PendingIntent.getActivity(mContext, 0, intent, PendingIntent.FLAG_UPDATE_CURRENT);
            mNotification.setLatestEventInfo(mContext, title, body, localPendingIntent);
            ...
            editor.putInt(BrowserActivity.EXTRA_SHOWN, 1);
            editor.apply();

            mNotificationPermissions.invokeNotificationID(mNotificationID, mPointer);
        }

        @Override
        public void onNotificationPermissionsHidePrompt() {
            Log.v(LOGTAG, "Inside tab.java onNotificationPermissionsHidePrompt");
            if ((mInForeground) && (mNotificationPermissionsPrompt != null)) {
                mNotificationPermissionsPrompt.hide();
            }
        }

        @Override
        public void onNotificationPermissionsShowPrompt(String string, NotificationPermissions.Callback callback) {
            Log.v(LOGTAG, "Inside tab.java onNotificationPermissionsShowPrompt");
            if (mInForeground) {
                getNotificationPermissionsPrompt().show(string, callback);
            }
        }
        /// @}
    };
    ...
    /// M : HTML5 web notification @{
    public void OnResponseBack(String event, int pointer) {
        Log.v(LOGTAG, "Inside OnResponseBack for event " + event);
        mNotificationPermissions.invokeEvent(event, pointer);
    }

    public void onTest(String title, String body, final int pointer) {
        Log.v(LOGTAG, "Inside on TEST pointer is " + pointer);
        new AlertDialog.Builder(mContext)
        .setTitle(title)
        ...
        .setPositiveButton(R.string.ok, new OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                mNotificationPermissions.invokeEvent("close", pointer);
            }
        }).show();
        OnResponseBack("display", pointer);
    }

    NotificationPermissionsPrompt getNotificationPermissionsPrompt() {
        Log.v(LOGTAG, "Inside tab.java getNotificationPermissionsPrompt");
        if (mNotificationPermissionsPrompt == null) {
            mNotificationPermissionsPrompt = ((NotificationPermissionsPrompt)((ViewStub)mContainer.findViewById(R.id.notification_permissions_prompt)).inflate());
        }
        return mNotificationPermissionsPrompt;
    }
    /// @}
    ...
    void destroy() {
        if (mMainView != null) {
            ...
            if (FeatureOption.MTK_WEB_NOTIFICATION_SUPPORT) {
                if ((!mTabPosition.isEmpty()) && (mTabPosition.containsKey(this))) {
                    NotificationManager notificationManager = (NotificationManager)mContext.getSystemService(Context.NOTIFICATION_SERVICE);
                    Log.v(LOGTAG, "mTabPosition size is " + mTabPosition.size());
                    List localList = (List)mTabPosition.get(this);
                    Log.v(LOGTAG, "List size is  " + localList.size());
                    for (int i = 0; i < localList.size(); i++) {
                        notificationManager.cancel(((Integer)localList.get(i)).intValue());
                    }
                    mTabPosition.remove(this);
                }
            }
            /// @}
            dismissSubWindow();
    ...
    public void updateBookmarkedStatus() {
        /// M: modify for site navigation @{
        if (mCurrentState.mUrl != null && mCurrentState.mUrl.equals(SiteNavigation.SITE_NAVIGATION_URI.toString())) {
            mDataController.queryBookmarkStatus(SiteNavigation.SITE_NAVIGATION_URI.toString(), mIsBookmarkCallback);
        } else {
    ...
    public void loadUrl(String url, Map<String, String> headers) {
        if (mMainView != null) {
            ...
            /// M: Operator Feature check whether the url is too long, if yes, trim the url. @{
            mBrowserSmallFeatureEx = Extensions.getSmallFeaturePlugin(mContext);
            if (mBrowserSmallFeatureEx.shouldCheckUrlLengthLimit()) {
                url = mBrowserSmallFeatureEx.checkAndTrimUrl(url);
            }
            /// @}
            mPageLoadProgress = INITIAL_PROGRESS;
            mInPageLoad = true;
