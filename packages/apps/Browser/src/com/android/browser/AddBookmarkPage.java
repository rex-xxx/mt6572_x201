...
public class AddBookmarkPage extends Activity
    ...
    private int haveToOverwriteBookmarkId(String title, String url, long parent) {
        if (!mSaveToHomeScreen && !mEditingFolder) {
            Xlog.d(XLOGTAG, "Add bookmark page haveToOverwriteBookmarkId mCurrentId:" + mCurrentId);
    ...
    private void displayAlertDialogForExistingBookmark() {
        ...
        new AlertDialog.Builder(this)
                ...
                .setMessage(AddBookmarkPage.this.getText(R.string.duplicated_bookmark_warning).toString())
    ...
    private static long getIdFromName(Context context, String name) {
        ...
        try {
            ...
            android.util.Log.e("AddBookmarkPage", "addFolderToRoot :" + e.getMessage());
        } finally {
            if (cursor != null) {
                cursor.close();
            }
    ...
    private void descendInto(String foldername, long id) {
        if (id != DEFAULT_FOLDER_ID) {
            ...
            Toast.makeText(getApplicationContext(), R.string.duplicated_folder_warning, Toast.LENGTH_LONG).show();
        }
    }
    ...
    protected void onCreate(Bundle icicle) {
        ...
        if (mMap != null) {
            ...
            if (b != null) {
                ...
                Xlog.d(XLOGTAG, "Add bookmark page onCreate mCurrentId:" + mCurrentId);
                mFakeTitle.setText(R.string.edit_bookmark);
                if (mEditingFolder) {
            ...
            Xlog.i(LOGTAG, "CurrentFolderId: " + mCurrentFolder);
        }
        ...
        /// M: Add the filter for bookmark address. @{
        ...
        if (null != optr && optr.equals("OP01")) {
            ...
            final int nAddressLimit = this.getResources().getInteger(R.integer.bookmark_address_maxlength);
            addressFilters[0] = new InputFilter.LengthFilter(nAddressLimit) {
                public CharSequence filter(CharSequence source, int start, int end,
                        Spanned dest, int dstart, int dend){
                    int keep = nAddressLimit - (dest.length() - (dend - dstart));
                    if (keep <= 0) {
                        showWarningDialog();
                        return "";
                    } else if (keep >= end - start) {
        ...
        contentFilter[0] = new InputFilter.LengthFilter(limit) {
            public CharSequence filter(CharSequence source, int start, int end,
                    Spanned dest, int dstart, int dend) {
        ...
        mFolderNamer.setFilters(contentFilter);
        /// @}

        mFolderNamer.setOnEditorActionListener(this);
        /// M: Workaround for EditText located in ListView. Disable long click. @ {
        mFolderNamer.setLongClickable(false);
        /// @ }
    ...
    private class SaveBookmarkRunnable implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                Xlog.i(LOGTAG, "mCurrentFolder: " + mCurrentFolder);
    ...
    private static class UpdateBookmarkTask extends AsyncTask<ContentValues, Void, Void> {
        ...
        protected Void doInBackground(ContentValues... params) {
            ...
            mContext.getContentResolver().update(
                    uri,
                    params[0], null, null);
            Xlog.d(XLOGTAG, "UpdateBookmarkTask doInBackground:");
            if (params.length > 1) {
        ...
        protected void onPostExecute(Void o) {
            Xlog.d(XLOGTAG, "UpdateBookmarkTask onPostExecute mBookmarkCurrentId:" + mBookmarkCurrentId);
            if (mBookmarkCurrentId > 0) {
                mContext.getContentResolver().delete(BrowserContract.Bookmarks.CONTENT_URI, 
                        BrowserContract.Bookmarks._ID + " = ?", new String[] {String.valueOf(mBookmarkCurrentId)});
            }
        }
    }
    ...
    boolean save() {
        ...
        if (mOverwriteBookmarkId > 0) {
            ...
            if (values.size() > 0) {
                new UpdateBookmarkTask(getApplicationContext(), mOverwriteBookmarkId).execute(values, valuesDeleteItem);
            }
            mOverwriteBookmarkId = -1;
            setResult(RESULT_OK);
            return true;
            ...
            if (mEditingExisting && mEditingFolder) {
                Xlog.d(XLOGTAG, "editing folder save");
                ...
                if (beforeParentId == currentParentId) {
                    ...
                    if (beforeEditTitle != null && beforeEditTitle.equals(title)) {
                        ...
                        Xlog.d(XLOGTAG, "edit folder save, does not change anything");
                        return true;
                    }
                }
            }
