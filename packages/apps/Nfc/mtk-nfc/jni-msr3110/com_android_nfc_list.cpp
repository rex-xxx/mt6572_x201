...
bool listInit(listHead* pList)
{
   ...
   {
      LogError("Mutex creation failed (errno=0x%08x)", errno);
      return false;
   }
...
bool listDestroy(listHead* pList)
{
   ...
   {
      LogError("Mutex destruction failed (errno=0x%08x)", errno);
      return false;
   }
...
bool listRemove(listHead* pList, void* pData)
{
   ...
   {
      /* Empty list */
      LogError("Failed to deallocate (list empty)");
      result = false;
      goto clean_and_return;
   }
   ...
   else
   {
      ...
      if (pNode->pNext == NULL)
      {
         /* Node not found */
          result = false;
          LogError("Failed to deallocate (not found %8p)", pData);
          goto clean_and_return;
      }
