...
import com.mediatek.nfc.wps.NfcBroadcastReceiver;
...
public class NfcService implements DeviceHostListener {
    ...
    public NfcService(Application nfcApplication) {
        ...
        {
            MtkHandoverManager.createSingleton(mContext,handoverManager);

            // This Activity is just for testing, does not need.
            // regist and init a broadcast receiver
            mBroadcastReceiver = new NfcBroadcastReceiver(mContext.getApplicationContext());
    ...
    class EnableDisableTask extends AsyncTask<Integer, Void, Void> {
        /// M: @ {
        EnableDisableTask() {}
        EnableDisableTask(boolean disableCardMode) {
            mDisableCardMode = disableCardMode;
        }
        private boolean mDisableCardMode;
        /// }
        @Override
        ...
        boolean disableInternal() {
            ...
            /// M : @ {
            /// Google seems to have a bug here
            /// the polling loop should be stopped, but it won't 
            /// original code is listed below:
            /* 
            applyRouting(true);
            */
            /// thus we change to below
            synchronized(NfcService.this) {
                MtkNfcAddonSequence.getInstance().applyDisableSequence(mDisableCardMode);
                Log.d(TAG, "NFC-C OFF (bug fix for Google)");
    ...
    /// M: @{
        //createSimStateObserver move to NFCAppliction.java

	private void initAddon() {
		IntentFilter filter;
        MtkNfcAddonSequence.createSingleton(mDeviceHost, mPrefs, mPrefsEditor, mContext);
	...
	private final BroadcastReceiver mAddonBroadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
			...
			if (action.equals("android.intent.action.ACTION_BOOT_IPO")) {
				...
				if (mPrefs.getBoolean(PREF_NFC_ON, NFC_ON_DEFAULT)) {
					Log.d(TAG, "should turn on NFC");
					new EnableDisableTask().execute(TASK_ENABLE);
				}
            } else if (action.equals("com.mediatek.test.nfc.idv.switch")) {
				Log.d(TAG, "Nfc individual mode switch");
				int mode = intent.getIntExtra("com.mediatek.test.nfc.idv.switch.mode", -1);
				int flag = intent.getIntExtra("com.mediatek.test.nfc.idv.switch.flag", -1);
				if (mode == -1 || flag == -1) {
					Log.d(TAG, "getIntExtra fail");
				} else {
					MtkNfcAddonSequence.getInstance().setModeFlag(isNfcEnabled(), mode, flag, NfcService.this);
				}
			}
