...
public class P2pLinkManager implements Handler.Callback, P2pEventListener.Callback {
    ...
    /// M: @ {   
    //static MtkHandoverManager mMtkHandoverManager;
    static String mHandoverUseCase;
    /// }
        ...
        synchronized (P2pLinkManager.this) {
            ...
            switch (mLinkState) {
                case LINK_STATE_DOWN:
                    ...
                    /// M: @ {   
                    MtkHandoverManager.getInstance().setP2pState(mSendState);
                    /// }
        ...
        synchronized (P2pLinkManager.this) {
            ...
            if (mCallbackNdef != null) {
                try {
                    ...
                    /// M: @ {   
                    if(MtkHandoverManager.BEAM_PLUS_SUPPORT == true){
                        mHandoverUseCase = mCallbackNdef.getUseCase();
                        if(MtkHandoverManager.getInstance().isWLSelector(mHandoverUseCase))
                            mUrisToSend = null;
                        Log.d(TAG, "prepareMessageToSend  mHandoverUseCase:"+mHandoverUseCase+"    mUrisToSend:"+mUrisToSend);
                    }
                    /// }
    ...
    NdefMessage createDefaultNdef(String pkgName) {
        ...
        /// M: @ {    
        if (DBG) Log.d(TAG, "==== createDefaultNdef  pkgName:" + pkgName);
        /// }
    ...
    static int doSnepProtocol(HandoverManager handoverManager,
        ...
        try {
            if (uris != null) {
                ...
                /// M: @ { 
                if((MtkHandoverManager.BEAM_PLUS_SUPPORT == true)&&(mHandoverUseCase != null)){
                    if(MtkHandoverManager.getInstance() == null){
                        Log.e(TAG, " doSnepProtocol : MtkHandoverManager.getInstance() == null " );
                        return SNEP_FAILURE;
                    }
                    
                    try{
                        MtkHandoverManager.getInstance().checkSendstateSending(SEND_STATE_SENDING);
                        MtkHandoverManager.getInstance().checkSendstateNTS(SEND_STATE_NOTHING_TO_SEND);
                        MtkHandoverManager.getInstance().checkSendstateNC(SEND_STATE_NEED_CONFIRMATION);
                    }catch (Exception ex){
                        Log.e(TAG, "Exception:"+ex);
                    }
        
                    request = MtkHandoverManager.getInstance().CreateHrMEntry(uris,mHandoverUseCase);
                ...
                if (request != null) {
                    ...
                    if (response == null) {
                        ...
                        /// M: @ { 
                        Log.d(TAG, " Get snepResponse ....."+snepResponse);
                        Log.d(TAG, "     snepResponse to NdefMessage....."+response);
                        /// }
                    }

                    /// M: @ { 
                    Log.d(TAG, " Get handoverClient Response ....."+response);
                    /// }
                ...
                if (response != null) {
                    ...
                    /// M: @ { 
                    if((MtkHandoverManager.BEAM_PLUS_SUPPORT == true)&&(mHandoverUseCase != null))
                        MtkHandoverManager.getInstance().doHsMHandoverEntry(uris, response,mHandoverUseCase);
                    else/// }
                    ...
                    /// M: @ { 
                    Log.d(TAG, "!!!!   (response ==null)   return SNEP_HANDOVER_UNSUPPORTED.. not to do doHsMHandoverEntry !!!!");
                    /// }
    ...
    final HandoverServer.Callback mHandoverCallback = new HandoverServer.Callback() {
        ...
        public void onHandoverRequestReceived() {
            /// M: @ {    
            Log.d(TAG, "====  HandoverServer.Callback  ====  call onReceiveHandover");
            /// }        
            onReceiveHandover();
        }
    };
    ...
    final NdefPushServer.Callback mNppCallback = new NdefPushServer.Callback() {
        ...
        public void onMessageReceived(NdefMessage msg) {
            ...
            /// M: @ {    
            Log.d(TAG, "====  NdefPushServer.Callback mNppCallback ====  call onReceiveComplete");
            /// }
    ...
    final SnepServer.Callback mDefaultSnepCallback = new SnepServer.Callback() {
        ...
        public SnepMessage doPut(NdefMessage msg) {
            ...
            /// M: @ {    
            Log.d(TAG, "====  doPut ====     call onReceiveComplete");
            /// }
        ...
        public SnepMessage doGet(int acceptableLength, NdefMessage msg) {
            ...
            /// M: @ {    
            Log.d(TAG, "====  doGet ==== tryMtkHandoverRequest  then     call onReceiveHandover");
			if(MtkHandoverManager.BEAM_PLUS_SUPPORT == true)
	            response = MtkHandoverManager.getInstance().tryMtkHandoverRequest(msg);
			else/// }
            ...
            if (response != null) {
                onReceiveHandover();
                /// M: @ {
                Log.d(TAG, "====  doGet ====  success End ");
                /// }
                return SnepMessage.getSuccessResponse(response);
            } else {
                /// M: @ {
                Log.d(TAG, "====  doGet ====   (response == null)   Fail End return  RESPONSE_NOT_FOUND ");
                /// }
    ...
    public boolean handleMessage(Message msg) {
        /// M: @ {    
        Log.d(TAG, "handleMessage "+describeP2pMessage(msg)
                +"  mLinkState:"+linkStateToString(mLinkState)+"  mSendState:"+sendStateToString(mSendState));
        /// }
        switch (msg.what) {
            ...
            case MSG_RECEIVE_HANDOVER:
                ...
                synchronized (this) {
                    ...
                    /// M: @ {   
                    if(MtkHandoverManager.BEAM_PLUS_SUPPORT == true)
                        MtkHandoverManager.getInstance().closeForegroundDispatchActivity(mHandoverUseCase);
                    /// }
    ...
    public void onP2pSendConfirmed() {
        ...
        /// M: @ {   
        Log.d(TAG, "onP2pSendConfirmed() mSendState:" +sendStateToString(mSendState));
        /// }        
        synchronized (this) {
    ...
    /// M: @ {   
	static String describeP2pMessage(Message msg) {
		String stateStr = "";
        
		switch (msg.what) {
		case MSG_DEBOUNCE_TIMEOUT:
			stateStr = "====  MSG_DEBOUNCE_TIMEOUT  ####";
			break;
		case MSG_RECEIVE_COMPLETE:
			stateStr = "====  MSG_RECEIVE_COMPLETE  !!!!";
			break;
		case MSG_RECEIVE_HANDOVER:
			stateStr = "====  MSG_RECEIVE_HANDOVER  ====";
			break;
		case MSG_SEND_COMPLETE:
			stateStr = "====  MSG_SEND_COMPLETE  ++++";
			break;
		case MSG_START_ECHOSERVER:
			stateStr = "==== MSG_START_ECHOSERVER";
			break;
		case MSG_STOP_ECHOSERVER:
			stateStr = "==== MSG_STOP_ECHOSERVER";
			break;
		case MSG_HANDOVER_NOT_SUPPORTED:
			stateStr = "====  MSG_HANDOVER_NOT_SUPPORTED  ****";
			break;      
		default:
			stateStr = " <error> ";
			break;                  
