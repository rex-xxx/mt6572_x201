...
public class HandoverService extends Service implements HandoverTransfer.Callback,
    ...
    /// M: @ {
    void sendBtOutgoingIntent(String url, String mimeType, boolean result, long doneBytes, long totalBytes) {

        Log.d(TAG, "sendBtOutgoingIntent()  url:"+url+" result:"+result+" doneBytes:"+doneBytes+" totalBytes:"+totalBytes );

        Intent outgoingIntent = new Intent(FilePushRecord.ACTION_BT_OUTGOING);

		outgoingIntent.putExtra(FilePushRecord.EXTRA_URL, url);
		outgoingIntent.putExtra(FilePushRecord.EXTRA_TYPE, mimeType);
		outgoingIntent.putExtra(FilePushRecord.EXTRA_RESULT, result);
		outgoingIntent.putExtra(FilePushRecord.EXTRA_DONE_BYTES, doneBytes);
		outgoingIntent.putExtra(FilePushRecord.EXTRA_TOTAL_BYTES, totalBytes);

        sendBroadcast(outgoingIntent);
    }

    @Override
    public void sendBtIncomingIntent(String url, String mimeType, boolean result, long doneBytes, long totalBytes) {

        Log.d(TAG, "sendBtIncomingIntent()  url:"+url+" result:"+result+" doneBytes:"+doneBytes+" totalBytes:"+totalBytes );

        Intent incomingIntent = new Intent(FilePushRecord.ACTION_BT_INCOMING);

		incomingIntent.putExtra(FilePushRecord.EXTRA_URL, url);
		incomingIntent.putExtra(FilePushRecord.EXTRA_TYPE, mimeType);
		incomingIntent.putExtra(FilePushRecord.EXTRA_RESULT, result);
		incomingIntent.putExtra(FilePushRecord.EXTRA_DONE_BYTES, doneBytes);
		incomingIntent.putExtra(FilePushRecord.EXTRA_TOTAL_BYTES, totalBytes);
            ...
            if (action.equals(BluetoothAdapter.ACTION_STATE_CHANGED)) {
                ...
                if (action.equals(ACTION_BT_OPP_TRANSFER_DONE)) {
                    ...
                    if (handoverStatus == HANDOVER_TRANSFER_STATUS_SUCCESS) {
                        ...
                        /// M: @ {
                        /// TRANSFER SUCCESS
                        /// only record outgoing transfer here, the incoming case is left for processFiles

                        Log.d(TAG, "[TRANSFER SUCCESS] FilePushRecord.insertBtOutgoingRecord incoming:"+ incoming +"  uriString:"+uriString);   
                        
                        if (!incoming) {                            
                            try {
                                File f = new File(uri.getPath());
                                sendBtOutgoingIntent(uriString, mimeType, true, f.length(), f.length());
                            } catch (Exception e) {
                                e.printStackTrace();
                            }
                        }
                        ...
                        /// M: @ {
						/// TRANSFER FAIL
                        String uriString = intent.getStringExtra("com.mediatek.bluetooth.opp.extra.BT_OPP_TRANSFER_OBJECT_NAME");
                        String mimeType = intent.getStringExtra(EXTRA_BT_OPP_TRANSFER_MIMETYPE);
                        long fileSize = intent.getLongExtra("com.mediatek.bluetooth.opp.extra.BT_OPP_TRANSFER_FILE_SIZE", 0);
                        long doneSize = intent.getLongExtra("com.mediatek.bluetooth.opp.extra.BT_OPP_TRANSFER_DONE_SIZE", 0);

                        Log.d(TAG, "[TRANSFER FAIL] FilePushRecord.insertBtxxRecord incoming:"+ incoming +"  uriString:"+uriString);   
                        
                        if (incoming) {
                            //FilePushRecord.getInstance().insertBtIncomingRecord(uriString, mimeType, false, doneSize, fileSize);
                            sendBtIncomingIntent(uriString, mimeType, false, doneSize, fileSize);
                        } else {
                            sendBtOutgoingIntent(uriString, mimeType, false, doneSize, fileSize);
                        }
						/// }                    
