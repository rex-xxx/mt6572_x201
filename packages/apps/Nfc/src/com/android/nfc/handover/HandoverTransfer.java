...
public class HandoverTransfer implements Handler.Callback,
    ...
    interface Callback {
        ...
        /// M: @ {
        void sendBtIncomingIntent(String url, String mimeType, boolean result, long doneBytes, long totalBytes);
        /// }
    ...
    void processFiles() {
        ...
        for (int i = 0; i < mBtUris.size(); i++) {
            ...
            if (!srcFile.renameTo(dstFile)) {
                ...
                /// M: @ {
                //FilePushRecord.getInstance().deleteRecord(srcFile,mimeType);
                long dstLength = dstFile.length();
                Log.d(TAG, "mimeType::" + mimeType);
                Log.d(TAG, "dstFile.length():" + dstFile.length()+"  dstFile.getAbsolutePath():" + dstFile.getAbsolutePath());                

                mCallback.sendBtIncomingIntent(dstFile.getAbsolutePath(), mimeType, true, dstLength, dstLength);
                /// }                
            }
        }
