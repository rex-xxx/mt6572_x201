...
import com.mediatek.contacts.list.MultiContactsShareFragment;
...
public class NfcShareContactsHandler implements NfcAdapter.CreateNdefMessageCallback {
    ...
    private final MultiContactsShareFragment mMultiContactsShareFragment;

    public static void register(Activity activity, MultiContactsShareFragment multiContactsShareFragment) {
        NfcAdapter adapter = NfcAdapter.getDefaultAdapter(activity.getApplicationContext());
        if (adapter == null) {
            return;  // NFC not available on this device
        }
        adapter.setNdefPushMessageCallback(new NfcShareContactsHandler(multiContactsShareFragment), activity);
    }

    public NfcShareContactsHandler(MultiContactsShareFragment multiContactsShareFragment) {
...
//            final String lookupKey = Uri.encode(contactUri.getPathSegments().get(2));
            ...
            // of determining this.
            shareUri = Contacts.CONTENT_MULTI_VCARD_URI.buildUpon().
