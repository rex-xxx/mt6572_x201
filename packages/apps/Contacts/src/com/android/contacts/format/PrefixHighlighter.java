...
import com.android.contacts.format.FormatUtils.OffsetValues;
...
public class PrefixHighlighter {
    ...
    public CharSequence apply(CharSequence text, char[] prefix) {
        OffsetValues offsetValue = new OffsetValues(0);
        int index = FormatUtils.indexOfWordPrefix(text, prefix, offsetValue);
        if (index != -1) {
            ...
            result.setSpan(mPrefixColorSpan, index, index + prefix.length + offsetValue.mOffset, 0 /* flags */);
            return result;
        } else {
            return text;
        }
