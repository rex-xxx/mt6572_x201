...
public class ContactSaveService extends IntentService {
    ...
    */
        ...
        if (slotId >= 0) {
            try {
                ...
                if (e.getErrorType() == USIMGroupException.GROUP_NAME_OUT_OF_BOUND) {
                    callbackIntent.putExtra(ContactEditorFragment.SAVE_MODE_EXTRA_KEY, SaveMode.RELOAD);
                } else if (e.getErrorType() > 0) {
                    callbackIntent.putExtra(GroupEditorFragment.SHOW_TOAST_EXTRA_KEY, false);
                }
    ...
    private void deleteGroup(Intent intent) {
        ...
        if (slotId >= 0 && !TextUtils.isEmpty(groupLabel)) {
            ...
            if (ugrpId > 0) {
                ...
                Log.i(TAG, "[deleteGroup]simId:" + simId + "|member count:" + (c == null ? "null" : c.getCount()));
                try {
    ...
    private void updateGroup(Intent intent) {
        ...
        if (groupId > 0) {
            ...
            removeMembersFromGroup(resolver, rawContactsToRemove, groupId, simIndexToRemoveArray,
                    slotId, ugrpId);
    ...
    */
     ...
     * Creates an intent that can be sent to this service to delete a group.
     */
    public static Intent createGroupDeletionIntent(Context context, long groupId, int simId,
            int slotId, String groupLabel) {
     ...
     *   Descriptions: 
     */
    public void bufferOperations(ArrayList<ContentProviderOperation> operations,
    ...
    private boolean checkGroupNameExist(String groupName, String accountName,
            String accountType, boolean showTips) {
