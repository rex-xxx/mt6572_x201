...
public class QuickContactActivity extends Activity {
    ...
    protected void onCreate(Bundle icicle) {
        ...
        if (FeatureOption.MTK_THEMEMANAGER_APP) {
        	int textColor = this.getResources().getThemeMainColor();
        	if (textColor != 0) {
        		 mSelectedTabRectangle.setBackgroundColor(textColor);
        		 mLineAfterTrack.setBackgroundColor(textColor);
        	}
        }
        // The previous lines are provided and maintained by Mediatek Inc.
    ...
    private View inflateAction(String mimeType, ResolveCache resolveCache, ViewGroup root) {
        ...
        /** M: New Feature unClickable when it only 1 type @{ */
        Log.i(TAG, "mSortedActionMimeTypes : " + mSortedActionMimeTypes);
        if (null != mSortedActionMimeTypes && mSortedActionMimeTypes.size() > 1) {
            Log.i(TAG, "mSortedActionMimeTypes.size() : " + mSortedActionMimeTypes.size());
    ...
    private LoaderCallbacks<Contact> mLoaderCallbacks =
        ...
        public void onLoadFinished(Loader<Contact> loader, Contact data) {
            ...
            if (isFinishing()) {
                Log.w(TAG, "#onLoadFinished(): activity is finishing!");
                close(false);
                return;
            }
            if (data.isError()) {
...
//                throw new IllegalStateException("Failed to load contact", data.getException());
                /** M: catch lookupuri type error exception ALPS00578060 @{ */
                Log.w(TAG,"#onLoadFinished(): lookuprui's type error! lookupUri: "
                                + ((ContactLoader) loader).getLookupUri());
                Toast.makeText(QuickContactActivity.this,
                        R.string.fail_reason_unknown, Toast.LENGTH_LONG).show();
                close(false);
                return;
                /**@}*/
            }
            if (data.isNotFound()) {
                Log.w(TAG, "No contact found: " + ((ContactLoader)loader).getLookupUri());
    ...
    private class ViewPagerAdapter extends FragmentPagerAdapter {
        ...
        public Fragment getItem(int position) {
             ...
             */
            QuickContactListFragment fragment = new QuickContactListFragment(QuickContactActivity.this);
            /*
             * New Feature  by Mediatek End.
             */
            ...
            final Runnable startAppRunnable = new Runnable() {
                ...
                public void run() {
                    try {
                         ...
                         */
                        if (alternate) {
                            Intent intent = action.getAlternateIntent();
