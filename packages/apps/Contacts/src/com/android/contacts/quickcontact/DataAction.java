        ...
        if (item.hasKindTypeColumn()) {
            final int typeValue = item.getKindTypeColumn();
                ...
                if (SimUtils.isAasPhoneType(typeValue)) {
                    int slotId = ExtensionManager.getInstance().getContactAccountExtension().getCurrentSlot(ExtensionManager.COMMD_FOR_AAS);
                    mSubtitle = (String) ExtensionManager.getInstance().getContactAccountExtension().getTypeLabel(
                            context.getResources(), typeValue, item.getContentValues().getAsString(Data.DATA3), slotId, ExtensionManager.COMMD_FOR_AAS);
            } else {
                /** M: @ } */
