...
public class GroupMembershipView extends LinearLayout
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            */
            Log.i(TAG, "getView position : " + position + " | mAccountType : "
                    + mAccountType);
            if (null != mAccountType && !mAccountType.equals(mUsimAccountType)
                    && !getItemIsCheckable(position)) {
                checkedTextView.setCheckMarkDrawable(null);
    ...
    public void setKind(DataKind kind) {
        ...
        if (FeatureOption.MTK_THEMEMANAGER_APP) {
            ...
            if (textColor != 0) {
                kindTitle.setTextColor(textColor);
            }
        }
        // MTK_THEMEMANAGER_APP
    ...
    private void changeAccountType(String accountName, String accountTpye, boolean needChange) {
        ...
        if (needChange) {
            if (!PhoneCapabilityTester.isUsingTwoPanes(mContext) && accountName == null
                    && accountTpye == null) {
                accountTpye = AccountType.ACCOUNT_TYPE_LOCAL_PHONE;
                accountName = AccountType.ACCOUNT_NAME_LOCAL_PHONE;
            }
        } else {
            if (!PhoneCapabilityTester.isUsingTwoPanes(mContext) && mAccountType == null
                    && mAccountName == null) {
                mAccountType = AccountType.ACCOUNT_TYPE_LOCAL_PHONE;
