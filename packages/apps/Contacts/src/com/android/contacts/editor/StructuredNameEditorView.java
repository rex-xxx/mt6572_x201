...
public class StructuredNameEditorView extends TextFieldsEditorView {
    ...
    /**
     * M:restore the StructuredNameDataItem in
     * {@link #onRestoreInstanceState(Parcelable)} This is fix for
     * ALPS00477285.if the dataItem is not an instanceof StructuredNameDataItem.
     * It will initial the item in
     * {@link #setValues(DataKind kind, ValuesDelta entry, RawContactDelta state, boolean readOnly, ViewIdGenerator vig)}
     * @param ss The SavedState the passed from
     *            {@link #onRestoreInstanceState(Parcelable)}
     */
    private void restoreStructuredNameDataItem(SavedState ss){
        DataItem dataItem = DataItem.createFrom(null, ss.mSnapshot);

        if (dataItem instanceof StructuredNameDataItem) {
            mSnapshot = (StructuredNameDataItem) dataItem;
        } else {
            Log.w(TAG,"#onRestoreInstanceState(),the dataItem is not an instance of StructuredNameDataItem!!! mimeType: "
                            + ss.mSnapshot.getAsString(Data.MIMETYPE));
        }
    }
}
