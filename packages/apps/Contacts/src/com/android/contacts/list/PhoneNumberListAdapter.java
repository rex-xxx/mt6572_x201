...
public class PhoneNumberListAdapter extends ContactEntryListAdapter {
    ...
    protected void bindPhoneNumber(ContactListItemView view, Cursor cursor) {
        ...
        if (!cursor.isNull(PhoneQuery.PHONE_TYPE)) {
            ...
            final int simId = cursor.getInt(PhoneQuery.PHONE_INDICATE_PHONE_SIM_COLUMN_INDEX);
            final int slotId = SIMInfoWrapper.getDefault().getSimSlotById(simId);
            label = ExtensionManager.getInstance().getContactAccountExtension().getTypeLabel(
                    getContext().getResources(), type, customLabel, slotId, ExtensionManager.COMMD_FOR_AAS);
            /** M: @ } */
        }
        view.setLabel(label);
    ...
    protected void bindPhoto(final ContactListItemView view, Cursor cursor) {
        ...
        if (indicatePhoneSim > 0) {
            photoId = getSimType(indicatePhoneSim, cursor.getInt(PhoneQuery.PHONE_IS_SDN_CONTACT));
        }
        /*
