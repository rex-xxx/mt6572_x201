...
public class ContactTileAdapter extends BaseAdapter {
    ...
    private void bindColumnIndices() {
        ...
        if (mDisplayType == DisplayType.GROUP_MEMBERS) {
            ...
            mIsSdnContact = GroupDetailQuery.CONTACT_INDEX_IS_SDN_CONTACT;
        } else {
            ...
            mIsSdnContact = ContactTileLoaderFactory.PHONE_IS_SDN_CONTACT;
        }
    }
    ...
    private ContactEntry createContactEntryFromCursor(Cursor cursor, int position) {
        ...
        int i = -1;
        Log.i(TAG, "[createContactEntryFromCursor] mSlot = " + slotId);
        ...
        if (slotId < 0) {
            ...
            int isSdnContact = cursor.getInt(mIsSdnContact);
            SIMInfo simInfo = mSimInfoWrapper.getSimInfoBySlot(slotId);
            if (simInfo != null) {
                i = simInfo.mColor;
            }
            contact.photoUri = Uri.parse(new SimContactPhotoUtils().getPhotoUri(isSdnContact, i));
            Log.i(TAG, "[createContactEntryFromCursor] i = " + i + " | isSdn : " + isSdnContact);
        }
        /*
         * New feature by Mediatek End
         */
        ...
        if (mDisplayType == DisplayType.STREQUENT_PHONE_ONLY) {
            ...
            contact.phoneLabel = (String) ExtensionManager.getInstance().getContactAccountExtension().getTypeLabel(
                    mResources, phoneNumberType, phoneNumberCustomLabel, slotId, ExtensionManager.COMMD_FOR_AAS);
            Log.i(TAG, "createContactEntryFromCursor() contact.phoneLabel=" + contact.phoneLabel);
            /** M: @ } */
