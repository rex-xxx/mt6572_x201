...
public abstract class ContactEntryListFragment<T extends ContactEntryListAdapter>
    ...
    public void onCreate(Bundle savedState) {
        ...
        intentFilter.addAction(ExtensionManager.RCS_CONTACT_PRESENCE_CHANGED);
        getActivity().registerReceiver(mRCSPresenceChangedListener, intentFilter);
        /*
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
        if (!mEnabled) {
            Log.i(TAG,"return in onLoad finish ");
            return;
        }
        ...
        if (loaderId == DIRECTORY_LOADER_ID) {
            ...
            Log.i(TAG,"onLoadFinished startloading");
            startLoading();
        } else {
    ...
    protected void onPartitionLoaded(int partitionIndex, Cursor data) {
        if (partitionIndex >= mAdapter.getPartitionCount()) {
            ...
            Log.i(TAG,"[onPartitionLoaded] return");
            return;
        }
        Log.i(TAG,"[onPartitionLoaded]");
    ...
    private void configureVerticalScrollbar() {
        ...
        if (mListView != null) {
           ...
           // mListView.setFastScrollAlwaysVisible(hasScrollbar);
            /** M: Fixed CR ALPS00571691, not always show fast scroll bar */
            mListView.setFastScrollAlwaysVisible(false);
            Log.d(TAG, "configureVerticalScrollbar,setFastScrollAlwaysVisible always false.");
     ...
     *   Descriptions: RCS
     */
    private long mPreReceiveTime = 0;
    private static final long WAIT_TIME = 10000;
    private BroadcastReceiver mRCSPresenceChangedListener = new BroadcastReceiver() {
    ...
    public void onDestroy() {
        super.onDestroy();
        Log.i(TAG,"onDestroy");
        getActivity().unregisterReceiver(mRCSPresenceChangedListener);
    }
    /*
     * New Feature by Mediatek End.
     */
    ...
    /**
     * in some case,restore the ListView last user will replace the new state.
