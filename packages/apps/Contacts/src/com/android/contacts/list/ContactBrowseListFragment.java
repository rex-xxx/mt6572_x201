...
public abstract class ContactBrowseListFragment extends
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
        if (isSearchMode()) {
            Log.i(TAG, "[Performance test][Contacts] Search contacts end [" + System.currentTimeMillis() + "] ");
        }
        ...
        if (selectedPosition != -1) {
            ...
            if (isSearchMode()) {
                ...
                if (mFilter != null
                    ...
                    reloadData();
                // Add this case to reloadData for editing profile
                } else if (mSelectedContactLookupKey != null) {
                    if ((mSelectedContactLookupKey.equals("profile"))
                            && (PhoneCapabilityTester.isUsingTwoPanes(getContext()))) {
                        reloadData();
                    }
                } else {
