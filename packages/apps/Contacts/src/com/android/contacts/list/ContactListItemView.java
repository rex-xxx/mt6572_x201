...
/** M: New Feature Phone Landscape UI @{ */
        if (PhoneCapabilityTester.isUsingTwoPanes(mContext))
        {
              ...
              setPadding(
                      a.getDimensionPixelOffset(40, 15),
...
/** @ }*/
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        ExtensionManager.getInstance().getContactDetailExtension().measureExtentionIcon(mExtentionIcon, ExtensionManager.COMMD_FOR_RCS);
        /*
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        rightBound = ExtensionManager.getInstance().getContactDetailExtension()
                .layoutExtentionIcon(leftBound, topBound, bottomBound, rightBound,
                        mGapBetweenImageAndText, mExtentionIcon, ExtensionManager.COMMD_FOR_RCS);
        ...
        if (!TextUtils.isEmpty(title)) {
            if (mHeaderTextView == null) {
                ...
                if (FeatureOption.MTK_THEMEMANAGER_APP) {
                    ...
                    if (textColor != 0) {
                        mHeaderTextView.setTextColor(textColor);
                    } else {
                ...
                //MTK_THEMEMANAGER_APP
                mHeaderTextView.setSingleLine(true);
            ...
            if (mHeaderDivider == null) {
                ...
                if (FeatureOption.MTK_THEMEMANAGER_APP) {
                    ...
                    int textColor = res.getThemeMainColor();
                    if (textColor != 0) {
                        mHeaderDivider.setBackgroundColor(textColor);
                    } else {
...
//                mLabelView.setTextSize(TypedValue.COMPLEX_UNIT_SP, mCountViewTextSize);
    ...
    public void showSnippet(Cursor cursor, int summarySnippetColumnIndex) {
         ...
         *   
         *   CR ID: ALPS00461043
         *   Descriptions:Should not do client side snippeting if provider didn't do it
         */
        String snippet = cursor.getString(summarySnippetColumnIndex);
        Bundle extras = cursor.getExtras();
        if (!extras.getBoolean(ContactsContract.DEFERRED_SNIPPETING) && snippet != null) {
            int from = 0;
    ...
    public void getExtentionIcon() {
        if (mExtentionIcon == null){
            mExtentionIcon = new ImageView(mContext);
        }
        mExtentionIcon.setBackgroundDrawable(null);
        
        ExtensionManager.getInstance().getContactListExtension().setExtentionImageView(
