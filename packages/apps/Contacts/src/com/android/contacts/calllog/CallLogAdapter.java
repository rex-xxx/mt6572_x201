   ...
   */
    ...
    protected void bindView(View view, Cursor c, int count) {
        ...
        if (VvmUtils.isVvmEnabled() && details.callType == Calls.VOICEMAIL_TYPE) {
            ...
            itemView.getCallButton().setTag(IntentProvider.getPlayVoicemailIntentProvider(rowId, voicemailUri));
        } else {
        ...
        if (isEmergencyNumber || isVoiceMailNumber) {
            ...
            int isSdnContact=-1;
            if (c != null) {
                isSdnContact = c.getInt(CallLogQuery.CALLS_JOIN_DATA_VIEW_IS_SDN_CONTACT);
            }
...
//            Log.i(TAG, "[bindView] i = " + i + " | isSdnContact : " + isSdnContact);
            contactInfo.photoId = new SimContactPhotoUtils().getPhotoId(isSdnContact, i);
        }
...
//            Log.i(TAG,"[bindView] number is "+number);
    ...
    protected void setPhoto(QuickContactBadgeWithPhoneNumber view, long photoId, Uri contactUri) {
        ...
        mContactPhotoManager.loadThumbnail(view, photoId, true);
    }
    
    protected void setPhoto(QuickContactBadgeWithPhoneNumber view, long photoId, String number,
            boolean isSipCallNumber) {
    ...
    protected void bindPlayButtonView(CallLogListItemView itemView) {
        itemView.getCallButton().setImageResource(R.drawable.ic_play_holo_dark);
    ...
    protected void bindCallButtonView(CallLogListItemView itemView, PhoneCallDetails details) {
        ...
        itemView.getCallButton().setImageResource(R.drawable.ic_ab_dialer_holo_dark);
        itemView.getCallButton().setColorFilter(Color.TRANSPARENT);
        /** @} */
        if (!TextUtils.isEmpty(details.number)) {
            ...
            itemView.getCallButton().setTag(
                    IntentProvider.getReturnCallIntentProvider((String) details.number,
                            (long) details.simId));
        } else {
            // No action enabled.
