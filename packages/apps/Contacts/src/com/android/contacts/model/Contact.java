...
public class Contact {
    ...
    private Contact(Uri requestedUri, Contact.Status status, Exception exception) {
        ...
        /** M: New Feature: JB MR1 Migration @{ */
        initSIMMembers();
        /** @} */
    }
    ...
    private boolean mBlockVideoCall = false;
    ...
    public Contact(Uri requestedUri, Uri uri, Uri lookupUri, long directoryId, String lookupKey,
            ...
            String photoUri, String displayName, String altDisplayName, String phoneticName,
            boolean starred, Integer presence, boolean sendToVoicemail, String customRingtone,
            boolean isUserProfile, int isSdnContact, boolean blockVideoCall) {
        this(requestedUri, uri, lookupUri, directoryId, lookupKey,id, nameRawContactId,
                displayNameSource, photoId, photoUri, displayName, altDisplayName, phoneticName,
                starred, presence, sendToVoicemail, customRingtone, isUserProfile);
        mIsSdnContact = isSdnContact;
        mBlockVideoCall = blockVideoCall;
    }

    /**
     * Initialize the SIM related members for MTK definitions.
     */
    private void initSIMMembers() {
        mIndicatePhoneOrSimContact = -1;
        mSimIndex = -1;
        mBlockVideoCall = false;
        mIsSdnContact = 0;
    }

    private void initSIMMembers(Contact from) {
        Log.i(TAG, "initSIMMembers begin mIndicatePhoneOrSimContact , mSimIndex , mBlockVideoCall, mIsSdnContact : "
                + mIndicatePhoneOrSimContact + " , " + mSimIndex + " , " + mBlockVideoCall + " , " + mIsSdnContact);
        mIndicatePhoneOrSimContact = from.mIndicatePhoneOrSimContact;
        mSimIndex = from.mSimIndex;
        mBlockVideoCall = from.mBlockVideoCall;
        mIsSdnContact = from.mIsSdnContact;
        Log.i(TAG, "initSIMMembers end mIndicatePhoneOrSimContact , mSimIndex , mBlockVideoCall, mIsSdnContact : "
                + mIndicatePhoneOrSimContact + " , " + mSimIndex + " , " + mBlockVideoCall + " , " + mIsSdnContact);
    }

    /**
     * Return if block video call
     */
    public boolean isBlockVideoCall() {
        return mBlockVideoCall;
    }

    /**
     * Set IndicatePhoneOrSimContact's value
     */
    public void setIndicate(int indicate) {
        mIndicatePhoneOrSimContact = indicate;
    }

    /**
     * Set index value for contact of sim card
     */
    ...
    /**
     * Get IndicatePhoneOrSimContact's value
     */
    ...
    /**
     * Set contact's slot value with sim card
     */
    ...
    /**
     * Get contact's slot value with sim card
     */
    public int getSlot() {
        return mSlot;
    }
