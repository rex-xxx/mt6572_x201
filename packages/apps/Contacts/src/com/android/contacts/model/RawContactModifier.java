...
public class RawContactModifier {
    ...
    public static ValuesDelta ensureKindExists(
        ...
        /** M:AAS @ { */
        if (SimUtils.ensureAASKindExists(state, accountType, mimeType, kind)) {
            return null;
        }
        /** M: @ } */
    ...
    private static ArrayList<EditType> getValidTypes(RawContactDelta state, DataKind kind,
        ...
        for (EditType type : kind.typeList) {
            /**
             * CR:ALPS00385770 M:change "overallCount < kind.typeOverallMax"
             * into "overallCount <= kind.typeOverallMax" in the next line ;
             * 
             * @{
             */
            final boolean validOverall = (kind.typeOverallMax == -1 ? true : overallCount <= kind.typeOverallMax);
            /**
             * @} Google default codes: overallCount < kind.typeOverallMax.
             * Cause: 1. overallCount is the current total items of the same
             * mimetypes of state, here when equal is to ensure rebuild
             * type-list; 2. all valid types be rebulit is ready for next time
             * bestvalidtypes.
             * */
            final boolean validSpecific = (type.specificMax == -1 ? true
                    : typeCount.get(type.rawValue) < type.specificMax);
    ...
    public static EditType getCurrentType(ValuesDelta entry, DataKind kind) {
        ...
        /** M:AAS @ { */
        if (SimUtils.isAasPhoneType(rawValue.intValue())) {
            return SimUtils.getAasEditType(entry, kind, rawValue.intValue());
        }
        /** m: @ } */
    ...
    public static void trimEmpty(RawContactDelta state, AccountType accountType) {
        ...
        for (DataKind kind : accountType.getSortedDataKinds()) {
            ...
            for (ValuesDelta entry : entries) {
                ...
                final boolean touched = entry.isInsert() || entry.isUpdate() || entry.isDelete();
                Log.i(TAG, "trimEmpty touched , entry.toString:" + touched + " , " + entry.toString());
                ...
                /** M: Change Feature @{ */
                //workround: mark deleted when photo is empty
                if (Photo.CONTENT_ITEM_TYPE.equals(kind.mimeType)) {
                    boolean hasPhoto = entry.containsKey(Data.DATA15);
                    Log.i(TAG, " hasPhoto : " + hasPhoto + " | entry.getPhoto() : "
                            + entry.getAsByteArray(Photo.PHOTO));
                    if (hasPhoto && entry.getAsByteArray(Photo.PHOTO) == null) {
                        entry.markDeleted();
                        continue;
                    }
                }
