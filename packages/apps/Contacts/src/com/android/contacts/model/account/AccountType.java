...
public abstract class AccountType {
    ...
    // Added Local Account Name - For Sim/Usim/Uim
    /**
     * M: [Gemini+] slot ==> accountNames(accountType ==> accountName)
     * e.g. slot 0 ==> {SIM_TYPE_SIM ==> "SIM0", SIM_TYPE_USIM ==> "USIM0", SIM_TYPE_UIM ==> "UIM0"}
     * 
     */
    ...
    /**
     * M: sim tag + postfix = sim account type like "USIM Account"
     */
    private static String SIM_ACCOUNT_TYPE_POSTFIX = " Account";
    // The previous lines are provided and maintained by Mediatek Inc.

    /**
     * M: [Gemini+] get SIM account name, such as SIM1, USIM2, and so on
     * 
     * @param simType
     *            sim type defined in SimCardUtils.SimType
     * @param slotId
     *            the slot of the account stands for
     * @return the sim account name for simtype and slotid
     */
    public static String getSimAccountName(int simType, int slotId) {
        return SIM_ACCOUNT_NAME_ARRAY.get(slotId).get(simType);
    }

    /**
     * M: [Gemini+] get the slotId from account name return -1 if the account is
     * not SIM account
     * 
     * @param accountName
     *            the account name
     * @return slotId of this account for example: USIM1, slotId is 1 return -1
     *         if the account name is not a SIM account
     */
    public static int getSlotIdBySimAccountName(String accountName) {
        for (int slotId : SlotUtils.getAllSlotIds()) {
            SparseArray<String> possibleAccountNames = SIM_ACCOUNT_NAME_ARRAY.get(slotId);
            for (int index = 0; index < possibleAccountNames.size(); index++) {
                if (possibleAccountNames.valueAt(index).equals(accountName)) { 
                    return slotId;
                }
            }
        }
    ...
    /**
     * M: [Gemini+] Considering SIM account will appear as it's display name, so
     ...
     */
    public static String getDisplayAccountName(String accountName) {
        int slotId = getSlotIdBySimAccountName(accountName);
        return slotId < 0 ? accountName : SIMInfoWrapper.getDefault().getSimDisplayNameBySlotId(slotId);
    }

    /**
     * M: [Gemini+] sim tag + postfix = sim account type like "USIM Account"
     * 
     * @param slotId
     * @return
     */
    public static String getSimAccountType(int slotId) {
        String simAccountTag = SimCardUtils.getSimTagBySlot(slotId);
        if (TextUtils.isEmpty(simAccountTag)) {
            Log.e(TAG, "failed to get account tag for slot: " + slotId);
            return null;
        }
        return simAccountTag + SIM_ACCOUNT_TYPE_POSTFIX;
    }
    ...
    public static boolean isAccountTypeIccCard(String accountTypeString) {
        boolean isIccCardAccount = (ACCOUNT_TYPE_SIM.equals(accountTypeString)
                || ACCOUNT_TYPE_USIM.equals(accountTypeString) || ACCOUNT_TYPE_UIM.equals(accountTypeString));
        Log.v(TAG, "account " + accountTypeString + " is IccCard? " + isIccCardAccount);
        return isIccCardAccount;
    }
    
    /**
     * M: [Gemini+] this method should be an abstract one.
     * but in order to reduce Google modification, we implement it and return default google 
     * account value: false
     * @return if the account is IccCard account, return true, otherwise false
     */
    ...
    /**
     * M: if the account is USIM Account(accountType: "USIM Account", accountName: "USIM0"
     * return true. otherwise, return false.
     * @return true if the AccountType object refer to a USIMAccountType
     */
