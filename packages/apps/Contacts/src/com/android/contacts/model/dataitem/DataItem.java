...
public class DataItem {
    ...
    public DataKind getDataKind() {
        ...
        if (mDataKind == null
                && null != getMimeType()
                && getMimeType().equals(
                        ExtensionManager.getInstance().getContactDetailExtension().getExtentionMimeType(
                                ExtensionManager.COMMD_FOR_RCS))) {
            mDataKind = new DataKind(getMimeType(), 0, 10, false, R.layout.text_fields_editor_view);
            mDataKind.actionBody = new SimpleInflater(Data.DATA1);
            mDataKind.titleRes = 0;
        }
        /** @} */
        return mDataKind;
    }
