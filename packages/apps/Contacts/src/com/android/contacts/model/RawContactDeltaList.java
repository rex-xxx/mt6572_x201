...
public class RawContactDeltaList extends ArrayList<RawContactDelta> implements Parcelable {
    ...
    public ArrayList<ContentProviderOperation> buildDiff() {
        ...
        /// M: split marked deleted contact first from a joint contact @{
        buildSplitDeleteContactIfNeeded(diff);
        /// @}
    ...
    /**
     * M: Builds AggregationExceptions to split some constituent raw contacts into
     * separate contacts from a joint contact
     */
    private void buildSplitContactDiff(final ArrayList<ContentProviderOperation> diff,
            ArrayList<Integer> indexList, int[] backRefs) {
        int count = size();
        for (int index : indexList) {
            for (int i = 0; i < count; i++) {
                if (i != index) {
                    buildSplitContactDiff(diff, index, i, backRefs);
                }
            }
        }
    }
    ...
    /**
     * M: If all fileds of a contact is cleared, then this contacts should be
     * removed. For a joint contact, if a composited raw contact is marked
     * delete, it should separate this raw contact firstly, and then remove it.
     * Otherwise, the joint contact data in database will not be updated, and it
     * will cause errors. issue ALPS00410285 
     */
    private void buildSplitDeleteContactIfNeeded(final ArrayList<ContentProviderOperation> diff) {
        if (size() <= 1) {
            return;
        }
        final int firstBatch = diff.size();
        int[] backRefArray = new int[size()];
        int rawIndex = 0;
        ArrayList<Integer> splitIndexList = new ArrayList<Integer>();
        for (RawContactDelta delta : this) {
            final boolean isInsert = delta.isContactInsert();
            backRefArray[rawIndex] = isInsert ? firstBatch : -1;
            ValuesDelta value = delta.getValues();
            if (value == null) {
                rawIndex++;
                continue;
            }
            if (value.isDelete()) {
                splitIndexList.add(rawIndex);
            }
            rawIndex++;
        }
        if (splitIndexList.size() > 0) {
            buildSplitContactDiff(diff, splitIndexList, backRefArray);
        }
    }
}
