...
public abstract class PhotoSelectionHandler implements OnClickListener {
        ...
        if (resultCode == Activity.RESULT_OK) {
            switch (requestCode) {
                ...
                case REQUEST_CODE_PHOTO_PICKED_WITH_DATA: {
                    ...
                    /**
                     * M: [ANR]This method is moved to Thread to avoid time consuming operation cause ANR.
                     * so, we have to do UI update via handler @{
                     */
                    Handler handler = new Handler(Looper.getMainLooper()) {
                        @Override
                        public void handleMessage(Message msg) {
                            Log.d(TAG, "[handlePhotoActivityResult][handleMessage]notify photo selected via "
                                    + "main looper, listener: " + listener);
                            listener.onPhotoSelected((Bitmap) msg.obj);
                        }
                    };
                ...
                default: {
                    Log.w(TAG, "[handlePhotoActivityResult]the request: " + requestCode + "is not photo request");
                    return false;
                }
            }
        }
    ...
    private void doCropPhoto(String fileName) {
        try {
            ...
            /** M: Change Feature get default storage patch @{ */
            /*
             * Original Code:
             * final String newPath = ContactPhotoUtils.pathForNewCameraPhoto(fileName);
             */
            final String newPath = ContactPhotoUtils.pathForNewCameraPhotoEx(fileName);
            /** @} */
    ...
    private void startPickFromGalleryActivity(String photoFile) {
        ...
        if (FeatureOption.MTK_DRM_APP) {
            Log.i(TAG, "use DRM intent : " + intent);
    ...
    private static Intent getTakePhotoIntent(String fileName) {
         ...
         */
        final String newPhotoPath = ContactPhotoUtils.pathForNewCameraPhotoEx(fileName);
        /** @} */
    ...
    /**
     * M: check whether the request code is photo related, and can be handled by
     ...
     * 
     * @return true if it can be handled by PhotoSelectionHandler
     */
    public boolean canHandleActivityResultRequest(int requestCode, int resultCode) {
        return (resultCode == Activity.RESULT_OK)
                && (requestCode == REQUEST_CODE_PHOTO_PICKED_WITH_DATA || requestCode == REQUEST_CODE_CAMERA_WITH_DATA);
    }
    ...
    /**
     * M: [ANR]This Runnable is used to run in another thread, avoid handlePhotoActivityResult()'s 
     * time consuming operation cause ANR
     */
    private class HandlePhotoResultThread implements Runnable {
        ...
        public HandlePhotoResultThread(int requestCode, int resultCode, Intent data) {
        ...
        @Override
        public void run() {
            Log.i(TAG, "[HandlePhotoResultThread]handle photo ActivityResult Async. request: " + mRequestCode + ", result: "
                    + mResultCode + ", data: " + mData);
            handlePhotoActivityResult(mRequestCode, mResultCode, mData);
        }
    }

    /**
     * M: [ANR]the Async version of handlePhotoActivityResult(), this would
     * start a new Thread to call handlePhotoActivityResult()
     * 
     * @param requestCode
     *            the request code
     * @param resultCode
     ...
     *            the data
     */
    public void handlePhotoActivityResultAsync(int requestCode, int resultCode, Intent data) {
        new Thread(new HandlePhotoResultThread(requestCode, resultCode, data)).start();
    }
