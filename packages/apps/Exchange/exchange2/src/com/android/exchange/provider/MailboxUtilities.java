...
public class MailboxUtilities {
     ...
     */
    public static synchronized void setParentKeyAndFlags(Context context, long accountId) {
        ...
        final int countToUpdateMailbox = 50;
        ...
        if (temporaryMailboxs.size() > 0) {
            for (TemporaryMailbox temporaryMailbox : temporaryMailboxs) {
                ...
                if (parentServerIdSet.contains(temporaryMailbox.mServerId)) {
                    temporaryMailbox.mFlags |= Mailbox.FLAG_HAS_CHILDREN
                            | Mailbox.FLAG_CHILDREN_VISIBLE;
                }
                ...
                ops.add(ContentProviderOperation
                        .newUpdate(ContentUris.withAppendedId(Mailbox.CONTENT_URI, id))
                        .withValues(cv).build());
                if (++commitCount == countToUpdateMailbox) {
    ...
    public static void updateMailbox(ContentResolver resolver,
    ...
    private static class TemporaryMailbox {
        ...
        public TemporaryMailbox(long id, String serverId, String parentServerId,
                long parentKey, int type, int flags) {
            mId = id;
            mServerId = serverId;
