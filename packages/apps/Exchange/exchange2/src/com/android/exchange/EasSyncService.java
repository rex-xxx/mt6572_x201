...
public class EasSyncService extends AbstractSyncService {
    ...
    public static final int COMMAND_TIMEOUT = 30*SECONDS;
    ...
    protected static final int WATCHDOG_TIMEOUT_ALLOWANCE = 30*SECONDS;
    // The timeout allowance especially for "force ping"
    protected static final int FORCEPING_WATCHDOG_TIMEOUT_ALLOWANCE = 10*SECONDS;
    // The timeout allowance for GAL to abort connections
    private static final int GAL_TIMEOUT = 10*SECONDS;
    ...
    private static final int POST_LOCK_TIMEOUT = 10 * SECONDS;
    private static final int POST_LOCK_CHECK_TIMEOUT = 2 * SECONDS;

    private int mSyncStatus = Eas.EAS_SYNC_IDLE;
    ...
    protected boolean mTrustSsl = false;
    ...
    private static volatile LinkedList<EasSyncService> sGalServiceList
        = new LinkedList<EasSyncService>();
    private static final int GAL_SEARCH_LIMIT = 10;

    /// M: Defined for "Bad Sync Key" recovery @{
    private int mWindowCount = 0;
    private int mWindowSyncAsNormal = 0;
    /// @}

    /// M: Support Injection mock HttpURLConnection to test Ping command. @{
    private HttpURLConnection mMockConnection = null;
    public void setMockConnection(HttpURLConnection mockConnection) {
        if (Configuration.IS_TEST) {
            mMockConnection = mockConnection;
        } else {
            mMockConnection = null;
        }
    }
    ...
    protected EasSyncService(Context context, Mailbox mailbox) {
        ...
        HostAuth ha = HostAuth.restoreHostAuthWithId(context, mAccount.mHostAuthKeyRecv);
        if (ha == null) {
            mIsValid = false;
            return;
        }
    ...
    public static EasSyncService setupServiceForAccount(Context context, Account account) {
        ...
        try {
            ...
            Logging.d("setupServiceForAccount return null because CertificateException:"
                    + e.getMessage());
            return null;
        }
            ...
            try {
                ...
                if (code == HttpStatus.SC_OK) {
                    ...
                    try {
                        ...
                        bundle.putString(EmailServiceProxy
                                .VALIDATE_BUNDLE_PROTOCOL_VERSION, mProtocolVersion);
                    ...
                    Account existingAccount = Utility.findExistingAccount(
                            context, -1L, hostAuth.mAddress, hostAuth.mLogin, hostAuth.mAddress);
                ...
                if (CommandStatus.isNeedsProvisioning(status)) {
                    ...
                    if (pp != null && pp.hasSupportablePolicySet()) {
                        ...
                        /// M: Modified for avoiding the exception of null pointer
                        bundle.putParcelable(EmailServiceProxy.VALIDATE_BUNDLE_POLICY_SET,
                                pp == null ? null : pp.getPolicy());
                    }
     ...
     */
    public static GalResult searchGal(Context context, long accountId, String filter, int limit) {
        ...
        if (acct != null) {
            ...
            try {
                ...
                s.end().end().end().done();
                EasResponse resp = svc.sendHttpClientPostForGal("Search",
                        new ByteArrayEntity(s.toByteArray()));
                try {
                    int code = resp.getStatus();
                    if (code == HttpStatus.SC_OK) {
                        InputStream is = resp.getInputStream();
                ...
                if (is != null) {
                    ...
                    if(Logging.DEBUG) {
                        Logging.d(Logging.LOG_TAG,
                                "+++ Empty input stream in sync command response");
                    }
                }
            } else {
                if(Logging.DEBUG) {
     ...
     * @throws IOException
     */
    public boolean tryProvision(EasSyncService svc) throws IOException {
        ...
        if (pp.getRemoteWipe()) {
            ...
            if (securitySyncKey != null) {
                ...
                /** M: Update securitySyncKey of mAccount which was come out at very beginning from
                 * DB @{ */
                if (!TextUtilities.stringOrNullEquals(mAccount.mSecuritySyncKey,
                        securitySyncKey)) {
                    mAccount.mSecuritySyncKey = securitySyncKey;
                    Logging.d(TAG, "tryProvision Account updated with new securitySyncKey: "
                            + securitySyncKey);
                } else {
                    Logging.d(TAG, "tryProvision Account not chenged");
                }
                /** @} */
     ...
     * @throws IOException
     */
    public ProvisionParser canProvision(EasSyncService svc) throws IOException {
        Serializer s = new Serializer();
        ...
        try {
            ...
            if (code == HttpStatus.SC_OK) {
              ...
              /// M: Should handle [451/Redirect] case. @{
            } else if (code == EAS_REDIRECT_CODE
                    && canHandleAccountMailboxRedirect(resp)) {
                // Cause this to re-run
                throw new IOException("Result code 451, redirect for get Provision and retry after a brief hold.");
              /// @}
            }
        } finally {
            resp.close();
    ...
    protected boolean isMailboxSyncable(long mailboxId) {
        /// M: Always return true,when run testcase. @{
        if (Configuration.IS_TEST) {
            Logging.d(TAG, "Not check mailbox " + mailboxId
    ...
    public void sync(AbstractSyncAdapter target) throws IOException {
        ...
        while (!mStop && (moreAvailable || hasPendingRequests() || hasMoreLocalChanges(target))) {
            ...
            //M: Ignore the connectivity when testing
            if (!hasConnectivity() && !Configuration.IS_TEST) {
                ...
                if (req instanceof PartRequest) {
                    if (!((PartRequest)req).mCancelled) {
                        try {
                            ...
                            TrafficStats.setThreadStatsTag(TrafficFlags.getSyncFlags(mContext,
                                    mAccount));
                        }
                    }
            ...
            if ((mSyncStatus == Eas.EAS_SYNC_IDLE)
                    && !syncKey.equals("0")) {
                mSyncStatus = Eas.EAS_SYNC_INIT;
                if (Logging.LOG_PERFORMANCE) {
                    Logging.d(Logging.LOG_TAG, "[Exchange Download Speed] EasSyncService:FirstSync " + syncKey);
                }
            } else if (mSyncStatus == Eas.EAS_SYNC_INIT && !syncKey.equals("0")) {
                mSyncStatus = Eas.EAS_SYNC_NORMAL;
            } else if (syncKey.equals("0")) {
                mSyncStatus = Eas.EAS_SYNC_IDLE;
            }

            /** M: The status machine for "Bad Sync Key" recovery, currently
                just for Email folder. Pass over Eas.EAS_SYNC_IDLE since
                for sync key "0", we can not do "getEstimate", we start the
                recovery from Eas.EAS_SYNC_INIT status @{ */
            if (mMailbox.mId == Exchange.sBadSyncKeyMailboxId
                    && mSyncStatus != Eas.EAS_SYNC_IDLE) {
                if (mSyncStatus == Eas.EAS_SYNC_INIT) {
                    mWindowCount = ((EmailSyncAdapter)target).getWindowCount();
                    userLog("[BSK recovery] window count: " + mWindowCount);
                    // If getEstimate failed, we cannot calculate the window count,
                    // or if we just need sync one window, we sync as "NORMAL",
                    if (mWindowCount <= 1) {
                        mSyncStatus = Eas.EAS_SYNC_NORMAL;
                    } else {
                        mSyncStatus = Eas.EAS_SYNC_RECOVER;
                    }
                    userLog("[BSK recovery] About to sync window: " + mWindowCount);
                    mWindowCount--;
                } else if (mSyncStatus == Eas.EAS_SYNC_RECOVER && mWindowCount-- > 1) {
                    userLog("[BSK recovery] About to sync window: " + (mWindowCount + 1));
                } else {
                    userLog("[BSK recovery] About to sync the last window");
                    mSyncStatus = Eas.EAS_SYNC_NORMAL;
                }
            }
            /** @} */
            ...
            if (moreAvailable) {
                ...
                target.sendSyncOptions(mProtocolVersionDouble, s, mSyncStatus);
            }
            if (mSyncStatus == Eas.EAS_SYNC_IDLE) {
            ...
            if (mUpsyncFailed) {
                ...
                if ((mMailbox.mType == Mailbox.TYPE_CONTACTS) && !canSendLocal) {
                    continue;
                }
            }
            ...
            if (Logging.LOG_PERFORMANCE) {
                if ("Email".equalsIgnoreCase(target.getCollectionName())) {
                    Logging.d(Logging.LOG_TAG,
                            "[Exchange Download Speed] EasSyncService:SendSyncMessages ["
            ...
            if (Logging.LOG_PERFORMANCE) {
                if ("Email".equalsIgnoreCase(target.getCollectionName())) {
                    Logging.d(Logging.LOG_TAG,
                            "[Exchange Download Speed] EasSyncService:ReceiveSyncMessages ["
                            + System.currentTimeMillis() + "]");
                }
            }
            try {
                ...
                if (code == HttpStatus.SC_OK) {
                    ...
                    if (!resp.isEmpty()) {
                        ...
                        try {
                            moreAvailable = target.parse(bis);
                            ...
                            if (mMailbox.mId == Exchange.sBadSyncKeyMailboxId
                                ...
                                int rowDeleted = mContentResolver.delete(Message.CONTENT_URI, selection , null);
                                userLog("[BSK recovery] "
                                        + rowDeleted + " local dirty mails were deleted");
                                ...
                                userLog("[BSK recovery] Bad sync key recovery process is finished");
                            }
                            /** @} */
...
//        }
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    // Don't care if this fails
                    Logging.d("RemoteException catched: " + e1.getMessage());
                }
                ...
                if (mSyncReason == ExchangeService.SYNC_PING
                    ...
                    Mailbox m = Mailbox.restoreMailboxOfType(mContext, mAccount.mId,
    ...
    private static class TrustAllCertsTrustManager implements javax.net.ssl.TrustManager,
    ...
    private void sendCallback(boolean success) {
        ...
        if (instance != null && null != instance.getExternalCallback()) {
            try {
                ...
                //Skip it
                Logging.d("RemoteException catched: " + e.getMessage());
            }
    ...
    /**
     * M: This method just for testing
     */
    public boolean getSendingStatus() throws IOException {
        boolean status = sendSettings();
        return status;
    }
     ...
     */
    protected boolean canHandleAccountMailboxRedirect(EasResponse resp) {
        userLog("AccountMailbox redirect error");
        Logging.d("QQQ canHandleAccountMailboxRedirect ");
        HostAuth ha =
