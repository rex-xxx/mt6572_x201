...
public class FolderSyncParser extends AbstractSyncParser {
    ...
    // DB commit duration for Performance test
    private long mCommitElapse = 0;
    ...
    public boolean parse() throws IOException, CommandStatusException {
        ...
        if (mInitialSync) {
            Logging.d(TAG, "Delete all but account mailbox for initial folder sync");
            ...
            if (tag == Tags.FOLDER_STATUS) {
                ...
                if (status != Eas.FOLDER_STATUS_OK) {
                    ...
                    // and EAS 14 style command status
                        ...
                        Logging.d(TAG, "Delete all but account mailbox for bad sync key");
                        mContentResolver.delete(Mailbox.CONTENT_URI, ALL_BUT_ACCOUNT_MAILBOX,
                                new String[] {Long.toString(mAccountId)});
        ...
        if (VALID_EAS_FOLDER_TYPES.contains(type)) {
            ...
            switch (type) {
                case INBOX_TYPE:
                    ...
                    /// M: If the updated interval were smart push, set the interval
                    // of Inbox as normal push
                    if (mAccount.mSyncInterval == Account.CHECK_INTERVAL_SMART_PUSH) {
                        mailbox.mSyncInterval = Mailbox.CHECK_INTERVAL_PUSH;
                    } else {
                        mailbox.mSyncInterval = mAccount.mSyncInterval;
                    }
                    /// M: Fixup flags for inbox (should accept moved mail)
    ...
    private boolean commitMailboxes(ArrayList<Mailbox> validMailboxes,
        ...
        try {
            ...
            mCommitElapse = mCommitElapse + System.currentTimeMillis() - start;
            return true;
    ...
    public void changesParser(final ArrayList<ContentProviderOperation> ops,
        ...
        // Reset Commit elapse to record current time
        mCommitElapse = 0;
        ...
        synchronized (FolderSyncParser.this) {
            ...
            if (Logging.LOG_PERFORMANCE) {
                Logging.d(Logging.LOG_TAG, "[Exchange Download Speed] EasSyncService:" +
                            "FolderSync:commit [" + mCommitElapse + "]");
            }
            ...
            if (mFixupUninitializedNeeded) {
                MailboxUtilities.setParentKeyAndFlags(mContext, mAccount.mId);
            }
