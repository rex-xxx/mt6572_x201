...
public class EasResponse {
    ...
    // M: For test
    public static Callbacks sCallback;
    /**
    ...
    /**
     * M:Callback interface for test purpose
     */
    public interface Callbacks {
        public EasResponse fromHttpRequest(HttpUriRequest request) throws IOException;
    }

    /**
     * M: This constructor just for testing
     */
    public EasResponse() {
        mResponse = null;
        mEntity = null;
    ...
    public static EasResponse fromHttpRequest(
            ...
            Context context)
            throws IOException {
        /// M:Only in test, Configuration.IS_TEST is true. @}
        if (Configuration.IS_TEST) {
            return sCallback.fromHttpRequest(request);
        }
        /// @{
        ConnRouteParams.setDefaultProxy(client.getParams(), Proxy.getPreferredHttpHost(context,
    ...
    /**
     * Determine whether an HTTP code represents a server error
     * @param code the HTTP code returned by the server
     * @return whether or not the code represents an server error
     */
    public static boolean isServerError(int code) {
        return (code == HttpStatus.SC_SERVICE_UNAVAILABLE)
