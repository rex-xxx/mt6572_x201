...
public class EasAccountService extends EasSyncService {
    ...
    private static final int ACCOUNT_MAILBOX_SLEEP_TIME = 20*MINUTES;
    private static final String ACCOUNT_MAILBOX_SLEEP_TEXT =
    ...
    private static final int PING_CONNECT_TIMEOUT = 1 * MINUTES;
    private static final int PING_SOCKET_TIMEOUT = 1 * MINUTES;

    protected EasAccountService(Context context, Mailbox mailbox) {
    ...
    /**
     * M: If possible, update the account to the new server address
     * when encountering the ping command redirect error! {@
     */
    private void handlePingRedirect(String location) {
        userLog("Ping command redirect error");
        ...
        try {
            ...
            if (hostAuth != null) {
                ...
                mHostAddress = Uri.parse(location).getHost();
                mBaseUriString = null;
                hostAuth.mAddress = mHostAddress;
                userLog("Redirecting to: " + location);

                ContentValues haValues = new ContentValues();
                haValues.put(HostAuthColumns.ADDRESS, hostAuth.mAddress);
                hostAuth.update(mContext, haValues);
                return;
            }
    ...
    public void sync() throws IOException, EasParserException {
        ...
        try {
            ...
            // Do not re-check protocol version at first sync because it has been checked at account
            // validating
            ...
            if (mProtocolVersionDouble >= 12.0 &&
                ...
                mAccount.mFlags = (account != null ? account.mFlags : mAccount.mFlags)
            ...
            while (!isStopped()) {
                ...
                if (Logging.LOG_PERFORMANCE) {
                    /**
                     * M: For Performance test, this is the start time point for "Folder Sync" @{
                     */
                    Logging.d(Logging.LOG_TAG,
                            "[Exchange Download Speed] EasSyncService:FolderSync:Start ["
                            + System.currentTimeMillis() + "]");
                    /** @} */
                    Logging.d(Logging.LOG_TAG,
                            "[Exchange Download Speed] EasSyncService:FolderSync: "
                            + mAccount.mSyncKey);
                    Logging.d(Logging.LOG_TAG,
                            "[Exchange Download Speed] EasSyncService:FolderSync:Send ["
                            + System.currentTimeMillis() + "]");
                }
                EasResponse resp = sendHttpClientPost("FolderSync", s.toByteArray());
                if (Logging.LOG_PERFORMANCE) {
                    Logging.d(Logging.LOG_TAG,
                            "[Exchange Download Speed] EasSyncService:FolderSync:Receive ["
                            + System.currentTimeMillis() + "]");
                }
                try {
                    if (isStopped()) {
                ...
                try {
                    if (Logging.LOG_PERFORMANCE) {
                        Logging.d(Logging.LOG_TAG,
                                "[Exchange Download Speed] EasSyncService:FinishFolderSync ["
                                + System.currentTimeMillis() + "]");
                    }
                    ...
                    // Don't care if this fails
                    Logging.d(Logging.LOG_TAG, "RemoteException catched: " + e1.getMessage());
                }
    ...
    private void sleep(long ms, boolean runAsleep) {
        // M: Change sleep to wait and use notify from ExchangeService when a sync ends @{
        if (runAsleep) {
            userLog("use the alarm sleep mode");
            ExchangeService.setEasSyncAlarm(mMailboxId, ms + (5 * SECONDS));
            ExchangeService.releaseEasWakeLock(mMailboxId);
            synchronized (this) {
                try {
                    wait();
                } catch (InterruptedException e) {
                    Logging.d(Logging.LOG_TAG, "InterruptedException catched: " + e.getMessage());
                } finally {
    ...
    protected byte[] sendPing(byte[] bytes, int heartbeat) throws IOException {
        ...
        try {
            /// M: Inject mockConnection if needed. @{
            if (!Configuration.IS_TEST) {
                mHttpConn = (HttpURLConnection) url.openConnection();
            } else {
                HttpURLConnection mockConnection = getMockConnection();
                if (mockConnection != null) {
                    userLog("Run testcase, inject mock connection ..");
                    mHttpConn = mockConnection;
                } else {
                    mHttpConn = (HttpURLConnection) url.openConnection();
                }
            }
            ...
            mHttpConn.setConnectTimeout(PING_CONNECT_TIMEOUT);
            ...
            /** M: Handling the redirect error. {@ */
            } else if (EAS_REDIRECT_CODE == mPingResponseCode) {
                userLog("Ping http response redirect code: ", mPingResponseCode);
                handlePingRedirect(mHttpConn.getHeaderField("X-MS-Location"));
                throw new IOException("ping redirect");
            /** @} */
            } else {
            ...
            synchronized(getSynchronizer()) {
                if (out != null) {
                    /// M: Add the close log.
                    userLog("call out.close()");
                    out.close();
                }
                if (in != null) {
                    /// M: Add the close log.
                    userLog("call in.close()");
                    in.close();
                }
                if (mHttpConn != null) {
                    /// M: Add the disconnect log.
                    userLog("call mHttpConn.disconnect()");
    ...
    void runPingLoop() throws IOException, StaleFolderListException,
        ...
        while ((System.currentTimeMillis() < endTime) && !isStopped()) {
            ...
            try {
                ...
                while (c.moveToNext()) {
                    ...
                    if (pingStatus == ExchangeService.PING_STATUS_OK) {
                        ...
                        pingFoldersMap.put(c.getString(Mailbox.CONTENT_SERVER_ID_COLUMN),
                                folderClass);
            ...
            /// M: always send ping when run testcase with flag Configuration.IS_TEST. @{
            if ((canPushCount > 0) && ((canPushCount == pushCount) || forcePing) || Configuration.IS_TEST) {
            /// @}
                Serializer s = new Serializer();
                Iterator<Entry<String, String>> entrySetIterator =
                        pingFoldersMap.entrySet().iterator();
                ...
                try {
                    ...
                    byte[] res = sendPing(s.toByteArray(), forcePing ? mPingForceHeartbeat
                            : pingHeartbeat);
                ...
                // DONE: TODO Change sleep to wait and use notify from ExchangeService when a sync
                // ends
    ...
    /// M: add some test method to run testcase. @{
    public boolean testHandleAccountMailboxRedirect(EasResponse resp) {
        return canHandleAccountMailboxRedirect(resp);
    }

    public int getPingResponseCode() {
        return mPingResponseCode;
    }

    public int testParsePing(InputStream is, ContentResolver cr,
            HashMap<String, Integer> errorMap) throws IOException,
            StaleFolderListException, IllegalHeartbeatException,
            CommandStatusException {
        return parsePingResult(is, cr, errorMap);
    }

    public ArrayList<String> getPingChangeList() {
        return mPingChangeList;
    }

    public void testPushFallback(long mailboxId) {
        pushFallback(mailboxId);
    }

    public boolean testIsLikelyNatFailure(String message) {
        return isLikelyNatFailure(message);
    }
    ///@}
}
