...
public class AttachmentLoader {
    ...
    private final PartRequest mRequest;
    ...
    public void readChunked(InputStream inputStream, OutputStream outputStream, int len)
        ...
        while (true) {
            if (mRequest.mCancelled) {
                if (mService.mHttpPostForAttachment != null) {
                    mService.mHttpPostForAttachment.abort();
                    mService.mHttpPostForAttachment = null;
                }
                throw new IOException(EasSyncService.CANCEL_REQUEST_MSG);
            }
    ...
    public void loadAttachment() throws IOException {
        ...
        try {
            mService.mIsForAttachment = true;
            ...
            if (eas14) {
                ...
                if (!mRequest.mCancelled) {
                    ...
                    Logging.d("Attachment download has been canceled");
                    return;
                }
            } else {
                String location = mAttachment.mLocation;
                // For Exchange 2003 (EAS 2.5), we have to look for illegal
                // characters in the file name that EAS sent to us!
            ...
            mService.mIsForAttachment = false;
        }
                    ...
                    try {
                        ...
                        if (eas14) {
                            ItemOperationsParser p = new ItemOperationsParser(this, is, os,
                                    mAttachmentSize);
                            p.parse();
                            /// M: Need to satisfy the following two requirements.
                            if (p.getStatusCode() == 1 && p.getDataBoolean()/* Success */) {
