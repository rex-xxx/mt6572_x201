...
public class ExchangeDirectoryProvider extends ContentProvider {
    ...
    /*package*/ static final HashMap<String, Long> sAccountIdMap = new HashMap<String, Long>();
    ...
    /*package*/ long getAccountIdByName(Context context, String accountName) {
        Long accountId = sAccountIdMap.get(accountName);
        if (accountId == null) {
            ...
            if (accountId != -1) {
                sAccountIdMap.put(accountName, accountId);
            }
        }
        return accountId;
    }
    ...
    /**
     * M: Delete the cache of the account info from sAccountIdMap
     * @param accountId the id of the account
     */
    public static void removeCache(long accountId) {
        Set<HashMap.Entry<String, Long>> entrySet = sAccountIdMap.entrySet();
        for (HashMap.Entry<String, Long> en : entrySet) {
            if (en.getValue() == accountId) {
                sAccountIdMap.remove(en.getKey());
                break;
            }
        }
    }
        ...
        switch (match) {
            ...
            case GAL_EMAIL_FILTER: {
                ...
                try {
                    ...
                    Logging.d(">>>> exchange search contacts with filter " + filter);
                    GalResult galResult = EasSyncService.searchGal(getContext(), accountId,
                            filter, limit);
                    if (galResult != null) {
                        Logging.d(">>>> exchange search contacts got " + galResult.total);
                        return buildGalResultCursor(projection, galResult);
                    } else {
                        Logging.d(">>>> exchange search contacts got nothing");
                    }
                } finally {
    ...
    /*package*/ Cursor buildGalResultCursor(String[] projection, GalResult galResult) {
        ...
        int idIndex = -1;
        int lookupIndex = -1;
        /// M: displayNameSource must be set in the result,
        /// otherwise RecipientEditTextView will not display the displayName
