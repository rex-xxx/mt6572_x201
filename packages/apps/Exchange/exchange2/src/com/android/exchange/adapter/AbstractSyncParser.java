            ...
            if (tag == Tags.SYNC_COLLECTION || tag == Tags.SYNC_COLLECTIONS) {
                ...
                if (status != 1) {
                    ...
                    if (status == 3 || CommandStatus.isBadSyncKey(status)) {
                        ...
                        mService.errorLog("status = " + status);
                        if (Build.TYPE.equals("eng")) {
                            Handler handler = new Handler(
                                    mContext.getApplicationContext().getMainLooper());
                            handler.post(new Runnable() {
                                public void run() {
                                    Toast.makeText(mContext, R.string.wipe_all_messages,
                        ...
                        /** M: Store bad sync key mailbox id in Preference in case of user rebooting the device
                            ...
                            process at present @{ */
                        if ("Email".equalsIgnoreCase(mAdapter.getCollectionName())) {
                            userLog("[BSK Recovery] Bad sync key occurs");
        ...
        if (Logging.LOG_PERFORMANCE) {
            if ("Email".equalsIgnoreCase(mAdapter.getCollectionName())) {
                Logging.d(Logging.LOG_TAG,
                        "[Exchange Download Speed] EasSyncService:Email.parse spend ["
                                + (System.currentTimeMillis() - startParse) + "] ms");
            }
        }
        // Commit any changes
        commit();
