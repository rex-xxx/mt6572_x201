...
public class ContactsSyncAdapter extends AbstractSyncAdapter {
    ...
    // Do not update/change more than 100 contacts at a time since 1) server will distribute
    // server id to each received "add" contact and response them to the client, we should 
    // decrease the overload of server 2) less data load and entry count implies more reliability.
    private static final int COUNT_LIMIT = 100;

    // For avoiding potential OOM as well as ensuring the data up sync reliability, 
    // we use a dynamic adjusting buffer in the light of current free heap size
    private static final int BUF_MAX = 500 * 1024;  // 500KB
    private static final int BUF_MIN = 50 * 1024;   // 50KB
    
    // The buffer may be expanded and copy as byte array soon, use a proper heap size divisor
    // for avoiding potential OOM
    private static final int DIVISOR = 4;
    private static final int RESERVED_HEAP = 1024 * 1024; // 1MB

    // Exchange protocol limited the picture of a contact not more than about 36.6k
    // The picture would not be received if more than this size, so we will ignore
    // these big pictures
    private static final int PHOTO_SIZE_LIMIT = 36 * 1024; // 36KB
    
    // sleep 3 seconds before try again if current heap size can not meet up-sync
    private static final int WAIT_FOR_GC = 3 * 1000;  // 3 seconds
    ...
    public boolean isSyncable() {
        return ContentResolver.getMasterSyncAutomatically()
                && ContentResolver.getSyncAutomatically(mAccountManagerAccount,
                        ...
                        try {
                            if (c.moveToFirst()) {
                                ...
                                try {
                                  ...
                                  // close entityIterator to prevent leaking
    ...
    private void sendPhoto(Serializer s, ContentValues cv) throws IOException {
        if (cv.containsKey(Photo.PHOTO)) {
            ...
            if (bytes.length > PHOTO_SIZE_LIMIT) {
                return;
            }
    ...
    public boolean sendLocalChanges(Serializer s) throws IOException {
        ...
        try {
            ...
            // Count the threshold of the buffer size
            Runtime rt = Runtime.getRuntime();
            long freeHeap = rt.maxMemory() - (rt.totalMemory() - rt.freeMemory());
            userLog("free heapsize=" + freeHeap);
            // We should reserve a few heap (e.g. 1MB) for others
            int bufSize = ((int)freeHeap- RESERVED_HEAP)/DIVISOR;
            if (bufSize < BUF_MIN) {
                userLog("Memory is too low to up-sync contacts, will try again later");
                rt.gc();
                Thread.sleep(WAIT_FOR_GC);
                mMoreLocalChanges = true;
                return false;
            }
            long bufThreshold = Math.min(bufSize, BUF_MAX);
            ...
            while (ei.hasNext()) {
                ...
                if ((++count >= COUNT_LIMIT || s.getBufferSize() >= bufThreshold) && ei.hasNext()) {
                    userLog("contacts up-syncing reached the threshold");
                    mMoreLocalChanges = true;
                    s.end();   // Commands
                    return true;
                }
