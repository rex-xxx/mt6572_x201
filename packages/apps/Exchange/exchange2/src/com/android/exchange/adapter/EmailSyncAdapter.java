...
public class EmailSyncAdapter extends AbstractSyncAdapter {
    ...
    private static final String EMAIL_FIRST_WINDOW_SIZE = "10";
    ...
    private static final String WHERE_MESSAGE_TIMESTAMP_LESS_THAN = MessageColumns.TIMESTAMP
    + " <? AND " + MessageColumns.MAILBOX_KEY + "=?";
    /// @}

    /// M: avoid binder buffer allocated fail.
    private static final int NEED_TRUNCATED_BODY_SIZE = 400 * 1024;
    ...
    @Override
    public void wipe() {
        /** M:  With the "Bad Sync Key" recovery mechanism, we no longer wipe all
            the local mails considering the network data usage for re-synchronizing.
            We just ignore all the local changes and service requests before the 
            recovery @{ */
        userLog("Wipe all message updates and undone requests");
...
//                return Eas.FILTER_AUTO;
    ...
    @Override
    public void sendSyncOptions(Double protocolVersion, Serializer s, int syncStatus)
            throws IOException  {
        if (syncStatus == Eas.EAS_SYNC_IDLE) {
                return;
        }
        mFetchRequestList.clear();
...
//        }
        ...
        if (mFetchRequestList.isEmpty()) {
            ...
            if (protocolVersion < Eas.SUPPORTED_PROTOCOL_EX2007_DOUBLE) {
                s.data(Tags.SYNC_WINDOW_SIZE, EMAIL_FIRST_WINDOW_SIZE);
            } else {
                s.data(Tags.SYNC_WINDOW_SIZE, (syncStatus <= Eas.EAS_SYNC_INIT)
                        ? EMAIL_FIRST_WINDOW_SIZE : ((syncStatus == Eas.EAS_SYNC_RECOVER)
                                ? String.valueOf(mWindowSizeRecover) : EMAIL_WINDOW_SIZE));
            }
...
//            }
            ...
            if (protocolVersion >= Eas.SUPPORTED_PROTOCOL_EX2007_DOUBLE) {
                ...
                s.data(Tags.BASE_TRUNCATION_SIZE, (syncStatus == Eas.EAS_SYNC_RECOVER) ? "0"
                                : Eas.EAS12_TRUNCATION_SIZE_PARTIAL);
                s.end();
            } else {
                /// M: Use MIME data for EAS 2.5
                // Fetch full content of message to make sure display correctly
                // Using EAS 2.5 may have a lot of data traffic
                s.data(Tags.SYNC_MIME_SUPPORT, Eas.MIME_BODY_PREFERENCE_MIME);
                s.data(Tags.SYNC_MIME_TRUNCATION, (syncStatus == Eas.EAS_SYNC_RECOVER) ? "0"
                        : Eas.EAS2_5_FULL_SIZE);
            }
            s.end();
        } else {
            s.start(Tags.SYNC_OPTIONS);
            ...
            s.data(Tags.SYNC_TRUNCATION, (syncStatus == Eas.EAS_SYNC_RECOVER) ? "0"
...
//        // TODO Code similar to this is used elsewhere (e.g. MailboxSettings);
...
//    }
    ...
    public class EasEmailSyncParser extends AbstractSyncParser {
        ...
        private static final String WHERE_LONG_ID_AND_MAILBOX_KEY =
            MessageColumns.PROTOCOL_SEARCH_INFO + "=? and " + MessageColumns.MAILBOX_KEY + "=?";
        ...
        public void addData (Message msg, int endingTag) throws IOException {
            ...
            while (nextTag(endingTag) != END) {
                switch (tag) {
                    ...
                    case Tags.BASE_ATTACHMENTS: // BASE_ATTACHMENTS is used in EAS 12.0 and up
                        ...
                        userLog("EMAIL_ATTACHMENTS atts: " + atts.size());
                        break;
                    case Tags.EMAIL_TO:
                        msg.mTo = Address.pack(Address.parse(getValue(), false));
                        mSize += "To:".length();
                        mSize += (msg.mTo != null ? msg.mTo : "").length();
                        break;
                    case Tags.EMAIL_FROM:
                        ...
                        mSize += "From:".length();
                        mSize += (msg.mFrom != null ? msg.mFrom : "").length();
                        break;
                    case Tags.EMAIL_CC:
                        msg.mCc = Address.pack(Address.parse(getValue(), false));
                        mSize += "CC:".length();
                        mSize += (msg.mCc != null ? msg.mCc : "").length();
                        break;
                    case Tags.EMAIL_REPLY_TO:
                        msg.mReplyTo = Address.pack(Address.parse(getValue(), false));
                        mSize += "Reply-To:".length();
                        mSize += (msg.mReplyTo != null ? msg.mReplyTo : "").length();
                        break;
                    case Tags.EMAIL_DATE_RECEIVED:
                        String date = getValue();
                        msg.mTimeStamp = Utility.parseEmailDateTimeToMillis(date);
                        mSize += "Date:".length();
                        mSize += (date != null ? date : "").length();
                        break;
                    case Tags.EMAIL_SUBJECT:
                        msg.mSubject = getValue();
                        mSize += "Subject:".length();
                        mSize += (msg.mSubject != null ? msg.mSubject : "").length();
                        break;
            ...
            /// M: Update attachments of this message if necessary
            if (atts.size() > 0) {
                if (msg.mAttachments == null) {
                    msg.mAttachments = atts;
                } else {
                    userLog("Attachment need to fix locations");
                    fixAttachmentLocation(msg, atts);
                }
        ...
        private Message fetchParser() throws IOException, CommandStatusException {
            Message msg = new Message();
            msg.mAccountKey = mAccount.mId;
            msg.mMailboxKey = mMailbox.mId;
            // For exchange we judge completed or not by "AirSyncBase::Truncated later"
            ...
            while (nextTag(Tags.SYNC_FETCH) != END) {
                switch (tag) {
                    case Tags.SYNC_SERVER_ID:
                        ...
                        Logging.d(Logging.LOG_TAG, "+++ Email responsesParser SYNC_SERVER_ID: "
                                + msg.mServerId);
                        break;
                    case Tags.SYNC_APPLICATION_DATA:
                        Logging.d(Logging.LOG_TAG,
                                "+++ Email responsesParser SYNC_APPLICATION_DATA");
                        addData(msg, tag);
                        break;
                    default:
                        skipTag();
        ...
        private Message addParser() throws IOException, CommandStatusException {
            ...
            while (nextTag(Tags.SYNC_ADD) != END) {
                switch (tag) {
                    case Tags.SYNC_SERVER_ID:
                        msg.mServerId = getValue();
                        mSize += (msg.mServerId != null ? msg.mServerId : "").length();
                        break;
            ...
            while (nextTag(Tags.EMAIL_BODY) != END) {
                switch (tag) {
                    ...
                    case Tags.BASE_TRUNCATED:
                        ...
                        Logging.d(Logging.LOG_TAG, "+++ airsyncbase body truncated: "
                                + bodyTruncated);
                        break;
                    default:
                        skipTag();
        ...
        private void mimeBodyParser(Message msg, String mimeData) throws IOException {
            try {
                ...
                userLog("mimeBodyParser before updateattachments: " + (msg.mAttachments != null ?
                        msg.mAttachments.size() : 0)
                        + " attachments: " + attachments.size());
                ConversionUtilities.updateAttachments(msg, attachments);
                userLog("mimeBodyParser after updateattachments: " + (msg.mAttachments != null ?
                        msg.mAttachments.size() : 0));
        ...
        private void attachmentParser(ArrayList<Attachment> atts, Message msg) throws IOException {
            ...
            if ((fileName != null) && (length != null) && (location != null)) {
                ...
                atts.add(att);
                userLog("attachmentParser add att : " + att);
        ...
        public String getMimeTypeFromFileName(String fileName) {
            return AttachmentUtilities.inferMimeType(fileName, null);
        }

        public Cursor getServerIdCursor(String serverId, String[] projection) {
        ...
        public Cursor getLongIdCursor(String protocolSearchInfo, String[] projection) {
            mBindArguments[0] = protocolSearchInfo;
            mBindArguments[1] = mMailboxIdAsString;
            Cursor c = mContentResolver.query(Message.CONTENT_URI, projection,
                    WHERE_LONG_ID_AND_MAILBOX_KEY, mBindArguments, null);
            if (c == null) {
                throw new ProviderUnavailableException();
            }
            if (c.getCount() > 1) {
                userLog("Multiple messages with the same LongId/mailbox: " + protocolSearchInfo);
            }
            return c;
        }
        ...
        @Override
        public void commit() {
            HashMap<Message,Integer> opIndex = new HashMap<Message,Integer>();
            // Use a batch operation to handle the changes
            ArrayList<ContentProviderOperation> ops = new ArrayList<ContentProviderOperation>();

            /// M: fix binder allocated fail in Exchange 2003 @ {
            boolean isServer03 = false;
            if (mAccount != null && Eas.SUPPORTED_PROTOCOL_EX2003.equalsIgnoreCase(mAccount.mProtocolVersion)) {
                isServer03 = true;
            }
            Log.d("EmailSyncAdapter", "commit : check is Exchange 2003 server : " + isServer03);
            /// @ }
...
//                            .build());
            ...
            if (mMailbox.mId == Exchange.sBadSyncKeyMailboxId
                    && newEmails.size() > 0) {
                userLog("[BSK recovery] newEmails count:" + newEmails.size());
                ...
                if (!isStaleMailsRemoved) {
                    ...
                    userLog("[BSK recovery] Oldest timestamp: " + oldestTimestamp);

                    // Delete all the local mails older than the time stamp
                    int rowDeleted = mContentResolver.delete(Message.CONTENT_URI, WHERE_MESSAGE_TIMESTAMP_LESS_THAN,
                            new String[]{String.valueOf(oldestTimestamp), String.valueOf(mMailbox.mId)});
                    userLog("[BSK recovery] " + rowDeleted + " local stale mails were deleted");
                    ...
                    ContentValues cv = new ContentValues();
                    cv.put(Message.DIRTY, "1");
                    mContentResolver.update(Message.CONTENT_URI, cv,
                            WHERE_MAILBOX_KEY, new String[]{String.valueOf(mMailbox.mId)});
                ...
                userLog("[BSK recovery] Finding all the local mails with the same timestamp");
                ...
                String selection = MessageColumns.MAILBOX_KEY + "=" + Long.toString(mMailbox.mId) + " AND "
                + MessageColumns.TIMESTAMP + " IN " + timestampList.toString();
                userLog("[BSK recovery] selection clause: " + selection);
                Cursor c = mContentResolver.query(Message.CONTENT_URI, new String[] {
                        MessageColumns.ID, MessageColumns.TIMESTAMP },
                        selection, null, null);
                try {
                    if (c != null) {
                        ...
                        userLog("[BSK recovery] For current window, found "
                                + c.getCount() + " mails existed in local DB");
                        ...
                        while (c.moveToNext()) {
                            ...
                            for (Message msg : newEmails) {
                                if (msg.mTimeStamp == timestamp) {
                                    ...
                                    ContentValues cv = new ContentValues();
                                    cv.put(Message.SERVER_ID, msg.mServerId);
                                    ...
                                    bskOps.add(ContentProviderOperation.newUpdate(ContentUris.withAppendedId(
                                            Message.CONTENT_URI, c.getInt(columnMessageId))).withValues(cv).build());
                        ...
                        mContentResolver.applyBatch(EmailContent.AUTHORITY, bskOps);
                    }
                } catch (RemoteException e) {
                    // There is nothing to be done here; fail by returning null
                    userLog("[BSK recovery] RemoteException when applyBatch");
                } catch (OperationApplicationException e) {
                    // There is nothing to be done here; fail by returning null
                    userLog("[BSK recovery] OperationApplicationException when applyBatch");
                } finally {
                    if (c != null) {
                        c.close();
                    }
                ...
                userLog("[BSK recovery] There are " + newEmails.size()
                        + " mail(s) remaining in the newEmails list");
            }
            /** @} */
            ...
            for (Message msg : newEmails) {
                ...
                if (isServer03) {
                    if (msg.mText != null) {
                        if (msg.mText.length() > NEED_TRUNCATED_BODY_SIZE) {
                            msg.mText = msg.mText.substring(0, NEED_TRUNCATED_BODY_SIZE);
                            msg.mFlags |= Message.FLAG_BODY_TOO_LARGE;
                        }
                    }
                    if (msg.mHtml != null) {
                        if (msg.mHtml.length() > NEED_TRUNCATED_BODY_SIZE) {
                            msg.mHtml = msg.mHtml.substring(0, NEED_TRUNCATED_BODY_SIZE);
                            msg.mFlags |= Message.FLAG_BODY_TOO_LARGE;
                        }
                    }
                    msg.save(mContext);
            ...
            synchronized (mService.getSynchronizer()) {
                ...
                try {
                    ...
                    if (Logging.LOG_PERFORMANCE) {
                        Logging.d(Logging.LOG_TAG, "[Exchange Download Speed] EmailProvider" +
                                ":applyBatch:EmailSyncAdapter:commit [" + (end - start) + "]");
                    }
                    if (newEmails.size() > 0) {
                        if (mIsFirstLoaded) {
                            ...
                            if (Logging.LOG_PERFORMANCE) {
                                Logging.d(Logging.LOG_TAG,
                                       "[Exchange Download Speed] EmailProvider:applyBatch:" +
                                       "EmailSyncAdapter:firstShownMessages:newEmail:"
                                       + newEmails.size() + " [" + end + "]");
                            }
                        } else {
                            if (Logging.LOG_PERFORMANCE) {
                    ...
                    try{
                        /// M: fix binder allocated fail in Exchange 2003.
                        if (isServer03) {
                            Message.logMessageReceiving(mContext, newEmails.toArray(new Message[] {}));
                        } else {
                            for (Message msg : opIndex.keySet()) {
                                Uri u = results[opIndex.get(msg)].uri;
                                msg.mId = Long.parseLong(u.getPathSegments().get(1));
                            }
                            Message.logMessageReceiving(mContext, opIndex.keySet().toArray(new Message[] {}));
                        }
                    }catch(Exception ex){
                        // Do nothing
                    }
                    for (Message msg : fetchedEmails) {
                     // Find the original message's id (by serverId and mailbox)
                        ...
                        try {
                            ExchangeService.callback().fetchMessageCallBack(Integer.valueOf(id),
                    ...
                    Log.d("EmailSyncAdapter", "commit : applyBatch fail : " + e);
                } catch (OperationApplicationException e) {
                    Log.d("EmailSyncAdapter", "commit : applyBatch fail : " + e);
                }
            }
    ...
    public boolean sendLocalChanges(Serializer s) throws IOException {
        ...
        try {
            ...
            while (c.moveToNext()) {
                ...
                // state, since the updated table saves the original state
                ...
                try {
                    ...
                    if (mailbox != c.getLong(Message.LIST_MAILBOX_KEY_COLUMN)) {
                        if (mailbox == trashMailboxId) {
                            ...
                            // Send the command to delete this message
                            s.start(Tags.SYNC_DELETE).data(Tags.SYNC_SERVER_ID, serverId).end();
                            // Mark the message as moved (so the copy will be
                            // deleted if/when the server version is synced)
     ...
     * @throws IOException
     * @{ */
    public int getWindowCount() throws IOException {
        int estimateCount = getEstimate(getEmailFilter());
        userLog("[BSK Recovery] estimate count: " + estimateCount);
        int syncAsNormal = Integer.valueOf(EMAIL_WINDOW_SIZE);
        try {
            if (estimateCount >= 0) {
                ...
                if (syncAsRecover > 0) {
                    ...
                    if (modResult == 0) {
                        ...
                        userLog("[BSK Recovery] syncAsRecover size = " + syncAsRecover);
                        userLog("[BSK Recovery] divideResult = " + divideResult);
                ...
                userLog("[BSK Recovery] get estimate failed");
                return -1;
            }
        } finally {
            userLog("[BSK Recovery] mWindowSizeRecover = " + mWindowSizeRecover);
        }
    }
    /** @} */

    /**
     * M: fix attachment location for mime message.
     * This should only run in 2.5.
     * @param message parsed from mime message which need to be updated
     * @param olderAtts attachments received in attachment fields
     */
    public void fixAttachmentLocation(Message message,
            ArrayList<Attachment> olderAtts) {
        ArrayList<Attachment> updatedAtts = message.mAttachments;
        boolean fixResult = true;
        for (Attachment att : updatedAtts) {
            for (Attachment olderAtt : olderAtts) {
                if (olderAtt.mFileName.equals(att.mFileName)
                        && olderAtt.mMimeType.equals(att.mMimeType)) {
                    att.mLocation = olderAtt.mLocation;
                    if (att.mSize <= 0 && olderAtt.mSize > 0) {
                        att.mSize = olderAtt.mSize;
                    }
                    continue;
                }
            }
            // Still an invalid attachment after fixed, use the outside parsed
            // data (by default design).
            if (att.mSize <= 0 || att.mLocation == null) {
                fixResult = false;
                break;
            }
        }
        ...
        if (!fixResult) {
            userLog("fixAttachmentLocation failed, use the default outside parsed data!");
            message.mAttachments = olderAtts;
        } else {
            userLog("fixAttachmentLocation successfully!");
        }
    }
}
