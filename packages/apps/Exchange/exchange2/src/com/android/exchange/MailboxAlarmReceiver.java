...
public class MailboxAlarmReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ExchangeService.log(">>>>> MailboxAlarmReceiver onReceive");
        long mailboxId = intent.getLongExtra("mailbox", ExchangeService.EXTRA_MAILBOX_ID);
        if (intent.getBooleanExtra("needNotifyEasSyncService", false)) {
            ExchangeService.log("MailboxAlarmReceiver"+"will notify the EasService");
            ExchangeService.notifyEasSyncService(mailboxId);
            ExchangeService.log("<<<<< MailboxAlarmReceiver onReceive");
            return;
        }
