...
public final class LatinIME extends InputMethodService implements KeyboardActionListener,
    ...
    /// M : Add flag to enable memory trim on dictionary when low memory.
    private static final boolean TRIM_ON_LOW_MEMORY = true;
    ...
    /// M:flag if the configuration changed.  @{
    ...
    /* package private */ void resetSuggestMainDict() {
        /// M: After onLowMemory called, mSuggest is null. @{
    ...
    ///M : Override InputMethodService#onKeyDown. The purpose is that when there is
    //     both moreSuggestionWindow and keyboardview on the screen, latin needs to
    //     dismiss moreSuggestionWindow first, and then dismiss keyboardview on the
    //     next KEYCODE_BACK. @{
    @Override
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        if ((event.getKeyCode() == KeyEvent.KEYCODE_BACK) && (mSuggestionStripView != null)) {
            if (mSuggestionStripView.dismissMoreSuggestions()) {
                if (DEBUG) Log.d(TAG, "MoreSuggestionWindow is showing, just dismiss it");
                return true;
            }
        }
    ...
    public void onConfigurationChanged(final Configuration conf) {
        ...
        if (mDisplayOrientation != conf.orientation) {
            ...
            /// M: mark the configuration changed. @{
            mIsConfigurationChanged = true;
            /// @}
            /// M: if the orientation not changed, unnecessary initViews @{
            //     on the super.onConfigurationChanged.
    ...
    private void onStartInputViewInternal(final EditorInfo editorInfo, final boolean restarting) {
        ...
        /// M:  when this method called in the onConfigurationChanged, the selection position
        ///     should not init. {@
    ...
    private void onFinishInputInternal() {
        ...
        ///M: If Commit success, Change the mWordComposer state. @{
    ...
    private void onFinishInputViewInternal(final boolean finishingInput) {
        ...
        ///M: If Commit success, Change the mWordComposer state. @{
        if (!finishingInput && getCurrentInputConnection() != null) {
            resetComposingState(true);
        }
        /// @}
    ...
    /// M: Volunteer Free Memory for support LCA project.
    //     Lines below are added to free heap at LatinIME at low memory environment.
    //     Since dictionary will consum lots of heap, and to release this heap at low memory
    //     is helpful for other processes.
    //     And also dictionaries will be brought back at the next soft keyboard is required
    //     to show.
    // TODO: Maybe a new machanism should be brought to notify LatinIME that low memory
    //       has passed, since we are facing a reality that when low memory, it`s not ok
    //       to use dictionary. @{
    @Override
    public void onLowMemory() {
        if (TRIM_ON_LOW_MEMORY) {
            Log.w(TAG, "on Low memory! " + mSuggest);
            //when user is interacting with the keyboard,we shouldn't release the dictionoarys.
            if (!isInputViewShown() && mSuggest != null) {
                mSuggest.close();
                mSuggest = null;
            }
        }
    ...
    public void showWindow(boolean showInput) {
        if (TRIM_ON_LOW_MEMORY) {
            if (mSuggest == null) {
                Log.w(TAG, "Load suggest in showWindow");
                initSuggest();
            }
