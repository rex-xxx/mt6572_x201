...
public class KeyboardView extends View implements PointerTracker.DrawingProxy {
    ...
    /*
     * M: Add for special case in Bengali Keyboard.
     *    Below characters's size is bigger than common ones.
     *    So we need special control for its size.
     */
    private boolean isSpecialChar(String label) {
        char c = 0;
    ...
    protected void onDrawKeyTopVisuals(final Key key, final Canvas canvas, final Paint paint,
        ...
        if (key.mLabel != null) {
            ...
            ///M: Add for Special case in Bengali keyboard. @{
            int tempSize = key.selectTextSize(params);
            final int labelSize;
            if (!isSpecialChar(key.mLabel)) {
                labelSize = tempSize;
            } else {
                labelSize = (int)(tempSize * 0.65);
            }
            ///@}
