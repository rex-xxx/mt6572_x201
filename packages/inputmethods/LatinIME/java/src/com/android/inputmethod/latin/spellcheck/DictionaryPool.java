...
public final class DictionaryPool extends LinkedBlockingQueue<DictAndProximity> {
    ...
    public DictAndProximity poll(final long timeout, final TimeUnit unit)
        ...
        synchronized(this) {
            ///M: Add for the condition that DictionaryPool has been Close,
            ///but there are other threads still attempt to fetch DictAndProximity @{
            if (mClosed) {
                Log.i(TAG,"The dictionaryPool has been closed. mSize = " + mSize);
                return null;
            }
            ///@}
            if (mSize >= mMaxSize) {
