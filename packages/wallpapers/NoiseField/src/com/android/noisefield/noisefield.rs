...
void positionParticles() {
    ...
    for(int i=0; i<size; i++) {
        ...
        /// M: particles' z value is expected to be zero and also should be initialized.
...
void touch(float x, float y) {
    ...
    if(!landscape){
        wRatio = 1.0;
        /// M: rsgGetHeight() and rsgGetWidth() should be cast from int to float.
        hRatio = (float)rsgGetHeight() / (float)rsgGetWidth();
    } else {
        hRatio = 1.0;
        /// M: rsgGetHeight() and rsgGetWidth() should be cast from int to float.
        wRatio = (float)rsgGetWidth() / (float)rsgGetHeight();
    }
