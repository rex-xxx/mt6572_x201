...
public class LiveWallpaperPreview extends Activity {
    ...
    /// M: Dialog for low storage warning
    ...
    /// M: Finish itself since current storage is too low to instantiate the live wallpaper service
    private boolean finishIfStorageLow() {
        final IPackageManager pm = ActivityThread.getPackageManager();
        try {
            if (pm.isStorageLow()) {
                Log.v(LOG_TAG, "Storage low, stop preview activity");
    ...
    public void onResume() {
        ...
        /// M: Stop doing anything if storage is low
        if (finishIfStorageLow()) {
            return;
        }
    ...
    @Override
    /// M: Close the warning dialog if activity no longer visible
