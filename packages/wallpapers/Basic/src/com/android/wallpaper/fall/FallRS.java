...
class FallRS extends RenderScriptScene {
    ...
    public void resize(int width, int height) {
        ...
        /// M: the glWidth and glHeight in RenderScript should be 2*(Height/Width) and 2, or the rotated result; @{
        if (width > height) {
            mScript.set_g_glWidth(2.0f * (float)(width / height));
            mScript.set_g_glHeight(2.0f);  
        } else {
            mScript.set_g_glWidth(2.0f);
            mScript.set_g_glHeight(2.0f * (float)(height / width));        	
        }
        /// @}
