...
public abstract class RenderScriptWallpaper<T extends RenderScriptScene> extends WallpaperService {
    ...
    private class RenderScriptEngine extends Engine {
        ...
        public void onSurfaceChanged(SurfaceHolder holder, int format, int width, int height) {
            ...
            /// M: if surface's size is zero, should not start or resize, it will update later. @{
            if (width == 0 || height == 0) {
                Xlog.d(TAG, "onSurfaceChanged holder = " + holder + ", width = " + width + ", height = " + height);
                return;
            }
            /// @}
        ...
        public Bundle onCommand(String action, int x, int y, int z,
            ...
            /// M: fix the null point exception @{
            if (mRenderer != null) {
