...
import com.mediatek.providers.calendar.extension.ExtConsts;
...
public class CalendarProvider2 extends SQLiteContentProvider implements OnAccountsUpdateListener {
    ...
    ///M: MAX_ALLOWED_ITERATIONS get from RecurrenceProcessor class.
    ...
    ///M: change delay time from 30s to 5s, to make update delay shorter
    private static final long SYNC_UPDATE_BROADCAST_TIMEOUT_MILLIS = 5 * DateUtils.SECOND_IN_MILLIS;
    ...
    private void regenerateInstancesTable() {
        Log.v(CalendarProvider2.TAG, "re generate instance table");
    ...
    void acquireInstanceRangeLocked(long begin, long end, boolean useMinimumExpansionWindow,
        ...
        if ((begin >= minInstance) && (end <= maxInstance)) {
            /**
             * M: when it satisfies the condition: 
             * 1. begin, end is in the range [minInstance, maxInstance]
             * 2. maxInstance-minInstance > MAX_ALLOWED_ITERATIONS days
             * expand the instance range, and update the value (minInstance, maxInstance)
             * to be (begin, end); otherwise it will not expand the instance. 
             * @{
             */
            if(maxInstance > minInstance && (maxInstance - minInstance) > MAX_ALLOWED_ITERATIONS * DAY_IN_MILLIS) {
                mInstancesHelper.expandInstanceRangeLocked(expandBegin, expandEnd, instancesTimezone);
                minInstance = expandBegin;
                maxInstance = expandEnd;
                Log.w(TAG + "-i", "###### instances range is already changed.");
            } else {
            /**@}*/
                if (DEBUG_INSTANCES) {
            ...
            if (createNewEvent) {
                ...
                if (modValues.containsKey(Events.SELF_ATTENDEE_STATUS)) {
                    ...
                    if (accountName != null) {
                        ...
                        /// M: We should try to match the email address with NOCASE @{
                        int count = mDb.update(Tables.ATTENDEES, attValues,
                                Attendees.EVENT_ID + "=? AND " + Attendees.ATTENDEE_EMAIL + "=? COLLATE NOCASE ",
    ...
    protected Uri insertInTransaction(Uri uri, ContentValues values, boolean callerIsSyncAdapter) {
        ...
        switch (match) {
            ...
            case EVENTS:
                ...
                ///M: added createTime column's value
                updatedValues.put(ExtConsts.PCSync.CREATE_TIME, System.currentTimeMillis());
                ///M: limit the text length to avoid potential ANR. if the title, description or location
                /// is too long, drop the tail text, replace with " ...".
                limitTextLength(updatedValues);
            ...
            case ATTENDEES:
                ...
                //FIXME: ///M: maybe the attendee email text length need to be limited.
    ...
    private int deleteReminders(Uri uri, boolean byId, String selection, String[] selectionArgs,
        ...
        /** M: because the reminder table changed, run schedule next alarm again. @{ */
    ...
    private int handleUpdateEvents(Cursor cursor, ContentValues updateValues,
        ...
        while (cursor.moveToNext()) {
            ...
            if (isUpdate) {
                ///M: add modify time column's value.
                modValues.put(ExtConsts.PCSync.MODIFY_TIME, System.currentTimeMillis());
                ///M: limit the text length to avoid potential ANR. if the title, description or location
                /// is too long, drop the tail text, replace with " ...".
                limitTextLength(modValues);
    ...
    private static final int TITLE_LENGTH_LIMIT = 2000;
    private static final int EVENT_LOCATION_LENGTH_LIMIT = 2000;
    private static final int DESCRIPTION_LENGTH_LIMIT = 10000;
    ...
    static {
        ...
        ///M: #extension#@{
        ///M: #lunar# 
        sEventsProjectionMap.put(ExtConsts.LunarEvent.IS_LUNAR, ExtConsts.LunarEvent.IS_LUNAR);
        sEventsProjectionMap.put(ExtConsts.LunarEvent.LUNAR_RRULE, ExtConsts.LunarEvent.LUNAR_RRULE);
        sEventEntitiesProjectionMap.put(ExtConsts.LunarEvent.IS_LUNAR, ExtConsts.LunarEvent.IS_LUNAR);
        sEventEntitiesProjectionMap.put(ExtConsts.LunarEvent.LUNAR_RRULE, ExtConsts.LunarEvent.LUNAR_RRULE);
        ///M: #pc sync#
        sEventsProjectionMap.put(ExtConsts.PCSync.CREATE_TIME, ExtConsts.PCSync.CREATE_TIME);
        sEventsProjectionMap.put(ExtConsts.PCSync.MODIFY_TIME, ExtConsts.PCSync.MODIFY_TIME);
        sEventEntitiesProjectionMap.put(ExtConsts.PCSync.CREATE_TIME, ExtConsts.PCSync.CREATE_TIME);
        sEventEntitiesProjectionMap.put(ExtConsts.PCSync.MODIFY_TIME, ExtConsts.PCSync.MODIFY_TIME);
        ///@}
    }
            ...
            for (String table : new String[]{Tables.CALENDARS, Tables.COLORS}) {
                ...
                while (c.moveToNext()) {
                    ...
                    if (c.getString(0) != null
                            && c.getString(1) != null
                            ///M: #PC sync# for GB complience, ACCOUNT_TYPE_LOCAL might be lower case.
                            && !TextUtils.equals(c.getString(1).toUpperCase(), CalendarContract.ACCOUNT_TYPE_LOCAL)) {
                    ...
                    } else if (TextUtils.equals(c.getString(1), "local")) {
                        ///M: add for upgrade database from GB to ICS.@{
                        Log.d(TAG, "update account type for 'local' account.");
                        ContentValues values = new ContentValues();
                        values.put(Calendars.ACCOUNT_TYPE, CalendarContract.ACCOUNT_TYPE_LOCAL);
                        String selection = Calendars.ACCOUNT_NAME + "=? AND " + Calendars.ACCOUNT_TYPE + "=?";
                        String[] params = new String[] { c.getString(0), c.getString(1) };
                        int result = mDb.update(Tables.CALENDARS, values, selection, params);
                        if (0 == result) {
                            Log.v(TAG, "Could not update Events table with values " + values);
                        }
                        ///@}
    ...
    ///M: limit the title, description and location to a acceptable length, to avoid potential ANR @{ 
    /**
     * limit the title, description and location to a acceptable length, to avoid potential ANR 
     */
    private void limitTextLength(ContentValues values) {
        limitByColumn(values, Events.TITLE, TITLE_LENGTH_LIMIT);
        limitByColumn(values, Events.DESCRIPTION, DESCRIPTION_LENGTH_LIMIT);
        limitByColumn(values, Events.EVENT_LOCATION, EVENT_LOCATION_LENGTH_LIMIT);
    }

    /**
     * the src will be limited to the given length
     * the tail text be replaced to " ..." 
     * @param src
     * @param lengthLimit
     * @return
     */
    private String limitedString(String src, int lengthLimit) {
        if (lengthLimit < TEXT_APPENDIX.length()) {
            return TEXT_APPENDIX;
        }
        if (src.length() > lengthLimit) {
            Log.d(TAG, "the string is too long(" + src.length() + "), limit it to " + lengthLimit);
            return src.subSequence(0, lengthLimit - TEXT_APPENDIX.length()) + TEXT_APPENDIX;
        } else {
            return src;
        }
    }
    ...
    /**
     * limit a single column text length to given length.
     * @param values
     * @param key
     * @param textLength
     */
    private void limitByColumn(ContentValues values, String key, int textLength) {
        if (values.containsKey(key)) {
            String text = values.getAsString(key);
            if (null != text) {
                values.put(key, limitedString(text, textLength));
            }
        }
    }
    /// @}
