...
import com.mediatek.providers.calendar.extension.ITableExt;
...
/* package */ class CalendarDatabaseHelper extends SQLiteOpenHelper {
    ...
    private void createEventsTable(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE TABLE " + Tables.EVENTS + " (" +
                ...
                ///M:#extension# #PC Sync# @{
                ExtConsts.PCSync.CREATE_TABLE +
                /// @}
                ///M: #extension# #lunar# @{
                ExtConsts.LunarEvent.CREATE_TABLE + ");");
                ///@}
    ...
    private void createCalendarsTable(SQLiteDatabase db) {
        ...
        ///M: #extension# Calendars table extension @{
        ITableExt ext = ExtensionFactory.getCalendarsTableExt(Tables.CALENDARS);
        ext.tableExtension(db);
        ///@}
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        ...
        try {
            ///M: #upgrade# extension for MTK upgrade @{
            IDatabaseUpgradeExt ext = ExtensionFactory.getDatabaseUpgradeExt();
            oldVersion = ext.downgradeMTKVersionsIfNeeded(oldVersion, db);
            ///@}
            if (oldVersion < 51) {
            ...
            ///M: #upgrade# extension for MTK upgrade @{
            if (oldVersion == IDatabaseUpgradeExt.MTK_JB_DB_VERSION_BASE) {
                oldVersion = ext.upgradeToMTKJBVersion(db);
                createEventsView = true;
            }
            ///@}
            if (oldVersion == IDatabaseUpgradeExt.MTK_JB_DB_VERSION) {
