...
class MediaThumbRequest {
    ...
    Uri updateDatabase(Bitmap thumbnail) {
        ...
        if (c == null) {
            /// M: add log
            MtkLog.e(TAG, "updateDatabase: Null cursor! id=" + mOrigId);
            return null;
        }
        try {
            if (c.moveToFirst()) {
    ...
    void execute() throws IOException {
        MtkLog.v(TAG, "execute: uri=" + mUri + ",magic=" + mMagic + ",origId=" + mOrigId);
        ...
        if (magic != 0) {
            ...
            if (fileMagic == magic) {
                ...
                try {
                    ...
                    MtkLog.e(TAG, "execute: IOException! id=" + mOrigId, ex);
                } finally {
                    if (c != null) c.close();
                    if (pfd != null) {
                        pfd.close();
                        /// M: Release fd opend by miniThumbFile. @{
        ...
        if (bitmap != null) {
            ...
            /// M: For a better quality, or losses is obvious for pure color.
            bitmap.compress(Bitmap.CompressFormat.JPEG, 100, miniOutStream);
            bitmap.recycle();
            byte [] data = null;
