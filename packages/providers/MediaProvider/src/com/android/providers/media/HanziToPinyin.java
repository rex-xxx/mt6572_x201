...
/**
 * An object to convert Chinese character to its corresponding pinyin string. For characters with multiple possible pinyin
 * string, only one is selected according to collator. Polyphone is not supported in this implementation. This class is
 * implemented to achieve the best runtime performance and minimum runtime resources with tolerable sacrifice of accuracy.
 * This implementation highly depends on zh_CN ICU collation data and must be always synchronized with ICU.
 * 
...
public class HanziToPinyin {
    ...
    public static class Token {
        ...
        /**
         * Constants indicating it is a Latin token.
         */
        public static final int LATIN = 1;
        /**
         * Constants indicating it is a pinyin token.
         */
        public static final int PINYIN = 2;
        /**
         * Constants indicating the token type is unknown.
         */
         ...
         * @param source original string before translation.
         * @param target translated string of source.
         */
        ...
        /**
         * Translated string of source. For Han, target is corresponding Pinyin. Otherwise target is original string in
         * source.
         */
     ...
     * Constructor.
     * 
     * @param hasChinaCollator indicates {@link Locale#CHINA} is supported or not.
     */
    ...
    /**
     * Gets HanziToPinyin instance.
     * 
     * @return HanziToPinyin instance.
     */
    ...
    private Token getToken(char character) {
        ...
        token.mSource = letter;
        int offset = -1;
        int cmp;
        if (character < 256) {
            token.mType = Token.LATIN;
            token.mTarget = letter;
            return token;
        } else if (character < FIRST_UNIHAN) {
            token.mType = Token.UNKNOWN;
            token.mTarget = letter;
            return token;
        } else {
        ...
        token.mTarget = pinyin.toString();
        return token;
    }

    /**
     * Convert the input to a array of tokens. The sequence of ASCII or Unknown characters without space will be put into a
     * Token, One Hanzi character which has pinyin will be treated as a Token. If these is no China collator, the empty token
     * array is returned.
     * 
     * @param input original string to be translated.
     */
        ...
        for (int i = 0; i < inputLength; i++) {
            ...
            if (character == ' ') {
                ...
                if (t.mType == Token.PINYIN) {
                    ...
                    if (tokenType != t.mType && sb.length() > 0) {
                        addToken(sb, tokens, tokenType);
                    }
                    tokenType = t.mType;
                    sb.append(character);
