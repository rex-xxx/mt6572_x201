...
public class ApplicationLauncher extends ListActivity {
    ...
    private void showSearchResults(String query) {
        ...
        /// M: When there is not result, it should display the string such as "no such results" @{
        if (mCursor == null || mCursor.getCount() == 0) {
            TextView tvEmpty = new TextView(this);
            tvEmpty.setText(R.string.search_empty);
            LayoutParams layoutPrarams = new LayoutParams(LayoutParams.MATCH_PARENT,
                    LayoutParams.MATCH_PARENT);
            tvEmpty.setLayoutParams(layoutPrarams);
            tvEmpty.setGravity(Gravity.CENTER);
            this.addContentView(tvEmpty, layoutPrarams);
        }
        /// @}
    }
