...
/** M: New Feature DialerSearch added @{ */
...
public class ContactAggregator {
    ...
    private interface RawContactsQuery {
        ...
        int STARRED = 11;
          ...
          * @{
          */
        int INDICATE_PHONE_SIM = 12;    /** M: New Feature Gemini added */
        int INDEX_IN_SIM = 13;          /** M: New Feature Gemini added */
        ...
        int SEND_TO_VOICEMAIL_VT = 18;  /** M: New Feature Gemini added */
        int SEND_TO_VOICEMAIL_SIP = 19; /** M: New Feature Gemini added */

        int PHOTO_FILE_ID = 20;
        int IS_SDN_CONTACT = 21;        /** M: New Feature Gemini added */
        /** @} */
    }
    ...
    private interface ContactReplaceSqlStatement {
        ...
        /** 
          ...
          * @{ 
          */
        int INDICATE_PHONE_SIM = 11;    /** M: New Feature Gemini added */
        int INDEX_IN_SIM = 12;          /** M: New Feature Gemini added */
        int SEND_TO_VOICEMAIL_VT = 13;  /** M: New Feature Gemini added */
        int SEND_TO_VOICEMAIL_SIP = 14; /** M: New Feature Gemini added */
        int IS_SDN_CONTACTS = 15;       /** M: New Feature Gemini added */
        int CONTACT_ID = 16;
        /** @} */
    }
    ...
    public void updateDisplayNameForContact(SQLiteDatabase db, long contactId) {
        ...
        if (mDisplayNameCandidate.rawContactId != -1) {
			...
			if (FeatureOption.MTK_DIALER_SEARCH_SUPPORT) {
				updateDialerSearchForJoin(db, contactId);
			}
            /** @} */
        }
	...
	/// M: The fillowing lines are provided and maintained by Mediatek inc. @{
    ...
    private SQLiteStatement mDSNameUpdate;
    private SQLiteStatement mDSNumberUpdate;
    private static final int DS_INVISIABLE = 0;
    private static final int DS_VISIABLE = 1;

	private void updateDialerSearchForJoin(SQLiteDatabase db, long contactId) {
        long nameRawContactId = mDisplayNameCandidate.rawContactId;
        mSelectionArgs1[0] = String.valueOf(contactId);
        ...
        try {
            while (c.moveToNext()) {
                long rawContactId = c.getLong(DisplayNameQuery._ID);
                if (nameRawContactId != rawContactId) {
                    Log.i("Aggregation", "[updateDialerSearchForJoin]rawContactId:" + rawContactId
                            + " ||contactId:" + contactId);
                    mDSNameUpdate.bindLong(1, DS_INVISIABLE);
                    mDSNameUpdate.bindLong(2, rawContactId);
                    mDSNameUpdate.bindLong(3, DS_VISIABLE);
                    mDSNameUpdate.execute();
                    Log.i("Aggregation", "[updateDialerSearchForJoin]nameRawContactId:"  + nameRawContactId);
                    mDSNumberUpdate.bindLong(1, nameRawContactId);
                    mDSNumberUpdate.bindLong(2, rawContactId);
                    mDSNumberUpdate.bindLong(3, DS_VISIABLE);
                    mDSNumberUpdate.execute();
                }
            }
        } finally {
            c.close();
    ...
    private void processCompileStatementEx(SQLiteDatabase db) {
        if (FeatureOption.MTK_DIALER_SEARCH_SUPPORT) {
            mDSNameUpdate = db.compileStatement(
                    "UPDATE " + Tables.DIALER_SEARCH +
                            " SET " + DialerSearchLookupColumns.IS_VISIABLE + "=? " +
                            " WHERE " + DialerSearchLookupColumns.RAW_CONTACT_ID + "=? AND " +
                            DialerSearchLookupColumns.NAME_TYPE + "=11 AND " +
                            DialerSearchLookupColumns.IS_VISIABLE + "=?");
            mDSNumberUpdate = db.compileStatement(
                    "UPDATE " + Tables.DIALER_SEARCH +
                            " SET " + DialerSearchLookupColumns.RAW_CONTACT_ID + "=?" +
                            " WHERE " + DialerSearchLookupColumns.RAW_CONTACT_ID + "=? AND " +
                            DialerSearchLookupColumns.NAME_TYPE + "=8 AND " +
                            DialerSearchLookupColumns.IS_VISIABLE + "=?");
        }
    }
    // / @}
