...
import com.mediatek.providers.contacts.SlotUtils;
...
public class ContactsProvider2 extends AbstractContactsProvider
   ...
   private static final int DIALER_SEARCH_INCREMENT = 90000;    //add by MTK
    ...
    private static final ProjectionMap sContactsColumns = ProjectionMap.builder()
            ...
            .add(Contacts.IS_SDN_CONTACT)           // add by MTK
            .build();
    ...
    private static final ProjectionMap sRawContactColumns = ProjectionMap.builder()
            ...
            .add(RawContacts.IS_SDN_CONTACT)        // add by MTK
            .build();
    ...
    private static final ProjectionMap sPhoneLookupProjectionMap = ProjectionMap.builder()
            ...
            .add(Phone.LABEL, Phone.LABEL)
            .add(Phone.DISPLAY_NAME, "contacts_view." + Contacts.DISPLAY_NAME)
            .add(Phone.CONTACT_ID, "contacts_view." + Contacts._ID)
            .add(Phone.CONTACT_PRESENCE, StatusUpdates.PRESENCE)
            .add(Phone.CONTACT_STATUS, StatusUpdates.STATUS)
            .add(Phone.NORMALIZED_NUMBER, Phone.NORMALIZED_NUMBER)
            .add(Contacts.SEND_TO_VOICEMAIL, "contacts_view." + Contacts.SEND_TO_VOICEMAIL)
            /// @}
    ...
    static {
        ...
        /** M: no number should also match PHONE_LOOKUP */
        ...
        /// M: @{
        matcher.addURI(ContactsContract.AUTHORITY, "data/phone_email", PHONE_EMAIL);
        matcher.addURI(ContactsContract.AUTHORITY, "data/phone_email/filter", PHONE_EMAIL_FILTER);
        matcher.addURI(ContactsContract.AUTHORITY, "data/phone_email/filter/*", PHONE_EMAIL_FILTER);
        matcher.addURI(ContactsContract.AUTHORITY, "dialer_search/filter/*", DIALER_SEARCH_INCREMENT);
        matcher.addURI(ContactsContract.AUTHORITY, "dialer_search_number/filter/*", DIALER_SEARCH_SIMPLE);
        /// @}
    }
    ...
    private boolean initialize() {
        ...
        /**
         * M: Move creation to method which can be overrided by tests
         * @{
    ...
    protected int deleteInTransaction(Uri uri, String selection, String[] selectionArgs) {
        ...
        switch (match) {
            ...
            case CONTACTS: {
                ...
                // TODO
                /// M:
                /// Support batch deleting process for contacts. Need client to set 
                /// the query parameter batch as true. 
                /// @{
                Log.i(TAG, "deleteInTransaction for CONTACTS uri is " + uri.toString()
                        + " selection is " + selection);
                if ("true".equals(uri.getQueryParameter("batch"))) {
                    Log.i(TAG, "Delete in one batch begin");
                    final StringBuilder whereBuilder = new StringBuilder();
                    final String[] questionMarks = new String[selectionArgs.length];
                    Arrays.fill(questionMarks, "?");
                    whereBuilder.append(RawContacts.CONTACT_ID + " IN (").append(
                            TextUtils.join(",", questionMarks)).append(")");

                    // Actual delete the local contacts
                    int localAccountDeletes = 0;
                    localAccountDeletes = deleteRawContactInOneBatch(RawContacts.CONTENT_URI,
                            whereBuilder.toString() + " AND (" + RawContacts.ACCOUNT_TYPE + " IS '"
                                    + ACCOUNT_TYPE_LOCAL_PHONE + "' OR " + RawContacts.ACCOUNT_TYPE
                                    + " IS NULL)", selectionArgs);
                    Log.i(TAG, "deleteRawContactInOneBatch count is " + localAccountDeletes);

                    // Mark delete for SyncAccount contacts
                    int syncAccountDeletes = 0;
                    Cursor c = db.query(
                            Views.RAW_CONTACTS,
                            new String[] {RawContacts._ID, RawContacts.CONTACT_ID},
                            whereBuilder.toString() + " AND (" + RawContacts.ACCOUNT_TYPE
                                    + " IS NOT '" + ACCOUNT_TYPE_LOCAL_PHONE + "' OR "
                                    + RawContacts.ACCOUNT_TYPE + " IS NOT NULL)", selectionArgs,
                            null, null, null);
                    try {
                        while (c.moveToNext()) {
                            final long contactId = c.getLong(1);
                            syncAccountDeletes += deleteContact(contactId, true);
                        }
                    } finally {
                        c.close();
                    }
                    Log.i(TAG, "deleteContactInSyncAccount count is " + syncAccountDeletes);
                    Log.i(TAG, "Delete in one batch end");
                    return localAccountDeletes + syncAccountDeletes;
                }
                /// @}
                return 0;
            }
            ...
            case RAW_CONTACTS:
                ...
                if ("true".equals(uri.getQueryParameter("sim"))) {
                    ...
                    if (FeatureOption.MTK_SEARCH_DB_SUPPORT) {
                        count = updateDialerSearchDataForMultiDelete(selection, selectionArgs);
                    } else {
                        count = db.delete(Tables.RAW_CONTACTS, selection, selectionArgs);
                    }
    ...
    public int deleteRawContact(long rawContactId, long contactId, boolean callerIsSyncAdapter) {
        ...
        if (callerIsSyncAdapter || rawContactIsLocal(rawContactId)) {
            ...
            if (FeatureOption.MTK_SEARCH_DB_SUPPORT) {
                updateDialerSearchDataForDelete(rawContactId);
            }
            return count;
        } else {
    ...
    private int markRawContactAsDeleted(SQLiteDatabase db, long rawContactId,
         ...
         * @{
         */
        int updateCount = updateRawContact(db, rawContactId, mValues, callerIsSyncAdapter);
        if (FeatureOption.MTK_SEARCH_DB_SUPPORT) {
            updateDialerSearchDataForDelete(rawContactId);
        }
        return updateCount;
        ...
        switch(match) {
            ...
            case CONTACTS_LOOKUP_ID: {
                ...
                if (segmentCount > 3) {
                    ...
                    count = updateContactOptions(db, Long.parseLong(contactId), values,
                            callerIsSyncAdapter);
                    break;
                }
                /// @}                
    ...
    private int updateContactOptions(SQLiteDatabase db, long contactId, ContentValues values,
        ...
        mValues.clear();
            ...
            int count = db.update(Tables.CONTACTS, 
    ...
    private boolean updateAccountsInBackground(Account[] systemAccounts) {
        ...
        try {
             ...
             */
                ...
                Log.e(TAG, "[updateAccountsInBackground]catch SQLiteCantOpenDatabaseException of endTransaction(), return false");
                return false;
            /**
             * [ALPS00596515] @}
             */
    ...
    protected Cursor queryLocal(final Uri uri, final String[] projection, String selection,
        ...
        switch (match) {
                ...
                if (uri.getBooleanQueryParameter(PhoneLookup.QUERY_PARAMETER_SIP_ADDRESS, false)) {
                    ...
                    try {
                        if (cursor.getCount() > 0) {
                            foundResult = true;
                            /**M: to find out the number exactly matched contact(the first one) @ {*/
                            Cursor exactCursor = DialerSearchUtils.findNumberExactMatchedCursor(cursor, normalizedNumber);
                            if (exactCursor != null) {
                                Log.i(TAG, "[queryLocal] Phone_Lookup: has found the exact number match Contact.");
                                cursor.close();
                                return exactCursor;
                            }
                            /**@}*/
            ...
            case PHONE_EMAIL_FILTER: {
                ...
                if (filterParam == null) {
                    ...
                    if (filterParam.length() > 0 && !filterParam.contains("@")) {
                        final String ftsMatchQuery = SearchIndexManager.getFtsMatchQuery(
                                filterParam, FtsQueryBuilder.UNSCOPED_NORMALIZING);
                        if (ftsMatchQuery.length() > 0) {
                            sb.append(Data.RAW_CONTACT_ID + " IN " + "(SELECT "
                                    + RawContactsColumns.CONCRETE_ID + " FROM "
                                    + Tables.SEARCH_INDEX + " JOIN " + Tables.RAW_CONTACTS
                                    + " ON (" + Tables.SEARCH_INDEX + "."
                ...
                groupBy = Phone.DATA + "," + Email.DATA + "," + RawContacts.CONTACT_ID;
                if (sortOrder == null) {
                    final String accountPromotionSortOrder = getAccountPromotionSortOrder(uri);
                    if (!TextUtils.isEmpty(accountPromotionSortOrder)) {
                        sortOrder = accountPromotionSortOrder + ", " + PHONE_FILTER_SORT_ORDER;
        ...
        /** M: New Feature: return the ids in the filter mode @{ */
        final Uri nonFilterUri = requestNonFilterIds(uri);
        if (nonFilterUri != null) {
            bundleNonFilterIdsExtras(cursor, nonFilterUri, db, qb, selection, selectionArgs, 
                    sortOrder, groupBy, having, limit, cancellationSignal);
        }
        /** @} */
        if (snippetDeferred) {
    ...
    public AssetFileDescriptor openAssetFileLocal(Uri uri, String mode)
        ...
        switch (match) {
            ...
            case CONTACTS_AS_MULTI_VCARD: {
                ...
                /**
                 *  for android beam feature by mediatek start.
                 */
                if (uri.getBooleanQueryParameter(
                        Contacts.QUERY_PARAMETER_VCARD_NO_PHOTO, false)) {
                    queryUri = Contacts.CONTENT_URI
                            .buildUpon()
                            .appendQueryParameter(
                                    Contacts.QUERY_PARAMETER_VCARD_NO_PHOTO, "true")
                            .build();
                }
                /**
                 *  for android beam feature by mediatek end.
                 */
    ...
    //Data structure definition for Dialer Search begin
    private int mDisplayOrder = -1;
    private int mSortOrder = -1;
    private int prevSearchNumberLeng = 0;
    private static final String TEMP_DIALER_SEARCH_VIEW = "temp_dialer_search_view"; 
    private static final String TEMP_DIALER_SEARCH_TABLE = "temp_dialer_search_table";
    private static final String TEMP_TABLE_ALIAS_NUMBER = "table_number";
    private static final String TEMP_TABLE_ALIAS_NAME = "table_name";
    private static final String TEMP_BASEVIEW_ALIAS = "temp_base_view";
    
    private interface DsTempTableColumns {
    	public static final String _ID = "temp_id";
        public static final String NAME_TYPE = "name_type";
        public static final String RAW_CONTACT_ID = "temp_raw_contact_id";
    	...
    	public static final String MATCHED_OFFSETS_NAME_ALIAS = "matched_name_offsets";
    	public static final String MATCHED_OFFSETS_NUMBER_ALIAS = "matched_number_offsets";
    }
    ...
    public interface DialerSearchQuery {
    	...
    	public static final int IS_SDN_CONTACT              = 14;  // add by MTK
    	public static final int DS_MATCHED_DATA_OFFSETS 	= 15;
        public static final int DS_MATCHED_NAME_OFFSETS 	= 16;
        public static final int CALL_GEOCODED_LOCATION_INDEX = 17;
	...
	private static final String DS_SEARCH_RESULTS_COLUMNS = DialerSearch.NAME_LOOKUP_ID + ","
						 + DialerSearch.CONTACT_ID + ","
				     	 + DialerSearch.CALL_DATE+ ","
					     + DialerSearch.CALL_LOG_ID + ","
				 	     + DialerSearch.CALL_TYPE + ","
    ...
    //The initial TEMP_DIALER_SEARCH_VIEW without display name column and sort key column
    private static final String DS_INIT_VIEW_COLUMNS = DialerSearch.NAME_LOOKUP_ID + ","
					    ...
					    + DialerSearch.INDICATE_PHONE_SIM + ","
                        + DialerSearch.IS_SDN_CONTACT;  // add by MTK
	//The first temporary dialer search table without matched offset. Last name first.
	private static final String DS_FIRST_TEMP_TABLE_COLUMNS_ALTERNATIVE = DialerSearchLookupColumns._ID + " AS " + DsTempTableColumns._ID + ","
						+ DialerSearchLookupColumns.NAME_TYPE + " AS " + DsTempTableColumns.NAME_TYPE + ","
						+ DialerSearchLookupColumns.RAW_CONTACT_ID + " AS " + DsTempTableColumns.RAW_CONTACT_ID + ","
						+ DialerSearchLookupColumns.NORMALIZED_NAME_ALTERNATIVE + " AS " +  DsTempTableColumns.NORMALIZED_NAME + ","
						+ DialerSearchLookupColumns.SEARCH_DATA_OFFSETS_ALTERNATIVE + " AS " + DsTempTableColumns.SEARCH_DATA_OFFSETS; 
	//The first temporary dialer search table without matched offset. First name first.
	private static final String DS_FIRST_TEMP_TABLE_COLUMNS = DialerSearchLookupColumns._ID + " AS " + DsTempTableColumns._ID + ","
						+ DialerSearchLookupColumns.NAME_TYPE + " AS " + DsTempTableColumns.NAME_TYPE + ","
						+ DialerSearchLookupColumns.RAW_CONTACT_ID + " AS " + DsTempTableColumns.RAW_CONTACT_ID + ","
						+ DialerSearchLookupColumns.NORMALIZED_NAME + " AS " +  DsTempTableColumns.NORMALIZED_NAME + ","
						+ DialerSearchLookupColumns.SEARCH_DATA_OFFSETS + " AS " + DsTempTableColumns.SEARCH_DATA_OFFSETS;
	//Temporary dialer search table without matched offset.
	private static final String DS_TEMP_TABLE_COLUMNS = DsTempTableColumns._ID + "," 
						+ DsTempTableColumns.NAME_TYPE + ","
						+ DsTempTableColumns.RAW_CONTACT_ID + ","
						+ DsTempTableColumns.NORMALIZED_NAME + ","
						+ DsTempTableColumns.SEARCH_DATA_OFFSETS;
	
	//Temporary dialer search view table without matched name offset and matched number offset
	private static final String DS_VIEW_NAME_COLUMNS = DialerSearch.NAME_LOOKUP_ID + ","
				 	     + DialerSearch.CONTACT_ID + ","
				 	     + DialerSearch.RAW_CONTACT_ID + ","
				 	     + DialerSearch.NAME + ","
				     	 + DialerSearch.CALL_DATE + ","
					     + DialerSearch.CALL_LOG_ID + ","
				 	     ...
				 	     + DialerSearch.NAME_ID + ","
				 	     + DialerSearch.IS_SDN_CONTACT;  // add by MTK
	//Data structure definition for Dialer Search end
    
    private int processMatchEx(int match, Uri uri) {
    	if (FeatureOption.MTK_DIALER_SEARCH_SUPPORT && FeatureOption.MTK_SEARCH_DB_SUPPORT) {
	        String uriStr = uri.toString();
	        int m = match;
	        Log.d(TAG, "query process pond for dialer search: " + uriStr);
	        //matcher will seam # as illegal char
	        if (m == -1 && uriStr.indexOf("#") > 0) {
	        	if (uriStr.indexOf("dialer_search_number") > 0) {
	        		m = DIALER_SEARCH_SIMPLE;
	        	}
	        	else if (uriStr.indexOf("dialer_search") > 0) {
	        		m  = DIALER_SEARCH_INCREMENT;
	        	}
	        }
	        Log.d(TAG, "revised match: " + m);
	        return m;
    	} else {
    		return match;
    	}
    ...
    private String getDialerSearchNameTableColumns(int displayOrder,String filterParam,boolean isFirstTime){
    	if(isFirstTime){
	    	if(displayOrder == ContactsContract.Preferences.DISPLAY_ORDER_ALTERNATIVE ){
	    		String searchParamList = DialerSearchLookupColumns.NORMALIZED_NAME_ALTERNATIVE + "," 
									+ DialerSearchLookupColumns.SEARCH_DATA_OFFSETS_ALTERNATIVE + ","
									+ DialerSearchLookupColumns.NAME_TYPE + ",'"
									+ filterParam + "'";
	    		return DS_FIRST_TEMP_TABLE_COLUMNS_ALTERNATIVE + ","
	    				+ "DIALER_SEARCH_MATCH(" + searchParamList + ") AS " + DialerSearch.MATCHED_DATA_OFFSETS;
	    	}else{
	    		String searchParamList = DialerSearchLookupColumns.NORMALIZED_NAME + "," 
									+ DialerSearchLookupColumns.SEARCH_DATA_OFFSETS + ","
									+ DialerSearchLookupColumns.NAME_TYPE + ",'"
									+ filterParam + "'";
				return DS_FIRST_TEMP_TABLE_COLUMNS + ","
						+ "DIALER_SEARCH_MATCH(" + searchParamList + ") AS " + DialerSearch.MATCHED_DATA_OFFSETS;
	    	}
    	}else{
			return DS_TEMP_TABLE_COLUMNS + ","
						+ "DIALER_SEARCH_MATCH(" + DsTempTableColumns.NORMALIZED_NAME + "," 
						+ DsTempTableColumns.SEARCH_DATA_OFFSETS + ","
						+ DsTempTableColumns.NAME_TYPE + ",'"
						+ filterParam + "'" + ") AS " + DialerSearch.MATCHED_DATA_OFFSETS;
    	}
    }
    ...
    private String getDialerSearchViewColumns(int displayOrder, int sortOrder){
    	...
    	if (sortOrder == ContactsContract.Preferences.SORT_ORDER_ALTERNATIVE) {
    		sb.append("," + DialerSearch.SORT_KEY_ALTERNATIVE + " AS " + DialerSearch.SORT_KEY_PRIMARY);
    	} else {
    ...
    private int updateDialerSearchDataForMultiDelete(String selection,
            String[] selectionArgs) {
        final SQLiteDatabase db = mDbHelper.get().getWritableDatabase();
         ...
         */
            ...
            Log.e(TAG, "[updateDialerSearchDataForMultiDelete]catch SQLiteFullException for delete");
            return 0;
        }
        /**
         * [ALPS00596515] @}
    ...
    private void updateDialerSearchDataForDelete(long rawContactId) {
        
        final SQLiteDatabase db = mDbHelper.get().getWritableDatabase();
        Cursor c = db.rawQuery(
                "SELECT _id,number,data_id FROM calls WHERE raw_contact_id = "
                        + rawContactId + " GROUP BY data_id;", null);
        StringBuilder exceptDataIds = new StringBuilder();
        if (c != null) {
            ...
            while(c.moveToNext()) {
                ...
                if (PhoneNumberUtils.isUriNumber(number)){
                    dataCursor = db.rawQuery(
                            "SELECT _id,raw_contact_id,contact_id FROM view_data "
                             + " WHERE data1 =? AND mimetype_id=6 AND raw_contact_id !=? LIMIT 1",
                             new String[] {number, String.valueOf(rawContactId) });
                } else {
                    dataCursor = db.rawQuery(
                              "SELECT _id,raw_contact_id,contact_id FROM view_data "
                               + " WHERE PHONE_NUMBERS_EQUAL(data1, '"+ number+ "' , "+ UseStrict+ " )"
                ...
                if (dataCursor != null && dataCursor.moveToFirst()) {
                        ...
                        log("[updateDialerSearchDataForDelete]newDataId:" + newDataId
                                + "|newRawId:" + newRawId);
                        db.execSQL(
                                "UPDATE calls SET data_id=?, raw_contact_id=? "
                    ...
                    db.execSQL(
                            "UPDATE calls SET data_id=null, raw_contact_id=null "
                                    + "WHERE data_id=?", new String[] {
                                    String.valueOf(dataId) });
                    db.execSQL("UPDATE dialer_search "
                            ...
                            + " WHERE data_id =?", 
                            new String[] { number, number, String.valueOf(dataId) });
                    exceptDataIds.append(dataId).append(",");
        ...
        if (exceptDataIds.length() > 0) {
            exceptDataIds.deleteCharAt(exceptDataIds.length() - 1);
        }
        String delStr = "DELETE FROM dialer_search WHERE raw_contact_id=" + rawContactId;
        if (exceptDataIds.length() > 0) {
            delStr = delStr + " AND data_id NOT IN (" + exceptDataIds.toString() + ")"; 
        }
    ...
    private class SimStateReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            mBackgroundHandler.sendEmptyMessageDelayed(BACKGROUND_TASK_LOAD_LOCAL_ACCOUNT, 500);
        }
    ...
    protected void loadAccountsInBackground() {
        ...
        ///M: [Gemini+] get the sim type, load sim account and delete invalid sim accounts @{
        for (int slotId : SlotUtils.getAllSlotIds()) {
            if (SimCardUtils.isSimInserted(slotId)) {
                int simType = SimCardUtils.getSimTypeBySlot(slotId);
                String simAccountType = SimCardUtils.getSimAccountType(simType);
                String simAccountName = SlotUtils.getSimAccountNameForSlot(simType, slotId);
                ...
                deleteLocalSimAccounts(SlotUtils.getPossibleSimAccountNamesForSlot(slotId));
            }
        }
        ///@}
    }
    ...
    protected void loadLocalSimAccounts(String accountName, String accountType, int slotId) {
        ...
        if (TextUtils.isEmpty(accountName) || TextUtils.isEmpty(accountType)) {
            Log.i(TAG, "Account Name or type is null");
            return;
        }
        ...
        try {
            ...
            ///M: [Gemini+] remove the invalid account names except valid sim type @{
            Collection<String> allAccountNames = SlotUtils.getPossibleSimAccountNamesForSlot(slotId);
            if (allAccountNames.remove(accountName)) {
                Log.d(TAG, "delete all possible sim account, except for " + accountName);
                deleteLocalSimAccounts(allAccountNames);
            }
            ///@}
             ...
             */
                ...
                Log.e(TAG, "[loadLocalSimAccounts]catch SQLiteCantOpenDatabaseException for endTransaction()");
                return;
            /**
             * [ALPS00596515] @}
             */
    ...
    /**
     * M: [Gemini+] delete the sim account names from a given list
     * @param accountNames all account names to be deleted
     */
    protected void deleteLocalSimAccounts(Collection<String> accountNames) {
        for (String accountName : accountNames) {
            ...
            try {
                 ...
                 */
                    ...
                    Log.e(TAG, "[deleteLocalSimAccounts]catch SQLiteCantOpenDatabaseException for endTransaction()");
                    return;
                /**
                 * [ALPS00596515] @}
                 */
      ...
      try {
         ...
         db.setTransactionSuccessful();
             ...
             */
                ...
                Log.e(TAG, "[loadLocalPhoneAccounts]catch SQLiteCantOpenDatabaseException. for endTransaction()");
                return;
            }
            /**
             * [ALPS00596515] @}
    ...
    private void registerReceiverOnSimStateAndInfoChanged() {
        ...
        if (mBroadcastReceiver == null) {
            mBroadcastReceiver = new SimStateReceiver();
    ...
    public int deleteRawContactInOneBatch(Uri uri, String selection, String[] selectionArgs) {
        ...
        /** M: Android update from 4.1.2 to 4.2_r1 */
        final SQLiteDatabase db = mDbHelper.get().getWritableDatabase();

        // Find and delete stream items associated with the raw contact.
        Cursor streamItemsCursor = db.query(Tables.STREAM_ITEMS,
                new String[]{StreamItems._ID},
                StreamItems.RAW_CONTACT_ID + " IN (SELECT " + RawContacts._ID + " FROM "
                + Views.RAW_CONTACTS + " WHERE (" + selection + "))", selectionArgs,
                null, null, null);
        try {
            while (streamItemsCursor.moveToNext()) {
                deleteStreamItem(db, streamItemsCursor.getLong(0));
            }
        } finally {
        ...
        Cursor rawContactsCursor = db.query(Views.RAW_CONTACTS, 
                new String[] { RawContacts._ID, RawContacts.CONTACT_ID}, 
        ...
        int count = db.delete(
                Tables.RAW_CONTACTS,
                RawContacts._ID + " IN (SELECT " + RawContacts._ID + " FROM " + Views.RAW_CONTACTS
                        + " WHERE (" + selection + "))", 
                selectionArgs);
        ...
        if (rawContactsList != null && rawContactsList.size() > 0) {
            for (RawContactEntry rawContact : rawContactsList) {
                ...
                Log.d(TAG, "updateDialerSearchDataForDelete begin");
                if (FeatureOption.MTK_SEARCH_DB_SUPPORT) {
                    updateDialerSearchDataForDelete(rawContact.rawContactId);
                }
                Log.d(TAG, "updateDialerSearchDataForDelete end");
            }
        }
    ...
    protected void createBackgroundStuff() {
        mBackgroundThread = new HandlerThread("ContactsProviderWorker",
                Process.THREAD_PRIORITY_BACKGROUND);
        mBackgroundThread.start();
        mBackgroundHandler = new Handler(mBackgroundThread.getLooper()) {
    ...
    private Uri requestNonFilterIds(Uri uri) {
        String nonFilterArg = getQueryParameter(uri, "non_filter_ids_arg");
        if (TextUtils.isEmpty(nonFilterArg)) {
            return null;
        }

        return Uri.parse(nonFilterArg);
    }

    private void bundleNonFilterIdsExtras(final Cursor cursor, final Uri nonFilterUri, 
            final SQLiteDatabase db, SQLiteQueryBuilder qb, String selection, 
            String[] selectionArgs, String sortOrder, String groupBy,
            String having, String limit, CancellationSignal cancellationSignal) {
        Cursor c = null;
        try {
            c = query(nonFilterUri, new String[] {"_id"}, selection, selectionArgs, 
                    "_id", cancellationSignal);
            if (c != null && c.getCount() != 0) {
                ...
                extras.putIntArray("non_filter_ids", ids);
                ((AbstractCursor) cursor).setExtras(extras);
