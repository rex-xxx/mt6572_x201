...
public final class DownloadProvider extends ContentProvider {
    ...
    private static final String[] sAppReadableColumnsArray = new String[] {
        ...
        // M: Add this to query OMA DL status
        ...
        private void upgradeTo(SQLiteDatabase db, int version) {
            switch (version) {
                ...
                case 106:
                    ...
                    addColumn(db, DB_TABLE, Downloads.Impl.COLUMN_USERNAME, "TEXT");
                    addColumn(db, DB_TABLE, Downloads.Impl.COLUMN_PASSWORD, "TEXT");
                    // M: Add these column to support Authenticate and OMA download for FOTA
                    if (Downloads.Impl.MTK_OMA_DOWNLOAD_SUPPORT) {
                        addColumn(db, DB_TABLE, Downloads.Impl.COLUMN_OMA_DOWNLOAD_DD_FILE_INFO_NAME, "TEXT");
                        addColumn(db, DB_TABLE, Downloads.Impl.COLUMN_OMA_DOWNLOAD_DD_FILE_INFO_VENDOR, "TEXT");
                        addColumn(db, DB_TABLE, Downloads.Impl.COLUMN_OMA_DOWNLOAD_DD_FILE_INFO_SIZE,
                                    "INTEGER NOT NULL DEFAULT 0");
                        addColumn(db, DB_TABLE, Downloads.Impl.COLUMN_OMA_DOWNLOAD_DD_FILE_INFO_TYPE, "TEXT");
                        addColumn(db, DB_TABLE, Downloads.Impl.COLUMN_OMA_DOWNLOAD_DD_FILE_INFO_DESCRIPTION, "TEXT");
                        addColumn(db, DB_TABLE, Downloads.Impl.COLUMN_OMA_DOWNLOAD_FLAG,
                                    "INTEGER NOT NULL DEFAULT 0");
                        addColumn(db, DB_TABLE, Downloads.Impl.COLUMN_OMA_DOWNLOAD_STATUS,
                                    "INTEGER NOT NULL DEFAULT 0");
                        addColumn(db, DB_TABLE, Downloads.Impl.COLUMN_OMA_DOWNLOAD_OBJECT_URL, "TEXT");
                        addColumn(db, DB_TABLE, Downloads.Impl.COLUMN_OMA_DOWNLOAD_NEXT_URL, "TEXT");
                        addColumn(db, DB_TABLE, Downloads.Impl.COLUMN_OMA_DOWNLOAD_INSTALL_NOTIFY_URL, "TEXT");
                    }
                    
                    /// M: Operator Feature add columns to DB. @{
                    sDownloadProviderFeatureEx = Extensions.getDefault(mContext);
                    sDownloadProviderFeatureEx.addCustomerDBColumn(db, DB_TABLE,
                            Downloads.Impl.CONTINUE_DOWNLOAD_WITH_SAME_FILENAME,
                            "BOOLEAN NOT NULL DEFAULT 0");

                    sDownloadProviderFeatureEx.addCustomerDBColumn(db, DB_TABLE,
                            Downloads.Impl.DOWNLOAD_PATH_SELECTED_FROM_FILEMANAGER, "TEXT");
                    /// @}
                    
                    /// M: Add to support MTK DRM
                    if (Constants.MTK_DRM_ENABLED) {
                        addColumn(db, DB_TABLE, Downloads.Impl.DRM_RIGHT_VALID,
                                "BOOLEAN NOT NULL DEFAULT 0");
                    }
                    break;
    ...
    public Uri insert(final Uri uri, final ContentValues values) {
        ...
        /// M: Add these three column to support OMA DL @{
        if (Downloads.Impl.MTK_OMA_DOWNLOAD_SUPPORT) {
            copyInteger(Downloads.Impl.COLUMN_OMA_DOWNLOAD_FLAG, values, filteredValues);
            String omaDLNextURl = values.getAsString(Downloads.Impl.COLUMN_OMA_DOWNLOAD_NEXT_URL);
            if (omaDLNextURl != null) {
                copyString(Downloads.Impl.COLUMN_OMA_DOWNLOAD_NEXT_URL, values, filteredValues);            
            }
            String omaDLInstallNotifyUrl = values.getAsString(Downloads.Impl.COLUMN_OMA_DOWNLOAD_INSTALL_NOTIFY_URL);
            if (omaDLInstallNotifyUrl != null) {
                copyString(Downloads.Impl.COLUMN_OMA_DOWNLOAD_INSTALL_NOTIFY_URL, values, filteredValues);
            }
        }
        /// @}
        
        /// M: Operator Feature copy DOWNLOAD_PATH_SELECTED_FROM_FILEMANAGER values from db. @{
        sDownloadProviderFeatureEx = Extensions.getDefault(getContext());
        sDownloadProviderFeatureEx.copyContentValues(Downloads.Impl.DOWNLOAD_PATH_SELECTED_FROM_FILEMANAGER,
...
/// M: Cursor leak fix, ALPS00445161 @{
    @Override
