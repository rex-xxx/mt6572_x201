...
public class DownloadInfo {
    ...
    public static class Reader {
        ...
        public void updateFromDatabase(DownloadInfo info) {
            ...
            Log.v(Constants.TAG,"DownloadInfo.updateFromDatabase(), id: " + info.mId + ",info.mTotalBytes: " 
                    + info.mTotalBytes );
            ...
            sDownloadProviderFeatureEx = Extensions.getDefault(info.mContext);
            if (sDownloadProviderFeatureEx.shouldSetContinueDownload()) {
                info.mContinueDownload = getInt(Downloads.Impl.CONTINUE_DOWNLOAD_WITH_SAME_FILENAME) == 1;
            }
            if (sDownloadProviderFeatureEx.shouldSetDownloadPath()) {
                info.mDownloadPath = getString(Downloads.Impl.DOWNLOAD_PATH_SELECTED_FROM_FILEMANAGER);
            }
            /// @}
    ...
    void startIfReady(long now, StorageManager storageManager) {
        ...
        //if (Constants.LOGV) {
            Log.v(Constants.TAG, "Service spawning thread to handle download " + mId 
                    +  "mTotalBytes : " + mTotalBytes + ", before startIfReady,download status: " + mStatus);
        //}
     ...
     */
    void notifyFileAlreadyExist(String fullFileName) {
        Intent intent = new Intent(Intent.ACTION_VIEW);
        intent.setData(getAllDownloadsUri());
        intent.setClassName(SizeLimitActivity.class.getPackage().getName(),
                SizeLimitActivity.class.getName());
