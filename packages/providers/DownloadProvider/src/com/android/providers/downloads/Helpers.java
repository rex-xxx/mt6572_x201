...
public class Helpers {
    ...
    static String generateSaveFile(
            ...
            int destination,
            long contentLength,
            boolean isPublicApi, StorageManager storageManager,
            boolean continueDownload,
            String notificationPackage,
            String downloadPath) throws StopRequestException {
        sContext = context;
        ...
        Xlog.i(Constants.DL_ENHANCE, "Helpers, " +
                "base is: " + base + " ,path: " + path);
    ...
    static String getFullPath(String filename, String mimeType, int destination, File base)
        ...
        //if (Constants.LOGVV) {
        Log.v(Constants.TAG, "target file: " + filename + " extension: " + extension);
        //}
        ...
        if (Constants.MTK_DRM_ENABLED) {
            if (isMtkDRMFLOrCDFile(mimeType)) {
                ...
                if (lastDotIndex != -1) {
                    extension = extension.substring(0, extension.lastIndexOf('.')) + ".dcf";
                } else {
                    extension = extension + ".dcf";
                }
                Xlog.v(Constants.TAG, "extension is: " + extension);
            }
        }
        /// @}
        
        /// M: Change the extension of *.bin if mimeType is not null
        if (mimeType != null && extension.equalsIgnoreCase(Constants.DEFAULT_DL_BINARY_EXTENSION)) {
            extension = chooseExtensionFromMimeType(mimeType, true);
        }
        /// @}
    ...
    private static void checkCanHandleDownload(Context context, String mimeType, int destination,
        ...
        if (destination == Downloads.Impl.DESTINATION_EXTERNAL
            ...
            if (!DownloadDrmHelper.isDrmMimeType(context, mimeType)) {
                 Comment these for download anyobject and for OMA DL
    ...
    private static String chooseUniqueFilename(int destination, String filename,
        ...
        sDownloadProviderFeatureEx = Extensions.getDefault(sContext);
        boolean processWhenFileExist = sDownloadProviderFeatureEx.shouldProcessWhenFileExist();
        if (!sIsDDFile && processWhenFileExist
            ...
            if (!sContinueDownload) {
                ...
                Xlog.d(Constants.DL_ENHANCE, "Helper.generateSaveFile: chooseUniqueFilename, " +
                        "File(fullFilename).exists(): " + fullFilename +
                        " continue download");
                return fullFilename;
            }
            /// @}
             ...
             * avoid collisions. It starts at 1. The next 9 iterations increment
             * it by 1 at a time (up to 10). The next 9 iterations increment it
             * by 1 to 10 (random) at a time. The next 9 iterations increment it
             * by 1 to 100 (random) at a time. ... Up to the point where it
             * increases by 100000000 at a time. (the maximum value that can be
             * reached is 1000000000) As soon as a number is reached that
             * generates a filename that doesn't exist, that filename is used.
             * If the filename coming in is [base].[ext], the generated
             * filenames are [base]-[sequence].[ext].
             */
            int sequence = 1;
    ...
    static boolean isFilenameValid(String filename, File downloadsDataDir) {
        ...
        return filename.startsWith(Environment.getDownloadCacheDirectory().toString())
                ...
                || filename.startsWith("/storage/sdcard1");
    }
