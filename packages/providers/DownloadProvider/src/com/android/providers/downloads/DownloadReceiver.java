...
public class DownloadReceiver extends BroadcastReceiver {
    ...
    public void onReceive(final Context context, final Intent intent) {
        ...
        /// M: Add to fix cr alps00408332/alps00641275. @{
        } else if (action.equals(Constants.ACTION_PS_STATE_RESUMED_BROADCAST) || 
                       action.equals(Constants.ACTION_EMMRRS_PS_RESUME_INDICATOR)) {
            Log.v(Constants.TAG, "Received broadcast intent : " +
                     action + "reset connection for uid : " + Binder.getCallingUid());
            new Thread() {
                public void run() {
                    NetworkUtils.resetConnectionByUid(Binder.getCallingUid());
                }
            }.run();
            startService(context);
        }
