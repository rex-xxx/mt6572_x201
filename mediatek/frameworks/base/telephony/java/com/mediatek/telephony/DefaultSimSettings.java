...
public class DefaultSimSettings {
    ...
    public static void setAllDefaultSim(Context context, ContentResolver contentResolver, List<SimInfoRecord> simInfos, 
            long[] simIdForSlot, boolean[] isSimInserted, int nNewCardCount, int nSimCount, boolean is3GSwitched, int[] mInsertSimState,
        ...
        if (!FeatureOption.MTK_BSP_PACKAGE) {
            ...
            if(isSimRemoved(oldSmsDefaultSIM,simIdForSlot,PhoneConstants.GEMINI_SIM_NUM)){
                ...
                if ("OP01".equals(SystemProperties.get("ro.operator.optr"))) {
                    if (nSimCount > 1) {
                        ...
                        defaultSmsSim = simInfos.get(0).mSimInfoId;
                    }
                }
            ...
            if (gprsDefaultSIM != Settings.System.DEFAULT_SIM_NOT_SET && gprsDefaultSIM != Settings.System.GPRS_CONNECTION_SIM_SETTING_NEVER) {
                int slot = SimInfoManager.getSlotById(context, gprsDefaultSIM);            
...
*/                        
    ...
    private static void logd(String message) {
        Log.d(LOG_TAG, "[DefaultSimSetting] " + message);
    }
}
