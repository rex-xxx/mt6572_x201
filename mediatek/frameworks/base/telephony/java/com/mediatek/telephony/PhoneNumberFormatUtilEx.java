    ...
    /*package*/static String getDefaultSimCountryIso(){
       ...
       if (FeatureOption.MTK_GEMINI_SUPPORT == true) {
            simId = SystemProperties.getInt(PhoneConstants.GEMINI_DEFAULT_SIM_PROP, -1);
             ...
             if(!TelephonyManager.getDefault().hasIccCardGemini(simId)){
                 //simId = Phone.GEMINI_SIM_2 ^ simId;
                 if(TelephonyManager.getDefault().hasIccCardGemini(PhoneConstants.GEMINI_SIM_1)){
                     simId = PhoneConstants.GEMINI_SIM_1;
                 }else if (TelephonyManager.getDefault().hasIccCardGemini(PhoneConstants.GEMINI_SIM_2)){
                     simId = PhoneConstants.GEMINI_SIM_2;
                 }else if (PhoneConstants.GEMINI_SIM_NUM >=3 && TelephonyManager.getDefault().hasIccCardGemini(PhoneConstants.GEMINI_SIM_3)){
                     simId = PhoneConstants.GEMINI_SIM_3;
                 }else if (PhoneConstants.GEMINI_SIM_NUM >=4 && TelephonyManager.getDefault().hasIccCardGemini(PhoneConstants.GEMINI_SIM_4)){
...
//    }
    public static void formatNumber(Editable text, int defaultFormattingType){
    	...
    	if(result != null && !result.equals(text.toString())){
	        ...
	        //Log.d(TAG,"OldIndex: "+oldIndex+", digitCount: "+digitCount + ", newCursor: "+i);
    	}
