...
public final class SimInfoManager implements BaseColumns {
    ...
    /**
     * @param ctx
     * @return the array list of currently inserted SimInfoRecords
     */
     ...
     * @param ctx
     * @param simInfoId the unique SimInfoRecord index in database
     * @return the slot which the SIM card currently inserted, -1 indicate SIM card missing
     */
    public static int getSlotById(Context ctx, long simInfoId) {
        logd("[getSlotById]+ simInfoId:" + simInfoId);
        if (simInfoId <= 0 ) {
            logd("[getSlotById]- simInfoId <= 0");
            return SLOT_NONE;
        }
        Cursor cursor = ctx.getContentResolver().query(ContentUris.withAppendedId(CONTENT_URI, simInfoId), 
                new String[]{SLOT}, null, null, null);
        try {
            if (cursor != null) {
                if (cursor.moveToFirst()) {
                    int slot = cursor.getInt(0);
                    logd("[getSlotById]- sim in slot:" + slot);
                    return slot;
                }
            }
        } finally {
    ...
    public static int getSlotByName(Context ctx, String simName) {
        logd("[getSlotByName]+ name:" + simName);
        if (simName == null) {
            logd("[getSlotByName]- simName is null, return -1");
            return SLOT_NONE;
        }
        Cursor cursor = ctx.getContentResolver().query(CONTENT_URI, 
                new String[]{SLOT}, DISPLAY_NAME + "=?", new String[]{simName}, null);
        try {
            if (cursor != null) {
                if (cursor.moveToFirst()) {
                    int slot = cursor.getInt(0);
                    logd("[getSlotByName]- sim in slot:" + slot);
                    return slot;
                }
            }
        } finally {
     ...
     * @param ctx
     * @param iccId the IccId of SIM card
     * @return the operator of this SIM, maybe ""
     */
    public static String getOperatorByIccId(Context ctx, String iccId) {
        logd("[getOperatorByIccId]+ iccId:" + iccId);
        SimInfoRecord simInfo = getSimInfoByIccId(ctx, iccId);
        if(simInfo != null) {
            logd("[getOperatorByIccId]- operator:" + simInfo.mOperator);
     ...
     * @return the operator of this SIM, maybe ""
     */
    public static String getOperatorBySlot(Context ctx, int simSlotId) {
        logd("[getOperatorBySlot]+ simSlotId:" + simSlotId);
        if (simSlotId < 0) {
        ...
        SimInfoRecord simInfo = getSimInfoBySlot(ctx, simSlotId);
        if(simInfo != null) {
            logd("[getOperatorBySlot]- operator:" + simInfo.mOperator);
            return simInfo.mOperator;
        } else {
     ...
     */
    @Deprecated
    public static int setWapPush(Context ctx, int enable, long simInfoId) {
        logd("[setWapPush] Sould not use WapPush, return -1");
        return -1;
        /*if (enable > 1 || enable < -1 || simInfoId <= 0) {
            return -1;
        }
        ContentValues value = new ContentValues(1);
        value.put(WAP_PUSH, enable);
        return ctx.getContentResolver().update(ContentUris.withAppendedId(CONTENT_URI, simInfoId),
                value, null, null);
        */
    }
    /**
     * Insert the IccId and slot if needed
     * @param ctx
     * @param iccId the IccId of the SIM card
     * @param simSlotId the slot which the SIM is inserted
     * @return
     */
    public static Uri insertIccId(Context ctx, String iccId, int simSlotId) {
        logd("[insertIccId]+ iccId:" + iccId + " simSlotId:" + simSlotId);
        if (iccId == null) {
            logd("[insertIccId]- null iccId");
        ...
        try {
            if (cursor == null || !cursor.moveToFirst()) {
                ...
                logd("[insertIccId]- new sim inserted in slot:" + simSlotId);
            } else {
                ...
                logd("[insertIccId]- existed sim in slot:" + simSlotId);
            }
        } finally {
            if (cursor != null) {
                cursor.close();
    ...
    public static int setDefaultName(Context ctx, long simInfoId, String name) {
        ...
        value.put(DISPLAY_NAME, displayName);
        logd("[setDefaultName]- name:" + displayName + " set");
    ...
    public static int setDefaultNameEx(Context ctx, long simInfoId, String name, long nameSource) {
        ...
        logd("[setDefaultNameEx]- name:" + displayName + " set, source:" + nameSource);
        return ctx.getContentResolver().update(uri, value, null, null);
    }
