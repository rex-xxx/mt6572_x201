...
public final class SimInfoManagerAdp {
    ...
    public static SIMInfo copyFromSimInfoRecord(SimInfoRecord simInfoRecord) {
        if (simInfoRecord == null) {
            ...
            if (info.mColor >= 0 && info.mColor < size) {
                info.mSimBackgroundRes = SimInfoManager.SimBackgroundRes[info.mColor];
                // add by mtk02772 for Consistent UI Design start
                info.mSimBackgroundDarkRes = SimInfoManager.SimBackgroundDarkRes[info.mColor];
                info.mSimBackgroundLightRes = SimInfoManager.SimBackgroundLightRes[info.mColor];
    ...
    public static int getSlotByIdAdp(Context ctx, long simId) {
        logd("[getSlotByIdAdp]");
        return SimInfoManager.getSlotById(ctx, simId);
    }

    public static long getIdBySlotAdp(Context ctx, int slot) {
        logd("[getIdBySlotAdp]");
        return SimInfoManager.getIdBySlot(ctx, slot);
    }
    
    public static int getSlotByNameAdp(Context ctx, String simName) {
        logd("[getSlotByNameAdp]");
        return SimInfoManager.getSlotByName(ctx, simName);
    }
    ...
    public static String getOperatorByIccIdAdp(Context ctx, String iccId) {
        logd("[getOperatorByIccIdAdp]");
        return SimInfoManager.getOperatorByIccId(ctx, iccId);
    }

    public static String getOperatorBySlotAdp(Context ctx, int slotId) {
        logd("[getOperatorBySlotAdp]");
        return SimInfoManager.getOperatorBySlot(ctx, slotId);
    }
    ...
    public static Uri insertIccIdAdp(Context ctx, String iccId, int slot) {
        logd("[insertIccIdAdp]");
        return SimInfoManager.insertIccId(ctx, iccId, slot);
    }
