...
public class WindowTest extends ActivityInstrumentationTestCase2<WindowStubActivity> {
 ...
 @TestTargetNew(
    ...
    public void testDisableHomeKey() throws Exception {
        sendKeys(KeyEvent.KEYCODE_HOME);
        assertTrue(mActivity.isOnHomeKeyDownCalled());
        assertTrue(mActivity.isOnHomeKeyUpCalled());
    }

    /**
     * Use enableGuiLog at WMS to enable the Gui log
     * If the values of system properties can't match,
     * the test case will fail. However, WMS log doesn't
     * use the system property. Therefore, it skips WMS check
     */
    public void testEnableGuiLog() throws Exception {
        try {
            IWindowManager.Stub.asInterface(
                    ServiceManager.getService("window")).enableGuiLog(true);
        } catch (RemoteException e) {
            assert(false);
        }
        assertTrue(SystemProperties.getInt("debug.sf.log_repaint", 0) == 1);
        assertTrue(SystemProperties.getInt("debug.sf.log_buffer", 0) == 1);
        assertTrue(SystemProperties.getInt("debug.viewroot.enable", 0) == 1);
        assertTrue(SystemProperties.getInt("debug.hwui.level", 0) == 7);
        assertTrue(SystemProperties.getInt("debug.skia.dump_bitmap", 0) == 1);
        assertTrue(SystemProperties.getInt("debug.skia.dump_pid", 0) == 1);
    }

    /**
     * Use enableGuiLog at WMS to disable the Gui log
     * If the values of system properties can't match,
     * the test case will fail. However, WMS log doesn't
     * use the system property. Therefore, it skips WMS check
     */
    public void testDisableGuiLog() throws Exception {
        try {
            IWindowManager.Stub.asInterface(
                    ServiceManager.getService("window")).enableGuiLog(false);
        } catch (RemoteException e) {
            assert(false);
        }
        assertTrue(SystemProperties.getInt("debug.sf.log_repaint", 1) == 0);
        assertTrue(SystemProperties.getInt("debug.sf.log_buffer", 1) == 0);
        assertTrue(SystemProperties.getInt("debug.viewroot.enable", 1) == 0);
        assertTrue(SystemProperties.getInt("debug.hwui.level", 1) == 0);
        assertTrue(SystemProperties.getInt("debug.skia.dump_bitmap", 0) == 0);
        assertTrue(SystemProperties.getInt("debug.skia.dump_pid", 0) == 0);
    }
