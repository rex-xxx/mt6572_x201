...
public class PackageManagerCRTest extends InstrumentationTestCase {
    ...
    public void testALPS00421371() {
        try {
            boolean ret = AppGlobals.getPackageManager().enforceDexOpt("com.android.launcher");
            /*
            if (!ret) {
    ...
    public void testALPS00418300() {
        try {
            ...
            //System.out.println(ApplicationInfo.FLAG_UNINSTALL_APP);
            //System.out.println(info.flags);
            assertEquals((int) (1<<30), ApplicationInfo.FLAG_UNINSTALL_APP);
            assertTrue((info.flags & ApplicationInfo.FLAG_UNINSTALL_APP) != 0);
        } catch (RemoteException re) {
            fail("Unexpected.");
        }
    }
