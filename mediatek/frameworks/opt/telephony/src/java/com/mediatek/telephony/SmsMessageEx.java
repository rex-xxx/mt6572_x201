...
public class SmsMessageEx implements ISmsMessageExt{
    ...
    public byte[] getTpdu(SmsMessage msg, int slotId) {
        if(FeatureOption.EVDO_DT_VIA_SUPPORT == true) {
            ...
            if (format.equals(SmsConstants.FORMAT_3GPP2)) {
                Log.d(TAG, "call getTpdu, format="+format+", slotId="+slotId);
                return null;
            }
        }

        Log.d(TAG, "call getTpdu, slotId="+slotId);
        byte[] pdu = msg.getPdu();
        if(pdu == null) {
            Log.d(TAG, "pdu is null");
            return null;
        }
    ...
    public byte[] getSmsc(SmsMessage msg, int slotId) {
        if(FeatureOption.EVDO_DT_VIA_SUPPORT == true) {
            String format = getSmsFormat(slotId);
            if (format.equals(SmsConstants.FORMAT_3GPP2)) {
                Log.d(TAG, "call getSmsc, format="+format+", slotId="+slotId);
                return null;
            }
        }

        Log.d(TAG, "call getSmsc, slotId="+slotId);
        byte[] pdu = msg.getPdu();
        if(pdu == null) {
            Log.d(TAG, "pdu is null");
            return null;
