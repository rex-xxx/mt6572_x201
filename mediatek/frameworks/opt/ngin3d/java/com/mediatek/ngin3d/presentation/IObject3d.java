...
public interface IObject3d extends Presentation {
     ...
     */
    void setAnimationPlaying(boolean flag);
    boolean isAnimationPlaying();
    void setAnimationProgress(float progress);
    float getAnimationProgress();
    void setAnimationLoopEnabled(boolean flag);
    boolean getAnimationLoopEnabled();
    void setAnimationSpeed(float speed);
    float getAnimationLength();

    /*
     * Required temporarily to allow coexistence of two animation systems.
     */
    void disableNewAnimation();

    /*
     * Tells the presentation to update the Glo state when applying animation
     * progress.  Progress application is disabled initally to avoid animations
     * from conficting with each other during actor realization.
     */
