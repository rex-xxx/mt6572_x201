...
public class ActorPresentationTest extends Ngin3dInstrumentationTestCase {
    public void testActorPresentation() throws ExecutionException, InterruptedException {
        ...
        assertEquals(Actor.PROP_ANCHOR_POINT.defaultValue().x,
            ((Point) actor.getPresentationValue(Actor.PROP_ANCHOR_POINT)).x, 0.f);
        assertEquals(Actor.PROP_ANCHOR_POINT.defaultValue().y,
            ((Point) actor.getPresentationValue(Actor.PROP_ANCHOR_POINT)).y, 0.f);
        assertEquals(Actor.PROP_ANCHOR_POINT.defaultValue().z,
            ((Point) actor.getPresentationValue(Actor.PROP_ANCHOR_POINT)).z, 0.f);
        actor.unrealize();
    }
}
