...
#define RTT_FOLDER_PATH         "/data/anr/"
#define RTT_FILE_PREFIX         "rtt_dump_"
...
status_t SFWatchDog::readyToRun()
{
    ...
    sprintf(cmds, "mkdir %s/sf_rtt", RTT_FOLDER_PATH);
    system(cmds);

    sprintf(cmds, "chmod 660 %s/sf_rtt", RTT_FOLDER_PATH);
    system(cmds);
    ...
    if (isSFThreadHang(ct))
    {
        ...
        for (uint32_t i = 0; i < mNodeList.size(); i++)
        {
            if (mNodeList[i]->mStartTransactionTime == 0)
                ...
                XLOGW("    [%s] stop = %lld ms", mNodeList[i]->mName.string(),
                    ns2ms(ct - mNodeList[i]->mStartTransactionTime));
        }
        ...
        {
            XLOGD("[SF-WD] remove rtt dump file");
            
            rtt_ct = SW_WATCHDOG_RTTCOUNT;

            char cmds[256];
            sprintf(cmds, "rm -r %ssf_rtt/*", RTT_FOLDER_PATH);
            system(cmds);
            
            sprintf(cmds, "mv %s%s* %s/sf_rtt", RTT_FOLDER_PATH, RTT_FILE_PREFIX, RTT_FOLDER_PATH);
            system(cmds);
            
            sprintf(cmds, "rm -r %s%s*", RTT_FOLDER_PATH, RTT_FILE_PREFIX);
            system(cmds);
        }

        char cmds[256];
        sprintf(cmds, "rtt -f bt -p %d > %s%s%lld.txt", getpid(), RTT_FOLDER_PATH, RTT_FILE_PREFIX, ct);
        system(cmds);

        XLOGD("[SF-WD] dump rtt file(%d): %s%s%lld.txt", SW_WATCHDOG_RTTCOUNT - rtt_ct, RTT_FOLDER_PATH, RTT_FILE_PREFIX, ct);
...
bool SFWatchDog::isSFThreadHang(nsecs_t& ct)
{
    ...
    for (uint32_t i = 0; i < mNodeList.size(); i++)
    {
        if (mNodeList[i]->mStartTransactionTime != 0 && mThreshold * 1000 * 1000 < now - mNodeList[i]->mStartTransactionTime)
        {
            ct = now;
            return true;
        }
