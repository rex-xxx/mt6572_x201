        ...
        mContext.registerReceiver(new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                if ("android.intent.action.BOOST_DOWNLOADING".equals(intent.getAction())) {
                    ...
                    int size = mService.mMainStack.mHistory.size();
                    ArrayList list = new ArrayList();
                    for ( int i = 0 ; i < size ; i++) {
                        ActivityRecord r = (ActivityRecord)mService.mMainStack.mHistory.get(i);
                        if ( r.intent.hasCategory(Intent.CATEGORY_HOME) && r.app != null ) {
                            // skip it
                            // r.app = null means the host process is killed but its ActivityRecord is still in stack,
                            // we also need to remove this kind of activity
                        } else {
                            Slog.i(TAG, "added activity = " + r.realActivity.getClassName());
                            list.add(r);
                        }
                    }
                    ...
                    synchronized (mService) {
                        for (int i = size-1; i >= 0; i--) {
                            ...
                            int idx = mService.mMainStack.indexOfTokenLocked(r.appToken); // since some activity's hosting process may be died, therefore, we need to use the "exact" index
                            Slog.i(TAG, "mHistory.index = " + idx + " list.index = " + i);
                            mService.mMainStack.finishActivityLocked(r, idx, Activity.RESULT_CANCELED,null, "clear", false /*TBD*/);
                        }
                    }
                    list.clear();
                    mService.mRecentTasks.clear();
                    Slog.i(TAG, "finished");
            ...
            public void run() {
                ...
                synchronized(mService) {                                           
                    ...
                    for (int i=0; i<mService.mStartedUsers.size(); i++) {
                        ...
                        mService.broadcastSpecificIntentLocked(null, null,intent,
