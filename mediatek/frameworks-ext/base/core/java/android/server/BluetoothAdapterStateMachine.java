...
final class BluetoothAdapterStateMachine extends StateMachine {
    ...
    //BT panic event
    private static final int PANIC_RESET = 107;
    ...
    ///M: flag to handle USER_TURN_ON in Switching state @{
    private boolean mIsUserTurnOnWhenSwitching = false;
    private Object mIsUserTurnOnObject = null;
    /// @}

    /// M: Slim @{
    private boolean mIsBootComplete = false;
    private boolean mIsNeedToEnableAfterBootComplete = false;  // Queue the USER_TURN_ON message until BOOT_COMPLETE is received
    /// @}
    ...
    BluetoothAdapterStateMachine(Context context, BluetoothService bluetoothService,
        ...
        bootIntentFilter.addAction(Intent.ACTION_SHUTDOWN);
        mContext.registerReceiver(mBootCompleteReceiver, bootIntentFilter);
        /// @}
    }
    ...
    private class PowerOff extends State {
        ...
        public boolean processMessage(Message message) {
            log("PowerOff process message: " + message.what + ", message.obj = " + message.obj);

            boolean retValue = HANDLED;
            switch(message.what) {
                case PANIC_RESET:
                    removeMessages(PANIC_RESET);
                case USER_TURN_ON:
                    /// M: Slim: Make sure the "enable" process is continued after BOOT_COMPLETE is received @{
                    if (false == mIsBootComplete) {
                        log("mIsBootComplete = false, pending turn on");
                        mIsNeedToEnableAfterBootComplete = true;
                        break;
                    }
                    log("mIsBootComplete = true, turn on");
                    /// @}
                    ...
                    if (prepareBluetooth()) {
                        ...
                        /// M: ALPS00574277
                        mBluetoothService.persistBluetoothState(true);
        ...
        private boolean prepareBluetooth() {
            ...
            //sendMessageDelayed(SERVICE_RECORD_LOADED, 1000);
            sendMessage(SERVICE_RECORD_LOADED);
            return true;
        }
    }
    ...
    private class WarmUp extends State {
        ...
        @Override
        public boolean processMessage(Message message) {
            log("WarmUp process message: " + message.what + ", message.obj = " + message.obj);
    ...
    private class HotOff extends State {
        ...
        @Override
        public boolean processMessage(Message message) {
            log("HotOff process message: " + message.what + ", message.obj = " + message.obj);
            ...
            switch(message.what) {
                case USER_TURN_ON:
                    ...
                    if(false == mIsBootComplete) {
                        log("mIsBootComplete = false, pending turn on");
                        mIsNeedToEnableAfterBootComplete = true;
                        break;  // Do not fall through TURN_ON_CONTINUE if BOOT_COMPLETE is Not received
                    }
                ...
                case AIRPLANE_MODE_ON:
                    /// M: ALPS00574277 @{
                    if ((Boolean) message.obj) {
                        mBluetoothService.persistBluetoothState(false);
                    }
                    /// @}
                    //$FALL-THROUGH$
                case TURN_COLD:
    ...
    private class Switching extends State {
        ...
        @Override
        public boolean processMessage(Message message) {
            log("Switching process message: " + message.what + ", message.obj = " + message.obj);
            ...
            switch(message.what) {
                ...
                case POWER_STATE_CHANGED:
                    ...
                    if (!((Boolean) message.obj)) {
                        if (mPublicState == BluetoothAdapter.STATE_TURNING_OFF) {
                            ...
                            ///M: defer USER_TURN_ON after TURN_COLD @{
                            if (mIsUserTurnOnWhenSwitching == true) {
                                Log.d(TAG, "defer USER_TURN_ON message");
                                deferMessage(obtainMessage(USER_TURN_ON,mIsUserTurnOnObject));
                                mIsUserTurnOnWhenSwitching = false;
                                mIsUserTurnOnObject = null;
                            }
                            /// @}
                ...
                case TURN_OFF_TIMEOUT:
                    ...
                    break;
                case USER_TURN_ON:
                    ///M: do not defer USER_TURN_ON here when STATE_TURNING_OFF @{
                    if (mPublicState == BluetoothAdapter.STATE_TURNING_OFF) {
                        Log.d(TAG, "USER_TURN_ON when Switching...");
                        mIsUserTurnOnWhenSwitching = true;
                        mIsUserTurnOnObject = message.obj;
                        break;
                    }
                    /// @}
                case AIRPLANE_MODE_OFF:
    ...
    private class BluetoothOn extends State {
        ...
        @Override
        public boolean processMessage(Message message) {
            log("BluetoothOn process message: " + message.what + ", message.obj = " + message.obj);
            ...
            switch(message.what) {
                ...
                case AIRPLANE_MODE_ON:
                    ...
                    if (message.what == AIRPLANE_MODE_ON || mBluetoothService.isAirplaneModeOn()) {
                        ...
                        if (message.what != AIRPLANE_MODE_ON) {
                            boolean bSaveState = false;
                            log("Not AIRPLANE_MODE_ON, don't save state");
                            Message msg = obtainMessage(AIRPLANE_MODE_ON, bSaveState);
                            log("New Airplane mode on message: msg.obj = " + msg.obj);
                            deferMessage(msg);
                        }
                        else {
                            log("AIRPLANE_MODE_ON, defer message: message.obj = " + message.obj);
                            deferMessage(message);
                        }
                        /// @}
                    }
                ...
                case POWER_STATE_CHANGED:
                    if ((Boolean) message.obj) {
                        ...
                        //deferMessage(obtainMessage(PANIC_RESET, false));
                        sendMessage(obtainMessage(PANIC_RESET, false));	
                    }
                    break;
                default:
                    return NOT_HANDLED;
    ...
    private class PerProcessState extends State {
        ...
        @Override
        public boolean processMessage(Message message) {
            log("PerProcessState process message: " + message.what + ", message.obj = " + message.obj);
    ...
    private boolean getBluetoothPersistedSetting() {
        ...
        int bluetoothPersistedSetting = Settings.Global.getInt(contentResolver,
                Settings.Global.BLUETOOTH_ON, 0);
        log("getBluetoothPersistedSetting: bluetoothPersistedSetting = " + bluetoothPersistedSetting);
        return (bluetoothPersistedSetting > 0);
    }
    ...
    private BroadcastReceiver mBootCompleteReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(Intent.ACTION_BOOT_COMPLETED)) {
                log("Boot Complete onReceive");
                mIsBootComplete = true;
                
                if (mIsNeedToEnableAfterBootComplete) {
                    log("mIsNeedToEnableAfterBootComplete = true, turn on");
                    sendMessage(USER_TURN_ON, true);
                    mIsNeedToEnableAfterBootComplete = false;
                }
            }
            else if (action.equals(Intent.ACTION_SHUTDOWN)) {
                log("Shutdown onReceive");
