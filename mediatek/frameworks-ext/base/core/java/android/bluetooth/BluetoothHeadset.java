 ...
 */
public final class BluetoothHeadset implements BluetoothProfile/*, BluetoothProfileManager.BluetoothProfileBehavior*/{
    ...
    public static boolean isBluetoothVoiceDialingEnabled(Context context) {
        ...
        List<ResolveInfo> filter = null;
        Intent intent = new Intent(Intent.ACTION_VOICE_COMMAND);

        if(pm != null){
            filter = pm.queryIntentActivities(intent, PackageManager.MATCH_DEFAULT_ONLY);
            if (DBG) Log.e(TAG, "Can not get PackageManager");
        }
        if(filter == null){
            if (DBG) Log.w(TAG, "No activity handle ACTION_VOICE_COMMAND");
        }
        return (filter != null
                && !filter.isEmpty()
                && context.getResources().getBoolean(com.android.internal.R.bool.config_bluetooth_sco_off_call));
    }
    ...
    private ServiceConnection mConnection = new ServiceConnection() {
        public void onServiceConnected(ComponentName className, IBinder service) {
            ...
            if (DBG) Log.d(TAG, "Proxy object connected, BluetoothHeadset.this: " + BluetoothHeadset.this + ", mService: " + mService + ", mServiceListener: " + mServiceListener);
        ...
        public void onServiceDisconnected(ComponentName className) {
            ...
            if (DBG) Log.d(TAG, "Proxy object disconnected, BluetoothHeadset.this: " + BluetoothHeadset.this + ", mService: " + mService + ", mServiceListener: " + mServiceListener);
    ...
    //    if (DBG) log("getConnectedDevices()");
    //    List<BluetoothDevice> connList = getConnectedDevices();
    //    Set connSet = new HashSet(connList);
    //    return connSet;
        /*
        HashSet<BluetoothDevice> connSet = new HashSet<BluetoothDevice>();
        BluetoothDevice remDev = getCurrentHeadset();
        if (remDev != null)
        {
            connSet.add(remDev);
        }
    ...
    /* There are same APIs defined already */
    /*
    public boolean connect(BluetoothDevice device){
        if (DBG) log("connect("+device+")");
