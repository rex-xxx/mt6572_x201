...
public final class BluetoothDeviceProfileState extends StateMachine {
    ...
    public static final int CONNECT_OTHER_PROFILES_DELAY = 6000; // 6 secs
    ...
    public static final Integer OBJ_CONNECT_A2DP_OUTGOING = new Integer(CONNECT_A2DP_OUTGOING);
    public static final Integer OBJ_CONNECT_HFP_OUTGOING = new Integer(CONNECT_HFP_OUTGOING);
    ...
    private BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(BluetoothA2dp.ACTION_CONNECTION_STATE_CHANGED)) {
                ...
                Log.d(TAG, "Receive action:" + action + ", old state:" + oldState + " --> new state:" + newState);
                ...
                int newState = intent.getIntExtra(BluetoothProfile.EXTRA_STATE, 0);
                int oldState = intent.getIntExtra(BluetoothProfile.EXTRA_PREVIOUS_STATE, 0);
                /** M: Add. Check if the intent is just for state sync after phone is started (or restarted) @{ */
                boolean bIsSync = intent.getBooleanExtra("sync", false);
                /** @} */
                ...
                if (oldState == BluetoothHeadset.STATE_CONNECTED &&
                    ...
                    /** M: Modify. Ignore the itnet if it is just for state sync after phone is started (or restarted) 
                                   It's not triggered by BT stack  @{ */
                    if(!bIsSync)
                    {
                ...
                */
                ...
                if (null != msg) { // can't obtain message if the state machine
                    // has quit.
                    msg.arg1 = val;
                    sendMessage(msg);
                } else {
                    Log.d(TAG, "discard CONNECTION_ACESS_REQUEST_REPLY state");
                }
    ...
    private BluetoothProfile.ServiceListener mBluetoothProfileServiceListener =
        ...
        public void onServiceConnected(int profile, BluetoothProfile proxy) {
            ...
            Log.i(TAG, "onServiceConnected, mHeadsetService: " + mHeadsetService);
        }
        public void onServiceDisconnected(int profile) {
            synchronized(BluetoothDeviceProfileState.this) {
                mHeadsetService = null;
            }
            /// M: ALPS00483847 re-connect if phone process is killed after IPO boot
            mAutoConnectionPending = true;
            
            Log.i(TAG, "onServiceDisconnected, mHeadsetService: " + mHeadsetService);
        }
    };
    ...
    private class OutgoingHandsfree extends State {
        ...
        public void enter() {
            ...
            Log.d(TAG, "OutgoingHandsfree state processCommand return " + mStatus);
            if (!mStatus) {
                sendMessage(TRANSITION_TO_STABLE);
    ...
    private class OutgoingA2dp extends State {
        ...
        public void enter() {
            ...
            mStatus = processCommand(mCommand);
            Log.d(TAG, "OutgoingA2dp state processCommand return " + mStatus);
            if (!mStatus) {
                sendMessage(TRANSITION_TO_STABLE);
  ...
  private class IncomingHid extends State {
    ...
    private boolean handleIncomingConnection(int command, boolean accept) {
        ...
        switch (command) {
            case CONNECT_HFP_INCOMING:
                // ALPS00519037 Remove connect other profile message sent before
                getHandler().removeMessages(CONNECT_OTHER_PROFILES, OBJ_CONNECT_HFP_OUTGOING);
                ...
                if (!accept) {
                    ...
                    writeTimerValue(0);
                    // ALPS00519037 If the profile state is not disconnected, don't connect the profile
                    if (mA2dpState == BluetoothA2dp.STATE_DISCONNECTED)
            ...
            case CONNECT_A2DP_INCOMING:
                // ALPS00519037 Remove connect other profile message sent before
                getHandler().removeMessages(CONNECT_OTHER_PROFILES, OBJ_CONNECT_A2DP_OUTGOING);
    ...
    synchronized boolean processCommand(int command) {
        ...
        switch(command) {
            ...
            case DISCONNECT_HFP_OUTGOING:
                ...
                break;
            case DISCONNECT_HFP_INCOMING:
                ///M: [ALPS00565009] To avoid the case when only 1 profile is connected for a BT headset 
                ///   Remove pending msg: OBJ_CONNECT_A2DP_OUTGOING if HFP is disconnected @{
                Log.d(TAG, "HFP disconnected -> Remove pending A2DP conn msg");
                getHandler().removeMessages(CONNECT_OTHER_PROFILES, OBJ_CONNECT_A2DP_OUTGOING);
                ///@ }
                return true;
            case DISCONNECT_A2DP_INCOMING:
                ///M: [ALPS00565009] To avoid the case when only 1 profile is connected for a BT headset
                ///   Remove pending msg: OBJ_CONNECT_HFP_OUTGOING if A2DP is disconnected @{
                Log.d(TAG, "A2DP disconnected -> Remove pending HFP conn msg");
                getHandler().removeMessages(CONNECT_OTHER_PROFILES, OBJ_CONNECT_HFP_OUTGOING);
                ///@ }
                return true;
            case DISCONNECT_A2DP_OUTGOING:
    ...
    private void processIncomingConnectCommand(int command) {
        ///M: Refer to ref phone, do not pop up dialog when CONNECT_HFP_INCOMING @{
        if (mService.getBondState(mDevice.getAddress()) != BluetoothDevice.BOND_BONDED && command == CONNECT_HFP_INCOMING ) {
    ...
    private void handleConnectionOfOtherProfiles(int command) {
        ...
        Log.i(TAG, "handleConnectionOfOtherProfiles: " + command);
        ...
        switch (command) {
            case CONNECT_HFP_INCOMING:
                ...
                if (mA2dpService.getPriority(mDevice) == BluetoothProfile.PRIORITY_ON ||
                    ...
                    msg.obj = OBJ_CONNECT_A2DP_OUTGOING;
            ...
            case CONNECT_A2DP_INCOMING:
                ...
                if (mHeadsetService != null &&
                    ...
                    msg.obj = OBJ_CONNECT_HFP_OUTGOING;
                    sendMessageDelayed(msg, CONNECT_OTHER_PROFILES_DELAY);
                }
                break;
            default:
