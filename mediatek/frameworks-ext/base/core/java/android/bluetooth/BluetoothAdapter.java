...
public final class BluetoothAdapter {
    ...
    /** used to indicate expected remote device type when discovering */
    /** @hide */
    ...
    public boolean startDiscovery() {
        if (getState() != STATE_ON) return false;
        try {
            return mService.startDiscovery(TYPE_BR_EDR_ONLY);
        } catch (RemoteException e) {Log.e(TAG, "", e);}
        return false;
    }

    /** @hide */
    public boolean startDiscovery(int type) {
        if (getState() != STATE_ON) return false;
        try {
            return mService.startDiscovery(type);
        } catch (RemoteException e) {Log.e(TAG, "", e);}
        return false;
    }
