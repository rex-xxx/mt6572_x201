...
static int acceptNative(JNIEnv *env, jobject obj, int timeout, jint fdHandle) 
{
	...
	if (JNI_TRUE == bSearchResult)
	{
		...
		{
			// Invalid parameter
			XLOGE("[JSR82][JNI] bindListenNative: Invalid timeout value (%d).", timeout);
			//jniThrowIOException(env, EINVAL);
			return -1;
		}
		else
		{
			...
			iWaitResult = pthread_cond_timedwait(&(g_jsr82MMIContext[iCntxIndex].jsr82ConnectCond), NULL, &nextTs);
		}
		...
		if (0 != iWaitResult)
		{
			XLOGI("[JSR82][JNI] acceptNative -- Wait result is not correct.");
			//jniThrowIOException(env, ECONNABORTED);
			return -1;
		}
