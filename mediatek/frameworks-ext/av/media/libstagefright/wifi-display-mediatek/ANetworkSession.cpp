...
///Add by MTK @{
#include "uibc/WifiDisplayUibcType.h"
///@}
namespace android {
...
///Add by MTK @{
            ...
            || mode == kModeCreateTCPUibcSessionPassive
...
///Add by MTK @{
    } else if (localAddr != NULL && (mode != kModeCreateRTSPServer && mode != kModeCreateTCPTextDataSessionPassive && mode != kModeCreateTCPUibcSessionPassive)) {
///@}
...
///@}
    ...
    switch (mode) {
        ...
        case kModeCreateTCPUibcSessionPassive:
...
///Add by MTK @{
    ...
    } else if (mode == kModeCreateTCPUibcSessionPassive) {
...
status_t ANetworkSession::createTCPUibcSession(
        const struct in_addr &addr, unsigned port,
        const sp<AMessage> &notify, int32_t *sessionID) {
    return createClientOrServer(
            kModeCreateTCPUibcSessionPassive,
            &addr,
            port,
            NULL /* remoteHost */,
            0 /* remotePort */,
