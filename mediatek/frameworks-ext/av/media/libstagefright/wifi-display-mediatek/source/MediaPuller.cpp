...
MediaPuller::MediaPuller(
    ...
    char name[128];
    sprintf(  name, "%s", mIsAudio?"audioPuller":"videoPuller"); 

    MMP_Event  MMP_WFD_DEBUG = MMProfileFindEvent(MMP_RootEvent, "WFD_Source");
    if(MMP_WFD_DEBUG !=0){
	    MMP_WFD_PULLER[mIsAudio] = MMProfileRegisterEvent(MMP_WFD_DEBUG, name);
	    MMProfileEnableEvent(MMP_WFD_PULLER[mIsAudio],1); 	
    }else{
...
#ifdef USE_MMPROFILE	
	if(MMP_WFD_PULLER[mIsAudio] !=0){
		MMProfileLogMetaString(MMP_WFD_PULLER[mIsAudio], MMProfileFlagPulse, "start done");
	}
#endif		 
                schedulePull();
            }
...
#ifdef USE_MMPROFILE	
		if(MMP_WFD_PULLER[mIsAudio]  !=0){
			MMProfileLogMetaStringEx(MMP_WFD_PULLER[mIsAudio] , MMProfileFlagPulse, (uint32_t)mbuf, (uint32_t)(timeUs / 1000),"read a buffer done");
		}
#endif	
		  ...
		  ALOGD("%s,timestamp=%lld ms",mIsAudio?"audio":"video",timeUs/1000);
                if (mIsAudio) {
                    mbuf->release();
                    mbuf = NULL;
                } else {
		      ...
		      //ALOGI("[video buffer]setPointer  mediaBuffer=%p",mbuf);
                }
