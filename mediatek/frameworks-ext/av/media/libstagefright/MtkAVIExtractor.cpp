...
static bool checkVideoResolution(int32_t width, int32_t height, const char* streamtype) {
    return true;
	...
	qinfo.u4Height = height;
   	...
   	if (!strcasecmp(streamtype, MEDIA_MIMETYPE_VIDEO_AVC)){
		qinfo.u4VideoFormat = VDEC_DRV_VIDEO_FORMAT_H264;
   	}else if(!strcasecmp(streamtype, MEDIA_MIMETYPE_VIDEO_MPEG4)){
   	 	qinfo.u4VideoFormat = VDEC_DRV_VIDEO_FORMAT_MPEG4;
   	}else if(!strcasecmp(streamtype, MEDIA_MIMETYPE_VIDEO_DIVX)){
   	 	qinfo.u4VideoFormat = VDEC_DRV_VIDEO_FORMAT_MPEG4;
	}else if(!strcasecmp(streamtype, MEDIA_MIMETYPE_VIDEO_DIVX3)){
   	 	qinfo.u4VideoFormat = VDEC_DRV_VIDEO_FORMAT_MPEG4;
	}else if(!strcasecmp(streamtype, MEDIA_MIMETYPE_VIDEO_XVID)){
   	 	qinfo.u4VideoFormat = VDEC_DRV_VIDEO_FORMAT_MPEG4;
   	}else if(!strcasecmp(streamtype, MEDIA_MIMETYPE_VIDEO_H263)){
   	 	qinfo.u4VideoFormat = VDEC_DRV_VIDEO_FORMAT_MPEG4;
   	}else if(!strcasecmp(streamtype, MEDIA_MIMETYPE_VIDEO_MPEG2)){ 
	...
	LOGE("[AVI DRV capability info] ret =%d ,MaxWidth=%d, MaxHeight=%d",  retval,outinfo.u4Width , outinfo.u4Height);

	int32_t MaxHightWidth=0;
	MaxHightWidth = outinfo.u4Width>outinfo.u4Height ? outinfo.u4Width : outinfo.u4Height;

   	return width <= MaxHightWidth && width > 0 &&
	        height <= MaxHightWidth && height > 0 &&
	        (width * height <= outinfo.u4Width*outinfo.u4Height);
#else
      return width <= kMaxVideoWidthHeight && width > 0 &&
        height <= kMaxVideoWidthHeight && height > 0 &&
        (width * height <= kMaxVideoDim);
#endif
}

static bool checkProfileLevel(int32_t width, int32_t height, int32_t scale, int32_t rate,uint32_t level) {
    return true;
...
/*	VDEC_DRV_QUERY_VIDEO_FORMAT_T qinfo;
	...
	VDEC_DRV_MRESULT_T retval = eVDecDrvQueryCapability(VDEC_DRV_QUERY_TYPE_VIDEO_FORMAT, &qinfo, &outinfo);
	LOGE("[AVI DRV capability info] ret =%d ,MaxWidth=%d, MaxHeight=%d, profile=%d, level=%d",  retval,outinfo.u4Width , outinfo.u4Height, outinfo.u4Profile, outinfo.u4Level);
*/
#ifndef MTK_REMOVE_H264_LEVEL_LIMIT
       return (((int64_t) rate * width * height <= 256ll * kMaxMacroPerSecond * scale) && level<=31);
#else
	return true;
#endif
...
status_t MtkAVIExtractor::checkCapability() {
    for(size_t i=0; i<mTracks.size(); ++i) {
		...
		source->getFormat()->findCString(kKeyMIMEType, &mime);
            if (!checkVideoResolution(mWidth, mHeight,mime)) {
                LOGE("[AVI capability error]Unsupported video resolution %dx%d", mWidth, mHeight);
                err = ERROR_UNSUPPORTED;
            } else if (source->mIsAVC && !checkProfileLevel(mWidth, mHeight, source->mScale, source->mRate, source->mLevel)) {
                LOGE("[H264 capability error]Unsupported video profile %dx%d@%d/%d, level:%d", mWidth, mHeight, source->mScale, source->mRate, source->mLevel);
                err = ERROR_UNSUPPORTED;
            }
        }
        if (err == ERROR_UNSUPPORTED) {
...
status_t MtkAVISource::generateCodecData(bool full) {
    ...
    if (!strcasecmp(mime, MEDIA_MIMETYPE_VIDEO_AVC)
        ...
        if (mSampleSizes.size() > 0) {
            ...
            if (isMPEG4 || isDIVX || isXVID || isDIVX3) { 
