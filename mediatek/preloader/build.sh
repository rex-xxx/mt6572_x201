...
function mkpadfile () {
    SOURCE=$1
    PADFILE=$2
    dd if=/dev/zero of=${PADFILE} bs=1 count=0 seek=$(echo 512-$(stat -c %s ${SOURCE})%512|bc) 2> /dev/null
}
function catfiles () {
    FIRSTFILE=$1
    LASTFILE=$2
#    chmod u+w ${FIRSTFILE} ${LASTFILE}
    cat ${LASTFILE} >> ${FIRSTFILE}
}
function post_process () {
    ...
    if [ "$1" != "" ]; then
        echo "============================================"
        echo "======= $1, pading to 512 bytes ======="
        echo "============================================"
        mkpadfile ${PL_PATH}/${PL_IMAGE} ${PL_PATH}/pad;
        catfiles ${PL_PATH}/${PL_IMAGE} ${PL_PATH}/pad;
    fi
    PRODUCT=""
}
...
# Main Flow
#
if [ ${MTK_MEM_PRESERVED_MODE_ENABLE} == "yes" ] && [ ${TARGET_BUILD_VARIANT} !="user" ]; then
