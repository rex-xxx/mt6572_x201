...
static void print_verinfo(char *info, int *len, char *tag, char *msg) 
{
    ...
    //int max_len = CUST_LCD_AVAIL_WIDTH / CHAR_WIDTH;
    int max_len = gr_fb_width() / CHAR_WIDTH;
    int msg_len = strlen(msg);
    //int buf_len = CUST_LCD_AVAIL_WIDTH / CHAR_WIDTH;
    int buf_len = gr_fb_width() / CHAR_WIDTH;
    //_len += sprintf(info + _len, tag);
    // Fix Android 2.3 build error
...
//                memset(at_command, 0, sizeof(at_command));
			...
			write_len = write(fd, test_result, strlen(test_result));
...
#if 0
static int test_module(int fd, char *origin_at_command)
 {
    int arg = 0;
	char at_command[128] = {0};
	int id = -1;
	...
	int i = 0;
	int len = strlen(origin_at_command);
	strcpy(temp_at_command, origin_at_command);
	char *p;
	items = get_item_list();
	arg = get_AT_command(temp_at_command, at_command);
	LOGD(TAG "at_command is %s\n", at_command);
	...
	{
		p = prop_name = at_command;
		len = strlen(at_command);
		LOGD(TAG "p=%s, at_command=%s\n", p, at_command);
		for(i = 0; i < len; i++,p++)
		{
			if((*p == '=') && (i + 1) < len)
			{
		...
		LOGD(TAG "prop_name=%s, prop_val=%s\n", prop_name, prop_val);
		ftm_set_prop(prop_name, prop_val);
		strcpy(test_result, result[1]);
	}
...
//		LOGD(TAG "time_use = %lf\n", time_use);
        if(fd != -1)
        {
            read_from_usb = read(fd, USB_read_buffer, sizeof(USB_read_buffer));
...
void *read_data_thread(int fd)
{
	...
	while(1){
		if(fd != -1)
    		{
            		read_from_usb = read_a_line(fd, USB_read_buffer, sizeof(USB_read_buffer));
		}
		if(read_from_usb == -1)
		{
            	continue;
		...
		else if(read_from_usb > 0)
		{
			...
			else if(strncmp(USB_read_buffer, REQUEST_DATA, strlen(REQUEST_DATA)) == 0)
			{
			    ...
			    int n = write(fd, temp_buf, sizeof(temp_buf));
...
//    int usb_com_port = fd;
	bool exit = false;
	bool print_write = false;
	int read_from_usb = 0;
	char USB_read_buffer[BUFSZ] = {0};
	char at_command[BUFSZ] = {0};
    int arg_num = 0;
	int id = -1;
	char test_item[BUFSZ] = {0};
	struct ftm_param param;
	char *prop1 = NULL;
	char *prop2 = NULL;
	char *prop = NULL;
	...
	struct ftm_module *mod;
	item_t *items;
	char result[3][16] = {"not test", "pass", "fail" };
	char test_result[8] = {0};
	items = get_item_list();
...
*/
		...
		else if(read_from_usb > 0)
		{
			...
			else if(strncmp(USB_read_buffer, REQUEST_DATA, strlen(REQUEST_DATA)) == 0)
			{
			    ...
			    if(n != sizeof(temp_buf))
			    {
				    LOGD(TAG "Write test_data fail\n");
			    }
				else
				{
					 LOGD(TAG "Write test_data successfully\n");
				}
			}
			else if(strncmp(USB_read_buffer, VERSION, strlen(VERSION))==0)
			{
...
//		return 0;
	...
	if(usb_com_port == -1)
	{
        LOGD(TAG "Open usb fail!\n");
		return -1;
	}
	else
	{
		LOGD(TAG "Open usb successfully!\n");
	}
#endif
	...
	{
		LOGD(TAG "pc_control_mode return %d\n", pc_control_mode(usb_com_port));
