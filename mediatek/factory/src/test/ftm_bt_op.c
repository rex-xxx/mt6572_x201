...
static BOOL BT_Send_HciCmd(BT_HCI_CMD *hci_cmd)
{
    ...
    memcpy(&HCI_CMD[4], hci_cmd->parms, hci_cmd->len);
...
void updateTextInfo(BT_Info *pBtInfoForList, char* output_buf, int buf_len)
{
    ...
    while(pInfoList){
        if(strlen((const char*)pInfoList->name)){
            int str_len = 0;
            str_len = strlen((const char*)pInfoList->name);
            
            strncpy(cBuf  + strlen(cBuf), (const char*)pInfoList->name, 12);
            if(str_len < 12){
                strncpy(cBuf  + strlen(cBuf), "            ", 12 - str_len);
            }
            sprintf(cBuf  + strlen(cBuf), " %d\n", pInfoList->rssi);
            strcpy(return_data.bt.bt[return_data.bt.num].bt_name, pInfoList->name);
        }else{
            /* Inquiry result */
            sprintf(cBuf  + strlen(cBuf), "%02x%02x%02x%02x%02x%02x %d\n",
            pInfoList->btaddr[0], pInfoList->btaddr[1], pInfoList->btaddr[2],
            pInfoList->btaddr[3], pInfoList->btaddr[4], pInfoList->btaddr[5],
            pInfoList->rssi);
            
            return_data.bt.bt[return_data.bt.num].bt_rssi = pInfoList->rssi;
