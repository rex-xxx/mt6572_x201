...
extern     int DdpFactoryModeTest(void * a_pArg);
extern "C" int main_factory(); 
...
static void *MT6589_DDPEMITestThread(void * a_pArg)
{
    ...
    if( DdpFactoryModeTest(a_pArg) == 0 )
    {
        *result = NO_ERROR;
    } else
    {
...
#if 0
pthread_t MT6575MFVEMITestThread;
static void *MT6575_MFVEMITestThread(void * a_pArg)
{
    unsigned long *u4ThreadMDPResult = (unsigned long *)a_pArg;
#ifdef FEATURE_FTM_EMI	
    if (main_factory())
    	{
    	printf("MT6575_MFVEMITestThread Test success!\n");
		*u4ThreadMDPResult = 0;
    	}
    else
    	{
		printf("MT6575_MFVEMITestThread Test fail!\n");
		*u4ThreadMDPResult = 1;
    	}
#endif
    return NULL;
...
#if 0    
    pthread_create(&MT6575MFVEMITestThread, NULL , MT6575_MFVEMITestThread , &u4MFVResult);
#endif
...
#if 0    
    pthread_join(MT6575MFVEMITestThread , NULL);
#endif
