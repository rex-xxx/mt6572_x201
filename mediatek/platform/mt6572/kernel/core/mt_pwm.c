...
enum {                                                                                     
	...
	PWM_SEND_DATA1,
	PWM_WAVE_NUM,
	PWM_DATA_WIDTH,		//PWM 4&5: SEND_WAVENUM 
	PWM_THRESH,			//PWM 4&5: DATA_WIDTH
	PWM_SEND_WAVENUM,	//PWM 4&5: THRESH
	PWM_VALID
}PWM_REG_OFF;
...
struct pwm_device {
	...
	dev_t           devno;
	spinlock_t      lock;
	unsigned long 	power_flag;//bitwise, bit(9):map to MT_CG_PWM_SW_CG
	struct device   dev;
};
...
void mt_pwm_power_on(U32 pwm_no, BOOL pmic_pad)
{
	if(0 == pwm_dev->power_flag){
		...
		if(!clock_is_on(MT_CG_PWM_SW_CG))
		{
			enable_clock(MT_CG_PWM_SW_CG, "PWM");
		}
		set_bit(PWM_NUM, &(pwm_dev->power_flag));
	}
...
void mt_pwm_power_off (U32 pwm_no, BOOL pmic_pad)
{
	...
	if((0x01<<PWM_NUM) == pwm_dev->power_flag){
		PWMDBG("disable_clock: main\n");
		if(clock_is_on(MT_CG_PWM_SW_CG))
		{
			disable_clock(MT_CG_PWM_SW_CG, "PWM");
		}
		clear_bit(PWM_NUM, &(pwm_dev->power_flag));
	}
}
...
*/
		...
		if(i==PWM4||i==PWM5){
			reg_val=INREG32(PWM_register[i]+4* (PWM_DATA_WIDTH+1));
		}else{
			reg_val=INREG32(PWM_register[i]+4* PWM_DATA_WIDTH);
		}
		PWMMSG("[PWM%d_WIDTH is:%x]\r\n",i+1,reg_val);
		if(i==PWM4||i==PWM5){
			reg_val=INREG32(PWM_register[i]+4* (PWM_THRESH+1));
		}else{
