...
extern int mt_gpio_delete_attr(struct device *dev);
extern int mt_gpio_create_attr(struct device *dev);
/*---------------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------------*/
static void mt_gpio_pin_decrypt(unsigned long *cipher)
{
	//*cipher &= ~0x80000000;
	return;
}
//				GPIOERR("base %d operation result %d\n",pin,retval);
//				GPIOERR("ext %d operation result %d\n",pin,retval);
