...
* glabal variable
**********************************/
volatile static int dormant_ret_flag[2] = {0,0};
volatile int power_state[2] = {STATUS_RUN,STATUS_RUN};
...
static void restore_gic_distributor_shared(u32 *pointer, unsigned gic_distributor_address)
{
    ...
    if (num_spis) {
        ...
        if (reg_read( IO_VIRT_TO_PHYS(SPM_SLEEP_ISR_RAW_STA) ) & WAKE_SRC_CONN_WDT) {
            i = MT_CONN_WDT_IRQ_ID / GIC_PRIVATE_SIGNALS;
            j = MT_CONN_WDT_IRQ_ID % GIC_PRIVATE_SIGNALS;
            id->pending.set[i] |= (1 << j);
        }
...
int cpu_power_down(int mode)
{
    ...
    reg_write(BOOT_SLV_CFG, (BOOT_SLV_CFG_KEY | 0x1));   
    reg_write(BOOT_SLV_ADDR, __pa(restore_ptr));   
