...
enum {                                                                                     
	...
	PWM_SEND_DATA1,                                                                       
	PWM_WAVE_NUM,                                                                         
	PWM_DATA_WIDTH,      //PWM4&5 SEND_WAVENUM  
	PWM_THRESH,          //PWM4&5 DATA_WIDTH
	PWM_SEND_WAVENUM,    //PWM4&5 THRESH                                                                 
	PWM_VALID                                                                             
}PWM_REG_OFF;
...
S32 mt_get_pwm_datawidth (U32 pwm_no)
{
	...
	if (pwm_no == PWM4 || pwm_no == PWM5) {
		reg_data = PWM_register[pwm_no] + 4 * (PWM_DATA_WIDTH+1);
	} else {
...
S32 pwm_set_easy_config ( struct pwm_easy_config *conf)
{
	...
	switch ( conf->clk_src ) {
		...
		case PWM_CLK_OLD_MODE_32K:
			...
			if ( conf->pwm_no == PWM4 || conf->pwm_no == PWM5 ){
...
void mt_pwm_dump_regs(void)
{
	...
	for ( i = PWM1;  i < PWM_NUM; i ++ ) 
	{
		...
		if ( i <= PWM3 ) {
			...
			PWMMSG("\r\n[PWM%d_SEND_WAVENUM is:%x]\r\n",i+1,reg_val);
			reg_val=INREG32(PWM_register[i]+4* (PWM_VALID)); // 6572 valid offset are same for all PWM
			PWMMSG("\r\n[PWM%d_SEND_VALID is:%x]\r\n",i+1,reg_val);
		}
	}		
}
