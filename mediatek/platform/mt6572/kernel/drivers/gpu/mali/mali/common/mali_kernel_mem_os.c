...
static mali_physical_memory_allocation_result os_allocator_allocate_page_table_block(void * ctx, mali_page_table_block * block)
{
   ...
   /// even 32KB conti. phy. might be hard to allocate. And it would cause ANR or KE
   /// Config the order to zero (4KB) will suffer some performance during page table block allocation
   int allocation_order = 0; /* _MALI_OSK_CPU_PAGE_SIZE << 0 */
#endif	
