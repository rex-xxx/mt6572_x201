...
static void mali_early_suspend_handler(struct early_suspend *h)
{   
   mali_pm_os_suspend(MALI_SLEEP_EARLY_SUSPEND);
...
{   
   ...
   mali_pm_os_resume(MALI_SLEEP_LATE_RESUME);
}
#endif
...
static ssize_t mali_early_suspend_register_write(struct file *file, const char *buffer, unsigned long count, void *data)
{
    ...
    if (sscanf(buffer, "%d", &debug) == 1)
    {
        ...
        {
            xlog_printk(ANDROID_LOG_INFO, "Mali/EarlySuspend", "bad argument!! should be 0 or 1 [0: disable, 1: enable]\n");
        }
    }
    else
    {
