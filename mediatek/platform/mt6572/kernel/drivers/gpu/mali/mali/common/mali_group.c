...
static void mali_group_complete_pp(struct mali_group *group, mali_bool success)
{
	...
	{
		MALI_DEBUG_PRINT(1, ("Mali group: Failed to reset PP, need to reset entire group\n"));
...
static void mali_group_complete_gp(struct mali_group *group, mali_bool success)
{
	...
	{
		MALI_DEBUG_PRINT(1, ("Mali group: Failed to reset GP, need to reset entire group\n"));
...
void mali_group_abort_session(struct mali_group *group, struct mali_session_data *session)
{
	...
	{
		MALI_DEBUG_PRINT(2, ("Aborting GP job 0x%08x from session 0x%08x\n", gp_job, session));
	...
	{
		MALI_DEBUG_PRINT(2, ("Mali group: Aborting PP job 0x%08x from session 0x%08x\n", pp_job, session));
...
{
	MALI_ASSERT_GROUP_LOCKED(group);
   MALI_DEBUG_PRINT(1, ("Mali group: mali_group_recovery_reset\n"));
   
   /// try power on the mali driver for recovery
   mali_platform_power_mode_change(MALI_POWER_MODE_ON);	
   
   /// this issue should not hit, dump the callstack for further analysis
   dump_stack();      
...
static void mali_group_bottom_half_mmu(void * data)
{
   ...
   MALI_DEBUG_PRINT(2,("mali_group_bottom_half_mmu\n"));
	...
	MALI_DEBUG_PRINT(2, ("Mali MMU: Bottom half, interrupt 0x%08X, status 0x%08X\n", rawstat, status));
...
static void mali_group_bottom_half_pp(void *data)
{
	...
	if (0 == irq_readout)
	{
		MALI_DEBUG_PRINT(2, ("Mali group: No interrupt found on core %s\n",
