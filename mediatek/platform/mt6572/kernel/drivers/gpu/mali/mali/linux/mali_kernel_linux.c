...
int mali_module_init(void)
{
...
#if defined(MALI_FAKE_PLATFORM_DEVICE)
	MALI_DEBUG_PRINT(1, ("mali_module_init() registering device\n"));
	...
	MALI_DEBUG_PRINT(1, ("mali_module_init() registering driver\n"));

	err = platform_driver_register(&mali_platform_driver);

	if (0 != err)
	{
		MALI_DEBUG_PRINT(1, ("mali_module_init() Failed to register driver (%d)\n", err));
#if defined(MALI_FAKE_PLATFORM_DEVICE)
...
{
	MALI_DEBUG_PRINT(1, ("Unloading Mali v%d device driver.\n",_MALI_API_VERSION));

	MALI_DEBUG_PRINT(1, ("mali_module_exit() unregistering driver\n"));
...
#if defined(MALI_FAKE_PLATFORM_DEVICE)
	MALI_DEBUG_PRINT(1, ("mali_module_exit() unregistering device\n"));
	mali_platform_device_unregister();
#endif
...
static int mali_probe(struct platform_device *pdev)
{
	...
	MALI_DEBUG_PRINT(1, ("mali_probe(): Called for platform device %s\n", pdev->name));
...
static int mali_remove(struct platform_device *pdev)
{
	MALI_DEBUG_PRINT(1, ("mali_remove() called for platform device %s\n", pdev->name));
...
static int mali_driver_suspend_scheduler(struct device *dev)
{
	mali_pm_os_suspend(MALI_SLEEP_NORMAL_SUSPEND);
	return 0;
}

static int mali_driver_resume_scheduler(struct device *dev)
{
	mali_pm_os_resume(MALI_SLEEP_NORMAL_RESUME);
	return 0;
}
...
#endif
{
	...
	switch(cmd)
	{
		...
		case MALI_IOC_PROFILING_REPORT_SW_COUNTERS: /* FALL-THROUGH */
			MALI_DEBUG_PRINT(1, ("Profiling not supported\n"));
			err = -ENOTTY;
			break;
		...
		case MALI_IOC_MEM_DMA_BUF_GET_SIZE: /* FALL-THROUGH */
			MALI_DEBUG_PRINT(1, ("DMA-BUF not supported\n"));
			err = -ENOTTY;
			break;
#endif
		...
		case MALI_IOC_FENCE_VALIDATE:
			...
			break;
#else
			MALI_DEBUG_PRINT(1, ("Sync objects not supported\n"));
			err = -ENOTTY;
			break;
#endif
		...
		default:
			MALI_DEBUG_PRINT(1, ("No handler for ioctl 0x%08X 0x%08lX\n", cmd, arg));
			err = -ENOTTY;
	};
