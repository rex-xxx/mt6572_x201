...
{
	/// MALI_DEBUG_PRINT(2, ("JH:Memory Validator installed for Mali physical address base=0x%08X, size=0x%08X\n",
	///                  mali_mem_validator.phys_base, mali_mem_validator.size));
   ...
   return _MALI_OSK_ERR_OK;	                       
	...
	///       if (phys_addr          >= mali_mem_validator.phys_base) 		
	///       {
	///           MALI_PRINT_ERROR(("1 = (0x%08X, 0x%08X)\n", phys_addr, mali_mem_validator.phys_base));
	///       }   
   /// 
	///       if ((phys_addr + (size - 1)) >= mali_mem_validator.phys_base) 		
	///       {
	///           MALI_PRINT_ERROR(("2 = (0x%08X, 0x%08X)\n", phys_addr + (size - 1), mali_mem_validator.phys_base));
	///       }   	      
	///       
	///       if (phys_addr          <= (mali_mem_validator.phys_base + (mali_mem_validator.size - 1)))
	///       {
	///          MALI_PRINT_ERROR(("3 = (0x%08X, 0x%08X)\n", phys_addr, mali_mem_validator.phys_base + (mali_mem_validator.size - 1)));
	///       }  
	///       
	///       if (((phys_addr + (size - 1)) <= (mali_mem_validator.phys_base + (mali_mem_validator.size - 1))))
	///       {
	///          MALI_PRINT_ERROR(("4 = (0x%08X, 0x%08X)\n", phys_addr + (size - 1), mali_mem_validator.phys_base + (mali_mem_validator.size - 1)));
	///       }
	///       
	///       MALI_DEBUG_PRINT(3, ("not OK1:Accepted range 0x%08X + size 0x%08X (= 0x%08X)\n", phys_addr, size, (phys_addr + size - 1)));
	/// 	}		
	/// 	MALI_DEBUG_PRINT(3, ("not OK2:Accepted range 0x%08X + size 0x%08X (= 0x%08X)\n", phys_addr, size, (phys_addr + size - 1)));
	/// }
	/// 	   
