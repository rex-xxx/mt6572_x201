...
#include <mach/mt_clkmgr.h> //Light, for enable_clock()
...
static u32 u_msdc_base[HOST_MAX_NUM] = {MSDC_0_BASE,MSDC_1_BASE};
...
static void simp_msdc_dump_register(struct simp_msdc_host *host)
{
    ...
    printk(KERN_EMERG "Reg[A8] DMA_LEN        = 0x%.8x", sdr_read32(base + 0xA8));
...
static unsigned int simp_mmc_power_up(struct simp_mmc_host *host,bool on)
{
    switch(host->mtk_host->id){
        case 0:
            ...
            //simp_msdc_ldo_power(on, MT6363_POWER_LDO_VEMC_1V8, VOL_1800); //Always on, no need to turn it on
...
{
    //Yuchi: To guarantee clock is enable, 
    //       toggle clock control register directly instead of invoking enable_clock()
    //enable_clock(msdc_cg_clk_id[host->mtk_host->id], "SD");

    //
    // MAINPLL setting
    #if 0
    //Artis: MAINPLL shall have been enabled, otherwise system may not work?
    //       Therefore, no need to enable MAINPLL again
    if (sdr_write32(0xF0000000) & 0x80)
    {
        sdr_write32(0xF0205124, 0x80F60000);
    }
    else // DDR3
    {
        sdr_write32(0xF0205124, 0x800CC000);
    }
    #endif
    
    //
    // MSDC MUX & CG
    sdr_write32(0xF0000054, 0x00060000);
    sdr_write32(0xF0000000, (DRV_Reg32(0xF0000000) & 0xFF8FC7FF) | 0x00201000);
    sdr_write32(0xF0000084, 0x00060000);
...
static unsigned int simp_msdc_init(struct simp_mmc_host *mmc_host)
{
    ...
    sdr_set_field(MSDC_CFG, MSDC_CFG_MODE, MSDC_SDMMC); // MSDC_SDMMC
	...
	switch (host->id) {
		case 0:
			sdr_set_field_discrete(MSDC0_PAD_CTRL_SMT_CFG, MSDC0_SMT_CK_CMD_DAT0_3_MASK, 1);
			sdr_set_field_discrete(MSDC0_PAD_CTRL_SMT_CFG, MSDC0_SMT_DAT4_7_MASK, 1);			
			break;
		case 1:
			sdr_set_field_discrete(MSDC1_PAD_CTRL_SMT_CFG, MSDC1_SMT_CK_CMD_DAT0_3_MASK, 1);
			break;
		default:
			break;
	}
    ...
    switch (host->id) {
		case 0:
			sdr_set_field(MSDC0_PAD_CTRL_DRV_CFG, MSDC0_DRV_CK_CMD_DAT0_3_MASK, 2);
			sdr_set_field(MSDC0_PAD_CTRL_DRV_CFG, MSDC0_DRV_DAT4_7_MASK, 		2);
			sdr_set_field(MSDC0_PAD_CTRL_DRV_CFG, MSDC0_DRV_RSTB_MASK, 			2);
			break;
		case 1:
			//if(sd_18){
			//	sdr_set_field(MSDC1_PAD_CTRL_DRV_CFG, MSDC1_DRV_CK_CMD_DAT0_3_MASK, 5);
			//} else{
			//	sdr_set_field(MSDC1_PAD_CTRL_DRV_CFG, MSDC1_DRV_CK_CMD_DAT0_3_MASK, 4);
	        //}
			sdr_set_field(MSDC1_PAD_CTRL_DRV_CFG, MSDC1_DRV_CK_CMD_DAT0_3_MASK, 4);
			break;
