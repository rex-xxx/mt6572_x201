...
void dumpRegDebugInfo(unsigned int engineFlag, int cmdqIndex, cmdq_buff_t bufferAddr)
{
    ...
    printk(KERN_DEBUG "[CMDQ]engineFlag = %x, MMSYS_CG_CON0 = 0x%08x\n", engineFlag, mmsys_cg_con0);
...
void cmdqHwClockOn(unsigned int engineFlag)
{
...
bool cmdqTaskAssigned(int taskID, unsigned int priority, unsigned int engineFlag, unsigned int blocksize)
{
    ...
    if((engineFlag & hwResTbl) == 0) //Free HW available
    {
       ...
       // CMDQ_MSG("======cmdqTaskAssigned====\n");
        ...
        cmdq_pc_head = pCmdqAddr->VA;
        cmdqBufTbl[buf_id].blocksize = blocksize;
        cmdqBufTbl[buf_id].blockTailAddr = (cmdq_pc_head+ins_leng-1);

        //DBG message
        printk(KERN_DEBUG "==========DISP_IOCTL_EXEC_COMMAND Task: %d ,Thread: %d, PC[0x%x], EOC[0x%x] =========\n",taskID, cmdqThread,pCmdqAddr->MVA ,pCmdqAddr->MVA + cmdqBufTbl[buf_id].blocksize);

        //Start! Power on(TODO)!
       
        cmdqHwClockOn(cmdqThreadResTbl[cmdqThread]);
 ...
 //       spin_lock_irqsave(&gCmdqMgrLock,flags); 
        ...
        for(i=0;i<CMDQ_THREAD_NUM;i++) //Find new free thread
        {
            ...
            if(cmdqThreadResTbl[i] == engineFlag) //Use Same HW
            {
                ...
                CMDQ_MSG("PC: 0x%x EOC: 0x%x\n",DISP_REG_GET(DISP_REG_CMDQ_THRx_PC(cmdqThread)));
            ...
            else if((cmdqThreadResTbl[i] & engineFlag) != 0) //Overlaped HW
            {
                ...
                CMDQ_MSG("PC: 0x%x\n",DISP_REG_GET(DISP_REG_CMDQ_THRx_PC(cmdqThread)),DISP_REG_GET(DISP_REG_CMDQ_THRx_END_ADDR(cmdqThread)));
                ...
                {
                   //POWER ON! (TODO)
                    cmdqHwClockOn((engineFlag&~(cmdqThreadResTbl[i])));
        ...
        else
        {
            ...
            *(pPre_cmdqAddr->blockTailAddr-1) = pCmdqAddr->MVA; //Jump to here
            ...
            CMDQ_MSG("\n==============Modify %d's Pre-ID %d Jump ADDR[0x%x] :0x%x , 0x%x ===================\n",cmdqThread, pre_task_id ,pPre_cmdqAddr->blockTailAddr ,*(pPre_cmdqAddr->blockTailAddr) ,*(pPre_cmdqAddr->blockTailAddr-1) );
        }
...
//0322
    if(cmdqThreadTaskList_R[cmdqThread] == cmdqThreadTaskList_W[cmdqThread]) //no task needed
    {
        ...
        // MDP was not busy but CMDQ didn't receive interrupt
        else if (false == cmdqIntStatus)
        {
            cmdq_isr_fail_cnt++;
        }
