...
int get_rtc_spare_fg_value(void){return 0;}
int set_rtc_spare_fg_value(int val){return 0;}
static void rtc_xosc_write(u16 val, bool reload){};
bool crystal_exist_status(void){return 0;}
...
bool rtc_gpio_32k_status(){return 0;}
...
static int rtc_ops_read_time(struct device *dev, struct rtc_time *tm){return 0;}
static int rtc_ops_set_time(struct device *dev, struct rtc_time *tm){return 0;}
static int rtc_ops_read_alarm(struct device *dev, struct rtc_wkalrm *alm){return 0;}
static void rtc_save_pwron_time(bool enable, struct rtc_time *tm, bool logo){};
static int rtc_ops_set_alarm(struct device *dev, struct rtc_wkalrm *alm){return 0;}
static int rtc_pdrv_probe(struct platform_device *pdev){return 0;}
static int rtc_pdrv_remove(struct platform_device *pdev){return 0;}
static int __init rtc_mod_init(void){return 0;}
static void __exit rtc_mod_exit(void){};
static int __init rtc_late_init(void){return 0;}
...
{
	u32 rdata=0;
	//pwrap_write(TOP_CKPDN1_CLR, PMIC_RG_RTC_MCLK_PDN_MASK << PMIC_RG_RTC_MCLK_PDN_SHIFT);
	pwrap_read((u32)addr, &rdata);
	//pwrap_write(TOP_CKPDN1_SET, PMIC_RG_RTC_MCLK_PDN_MASK << PMIC_RG_RTC_MCLK_PDN_SHIFT);
	return (u16)rdata;
}
...
{
	/*RTC_SPAR0: 
	bit 7 	 : low power detected in preloader. True:detected, False:NOT detected*/
	u16 spar0;
	spar0 = rtc_read(RTC_SPAR0);
	rtc_xinfo("[RTC] RTC_SPAR0=0x%x \n", spar0);
	if(spar0 & 0x0080)
		return true;
	else
		return false;
...
unsigned long rtc_read_hw_time(void)
{
	...
	spin_unlock_irqrestore(&rtc_lock, flags);
	rtc_xinfo("read tc time do_gettimeofday = %09d.%09d\n",start_timer.tv_sec, start_timer.tv_usec);
