/* mediatek/platform/mt6572/kernel/drivers/uart/uart.c
 *
 * (C) Copyright 2008 
...
static struct mtk_uart_setting mtk_uart_default_settings[] = 
{
    ...
    },
    {
        ...
        .sysrq = FALSE, .hw_flow = FALSE, .vff = TRUE,
    },
};
/*---------------------------------------------------------------------------*/
...
static int __init mtk_uart_console_setup(struct console *co, char *options)
{
    ...
    printk(KERN_ALERT DBG_TAG "mt6572 console setup : co->index %d options:%s\n",
        co->index, options);
	...
	//enable_dpidle_by_bit(PDN_FOR_UART1);
    }else if(uart->nport == 1){ // UART2 as log port
        mtk_uart_default_settings[1].tx_mode = UART_NON_DMA;
        mtk_uart_default_settings[1].rx_mode = UART_NON_DMA;
        uart->tx_mode = UART_NON_DMA;
        uart->rx_mode = UART_NON_DMA;
	//enable_dpidle_by_bit(PDN_FOR_UART2);
    }
    ...
    printk(KERN_ALERT DBG_TAG "mt6572 console setup : uart_set_option port(%d) "
          "baud(%d) parity(%c) bits(%d) flow(%c) - ret(%d)\n",
           co->index, baud, parity, bits, flow, ret);
    
    printk(KERN_ALERT DBG_TAG "mt6572 setting: (%d, %d, %d, %lu, %lu)\n", 
...
#if 0
            mt6572_irq_set_sens(vfifo->irq_id, MT65xx_LEVEL_SENSITIVE);
            mt6572_irq_set_polarity(vfifo->irq_id, MT65xx_POLARITY_LOW);
#endif
            ret = request_irq(vfifo->irq_id, (irq_handler_t)mtk_vfifo_irq_handler, IRQF_TRIGGER_LOW, DRV_NAME, vfifo); 
            if (ret)
                return ret;
...
inline static void mtk_uart_power_up(struct mtk_uart *uart)
{
    ...
    MSG(FUC, "%s(%d) => up \n", __func__, uart->poweron_count); 
#endif /* End of CONFIG_MT6572_FPGA */
}
/*---------------------------------------------------------------------------*/
