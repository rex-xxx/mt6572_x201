...
{
	/* XXX: MT_CG_SPINFI_SW_CG is shared by SPI and SPINFI */
	enable_clock(MT_CG_SPINFI_SW_CG, SPI_DRV_NAME);
	return;
}

static void disable_clk(void)
{
	disable_clock(MT_CG_SPINFI_SW_CG, SPI_DRV_NAME);
	return;
}
...
static int __init mt_spi_init(void)
{
	...
	emi_mpu_notifier_register(MST_ID_PERI_2, spi_check_mpu_violation);
