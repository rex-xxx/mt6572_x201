...
static bool valid_bmt_data(phys_bmt_struct * phys_table)
{
    ...
    for (i = 0; i < phys_table->header.mapped_count; i++)
    {
        ...
        {
            MSG(ERR, "index error: bad_index: %d, mapped_index: %d\n", phys_table->table[i].bad_index, phys_table->table[i].mapped_index);
            return false;
        }
    }

    MSG(INFO, "Valid BMT, version v%d\n", phys_table->header.version);
    return true;
}
...
static int load_bmt_data(int start, int pool_size)
{
    ...
    {
        MSG(INFO, "[%s]: bmt_index=0x%x-- ", __FUNCTION__, bmt_index);
        if (nand_block_bad_bmt(OFFSET(bmt_index)))
        {
            MSG(INFO, "Skip bad block: %d\n", bmt_index);
            continue;
        }
        ...
        {
            MSG(ERR, "Error found when read block %d\n", bmt_index);
            continue;
        }

        if (!match_bmt_signature(dat_buf, oob_buf))
        {
            MSG(ERR, "[%s]: match_bmt_signature  out! \n", __FUNCTION__);
            continue;
        }

        MSG(INFO, "Match bmt signature @ block: 0x%x\n", bmt_index);

        memcpy(&phys_table, dat_buf + MAIN_SIGNATURE_OFFSET, sizeof(phys_table));

        if (!valid_bmt_data(&phys_table))
        {
            MSG(ERR, "BMT data is not correct %d\n", bmt_index);
            continue;
        } else
        {
            ...
            MSG(INFO, "bmt found at block: %d, mapped block: %d\n", bmt_index, bmt.mapped_count);

            for (i = 0; i < bmt.mapped_count; i++)
            {
                if (!nand_block_bad_bmt(OFFSET(bmt.table[i].bad_index)))
                {
                    MSG(INFO, "block 0x%x is not mark bad, should be power lost last time\n", bmt.table[i].bad_index);
...
bmt_struct *init_bmt(struct nand_chip * chip, int size)
{
    ...
    {
        MSG(INFO, "Load bmt data success @ block 0x%x\n", bmt_block_index);
        //dump_bmt_info(&bmt);
        return &bmt;
    } else
    {
        MSG(INFO, "Load bmt data fail, need re-construct!\n");
