...
{
#if DAI_MEMIF_SUPPORT || defined(BT_SW_CVSD)
...
ssize_t AudioMTKStreamIn::read(void *buffer, ssize_t bytes)
{
    ...
    if (mStarting == false)
    {
        ...
        mPAdcPCMDumpFile = AudioOpendumpPCMFile(streamin, streamin_propty);
        mPAdcPCMInDumpFile = AudioOpendumpPCMFile(streaminOri, streamin_propty);
        ...
        case AUDIO_MODE_IN_COMMUNICATION:
        {
            ...
            if((mAttribute.mSource == AUDIO_SOURCE_VOICE_RECOGNITION) ||mHDRecTunning16K || mIsDigitalMIC)
            {
    ...
    if (mAudioResourceManager->IsModeIncall() == false) { // Workaround for not do AP side enhancement & mono->stereo
        StreamInPreprocess(buffer, bytes);
        CheckNeedDataConvert((short *)buffer,bytes);
    }

    if (mPAdcPCMDumpFile) //fwrite((void *)buffer, sizeof(char), bytes, mPAdcPCMDumpFile);
...
void AudioMTKStreamIn::LoadHDRecordParams(void)
{
    ...
    {
        if(GetWBSpeechParamFromNVRam(&mSphParamWB)==0)
        {
            ALOGD("GetWBSpeechParamFromNVRam fail");
        }
    
        if(GetNBSpeechParamFromNVRam(&mSphParamNB)==0)
        {
            ALOGD("GetNBSpeechParamFromNVRam fail");
        }
    }
#endif
...
void AudioMTKStreamIn::ConfigHDRecordParams(SPE_MODE mode)
{
    ...
    for (int i = 0; i < EnhanceParasNum; i++)   //EnhanceParasNum = 16+12(common parameters)
    {
        ...
        {                        
            ...
            {
                //specific parameters
                HDRecordEnhanceParas[i] = mSphParamWB.speech_mode_wb_para[0][i];     //use loud speaker mode speech params
            }
            else
            {
                //common parameters
                HDRecordEnhanceParas[i] = mSphParamNB.speech_common_para[i - SPEECH_PARA_NUM];
            }
        }
    ...
    for (int i = 0; i < WB_FIR_NUM; i++)
    {
        ...
        {
            HDRecordCompenFilter[i] = mSphParamWB.sph_wb_in_fir[0][i];
            ALOGD("HDRecordCompenFilter[%d]=%d", i, HDRecordCompenFilter[i]);
            HDRecordCompenFilter[i + WB_FIR_NUM] = mSphParamWB.sph_wb_in_fir[0][i];
        }
    }
    //FIR parameters---
    ...
    //DMNR parameters+++
#ifdef MTK_DUAL_MIC_SUPPORT
    if((mAttribute.mSource == AUDIO_SOURCE_VOICE_RECOGNITION) || mIsAPDMNRTuningEnable)
    {
        AUDIO_CUSTOM_EXTRA_PARAM_STRUCT eSphParamDualMic;
        GetDualMicSpeechParamFromNVRam(&eSphParamDualMic);
        //only need wideband params for tool tuning
        ALOGD("DMNR setting");
        mpSPELayer->SetDMNRPara(mode, (short*)&eSphParamDualMic.ABF_para[NUM_ABF_PARAM]);
    }
#endif
    //DMNR parameters---
...
{
    if (mAttribute.mdevices != AUDIO_DEVICE_IN_FM)
