...
status_t AudioMachineDevice::AnalogOpen(AudioAnalogType::DEVICE_TYPE DeviceType)
{
    ...
    {
        ALOGW("AudioMachineDevice Bypass AnalogOpen with DeviceType = %d",DeviceType);
        mLock.unlock ();
        return NO_ERROR;;
    }
    ...
    switch (DeviceType) {
        ...
        case AudioAnalogType::DEVICE_OUT_EARPIECEL:
            ...
            usleep(10 * 1000); 
            mAudioAnalogReg->SetAnalogReg(AUDTOP_CON7, 0x25B0, 0xffff); // enable voice buffer and -1dB gain. didn't ramp volume here 
    ...
    switch (DeviceType) {
        ...
        case AudioAnalogType::DEVICE_IN_ADC2:
            ...
            if(GetDownLinkStatus() == false){
                ...
                mAudioAnalogReg->SetAnalogReg(AUDTOP_CON2 , 0x00C0, 0x00ff);//Disable RCH 1.4V, 2.4V
            }
            else{
                mAudioAnalogReg->SetAnalogReg(AUDTOP_CON0 , 0x7000, 0x7f80);//Disable LCH 2.4V, keep 1.4V
                mAudioAnalogReg->SetAnalogReg(AUDTOP_CON2 , 0x00E0, 0x00ff);//Disable RCH 2.4V, keep 1.4V
            }
