...
#define AUXADC_VIBSPK_CHANNEL 16
...
status_t AudioMTKHardware::SetVibSpkCalibrationParam(AUDIO_ACF_CUSTOM_PARAM_STRUCT *cali_param)
{
...
uint32_t AudioMTKHardware::GetVibSpkCalibrationStatus()
{
    AUDIO_ACF_CUSTOM_PARAM_STRUCT audioParam;
    GetAudioCompFltCustParamFromNV(AUDIO_COMP_FLT_VIBSPK,&audioParam);
    if(audioParam.bes_loudness_WS_Gain_Max != VIBSPK_CALIBRATION_DONE)
        return 0;
    else
        return audioParam.bes_loudness_WS_Gain_Min;
}

void AudioMTKHardware::SetVibSpkEnable(bool enable, uint32_t freq)
{
    if(enable)
...
void AudioMTKHardware::SetVibSpkRampControl(uint8_t rampcontrol)
{
...
status_t AudioMTKHardware::HardwareInit(bool bEnableSpeech)
{
    ...
    ALOGD("%s() contrcutor! ", __FUNCTION__);
    ...
    memset((void*)&mAudio_Control_State,0,sizeof(SPH_Control));
		...
		if(!mAudioBTCVSDControl){
			ALOGE("HardwareInit AudioBTCVSDControlInterface::getInstance() fail");
		}
    ...
    mAudioTuningInstance->SetHardwarePointer(this);
#endif
#endif

	AudioLoopbackController::GetInstance()->SetHardwarePointer(this);
...
AudioMTKHardware::~AudioMTKHardware()
{
	...
	if(mAudioBTCVSDControl){
		...
		AudioBTCVSDControlInterface::freeInstance();;
        mAudioBTCVSDControl = NULL;
	}
    #endif
}
...
status_t AudioMTKHardware::SetAudioCommand(int par1, int par2)
{
    ...
    switch (par1) {
        ...
        case AUDIO_USER_TEST: {
            ...
            {
                LoopbackManager::GetInstance()->SetLoopbackOn(AP_I2S_IN_DAC_AND_ADC_IN_TO_I2S_OUT, LOOPBACK_OUTPUT_EARPHONE, 48000, 48000);
            }
            else if (par2 == 0x33)
            {
                LoopbackManager::GetInstance()->SetLoopbackOn(AP_I2S_IN_DAC_AND_ADC_IN_TO_I2S_OUT, LOOPBACK_OUTPUT_EARPHONE, 16000, 16000);
            }
            else if (par2 == 0x34) {
                SetFmDigitalEnable(true);
            }
            else if (par2 == 0x35) {
            ...
            {
                LoopbackManager::GetInstance()->SetLoopbackOn(AP_I2S_IN_DAC_AND_ADC_IN_TO_I2S_OUT, LOOPBACK_OUTPUT_EARPHONE, 8000, 8000);
            }
            break;
        }
        ...
        case SET_CUREENT_SENSOR_ENABLE:
        {
            uint16_t temp;
            temp = 0x23 | (par2<<9);
            mAudioAnaRegInstance->SetAnalogReg(SPK_CON10, temp, 0xffff);
        ...
        case SET_CURRENT_SENSOR_RESET:
        {
            uint16_t temp;
            temp = par2<<8;
            mAudioAnaRegInstance->SetAnalogReg(SPK_CON10, temp, 0x0100);
        }
            break;
        default:
            break;
...
status_t AudioMTKHardware::SetFmDirectConnection(bool enable,bool bforce)
{
    ...
    if (enable == true) {
        ...
        mAudioDigitalInstance->SetHwDigitalGainEnable(AudioDigitalType::HW_DIGITAL_GAIN2, true);        // AFE_ON
        //Enable I0I1 I2S       
        mAudioDigitalInstance->Set2ndI2SInConfig(32000,true);//FM Rx I2S in from connsys is 32K/Master
        mAudioDigitalInstance->SetI2SASRCConfig(true,44100);// Covert to 44100
        ALOGD("Enable ASRC");
        ...
        //Disable I0I1 I2S: ASRC close than I2S close
        mAudioDigitalInstance->SetI2SASRCEnable(false);
        mAudioDigitalInstance->SetI2SASRCConfig(false,44100);// Setting to bypass ASRC
...
#else
//        if (mAudioResourceManager->IsWiredHeadsetOn()) {//Tina 1220 remove, already speaker connection
#if defined(MTK_MERGE_INTERFACE_SUPPORT)
            // L-Channel
...
status_t AudioMTKHardware::SetMatvDigitalEnable(bool enable)
{
    ...
    if (enable == true) {
        ...
        mAudioDigitalInstance->Set2ndI2SInEnable(enable);
...
status_t AudioMTKHardware::setMicMute(bool state)
{
    ...
    if (ModeInCall(mMode) == true) { // modem phone call
        ...
        SpeechPhoneCallController::GetInstance()->SetMicMuteStateOnly(state);
...
bool AudioMTKHardware::ReadAuxadcData(int channel, int *value)
{
    int ret;
...
status_t AudioMTKHardware::DcCalibrationProcess(AUDIO_BUFFER_DC_CALIBRATION_STRUCT *cali_param)
{
    ...
    { //Enter Calibration and save to NV
        ...
        ALOGD("Enter DC Calibration\n");
        ReadAuxadcData(AUXADC_HP_L_CHANNEL, &val_hpl_off); //LCH
        ReadAuxadcData(AUXADC_HP_R_CHANNEL, &val_hpr_off); //RCH
        ...
        for(int i = 0 ; i < 5 ; i ++){
            ReadAuxadcData(AUXADC_HP_L_CHANNEL, &val_hpl_on); //LCH
            ReadAuxadcData(AUXADC_HP_R_CHANNEL, &val_hpr_on); //RCH
    ...
    return NO_ERROR;
    ALOGD("DcCalibrationProcess 0x%x HPL= 0x%x, HPR = 0x%x", cali_param->cali_flag, cali_param->cali_val_hp_left, cali_param->cali_val_hp_right);
}

status_t AudioMTKHardware::SetDcCalibration(AUDIO_BUFFER_DC_CALIBRATION_STRUCT *cali_param)
{
    ALOGD("%s(), headphoneL = 0x%x, headphoneR = 0x%x ", __FUNCTION__, cali_param->cali_val_hp_left, cali_param->cali_val_hp_right);
