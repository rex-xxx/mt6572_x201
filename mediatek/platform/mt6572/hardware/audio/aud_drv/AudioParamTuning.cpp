...
static void *DMNR_Play_Rec_ApSide_Routine(void *arg)
{
    ...
    if(pInFp)//open output stream for playback
    {
	    ...
	    streamOutput = pDMNRTuning->getHardwarePointer()->openOutputStream(0, &format, &channels, &sampleRate, &status);
    ...
    if(pOutFp)//open input stream for record
    {
        ...
        pDMNRTuning->getHardwarePointer()->setParameters(paramVoiceMode.toString());
        ...
        status_t status = 0;
        streamInput = pDMNRTuning->getHardwarePointer()->openInputStream(device,&format,&channel,&sampleRate,&status,(android_audio_legacy::AudioSystem::audio_in_acoustics)0);
    ...
    if(pInFp)
    {
      ...
      pDMNRTuning->getHardwarePointer()->closeOutputStream(streamOutput);
      if(inBuffer)
      {
        delete[] inBuffer;
        inBuffer = NULL;
    ...
    {
    	streamInput->standby();
      pDMNRTuning->getHardwarePointer()->closeInputStream(streamInput);
    }
...
status_t AudioParamTuning::setMode(uint32 mode)
{
    ...
    mMode = (modem_index==MODEM_1) ? AUDIO_MODE_IN_CALL:AUDIO_MODE_IN_CALL_2;
    pthread_mutex_unlock(&mP2WMutex);
    return NO_ERROR;
}
...
status_t AudioParamTuning::enableModemPlaybackVIASPHPROC(bool bEnable, bool bWB)//need record path?
{
    ...
    if (bEnable&&(isPlaying()==false)) {
        ...
        mSphPhonecallCtrl->OpenModemSpeechControlFlow((audio_mode_t)mMode);
        ...
        mSphPhonecallCtrl->CloseModemSpeechControlFlow((audio_mode_t)mMode);
...
#else
void AudioParamTuning::SetHardwarePointer(void *pAudioHardware)
{
