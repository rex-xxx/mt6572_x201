...
public:
...
#if defined(BT_SW_CVSD) && defined(BTCVSD_LOOPBACK_WITH_CODEC)
		class AudioMTkRecordThread : public Thread
        {
            public:
                AudioMTkRecordThread(void *mAudioManager, uint32 Mem_type, char *mRingBuffer, uint32 mBufferSize);
    ...
    uint32_t SetVibSpkDefaultParam2();
...
private:
    ...
    sp<AudioMTkRecordThread>  mBTCVSDRxTestThread;
#endif
#endif
    ...
    bool    mDL2Vibspk;
    int     mDL2DelayOff;
    uint8_t *mSwapBufferVibSpk;
    uint8_t *mSwapBufferTone;
#endif
    Mutex mEffectLock;
