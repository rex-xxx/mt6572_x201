...
ImgSensorDrv::impSearchSensor(pfExIdChk pExIdChkCbf)
{
    ...
    LOG_WRN("SENSOR search [0]0x%x / [1]0x%x start \n", m_pstSensorInitFunc[0].SensorId, m_pstSensorInitFunc[1].SensorId);
    ...
    for (SensorEnum = DUAL_CAMERA_MAIN_SENSOR; SensorEnum <= DUAL_CAMERA_SUB_SENSOR; SensorEnum <<= 1)  {
        ...
        for (i = 0; i < MAX_NUM_OF_SUPPORT_SENSOR; i++) {
            ...
            err = ioctl(m_fdSensor, KDIMGSENSORIOC_X_SET_DRIVER,&id[KDIMGSENSOR_INVOKE_DRIVER_0] );
                ...
                //pSeninfDrv->setPdnRst(SensorEnum, 1);         

                //err = open();
                err = ioctl(m_fdSensor, KDIMGSENSORIOC_T_CHECK_IS_ALIVE);
                
                 //pSeninfDrv->setPdnRst(SensorEnum, 0);
    ...
    if (BAD_SENSOR_INDEX != m_subSensorDrv.index[0]) {
        ...
        sensorDevs |= SENSOR_ATV;
        LOG_MSG("[Sensor Camera]sensorDevs |= SENSOR_ATV!!!!!!");
...
)
{
    ...
    if(SENSOR_MAIN & sensorIdx ) {
        ...
        if (FeaturePara32) {
            ...
            m_LineTimeInus[0] = (MUINT32)((pFeaturePara16[0]*1000000+(FeaturePara32>>1))/FeaturePara32);
        }
    ...
    if(SENSOR_SUB & sensorIdx) {
        ...
        if (FeaturePara32) {
            //in setting domain, use preview line time only
            //sensor drv will convert to capture line time when setting to capture mode.
            m_LineTimeInus[1] = (MUINT32)((pFeaturePara16[0]*1000000+(FeaturePara32>>1))/FeaturePara32);
        }
        ...
        if(ACDK_SCENARIO_ID_MAX != sId[i]) {
            ...
            if (FeaturePara32) {
                if(FeaturePara32 >= 1000) {
                    m_LineTimeInus[i] = (MUINT32)((pFeaturePara16[0]*1000000 + ((FeaturePara32/1000)-1))/(FeaturePara32/1000));   // 1000 base , 33657 mean 33.657 us
                } 
                else {
                    LOG_ERR("[setScenario]: Sensor clock too slow = %d %d\n", FeaturePara32, pFeaturePara16[0]);        
                }
                LOG_MSG("[setScenario] m_LineTimeInus[%d] = %d id = %d PixelClk = %d PixelInLine = %d\n", i,m_LineTimeInus[i], sId[i], FeaturePara32, pFeaturePara16[0]); 
            }
...
{ 
    ...
    case CMD_SENSOR_GET_INPUT_BIT_ORDER:
        ...
        switch  (sensorDevId)
        {
            ...
            default:
                LOG_ERR("[sendCommand]<CMD_SENSOR_GET_INDATA_FORMAT> - bad sensor id(%x)", (int)parg2);
                *parg1 = 0;
                err = -1;
                break;
        }
    ...
    case CMD_SENSOR_GET_PAD_PCLK_INV:
        switch(sensorDevId)
        {
            ...
            default:
                LOG_ERR("[sendCommand]<CMD_SENSOR_GET_PAD_PCLK_INV> - bad sensor id(%x)", (int)parg2);
                *parg1 = 0;
                err = -1;
                break;
        }
    ...
    case CMD_SENSOR_GET_SENSOR_FEATURE_INFO:
        switch(sensorDevId)
        {
            ...
            default:
                LOG_ERR("[sendCommand]<CMD_SENSOR_GET_SENSOR_FEATURE_INFO> - bad sensor id(%x)", (int)parg2);
                parg1 = NULL;
                err = -1;
                break;
        }
    ...
    case CMD_SENSOR_GET_FAKE_ORIENTATION:
         switch(sensorDevId)
         {
             ...
             default:
                 LOG_ERR("[sendCommand]<CMD_SENSOR_GET_FAKE_ORIENTATION> - bad sensor id(%x)", (int)parg2);
                 *parg1 = 0;
                 err = -1;
                 break;
         }
