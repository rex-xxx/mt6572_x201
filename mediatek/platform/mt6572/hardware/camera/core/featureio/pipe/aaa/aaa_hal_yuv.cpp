...
MBOOL Hal3AYuv::sendCommand(ECmd_T const eCmd, MINT32 const i4Arg)
{
    ...
    else if (eCmd == ECmd_RecordingStart)
    {
        ...
        #if 0
        if (m_pStrobeDrvObj && m_rParam.u4StrobeMode == FLASHLIGHT_TORCH)
        {
            MY_LOG("ECmd_CaptureEnd: strobeMode = %d\n", m_rParam.u4StrobeMode);
            setFlashMode(FLASHLIGHT_TORCH);
        }
        #endif
        return MTRUE;
    }
    ...
    {
        MY_LOG("ECmd_CaptureEnd:\n");
        #if 0
        if (m_pStrobeDrvObj && m_rParam.u4StrobeMode == FLASHLIGHT_TORCH)
        {
            MY_LOG("ECmd_CaptureEnd: strobeMode = %d\n", m_rParam.u4StrobeMode);
            setFlashMode(FLASHLIGHT_FORCE_OFF);
            //m_rParam.u4StrobeMode = FLASHLIGHT_FORCE_OFF;
        }
        #endif
        return MTRUE;
    }
                    ...
                    {                    
                        ...
                        else //(m_i4AFSwitchCtrl == -1)
                        {
                            ...
                            if (m_i4WinState == 2)
                            {
                                ...
                                MY_LOG("Leave FD and reset window\n");
                                m_i4WinState = 0;
                                resetAFAEWindow();
                            }
                        }
