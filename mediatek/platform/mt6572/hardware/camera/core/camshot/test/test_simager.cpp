...
int main_simager(int argc, char** argv)
{
    ...
    if ((g_eImgFmt[srcFmt] == eImgFmt_YUY2) || (eImgFmt_YV16 == g_eImgFmt[srcFmt]))
    {
        rInMem.size = (g_u4SrcWidth * g_u4SrcHeight * 2 + L1_CACHE_BYTES-1) & ~(L1_CACHE_BYTES-1); 
    } 
    else if ((eImgFmt_NV21 == g_eImgFmt[srcFmt]) || (eImgFmt_I420 == g_eImgFmt[srcFmt]))
    {
        rInMem.size = (g_u4SrcWidth * g_u4SrcHeight * 3/2 + L1_CACHE_BYTES-1) & ~(L1_CACHE_BYTES-1); 
    }
    else 
    {
    rInMem.size = (g_u4SrcWidth * g_u4SrcHeight * 2 + L1_CACHE_BYTES-1) & ~(L1_CACHE_BYTES-1);     
    ...
    allocMem(rInMem); 
    loadFileToBuf(filename, reinterpret_cast<MUINT8*>(rInMem.virtAddr), rInMem.size); 
    printf("load image:%s\n", filename); 
