...
IMultiShot* g_pMultiShot;
static MUINT32 volatile		g_u4ShotConut;
static bool volatile			g_bLastImage;
static bool volatile		   	g_bShotStoped;
//static Mutex                  	g_ShotStopMtx;
static sem_t 			       g_semMShotEnd;
...
{
    MY_LOGD("fgCamShotNotifyCb"); 
    if (ECamShot_NOTIFY_MSG_CSHOT_END == msg.msgType) 
    {
        MY_LOGD("ECamShot_NOTIFY_MSG_CSHOT_END msg is received"); 
        g_bLastImage = true;
    }
    return MTRUE;
}
...
static MBOOL handleBayerDataCallback(CamShotDataInfo const msg) 
{
  ...
  /*  char fileName[256] = {'\0'}; 
    sprintf(fileName, "/data/bayer%02d.raw", g_u4ShotConut); 
...
static MBOOL handleYuvDataCallback(CamShotDataInfo const msg)
{
 ...
 /*   char fileName[256] = {'\0'}; 
    sprintf(fileName, "/data/yuv%02d.yuv", g_u4ShotConut); 
    saveBufToFile(fileName, msg.puData, msg.u4Size); 
 */
    return 0; 
}
...
static MBOOL handlePostViewCallback(CamShotDataInfo const msg)
{
  ...
  /*  char fileName[256] = {'\0'}; 
    sprintf(fileName, "/data/postview%02d.yuv", g_u4ShotConut); 
    saveBufToFile(fileName, msg.puData, msg.u4Size); 
 */
    return 0; 
}
...
static MBOOL handleJpegCallback(CamShotDataInfo const msg)
{
    ...
    if(g_bLastImage)
    {
        sprintf(fileName, "/data/jpeg%02d.jpg", g_u4ShotConut); 
       saveBufToFile(fileName, msg.puData, msg.u4Size); 
 
       memset(fileName, '\0', 256); 
       sprintf(fileName, "/data/thumb%02d.jpg", g_u4ShotConut); 
      saveBufToFile(fileName, reinterpret_cast<MUINT8*> (msg.ext1), msg.ext2); 
        MY_LOGD("CShot end, post end sem"); 
        ::sem_post(&g_semMShotEnd); 
    }
...
static MUINT32 u4Capture_Cmd(int argc, char** argv)
{
    ...
    if (g_u4Width>g_u4SensorWidth || g_u4Height>g_u4SensorHeight)
    {
    	g_u4Width = g_u4SensorWidth;
    	g_u4Height = g_u4SensorHeight;
    }
   ...
   g_bShotStoped = false;
    ...
    g_pMultiShot = IMultiShot::createInstance(eShotMode_ContinuousShot, "ContinuousShot", SHOT_CC); 
    // 
    g_pMultiShot->init(); 

    g_pMultiShot->enableNotifyMsg(ECamShot_NOTIFY_MSG_EOF); 
    // 
    g_pMultiShot->enableDataMsg(u4EnableMsg ); 
    // set buffer 
    //
#if ALLOCA_MEM
    allocateMem(u4EnableMsg); 
    //
    g_pMultiShot->registerImgBufInfo(ECamShot_BUF_TYPE_BAYER, g_rRawBufInfo); 
    g_pMultiShot->registerImgBufInfo(ECamShot_BUF_TYPE_YUV, g_rYuvBufInfo); 
    g_pMultiShot->registerImgBufInfo(ECamShot_BUF_TYPE_POSTVIEW, g_rPostViewBufInfo); 
    g_pMultiShot->registerImgBufInfo(ECamShot_BUF_TYPE_JPEG, g_rJpegBufInfo); 
#endif 
    ...
    //
    g_pMultiShot->setCallbacks(fgCamShotNotifyCb, fgCamShotDataCb, NULL); 
    //     
    g_pMultiShot->setShotParam(rShotParam); 
    //
    g_pMultiShot->setJpegParam(rJpegParam); 
    // 
    ::sem_init(&g_semMShotEnd, 0, 0);
    // 
    ret = g_pMultiShot->start(rSensorParam, u4ShotCnt); 
    
    ::sem_wait(&g_semMShotEnd); 

    g_pMultiShot->stop();
           
    ret = g_pMultiShot->uninit(); 

    //
    g_pMultiShot->destroyInstance(); 
    // 
...
static MUINT32 u4CaptureStop_Cmd(int argc, char** argv)
{
   /* Mutex::Autolock lock(mShotStopMtx);
    if(!mbShotStoped)
    {
        printf("real need stop MultiShot");
        pMultiShot->stop();
        g_bShotStoped = true;
    }*/
    return 0; 
...
*  Main Function 
********************************************************************************/
int main_multishot(int argc, char** argv)
{
