...
initBuffers()
{
    ...
    mpImgBufMgr = RecBufManager::alloc(
                    MtkCameraParameters::PIXEL_FORMAT_YUV420I,//CameraParameters::PIXEL_FORMAT_YUV420P, 
                    mi4RecWidth,
                    mi4RecHeight, 
...
prepareAllTodoBuffers(sp<IImgBufQueue>const& rpBufQueue, sp<RecBufManager>const& rpBufMgr)
{
    ...
    for(i = 0; i < mvRecBufInfo.size(); i++)
    {
        if(mvRecBufInfo[i].Sta == REC_BUF_STA_EMPTY)
        {
            ...
            MY_LOGD("enque:idx(%d),VA(0x%08X)",i,(uint32_t)(pCameraImgBuf->getVirAddr()));
            findEmpty = true;
...
handleReturnBuffers(Vector<ImgBufQueNode>const& rvQueNode)
{
    ...
    for (i = 0; i < queSize; i++)
    {
        ...
        MY_LOGD_IF(
            ...
            "CB:i(%d/%d),Idx(%d),Sta(%d),Info(0x%08X/%d/%d.%06d),CDE(%08X)", 
            i,
            queSize-1,
            j,
            rQueNode.getStatus(),
            (uint32_t)(rpQueImgBuf->getVirAddr()),
            rpQueImgBuf->getBufSize(),
