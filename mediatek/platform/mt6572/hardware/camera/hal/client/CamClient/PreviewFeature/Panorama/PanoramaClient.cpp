...
init(int bufwidth,int bufheight)
{
    ...
    if(!mpIMemDrv->init())
    {
        MY_LOGE("[init] mpIMemDrv->init error");
        return false;
    }
...
uninit()
{
    ...
    // Set AF to continuous mode for 6572 YUV imgsensor
    NS3A::Param_T cam3aParam;
    if ( ! mpHal3A->getParams(cam3aParam) )
    {
       MY_LOGE("getParams fail");
    }
    cam3aParam.u4AfMode = AF_MODE_AFC;
    if ( ! mpHal3A->setParams(cam3aParam) )
    {
       MY_LOGE("setParams fail");
    }
    MY_LOGD("Set AFC done");
    ...
    {
        mpPanoramaWorkingBuf.size = 0;
        MY_LOGE("[uninit] mpPanoramaWorkingBuf dealloc fail");
        return  MFALSE;
    }       
    if(!(deallocMem(mpMotionBuffer)))
    {
        mpMotionBuffer.size = 0;
        MY_LOGE("[uninit] mpMotionBuffer dealloc fail");
        return  MFALSE;
    }            
    ...
    if (!mpIMemDrv)
    {
        MY_LOGE("[uninit] mpIMemDrv==NULL");
    }
    else
    { 
        if (!mpIMemDrv->uninit()) 
        {
            MY_LOGE("[uninit] mpIMemDrv->uninit error");
        }
...
stopFeature(int cancel)
{
    ...
    //Unlock AE and AWB for 6572 YUV imgsensor
    mpHal3A->enableAELimiterControl(false);
    MY_LOGD("unlock AE/AWB done");
...
PanoramathreadFunc(void *arg)
{
    ...
    ::prctl(PR_SET_NAME,(unsigned long)"PanoTHREAD", 0, 0, 0);
