...
STATE_INST_DEFINITION(Recording);
STATE_INST_DEFINITION(VideoSnapshot);
...
waitState(ENState eState, nsecs_t const timeout)
{
    ...
    if  ( eState != meCurrState )
    {
        MY_LOGD_IF(
            1, 
            "<StateObserver> current/waited=%s/%s, timeout(%lld)", 
        ...
        //
        if  ( eState != meCurrState || OK != status ) {
            MY_LOGW(
                "<StateObserver> Timeout: current/waited=%s/%s, status[%s(%d)]", 
                StateManager::getStateInst(meCurrState)->getName(), 
                StateManager::getStateInst(eState)->getName(), 
