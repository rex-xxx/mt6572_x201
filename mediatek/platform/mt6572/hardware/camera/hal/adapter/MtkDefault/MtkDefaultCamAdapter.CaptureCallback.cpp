...
private:
    ...
    threadLoop()
    {
        if  ( mfgIsFinalImage )
        {
            ...
            if( pStateManager->isState(IState::eState_Recording) ||
                pStateManager->isState(IState::eState_VideoSnapshot))
            {
                CAM_LOGD("Don nothing for VR/VSS");
            }
            else
            {
                ...
                stateWaiter.waitState(IState::eState_Idle);
            }
        }

#if defined(MTK_CAMERA_BSP_SUPPORT)
        CAM_LOGD("(%d)[%s] MTK_CAMERA_MSG_EXT_DATA_COMPRESSED_IMAGE - index(%d)", ::gettid(), mpszThreadName, mu4CallbackIndex);
        mpCamMsgCbInfo->mDataCb(MTK_CAMERA_MSG_EXT_DATA, mpImage, 0, NULL, mpCamMsgCbInfo->mCbCookie);
        mpImage->release(mpImage);
#else
        CAM_LOGD("(%d)[%s] CAMERA_MSG_COMPRESSED_IMAGE - index(%d)", ::gettid(), mpszThreadName, mu4CallbackIndex);
...
)
{
    ...
    if  ( image )
    {
        ...
        pCBData[0] = MTK_CAMERA_MSG_EXT_DATA_COMPRESSED_IMAGE;
    ...
    if  ( image )
    {
        ...
        //
        sp<Thread> pThread = new ZipImageCallbackThread(mpCamMsgCbInfo, image, u4CallbackIndex, fgIsFinalImage);
        if  ( pThread == 0 || pThread->run() != OK )
        {
