...
PreviewCmdQueThread::init()
{
    ...
    if( ! (ret = mSensorInfo.init(ACDK_SCENARIO_ID_CAMERA_PREVIEW, tg_out_mode)))
    {
    ...
    if (camera_mode != 0)
    {
        mpRawDumpCmdQueThread = IRawDumpCmdQueThread::createInstance(mSensorInfo.getImgWidth(), mSensorInfo.getImgHeight(), mspParamsMgr);
