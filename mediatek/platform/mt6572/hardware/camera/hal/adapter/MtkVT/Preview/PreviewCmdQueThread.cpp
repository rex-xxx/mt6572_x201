...
private:
    ...
    vector<IhwScenario::PortQTBufInfo> mvVssImgBuf;
    #endif
};
...
PreviewCmdQueThread::init()
{
    ...
    //
    int32_t eScenarioId = ACDK_SCENARIO_ID_VIDEO_PREVIEW;
    ...
    MY_LOGD_IF(ENABLE_LOG_PER_FRAME, "Jmac VT, id(%d),orientation(%d),facing(%d),mRot(%d)", 
               gInfo.openId,cameraInfo.orientation, cameraInfo.facing, mRotation);
...
PreviewCmdQueThread::readyToRun()
{
    ...
    //
    // (1) set thread name
    ::prctl(PR_SET_NAME,(unsigned long)"PreviewCmdQueThread", 0, 0, 0);
...
updateOne()
{
        ...
        pass1LatestBufIdx >= 0)
    {
        ...
        //
        mvVssImgBuf.push_back(vDeBufPass1Out[0]);
    }
    else
    {
        if( mpVideoSnapshotScenario->getStatus() == IVideoSnapshotScenario::Status_Idle &&
            !(mvVssImgBuf.empty()))
        {
            MY_LOGD("VSS:Return buffer to pass 1");
            getHw()->enque(mvVssImgBuf);
            mvVssImgBuf.clear();
        }
    #endif
    //
