...
#define __ENABLE_SENSOR_FULL_SIZE__
#define STORED_BUFFER_CNT        (3)
//#define __ENABLE_SHORTEN_UI_DELAY__
#define __ENABLE_CAPTURE_BUFFER__
//
/******************************************************************************
*
*******************************************************************************/
...
PreviewCmdQueThread::~PreviewCmdQueThread()
{
    ...
    #ifdef __ENABLE_CAPTURE_BUFFER__
...
PreviewCmdQueThread::readyToRun()
{
    ...
    //
    // (1) set thread name
    ::prctl(PR_SET_NAME,(unsigned long)"ZsdPreviewCmdQueThread", 0, 0, 0);
...
#else
    getCfg(eID_Pass1In|eID_Pass1DispOut, vimgInfo); // zsd modified
#endif
...
#else
    getHw()->deque((EHwBufIdx)(eID_Pass1DispOut), &vDeBufPass1Out);// zsd modified
#endif
...
initCfg()
{
    ...
    IhwScenario::Rect_t DispOutSize(i4PreviewWidth, i4PreviewHeight);
...
mapQT2BufInfo(EHwBufIdx eInPort, EHwBufIdx eOutPort, vector<IhwScenario::PortQTBufInfo> const &src, vector<IhwScenario::PortBufInfo> &dst)
{
    ...
    for (it = src.begin(); it != src.end(); it++ ) {
        if ((*it).ePortIndex == eInPort) {
            MY_LOGD_IF(ENABLE_LOG_PER_FRAME, "[map] find pass 1 out buffer");
            isFindBuffer = true;
            break;
        }
    }
                    ...
                    list<CapBufQueNode> &vOutNode)
{
    ...
    MY_LOGD_IF(ENABLE_LOG_PER_FRAME, "raw cnt(%d)", vQTBuf.at(0).bufInfo.vBufInfo.size());
    ...
    for (MINT32 i =0; i < size; i++)
    {
        ...
        for (it = vInNode.begin(); it != vInNode.end(); it++ )
        {
            ...
            {
                MY_LOGD_IF(ENABLE_LOG_PER_FRAME, "map QT->Node (0x%x)", vQTBuf.at(0).bufInfo.vBufInfo[i].u4BufVA);
