...
/** CCCI driver & ioctl */
static const char CCCI_PCM_RX_DEV[NUM_MODEM][32] = {"/dev/ccci_pcm_rx", "/dev/ccci2_pcm_rx"};
static const char CCCI_PCM_TX_DEV[NUM_MODEM][32] = {"/dev/ccci_pcm_tx", "/dev/ccci2_pcm_tx"};
...
status_t SpeechMessengerCCCI::Initial()
{
    ...
    fHdlRead  = open(CCCI_PCM_RX_DEV[mModemIndex], O_RDWR);
    fHdlWrite = open(CCCI_PCM_TX_DEV[mModemIndex], O_RDWR);
    if (fHdlRead < 0 || fHdlWrite < 0) {
        ALOGE("%s() fail to open %s or %s", __FUNCTION__, CCCI_PCM_RX_DEV[mModemIndex], CCCI_PCM_TX_DEV[mModemIndex]);
    ...
    ::ioctl(fHdlRead, CCCI_IOC_SHARE_MEMORY_SIZE, &share_buf_length);
...
{
    char status = MODEM_STATUS_NOT_RUN;

    int retval = ::ioctl(fHdlRead, CCCI_IOC_GET_MODEM_STATE, &status);
    if (retval < 0) {
        ALOGE("%s() ioctl _IOR(CCCI_IOC_MAGIC, 1, unsigned int) fail!! retval = %d", __FUNCTION__, retval);
...
int32_t SpeechMessengerCCCI::SendMessage(const ccci_buff_t &ccci_buff)
{
    ...
    if (fHdlWrite < 0) {
        ALOGW("%s(), fHdlWrite < 0 : 0x%x", __FUNCTION__, fHdlWrite);
    ...
    for (i = 0; i < 150; i++) { // try 150 times for every 2 ms if sent message fail
        write_length = write(fHdlWrite, (void *)&ccci_buff, sizeof(ccci_buff));
        if (write_length == sizeof(ccci_buff)) {
            ret = NO_ERROR;
            break;
        }
        else {
...
status_t SpeechMessengerCCCI::ReadMessage(ccci_buff_t &ccci_buff)
{
    /* check if already initialized */
    if (fHdlRead < 0) {
        ALOGW("%s(), fHdlRead < 0 : 0x%x", __FUNCTION__, fHdlRead);
        if (Initial() != NO_ERROR) return UNKNOWN_ERROR;
    }

    /* read message */
    int read_length = read(fHdlRead, (void *)&ccci_buff, sizeof(ccci_buff));
    if (read_length != sizeof(ccci_buff) && GetModemCurrentStatus() != MODEM_STATUS_READY) {
        ...
        switch (m2a_message_id) {
            ...
            case MSG_M2A_SPH_ON_ACK: { // ack of MSG_A2M_SPH_ON
                ...
                if (phone_call_mode == RAT_3G324M_MODE) {
                    ...
                    pCCCI->SetModemSideRealModemStatus(VT_STATUS_MASK);
                }
                else {
                    pCCCI->SetModemSideModemStatus(SPEECH_STATUS_MASK);
                    pCCCI->SetModemSideRealModemStatus(SPEECH_STATUS_MASK);
                }
            ...
            case MSG_M2A_SPH_OFF_ACK: { // ack of MSG_A2M_SPH_OFF
            	  ALOGD("--SpeechOff Ack done, ModemSideRealModemStatus = 0x%x", pCCCI->GetAllModemSideRealModemStatus());
            ...
            case MSG_M2A_REC_DATA_NOTIFY: { // meaning that we are recording, modem have some data
                ...
                AudioResourceManager::getInstance()->EnableAudioLock(AudioResourceManagerInterface::AUDIO_STREAMINMANAGERCLIENT_LOCK, 3000);
            ...
            case MSG_M2A_CTM_ON_ACK: { // ack of MSG_A2M_CTM_ON
                ...
                pCCCI->SetModemSideRealModemStatus(TTY_STATUS_MASK);
            ...
            case MSG_M2A_SET_ACOUSTIC_LOOPBACK_ACK: { // ack of MSG_A2M_SET_ACOUSTIC_LOOPBACK
                ...
                if (loopback_on == true) { // loopback on
                    ...
                    pCCCI->SetModemSideRealModemStatus(LOOPBACK_STATUS_MASK);
                    // dump speech enhancement parameter in modem log
                    pCCCI->mLad->ModemDumpSpeechParam();
                }
                else { // loopback off
            ...
            case MSG_M2A_EM_DATA_REQUEST: { // Modem reset. Requset for all EM data(NB/DMNR/WB)
                ...
                pCCCI->ResetModemSideRealModemStatusAfterMDReset();
...
{
    const uint32_t kA2MBufLockTimeout = 10000; // 10 sec
...
{
    mModemSideModemStatus &= (~modem_status_mask);
    ResetModemSideRealModemStatus(modem_status_mask);
...
void SpeechMessengerCCCI::SetModemSideRealModemStatus(const modem_status_mask_t modem_status_mask)
{
    mModemSideRealModemStatus |= modem_status_mask;
}

void SpeechMessengerCCCI::ResetModemSideRealModemStatus(const modem_status_mask_t modem_status_mask)
{
    mModemSideRealModemStatus &= (~modem_status_mask);
}

void SpeechMessengerCCCI::ResetModemSideRealModemStatusAfterMDReset()
{
    ALOGD("%s(), Reset Speech,VT,TTY,LOOPBACK  Modem side status", __FUNCTION__);
    mModemSideRealModemStatus = 0;
}

uint32_t SpeechMessengerCCCI::GetAllModemSideRealModemStatus()
{
    return mModemSideRealModemStatus;
}

bool SpeechMessengerCCCI::GetModemSideRealModemStatus(const modem_status_mask_t modem_status_mask) const
{
    return ((mModemSideRealModemStatus & modem_status_mask) > 0);
}
