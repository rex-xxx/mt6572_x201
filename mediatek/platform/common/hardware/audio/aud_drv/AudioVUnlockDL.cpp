...
AudioVUnlockDL::~AudioVUnlockDL()
{
   /* if (mRingBufIn!=NULL)
    ...
    if (mpSrcHdl != NULL )
    {
        BLI_Close(mpSrcHdl, NULL);
    }
}
int32_t AudioVUnlockDL::GetUplinkSystemTime()
{
    if(mULtime.tv_sec  == 0 && mULtime.tv_nsec == 0 )
    {
        clock_gettime(CLOCK_MONOTONIC, &mULtime);
        SXLOGD("[GetUplinkSystemTime] mULtime sec %ld nsec %ld", mULtime.tv_sec, mULtime.tv_nsec);
    }
    else
    {
        SXLOGD("[GetUplinkSystemTime] already get UL time :mULtime sec %ld nsec %ld", mULtime.tv_sec, mULtime.tv_nsec);
    }
    return 0;
                    ...
                    uint32_t inChannel             /* Input, input channel number of the conversion */)
{
    ...
    {
        BLI_GetMemSize( mInputSampleRate, mInChannel, mOutputSampleRate, mOutChannel, &mSrcBufLen);
        mSRCbuffer = new char[mSrcBufLen];
        mpSrcHdl = BLI_Open( mInputSampleRate, mInChannel, mOutputSampleRate, mOutChannel, mSRCbuffer, NULL);
    }
    if ( !mpSrcHdl ) 
    {
                    ...
                    uint32_t outChannel            /* Input, output channel number of the conversion */)
{
    ...
    //set blisrc
    if(mpSrcHdl == NULL && mInputSampleRate != 0 && mInChannel != 0 && mOutputSampleRate != 0 && mOutChannel != 0)
    {
        BLI_GetMemSize( mInputSampleRate, mInChannel, mOutputSampleRate, mOutChannel, &mSrcBufLen);
        mSRCbuffer = new char[mSrcBufLen];
        mpSrcHdl = BLI_Open( mInputSampleRate, mInChannel, outSR, outChannel, mSRCbuffer, NULL);
    }
    if ( !mpSrcHdl ) 
    {    
...
int32_t AudioVUnlockDL:: DoSRC(uint8_t* inbuf, uint32_t *datasz , uint8_t* outbuf, uint32_t *outlength)
{
    ...
    while(count)
    {
        //SXLOGD("count %d ,in offset %d, datasz %d, out offset %d, outlength %d ", count,inPtr - inbuf, *datasz,outPtr - outbuf, *outlength);
        dataconsumed += BLI_Convert(mpSrcHdl, (int16_t*)inPtr, datasz, (int16_t*)outPtr, outlength);
        //SXLOGD("after count %d , datasz %d,  outlength%d ", count,  *datasz,*outlength);
...
int32_t AudioVUnlockDL::WriteStreamOutToRing(const void* buf, uint32_t datasz)
{
    ...
    if(mGetTime == true && mInputStandby == false)
    {
        ...
        SXLOGD("[WriteStreamOutToRing] get DL time: mNewDLtime.tv_sec %d, mNewDLtime.tv_nsec %d", mNewDLtime.tv_sec,mNewDLtime.tv_nsec);
        mGetTime = false;
        if(mDLtime.tv_sec != 0)
        {
            mInRemaining = mRingBufIn.GetBufDataSz();
            mOutRemaining = mRingBufOut.GetBufDataSz();
            SXLOGD("[WriteStreamOutToRing] input buf never cleared IN remaining %d, Out remaining %d",mInRemaining,mOutRemaining);
        }
    }
...
int32_t AudioVUnlockDL::ReadRefFromRing(void*buf, uint32_t datasz, void* DLtime)
{
    ...
    while(count)
    {
        ...
        *ptr =  mDLtime;        
        ...
        if(mInputStandby/* && StateInputStart()*/)
        {
            if((mRingBufOut.GetBufDataSz() !=0 || mRingBufIn.GetBufDataSz() !=0) && count ==1 )
            {   
                count ++;                
...
bool AudioVUnlockDL::startInput()
{
...
#ifdef DUMP_VPW_StreamIn_DATA
   if( mOutFile == NULL)
   mOutFile = fopen("/system/VPW_In.pcm","wb");
   if( mOutFile == NULL)
   {
   ...
   if( mOutFile_1 == NULL)
   mOutFile_1 = fopen("/system/VPW_SRC.pcm","wb");
   if( mOutFile_1 == NULL)
   {
   ...
   if( mOutFile_2 == NULL)
   mOutFile_2 = fopen("/system/VPW_Out.pcm","wb");
   if( mOutFile_2 == NULL)
   {
