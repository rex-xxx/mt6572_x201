...
static void btif_bbs_wr_byte_chk(p_btif_buf_str p_bbs, unsigned char val);
...
static int g_max_pkg_len = 4096;
static int g_max_pding_data_size = BTIF_RX_BUFFER_SIZE / 2;
...
const struct platform_driver mtk_btif_dev_drv = {
...
int btif_clock_ctrl(p_mtk_btif p_btif, int en)
{
	...
	if (BTIF_MODE_DMA == p_btif->rx_mode)
		i_ret += hal_btif_dma_clk_ctrl(p_btif->p_rx_dma, ctrl_flag);

	if (BTIF_MODE_DMA == p_btif->tx_mode)
		i_ret += hal_btif_dma_clk_ctrl(p_btif->p_tx_dma, ctrl_flag);
...
int btif_rx_cb_reg(p_mtk_btif p_btif, MTK_WCN_BTIF_RX_CB rx_cb)
{
	if (p_btif->rx_cb) {
		BTIF_WARN_FUNC
		    ("rx cb already exist, rewrite from (0x%x) to (0x%x)\n",
		     p_btif->rx_cb, rx_cb);
	}
	p_btif->rx_cb = rx_cb;
...
/*vFIFO memory allocation*/
	p_vfifo->p_vir_addr = dma_zalloc_coherent(NULL,
						  p_vfifo->vfifo_size,
						  &p_vfifo->phy_addr, GFP_DMA);
	if (NULL == p_vfifo->p_vir_addr) {
		BTIF_ERR_FUNC("alloc vFIFO memory for BTIF failed\n");
		return E_BTIF_FAIL;
	}
	BTIF_INFO_FUNC
	    ("alloc vFIFO for BTIF succeed,vir addr:0x%08x, phy addr:0x%08x\n",
	...
	do {
		if (0 < length) {
			...
			if (p_btif->rx_cb) {
				...
				BTIF_WARN_FUNC
				    ("p_btif:0x%08x, both rx_notify and rx_cb are NULL\n",
				     p_btif);
				break;
			}
		} else {
...
{
/*Here we check if available size ,
    left 1 bit to prevent from read index and write index are equal to each other*/
