...
void ccci_check_md_no_physical_channel(int md_id, unsigned int args)
{
	...
	if (exp_ee_info->exception_occur){
		ee_info = exp_ee_info->exception_occur << MD_EE_INFO_OFFSET;
...
int ccci_start_modem(int md_id)
{
	...
	if(ctl_b->ipo_h_restore) {
		...
		CCCI_MSG_INF(md_id, "ctl", "re-load firmware for IPO-H!\n");
...
int send_enter_flight_mode_request(int md_id)
{
	...
	ctl_b = md_ctlb[md_id];
	if(ctl_b == NULL) {	
		CCCI_MSG_INF(md_id, "ctl", "enter filght mode fail @md_ctlb NULL\n");
		return -CCCI_ERR_FATAL_ERR;
	}
	if(ctl_b->md_boot_stage != MD_BOOT_STAGE_2){
		CCCI_MSG_INF(md_id, "ctl", "enter filght mode fail @md_stage_%d\n", ctl_b->md_boot_stage);
