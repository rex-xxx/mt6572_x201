...
unsigned int tty_debug_enable[MAX_MD_NUM] = {0x0}; 
//1UL<<0, tty_modem; 1UL<<1, tty_meta; 1UL<<2, tty_rpc

static void ccci_tty_read(unsigned long arg)
{
	int				part, size, accept, ret, push_pending = 0;
	int size_bk = 0;
	int accept_reversed, accept_ordered = 0;
	unsigned		read, write;
  ...
  read_lock_bh(&tty_instance->ccci_tty_rwlock);
	...
	if (read > write) {
		...
		accept_reversed = tty_insert_flip_string(tty_instance->tty, &rx_buffer[read], part);

		if (accept_reversed < part) {
			size -= accept_reversed;
			read += accept_reversed;
			push_pending = 1;
	...
	accept_ordered = tty_insert_flip_string(tty_instance->tty, &rx_buffer[read], size);

	if (accept_ordered < size) {
		size -= accept_ordered;
		read += accept_ordered;
		push_pending = 1;
	}
...
__ccci_read_ack:
	...
	if (push_pending)
	{
		CCCI_DBG_MSG(md_id, "tty", "push pending, total=%x left=%x\n", size_bk, size_bk-accept_reversed-accept_ordered);
	...
	if(tty_debug_enable[md_id] & (1UL << tty_instance->tty->index))
		CCCI_DBG_MSG(md_id, "tty", "[after  Read]:[RX] tty=%04d data_len=%04d consumed=%04d write=%04d read=%4d\n",
			tty_instance->tty->index, size_bk, accept_reversed+accept_ordered, tty_instance->shared_mem->rx_control.write, 
...
static int ccci_tty_open(struct tty_struct *tty, struct file *f)
{
	...
	if(tty_instance->count > 1){
		...
		snprintf(tty_instance->wakelock_name, sizeof(tty_instance->wakelock_name), "%s%d", name, (md_id+1));
		wake_lock_init(&tty_instance->wake_lock, WAKE_LOCK_SUSPEND, tty_instance->wakelock_name);
...
static void tty_timer_func(unsigned long data)
{
...
int ccci_tty_init(int md_id)
{
	...
	setup_timer(&ctlb->ccci_tty_meta.timer, tty_timer_func, (unsigned long)&ctlb->ccci_tty_meta);
	...
	setup_timer(&ctlb->ccci_tty_modem.timer, tty_timer_func, (unsigned long)&ctlb->ccci_tty_modem);
	...
	setup_timer(&ctlb->ccci_tty_ipc.timer, tty_timer_func, (unsigned long)&ctlb->ccci_tty_ipc);
