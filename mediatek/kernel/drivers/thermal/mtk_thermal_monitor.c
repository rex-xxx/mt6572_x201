...
#else
#if defined(CONFIG_ARCH_MT6577) || defined(CONFIG_ARCH_MT6572)
...
#define CONFIG_MTK_THERMAL_TIME_BASE_PROTECTION //low cost project uses PMIC charging, enable it always
#endif
...
#if defined(CONFIG_ARCH_MT6589) || defined(CONFIG_ARCH_MT6572)
...
static int mtk_sysinfo_get_info(void)
{
    ...
    nBattCurrentCnsmpt = get_sys_battery_info("/sys/devices/platform/battery/FG_Battery_CurrentConsumption");
    // the return value is 0.1mA
    if (nBattCurrentCnsmpt%10 <5)
        nBattCurrentCnsmpt /= 10;
    else
