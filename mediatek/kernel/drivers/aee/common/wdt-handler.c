...
static void aee_save_reg_stack_sram(void)
{
	...
	for (i = 0; i< num_possible_cpus(); i++) {
		...
		snprintf(str_buf, sizeof(str_buf), 
				...
				((wdt_percpu_preempt_cnt[i] & PREEMPT_MASK) >> PREEMPT_SHIFT), 
				((wdt_percpu_preempt_cnt[i] & SOFTIRQ_MASK) >> SOFTIRQ_SHIFT), 
				((wdt_percpu_preempt_cnt[i] & HARDIRQ_MASK) >> HARDIRQ_SHIFT));
		aee_sram_fiq_log(str_buf);
	}
}
