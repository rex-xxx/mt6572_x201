...
static UCHAR WMT_QUERY_STP_EVT[] = {0x02, 0x04, 0x06, 0x00, 0x00, 0x04, 0xDF, 0x0E, 0x68, 0x01};
...
static UCHAR WMT_PATCH_ADDRESS_CMD[] = {0x01,0x08,0x10,0x00,0x01,0x01,0x00,0x01,0x3c,0x02,0x09,0x02,0x00,0x00,0x00,0x00,0xff,0xff,0xff,0xff};
static UCHAR WMT_PATCH_ADDRESS_EVT[] = {0x02,0x08,0x04,0x00,0x00,0x00,0x00,0x01};
static UCHAR WMT_PATCH_P_ADDRESS_CMD[] = {0x01,0x08,0x10,0x00,0x01,0x01,0x00,0x01,0xc4,0x04,0x09,0x02,0x00,0x3f,0x00,0x01,0xff,0xff,0xff,0xff};
...
static UCHAR  WMT_CORE_START_RF_CALIBRATION_CMD[] = {0x1,0x14,0x1,0x00,0x01};
static UCHAR  WMT_CORE_START_RF_CALIBRATION_EVT[] = {0x2,0x14,0x02,0x00,0x00,0x01};
    ...
    )
{
	...
	ctrlData.au4CtrlData[1] = (UINT32)gp_soc_info->cChipVersion;
	ctrlData.au4CtrlData[2] = (UINT32)&gp_soc_patch_info;
    ...
    )
{
    ...
    /* full match */
    while ( (0 <= index)
        && (hw_ver != mtk_wcn_soc_info_table[index].u4HwVer) /* full match */
        ) {
        --index;
    }
    if (0 <= index) {
