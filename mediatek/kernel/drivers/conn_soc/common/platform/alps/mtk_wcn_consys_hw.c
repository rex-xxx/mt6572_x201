...
********************************************************************************
*/
UINT32 gConsysHwChipId = 0;
volatile UINT8 __iomem *pEmibaseaddr = NULL;
...
{
    INT32 iRet = 0;
	WMT_INFO_FUNC("CONSYS-HW-GPIO-CTRL(0x%08x), start\n",on);
	...
	if(on)
	{
	    ...
	    WMT_INFO_FUNC("CONSYS-HW, BGF IRQ registered and disabled \n");
	    ...
	    WMT_INFO_FUNC("CONSYS-HW, BGF IRQ unregistered and disabled\n");
	...
	WMT_INFO_FUNC("CONSYS-HW-GPIO-CTRL(0x%08x), finish\n",on);
    return iRet;
...
INT32 mtk_wcn_consys_hw_pwr_off (VOID)
{
	...
	WMT_INFO_FUNC("CONSYS-HW-PWR-OFF, start\n");

	iRet += mtk_wcn_consys_hw_reg_ctrl(0,0);
	iRet += mtk_wcn_consys_hw_gpio_ctrl(0);

	WMT_INFO_FUNC("CONSYS-HW-PWR-OFF, finish(%d)\n",iRet);
	return iRet;
}

INT32 mtk_wcn_consys_hw_chipid(VOID)
{
	return gConsysHwChipId;
}
...
{
    INT32 iRet = 0;
    WMT_INFO_FUNC("CONSYS-HW, hw_rst start, eirq should be disabled before this step\n");
    ...
    WMT_INFO_FUNC("CONSYS-HW, hw_rst finish, eirq should be enabled after this step\n");
    return iRet;
}
...
#endif
		WMT_INFO_FUNC("WMT do BT PMIC on\n");
	}else{
		/*do BT PMIC off*/
...
#endif
		WMT_INFO_FUNC("WMT do BT PMIC off\n");
	}
	return 0;
}
...
#endif
		WMT_INFO_FUNC("WMT do WIFI PMIC on\n");
	}else{
		/*do WIFI PMIC off*/
...
#endif
		WMT_INFO_FUNC("WMT do WIFI PMIC off\n");
	}
	return 0;
}
...
INT32 mtk_wcn_consys_hw_vcn28_ctrl(UINT32 enable)
{
	if(enable){
		...
		hwPowerOn(MT6323_POWER_LDO_VCN28,VOL_DEFAULT,"MOD_WMT");
#endif
		WMT_INFO_FUNC("turn on vcn28 for fm usage in co-clock mode\n");
	}else{
		/*in co-clock mode,need to turn off vcn28 when fm off*/
#if CONSYS_PMIC_CTRL_ENABLE 
		hwPowerDown(MT6323_POWER_LDO_VCN28,"MOD_WMT");
#endif
		WMT_INFO_FUNC("turn off vcn28 for fm usage in co-clock mode\n");
	}
	return 0;
}
...
INT32 mtk_wcn_consys_hw_init()
{
	...
	if(pEmibaseaddr)
	{
		WMT_INFO_FUNC("EMI mapping OK(0x%p)\n",pEmibaseaddr);
		osal_memset(pEmibaseaddr,0,CONSYS_EMI_MEM_SIZE);
		iRet = 0;
	}else{
		WMT_ERR_FUNC("EMI mapping fail\n");
	}
...
UINT8  *mtk_wcn_consys_emi_ctrl_state_addr_get(ENUM_EMI_CTRL_STATE_OFFSET ctrl_state_offset)
{
	UINT8 *p_virtual_addr = NULL;
	...
	WMT_DBG_FUNC("ctrl_state_offset(%08x)\n",ctrl_state_offset);
