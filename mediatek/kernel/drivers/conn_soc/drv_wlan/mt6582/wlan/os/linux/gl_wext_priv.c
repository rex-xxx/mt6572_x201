    ...
    )
{
    ...
    case PRIV_CMD_POWER_MODE:
            ...
            if (pu4IntBuf[1] & 0xf0) {
                if (pu4IntBuf[1] == 0xff) {
                    prGlueInfo->u4ManualPwrState = 0;
                    DBGLOG(REQ, INFO, ("Clear manual power mode setting\n"));
                    break;
                } else {
                    prGlueInfo->u4ManualPwrState = pu4IntBuf[1] & 0xff;
                    pu4IntBuf[1] &= 0xf;
                    DBGLOG(REQ, INFO, ("Set to manual power mode\n"));
                }
            }
