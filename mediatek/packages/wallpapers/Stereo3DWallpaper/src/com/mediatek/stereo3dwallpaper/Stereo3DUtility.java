...
public final class Stereo3DUtility {
    ...
    public static int decodeMpoUri(ContentResolver resolver, Uri uri, Bitmap[] bitmapArray) {
        ...
        return decodeMpo(mpoDecoder, bitmapArray);
    }
     ...
     * @return true if MPO contains two views, false, otherwise.
     */
    private static int decodeMpo(IMpoDecoder mpoDecoder, Bitmap[] bitmapArray) {
        int result = -1;
        ...
        if (mpoDecoder != null) {
            ...
            while (index < 2) {
                ...
                if (bitmapArray[index] != null) {
                    ...
                    bitmapArray[index] = scaleHalfSideImage(bitmapArray[index]);
    ...
    public static void decodeJpsUri(ContentResolver resolver, Uri uri, Bitmap[] bitmapArray) {
        ...
        try {
            ...
            if (bitmapL != null && bitmapR != null) {
                bitmapArray[0] = scaleHalfSideImage(bitmapL);
                bitmapArray[1] = scaleHalfSideImage(bitmapR);
     ...
     */
    private static Bitmap scaleHalfSideImage(Bitmap bitmap) {
        Stereo3DLog.log(TAG, "Scale half side image");
        int width = bitmap.getWidth();
        int height = bitmap.getHeight();

        float scaleW = (float)sScreenMaxSide / (float) width;
        float scaleH = (float)sScreenMaxSide / (float) height;
