        ...
        if (FeatureOption.MTK_GEMINI_SUPPORT == true)
        {
            if (mPhone != null && !mPhoneStateChangeReg)
            {
                ((GeminiPhone)mPhone).registerForPreciseCallStateChangedGemini(mCallHandler, 
                    PHONE_STATE_CHANGED, null, PhoneConstants.GEMINI_SIM_1);
                ((GeminiPhone)mPhone).registerForPreciseCallStateChangedGemini(mCallHandler2, 
                    PHONE_STATE_CHANGED, null, PhoneConstants.GEMINI_SIM_2);
                if (FeatureOption.MTK_GEMINI_3SIM_SUPPORT) {
                    ((GeminiPhone)mPhone).registerForPreciseCallStateChangedGemini(mCallHandler3, PHONE_STATE_CHANGED, null, PhoneConstants.GEMINI_SIM_3);
                    ((GeminiPhone)mPhone).registerForSuppServiceFailedGemini(mCallHandler, SUPP_SERVICE_FAILED, null, PhoneConstants.GEMINI_SIM_3);
                }
                if (FeatureOption.MTK_GEMINI_4SIM_SUPPORT) {
                    ((GeminiPhone)mPhone).registerForPreciseCallStateChangedGemini(mCallHandler3, PHONE_STATE_CHANGED, null, PhoneConstants.GEMINI_SIM_3);
                    ((GeminiPhone)mPhone).registerForSuppServiceFailedGemini(mCallHandler, SUPP_SERVICE_FAILED, null, PhoneConstants.GEMINI_SIM_3);
                    ((GeminiPhone)mPhone).registerForPreciseCallStateChangedGemini(mCallHandler4, PHONE_STATE_CHANGED, null, PhoneConstants.GEMINI_SIM_4);
                    ((GeminiPhone)mPhone).registerForSuppServiceFailedGemini(mCallHandler2, SUPP_SERVICE_FAILED, null, PhoneConstants.GEMINI_SIM_4);
                }
                ((GeminiPhone)mPhone).registerForSuppServiceFailedGemini(mCallHandler, SUPP_SERVICE_FAILED, null, PhoneConstants.GEMINI_SIM_1);
                ((GeminiPhone)mPhone).registerForSuppServiceFailedGemini(mCallHandler2, SUPP_SERVICE_FAILED, null, PhoneConstants.GEMINI_SIM_2);
...
//              msg.title = mstrbackDisplayTitle;
        ...
        case SELECT_ITEM:
            ...
            if (FeatureOption.MTK_GEMINI_SUPPORT == true)
            {
                ...
                if (iTel != null){
                    try{
                        /* TODO: Gemini and non-Gemini are different begine */
                        if(iTel.isRadioOnGemini(sim_id) == true) {
                        /* TODO: Gemini and non-Gemini are different end */
                            if(mStkContext[sim_id].mMenuIsVisible == true || mStkContext[sim_id].isUserAccessed == false) {
            ...
            if (FeatureOption.MTK_GEMINI_SUPPORT == true) {
                try {
                    ...
                    if (phone != null) {            
                        ...
                        {
                            radio_on = phone.isRadioOnGemini(sim_id);
                        }
    ...
    private void processPhoneStateChanged(int sim_id) {
        ...
        if(mStkContext[sim_id].mCurrentCmd != null ) {
            ...
            case SET_UP_CALL: 
                ...
                Call fg;
                if (FeatureOption.MTK_GEMINI_SUPPORT == true)
                    fg = ((GeminiPhone)mPhone).getForegroundCallGemini(sim_id);
                else
    ...
    private Call.State getBackgroundCallState(int sim_id) {
        ...
        Call bg;
        if (FeatureOption.MTK_GEMINI_SUPPORT == true)
            bg = ((GeminiPhone)mPhone).getBackgroundCallGemini(sim_id);
        else
    ...
    private Call.State isPhoneIdle(int SIMid) {
        ...
        Call fg = null;
        if (FeatureOption.MTK_GEMINI_SUPPORT == true)
            fg = ((GeminiPhone)mPhone).getForegroundCallGemini(SIMid);
        else
            fg = mPhone.getForegroundCall();
        /* TODO: Gemini and non-Gemini are different end */
        if(fg != null ) {
        ...
        Call bg = null;
        if (FeatureOption.MTK_GEMINI_SUPPORT == true)
            bg = ((GeminiPhone)mPhone).getBackgroundCallGemini(SIMid);
        else
            bg = mPhone.getBackgroundCall();
        /* TODO: Gemini and non-Gemini are different end */
        if(bg != null ) {
        ...
        Call ring = null;
        if (FeatureOption.MTK_GEMINI_SUPPORT == true)
            ring = ((GeminiPhone)mPhone).getRingingCallGemini(SIMid);
        else
    ...
    private void processSetupCall(int sim_id) {
        ...
        boolean isDualTalkMode = PhoneFactory.isDualTalkMode();
    ...
    private void processHoldCallResponse(int sim_id) {
        ...
        case ACTIVE:
            ...
            try {
                ...
                /* TODO: Gemini and non-Gemini are different begine */
                if (FeatureOption.MTK_GEMINI_SUPPORT == true)
                    ((GeminiPhone)mPhone).switchHoldingAndActiveGemini(sim_id);
                else
                    mPhone.switchHoldingAndActive();
                /* TODO: Gemini and non-Gemini are different end */
        ...
        default:
            ...
            if(is1A1H(sim_id)) {
                try {
                    ...
                    /* TODO: Gemini and non-Gemini are different begine */
                    if (FeatureOption.MTK_GEMINI_SUPPORT == true)
                        ((GeminiPhone)mPhone).hangupAllGemini(sim_id);
                    else
                        mPhone.hangupAll();
                    /* TODO: Gemini and non-Gemini are different end */
                } catch (Exception ex) {
