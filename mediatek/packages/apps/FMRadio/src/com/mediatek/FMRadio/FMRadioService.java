...
public class FMRadioService extends Service implements FMRecorder.OnRecorderStateChangedListener {
    ...
    private class FMServiceBroadcastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (ACTION_TOFMSERVICE_POWERDOWN.equals(action) || ACTION_FROMATVSERVICE_POWERUP.equals(action)
                ...
                /*
                 * if (!mIsPowerUp) { // If not even powered up, just return and
                 * do nothing LogUtils.w(TAG,
                 * "ACTION_HEADSET_PLUG: FM not powered up yet!!"); return; }
                 */
    ...
    public boolean powerDown() {
        ...
        mIsPowerUp = false;
        // notifyActivityStateChanged(FMRadioListener.LISTEN_POWERUP_CHANGED);
    ...
    private int[] startScan() {
        ...
        if (mIsStopScanCalled) {
            ...
            mIsStopScanCalled = false;
            // if powerup state, should resume fm audio
            resumeFMAudio();
            setMute(false);
        } else {
    ...
    public boolean stopScan() {
        ...
        if (mIsNativeScanning || mIsSeeking) {
    ...
    private void startRecording() {
        ...
        LogUtils.d(TAG, "default sd card file path: " + mDefaultSDCardPath);
        if (mFMRecorder == null) {
...
//        initService(iCurrentStation);
                ...
                while (true) {
                    ...
                    if (RDS_EVENT_AF == (RDS_EVENT_AF & iRDSEvents)) {
                        ...
                        if (FMRadioUtils.isValidStation(iFreq)) {
                            // Valid alternative frequency. Should update
                            // the current station if
                            // the new frequency is not equal to current
                            // frequency.
                            if (mCurrentStation == iFreq) {
                                LogUtils.w(TAG, "Error: the new frequency is the same as current.");
                            } else {
                                setPS("");
                                setLRText("");
                                ...
                                updateNotification();
                                tuneStationAsync(FMRadioUtils.computeFrequency(mCurrentStation));
                                FMRadioStation.setCurrentStation(mContext, mCurrentStation);
                                Bundle bundle = new Bundle(2);
                                bundle.putInt(FMRadioListener.CALLBACK_FLAG, FMRadioListener.LISTEN_RDSSTATION_CHANGED);
                                bundle.putInt(FMRadioListener.KEY_RDS_STATION, mCurrentStation);
                ...
                public void onReceive(Context context, Intent intent) {
                    ...
                    if (Intent.ACTION_MEDIA_EJECT.equals(action) || Intent.ACTION_MEDIA_UNMOUNTED.equals(action)) {
                        ...
                        if (isRecordingCardUnmount(intent)) {
                            ...
                            bundle.putInt(FMRadioListener.KEY_RECORDING_STATE, FMRecorder.STATE_IDLE);
                            notifyActivityStateChanged(bundle);
                        }
                        return;
                    }
    ...
    private void stopFMFocusLoss(int focusState) {
        ...
        // avoid not execute stop recording immediately, sound recorder will abnormal.
        if (FeatureOption.MTK_FM_RECORDING_SUPPORT && mFMRecorder != null) {
            int fmState = mFMRecorder.getState();
     ...
     */
    private void updateAudioFocusAync(int focusState) {
        final int bundleSize = 1;
        mFmServiceHandler.removeMessages(FMRadioListener.MSGID_AUDIOFOCUS_CHANGED);
        Bundle bundle = new Bundle(bundleSize);
    ...
    public boolean isRecordingCardUnmount(Intent intent) {
        ...
        return unmountSDCard.equalsIgnoreCase("file://" + mDefaultSDCardPath) ? true : false;
    }
    ...
    class FmRadioServiceHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            case FMRadioListener.MSGID_POWERUP_FINISHED:
                ...
                isSwitch = (switchAntenna(1) == 0) ? true : false;

                // Antenna not ready, try short antenna,
                // and if not switch antenna success, return to show dialog
                if (!isAntennaAvailable() && !isSwitch) {
                    ...
                    bundle.putBoolean(FMRadioListener.KEY_IS_SWITCH_ANNTENNA, isSwitch);
                    notifyActivityStateChanged(bundle);
                    return;
                }
            ...
            case FMRadioListener.MSGID_SWITCH_ANNTENNA:
                bundle = msg.getData();
                isSwitch = (switchAntenna(bundle.getInt(FMRadioListener.SWITCH_ANNTENNA_VALUE)) == 0) ? true : false;
            ...
            case FMRadioListener.MSGID_SEEK_FINISHED:
                ...
                // boolean isSeek = (seekStation == -1 ? false : true);
