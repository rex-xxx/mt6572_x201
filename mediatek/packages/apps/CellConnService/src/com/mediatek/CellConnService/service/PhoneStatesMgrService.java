...
/**************************************************/
    ...
    public void onStart(Intent intent, int startId) {
        ...
        if (START_TYPE_RSP.equals(intent.getStringExtra(START_TYPE))) {
            ...
            if (0 != confirmType) {
                ...
                case CONFIRM_TYPE_PIN:
                    if (CONFIRM_RESULT_OK == confirmRet) {
                        if (PHONE_STATE_PUK1LOCKED == mPhoneStates[reqItem.getReqSlot()] 
                                || pukRequest(reqItem.getReqSlot())) {
                            showVerifyDlg(reqItem, VERIFY_TYPE_PUK);
                        } else {
                            showVerifyDlg(reqItem, VERIFY_TYPE_PIN);
                        }
    ...
    private void showConfirmDlg(RequestItem reqItem, int confirmType) {
        ...
        SIMInfo simInfo;
        if (true == FeatureOption.MTK_GEMINI_SUPPORT) {
            simInfo = SIMInfo.getSIMInfoBySlot(getBaseContext(), reqItem.getReqSlot());
            if (null == simInfo) {
    ...
    private void showVerifyDlg(RequestItem reqItem, int verifyType) {
        ...
        it.setAction(ACTION_UNLOCK_SIM_LOCK);
        it.putExtra(EXTRA_SIM_SLOT, slot);
        it.putExtra(EXTRA_UNLOCK_TYPE, verifyType);
        ...
        if(verifyType == VERIFY_TYPE_SIMMELOCK) {
            ...
            if (true == FeatureOption.MTK_GEMINI_SUPPORT) {
                ...
                nMELockType = mGeminiPhone.getIccCardGemini(slot).getNetworkPersoType();
            } else {
            ...
            it.putExtra(EXTRA_SIMME_LOCK_TYPE, nMELockType);
        }
            ...
            public void handleMessage(Message msg) {
                ...
                case GET_MELOCK_RETRYCOUNT:
                {
                    ...
                    if (true == FeatureOption.MTK_GEMINI_SUPPORT) {
                        ...
                        mGeminiPhone.getIccCardGemini(slot).QueryIccNetworkLock(nMELockType, 0,
                                null, null, null, null, 
