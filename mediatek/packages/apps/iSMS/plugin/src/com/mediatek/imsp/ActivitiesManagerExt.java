    ...
    public void startRemoteActivity(Context context, Intent intent) {
        ...
        switch (actionIndex) {
            ...
            case RemoteActivities.ID_ACTIVITION: {   // activition
                int simId = intent.getIntExtra(IpMessageConsts.RemoteActivities.KEY_SIM_ID, NmsConsts.INVALID_SIM_ID);
                NmsStartActivityApi.nmsStartActivitionActivity(context, (long) simId);
                break;
            }
            
            case RemoteActivities.ID_LOCATION: {   // location
                int requestCode = intent.getIntExtra(IpMessageConsts.RemoteActivities.KEY_REQUEST_CODE, 0);
            ...
            case RemoteActivities.ID_CHAT_DETAILS_BY_THREAD_ID: {   // chat_details_by_thread_id
                long threadId = intent.getLongExtra(IpMessageConsts.RemoteActivities.KEY_THREAD_ID, 0);
                long messageId = intent.getLongExtra(IpMessageConsts.RemoteActivities.KEY_MESSAGE_ID, -1);
                boolean needShowLoadAllMsg = intent.getBooleanExtra(IpMessageConsts.RemoteActivities.KEY_BOOLEAN, false);
                boolean needNewTask = intent.getBooleanExtra(IpMessageConsts.RemoteActivities.KEY_NEED_NEW_TASK, true);
                NmsStartActivityApi.nmsStartChatDetailsActivity(context, threadId, messageId, needShowLoadAllMsg, needNewTask);
                break;
            }
            ...
            case RemoteActivities.ID_CONTACT: {   // contact_selection
                ...
                if (type == IpMessageConsts.SelectContactType.ALL) {
                    NmsStartActivityApi.nmsStartAllContactSelectionActivityForResult(context, requestCode);
                } else if (type == IpMessageConsts.SelectContactType.IP_MESSAGE_USER) {
                    NmsStartActivityApi.nmsStartContactSelectionActivityForResult(context, pluginContext, requestCode);
                } else if (type == IpMessageConsts.SelectContactType.NOT_IP_MESSAGE_USER) {
                    NmsStartActivityApi.nmsStartNoniSMSContactSelectionActivityForResult(context, requestCode);
                } else {
            ...
            case RemoteActivities.ID_NEW_GROUP_CHAT: {   // new_group_chat
                ...
                String[] contactIdStrs = intent.getStringArrayExtra(IpMessageConsts.RemoteActivities.KEY_ARRAY);
