...
public class NmsChatDetailsActivity extends NmsShareBaseActivity {
    ...
    private void sendImage(int requestCode, Handler handler) {
        ...
        if (NmsConfig.getSendAsSMSFlag() && mContact.getType() != NmsContactType.HISSAGE_GROUP_CHAT) {
            msg.send(mContact.getNumber(), mDstPath, "",
            ...
            msg.send(mContact.getNumber(), mDstPath, "",
                    (int) NmsPlatformAdapter.getInstance(mContext).getCurrentSimId(),
                    NmsIpMessageConsts.NmsIpMessageSendMode.NORMAL, mContact.getId(),false, handler);
        }
    }
    ...
    public void onDestroy() {
        NmsLog.error(TAG, "fuck onDestory on memeory leak 7777777");
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        case GET_FORWARD_CONTACT:
            ...
            if (!TextUtils.isEmpty(contactStr)) {
                if (NmsSMSMMSManager.getInstance(mContext)
                        .isExtentionFieldExsit() == 1) {
    ...
    private void updateTitle() {
        ...
        NmsChatSettings chatSetting = new NmsChatSettings(mContext, mContactId);
        if (null != chatSetting && chatSetting.isMute()) {
        ...
        if (mNeedShowLoadMsg) {
            if (importantCount != 0) {
                ...
                if (allId.length == importantCount) {
                    ...
                    mChatListAdapter = new NmsChatListAdapter(mContext, mContactId, NmsImportantList
                            .get().getIdList());
                }
            } else {
                mChatListAdapter = new NmsChatListAdapter(mContext, mContactId, null);
    ...
    private void initMenu(Menu menu) {
        ...
        if (mContact.getType() == NmsContactType.HISSAGE_GROUP_CHAT) {
            ...
            if(gc.isAlive()&& gc.getMemberIds()!= null 
            		&& MessageUtils.isCurrentSim(mContext, gc.getSimId()) 
            		&& (NmsIpMessageApiNative.nmsGetActivationStatus(gc.getSimId()) == NmsSimActivateStatus.NMS_SIM_STATUS_ACTIVATED)){
            	menu.setGroupVisible(R.id.InsertQuickTextP, true);
            }else{
            ...
            if (gc.isAlive() && null != mChatListAdapter
                    && mChatListAdapter.countInfo.allMsgCount != 0) {
                menu.setGroupVisible(R.id.addShortcutP, true);
            } else {
                menu.setGroupVisible(R.id.addShortcutP, false);
            }
        } else {
    ...
    public boolean onMenuItemSelected(int featureId, MenuItem item) {
        ...
        if (id == android.R.id.home) {
            ...
            if (MessageUtils.addShortcutToHomeScr(mContext, mContactId)) {
                String name = mContact.getName();
                String s = String.format(mContext.getString(R.string.STR_NMS_SHORTCUT_SUCCESS),
                        name);
        ...
        if (currentSimId > 0) {
            ...
            if (info != null) {
                if (NmsSimActivateStatus.NMS_SIM_STATUS_ACTIVATED != info.status) {
                    ...
                    NmsStartActivityApi.nmsStartNewGroupChatActivity(mContext, contacts, currentSimId);
                }
            } else {
    ...
    private void markAsSpam(boolean spamFlag) {
        ...
        if (spamFlag) {
            NmsIpMessageApiNative.nmsAddContactToSpamList(ids);
        } else {
            NmsIpMessageApiNative.nmsDeleteContactFromSpamList(ids);
        }
    ...
    private boolean downloadSelectMsgResult(short dbId, int code) {
        ...
        if (getSelectList().length == SelectRecordIdList.get().getDownloadSize()) {
            Toast.makeText(mContext, R.string.STR_NMS_FORWARD_ALL_FAILED, Toast.LENGTH_SHORT).show();
        } else {
    ...
    private void forwardSelectMsg() {
        ...
        if (forwardMsgIsInMsgDownloalable()) {
            new AlertDialog.Builder(mContext)
                    .setTitle(R.string.STR_NMS_FORWARD_DOWNLOAD_TITLE)
                    .setMessage(R.string.STR_NMS_FORWARD_DOWNLOAD_MULTI)
                    .setNegativeButton(R.string.STR_NMS_FORWARD_CONTIUE,
                    ...
                    .setPositiveButton(R.string.STR_NMS_CANCEL,
                            new DialogInterface.OnClickListener() {
    ...
    private void changeWallPaper() {
        NmsChatSettings chatSetting = new NmsChatSettings(mContext, mContactId);
        if (chatSetting != null && chatSetting.mContactId > 0) {
            Uri uri = chatSetting.nmsGetChatWallpaper();
            if (null != uri) {
                String path = uri.getEncodedPath();
                Bitmap bt = NmsBitmapUtils.getBitmapByPath(path, NmsBitmapUtils.getOptions(path),
                        mDisplay.widthPixels, mDisplay.heightPixels);
                if (null != bt) {
//                    Drawable d = new BitmapDrawable(getResources(), bt);
            ...
            if (NmsIpMessageConsts.NmsDownloadAttachStatus.NMS_DOWNLOAD_ATTACH_STATUS_ACTION
                ...
                if (status == NmsIpMessageConsts.NmsDownloadAttachStatus.DONE) {
                    ...
                    if (status == NmsIpMessageConsts.NmsDownloadAttachStatus.FAILED) {
                        ...
                        if (!downloadSelectMsgResult(dbId, -1) && !NmsChatLongPressDialog.isForwardDonwload(dbId)) {
