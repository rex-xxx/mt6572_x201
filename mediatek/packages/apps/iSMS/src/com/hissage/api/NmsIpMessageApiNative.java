...
public class NmsIpMessageApiNative {
    ...
    public static NmsIpMessage nmsGetIpMsgInfo(long msgId) {
        ...
        if (ipMsg == null) {
            ...
            ipMsg = new NmsIpTextMessage((int) msgId, 0, 0,
                    ...
                    NmsIpMessageConsts.NmsMessageProtocol.IP, true, 0, 0, "11111111", "11111111",
                    NmsService.getInstance().getString(R.string.STR_NMS_SHORTCUT_REFRESHING));
        }
     ...
     */
    public static int nmsSaveIpMsg(NmsIpMessage msg, int sendMsgMode, boolean delDraft, boolean delDraftInSysDb) {
    ...
    public static void nmsSaveChatHistory(long threadId[]) {
        ...
        for (int i = 0; i < threadId.length; ++i) {
            ...
            if (id <= 0) {
                NmsLog.error(HissageTag.api, "nmsSaveChatHistory get contactid from hissage db via threadid: "
                        + threadId[i]);
                NmsDownloadMessageHistory.sendStatus(NmsSaveHistory.NMS_DOWNLOAD_HISTORY_DONE,
                        NmsSaveHistory.NMS_ERROR, "");
                return;
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                for (int i = 0; i < engineContactIds.length; ++i) {
                    ...
                    NmsChatSettings settings = new NmsChatSettings(NmsService.getInstance(),
                            contactId);
    ...
    public static int nmsDeleteDraftMsgInThread(long threadId) {
        ...
        if (engineContactId <= 0) {
            NmsLog.error(HissageTag.api,
                    "engineContactId is invalid in nmsDeleteDraftMsgInThread");
            return 0;
        }
