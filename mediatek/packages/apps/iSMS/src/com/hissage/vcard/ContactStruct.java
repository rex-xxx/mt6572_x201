...
public class ContactStruct {
            ...
            if (VCardConfig.isJapaneseDevice(vcardType)) {
                ...
                for (int i = 0; i < ADDR_MAX_DATA_SIZE; i++) {
                    ...
                    if (!TextUtils.isEmpty(addressPart)) {
                        ...
                        String retStr = ConvertVCardUTF2String(addressPart);
    ...
    static public class OrganizationData {
        ...
        public OrganizationData(int type, String companyName, String positionName, boolean isPrimary) {
            this.type = type;
    ...
    static public class ImData {
        ...
        public boolean equals(Object obj) {
            ...
            return (type == imData.type && data.equals(imData.data) && label.equals(imData.label) && isPrimary == imData.isPrimary);
        }
        ...
        if (!(TextUtils.isEmpty(mFamilyName) && TextUtils.isEmpty(mGivenName))) {
            ...
            case VCardConfig.NAME_ORDER_JAPANESE:
                if (VCardUtils.containsOnlyPrintableAscii(mFamilyName)
                    ...
                    nameList = Arrays
                            .asList(mPrefix, mFamilyName, mMiddleName, mGivenName, mSuffix);
                }
                break;
    ...
    public static String ConvertVCardUTF2String(String srcVCardStr) {
    ...
    private boolean IsContactifExsit(ContentResolver resolver) {
        boolean ret = false;

        if (!TextUtils.isEmpty(UID) && !UID.equals("0")) {
            Cursor cursorUID = NmsContentResolver.query(resolver,
                    ContactsContract.RawContacts.CONTENT_URI,
                    new String[] { ContactsContract.RawContacts.CONTACT_ID },
                    ContactsContract.RawContacts.CONTACT_ID + "=?", new String[] { UID }, null);
            while (null != cursorUID && cursorUID.moveToNext()) {
                NmsLog.trace(HissageTag.vcard, "found exsit contact phone, id is: " + UID);
                ret = true;
            }
            if (null != cursorUID)
                cursorUID.close();
        ...
        String strTemp = getDisplayName();
        Cursor cursor = null;
        if (!TextUtils.isEmpty(strTemp)) {
            strTemp = strTemp.trim();
            String test = ConvertVCardUTF2String(strTemp);

            cursor = NmsContentResolver.query(resolver, ContactsContract.Contacts.CONTENT_URI,
                    new String[] { ContactsContract.Contacts._ID },
                    ContactsContract.Contacts.DISPLAY_NAME + "=?", new String[] { test }, null);
            while (null != cursor && cursor.moveToNext() && !ret) {
                ...
                String contactIDName = cursor.getString(index);
                // NmsUtils.trace(HissageTag.vcard,
                // "found exsit contact name, id is: " + contactIDName +
                // ", dispName is: " + test);
                ...
                if (mPhoneList != null) {
                    for (PhoneData phoneData : mPhoneList) {
                        if (ret)
                            break;
                        Cursor cursorPN = NmsContentResolver.query(resolver,
                                ContactsContract.CommonDataKinds.Phone.CONTENT_URI,
                                new String[] { ContactsContract.CommonDataKinds.Phone.CONTACT_ID },
                                ContactsContract.CommonDataKinds.Phone.NUMBER + "=?",
                                new String[] { phoneData.data }, null);

                        while (null != cursorPN && cursorPN.moveToNext()) {
                            String contactIDPN = cursorPN
                                    .getString(cursorPN
                            ...
                            // contactID,
                            // null);
                            if (contactIDName.endsWith(contactIDPN)) {
                                NmsLog.trace(HissageTag.vcard, "found exsit contact phone, id is: "
                                        + contactIDName + ", phone is: " + phoneData.data);
                                UID = contactIDName;
                                ret = true;
                                break;
                            }
                ...
                if (!ret && mEmailList != null) {
                    for (EmailData emailData : mEmailList) {
                        ...
                        try {
                            cursorEMail = NmsContentResolver
                                    .query(resolver,
                                            ContactsContract.CommonDataKinds.Email.CONTENT_URI,
                                            new String[] { ContactsContract.CommonDataKinds.Email.CONTACT_ID },
                                            ContactsContract.CommonDataKinds.Email.DATA + "=?",
                                            new String[] { emailData.data }, null);
                            while (null != cursorEMail && cursorEMail.moveToNext()) {
                                String contactIDEMail = cursorEMail
                                ...
                                // contactID,
                                // null);
                                if (contactIDName.endsWith(contactIDEMail)) {
                                    NmsLog.trace(HissageTag.vcard, "found exsit contact, id is: "
                                            + contactIDName + ", EMail is: " + emailData.data);
                                    UID = contactIDName;
                                    ret = true;
                                    break;
                                }
    ...
    private boolean isEmailExsit(String email, int type, ContentResolver resolver) {
        boolean ret = false;
        Cursor cursor = NmsContentResolver
                .query(resolver, ContactsContract.Data.CONTENT_URI,
                        new String[] { Data.CONTACT_ID }, Data.CONTACT_ID + "=?" + " AND "
                                + ContactsContract.Data.MIMETYPE + " = ?" + " AND " + Email.TYPE
                                + "=?", new String[] { String.valueOf(UID),
                                Email.CONTENT_ITEM_TYPE, String.valueOf(type) }, null);
        if (null != cursor && cursor.moveToNext()) {
            NmsLog.trace(HissageTag.vcard, email + " exsit in : " + mDisplayName);
            ret = true;
        }
        if (null != cursor)
            cursor.close();
    ...
    private boolean isPhoneExsit(String phoneNumber, int type, ContentResolver resolver) {
        boolean ret = false;
        Cursor cursor = NmsContentResolver
                .query(resolver, ContactsContract.Data.CONTENT_URI,
                        new String[] { Data.CONTACT_ID }, Data.CONTACT_ID + "=?" + " AND "
                                ...
                                + "=?", new String[] { String.valueOf(UID),
                                Phone.CONTENT_ITEM_TYPE, String.valueOf(type) }, null);
        if (null != cursor && cursor.moveToNext()) {
            NmsLog.trace(HissageTag.vcard, phoneNumber + " exsit in : " + mDisplayName);
            ret = true;
        }
        if (null != cursor)
            cursor.close();
    ...
    private void updateContact(ContentResolver resolver) {
        ...
        // " AND " + Phone.TYPE + "=?",
        //
        // new String[] { String.valueOf(UID), Phone.CONTENT_ITEM_TYPE,
        // String.valueOf(phoneData.type) })
        //
        ...
        if (mPhoneList != null) {
            for (PhoneData phoneData : mPhoneList) {
                if (!isPhoneExsit(phoneData.data, phoneData.type, resolver)) {
                    builder = ContentProviderOperation.newInsert(Data.CONTENT_URI);
                    builder.withValue(Data.RAW_CONTACT_ID, UID);
        ...
        // " AND " + Email.TYPE + "=?",
        //
        // new String[] { String.valueOf(UID), Email.CONTENT_ITEM_TYPE,
        // String.valueOf(emailData.type) })
        //
        // .withValue(Email.DATA, emailData.data)
        //
        // .build());
        // }
        // }
        ...
        if (mEmailList != null) {
            for (EmailData emailData : mEmailList) {
                if (isEmailExsit(emailData.data, emailData.type, resolver)) {
                    builder = ContentProviderOperation.newInsert(Data.CONTENT_URI);
                    builder.withValue(Data.RAW_CONTACT_ID, UID);
    ...
    private void createContact(ContentResolver resolver) {
        ArrayList<ContentProviderOperation> operationList = new ArrayList<ContentProviderOperation>();
        ContentProviderOperation.Builder builder;
        ...
        if (mAccount != null) {
            ...
            if (ACCOUNT_TYPE_GOOGLE.equals(mAccount.type)) {
                Cursor cursor = null;
                try{
                    cursor = NmsContentResolver.query(resolver, Groups.CONTENT_URI,
                        new String[] { Groups.SOURCE_ID }, Groups.TITLE + "=?",
                        new String[] { GOOGLE_MY_CONTACTS_GROUP }, null);
...

        {
            if (!TextUtils.isEmpty(mGivenName))
                mGivenName = ConvertVCardUTF2String(mGivenName);
            if (!TextUtils.isEmpty(mFamilyName))
                mFamilyName = ConvertVCardUTF2String(mFamilyName);
            if (!TextUtils.isEmpty(mMiddleName))
                mMiddleName = ConvertVCardUTF2String(mMiddleName);
            if (!TextUtils.isEmpty(mPhoneticGivenName))
                mPhoneticGivenName = ConvertVCardUTF2String(mPhoneticGivenName);
            if (!TextUtils.isEmpty(mPhoneticFamilyName))
                mPhoneticFamilyName = ConvertVCardUTF2String(mPhoneticFamilyName);
            if (!TextUtils.isEmpty(mPhoneticMiddleName))
                mPhoneticMiddleName = ConvertVCardUTF2String(mPhoneticMiddleName);
            ...
            if (!TextUtils.isEmpty(strTemp)) {
                ...
                String test = ConvertVCardUTF2String(strTemp);
                builder.withValue(StructuredName.DISPLAY_NAME, test);
            }
        ...
        if (mNickNameList != null && mNickNameList.size() > 0) {
            ...
            for (String nickName : mNickNameList) {
                ...
                if (!TextUtils.isEmpty(nickName))
                    nickName = ConvertVCardUTF2String(nickName);
        ...
        if (mOrganizationList != null) {
            ...
            for (OrganizationData organizationData : mOrganizationList) {
                ...
                if (!TextUtils.isEmpty(organizationData.companyName))
                    builder.withValue(Organization.COMPANY,
                            ConvertVCardUTF2String(organizationData.companyName));

                if (!TextUtils.isEmpty(organizationData.positionName))
                    builder.withValue(Organization.TITLE,
                            ConvertVCardUTF2String(organizationData.positionName));
                if (first) {
                    builder.withValue(Data.IS_PRIMARY, 1);
                }
                operationList.add(builder.build());
        ...
        if (mNoteList != null) {
            for (String note : mNoteList) {
                ...
                if (!TextUtils.isEmpty(note)) {
                    String test = ConvertVCardUTF2String(note);
                    builder.withValue(Note.NOTE, test);
                    operationList.add(builder.build());
                }
    ...
    public void pushIntoContentResolver(ContentResolver resolver)
        ...
        NmsLog.trace(HissageTag.vcard, "fucking google's PIM......");
        if (IsContactifExsit(resolver)) {
