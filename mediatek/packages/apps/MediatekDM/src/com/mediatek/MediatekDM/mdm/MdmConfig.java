...
public class MdmConfig {
    ...
    public static enum NotifVerificationMode {
        ...
        DISABLED(0),
        /**
         * When the client receives a notification message and the verification
         * of the notification fails, the client tries to authenticate the
         * notification again using the special nonce value, 0x00000000.
         */
        RESET_NONCE(1),
        /**
         * When the client receives a notification message and the verification
        ...
        REVERIFY(2),
        /**
         * Upon authentication failure, the notification message is ignored and
    ...
    /**
     * Setting this configuration to TRUE contradicts OMA DL standard behavior
    ...
    /**
     * Set whether a missing eTag should be ignored while resuming a download.
     * When set to TRUE, the object URL will be used as the object identifier
     * for resuming the download. <b>NOTE:</b> Using this setting is against the
     * standard and not recommended. It should be used only if the server is
     * committed not to change the content of the object file (but replace it
     * and its URL, if needed). errors or only download success. Reports are
     * sent only if InstallNotifyURI attribute exists in the download
     * descriptor. The initial value is FALSE, indicating all results are
     * reported.
     * 
     * @throws MdmException
     */
    ...
    /**
     * Set whether to use 'If-Range' HTTP header - instead of 'If-Match' - in
     * OMA DL retries. The initial value is False, indicating that 'If-Match' is
     * used.
     * 
     * @param useIfRange
     * @throws MdmException
    ...
    public void setDlHttpAuthentication(HttpAuthLevel level, String username, String password)
            throws MdmException {
    ...
    public void setDlProxyAuthentication(HttpAuthLevel level, String username, String password)
            throws MdmException {
    ...
    public void setDmHttpAuthentication(HttpAuthLevel level, String username, String password)
            throws MdmException {
    ...
    public void setDmProxyAuthentication(HttpAuthLevel level, String username, String password)
            throws MdmException {
    ...
    /**
     * Set whether to allow challenge with Package 1. The initial value is
     * FALSE, indicating not to allow a challenge with Package 1. Call after
     * VdmEngine has been created and before VdmEngine.start(). <b>Note</b>:
     * Setting this configuration to TRUE contradicts OMA DM standard behavior
     * and is not recommended.
     * 
     * @param isAllowed
     * @throws MdmException
    ...
    /**
     * Set whether to verify the version and the type of the parsed Download
     * Descriptor. The initial value is FALSE, indicating no verification. Call
     * after VdmEngine has been created and before VdmEngine.start().
     * <b>Note:</b> Setting this configuration to TRUE contradicts OMA DM
     * standard behavior and is not recommended. Verification should be used
     * only when working with servers producing invalid Download Descriptors.
     * 
     * @param check
     * @throws MdmException
     */
    ...
    /**
     * Set whether URL must be absolute, which means that the URL must include
     * the "http://" or "https://" prefix. The initial value is FALSE,
     * indicating that the URL may be relative. Call after MdmEngine has been
     * created and before MdmEngine.start().
     * 
     * @param absolute TRUE if the URL must be absolute, FALSE otherwise.
     * @throws MdmException OUT_OF_SYNC if the function is called after
     *             MdmEngine.start().
     */
    ...
    /**
     * Set whether TNDS support is enabled or not. The initial value is true.
     * Call after MdmEngine has been created and before MdmEngine.start().
     * 
     * @param isEnabled true if TNDS should be enabled.
     * @throws MdmException OUT_OF_SYNC if the function is called after
     *             MdmEngine.start().
     */
