...
public class MTKMediaContainer {
    private IMediaContainerService imcs;

    public MTKMediaContainer(IBinder service) {
        imcs = IMediaContainerService.Stub.asInterface(service);
    }
    ...
    public boolean checkSpace(String archive, long shreshold) {
        try {
            PackageInfoLite pkgLite = imcs.getMinimalPackageInfo(archive, 0, shreshold);
