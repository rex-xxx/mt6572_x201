...
public class DataUsageStageView extends StageTextureView implements IKeyguardLayer {
    ...
    public DataUsageStageView(Context context) {
        ...
        setCacheDir(mContext, "/data/system/lockscreen");
    ...
    public class DataUsageCreateRunnable implements Runnable {
        ...
        public void run() {
            Log.v(TAG, "create, handleDataUsageLockScreenView() , mServiceState:" + mServiceState);
            handleDataUsageView(mSim, mServiceState);
            Log.v(TAG, "create, end");
            mServiceState = null;
        }
    ...
    public View createInternal(int sim, ServiceState serviceState) {
        ...
        Thread myThread = new Thread(runInstance);
        myThread.start();

        Log.v(TAG, "thread id: " + myThread.getId());
        return this;
    ...
    private void getSimCardInfo() {
        int simnumber = SIMInfo.getInsertedSIMCount(mContext);
        log(TAG, "getSimCardInfo, simnumber= " + simnumber);

        mCardCount = 0;
        simList = SIMInfo.getInsertedSIMList(mContext);
        if (simList == null) {
        ...
        for (SIMInfo info : simList) {
            if (info.mSlot == PhoneConstants.GEMINI_SIM_1) {
                mSimId1 = info.mSimId;
                mSlot1 = SIMInfo.getSlotById(mContext, mSimId1);
                ...
                mSimId2 = info.mSimId;
                mSlot2 = SIMInfo.getSlotById(mContext, mSimId2);
    ...
    public long getDataUsageLimit(int slotId) {
        ...
        if (FeatureOption.MTK_GEMINI_SUPPORT == false) { // Single SIM
            ...
            subsId = telephony.getSubscriberIdGemini(slotId);
        }
    ...
    private long getCurrentDataUsage(long simId, int slot) {
        ...
        if (FeatureOption.MTK_GEMINI_SUPPORT == false) { // Single SIM
            ...
            subsId = telephony.getSubscriberIdGemini(slot);
        }
        ...
        try {
            ...
            if (FeatureOption.MTK_GEMINI_SUPPORT == false) { // Single SIM
                ethernetBytes = mStatsSession.getSummaryForNetwork(
                        NetworkTemplate.buildTemplateMobileAll(subsId),
    ...
    private int getSlotBySubscriberId(String subscriberId, ITelephony iTel) {
        ...
        for (int i = 0; i < PhoneConstants.GEMINI_SIM_NUM; i++) {
            try {
                if (iTel == null) {
                    log(TAG, "getSlotBySubscriberId  ITelephony is not ready");
                    return -1;
                }
    ...
    private BroadcastReceiver mStatsReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            log(TAG, "mStatsReceiver action = " + action);
            if (ACTION_NETWORK_STATS_UPDATED.equals(action)) {
                mHandler.obtainMessage(MSG_DATA_STATS_CHANGED).sendToTarget();
    ...
    private BroadcastReceiver mSimStateReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            String action = intent.getAction();
            log(TAG, "mSimStateReceiver action = " + action);
            if (TelephonyIntents.ACTION_SIM_STATE_CHANGED.equals(action)) {
                log(TAG, "ACTION_SIM_STATE_CHANGED");
                String stateExtra = intent.getStringExtra(IccCardConstants.INTENT_KEY_ICC_STATE);
                if (FeatureOption.MTK_GEMINI_SUPPORT) {
                    int slotId = intent.getIntExtra(PhoneConstants.GEMINI_SIM_ID_KEY, PhoneConstants.GEMINI_SIM_1);
                    if (IccCardConstants.INTENT_VALUE_ICC_ABSENT.equals(stateExtra)) {
                        mHandler.obtainMessage(MSG_SIM_STATS_CHANGED, slotId, 0).sendToTarget();
                    } else if (IccCardConstants.INTENT_VALUE_ICC_LOADED.equals(stateExtra)) {
                        mHandler.obtainMessage(MSG_SIM_STATS_CHANGED, slotId, 1).sendToTarget();
                    }
                } else {
                    if (IccCardConstants.INTENT_VALUE_ICC_ABSENT.equals(stateExtra)) {
                        mHandler.obtainMessage(MSG_SIM_STATS_CHANGED, 0, 0).sendToTarget();
                    } else if (IccCardConstants.INTENT_VALUE_ICC_LOADED.equals(stateExtra)) {
                        mHandler.obtainMessage(MSG_SIM_STATS_CHANGED, 0, 1).sendToTarget();
                    }
                } 
            }
        }
    ...
    private void startListenState() {
        ...
        if (FeatureOption.MTK_GEMINI_SUPPORT) {
            mTelephonyManager.listenGemini(mPhoneStateListener_1,
                    PhoneStateListener.LISTEN_SERVICE_STATE, PhoneConstants.GEMINI_SIM_1);  
            mTelephonyManager.listenGemini(mPhoneStateListener_2,
                    PhoneStateListener.LISTEN_SERVICE_STATE, PhoneConstants.GEMINI_SIM_2);

        } else {
            mTelephonyManager.listen(mPhoneStateListener_1, PhoneStateListener.LISTEN_SERVICE_STATE);
        }
    }
    ...
    private void stopListenState() {
        ...
        if (FeatureOption.MTK_GEMINI_SUPPORT) { // gemini support
            ...
            mTelephonyManager.listen(mPhoneStateListener_1, PhoneStateListener.LISTEN_NONE);
        }
    }
