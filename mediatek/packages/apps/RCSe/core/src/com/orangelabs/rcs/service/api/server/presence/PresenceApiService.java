...
public class PresenceApiService extends IPresenceApi.Stub {
	 ...
	 */
	/**
	 * Polite block the sharing invitation
	 * 
	 * @param contact
	 *            The target contact to deal with
	 * @return Returns true if XDM request was successful, else false
	 */
	public boolean politeRejectSharingInvitation(String contact)
			throws ServerApiException {
		if (logger.isActivated()) {
			logger.info("Polite reject sharing invitation from " + contact);
		}
		...
		try {
			// Update presence server
			boolean result = Core.getInstance().getPresenceService()
					.politeBlockPresenceSharingInvitation(contact);
			if (result) {
				// Set this contact presence status to "polite-blocked"
				ContactsManager.getInstance().setContactSharingStatus(contact,
						PresenceInfo.RCS_POLITE_BLOCKED, "");
			}
			return result;
		} catch (Exception e) {
			throw new ServerApiException(e.getMessage());
	...
	/**
	 * Polite unblock a blocked contact
	 * 
	 * @param contact
	 *            The target contact to deal with
	 * @return Returns true if XDM request was successful, else false
	 * @throws ServerApiException
	 */
	public boolean unblockPoliteBlockedContact(String contact)
			throws ServerApiException {
		if (logger.isActivated()) {
			logger.info("Polite unblock contact " + contact);
		}
		...
		try {
			// Update presence server
			boolean result = Core.getInstance().getPresenceService()
					.removePoliteBlockedContact(contact);
			if (result) {
				// Remove contact from polite blocked contacts list of EAB
				// content provider
				ContactsManager.getInstance().unblockPoliteBlockedContact(
						contact);
			}
			return result;
		} catch (Exception e) {
	...
	/**
	 * Get the list of polite blocked contacts
	 * 
	 * @return Returns the list of polite blocked contacts
	 * @throws ServerApiException
	 */
	public List<String> getPoliteBlockedContacts() throws ServerApiException {
		if (logger.isActivated()) {
			logger.info("Get polite blocked contacts");
		}
		...
		try {
			return Core.getInstance().getPresenceService().getXdmManager()
					.getPoliteBlockedContacts();
		} catch (Exception e) {
			throw new ServerApiException(e.getMessage());
		}
	}
