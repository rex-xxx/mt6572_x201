		...
		try {
            ...
            if (PROTOCOL_TLS.equals(protocol)) {
                ...
                sdp = "v=0" + SipUtils.CRLF + "o=- " + ntpTime + " " + ntpTime + " "
                        ...
                        + SipUtils.CRLF + "c=" + SdpUtils.formatAddressType(ipAddress)
                        + SipUtils.CRLF + "t=0 0" + SipUtils.CRLF + "m=message " + localMsrpPort
                        + " TCP/MSRP *" + SipUtils.CRLF + "a=accept-types:" + CpimMessage.MIME_TYPE
                        + " " + IsComposingInfo.MIME_TYPE + SipUtils.CRLF
                        + "a=accept-wrapped-types:" + InstantMessage.MIME_TYPE + " "
                        + ImdnDocument.MIME_TYPE + SipUtils.CRLF + "a=setup:" + localSetup
                        + SipUtils.CRLF + "a=path:" + getMsrpMgr().getLocalMsrpPath()
                        + SipUtils.CRLF + "a=recvonly" + SipUtils.CRLF;
            }
            /** @} */
		...
		if (ChatUtils.isMessageCpimType(mimeType)) {
			...
			try {
				...
				if (cpimMsg != null) {
                    ...
                    String dateHeader = cpimMsg.getHeader(CpimMessage.HEADER_DATETIME);
                    long date = -1L;
                    if (dateHeader != null) {
                        date = DateUtils.decodeDate(dateHeader);
                    }
                    /** @} */
			    	if (ChatUtils.isMessageImdnType(contentType)) {
						// Receive an IMDN report
			    	    /** M: add server date for delivery status @{ */
                        receiveMessageDeliveryStatus(from, cpimMsg.getMessageContent(), date);
                        /** @} */
			    	}
				}
	    	} catch(Exception e) {
