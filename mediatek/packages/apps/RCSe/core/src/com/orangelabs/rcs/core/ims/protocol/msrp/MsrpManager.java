...
/**
	...
	public MsrpSession createMsrpClientSession(String remoteHost, int remotePort, String remoteMsrpPath, MsrpEventListener listener) throws MsrpException {
        try {
	        ...
	        if (logger.isActivated()) {
				logger.info("Create MSRP client end point at " + remoteHost + ":" + remotePort + ", remoteMsrpPath = " + remoteMsrpPath);
			}
            ...
            if (ListeningPoint.TLS.equals(Core.getInstance().getImsModule()
                    .getCurrentNetworkInterface().getMsrpProtocol())) {
                if (logger.isActivated()) {
                    logger.debug("Create a MSRPs client connection.");
                }
                ...
                if (logger.isActivated()) {
                    logger.debug("Create a MSRP client connection");
                }
                msrpSession.setFrom(getLocalMsrpPath());
                // Create a MSRP client connection
                final MsrpConnection connection = new MsrpClientConnection(msrpSession, remoteHost,
                        remotePort);
                // Associate the connection to the session
                msrpSession.setConnection(connection);
            }
	...
	public MsrpSession createMsrpServerSession(String remoteMsrpPath, MsrpEventListener listener) throws MsrpException {
		if (logger.isActivated()) {
			logger.info("Create MSRP server end point at " + localMsrpPort + ", remoteMsrpPath = " + remoteMsrpPath);
		}
        ...
        if (ListeningPoint.TLS.equals(Core.getInstance().getImsModule()
                .getCurrentNetworkInterface().getMsrpProtocol())) {
            if (logger.isActivated()) {
                logger.debug("Create a MSRPs server connection");
            }
            msrpSession.setFrom(getLocalMsrpsPath());
            ...
            if (logger.isActivated()) {
                logger.debug("Create a MSRP server connection");
            }
            msrpSession.setFrom(getLocalMsrpPath());
            // Create a MSRP server connection
