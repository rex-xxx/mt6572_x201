...
public class EventsLogApi extends ClientApi {
    ...
    /**
     * Get the last messages from a given contact
     * 
     * @param contact The contact.
     * @param start From when the messages to return
     * @param count The count of messages to return
     * @return Instant messages
     */
    public Map<InstantMessage, Boolean> getLastChatMessages(String contact, String start,
            String count) {
        Map<InstantMessage, Boolean> map = new HashMap<InstantMessage, Boolean>();
        String[] projection = {
                RichMessagingData.KEY_MESSAGE_ID, RichMessagingData.KEY_CONTACT,
                RichMessagingData.KEY_DATA, RichMessagingData.KEY_TIMESTAMP,
                RichMessagingData.KEY_STATUS, RichMessagingData.KEY_TYPE
        };
        // Do not take the chat terminated messages
        String chatTerminatedExcludedSelection = " AND NOT((" + RichMessagingData.KEY_TYPE + "=="
                + TYPE_CHAT_SYSTEM_MESSAGE + ") AND (" + RichMessagingData.KEY_STATUS + "=="
                + STATUS_TERMINATED + "))";
        // Do not take the group chat entries concerning this contact
        chatTerminatedExcludedSelection += " AND NOT(" + RichMessagingData.KEY_TYPE + "=="
                + TYPE_GROUP_CHAT_SYSTEM_MESSAGE + ")";
        if (!"-1L".equalsIgnoreCase(start)) {
            chatTerminatedExcludedSelection += " AND (" + RichMessagingData.KEY_TIMESTAMP + "<"
                    + start + ")";
        } else {
            chatTerminatedExcludedSelection += " AND (" + RichMessagingData.KEY_TIMESTAMP + "<MAX("
                    + RichMessagingData.KEY_TIMESTAMP + ")";
        }
        String selection = RichMessagingData.KEY_CONTACT + "='"
                + PhoneUtils.formatNumberToInternational(contact) + "'"
                + chatTerminatedExcludedSelection;
        Cursor cursor = ctx.getContentResolver().query(RichMessagingData.CONTENT_URI, projection,
                selection, null, RichMessagingData.KEY_TIMESTAMP + " ASC LIMIT " + count);
        try {
            if (cursor != null) {
                while (cursor.moveToNext()) {
                    ...
                    Date serverReceiptAt = new Date(cursor.getLong(3));
                    boolean imdnDisplayedRequested = (cursor.getInt(4) == EventsLogApi.STATUS_REPORT_REQUESTED);
                    Boolean isIncoming = (cursor.getInt(5) == EventsLogApi.TYPE_INCOMING_CHAT_MESSAGE);
                    InstantMessage instantMessage = new InstantMessage(messageId, remote, message,
                            imdnDisplayedRequested, serverReceiptAt);
                    map.put(instantMessage, isIncoming);
                }
            }
        } finally {
            if (cursor != null) {
