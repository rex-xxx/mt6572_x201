    ...
    public boolean parse() {
        try {
            ...
            do {
                ...
                if (childnode.getNodeName().trim().equals("characteristic".trim())) {
                    if (childnode.getAttributes().getLength() > 0) {
                        ...
                        if (typenode != null) {
                            ...
                            if (typenode.getNodeValue().trim().equals("VERS".trim())) {
                                parseVersion(childnode);
                            } else if (typenode.getNodeValue().trim().equals("MSG".trim())) {
                                parseTermsMessage(childnode);
                            } else if (typenode.getNodeValue().trim().equals("APPLICATION".trim())) {
                                parseApplication(childnode);
                            } else if (typenode.getNodeValue().trim().equals("IMS".trim())) {
                                parseIMS(childnode);
                            } else if (typenode.getNodeValue().trim().equals("PRESENCE".trim())) {
                                parsePresence(childnode);
                            } else if (typenode.getNodeValue().trim().equals("XDMS".trim())) {
                                parseXDMS(childnode);
                            } else if (typenode.getNodeValue().trim().equals("IM".trim())) {
                                parseIM(childnode);
                            } else if (typenode.getNodeValue().trim().equals("CAPDISCOVERY".trim())) {
                                parseCapabilityDiscovery(childnode);
                            } else if (typenode.getNodeValue().trim().equals("APN".trim())) {
                                parseAPN(childnode);
                            } else if (typenode.getNodeValue().trim().equals("OTHER".trim())) {
                ...
                if (childnode.getNodeName().trim().equals("characteristic".trim())) {
                    if (childnode.getAttributes().getLength() > 0) {
                        ...
                        if (typenode != null) {
                            ...
                            if (typenode.getNodeValue().trim().equals("FAVLINK".trim())) {
                                parseFavoriteLink(childnode);
                            } else if (typenode.getNodeValue().trim().equals("SERVCAPWATCH".trim())) {
                                parsePresenceWatcher(childnode);
                            } else if (typenode.getNodeValue().trim().equals("ServCapPresentity".trim())) {
                    ...
                    if ((psMedia = getValueByParamName("psMedia", childnode)) != null) {
                        ...
                        if (psMedia.equals("MSRP")) {
                            instance.writeParameter(RcsSettingsData.MSRP_PROTOCOL_FOR_MOBILE,
                                    ListeningPoint.TCP);
                        } else if (psMedia.equals("MSRPoTLS")) {
                            instance.writeParameter(RcsSettingsData.MSRP_PROTOCOL_FOR_MOBILE,
                                    ListeningPoint.TLS);
                        }
                        /** @} */
                ...
                if (childnode.getNodeName().equals("characteristic")) {
                    if (childnode.getAttributes().getLength() > 0) {
                        ...
                        if (typenode != null) {
                            ...
                            if (typenode.getNodeValue().trim().equals("transportProto".trim())) {
                ...
                if (childnode.getNodeName().equals("characteristic")) {
                    if (childnode.getAttributes().getLength() > 0) {
                        ...
                        if (typenode != null) {
                            ...
                            if (typenode.getNodeValue().trim().equals("IMS".trim())) {
                                parseIMS(childnode);
                            } else if (typenode.getNodeValue().trim().equals("PRESENCE".trim())) {
                                parsePresence(childnode);
                            } else if (typenode.getNodeValue().trim().equals("XDMS".trim())) {
                ...
                if (childnode.getNodeName().trim().equals("characteristic".trim())) {
                    if (childnode.getAttributes().getLength() > 0) {
                        ...
                        if (typenode != null) {
                            ...
                            if (typenode.getNodeValue().trim().equals("ConRefs".trim())) {
                                parseConRefs(childnode);
                            } else if (typenode.getNodeValue().trim().equals("Public_user_identity_List".trim())) {
                                parsePublicUserIdentity(childnode);
                            } else if (typenode.getNodeValue().trim().equals("Ext".trim())) {
                                parseExt(childnode);
                            } else if (typenode.getNodeValue().trim().equals("ICSI_List".trim())) {
                                parseICSI(childnode);
                            } else if (typenode.getNodeValue().trim().equals("LBO_P-CSCF_Address".trim())) {
                                parsePcscfAddress(childnode);
                            } else if (typenode.getNodeValue().trim().equals("PhoneContext_List".trim())) {
                                parsePhoneContextList(childnode);
                            } else if (typenode.getNodeValue().trim().equals("APPAUTH".trim())) {
