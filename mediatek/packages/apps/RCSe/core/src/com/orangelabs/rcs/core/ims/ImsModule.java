...
public class ImsModule implements SipEventListener {
	...
	public void abortAllSessions() {
        ...
        for (int i = 0; i < services.length; i++) {
            ...
            /**
             * M: Changed to fix the null pointer exception. @{
             */
            if (service != null) {
                Enumeration<ImsServiceSession> e = service.getSessions();
                if (e != null) {
                    while (e.hasMoreElements()) {
                        ...
                        if (session != null) {
                            ...
                            if (logger.isActivated()) {
                                logger.error("The session is null");
                            }
                        }
                    }
                } else {
                    if (logger.isActivated()) {
                        logger.error("The instance of Enumeration<ImsServiceSession> is null");
                    }
                }
            } else {
                if (logger.isActivated()) {
                    logger.error("The service is null");
                }
            }
            /**
             * @}
