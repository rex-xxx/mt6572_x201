...
public class PresenceService extends ImsService implements AddressBookEventListener{
    ...
    public synchronized void start() {
        ...
        if (StartService.getNewUserAccount(AndroidFactory.getApplicationContext())) {
            ...
            /**
             * M:Modified for add the polite blocked contacts.@{T-Mobile
             */
            List<String> politeBlockedContacts = xdm.getPoliteBlockedContacts();
            firstLaunchOrAccountChangedCheck(grantedContacts, blockedContacts, politeBlockedContacts);
            /**
             * T-Mobile@}
             */
		}
     ...
     * M:Modified for add the polite blocked contacts.@{T-Mobile
     */
    public void firstLaunchOrAccountChangedCheck(List<String> grantedContacts, List<String> blockedContacts, List<String> politeBlockedContacts) {
        /**
         * T-Mobile@}
         */
		if (logger.isActivated()){
	     ...
	     * M:Modified for add the polite blocked contacts.@{T-Mobile
	     */
		// Treat the polite blocked contact list
		for (String politeBlockedContact : politeBlockedContacts) {
            
            // For each RCS polite blocked contact
            String rcsNumber = PhoneUtils.extractNumberFromUri(politeBlockedContact);
            if (!ContactUtils.isNumberInAddressBook(rcsNumber)){
                // If it is not present in the address book
                if (logger.isActivated()){
                    logger.debug("The RCS number " + rcsNumber + " was not found in the address book: add it");
                ...
                // Set the presence sharing status to polite blocked
                try {
                    ContactsManager.getInstance().politeBlockContact(rcsNumber);
                } catch (ContactsManagerException e) {
                    if (logger.isActivated()){
                        logger.error("Something went wrong when polite blocking contact "+rcsNumber,e);
                    }
                }

                // Add the contact to the rich address book provider
                ContactsManager.getInstance().modifyRcsContactInProvider(rcsNumber, ContactInfo.RCS_POLITE_BLOCKED);
            }
        }
        /**
         * T-Mobile@}
 ...
 " xmlns:ci=\"urn:ietf:params:xml:ns:pidf:cipid\""
		...
		for(int i=0;i<rcsNumbers.size(); i++){
			...
			if (!ContactUtils.isNumberInAddressBook(rcsNumber)){
				...
				if (ContactsManager.getInstance().isNumberShared(rcsNumber)
					...
					/**
					 * M:add remove the polite blocked contact from rich address book provider.@{T-Mobile
					 */
				}else if (ContactsManager.getInstance().isNumberPoliteBlocked(rcsNumber)){
					// Polite Blocked
					if (logger.isActivated()){
						logger.debug(rcsNumber + " is polite blocked");
						logger.debug("We remove it from the polite blocked list");
					}
					// We unpoliteblock it
					boolean result = removePoliteBlockedContact(rcsNumber);
					if (result){
						// Remove entry from rich address book provider
						ContactsManager.getInstance().modifyRcsContactInProvider(rcsNumber, ContactInfo.RCS_CAPABLE);
					}else{
						if (logger.isActivated()){
							logger.error("Something went wrong when removing polite blocked contact");
						}
					}
					/**
					 * T-Mobile@}
     ...
     */
    /**
     * Polite block a presence sharing invitation
     * 
     * @param contact The target contact to deal with
     * @returns Returns true if XDM request was successful, else false
     */
    public boolean politeBlockPresenceSharingInvitation(String contact) {
        // Add contact in the blocked contacts list
        String contactUri = PhoneUtils.formatNumberToSipUri(contact);
        HttpResponse response = xdm.addContactToPoliteBlockedList(contactUri);
        if ((response != null) && response.isSuccessfullResponse()) {
            return true;
        } else {
            return false;
    ...
    /**
     * Remove a polite blocked contact
     * 
     * @param contact The target contact to deal with
     * @returns Returns true if XDM request was successful, else false
     */
    public boolean removePoliteBlockedContact(String contact) {
        // Remove contact from the revoked contacts list
        String contactUri = PhoneUtils.formatNumberToSipUri(contact);
        HttpResponse response = xdm.removeContactFromPoliteBlockedList(contactUri);
        if ((response != null)
                && (response.isSuccessfullResponse() || response.isNotFoundResponse())) {
            return true;
        } else {
