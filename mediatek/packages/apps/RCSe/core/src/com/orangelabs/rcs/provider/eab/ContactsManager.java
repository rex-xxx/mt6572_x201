...
public final class ContactsManager {
     ...
     */
    //private static final String SIM_ACCOUNT_NAME = "SIM Account";
    /**
     * @}
     */
		...
		for (int i=0;i<rawContactIds.size();i++){
            ...
            if (!isRcsContactExisted(rcsRawContactId, contact)) {
                if (newInfo.getRcsStatus() != ContactInfo.NOT_RCS
                    ...
                    createRcsContact(newInfo, rawContactId);
                    // Nothing to modify, as the new contact will have taken the
                    // new infos
                }
                continue;
            }
    ...
    public List<String> getImSessionCapableContacts() {
        ...
        String selectionImCapOff = Data.MIMETYPE + "=? AND " + Data.DATA5 + "=? ";
        String[] selectionArgsImCapOff = {
                MIMETYPE_RCSE_CAPABILITIES, String.valueOf(1)
        };
        Cursor c = ctx.getContentResolver().query(Data.CONTENT_URI, projection, selectionImCapOff,
                selectionArgsImCapOff, null);
        /**
         * M: Modified to resolve the issue of that error exists after editing a
         * RCS-e contact to another RCS-e contact which displays in Chat app.@{
         */
     ...
     */
    public boolean setImBlockedForContact(String contact, boolean flag){
        if (flag) {
