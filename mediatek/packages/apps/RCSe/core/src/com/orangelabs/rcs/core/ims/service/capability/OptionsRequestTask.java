...
public class OptionsRequestTask implements Runnable {
        ...
        if (ctx.isSipResponse()) {
             ...
             */
                ...
                if (isRegistered) {
                    ...
                    if (logger.isActivated()) {
                        logger.debug("sendOptions() registration triggered by 403 is done, mCurrentRetry: "
                                + mCurrentRetry);
                    }
	...
	private void handleUserNotRegistered(SipTransactionContext ctx) {
        ...
        if (info.getRcsStatus() == ContactInfo.NO_INFO) {
            ...
            // We update the database with its previous infos and set the
            // registration state to offline
