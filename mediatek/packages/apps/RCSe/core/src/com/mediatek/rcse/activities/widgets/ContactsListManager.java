...
public final class ContactsListManager implements OnLoadCompleteListener<Cursor> {
    ...
    private void getContactListVodafone(List<RcsContact> contacts) {
        ...
        if (contacts != null && contacts.size() != 0) {
            for (RcsContact contact : contacts) {
                if (!CONTACTS_LIST.contains(contact) && contact.mNumber.startsWith("+34")
                        && !BLOCKED_CONTRACTS.contains(contact.mNumber)) {
                    ...
                    Logger.d(TAG,
                            "getContactListVodafone, a duplicate contact, not need to add");
                }
            }
        }
    }
