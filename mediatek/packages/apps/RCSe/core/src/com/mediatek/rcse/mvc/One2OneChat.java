...
public class One2OneChat extends ChatImpl implements SelfCapabilitiesChangedListener {
    ...
    private FileTransferController mFileTransferController = null;
    ...
    private class One2OneChatListener extends IChatEventListener.Stub {
        ...
        @Override
        public void handleMessageDeliveryStatus(final String msgId, final String status, final long timeStamp)
                throws RemoteException {
    ...
    private static class QueryHandler extends AsyncQueryHandler {
        IChatWindow mWindow;
        ArrayList<IChatMessage> mAllMessages;

        public QueryHandler(Context context, IChatWindow chatWindow,
                ArrayList<IChatMessage> allMessages) {
     ...
     * status itself
     */
    private class ReceiveFileTransfer {
        ...
        public ReceiveFileTransfer(IFileTransferSession fileTransferSession) {
            if (null != fileTransferSession) {
                Logger.d(TAG, "ReceiveFileTransfer() constructor IFileTransferSession is "
                        + fileTransferSession);
                handleFileTransferInvitation(fileTransferSession);
            }
        }

        private void handleFileTransferInvitation(IFileTransferSession fileTransferSession) {
            ...
            if (mFileStruct != null) {
                mFileTransfer = ((IOne2OneChatWindow) mChatWindow).addReceivedFileTransfer(mFileStruct);
        ...
        private void acceptFileTransferInvitation() {
        ...
        private void rejectFileTransferInvitation() {
            try {
                if (mFileTransferSession != null) {
         ...
         */
        private void onFileTransferCancel() {
            Logger.v(TAG,
                    "canceledFileTransfer() entry : mFileTransferListener = "
                            + mFileTransferListener + ", mFileTransfer = "
                            + mFileTransfer + ", mFileTransferSession= "
    ...
    private ReceiveFileTransferManager mReceiveFileTransferManager =
     ...
     */
    private class ReceiveFileTransferManager {
         ...
         */
        public synchronized void addReceiveFileTransfer(IFileTransferSession fileTransferSession) {
            Logger.d(TAG, "addReceiveFileTransfer() entry, fileTransferSession = "
                    + fileTransferSession);
            if (null != fileTransferSession) {
                ReceiveFileTransfer receiveFileTransfer =
                        new ReceiveFileTransfer(fileTransferSession);
    ...
    public void onCapabilityChanged(String contact, Capabilities capabilities) {
        ...
        Logger.d(TAG, "onCapabilityChanged() participantNumbert is " + participantNumber);
        if (participantNumber.equals(contact)) {
            ...
            if (capabilities.isSupportedRcseContact() || ContactsListManager.getInstance().isLocalContact(participantNumber)
                ...
                Logger.d(TAG, "onCapabilityChanged() the participant is rcse contact " + contact);
                onCapabilityChangedWhenRemoteIsRcse(contact, capabilities);
            } else {
                Logger.d(TAG, "onCapabilityChanged() the participant is not rcse contact "
                        + contact);
                if (mFileTransferController != null) {
                ...
                mReceiveFileTransferManager.cancelReceiveFileTransfer();
                Logger.d(TAG, "mChatWindow = " + mChatWindow);
                if (mChatWindow != null) {
                    ((IOne2OneChatWindow) mChatWindow)
    ...
    private void onCapabilityChangedWhenRemoteIsRcse(String number, Capabilities capabilities) {
        ...
        if (ContactInfo.REGISTRATION_STATUS_ONLINE == registrationState) {
            Logger.d(TAG, "onCapabilityChangedWhenRemoteIsRcse() ,the participant " + number
                    + " is online");
            ((IOne2OneChatWindow) mChatWindow).setRemoteOfflineReminder(false);
        } else {
    ...
    private SentMessageManager mSentMessageManager = new SentMessageManager();

    /**
     * This class is used manage the sent messages, especially for the time-out
     * ones
     */
    private class SentMessageManager {
        public final String tag = "SentMessageManager@" + mTag;

        private static final long TIME_OUT_MILLI = 30000;
        private final Map<String, SendMessageWatcher> mSendingMessage =
                new HashMap<String, SendMessageWatcher>();
        ...
        public void onMessageDelivered(final String messageId, final Status status, final long timeStamp) {
        ...
        private void onTimeout(SendMessageWatcher timerTask) {
            synchronized (mSendingMessage) {
         ...
         */
        private class SendMessageWatcher extends TimerTask {
            public String tag = "SendMessageWatcher:";
            private ISentChatMessage mMessage = null;
            private ISentChatMessage.Status mStatus = Status.SENDING;
            ...
            if (!TextUtils.isEmpty(content)) {
                ...
                if (apiManager != null) {
                    ...
                    if (context != null && mParticipant != null) {
                        ...
                        return new InstantMessage(null, mParticipant.getContact(), content, true, new Date());
                    }
                }
            }
            return null;
        ...
        Runnable worker = new Runnable() {
            ...
            public void run() {
                ...
                if (EventsLogApi.TYPE_INCOMING_FILE_TRANSFER == transferType) {
                    fileTransfer = ((IOne2OneChatWindow) mChatWindow).addReceivedFileTransfer(fileStruct);
