...
public class ChatWindowManagerAdapter implements IChatWindowManager {
    ...
    public class GroupChatWindowAdapter extends BaseChatWindow implements IGroupChatWindow {
        ...
        public void setIsRejoining(boolean isRejoining) {
            Logger.d(TAG, "setIsRejoining() entry! isRejoining = " + isRejoining);
            try {
                Logger.d(TAG,
                        "setIsRejoining() : mRemoteGroupChatWindow = "
                                + mRemoteGroupChatWindow);
                if (mRemoteGroupChatWindow != null) {
                    mRemoteGroupChatWindow.setIsRejoining(isRejoining);
                }
            } catch (RemoteException e) {
                e.printStackTrace();
            }
    ...
    public class One2OneChatWindowAdapter extends BaseChatWindow implements IOne2OneChatWindow {
        ...
        public IFileTransfer addReceivedFileTransfer(FileStruct file) {
            ...
            try {
                fileTransfer = mRemoteOne2OneChatWindow.addReceivedFileTransfer(fileStructForBinder);
            } catch (RemoteException e) {
                e.printStackTrace();
            }
