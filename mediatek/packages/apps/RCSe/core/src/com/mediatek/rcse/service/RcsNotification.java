...
public class RcsNotification {
     ...
     */
    public static final String GROUP_AUTO_ACCEPT = "autoAccept";
    ...
    private void handlePluginGroupChatInvitation(Context context, Intent invitation) {
        ...
        Logger.d(TAG, "handlePluginGroupChatInvitation chatSession is " + chatSession);
    ...
    private void showFileTransferNotification(Context context, Intent invitation,
        ...
        boolean result = chatManager.handleFileTransferInvitation(sessionId);
        Logger.d(
                TAG,
                "showFileTransferNotification(), result: " + result + " mode: "
                        + Logger.getIsIntegrationMode());
        if (!result) {
            if (!Logger.getIsIntegrationMode()) {
                int size = mFileInvitationInfos.size();
                Logger.d(TAG, "showFileTransferNotification(), size of mFileInvitationInfos is"
                        + size);
                boolean exist = false;
                for (int i = 0; i < size; i++) {
        ...
        if (MessagingApiIntents.CHAT_INVITATION.equalsIgnoreCase(action)) {
            ...
            try {
                if (chatSession.isGroupChat()) {
                    boolean autoAccept = intent.getBooleanExtra(GROUP_AUTO_ACCEPT, false);
                    String chatId = intent.getStringExtra(RcsNotification.CHAT_ID);
                    // TODO
                    // work around for group restart, because auto-accept is not
                    // already supported.
                    autoAccept = (((ModelImpl) ModelImpl.getInstance()).getGroupChat(chatId) != null) ? true
                            : false;
                    RcsNotification.getInstance().handleGroupChatInvitation(context, intent,
                            autoAccept);
                } else {
