...
public class GroupChatWindow extends ChatScreenWindow implements ChatView.IGroupChatWindow {
    ...
    public ISentChatMessage addSentMessage(InstantMessage message, int messageTag) {
        ...
        if (mGroupChatFragment != null) {
            mGroupChatFragment.addSentMessage(message, messageTag);
        } else {
    ...
    /**
     * When rejoining, this method should be called to reminder the rejoining
     * message.
     * 
     * @param isRejoing whether to remind the message.
     */
    @Override
    public void setIsRejoining(boolean isRejoining) {
        Logger.d(TAG, "setIsRejoining entry, the tag is: " + mTag + "isRejoining" + isRejoining);
        if (!shouldLoadData()) {
            Logger.d(TAG, "setIsRejoining this window is not focused, the tag is: " + mTag);
            return;
        }
        if (mGroupChatFragment != null) {
            mGroupChatFragment.setIsRejoining(isRejoining);
        } else {
            Logger.d(TAG, "setIsRejoining mGroupChatFragment is null");
        }
    }
