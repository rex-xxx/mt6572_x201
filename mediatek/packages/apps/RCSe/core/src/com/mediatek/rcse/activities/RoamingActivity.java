...
public class RoamingActivity extends Activity {
     ...
     */
    class RoamingMessageDialog extends DialogFragment {
            ...
            dialogBuilder.setPositiveButton(this.getResources().getString(
                ...
                public void onClick(DialogInterface arg0, int arg1) {
                    if (mType == ROAMING_WITH_ENABLE) {
                        ...
                        // Launch the RcseSystemSettingsActivity
                        Intent intent = new Intent();
                        intent.setAction(RCSE_SETTING_ACTION);
                        intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                        startActivity(intent);
                    } else if (mType == ROAMING_WITH_DISABLE) {
                        Logger
                                .d(TAG,
                                        "onClick()-Currently is roaming, and the 'Roaming' check box is unselected.");
                        // Terminate the RCS-e service automatically
                        RcsSettings.getInstance().setRoamingAuthorizationState(false);
                        LauncherUtils.stopRcsService(RoamingActivity.this.getApplicationContext());
                    } else if (mType == UNROAMING) {
