    ...
    private ArrayList<Participant> removeContactsExceed(List<RcsContact> contacts) {
        ...
        for (i = 0; i < contactsSize; i++) {
            ...
            if (contact.mSelected) {
                ...
                if (mTagName != null && participants.size() >= maxParticipants) {
                    Logger.d(TAG, "Create a new group chat window. The select num is exceed.");
                    break;
                }
                if (mTagName == null && (participants.size() + existContactsNum) >= maxParticipants) {
                    Logger.d(TAG, "Open group chat window. The select num is exceed.");
                    break;
                }
        ...
        if (mTagName != null && participants.size() == maxParticipants) {
            Logger.d(TAG, "The select num is exceed, then modify listview's item");
            showToast(mContext, mContext.getString(
                    com.orangelabs.rcs.R.string.cannot_add_members_over, maxParticipants));
