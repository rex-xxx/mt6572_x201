    ...
    protected class TabManager {
        ...
        protected void addTab(String text) {
            ...
            if (mTabNameList.isEmpty()) {
                ...
                btn.setBackgroundDrawable(getResources().getDrawable(R.drawable.custom_tab));
                if (text.length() <= TAB_TEXT_LENGTH) {
                    ((Button) btn).setText(text);
                } else {
                    String tabItemText = text.substring(0, TAB_TEXT_LENGTH - 3) + "...";
                    LogUtils.d(TAG, "updateNavigationBar,text is: " + tabItemText);
        ...
        protected void updateNavigationBar(int id) {
            ...
            if (id < mTabNameList.size() - 1) {
                ...
                if (id == 0) {
                    ...
                    int loopCount = id;
                    if (mCurFilePath.contains(MountPointManager.EMULATED)) {
                        //TODO:find better way to do.
                        //if the shared sdcard support,it will return "/storage/emulated/0"
                        //keep the "emualted/0" as one item
                        loopCount += 1;
                    }
