...
public class BluetoothShareGatewayActivity extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (Intent.ACTION_SEND.equals(action) || Intent.ACTION_SEND_MULTIPLE.equals(action)
                || OppConstants.ACTION_HANDOVER_SEND.equals(action)
                || OppConstants.ACTION_HANDOVER_SEND_MULTIPLE.equals(action)) {

            if (OppConstants.ACTION_HANDOVER_SEND.equals(action)
                    || OppConstants.ACTION_HANDOVER_SEND_MULTIPLE.equals(action)) {
                    sBip = false;
            }
    ...
    private void startDevicePicker() {
        ...
        if (FeatureOption.MTK_BT_PROFILE_BIP) {
            inToBDP.putExtra(BluetoothDevicePicker.EXTRA_FILTER_TYPE_1, BluetoothDevicePicker.FILTER_TYPE_BIP);
        }
