	...
	//		cv.put(Mms.MESSAGE_SIZE, size);
		...
		if (mailbox == Sms.MESSAGE_TYPE_OUTBOX && recipient != null){
			...
			if(FeatureOption.MTK_GEMINI_SUPPORT != true) {
				...
				GeminiSmsManager.sendMultipartTextMessageGemini(recipient, null, messages, mSimId, sentIntents, deliveryIntents);
			}
		}
	...
	public	boolean deleteMessage(long id) {
		...
		if(cs != null && cs.moveToFirst()) {
			...
			if (mailbox == MESSAGE_TYPE_DELETE) {
				...
				ContentValues cv = new ContentValues();
				cv.put(Sms.TYPE, Integer.valueOf(MESSAGE_TYPE_DELETE));
		...
		if(cs != null && cs.moveToFirst()) {
			...
			if (mailbox == MESSAGE_TYPE_DELETE) {
				...
				if (orignalFolder != null) {
					ContentValues cv = new ContentValues();
					cv.put(Sms.TYPE, Integer.valueOf(orignalFolder));
					mContentResolver.update(uri, cv, null, null);
				} else {
...
//		msg.setSize(cs.getInt(SIZE_COLUMN));
	...
	private boolean convertFilterToSelection(MessageListRequest req, 
		...
		if (NetworkUtil.SIM1 == mSimId) {
			SIMInfo simInfo = SIMInfo.getSIMInfoBySlot(mContext, NetworkUtil.SIM2);
			if (simInfo != null) {
				if (selection.length() > 0) {
					selection.append(" AND ");
				}
				selection.append(Sms.SIM_ID+"<>?");
				selectionArgs.add(Long.toString(simInfo.mSimId));
			} else {
				//No message is in SIM 2, all message should be put into intance 0
				//igore
			}
		} else {
