...
public class NotificationReceiver extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent) {
		...
		if (Constants.ACTION_NEW_DATA_DETECTED.equals(intent.getAction())) {
			...
			MyLogger.logD(CLASS_TAG, " NotificationReceiver----->ACTION_NEW_DATA_DETECTED Extra:Folder = "+folder);
			...
			MyLogger.logD(CLASS_TAG, " NotificationReceiver----->ACTION_NEW_DATA_DETECTED Extra:Type = "+intentFilter);
			Intent contentIntent = new Intent(intentFilter);
			contentIntent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
			if (type != NotifyManager.FP_NEW_DETECTION_NOTIFY_TYPE_LIST && folder != null && folder.length() > 0) {
