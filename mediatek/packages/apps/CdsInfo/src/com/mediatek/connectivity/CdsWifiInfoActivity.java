...
public class CdsWifiInfoActivity extends Activity {
    ...
    private static final int MAC_ADDRESS_ID = 29;
            ...
            if(!mTestMode){
                ...
                if(!isMacAddressGen){
                    ...
                    for(int i = 0; i < MAC_ADDRESS_DIGITS; i++){
                        mRandomMacAddr[i] = (short) r.nextInt(MAX_ADDRESS_VALUE);
                        System.out.println(Integer.toHexString(mRandomMacAddr[i]));
                        sb.append(Integer.toHexString(mRandomMacAddr[i]));
