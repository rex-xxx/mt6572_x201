...
public class SchPwrOffReceiver extends BroadcastReceiver {
    ...
    /**
     * If the alarm is older than STALE_WINDOW seconds, ignore. It is probably the result of a time or timezone change
     */
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (SHUTDOWN_IPO.equals(intent.getAction())) {
            if (ShutdownActivity.sCountDownTimer == null) {
                Xlog.d(TAG, "SchPwrOffReceiver , ShutdownActivity.sCountDownTimer == null");
            } else {
                Xlog.d(TAG, "SchPwrOffReceiver , ShutdownActivity.sCountDownTimer != null");
        ...
        if (alarm.mId == 1) {
            ...
            if (isInCall || isAlarmBoot()) {
                ...
                am.set(AlarmManager.RTC_WAKEUP, System.currentTimeMillis() + schduleTimeOff, pendingIntent);
            }
    ...
    /**
     * check if is alarm boot; if it alarm boot, we don't fire the shutdownactivity pop dialog.
     * 
