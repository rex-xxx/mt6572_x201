...
final class HttpManager {
    ...
    private HttpManager(Context context) {
        ...
        new Thread() {
            public void run() {
                ...
                Looper.prepare();
                mNetworkHandler = new Handler(Looper.myLooper()) {
    ...
    private boolean handsakeAuthentication() {
        ...
        Xlog.i(TAG, "onHandsakeAuthentication, imei = " + deviceInfo.mImei + ", sn = "
                + deviceInfo.mSnNumber + ", sim = " + deviceInfo.mSim + ", operator = "
                + deviceInfo.mOperator);
        ...
        try {
            ...
            Xlog.i(TAG, "onHandsakeAuthentication, rand = " + res.mRand + ", sessionId = "
                    + res.mSessionId);
        } catch (IOException e) {
            e.printStackTrace();
            return false;
    ...
    private boolean checkNewVersion() {
        ...
        try {
            ...
            BasicNameValuePair version = new BasicNameValuePair("version",
                    Util.getDeviceVersionInfo(mContext));
            ArrayList<BasicNameValuePair> bnvpa = new ArrayList<BasicNameValuePair>();
            bnvpa.add(version);
            bnvpa.add(token);
            HttpResponse response = doPost(url, null, bnvpa);
        ...
        try {
            ...
            try {
                if (url.contains("https")) {
                    ...
                    Xlog.i(TAG, "mHttpClient =" + httpClient + "httpPost = " + httpPost
    ...
    private String getChunkedContent(HttpEntity entity) throws IOException {
        ...
        Xlog.i(TAG, "getChunkedContent, isChunked = "
