...
public class OtaPkgManagerActivity extends PkgManagerBaseActivity {
    ...
    private static final String OTA_PKG_PATH = Util.PathName.OTA_PKG_FOLDER
    ...
    static OtaPkgManagerActivity sInstance;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        sInstance = this;
        if(SdPkgInstallActivity.sInstance != null){
            SdPkgInstallActivity.sInstance.finish();
        }
        ...
        case KeyEvent.KEYCODE_BACK:
            ...
            if(!mDownloadInfo.getIfNeedRefreshMenu()){
                ...
                if (mDownloadInfo.getActivityID() < 0) {
                    mGoToMainEntry = true;
                    OtaPkgManagerActivity.this.startActivity(new Intent(OtaPkgManagerActivity.this,
                            MainEntry.class));
                }
            }
            break;
        ...
        if (mMenuStatus == MenuStatus.Menu_Download) {
            ...
            if (status == DownloadInfo.STATE_NEWVERSION_READY) {
                menu.add(
                        ...
                        MENU_ID_DOWNLOAD,
                        0,
                        R.string.btn_download).setShowAsAction(
                ...
                if (mDownloadInfo.getIfNeedRefreshMenu()) {
                    menu.add(
                            ...
                            MENU_ID_REFRESH,
                            0,
                            R.string.menu_stats_refresh).setShowAsAction(
                ...
                menu.add(
                        Menu.NONE,
                        MENU_ID_DOWNLOAD,
                        0,R.string.btn_resume).setShowAsAction(
                        MenuItem.SHOW_AS_ACTION_ALWAYS);
            }
            ...
            menu.add(Menu.NONE, MENU_ID_CANCEL, 0, R.string.btn_cancel).setShowAsAction(
                    MenuItem.SHOW_AS_ACTION_ALWAYS);
        }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        case MENU_ID_DOWNLOAD:
            ...
            if (!Util.isSdcardAvailable(this)) {
                ...
                Toast.makeText(this, getString(R.string.sdcard_crash_or_unmount), Toast.LENGTH_LONG)
                        .show();
                return true;
            }
...
/*            if (item.getItemId() == MENU_ID_DOWNLOAD) {
    ...
    private boolean downloadPkg() {
        ...
        long lSizeNeeded = Util
                .getExtraSpaceNeeded(this, (long) (mDownloadInfo.getUpdateImageSize()
        ...
        if (lSizeNeeded == -1) {
            ...
            Toast.makeText(this, getString(R.string.insufficient_space_content, lSizeNeeded),
    ...
    private void updateRatio() {
        ...
        if (mDownloadInfo.getDLSessionRenameState()
                && mDownloadInfo.getDownLoadPercent() == Util.MAX_PERCENT) {
            currSize = totalSize;
        ...
        /*
         * CharSequence ratioText = Integer.toString(ratio) + "%"; TextView dlRatio = (TextView)
         * findViewById(R.id.downloadingPercent);
         * 
         * dlRatio.setText(ratioText); fsize = (float) ((float) currSize / 1024.0); fsize = (float)
         * (((int) (fsize * 100)) / 100.0); tsize = (float) ((float) totalSize / 1024.0); tsize =
         * (float) (((int) (tsize * 100)) / 100.0); TextView dlProgress = (TextView)
         * findViewById(R.id.downloadingRatio);
         * dlProgress.setText(getString(R.string.download_per_info, fsize, tsize));
         */

        TextView dlRatio = (TextView) findViewById(R.id.downloadingRatio);
        if (totalSize >= Util.M_SIZE) {
            dlRatio.setText(getString(R.string.download_per_info_M, ratio, (double) currSize
                    / Util.M_SIZE, (double) totalSize / Util.M_SIZE));
        } else if (totalSize >= Util.K_SIZE) {
            dlRatio.setText(getString(R.string.download_per_info_K, ratio, (double) currSize
                    / Util.K_SIZE, (double) totalSize / Util.K_SIZE));
        } else {
...
/*      mMenuStatus = MenuStatus.Menu_Cancel;
    ...
    private void showDlInterface() {
        ...
        fillPkgInfo(mDownloadInfo.getAndroidNum(), mDownloadInfo.getVerNum(),
                mDownloadInfo.getUpdateImageSize(), Util.getPackageFileName(this));
    }
    ...
    private void showUILayout(int state) {
        ...
        case DownloadInfo.STATE_DLPKGCOMPLETE:
            ...
            fillPkgInfo(mDownloadInfo.getAndroidNum(), mDownloadInfo.getVerNum(),
                    mDownloadInfo.getUpdateImageSize(), null);
    ...
    protected Dialog onCreateDialog(int id, Bundle bundle) {
        ...
        case DIALOG_NOENOUGHSPACE:
            mDownloadStorageDialog = new AlertDialog.Builder(this)
        ...
        case DIALOG_NOSDCARD:
            mDownloadStorageDialog = new AlertDialog.Builder(this)
                    ...
                    .setMessage(
                            OtaPkgManagerActivity.this.getString(R.string.sdcard_crash_or_unmount))
                    .setPositiveButton(android.R.string.ok, new DialogInterface.OnClickListener() {
                        public void onClick(DialogInterface dialog, int whichButton) {
        ...
        case DIALOG_UNKNOWNERROR:
            mAlertDialog = new AlertDialog.Builder(this)
                    ...
                    .setMessage(
                            OtaPkgManagerActivity.this.getString(R.string.unknown_error_content))
                    .setPositiveButton(android.R.string.ok, new DialogInterface.OnClickListener() {
                        public void onClick(DialogInterface dialog, int whichButton) {
                            OtaPkgManagerActivity.this.finish();
                        }
        ...
        case DIALOG_CANCELDOWNLOAD:
            mDownloadCancelDialog = new AlertDialog.Builder(this)
                    .setTitle(R.string.cancel_download_title)
                    .setMessage(
                            OtaPkgManagerActivity.this.getString(R.string.cancel_download_content))
        ...
        case DIALOG_WARN_DATA_COST:
            mAlertDialog = new AlertDialog.Builder(this)
                    .setIcon(R.drawable.alert_downloading)
                    .setTitle(OtaPkgManagerActivity.this.getString(R.string.warn_data_cost_title))
                    .setMessage(
                            OtaPkgManagerActivity.this.getString(R.string.warn_data_cost_content))
        ...
        case DIALOG_CHANGE_NETWORK:
			...
			mAlertDialog = new AlertDialog.Builder(this)
					...
					.setMessage(R.string.change_network_content)
					.setCancelable(false)
					.setPositiveButton(R.string.change_network_wifi_btn,
					...
					.setNeutralButton(R.string.change_network_apn_btn,
							new DialogInterface.OnClickListener() {
								public void onClick(DialogInterface dialog,
										int whichButton) {
							        ...
							        if (FeatureOption.MTK_GEMINI_SUPPORT) {
										long simid = Settings.System.getLong(getContentResolver(), Settings.System.GPRS_CONNECTION_SIM_SETTING,
					...
					.setNegativeButton(R.string.change_network_discard_btn,
							new DialogInterface.OnClickListener() {
								public void onClick(DialogInterface dialog,
										int whichButton) {
        ...
        case DIALOG_PACKAGE_DELETED:
            mAlertDialog = new AlertDialog.Builder(this)
            .setTitle(R.string.btn_resume)
            .setMessage(R.string.package_deleted_error)
            .setCancelable(false)
            .setPositiveButton(android.R.string.ok, new DialogInterface.OnClickListener() {
                public void onClick(DialogInterface dialog, int whichButton) {
    ...
    protected boolean checkUpgradePackage(String strPkgPath) {

        CheckPkg checkPkg = new CheckPkg(this, UpgradePkgManager.OTA_PACKAGE, strPkgPath);
    ...
    protected boolean checkUpgradePackage() {
        ...
        CheckPkg checkPkg = new CheckPkg(this, UpgradePkgManager.OTA_PACKAGE,
    ...
    protected void notifyUserInstall() {
        ...
        mHandler.sendMessage(mHandler
                .obtainMessage(SystemUpdateService.MSG_NOTIFY_POWEROFF));
    ...
    private Handler mHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            /*
             * case SystemUpdateService.MSG_DELTADELETED: onRequeryNeed(); break;* case
             * SystemUpdateService.MSG_SDCARDCRASHORUNMOUNT:
    ...
    private ServiceConnection mConnection = new ServiceConnection() {
        ...
        public void onServiceConnected(ComponentName className, IBinder service) {
			...
			case DownloadInfo.STATE_NEWVERSION_READY:
				...
				clearNotification(OtaPkgManagerActivity.this,
						NotifyManager.NOTIFY_NEW_VERSION);
				mService.startDlPkg();
				// M: add by mtk80800: enter the downloading & installing
				// process, mark it
				DownloadInfo.getInstance(getApplicationContext())
						.setActivityID(Integer.MAX_VALUE);
