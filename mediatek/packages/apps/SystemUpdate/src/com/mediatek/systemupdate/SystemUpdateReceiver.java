    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (action.equalsIgnoreCase(ConnectivityManager.CONNECTIVITY_ACTION)) {
            ...
            if ((status == DownloadInfo.STATE_PAUSEDOWNLOAD)
            		&& (downloadInfo.getOtaAutoDlStatus())){

            	if ((networkStatus == Util.NETWORK_STATUS.STATE_WIFI)
            			&& downloadInfo.getIfPauseWithinTime()) {
            ...
            if (status == DownloadInfo.STATE_QUERYNEWVERSION
                ...
                Util.setAlarm(context, AlarmManager.RTC, Calendar.getInstance()
                        .getTimeInMillis() + QUERY_TIME_DELAY,
                        Util.Action.ACTION_AUTO_QUERY_NEWVERSION);
            } else {
                Util.setAlarm(context, AlarmManager.RTC, getNextAlarmTime(),
                        Util.Action.ACTION_AUTO_QUERY_NEWVERSION);
...
/*            boolean updateFlag = getUpdateStatus();
            ...
            if (status == DownloadInfo.STATE_DOWNLOADING) {
                mDownLoadStatus
                        .setDLSessionStatus(DownloadInfo.STATE_PAUSEDOWNLOAD);
				...
				Intent installNotify = new Intent(mContext,
						ForegroundDialogService.class);
				installNotify.putExtra(ForegroundDialogService.DLG_ID,
						ForegroundDialogService.DIALOG_INSTALL_REMINDER);
        	...
        	if (getUpdateStatus()) {
                Util.setAlarm(context, AlarmManager.RTC, Calendar.getInstance()
                        .getTimeInMillis() + SHOW_RESULT_TIME_DELAY, Util.Action.ACTION_UPDATE_REPORT);
                if (status == DownloadInfo.STATE_QUERYNEWVERSION) {
                    deleteUpdatePkg();
                }
            ...
            if (status == DownloadInfo.STATE_QUERYNEWVERSION
                ...
                Util.setAlarm(context, AlarmManager.RTC, getNextAlarmTime(),
                        Util.Action.ACTION_AUTO_QUERY_NEWVERSION);
                Xlog.i(TAG, "ACTION_BOOT_COMPLETED, status = " + status
                        + ", need not query, set next query alarm");
            }
    ...
    private void showDlReminderNotification(Context context, boolean IsForDL) {
		...
		if (IsForDL) {
			...
			if (Util.isSdcardAvailable(context)) {
				...
				notification.showDownloadingNotificaton(downloadInfo
						.getVerNum(), (int) (((double) Util.getFileSize(Util
						.getPackageFileName(mContext)) / (double) downloadInfo
    ...
    private long getNextAlarmTime() {
        ...
        if (currDayOfWeek < NEWVERSION_AUTO_QUERY_DAY_OF_WEEK1) {
            ...
            daysToAdd = NEWVERSION_AUTO_QUERY_DAY_OF_WEEK1 + Calendar.SATURDAY
                    - currDayOfWeek;
        }
    ...
    boolean getQueryRequest() {
        Calendar calendar = Calendar.getInstance();
        if (NEWVERSION_AUTO_QUERY_DAY_OF_WEEK1 == calendar
                .get(Calendar.DAY_OF_WEEK)
                || NEWVERSION_AUTO_QUERY_DAY_OF_WEEK2 == calendar
                        .get(Calendar.DAY_OF_WEEK)) {
            return true;
        }
        return false;
    ...
    private boolean getUpdateStatus() {
		...
		DownloadInfo downloadStatus = DownloadInfo.getInstance(mContext
    ...
    private String getResultFileName() {
        return mContext.getResources()
                .getString(R.string.address_update_result);
    }
...
/*            try {
                ...
                if (!file.exists()) {
                    Xlog.i(TAG, "getUpdateResult, report file not exist");
                    return false;
                }
                InputStream resStr = mContext
                        .openFileInput(getResultFileName());
                if (resStr == null) {
                    Xlog.i(TAG, "getUpdateResult, inputstream error");
                    return false;
                }
                byte[] b = new byte[10];
                int num = resStr.read(b);
                Xlog.i(TAG, "getUpdateResult, num = " + num);
                if (num < 0) {
                    return false;
                }
                resStr.close();
                ...
                Xlog.i(TAG, "getUpdateResult, result = " + result);
