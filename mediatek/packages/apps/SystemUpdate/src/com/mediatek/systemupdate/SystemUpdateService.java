...
//    static final int MSG_SDCARDCRASHORUNMOUNT = 9;
        ...
        if (Util.Action.ACTION_AUTO_QUERY_NEWVERSION.equals(action)) {
            ...
            if (downloadInfo.getActivityID() >= 0) {
                ...
                if (mSdcardScanner != null) {
                    ...
                    Xlog.v(TAG,
                            "[onStartCommand] mounted,Package scaning, tell scanner the mounted path "
                                    + storagePath);
            ...
            if (order >= 0) {
                ...
                if ((path != null && path.contains(storagePath))
                    ...
                    Xlog.v(TAG,
                            "[onStartCommand] removed,Not downloading/installing SD card crash, ignore.");
                }
            } else {
                if (Util.getUpdateType().equals(Util.UPDATE_TYPES.OTA_UPDATE_ONLY)) {
                    Xlog.v(TAG,
                            "[onStartCommand] removed, OTA only ,and NOT downloading/installing process, ignore");
                } else {

                    Xlog.v(TAG,
                            "[onStartCommand] removed, NOT downloading/installing process, requery");

                    if (isQuerying()) {
                        if (mSdcardScanner != null) {
                            Xlog.v(TAG,
                                    "[onStartCommand] removed,Package scaning, tell scanner the crashed path");
                            mSdcardScanner.onSdcardCrashed(storagePath);
                        }
                        Toast.makeText(this, R.string.sdcard_unmount, Toast.LENGTH_LONG).show();
                    } else {
                        Xlog.v(TAG,
                                "[onStartCommand] removed,Package scan done, setIfNeedRefresh true");
                        downloadInfo.setIfNeedRefresh(true);
                        NotifyManager notification = new NotifyManager(this);
                        notification.clearNotification(NotifyManager.NOTIFY_NEW_VERSION);
    ...
    boolean isQuerying() {
        if (sQueryNewVersionThread != null && sQueryNewVersionThread.isAlive()) {
            Xlog.i(TAG, "onQueryNewVersion back from interrupt, mQueryNewVersionThread="
    ...
    boolean isDownloading() {
        if (sDlPkgProgressThread != null && sDlPkgProgressThread.isAlive()) {
            Xlog.i(TAG, "sDlPkgProgressThread back from interrupt, sDlPkgProgressThread="
    ...
    class SessionStateControlThread extends Thread {
        ...
        public void run() {
            ...
            Xlog.i(TAG, "SessionStateControlThread, status = " + status);
