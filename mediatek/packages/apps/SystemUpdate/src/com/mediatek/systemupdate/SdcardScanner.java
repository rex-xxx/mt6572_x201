...
public class SdcardScanner {
    ...
    private static final String[] FILE_lIST = new String[] { "update.zip", "configure.xml" };
    ...
    private SdcardScanner(Context context) {
        ...
        mPackagePath = mTempUnzipDir + Util.PathName.PACKAGE_NAME;
        mMd5sumPath = mTempUnzipDir + Util.PathName.MD5_FILE_NAME;
    ...
    private void getPackageFile(File file) {
        ...
        for (File f : fileList) {
            if (f.isDirectory()) {
                Xlog.v(TAG, "Look for folder: " + f.getAbsolutePath());
                getPackageFile(f);
            } else {
    ...
    private void unpackAndVerify(File updateFile) {
        ...
        Log.d(TAG, "[unpackAndVerify]++ start unpacking & verification ++" + absolutPath);

        if (!unzipAndCheckSum(updateFile)) {
            Xlog.w(TAG, "[unpackAndVerify] unzipAndCheckSum fail");
        ...
        // Check OTA
        CheckPkg checkPkg = new CheckPkg(mContext, UpgradePkgManager.SD_PACKAGE, mPackagePath);
    ...
    private boolean unzipAndCheckSum(File updateFile) {

        if (Util.getExtraSpaceNeeded(mContext, (long) (1.5 * updateFile.length())) < 0) {
            Log.e(TAG, "[unzipAndCheckSum] Storage not enough");
            mErrorCode = Util.SDresult.ERROR_OUT_OF_MEMORY;
            mSpaceHandler.sendMessage(mSpaceHandler.obtainMessage(SPACE_TOAST, updateFile.getPath()));
            return false;
        }
        ...
        if (zipFile == null) {
            Xlog.w(TAG, "[unzipAndCheckSum] zipfile is null");
            return false;
        }

        int result = Util.unzipFileElement(zipFile, Util.PathName.MD5_FILE_NAME.substring(1),
                mMd5sumPath);
        if (Util.OTAresult.OTA_FILE_UNZIP_OK != result) {
            Log.w(TAG, "[unzipAndCheckSum] unzip md5sum file fail : " + result);
            mErrorCode = result;
            try {
                zipFile.close();
            } catch (IOException e) {
        ...
        result = Util.unzipFileElement(zipFile, Util.PathName.ENTRY_CONFIGURE, mConfigPath);
        if (Util.OTAresult.OTA_FILE_UNZIP_OK != result) {
            Log.w(TAG, "[unzipAndCheckSum] unzip configure.xml file fail : " + result);
            mErrorCode = result;
            try {
                zipFile.close();
            } catch (IOException e) {
        ...
        result = Util.unzipFileElement(zipFile, Util.PathName.PACKAGE_NAME.substring(1),
                mPackagePath);
        try {
            zipFile.close();
        } catch (IOException e) {
        ...
        if (Util.OTAresult.OTA_FILE_UNZIP_OK != result) {
            Log.w(TAG, "[unzipAndCheckSum] unzip update.zip fail : " + result);
            mErrorCode = result;
            return false;
        }
        return UpgradePkgManager.checkSdPackage(mTempUnzipDir, FILE_lIST);
    }

    private boolean checkVersion(PackageInfoReader sdPkgInfoReader) {

        if (OEM_DEV == null || !OEM_DEV.equals(sdPkgInfoReader.getOem())) {
            Xlog.w(TAG, "not correct oem for this package");
        ...
        if (PRODUCT_DEV == null || !PRODUCT_DEV.equals(sdPkgInfoReader.getProduct())) {
            Xlog.w(TAG, "not correct product for this package");
            mErrorCode = Util.SDresult.ERROR_VERSION_ERR;
            Util.deleteFile(mTempUnzipDir);
            return false;
        }
        ...
        if (!((TextUtils.isEmpty(OPER_DEV) && TextUtils.isEmpty(operPkg)) || OPER_DEV
                .equals(operPkg))) {
            Xlog.w(TAG, "not correct operator for this package");
            mErrorCode = Util.SDresult.ERROR_VERSION_ERR;
            Util.deleteFile(mTempUnzipDir);
            return false;
        }
        ...
        if (!((TextUtils.isEmpty(FLAVOR_DEV) && TextUtils.isEmpty(flavorPkg)) || FLAVOR_DEV
                .equals(flavorPkg))) {
            Xlog.w(TAG, "not correct flavor for this package");
            mErrorCode = Util.SDresult.ERROR_VERSION_ERR;
            Util.deleteFile(mTempUnzipDir);
            return false;
        }
        ...
        if (!((TextUtils.isEmpty(LANG_DEV) && TextUtils.isEmpty(langPkg)) || LANG_DEV
                .equals(langPkg))) {
            Xlog.w(TAG, "not correct language for this package");
            mErrorCode = Util.SDresult.ERROR_VERSION_ERR;
            Util.deleteFile(mTempUnzipDir);
            return false;
        }
        ...
        if (time == null || time.compareTo(BUILDTIME_DEV) <= 0) {
            Xlog.w(TAG, "not correct buildTime for this package");
            mErrorCode = Util.SDresult.ERROR_VERSION_ERR;
            Util.deleteFile(mTempUnzipDir);
            return false;
        }
