...
/**
 * CheckOta is used to check if the upgrade package matches the load. The checked items include
 * product, finger-print, partition layout, etc
 * 
 * @author mtk80357
 */
class CheckPkg {
    ...
    private static final String LAST_PARTITION_NAME = "bmtpool";
    ...
    private static final String[][] PARTITION_NAME = {
            { "", "USRDATA", "__NODL_CACHE", "ANDROID" }, { "", "USRDATA", "CACHE", "ANDROID" },
    ...
    /**
     * PartitionInfo is used to record one partition information of name and offset.
     * 
     * @author mtk80357
     */
    ...
    /**
     * LoadVersionInfo records version related information of one loads or upgrade package.
     * 
     * @author mtk80357
     */
    ...
    private boolean checkVersion(ZipFile zipFile, int otaType) {
        ...
        Xlog.i(TAG, "infoPkg: " + infoPkg.mDevNum + " " + infoPkg.mProNum + " "
                + infoPkg.mFingerPrint1 + " " + infoPkg.mFingerPrint2);
        getVersionInfoDev(infoDev, otaType);
        Xlog.i(TAG, "infoDev: " + infoDev.mDevNum + " " + infoDev.mProNum + " "
                + infoDev.mFingerPrint1 + " " + infoDev.mFingerPrint2);
    ...
    private boolean comparePartition(List<PartitionInfo> partitionDev,
        ...
        if (!mIsEmmcPhone) {
            ...
            Xlog.i(TAG, "partitionDev.get(length-1).mOffset = "
                    + partitionDev.get(length - 1).mOffset
                    + " partitionPkg.get(length - 1).mOffset = "
                    + partitionPkg.get(length - 1).mOffset);

            if (partitionDev.get(length - 1).mOffset != partitionPkg.get(length - 1).mOffset) {
                mOtaResult = Util.OTAresult.ERROR_PARTITION_SETTING;
                return false;
            }

            if (otaType == DELTA_PKG) {
                for (int k = (length - 3); k < length - 1; k++) {
                    Xlog.i(TAG,
                            "k = " + k + " partitionDev.get(k).mOffset = "
                                    + partitionDev.get(k).mOffset
                                    + " partitionPkg.get(k).mOffset = "
    ...
    private boolean getPartitionInfoPkg(ZipFile zipFile, List<PartitionInfo> partitionPkg) {
        ...
        if (!unzipFileElement(zipFile, Util.PathName.ENTRY_SCATTER, mScatterFilePath)) {
            Xlog.i(TAG, "unzip fail from " + Util.PathName.ENTRY_SCATTER + " to "
                    + mScatterFilePath);
            return false;
        }
        try {
    ...
    private boolean getPartitionInfoDev(List<PartitionInfo> partitionDev) {
        ...
        try {
            ...
            while ((line = bfReader.readLine()) != null) {
                ...
                if (LAST_PARTITION_NAME.equals(partitionInfoArray[0])) {

                    break;
                }

                PartitionInfo info = new PartitionInfo(partitionInfoArray[0], offset);
                Xlog.i(TAG, "name = " + partitionInfoArray[0] + " offset is " + offset);
				...
				if (FeatureOption.MTK_EMMC_SUPPORT
						&& (FAT_PARTITION_NAME
								.equalsIgnoreCase(partitionInfoArray[0]))) {
					break;
				}
                ...
                if ((partitionInfoArray[1] != null) && (partitionInfoArray[1].length() >= 3)) {
                    ...
                    String strTemp = partitionInfoArray[1].substring(2);
                    offset += Long.parseLong(strTemp, OFFSET_RADIX);
        ...
        for (int k = 0; k < PARTITION_NAME.length; k++) {
            ...
            for (; index < num; index++) {
                ...
                if ((strName != null) && (strName.length() >= length)) {
                    ...
                    Xlog.i(TAG, "PARTITION_NAME[k][index] = " + PARTITION_NAME[k][index]
                            + " strName = " + strName);
