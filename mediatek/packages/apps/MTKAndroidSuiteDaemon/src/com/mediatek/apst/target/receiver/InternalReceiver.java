...
public class InternalReceiver extends BroadcastReceiver {
    ...
    public static final String ACTION_USB_DISCONNECT = "android.intent.action.UMS_DISCONNECTED";
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (null == strAction) {
            ...
            /*
             * boolean isSimInfoChanged = false; boolean isSim1InfoChanged =
             * false; boolean isSim2InfoChanged = false; if
             * (Intent.SIM_SETTINGS_INFO_CHANGED.equals(strAction)) { if
             * (Config.MTK_GEMINI_SUPPORT){ long simId =
             * intent.getLongExtra(SIM_ID, -1); SimDetailInfo simInfo =
             * Global.getSimInfoById((int) simId); if (simInfo != null) { int
             * slotId = simInfo.getSlotId(); if (0 == slotId) {
             * isSim1InfoChanged = true; } else if (1 == slotId) {
             * isSim2InfoChanged = true; } }
             * 
             * } else { isSimInfoChanged = true; } } if
             * (Config.MTK_GEMINI_SUPPORT){ int sim1State =
             * SystemInfoProxy.getSimState(Message.SIM1_ID); int sim2State =
             * SystemInfoProxy.getSimState(Message.SIM2_ID); boolean sim1OK =
             * SystemInfoProxy.isSimAccessible(sim1State); boolean sim2OK =
             * SystemInfoProxy.isSimAccessible(sim2State); SimDetailInfo
             * sim1Info = Global.getSimInfoBySlot(Message.SIM1_ID);
             * SimDetailInfo sim2Info =
             * Global.getSimInfoBySlot(Message.SIM2_ID); if (null == mSim1OK ||
             * mSim1OK != sim1OK || isSim1InfoChanged == true){
             * EventDispatcher.dispatchSimStateChangedEvent( new Event()
             * .put(ISimStateListener.STATE, sim1State)
             * .put(ISimStateListener.SIM_ID, Message.SIM1_ID) // Modified by
             * Shaoying Han .put(ISimStateListener.SIM_INFO, sim1Info)
             * .put(ISimStateListener.SIM_INFO_FLAG, isSimInfoChanged)); } if
             * (null == mSim2OK || mSim2OK != sim2OK || isSim2InfoChanged ==
             * true){ EventDispatcher.dispatchSimStateChangedEvent( new Event()
             * .put(ISimStateListener.STATE, sim2State)
             * .put(ISimStateListener.SIM_ID, Message.SIM2_ID) // Modified by
             * Shaoying Han .put(ISimStateListener.SIM_INFO, sim2Info)
             * .put(ISimStateListener.SIM_INFO_FLAG, isSimInfoChanged)); }
             * mSim1OK = sim1OK; mSim2OK = sim2OK; } else { int simState =
             * SystemInfoProxy.getSimState(Message.SIM_ID); boolean simOK =
             * SystemInfoProxy.isSimAccessible(simState); SimDetailInfo simInfo
             * = Global.getSimInfoBySlot(Message.SIM_ID); if (null == mSimOK ||
             * mSimOK != simOK || isSimInfoChanged == true){
             * EventDispatcher.dispatchSimStateChangedEvent( new Event()
             * .put(ISimStateListener.STATE, simState)
             * .put(ISimStateListener.SIM_ID, Message.SIM_ID) // Modified by
             * Shaoying Han .put(ISimStateListener.SIM_INFO, simInfo)
             * .put(ISimStateListener.SIM_INFO_FLAG, isSimInfoChanged)); }
             * mSimOK = simOK; }
             */
        }
