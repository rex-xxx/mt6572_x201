...
public class VCalServiceTest extends AndroidTestCase {
    ...
    public void test04_VCalComposer() {
        mHelper = new BindServiceHelper(getContext(), new VCalServiceConnectedListener());
        mHelper.onBindService();
        TestUtils.sleepForTime(TestUtils.DEFAULT_SLEEP_TIME);
        int sleepTimes = 0;
        ...
        Uri testEventUri = TestUtils.addOneEventsToDB(getContext().getContentResolver());
        Long eventId = Long.valueOf(testEventUri.getLastPathSegment());
        createUiHandler();
        File vcsFile = TestUtils.addFile("testVcs.vcs", null);
        Log.d(TAG,"file length: " + vcsFile.length());
        Uri dstFileUri = Uri.fromFile(vcsFile);
        Account account = new Account("PC Sync","LOCAL");
        ExportProcessor exportProcessor = new ExportProcessor(mService, mUiHandler, account, dstFileUri);
        exportProcessor.run();
        TestUtils.sleepForTime(TestUtils.DEFAULT_SLEEP_TIME);
        Log.d(TAG,"end file length: " + vcsFile.length());

        VCalComposer mComposer = new VCalComposer(vcsFile,getContext(),eventId , exportProcessor);
        mComposer.startEventCompose(eventId);
        assertNotNull(mComposer.buildVEventString(eventId));
        Log.d(TAG,"mComposer end file length: " + vcsFile.length());
        assertTrue(vcsFile.length() > 0);
        mUiHandler = null;
        mService.disconnected(mHelper.getClass().getName());
        mService.onFinish();
        TestUtils.removeTheAddedEvent(getContext().getContentResolver(),testEventUri);
        TestUtils.removeFile(vcsFile);
    }
