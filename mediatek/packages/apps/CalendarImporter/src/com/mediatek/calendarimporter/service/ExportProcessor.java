...
public class ExportProcessor extends BaseProcessor implements VCalStatusChangeOperator {
    ...
    public ExportProcessor(VCalService service, Handler uiHandler, Uri eventsUri) throws IllegalArgumentException {
        ...
        mComposer = new VCalComposer(null, mService, selection, this);
        mMode = MEMORY_FILE_MODE;
    }

    /**
     * Constructor for single account compose, do not be used at present.
     * 
     * @param service
     *            the service to control this processor
     * @param uiHandler
     ...
     */
    public ExportProcessor(VCalService service, Handler uiHandler, Account srcAccount, Uri dstFileUri) {
        mService = service;
        mUiHandler = uiHandler;
        mComposer = new VCalComposer(new File(dstFileUri.getPath()), srcAccount.name, mService, this);
        mMode = FILE_MODE;
    }
    ...
    public void run() {
        ...
        if (mMode == MEMORY_FILE_MODE) {
            ...
            mComposer.startAccountsCompose();
        }
