...
public class AudioTest extends ActivityInstrumentationTestCase2<Audio> {
    ...
    private static final int LIST_ITEMS_COUNT = 8;
    ...
    public void testCase07_TestSpeechEnhanc() {
        ...
        mSolo.sleep(EmOperate.TIME_MID);
        mSolo.goBack();
        ArrayList<Spinner> spiners = mSolo.getCurrentSpinners();
    ...
    public void testCase09_TestSpeechLog() {
        verifyPreconditions();
        mSolo.clickOnText(mAudioFuncList.getAdapter().getItem(6).toString());
        mSolo.sleep(EmOperate.TIME_MID);
        ...
        ArrayList<RadioButton> radioButtons = mSolo.getCurrentRadioButtons();
        assertNotNull(radioButtons);
        assertEquals(2, radioButtons.size());

        ArrayList<CheckBox> checkBoxs = mSolo.getCurrentCheckBoxes();
