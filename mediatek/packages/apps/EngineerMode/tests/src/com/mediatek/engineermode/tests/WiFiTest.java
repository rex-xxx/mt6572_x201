...
//    private static final String KEY_PROP_OPEN_AP_WPS = "mediatek.wlan.openap.wps";
    ...
    private static final int MODE_NUMBER = 5;
    ...
    public void test06_CbWps() {
        if (sSuccess) {
            ...
            boolean expect = VALUE_TRUE.equals(SystemProperties.get(
                    KEY_PROP_WPS2_SUPPORT, VALUE_FALSE));
            assertEquals(expect, actual);
    ...
    public void test08_TxChannel() {
        if (sSuccess) {
            ...
            assertEquals(CHANNEL_NUMBER - 4, mWifiChannel.getAdapter()
                    .getCount());
    ...
    public void test11_TxGo() {
        if (sSuccess) {
            ...
            assertEquals("54M", mWifiRate.getSelectedItem().toString());
