...
public class FreqHoppingSet extends Activity {
    ...
    private int[] mFreqHopping = new int[FREQ_HOPPING_SIZE];
    ...
    private boolean validateInput(boolean bEtInput) {
        ...
        if (result) {
            ...
            if (select < 0 || select >= FREQ_HOPPING_SIZE) {
                result = false;
            }
        }
        if (!result) {
    ...
    private boolean updateHoppingStatus() {
        ...
        int[] order = { 1, 0, 3, 4, 5, 2 };
        if (null != debugMsg) {
            Pattern pattern = Pattern.compile(PATSTR);
            Matcher m = pattern.matcher(debugMsg);
            int index = 0;
            try {
                while (m.find() && index < FREQ_HOPPING_SIZE) {
                    String msg = m.group();
                    if (null != msg) {
                        int value = Integer.parseInt(msg.substring(1,
                                msg.length() - 1).trim());
                        Xlog.v(TAG, "index: " + index + " value: " + value);
                        mFreqHopping[order[index]] = value;
                        index++;
                    }
                }
                result = index == FREQ_HOPPING_SIZE;
    ...
    private void updateBtnStatus(int position) {
        if (position < 0 || position >= FREQ_HOPPING_SIZE
