...
import com.mediatek.telephony.TelephonyManagerEx; // For M API Refactor

/**
 * Description: To change network type.
 * 
 * 
 * 
 */

public class DigitalStandard extends PreferenceActivity implements Preference.OnPreferenceChangeListener {

    private static final String TAG = "DigitalStandard";

    private boolean mHasNetworkMode;
    ...
    static final int PREFERRED_NT_MODE = Phone.PREFERRED_NT_MODE;

//    private String[] mNetworkModeSummary;
    private static final int NETMODESUMMARY_LENGTH = 3;

//    private TelephonyManager mTelephonyManager;
    private TelephonyManagerEx mTelephonyManager; // For M API Refactor
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(Intent.ACTION_AIRPLANE_MODE_CHANGED)) {
                ...
                if (mHasNetworkMode) {
                    ...
                    if (mIsSlot1Insert && TelephonyManager.getDefault().getSimState() == TelephonyManager.SIM_STATE_READY) {
                        sim1Ready = true;
                    }
                ...
                if (intent.getBooleanExtra(NETWORK_MODE_CHANGE_RESPONSE, true)) {
                    Elog.d(TAG, "Receiver: network mode change succeed! set the new value.");
                    Settings.Global.putInt(getContentResolver(), Settings.Global.PREFERRED_NETWORK_MODE, intent.getIntExtra(
                            NEW_NETWORK_MODE, 0));
                } else {
                    Elog.d(TAG, "Receiver: network mode change failed! restore the old value.");
                    Settings.Global.putInt(getContentResolver(), Settings.Global.PREFERRED_NETWORK_MODE, intent.getIntExtra(
                            OLD_NETWORK_MODE, 0));
                }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mHasNetworkMode = !FeatureOption.MTK_GEMINI_SUPPORT
        ...
        mIsSlot1Insert = mGeminiPhone.isSimInsert();
        // mIsSlot1Insert = mGeminiPhone.isSimInsert(PhoneConstants.GEMINI_SIM_1);
        // mIsSlot2Insert = mGeminiPhone.isSimInsert(PhoneConstants.GEMINI_SIM_2);

        if (mIsSlot1Insert) {
            generalSettingsCategory.removePreference(mNetworkModeGemini);
            mNetworkMode.setOnPreferenceChangeListener(this);
            intentFilter.addAction(NETWORK_MODE_CHANGE_RESPONSE);
        } else {
        ...
        // mNetworkModeSummary = new String[NETMODESUMMARY_LENGTH];
...
//        mTelephonyManager = (TelephonyManager) getSystemService(Context.TELEPHONY_SERVICE);
    ...
    protected void onResume() {
        ...
        if (mHasNetworkMode) {
            ...
            if (isCallIdle) {
                boolean sim1Ready = false;
                if (mIsSlot1Insert) {
                    // sim1Ready = mGeminiPhone.isRadioOn();
            ...
            if ((mIsSlot1Insert) && (mIsSlot2Insert)) {
                Elog.w(TAG, "Only support TD single card");
    ...
    public boolean onPreferenceChange(Preference arg0, Object arg1) {
        ...
        if (KEY_NETWORK_MODE_SETTING.equals(key)) {
            ...
            int oldNetworkMode = Settings.Global.getInt(getContentResolver(), Settings.Global.PREFERRED_NETWORK_MODE,
                    PREFERRED_NT_MODE);
            ...
            if (newNetworkMode != oldNetworkMode) {
                ...
                Intent intent = new Intent(NETWORK_MODE_CHANGE_BROADCAST, null);
    ...
    protected Dialog onCreateDialog(int id) {
        ...
        if (id == DIALOG_NETWORK_MODE_CHANGE) {
    ...
    private boolean isCallStateIdle() {
        // API Refactor
//        int stateSim1 = mTelephonyManager.getCallStateGemini(PhoneConstants.GEMINI_SIM_1);
//        int stateSim2 = mTelephonyManager.getCallStateGemini(PhoneConstants.GEMINI_SIM_2);
        int stateSim1 = mTelephonyManager.getCallState(PhoneConstants.GEMINI_SIM_1);
        int stateSim2 = mTelephonyManager.getCallState(PhoneConstants.GEMINI_SIM_2);
