...
public class ModemTestActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mNewVersion = ChipSupport.getChip() >= ChipSupport.MTK_6589_SUPPORT;
        if (mNewVersion) {
    ...
    private void handleQuery(String[] data) {
        ...
        if (data[0].length() > CMD_LENGTH) {
            ...
            if (mode.length() >= MODE_LENGTH) {
                ...
                if ("0".equals(subMode)) {
                    ...
                    if (mNewVersion) {
                        ...
                        try {
                            ...
                            for (int j = 0; j < iotLength; j++) {
                                Elog.i(TAG, "j ==== " + j);
                                Elog.i(TAG, "(val & (1 << j)) is "
                                        + (val & (1 << j)));
                                if ((val & (1 << j)) != 0) {
                                    text = text + mIotOptionsArray[j] + ",";
                                }
                            }
                            // Drop the last ","
                            text = text.substring(0, text.length() - 1);
                ...
                } else if (mNewVersion && "5".equals(subMode)) {
    ...
    protected Dialog onCreateDialog(int id) {
        ...
        case MODEM_IOT:
            return new AlertDialog.Builder(ModemTestActivity.this).setTitle(
                    "MODEM TEST").setMultiChoiceItems(
                    mNewVersion ? R.array.modem_test_iot_options_6589
        ...
        case MODEM_OPERATOR:
            return new AlertDialog.Builder(ModemTestActivity.this).setTitle(
                    "MODEM TEST").setMultiChoiceItems(
                    mNewVersion ? R.array.modem_test_operator_options_6589
