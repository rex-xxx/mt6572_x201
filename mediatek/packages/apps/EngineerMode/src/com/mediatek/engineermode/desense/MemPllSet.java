...
public class MemPllSet extends Activity {
    ...
    protected void onResume() {
        ...
        mTvCurrent.setText(CURRENT_PRE
                + (current < 266 ? VALUE_200 : VALUE_266));
        updateBtnStatus(mTvCurrent.getText().toString().contains(VALUE_200));
    }

    private void doConvert(boolean bValue200) {
        boolean result = setCurrentDramc(bValue200 ? VALUE_200 : VALUE_266);
        if (result) {
            mTvCurrent.setText(CURRENT_PRE
                    + (bValue200 ? VALUE_200 : VALUE_266));
