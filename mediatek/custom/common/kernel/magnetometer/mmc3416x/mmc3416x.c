...
#define MMC3416X_DELAY_RM	10	/* ms */
#define READMD			0
...
//static int mmc3416x_i2c_detect(struct i2c_client *client, int kind, struct i2c_board_info *info);
...
time1 = current_kernel_time();
...
#if 0	
	/* do RESET/SET every MMC3416X_RESET_INTV times read */
	if (!(read_idx % MMC3416X_RESET_INTV))
	{
		...
		I2C_TxData(data, 2);
		//delay.tv_sec =0;
		//delay.tv_nsec = MMC3416X_DELAY_STDN *1000;
...
//static int mmc3416x_i2c_detect(struct i2c_client *client, int kind, struct i2c_board_info *info) 
//{    
//	strcpy(info->type, MMC3416X_DEV_NAME);
//	return 0;
//}
...
//modified for mmc3416x----changhua start
#if 0
...
#endif
	//modified for mmc3416x----changhua end
