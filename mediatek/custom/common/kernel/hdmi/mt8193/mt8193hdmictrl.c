...
void vSetHDMIDataEnable(u8 bResIndex)
{
  ...
  if(bResIndex < HDMI_VIDEO_RESOLUTION_NUM)
  {
   ...
   vWriteHdmiDGIMsk(tg_ctrl04, HVSYNC_TOTAL_WIDTH_ACTIVE[bResIndex][5]<<5, 0xffffffff);
...
void vSetHDMISyncDelay(u8 bResIndex)
{
  ...
  vWriteHdmiDGIMsk(tg_ctrl01, HVSYNC_DELAY[bResIndex], rg_vsync_delay|rg_hsync_delay|rg_vsync_forward);
}
...
void vShowHpdRsenStatus(void)
{
  ...
  printk("[HDMI]SI_HDMI_RECEIVER_STATUS = HDMI_PLUG_IN_ONLY\n");
  else if(i4SharedInfo(SI_HDMI_RECEIVER_STATUS) == HDMI_PLUG_IN_AND_SINK_POWER_ON)
  printk("[HDMI]SI_HDMI_RECEIVER_STATUS = HDMI_PLUG_IN_AND_SINK_POWER_ON\n");
  else if(i4SharedInfo(SI_HDMI_RECEIVER_STATUS) == HDMI_PLUG_OUT)
  printk("[HDMI]SI_HDMI_RECEIVER_STATUS = HDMI_PLUG_OUT\n");
  else
  printk("[HDMI]SI_HDMI_RECEIVER_STATUS error\n");
...
void vShowDeepColor(void)
{
  ...
  printk("[HDMI]HDMI output deepcolor = HDMI_NO_DEEP_COLOR\n");
  else if(_stAvdAVInfo.e_deep_color_bit == HDMI_DEEP_COLOR_10_BIT)
  printk("[HDMI]HDMI output deepcolor = HDMI_DEEP_COLOR_10_BIT\n");
  else if(_stAvdAVInfo.e_deep_color_bit == HDMI_DEEP_COLOR_12_BIT)
  printk("[HDMI]HDMI output deepcolor = HDMI_DEEP_COLOR_12_BIT\n");
  else if(_stAvdAVInfo.e_deep_color_bit == HDMI_DEEP_COLOR_16_BIT)
  printk("[HDMI]HDMI output deepcolor = HDMI_DEEP_COLOR_16_BIT\n");
  else
  printk("[HDMI]HDMI output deepcolor error\n");
  ...
  for(i=0; i<10; i++)
  {
    ...
    else
    {
     if((u4Data!=(dReadHdmiDGI(dgi1_crc_out)&0xffff))||(u4Data==0))
     {
      ...
      printk("[HDMI]dgi crc error \n");
      return ;
     }
    }
  }
  printk("[HDMI]dgi crc pass \n");
}
