...
UINT32 S5K3H7YMIPISetTestPatternOutput(bool on)
{    
	if(on == KAL_TRUE) 
	{        
	}    
                                                                ...
                                                                UINT8 *pFeaturePara,UINT32 *pFeatureParaLen)
{
    ...
    switch (FeatureId)
    {
		...
		case SENSOR_FEATURE_GET_DEFAULT_FRAME_RATE_BY_SCENARIO:
			...
			break;
		case SENSOR_FEATURE_SET_TEST_PATTERN_OUTPUT://for factory mode auto testing 	   
			S5K3H7YMIPISetTestPatternOutput((bool)*pFeatureData16); 		
			break;		
