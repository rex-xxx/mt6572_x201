...
static const struct i2c_device_id tpd_i2c_id[] = {{"gt928", 0}, {}};
static unsigned short force[] = {0, 0xBA, I2C_CLIENT_END, I2C_CLIENT_END};
static const unsigned short *const forces[] = { force, NULL };
//static struct i2c_client_address_data addr_data = { .forces = forces,};
static struct i2c_board_info __initdata i2c_tpd = { I2C_BOARD_INFO("gt928", (0xBA >> 1))};
...
static struct proc_dir_entry *gt928_config_proc = NULL;
...
static int gt928_config_read_proc(char *page, char **start, off_t off, int count, int *eof, void *data)
{
    char *ptr = page;
    char temp_data[GTP_CONFIG_MAX_LENGTH + 2] = {0};
    int i;

    ptr += sprintf(ptr, "==== GT928 config init value====\n");
    ...
    ptr += sprintf(ptr, "==== GT928 config real value====\n");
...
static int gt928_config_write_proc(struct file *file, const char *buffer, unsigned long count, void *data)
{
    s32 ret = 0;
...
static s32 tpd_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    */
    ...
    gt928_config_proc = create_proc_entry(GT928_CONFIG_PROC_FILE, 0666, NULL);

    if (gt928_config_proc == NULL)
    {
        GTP_ERROR("create_proc_entry %s failed\n", GT928_CONFIG_PROC_FILE);
    }
    else
    {
        gt928_config_proc->read_proc = gt928_config_read_proc;
        gt928_config_proc->write_proc = gt928_config_write_proc;
    }
...
{
    GTP_INFO("MediaTek gt928 touch panel driver init\n");
...
{
    GTP_INFO("MediaTek gt928 touch panel driver exit\n");
