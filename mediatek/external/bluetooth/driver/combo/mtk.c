...
int mtk(int fd, struct uart_t *u, struct termios *ti)
{
    ...
    unsigned char ucNvRamData[sizeof(ap_nvram_btradio_mt6610_struct)] 
    = {0};
    unsigned char zero[sizeof(ap_nvram_btradio_mt6610_struct)] 
    = {0};
    ...
    if(0 == memcmp(ucNvRamData, zero, sizeof(ap_nvram_btradio_mt6610_struct))){
        LOG_ERR("Nvram data all zero! Maybe config error of MTK_BT_CHIP\n");
        goto error;
    }
