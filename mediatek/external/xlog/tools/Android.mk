...
mtklog_config_prop_src := $(LOCAL_PATH)/mtklog-config-72nand-eng.prop
else
ifeq ($(MTK_MULTI_STORAGE_SUPPORT),yes)
...
ifeq ($(MTK_FAT_ON_NAND),yes)
mtklog_config_prop_src := $(LOCAL_PATH)/mtklog-config-72nand-user.prop
else
ifeq ($(MTK_MULTI_STORAGE_SUPPORT),yes)
...
$(xlog_filter_tags_file): $(xlog_filter_tags_src)
	mkdir -p $(dir $@)
	chmod +x mediatek/external/xlog/tools/merge-xlog-filter-tags.py
