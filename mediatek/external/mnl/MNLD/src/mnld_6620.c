...
static int launch_daemon_thread(void) 
{
    ...
    if ((pid = fork()) < 0) {
...
#if defined(MTK_COMBO_SUPPORT)
#if defined(MTK_GPS_MT6572)
		...
		argv[0] = mnl6572;
			...
			argv[0] = mnl3332;
#elif defined(MTK_GPS_MT6620)||defined(MTK_GPS_MT6628)||defined(MTK_GPS_MT6630)
		int get_time = 5;
		...
		{
			MND_ERR("combo chip is unknown, chip id is %s\n", chip_id);
			goto error; 
		}
#endif
#elif (defined(MTK_GPS_MT3332)||(!defined(MTK_COMBO_SUPPORT)))	//no combo chip, but GPS chip only no need support multi-combo chip

			char *mnl3332 = "/system/xbin/libmnlp_mt3332";
			argv[0] = mnl3332;	
#else
			MND_ERR("actually, I don't know which chip are you using, launch default libmnlp\n");
			strcpy(argv[0], "/system/xbin/libmnlp");	
#endif//end #if defined(MTK_GPS_MT3332) without combo chip
