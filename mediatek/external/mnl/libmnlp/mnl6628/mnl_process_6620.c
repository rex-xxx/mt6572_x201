...
{
	static int mnl_restart_done = 0;
    switch (msg)
    {
    	case MTK_GPS_MSG_MNL_START:
    		mnl_restart_done = 0;
    		MNL_MSG("MTK_GPS_MSG_MNL_START\n");
    		break;
    	case MTK_GPS_MSG_MNL_RESTART_NTF:
    		mnl_restart_done = 0;
    		MNL_MSG("MTK_GPS_MSG_MNL_RESTART_NTF\n");
    		break;
...
/* FIXME!! George Kuo: 26MHz TCXO on EVBv1!!! */
    ...
    */
    ...
    if (ioctl(dsp_fd, COMBO_IOC_GPS_HWVER, &hw_ver) < 0)
    {
        init_cfg.reservedy = NULL;
	init_cfg.reservedx = (UINT32)NULL;	 
        //status = MTK_GPS_ERROR;
        //return status;       
    }else {
        init_cfg.reservedy = (void *)MT6620_GPS_CHIP_VERSION;
        init_cfg.reservedx = hw_ver;
    }
...
//    strcpy(driver_cfg.dbg_file_name, DSP_BIN_LOG_FILE);
    ...
    //driver_cfg.pmtk_conn    =   mnl_config.pmtk_conn; 
...
void get_gps_version()
{
    ...
    //for version file
#if 0
    char fname[] = "/data/data/com.mediatek.ygps/sharefile/gps_version";
    int fd = -1;

    //get MNL version    
    sockfd = socket(AF_INET, SOCK_STREAM, 0);
    if (sockfd < 0){
        MNL_ERR("ERROR opening socket");	
    }

    server = gethostbyname("127.0.0.1");
    if (server == NULL) {
        MNL_ERR("ERROR, no such host\n");
    }
    ...
    //wait for MNL to finish initialization
    while ((0 == callback_flags) && (retry < 5))
    {
        usleep(200000);
        retry++;
        MNL_MSG("retry = %d", retry);
    }
		
    if(0 != callback_flags){ 
        /* Now connect to the server */
        if ((con_fd = connect(sockfd, (struct sockaddr *)&serv_addr, sizeof(serv_addr))) < 0){
            MNL_ERR("ERROR connecting");
        } else {					
            MNL_MSG("Connect success!!");   
        }
    } else {
        MNL_ERR("Connect socket fail!!");
    }
#endif
    //get chip version
    memset(CHIP_VERS, 0, sizeof(CHIP_VERS));
    #ifdef MTK_GPS_MT3326
    //strcpy(CHIP_VERS, "Chip_version=MTK_GPS_MT3326\r\n");
    property_set("gps.gps.version", "MTK_GPS_MT3326");
    #endif
    #ifdef MTK_GPS_MT6620
    //strcpy(CHIP_VERS, "Chip_version=MTK_GPS_MT6620\r\n");
    property_set("gps.gps.version", "MTK_GPS_MT6620");
    #endif
    #ifdef MTK_GPS_MT3336
    //strcpy(CHIP_VERS, "Chip_version=MTK_GPS_MT3336\r\n");
    property_set("gps.gps.version", "MTK_GPS_MT3336");
    #endif
    #ifdef MTK_GPS_MT6628
    //strcpy(CHIP_VERS, "Chip_version=MTK_GPS_MT6628\r\n");
    property_set("gps.gps.version", "MTK_GPS_MT6628");
    #endif
    #ifdef MTK_GPS_MT3332
    //strcpy(CHIP_VERS, "Chip_version=MTK_GPS_MT3332\r\n");
    property_set("gps.gps.version", "MTK_GPS_MT3332");
    #endif
    ...
    if (con_fd == 0){
        ...
        MNL_MSG("len = %d, cmd = %s", len, cmd);
        if(len < 0){
            MNL_ERR("Send cmd fail!!"); 	   
        } else {
            MNL_MSG("Send command PMTK605*31 to MNL success!");
        }
    }

    memset(buf, 0, sizeof(buf));
    if (read(sockfd, buf, sizeof(buf)) < 0){
        MNL_VER("Read MNL version fail!");
    } else {
        MNL_MSG("mnl_ver = %s\n", buf);
    ...
    //write to file
#if 0
    fd = open(fname, O_RDWR | O_CREAT, S_IRUSR | S_IROTH | S_IRGRP);	

    if (fd != -1){
        if (write(fd, CHIP_VERS, strlen(CHIP_VERS)) < 0){
            MNL_ERR("write chip version fail!!");            
        }

        if (write(fd, MNL_VERS, strlen(MNL_VERS)) < 0){
            MNL_ERR("write mnl version fail!!");  
        }		
	
        close(fd);	
    } else {
        MNL_ERR("Write file fail!!");
    }
#endif
}
