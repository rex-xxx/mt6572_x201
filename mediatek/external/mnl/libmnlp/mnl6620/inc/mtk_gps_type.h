...
typedef enum
{
    ...
    MTK_AGPS_MSG_INFO,   // for GPEVT sentence using 
...
typedef struct
{
  ...
  MTK_GPS_BOOL fgEnableHS;// Enable/Disable HS
  UINT8        DebugType;       
  ...
  char         dsp_dbg_file_name[30]; // DSP dbg file name
  ...
  UINT8        pmtk_conn;   /*PMTK_CONNECTION_SOCKET | PMTK_CONNECTION_SERIAL*/
  INT32        socket_port;  // PMTK sockect port name
...
typedef enum
{
  ...
  MTK_MUTEX_GPEVT_CNT, // for protection GPEVT sentence!
  MTK_MUTEX_END
} MTK_GPS_MUTEX_ENUM;
...
#define MTK_GPS_THIP_KEY_MT6620   (0xFFFF6620)
#define MTK_GPS_THIP_KEY_MT6628   (0xFFFF6628)
#define MTK_GPS_THIP_KEY_MT3332   (0xFFFF3332)
#define MTK_GPS_THIP_KEY_MT3336   (0xFFFF3336)
