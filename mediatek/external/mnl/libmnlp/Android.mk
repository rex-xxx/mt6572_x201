...
# Configuration
###############################################################################
#MTK_GPS_CHIP =  MTK_GPS_MT6628 MTK_GPS_MT6620
###############################################################################
# build start
###############################################################################
...
LOCAL_STATIC_LIBRARIES += libmnl_6628 libcutils libc libm libsupl libagent_6628 libhotstill
endif
##################6620##########################
#ifeq ($(MTK_GPS_CHIP), MTK_GPS_MT6620)
ifneq ($(filter MTK_GPS_MT6620,$(MTK_GPS_CHIP)),)
...
ifneq ($(filter MTK_GPS_MT3332,$(MTK_GPS_CHIP)),)
...
LOCAL_C_INCLUDES:= $(LOCAL_PATH)/mnl3332/inc \
                   ...
                   $(MTK_PATH_CUSTOM)/cgen/inc  
LOCAL_SHARED_LIBRARIES += libcutils libc libm libnvram
LOCAL_STATIC_LIBRARIES += libmnl_3332 libcutils libc libm libsupl libagent_3332 libhotstill
endif
##########################6572#######################
ifneq ($(filter MTK_GPS_MT6572,$(MTK_GPS_CHIP)),)
...
LOCAL_C_INCLUDES:= $(LOCAL_PATH)/mnl6572/inc \
                   ...
                   $(MTK_PATH_CUSTOM)/cgen/inc  
LOCAL_SHARED_LIBRARIES += libcutils libc libm libnvram
LOCAL_STATIC_LIBRARIES += libmnl_6572 libcutils libc libm libsupl libagent_6572 libhotstill
endif
#########################end########################
LOCAL_MODULE_TAGS := optional
...
#$(foreach item,$(MTK_GPS_CHIP),$(eval $(call my_libmnlp,$(item))))
