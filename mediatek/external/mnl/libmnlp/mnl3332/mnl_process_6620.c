...
static void * thread_agpsdispatch_func(void * arg)
{
    ...
    while(!g_ThreadExitAgpsDispatch)
    {
        ...
        if(MTK_AGPS_SUPL_NI_REQ  == type)
        {
            ...
            //update_supl_prop();              //remove by ZQH            
            state_mode = ST_NI;   
...
//    init_cfg.if_init_spd = mnl_config.init_speed;               // 38400bps
    init_cfg.if_link_spd = mnl_config.link_speed;  
...
/* FIXME!! George Kuo: 26MHz TCXO on EVBv1!!! */
    ...
    */
    ...
    if ( dsp_fd == -1)    
    {        
        MNL_ERR("open_port: Unable to open - %s %d\n", mnl_config.dev_dsp,__LINE__); 	   
        return MTK_GPS_ERROR;    
    } else {
        MNL_MSG("open dsp successfully, dsp_fd = 0x%x\n", dsp_fd);
    }
...
int linux_gps_dev_init(void)
{
	...
	MNL_MSG("dev_gps is %s, %d\n", mnl_config.dev_gps, __LINE__);
...
{
	int a;
    a = property_set("gps.gps.version", "MTK_GPS_MT3332");
    if (a < 0)
    	MNL_ERR("set gps property error, message is %s\n", strerror(errno));
    else
    	MNL_MSG("set gps chip version ok\n");
    return;
}
