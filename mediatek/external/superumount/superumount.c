...
int destroyByDevice(const char *loopDevice)
{
	...
	{
		if( debug )
			my_printf(print_flag,"  Failed to open loop (%d)", errno);
		return -1;
	}
	...
	{
		if( debug )
			my_printf(print_flag,"  Failed to destroy loop (%d)", errno);
		close(device_fd);
		return -1;
	}
	if( debug )
 ...
 *	
 *	@return[out]	@c 1=>if find matched, 0=>if not matched 
 */
		...
		if (!io2) {
		    	...
		    	while (pch != NULL)
			{
				...
				if( round == dm_dev )
				{
					...
					if( mount_dir!=0 && lookupLoopdevice(mount_dir, pch, (char *)loopdevice, &loop_number) )
					{
						if( debug )
...
int umount_subdir(char *src_dir, int *major, int *minor)
{
	...
	do {
		...
		if( match == 6 && !strcmp( mount_dir, src_dir) )
		{
			if( major && minor)
			{
...
void super_umount(char *src_dir)
{
	...
	for( i=0; i< matched_mount_cnt; i++,mountlist++)
	{
		lookupLoopdevice(mountlist->name, 0, (char *)dir, &loopnumber);
