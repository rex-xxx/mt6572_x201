...
long getIPOPowerOffTime(void)
{
    ...
    if(property_get(IPO_POWER_OFF_TIME, buf, "0")) {
        time = atol(buf);
        if(time > 0)
            SXLOGI("found ipo power off time: %ld \n",time);
        else
            time = IPO_DEFAULT_POWEROFF_TIME;
    }

    return (time < IPO_MIN_POWEROFF_TIME ? IPO_MIN_POWEROFF_TIME : time);
}
...
void updateLogoBacklightVoltageMode(long params[])
{
	...
	{
	    params[PARAM_POWER_ON_VOLTAGE] = 50; //keep powerpath working
	    params[PARAM_POWER_OFF_VOLTAGE] = 1; //keep powerpath working
	}
}
#endif
#endif
