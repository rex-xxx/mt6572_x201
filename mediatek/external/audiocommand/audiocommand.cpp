...
{
    printf("GetAudioMode enter audio mode 0:normal 1:ringtone 2:incall 3:commuinication mode\n");
    scanf("%x",&AudioMode);
    return AudioMode;
}
...
void AudioSystemCommand()
{
    int SecondCommand;
    while(true){
    printf("please enter audio command 0:device connnect 1:device disconnect 2.mode set \n");
...
int main()
{
   ...
   while(true)
   {
   	  printf("please enter audio command 0:audioplayback(music) 1:audioplayback(ringtone) \
   	      2:audiorecord 3.audiosystem command 4 audiohardwarecoomand 5.set afe register 6 set pmic register");
   	  scanf("%x",&MainCommand);
   	  switch(MainCommand)
   	  {
   	      case 0:
