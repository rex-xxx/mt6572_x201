...
jshortArray scannew(JNIEnv *env, jobject thiz, jint upper, jint lower, jint space)
{
    int ret = 0;
    jshortArray scanChlarray;
    struct fm_ch_rssi *buf = NULL;
    int i, j;
    
    ret = FMR_scan_new(g_idx, (void**)&buf, upper, lower, space, NULL);
    if (ret < 0) {
        LOGE("scan failed!\n");
        scanChlarray = NULL;
        goto out;
    } else if (ret > 0){
        scanChlarray = env->NewShortArray(ret);
        j = 0;
        for (i = 0; i < ret; i++) {
            LOGD("%d %d %d\n", i, buf[i].freq, buf[i].rssi);
            if (buf[i].freq > 0) {
				env->SetShortArrayRegion(scanChlarray, j, 1, (const jshort*)&buf[i].freq);
                j++;
            }
        }
    } else {
