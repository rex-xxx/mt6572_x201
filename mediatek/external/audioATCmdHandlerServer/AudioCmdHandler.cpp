...
ACHStatus AudioCmdHandler::DLCustDualMicParamToNV(void *pParam)
{
	...
	size = sizeof(AUDIO_CUSTOM_EXTRA_PARAM_STRUCT) + sizeof(int);
	dataDecode((char *)pParam, size);

	dataLen = *((int *)pParam);
	size -= sizeof(int);
	if (dataLen!=size) {
		SXLOGE("DLCustDualMicParamToNV data miss !! dataLen=%d, size=%d", dataLen, size);
		if (pDualMicPara!=NULL)
			free(pDualMicPara);	
		return ACHFailed;
	}

	memcpy((void *)pDualMicPara, pParam+sizeof(int), sizeof(AUDIO_CUSTOM_EXTRA_PARAM_STRUCT));
...
#endif
	{
		SXLOGD("Dual Mic parameters ABF[%d]=%d", i, pDualMicPara->ABF_para[i]);
	}
...
ACHStatus AudioCmdHandler::ULCustDualMicParamFromNV(void *pParam, int *len)
{
	...
	SXLOGD("ULCustDualMicParamFromNV ,dual mic param size=%d", sizeof(AUDIO_CUSTOM_EXTRA_PARAM_STRUCT));
	size = GetDualMicSpeechParamFromNVRam(pDualMicPara);
	if (size!=sizeof(AUDIO_CUSTOM_EXTRA_PARAM_STRUCT)) {
		SXLOGD("ULCustDualMicParamFromNV Up load from NVRAM fail, structure size=%d,read_size=%d",sizeof(AUDIO_CUSTOM_EXTRA_PARAM_STRUCT),size);
		free(pDualMicPara);
		pDualMicPara = NULL;
		return ACHFailed;
	}
...
ACHStatus AudioCmdHandler::getPhoneSupportInfo(unsigned int *supportInfo)
{
	...
	*supportInfo = *supportInfo | SUPPORT_HD_RECORD;
	...
	int value = 0;
	AudioSystem::GetAudioCommand(HD_REC_48K_SUPPORT, &value);
	*supportInfo = value ? (*supportInfo | SUPPORT_HD_48K_REC) : (*supportInfo);

	SXLOGD("HDRecording support 48k sample rate value: %d", value);
#endif
