...
struct jpeg_compress_struct {
  ...
  int for_huff_opt[MAX_COMPS_IN_SCAN];
  int last_coffs_zero[C_MAX_BLOCKS_IN_MCU];
#if 1//def JPEG_THREAD_SUPPORT
  J_ENCODE_SPEED encode_speed;
  int core_num;
  int thread_num;
  int cur_iMCU_row;  

  JSAMPIMAGE data_in;
  int last_coffs_zero_thread[MAX_CORE_NUM][C_MAX_BLOCKS_IN_MCU];
  struct jpeg_destination_mgr *dest_thread[MAX_CORE_NUM];
  struct jpeg_output_table *output_table;

  ThreadContext_t thrcnxt[MAX_CORE_NUM];
  Thread_t pthread[MAX_CORE_NUM];
  Thread_mutex_t mutex_input;
  Thread_mutex_t mutex_state[MAX_CORE_NUM];
  Thread_cond_t cond_state[2*MAX_CORE_NUM];
#endif
};
...
#if 1//def JPEG_THREAD_SUPPORT
  JMETHOD(boolean, output_buffer_thread, (j_compress_ptr cinfo, JOCTET *output_addr, int output_size));
  JMETHOD(JOCTET *, memcpy_buffer_thread, (j_compress_ptr cinfo, JOCTET *last_addr, int index));
#endif
};
