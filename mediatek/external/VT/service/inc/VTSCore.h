...
namespace VTService{
	...
	class VTSCore: public IVTSInterface, public IVTSCoreCallback
	{
		public:														
			...
			status_t getStatus();
			status_t setVTSStatus(int status, bool checkCore=VT_FALSE, bool checkStk =VT_FALSE, bool checkTcv=VT_FALSE);
			void setInterStatus(int status, int type);
			int sendCmdToMAL(int cmdid,void* param,const int paramLenth);
			
			int setIncomingVideoChannelConnected(int flag);
			int getIncomingVideoChannelConnected();
			VTSString getSReplacePeerVideoFilePath();
