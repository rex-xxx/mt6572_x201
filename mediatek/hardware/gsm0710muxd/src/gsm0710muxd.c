	...
	while ((frame = cmux_mode ? gsm0710_advanced_buffer_get_frame(buf) : gsm0710_base_buffer_get_frame(buf, 0))) {
		...
		if ((GSM0710_FRAME_IS(GSM0710_TYPE_UI, frame) || GSM0710_FRAME_IS(GSM0710_TYPE_UIH, frame))) {
			...
			if (frame->channel > 0) {
				...
				LOGMUX(LOG_DEBUG, "Writing %d byte frame received on channel %d to %s", frame->length, frame->channel, channel->ptsname);
				...
				if (frame != NULL) {
					...
					while (1) {
						if ((write_result = write(channel->fd, frame->data, frame->length)) >= 0) {
							LOGMUX(LOG_DEBUG, "write() returned. Written %d/%d bytes of frame to %s", write_result, frame->length, channel->ptsname);
...
static int close_devices()
{
	...
	if (serial.fd >= 0) {
		...
		LOGMUX(LOG_DEBUG, "close_devices SYSCHECK(close(serial.fd));;");
...
static int shutdown_devices(int shutdown_type)
{
	...
	if (serial.fd >= 0) {
		...
		SYSCHECK(close(serial.fd));
		LOGMUX(LOG_DEBUG, "shutdown_devices SYSCHECK(close(serial.fd));;");
		serial.fd = -1;
	}
