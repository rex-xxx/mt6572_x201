...
#if defined(MTKCAM_HAVE_GRALLOC_ION)
    {
        int idx = 0, num = 0;
        err = GraphicBufferMapper::get().getIonFd(*phBuffer, &idx, &num);
...
lbExit:
    MY_LOGD_IF(
        (2<=miLogLevel), 
        "- ret(%d), (fdIon, address, phBuffer, stride)=(%d, %p, %p, %d), rpImgBuf.get(%p)", 
        ret, fdIon, address, phBuffer, stride, rpImgBuf.get()
    );
    return  ret;
}
...
enquePrvOps(sp<StreamImgBuf>const& rpImgBuf)
{
    ...
    //
    //  [4] unlocks and post the buffer to display.
