    ...
    virtual uint32_t                getImgWidthStride(
                                    ...
                                    )  const
                                    {
                                        return  (0 == uPlaneIndex)
                                            ?   mi4Stride
                                            :   FmtUtils::queryImgWidthStride(getImgFormat(), getImgWidth(), uPlaneIndex)
                                                ;
                                    }
...
public:     ////                    Debug.
    void                            dump() const
                                    {
                                        CAM_LOGD(
                                            "[%s](%s@%dx%d@%d-bit@%d), fdIon(%d), pBuf/pBufHndl=%p/%p, Stride(%d), Timestamp(%lld)", 
                                            getBufName(), getImgFormat().string(), getImgWidth(), getImgHeight(), 
                                            getBitsPerPixel(), getBufSize(), 
                                            mfdIon, 
                                            mpBufBase, mpBufHndl, mi4Stride, mi8Timestamp
                                        );
                                    }
