...
initFeatures_Flashlight()
{
    return  queryCustomFeature(mFeatureMap, String8(DLSYM_MODULE_NAME_FLASHLIGHT));
}
...
initFeatures_AFLamp()
{
    ssize_t const fkey = mFeatureMap.indexOfKey(FKEY_T(MtkCameraParameters::KEY_FOCUS_MODE));
    if  ( fkey < 0 )
    {
        MY_LOGW("Not support KEY_FOCUS_MODE");
        return  false;
    }
    //
    FeatureInfo const& rFInfo = mFeatureMap.valueAt(fkey).getDefault();
    if  ( 0 == rFInfo.getSupportedSize() )
    {
        MY_LOGW("KEY_FOCUS_MODE has no support list");
        return  false;
    }
    //
    return  queryCustomFeature(mFeatureMap, String8(DLSYM_MODULE_NAME_AFLAMP));
}
