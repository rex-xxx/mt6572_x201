...
#include <cam/dbg_cam_param.h>
#include <eis/dbg_eis_param.h>
//
...
)
{
    ...
    if (mi4DbgModuleType & dbgModuleID)
    {
        ...
        if (dbgModuleSize > 2)
        {
            ::exifAppnMake(dbgAppn, *ppuAppnBuf, pDbgModuleInfo, dbgModuleSize, &appnReturnSize);
            //
        ...
        {
            MY_LOGW("[appendDebugInfo] dbgModuleSize(%d) < 0", dbgModuleSize);
            ret = MFALSE;
        }
    }
    else
...
appendDebugExif(MUINT8* const puAppnBuf, MUINT32*const pu4AppnSize)
{
    ...
    //// CAM debug info: Common, MF, N3D, Sensor, Shading
    {
        ...
        {
            ::exifAppnMake(5, pDst, pDbgCamInfo, dbgCamSize, &app5ReturnSize);
            //
            pDst += app5ReturnSize;
