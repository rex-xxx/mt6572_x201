...
#define MY_LOGW(fmt, arg...)        ALOGW(fmt, ##arg)
#define MY_LOGE(fmt, arg...)        ALOGE(fmt"<%s:#%d>", ##arg, __FILE__, __LINE__)
//
#if (SHOT_ASSERT_ON)
    #define MY_LOGA(x, fmt, arg...) \
        if ( ! (x) ) { \
            printf(fmt"<Assert %s:#%d>", ##arg, __FILE__, __LINE__); \
            ALOGE (fmt"<Assert %s:#%d>", ##arg, __FILE__, __LINE__); \
...
)
{
    ...
    {
        MY_LOGE(
            "[makeExifApp1] invalid parameters:(puApp1Buf, u4ImgWidth, u4ImgHeight)=(%p, %d, %d)"
    ...
    {
        MY_LOGE("[makeExifApp1] queryExifApp1Info");
        goto lbExit;
    }
    ...
    if  ( err )
    {
        MY_LOGE("[makeExifApp1] exifApp1Make - err(%x)", err);
        goto lbExit;
    }
...
)
{
    ...
    MY_LOGD("[makeExifApp3] jpsSize  = %d", jpsSize);
    //
...
{
    MY_LOGI("[queryExifApp1Info] E");

    if  ( ! pexifApp1Info )
    {
        MY_LOGE("[queryExifApp1Info] NULL pexifApp1Info");
        return  MFALSE;
    }
    ...
    **********************************************************************************/
    ...
    pexifApp1Info->bestFocusL   = mCamExifParam.u4FocusH;
    ...
    **********************************************************************************/
    if (mp3AEXIFInfo != NULL) {
        ...
        // [iso speed]
        if (mp3AEXIFInfo->u4AEISOSpeed == NSFeature::AE_ISO_AUTO) {
        ...
        // [white balance mode]
        pexifApp1Info->whiteBalanceMode = (NSFeature::AWB_MODE_AUTO != mp3AEXIFInfo->u4AWBMode)?1:0;
        ...
        if  ( NSFeature::SCENE_MODE_NUM <= u4SceneMode )
        {
            u4SceneMode = NSFeature::SCENE_MODE_OFF;
        }
        ...
        MY_LOGE("[queryExifApp1Info] mp3AEXIFInfo == NULL");
    }
...
set3AEXIFInfo(EXIF_INFO_T* p3AEXIFInfo)
{
    ...
    {
        MY_LOGE("[set3AEXIFInfo] mp3AEXIFInfo == NULL");
    }
