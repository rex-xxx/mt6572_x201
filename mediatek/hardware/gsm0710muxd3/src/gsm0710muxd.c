...
//		LOGMUX(LOG_WARNING, "Didn't get frame-flag after wakeup");
    ...
    if (!cmux_mode)
    {
        ...
        #if 0
        {
        LOGMUX( LOG_DEBUG, "Write 256 bytes of F9 to serial.devicename=%s", serial.devicename); 
        int i;
        char F9string[256];
        for (i = 0; i < 255; i++)
            F9string[i] = 0xF9;
        F9string[255] = '\0';
        c = write(serial.fd, F9string, 256);
        }
        #endif
        ...
        LOGMUX( LOG_DEBUG, "Write prefix len=%d written=%d into serial.devicename=%s", prefix_length, c, serial.devicename);        
        if (c != prefix_length)
        {
	...
	int id)
{
    ...
    /* Comment by LS:Because channel num#0 is used by Gsm0710Muxd3 locally and not exported to RILD or network interface driver: */
	...
	int to)
{
    ...
    LOGMUX(LOG_DEBUG, "gsm0710muxd3: wait for modem boot up");
    do
    {
...
int boot_modem(int is_reset)
{
...
#if 0   // It's implemented by Chao, maybe removed later
	ext_md_ctl_n0 = open("/dev/ext_md_ctl0", O_RDWR);
	if(ext_md_ctl_n0 <0) {
    	LOGMUX(LOG_DEBUG, "open ext_md_ctl0 fail");
		return	ext_md_ctl_n0;
	}
	ret = ioctl(ext_md_ctl_n0, EXT_MD_IOCTL_POWER_ON_HOLD, NULL);
	if (ret < 0) {
    	LOGMUX(LOG_DEBUG, "power on modem fail");
		return	ret;
	}
#endif
	...
	if(ext_md_ctl_n1 <0) {
    	LOGMUX(LOG_DEBUG, "open ext_md_ctl_n1 fail");
		return	ext_md_ctl_n1;
	}
	ret = ioctl(ext_md_ctl_n1, EXT_MD_IOCTL_LET_MD_GO, NULL);
	if (ret < 0) {
    	LOGMUX(LOG_DEBUG, "EXT_MD_IOCTL_LET_MD_GO fail");
		return	ret;
	}
	
	/*
	ret = ioctl(ext_md_ctl_n1, EXT_MD_IOCTL_REQUEST_RESET, NULL);
	if (ret < 0) {
    	LOGMUX(LOG_DEBUG, "EXT_MD_IOCTL_REQUEST_RESET fail");
		return  ret;
	}
	*/
	...
	Serial* serial )
{
    ...
    {   
        ...
        {
            LOGMUX(LOG_INFO, "Opened serial port: %s", serial->devicename);
...
//ALPS00374816: to enable HW flow control 
            ...
            uart_cfg_opt.c_cflag &= ~(CRTSCTS);
            uart_cfg_opt.c_lflag &= ~(ICANON | ECHO | ECHOE | ECHOK | ECHONL | ISIG);
            uart_cfg_opt.c_iflag &= ~(INPCK | IGNPAR | PARMRK | ISTRIP | IXANY | ICRNL);
            uart_cfg_opt.c_iflag &= ~(IXON | IXOFF);
            uart_cfg_opt.c_oflag &= ~(OPOST | OCRNL);
            uart_cfg_opt.c_cc[VMIN] = 1;
            uart_cfg_opt.c_cc[VTIME] = 0;
            ...
            uart_cfg_opt.c_cc[VINTR] = posix_vdisable;
            uart_cfg_opt.c_cc[VQUIT] = posix_vdisable;
            uart_cfg_opt.c_cc[VSTART] = posix_vdisable;
            uart_cfg_opt.c_cc[VSTOP] = posix_vdisable;
            uart_cfg_opt.c_cc[VSUSP] = posix_vdisable;
	...
	)
{
    ...
    //chat(serial->fd, "Wait +EIND: 128...", 1);
...
static int close_devices()
{
    ...
    /* Note by LS: If it stops the rild process by property_set("ctl.stop","ril3-daemon"), it should also set rild_started as 0 */
#if 0   
    ...
    switch (serial->state)
    {
        ...
        case MUX_STATE_INITILIZING:
            ...
            if (rild_started == 0)
            {
                ...
                LOGMUX(LOG_ERR,"ril3-daemon started!");
            }
#endif
            break;
	...
	)
{
    ...
    fprintf(stderr, "\t-o <output log to file>: Output log to /tmp/gsm0710muxd3.log [%s]\n", logtofile?"yes":"no");
...
int main(int argc,char *argv[])
{
    ...
    while ((opt = getopt(argc, argv, "dov:s:t:p:f:n:h?m:b:P:")) > 0)
    {
        switch (opt)
        {
            ...
            case 'o':
                logtofile = 1;
                if ((muxlogfile=fopen("/tmp/gsm0710muxd3.log", "w+")) == NULL){
                    fprintf(stderr, "Error: %s.\n", strerror(errno));
                    usage(argv[0]);
                    exit(0);
                }
                else
                    fprintf(stderr, "gsm0710muxd log is output to /tmp/gsm0710muxd3.log\n");
                break;			
            case 'd':
                no_daemon = !no_daemon;
                break;
    ...
    LOGMUX(LOG_NOTICE,"\t-o <output log to file>: Output log to /tmp/gsm0710muxd3.log [%s]", logtofile?"yes":"no");
    ...
    LOGMUX(LOG_ERR, "Terminating Gsm0710Muxd3 process ...");
#ifndef MUX_ANDROID	
