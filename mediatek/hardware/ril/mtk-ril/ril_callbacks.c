...
static void *mainLoop(void *param)
{
	...
	while (s_main_loop) {
        ...
        int continueInit = 1;
        int sim = get3GCapabilitySim(NULL);
        int telephonyMode = getTelephonyMode();
        int firstModem = getFirstModem();
        LOGI("current telephony mode and 3G SIM: [mode:%d, 3G SIM:%d, FirstMD:%d]", telephonyMode, sim, firstModem);
        ...
        if (continueInit) {
            if (isDualTalkMode()) {
                ...
                LOGI("Is 3G Switch now: [%d], 3G SIM mapped for framework: [SIM%d]", s_is3GSwitched, sim);
                char* value = NULL;
                asprintf(&value, "%d", sim);
...
#endif  /* MTK_RIL */
{
	...
	LOGI("setRadioState(): newState=%d, oldState=%d, *pState=%d\n", newState, oldState, *pState);
...
int RIL_get3GSIM()
{
    char *simId = malloc(sizeof(char) * 2);
    memset(simId, 0, sizeof(char) * 2);	
...
int isEmulatorRunning()
{
	...
	char *qemu = malloc(sizeof(char) * 2);
	memset(qemu, 0, sizeof(char) * 2);
