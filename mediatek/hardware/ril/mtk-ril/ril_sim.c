...
//        case SIM_NETWORK_PERSONALIZATION:
        ...
        case USIM_NOT_READY:
            RIL_requestProxyTimedCallback (pollSIMState, param, &TIMEVAL_SIMPOLL,
                    getChannelCtxbyProxy(rid)->id, "pollSIMState");
        return;
...
static void requestGetSimStatus(void *data, size_t datalen, RIL_Token t)
{
    ...
    if (result == RIL_E_SUCCESS) {
        ...
        if (RIL_CARDSTATE_PRESENT == p_card_status->card_state) {
            ...
            if ( datalen == 3*sizeof(char*) ) {
                if (CHANGE_PIN1 == op) {
                    ...
                    if ((sim_status == SIM_PIN)||(sim_status == USIM_PIN)) {
                        ...
                        err = at_send_command(cmd, &p_response, SIM_CHANNEL_CTX);
                        bIgnoreCmd = TRUE;
                        if (CME_SUCCESS == at_get_cme_error(p_response))
                        {
                           bIgnoreCmd = FALSE;
                           free(cmd);
...
static void makeSimRspFromUsimFcp(unsigned char ** simResponse)
{
...
static void simFacilityLock(void *data, size_t datalen, RIL_Token t)
{
    ...
    // solution: request again if modem response busy, max 1s
    int isSimBusy = 0;
    int count = 0;
...
static void onPhbReady(RILId rid)
{
#ifdef MTK_GEMINI
