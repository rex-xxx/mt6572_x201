...
nmea_reader_set_callback( NmeaReader* const r, GpsCallbacks* const cbs)
{
    if (!r) {           /*this should not happen*/
        ...
        if(MNL_AT_TEST_FLAG) {            
            ...
            DBG("GPS normal mode, register callbacks!!");
        if (r->callbacks.location_cb == NULL) {
            //GpsStatus sta;                                                                                                 //Modify to global
...
static int
nmea_reader_update_sv_status( NmeaReader* const r, int sv_index,
...
//				DBG("[debug mask] has been fixed and mask = %d\n ", r->sv_status.used_in_fix_mask);
            for (idx = 0; idx < max; idx++) {
                ...
                if (tok_satellite.p == tok_satellite.end) {                    
                    DBG("GPGSA: found %d active satellites\n", idx);					
                    break;
                }
...
gps_state_init( GpsState*  state )
{
    ...
    //if ( state->callbacks.create_thread_cb(gps_native_thread, gps_state_thread, state) <= 0 ) {
    
    state->thread = state->callbacks.create_thread_cb(gps_native_thread, gps_state_thread, state);
    if (!state->thread){
...
mt3326_gps_start()
{
    ...
    {
 		DBG("no EPOHAL file, the EPO.DAT is not exsitence, or is the latest one\n");
    }
    else
    {
...
int
mt3326_gps_sys_read_lock(int fd, off_t offset, int whence, off_t len){   
...
int
mt3326_gps_sys_write_lock(int fd, off_t offset, int whence, off_t len){
	struct flock lock;
...
static int
mt3326_gps_sys_epo_period_start(int fd, unsigned int* u4GpsSecs, time_t* uSecond){      //no file lock                  
...
static int
mt3326_gps_sys_epo_period_end(int fd, unsigned int *u4GpsSecs, time_t* uSecond) {        //no file lock   
    int fileSize;
    char szBuf[MTK_EPO_ONE_SV_SIZE]; 
    int pi2WeekNo; 
    unsigned int pu4Tow;	

    fileSize = mt3326_gps_sys_get_file_size();
...
int
mt3326_gps_epo_file_time_hal(long long uTime[]) {
   ...
   //Add file lock 
   if(mt3326_gps_sys_read_lock(fd, 0, SEEK_SET, 0) < 0){
    ...
    //EPO start time
    if(mt3326_gps_sys_epo_period_start(fd, &u4GpsSecs_start, &uSecond_start)) {
    ...
    if(mt3326_gps_sys_epo_period_end(fd, &u4GpsSecs_expire, &uSecond_expire)){
...
int
mt3326_gps_epo_file_update_hal(){
    TRC();
    ...
    if(access(EPO_FILE_HAL, 0) == -1){
        DBG("EPO HAL file does not exist, rename it directly\n");
        result = rename(EPO_FILE_NEW, EPO_FILE_HAL);
    ...
    fd_write = open(EPO_FILE_HAL, O_WRONLY | O_TRUNC);
    if(fd_write < 0){
	ERR("open /data/misc/EPO.dat error!\n");		
	return -1;
    }
 
   if(mt3326_gps_sys_write_lock(fd_write, 0, SEEK_SET, 0) < 0){
        ERR("Add read lock failed, return\n");
        close(fd_write);	   
	 return -1;
   }
    ...
    if(mt3326_gps_sys_read_lock(fd_read, 0, SEEK_SET, 0) < 0){
   	ERR("Add read lock failed, return\n");
	close(fd_read);
	return -1;		
    }
    ...
    if ((res = chmod(EPO_FILE_HAL, S_IRUSR | S_IWUSR | S_IXUSR | S_IRGRP | S_IWGRP | S_IROTH)) < 0)
		DBG("chmod res = %d, %s", res, strerror(errno));
    DBG("Updates EPO HAL file done\n");
    return 0;
 }


int mt3326_gps_epo_file_time(long long uTime[])
{
    TRC();
    char cmd = HAL_CMD_STOP_UNKNOWN;
    int ret;
	 ...
	 DBG("GPS driver is running, read epo time via GPS driver\n");         
        ...
        if (-1 == ret){
	     ...
	     //usleep(100000);                                                          
          ...
          ret = mt3326_gps_epo_file_time_hal(uTime);
      }
    return ret;	
}
	
int mt3326_gps_epo_file_update(){
    int ret;
    int res;
    DBG("sta.status = %d\n", sta.status);	
    DBG("update EPOTMP.DAT to EPOHAL.DAT\n");
    ret = mt3326_gps_epo_file_update_hal();
    if(ret < 0)
    {
...
static const GpsInterface  mt3326GpsInterface = {
    ...
    mt3326_gps_epo_file_time,
    mt3326_gps_epo_file_update,    
