...
static uint8_t *kdump_core_write_cpu_note(const struct kdump_params *kparams, int cpu, struct elf_phdr *nhdr, uint8_t *bufp)
{
    ...
    memcpy(&prstatus.pr_reg, (unsigned long*)&kparams->crash_record->cpu_regs[cpu], sizeof(elf_gregset_t));
                
    nhdr->p_filesz += notesize(&notes);
    return storenote(&notes, bufp);
}

static uint8_t *kdump_core_write_machdesc(const struct kdump_params *kparams, struct elf_phdr *nhdr, uint8_t *bufp)
{
    struct memelfnote notes;
...
void *kdump_core_header_init(const struct kdump_params *kparams, uint32_t kmem_address, uint32_t kmem_size)
{
	...
	prpsinfo.pr_sname = 'R';
	prpsinfo.pr_zomb = 0;
	prpsinfo.pr_gid = prpsinfo.pr_uid = kparams->crash_record->fault_cpu + 1;
	...
	bufp = kdump_core_write_machdesc(kparams, nhdr, bufp);

        /* Store pre-cpu backtrace */
        bufp = kdump_core_write_cpu_note(kparams, kparams->crash_record->fault_cpu, nhdr, bufp);
	for (unsigned int cpu = 0; cpu < kparams->nr_cpus; cpu++) {
            if (cpu != kparams->crash_record->fault_cpu) {
                bufp = kdump_core_write_cpu_note(kparams, cpu, nhdr, bufp);
            }
        }
