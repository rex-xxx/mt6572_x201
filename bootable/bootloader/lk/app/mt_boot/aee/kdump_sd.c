...
uint32_t FindFirstFreeClusInFAT_sd(FileHandler *pFileHandler)
{
    ...
    while (FATSector < (m_bFATInfo.BPB_FATSz-1)) {
        ...
        if (!dumpdev->read(dumpdev, SectorNum, pFileHandler->FATBuffer, 1)) {
            voprintf_error("SDCard: DeleteFile_sd dumpdev->read failed\n"); 
            return 0;
        }
        ...
        for (i=NextFATOffset; i<EntryPerSector; i++) {
            ...
            if (value == 0) {
                ...
                if(m_bFATInfo.FileSysType==FAT_32) {
                    ...
                    *(uint32_t*)(pFileHandler->FATBuffer+CurrFATOffset*BytsPerAdd) = (uint32_t)FreeClusterNum;
                }
                else {
                    *(uint16_t*)(pFileHandler->FATBuffer+CurrFATOffset*BytsPerAdd) = (uint16_t)FreeClusterNum;
                }
...
bool MarkEndClusInFAT_sd(FileHandler *pFileHandler)
{
    ...
    if (m_bFATInfo.FileSysType == FAT_32) {
        *(uint32_t*)(pFileHandler->FATBuffer+FATOffset*BytsPerAdd) = 0x0FFFFFFF;// 0xFFFFFF8
    }
    else {
        *(uint16_t*)(pFileHandler->FATBuffer+FATOffset*BytsPerAdd) = 0xFFFF;//0xFFF8
    }
...
bool DeleteFileInFAT_sd(FileHandler *pFileHandler)
{
    ...
    while (!LastEntry)
	{
            ...
            if(m_bFATInfo.FileSysType==FAT_32) {
                ...
                *(uint32_t*)(pFileHandler->FATBuffer+FATOffset*BytsPerAdd) = 0x0;
                if ((NextClusNum >= 0xFFFFFF8) || (NextClusNum == 0)) {
                    LastEntry = true;
                }
            }
            else {
                *(uint16_t*)(pFileHandler->FATBuffer+FATOffset*BytsPerAdd) = 0x0;
...
static bool OpenDumpFile_sd(FileHandler *pFileHandler)
{
    ...
    if(!GetBPBInfo_sd(pFileHandler->FileBuffer)) {
        voprintf_error("SDCard: Failed to GetBPBInfo_sd buffer \n");  
        return false;
    }
  ...
  Done:
    ...
    if(!pFileHandler->CurrClusterNum)
	{
            ...
            voprintf_error("SDCard: free cluster not found!\n");
            return false;
	}
...
static bool WriteDumpFile_sd(FileHandler *pFileHandler, uint8_t *Ptr, uint32_t Length, uint32_t Total)
{
    ...
    while (Length > 0) {
	    ...
	    if(!dumpdev->write(dumpdev, SectorNum, pFileHandler->FileBuffer, m_bFATInfo.BPB_SecPerClus)) {
            ...
            if (FreeClusterNum == 0) {
                pFileHandler->DiskFull = true;
                voprintf_error("SDCard: WriteDumpFile_sd() disk full!\n"); 
                return false;
            }
...
int kdump_sdcard_output(uint32_t total_dump_size)
{
    ...
    uint32_t start_address = kparams->phys_offset;
