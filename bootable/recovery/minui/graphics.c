...
static int get_framebuffer(GGLSurface *fb)
{
...
#else
    memset(fb->data, 0, vi.yres * vi.xres_virtual * PIXEL_SIZE);
#endif
    ...
    if (vi.yres * fi.line_length * 2 > fi.smem_len)
        return fd;
#else
    if (vi.yres * vi.xres_virtual * PIXEL_SIZE * 2 > fi.smem_len)
        return fd;
#endif
...
#else
    fb->data = (void*) (((unsigned) bits) + vi.yres * vi.xres_virtual * PIXEL_SIZE);
#endif
    fb->format = PIXEL_FORMAT;
#if 0 //wschen 2013-02-19
    memset(fb->data, 0, vi.yres * fi.line_length);
...
#endif
  ms->data = malloc( vi.xres_virtual * PIXEL_SIZE* vi.yres);
