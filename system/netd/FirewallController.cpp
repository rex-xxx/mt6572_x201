...
int FirewallController::setUdpForwarding(const char* inInterface, const char* extInterface, const char* ipAddr) {
    ...
    ALOGI("setUdpForwarding:%s:%s:%s", inInterface, extInterface, ipAddr);

    sprintf(cmdStr, "-D FORWARD -i %s -o %s -m state --state ESTABLISHED,RELATED -j ACCEPT", inInterface, extInterface);
    
    //Delete the old IPTABLE rule
    ALOGI("Iptable command 1:%s", cmdStr);
    res |= execIptables(target, cmdStr, NULL);

    sprintf(cmdStr, "-A FORWARD -i %s -o %s j ACCEPT", inInterface, extInterface);
    ALOGI("Iptable command 2:%s", cmdStr);
    res |= execIptables(target, cmdStr, NULL);

    sprintf(cmdStr, "-t nat -A PREROUTING -p udp -i %s -j DNAT --to %s", inInterface, ipAddr);
    ALOGI("Iptable command 3:%s", cmdStr);
    res |= execIptables(target, cmdStr, NULL);
