...
int pm_map_usage(pm_map_t *map, pm_memusage_t *usage_out) {
    ...
    usage.vss = map->end - map->start;

    for (i = 0; i < len; i++) {
        /// M: get swap usage @{
        if (PM_PAGEMAP_SWAPPED(pagemap[i])) {
            unsigned long swap_offset = PM_PAGEMAP_PFN(pagemap[i]);
            // pte_entry to swap_entry_t
            swap_offset = (swap_offset >> 5) + (swap_offset << 27);
            //printf("swap offset: %x\n", swap_offset);
            error = pm_kernel_swapn(map->proc->ker, swap_offset,
                                    &count);
            if (error) goto out;
            usage.swap += map->proc->ker->pagesize;
            usage.pswap += (count >= 1) ? (map->proc->ker->pagesize / count) : (0);
            continue;
        }
        /// @}
...
int pm_map_workingset(pm_map_t *map, pm_memusage_t *ws_out) {
    ...
    for (i = 0; i < len; i++) {
        ...
        if( PM_PAGEMAP_SWAPPED(pagemap[i]) ) {
            ...
            ws.pswap += (count >= 1) ? (map->proc->ker->pagesize / count) : (0);
            continue;
        }
        /// @}
