...
static void enqueue_map(mapinfo **head, mapinfo *map, int sort_by_address, int coalesce_by_name) {
    ...
    for (;;) {
        if (current && coalesce_by_name && !strcmp(map->name, current->name)) {
            ...
            current->rss += map->rss;
            current->pss += map->pss;
            current->rswap += map->rswap;
            current->pswap += map->pswap;
...
static void print_header()
{
    ...
    if (swap) {
        printf("    size      RSS      PSS    RSWAP    PSWAP    clean    dirty    clean    dirty ");
    }
    else {
...
out:
    ...
    if (swap) {
        printf("%8d %8d %8d %8d %8d %8d %8d %8d %8d ", size,
                rss, pss,
                rswap, pswap,
