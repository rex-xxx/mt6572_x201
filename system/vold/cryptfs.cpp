...
void stop_modem(){
    ...
    SLOGD("Just use muxreport to stop modem\n");
}
...
static char *get_fstab_filename(void)
{
    if (fstab_filename[0] == 0) {
        ...
        /* This table currently is used by crypt module, and crypt is not hardware dependency. So we decide not give the name by hardware platform, 
            make the naming general */
...
#define WAIT_UNMOUNT_COUNT 20
    ...
    if (i < WAIT_UNMOUNT_COUNT) {
      ...
      SLOGD("try to kill related process\n");

      int retries = 3;
      while (retries--) {
         if (!umount(mountpoint) || errno == EINVAL || errno == ENOENT) {
             SLOGI("%s sucessfully unmounted", mountpoint);
             rc = 0;
             return rc;
         }
         ...
         SLOGW("Failed to unmount %s (%s, retries %d, action %d)",
                 mountpoint, strerror(errno), retries, action);
  
         if (retries == 1) { 
            char umount_cmd[PATH_MAX];
            sprintf(umount_cmd, "/system/bin/superumount %s", mountpoint);
            SLOGI("force to umount via superumount: %s", umount_cmd);
            system(umount_cmd);
         }
         else {
...
int cryptfs_restart(bool is_ipo_reboot, int boot_reason)
{
    ...
    if (! (rc = wait_and_unmount(DATA_MNT_POINT)) ) {
        ...
        /* If that succeeded, then mount the decrypted filesystem */             
                if (Ext4::autoRepair(crypto_blkdev) < 0) {
                      SLOGE("%s fs Auto-Repair failed (%s)", crypto_blkdev, strerror(errno));
                }
                else {
                      SLOGD("%s fs Auto-Repair Successfully", crypto_blkdev);   
                }

               if (fs_mgr_do_mount(get_fstab_filename(), DATA_MNT_POINT, crypto_blkdev, 0)) {
