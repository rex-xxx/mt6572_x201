...
#define E2FSCK_PATH "/sbin/e2fsck";
...
int Ext4::autoRepair(const char *fsPath) {
    int fd;
    const char *args[4];
    int rc;
    ...
    if (rc == 0) {
        SLOGI("Filesystem (ext4) auto-repair OK");
        return 0;
    } else {
        SLOGE("Auto-repair (ext4) failed (unknown exit code %d)", rc);
        errno = EIO;
        return -1;
    }
    return 0;
