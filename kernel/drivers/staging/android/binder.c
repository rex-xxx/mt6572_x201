...
static void binder_print_buf(struct binder_buffer *buffer, char *dest, int success, int check)
{
	...
	if (NULL == t) {
		...
		if ((log_entry != NULL) && (buffer->debug_id == log_entry->debug_id))
		{
			...
			int len_s = binder_proc_pid_cmdline(sender_tsk, sender_name);
			int len_r = binder_proc_pid_cmdline(rec_tsk, rec_name);
		...
		size_t size, int is_async)
{
	...
	int len_s = binder_proc_pid_cmdline(sender, sender_name);
	int len_r = binder_proc_pid_cmdline(target_proc->tsk, rec_name);
	if (size > threshold)
	{
		...
		{
			db_flag = db_flag | DB_OPT_VM_TRACES;
			binder_call_jbt(binder_check_buf_pid);
		}
		else
			printk(KERN_INFO "binder:sender pid %d (%s) is not javap, no sender bt!\n",
	...
	if (binder_check_is_javap(target_proc->tsk) > 0)
	{
		...
		printk(KERN_INFO "binder:target_proc %p pid %d (%s) is not javap, no rece bt!\n",
				target_proc, target_proc->pid,
				(target_proc->tsk != NULL) ? target_proc->tsk->comm : "");
	if (db_flag & DB_OPT_VM_TRACES)
	{
		binder_unlock(__func__);
		msleep(500);
		binder_lock(__func__);
...
static void mt_sched_setscheduler_nocheck(struct task_struct *p, int policy, struct sched_param *param)
{
 ...
 #ifdef CONFIG_MT_PRIO_TRACER
	sched_setscheduler_nocheck_binder(p, policy, param);
 #else
