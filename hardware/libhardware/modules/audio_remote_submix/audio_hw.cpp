...
const char * r_submix_streamout = "/sdcard/mtklog/audio_dump/r_submix_streamout.pcm";
                         ...
                         size_t bytes)
{
    ...
    ALOGV("out_write sink->write complete");
    pthread_mutex_lock(&out->dev->lock);
                       ...
                       size_t bytes)
{
...
#ifndef ANDROID_DEFAULT_CODE
    MonoPipe* sink = in->dev->rsxSink.get();
    if (sink != NULL) {
        sink->incStrong(buffer);//ALPS00504210 resolve NE cased by monopipe is closed during retry reading.
    } else {
        ALOGV("in_read without a pipe!");
    }
#endif
    ...
    while ((remaining_frames > 0) && (attempts < MAX_READ_ATTEMPTS)) {
        ...
        if (frames_read > 0) {
            ...
            ALOGV("  in_read read returned %ld", frames_read);
    ...
    pthread_mutex_lock(&in->dev->lock);
#ifndef ANDROID_DEFAULT_CODE
    if (sink != NULL) {sink->decStrong(buffer);}
#endif
...
{
    ALOGV("adev_get_supported_devices");
                     ...
                     hw_device_t** device)
{
    ...
    rsxadev->device.common.version = AUDIO_DEVICE_API_VERSION_2_0;//MTK_AUDIO
...
#define MTK_AUDIO_HARDWARE_MODULE_ID "libaudio"
struct audio_module HAL_MODULE_INFO_SYM = {
    /* common */ {
        /* tag */                HARDWARE_MODULE_TAG,
        /* module_api_version */ AUDIO_MODULE_API_VERSION_0_1,
        /* hal_api_version */    HARDWARE_HAL_API_VERSION,
        /* id */                 MTK_AUDIO_HARDWARE_MODULE_ID/*AUDIO_HARDWARE_MODULE_ID*/,
