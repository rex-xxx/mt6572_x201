...
import com.vladium.emma.filter.MethodFilter;
...
//    static{//for testing...
//        Strings.excludeMethod=true;
//        Strings.getExcludeMethods(new File("/proj/mtk54039/dev/mt6577_phone_ALPS.ICS.p114_eng_sub/external/emma/lib/exclude_methods.emma"));
//    }
    ...
    public Object visit (final IMethodCollection methods, final Object ctx)
    {
        ...
        for (int m = 0; m < originalMethodCount; ++ m)
        {
            ...
            //added by MTK54039 start
            // exclude method we don't want., don't instrument those codes.
            if(MethodFilter.sHasMethodFilter){
                String methodSignature=cls.getName().replace('/',  '.')+"."+Descriptors.methodVMNameToJavaName(cls.getName(), 
                        m_methodName, method.getDescriptor(cls), true, true, true);
                excluded=MethodFilter.isMethodExcluded(methodSignature);
            }
            //added by MTK54039 end
            if (excluded)
            {
                if (constructMetadata)
                {
