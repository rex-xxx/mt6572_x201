...
typedef struct s_smf_stream_tag
{
    ...
    EAS_CHAR            flag_isChannelMsg;  /* flag if the current event in the track is channel message */
} S_SMF_STREAM;
...
typedef struct s_smf_data_tag
{
    ...
    //Tina change type to I64 for calculation
    EAS_I64             time;               /* current time in milliseconds/256 */
    /*@ For MIDI seek table code */
    EAS_U32             nextTrackNum;
    EAS_I64             seekTime;
