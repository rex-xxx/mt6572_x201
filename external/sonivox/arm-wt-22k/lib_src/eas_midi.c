...
#include <utils/Log.h>
#undef LOG_TAG
#define LOG_TAG "WT_MIDI"
#define ENABLE_SKIP_USELESS_TICKS_BEFORE_1ST_NOTE

#define LOGV ALOGV
#define LOGD ALOGD
#define LOGE ALOGE

//#define ENABLE_LOG_MIDI
#ifdef ENABLE_LOG_MIDI
#undef LOGV
...
void EAS_InitMIDIStream (S_MIDI_STREAM *pMIDIStream)
{
    ...
    //Tina add for reseting midi stream flag 
...
EAS_RESULT EAS_ParseMIDIStream (S_EAS_DATA *pEASData, S_SYNTH *pSynth, S_MIDI_STREAM *pMIDIStream, EAS_U8 c, EAS_INT parserMode)
{
    ...
    if (c & 0x80)
    {
        ...
        if (c < 0xf8)
        {
            ...
            if ((c == 0xf7) || (c == 0xf0))
            {
#ifndef ENABLE_SKIP_USELESS_TICKS_BEFORE_1ST_NOTE
                if (parserMode == eParserModeMetaData)
                    return EAS_SUCCESS;
#endif
                LOGV("EAS_ParseMIDIStream call ProcessSysExMessage 01");
    ...
    if (pMIDIStream->byte3)
    {
        ...
        if (parserMode == eParserModeMetaData)
        {            
#ifdef ENABLE_SKIP_USELESS_TICKS_BEFORE_1ST_NOTE
            //Tina add
            if (((pMIDIStream->status&0xf0)==0x90)&&(pMIDIStream->d2))
            {
                pMIDIStream->flags |= MIDI_FLAG_FIRST_NOTE;
                LOGV("set pMIDIStream->flags=0x%x MIDI_FLAG_FIRST_NOTE", pMIDIStream->flags);
            }
#endif
            return EAS_SUCCESS;
        }
    ...
    if (pMIDIStream->runningStatus)
    {
        ...
        if (pMIDIStream->status == 0xF0)
        {
            ...
            if (parserMode == eParserModeMetaData)
                return EAS_SUCCESS;
#endif
            LOGV("EAS_ParseMIDIStream call ProcessSysExMessage 02");
            return ProcessSysExMessage(pEASData, pSynth, pMIDIStream, c, parserMode);
        }
...
#else
                pMIDIStream->flags |= MIDI_FLAG_GM_ON;
                LOGV("Set pMIDIStream->flags MIDI_FLAG_GM_ON, c=0x%x, pMIDIStream->flags=0x%x", c, pMIDIStream->flags);
                /* GM 1 or GM2 On, reset synth */
                if (parserMode != eParserModeMetaData)
                {
                    VMReset(pEASData->pVoiceMgr, pSynth, EAS_FALSE);
