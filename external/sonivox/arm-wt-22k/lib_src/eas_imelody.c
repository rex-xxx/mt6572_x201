...
#include <utils/Log.h>
#undef LOG_TAG
#define LOG_TAG "WT_IMELODY"

#define LOGV ALOGV
#define LOGD ALOGD
#define LOGE ALOGE

//#define ENABLE_LOG_IMELODY
#ifdef ENABLE_LOG_IMELODY
#undef LOGV
...
static EAS_RESULT IMY_Time (S_EAS_DATA *pEASData, EAS_VOID_PTR pInstData, EAS_I64 *pTime);
...
static EAS_RESULT IMY_PlayNote (S_EAS_DATA *pEASData, S_IMELODY_DATA *pData, EAS_I8 note, EAS_INT parserMode);
static EAS_RESULT IMY_GetDuration (EAS_HW_DATA_HANDLE hwInstData, S_IMELODY_DATA *pData, EAS_I32 *pDuration);
static EAS_RESULT IMY_GetVolume (EAS_HW_DATA_HANDLE hwInstData, S_IMELODY_DATA *pData, EAS_BOOL inHeader);
static EAS_RESULT IMY_GetNumber (EAS_HW_DATA_HANDLE hwInstData, S_IMELODY_DATA *pData, EAS_INT *temp, EAS_BOOL inHeader);
...
static EAS_RESULT IMY_CheckFileType (S_EAS_DATA *pEASData, EAS_FILE_HANDLE fileHandle, EAS_VOID_PTR *ppHandle, EAS_I32 offset)
{
    ...
    *ppHandle = NULL;
    if ((result = IMY_ReadLine(pEASData->hwInstData, fileHandle, buffer, NULL)) != EAS_SUCCESS)
    {
        if (result != EAS_ERROR_IMY_LINE_OVER75)
        return EAS_SUCCESS;
    }
...
static EAS_RESULT IMY_Time (S_EAS_DATA *pEASData, EAS_VOID_PTR pInstData, EAS_I64 *pTime)
{
    ...
    *pTime = pData->time >> 8;
    //LOGV("IMY_Time: *pTime = %lld, pData->time = %lld", *pTime, pData->time);
    return EAS_SUCCESS;
}
...
static EAS_RESULT IMY_Event (S_EAS_DATA *pEASData, EAS_VOID_PTR pInstData, EAS_INT parserMode)
{
    ...
    pData = (S_IMELODY_DATA*) pInstData;
    LOGV("IMY_Event pData->state =%d", pData->state);
// Tina: if ENDTOKEN is recognized, the state is EAS_STATE_PRESTOPPING        
// change state to EAS_STATE_STOPPING till next IMY_Event due to give to for last melody rendering
    if(pData->state == EAS_STATE_PRESTOPPING)
    {
        LOGV("IMY_Event set EAS_STATE_PRESTOPPING -> EAS_STATE_STOPPING");
        pData->state = EAS_STATE_STOPPING;
        LOGV("IMY_Event VMReleaseAllVoices");
        VMReleaseAllVoices(pEASData->pVoiceMgr, pData->pSynth);
        return EAS_SUCCESS;
    }    
    else if (pData->state >= EAS_STATE_OPEN)
        return EAS_SUCCESS;
    ...
    while (!eof)
    {
        ...
        //Tina change character to lower case
        c = ToLower(c);

        LOGV("IMY_Event: mode = %d, parse %c", parserMode, c);
        switch (c)
        {
            /* start repeat */
            case '(':
            ...
            case ')':
...
#endif
                LOGV("IMY_Event:  pData->repeatTime = %lld, pData->time = %lld", pData->repeatTime, pData->time);
                ...
                if (pData->repeatTime == pData->time) {
                    ...
                    else//Tina: the case is actually infinite loop
                    { 
                        if ((parserMode == eParserModeMetaData))
                        {
                            pData->time = (EAS_I64)0x7fffffffff;
                            eof = EAS_TRUE;
                            pEASData->infiniteLoopFile = EAS_TRUE;
                        }
                        LOGV("IMY_Event infinite loop, pData->time = %lld", pData->time);
                    }
...
//3 We need a different mode for metadata parsing where we don't loop at all
            ...
            /* repeat count */
            case '@':
                // Tina: modify the return type to EAS_RESULT, if it is EAS_TOKENEND, means "END:IMELODY" is parsed and ready to set STATE_PRESTOPPING
                result = IMY_GetNumber(pEASData->hwInstData, pData, &temp, EAS_FALSE);
                if ( result == EAS_FAILURE)
                    eof = EAS_TRUE;
                else if(result == EAS_TOKENEND)
                {                        
                    LOGV("IMY_Event @ set EAS_STATE_PRESTOPPING");
                    pData->state = EAS_STATE_PRESTOPPING;
                    eof = EAS_TRUE;
                }
            ...
            case 'l':
                if (!IMY_GetLEDState(pEASData, pData))
                    eof = EAS_TRUE;
                break;
            //Tina combine the vibrator and volume control to one case 
            /* vibeon or vibeoff or volume*/
            case 'v':
                if (!IMY_GetVibeState(pEASData, pData))
                {       
                    // Tina: modify the return type to EAS_RESULT, if it is EAS_TOKENEND, means "END:IMELODY" is parsed and ready to set STATE_PRESTOPPING
                    result = IMY_GetVolume(pEASData->hwInstData, pData, EAS_FALSE);
                    if ( result == EAS_FAILURE)
                        eof = EAS_TRUE;
                    else if(result == EAS_TOKENEND)
                    {                        
                        LOGV("IMY_Event V set EAS_STATE_PRESTOPPING");
                        pData->state = EAS_STATE_PRESTOPPING;                        
                    eof = EAS_TRUE;
                    }
                }
            ...
            case 'b':
                ...
                else
                {
                    PutBackChar(pData);
                    // Tina: modify the return type to EAS_RESULT, if it is EAS_TOKENEND, means "END:IMELODY" is parsed and ready to set STATE_PRESTOPPING
                    result = IMY_PlayNote(pEASData, pData, c, parserMode);                    
                    LOGV("IMY_Event IMY_PlayNote result = %d", result);
                    if (result == EAS_SUCCESS)
                        return EAS_SUCCESS;
                    else if(result == EAS_TOKENEND)
                    {                        
                        LOGV("IMY_Event B set EAS_STATE_PRESTOPPING");
                        pData->state = EAS_STATE_PRESTOPPING;
                    }
                    eof = EAS_TRUE;
                }
            ...
            /* must be a note */
            default:
                //LOGV("IMY_Event pData->volume = %d", pData->volume);
                if ((c >= 'a') && (c <= 'g'))
                {
                    // Tina: modify the return type to EAS_RESULT, if it is EAS_TOKENEND, means "END:IMELODY" is parsed and ready to set STATE_PRESTOPPING
                    result = IMY_PlayNote(pEASData, pData, c, parserMode);
...
#endif
    // Tina: if state is STATE_PRESTOPPING, means "END:IMELODY" is parsed, set STATE_PRESTOPPING and release till next IMY_EVENT
    if(pData->state != EAS_STATE_PRESTOPPING)
    {
        LOGV("IMY_Event set EAS_STATE_STOPPING");
    pData->state = EAS_STATE_STOPPING;
        LOGV("IMY_Event VMReleaseAllVoices");
    VMReleaseAllVoices(pEASData->pVoiceMgr, pData->pSynth);
    }
...
{
    S_IMELODY_DATA *pData;
    LOGV("IMY_GetData param =%d", param);
    pData = (S_IMELODY_DATA*) pInstData;
    ...
    //LOGV("IMY_GetData param =%d, *pValue=%d,", param, *pValue);
...
static EAS_RESULT IMY_PlayNote (S_EAS_DATA *pEASData, S_IMELODY_DATA *pData, EAS_I8 note, EAS_INT parserMode)
{
    ...
    /* get the duration */
    if ((result = IMY_GetDuration(pEASData->hwInstData, pData, &duration)) == EAS_FAILURE)
        return result;
    
    LOGV("IMY_PlayNote: result =%d", result);
    //Tina check the note range if # and &
    if(pData->noteModifier != 0)
    {
        if(pData->noteModifier == 1)
        {
            if(( note == 'b')||( note == 'e'))
             {
                LOGV("IMY_PlayNote: set # pData->noteModifier = 0, note =%d", note);
    ...
    {
        LOGV("IMY_PlayNote: VMStartNote pData->note=%d", pData->note);
    ...
    //LOGV("IMY_PlayNote: pData->time=%lld", pData->time);
...
static EAS_BOOL IMY_PlayRest (S_EAS_DATA *pEASData, S_IMELODY_DATA *pData)
{
    ...
    /* get the duration */
    if ((result_GetDuration = IMY_GetDuration(pEASData->hwInstData, pData, &duration)) == EAS_FAILURE)
        return EAS_FALSE;
    else if(result_GetDuration == EAS_TOKENEND)
        result = EAS_FALSE;
    else
        result = EAS_TRUE;
    ...
    /* next event is at end of this note */
    pData->time += duration;
    LOGV("IMY_PlayRest: pData->time=%lld, duration=%d,", pData->time, duration);
    return result;
}
...
static EAS_RESULT IMY_GetDuration (EAS_HW_DATA_HANDLE hwInstData, S_IMELODY_DATA *pData, EAS_I32 *pDuration)
{
    ...
    /* get the duration */
    *pDuration = 0;
    LOGV("IMY_GetDuration: IMY_GetNextChar");
    ...
    LOGV("IMY_GetDuration: 2 IMY_GetNextChar");
    ...
    for (i = 0; i < 6; i++)
    {
        ...
        switch (i)
        {
            ...
            default:
                if (c != vibeStr[i])
                {
                    //Tina: rollback the pointer of pData if it is not 'ibeo' string                    
                    for (j = i; j >=0; j--)
                        PutBackChar(pData);
                    return EAS_FALSE;
                }
...
static EAS_RESULT IMY_GetVolume (EAS_HW_DATA_HANDLE hwInstData, S_IMELODY_DATA *pData, EAS_BOOL inHeader)
{
    ...
    if (IsDigit(c))
        ...
        PutBackChar(pData);
    //Tina add: c==0, if not inHeader, it means TOKEN_END
    else if(!inHeader)                
        result = EAS_TOKENEND;
    
    if ((temp >= 0) && (temp <= 15))
    {
//Tina remove the original design: Ignoring V0 in header        
#if 0
        if (inHeader && (temp == 0))
...
static EAS_RESULT IMY_GetNumber (EAS_HW_DATA_HANDLE hwInstData, S_IMELODY_DATA *pData, EAS_INT *temp, EAS_BOOL inHeader)
{
    ...
    *temp = 0;
    ...
    for (;;)
    {
        //LOGV("IMY_GetNumber: IMY_GetNextChar");
        c = IMY_GetNextChar(hwInstData, pData, inHeader);
        
        if (IsDigit(c))
        {
            //Tina add for restricting maximum number to 9999
            //only for beat/repeat count
            if(*temp < 9999)
            {
            *temp = *temp * 10 + c - '0';
                if(*temp > 9999)
...
#endif
//Tina add the handling of exceeds 75 characters in a line
//    LOGV("IMY_ReadLine last i = %d",i);
//Tina remove the setting 2010/05/25
#if 0
    if( i == MAX_LINE_SIZE )
    {
        if ((result = EAS_HWGetByte(hwInstData, fileHandle, &c)) == EAS_SUCCESS)
            ...
            if(c != '\n')
            {
                ...
                {                
                    if ((result = EAS_HWGetByte(hwInstData, fileHandle, &c)) == EAS_SUCCESS)
                    {
                        if(c != '\n')
                        {
                            LOGV("IMY_ReadLine EAS_ERROR_IMY_LINE_OVER75");
                            return EAS_ERROR_IMY_LINE_OVER75;
                        }
                    }
                }
                else
