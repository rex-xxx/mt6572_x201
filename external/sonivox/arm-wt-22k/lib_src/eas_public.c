...
#include <utils/Log.h>
#undef LOG_TAG
#define LOG_TAG "LOG_EAS_public"

#define LOGV ALOGV
#define LOGD ALOGD
#define LOGE ALOGE

//#define ENABLE_LOG_EAS_public
#ifdef ENABLE_LOG_EAS_public
#undef LOGV
...
/* local prototypes */
static EAS_RESULT EAS_ParseEvents (S_EAS_DATA *pEASData, S_EAS_STREAM *pStream, EAS_I64 endTime, EAS_INT parseMode);
 ...
 *----------------------------------------------------------------------------
*/
static EAS_RESULT EAS_ParseEvents (S_EAS_DATA *pEASData, EAS_HANDLE pStream, EAS_I64 endTime, EAS_INT parseMode)
{
    S_FILE_PARSER_INTERFACE *pParserModule;
    EAS_RESULT result;
    EAS_I32 parserState;
    ...
    //EAS_U32 time = 0;
    EAS_I64 time = 0, currentTime = 0;
    EAS_I32 FileType;

	/*@ For MIDI seek table code */
	static EAS_BOOL havePreSeek = EAS_FALSE;
	
    LOGV("EAS_ParseEvents start: pStream->time %lld, endTime %lld", pStream->time, endTime);
    ...
    while (!done)
    {
        ...
        if ((result = (*pParserModule->pfState)(pEASData, pStream->handle, &parserState)) != EAS_SUCCESS)
        {
             LOGV("EAS_ParseEvents Stop done: State result!= EAS_SUCCESS");
            return result;
        } 
        
        //LOGV("EAS_ParseEvents parserState: %d", parserState);
        //Tina: parsing is completed
        //prestopping state need one more parsing to change state to stopping
        if ((parserState > EAS_STATE_PLAY)&&(parserState != EAS_STATE_PRESTOPPING))
        {
#if 0
...
#elif 0
            // Tina: if not in EAS_STATE_PLAY
            // always update currentTime
            if ((result = (*pParserModule->pfTime)(pEASData, pStream->handle, &currentTime)) != EAS_SUCCESS)
            {
                return result;
            }
            LOGV("EAS_ParseEvents: time = %lld, currentTime = %lld", time, currentTime);
            pStream->time = currentTime << 8;
            ...
            EAS_GetFileType (pEASData, pStream, &FileType);
            if(FileType == EAS_FILE_IMELODY)
            {    
                //Tina: add condition except iMelody, because iMelody play before updating time, should use current time 
                //smf  updating time before playing, should use previous available time 
                //iMelody: read current time
                pStream->time = currentTime << 8;                
                LOGV("EAS_ParseEvents: time = %lld, currentTime = %lld, pStream->time = %lld", time, currentTime, pStream->time);
            }
            //Tina: other format, keep original code
            //smf: during play mode, if update current time, the last time will larger than duration
            /* save accumulated time before previous parsing if we're not in play mode */                
            else if (parseMode != eParserModePlay)
            {
                pStream->time = currentTime << 8;
            }
#endif
            done = EAS_TRUE;
            LOGV("EAS_ParseEvents Stop done: parserState %d > EAS_STATE_PLAY %d", parserState, EAS_STATE_PLAY);
            break;
        }
        //Tina add for forcely stop the duration processing
        if ((parseMode == eParserModeMetaData)&&(pEASData->mForceStopDuration))
        {            
            done = EAS_TRUE;
            parserState == EAS_STATE_STOPPING;
            LOGV("EAS_ParseEvents Stop done: pEASData->mForceStopDuration =%d, parserState %d ", pEASData->mForceStopDuration, parserState);
            return EAS_ERROR_ALREADY_STOPPED;
        }
        /* get the next event time */
        if (pParserModule->pfTime)
        {
            //LOGV("EAS_ParseEvents call *pParserModule->pfTime");
repeat:     if ((result = (*pParserModule->pfTime)(pEASData, pStream->handle, &time)) != EAS_SUCCESS)
            {
                //LOGV("EAS_ParseEvents Stop done: Time result!= EAS_SUCCESS");
                return result;
            } 
            //LOGV("EAS_ParseEvents: before event time = %lld, pStream->time = %lld", time, pStream->time);

            /* if next event is within this frame, parse it */
            if (time < (endTime >> 8))
            {
               /*@ For MIDI seek table code */
			    if (!havePreSeek && (parseMode == eParserModeLocate) && pParserModule->pfSetData){
					havePreSeek = EAS_TRUE;
			    	(*pParserModule->pfSetData)(pEASData, pStream->handle, PARSER_DATA_LOCATE, endTime>>8);
					goto repeat;
			    }
                ...
                if (pParserModule->pfEvent){
                    ...
                    {
                        LOGV("EAS_ParseEvents Stop done: Event result!= EAS_SUCCESS");
                        return result;
            }
                }
            }
...
#else
                if(parseMode == eParserModeLocate)
                    pStream->time = time<<8;
                else                    
                    pStream->time = endTime;
#endif
                LOGV("EAS_ParseEvents Stop done: pStream->time %lld = endTime = %lld", pStream->time, endTime);

				/*@ For MIDI seek table code */
				if(parseMode == eParserModeLocate){
                	LOGD("EAS_ParseEvents Stop done: pStream->time %lld = endTime = %lld", pStream->time, endTime);
 					havePreSeek = EAS_FALSE;
				//	SMF_DumpStreamInfo(pEASData, pStream->handle, parseMode);
				}
        ...
        if (VMCheckWorkload(pEASData->pVoiceMgr))
        {
             ...
             */
            LOGV("EAS_ParseEvents Stop done: max workload exceeded");
            break;
        }
...
EAS_PUBLIC EAS_RESULT EAS_ParseMetaData (EAS_DATA_HANDLE pEASData, EAS_HANDLE pStream, EAS_I32 *playLength)
{
    ...
    pStream->time = 0;
    VMInitWorkload(pEASData->pVoiceMgr);
    //Tina enlarge the maximum ticks for total duration calculation
    if ((result = EAS_ParseEvents(pEASData, pStream, 0x7fffffffff, eParserModeMetaData)) != EAS_SUCCESS)
    {        
        // reset file
        if (pEASData->mForceStopDuration && EAS_ERROR_ALREADY_STOPPED == result) {
            pEASData->mForceStopDuration = EAS_FALSE;
            (*pParserModule->pfReset)(pEASData, pStream->handle);
        }
        LOGV("EAS_ParseMetaData: ERROR result from EAS_ParseEvents = %d,", result);
        return result;
    }
    /* get the parser time */
    if ((result = EAS_GetLocation(pEASData, pStream, playLength)) != EAS_SUCCESS)
    {
        LOGV("EAS_ParseMetaData: ERROR duration from EAS_GetLocation = %ld,", *playLength);
        return result;
    }
    else
    {
        LOGV("EAS_ParseMetaData: SUCCESS duration from EAS_GetLocation = %ld,", *playLength);
    }
    /* reset the parser to the beginning */
    pStream->time = 0;
...
#ifdef MTK_DRM_APP
EAS_PUBLIC EAS_RESULT EAS_ConsumeRights(EAS_DATA_HANDLE pEASData) 
{
	EAS_HWConsumeRights(pEASData->hwInstData);
	return EAS_SUCCESS;
}
#endif
...
EAS_PUBLIC EAS_RESULT EAS_State (EAS_DATA_HANDLE pEASData, EAS_HANDLE pStream, EAS_STATE *pState)
{
    ...
    LOGV("EAS_State: pStream->repeatCount = %d, pState=%d", pStream->repeatCount, *pState);
    if (pStream->repeatCount && (*pState == EAS_STATE_STOPPED))
    {
        LOGV("EAS_State: *pState = EAS_STATE_PLAY");
...
/* Tina add: Initialize and Set Reverb effect Parameters to default*/
    LOGD("EAS_Locate: Reverb effect Init and set EAS_PARAM_REVERB_BYPASS = EAS_FALSE...");
    if (pEASData->effectsModules[EAS_MODULE_REVERB].effectData)
    {
        if ((result = (*pEASData->effectsModules[EAS_MODULE_REVERB].effect->pfInit)(pEASData, &pEASData->effectsModules[EAS_MODULE_REVERB].effectData)) != EAS_SUCCESS)
        {
            LOGE("EAS_Locate: Reverb effect Init Fail !!!");
            return result;
        }
    }
    ...
    if (pEASData->infiniteLoopFile) {
        // force any seeking request to play from the initial position
        requestedTime = 0;
    }
 ...
 *----------------------------------------------------------------------------
 * Purpose:
 * Set the total duration time
 *
 * Inputs:
 *  pEASData        - buffer for internal EAS data
 *  duration_value  - value of duration
 *
 * Outputs:
 *  EAS_SUCCESS if total duration time was successfully set
 *
 *----------------------------------------------------------------------------
*/

EAS_PUBLIC EAS_RESULT EAS_SetDuration (EAS_DATA_HANDLE pEASData, EAS_I32 duration_value)
{
    if(pEASData != NULL)
    {    
        pEASData->durationTime = duration_value;
        //LOGV("duration_value=%ld, pEASData->durationTime=%ld, ", duration_value, pEASData->durationTime);        
        return EAS_SUCCESS;
    }
    else
    {
 ...
 *----------------------------------------------------------------------------
 * Purpose:
 * Set the flag to forcely stop the duration processing
 *
 * Inputs:
 *  pEASData             - buffer for internal EAS data
 *  mForceStopDuration   - true: forcely stop duration processing
 *
 * Outputs:
 *  EAS_SUCCESS if total duration time was successfully set
 *
...
EAS_PUBLIC EAS_RESULT EAS_SetForceStopDuration (EAS_DATA_HANDLE pEASData, EAS_BOOL mForceStopDuration)
{
    if(pEASData != NULL)
    {    
        pEASData->mForceStopDuration = mForceStopDuration;
        LOGV("mForceStopDuration=%ld, pEASData->mForceStopDuration=%ld, ", mForceStopDuration, pEASData->mForceStopDuration);        
        return EAS_SUCCESS;
    }
    else
    {
...
EAS_PUBLIC EAS_RESULT EAS_GetLocation (EAS_DATA_HANDLE pEASData, EAS_HANDLE pStream, EAS_I32 *pTime)
{
    ...
    *pTime = pStream->time >> 8;
    ...
    LOGV("EAS_GetLocation: *pTime =%ld, temp =%ld, pStream->time = %lld", *pTime, temp, pStream->time);        
#endif
    return EAS_SUCCESS;
}
