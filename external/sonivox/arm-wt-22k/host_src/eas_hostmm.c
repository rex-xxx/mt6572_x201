...
ssize_t readAtDRM(EAS_HW_DATA_HANDLE hwInstData, EAS_FILE_HANDLE file, off_t offset, void *data, size_t size) {
    ...
    if (file->mDrmBuf != NULL && file->mDrmBufSize > 0 && offset >= file->mDrmBufOffset && (offset + size) <= (file->mDrmBufOffset + file->mDrmBufSize)) {
        ...
        if (file->mDrmBufSize > 0) {
            ...
            ALOGE("readAtDRM: file [%d], failed to read any valid data from DRM file. mDrmBufSize [%lld]",
        ...
        if (result <= 0) {
            ALOGE("readAtDRM: file [%d], failed to read any valid data from DRM file. result [%ld]",
                    (int)file, result);
        }
        return result;
    }
    ...
    *pFile = NULL;
    ...
    for (i = 0; i < EAS_MAX_FILE_HANDLES; i++)
    {
        ...
        if (file->fd < 0)
        {
...
#ifdef MTK_DRM_APP
            ALOGV("EAS_HWOpenFile ----> file [%d] mDrmManagerClient [%d] mDecryptHandle [%d] mDrmBufOffset [%lld] mDrmBufSize [%lld] mDrmBuf [%d]",
                    (int)file, file->mDrmManagerClient, file->mDecryptHandle, file->mDrmBufOffset, file->mDrmBufSize, file->mDrmBuf);

            int isDcf = 0;
            if (locator->path) {
                ALOGD("EAS_HWOpenFile ----> use path [%s]", locator->path);
                isDcf = isDcf_path(locator->path);
            } else {
                ALOGD("EAS_HWOpenFile ----> use file descriptor [%d]", file->fd);
                isDcf = isDcf_fd(file->fd);
            }

            if (1 == isDcf) { // check dcf first so we can avoid create DrmManagerClient unnecessarily.
                ALOGD("EAS_HWOpenFile ----> identified a DRM file.");
                ...
                if (file->mDecryptHandle != NULL) {
                    ...
                    ALOGD("EAS_HWOpenFile ----> file [%d], valid key, realFileSize [%ld]", (int)file, realFileSize);
                    if (realFileSize >= 0) {
                        file->fileSize = realFileSize;
                    }
                } else {
                    ALOGD("EAS_HWOpenFile ----> file [%d], invalid key.", (int)file);
                    destroy_DrmManagerClient(client);
                    file->mDrmManagerClient = NULL;
                }
            } else {
                ALOGD("EAS_HWOpenFile ----> checked, not a DRM file.");
                file->mDrmManagerClient = NULL;
            ...
            file->mDrmBufSize = 0;
            file->mDrmBuf = NULL;
            ALOGV("EAS_HWOpenFile [result] ----> file [%d] mDrmManagerClient [%d] mDecryptHandle [%d] mDrmBufOffset [%lld] mDrmBufSize [%lld] mDrmBuf [%d]",
                    (int)file, file->mDrmManagerClient, file->mDecryptHandle, file->mDrmBufOffset, file->mDrmBufSize, file->mDrmBuf);
#endif
...
EAS_RESULT EAS_HWDupHandle (EAS_HW_DATA_HANDLE hwInstData, EAS_FILE_HANDLE file, EAS_FILE_HANDLE *pDupFile)
{
    ...
    for (i = 0; i < EAS_MAX_FILE_HANDLES; i++)
    {
        ...
        if (dupFile->fd < 0)
        {
            ...
            ALOGV("EAS_HWDupHandle: dupFile->mDrmBuf [%d] , file->mDrmBuf [%d]",
                    dupFile->mDrmBuf, file->mDrmBuf);
            if (dupFile->mDrmManagerClient != NULL && dupFile->mDecryptHandle != NULL && dupFile->mDrmBuf == NULL) {
                ALOGW("EAS_HWDupHandle: ***** Potential memory leak *****");
                ALOGW("Duplicate a DRM file with NULL drm buf, so 2 duplicated DRM file will not share drm buf.");
                ALOGW("And we only free one drm buf.");
            }
            ALOGV("EAS_HWDupHandle: file [%d] dupFile [%d] mDrmManagerClient [%d] mDecryptHandle [%d] mDrmBufOffset [%lld] mDrmBufSize [%lld] mDrmBuf [%d]",
...
EAS_RESULT EAS_HWCloseFile (EAS_HW_DATA_HANDLE hwInstData, EAS_FILE_HANDLE file1)
{
...
#ifdef MTK_DRM_APP
    ALOGV("EAS_HWCloseFile ----> file [%d] mDrmManagerClient [%d] mDecryptHandle [%d] mDrmBufOffset [%lld] mDrmBufSize [%lld] mDrmBuf [%d]",
    ...
    file1->mDrmBufOffset = 0;
    file1->mDrmBufSize = 0;
    ALOGV("EAS_HWCloseFile [result] ----> file [%d] mDrmManagerClient [%d] mDecryptHandle [%d] mDrmBufOffset [%lld] mDrmBufSize [%lld] mDrmBuf [%d]",
            (int)file1, file1->mDrmManagerClient, file1->mDecryptHandle, file1->mDrmBufOffset, file1->mDrmBufSize, file1->mDrmBuf);
#endif
