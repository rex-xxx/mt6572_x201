...
public final class HttpConnectionParams implements CoreConnectionPNames {
    ...
    /**
     * M: Support TCP MAX RTO option in java
     * Returns the maximum RTO (<tt>TCP_MAX_RTO</tt>) in seconds. 
     ...
     * @return timeout in milliseconds
     */
    public static int getTcpMaxRto(final HttpParams params) {
        if (params == null) {
            throw new IllegalArgumentException("HTTP parameters may not be null");
        }
        return params.getIntParameter(CoreConnectionPNames.TCP_MAX_RTO, 0);
    }
    
    /**
     * M: Support configure TCP RTO value in java
     * Sets the default socket timeout (<tt>TCP_MAX_RTO</tt>) in seconds which is the 
     * timer value for TCP retranmission. 
     * @hide 
     *
     * @param timeout Timeout in seconds
     */
   public static void setTcpMaxRto(final HttpParams params, int rto) {
        if (params == null) {
                throw new IllegalArgumentException("HTTP parameters may not be null");
        }
        params.setIntParameter(CoreConnectionPNames.TCP_MAX_RTO, rto);            
    }
