...
public class DefaultRequestDirector implements RequestDirector {
    ...
    /// M: Bug fix for CMCC Proxy
    private final int maxRetryRedirect = 2;
    private final String ErrorCode = "x-error-code";
            ...
            while (!done) {
                ...
                while (retrying) {
                    ...
                    try {
                        ...
                        /** M: debug purpose to print the exception */
    ...
    protected RoutedRequest handleResponse(RoutedRequest roureq,
        ...
        /** M: Bug fix for CMCC Proxy @{ */
        Boolean retryOrig = false;
        RoutedRequest newRequest = null;
        /** @} */
        ...
        if (HttpClientParams.isRedirecting(params) && 
            ...
            /** M: Bug fix for CMCC Proxy @{ */
            //Check the query string in the URI
            //Contain error code which is sent by CMCC proxy
            String uriQuery = uri.getQuery();
            if(uriQuery != null){
                System.out.println("uriQuery:" + uriQuery + " with " + redirectCount);
                if((redirectCount < maxRetryRedirect) && uriQuery.startsWith(ErrorCode)){
                    System.out.println("[ERROR in redirect]");
                    retryOrig = true;
                }
            }

            if(!retryOrig){ //No retry for HTTP Redirect resopnse
            ...
            }else{ //Retry for http redirect response
                ...
                //Get the original host from attribute
                HttpHost newTarget = (HttpHost) context.getAttribute(ExecutionContext.HTTP_TARGET_HOST);
                if (newTarget == null) {
                    throw new IllegalStateException("Target host not available in the HTTP context");
                }

                //Configure the new HTTP GET based on the original 
                HttpGet origReq = new HttpGet(origUri);
                origReq.setHeaders(orig.getAllHeaders());
                RequestWrapper wrapper = new RequestWrapper(origReq);
                wrapper.setParams(params);
                ...
                //Avoid CircularRedirectException
                context.setAttribute("http.protocol.redirect-locations", null);
                System.out.println("Redirecting to '" + origUri + "' via " + newRoute);
            }
            /** @} */
