...
long long Segment::ParseHeaders()
{
    ...
    if (status < 0) //error
    {
    	ALOGE("ParseHeaders, status < 0, return error!");
        return status;
    }
...
#ifndef ANDROID_DEFAULT_CODE
void ContentEncoding::ParseCompressionEntry( 
    long long start,
    long long size,
  ...
  while (pos < stop) {
    ...
    long long value=0;
    if (Match(pReader, pos, 0x0254, value)){
		       compression->algo = value;
			ALOGE("compression algo=%lld", compression->algo);
		}
		else if (Match(pReader, 
					   pos, 
					   0x0255, 
					   compression->settings, 
					   compression->settingSize)){
					   ALOGE("compression  settingsize =%d", compression->settingSize);
		}	  
		else {
                    long len;
                    const long long id = ReadUInt(pReader, pos, len);
...
void Track::GetContentAddInfo(unsigned char** data, size_t* size) const
{
	...
	if(GetContentEncodingCount()>0){
		...
		if(content->GetCompressionCount()>0){
			*size = content->GetCompressionByIndex(0)->settingSize;
...
long Tracks::Parse()
{
    ...
    while (pos < stop)
    {
        ...
        if (status < 0)  {//error
            ALOGE("Tracks::Parse, ParseElementHeader 1, return error!");
            return status;
        }
    ...
    while (pos < stop)
    {
        ...
        if (status < 0)  {//error
            ALOGE("Tracks::Parse, ParseElementHeader 2, return error!");
            return status;
        }
        ...
        if (id == 0x2E)  //TrackEntry ID
        {
            ...
            if (status){
                ALOGE("Tracks::Parse, ParseTrackEntry, return error!");
                return status;
            }
    ...
    Track*& pTrack) const
{
    ...
    while (pos < track_stop)
    {
        ...
        {
#ifndef ANDROID_DEFAULT_CODE
            if (size < 0)//Setting info can be none
#else
            if (size <= 0)
#endif
	     {
		        ALOGE("Tracks::Parse, if (id == 0x60) , return error!");
                return E_FILE_FORMAT_INVALID;
            }
...
#endif
	     {
		        ALOGE("Tracks::Parse, if (id == 0x61) , return error!");
                return E_FILE_FORMAT_INVALID;
            }
        ...
        {
            if (size <= 0){
		        ALOGE("Tracks::Parse, if (id == 0x2D80)  , return error!");
                return E_FILE_FORMAT_INVALID;
            	}
            e.start = start;
            e.size = size;
        ...
        {
            if ((size <= 0) || (size > 8)){
		        ALOGE("Tracks::Parse, if (id == 0x33C5)  , return error!");
                return E_FILE_FORMAT_INVALID;
            	}
        ...
        else if (id == 0x57)  //Track Number
        {
            ...
            if ((num <= 0) || (num > 127)){
		        ALOGE("Tracks::Parse, if (id == 0x57) , return error!");
                return E_FILE_FORMAT_INVALID;
            	}
        ...
        else if (id == 0x03)  //Track Type
        {
            ...
            if ((type <= 0) || (type > 254)){
                ALOGE("Tracks::Parse, if (id == 0x03) , return error!");
                return E_FILE_FORMAT_INVALID;
            	}
        ...
        else if (id == 0x23A2)  //Codec Private
        {
            ...
            if (size <= 0){
                ALOGE("Tracks::Parse, if (id == 0x23A2), return error!");
                return E_FILE_FORMAT_INVALID;
            	}
    ...
    if (i.number <= 0) { //not specified
        ALOGE("Tracks::Parse, if (i.number <= 0) , return error!");
        return E_FILE_FORMAT_INVALID;
    	}

    if (GetTrackByNumber(i.number)){
        ALOGE("Tracks::Parse,  if (GetTrackByNumber(i.number)) , return error!");
        return E_FILE_FORMAT_INVALID;
    }

    if (i.type <= 0)  {//not specified
        ALOGE("Tracks::Parse,  if (i.type <= 0) , return error!");
        return E_FILE_FORMAT_INVALID;
    	}
    ...
    if (status){
	    ALOGE("Tracks::Parse,  Parse track , return error!");
        return status;
    	}
...
#endif
	 //ALOGE("stop = %lld, pos=%lld, frame_size=%lld",stop,pos,frame_size);			
