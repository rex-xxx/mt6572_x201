...
#include <pthread.h>
#include <semaphore.h>
static pthread_mutex_t jpgFileMutex = PTHREAD_MUTEX_INITIALIZER;
static pthread_cond_t jpgFilecond = PTHREAD_COND_INITIALIZER;
static unsigned long g_u4InUse = 0;
...
//extern int js_getc();
//extern int js_read(void* ptr,size_t size,size_t count);
...
void DiscardData(void)
{
    ...
    SectionsRead = 0;
    HaveAll = 0;
pthread_mutex_lock(&jpgFileMutex);
g_u4InUse = 0;
pthread_cond_signal(&jpgFilecond);
pthread_mutex_unlock(&jpgFileMutex);
}
...
{
pthread_mutex_lock(&jpgFileMutex);
if(1 == g_u4InUse)//If in use
{
ALOGD("[EXIF]other thread is parsing exif, wait for this thread done.");
    pthread_cond_wait(&jpgFilecond , &jpgFileMutex);
ALOGD("[EXIF]Wait thread done.");
}
g_u4InUse = 1;
pthread_mutex_unlock(&jpgFileMutex);
    if (Sections == NULL){
