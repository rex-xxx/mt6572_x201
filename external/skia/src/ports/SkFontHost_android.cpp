...
SK_API bool SkIsTypefaceCoverScript(HB_Script script, SkTypeface* typeface) {
    SkUnichar unichar = getUnicodeFromHBScript(script);

    // If script is common script, we regard as the typeface can cover the script
    if (!unichar) {
        return true;
    }
    ...
    SkGlyphCache*    cache = autoCache.getCache();
    SkScalerContext* ctx = cache->getScalerContext();
    unsigned int glyphCount = ctx->getGlyphCount();

    uint16_t glyphId = ctx->charToGlyphID(unichar);
