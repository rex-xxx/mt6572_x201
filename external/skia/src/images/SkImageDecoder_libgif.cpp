...
bool SkGIFImageDecoder::onDecode(SkStream* sk_stream, SkBitmap* bm, Mode mode) {
    ...
    size_t length = sk_stream->getLength();

    size_t maxSize ;
    size_t rSize ;
#ifdef USE_MTK_ALMK
    if(!almkGetMaxSafeSize(getpid(), &maxSize)){
      maxSize = MTK_MAX_SRC_GIF_FILE_SIZE ;
      SkDebugf("ALMK::GIF::get Max Safe Size (Max %d bytes) for FileSize (%d bytes) fail!!\n", maxSize, length);
    }else{
      if(maxSize > length)
        SkDebugf("ALMK::GIF::Max Safe Size (Max %d bytes) for FileSize (%d bytes)=> PASS !! \n", maxSize, length);
      else
        SkDebugf("ALMK::GIF::Max Safe Size (Max %d bytes) for FileSize (%d bytes)=> MemoryShortage!! \n", maxSize, length);        
    }
#else
      maxSize = MTK_MAX_SRC_GIF_FILE_SIZE ;    
#endif    
    if (length > maxSize) {
        SkDebugf("Too large gif source (%d bytes)\n", length);
        return false;
    }
    ...
    rSize = gif->SWidth*gif->SHeight ;
#ifdef USE_MTK_ALMK     
    if(!almkGetMaxSafeSize(getpid(), &maxSize)){
      maxSize = MTK_MAX_SRC_GIF_PIXELS ;
      SkDebugf("ALMK::GIF::get Max Safe Size (Max %d bytes) for ImageSize (%d bytes) fail!!\n", maxSize, rSize);
    }else{
      if(maxSize > length)
        SkDebugf("ALMK::GIF::Max Safe Size (Max %d bytes) for ImageSize (%d bytes)=> PASS !! \n", maxSize, rSize);
      else
        SkDebugf("ALMK::GIF::Max Safe Size (Max %d bytes) for ImageSize (%d bytes)=> MemoryShortage!! \n", maxSize, rSize);        
    }
#else
      maxSize = MTK_MAX_SRC_GIF_PIXELS ;
#endif    
    //maximum pixels in a single frame is constrained to 1.5M
    if(gif->SWidth*gif->SHeight > maxSize) {
        SkDebugf("GIF : Too large dimension (%d %d)", gif->SWidth, gif->SHeight);
        return error_return(gif, *bm, "Pixels > FreeMemory");
    }
