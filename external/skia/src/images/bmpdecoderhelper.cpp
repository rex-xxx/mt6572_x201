...
#define MAX_BYTES   (MTK_MAX_SRC_BMP_PIXELS)
...
bool BmpDecoderHelper::DecodeImage(const char* p,
  ...
  len_ = len;
  inverted_ = true;
  unsigned int maxSize = MAX_BYTES;
  unsigned int rSize ;
  ...
  rSize = ((width_ * height_ * bpp_) >> 3) ;
#ifdef USE_MTK_ALMK 
  if(!almkGetMaxSafeSize(getpid(), &maxSize)){
    maxSize = MAX_BYTES ;
    SkDebugf("ALMK::BMP::Max Safe Size (Max %d bytes) for ImageSize(%d bytes) fail!!\n", maxSize, ((width_ * height_ * bpp_) >> 3));
  }else{
    if(maxSize > rSize)
      SkDebugf("ALMK::BMP::Max Safe Size (Max %d bytes) for ImageSize (%d bytes) => PASS !! \n", maxSize, rSize);
    else
      SkDebugf("ALMK::BMP::Max Safe Size (Max %d bytes) for ImageSize(%d bytes)=> MemoryShortage!! \n", maxSize, rSize);      
  }  
#endif
  if (rSize > maxSize) {
    return false;
