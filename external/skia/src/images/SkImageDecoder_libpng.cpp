...
bool SkPNGImageDecoder::onDecodeRegion(SkBitmap* bm, SkIRect region) {
    ...
    if (setjmp(png_jmpbuf(png_ptr))) {
        SkDebugf("SkPNGImageDecoder::onDecodeRegion Fail L:%d!!\n", __LINE__);	
        return false;
    }
    ...
    */
    ...
    if (SkBitmap::kIndex8_Config == config && 1 == sampleSize) {
        ...
        if (number_passes > 1) {
            ...
            if((srcBytesPerPixel*png_ptr->width) > (decodedBitmap->rowBytes()*decodedBitmap->height()) ){
               SkDebugf("SkPNG::onDecodeRegion useSrcRow aTop %d, fTop %d, BPP %d, dep %d, png_width %d, bufSize %d %d, sc %d,L:%d!!\n",
               actualTop, rect.fTop, srcBytesPerPixel, png_ptr->row_info.pixel_depth, png_ptr->width,
               decodedBitmap->rowBytes(),decodedBitmap->height(), sc,__LINE__);
               overWrite_flag = 1;
            }
