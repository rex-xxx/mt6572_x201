...
typedef struct ASimCardRec_ {
    ...
    char        out_buff[ 256 ];
    int         out_size;
    ASimStatus  startstatus;
    int         puk_retries;
} ASimCardRec;

static ASimCardRec  _s_card[A_SIM_CARD_COUNT];
...
*/
    ...
    if (Sim_count>A_SIM_CARD_COUNT)
    {
    	return NULL;
    }
   
    ASimCard card = &_s_card[Sim_count];
    asimcard_init( card);
    Sim_count++;
    card->port = port;
    return card;
...
void
asimcard_init(ASimCard card)
{
    card->status      = A_SIM_STATUS_READY;
    card->startstatus = A_SIM_STATUS_PIN;
    card->pin_retries = 3;
    card->puk_retries = 10;
    strncpy( card->pin, "1234", sizeof(card->pin) );
    strncpy( card->puk, "12345678", sizeof(card->puk) );
}
...
asimcard_check_puk( ASimCard  sim, const char* puk, const char*  pin )
{
    ...
    if ( !strcmp( sim->puk, puk ) ) {
        ...
        sim->puk_retries = 10;
        sim->pin_retries = 3;
        printf("[Emu] asimcard_check_puk3 \r\n");	
        return 1;
    }

    if ( --sim->puk_retries == 0 ) {
...
{
	return sim->pin_retries;
}

int 
asimcard_get_puk_retries(ASimCard  sim)
{
	return sim->puk_retries;
}

ASimStatus
asimcard_get_startstatus( ASimCard  sim )
{
    return sim->startstatus;
}

void
asimcard_set_startstatus( ASimCard  sim, ASimStatus  startstatus )
{
    sim->startstatus = startstatus;
}
