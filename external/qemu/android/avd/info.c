...
struct AvdInfo {
    ...
    int       numSearchPaths;
    char*     contentPath;
    // Add for device target information
    char*     contentTarget;
...
static int
_getContentTarget(AvdInfo* i)
{
    i->contentTarget = iniFile_getString(i->rootIni, ROOT_TARGET_KEY, NULL);
    if (i->contentTarget == NULL) {
        return -1;
    }
    D("virtual device target is %s", i->contentTarget);
    return 0;
}
...
// Add for device target infromation.
const char*
avdInfo_getContentTarget(AvdInfo* i)
{
	return i->contentTarget;
}
