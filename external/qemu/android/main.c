...
int main(int argc, char **argv)
{
    ...
    char*             skinPath;
    int               inAndroidBuild;
    uint64_t          defaultPartitionSize = convertMBToBytes(360);
    ...
    if (opts->version) {
        ...
        // Print MediaTek version information.
        printf("MediaTek emulator (build: %s)\n", STRINGIFY(MTK_BUILD_VERNO));
...
#else
        // should keep the rwImage and initImage information for creating system-qemu.img which is same as system.img but the attribute is writable.
        hw->disk_systemPartition_path	  = rwImage;
        hw->disk_systemPartition_initPath = initImage;
#endif
