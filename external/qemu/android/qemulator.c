...
static void
qemulator_set_led_notification( void* opaque, const char*  name, int  on, int off, int color )
{
    QEmulator*  emulator = opaque;

    VERBOSE_PRINT(hw_control,"%s: name='%s' on=%d off=%d color=%d window=%p", __FUNCTION__, name, on, off, color, emulator->window);
    if ( !strcmp(name, "led")) {
        skin_window_set_led_notification(emulator->window, name, on, off, color);
        return;
    }
}

static void
qemulator_set_vibrator_notification( void* opaque, const char*  name, int  on, int off)
{
    QEmulator*  emulator = opaque;

    VERBOSE_PRINT(hw_control,"%s: name='%s' on=%d off=%d window=%p", __FUNCTION__, name, on, off, emulator->window);
    if (!strcmp(name, "vibrator")) {
        skin_window_set_vibrator_notification(emulator->window, name, on, off);
        return;
    }
}
...
qemulator_setup( QEmulator*  emulator )
{
    ...
    uicmd_set_control_change_callback(qemulator_light_brightness, qemulator_set_led_notification, qemulator_set_vibrator_notification,
                                         emulator);
}
