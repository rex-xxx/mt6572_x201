...
static void
_uiCmdProxy_set_led_notification_callback(void* opaque,
                                       const char* name,
                                       int on, int off, int color)
{
    // Calculate size of the command parameters.
    const size_t cmd_size = sizeof(UICmdSetNotification) + strlen(name) + 3;
    // Allocate and initialize parameters.
    UICmdSetNotification* cmd =
        (UICmdSetNotification*)qemu_malloc(cmd_size);
    cmd->color = color;
    cmd->on = on;
    cmd->off = off;
    strcpy(cmd->name, name);
    dprint("%s: name='%s' name='%s' on=%d off=%d color=%d ", __FUNCTION__, name, cmd->name, on, off, color);
    // Send the command.
    _uiCmdProxy_send_command(AUICMD_SET_LED_NOTIFICATION, cmd, cmd_size);
    qemu_free(cmd);
}

static void
_uiCmdProxy_set_vibrator_notification_callback(void* opaque,
                                       const char* name,
                                       int on, int off)
{
    // Calculate size of the command parameters.
    ...
    cmd->off = off;
    strcpy(cmd->name, name);
    dprint("%s: name='%s' name='%s' on=%d off=%d", __FUNCTION__, name, cmd->name, on, off);
    // Send the command.
    _uiCmdProxy_send_command(AUICMD_SET_VIBRATOR_NOTIFICATION, cmd, cmd_size);
    qemu_free(cmd);
}
...
uiCmdProxy_create(int fd)
{
    ...
    }
    {
        ...
        funcs.led_notification = _uiCmdProxy_set_led_notification_callback;
        funcs.vibrator_notification = _uiCmdProxy_set_vibrator_notification_callback;
