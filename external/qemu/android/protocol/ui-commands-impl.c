...
static AndroidHwLedNotificationCallback _set_led_notification_callback = NULL;

static AndroidHwVibratorNotificationCallback _set_vibrator_notification_callback = NULL;
    ...
    switch (header->cmd_type) {
				...
				case AUICMD_SET_LED_NOTIFICATION:
        {
            ...
            if (_set_led_notification_callback != NULL) {
                _set_led_notification_callback(_brightness_change_callback_param,
                                            cmd->name, cmd->on, cmd->off, cmd->color);
            }
            break;
        }
				...
				case AUICMD_SET_VIBRATOR_NOTIFICATION:
        {
            ...
            if (_set_vibrator_notification_callback != NULL) {
                _set_vibrator_notification_callback(_brightness_change_callback_param,
                                            cmd->name, cmd->on, cmd->off);
            }
            break;
        }
                                     ...
                                     void* opaque)
{
    ...
    _set_vibrator_notification_callback = callback3;
    _brightness_change_callback_param = opaque;
    return 0;
}
