...
static SkinNotification*
skin_notification_create_from( AConfig*   node, const char*  basepath )
{
    SkinNotification*  button;
    ANEW0(button);
    if (button) {
        const char*  img = aconfig_str(node, "image", NULL);
        int          x   = aconfig_int(node, "x", 0);
        ...
        button->rect.pos.x = x;
        button->rect.pos.y = y;
				dprint( "%s: skin_notification_create_from '%s'\n", __FUNCTION__, button->name);
        if (img != NULL)
            button->image = skin_image_find_in( basepath, img );
...
skin_part_create_from_v2( AConfig*  root, const char*  basepath )
{
    ...
    node = aconfig_find(root, "notifications");
    if (node) {
        for (node = node->first_child; node != NULL; node = node->next)
        {
            SkinNotification*  button = skin_notification_create_from(node, basepath);

            if (button != NULL) {
            		//dprint( "## LOG: skin_notification_create_from '%s'\n", button->name ? "<NULL>" : button->name );
            		dprint( "## LOG: skin_notification_create_from '%s'\n", button->name);

                button->next  = part->notifications;
    ...
    SKIN_PART_LOOP_NOTIFICATIONS(part, button)
        skin_box_minmax_update( &box, &button->rect );
    SKIN_PART_LOOP_END2
