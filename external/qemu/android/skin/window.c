...
static void
notification_done( Notification*  notification )
{
    skin_image_unref( &notification->image );
    notification->background = NULL;
}

static void
notification_init( Notification*  notification, SkinNotification*  snotification, SkinLocation*  loc, Background*  back, SkinRect*  frame, SkinLayout*  slayout )
{
    SkinRect  r;
    ...
    notification->shown       = 0;
		dprint("%s shown = %d", __FUNCTION__, notification->shown);
    skin_rect_rotate( &r, &snotification->rect, loc->rotation );
    r.pos.x += loc->anchor.x;
    r.pos.y += loc->anchor.y;
    notification->origin = r.pos;
    skin_rect_intersect( &notification->rect, &r, frame );
}

static void
notification_redraw( Notification*  notification, SkinRect*  rect, SDL_Surface*  surface )
{	
    SkinRect  r;

		dprint("notification_redraw");
    if (skin_rect_intersect( &r, rect, &notification->rect ))
    {
    	dprint("skin_rect_intersect -- outer %d", notification->shown);
    	dprint("%s shown = %d", __FUNCTION__, notification->shown);
        if ( notification->shown && notification->image != SKIN_IMAGE_NONE )
        {
        	dprint("skin_rect_intersect -- inter");
            SDL_Rect  rs, rd;

            rs.x = r.pos.x - notification->origin.x;
            rs.y = r.pos.y - notification->origin.y;
            rs.w = r.size.w;
            rs.h = r.size.h;
            ...
            SDL_BlitSurface( skin_image_surface(notification->image), &rs, surface, &rd );
        }
    }
}
...
#define  LAYOUT_LOOP_NOTIFICATIONS(layout,notification)                          \
    do {                                                             \
        Notification*  __button = (layout)->notifications;                       \
        Notification*  __button_end = __button + (layout)->num_notifications;    \
        for ( ; __button < __button_end; __button ++ ) {             \
            Notification*  notification = __button;

#define  LAYOUT_LOOP_END_NOTIFICATIONS \
        }                        \
    } while (0);
...
layout_done( Layout*  layout )
{
    ...
    for (nn = 0; nn < layout->num_notifications; nn++)
        notification_done( &layout->notifications[nn] );
...
{
    int       n_buttons, n_backgrounds, n_displays, n_notifications;
    ...
    SKIN_LAYOUT_LOOP_LOCS(slayout,loc)
        ...
        SKIN_PART_LOOP_NOTIFICATIONS(part, snotification)
            n_notifications += 1;
    ...
    AARRAY_NEW0(layout->notifications,     n_notifications);
    ...
    SKIN_LAYOUT_LOOP_LOCS(slayout,loc)
        ...
        SKIN_PART_LOOP_NOTIFICATIONS(part, snotification)
            Notification*  notification = layout->notifications + n_notifications;
            notification_init( notification, snotification, loc, back, &layout->rect, slayout );
            n_notifications += 1;
        SKIN_PART_LOOP_END
...
struct SkinWindow {
    ...
    LedNotificationState		led_notification;
    VibratorNotificationState		vibrator_notification;
};
