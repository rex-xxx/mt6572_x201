...
typedef struct SkinNotification {
    SkinImage*  image;
    SkinRect    rect;
    const char* name;
    struct SkinNotification*  next;
} SkinNotification;
...
typedef struct SkinPart {
    ...
    SkinButton*        buttons;
    SkinNotification*  notifications;
...
#define  SKIN_PART_LOOP_NOTIFICATIONS(part,button)              \
    do {                                                  \
        SkinNotification*  __button = (part)->notifications;          \
        while (__button != NULL) {                        \
            SkinNotification*  __button_next = __button->next;  \
            SkinNotification*  button        = __button;

#define   SKIN_PART_LOOP_END2             \
            __button = __button_next;    \
        }                                \
    } while (0);
