...
_hwSensorClient_tick( void*  opaque )
{
    ...
    if (_hwSensorClient_enabled(cl, ANDROID_SENSOR_GYROSCOPE)) {
        sensor = &hw->sensors[ANDROID_SENSOR_GYROSCOPE];
        snprintf(buffer, sizeof buffer, "gyroscope:%g:%g:%g",
                 sensor->u.gyroscope.x,
                 sensor->u.gyroscope.y,
                 sensor->u.gyroscope.z);
        _hwSensorClient_send(cl, (uint8_t*)buffer, strlen(buffer));
    }
...
_hwSensors_setSensorValue( HwSensors*  h, int sensor_id, float a, float b, float c )
{
    ...
    //mark below three lines because this does not work
    //s->u.value.a = a;
    //s->u.value.b = b;
    //s->u.value.c = c;
    //add below codes for setting sensor values
	switch(sensor_id)
	{
	   case ANDROID_SENSOR_ACCELERATION:
	   	   s->u.acceleration.x = a;
    	   s->u.acceleration.y = b;
           s->u.acceleration.z = c;
		   break;
	   ...
	   case ANDROID_SENSOR_ORIENTATION:
	   	   s->u.orientation.azimuth = a;
           s->u.orientation.pitch   = b;
           s->u.orientation.roll    = c;
		   break;
	   case ANDROID_SENSOR_TEMPERATURE:
	   	   s->u.temperature.celsius = a;   
		   break;
...
_hwSensors_save( QEMUFile*  f, QemudService*  sv, void*  opaque)
{
    ...
    for (i = 0 ; i < MAX_SENSORS; i++) {
        ...
        case ANDROID_SENSOR_PROXIMITY:
            ...
            break;
        case ANDROID_SENSOR_GYROSCOPE:
            qemu_put_float(f, s->u.gyroscope.x);
            qemu_put_float(f, s->u.gyroscope.y);
            qemu_put_float(f, s->u.gyroscope.z);
            break;
        case MAX_SENSORS:
            break;
        }
...
_hwSensors_load( QEMUFile*  f, QemudService*  s, void*  opaque)
{
    ...
    for (i = 0 ; i < num_sensors; i++) {
        ...
        case ANDROID_SENSOR_PROXIMITY:
            ...
            break;
        case ANDROID_SENSOR_GYROSCOPE:
            s->u.gyroscope.x = qemu_get_float(f);
            s->u.gyroscope.y = qemu_get_float(f);
            s->u.gyroscope.z = qemu_get_float(f);
            break;
        case MAX_SENSORS:
            break;
        }
...
_hwSensors_init( HwSensors*  h )
{
    ...
    if (android_hw->hw_gyroscope) {
        h->sensors[ANDROID_SENSOR_GYROSCOPE].enabled = 1;
      }
