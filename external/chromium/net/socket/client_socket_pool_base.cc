...
#define LOG_TAG "chromium/tcp"
#define XLOG_TAG "chromium/tcp"
...
int ConnectJob::Connect() {
  ...
  XLOGD2("LogConnectStart:%s", group_name_.c_str());

  int rv = ConnectInternal();

  XLOGD2("LogConnectEnd:%s:%d", group_name_.c_str(), rv);

  if (rv != ERR_IO_PENDING) {
    if( rv != OK){
        XLOGD("Connect Error:%d", rv);
    }
...
void ConnectJob::LogConnectCompletion(int net_error) {
  if(net_error != OK){
    XLOGD("LogConnectCompletion with error:%d", net_error);
  }
