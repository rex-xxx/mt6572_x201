...
void HttpUtil::ParseContentType(const string& content_type_str,
  ...
  /// M: workaround for the contenttype was ".jar" @{
  else if (content_type_str == ".jar"){
    mime_type->assign("application/java-archive");
    //LOG(ERROR) << "ParseContentType, set application/java-archive for .jar" << std::endl;
  }
  /// @}
}
