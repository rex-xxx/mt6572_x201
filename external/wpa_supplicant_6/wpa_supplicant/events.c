...
static int wpa_supplicant_ssid_wapi_bss_match(struct wpa_supplicant *wpa_s,
					 struct wpa_ssid *ssid,
					 struct wpa_scan_res *bss)
{
	...
	while ((ssid->proto & WPA_PROTO_WAPI) && wapi_ie) {
		...
		if (wpa_parse_wapi_ie(wapi_ie, 2 + wapi_ie[1], &ie)) {
	...
	if (proto_match == 0)
		wpa_printf(MSG_DEBUG, "   skip - no WAPI proto match");
...
#ifdef CONFIG_ORANGE_SUPPORT
static struct wpa_scan_res *
wpa_supplicant_select_bss_wep(struct wpa_supplicant *wpa_s,
				  struct wpa_ssid *group,
				  struct wpa_ssid **selected_ssid)
{
	struct wpa_ssid *ssid;
	...
	wpa_printf(MSG_DEBUG, "Try to find WEP AP");
	for (i = 0; i < wpa_s->scan_res->num; i++) {
		const u8 *ssid_;
		u8 wpa_ie_len, rsn_ie_len, ssid_len;
		bss = wpa_s->scan_res->res[i];
		...
		for (ssid = group; ssid; ssid = ssid->pnext) {
			...
			if (!(bss->caps & IEEE80211_CAP_PRIVACY) || !wpa_supplicant_match_privacy(bss, ssid)) {
				wpa_printf(MSG_DEBUG, "   skip - "
					   "privacy mismatch");
				continue;
			}
				  ...
				  struct wpa_ssid **selected_ssid)
{
	...
	wpa_printf(MSG_DEBUG, "Try to find OPEN AP");
	for (i = 0; i < wpa_s->scan_res->num; i++) {
		const u8 *ssid_;
		u8 wpa_ie_len, rsn_ie_len, ssid_len;
		bss = wpa_s->scan_res->res[i];
...
static struct wpa_scan_res *
wpa_supplicant_select_bss_public(struct wpa_supplicant *wpa_s, struct wpa_ssid *group,
			  struct wpa_ssid **selected_ssid)
{
	struct wpa_scan_res *selected;
	...
	return wpa_supplicant_select_bss_open(wpa_s, group, selected_ssid);
}
#endif
...
/* WPS availability is fetched from scan results. Reduce logging. */
	...
	while (selected == NULL) {
		...
		for (prio = 0; prio < wpa_s->conf->num_prio; prio++) {
			selected = wpa_supplicant_select_bss_private(
		...
		{
			for (prio = 0; prio < wpa_s->conf->num_prio; prio++) {
				selected = wpa_supplicant_select_bss_public(
					wpa_s, wpa_s->conf->pssid[prio], &ssid);
				if (selected)
					break;
			}
	...
	if (selected) {
		...
		else
		{
			...
			{
				wpa_msg(wpa_s, MSG_INFO, WPS_EVENT_NO_OVERLAP);
			}
		}
        ...
        wpa_printf(MSG_DEBUG, "[associate] %s:: wpa_s->reassociate = %d, selected->bssid ="
				   MACSTR "\nwpa_s->bssid = "  MACSTR "wpa_s->wpa_state = %d"  "wpa_s->pending_bssid = " MACSTR,
				   __FUNCTION__,
				   wpa_s->reassociate,
		...
		if (wpas_wps_searching(wpa_s)) {
			...
			wpa_msg(wpa_s, MSG_INFO, WPA_EVENT_NO_SUITABLE_AP);
		}
#else
		timeout = 5;
#endif
					   ...
					   union wpa_event_data *data)
{
	...
	while (p && l >= 2) {
		...
		if (!wapi_found &&
			...
			wapi_found = 1;
			//wapi_sm_set_ap_wapi_ie(wpa_s->wpa, p, len);
		}
#endif
...
#ifdef CONFIG_WAPI_SUPPORT
	//if (!wapi_found)
		//wapi_sm_set_ap_wapi_ie(wpa_s->wapi, NULL, 0);
#endif
