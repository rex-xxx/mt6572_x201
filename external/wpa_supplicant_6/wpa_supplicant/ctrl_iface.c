...
static int wpa_supplicant_ctrl_iface_wps_abort(struct wpa_supplicant *wpa_s)
{
...
#ifdef CONFIG_WAPI_SUPPORT
	   wpa_printf(MSG_DEBUG,"[Debug] %s => ", __FUNCTION__);
	if(os_memcmp(proto,"WAPI", 4)==0){
			   wpa_printf(MSG_DEBUG,"[Debug] %s => WAPI ie ", __FUNCTION__);
...
#ifdef CONFIG_WAPI_SUPPORT
		iewapi = wpa_scan_get_ie(res, 68);
		if (iewapi)
			pos = wpa_supplicant_ie_txt(pos, end, "WAPI", iewapi, 2 + iewapi[1]);

		if (!ie && !ie2 && !iewapi && res->caps & IEEE80211_CAP_PRIVACY) {
...
#if 1 /* MT6620 */
static int wpa_supplicant_driver_sw_cmd(struct wpa_supplicant *wpa_s,
                                     char *cmd, int set, char *buf, size_t buflen)
{
    int ret;
    unsigned long adr = 0, dat = 0;
    char * pEnd;

    adr = strtoul(cmd, &pEnd, 16);
    if (set) {
        dat = strtoul(pEnd, &pEnd, 16);
    }

    ret = wpa_drv_driver_sw_cmd(wpa_s, set, &adr, &dat);
    ...
    if (set) {
        ...
        if (ret == 0) {
            wpa_printf(MSG_DEBUG, "return value %x\n", dat);
            os_snprintf(buf, buflen, "%08lx", dat);
            ret = 8;
        }
    }
    return( ret );
					 ...
					 char *buf, size_t *resp_len)
{
	...
	if (os_strcmp(buf, "PING") == 0) {
		...
		if (wpa_supplicant_ctrl_iface_wps_reg(wpa_s, buf + 8))
			reply_len = -1;
	} else if (os_strncmp(buf, "WPS_ABORT", 9) == 0) {
		if (wpa_supplicant_ctrl_iface_wps_abort(wpa_s))
			reply_len = -1;
...
#if 1 /* MT6620 */
    } else if (os_strncmp(buf, "SW_CMD_SET ", 11) == 0) {
        reply_len = wpa_supplicant_driver_sw_cmd(wpa_s, buf + 11, 1, reply, reply_size);
    } else if (os_strncmp(buf, "SW_CMD_GET ", 11) == 0) {
        reply_len = wpa_supplicant_driver_sw_cmd(wpa_s, buf + 11, 0, reply, reply_size);
#endif
