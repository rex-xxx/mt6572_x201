...
SUPPLICANT_CONF = default

ifeq ($(findstring OP01, $(strip $(OPTR_SPEC_SEG_DEF))),OP01)
CONFIG_CMCC_SUPPORT=y
SUPPLICANT_CONF = CMCC
endif

ifeq ($(OPTR_SPEC_SEG_DEF), OP03_SPEC0200_SEGDEFAULT)
CONFIG_ORANGE_SUPPORT=y
endif

ifeq ($(MTK_EAP_SIM_AKA), yes)
CONFIG_EAP_SIM_AKA=y
CONFIG_USIM_SIMULATOR=y
CONFIG_SIM_SIMULATOR=y
L_CFLAGS += -DMTK_EAP_SIM_AKA
endif

#ICS Legacy WLAN
L_CFLAGS += -DICS_LEGACY_WLAN_SUPPORT
#NEED_RFKILL=y
...
OBJS += src/utils/rild_porting.c
OBJS += src/utils/rild_sim.c
endif
...
L_CFLAGS += -DCONFIG_CMCC_SUPPORT
endif

ifdef CONFIG_ORANGE_SUPPORT
L_CFLAGS += -DCONFIG_ORANGE_SUPPORT
endif
...
LOCAL_SHARED_LIBRARIES += libbwcs_mtk
endif
...
ifdef CONFIG_EAP_SIM_AKA
include $(CLEAR_VARS)
LOCAL_MODULE := test_rild_porting
LOCAL_MODULE_TAGS := eng
LOCAL_SHARED_LIBRARIES := libc libcutils libcrypto libssl libnetutils
LOCAL_CFLAGS :=$(L_CFLAGS)
LOCAL_SRC_FILES := tests/test_rild_porting.c
...
#include $(CLEAR_VARS)
#LOCAL_MODULE := libwapi.so
#LOCAL_MODULE_TAGS := user
#LOCAL_MODULE_CLASS := SHARED_LIBRARIES
#LOCAL_MODULE_PATH := $(local_target_dir)
#LOCAL_SRC_FILES := ../wapi/libwapi.so
...
LOCAL_MODULE_PATH := $(local_target_dir)
ifneq ($(HAVE_INTERNAL_WPA_SUPPLICANT_CONF),true)
LOCAL_SRC_FILES := $(LOCAL_MODULE)
else

ifeq ($(SUPPLICANT_CONF), CMCC)
LOCAL_SRC_FILES := CMCC-wpa_supplicant.conf
endif

ifeq ($(SUPPLICANT_CONF), default)
