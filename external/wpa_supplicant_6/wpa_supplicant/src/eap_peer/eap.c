...
SM_STATE(EAP, FAILURE)
{
	...
	{
	   struct wpa_supplicant* wpa_ss = (struct wpa_supplicant *)sm->msg_ctx;
	   if (wpa_ss->current_ssid){
	       if(!(wpa_ss->current_ssid->key_mgmt & WPA_KEY_MGMT_WPS)){
...
#ifdef MTK_EAP_SIM_AKA
	os_strncpy(imsi, conf->imsi, conf->imsi_len + 1);
	imsi_len = conf->imsi_len;
...
//	conf->identity[0] = aka ? '0' : '1';
//	os_memcpy(conf->identity + 1, imsi, imsi_len);
	os_memcpy(conf->identity, imsi, imsi_len);
	conf->identity[imsi_len] = '\0';
	conf->identity_len = imsi_len;
