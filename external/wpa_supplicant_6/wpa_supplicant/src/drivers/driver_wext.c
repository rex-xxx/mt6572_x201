...
//	wpa_printf(MSG_DEBUG, "ifi->ifi_index=%d drv->ifname = %s ifindex=%d %d\n", ifi->ifi_index, drv->ifname,
		//		    drv->ifindex, drv->ifindex2);
				 ...
				 char *end)
{
	...
	if (clen > 7 && os_strncmp(custom, "wpa_ie=", 7) == 0) {
		...
		res->ie_len += bytes;
				   ...
				   os_strncmp(custom, "wapi_ie=", 8) == 0) {
				wpa_printf(MSG_DEBUG,"[WAPI] Get wapi IE by IWEVCUSTOM");
				char *spos;
				int bytes;
				spos = custom + 8;
				bytes = custom + clen - spos;
				...
				if (bytes > SSID_MAX_WAPI_IE_LEN) {
					wpa_printf(MSG_INFO, "[WAPI] Too long WAPI IE "
						   "(%d)", bytes);
					return;
				}
				tmp = os_realloc(res->ie, res->ie_len + bytes);
				...
				res->ie = tmp;
				res->ie_len += bytes;
                wpa_hexdump_ascii(MSG_DEBUG, "wapi ie", (u8 *)tmp, bytes);
			}
#endif
				       ...
				       const u8 *key, size_t key_len)
{
	...
	if(alg == WAPI_ALG_SMS4){
		//wpa_printf(MSG_DEBUG, "%s: 1", __FUNCTION__);
    ...
    else if(alg != WAPI_ALG_SMS4){
			wpa_printf(MSG_DEBUG, "%s: 2", __FUNCTION__);
...
#ifdef CONFIG_WAPI_SUPPORT
	case WAPI_ALG_SMS4:
		wpa_printf(MSG_DEBUG, "[WAPI-DEBUG]3 %s:",
			   __FUNCTION__);
...
#ifdef CONFIG_WAPI_SUPPORT
		if(alg == WAPI_ALG_SMS4) {
					wpa_printf(MSG_DEBUG, "[WAPI-DEBUG]4 %s: ",
			   __FUNCTION__);

			memcpy(ext->tx_seq, seq,seq_len);
...
//#endif
	...
	if (iwr.u.mode == IW_MODE_INFRA) {
		...
		wpa_driver_wext_set_bssid(drv, null_bssid);
                ...
                //Setting Non-printable SSID
                for (i = 0; i < 32; i++)
                    ssid[i] = (rand() & 0x1e) + 0x01;
...
int wpa_driver_wext_keymgmt2wext(int keymgmt)
{
	...
	case KEY_MGMT_WAPI_PSK:
		return IW_AUTH_KEY_MGMT_WAPI_PSK;
	case KEY_MGMT_WAPI_CERT:
		return IW_AUTH_KEY_MGMT_WAPI_CERT;
#endif
...
/* MTK, Infinity, 20090716 { */
static u8 *wpa_driver_wext_driver_get_hwaddr(struct wpa_driver_wext_data *drv,
    struct ifreq *ifr)

{
    ...
    if (ioctl(drv->ioctl_sock, SIOCGIFHWADDR, (caddr_t)ifr) < 0) {
        wpa_printf(MSG_DEBUG, "ioctl[SIOCGIFHWADDR]");
        return NULL;
    }
    return (u8*)ifr->ifr_hwaddr.sa_data;
}

static s32 wpa_driver_wext_driver_get_linkspeed(struct wpa_driver_wext_data *drv)

{
    ...
    if (ioctl(drv->ioctl_sock, SIOCGIWRATE, &iwr) < 0) {
        wpa_printf(MSG_DEBUG, "ioctl[SIOCGIWRATE]");
        return 0;
    }
    return iwr.u.bitrate.value; /* bps */
}

static s32 wpa_driver_wext_driver_get_rssi(struct wpa_driver_wext_data *drv)
{
    struct iwreq iwr;
    ...
    if (ioctl(drv->ioctl_sock, SIOCGIWSTATS, &iwr) < 0) {
        wpa_printf(MSG_DEBUG, "ioctl[SIOCGIWSTATS]");
        return 0;
    }
    return (s32)(stats.qual.level - 0x100);
}
...
static int wpa_driver_wext_driver_set_power(struct wpa_driver_wext_data *drv, u32 mode)
{
    struct iwreq iwr;
    ...
    wpa_printf(MSG_DEBUG,"Set Power Mode command = %u", mode);

    /* Upper layer, 0: AUTO, 1:ACTIVE, 2:SHORT DOZE, 3:LONG DOZE */
    iwr.u.power.disabled = 0;
    switch(mode) {
    case 0: /* AUTO */
    case 2: /* SHORT DOZE */
        iwr.u.power.value = Param_PowerModeFast_PSP; /* fast power save profile */
        break;
    case 1: /* ACTIVE */
        iwr.u.power.value = Param_PowerModeCAM;      /* continuous awake mode */
        break;
    case 3: /* LONG DOZE */
        iwr.u.power.value = Param_PowerModeMAX_PSP;  /* always power save mode */
        break;
    }

    if (ioctl(drv->ioctl_sock, SIOCSIWPOWER, &iwr) < 0) {
        wpa_printf(MSG_DEBUG, "ioctl[SIOCGIWRTS]");
        return -1;
    }
    return 0;
}

static int wpa_driver_wext_driver_get_power(struct wpa_driver_wext_data *drv, u32 *mode)
{
    struct iwreq iwr;
    ...
    if (ioctl(drv->ioctl_sock, SIOCGIWPOWER, &iwr) < 0) {
        wpa_printf(MSG_DEBUG, "ioctl[SIOCGIWRTS]");
        return -1;
    }
    /* Upper layer, 0: AUTO, 1:ACTIVE, 2:SHORT DOZE, 3:LONG DOZE */
    switch(iwr.u.power.value) {
    default:
    ...
    wpa_printf(MSG_DEBUG,"Get Power Mode command = %u", *mode);
    return 0;
}

static int wpa_driver_wext_driver_get_rts(struct wpa_driver_wext_data *drv, u32 *thd)
{
    struct iwreq iwr;
    ...
    *thd = (u32)iwr.u.rts.value;
    return 0;
}

static int wpa_driver_wext_driver_set_rts(struct wpa_driver_wext_data *drv, u32 thd)
{
    struct iwreq iwr;
    ...
    if (!thd)
        ...
        iwr.u.rts.disabled = (thd > 2347) ? 1 : 0;
...
static int wpa_driver_wext_driver_start(struct wpa_driver_wext_data *drv, int stop)
{
    struct iwreq iwr;
    ...
    iwr.u.txpower.disabled = stop;

    if (ioctl(drv->ioctl_sock, SIOCSIWTXPOW, &iwr) < 0) {
        wpa_printf(MSG_DEBUG, "ioctl[SIOCSIWTXPOW]");
        return -1;
    }
    return 0;
}
...
#if 1
static int wpa_driver_wext_init_iface(void *priv)
{
    ...
    ret = wpa_driver_wext_set_mode(priv, 0);
    if (ret < 0) {
        wpa_printf(MSG_DEBUG, "wpa_driver_wext_set_mode");
        goto exit;
    }
//removed by mtk80743, it's not needed any more.
#if 0
    ret = wpa_driver_wext_set_ssid(priv, ssid, strlen(ssid));
    if (ret < 0) {
        wpa_printf(MSG_DEBUG, "wpa_driver_wext_set_ssid");
        goto exit;
    }
#endif
...
#define WIFI_POWER_STATE_PATH "/sys/class/rfkill/rfkill1/state"
static int wpa_driver_wext_set_power(void *priv, int on)
{
    struct wpa_driver_wext_data *drv = (struct wpa_driver_wext_data *)priv;
    int sz;
    int fd = -1;
    ...
    if (fd < 0) {
        wpa_printf(MSG_INFO, "open(%s) for write failed: %s (%d)",
            WIFI_POWER_STATE_PATH, strerror(errno), errno);
        goto out;
    }
    sz = write(fd, &buffer, 1);
    if (sz < 0) {
        wpa_printf(MSG_INFO, "write(%s) failed: %s (%d)",
            WIFI_POWER_STATE_PATH, strerror(errno), errno);
        goto out;
    }
    if (on) {
        ...
        while (count-- > 0) {
            if (ifc_init() == 0) {
                if (ifc_up(drv->ifname) == 0) {
                    ret = wpa_driver_wext_init_iface(priv);
                    if (ret == 0) {
                        wpa_printf(MSG_INFO, "wpa_driver_wext_init_iface ok\n");
                        ifc_close();
                        break;
                    }
                    wpa_printf(MSG_INFO, "wpa_driver_wext_init_iface failed\n");
                }
                wpa_printf(MSG_INFO, "ifc_up(wlan0) failed\n");
                ifc_close();
            } else {
                wpa_printf(MSG_INFO, "ifc_init() failed\n");
            }
            usleep(100000);
        }
    } else {
...
#if 1 /* For MT6620 at MT6573 SMT CTIA Test code */
static int wpa_driver_wext_driver_sw_cmd(void *priv,
    int set, unsigned long *adr, unsigned long *dat)
{
    struct wpa_driver_wext_data *drv = priv;
    int ret = 0;
    u16 ioctl_dir = SIOCIWFIRSTPRIV + 8;
    struct iwreq iwr;
    ...
    iwr.u.data.pointer = (caddr_t)&cmd_in_tmp;
    iwr.u.data.length = sizeof(SW_CMD_TRANSPORT_STRUCT);

    if (!set)
        ioctl_dir = SIOCIWFIRSTPRIV + 9;
    if (set){
        if(ioctl(drv->ioctl_sock, ioctl_dir, &iwr) < 0) {
            wpa_printf(MSG_DEBUG, "ioctl[SIOCIWFIRSTPRIV + 8] fail");
            ret = -1;
        }
    }
    else {
        if (ioctl(drv->ioctl_sock, ioctl_dir, &iwr) < 0) {
            wpa_printf(MSG_DEBUG, "ioctl[SIOCIWFIRSTPRIV + 9] fail");
            ret = -1;
        }
        else {
...
static int wpa_driver_signal_poll(void *priv, struct wpa_signal_info *si)
{
	...
	rssi = wpa_driver_wext_driver_get_rssi(drv);
	if (rssi) {
		len = wpa_driver_wext_get_ssid(drv, ssid);
		if ((len > 0) && (len <= 32)) {
            si->current_signal = rssi;
            mbps = wpa_driver_wext_driver_get_linkspeed(drv) / 1000;
			si->current_txrate = mbps;
			si->frequency = 0;
...
#if 0
static int wpa_driver_wext_driver_set_int(struct wpa_driver_wext_data *drv, u32 mode, u32 value)
{
    struct iwreq iwr;
    u32 *cp;
    ...
    if (ioctl(drv->ioctl_sock, SIOCIWFIRSTPRIV, &iwr) < 0) {
        wpa_printf(MSG_INFO, "ioctl[SIOCIWFIRSTPRIV]");
        return -1;
    }
    return 0;
}
...
static int wpa_driver_wext_driver_get_int(struct wpa_driver_wext_data *drv, u32 mode, u32 *value)
{
    struct iwreq iwr;
    u32 *cp;
    ...
    if (ioctl(drv->ioctl_sock, SIOCIWFIRSTPRIV + 1, &iwr) < 0) {
        wpa_printf(MSG_DEBUG, "ioctl[SIOCIWFIRSTPRIV + 1]");
        return -1;
    }
...
static int wpa_driver_wext_driver_get_ints(struct wpa_driver_wext_data *drv, u32 mode, u32 *value)
{
    struct iwreq iwr;
	u8 buffer[256];
    u32 *cp;
    ...
    iwr.u.data.flags = mode;
	iwr.u.data.pointer = (caddr_t)buffer;

    if (ioctl(drv->ioctl_sock, SIOCIWFIRSTPRIV + 13, &iwr) < 0) {
        wpa_printf(MSG_DEBUG, "ioctl[SIOCIWFIRSTPRIV + 13]");
        return -1;
    }

    cp = iwr.u.data.pointer;

	wpa_printf(MSG_INFO, "%d %d %d %d %d\n", iwr.u.data.length, cp[0], cp[1], cp[2], cp[3] );

    return 0;
}

static int wpa_driver_wext_driver_set_ints(struct wpa_driver_wext_data *drv, u32 mode, s32 *value, s32 count)
{
    struct iwreq iwr;
    u32 *cp;
    ...
    if (count == 1)  {
	    ...
	    iwr.u.data.flags = mode;
	    iwr.u.data.length = count * sizeof(s32);
	    iwr.u.data.pointer = (caddr_t) value;

	    if (ioctl(drv->ioctl_sock, SIOCIWFIRSTPRIV + 12, &iwr) < 0) {
	        wpa_printf(MSG_INFO, "ioctl[SIOCIWFIRSTPRIV + 12]");
	        return -1;
	    }
	}
...
static int wpa_driver_wext_driver_cmd(void *priv,
    char *cmd, char *buf, size_t buf_len)
{
    ...
    if (os_strcasecmp(cmd, "start") == 0) {
		if (linux_set_iface_flags(drv->ioctl_sock, drv->ifname, 1)) {
			wpa_printf(MSG_INFO, "WEXT: Could not set interface UP \n");
		}
		else {
			int flags;
			wpa_driver_wext_get_ifflags(drv, &flags);
			wpa_printf(MSG_DEBUG, "WEXT: interface=%x\n", flags);
		}
        ret = wpa_driver_wext_driver_start(drv, 0);
		ret = wpa_driver_wext_init_iface(drv);
        //ret = wpa_driver_wext_driver_set_power(drv, 0);
        //ret = wpa_driver_wext_set_power(priv, 1);
        if (ret == 0)
        ...
        //ret = wpa_driver_wext_driver_set_power(drv, 3);
        //ret = wpa_driver_wext_set_power(priv, 0);
        if (ret == 0)
		...
		if (linux_set_iface_flags(drv->ioctl_sock, drv->ifname, 0)) {
			wpa_printf(MSG_INFO, "WEXT: Could not set interface Down \n");
		}
		else {
			int flags;
			wpa_driver_wext_get_ifflags(drv, &flags);
        ...
        struct ifreq ifr;
        u8 *macaddr = (u8 *)wpa_driver_wext_driver_get_hwaddr(drv, &ifr);
        ...
        s32 mbps = wpa_driver_wext_driver_get_linkspeed(drv) / 1000000;
        ...
        if (rssi) {
            len = wpa_driver_wext_get_ssid(drv, ssid);
            if( (len > 0) && (len <= 32) && (len < (int)buf_len)) {
        ...
        if (*cp != '\0') {
            band = (u32)strtol(cp, &endp, 0);
            if (endp != cp) {
                ret = 0;
                if (band <= 2)
                    ret = wpa_driver_wext_driver_set_ints(drv, PRIV_CMD_BAND_CONFIG, &band, 1);
            }
        }
    } else if (os_strncasecmp(cmd, "GETBAND", 7) == 0 ) {
        u32 band;
        ret = wpa_driver_wext_driver_get_int(drv, PRIV_CMD_BAND_CONFIG, &band);
        if (ret == 0) {
            ret = snprintf(buf, buf_len, "Band = %d\n", band);
            wpa_printf(MSG_DEBUG, "%s", buf);
            if (ret < (int)buf_len) {
                return( ret );
            }
        ...
        if (*cp != '\0') {
            ...
            if (endp != cp) {
                wpa_driver_wext_driver_sw_cmd(drv, 1, &sw_cmd, &idx);
                ret = 0;
            }
        }
...
int wpa_driver_wext_send_msg(void *priv, const u8 *msg_in, int msg_in_len,
							 u8 *msg_out, int *msg_out_len)
{
	struct wpa_supplicant *pwpa_s = priv;
	struct wpa_driver_wext_data *drv = pwpa_s->drv_priv;
	struct iwreq iwr;
	char msg_in_tmp[1024];
	int ret = 0;
	...
	if(msg_in_len > 1024){
		wpa_printf(MSG_DEBUG, "wpa_driver_wext_send_msg: msg too long");
		return -1;
	}
	os_memcpy(msg_in_tmp, msg_in, msg_in_len);

	iwr.u.data.pointer = (caddr_t)&msg_in_tmp;
	iwr.u.data.length = msg_in_len;
	iwr.u.data.flags = 16;
	wpa_printf(MSG_DEBUG, "%s(%s) len = %d", __func__, drv->ifname, msg_in_len);
	if (ioctl(drv->ioctl_sock, (SIOCIWFIRSTPRIV + 8), &iwr) < 0) {
		wpa_printf(MSG_DEBUG, "ioctl[SIOCIWFIRSTPRIV + 8]");
		ret = -1;
	}
	else {
		wpa_printf(MSG_DEBUG, "%s message back!\n", __func__);
		...
		/*
			Take the first two byte for the length of use
		*/
		*msg_out_len = *((unsigned short *)iwr.u.data.pointer);
		wpa_printf(MSG_DEBUG, "%s message back! s len = %d\n", __func__, *msg_out_len);
		if(*msg_out_len > 1022){
			wpa_printf(MSG_DEBUG, "wpa_driver_wext_send_msg: msg out too long");
			return -1;
		}
		os_memcpy(msg_out, (char *)iwr.u.data.pointer + 2, *msg_out_len);
		buffer_hexdump(MSG_DEBUG, "wpa_driver_wext_send_msg out:", (const u8 *)iwr.u.data.pointer, 32);
	}
	return ret;
}
#endif
...
static int wpa_driver_priv_driver_cmd( void *priv, char *cmd, char *buf, size_t buf_len )
{
	...
	if ((ret = ioctl(drv->ioctl_sock, SIOCSIWPRIV, &iwr)) < 0) {
		wpa_printf(MSG_DEBUG, "ioctl[SIOCSIWPRIV]");
	}
 ...
 * Returns: 0 on success, -1 on failure
 */
int wpa_driver_wext_set_probe_req_ie(void *priv, const u8 *ies, size_t ies_len)
{
	struct wpa_driver_wext_data *drv = priv;
	struct iwreq iwr;
	int ret = 0;
	...
	os_memset(&iwr, 0, sizeof(iwr));
	os_strlcpy(iwr.ifr_name, drv->ifname, IFNAMSIZ);
	iwr.u.data.pointer = (caddr_t) ies;
	iwr.u.data.length = ies_len;
	iwr.u.data.flags = 22;
	wpa_printf(MSG_DEBUG, "%s(%s) ies_len = %d", __func__, drv->ifname, ies_len);
	if (ioctl(drv->ioctl_sock, SIOCIWFIRSTPRIV + 8, &iwr) < 0) {
		perror("ioctl[SIOCIWFIRSTPRIV + 8]");
		ret = -1;
	}
...
// MTK, whsu, 20110112, MT6620 not support yet
...
/* MTK, whsu, 20110125 */
    .driver_sw_cmd = wpa_driver_wext_driver_sw_cmd,
/* MTK, Infinity, 20090716 } */
#ifdef CONFIG_WAPI_SUPPORT
	.msg_send = wpa_driver_wext_send_msg
#endif
#endif
};
