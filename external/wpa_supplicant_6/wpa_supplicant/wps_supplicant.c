...
{
	wpa_msg(wpa_s, MSG_INFO, WPS_EVENT_FAIL "msg=%d config_error=%d", fail->msg, fail->config_error);
	wpas_clear_wps(wpa_s);
}
...
static void wpas_wps_timeout(void *eloop_ctx, void *timeout_ctx)
{
	...
	wpa_msg_ctrl(wpa_s, MSG_INFO, WPS_EVENT_TIMEOUT "Requested operation timed "
		   "out");
	wpas_clear_wps(wpa_s);
