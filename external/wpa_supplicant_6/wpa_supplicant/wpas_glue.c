...
void wpa_supplicant_set_state_helper(void *ctx, int state)
{
    ...
    wpa_supplicant_set_state(wpa_s,(wpa_states) state);
}
...
static int wpa_supplicant_set_key_helper(void *_wpa_s, int alg,
				  ...
				  const u8 *key, size_t key_len)
{
    return wpa_supplicant_set_key(_wpa_s,(wpa_alg)alg, addr, key_idx, set_tx, seq, seq_len, key, key_len);
}
...
int wpa_supplicant_init_wapi(struct wpa_supplicant *wpa_s)
{
    ...
    ctx->ctx = wpa_s;
    ctx->wapi_printf = android_printf;
    ctx->msg_send = wpa_s->driver->msg_send;
    ctx->wpa_msg = wpa_msg;
    ctx->get_state = wpa_supplicant_get_state_helper;
    /*
    ctx->deauthenticate = wpa_supplicant_deauthenticate;
    */
    ctx->deauthenticate = wpa_supplicant_disassociate_helper;
