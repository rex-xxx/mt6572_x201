...
/// M: @{
    ...
    private static final String TAG="mp4parser-Mp4TrackImpl";
/// @}
...
/// M: add error handle null pointer@{
	if (stbl == null) {
              ready = false;
              Log.w(TAG, "stbl is null");
              return;
	}
        TimeToSampleBox timeBox = stbl.getTimeToSampleBox();
	if (timeBox == null) {
             Log.w(TAG, "timeBox is null");
             ready = false;
             return;
	}
        decodingTimeEntries.addAll(timeBox.getEntries());
/// @}
...
/// M:  check Mp4Track is ready or not @{
