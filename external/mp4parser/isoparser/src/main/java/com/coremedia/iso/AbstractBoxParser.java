...
/// M: check contentSize @{
	if (contentSize < 0) {
	    Log.w("mp4parser-AbstractBoxParser", "contenSize:" + contentSize + " < 0"); 
	    Log.w("mp4parser-AbstractBoxParser", "parsing " + box.getType() + " " + box.getClass().getName() + " size=" + size);
           return null;
	}
/// M: @}
...
/// M: check size @{
	if (size != box.getSize()) {
	   Log.w("mp4parser-AbstractBoxParser", "Reconstructed Size is not x to the number of parsed bytes! (" +
		box.getType() + ")"
		+ " Actual Box size: " + size + " Calculated size: " + box.getSize());
           return null;
	}
