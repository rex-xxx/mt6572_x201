...
public:
	...
	CSSStyleRule* rule() const {
		return m_rule;
	}
	CSSSelector* selector() const {
		return m_selector;
	}

	bool hasFastCheckableSelector() const {
		return m_hasFastCheckableSelector;
	}
	bool hasMultipartSelector() const {
		return m_hasMultipartSelector;
	}
	bool hasTopSelectorMatchingHTMLBasedOnRuleHash() const {
		return m_hasTopSelectorMatchingHTMLBasedOnRuleHash;
	}
...
public:
	...
	void addRulesFromSheet(CSSStyleSheet*, const MediaQueryEvaluator&,
			CSSStyleSelector* = 0);
	...
	void disableAutoShrinkToFit() {
		m_autoShrinkToFitEnabled = false;
	}

	void collectFeatures(CSSStyleSelector::Features&) const;

	const Vector<RuleData>* getIDRules(AtomicStringImpl* key) const {
		return m_idRules.get(key);
	}
	const Vector<RuleData>* getClassRules(AtomicStringImpl* key) const {
		return m_classRules.get(key);
	}
	const Vector<RuleData>* getTagRules(AtomicStringImpl* key) const {
		return m_tagRules.get(key);
	}
	const Vector<RuleData>* getPseudoRules(AtomicStringImpl* key) const {
		return m_pseudoRules.get(key);
	}
	const Vector<RuleData>* getUniversalRules() const {
		return &m_universalRules;
	}
	const Vector<RuleData>* getPageRules() const {
		return &m_pageRules;
	}
...
static const char* simpleUserAgentStyleSheet =
...
static inline bool elementCanUseSimpleDefaultStyle(Element* e) {
	return e->hasTagName(htmlTag) || e->hasTagName(headTag)
			|| e->hasTagName(bodyTag) || e->hasTagName(divTag)
			|| e->hasTagName(spanTag) || e->hasTagName(brTag)
			|| e->hasTagName(aTag);
}
...
static CSSMutableStyleDeclaration* leftToRightDeclaration() {
	DEFINE_STATIC_LOCAL(RefPtr<CSSMutableStyleDeclaration>, leftToRightDecl,
...
static CSSMutableStyleDeclaration* rightToLeftDeclaration() {
	DEFINE_STATIC_LOCAL(RefPtr<CSSMutableStyleDeclaration>, rightToLeftDecl,
			(CSSMutableStyleDeclaration::create()));
...
CSSStyleSelector::CSSStyleSelector(Document* document,
		StyleSheetList* styleSheets, CSSStyleSheet* mappedElementSheet,
		CSSStyleSheet* pageUserSheet,
		const Vector<RefPtr<CSSStyleSheet> >* pageGroupUserSheets,
		bool strictParsing, bool matchAuthorAndUserStyles) :
		m_backgroundData(BackgroundFillLayer), m_checker(document,
				strictParsing), m_element(0), m_styledElement(0), m_elementLinkState(
				NotInsideLink), m_fontSelector(
				CSSFontSelector::create(document)), m_applyProperty(
				CSSStyleApplyProperty::sharedCSSStyleApplyProperty()) {
	...
	if (m_rootDefaultStyle && view)
		m_medium = adoptPtr(
				new MediaQueryEvaluator(view->mediaType(), view->frame(),
						m_rootDefaultStyle.get()));
	...
	if (pageGroupUserSheets) {
		...
		for (unsigned i = 0; i < length; i++) {
			if (pageGroupUserSheets->at(i)->isUserStyleSheet())
				tempUserStyle->addRulesFromSheet(
						pageGroupUserSheets->at(i).get(), *m_medium, this);
			else
	...
	for (unsigned i = 0; i < length; i++) {
		...
		if (sheet->isCSSStyleSheet() && !sheet->disabled())
			m_authorStyle->addRulesFromSheet(static_cast<CSSStyleSheet*>(sheet),
					*m_medium, this);
	}
...
void CSSStyleSelector::addKeyframeStyle(
		PassRefPtr<WebKitCSSKeyframesRule> rule) {
...
CSSStyleSelector::Features::Features() :
		usesFirstLineRules(false), usesBeforeAfterRules(false), usesLinkRules(
				false) {
}
...
static void loadFullDefaultStyle() {
	...
	// Strict-mode rules.
	String defaultRules = String(htmlUserAgentStyleSheet,
			sizeof(htmlUserAgentStyleSheet))
...
static void loadSimpleDefaultStyle() {
	...
	simpleDefaultStyleSheet = parseUASheet(simpleUserAgentStyleSheet,
			strlen(simpleUserAgentStyleSheet));
...
static void loadViewSourceStyle() {
	...
	defaultViewSourceStyle->addRulesFromSheet(
			parseUASheet(sourceUserAgentStyleSheet,
					sizeof(sourceUserAgentStyleSheet)), screenEval());
}

static inline void collectElementIdentifierHashes(const Element* element,
	...
	const StyledElement* styledElement =
			element->isStyledElement() ?
					static_cast<const StyledElement*>(element) : 0;
...
void CSSStyleSelector::pushParentStackFrame(Element* parent) {
	...
	ASSERT(
			m_parentStack.isEmpty()
					|| m_parentStack.last().element == parent->parentElement());
...
void CSSStyleSelector::matchRules(RuleSet* rules, int& firstRuleIndex,
	...
	if (m_element->hasID())
		matchRulesForList(
				rules->getIDRules(m_element->idForStyleResolution().impl()),
	...
	if (m_element->hasClass()) {
		...
		for (size_t i = 0; i < size; ++i)
			matchRulesForList(rules->getClassRules(classNames[i].impl()),
					firstRuleIndex, lastRuleIndex, includeEmptyRules);
	}
	if (!m_element->shadowPseudoId().isEmpty()) {
		ASSERT(m_styledElement);
		matchRulesForList(
				rules->getPseudoRules(m_element->shadowPseudoId().impl()),
				firstRuleIndex, lastRuleIndex, includeEmptyRules);
	}
	matchRulesForList(rules->getTagRules(m_element->localName().impl()),
			firstRuleIndex, lastRuleIndex, includeEmptyRules);
...
inline bool CSSStyleSelector::fastRejectSelector(
		const RuleData& ruleData) const {
	ASSERT(m_ancestorIdentifierFilter);
	const unsigned* descendantSelectorIdentifierHashes =
			ruleData.descendantSelectorIdentifierHashes();
	for (unsigned n = 0;
			n < RuleData::maximumIdentifierCount
					&& descendantSelectorIdentifierHashes[n]; ++n) {
		if (!m_ancestorIdentifierFilter->mayContain(
				descendantSelectorIdentifierHashes[n]))
			return true;
	}
	return false;
}

void CSSStyleSelector::matchRulesForList(const Vector<RuleData>* rules,
		int& firstRuleIndex, int& lastRuleIndex, bool includeEmptyRules) {
	if (!rules)
		return;
	...
	bool canUseFastReject = !m_parentStack.isEmpty()
			&& m_parentStack.last().element == m_parentNode;
	...
	for (unsigned i = 0; i < size; ++i) {
		...
		if (checkSelector(ruleData)) {
			...
			if (!decl || (!decl->length() && !includeEmptyRules))
				continue;
			if (m_checker.m_sameOriginOnly
					&& !m_checker.m_document->securityOrigin()->canRequest(
							rule->baseURL()))
				continue;
			// If we're matching normal rules, set a pseudo bit if
			// we really just matched a pseudo-element.
			if (m_dynamicPseudo != NOPSEUDO
					&& m_checker.m_pseudoStyle == NOPSEUDO) {
				if (m_checker.m_collectRulesOnly)
					continue;
...
static inline bool compareRules(const RuleData* r1, const RuleData* r2) {
	...
	return (specificity1 == specificity2) ?
			r1->position() < r2->position() : specificity1 < specificity2;
}
...
inline EInsideLink CSSStyleSelector::SelectorChecker::determineLinkState(
...
inline void CSSStyleSelector::initElement(Element* e) {
	if (m_element != e) {
		m_element = e;
		m_styledElement =
				m_element && m_element->isStyledElement() ?
						static_cast<StyledElement*>(m_element) : 0;
...
inline void CSSStyleSelector::initForStyleResolve(Element* e,
		RenderStyle* parentStyle, PseudoId pseudoID) {
	...
	m_rootElementStyle =
			docElement && e != docElement ?
					docElement->renderStyle() : docStyle;
...
CSSStyleSelector::SelectorChecker::SelectorChecker(Document* document,
		bool strictParsing) :
		m_document(document), m_strictParsing(strictParsing), m_collectRulesOnly(
				false), m_sameOriginOnly(false), m_pseudoStyle(NOPSEUDO), m_documentIsHTML(
				document->isHTMLDocument()), m_matchVisitedPseudoClass(false) {
}

EInsideLink CSSStyleSelector::SelectorChecker::determineLinkStateSlowCase(
		Element* element) const {
	...
	return page->group().isLinkVisited(hash) ?
			InsideVisitedLink : InsideUnvisitedLink;
#endif
}

bool CSSStyleSelector::SelectorChecker::checkSelector(CSSSelector* sel,
		Element* element) const {
...
Node* CSSStyleSelector::locateCousinList(Element* parent,
		unsigned& visitedNodeCount) const {
	...
	if (p->inlineStyleDecl())
		return 0;
	if (p->hasID()
			&& m_features.idsInRules.contains(p->idForStyleResolution().impl()))
		return 0;
	...
	while (thisCousin) {
		...
		currentNode = locateCousinList(thisCousin->parentElement(),
				visitedNodeCount);
...
bool CSSStyleSelector::matchesSiblingRules() {
	...
	matchRules(m_features.siblingRules.get(), firstSiblingRule, lastSiblingRule,
			false);
...
bool CSSStyleSelector::canShareStyleWithElement(Node* node) const {
	...
	if (element->hasMappedAttributes()
			!= m_styledElement->hasMappedAttributes())
		return false;
	...
	if (m_element == m_element->document()->cssTarget())
		return false;
	if (element->fastGetAttribute(typeAttr)
			!= m_element->fastGetAttribute(typeAttr))
		return false;
	...
	if (element->fastGetAttribute(langAttr)
			!= m_element->fastGetAttribute(langAttr))
		return false;
	if (element->fastGetAttribute(readonlyAttr)
			!= m_element->fastGetAttribute(readonlyAttr))
		return false;
	if (element->fastGetAttribute(cellpaddingAttr)
			!= m_element->fastGetAttribute(cellpaddingAttr))
		return false;

	if (element->hasID()
			&& m_features.idsInRules.contains(
					element->idForStyleResolution().impl()))
		return false;
	...
	if (isControl) {
		...
		if (thisInputElement->isAutofilled()
				!= otherInputElement->isAutofilled())
			return false;
		if (thisInputElement->isChecked() != otherInputElement->isChecked())
			return false;
		if (thisInputElement->isIndeterminate()
				!= otherInputElement->isIndeterminate())
			return false;

		if (element->isEnabledFormControl()
				!= m_element->isEnabledFormControl())
			return false;

		if (element->isDefaultButtonForForm()
				!= m_element->isDefaultButtonForForm())
			return false;
		...
		if (willValidate
				&& (element->isValidFormControlElement()
						!= m_element->isValidFormControlElement()))
			return false;
	...
	if (equalIgnoringCase(element->fastGetAttribute(dirAttr), "auto")
			|| equalIgnoringCase(m_element->fastGetAttribute(dirAttr), "auto"))
		return false;

	if (element->hasClass()
			&& m_element->fastGetAttribute(classAttr)
					!= element->fastGetAttribute(classAttr))
		return false;

	if (element->hasMappedAttributes()
			&& !element->attributeMap()->mappedMapsEquivalent(
					m_styledElement->attributeMap()))
		return false;
...
inline Node* CSSStyleSelector::findSiblingForStyleSharing(Node* node,
...
ALWAYS_INLINE RenderStyle* CSSStyleSelector::locateSharedStyle() {
	...
	if (m_styledElement->hasID()
			&& m_features.idsInRules.contains(
					m_styledElement->idForStyleResolution().impl()))
		return 0;
...
void CSSStyleSelector::matchUARules(int& firstUARule, int& lastUARule) {
	...
	RuleSet* userAgentStyleSheet =
			m_medium->mediaTypeMatchSpecific("print") ?
...
PassRefPtr<RenderStyle> CSSStyleSelector::styleForDocument(Document* document) {
	...
	documentStyle->setUserModify(
			document->inDesignMode() ? READ_WRITE : READ_ONLY);
	...
	if (Settings* settings = document->settings()) {
		...
		int size = CSSStyleSelector::fontSizeForKeyword(document,
				CSSValueMedium, false);
		fontDescription.setSpecifiedSize(size);
		bool useSVGZoomRules = document->isSVGDocument();
		fontDescription.setComputedSize(
				CSSStyleSelector::getComputedSizeFromSpecifiedSize(document,
						documentStyle.get(), fontDescription.isAbsoluteSize(),
						size, useSVGZoomRules));
	}
...
PassRefPtr<RenderStyle> CSSStyleSelector::styleForElement(Element* e,
		RenderStyle* defaultParent, bool allowSharing,
		bool resolveForRootDefault, bool matchVisitedPseudoClass) {
	...
	if (!matchVisitedPseudoClass && m_parentStyle
			&& (m_parentStyle->insideLink() || e->isLink())
			&& e->document()->usesLinkRules()) {
		// Fetch our parent style.
		RenderStyle* parentStyle = m_parentStyle;
		if (!e->isLink()) {
			// Use the parent's visited style if one exists.
			RenderStyle* parentVisitedStyle =
					m_parentStyle->getCachedPseudoStyle(VISITED_LINK);
	...
	if (visitedStyle) {
		...
		// links.
		for (unsigned pseudo = FIRST_PUBLIC_PSEUDOID;
				pseudo < FIRST_INTERNAL_PSEUDOID; ++pseudo) {
	...
	if (!resolveForRootDefault) {
		...
		if (m_styledElement) {
			...
			if (m_styledElement->canHaveAdditionalAttributeStyleDecls()) {
				...
				m_styledElement->additionalAttributeStyleDecls(
						m_additionalAttributeStyleDecls);
				if (!m_additionalAttributeStyleDecls.isEmpty()) {
					unsigned additionalDeclsSize =
							m_additionalAttributeStyleDecls.size();
					if (firstAuthorRule == -1)
						firstAuthorRule = m_matchedDecls.size();
					lastAuthorRule = m_matchedDecls.size() + additionalDeclsSize
							- 1;
					for (unsigned i = 0; i < additionalDeclsSize; i++)
						addMatchedDeclaration(
								m_additionalAttributeStyleDecls[i]);
				}
			}
			if (m_styledElement->isHTMLElement()) {
				bool isAuto;
				TextDirection textDirection =
						toHTMLElement(m_styledElement)->directionalityIfhasDirAutoAttribute(
								isAuto);
				if (isAuto)
					addMatchedDeclaration(
							textDirection == LTR ?
		...
		if (m_matchAuthorAndUserStyles && m_styledElement) {
			CSSMutableStyleDeclaration* inlineDecl =
					m_styledElement->inlineStyleDecl();
			if (inlineDecl) {
				lastAuthorRule = m_matchedDecls.size();
				if (firstAuthorRule == -1)
					firstAuthorRule = lastAuthorRule;
	...
	if (!resolveForRootDefault) {
		applyDeclarations<false>(false, lastUARule + 1,
				m_matchedDecls.size() - 1);
...
PassRefPtr<RenderStyle> CSSStyleSelector::styleForKeyframe(
		const RenderStyle* elementStyle,
		const WebKitCSSKeyframeRule* keyframeRule, KeyframeValue& keyframe) {
	...
	if (keyframeRule->style()) {
		CSSMutableStyleDeclaration::const_iterator end =
				keyframeRule->style()->end();
		for (CSSMutableStyleDeclaration::const_iterator it =
				keyframeRule->style()->begin(); it != end; ++it) {
			int property = (*it).id();
...
void CSSStyleSelector::keyframeStylesForAnimation(Element* e,
		const RenderStyle* elementStyle, KeyframeList& list) {
	list.clear();
	...
	const WebKitCSSKeyframesRule* rule = m_keyframesRuleMap.find(
			list.animationName().impl()).get()->second.get();
	...
	for (unsigned i = 0; i < rule->length(); ++i) {
		...
		keyframe.setStyle(
				styleForKeyframe(elementStyle, keyframeRule, keyframe));
	...
	if (initialListSize > 0 && list[0].key() != 0) {
		RefPtr<WebKitCSSKeyframeRule> keyframeRule =
				WebKitCSSKeyframeRule::create();
		keyframeRule->setKeyText("0%");
		KeyframeValue keyframe(0, 0);
		keyframe.setStyle(
				styleForKeyframe(elementStyle, keyframeRule.get(), keyframe));
		list.insert(keyframe);
	}
...
PassRefPtr<RenderStyle> CSSStyleSelector::pseudoStyleForElement(PseudoId pseudo,
		Element* e, RenderStyle* parentStyle, bool matchVisitedPseudoClass) {
	if (!e)
		return 0;
	...
	if (!matchVisitedPseudoClass && parentStyle && parentStyle->insideLink()) {
		...
		RenderStyle* parentVisitedStyle = parentStyle->getCachedPseudoStyle(
				VISITED_LINK);
		visitedStyle = pseudoStyleForElement(pseudo, e,
				parentVisitedStyle ? parentVisitedStyle : parentStyle, true);
		if (visitedStyle)
			visitedStyle->setStyleType(VISITED_LINK);
	}
	...
	int firstUARule = -1, lastUARule = -1, firstUserRule = -1,
			lastUserRule = -1, firstAuthorRule = -1, lastAuthorRule = -1;
	matchUARules(firstUARule, lastUARule);
...
void CSSStyleSelector::adjustRenderStyle(RenderStyle* style,
		RenderStyle* parentStyle, Element *e) {
	...
	if (style->display() != NONE) {
		...
		if (!m_checker.m_strictParsing && e) {
			if (e->hasTagName(tdTag)) {
				...
				style->setDisplay(
						style->isDisplayInlineType() ? INLINE_TABLE : TABLE);
		}
		...
		if (e && e->hasTagName(tableTag)
				&& (style->textAlign() == WEBKIT_LEFT
		...
		if (style->display() == INLINE && style->styleType() == NOPSEUDO
				&& parentStyle
		...
		// on some sites).
		if ((style->display() == TABLE_HEADER_GROUP
		...
		if (style->display() == TABLE_COLUMN
				|| style->display() == TABLE_COLUMN_GROUP
				|| style->display() == TABLE_FOOTER_GROUP
				|| style->display() == TABLE_HEADER_GROUP
				|| style->display() == TABLE_ROW
				|| style->display() == TABLE_ROW_GROUP
	...
	if (e && e->hasTagName(textareaTag)) {
		style->setOverflowX(
				style->overflowX() == OVISIBLE ? OAUTO : style->overflowX());
		style->setOverflowY(
				style->overflowY() == OVISIBLE ? OAUTO : style->overflowY());
	}
	...
	if (style->display() == TABLE || style->display() == INLINE_TABLE
			...
			|| style->display() == INLINE_BOX || isAtShadowBoundary(e))
	...
	if (style->hasAppearance())
		RenderTheme::defaultTheme()->adjustStyle(this, style, e,
				m_hasUAAppearance, m_borderData, m_backgroundData,
				m_backgroundColor);
...
PassRefPtr<CSSRuleList> CSSStyleSelector::styleRulesForElement(Element* e,
		bool authorOnly, bool includeEmptyRules, CSSRuleFilter filter) {
	return pseudoStyleRulesForElement(e, NOPSEUDO, authorOnly,
			includeEmptyRules, filter);
}

PassRefPtr<CSSRuleList> CSSStyleSelector::pseudoStyleRulesForElement(Element* e,
		PseudoId pseudoId, bool authorOnly, bool includeEmptyRules,
		CSSRuleFilter filter) {
...
inline bool CSSStyleSelector::checkSelector(const RuleData& ruleData) {
	...
	if (ruleData.hasFastCheckableSelector() && !m_element->isSVGElement()) {
		...
		if (ruleData.hasTopSelectorMatchingHTMLBasedOnRuleHash()
				&& !ruleData.hasMultipartSelector()
				&& m_element->isHTMLElement())
			return true;
		return SelectorChecker::fastCheckSelector(ruleData.selector(),
				m_element);
	}

	// Slow path.
	SelectorMatch match = m_checker.checkSelector(ruleData.selector(),
			m_element, &m_selectorAttrs, m_dynamicPseudo, false, false, style(),
			m_parentNode ? m_parentNode->renderStyle() : 0);
	if (match != SelectorMatches)
		return false;
	if (m_checker.m_pseudoStyle != NOPSEUDO
			&& m_checker.m_pseudoStyle != m_dynamicPseudo)
		return false;
	return true;
}

static inline bool selectorTagMatches(const Element* element,
...
static inline bool isFastCheckableSelector(const CSSSelector* selector) {
	for (; selector; selector = selector->tagHistory()) {
		if (selector->relation() != CSSSelector::Descendant
				...
				&& selector->relation() != CSSSelector::SubSelector)
			return false;
...
inline bool fastCheckSingleSelector(const CSSSelector*& selector,
		const Element*& element, const CSSSelector*& topChildOrSubselector,
		const Element*& topChildOrSubselectorMatchElement) {
	AtomicStringImpl* value = selector->value().impl();
	for (; element; element = element->parentElement()) {
		if (ValueChecker::checkValue(element, value)
				&& selectorTagMatches(element, selector)) {
			if (selector->relation() == CSSSelector::Descendant)
...
struct ClassCheck {
	static bool checkValue(const Element* element, AtomicStringImpl* value) {
		return element->hasClass()
				&& static_cast<const StyledElement*>(element)->classNames().contains(
						value);
	}
};
struct IdCheck {
	static bool checkValue(const Element* element, AtomicStringImpl* value) {
		return element->hasID()
				&& element->idForStyleResolution().impl() == value;
	}
};
struct TagCheck {
...
bool CSSStyleSelector::SelectorChecker::fastCheckSelector(
		const CSSSelector* selector, const Element* element) {
	...
	if (selector->relation() == CSSSelector::Child
			|| selector->relation() == CSSSelector::SubSelector)
		topChildOrSubselector = selector;
	...
	while (selector) {
		...
		case CSSSelector::Class:
			if (!fastCheckSingleSelector<ClassCheck>(selector, element,
					topChildOrSubselector, topChildOrSubselectorMatchElement))
				return false;
			break;
		case CSSSelector::Id:
			if (!fastCheckSingleSelector<IdCheck>(selector, element,
					topChildOrSubselector, topChildOrSubselectorMatchElement))
				return false;
			break;
		case CSSSelector::None:
			if (!fastCheckSingleSelector<TagCheck>(selector, element,
					topChildOrSubselector, topChildOrSubselectorMatchElement))
				return false;
			break;
		default:
...
CSSStyleSelector::SelectorMatch CSSStyleSelector::SelectorChecker::checkSelector(
		CSSSelector* sel, Element* e, HashSet<AtomicStringImpl*>* selectorAttrs,
		PseudoId& dynamicPseudo, bool isSubSelector, bool encounteredLink,
	...
	if (!checkOneSelector(sel, e, selectorAttrs, dynamicPseudo, isSubSelector,
			encounteredLink, elementStyle, elementParentStyle))
	...
	if (m_matchVisitedPseudoClass && !isSubSelector) {
		RenderStyle* currentStyle =
				elementStyle ? elementStyle : e->renderStyle();
	...
	case CSSSelector::Descendant:
		while (true) {
			...
			SelectorMatch match = checkSelector(sel, e, selectorAttrs,
					dynamicPseudo, false, encounteredLink);
	...
	case CSSSelector::Child: {
		...
		if (!n || !n->isElementNode())
			return SelectorFailsCompletely;
		e = static_cast<Element*>(n);
		return checkSelector(sel, e, selectorAttrs, dynamicPseudo, false,
				encounteredLink);
	}
	case CSSSelector::DirectAdjacent: {
		if (!m_collectRulesOnly && e->parentNode()
				&& e->parentNode()->isElementNode()) {
	...
	case CSSSelector::SubSelector:
		...
		if ((elementStyle || m_collectRulesOnly) && dynamicPseudo != NOPSEUDO
			...
			return SelectorFailsCompletely;
		return checkSelector(sel, e, selectorAttrs, dynamicPseudo, true,
				encounteredLink, elementStyle, elementParentStyle);
...
static void addLocalNameToSet(HashSet<AtomicStringImpl*>* set,
...
static bool htmlAttributeHasCaseInsensitiveValue(const QualifiedName& attr) {
	static HashSet<AtomicStringImpl*>* htmlCaseInsensitiveAttributesSet =
			createHtmlCaseInsensitiveAttributesSet();
	bool isPossibleHTMLAttr = !attr.hasPrefix()
			&& (attr.namespaceURI() == nullAtom);
	return isPossibleHTMLAttr
			&& htmlCaseInsensitiveAttributesSet->contains(
					attr.localName().impl());
}

bool CSSStyleSelector::SelectorChecker::checkOneSelector(CSSSelector* sel,
		Element* e, HashSet<AtomicStringImpl*>* selectorAttrs,
		PseudoId& dynamicPseudo, bool isSubSelector, bool encounteredLink,
		RenderStyle* elementStyle, RenderStyle* elementParentStyle) const {
	ASSERT(e);
	if (!e)
	...
	if (sel->hasAttribute()) {
		...
		if (elementStyle
				&& (!e->isStyledElement()
						|| (!static_cast<StyledElement*>(e)->isMappedAttribute(
								attr) && attr != typeAttr
								&& attr != readonlyAttr))) {
		...
		bool caseSensitive = !m_documentIsHTML
				|| !htmlAttributeHasCaseInsensitiveValue(attr);

		switch (sel->m_match) {
		case CSSSelector::Exact:
			if (caseSensitive ?
					sel->value() != value :
					!equalIgnoringCase(sel->value(), value))
				return false;
			break;
		...
		case CSSSelector::Contain:
			if (!value.contains(sel->value(), caseSensitive)
					|| sel->value().isEmpty())
				return false;
			break;
		...
		case CSSSelector::End:
			if (!value.endsWith(sel->value(), caseSensitive)
					|| sel->value().isEmpty())
				return false;
			break;
	...
	if (sel->m_match == CSSSelector::PseudoClass) {
		...
		if (sel->pseudoType() == CSSSelector::PseudoNot) {
			...
			for (CSSSelector* subSel = sel->selectorList()->first(); subSel;
				...
				if (!checkOneSelector(subSel, e, selectorAttrs, dynamicPseudo,
						true, encounteredLink, elementStyle,
						elementParentStyle))
					return true;
			}
		} else if (dynamicPseudo != NOPSEUDO
				&& (RenderScrollbar::scrollbarForStyleResolve()
						|| dynamicPseudo == SCROLLBAR_CORNER
						|| dynamicPseudo == RESIZER)) {
			...
			if (!m_collectRulesOnly) {
				if (elementStyle)
					elementStyle->setEmptyState(result);
						...
						&& (e->document()->usesSiblingRules()
								|| e->renderStyle()->unique()))
		...
		case CSSSelector::PseudoFirstOfType: {
			...
			if (e->parentNode() && e->parentNode()->isElementNode()) {
				...
				while (n) {
					if (n->isElementNode()
							&& static_cast<Element*>(n)->hasTagName(type))
						break;
					n = n->previousSibling();
				}
				if (!n)
		...
		case CSSSelector::PseudoAny:
			for (CSSSelector* selector = sel->selectorList()->first(); selector;
					selector = CSSSelectorList::next(selector)) {
				if (checkSelector(selector, e, selectorAttrs, dynamicPseudo,
						true, encounteredLink, elementStyle, elementParentStyle)
						== SelectorMatches)
					return true;
			}
			break;
		...
		case CSSSelector::PseudoFocus:
			if (e && e->focused() && e->document()->frame()
		...
		case CSSSelector::PseudoHover: {
			...
			if (m_strictParsing || isSubSelector
		...
		case CSSSelector::PseudoChecked: {
			...
			if (inputElement && inputElement->isChecked()
...
bool CSSStyleSelector::SelectorChecker::checkScrollbarPseudoClass(
		CSSSelector* sel, PseudoId&) const {
	...
	case CSSSelector::PseudoHover: {
		...
		if (part == TrackBGPart)
			return hoveredPart == BackTrackPart
					|| hoveredPart == ForwardTrackPart
					|| hoveredPart == ThumbPart;
	...
	case CSSSelector::PseudoDecrement:
		return part == BackButtonStartPart || part == BackButtonEndPart
				|| part == BackTrackPart;
	case CSSSelector::PseudoIncrement:
		return part == ForwardButtonStartPart || part == ForwardButtonEndPart
	...
	case CSSSelector::PseudoDoubleButton: {
		ScrollbarButtonsPlacement buttonsPlacement =
				scrollbar->theme()->buttonsPlacement();
		if (part == BackButtonStartPart || part == ForwardButtonStartPart
				|| part == BackTrackPart)
			return buttonsPlacement == ScrollbarButtonsDoubleStart
					|| buttonsPlacement == ScrollbarButtonsDoubleBoth;
		if (part == BackButtonEndPart || part == ForwardButtonEndPart
				|| part == ForwardTrackPart)
			return buttonsPlacement == ScrollbarButtonsDoubleEnd
					|| buttonsPlacement == ScrollbarButtonsDoubleBoth;
		return false;
	}
	case CSSSelector::PseudoSingleButton: {
		...
		if (part == BackButtonStartPart || part == ForwardButtonEndPart
				|| part == BackTrackPart || part == ForwardTrackPart)
	...
	case CSSSelector::PseudoNoButton: {
		...
		if (part == BackTrackPart)
			return buttonsPlacement == ScrollbarButtonsNone
...
static inline bool isSelectorMatchingHTMLBasedOnRuleHash(
		const CSSSelector* selector) {
...
RuleData::RuleData(CSSStyleRule* rule, CSSSelector* selector, unsigned position) :
		m_rule(rule), m_selector(selector), m_specificity(
				selector->specificity()), m_position(position), m_hasFastCheckableSelector(
				isFastCheckableSelector(selector)), m_hasMultipartSelector(
				selector->tagHistory()), m_hasTopSelectorMatchingHTMLBasedOnRuleHash(
				isSelectorMatchingHTMLBasedOnRuleHash(selector)) {
	collectDescendantSelectorIdentifierHashes();
}

inline void RuleData::collectIdentifierHashes(const CSSSelector* selector,
		unsigned& identifierCount) {
	if ((selector->m_match == CSSSelector::Id
			|| selector->m_match == CSSSelector::Class)
			&& !selector->value().isEmpty())
		m_descendantSelectorIdentifierHashes[identifierCount++] =
...
inline void RuleData::collectDescendantSelectorIdentifierHashes() {
	...
	for (const CSSSelector* selector = m_selector->tagHistory(); selector;
			selector = selector->tagHistory()) {
...
RuleSet::RuleSet() :
		m_ruleCount(0), m_autoShrinkToFitEnabled(true) {
}
...
void RuleSet::addRulesFromSheet(CSSStyleSheet* sheet,
		const MediaQueryEvaluator& medium, CSSStyleSelector* styleSelector) {
	if (!sheet)
		return;
	...
	for (int i = 0; i < len; i++) {
		...
		if (item->isStyleRule())
			...
			if ((!r->media() || medium.eval(r->media(), styleSelector))
					&& rules) {
				...
				for (unsigned j = 0; j < rules->length(); j++) {
					...
					if (childItem->isStyleRule()) {
						...
						styleSelector->addKeyframeStyle(
								static_cast<WebKitCSSKeyframesRule*>(childItem));
					}
				} // for rules
			} // if rules
			...
			styleSelector->addKeyframeStyle(
					static_cast<WebKitCSSKeyframesRule*>(item));
	}
...
void RuleSet::addStyleRule(CSSStyleRule* rule) {
	if (rule->isPageRule()) {
		...
		for (CSSSelector* s = rule->selectorList().first(); s; s =
				CSSSelectorList::next(s))
			addRule(rule, s);
	}
}

static inline void collectFeaturesFromSelector(
		CSSStyleSelector::Features& features, const CSSSelector* selector) {
...
static void collectFeaturesFromList(CSSStyleSelector::Features& features,
		const Vector<RuleData>& rules) {
	unsigned size = rules.size();
	for (unsigned i = 0; i < size; ++i) {
		const RuleData& ruleData = rules[i];
		bool foundSiblingSelector = false;
		for (CSSSelector* selector = ruleData.selector(); selector; selector =
				selector->tagHistory()) {
			collectFeaturesFromSelector(features, selector);

			if (CSSSelectorList* selectorList = selector->selectorList()) {
				for (CSSSelector* subSelector = selectorList->first();
						subSelector;
		...
		if (foundSiblingSelector) {
			...
			features.siblingRules->addRule(ruleData.rule(),
					ruleData.selector());
		}
	}
}
...
void RuleSet::collectFeatures(CSSStyleSelector::Features& features) const {
	...
	for (AtomRuleMap::const_iterator it = m_pseudoRules.begin(); it != end;
			++it)
		collectFeaturesFromList(features, *it->second);
...
static Length convertToLength(CSSPrimitiveValue* primitiveValue,
		RenderStyle* style, RenderStyle* rootStyle, bool toFloat,
		double multiplier = 1, bool *ok = 0) {
	...
	if (!primitiveValue) {
		...
		if (!style
			...
			if (toFloat)
				l = Length(
						primitiveValue->computeLengthDouble(style, rootStyle,
								multiplier), Fixed);
			else
				l = Length(
						primitiveValue->computeLengthIntForLength(style,
								rootStyle, multiplier), Fixed);
...
static Length convertToIntLength(CSSPrimitiveValue* primitiveValue,
		RenderStyle* style, RenderStyle* rootStyle, double multiplier = 1,
		bool *ok = 0) {
	return convertToLength(primitiveValue, style, rootStyle, false, multiplier,
			ok);
}

static Length convertToFloatLength(CSSPrimitiveValue* primitiveValue,
		RenderStyle* style, RenderStyle* rootStyle, double multiplier = 1,
		bool *ok = 0) {
...
void CSSStyleSelector::applyDeclarations(bool isImportant, int startIndex,
		int endIndex) {
	if (startIndex == -1)
		return;
	...
	for (int i = startIndex; i <= endIndex; i++) {
		...
		for (CSSMutableStyleDeclaration::const_iterator it = decl->begin();
				it != end; ++it) {
			...
			if (isImportant == current.isImportant()) {
				...
				if (applyFirst) {
					COMPILE_ASSERT(firstCSSProperty == CSSPropertyColor,
							CSS_color_is_first_property);
					COMPILE_ASSERT(CSSPropertyZoom == CSSPropertyColor + 14,
							CSS_zoom_is_end_of_first_prop_range);
					COMPILE_ASSERT(CSSPropertyLineHeight == CSSPropertyZoom + 1,
							CSS_line_height_is_after_zoom);
...
void CSSStyleSelector::matchPageRules(RuleSet* rules, bool isLeftPage,
	...
	matchPageRulesForList(rules->getPageRules(), isLeftPage, isFirstPage,
			pageName);
...
void CSSStyleSelector::matchPageRulesForList(const Vector<RuleData>* rules,
	...
	for (unsigned i = 0; i < size; ++i) {
		const RuleData& ruleData = rules->at(i);
		CSSStyleRule* rule = ruleData.rule();
		const AtomicString& selectorLocalName =
				ruleData.selector()->tag().localName();
...
static void applyCounterList(RenderStyle* style, CSSValueList* list,
		bool isReset) {
	...
	for (int i = 0; i < length; ++i) {
		...
		AtomicString identifier =
				static_cast<CSSPrimitiveValue*>(pair->first())->getStringValue();
		// FIXME: What about overflow?
		int value =
				static_cast<CSSPrimitiveValue*>(pair->second())->getIntValue();
		CounterDirectives& directives = map.add(identifier.impl(),
				CounterDirectives()).first->second;
		if (isReset) {
...
void CSSStyleSelector::applyPropertyToStyle(int id, CSSValue *value,
		RenderStyle* style) {
	initElement(0);
...
void CSSStyleSelector::applyProperty(int id, CSSValue *value) {
	...
	bool isInitial = valueType == CSSValue::CSS_INITIAL
			|| (!m_parentNode && valueType == CSSValue::CSS_INHERIT);

	id = CSSProperty::resolveDirectionAwareProperty(id, m_style->direction(),
			m_style->writingMode());

	if (m_checker.m_matchVisitedPseudoClass
...
// ident only properties
	...
	case CSSPropertyOutlineStyle:
		HANDLE_INHERIT_AND_INITIAL_WITH_VALUE(outlineStyle, OutlineStyle,
				BorderStyle)
		if (primitiveValue) {
	...
	case CSSPropertyFontStyle: {
		...
		if (isInherit)
			fontDescription.setItalic(
					m_parentStyle->fontDescription().italic());
		else if (isInitial)
	...
	case CSSPropertyFontVariant: {
		FontDescription fontDescription = m_style->fontDescription();
		if (isInherit)
			fontDescription.setSmallCaps(
					m_parentStyle->fontDescription().smallCaps());
		else if (isInitial)
	...
	case CSSPropertyFontWeight: {
		...
		if (isInherit)
			fontDescription.setWeight(
					m_parentStyle->fontDescription().weight());
		else if (isInitial)
	...
	case CSSPropertyListStylePosition:
		HANDLE_INHERIT_AND_INITIAL_AND_PRIMITIVE(listStylePosition,
				ListStylePosition)
		return;
	...
	case CSSPropertyPageBreakBefore:
		HANDLE_INHERIT_AND_INITIAL_AND_PRIMITIVE_WITH_VALUE(pageBreakBefore,
				PageBreakBefore, PageBreak)
		return;
	case CSSPropertyPageBreakAfter:
		HANDLE_INHERIT_AND_INITIAL_AND_PRIMITIVE_WITH_VALUE(pageBreakAfter,
				PageBreakAfter, PageBreak)
		return;
	case CSSPropertyPageBreakInside: {
		HANDLE_INHERIT_AND_INITIAL_WITH_VALUE(pageBreakInside, PageBreakInside,
				PageBreak)
		if (!primitiveValue)
			return;
	...
	case CSSPropertyBorderSpacing: {
		if (isInherit) {
			m_style->setHorizontalBorderSpacing(
					m_parentStyle->horizontalBorderSpacing());
			m_style->setVerticalBorderSpacing(
					m_parentStyle->verticalBorderSpacing());
		} else if (isInitial) {
	...
	case CSSPropertyWebkitBorderHorizontalSpacing: {
		HANDLE_INHERIT_AND_INITIAL(horizontalBorderSpacing,
				HorizontalBorderSpacing)
		if (!primitiveValue)
			return;
		short spacing = primitiveValue->computeLengthShort(style(),
				m_rootElementStyle, zoomFactor);
	...
	case CSSPropertyCursor:
		...
		if (value->isValueList()) {
			...
			for (int i = 0; i < len; i++) {
				...
				if (type == CSSPrimitiveValue::CSS_URI) {
					if (primitiveValue->isCursorImageValue()) {
						CSSCursorImageValue* image =
								static_cast<CSSCursorImageValue*>(primitiveValue);
						if (image->updateIfSVGCursorIsUsed(m_element)) // Elements with SVG cursors are not allowed to share style.
							m_style->setUnique();
						m_style->addCursor(
								cachedOrPendingFromValue(CSSPropertyCursor,
										image), image->hotSpot());
					}
			...
			if (type == CSSPrimitiveValue::CSS_IDENT
					&& m_style->cursor() != ECursor(*primitiveValue))
				m_style->setCursor(*primitiveValue);
		}
		return;
...
// uri || inherit
	...
	case CSSPropertyListStyleImage: {
		...
		m_style->setListStyleImage(
				styleImage(CSSPropertyListStyleImage, value));
		return;
	}
// length
	...
	case CSSPropertyWebkitColumnRuleWidth: {
		if (isInherit) {
			HANDLE_INHERIT_COND(CSSPropertyBorderTopWidth, borderTopWidth,
					BorderTopWidth)
			HANDLE_INHERIT_COND(CSSPropertyBorderRightWidth, borderRightWidth,
					BorderRightWidth)
			HANDLE_INHERIT_COND(CSSPropertyBorderBottomWidth, borderBottomWidth,
					BorderBottomWidth)
			HANDLE_INHERIT_COND(CSSPropertyBorderLeftWidth, borderLeftWidth,
					BorderLeftWidth)
			HANDLE_INHERIT_COND(CSSPropertyOutlineWidth, outlineWidth,
					OutlineWidth)
			HANDLE_INHERIT_COND(CSSPropertyWebkitColumnRuleWidth,
					columnRuleWidth, ColumnRuleWidth)
			return;
		} else if (isInitial) {
			HANDLE_INITIAL_COND_WITH_VALUE(CSSPropertyBorderTopWidth,
					BorderTopWidth, BorderWidth)
			HANDLE_INITIAL_COND_WITH_VALUE(CSSPropertyBorderRightWidth,
					BorderRightWidth, BorderWidth)
			HANDLE_INITIAL_COND_WITH_VALUE(CSSPropertyBorderBottomWidth,
					BorderBottomWidth, BorderWidth)
			HANDLE_INITIAL_COND_WITH_VALUE(CSSPropertyBorderLeftWidth,
					BorderLeftWidth, BorderWidth)
			HANDLE_INITIAL_COND_WITH_VALUE(CSSPropertyOutlineWidth,
					OutlineWidth, BorderWidth)
			HANDLE_INITIAL_COND_WITH_VALUE(CSSPropertyWebkitColumnRuleWidth,
	...
	case CSSPropertyWebkitFontSmoothing: {
		FontDescription fontDescription = m_style->fontDescription();
		if (isInherit)
			fontDescription.setFontSmoothing(
					m_parentStyle->fontDescription().fontSmoothing());
		else if (isInitial)
	...
	case CSSPropertyWordSpacing: {
		if (isInherit) {
			HANDLE_INHERIT_COND(CSSPropertyLetterSpacing, letterSpacing,
					LetterSpacing)
			HANDLE_INHERIT_COND(CSSPropertyWordSpacing, wordSpacing,
					WordSpacing)
			return;
		} else if (isInitial) {
			HANDLE_INITIAL_COND_WITH_VALUE(CSSPropertyLetterSpacing,
					LetterSpacing, LetterWordSpacing)
			HANDLE_INITIAL_COND_WITH_VALUE(CSSPropertyWordSpacing, WordSpacing,
					LetterWordSpacing)
			return;
		}
		...
		if (primitiveValue && primitiveValue->getIdent() == CSSValueNormal) {
			...
			width = primitiveValue->computeLengthInt(style(),
					m_rootElementStyle,
					useSVGZoomRules(m_element) ? 1.0f : zoomFactor);
		}
		switch (id) {
		case CSSPropertyLetterSpacing:
	...
	case CSSPropertyResize: {
		...
		if (primitiveValue->getIdent() == CSSValueAuto) {
			if (Settings* settings = m_checker.m_document->settings())
				r = settings->textAreasAreResizable() ?
						RESIZE_BOTH : RESIZE_NONE;
		} else
			r = *primitiveValue;
	...
	case CSSPropertyMarginLeft:
		// +inherit +auto
		if (id == CSSPropertyWidth || id == CSSPropertyMinWidth
				|| id == CSSPropertyMaxWidth) {
			if (primitiveValue
					&& primitiveValue->getIdent() == CSSValueIntrinsic) {
				l = Length(Intrinsic);
				apply = true;
			} else if (primitiveValue
					&& primitiveValue->getIdent() == CSSValueMinIntrinsic) {
				l = Length(MinIntrinsic);
				apply = true;
			}
		}
		if (id != CSSPropertyMaxWidth && primitiveValue
		...
		// +inherit
	{
		if (isInherit) {
			...
			HANDLE_INHERIT_COND(CSSPropertyPaddingRight, paddingRight,
					PaddingRight)
			HANDLE_INHERIT_COND(CSSPropertyPaddingBottom, paddingBottom,
					PaddingBottom)
			HANDLE_INHERIT_COND(CSSPropertyPaddingLeft, paddingLeft,
					PaddingLeft)
			HANDLE_INHERIT_COND(CSSPropertyMarginTop, marginTop, MarginTop)
			HANDLE_INHERIT_COND(CSSPropertyMarginRight, marginRight,
					MarginRight)
			HANDLE_INHERIT_COND(CSSPropertyMarginBottom, marginBottom,
					MarginBottom)
			...
			HANDLE_INITIAL_COND_WITH_VALUE(CSSPropertyMaxWidth, MaxWidth,
					MaxSize)
			...
			HANDLE_INITIAL_COND_WITH_VALUE(CSSPropertyMinWidth, MinWidth,
					MinSize)
			HANDLE_INITIAL_COND_WITH_VALUE(CSSPropertyPaddingTop, PaddingTop,
					Padding)
			HANDLE_INITIAL_COND_WITH_VALUE(CSSPropertyPaddingRight,
					PaddingRight, Padding)
			HANDLE_INITIAL_COND_WITH_VALUE(CSSPropertyPaddingBottom,
					PaddingBottom, Padding)
			HANDLE_INITIAL_COND_WITH_VALUE(CSSPropertyPaddingLeft, PaddingLeft,
					Padding)
			HANDLE_INITIAL_COND_WITH_VALUE(CSSPropertyMarginTop, MarginTop,
					Margin)
			HANDLE_INITIAL_COND_WITH_VALUE(CSSPropertyMarginRight, MarginRight,
					Margin)
			HANDLE_INITIAL_COND_WITH_VALUE(CSSPropertyMarginBottom,
					MarginBottom, Margin)
			HANDLE_INITIAL_COND_WITH_VALUE(CSSPropertyMarginLeft, MarginLeft,
					Margin)
	...
	case CSSPropertyMinHeight:
		if (primitiveValue && primitiveValue->getIdent() == CSSValueIntrinsic) {
			...
			l = Length(MinIntrinsic);
			apply = true;
		} else if (id != CSSPropertyMaxHeight && primitiveValue
				&& primitiveValue->getIdent() == CSSValueAuto)
			apply = true;
		if (isInherit) {
		...
		if (isInitial) {
			HANDLE_INITIAL_COND_WITH_VALUE(CSSPropertyMaxHeight, MaxHeight,
					MaxSize)
			HANDLE_INITIAL_COND_WITH_VALUE(CSSPropertyHeight, Height, Size)
			HANDLE_INITIAL_COND_WITH_VALUE(CSSPropertyMinHeight, MinHeight,
					MinSize)
			return;
		}
	...
	case CSSPropertyFontSize: {
		...
		if (m_parentNode) {
			...
			parentIsAbsoluteSize =
					m_parentStyle->fontDescription().isAbsoluteSize();
		}

		if (isInherit) {
			size = oldSize;
			if (m_parentNode)
				fontDescription.setKeywordSize(
						m_parentStyle->fontDescription().keywordSize());
		} else if (isInitial) {
			size = fontSizeForKeyword(m_checker.m_document, CSSValueMedium,
					fontDescription.useFixedDefaultSize());
			fontDescription.setKeywordSize(
					CSSValueMedium - CSSValueXxSmall + 1);
			...
			case CSSValueWebkitXxxLarge:
				size = fontSizeForKeyword(m_checker.m_document,
						primitiveValue->getIdent(),
						fontDescription.useFixedDefaultSize());
				fontDescription.setKeywordSize(
						primitiveValue->getIdent() - CSSValueXxSmall + 1);
				break;
			case CSSValueLarger:
				size = largerFontSize(oldSize,
						m_checker.m_document->inQuirksMode());
				break;
			case CSSValueSmaller:
				size = smallerFontSize(oldSize,
						m_checker.m_document->inQuirksMode());
				break;
			default:
				return;
			...
			fontDescription.setIsAbsoluteSize(
					parentIsAbsoluteSize
			...
			fontDescription.setIsAbsoluteSize(
					parentIsAbsoluteSize
							|| (type != CSSPrimitiveValue::CSS_PERCENTAGE
									...
									&& type != CSSPrimitiveValue::CSS_REMS));
			if (CSSPrimitiveValue::isUnitTypeLength(type))
				size = primitiveValue->computeLengthFloat(m_parentStyle,
						m_rootElementStyle, true);
			else if (type == CSSPrimitiveValue::CSS_PERCENTAGE)
...
// length, percent, number
	case CSSPropertyLineHeight: {
		...
		if (primitiveValue->getIdent() == CSSValueNormal)
			...
			lineHeight = Length(
					primitiveValue->computeLengthIntForLength(style(),
							m_rootElementStyle, multiplier), Fixed);
		} else if (type == CSSPrimitiveValue::CSS_PERCENTAGE)
			lineHeight = Length(
					(m_style->fontSize() * primitiveValue->getIntValue()) / 100,
					Fixed);
		else if (type == CSSPrimitiveValue::CSS_NUMBER)
			lineHeight = Length(primitiveValue->getDoubleValue() * 100.0,
					Percent);
		else
			return;
	...
	case CSSPropertyTextAlign: {
		...
		if (primitiveValue->getIdent() == CSSValueWebkitMatchParent) {
			if (m_parentStyle->textAlign() == TASTART)
				m_style->setTextAlign(
						m_parentStyle->isLeftToRightDirection() ? LEFT : RIGHT);
			else if (m_parentStyle->textAlign() == TAEND)
				m_style->setTextAlign(
						m_parentStyle->isLeftToRightDirection() ? RIGHT : LEFT);
			else
	...
	case CSSPropertyClip: {
		...
		if (isInherit) {
			...
			top = convertToIntLength(rect->top(), style(), m_rootElementStyle,
					zoomFactor);
			right = convertToIntLength(rect->right(), style(),
					m_rootElementStyle, zoomFactor);
			bottom = convertToIntLength(rect->bottom(), style(),
					m_rootElementStyle, zoomFactor);
			left = convertToIntLength(rect->left(), style(), m_rootElementStyle,
					zoomFactor);
...
// lists
		...
		// list of string, uri, counter, attr, i
	{
		...
		for (int i = 0; i < len; i++) {
			...
			if (item->isImageGeneratorValue()) {
				m_style->setContent(
						static_cast<CSSImageGeneratorValue*>(item)->generatedImage(),
						didSet);
				didSet = true;
			}
			...
			CSSPrimitiveValue* contentValue =
					static_cast<CSSPrimitiveValue*>(item);
			switch (contentValue->primitiveType()) {
			case CSSPrimitiveValue::CSS_STRING:
				m_style->setContent(contentValue->getStringValue().impl(),
						didSet);
				didSet = true;
				break;
			case CSSPrimitiveValue::CSS_ATTR: {
				...
				QualifiedName attr(nullAtom,
						contentValue->getStringValue().impl(), nullAtom);
				m_style->setContent(m_element->getAttribute(attr).impl(),
						didSet);
				didSet = true;
			...
			case CSSPrimitiveValue::CSS_URI: {
				if (!contentValue->isImageValue())
					break;
				m_style->setContent(
						cachedOrPendingFromValue(CSSPropertyContent,
								static_cast<CSSImageValue*>(contentValue)),
						didSet);
				didSet = true;
				break;
			}
			case CSSPrimitiveValue::CSS_COUNTER: {
				...
				OwnPtr<CounterContent> counter =
						adoptPtr(
								new CounterContent(counterValue->identifier(),
	...
	case CSSPropertyFontFamily: {
		...
		if (isInherit) {
			FontDescription parentFontDescription =
					m_parentStyle->fontDescription();
			FontDescription fontDescription = m_style->fontDescription();
			fontDescription.setGenericFamily(
					parentFontDescription.genericFamily());
			fontDescription.setFamily(parentFontDescription.firstFamily());
			fontDescription.setIsSpecifiedFont(
					parentFontDescription.isSpecifiedFont());
			if (m_style->setFontDescription(fontDescription))
				m_fontDirty = true;
			return;
		} else if (isInitial) {
			...
			// to non-monospace.
			if (fontDescription.keywordSize()
					&& fontDescription.useFixedDefaultSize())
				setFontSize(fontDescription,
						fontSizeForKeyword(m_checker.m_document,
								CSSValueXxSmall + fontDescription.keywordSize()
										- 1, false));
		...
		bool oldFamilyUsedFixedDefaultSize =
				fontDescription.useFixedDefaultSize();
		...
		for (int i = 0; i < len; i++) {
			...
			if (contentValue->primitiveType()
					== CSSPrimitiveValue::CSS_STRING) {
				if (contentValue->isFontFamilyValue())
					face =
							static_cast<FontFamilyValue*>(contentValue)->familyName();
			} else if (contentValue->primitiveType()
					== CSSPrimitiveValue::CSS_IDENT && settings) {
				switch (contentValue->getIdent()) {
			...
			if (!face.isEmpty()) {
				if (!currFamily) {
					...
					RefPtr<SharedFontFamily> newFamily =
		...
		if (currFamily) {
			if (fontDescription.keywordSize()
				...
				setFontSize(fontDescription,
						fontSizeForKeyword(m_checker.m_document,
								CSSValueXxSmall + fontDescription.keywordSize()
										- 1, !oldFamilyUsedFixedDefaultSize));
	...
	case CSSPropertyZoom: {
		...
		// zoom in effect.
		m_style->setEffectiveZoom(
				m_parentStyle ?
						m_parentStyle->effectiveZoom() :
...
// shorthand properties
	case CSSPropertyBackground:
		if (isInitial) {
			...
			m_style->inheritBackgroundLayers(
					*m_parentStyle->backgroundLayers());
	...
	case CSSPropertyBorderColor:
		if (id == CSSPropertyBorder || id == CSSPropertyBorderColor) {
			if (isInherit) {
				m_style->setBorderTopColor(
						m_parentStyle->borderTopColor().isValid() ?
								m_parentStyle->borderTopColor() :
								m_parentStyle->color());
				m_style->setBorderBottomColor(
						m_parentStyle->borderBottomColor().isValid() ?
								m_parentStyle->borderBottomColor() :
								m_parentStyle->color());
				m_style->setBorderLeftColor(
						m_parentStyle->borderLeftColor().isValid() ?
								m_parentStyle->borderLeftColor() :
								m_parentStyle->color());
				m_style->setBorderRightColor(
						m_parentStyle->borderRightColor().isValid() ?
								m_parentStyle->borderRightColor() :
								m_parentStyle->color());
			} else if (isInitial) {
		...
		if (id == CSSPropertyBorder || id == CSSPropertyBorderStyle) {
			if (isInherit) {
				...
				m_style->setBorderBottomStyle(
						m_parentStyle->borderBottomStyle());
				...
				m_style->setBorderBottomStyle(
						RenderStyle::initialBorderStyle());
		...
		if (id == CSSPropertyBorder || id == CSSPropertyBorderWidth) {
			if (isInherit) {
				...
				m_style->setBorderBottomWidth(
						m_parentStyle->borderBottomWidth());
	...
	case CSSPropertyFont:
		if (isInherit) {
			...
			const AtomicString& standardFontFamily =
					m_checker.m_document->settings()->standardFontFamily();
			...
			fontDescription.setKeywordSize(
					CSSValueMedium - CSSValueXxSmall + 1);
			setFontSize(fontDescription,
					fontSizeForKeyword(m_checker.m_document, CSSValueMedium,
							false));
			...
			FontDescription fontDescription;
			RenderTheme::defaultTheme()->systemFont(primitiveValue->getIdent(),
					fontDescription);
			...
			if (fontDescription.isAbsoluteSize()) {
				...
				if (!settings)
					return;
				fontDescription.setRenderingMode(settings->fontRenderingMode());
				fontDescription.setUsePrinterFont(
						m_checker.m_document->printing());

				// Handle the zoom factor.
				fontDescription.setComputedSize(
						getComputedSizeFromSpecifiedSize(m_checker.m_document,
								m_style.get(), fontDescription.isAbsoluteSize(),
								fontDescription.specifiedSize(),
								useSVGZoomRules(m_element)));
				if (m_style->setFontDescription(fontDescription))
					m_fontDirty = true;
			}
		} else if (value->isFontValue()) {
			FontValue *font = static_cast<FontValue*>(value);
			if (!font->style || !font->variant || !font->weight || !font->size
					|| !font->lineHeight || !font->family)
				return;
	...
	case CSSPropertyListStyle:
		if (isInherit) {
			...
			m_style->setListStylePosition(
					RenderStyle::initialListStylePosition());
		}
		return;
	case CSSPropertyOutline:
		if (isInherit) {
			m_style->setOutlineWidth(m_parentStyle->outlineWidth());
			m_style->setOutlineColor(
					m_parentStyle->outlineColor().isValid() ?
							m_parentStyle->outlineColor() :
							m_parentStyle->color());
	...
	case CSSPropertyWebkitMaskBoxImage: {
		if (isInherit) {
			HANDLE_INHERIT_COND(CSSPropertyBorderImage, borderImage,
					BorderImage)
			HANDLE_INHERIT_COND(CSSPropertyWebkitBorderImage, borderImage,
					BorderImage)
			HANDLE_INHERIT_COND(CSSPropertyWebkitMaskBoxImage, maskBoxImage,
					MaskBoxImage)
			return;
		} else if (isInitial) {
			HANDLE_INITIAL_COND_WITH_VALUE(CSSPropertyBorderImage,
					BorderImage, NinePieceImage)
			HANDLE_INITIAL_COND_WITH_VALUE(CSSPropertyWebkitBorderImage,
					BorderImage, NinePieceImage)
			HANDLE_INITIAL_COND_WITH_VALUE(CSSPropertyWebkitMaskBoxImage,
					MaskBoxImage, NinePieceImage)
			return;
		}
	...
	case CSSPropertyWebkitMaskBoxImageSlice: {
		...
		NinePieceImage image(
				isBorderImage ?
						m_style->borderImage() : m_style->maskBoxImage());
		if (isInherit)
...
//        return;
	...
	case CSSPropertyWebkitBorderRadius:
		if (isInherit) {
			m_style->setBorderTopLeftRadius(
					m_parentStyle->borderTopLeftRadius());
			m_style->setBorderTopRightRadius(
					m_parentStyle->borderTopRightRadius());
			m_style->setBorderBottomLeftRadius(
					m_parentStyle->borderBottomLeftRadius());
			m_style->setBorderBottomRightRadius(
	...
	case CSSPropertyBorderBottomRightRadius: {
		if (isInherit) {
			HANDLE_INHERIT_COND(CSSPropertyBorderTopLeftRadius,
					borderTopLeftRadius, BorderTopLeftRadius)
			HANDLE_INHERIT_COND(CSSPropertyBorderTopRightRadius,
					borderTopRightRadius, BorderTopRightRadius)
			HANDLE_INHERIT_COND(CSSPropertyBorderBottomLeftRadius,
		...
		if (isInitial) {
			...
			HANDLE_INITIAL_COND_WITH_VALUE(CSSPropertyBorderBottomLeftRadius,
					BorderBottomLeftRadius, BorderRadius)
			HANDLE_INITIAL_COND_WITH_VALUE(CSSPropertyBorderBottomRightRadius,
		...
		if (pair->first()->primitiveType() == CSSPrimitiveValue::CSS_PERCENTAGE)
			...
			radiusWidth = Length(
					max(intMinForLength,
							min(intMaxForLength,
									pair->first()->computeLengthInt(style(),
											m_rootElementStyle, zoomFactor))),
					Fixed);
		if (pair->second()->primitiveType()
			...
			radiusHeight = Length(
					max(intMinForLength,
							min(intMaxForLength,
									pair->second()->computeLengthInt(style(),
											m_rootElementStyle, zoomFactor))),
					Fixed);
	...
	case CSSPropertyTextRendering: {
		FontDescription fontDescription = m_style->fontDescription();
		if (isInherit)
			fontDescription.setTextRenderingMode(
					m_parentStyle->fontDescription().textRenderingMode());
		else if (isInitial)
	...
	case CSSPropertyWebkitBoxShadow: {
		if (isInherit) {
			if (id == CSSPropertyTextShadow)
				return m_style->setTextShadow(
						m_parentStyle->textShadow() ?
								new ShadowData(*m_parentStyle->textShadow()) :
								0);
			return m_style->setBoxShadow(
					m_parentStyle->boxShadow() ?
							new ShadowData(*m_parentStyle->boxShadow()) : 0);
		}
		if (isInitial || primitiveValue) // initial | none
			return id == CSSPropertyTextShadow ?
					m_style->setTextShadow(0) : m_style->setBoxShadow(0);
		...
		for (int i = 0; i < len; i++) {
			...
			if (!currValue->isShadowValue())
				continue;
			ShadowValue* item =
					static_cast<ShadowValue*>(list->itemWithoutBoundsCheck(i));
			...
			int blur =
					item->blur ?
							item->blur->computeLengthInt(style(),
									m_rootElementStyle, zoomFactor) :
							0;
			int spread =
					item->spread ?
							item->spread->computeLengthInt(style(),
									m_rootElementStyle, zoomFactor) :
							0;
			ShadowStyle shadowStyle =
					item->style && item->style->getIdent() == CSSValueInset ?
							Inset : Normal;
			Color color;
			if (item->color)
				color = getColorFromPrimitiveValue(item->color.get());
			ShadowData* shadowData = new ShadowData(x, y, blur, spread,
					shadowStyle, id == CSSPropertyWebkitBoxShadow,
					color.isValid() ? color : Color::transparent);
			if (id == CSSPropertyTextShadow)
	...
	case CSSPropertyWebkitBoxReflect: {
		...
		if (reflectValue->offset()) {
			...
			if (type == CSSPrimitiveValue::CSS_PERCENTAGE)
				reflection->setOffset(
						Length(reflectValue->offset()->getDoubleValue(),
								Percent));
			else
				reflection->setOffset(
						Length(
								reflectValue->offset()->computeLengthIntForLength(
	...
	case CSSPropertyOpacity:
		...
		m_style->setOpacity(
				min(1.0f, max(0.0f, primitiveValue->getFloatValue())));
		return;
	...
	case CSSPropertyWebkitBackfaceVisibility:
		...
		if (primitiveValue)
			m_style->setBackfaceVisibility(
					(primitiveValue->getIdent() == CSSValueVisible) ?
	...
	case CSSPropertyWebkitBoxFlexGroup:
		...
		if (!primitiveValue
				|| primitiveValue->primitiveType()
						!= CSSPrimitiveValue::CSS_NUMBER)
			return; // Error case.
		m_style->setBoxFlexGroup(
				(unsigned int) (primitiveValue->getDoubleValue()));
		return;
	...
	case CSSPropertyWebkitColumnWidth: {
		...
		m_style->setColumnWidth(
				primitiveValue->computeLengthFloat(style(), m_rootElementStyle,
						zoomFactor));
		return;
	}
	case CSSPropertyWebkitColumnRuleStyle:
		HANDLE_INHERIT_AND_INITIAL_AND_PRIMITIVE_WITH_VALUE(columnRuleStyle,
				ColumnRuleStyle, BorderStyle)
		return;
	case CSSPropertyWebkitColumnBreakBefore:
		HANDLE_INHERIT_AND_INITIAL_AND_PRIMITIVE_WITH_VALUE(columnBreakBefore,
				ColumnBreakBefore, PageBreak)
		return;
	case CSSPropertyWebkitColumnBreakAfter:
		HANDLE_INHERIT_AND_INITIAL_AND_PRIMITIVE_WITH_VALUE(columnBreakAfter,
				ColumnBreakAfter, PageBreak)
		return;
	case CSSPropertyWebkitColumnBreakInside: {
		HANDLE_INHERIT_AND_INITIAL_WITH_VALUE(columnBreakInside,
				ColumnBreakInside, PageBreak)
	...
	case CSSPropertyWebkitColumnRule:
		if (isInherit) {
			m_style->setColumnRuleColor(
					m_parentStyle->columnRuleColor().isValid() ?
							m_parentStyle->columnRuleColor() :
							m_parentStyle->color());
	...
	case CSSPropertyWebkitMarquee:
		if (valueType != CSSValue::CSS_INHERIT || !m_parentNode)
			return;
	...
	case CSSPropertyWebkitMarqueeSpeed: {
		...
		if (primitiveValue->getIdent()) {
			...
			m_style->setMarqueeSpeed(primitiveValue->getIntValue());
				...
				== CSSPrimitiveValue::CSS_NUMBER) // For scrollamount support.
			m_style->setMarqueeSpeed(primitiveValue->getIntValue());
		return;
	}
	case CSSPropertyWebkitMarqueeIncrement: {
		...
		if (primitiveValue->getIdent()) {
			...
			bool ok = true;
			Length marqueeLength = convertToIntLength(primitiveValue, style(),
					m_rootElementStyle, 1, &ok);
			if (ok)
	...
	case CSSPropertyWebkitMarqueeStyle:
		HANDLE_INHERIT_AND_INITIAL_AND_PRIMITIVE(marqueeBehavior,
				MarqueeBehavior)
		return;
#if ENABLE(WCSS)
	...
	case CSSPropertyWebkitMarqueeDirection:
		HANDLE_INHERIT_AND_INITIAL_AND_PRIMITIVE(marqueeDirection,
				MarqueeDirection)
		return;
	...
	case CSSPropertyTextOverflow: {
		...
		if (!primitiveValue || !primitiveValue->getIdent())
			return;
		m_style->setTextOverflow(
				primitiveValue->getIdent() == CSSValueEllipsis);
		return;
	}
	case CSSPropertyWebkitMarginCollapse: {
		if (isInherit) {
			m_style->setMarginBeforeCollapse(
					m_parentStyle->marginBeforeCollapse());
			m_style->setMarginAfterCollapse(
					m_parentStyle->marginAfterCollapse());
		} else if (isInitial) {
	...
	case CSSPropertyWebkitMarginTopCollapse:
		HANDLE_INHERIT_AND_INITIAL_AND_PRIMITIVE(marginBeforeCollapse,
				MarginBeforeCollapse)
		return;
	case CSSPropertyWebkitMarginAfterCollapse:
	case CSSPropertyWebkitMarginBottomCollapse:
		HANDLE_INHERIT_AND_INITIAL_AND_PRIMITIVE(marginAfterCollapse,
				MarginAfterCollapse)
		return;
	...
	case CSSPropertyWebkitHyphenateLimitAfter: {
		HANDLE_INHERIT_AND_INITIAL(hyphenationLimitAfter,
				HyphenationLimitAfter);
		if (primitiveValue->getIdent() == CSSValueAuto)
			m_style->setHyphenationLimitAfter(-1);
		else
			m_style->setHyphenationLimitAfter(
					min(
							primitiveValue->getIntValue(
									CSSPrimitiveValue::CSS_NUMBER),
							static_cast<int>(numeric_limits<short>::max())));
		return;
	}
	case CSSPropertyWebkitHyphenateLimitBefore: {
		HANDLE_INHERIT_AND_INITIAL(hyphenationLimitBefore,
				HyphenationLimitBefore);
		if (primitiveValue->getIdent() == CSSValueAuto)
			m_style->setHyphenationLimitBefore(-1);
		else
			m_style->setHyphenationLimitBefore(
					min(
							primitiveValue->getIntValue(
									CSSPrimitiveValue::CSS_NUMBER),
							static_cast<int>(numeric_limits<short>::max())));
	...
	case CSSPropertyWebkitRtlOrdering:
		...
		if (!primitiveValue || !primitiveValue->getIdent())
			return;
		m_style->setVisuallyOrdered(
				primitiveValue->getIdent() == CSSValueVisual);
		return;
	case CSSPropertyWebkitTextStrokeWidth: {
		...
		case CSSValueThick: {
			...
			if (primitiveValue->getIdent() == CSSValueMedium)
				...
				result *= 5;
			width = CSSPrimitiveValue::create(result,
					CSSPrimitiveValue::CSS_EMS)->computeLengthFloat(style(),
					m_rootElementStyle, zoomFactor);
			break;
		}
		default:
	...
	case CSSPropertyWebkitTransformOriginZ: {
		...
		int type = primitiveValue->primitiveType();
		if (CSSPrimitiveValue::isUnitTypeLength(type))
			f = static_cast<float>(primitiveValue->computeLengthIntForLength(
					style(), m_rootElementStyle));
		else
			return;
	...
	case CSSPropertyWebkitTransformStyle:
		...
		if (primitiveValue)
			m_style->setTransformStyle3D(
					(primitiveValue->getIdent() == CSSValuePreserve3d) ?
							TransformStyle3DPreserve3D : TransformStyle3DFlat);
		return;
	case CSSPropertyWebkitPerspective: {
		...
		if (CSSPrimitiveValue::isUnitTypeLength(type))
			perspectiveValue =
					static_cast<float>(primitiveValue->computeLengthIntForLength(
							style(), m_rootElementStyle, zoomFactor));
		else if (type == CSSPrimitiveValue::CSS_NUMBER) {
			// For backward compatibility, treat valueless numbers as px.
			perspectiveValue =
					CSSPrimitiveValue::create(primitiveValue->getDoubleValue(),
							CSSPrimitiveValue::CSS_PX)->computeLengthFloat(
							style(), m_rootElementStyle, zoomFactor);
		} else
			return;
	...
	case CSSPropertyWebkitWritingMode: {
		...
		if (!isInherit && !isInitial && m_element
				&& m_element == m_element->document()->documentElement())
			m_element->document()->setWritingModeSetOnDocumentElement(true);
		FontDescription fontDescription = m_style->fontDescription();
		fontDescription.setOrientation(
				m_style->isHorizontalWritingMode() ? Horizontal : Vertical);
		if (m_style->setFontDescription(fontDescription))
			m_fontDirty = true;
		return;
	}
	...
	case CSSPropertyWebkitTextEmphasisPosition:
		HANDLE_INHERIT_AND_INITIAL_AND_PRIMITIVE(textEmphasisPosition,
	...
	case CSSPropertyWebkitTextEmphasisStyle:
		...
		HANDLE_INHERIT_AND_INITIAL(textEmphasisCustomMark,
		...
		if (value->isValueList()) {
			...
			for (unsigned i = 0; i < 2; ++i) {
				...
				if (value->getIdent() == CSSValueFilled
						|| value->getIdent() == CSSValueOpen)
		...
		if (primitiveValue->primitiveType() == CSSPrimitiveValue::CSS_STRING) {
			...
			m_style->setTextEmphasisCustomMark(
		...
		if (primitiveValue->getIdent() == CSSValueFilled
				|| primitiveValue->getIdent() == CSSValueOpen) {
	...
	case CSSPropertyWebkitLineBoxContain: {
		...
		CSSLineBoxContainValue* lineBoxContainValue =
				static_cast<CSSLineBoxContainValue*>(value);
...
void CSSStyleSelector::applyPageSizeProperty(CSSValue* value) {
	...
	case 2: {
		...
		if (!valueList->item(0)->isPrimitiveValue()
				|| !valueList->item(1)->isPrimitiveValue())
			return;
		CSSPrimitiveValue* primitiveValue0 =
				static_cast<CSSPrimitiveValue*>(valueList->item(0));
		CSSPrimitiveValue* primitiveValue1 =
		...
		if (CSSPrimitiveValue::isUnitTypeLength(type0)) {
			...
			if (!CSSPrimitiveValue::isUnitTypeLength(type1))
				return;
			width = Length(
					primitiveValue0->computeLengthIntForLength(style(),
							m_rootElementStyle), Fixed);
			height = Length(
					primitiveValue1->computeLengthIntForLength(style(),
							m_rootElementStyle), Fixed);
		} else {
			// <page-size> <orientation>
			// The value order is guaranteed. See CSSParser::parseSizeParameter.
			if (!pageSizeFromName(primitiveValue0, primitiveValue1, width,
					height))
				return;
		}
		break;
...
bool CSSStyleSelector::pageSizeFromName(CSSPrimitiveValue* pageSizeName,
	...
	static const Length letterWidth = inchLength(8.5), letterHeight =
			inchLength(11);
	static const Length legalWidth = inchLength(8.5), legalHeight = inchLength(
			14);
	static const Length ledgerWidth = inchLength(11), ledgerHeight = inchLength(
			17);

	if (!pageSizeName
			|| pageSizeName->primitiveType() != CSSPrimitiveValue::CSS_IDENT)
		return false;
...
Length CSSStyleSelector::mmLength(double mm) const {
	return Length(
			CSSPrimitiveValue::create(mm, CSSPrimitiveValue::CSS_MM)->computeLengthIntForLength(
					style(), m_rootElementStyle), Fixed);
}

Length CSSStyleSelector::inchLength(double inch) const {
	return Length(
			CSSPrimitiveValue::create(inch, CSSPrimitiveValue::CSS_IN)->computeLengthIntForLength(
					style(), m_rootElementStyle), Fixed);
}

void CSSStyleSelector::mapFillAttachment(CSSPropertyID, FillLayer* layer,
		CSSValue* value) {
...
void CSSStyleSelector::mapFillClip(CSSPropertyID, FillLayer* layer,
		CSSValue* value) {
	if (value->cssValueType() == CSSValue::CSS_INITIAL) {
...
void CSSStyleSelector::mapFillComposite(CSSPropertyID, FillLayer* layer,
		CSSValue* value) {
	if (value->cssValueType() == CSSValue::CSS_INITIAL) {
...
void CSSStyleSelector::mapFillOrigin(CSSPropertyID, FillLayer* layer,
		CSSValue* value) {
	if (value->cssValueType() == CSSValue::CSS_INITIAL) {
...
StyleImage* CSSStyleSelector::styleImage(CSSPropertyID property,
		CSSValue* value) {
	if (value->isImageValue())
		return cachedOrPendingFromValue(property,
				static_cast<CSSImageValue*>(value));
...
StyleImage* CSSStyleSelector::cachedOrPendingFromValue(CSSPropertyID property,
		CSSImageValue* value) {
...
void CSSStyleSelector::mapFillImage(CSSPropertyID property, FillLayer* layer,
		CSSValue* value) {
	if (value->cssValueType() == CSSValue::CSS_INITIAL) {
...
void CSSStyleSelector::mapFillRepeatX(CSSPropertyID, FillLayer* layer,
		CSSValue* value) {
	if (value->cssValueType() == CSSValue::CSS_INITIAL) {
...
void CSSStyleSelector::mapFillRepeatY(CSSPropertyID, FillLayer* layer,
		CSSValue* value) {
	if (value->cssValueType() == CSSValue::CSS_INITIAL) {
...
void CSSStyleSelector::mapFillSize(CSSPropertyID, FillLayer* layer,
		CSSValue* value) {
	if (!value->isPrimitiveValue()) {
	...
	if (value->cssValueType() == CSSValue::CSS_INITIAL
			|| primitiveValue->getIdent() == CSSValueContain
	...
	if (firstType == CSSPrimitiveValue::CSS_UNKNOWN)
		...
		firstLength = Length(
				first->computeLengthIntForLength(style(), m_rootElementStyle,
						zoomFactor), Fixed);
	...
	if (secondType == CSSPrimitiveValue::CSS_UNKNOWN)
		...
		secondLength = Length(
				second->computeLengthIntForLength(style(), m_rootElementStyle,
						zoomFactor), Fixed);
...
void CSSStyleSelector::mapFillXPosition(CSSPropertyID, FillLayer* layer,
		CSSValue* value) {
	if (value->cssValueType() == CSSValue::CSS_INITIAL) {
...
void CSSStyleSelector::mapFillYPosition(CSSPropertyID, FillLayer* layer,
		CSSValue* value) {
	if (value->cssValueType() == CSSValue::CSS_INITIAL) {
...
void CSSStyleSelector::mapAnimationDelay(Animation* animation,
		CSSValue* value) {
	if (value->cssValueType() == CSSValue::CSS_INITIAL) {
...
void CSSStyleSelector::mapAnimationDirection(Animation* layer,
		CSSValue* value) {
	if (value->cssValueType() == CSSValue::CSS_INITIAL) {
	...
	CSSPrimitiveValue* primitiveValue = static_cast<CSSPrimitiveValue*>(value);
	layer->setDirection(
			primitiveValue->getIdent() == CSSValueAlternate ?
					Animation::AnimationDirectionAlternate :
					Animation::AnimationDirectionNormal);
}

void CSSStyleSelector::mapAnimationDuration(Animation* animation,
		CSSValue* value) {
	if (value->cssValueType() == CSSValue::CSS_INITIAL) {
...
void CSSStyleSelector::mapAnimationIterationCount(Animation* animation,
		CSSValue* value) {
	if (value->cssValueType() == CSSValue::CSS_INITIAL) {
		animation->setIterationCount(
				Animation::initialAnimationIterationCount());
		return;
	}
...
void CSSStyleSelector::mapAnimationPlayState(Animation* layer,
		CSSValue* value) {
	if (value->cssValueType() == CSSValue::CSS_INITIAL) {
	...
	CSSPrimitiveValue* primitiveValue = static_cast<CSSPrimitiveValue*>(value);
	EAnimPlayState playState =
			(primitiveValue->getIdent() == CSSValuePaused) ?
					AnimPlayStatePaused : AnimPlayStatePlaying;
	layer->setPlayState(playState);
}

void CSSStyleSelector::mapAnimationProperty(Animation* animation,
		CSSValue* value) {
	if (value->cssValueType() == CSSValue::CSS_INITIAL) {
	...
	if (primitiveValue->getIdent() == CSSValueAll)
		...
		animation->setProperty(
				static_cast<CSSPropertyID>(primitiveValue->getIdent()));
}

void CSSStyleSelector::mapAnimationTimingFunction(Animation* animation,
		CSSValue* value) {
	if (value->cssValueType() == CSSValue::CSS_INITIAL) {
		animation->setTimingFunction(
				Animation::initialAnimationTimingFunction());
		return;
	}
	...
	if (value->isPrimitiveValue()) {
		...
		case CSSValueEaseOut:
			animation->setTimingFunction(
					CubicBezierTimingFunction::create(0.0, 0.0, 0.58, 1.0));
			break;
		case CSSValueEaseInOut:
			animation->setTimingFunction(
					CubicBezierTimingFunction::create(0.42, 0.0, 0.58, 1.0));
			break;
	...
	if (value->isTimingFunctionValue()) {
		if (value->isCubicBezierTimingFunctionValue()) {
			...
			animation->setTimingFunction(
					CubicBezierTimingFunction::create(cubicTimingFunction->x1(),
							cubicTimingFunction->y1(),
							cubicTimingFunction->x2(),
			...
			animation->setTimingFunction(
					StepsTimingFunction::create(
							stepsTimingFunction->numberOfSteps(),
...
void CSSStyleSelector::mapNinePieceImageSlice(CSSValue* value, NinePieceImage& image)
{
	...
	// Retrieve the border image value.
	CSSBorderImageSliceValue* borderImageSlice =
			static_cast<CSSBorderImageSliceValue*>(value);
...
LengthBox CSSStyleSelector::mapNinePieceImageQuad(CSSValue* value)
{
    ...
    if (slices->top()->primitiveType() == CSSPrimitiveValue::CSS_NUMBER)
        ...
        box.m_top = Length(slices->top()->computeLengthIntForLength(style(), rootElementStyle(), zoom), Fixed);
    ...
    if (slices->right()->primitiveType() == CSSPrimitiveValue::CSS_NUMBER)
        ...
        box.m_right = Length(slices->right()->getDoubleValue(CSSPrimitiveValue::CSS_PERCENTAGE), Percent);
          ...
          box.m_right = Length(slices->right()->computeLengthIntForLength(style(), rootElementStyle(), zoom), Fixed);
    ...
    if (slices->bottom()->primitiveType() == CSSPrimitiveValue::CSS_NUMBER)
        ...
        box.m_bottom = Length(slices->bottom()->computeLengthIntForLength(style(), rootElementStyle(), zoom), Fixed);
    ...
    if (slices->left()->primitiveType() == CSSPrimitiveValue::CSS_NUMBER)
        ...
        box.m_left = Length(slices->left()->computeLengthIntForLength(style(), rootElementStyle(), zoom), Fixed);
...
void CSSStyleSelector::checkForZoomChange(RenderStyle* style,
		RenderStyle* parentStyle) {
...
void CSSStyleSelector::checkForGenericFamilyChange(RenderStyle* style,
		RenderStyle* parentStyle) {
	const FontDescription& childFont = style->fontDescription();
	...
	if (childFont.keywordSize())
		size = fontSizeForKeyword(m_checker.m_document,
				CSSValueXxSmall + childFont.keywordSize() - 1,
				childFont.useFixedDefaultSize());
	else {
		Settings* settings = m_checker.m_document->settings();
...
void CSSStyleSelector::setFontSize(FontDescription& fontDescription,
		float size) {
	fontDescription.setSpecifiedSize(size);
	...
	fontDescription.setComputedSize(
			getComputedSizeFromSpecifiedSize(m_checker.m_document,
					m_style.get(), fontDescription.isAbsoluteSize(), size,
					useSVGZoomRules));
}

float CSSStyleSelector::getComputedSizeFromSpecifiedSize(Document* document,
		RenderStyle* style, bool isAbsoluteSize, float specifiedSize,
...
static const int quirksFontSizeTable[fontSizeTableMax - fontSizeTableMin + 1][totalKeywords] =
		{ { 9, 9, 9, 9, 11, 14, 18, 28 }, { 9, 9, 9, 10, 12, 15, 20, 31 }, { 9,
				9, 9, 11, 13, 17, 22, 34 }, { 9, 9, 10, 12, 14, 18, 24, 37 }, {
				9, 9, 10, 13, 16, 20, 26, 40 }, // fixed font default (13)
				{ 9, 9, 11, 14, 17, 21, 28, 42 }, { 9, 10, 12, 15, 17, 23, 30,
						45 }, { 9, 10, 13, 16, 18, 24, 32, 48 } // proportional font default (16)
		};
// HTML       1      2      3      4      5      6      7
...
static const int strictFontSizeTable[fontSizeTableMax - fontSizeTableMin + 1][totalKeywords] =
		{ { 9, 9, 9, 9, 11, 14, 18, 27 }, { 9, 9, 9, 10, 12, 15, 20, 30 }, { 9,
				9, 10, 11, 13, 17, 22, 33 }, { 9, 9, 10, 12, 14, 18, 24, 36 }, {
				9, 10, 12, 13, 16, 20, 26, 39 }, // fixed font default (13)
				{ 9, 10, 12, 14, 17, 21, 28, 42 }, { 9, 10, 13, 15, 18, 23, 30,
						45 }, { 9, 10, 13, 16, 18, 24, 32, 48 } // proportional font default (16)
		};
// HTML       1      2      3      4      5      6      7
// CSS  xxs   xs     s      m      l     xl     xxl
...
float CSSStyleSelector::fontSizeForKeyword(Document* document, int keyword,
	...
	if (mediumSize >= fontSizeTableMin && mediumSize <= fontSizeTableMax) {
		...
		return quirksMode ?
				quirksFontSizeTable[row][col] : strictFontSizeTable[row][col];
	}

	// Value is outside the range of the table. Apply the scale factor instead.
	float minLogicalSize = max(settings->minimumLogicalFontSize(), 1);
	return max(fontSizeFactors[keyword - CSSValueXxSmall] * mediumSize,
			minLogicalSize);
}

template<typename T>
static int findNearestLegacyFontSize(int pixelFontSize, const T* table,
		int multiplier) {
...
int CSSStyleSelector::legacyFontSize(Document* document, int pixelFontSize,
		bool shouldUseFixedDefaultSize) {
	Settings* settings = document->settings();
	if (!settings)
		return 1;
	...
	if (mediumSize >= fontSizeTableMin && mediumSize <= fontSizeTableMax) {
		int row = mediumSize - fontSizeTableMin;
		return findNearestLegacyFontSize<int>(pixelFontSize,
				quirksMode ?
						quirksFontSizeTable[row] : strictFontSizeTable[row], 1);
	}
...
static Color colorForCSSValue(int cssValueId) {
	...
	static const ColorValue colorValues[] = { { CSSValueAqua, 0xFF00FFFF }, {
			CSSValueBlack, 0xFF000000 }, { CSSValueBlue, 0xFF0000FF }, {
			CSSValueFuchsia, 0xFFFF00FF }, { CSSValueGray, 0xFF808080 }, {
			CSSValueGreen, 0xFF008000 }, { CSSValueGrey, 0xFF808080 }, {
			CSSValueLime, 0xFF00FF00 }, { CSSValueMaroon, 0xFF800000 }, {
			CSSValueNavy, 0xFF000080 }, { CSSValueOlive, 0xFF808000 }, {
			CSSValueOrange, 0xFFFFA500 }, { CSSValuePurple, 0xFF800080 }, {
			CSSValueRed, 0xFFFF0000 }, { CSSValueSilver, 0xFFC0C0C0 }, {
			CSSValueTeal, 0xFF008080 }, { CSSValueTransparent, 0x00000000 }, {
			CSSValueWhite, 0xFFFFFFFF }, { CSSValueYellow, 0xFFFFFF00 },
			{ 0, 0 } };
...
Color CSSStyleSelector::getColorFromPrimitiveValue(
		CSSPrimitiveValue* primitiveValue) const {
	Color col;
	...
	if (ident) {
		if (ident == CSSValueWebkitText)
			...
			col = m_element->isLink() && m_checker.m_matchVisitedPseudoClass ?
					m_element->document()->visitedLinkColor() :
					m_element->document()->linkColor();
...
bool CSSStyleSelector::hasSelectorForAttribute(
		const AtomicString &attrname) const {
	return m_selectorAttrs.contains(attrname.impl());
}

void CSSStyleSelector::addViewportDependentMediaQueryResult(
		const MediaQueryExp* expr, bool result) {
	m_viewportDependentMediaQueryResults.append(
			new MediaQueryResult(*expr, result));
}
...
bool CSSStyleSelector::affectedByViewportChange() const {
	...
	for (unsigned i = 0; i < s; i++) {
		if (m_medium->eval(
				&m_viewportDependentMediaQueryResults[i]->m_expression)
				!= m_viewportDependentMediaQueryResults[i]->m_result)
			return true;
	}
	return false;
}
...
void CSSStyleSelector::SelectorChecker::visitedStateChanged(
		LinkHash visitedHash) {
	if (!m_linksCheckedForVisitedState.contains(visitedHash))
		return;
	for (Node* node = m_document; node; node = node->traverseNextNode()) {
		const AtomicString* attr = linkAttribute(node);
		if (attr
				&& visitedLinkHash(m_document->baseURL(), *attr) == visitedHash)
			node->setNeedsStyleRecalc();
	}
}

static TransformOperation::OperationType getTransformOperationType(
		WebKitCSSTransformValue::TransformOperationType type) {
	switch (type) {
...
bool CSSStyleSelector::createTransformOperations(CSSValue* inValue,
	...
	for (unsigned i = 0; i < size; i++) {
		...
		WebKitCSSTransformValue* transformValue =
				static_cast<WebKitCSSTransformValue*>(list->itemWithoutBoundsCheck(
						i));
		...
		CSSPrimitiveValue* firstValue =
				static_cast<CSSPrimitiveValue*>(transformValue->itemWithoutBoundsCheck(
						0));
		...
		case WebKitCSSTransformValue::ScaleYTransformOperation: {
			double sx = 1.0;
			double sy = 1.0;
			if (transformValue->operationType()
					== WebKitCSSTransformValue::ScaleYTransformOperation)
				sy = firstValue->getDoubleValue();
			else {
				sx = firstValue->getDoubleValue();
				if (transformValue->operationType()
						!= WebKitCSSTransformValue::ScaleXTransformOperation) {
			...
			operations.operations().append(
					ScaleTransformOperation::create(sx, sy, 1.0,
							getTransformOperationType(
									transformValue->operationType())));
			break;
		}
		case WebKitCSSTransformValue::ScaleZTransformOperation:
		case WebKitCSSTransformValue::Scale3DTransformOperation: {
			...
			double sz = 1.0;
			if (transformValue->operationType()
					== WebKitCSSTransformValue::ScaleZTransformOperation)
				sz = firstValue->getDoubleValue();
			else if (transformValue->operationType()
					== WebKitCSSTransformValue::ScaleYTransformOperation)
				sy = firstValue->getDoubleValue();
			else {
				sx = firstValue->getDoubleValue();
			...
			operations.operations().append(
					ScaleTransformOperation::create(sx, sy, sz,
							getTransformOperationType(
									transformValue->operationType())));
			break;
		}
		...
		case WebKitCSSTransformValue::TranslateYTransformOperation: {
			...
			if (transformValue->operationType()
					== WebKitCSSTransformValue::TranslateYTransformOperation)
				ty = convertToFloatLength(firstValue, style, rootStyle,
						zoomFactor, &ok);
			else {
				tx = convertToFloatLength(firstValue, style, rootStyle,
						zoomFactor, &ok);
				if (transformValue->operationType()
						!= WebKitCSSTransformValue::TranslateXTransformOperation) {
					if (transformValue->length() > 1) {
						CSSPrimitiveValue* secondValue =
								static_cast<CSSPrimitiveValue*>(transformValue->itemWithoutBoundsCheck(
										1));
						ty = convertToFloatLength(secondValue, style, rootStyle,
								zoomFactor, &ok);
					}
				}
			}
			...
			operations.operations().append(
					TranslateTransformOperation::create(tx, ty,
							Length(0, Fixed),
							getTransformOperationType(
									transformValue->operationType())));
			break;
		...
		case WebKitCSSTransformValue::Translate3DTransformOperation: {
			...
			if (transformValue->operationType()
					== WebKitCSSTransformValue::TranslateZTransformOperation)
				tz = convertToFloatLength(firstValue, style, rootStyle,
						zoomFactor, &ok);
			else if (transformValue->operationType()
					== WebKitCSSTransformValue::TranslateYTransformOperation)
				ty = convertToFloatLength(firstValue, style, rootStyle,
				...
				if (transformValue->operationType()
					...
					if (transformValue->length() > 2) {
						CSSPrimitiveValue* thirdValue =
								static_cast<CSSPrimitiveValue*>(transformValue->itemWithoutBoundsCheck(
										2));
						tz = convertToFloatLength(thirdValue, style, rootStyle,
								zoomFactor, &ok);
					}
					if (transformValue->length() > 1) {
						CSSPrimitiveValue* secondValue =
			...
			operations.operations().append(
					TranslateTransformOperation::create(tx, ty, tz,
							getTransformOperationType(
									transformValue->operationType())));
			break;
		}
		case WebKitCSSTransformValue::RotateTransformOperation: {
			...
			operations.operations().append(
					RotateTransformOperation::create(0, 0, 1, angle,
							getTransformOperationType(
									transformValue->operationType())));
			break;
		}
		...
		case WebKitCSSTransformValue::RotateZTransformOperation: {
			...
			if (transformValue->operationType()
					== WebKitCSSTransformValue::RotateXTransformOperation)
				x = 1;
			else if (transformValue->operationType()
					== WebKitCSSTransformValue::RotateYTransformOperation)
				y = 1;
			else
				z = 1;
			operations.operations().append(
					RotateTransformOperation::create(x, y, z, angle,
							getTransformOperationType(
									transformValue->operationType())));
			break;
		}
		case WebKitCSSTransformValue::Rotate3DTransformOperation: {
			...
			if (fourthValue->primitiveType() == CSSPrimitiveValue::CSS_RAD)
				angle = rad2deg(angle);
			else if (fourthValue->primitiveType()
		...
		case WebKitCSSTransformValue::SkewYTransformOperation: {
			...
			if (firstValue->primitiveType() == CSSPrimitiveValue::CSS_RAD)
				...
				angle = turn2deg(angle);
			if (transformValue->operationType()
					== WebKitCSSTransformValue::SkewYTransformOperation)
				angleY = angle;
			else {
				angleX = angle;
				if (transformValue->operationType()
						== WebKitCSSTransformValue::SkewTransformOperation) {
					if (transformValue->length() > 1) {
						CSSPrimitiveValue* secondValue =
								static_cast<CSSPrimitiveValue*>(transformValue->itemWithoutBoundsCheck(
										1));
						angleY = secondValue->getDoubleValue();
						if (secondValue->primitiveType()
								== CSSPrimitiveValue::CSS_RAD)
							angleY = rad2deg(angleY);
						else if (secondValue->primitiveType()
								== CSSPrimitiveValue::CSS_GRAD)
							angleY = grad2deg(angleY);
						else if (secondValue->primitiveType()
								== CSSPrimitiveValue::CSS_TURN)
							angleY = turn2deg(angleY);
					}
				}
			}
			operations.operations().append(
					SkewTransformOperation::create(angleX, angleY,
							getTransformOperationType(
									transformValue->operationType())));
			break;
		}
		...
		case WebKitCSSTransformValue::PerspectiveTransformOperation: {
			...
			if (CSSPrimitiveValue::isUnitTypeLength(
					firstValue->primitiveType()))
				p = convertToFloatLength(firstValue, style, rootStyle,
						zoomFactor, &ok);
			else {
...
void CSSStyleSelector::loadPendingImages() {
	...
	for (HashSet<int>::const_iterator it = m_pendingImageProperties.begin();
			it != end; ++it) {
		CSSPropertyID currentProperty = static_cast<CSSPropertyID>(*it);

		CachedResourceLoader* cachedResourceLoader =
				m_element->document()->cachedResourceLoader();

		switch (currentProperty) {
		case CSSPropertyBackgroundImage: {
			for (FillLayer* backgroundLayer = m_style->accessBackgroundLayers();
					backgroundLayer; backgroundLayer =
							backgroundLayer->next()) {
				if (backgroundLayer->image()
						&& backgroundLayer->image()->isPendingImage()) {
					CSSImageValue* imageValue =
							static_cast<StylePendingImage*>(backgroundLayer->image())->cssImageValue();
					backgroundLayer->setImage(
							imageValue->cachedImage(cachedResourceLoader));
				}
			}
			break;
		}
		...
		case CSSPropertyContent: {
			for (ContentData* contentData =
					const_cast<ContentData*>(m_style->contentData());
					contentData; contentData = contentData->next()) {
				if (contentData->isImage()
						&& contentData->image()->isPendingImage()) {
					CSSImageValue* imageValue =
							static_cast<StylePendingImage*>(contentData->image())->cssImageValue();
					if (StyleCachedImage* cachedImage = imageValue->cachedImage(
				...
				for (size_t i = 0; i < cursorList->size(); ++i) {
					...
					if (StyleImage* image = currentCursor.image()) {
						if (image->isPendingImage()) {
							CSSImageValue* imageValue =
									static_cast<StylePendingImage*>(image)->cssImageValue();
							currentCursor.setImage(
									imageValue->cachedImage(
											cachedResourceLoader));
						}
					}
				}
		...
		case CSSPropertyListStyleImage: {
			if (m_style->listStyleImage()
					&& m_style->listStyleImage()->isPendingImage()) {
				CSSImageValue* imageValue =
						static_cast<StylePendingImage*>(m_style->listStyleImage())->cssImageValue();
				m_style->setListStyleImage(
						imageValue->cachedImage(cachedResourceLoader));
			}
			break;
