...
open ALIAS, "<CSSPropertyAliasNames.in" || die "Could not find CSSPropertyAliasNames.in";
my @aliasNames = ();
while (<ALIAS>) {
  next if (m/(^#)|(^\s*$)/);
  # Input may use a different EOL sequence than $/, so avoid chomp.
  #$_ =~ s/[\r\n]+$//g;
  chomp($_);
  my @tmp_alias = split(/ /, $_);
  push @aliasNames, @tmp_alias;
}
close(ALIAS);
...
for(my $i=0; $i<=$#aliasNames; $i+=2)
{
  my $name = $aliasNames[$i];
  my $id = $aliasNames[$i+1];
  $id =~ s/(^[^-])|-(.)/uc($1||$2)/ge;
  print GPERF $name . ", CSSProperty" . $id . "\n";
}
