...
static const int computedProperties[] = {
    ...
    CSSPropertyBorderImageOutset,	//M+: CSS3 border image support
...
static PassRefPtr<CSSBorderImageSliceValue> valueForNinePieceImageSlice(const NinePieceImage& image, CSSPrimitiveValueCache* primitiveValueCache)
{
    ...
    PassRefPtr<CSSBorderImageSliceValue> res = CSSBorderImageSliceValue::create(primitiveValueCache->createValue(quad.release()), image.fill());
    XLOGD2("CSSBorderImageSliceValue), %s", res->cssText().utf8().data());
    return res;
}
...
static PassRefPtr<CSSValue> valueForNinePieceImageRepeat(const NinePieceImage& image, CSSPrimitiveValueCache* primitiveValueCache)
{
    ...
    PassRefPtr<CSSValue> res = primitiveValueCache->createValue(Pair::create(horizontalRepeat.release(), verticalRepeat.release()));
    XLOGD2("valueForNinePieceImageRepeat), %s", res->cssText().utf8().data());
    return res;
}
