...
{
    /// force check ancestor's z-order
    RenderLayer* stackingLayer = parent();
    while (stackingLayer) {
        int pos = 1;
        ...
        while (zOrderList && pos > 0) {
            ...
            for (size_t i = 0; i < listSize; ++i) {
                if (zOrderList->at(i) == oldChild) {
                    /// dirty current renderLayer's z-order, and ask repaint
                    dirtyZOrderLists();
                    dirtyStackingContextZOrderLists();
                    setNeedsFullRepaint();
                    /// dirty parentLayer's z-order, and ask repaint
                    stackingLayer->dirtyZOrderLists();
                    stackingLayer->dirtyStackingContextZOrderLists();
                    stackingLayer->setNeedsFullRepaint();
                    stackingLayer->clearClipRectsIncludingDescendants();
                    return;
                }
            }
            if (2 == pos)
        ...
        stackingLayer = stackingLayer->parent();
    }
}
