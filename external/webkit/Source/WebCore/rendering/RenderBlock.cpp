...
void RenderBlock::styleDidChange(StyleDifference diff, const RenderStyle* oldStyle)
{
    ...
    /// M: Fix Youtube NE for ALPS00416701 @{
    ///   child->setStyle will cause styleDidChange -> childBecameNonInline -> removeLeftoverAnonymousBlock
    ///   the renderBlock destroyed by child.
    if (m_beingDestroyed)
        return;
    /// @}
...
{
/// M: Multiline MenuList already drawn in paintBoxDecorations(), skipping paintChildren() @{
#if ENABLE(NO_LISTBOX_RENDERING)
    if (isMenuList() && node() && node()->hasTagName(HTMLNames::selectTag)) {
