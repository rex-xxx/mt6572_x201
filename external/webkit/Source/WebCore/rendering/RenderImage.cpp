...
void RenderImage::paintReplaced(PaintInfo& paintInfo, int tx, int ty)
{
    ...
    if (!m_imageResource->hasImage() || m_imageResource->errorOccurred()) {
        ...
        if (cWidth > 2 && cHeight > 2) {
            ...
            /// M: Calculate the text size @{
            bool drawAltText = false;
            int textHeight = 0;
            if (!m_altText.isEmpty()) {
            ...
            if (m_imageResource->errorOccurred() && !image->isNull() && usableWidth >= image->width() && usableHeight >= image->height()) {
                ...
                /// M: Check if the image and alt text can draw together @{
                if (!drawAltText || textHeight <= imageY) {
            ...
            if (!m_altText.isEmpty()) {
                ...
                if (errorPictureDrawn) {
                    if (cWidth >= textWidth && fontMetrics.height() <= imageY)  /// M: use cWidth instead of usableWidth
                        context->drawText(font, textRun, IntPoint(ax, ay + ascent));
                } else if (cWidth >= textWidth && cHeight >= fontMetrics.height())  /// M: use cWidth instead of usableWidth
                    context->drawText(font, textRun, IntPoint(ax, ay + ascent));
            }
        }
