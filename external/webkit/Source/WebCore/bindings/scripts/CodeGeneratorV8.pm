...
END
    ...
    if ($implClassName eq "DOMWindow" || $dataNode->extendedAttributes->{"CheckDomainSecurity"}) {
        push(@implContentDecls, <<END);
    ${implClassName}* imp = V8${implClassName}::toNative(info.Holder());
...
sub GenerateSingleBatchedAttribute
{
    ...
    if ($attribute->signature->type =~ /Constructor$/) {
        ...
        $setter = "${interfaceName}Internal::${interfaceName}ReplaceableAttrSetter";
    } else {
...
END
    ...
    use File::Basename;
    my $currentDir = dirname($0);
    my $mkFileName = "$currentDir/../../Android.derived.v8bindings.mk";
    my $workerOption = "WORKERS\=1";

    my $workerSupport = 0;
    open FP, "<$mkFileName" or die "Couldn't open file $mkFileName";
    while(<FP>) {
        chomp;
        if (/$workerOption/) {
            $workerSupport = 1;
