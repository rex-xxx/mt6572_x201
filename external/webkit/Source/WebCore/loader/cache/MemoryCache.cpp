...
#define MEMORY_CACHE_ARGS(s) s.count, s.size, s.liveSize, s.decodedSize, s.purgeableSize, s.purgedSize 
void MemoryCache::dumpStats()
{
    Statistics s = getStatistics();
    DUMP_MEM_USAGE_LOG("%-13s %-13s %-13s %-13s %-13s %-13s %-13s\n", "", "Count", "Size", "LiveSize", "DecodedSize", "PurgeableSize", "PurgedSize");
    DUMP_MEM_USAGE_LOG("%-13s %-13s %-13s %-13s %-13s %-13s %-13s\n", "-------------", "-------------", "-------------", "-------------", "-------------", "-------------", "-------------");
    DUMP_MEM_USAGE_LOG("%-13s %13d %13d %13d %13d %13d %13d\n", "Images", MEMORY_CACHE_ARGS(s.images));
    DUMP_MEM_USAGE_LOG("%-13s %13d %13d %13d %13d %13d %13d\n", "CSS", MEMORY_CACHE_ARGS(s.cssStyleSheets));
#if ENABLE(XSLT)
    DUMP_MEM_USAGE_LOG("%-13s %13d %13d %13d %13d %13d %13d\n", "XSL", MEMORY_CACHE_ARGS(s.xslStyleSheets));
#endif
    DUMP_MEM_USAGE_LOG("%-13s %13d %13d %13d %13d %13d %13d\n", "JavaScript", MEMORY_CACHE_ARGS(s.scripts));
    DUMP_MEM_USAGE_LOG("%-13s %13d %13d %13d %13d %13d %13d\n", "Fonts", MEMORY_CACHE_ARGS(s.fonts));
    DUMP_MEM_USAGE_LOG("%-13s %-13s %-13s %-13s %-13s %-13s %-13s\n\n", "-------------", "-------------", "-------------", "-------------", "-------------", "-------------", "-------------");
}
#ifndef NDEBUG
