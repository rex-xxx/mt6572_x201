...
#if USE(ACCELERATED_COMPOSITING)
        view()->updateCompositingLayers();
        /// M: add layer check, some timing this function called when full screen layer didn't create for ALPS00416701
        if (m_fullScreenRenderer->hasLayer() && m_fullScreenRenderer->layer()->isComposited())
            page()->chrome()->client()->setRootFullScreenLayer(m_fullScreenRenderer->layer()->backing()->graphicsLayer());
#endif
    }
}
...
void Document::setFullScreenRenderer(RenderFullScreen* renderer)
{
    if (renderer == m_fullScreenRenderer)
        return;
    /// M: There is a time interval between m_fullScreenRenderer being released but not being rest as 0.
    /// M: Add beingDestroyed() checking to avoid released twice.
    if (m_fullScreenRenderer && !m_fullScreenRenderer->beingDestroyed())
...
/// M: enable HTML5 features - request animation @{
