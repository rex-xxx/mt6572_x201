...
public class ClassInfo extends DocInfo implements ContainerInfo, Comparable, Scoped, Resolvable {
  ...
  private static void gatherHiddenInterfaces(ClassInfo cl, HashSet<ClassInfo> interfaces) {
    for (ClassInfo iface : cl.mRealInterfaces) {
    	if (iface==null) {
    		System.out.println("iface==null gatherHiddenInterfaces for Class: " + cl.name());
    		continue;
    	}
      if (iface.checkLevel()) {
  ...
  public boolean isConsistent(ClassInfo cl) {
    ...
    for (ClassInfo iface : mRealInterfaces) {
    	if (iface==null)
    		System.err.println("iface==null: "+cl.name());
      if (iface!=null&& !implementsInterface(cl, iface.mQualifiedName)) {
    ...
    for (ClassInfo iface : cl.mRealInterfaces) {
    	if (iface==null)
    		System.err.println("iface==null: "+cl.name());
      if (iface!=null && !implementsInterface(this, iface.mQualifiedName)) {
