...
struct wpa_driver_ops {
	 ...
	 */
	 int (*driver_cmd)(void *priv, char *cmd, char *buf, size_t buf_len);
	  /**
      * driver_sw_cmd - execute driver-sw specific command
      * @priv: private driver interface data from init()
      * @set: set or get
      * @adr: the sw cmd
      * @dat: the write value
      ...
      *
      */
	 int (*driver_sw_cmd)(void *priv, int set, unsigned long *adr, unsigned long *dat);
#endif
	/**
	...
	int (*switch_channel)(void *priv, unsigned int freq);
	    /**
        * set_test_mode - Setting the test mode 
        * @priv: private driver interface data
        * Returns: 0 on success, -1 on failure
        */
    int (*set_test_mode)(void *priv, const u8 *buf, size_t buf_len);
    
#ifdef CONFIG_WAPI_SUPPORT
    /**
     * msg_send - send some information to driver
     * @priv: private driver interface data from init()
