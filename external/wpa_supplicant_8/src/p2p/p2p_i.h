...
struct p2p_data {
	...
	struct wfd_data_s *wfd_params; /* pointer to global */
	struct wfd_data_s *wfd_params_cmd; /* pointer to global */
#endif
...
#ifdef CONFIG_MTK_WFD
    /* WFD IE */
    const u8 *wfd_device_info;
    const u8 *wfd_associated_bssid;
    const u8 *wfd_coupled_sink_info;
    const u8 *wfd_session_info;
    size_t    wfd_session_info_len;
    const u8 *wfd_ext_capa_bitmap_info;
#endif
...
/* CONFIG_MTK_WFD */
struct wfd_session_info {
	unsigned int num_clients;
	struct wfd_client_info {
		const u8 *wfd_device_addr;
		u16 wfd_dev_info;
		u16 wfd_max_throughput;        
   		const u8 *wfd_associated_bssid;
		u8 wfd_sink_status;
   		const u8 *wfd_sink_mac;
	} client[P2P_MAX_GROUP_ENTRIES];
};
