...
static int wfd_add_session_clients(struct p2p_data *p2p, const u8 *go_dev_addr,
				 const u8 *go_interface_addr, int freq,
				 const u8 *si, size_t si_len)
{
	...
	for (c = 0; c < info.num_clients; c++) {
		struct wfd_client_info *cli = &info.client[c];
            dev = p2p_get_device(p2p, cli->wfd_device_addr);
            if (dev) {
                /*
                 * Update information only if we have not received this
                 * directly from the client.
                 */
                dev->info.wfd_data_params.WfdEnable = 1;
                dev->info.wfd_data_params.WfdDevInfo = cli->wfd_dev_info;
                dev->info.wfd_data_params.WfdControlPort = 0;
                dev->info.wfd_data_params.WfdMaximumTp = cli->wfd_max_throughput;
                dev->info.wfd_data_params.WfdCoupleSinkStatus= cli->wfd_sink_status;

                if(cli->wfd_sink_mac) {
                    os_memcpy(dev->info.wfd_data_params.WfdCoupleSinkAddress, 
                        cli->wfd_sink_mac, ETH_ALEN);
                }
                
                if(cli->wfd_associated_bssid) {
                    os_memcpy(dev->info.wfd_data_params.WfdAssociatedBssid, 
                        cli->wfd_associated_bssid, ETH_ALEN);
                }
...
#endif
{
	...
	for (c = 0; c < info.num_clients; c++) {
		...
		os_memcpy(dev->member_in_go_iface, go_interface_addr,
			  ETH_ALEN);
#ifdef CONFIG_MTK_WFD
        wfd_add_session_clients(p2p, go_dev_addr, go_interface_addr, freq, si, si_len);
#endif
		   ...
		   const u8 *ies, size_t ies_len, int scan_res)
{
...
#ifdef CONFIG_MTK_WFD
    /* Eddie */
    /* Detect the GO->DEV DEV->GO */
	u8 dev_capab_orig = dev->info.dev_capab;
    u8 group_capab_orig =  dev->info.group_capab;
    /* Eddie end */
#endif
...
#ifdef CONFIG_MTK_WFD
	/* Eddie */
    if(dev_capab_orig != dev->info.dev_capab
       || group_capab_orig !=dev->info.group_capab
            ) {
        dev->flags &= ~P2P_DEV_REPORTED;
    }
    /* Eddie End */
    ...
    /* Eddie set interface_addr if peer is not device */
    /* */
	if( dev->info.group_capab & P2P_GROUP_CAPAB_GROUP_OWNER) {
        os_memcpy(dev->interface_addr, addr, ETH_ALEN);
        if(dev->oper_freq != freq) {
            /* Eddie operation channel for multi channel */
            wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG,
                "P2P: Update Operating frequency based on scan "
                "results (" MACSTR " %d -> %d MHz (DS param %d)",
                MAC2STR(dev->info.p2p_device_addr), dev->oper_freq,
                freq, msg.ds_params ? *msg.ds_params : -1);
            dev->oper_freq = freq;
        }
    }
	...
	if (scan_res) {
        p2p_add_group_clients(p2p, p2p_dev_addr, addr, freq, msg.group_info,
                      msg.group_info_len, msg.wfd_session_info,msg.wfd_session_info_len);
    }
#else
	if (scan_res) {
...
#ifdef CONFIG_MTK_WFD
    p2p_copy_wfd_info(dev, 0, &msg, BIT(0));
#endif
	...
	p2p->cfg->dev_found(p2p->cfg->cb_ctx, addr, &dev->info,
			    !(dev->flags & P2P_DEV_REPORTED_ONCE),dev);
#else
	p2p->cfg->dev_found(p2p->cfg->cb_ctx, addr, &dev->info,
...
#ifdef CONFIG_MTK_WFD
    p2p_copy_wfd_info(dev, 1, &msg, BIT(0));
#endif
	if (msg.wfd_subelems) {
		wpabuf_free(dev->info.wfd_subelems);
		dev->info.wfd_subelems = wpabuf_dup(msg.wfd_subelems);
		...
		const u8 *bssid, const u8 *ie, size_t ie_len)
{
	...
	if(p2p->num_groups > 0 && !wpas_p2p_is_client(p2p->cfg->msg_ctx)) {
        /* not in GO mode - ignore Probe Request */
    ...
    /* Sigma 4.1.3 Not allowed to send probe resp to the 11b only device */
	if (elems.supp_rates && elems.supp_rates_len) {
        for(i = 0; i < elems.supp_rates_len; i++) {
        	if(elems.supp_rates[i] != 2 && elems.supp_rates[i] != 4 && 
				elems.supp_rates[i] != 11 && elems.supp_rates[i] != 22 ) {
				is_11b_only = 0;
				break;
        	}
        }
	 ...
	 * parameters.
	 */
	if(!wpa_supplicant_hotspot_cipher_tkip(p2p->cfg->msg_ctx))
	    ies = p2p_build_probe_resp_ies(p2p);
	if (ies == NULL)
		return P2P_PREQ_NOT_PROCESSED;
...
#ifdef CONFIG_MTK_WFD
int wfd_ie_text(struct wpabuf *wfd_ie, char *buf, char *end)
{
	return wfd_attr_text(wfd_ie, buf, end);
}

int wfd_ie_text_append(struct wpabuf *wfd_ie, char *buf, char *end, unsigned int flags)
{
	return wfd_attr_text_append(wfd_ie, buf, end, flags);
}
#endif
...
int p2p_get_oper_freq(struct p2p_data *p2p, const u8 *iface_addr)
{
	...
	if (dev == NULL) {
        /* Eddie */
		wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG, "P2P: p2p_get_oper_freq: " MACSTR "is not found\n", MAC2STR(iface_addr));
		return -1;
    }
	if (dev->oper_freq <= 0) {
        /* Eddie */
		wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG, "P2P: p2p_get_oper_freq: freq %d is invalid\n", dev->oper_freq);
		return -1;
    }
...
#ifdef CONFIG_MTK_WFD
int wfd_process_request_by_policy(struct p2p_data *p2p, 
                             struct p2p_message *msg, u8 *status, unsigned int flags)
{
    u8 res_status = P2P_SC_FAIL_INVALID_PARAMS;


    if(p2p->wfd_params) {

            wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG, 
                 "WFD: wfd_process_request_by_policy wfd_en %u", p2p->wfd_params->WfdEnable);

        if(p2p->wfd_params->WfdEnable) {

            if(p2p->wfd_params->WfdPolicy & (1<<6)){
                res_status = P2P_SC_FAIL_REJECTED_BY_USER;
        		wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG,
        			"WFD: device info attribute is worng "
        			"Reject all");                 
                goto fail;
            }
            if(p2p->wfd_params->WfdPolicy & (1<<5)){
                if(msg->wfd_subelems){
        		wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG,
        			"WFD: device info attribute is worng "
        			"Reject with WFD IE");                    
            ...
            if(p2p->wfd_params->WfdPolicy & (1<<0)){
                if(msg->wfd_subelems == NULL ||  msg->wfd_device_info == NULL){
            		wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG,
        			"WFD: device info attribute is worng "
        			"Reject without WFD IE");                    
                    res_status = P2P_SC_FAIL_REJECTED_BY_USER;
                    goto fail;
                }
            }
            if(p2p->wfd_params->WfdPolicy & (1<<1)){
                if(msg->wfd_subelems && msg->wfd_device_info){
                    P_WFD_ATTRI_WFD_DEV_INFO_T prWfdDevInfo;
                    u16                        u2DevInfo;
                    prWfdDevInfo = (P_WFD_ATTRI_WFD_DEV_INFO_T) msg->wfd_device_info;
                    u2DevInfo = WPA_GET_BE16((u8 *)(&prWfdDevInfo->u2DevInfo));
                    if((u2DevInfo &0x3) == 0x0 /* Source*/) {
                		wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG,
                			"WFD: device info attribute is worng "
                			"Source");                        
                        res_status = P2P_SC_FAIL_REJECTED_BY_USER;
            ...
            if(p2p->wfd_params->WfdPolicy & (1<<2)){
                if(msg->wfd_subelems &&msg->wfd_device_info){
                    P_WFD_ATTRI_WFD_DEV_INFO_T prWfdDevInfo;
                    u16                        u2DevInfo;
                    prWfdDevInfo = (P_WFD_ATTRI_WFD_DEV_INFO_T) msg->wfd_device_info;
                    u2DevInfo = WPA_GET_BE16((u8 *)(&prWfdDevInfo->u2DevInfo));
                    if((u2DevInfo &0x3) == 0x01 /* Primary sink*/) {
                		wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG,
                			"WFD: device info attribute is worng "
                			"Primary sink");                        
                        res_status = P2P_SC_FAIL_REJECTED_BY_USER;
            ...
            if(p2p->wfd_params->WfdPolicy & (1<<3)){
                if(msg->wfd_subelems && msg->wfd_device_info){
                    P_WFD_ATTRI_WFD_DEV_INFO_T prWfdDevInfo;
                    u16                        u2DevInfo;
                    prWfdDevInfo = (P_WFD_ATTRI_WFD_DEV_INFO_T) msg->wfd_device_info;
                    u2DevInfo = WPA_GET_BE16((u8 *)(&prWfdDevInfo->u2DevInfo));
                    if((u2DevInfo &0x3) == 0x02 /* Secondary sink*/) {
                		wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG,
                			"WFD: device info attribute is worng "
                			"Secondary sink");                        
                        res_status = P2P_SC_FAIL_REJECTED_BY_USER;
            ...
            if(p2p->wfd_params->WfdPolicy & (1<<4)){
                if(msg->wfd_subelems && msg->wfd_device_info){
                    P_WFD_ATTRI_WFD_DEV_INFO_T prWfdDevInfo;
                    u16                        u2DevInfo;
                    prWfdDevInfo = (P_WFD_ATTRI_WFD_DEV_INFO_T) msg->wfd_device_info;
                    u2DevInfo = WPA_GET_BE16((u8 *)(&prWfdDevInfo->u2DevInfo));
                    if((u2DevInfo &0x3) == 0x03 /* Source, Primary sink*/) {
                		wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG,
                			"WFD: device info attribute is worng "
                			"Source, Primary sink");                        
                        res_status = P2P_SC_FAIL_REJECTED_BY_USER;
...
int wfd_process_request_and_switch_role(struct p2p_data *p2p, 
                             struct p2p_message *msg, unsigned int flags)
{
    ...
    if(p2p->wfd_params) {
       /* wpa_s->wfd_data */
        ...
        wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG, 
                 "WFD: wfd_process_request_and_switch_role wfd_en %u %p %p", 
                 p2p->wfd_params->WfdEnable, msg->wfd_subelems, msg->wfd_device_info);
        ...
        if(p2p->wfd_params->WfdEnable) {
            if(msg->wfd_subelems && msg->wfd_device_info){
                ...
                wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG, 
                            "WFD: My dev info is 0x%x. Peer is %x", p2p->wfd_params->WfdDevInfo, u2DevInfo);

                if((p2p->wfd_params->WfdDevInfo & 0x3) == 0x3) {
                    /* I am dual role */
                    if((u2DevInfo &0x3) == 0x00 /* Peer is Source*/) {
                        wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG, 
                            "WFD: Switch role to primary sink ");

                        p2p->wfd_params->WfdDevInfo &= ~0x3;
                        p2p->wfd_params->WfdDevInfo |= 0x1;

                        p2p->wfd_params_cmd->WfdDevInfo &= ~0x3;
                        p2p->wfd_params_cmd->WfdDevInfo |= 0x1;

                        wpas_wfd_data_update(p2p->cfg->cb_ctx, p2p->wfd_params);
                        /* event to annonce the role is chanaged to P-Sink */
                        //wpa_msg(p2p->cfg->msg_ctx, MSG_INFO, WFD_EVENT_AUTO_SWITCH_ROLE "role=1");

             
                    }
                    else if((u2DevInfo &0x3) == 0x01 /* Peer is P-Sink */) {
                        wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG,
                            "WFD: Switch role to source ");

                        p2p->wfd_params->WfdDevInfo &= ~0x3;
                        p2p->wfd_params_cmd->WfdDevInfo &= ~0x3;
                        wpas_wfd_data_update(p2p->cfg->cb_ctx, p2p->wfd_params);
                        /* event to annonce the role is chanaged to Source */
                        //wpa_msg(p2p->cfg->msg_ctx, MSG_INFO, WFD_EVENT_AUTO_SWITCH_ROLE "role=0");

                   }
                   else {

                        wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG,
                            "WFD: Peer role is wrong type(%u) ", (u2DevInfo &0x3));
                        wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG,
                            "WFD: Switch role to source ");
               ...
               if((p2p->wfd_params->WfdDevInfo & 0x3) == 0x0) {
                     wpa_msg(p2p->cfg->msg_ctx, MSG_INFO, WFD_EVENT_AUTO_SWITCH_ROLE "role=0");
               }
               else if((p2p->wfd_params->WfdDevInfo & 0x3) == 0x1) {
                     wpa_msg(p2p->cfg->msg_ctx, MSG_INFO, WFD_EVENT_AUTO_SWITCH_ROLE "role=1");
               }
               else {

                    wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG,
                         "WFD: Final our role is wrong type(%u) ", ( p2p->wfd_params->WfdDevInfo & 0x3));
                     wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG,
                         "WFD: Switch role to source ");
                    p2p->wfd_params->WfdDevInfo &= ~0x3;
                    p2p->wfd_params_cmd->WfdDevInfo &= ~0x3;
...
void p2p_copy_wfd_info(struct p2p_device *dev, int probe_req,
			      const struct p2p_message *msg, unsigned int flags)
{
    P_WFD_ATTRI_WFD_DEV_INFO_T prWfdDevInfo;
    ...
    if(msg->wfd_device_info) {
	    prWfdDevInfo = (P_WFD_ATTRI_WFD_DEV_INFO_T) msg->wfd_device_info;
	    dev->info.wfd_data_params.WfdDevInfo = WPA_GET_BE16((u8 *)(&prWfdDevInfo->u2DevInfo));
	    dev->info.wfd_data_params.WfdControlPort = WPA_GET_BE16((u8*)(&prWfdDevInfo->u2ControlPort)); 	
	    dev->info.wfd_data_params.WfdMaximumTp = WPA_GET_BE16((u8*)(&prWfdDevInfo->u2MaximumTp));
        dev->info.wfd_data_params.WfdEnable = 1;
        wfdFlags |= WFD_FLAG_DEV_INFO_VALID;
        if(flags & BIT(0)) {
            dev->info.wfd_data_params.WfdDevInfoProbeRsp = WPA_GET_BE16((u8 *)(&prWfdDevInfo->u2DevInfo));
        }
    }
	else {
        dev->info.wfd_data_params.WfdEnable = 0;
		dev->info.wfd_data_params.WfdDevInfo = 0;
    }


    if(msg->wfd_associated_bssid) {
		prWfdAssociatedBssid  = (P_WFD_ATTRI_ASSOCIATED_BSSID_T) msg->wfd_associated_bssid;
		os_memcpy(dev->info.wfd_data_params.WfdAssociatedBssid, prWfdAssociatedBssid->aucAssociatedBssid, ETH_ALEN);
        wfdFlags |= WFD_FLAG_ASSOC_BSSID_VALID;
	}
	else {
		os_memset(dev->info.wfd_data_params.WfdAssociatedBssid, 0, ETH_ALEN);

	}

    if(msg->wfd_coupled_sink_info) {
		prWfdCoupledSinkInfo  = (P_WFD_ATTRI_COUPLED_SINK_STATUS_T) msg->wfd_coupled_sink_info;
        dev->info.wfd_data_params.WfdCoupleSinkStatus = prWfdCoupledSinkInfo->ucCoupledSinkStatus;
		os_memcpy(dev->info.wfd_data_params.WfdCoupleSinkAddress, prWfdCoupledSinkInfo->aucCoupledSinkAddress, ETH_ALEN);
        wfdFlags |= WFD_FLAG_COUPLED_SINK_INFO_VALID;
	}
	else {
		os_memset(dev->info.wfd_data_params.WfdCoupleSinkAddress, 0, ETH_ALEN);
	}

    if(msg->wfd_ext_capa_bitmap_info) {
		prWfdExtCapaBitmap = (P_WFD_ATTRI_EXT_CAPA_BITMAP_T) msg->wfd_ext_capa_bitmap_info;
        dev->info.wfd_data_params.WfdExtendCap= WPA_GET_BE16((u8*)(&prWfdExtCapaBitmap->u2WfdExtCapBitmap));
        wfdFlags |= WFD_FLAG_EXT_CAPA_VALID;
	}

    dev->info.wfd_data_params.WfdFlag = wfdFlags;
