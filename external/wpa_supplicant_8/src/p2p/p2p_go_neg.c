					    ...
					    struct p2p_device *peer)
{
    ...
    if(p2p->wfd_params) {
        if(p2p->wfd_params->WfdEnable) {
           if(
              ((p2p->wfd_params->WfdDevInfo & 0x3) == 0x0) /* I am source */
                   ) {
                p2p->next_tie_breaker = 1;

                wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG, 
                    "WFD: Force tie_break to 1 (I am source)");
            }
        }
    }
#endif
			    ...
			    const u8 *data, size_t len, int rx_freq)
{
	...
	if (dev && dev->flags & P2P_DEV_USER_REJECTED) {
		...
		wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG,
			...
			MAC2STR(sa));
		wpa_printf(MSG_INFO, "P2P: Add patch for Provision");	
		if (dev && msg.listen_channel) {
		    int freq;
			...
			if (freq > 0) {
			    ...
			    wpa_printf(MSG_INFO, "P2P: Update listen channel to %u MHz from GO Request",
			    dev->listen_freq);	
			}		   
		}	    
    	...
    	if (msg.operating_channel) {
    		...
    		wpa_msg(p2p->cfg->msg_ctx, MSG_INFO, "P2P: Peer operating "
    			"channel preference: %d MHz in GO Req", dev->oper_freq);
    	}
			     ...
			     const u8 *data, size_t len, int rx_freq)
{
	...
	if (msg.operating_channel) {
		dev->oper_freq = p2p_channel_to_freq((const char *)
						     ...
						     msg.operating_channel[4]);
		wpa_msg(p2p->cfg->msg_ctx, MSG_INFO, "P2P: Peer operating "
			"channel preference: %d MHz in GO Resp", dev->oper_freq);
	} else
		dev->oper_freq = 0;
			     ...
			     const u8 *data, size_t len)
{
	...
	if (msg.operating_channel) {
		dev->oper_freq = p2p_channel_to_freq((const char *)
						     ...
						     msg.operating_channel[4]);
		wpa_msg(p2p->cfg->msg_ctx, MSG_INFO, "P2P: Peer operating "
			"channel preference: %d MHz in GO Conf", dev->oper_freq);
	} else
		dev->oper_freq = 0;
#endif
