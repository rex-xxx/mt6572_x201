...
#define ANDROID_PRINTF_BUFLEN 2048
char android_printf_buf[ANDROID_PRINTF_BUFLEN];
/**
...
void wpa_printf(int level, const char *fmt, ...)
{
	va_list ap;
	va_start(ap, fmt);
	vsprintf(android_printf_buf, fmt, ap);
	va_end(ap);
...
#define WPA_HEXDUMP_BUFLEN 500
char wpa_hexdump_printbuf[WPA_HEXDUMP_BUFLEN*2+1];
void wpa_hexdump(int level, const char *title, const u8 *hexbuf, size_t hexlen)
{
	if (level < wpa_debug_level)
		return;
	wpa_printf(level,"%s",title);
	wpa_snprintf_hex(wpa_hexdump_printbuf, WPA_HEXDUMP_BUFLEN*2+1, hexbuf, hexlen);
	wpa_printf(level,"%s",wpa_hexdump_printbuf);
}
