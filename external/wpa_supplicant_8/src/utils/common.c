...
const char * wpa_ssid_txt(const u8 *ssid, size_t ssid_len)
{
	...
	for (pos = ssid_txt; *pos != '\0'; pos++) {
	    /* Use the under line instead of new line */
        if((u8) *pos == 0x0d || (u8) *pos == 0x0a)
            *pos = '_';
...
int ipv4_port_to_value(char *value, unsigned char *ip, unsigned short *port, unsigned int flag) {

    unsigned int fgIgnorePort = 0;
    char *pSep = NULL;
    char *saveptr,*pch;
    unsigned int i;
    ...
    // parse port
    pSep = (char *)os_strstr(value,":");
    if(!fgIgnorePort) {
        if(pSep) {
            if(port) {
                *port  = (unsigned short) strtoul((pSep+1), NULL, 10);
            }
        }
        else {
            return -1;
    ...
    pch = strtok_r(value, ".", &saveptr);
    i = 0;
    while(pch && i<4) {
      ip[i] = strtoul(pch, NULL, 0);
