...
struct wpa_ctrl * wpa_ctrl_open(const char *ctrl_path)
{
	...
	if (ctrl->s > 1023) {
		os_free(ctrl);
		printf("Error: PF_UNIX sockets' number is already up to 1024");
		return NULL;
	}
...
retry_send:
	...
	for (;;) {
		...
		if (res < 0)
			return res;
		if (ctrl->s<0 || ctrl->s > 1023) {
			ALOGD("socket fd=%d",ctrl->s);
			return -1;
		}
...
int wpa_ctrl_randum(char *buf, size_t len)
{
	u8 val;
	size_t i;
	u8 letters = 'Z' - 'A' + 1;
