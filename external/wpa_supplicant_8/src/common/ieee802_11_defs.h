...
#define WFD_DEV_INFO_TYPE_PRI_SECSEC_SINK   3
...
#define WFD_IE_OUI_HDR                      (ELEM_HDR_LEN + WFD_OUI_TYPE_LEN) /* == OFFSET_OF(IE_WFD_T, aucP2PAttributes[0])  */
...
#define WFD_SUBELEMENT_ID_WFD_EXT_CAPA_BITMAP_INFO          7
...
#define WFD_ATTRI_MAX_LEN_EXT_CAPA_BITMAP_INFO              2
#define WFD_ATTRI_MAX_LEN_WFD_SESSION_INFO                  23  /* TODO */

#define WFD_MAXIMUM_ATTRIBUTES_CACHE_SIZE                   300
#define WFD_MAXIMUM_ATTRIBUTE_LEN                           251

/* WFD 1.10 5.1.2 */
typedef struct _WFD_ATTRI_HDR_T {
    u8     ucId;                   /* Subelement ID */
    u16      u2Length;               /* Length */
} __attribute__ ((packed)) WFD_ATTRI_HDR_T, *P_WFD_ATTRI_HDR_T;


typedef struct _WFD_ATTRI_WFD_DEV_INFO_T {
//    UINT_8     ucId;                   /* Subelement ID */
//    UINT_16      u2Length;               /* Length */
    u16    u2DevInfo;
    u16    u2ControlPort;
    u16    u2MaximumTp;
} __attribute__ ((packed)) WFD_ATTRI_WFD_DEV_INFO_T, *P_WFD_ATTRI_WFD_DEV_INFO_T;

/* WFD 1.10 5.1.3 */
typedef struct _WFD_ATTRI_ASSOCIATED_BSSID_T {
   // UINT_8     ucId;                   /* Subelement ID */
   // UINT_16      u2Length;               /* Length */
    u8     aucAssociatedBssid[ETH_ALEN];
}__attribute__ ((packed)) WFD_ATTRI_ASSOCIATED_BSSID_T, *P_WFD_ATTRI_ASSOCIATED_BSSID_T;

/* WFD 1.10 5.1.4 */
typedef struct _WFD_ATTRI_COUPLED_SINK_STATUS_T {
   // UINT_8     ucId;                   /* Subelement ID */
   // UINT_16      u2Length;               /* Length */
    u8     ucCoupledSinkStatus;
    u8     aucCoupledSinkAddress[ETH_ALEN];
}__attribute__ ((packed)) WFD_ATTRI_COUPLED_SINK_STATUS_T, *P_WFD_ATTRI_COUPLED_SINK_STATUS_T;

/* WFD 1.10 5.1.14 */
typedef struct _WFD_DEV_INFO_DESC_T {
    u8     ucLength;               /* Length = 23*/
    u8     aucDeviceAddress[ETH_ALEN];
    u8     aucAssociatedBssid[ETH_ALEN];
    u16    u2DevInfo;
    u16    u2MaximumTp;
    u8     ucCoupledSinkStatus;
    u8     aucCoupledSinkAddress[ETH_ALEN];
}__attribute__ ((packed)) WFD_DEV_INFO_DESC_T, *P_WFD_DEV_INFO_DESC_T;

typedef struct _WFD_ATTRI_WFD_SESSION_INFO_T {
    u8     ucId;                   /* Subelement ID */
    u16      u2Length;               /* Length */
    WFD_DEV_INFO_DESC_T  arWfdDevDesc[1];
}__attribute__ ((packed)) WFD_ATTRI_WFD_SESSION_INFO_T, *P_WFD_ATTRI_WFD_SESSION_INFO_T;

typedef struct _WFD_ATTRI_EXT_CAPA_BITMAP_T {
   // UINT_8     ucId;                   /* Subelement ID */
   // UINT_16      u2Length;               /* Length */
    u16    u2WfdExtCapBitmap;
}__attribute__ ((packed)) WFD_ATTRI_EXT_CAPA_BITMAP_T, *P_WFD_ATTRI_EXT_CAPA_BITMAP_T;
