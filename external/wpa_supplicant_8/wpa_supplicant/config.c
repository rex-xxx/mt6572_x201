				...
				const char *value)
{
	...
	if (*value == '"') {
        ...
        if ((ssid->proto & WPA_PROTO_WAPI)!=0) {
                if(len >= 127){
                    wpa_printf(MSG_INFO, "WAPI cannot accept any length >= 127 psk\n");
                } else {
                    wpa_printf(MSG_INFO, "Accept the length = %d psk\n", len);
                }
                ssid->len_wapi_hex_psk = 0;
        } else {
		if (len < 8 || len > 63) {
...
#ifdef CONFIG_WAPI_SUPPORT
	if ((ssid->proto & WPA_PROTO_WAPI)!=0) {
		wpa_printf( MSG_INFO, "[WAPI-Debug]Ready to translate hex string\n");
        ssid->len_wapi_hex_psk = os_strlen(value);
        if(ssid->len_wapi_hex_psk > 64){
            wpa_printf(MSG_ERROR, "Line %d: Invalid WAPI Hex '%s'. lenght too long",line, value);
				return -1;
        }

        os_memset(ssid->wapi_hex_psk, 0, 64);
        if(hexstr2bin_wapi(value, ssid->wapi_hex_psk, 64))
		{
				wpa_printf(MSG_ERROR, "Line %d: Invalid WAPI Hex '%s'. parser error",line, value);
				return -1;
		}
	}
	/*original hex mode, the string MUST have 64 bytes*/
	else if (hexstr2bin(value, ssid->psk, PMK_LEN) ||
	    value[PMK_LEN * 2] != '\0') {
...
#ifdef CONFIG_WAPI_SUPPORT
    if ((ssid->proto & WPA_PROTO_WAPI)!=0 && ssid->psk_set) {
        if(ssid->len_wapi_hex_psk <= 64){
            char *buf;
            int  len=0;
            len = ssid->len_wapi_hex_psk;
            
        	buf = os_zalloc(len + 1);
        	if (buf == NULL)
        		return NULL;
        	wpa_snprintf_hex(buf, len + 1, ssid->wapi_hex_psk, len);

        	return buf;
        }   
        else { 
            wpa_printf(MSG_ERROR, "[WAPI-Debug]ssid->len_wapi_hex_psk is too long\n");
            return NULL;
        }
    }
    else
		...
		*end = '\0';
		if (os_strcmp(start, "WPA") == 0)
			...
			val |= WPA_PROTO_RSN;
		        ...
		        wpa_printf(MSG_DEBUG, "Line %d: set '%s' proto",
				   line, start);
			val |= WPA_PROTO_WAPI;
                }
#endif
				     ...
				     struct wpa_ssid *ssid)
{
	...
	if (ssid->proto & WPA_PROTO_WAPI) {
			wpa_printf(MSG_DEBUG,"[Debug] %s =>  proto: 0x%x", __FUNCTION__, ssid->proto & WPA_PROTO_WAPI);
...
static char * wpa_config_write_cipher(int cipher)
{
	...
	if (cipher & WAPI_CIPHER_SMS4) {
			wpa_printf(MSG_DEBUG,"[Debug] %s =>  cipher: SMS4", __FUNCTION__);
