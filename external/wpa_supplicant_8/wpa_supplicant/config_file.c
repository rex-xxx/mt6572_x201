...
#if 1 /* Check the config file, if file size is too small, delete it and use default config file at next start */
    fseek (f , 0 , SEEK_END);
    lSize = ftell (f);
    rewind (f);

    /* Make sure the file size has baisc filed */
    if (lSize < 30) {
        fclose(f);
        if( remove( name ) != 0 )
            wpa_printf(MSG_WARNING, "Error deleting file %s", name );
        else
            wpa_printf(MSG_WARNING, "File successfully deleted %s", name );
...
#define INT_DEFe(t, def) write_int(f, #t, ssid->eap.t, def)
    ...
    if ((ssid->proto & WPA_PROTO_WAPI)!=0){
        /*WAPI Case need to place proto type to network setting header!!!*/
        /*We will use proto to decide the seting if WAPI setting*/
        write_proto(f, ssid);
        write_bssid(f, ssid);
    		write_psk(f, ssid);
    }
...
static void wpa_config_write_global(FILE *f, struct wpa_config *config)
{
	...
	if (config->ap_oper_channel)	
		fprintf(f, "ap_oper_channel=%d\n", config->ap_oper_channel);
}
...
#endif /* CONFIG_NO_CONFIG_BLOBS */
	int ret = 0;
#define PERSISTENT_CONFIG_MAX 16
