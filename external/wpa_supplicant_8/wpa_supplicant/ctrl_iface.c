					 ...
					 char *cmd)
{
	...
	*value++ = '\0';
    ...
    } else if(os_strcasecmp(cmd, "mcc") == 0) {
        ...
        if(wpa_s->drv_priv) {
            ...
            if(mcc) {
                ...
                for (_wpa_s = wpa_s->global->ifaces; _wpa_s; _wpa_s = _wpa_s->next) {
                    _wpa_s->drv_flags |= WPA_DRIVER_FLAGS_MULTI_CHANNEL_CONCURRENT;
                    if (_wpa_s->global->p2p && _wpa_s->global->p2p->cfg ) {
                        _wpa_s->global->p2p->cfg->p2p_concurrency = P2P_MULTI_CHANNEL_CONCURRENT;
                    }
                }
            }
            else {
                ...
                for (_wpa_s = wpa_s->global->ifaces; _wpa_s; _wpa_s = _wpa_s->next) {
                    _wpa_s->drv_flags &= ~WPA_DRIVER_FLAGS_MULTI_CHANNEL_CONCURRENT;
                    if (_wpa_s->global->p2p && _wpa_s->global->p2p->cfg ) {
                        _wpa_s->global->p2p->cfg->p2p_concurrency = P2P_SINGLE_CHANNEL_CONCURRENT;
                    }
                }
...
static int wpa_supplicant_ctrl_iface_own_mac_addr(struct wpa_supplicant *wpa_s,
					char *cmd, char *buf, size_t buflen)
{
	char *pos, *end;
	int ret = 0;
...
#ifdef CONFIG_P2P
static int wpa_supplicant_ctrl_iface_p2p_list_networks(
	struct wpa_supplicant *wpa_s, char *cmd, char *buf, size_t buflen)
{
	char *pos, *end, *stamp;
	struct wpa_ssid *ssid;
	...
	ret = os_snprintf(pos, end - pos,
			  "network id / ssid / bssid / disable / pri / mode / client\n");
	if (ret < 0 || ret >= end - pos)
		return pos - buf;
	pos += ret;
	...
	while (ssid) {
		...
		ret = os_snprintf(pos, end - pos, "\t%u\t%u\t%u",
				  ssid->disabled,
                  ssid->priority,
                  ssid->mode
                  );
		...
		if (ssid->mode == WPAS_MODE_P2P_GO && ssid->p2p_client_list) {
            for (i = 0; i < ssid->num_p2p_clients; i++) {
                ret = os_snprintf(pos, end - pos, MACSTR ",", MAC2STR(ssid->p2p_client_list + i * ETH_ALEN));
                if (ret < 0 || ret >= end - pos)
                    return pos - buf;
                pos += ret;
            }
...
static char * wpa_supplicant_cipher_txt(char *pos, char *end, int cipher)
{
	...
	if (cipher & WAPI_CIPHER_SMS4) {
		ret = os_snprintf(pos, end - pos, "%sSMS4", first ? "" : "+");
		if (ret < 0 || ret >= end - pos)
			return pos;
		pos += ret;
		first = 0;
				    ...
				    const u8 *ie, size_t ie_len)
{
	...
	pos += ret;
#ifdef CONFIG_WAPI_SUPPORT
	if(os_memcmp(proto,"WAPI", 4)==0){
		if (wpa_parse_wapi_ie(ie, ie_len, &data) < 0) {
			ret = os_snprintf(pos, end - pos, "?]");
			if (ret < 0 || ret >= end - pos)
				return pos;
			pos += ret;
...
#ifdef CONFIG_WAPI_SUPPORT
	if (data.key_mgmt & WAPI_KEY_MGMT_CERT) {
		wpa_printf(MSG_DEBUG,"[Debug] %s => WAPI IS CERT ", __FUNCTION__);
		ret = os_snprintf(pos, end - pos, "%sCERT", first ? "" : "+");
		if (ret < 0 || ret >= end - pos)
			return pos;
		pos += ret;
		first = 0;
	}
	if (data.key_mgmt & WAPI_KEY_MGMT_PSK) {
		wpa_printf(MSG_DEBUG,"[Debug] %s => WAPI IS PSK ", __FUNCTION__);
		ret = os_snprintf(pos, end - pos, "%sPSK", first ? "" : "+");
		if (ret < 0 || ret >= end - pos)
			return pos;
		pos += ret;
...
#ifdef CONFIG_WAPI_SUPPORT
		iewapi = wpa_bss_get_ie(bss, WAPI_INFO_ELEM);
		if (iewapi)
			pos = wpa_supplicant_ie_txt(pos, end, "WAPI", iewapi, 2 + iewapi[1]);

		if (!ie && !ie2 && !iewapi && bss->caps & IEEE80211_CAP_PRIVACY) {
	...
	struct wpa_supplicant *wpa_s, char *cmd)
{
	...
	*value++ = '\0';
	...
	{
		if(wpa_config_get(ssid,"eap")!=NULL)
		{
			if( os_strcmp(name,"imsi")==0 && os_strcmp(wpa_config_get(ssid,"eap"),"SIM")==0 )
			{
				char str[] = "\"1232010000000000@wlan.mnc001.mcc232.3gppnetwork.org\"";
				value = str;
			}
			if( os_strcmp(name,"imsi")==0 && os_strcmp(wpa_config_get(ssid,"eap"),"AKA")==0 )
			{
				char str[] = "\"0232010000000000@wlan.mnc001.mcc232.3gppnetwork.org\"";
				value = str;
			}
			if( os_strcmp(name,"imsi")==0 && os_strcmp(wpa_config_get(ssid,"eap"),"AKA'")==0 )
			{
				char str[] = "\"0555444333222111@wlan.mnc001.mcc232.3gppnetwork.org\"";
				value = str;
			}
		}
	}
	
	if (os_strcmp(name, "eap") == 0) { //CRID:409191 workaround for CMCC-AUTO eap-sim/aka
			  ...
			  unsigned long mask, char *buf, size_t buflen)
{
	...
	if (mask & WPA_BSS_MASK_WIFI_DISPLAY) {
        ...
        if(wfd) {
            ie = (const u8 *) (bss + 1);
            ret = wpas_wfd_scan_result_text(ie, bss->ie_len, pos, end);
            if (ret < 0 || ret >= end - pos)
                return pos - buf;
            pos += ret;
        }
...
#ifdef CONFIG_AP
static int wpa_supplicant_ctrl_iface_sta_disconnect(
    struct wpa_supplicant *wpa_s, char *cmd)
{
    u8 bssid[ETH_ALEN];//, *_bssid = bssid;
    struct wpa_ssid *ssid;
    ...
    if (hwaddr_aton(cmd, bssid)) {
        wpa_printf(MSG_DEBUG, "CTRL_IFACE sta_disconnect : invalid DEVICE ADDRESS '%s'",
               cmd);
        return -1;
    }

    if (os_memcmp(bssid, "\xff\xff\xff\xff\xff\xff", ETH_ALEN) == 0) {
        wpa_printf(MSG_DEBUG, "CTRL_IFACE Disconnect all STAs");
		wpas_p2p_disconnect(wpa_s);
    }
	else
	{
	    wpa_printf(MSG_DEBUG, "CTRL_IFACE Disconnect a STA "MACSTR, MAC2STR(bssid));
		ap_ctrl_iface_sta_disconnect(wpa_s, bssid, WLAN_REASON_PREV_AUTH_NOT_VALID);
	}
...
static int wpa_supplicant_ctrl_iface_sta_block(
    struct wpa_supplicant *wpa_s, char *cmd)
{
    u8 bssid[ETH_ALEN];//, *_bssid = bssid;
    struct wpa_ssid *ssid;
    ...
    /* Block p2p device */
    if (hwaddr_aton(cmd, bssid)) {
        wpa_printf(MSG_DEBUG, "CTRL_IFACE sta_block : invalid DEVICE ADDRESS '%s'",
               cmd);
        return -1;
    }

    wpa_printf(MSG_DEBUG, "CTRL_IFACE Block a STA "MACSTR, MAC2STR(bssid));
    
    wpas_hotspot_block_list_update(wpa_s, bssid, blocked);
...
static int wpa_supplicant_ctrl_iface_sta_unblock(
    struct wpa_supplicant *wpa_s, char *cmd)
{
    u8 bssid[ETH_ALEN];//, *_bssid = bssid;
    struct wpa_ssid *ssid;
    ...
    /* Unblock p2p device */
    if (hwaddr_aton(cmd, bssid)) {
        wpa_printf(MSG_DEBUG, "CTRL_IFACE sta_unblock : invalid DEVICE ADDRESS '%s'",
               cmd);
        return -1;
    }

    wpa_printf(MSG_DEBUG, "CTRL_IFACE Unblock a STA "MACSTR, MAC2STR(bssid));
...
#ifdef CONFIG_MTK_WFD
static int wfd_ctrl_iface_cmd(struct wpa_supplicant *wpa_s, char *cmd,
                  char *buf, size_t buflen)
{
    char *value;
    char *value1;
    ...
    p_wfd_data_cmd = &wpa_s->global->wfd_data_cmd;
	
    wpa_printf(MSG_DEBUG, "CTRL_IFACE: wfd_cmd name='%s'", cmd);
    ...
    if(value != NULL) {
        *value++ = '\0';
        ...
        wpa_printf(MSG_DEBUG, "CTRL_IFACE: wfd_cmd name='%s' value=%s",
               cmd, value);

        value1 = os_strchr(value, ' ');

        if(value1 != NULL) {
            *value1++ = '\0';
            wpa_printf(MSG_DEBUG, "CTRL_IFACE: wfd_cmd name='%s' value1=%s",
               cmd, value1);
        }
    ...
    if(value) {
        if (os_strncmp(cmd, "wfd_en", os_strlen("wfd_en")) == 0) {
            p_wfd_data_cmd->WfdEnable = strtol(value, NULL, 0);
            wpa_printf(MSG_INFO, "wfd_en = %u", p_wfd_data_cmd->WfdEnable);
        }
        else if (os_strncmp(cmd, "wfd_info", os_strlen("wfd_info")) == 0){
            p_wfd_data_cmd->WfdDevInfo = strtol(value, NULL, 0);
            wpa_printf(MSG_INFO, "wfd_info = 0x%x", p_wfd_data_cmd->WfdDevInfo);
        }
        else if (os_strncmp(cmd, "wfd_ctrl_port", os_strlen("wfd_ctrl_port")) == 0) {
            p_wfd_data_cmd->WfdControlPort = strtol(value, NULL, 0);
            wpa_printf(MSG_INFO, "wfd_ctrl_port = %u", p_wfd_data_cmd->WfdControlPort);
        }
        else if (os_strncmp(cmd, "wfd_max_tp", os_strlen("wfd_max_tp"))==0) {
            p_wfd_data_cmd->WfdMaximumTp = strtol(value, NULL, 0);
            wpa_printf(MSG_INFO, "wfd_max_tp = %d\n", p_wfd_data_cmd->WfdMaximumTp);
        }
        else if (os_strncmp(cmd, "wfd_sink_status", os_strlen("wfd_sink_status")) == 0) {
            p_wfd_data_cmd->WfdCoupleSinkStatus = strtol(value, NULL, 0);
            wpa_printf(MSG_INFO, "wfd_sink_status = 0x%x\n", p_wfd_data_cmd->WfdCoupleSinkStatus);
        }
        else if (os_strncmp(cmd, "wfd_sink_mac", os_strlen("wfd_sink_mac")) == 0) {
            if (hwaddr_aton(value, p_wfd_data_cmd->WfdCoupleSinkAddress)) return -1;
            wpa_printf(MSG_INFO, "wfd_sink_mac = "MACSTR"\n", MAC2STR(p_wfd_data_cmd->WfdCoupleSinkAddress));
        }
        else if (os_strncmp(cmd, "wfd_assoc_mac", os_strlen("wfd_assoc_mac")) == 0) {
            if (hwaddr_aton(value, p_wfd_data_cmd->WfdAssociatedBssid)) return -1;
            wpa_printf(MSG_INFO, "wfd_assoc_mac = "MACSTR"\n", MAC2STR(p_wfd_data_cmd->WfdAssociatedBssid));
        }
        else if (os_strncmp(cmd, "wfd_extend_capa", os_strlen("wfd_extend_capa")) == 0) {
            p_wfd_data_cmd->WfdExtendCap = strtol(value, NULL, 0);
            wpa_printf(MSG_INFO, "wfd_extend_capa = 0x%x\n", p_wfd_data_cmd->WfdExtendCap);
        }
        else if (os_strncmp(cmd, "wfd_flags", os_strlen("wfd_flags")) == 0) {
            p_wfd_data_cmd->WfdFlag = strtol(value, NULL, 0);
            wpa_printf(MSG_INFO, "wfd_flags = %u", p_wfd_data_cmd->WfdFlag);
        }
        else if (os_strncmp(cmd, "wfd_policy", os_strlen("wfd_policy")) == 0) {
            p_wfd_data_cmd->WfdPolicy = strtol(value, NULL, 0);
            wpa_printf(MSG_INFO, "wfd_policy = %u", p_wfd_data_cmd->WfdPolicy);
        }
        else if (os_strncmp(cmd, "wfd_state", os_strlen("wfd_state")) == 0) {
            p_wfd_data_cmd->WfdState = strtol(value, NULL, 0);
            wpa_printf(MSG_INFO, "wfd_state = %u", p_wfd_data_cmd->WfdState);
        }
        else if (os_strncmp(cmd, "wfd_video_udp_ip_port", os_strlen("wfd_video_udp_ip_port")) == 0) {
            ipv4_port_to_value(value, p_wfd_data_cmd->WfdVideoIp, &p_wfd_data_cmd->WfdVideoPort, 0);
            wpa_printf(MSG_INFO, "wfd_video_udp_ip_port = %u %u %u %u %u",
                    p_wfd_data_cmd->WfdVideoIp[0],
                    p_wfd_data_cmd->WfdVideoIp[1],
                    p_wfd_data_cmd->WfdVideoIp[2],
                    p_wfd_data_cmd->WfdVideoIp[3],
                    p_wfd_data_cmd->WfdVideoPort);
        }
        else if (os_strncmp(cmd, "wfd_audio_udp_ip_port", os_strlen("wfd_audio_udp_ip_port")) == 0) {
            ipv4_port_to_value(value, p_wfd_data_cmd->WfdAudioIp, &p_wfd_data_cmd->WfdAudioPort, 0);
            wpa_printf(MSG_INFO, "wfd_audio_udp_ip_port = %u %u %u %u %u",
                    p_wfd_data_cmd->WfdAudioIp[0],
                    p_wfd_data_cmd->WfdAudioIp[1],
                    p_wfd_data_cmd->WfdAudioIp[2],
                    p_wfd_data_cmd->WfdAudioIp[3],
                    p_wfd_data_cmd->WfdAudioPort);
        }
        else if (os_strncmp(cmd, "wfd_pri_sink_mac", os_strlen("wfd_pri_sink_mac")) == 0) {
            if (hwaddr_aton(value, p_wfd_data_cmd->WfdPrimarySinkMac)) return -1;
            wpa_printf(MSG_INFO, "wfd_pri_sink_mac = "MACSTR"\n", MAC2STR(p_wfd_data_cmd->WfdPrimarySinkMac));
        }
        else if (os_strncmp(cmd, "wfd_sec_sink_mac", os_strlen("wfd_sec_sink_mac")) == 0) {
            if (hwaddr_aton(value, p_wfd_data_cmd->WfdSecondarySinkMac)) return -1;
            wpa_printf(MSG_INFO, "wfd_sec_sink_mac = "MACSTR"\n", MAC2STR(p_wfd_data_cmd->WfdSecondarySinkMac));
        }
        else if (os_strncmp(cmd, "wfd_adv_flags", os_strlen("wfd_adv_flags")) == 0) {
            p_wfd_data_cmd->WfdAdvancedFlag = strtol(value, NULL, 0);
            wpa_printf(MSG_INFO, "wfd_adv_flags = %u", p_wfd_data_cmd->WfdAdvancedFlag);
        }
        else if (os_strncmp(cmd, "wfd_local_ip", os_strlen("wfd_local_ip")) == 0) {
            ipv4_port_to_value(value, p_wfd_data_cmd->WfdLocalIp, NULL, 1);
            wpa_printf(MSG_INFO, "wfd_local_ip = %u %u %u %u",
                    p_wfd_data_cmd->WfdLocalIp[0],
                    p_wfd_data_cmd->WfdLocalIp[1],
                    p_wfd_data_cmd->WfdLocalIp[2],
                    p_wfd_data_cmd->WfdLocalIp[3]);
        }
        else if (os_strncmp(cmd, "wfd_session_available", os_strlen("wfd_session_available")) == 0) {
            p_wfd_data_cmd->WfdSessionAvailable = strtol(value, NULL, 0);
        }
        else if (os_strncmp(cmd, "wfd_sigma_mode", os_strlen("wfd_sigma_mode")) == 0) {
            p_wfd_data_cmd->WfdSigmaMode = strtol(value, NULL, 0);
        }
    }

    {
        /* For MR0 and MR1 sync */
         if (os_strncmp(cmd, "wfd_update", os_strlen("wfd_update")) == 0) {
            struct wpabuf *cmd;
            cmd = wpabuf_alloc(200);

            if(p_wfd_data_cmd->WfdFlag & WFD_FLAG_DEV_INFO_VALID)
            { 
                u16 _WfdDevInfo = p_wfd_data_cmd->WfdDevInfo;
                if(cmd) {

                    if(p_wfd_data_cmd->WfdSessionAvailable==1) {
                        _WfdDevInfo &= ~BITS(4,5);
                        _WfdDevInfo |= BIT(4);
                    }
                    else if(p_wfd_data_cmd->WfdSessionAvailable==2) {
                        _WfdDevInfo &= ~BITS(4,5);
                    }

                    wpabuf_printf(cmd,"%u ", WFD_SUBELEM_DEVICE_INFO);
                    wpabuf_printf(cmd,"%04X%04x%04x%04x",
                            WFD_ATTRI_MAX_LEN_WFD_DEV_INFO,
                            _WfdDevInfo, 
                            p_wfd_data_cmd->WfdControlPort,
                            p_wfd_data_cmd->WfdMaximumTp);

                    wifi_display_subelem_set(wpa_s->global, (char *)wpabuf_mhead_u8(cmd));
                } /* cmd */
            }
            else {
                if(cmd) {
            ...
            /* Last step , Dont move to first becasue the other setting will be overwrite when enabling */
            if(p_wfd_data_cmd->WfdEnable) {
                wifi_display_enable(wpa_s->global,1);
            }
            else {
                wifi_display_enable(wpa_s->global,0);
            }

            wpabuf_free(cmd);
             /* process the command and copy to another place */
            wpas_wfd_data_process(wpa_s, p_wfd_data_cmd);
            wpas_wfd_data_update(wpa_s, &wpa_s->global->wfd_data);
         }
         else if (os_strncmp(cmd, "wfd_reset", os_strlen("wfd_reset")) == 0) {
            os_memset(p_wfd_data_cmd, 0, sizeof(struct wfd_data_s));
            os_memset(&wpa_s->global->wfd_data, 0, sizeof(struct wfd_data_s));
         }
    }
...
/* Tethering */
static int p2p_ctrl_iface_cfg_ap(struct wpa_supplicant *wpa_s,
                         char *cmd)
{
    char *type;
    char *value;
    ...
    u8 key[64];
    size_t key_len;
    char current3gband[3]="";

    value = os_strchr(cmd, ' ');
    *value++ = '\0';

    wpa_printf(MSG_DEBUG, "CTRL_IFACE: CFG_AP name='%s' value=%s",
           cmd, value);

    if (os_strncmp(cmd, "ssid", os_strlen("ssid"))==0) {
        type = wpa_config_parse_string(value, &res_len);
        if (type == NULL)
            goto fail;

		//os_free(wpa_s->ssid_t->ssid);
		if (res_len > 32)
			res_len = 32;
        wpa_s->ssid_t.ssid = os_malloc(res_len);
        os_memcpy(wpa_s->ssid_t.ssid, type, res_len);
        wpa_s->ssid_t.ssid_len = res_len;
        os_free(type);
        wpa_printf(MSG_INFO, "ssid (%d)=%s", wpa_s->ssid_t.ssid_len,  wpa_s->ssid_t.ssid);

    }
    else if (os_strncmp(cmd, "sec", os_strlen("sec"))==0){
        type = wpa_config_parse_string(value, &res_len);
        if (type == NULL)
            goto fail;

        wpa_printf(MSG_INFO, "sec=%s", type);

        if (os_strncmp(type, "open", os_strlen("open"))==0) {
            wpa_s->ssid_t.pairwise_cipher = WPA_CIPHER_NONE;
            wpa_s->ssid_t.auth_alg = WPA_AUTH_ALG_OPEN;
            wpa_s->ssid_t.key_mgmt = WPA_KEY_MGMT_NONE;
            wpa_s->ssid_t.proto = 0;
        }
        else if (os_strncmp(type, "wpa2-psk", os_strlen("wpa2-psk"))==0) {
            wpa_s->ssid_t.pairwise_cipher = WPA_CIPHER_CCMP;
            wpa_s->ssid_t.auth_alg = WPA_AUTH_ALG_OPEN;
            wpa_s->ssid_t.key_mgmt = WPA_KEY_MGMT_PSK;
            wpa_s->ssid_t.proto = WPA_PROTO_RSN;
        }
		else if (os_strncmp(type, "wpa-psk", os_strlen("wpa-psk"))==0) {
            wpa_s->ssid_t.pairwise_cipher = WPA_CIPHER_TKIP;
            wpa_s->ssid_t.auth_alg = WPA_AUTH_ALG_OPEN;
            wpa_s->ssid_t.key_mgmt = WPA_KEY_MGMT_PSK;
            wpa_s->ssid_t.proto = WPA_PROTO_WPA;
        }
		
        os_free(type);
        wpa_printf(MSG_INFO, "auth=%d key_mgmt=%d proto=%d", wpa_s->ssid_t.auth_alg,
            wpa_s->ssid_t.key_mgmt, wpa_s->ssid_t.proto);
    }
    else if (os_strncmp(cmd, "key", os_strlen("key"))==0) {
         type = wpa_config_parse_string(value, &res_len);
        if (type == NULL)
            goto fail;

        os_free(wpa_s->ssid_t.passphrase);
        if (res_len > 64)
			res_len = 63;
        if (res_len == 64) {
			wpa_s->ssid_t.passphrase = NULL;
	        if (hexstr2bin((const char *) type, wpa_s->ssid_t.psk,
	                       PMK_LEN)) {
	            wpa_printf(MSG_ERROR, "WPS: Invalid Network Key");
				goto fail;
	        }
            wpa_s->ssid_t.psk_set = 1;
        }
		else {
            wpa_s->ssid_t.passphrase = os_malloc(res_len + 1);
            os_memcpy(wpa_s->ssid_t.passphrase, type, res_len);
            wpa_s->ssid_t.passphrase[res_len] = '\0';
            wpa_config_update_psk(&wpa_s->ssid_t);
			wpa_printf(MSG_INFO, "passphrase=%s", wpa_s->ssid_t.passphrase);
		}
        os_free(type);
    }
	else if (os_strncmp(cmd, "ch_width", os_strlen("ch_width"))==0) {
		/*cfg_ap ch_width  0: 20 Mhz  1:20/40 Mhz auto*/
        wpa_printf(MSG_INFO, "ch_width=%d", atoi(value));
		wpa_s->best_ch_width= atoi(value);
    }
    else if (os_strncmp(cmd, "ch", os_strlen("ch"))==0) {
			int ch;
			if(wpa_s->conf->ap_oper_channel)	{
	     	wpa_printf(MSG_INFO, "ap_oper_channel from conf file: %d", wpa_s->conf->ap_oper_channel);
	     	ch = wpa_s->conf->ap_oper_channel;
	    }
	    else {
	     	wpa_printf(MSG_INFO, "ap_oper_channel from framework: %d", atoi(value));
	     	ch = atoi(value);
	    }
		if(ch > 0 && ch < 14)
		{
			wpa_s->ssid_t.frequency = 2407 + 5 * ch;
			wpa_printf(MSG_INFO, "P2P: Use 2.4 GHz band "
				   "channel: %d MHz", wpa_s->ssid_t.frequency);
		}
		else if(ch == 14)
		{
			wpa_s->ssid_t.frequency = 2484;
			wpa_printf(MSG_INFO, "P2P: Use 2.4 GHz band "
				   "channel: %d MHz", wpa_s->ssid_t.frequency);
		}
		else if(ch > 35 && ch < 165){
			wpa_s->ssid_t.frequency = 5000 + 5 * ch;
			wpa_printf(MSG_INFO, "P2P: Use 5 GHz band "
				   "channel: %d MHz", wpa_s->ssid_t.frequency);
		}
		else{
			if(!property_get("bgw.current3gband", current3gband, NULL)){
				wpa_printf(MSG_DEBUG, "GPS desense: bgw.current3gband is not set");
				wpa_s->ssid_t.frequency = 2412;
			}
			else{
				wpa_printf(MSG_DEBUG, "GPS desense: bgw.current3gband is %s", current3gband);
				if(current3gband[0]=='5') wpa_s->ssid_t.frequency = 2462;
				else if(current3gband[0]=='8') wpa_s->ssid_t.frequency = 2412;
				else wpa_s->ssid_t.frequency = 2412;
			}
			wpa_printf(MSG_INFO, "P2P: Use 2.4 GHz band "
				   "channel: %d MHz", wpa_s->ssid_t.frequency);
		}
    ...
    else if (os_strncmp(cmd, "preamble", os_strlen("preamble"))==0) {
        wpa_printf(MSG_INFO, "preamble=%d", atoi(value));
    }
    else if (os_strncmp(cmd, "max_scb", os_strlen("max_scb"))==0) {
        wpa_printf(MSG_INFO, "max_scb=%d", atoi(value));
    }
    else if (os_strncmp(cmd, "max_sta", os_strlen("max_sta"))==0) {
        wpa_printf(MSG_INFO, "max_sta=%d", atoi(value));
		wpa_s->max_stations = atoi(value);
        wpas_p2p_sigma_test_mode(wpa_s, 109, wpa_s->max_stations);
    }
	else if (os_strncmp(cmd, "wps_test", os_strlen("wps_test"))==0) {
        wpa_printf(MSG_INFO, "wps_test=%d", atoi(value));
        wpas_p2p_sigma_test_mode(wpa_s, 110, atoi(value));
    }
    else {
        goto fail;
    }
...
fail:
    wpa_printf(MSG_ERROR, "failed to parse %s", cmd);
...
static int p2p_ctrl_iface_commit_setting(struct wpa_supplicant *wpa_s)
{
	...
	wpa_printf(MSG_DEBUG, "p2p_ctrl_iface_commit_setting");
...
static int p2p_ctrl_iface_create_ap(struct wpa_supplicant *wpa_s)
{
    int ret, len = 0;
    u8 key[64];
    size_t key_len;
	...
	/* ASCII_CMD=AP_CFG,SSID=\"AndroidAP\",SEC=\"open\",KEY=12345,CHANNEL=1,PREAMBLE=0,MAX_SCB=8 */
	
    wpa_s->fgHotspot = 1;
  

	wpa_printf(MSG_DEBUG, "ssid (%d)=%s", wpa_s->ssid_t.ssid_len, wpa_s->ssid_t.ssid);
    wpa_printf(MSG_DEBUG, "auth=%d key_mgmt=%d proto=%d", wpa_s->ssid_t.auth_alg,
            wpa_s->ssid_t.key_mgmt, wpa_s->ssid_t.proto);
    if (wpa_s->ssid_t.proto) {
        if (wpa_s->ssid_t.passphrase == NULL && !wpa_s->ssid_t.psk_set) {
            wpa_printf(MSG_ERROR, "wpa2-psk setting but no passphrase!!!");
            return 0;
        }
	    if (wpa_s->ssid_t.passphrase)
            wpa_printf(MSG_DEBUG, "passphrase=%s", wpa_s->ssid_t.passphrase);
    }
	os_memset(&params, 0, sizeof(params));
	params.role_go = 1;
    params.freq = wpa_s->ssid_t.frequency;
	if (wpa_s->ssid_t.ssid) {
		os_memcpy(params.ssid, wpa_s->ssid_t.ssid, wpa_s->ssid_t.ssid_len);
		params.ssid_len = wpa_s->ssid_t.ssid_len;
	}
	if(wpa_s->ssid_t.passphrase)
	    os_memcpy(params.passphrase, wpa_s->ssid_t.passphrase, os_strlen(wpa_s->ssid_t.passphrase));
	
	wpas_start_tethering(wpa_s, &params);

    //os_snprintf(buf, buflen, "return "MACSTR " \"%s\"", MAC2STR(devAddr), ssid->ssid);
    //wpa_printf(MSG_DEBUG, "CTRL_IFACE start_ap %s \n", buf);
    //len = os_snprintf(buf, buflen, "OK");
...
/* Sigma */
static int p2p_ctrl_iface_send_presence(
    struct wpa_supplicant *wpa_s, char *cmd, char *buf, size_t buflen)
{
    char *str = NULL;
    ...
    wpa_printf(MSG_DEBUG, "CTRL_IFACE send_presence cmd=%s\n", cmd);
    ...
    *str ++ = '\0';
    ...
    if (str) {
        ...
        *str ++ = '\0';
        ...
        if (ssid) {
            wpa_printf(MSG_DEBUG, "CTRL_IFACE send_presence duration=%lu interval=%lu "MACSTR" SSID(%d)%s\n",
                duration, interval, MAC2STR(addr), ssid_len, ssid);
            os_free(ssid);
        }
        else {
            wpa_printf(MSG_DEBUG, "CTRL_IFACE send_presence duration=%lu interval=%lu "MACSTR" SSID(%d)\n",
                duration, interval, MAC2STR(addr), ssid_len);
        }
    }
    else {
        wpa_printf(MSG_DEBUG, "CTRL_IFACE send_presence duration=%lu interval=%lu\n",
            duration, interval);
    }

    wpas_p2p_sigma_test_mode(wpa_s, (int)104, (int)duration);
	
    wpas_p2p_sigma_test_mode(wpa_s, (int)105, (int)interval);
				     ...
				     char *buf, size_t buflen)
{
    ...
    if (os_strncmp(cmd, "MACADDR", os_strlen("MACADDR"))==0) {
       os_memcpy(&macaddr, wpa_s->own_addr, ETH_ALEN);
        ret = snprintf(buf, buflen, "Macaddr = " MACSTR "\n", MAC2STR(macaddr));
...
#endif /* CONFIG_IEEE80211R */
#ifdef CONFIG_EAP_SIM_AKA
	} else if (os_strcmp(buf, "ENABLE_SW_SIM") == 0) {
		wpa_s->conf->software_sim=1;
		if (wpa_supplicant_ctrl_iface_save_config(wpa_s))
			reply_len = -1;
	} else if (os_strcmp(buf, "DISABLE_SW_SIM") == 0) {
		wpa_s->conf->software_sim=0;
		if (wpa_supplicant_ctrl_iface_save_config(wpa_s))
			reply_len = -1;
#endif
#ifdef CONFIG_WPS
...
#ifdef CONFIG_P2P
/* Tethering */
    }else if (os_strncmp(buf, "cfg_ap ", 7) == 0) {
        if (p2p_ctrl_iface_cfg_ap(wpa_s, buf + 7))
            reply_len = -1;
    } else if (os_strncmp(buf, "p2p_enable_device", 17) == 0) {
        if (p2p_ctrl_iface_commit_setting(wpa_s))
            reply_len = -1;
    } else if (os_strncmp(buf, "start_ap", 8) == 0) {
        if(p2p_ctrl_iface_create_ap(wpa_s))
            reply_len = -1;
/* Tethering */
/* Sigma */
    } else if (os_strncmp(buf, "p2p_send_presence ", 18) == 0) {
        reply_len = p2p_ctrl_iface_send_presence(wpa_s, buf + 18, 
                            reply, reply_size);
    } else if (os_strncmp(buf, "p2p_set_sleep ", 14) == 0) {
        reply_len = p2p_ctrl_iface_set_sleep(wpa_s, buf + 14, 
                            reply, reply_size);
    } else if (os_strncmp(buf, "p2p_set_sleep", 13) == 0) {
        char cmd2[] = {"ff:ff:ff:ff:ff:ff \"\""};
        reply_len = p2p_ctrl_iface_set_sleep(wpa_s, cmd2, 
                             reply, reply_size);
    } else if (os_strncmp(buf, "p2p_set_opps ", 13) == 0) {
        reply_len = p2p_ctrl_iface_set_opps(wpa_s, buf + 13, 
                             reply, reply_size);
    } else if (os_strncmp(buf, "p2p_set_power_save ", 19) == 0) {
        reply_len = p2p_ctrl_iface_set_power_save(wpa_s, buf + 19, 
                             reply, reply_size);
/* Sigma */
		...
		if (p2p_ctrl_ext_listen(wpa_s, "") < 0)
			reply_len = -1;
	} else if (os_strncmp(buf, "P2P_LIST_NETWORKS",17) == 0) {
		reply_len = wpa_supplicant_ctrl_iface_p2p_list_networks(
			wpa_s, buf+17, reply, reply_size);
#endif /* CONFIG_P2P */
#ifdef CONFIG_MTK_WFD
    } else if (os_strncmp(buf, "wfd_cmd ", 8) == 0) {
        if (wfd_ctrl_iface_cmd(wpa_s, buf + 8, reply, reply_size) < 0)
            reply_len = -1;
#endif /* CONFIG_MTK_WFD */
#ifdef CONFIG_WIFI_DISPLAY
	...
	} else if (os_strncmp(buf, WPA_CTRL_RSP, os_strlen(WPA_CTRL_RSP)) == 0)
	{
		...
		if (wpa_supplicant_ctrl_iface_bssid(wpa_s, buf + 6))
			reply_len = -1;
  } else if (os_strncmp(buf, "OWN_MAC_ADDR", 12) == 0) {
        reply_len = wpa_supplicant_ctrl_iface_own_mac_addr(wpa_s, buf + 12,
            reply, reply_size);
		...
		reply_len = ap_ctrl_iface_sta_next(wpa_s, buf + 9, reply,
						   reply_size);
    } else if (os_strncmp(buf, "STA-DISCONNECT ", 15) == 0) {
        if(wpa_supplicant_ctrl_iface_sta_disconnect(wpa_s, buf + 15))
            reply_len = -1;
    } else if (os_strncmp(buf, "STA-BLOCK ", 10) == 0) {
        if(wpa_supplicant_ctrl_iface_sta_block(wpa_s, buf + 10))
            reply_len = -1;
    } else if (os_strncmp(buf, "STA-UNBLOCK ", 12) == 0) {
        if(wpa_supplicant_ctrl_iface_sta_unblock(wpa_s, buf + 12))
            reply_len = -1;
