				     ...
				     const u8 *ssid, size_t ssid_len)
{
	...
	if (wpa_s->go_params)
		...
		bssid = wpa_s->bssid;
	// peter-jc.lin add debug message
	wpa_printf(MSG_INFO, "P2P: bssid " MACSTR "from %s", MAC2STR(bssid), 
	wpa_s->go_params? "go_params":"bssid");
...
void wpas_go_neg_req_rx(void *ctx, const u8 *src, u16 dev_passwd_id)
{
    ...
    struct wfd_data_s *params = NULL;

    struct p2p_device *dev = p2p_get_device(wpa_s->global->p2p, src);
	if (dev != NULL )
    {   
        params = &dev->info.wfd_data_params;
        
        wpa_msg(wpa_s, MSG_INFO, P2P_EVENT_GO_NEG_REQUEST MACSTR
            " dev_passwd_id=%u "
            "wfd_en=%u " 
            "wfd_info=0x%x wfd_ctrl_port=%u wfd_tp=%u "
            "wfd_assoc_mac=" MACSTR " "
            "wfd_sink_status=0x%x wfd_sink_mac=" MACSTR " "            
            "wfd_extend_capa=0x%x wfd_flags=0x%x",
            MAC2STR(src), dev_passwd_id,
            params->WfdEnable,
            params->WfdDevInfo, params->WfdControlPort, 
            params->WfdMaximumTp,MAC2STR(params->WfdAssociatedBssid),
            params->WfdCoupleSinkStatus, MAC2STR(params->WfdCoupleSinkAddress),
            params->WfdExtendCap, params->WfdFlag);
    }
    else {
        wpa_msg(wpa_s, MSG_INFO, P2P_EVENT_GO_NEG_REQUEST MACSTR
            " dev_passwd_id=%u wfd_en=0", MAC2STR(src), dev_passwd_id);
    }
#else
...
#ifdef CONFIG_MTK_WFD
    struct p2p_device *p2pDev = (struct p2p_device *)dev;
    char str_go_intf[18] = {0};
...
#ifdef CONFIG_MTK_WFD
    if(p2pDev) {
        os_snprintf(str_go_intf, 18, MACSTR, MAC2STR(p2pDev->member_in_go_iface));
    }
    else {
        strcpy(str_go_intf, "00:00:00:00:00:00");
    }
#endif /* CONFIG_MTK_WFD */

#ifdef CONFIG_MTK_WFD
    struct wfd_data_s *params ;
    params = (struct wfd_data_s *)&info->wfd_data_params;
...
#ifdef CONFIG_MTK_WFD
    struct wfd_data_s *wfd_params = NULL;
    struct p2p_device *dev = p2p_get_device(wpa_s->global->p2p, peer);
    
    if (dev != NULL) {
                  
        wfd_params = &dev->info.wfd_data_params;
        ...
        os_snprintf(params, sizeof(params), " p2p_dev_addr=" MACSTR
            ...
            wfd_params->WfdEnable,
            wfd_params->WfdDevInfo, wfd_params->WfdControlPort, 
            wfd_params->WfdMaximumTp,MAC2STR(wfd_params->WfdAssociatedBssid),
            wfd_params->WfdCoupleSinkStatus, MAC2STR(wfd_params->WfdCoupleSinkAddress),
            wfd_params->WfdExtendCap, wfd_params->WfdFlag);
    }
    else {
        os_snprintf(params, sizeof(params), " p2p_dev_addr=" MACSTR
		    " pri_dev_type=%s name='%s' config_methods=0x%x "
				     ...
				     int op_freq)
{
	...
	if (status == P2P_SC_SUCCESS) {
		wpa_printf(MSG_DEBUG, "P2P: Invitation from peer " MACSTR
			   ...
			   MAC2STR(sa), op_freq);
		wpas_p2p_group_delete(wpa_s, P2P_GROUP_REMOVAL_REQUESTED); //[2013.03.25 Jian Sun] Delete temp network profile to show P2P-GROUP-STARTED after received invitation/and then connect to the persistent group. If framework do not receive "P2P-GROUP-STARTED" from supplicant, UI will not update status to CONNECTED
		if (s) {
			int go = s->mode == WPAS_MODE_P2P_GO;
                        wpa_msg(wpa_s, MSG_INFO, P2P_EVENT_PERSISTENT_GROUP_INVITED);
...
#ifdef CONFIG_MTK_WFD
    struct wfd_data_s *wfd_params = NULL;
    struct p2p_device *dev = p2p_get_device(wpa_s->global->p2p, sa);
    ...
    if (dev != NULL) {
        ...
        wfd_params = &dev->info.wfd_data_params;
            ...
            if (bssid) {
                wpa_msg(wpa_s, MSG_INFO, P2P_EVENT_INVITATION_RECEIVED
                    "sa=" MACSTR " go_dev_addr=" MACSTR
                    " bssid=" MACSTR " unknown-network wfd_en=0",
                    MAC2STR(sa), MAC2STR(go_dev_addr),
                    MAC2STR(bssid));
            } else {
                wpa_msg(wpa_s, MSG_INFO, P2P_EVENT_INVITATION_RECEIVED
                    "sa=" MACSTR " go_dev_addr=" MACSTR
                    " unknown-network wfd_en=0",
                    MAC2STR(sa), MAC2STR(go_dev_addr));
            }
            return;
        }  
...
int wpas_p2p_init(struct wpa_global *global, struct wpa_supplicant *wpa_s)
{
	...
	if (wpa_s->conf->p2p_listen_reg_class &&
        ...
        if( (p2p.op_channel == 1 || p2p.op_channel == 6 || p2p.op_channel==11)) {
            /* Eddie */
            /* Set listen channel as operation channel */
            p2p.reg_class = 81;
            p2p.channel = p2p.op_channel;
            wpa_printf(MSG_DEBUG, "P2P: Set listen channel by operating channel %d", 
                    p2p.channel);
        }
        else 
        {
...
#ifdef CONFIG_MTK_WFD
	global->p2p->wfd_params = &global->wfd_data;
	global->p2p->wfd_params_cmd = &global->wfd_data_cmd;
#endif
				  ...
				  int freq, int ht40)
{
	...
	if (ssid->mode == WPAS_MODE_INFRA) {
	    // Peter-jc.lin Clear GO parameters 
	    if (wpa_s->go_params) {
	        wpa_printf(MSG_INFO, "P2P: Remove GO params to avoid incorrect persistent GC behavior");
	        os_free(wpa_s->go_params);
	        wpa_s->go_params = NULL;
	    }
...
int wpas_p2p_handle_frequency_conflicts(struct wpa_supplicant *wpa_s, int freq)
{
	...
	for (iface = wpa_s->global->ifaces; iface; iface = iface->next) {
		if (iface == wpa_s)
			continue;
		wpa_printf(MSG_DEBUG, "Shared interface %s p2p_group_interface %u", iface->ifname, iface->p2p_group_interface);	    
		if(iface->current_ssid) {
			wpa_printf(MSG_DEBUG, "current_ssid freq %u ssid %s p2p_group=%s", 
			iface->current_ssid->frequency,iface->current_ssid->ssid, (iface->current_ssid->p2p_group)? "TRUE":"FALSE");
		}
		...
		if ((iface->current_ssid) &&
		    ...
		    if(((iface->p2p_group_interface) ||
		     (iface->current_ssid->p2p_group))) {
		        wpa_printf(MSG_INFO, "Interface is P2P. Try to handle conflict...");
			...
			if(!wpas_is_p2p_prioritized(wpa_s)) {
		        ...
		        wpa_printf(MSG_INFO, "Interface is WLAN. Avoid conflict...");
                    return -1;
    		}	
		}
	}
...
#ifdef CONFIG_MTK_WFD
int wpas_wfd_data_process(struct wpa_supplicant *wpa_s,
	                           struct wfd_data_s *p_wfd_data_cmd)
{
    struct wfd_data_s *p_wfd_data;

    p_wfd_data = &wpa_s->global->wfd_data;

    wpa_printf(MSG_DEBUG, "WFD: wpas_wfd_data_process %u wfd_dev_info 0x%x wfd_ctrl_port %u", 
            p_wfd_data_cmd->WfdEnable, p_wfd_data_cmd->WfdDevInfo, p_wfd_data_cmd->WfdControlPort); 

	p_wfd_data->WfdCmdType = p_wfd_data_cmd->WfdCmdType;
	p_wfd_data->WfdEnable = p_wfd_data_cmd->WfdEnable;
	p_wfd_data->WfdCoupleSinkStatus = p_wfd_data_cmd->WfdCoupleSinkStatus;
	p_wfd_data->WfdDevInfo = p_wfd_data_cmd->WfdDevInfo;
	p_wfd_data->WfdControlPort = p_wfd_data_cmd->WfdControlPort;
	p_wfd_data->WfdMaximumTp = p_wfd_data_cmd->WfdMaximumTp;
	p_wfd_data->WfdExtendCap = p_wfd_data_cmd->WfdExtendCap;
	os_memcpy(p_wfd_data->WfdCoupleSinkAddress, p_wfd_data_cmd->WfdCoupleSinkAddress, ETH_ALEN);
	os_memcpy(p_wfd_data->WfdAssociatedBssid, p_wfd_data_cmd->WfdAssociatedBssid, ETH_ALEN);
	os_memcpy(p_wfd_data->WfdVideoIp, p_wfd_data_cmd->WfdVideoIp, sizeof(p_wfd_data_cmd->WfdVideoIp));
	os_memcpy(p_wfd_data->WfdAudioIp, p_wfd_data_cmd->WfdAudioIp, sizeof(p_wfd_data_cmd->WfdAudioIp));
	p_wfd_data->WfdVideoPort = p_wfd_data_cmd->WfdVideoPort;
	p_wfd_data->WfdAudioPort = p_wfd_data_cmd->WfdAudioPort;
	p_wfd_data->WfdFlag = p_wfd_data_cmd->WfdFlag;
	p_wfd_data->WfdPolicy = p_wfd_data_cmd->WfdPolicy;
	p_wfd_data->WfdState = p_wfd_data_cmd->WfdState;
	//p_wfd_data->WfdSessionInformationIELen = p_wfd_data_cmd->WfdSessionInformationIELen;
	//os_memcpy(p_wfd_data->WfdSessionInformationIE, p_wfd_data_cmd->WfdSessionInformationIE,
    //        p_wfd_data_cmd->WfdSessionInformationIELen);
    os_memcpy(p_wfd_data->WfdPrimarySinkMac, p_wfd_data_cmd->WfdPrimarySinkMac, ETH_ALEN);	
    os_memcpy(p_wfd_data->WfdSecondarySinkMac, p_wfd_data_cmd->WfdSecondarySinkMac, ETH_ALEN);
	p_wfd_data->WfdAdvancedFlag = p_wfd_data_cmd->WfdAdvancedFlag;
	
	p_wfd_data->WfdSessionAvailable = p_wfd_data_cmd->WfdSessionAvailable;
	os_memcpy(p_wfd_data->WfdLocalIp, p_wfd_data_cmd->WfdLocalIp, sizeof(p_wfd_data_cmd->WfdLocalIp));
...
int wpas_wfd_scan_result_text(const u8 *ies, size_t ies_len, char *buf,
			      char *end)
{
	return wfd_scan_result_text(ies, ies_len, buf, end);
}
