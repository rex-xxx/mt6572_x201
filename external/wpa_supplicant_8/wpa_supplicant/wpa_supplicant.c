...
char *responses[256] = {
    ...
    if (length < 0 || length > 65535) {
    		wpa_printf(MSG_INFO,"[WAPI]Wrong length:%d\n",length);
        return -1;
    }
    ...
    sock = socket_local_client("keystore", ANDROID_SOCKET_NAMESPACE_RESERVED,
                               SOCK_STREAM);
    if (sock == -1) {
			wpa_printf(MSG_INFO,"[WAPI]Connect to keystore failed:%s\n",strerror(errno));
			return 1;
    }
    ...
    wpa_printf(MSG_INFO,"shutdown %s\n", key);
    ...
    if(code == LOCKED) //User has not unlock the screen yet, wait framework trigger scan after screen unlocked
    {
        wpa_supplicant_set_state(wpa_s, WPA_DISCONNECTED);
        return -1;
    }
		...
		*/
		...
		#ifdef WAPI_CERT_FROM_KEYSTORE
		{
			...
			if (os_strncmp("keystore://", (char *)user_cert, 11) == 0) {
                ...
                wpa_hexdump(MSG_DEBUG, "user_cert from supplicant=>",(const u8 *) value, 512);
				if (length == -1) {
			...
			if (os_strncmp("keystore://", (char *)as_cert, 11) == 0) {
                ...
                wpa_hexdump(MSG_DEBUG, "as_cert from suppliant=>",(const u8 *) value, 512);
				if (length == -1) {
			      ...
			      struct wpa_bss *bss, struct wpa_ssid *ssid)
{
	...
	wpa_s->reassociate = 0;
	if (bss) {// && !wpas_driver_bss_selection(wpa_s) 2012.12.26: Connect the wanted bssid to make blacklist useful
#ifdef CONFIG_IEEE80211R
		const u8 *ie, *md = NULL;
#endif /* CONFIG_IEEE80211R */
	...
	if (bss) {
		wpa_printf(MSG_DEBUG, "bss->freq (%u)",bss->freq);
		params.ssid = bss->ssid;
		params.ssid_len = bss->ssid_len;
		params.freq = bss->freq; /* Eddie */
	...
	wpa_printf(MSG_DEBUG, "drv_flags 0x%x, shared_freq %u, assoc_freq %u",
	wpa_s->drv_flags, freq, params.freq);

	if (!(wpa_s->drv_flags & WPA_DRIVER_FLAGS_MULTI_CHANNEL_CONCURRENT) &&
		(freq > 0) && (freq != params.freq)) {
	...
	if (ret < 0) {
		wpa_msg(wpa_s, MSG_INFO, "Association request to the driver "
			"failed");
/* Try to continue anyway, after timeout, supplicant will send disassoc request to driver */
#if 0
...
#ifndef CONFIG_MTK_WFD
    /* Eddie move it after p2p init */
