...
{
	if (!wpa_s->driver->set_test_mode)
		return -1;
	return wpa_s->driver->set_test_mode(wpa_s->drv_priv, buf, buf_len);
}

static inline int wpa_drv_driver_sw_cmd(struct wpa_supplicant *wpa_s,
                      int set, unsigned long *adr, unsigned long *dat)
{
    if (wpa_s->driver->driver_sw_cmd)
        return wpa_s->driver->driver_sw_cmd(wpa_s->drv_priv, set, adr, dat);
    return -1;
}
