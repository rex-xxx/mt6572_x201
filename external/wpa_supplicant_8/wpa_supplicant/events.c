...
#ifdef CONFIG_WAPI_SUPPORT
	if ( (ssid->key_mgmt & WAPI_KEY_MGMT_PSK) || (ssid->key_mgmt & WAPI_KEY_MGMT_CERT) )
		privacy = 1;
#endif
					    ...
					    struct wpa_ssid *group)
{
...
#ifdef CONFIG_WAPI_SUPPORT
	wapi_ie = wpa_bss_get_ie(bss, WAPI_INFO_ELEM);
	...
	for (ssid = group; ssid; ssid = ssid->pnext) {
...
#ifdef CONFIG_WAPI_SUPPORT
		if (!wapi &&
				( (ssid->key_mgmt & WAPI_KEY_MGMT_PSK) || (ssid->key_mgmt & WAPI_KEY_MGMT_CERT) )
				) {
		...
		if (!wapi && wpa && (ssid->key_mgmt & WPA_KEY_MGMT_NONE)) {
			wpa_dbg(wpa_s, MSG_DEBUG, "   skip - non-WPA network "
				"not allowed");
			continue;
		}
		...
		if ( wpa && (ssid->key_mgmt & WPA_KEY_MGMT_NONE)) {
			wpa_dbg(wpa_s, MSG_DEBUG, "   skip - non-WPA network "
				"not allowed");
			continue;
		}
	...
	for (i = 0; i < wpa_s->last_scan_res_used; i++) {
		...
		*selected_ssid = wpa_scan_res_match(wpa_s, i, bss, group);
		...
		wpa_dbg(wpa_s, MSG_DEBUG, "   selected BSS " MACSTR
			" ssid='%s' freq=%u",
			MAC2STR(bss->bssid), wpa_ssid_txt(bss->ssid, bss->ssid_len),bss->freq);
		return bss;
	}
...
static struct wpa_bss *
wpa_supplicant_select_bss_private(struct wpa_supplicant *wpa_s,
			  struct wpa_ssid *group,
			  struct wpa_ssid **selected_ssid)
{
	unsigned int i;
	...
	for (i = 0; i < wpa_s->last_scan_res_used; i++) {
		struct wpa_bss *bss = wpa_s->last_scan_res[i];
		if (!(bss->caps & IEEE80211_CAP_PRIVACY)) continue;
		*selected_ssid = wpa_scan_res_match(wpa_s, i, bss, group);
		if (!*selected_ssid)
			continue;
		wpa_dbg(wpa_s, MSG_DEBUG, "   selected BSS " MACSTR
...
static struct wpa_bss *
wpa_supplicant_select_bss_public(struct wpa_supplicant *wpa_s,
			  struct wpa_ssid *group,
			  struct wpa_ssid **selected_ssid)
{
	unsigned int i;
			   ...
			   struct wpa_ssid *ssid)
{
	...
	if (wpa_s->reassociate ||
		...
		wpa_msg(wpa_s, MSG_DEBUG, "Request association: "
			...
			"  pending: " MACSTR "  wpa_state: %s freq: %u",
			wpa_s->reassociate, MAC2STR(selected->bssid),
			MAC2STR(wpa_s->bssid), MAC2STR(wpa_s->pending_bssid),
			wpa_supplicant_state_txt(wpa_s->wpa_state), selected->freq);
	...
	if (!wpa_s->auto_reconnect_disabled ||
		...
		if (wpa_s->wpa_state == WPA_COMPLETED &&
			...
			if (!wpa_s->global->p2p_disabled && wpa_s->global->p2p != NULL &&
			    (wpa_s->key_mgmt != WPA_KEY_MGMT_WPS)) {
	            wpa_msg(wpa_s, MSG_INFO, "P2P: Skip auto connect for P2P");
...
int wpa_supplicant_hotspot(void *ctx)
{
...
int wpa_supplicant_hotspot_cipher_tkip(void *ctx)
{
    struct wpa_supplicant *wpa_s = ctx;
...
#ifndef CONFIG_NO_STDOUT_DEBUG
{
	...
	case EVENT_P2P_DEV_FOUND: {
		...
		wpas_dev_found(wpa_s, data->p2p_dev_found.addr, &peer_info, 1, NULL);
#else
