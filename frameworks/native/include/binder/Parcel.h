...
status_t Parcel::write(const LightFlattenable<T>& val) {
    ...
    if (size) {
        ALOG_ASSERT(size<512*1024*1024, "Parcel: write size larger than 512MB");
...
status_t Parcel::read(LightFlattenable<T>& val) const {
    ...
    if (size) {
#ifdef _MTK_ENG_
        // To let GCC compiler add stack protect for us
        long dummy[4];
        dummy[1] = 0x12345678;
#endif
        ALOG_ASSERT(size<512*1024*1024, "Parcel: read size larger than 512MB");
