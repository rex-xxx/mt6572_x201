...
void HWComposer::dump(String8& result, char* buffer, size_t SIZE) const {
    ...
    // also dump hwc features state
    if (mHwc) {
        result.appendFormat(
            "[HWC Features Support] ext:%d, vir:%d enhance:%d stereo:%d)\n",
            mFeaturesState.externals, mFeaturesState.virtuals,
            mFeaturesState.enhance, mFeaturesState.stereoscopic);
    }
    // [MTK] }}}
}
