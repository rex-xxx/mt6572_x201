        ...
        const Region& inDirtyRegion)
{
    ...
    if (true == inDirtyRegion.isEmpty()) {
        XLOGI("[%s] skip composition for DisplayDevice(type:%d) since dirtyRegion is empty",
            __FUNCTION__, hw->getDisplayType());
        return;
    }
...
uint32_t SurfaceFlinger::setDisplayStateLocked(const DisplayState& s)
{
    ...
    if (disp.isValid()) {
        ...
        if (what & DisplayState::eDisplayProjectionChanged) {
            ...
            int shutanimRunning = atoi(value);
            if (0 == shutanimRunning) {
                const KeyedVector<wp<IBinder>, DisplayDeviceState>& curr(mCurrentState.displays);
                const wp<IBinder>& display(curr.keyAt(dpyIdx));
                const sp<DisplayDevice> hw(getDisplayDevice(display));
                int hwOrientation = hw->getHwOrientation();
                if (disp.orientation != hwOrientation)
                {
                    disp.orientation = hwOrientation;
                    flags |= eDisplayTransactionNeeded;
                }
                XLOGI("Shutdown animation is running, apply HW orientation (%d)", hwOrientation);
            }
            // [MTK] }}}
        }
    }
    ...
    uint32_t code, const Parcel& data, Parcel* reply, uint32_t flags)
{
    switch (code) {
        ...
        case CAPTURE_SCREEN:
        {
            ...
            ALOGD("Request capture screen (pid=%d, uid=%d)", pid, uid);
            break;
        }
    }
