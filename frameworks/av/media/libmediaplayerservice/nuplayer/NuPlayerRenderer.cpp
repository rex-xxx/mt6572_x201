...
void NuPlayer::Renderer::signalTimeDiscontinuity() {
    ...
    if (!mAudioQueue.empty()) {
        ALOGE("------should not be here (size=%d)----", (int)mAudioQueue.size());
    ...
    if (!mVideoQueue.empty()) {
        ALOGE("------should not be here (size=%d)----", (int)mVideoQueue.size());
...
void NuPlayer::Renderer::onDrainVideoQueue() {
    ...
    if (tooLate) {
        ALOGD("video (%.2f) late by %lld us (%.2f secs)",
            mediaTimeUs / 1E6, mVideoLateByUs, mVideoLateByUs / 1E6);
    } else {
...
void NuPlayer::Renderer::onQueueBuffer(const sp<AMessage> &msg) {
    ...
    if (audio) {
        ...
        if (property_get("debug.dump.hls", value, "0")) {
            if (!strcmp(value, "1")) {
                dumpBuffer("/sdcard/audio.raw", (char*)buffer->data(), buffer->size());
                ALOGD("dumping audio, size = %d", buffer->size());
            }
        }
#endif
...
#ifndef ANDROID_DEFAULT_CODE
    syncQueuesDone();
    // mtk80902: debug for ALPS00504270
    ALOGD("%s onQueueEOS.", audio?"audio":"video");
#endif
    int32_t finalResult;
...
void NuPlayer::Renderer::onPause() {
    ...
    if (mPaused)
    {
        ALOGE("NuPlayer::Renderer::onPause already paused");
        return;
    }
#else
    CHECK(!mPaused);
