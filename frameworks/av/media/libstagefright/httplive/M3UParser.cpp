...
#ifndef ANDROID_DEFAULT_CODE
bool M3UParser::isVendorSource(int *VendorType) const {
    if (VendorType != NULL) {
        *VendorType = 0;
    }
    return mSourceLETV;
...
status_t M3UParser::parse(const void *_data, size_t size) {
    ...
    while (offset < size) {
...
#ifndef ANDROID_DEFAULT_CODE
        if (offsetLF > size) {      //the last line may have no '\n'
            break;
        }
#else
        if (offsetLF >= size) {
        ...
        if (mIsExtM3U) {
            ...
            } else if (line.startsWith("#EXT-LETV-VER")) {
                mSourceLETV = true;
                ALOGI("it's letv.com source, please do some error handle");
                ssize_t colonPos = line.find(":"); 
                if (colonPos < 0) {
                    ALOGE("\t\tbad letv.com source");
                } else {
                    ALOGI("\t\t supported version is 1.1.0-0.3.12");
                    ALOGI("\t\t current version = %s", line.c_str() + colonPos + 1);
                }
