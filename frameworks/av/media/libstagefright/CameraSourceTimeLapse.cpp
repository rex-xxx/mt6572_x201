...
CameraSourceTimeLapse::CameraSourceTimeLapse(
	...
	mMaxDurationUs = -1;
	mMaxDurationReached = false;
	mForcePick = false;
	mReachedStartTime = false;
#endif
}
...
bool CameraSourceTimeLapse::skipFrameAndModifyTimeStamp(int64_t *timestampUs) {
...
#ifndef ANDROID_DEFAULT_CODE//Drop frame before CameraSource and keep time lapse initial status
	if (!mReachedStartTime) {
		if (*timestampUs < mStartTimeUs) {
			ALOGD("Drop frame before start time %lld/%lld", *timestampUs, mStartTimeUs);
			return true;
		}
		else {
			mReachedStartTime = true;
			ALOGD("Reach start time %lld/%lld", *timestampUs, mStartTimeUs);
		}
	}
#endif
...
#ifndef ANDROID_DEFAULT_CODE
		mFirstPickedFrameTimeUs = *timestampUs;
#endif
...
#ifndef ANDROID_DEFAULT_CODE
	if (/*mNumFramesEncoded*/mNumFramesReceived >= 1 && *timestampUs <//We can not expect when encoder return the buffer, which leads to mNumFramesEncoded++. So we use mNumFramesReceived.
		(mLastTimeLapseFrameRealTimestampUs + mTimeBetweenFrameCaptureUs) 
		&& !mMaxDurationReached) //force push frame when reachec file duration limitation for quick stop
#else
...
#endif
	{
...
#ifndef ANDROID_DEFAULT_CODE//force pick frame for some quick return case and set time stamp to the previous ts
		if (mForcePick) {
			if (1 == mForcePickMode) {
				*timestampUs = mLastFrameTimestampUs;
				//*timestampUs = mLastFrameTimestampUs + mTimeBetweenTimeLapseVideoFramesUs;
				ALOGD("Force pick ts %lld, mode 1", *timestampUs);
				return false;
			} else if (2 == mForcePickMode){
				*timestampUs = mLastFrameTimestampUs + mTimeBetweenTimeLapseVideoFramesUs;
				ALOGD("Force pick ts %lld, mode 2", *timestampUs);
				return false;
			}
		}
#endif
...
#ifndef ANDROID_DEFAULT_CODE//avoid error accumulate
		mLastTimeLapseFrameRealTimestampUs += mTimeBetweenFrameCaptureUs;
#else
        mLastTimeLapseFrameRealTimestampUs = *timestampUs;
#endif
        *timestampUs = mLastFrameTimestampUs + mTimeBetweenTimeLapseVideoFramesUs;
#ifndef ANDROID_DEFAULT_CODE
		if ((mMaxDurationUs >= 0) && (*timestampUs - mFirstPickedFrameTimeUs + 500 > mMaxDurationUs))
		{
			ALOGD("Max duration reached");
