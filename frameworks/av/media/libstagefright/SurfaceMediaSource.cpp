...
status_t SurfaceMediaSource::start(MetaData *params)
{
    ...
    if (params) {
        ...
        ALOGD ("@@ SMS mStartTimeNs(%lld)", mStartTimeNs);
    }
                                    ...
                                    const ReadOptions *options)
{
    ...
    *buffer = NULL;
    ...
    while (mStarted) {
        ...
        if (err == BufferQueue::NO_BUFFER_AVAILABLE) {
            ...
            if (mNumFramesReceived == 0 && !mUseAbsoluteTimestamps) {
                ...
                ALOGD ("@@ mFirstFrameTimestamp = %lld, mStartTimeNs = %lld", mFirstFrameTimestamp, mStartTimeNs);
                // Initial delay
                if (mStartTimeNs > 0) {
...
#ifndef ANDROID_DEFAULT_CODE
					ALOGI("@@ real mStartTimeNs=%lld",mStartTimeNs);
#endif 
                }
            }
#ifdef ANDROID_DEFAULT_CODE
            ALOGD ("@@ + mStartTimeNs = %lld, item.mTimestamp = %lld", mStartTimeNs, item.mTimestamp);
#endif 
            item.mTimestamp = mStartTimeNs + (item.mTimestamp - mFirstFrameTimestamp);
            ALOGD ("@@ - item.mTimestamp = %lld", item.mTimestamp);
    ...
    ALOGD("@@ SMS Frames encoded = %d, timestamp = %lld, time diff = %lld",
