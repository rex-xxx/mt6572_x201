...
FileSource::FileSource(const char *filename)
    ...
    if(MMP_Player !=0){
	    MMP_FILEREAD= MMProfileRegisterEvent(MMP_Player, "Fread");
	    MMProfileEnableEvent(MMP_FILEREAD,1); 	
    } 
#endif	
...
#ifdef  MMPROFILE_HTTP
    MMProfileLogMetaStringEx(MMP_FILEREAD, MMProfileFlagPulse,offset, size,"readAt+");
...
#ifndef ANDROID_DEFAULT_CODE
#ifdef MTK_DRM_APP
    // OMA DRM v1.0 implementation
    // if it is drm file then we use DRM apis to get the raw content size, and update it to mLength
    // because some codec (E.g. AMR requires the size to be exactly correct, otherwise setDataSource() fails)
    if (mDecryptHandle != NULL
        && DecryptApiType::CONTAINER_BASED == mDecryptHandle->decryptApiType) {
        if (mLength > DrmMtkUtil::getContentSize(mDecryptHandle.get())) {
            long tmp = DrmMtkUtil::getContentRawSize(mDecryptHandle, mDrmManagerClient);
            if (tmp >= 0) {
                mLength = tmp; // update mLength
            }
        }
        *size = mLength;
        return OK;
...
ssize_t FileSource::readAtDRM(off64_t offset, void *data, size_t size) {
    size_t DRM_CACHE_SIZE = 4096; // OMA DRM v1.0 implementation: the size is enhanced from 1024 to 4096
    if (mDrmBuf == NULL) {
