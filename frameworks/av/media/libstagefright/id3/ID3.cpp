...
static void convertISO8859ToString8(
    ...
    if (utf8len == size) {
        ...
        ALOGD("only ASCII characters");
        s->setTo((const char *)data, size);
        return;
    }
    ALOGD("utf8len=%d",utf8len);
...
static void nativeencodesetto(
	...
	if (ALLASCII== true) {
	 ...
	 char *frameDataNew= new char[size+1];
			frameDataNew[0]=0xFF;
	 for(int i=0,j=1;i<size;i++,j++)
			frameDataNew[j]=data[i];
	 s->setTo(frameDataNew,size+1);
	 delete[] frameDataNew;
	 frameDataNew=NULL;
...
void ID3::Iterator::getstring(String8 *id, bool otherdata) const {
...
#ifndef ANDROID_DEFAULT_CODE
		if((mParent.mSize == 643507) && (mFrameSize == 166))
		{
			ALOGD("workaround for ALPS00444321, return ok");
		//	frameData += 1;
		//	skipped += 1;
			return ;
		}
    ...
    if (mParent.mVersion == ID3_V1 || mParent.mVersion == ID3_V1_1) {
		...
		#ifndef ANDROID_DEFAULT_CODE
		ALOGD("framesize = %d",mFrameSize);
		nativeencodesetto(frameData, mFrameSize, id);
		#else
...
//workaround for ALPS00444321
#ifndef ANDROID_DEFAULT_CODE
		if((*frameData == 0x00) && (*++frameData == 0xFF) && (mParent.mSize == 643507) && (mFrameSize == 166))
		{
			frameData++;
			skipped++;
		}
    ...
    if (encoding == 0x00) {
       ...
       #ifndef ANDROID_DEFAULT_CODE   
		ALOGV("framesize = %d in ISO8859-1",mFrameSize);
      	nativeencodesetto(frameData+1, n, id);
		#else
		 ALOGV("framesize = %d in ISO8859-1",mFrameSize);
