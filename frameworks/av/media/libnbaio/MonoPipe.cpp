...
#else
    N = tmpHelper.getLocalFreq();
    ALOGD("LocalFreq(%ld)",N);
#endif
...
ssize_t MonoPipe::write(const void *buffer, size_t count)
{
    ...
    while (count > 0) {
        ...
        block_ns += ns;
        if (++try_count>=5){
            ALOGW("%s is blocked (%ldns), please check if MonoPipeReader::read is normal",__FUNCTION__,try_count,block_ns);
            break;
        }
#endif
    }
