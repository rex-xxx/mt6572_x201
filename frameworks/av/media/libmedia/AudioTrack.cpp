...
#define WAIT_ObtainServer_MS 52
...
#ifndef ANDROID_DEFAULT_CODE
    SXLOGD("audiotrack %p set Type %d, rate %d, fmt %d, chn %d, fcnt %d, flag %d", 
...
#ifndef ANDROID_DEFAULT_CODE
    mLatency = afLatency*3/2 + (1000*mFrameCount) / sampleRate;
#else
...
#ifndef ANDROID_DEFAULT_CODE
    uint32_t afLatency;
	sp<IMemory> cblktemp = mAudioTrack->getCblk();
	if (cblktemp == 0){
		SXLOGE("obtainBuffer cblk is null %p", this);
		return DEAD_OBJECT;
	}

    if (AudioSystem::getOutputLatency(&afLatency, streamType()) != NO_ERROR) {
	   SXLOGD("no get afLatency!");
        afLatency=WAIT_ObtainServer_MS;
    }           
    waitTimeMs = (waitCount < 0) ? cblk->bufferTimeoutMs : afLatency+2;
#endif
    ...
    if (framesAvail == 0) {
        ...
        while (framesAvail == 0) {
            ...
            if (CC_UNLIKELY(!active)) {
                ALOGD("%p, Not active and NO_MORE_BUFFERS", this);
                cblk->lock.unlock();
            ...
            ALOGD_IF(cblk->flags & CBLK_DISABLED_MSK, "obtainBuffer track %p, ative=%d, user=%08x, server=%08x, loopend=%08x",
                   this, mActive, cblk->user, cblk->server,cblk->loopEnd);
            if (!(cblk->flags & CBLK_INVALID_MSK)) {
                mLock.unlock();
...
bool AudioTrack::processAudioBuffer(const sp<AudioTrackThread>& thread)
{
    ...
    if (active && (cblk->framesAvailable() == cblk->frameCount)) {
        ALOGV("Underrun %p, user: %x, server: %x, flags %04x", this, cblk->user, cblk->server, cblk->flags);
    ...
    ///SXLOGV("processAudioBuffer %p, atvice %d, %d, user: %x, server: %x, flowControlFlag %d", this, mActive, mRemainingFrames, mCblk->user, mCblk->server, mCblk->flags);
...
uint32_t audio_track_cblk_t::stepUser(uint32_t frameCount)
{
    ...
    ///SXLOGV("track_cblk %p stepUser, u %p, s %p, %d, %d", this, this->user, this->server, flags, bufferTimeoutMs);
    return u;
}
...
bool audio_track_cblk_t::stepServer(uint32_t frameCount)
{
...
#ifndef ANDROID_DEFAULT_CODE
        cv.broadcast(); //notify write thread and audiotrack thread
#else
        cv.signal();
#endif
    }
    ...
    ///SXLOGV("track_cblk %p stepServer, u %p, s %p, %d, %d", this, this->user, this->server, flags, bufferTimeoutMs);
    return true;
}
