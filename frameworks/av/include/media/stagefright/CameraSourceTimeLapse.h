...
public:
	void setMaxDurationUs(int64_t durationUs) { mMaxDurationUs = durationUs; }
	void setForcePick(bool force, int32_t mode) { mForcePick = force; mForcePickMode = mode; }
private:
	int64_t mMaxDurationUs;
	int64_t mFirstPickedFrameTimeUs;
	bool mMaxDurationReached;
	bool mForcePick;
	int32_t mForcePickMode;
	bool mReachedStartTime;
#endif
};
