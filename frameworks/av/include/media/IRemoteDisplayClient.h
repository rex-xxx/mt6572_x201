...
public:
    ...
    virtual void onDisplayKeyEvent(uint32_t keyCode, uint32_t flags) = 0;

    // Indicates that a touch event from UIBC channel is received and pass this touch event to hanlde in framework layer.
    virtual void onDisplayTouchEvent(uint32_t x, uint32_t y, uint32_t flags) = 0;
