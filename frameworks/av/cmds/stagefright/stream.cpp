...
int main(int argc, char **argv) {
    ...
    if ((argc != 2) && !(argc > 2 && !strcasecmp("-l", argv[1]))) {
        fprintf(stderr, "Usage: %s filename\n", argv[0]);
        fprintf(stderr, "or %s -l filename1 filename2\n", argv[0]);
        return 1;
    }
    ...
    } else if ( !strcasecmp("-l", argv[1])) {
        List<String8> filelist;
        for (int i = 2; i < argc; i ++) {
            filelist.push_back(String8(argv[i]));
        }
        source = new PlayListSource(filelist);
