...
/*******************************************************************************/
{
    ...
    M4OSA_TRACE2_0(" M4MP4W_newAudioChunk - flush audio");
    M4OSA_TRACE2_2("current chunk = %d  offset = 0x%x",
#else
...
#ifndef ANDROID_DEFAULT_CODE
    M4OSA_TRACE2_3("M4MPW_startAU: start address:%p, size:%lu, stream:%d",
#else
...
#ifndef ANDROID_DEFAULT_CODE
    M4OSA_TRACE2_4("processAU : size 0x%x mode %d filesize %lu limit %lu",
#else
...
/*******************************************************************************/
{
    ...
    if (bVideo)
    {
        ...
        M4OSA_TRACE1_3("video | CLOSE: current chunk = %d  offset = 0x%x size = 0x%08X",
            mMp4FileDataPtr->videoTrackPtr->currentChunk,
            mMp4FileDataPtr->absoluteCurrentPos,
            mMp4FileDataPtr->videoTrackPtr->currentPos);
        ...
        M4OSA_TRACE1_1("MaxFileDuration = %d", mMp4FileDataPtr->MaxFileDuration);
        if (mMp4FileDataPtr->MaxFileDuration > 0)
        {
            ...
            if (mMp4FileDataPtr->videoTrackPtr->CommonData.sampleNb > 1)
            {
                ...
                if (8 *mMp4FileDataPtr->videoTrackPtr->CommonData.sttsTableEntryNb
                    ...
                    *M4MP4W_STTS_ALLOC_SIZE)
                {
                    M4OSA_TRACE1_2("sttsTableEntryNb %d, nbOfAllocatedSttsBlocks %d", 
                        mMp4FileDataPtr->videoTrackPtr->CommonData.sttsTableEntryNb,
                        mMp4FileDataPtr->videoTrackPtr->nbOfAllocatedSttsBlocks);
                    mMp4FileDataPtr->videoTrackPtr->nbOfAllocatedSttsBlocks += 1;
                    mMp4FileDataPtr->videoTrackPtr->TABLE_STTS =
                        (M4OSA_UInt32 *)M4MP4W_realloc(
                        (M4OSA_MemAddr32)mMp4FileDataPtr->videoTrackPtr->
                        TABLE_STTS, ( mMp4FileDataPtr->videoTrackPtr->nbOfAllocatedSttsBlocks- 1) * M4MP4W_STTS_ALLOC_SIZE,
                        mMp4FileDataPtr->videoTrackPtr->nbOfAllocatedSttsBlocks* M4MP4W_STTS_ALLOC_SIZE);
                    //ERR_CHECK(mMp4FileDataPtr->videoTrackPtr->TABLE_STTS
                    //    != M4OSA_NULL, M4ERR_ALLOC);
                    if( mMp4FileDataPtr->videoTrackPtr->TABLE_STTS == M4OSA_NULL )
                    {
                        M4OSA_TRACE1_0("M4MP4W_realloc new TABLE_STTS failed");
                        mMp4FileDataPtr->fileWriterFunctions->closeWrite(
                        fileWriterContext); /**< close the stream anyway */
                        M4MP4W_freeContext(context); /**< Free the context content */
                        return M4ERR_ALLOC;
            ...
            M4OSA_TRACE1_4("MaxFileDuration = %d  delta = %d sttsTableEntryNb = %d, nbOfAllocatedSttsBlocks %d",
                        mMp4FileDataPtr->MaxFileDuration,
                        delta,
                        mMp4FileDataPtr->videoTrackPtr->CommonData.sttsTableEntryNb,
                        mMp4FileDataPtr->videoTrackPtr->nbOfAllocatedSttsBlocks);
        ...
        else
        {
            ...
            if (mMp4FileDataPtr->videoTrackPtr->CommonData.sampleNb > 1)
            {
                ...
                M4OSA_TRACE1_3("sampleNb = %d, sttsTableEntryNb = %d, nbOfAllocatedSttsBlocks %d",
                            mMp4FileDataPtr->videoTrackPtr->CommonData.sampleNb,
                            mMp4FileDataPtr->videoTrackPtr->CommonData.sttsTableEntryNb,
                            mMp4FileDataPtr->videoTrackPtr->nbOfAllocatedSttsBlocks);
