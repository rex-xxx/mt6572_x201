...
//#define LOG_NDEBUG 0
#define LOG_TAG "M4MP4W_Interface"
#include "utils/Log.h"
#endif
        ...
        M4SYS_AccessUnit* pAU)
{
...
#ifndef ANDROID_DEFAULT_CODE
    ALOGD("M4WRITER_3GP_processAU: AU: "
	...
	if (pAU->size == 6)
	{
		ALOGD("M4WRITER_3GP_processAU: len6: "
			  "0x%x, 0x%x, 0x%x, 0x%x,0x%x,0x%x,",
			  *(M4OSA_UInt8 *)(pAU->dataAddress), *((M4OSA_UInt8 *)(pAU->dataAddress)+1), *((M4OSA_UInt8 *)(pAU->dataAddress)+2),
			  *((M4OSA_UInt8 *)(pAU->dataAddress)+3),*((M4OSA_UInt8 *)(pAU->dataAddress)+4),*((M4OSA_UInt8 *)(pAU->dataAddress)+5));
	}
