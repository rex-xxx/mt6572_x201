...
typedef struct {
	...
	M4OSA_Bool 					mHasAudio;
	M4OSA_Bool                  mHasVideo;
#endif
...
#ifndef ANDROID_DEFAULT_CODE
	pC->mHasAudio = M4OSA_FALSE;
    pC->mHasVideo = M4OSA_FALSE;
#endif
    *pContext=pC;
...
M4OSA_ERR VideoEditor3gpReader_setOption(M4OSA_Context context,
    ...
    // Demon, don't fail on null pValue since we will pass null for this pointer
    M4OSA_DEBUG_IF1((M4OSA_NULL == pValue), M4ERR_PARAMETER,
...
#ifndef ANDROID_DEFAULT_CODE
		M4OSA_Int32 pTime=0;
		err =  VideoEditor3gpReader_jump(context,pStreamHandler,&pTime);
		if (M4NO_ERROR != err) {
			ALOGD("VideoEditor3gpReader_reset @@no seek it to begin!");
		}
#endif
...
#ifdef ANDROID_DEFAULT_CODE // Morris Yang 20120320 remove the weird behavior
...
#ifndef ANDROID_DEFAULT_CODE	
	ALOGV("VideoEditor3gpReader_getNextStreamHandler trackCount %d",trackCount);
#endif
    if(temp >= trackCount) {
...
#ifndef ANDROID_DEFAULT_CODE
		if (!pC->mHasVideo && !strncasecmp(mime, "video/", 6))
#else
		if (!haveVideo && !strncasecmp(mime, "video/", 6)) 
#endif	
		{
            pC->mVideoSource = pC->mExtractor->getTrack(temp);
#ifndef ANDROID_DEFAULT_CODE // Morris Yang 20120320 remove NAL prefix when using MTK AVC decoder [
            if (!strncasecmp(mime, "video/avc", 9)) {
                //ALOGD ("@@ remove NAL prefix for MTK AVC decoder !!!");
                sp<MetaData> _tempMeta = new MetaData;
                _tempMeta->setInt32(kKeyWantsNALFragments, true);
                pC->mVideoSource->start(_tempMeta.get());
            }
            else {
...
#ifndef ANDROID_DEFAULT_CODE
                if ( (!meta->findInt32(kKeyFrameRate,(int32_t*)&(avgFPS))) ||(!avgFPS)) {
                	ALOGE ("## Cannot find kKeyFrameRate");
...
#ifndef ANDROID_DEFAULT_CODE
		else if (!pC->mHasAudio && !strncasecmp(mime, "audio/", 6))
#else
        ...
        // <--- Morris Yang 20120402 for (128160 H264 mp3 32k 35 fps.mp4)
#ifndef ANDROID_DEFAULT_CODE
            else {
                ALOGD("VE unknown audio sream:pC->mCurrTrack = %d", pC->mCurrTrack);
