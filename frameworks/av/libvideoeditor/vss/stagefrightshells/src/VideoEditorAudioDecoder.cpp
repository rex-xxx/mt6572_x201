...
#else
#define VIDEOEDITOR_FORCECODEC kMtkAudDecForVE
//#define DUMPOUTPUTPCM
#endif
namespace android {
...
M4OSA_ERR VideoEditorAudioDecoder_create(M4AD_Type decoderType,
...
#ifdef VE_OMX_DEC
	int32_t outbufsize =0;
	sp<MetaData> metaPara = NULL;
#endif
...
#ifdef	VE_OMX_DEC
			pDecoderContext->aactype = M4_kAAC;
#endif
            // Engine parameters
...
#ifdef 	VE_OMX_DEC
	decoderMetaData->setInt32(kKeyMaxInputSize, pDecoderContext->mAudioStreamHandler->m_basicProperties.m_maxAUSize);
#endif	
    ...
    // Start the decoder
#ifdef VE_OMX_DEC 
	metaPara = new MetaData;
	metaPara->setInt32(kKeyInputBufferNum, 4);//for android omx decoder queue num
	metaPara->setInt32(kKeyOutputBufferNum, 4);
    if(pDecoderContext->mDecoderType == M4AD_kTypeAMRNB ||
		pDecoderContext->mDecoderType == M4AD_kTypeAMRWB ){
		outbufsize =pDecoderContext->mAudioStreamHandler->m_byteFrameLength *
					 pDecoderContext->mAudioStreamHandler->m_byteSampleSize *
					 pDecoderContext->mAudioStreamHandler->m_nbChannels;
		metaPara->setInt32(kKeyOutBufSize,outbufsize);
		ALOGV("VideoEditorAudioDecoder_create:outbufsize=%d",outbufsize);
    }
	if(pDecoderContext->mDecoderType == M4AD_kTypeMP3){
		metaPara->setInt32(kKeyFrameNum,1);
	}
	result = pDecoderContext->mDecoder->start(metaPara.get());
#else
...
M4OSA_ERR VideoEditorAudioDecoder_processOutputBuffer(M4AD_Context pContext,
    ...
    status_t result = OK;
#ifdef DUMPOUTPUTPCM
	FILE *fp = fopen("/sdcard/audiodec_output.pcm", "ab");
#endif
#ifdef VE_OMX_DEC
	buffer->meta_data()->findInt64(kKeyTime, &i64Tmp);
	ALOGV("VideoEditorAudioDecoder_processOutputBuffer begin buffer timestamp %lld 0x%x  len=%d",i64Tmp,buffer->data(),buffer->range_length());
#else
	...
	ALOGD("buffer->range_length =%d\n",buffer->range_length());
    // Process the returned data
    if( 0 == buffer->range_length() ) {
        // Decoder has no data yet, nothing unusual
#ifndef ANDROID_DEFAULT_CODE        
		int32_t outbufsize = (pDecoderContext->mAudioStreamHandler->m_byteFrameLength *
					 pDecoderContext->mAudioStreamHandler->m_byteSampleSize *
					 pDecoderContext->mAudioStreamHandler->m_nbChannels);		
		memset( (void *)pOuputBuffer->m_dataAddress, 0, outbufsize);
		ALOGD("buffer->range_length 0 sze%d %d %d %d",outbufsize, 
			pDecoderContext->mAudioStreamHandler->m_byteFrameLength,
			pDecoderContext->mAudioStreamHandler->m_byteSampleSize,
			pDecoderContext->mAudioStreamHandler->m_nbChannels);
	...
	if(fp)
	{
		fwrite(buffer->data(), 1, buffer->range_length(), fp);
		fclose(fp);
	}
...
#ifdef	VE_OMX_DEC
	ALOGV("s1 needSeek=%d ,time=%d",needSeek,pDecoderContext->timeStampMs);
	if (needSeek) {
		MediaSource::ReadOptions options;
		options.setSeekTo(pDecoderContext->timeStampMs);
		result = pDecoderContext->mDecoder->read(&outputBuffer, &options);
		needSeek = false;
	}else{
#endif
    // Read
...
#ifdef VE_OMX_DEC
		if (pDecoderContext->mDecoderType == M4AD_kTypeAAC){
			if(meta->findInt32(kKeyAacProfile, &profile)&& profile >0){			
				switch (profile){
					case 2:
			            /* Audio Object Type is 2 (AAC Low Complexity) */
						pDecoderContext->aactype = M4_kAAC;
			        ...
			        case 5:
			            /* Audio Object Type is 5 (Spectral Band Replication) */
						pDecoderContext->aactype = M4_kAACplus;
			            break;
			        case 29:
			            /* Audio Object Type is 29 (Parametric Stereo) */
						pDecoderContext->aactype = M4_keAACplus;
			            break;
					default:
						pDecoderContext->aactype = M4_kAAC;
						break;
			...
			ALOGD("VideoEditorAudioDecoder_step M4WAR_INFO_FORMAT_CHANGE profile %d",profile);
		}
#endif
    ...
    switch( optionID ) {
        ...
        case M4AD_kOptionID_AuCTS:
            *(uint32_t *)optionValue = pDecoderContext->timeStampMs;
            break;
#ifdef VE_OMX_DEC
		case M4AD_kOptionID_StreamType:
			if(M4DA_StreamTypeAudioAac ==
                pDecoderContext->mAudioStreamHandler->m_basicProperties.m_streamType){               
				*(uint32_t *)optionValue = pDecoderContext->aactype;
			}
			break;
#endif
