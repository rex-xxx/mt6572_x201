...
#ifndef ANDROID_DEFAULT_CODE
	memset(pC , 0, sizeof(M4MCS_InternalContext));
#endif
...
#ifndef ANDROID_DEFAULT_CODE
	pC->iReaderCut = M4OSA_FALSE;
	pC->dAuDecCurrentCts = 0;
#endif
    /**
    * Video decoder stuff */
                     ...
                     M4OSA_Void *pTempFile )
{
    ...
    * Reset reader related variables */
    ...
    pC->pReaderAudioStream = M4OSA_NULL;
#ifndef ANDROID_DEFAULT_CODE
    ALOGD("M4MCS_open():InputFileType=%d",InputFileType);
#endif
    /*******************************************************/
        ...
        * In case we found a BIFS stream or something else...*/
        ...
        if( M4NO_ERROR == err ) /**< One stream found */
        {
...
#ifndef ANDROID_DEFAULT_CODE            
            ALOGD("M4MCS_open():find Video Stream:m_streamType=%d (0 mpeg4/1 h263/9 avc)", pStreamHandler->m_streamType);
#endif            
            ...
            * Found the first audio stream */
            ...
            {
#ifndef ANDROID_DEFAULT_CODE            	
             ALOGD("M4MCS_open():find Audio Stream:m_streamType=%d (aac/4, mp3/5)", pStreamHandler->m_streamType);
#endif             
    ...
    * Check state automaton */
    ...
    switch( pC->State )
    {
        case M4MCS_kState_READY:
            *pProgress = 0;
#ifndef ANDROID_DEFAULT_CODE            
		ALOGD("M4MCS_step:M4MCS_kState_READY,*pProgress=%d",*pProgress);
#endif		
...
#ifndef ANDROID_DEFAULT_CODE                
		   if(*pProgress > 99)
		   	ALOGD("M4MCS_step:M4MCS_kState_BEGINVIDEOJUMP,*pProgress=%d, EncodingVideoFormat=%d",*pProgress,pC->EncodingVideoFormat);
#endif		   	
                return err;
            }
            break;
...
#ifndef ANDROID_DEFAULT_CODE
    ALOGD("M4MCS_pause(): returning M4NO_ERROR");
#endif    
    return M4NO_ERROR;
}
...
#ifndef ANDROID_DEFAULT_CODE
    ALOGD("M4MCS_resume(): returning M4NO_ERROR");
#endif    
    return M4NO_ERROR;
}
    ...
    if( M4OSA_NULL != pC->pWriterContext ) /* happens in state _SET */
    {
        ...
        if( pC->novideo != M4OSA_TRUE )
        {
                ...
                && (M4ENCODER_kNULL == pC->EncodingVideoFormat) )
            {
                ...
                {
#ifndef ANDROID_DEFAULT_CODE                	
                    ALOGV("Decoder specific info size = %d",
                        pC->m_pInstance->m_decoderSpecificInfoSize);
#endif	                        
                    ...
                    {
#ifndef ANDROID_DEFAULT_CODE                    	
                        ALOGE("instance->m_pFinalDSI: allocation error");
#endif                        
                        return M4ERR_ALLOC;
                    }
...
#ifndef ANDROID_DEFAULT_CODE                    
                ALOGV("calling set option M4WRITER_kDSI");
#endif                
                err = pC->pWriterGlobalFcts->pFctSetOption(pC->pWriterContext,
                    M4WRITER_kDSI, &streamHeader);
                M4OSA_TRACE1_0("set option done");
...
#ifndef ANDROID_DEFAULT_CODE
            {
                ...
                ALOGI("set duration as %d, clip=%d, begin=%d, end=%d", 
                        duration, pC->InputFileProperties.uiClipDuration, pC->uiBeginCutTime, pC->uiEndCutTime);

                err = pC->pWriterGlobalFcts->pFctSetOption(pC->pWriterContext,
                        M4WRITER_kMaxFileDuration, &duration);
                if( ( M4NO_ERROR != err) )
                {
                    M4OSA_TRACE1_1(
                        "M4MCS_close: failed to set the video duration (err 0x%x)",
                        err);
                    /**< no return here, we still have stuff to deallocate after close, even
                     if it fails. */
                }
...
#ifndef ANDROID_DEFAULT_CODE
    ALOGD("M4MCS_close(): returning M4NO_ERROR");
#endif    
    return err;
}
                                ...
                                M4MCS_OutputParams *pParams )
{
    ...
    M4OSA_TRACE2_2(
        ...
        pContext, pParams);
#ifndef ANDROID_DEFAULT_CODE        
	ALOGD("==========================M4MCS_setOutputParams========================== \n");
	ALOGD("M4MCS_setOutputParams: OutputFileType=%d,OutputVideoFormat=%d,OutputVideoFrameRate=%d, pC->InputFileProperties.VideoStreamType=%d",
   	pParams->OutputFileType,pParams->OutputVideoFormat,pParams->OutputVideoFrameRate, pC->InputFileProperties.VideoStreamType);

	ALOGD("M4MCS_setOutputParams: outputVideoProfile=%d,outputVideoLevel=%d,OutputAudioFormat=%d ",
	   							pParams->outputVideoProfile,pParams->outputVideoLevel,pParams->OutputAudioFormat );

	ALOGD("M4MCS_setOutputParams: MediaRendering=%d,nbEffects=%d,OutputVideoFrameSize=%d ",
	   							pParams->MediaRendering,pParams->nbEffects,pParams->OutputVideoFrameSize );
	ALOGD("========================================================================= \n");
...
#ifndef ANDROID_DEFAULT_CODE            
		ALOGD("M4MCS_setOutputParams():pParams->pEffects[%d].AudioEffectType=%d",j,pParams->pEffects[j].AudioEffectType);
#endif		
...
#ifndef ANDROID_DEFAULT_CODE    
 ALOGD("=============================M4MCS_setEncodingParams=============================");
 ALOGD("M4MCS_setEncodingParams:uiVideoBitrate=%d,uiAudioBitrate=%d,uiBeginCutTime=%d,uiEndCutTime=%d",
		pRates->OutputVideoBitrate,pRates->OutputAudioBitrate,pRates->BeginCutTime,pRates->EndCutTime);
 
 ALOGD("M4MCS_setEncodingParams:uiClipDuration=%d,OutputVideoTimescale=%d, OutputFileSize=%d",
		pC->InputFileProperties.uiClipDuration,pRates->OutputVideoTimescale,pRates->OutputFileSize);
 ALOGD("=============================M4MCS_setEncodingParams=============================");
#endif 
...
M4OSA_ERR M4MCS_checkParamsAndStart( M4MCS_Context pContext )
{
    ...
    M4OSA_ERR err;
#ifndef ANDROID_DEFAULT_CODE    
   ALOGD("M4MCS_checkParamsAndStart:");
#endif
    /**
    * Check input parameters */
    M4OSA_DEBUG_IF2((M4OSA_NULL == pContext), M4ERR_PARAMETER,
...
#ifndef ANDROID_DEFAULT_CODE        
	  ALOGD("M4MCS_intStepSet:pC->bH264Trim = M4OSA_TRUE;");
#endif	  
    }
...
static M4OSA_ERR M4MCS_intPrepareVideoEncoder( M4MCS_InternalContext *pC )
{
    ...
    if( pC->novideo )
        return M4NO_ERROR;
#ifndef ANDROID_DEFAULT_CODE	
    ALOGD("M4MCS_intPrepareVideoEncoder:EncodingVideoFormat=%d,uiBeginCutTime=%d",
		pC->EncodingVideoFormat,pC->uiBeginCutTime );
#endif
...
#ifndef ANDROID_DEFAULT_CODE    
    ALOGD("M4MCS_intPrepareVideoEncoder: M4MCS_kEncoderRunning");
#endif	
...
#ifndef ANDROID_DEFAULT_CODE
        || pC->InputFileProperties.videoRotationDegrees != 0
#endif
        || ((M4OSA_UInt32)EncParams.FrameHeight
        != pC->pReaderVideoStream->m_videoHeight) )
    {
#ifndef ANDROID_DEFAULT_CODE
	ALOGD("M4MCS_intPrepareVideoEncoder: need resize (W %d,H %d)-> (W %d,H %d)",
			pC->pReaderVideoStream->m_videoWidth,pC->pReaderVideoStream->m_videoHeight,
			EncParams.FrameWidth,EncParams.FrameHeight);
#endif			
        /**
...
#ifndef ANDROID_DEFAULT_CODE
        // Demon Deng
        pC->pPreResizeFrame[0].u_stride = (pC->pPreResizeFrame[0].u_width + 0xf) & (~0xf);
#else
...
#ifndef ANDROID_DEFAULT_CODE
        pC->pPreResizeFrame[1].u_stride = (pC->pPreResizeFrame[1].u_width + 0x7) & (~0x7);
#else
...
#ifndef ANDROID_DEFAULT_CODE
        pC->pPreResizeFrame[2].u_stride = (pC->pPreResizeFrame[2].u_width + 0x7) & (~0x7);
#else
...
static M4OSA_ERR M4MCS_intPrepareAudioProcessing( M4MCS_InternalContext *pC )
{
...
#ifdef MCS_DUMP_PCM_TO_FILE
#ifndef ANDROID_DEFAULT_CODE
	file_au_reader = fopen("sdcard/mcs_ReaderOutput.raw", "wb");
	file_pcm_decoder = fopen("sdcard/mcs_DecoderOutput.pcm", "wb");
	file_pcm_encoder = fopen("sdcard/mcs_EncoderInput.pcm", "wb");
    ...
    * Allocate output buffer for the audio decoder */
...
#ifndef ANDROID_DEFAULT_CODE
	if(M4VIDEOEDITING_kAAC == pC->InputFileProperties.AudioStreamType){
		pC->InputFileProperties.uiDecodedPcmSize =
		        pC->pReaderAudioStream->m_byteFrameLength
		        * pC->pReaderAudioStream->m_byteSampleSize
		        * pC->pReaderAudioStream->m_nbChannels;		
	}//aac max output buffer size is 2048 sample.
	else if(M4VIDEOEDITING_kAACplus == pC->InputFileProperties.AudioStreamType ||
		 M4VIDEOEDITING_keAACplus == pC->InputFileProperties.AudioStreamType){
		pC->InputFileProperties.uiDecodedPcmSize =
		        pC->pReaderAudioStream->m_byteFrameLength*2
		        * pC->pReaderAudioStream->m_byteSampleSize
		        * pC->pReaderAudioStream->m_nbChannels;		
	}//aac max output buffer size is 2048 sample.

    ALOGD("pC->InputFileProperties.AudioStreamType %d pC->InputFileProperties.uiDecodedPcmSize %d",pC->InputFileProperties.AudioStreamType,pC->InputFileProperties.uiDecodedPcmSize);
#endif
...
#ifndef ANDROID_DEFAULT_CODE
	memset (pC->AudioDecBufferOut.m_dataAddress, 0, pC->AudioDecBufferOut.m_bufferSize * sizeof(short));
#endif
    ...
    switch( pC->pReaderAudioStream->m_samplingFrequency )
    {
        ...
        case 96000:
            ssrcParams.SSRC_Fs_In = LVM_FS_96000;
            break;
...
static M4OSA_ERR M4MCS_intPrepareWriter( M4MCS_InternalContext *pC )
{
    ...
    * If there is a video input, allocate and fill the video stream structures for the writer */
    if( pC->novideo == M4OSA_FALSE )
    {
        ...
        * Update Encoder Header properties for Video stream if needed */
        ...
        {
#ifndef ANDROID_DEFAULT_CODE        	
            ALOGD("M4MCS_intPrepareWriter:M4WRITER_kMUL_PPS_SPS");
#endif            
...
#ifndef ANDROID_DEFAULT_CODE
    ALOGD("M4MCS_intPrepareWriter:W=%d,H=%d,EncodingVideoFormat=%d,EncodingVideoFramerate=%d,uiMaxFileSize=%d\n\
		noaudio=%d,novideo=%d,uiBeginCutTime=%d,uiEndCutTime=%d",
		pC->EncodingWidth,pC->EncodingHeight,pC->EncodingVideoFormat,pC->EncodingVideoFramerate, pC->uiMaxFileSize,
		pC->noaudio,	 pC->novideo,pC->uiBeginCutTime,pC->uiEndCutTime);
#endif		
    if( M4NO_ERROR != err )
    {
        M4OSA_TRACE1_1(
    ...
    * Return with no error */
#ifndef ANDROID_DEFAULT_CODE    
    ALOGD("M4MCS_intPrepareWriter(): returning M4NO_ERROR");
#endif    
    return M4NO_ERROR;
}
...
static M4OSA_ERR M4MCS_intPrepareAudioBeginCut( M4MCS_InternalContext *pC )
{
    ...
    * In that case we need to slice the first AU to keep the 20 ms cut precision */
	...
	pC->dAuDecCurrentCts = pC->ReaderAudioAU.m_CTS;
	pC->iReaderCut = M4OSA_TRUE;
	ALOGD("M4MCS_intPrepareAudioBeginCut(): returning M4NO_ERROR pC->ReaderAudioAU.m_CTS %f pC->iAudioCtsOffset %d",pC->ReaderAudioAU.m_CTS,pC->iAudioCtsOffset);
#endif
    /**
    * Return with no error */
...
#ifndef ANDROID_DEFAULT_CODE
				*pProgress =
                    (M4OSA_UInt8)(( ( (M4OSA_UInt32)pC->dAuDecCurrentCts
                    - pC->uiBeginCutTime) * 100)
                    / (pC->uiEndCutTime - pC->uiBeginCutTime));
...
#ifndef ANDROID_DEFAULT_CODE
			if((pC->dAuDecCurrentCts+pC->m_audioAUDuration)> pC->uiEndCutTime){
				ALOGD("M4MCS_intStepEncoding pC->AudioState = M4MCS_kStreamState_FINISHED %f",pC->dAuDecCurrentCts);
				pC->AudioState = M4MCS_kStreamState_FINISHED;
                break;
			}
#else
...
#ifndef ANDROID_DEFAULT_CODE        
	  ALOGD("M4MCS_intStepBeginVideoJump:H264MCS_ProcessSPS_PPS");
#endif	  
...
static M4OSA_ERR M4MCS_intAudioNullEncoding( M4MCS_InternalContext *pC )
{
    ...
    * Prepare the writer AU */
	...
	if (pC->WriterAudioAU.size == 6)
	{
		M4OSA_TRACE1_5("WriterpNULLOutputfile len6 0x%x,0x%x,0x%x,0x%x,0x%x\n",*((M4OSA_UInt8 *)(pC->WriterAudioAU.dataAddress)),*((M4OSA_UInt8 *)(pC->WriterAudioAU.dataAddress)+1),
			*((M4OSA_UInt8 *)(pC->WriterAudioAU.dataAddress)+2),*((M4OSA_UInt8 *)(pC->WriterAudioAU.dataAddress)+3),*((M4OSA_UInt8 *)(pC->WriterAudioAU.dataAddress)+4)); 		

	}


#ifdef MCS_DUMP_NULL_ENCODEDATA_TO_FILE
	
#ifndef ANDROID_DEFAULT_CODE
			FILE *file_nullencoder = fopen("sdcard/mcs_nullEncoderOutput.aac", "ab");
			
#else
			FILE *file_nullencoder = fopen("mcs_nullEncoderOutput.aac", "ab");
#endif
	
			fwrite((void *)pC->WriterAudioAU.dataAddress, pC->WriterAudioAU.size, 1,
				file_nullencoder);
	
			if( file_nullencoder )
			{
				fclose(file_nullencoder);
				file_nullencoder = NULL;
			}
#endif		
...
#ifndef	ANDROID_DEFAULT_CODE
	if( M4WAR_NO_MORE_AU == err2 )
	{
		/* The audio transcoding is finished */
		pC->AudioState = M4MCS_kStreamState_FINISHED;
		ALOGD( "M4MCS_intAudioNullEncoding(): \
			m_pReaderDataIt->m_pFctGetNextAu(audio) returns M4WAR_NO_MORE_AU,M4MCS_kStreamState_FINISHED");
		return err2;
	}
...
#ifndef ANDROID_DEFAULT_CODE
	if(pC->iReaderCut){
		if (pC->AudioDecBufferOut.m_dataAddress != NULL)
		{
			memset (pC->AudioDecBufferOut.m_dataAddress, 0, pC->AudioDecBufferOut.m_bufferSize * sizeof(short));
		}			
		err = pC->m_pAudioDecoder->m_pFctStepAudioDec(pC->pAudioDecCtxt,
				M4OSA_NULL, &pC->AudioDecBufferOut, M4OSA_TRUE);
		pC->iReaderCut = M4OSA_FALSE;
	}else{
#endif
    err = pC->m_pAudioDecoder->m_pFctStepAudioDec(pC->pAudioDecCtxt,
...
#ifdef MCS_DUMP_PCM_TO_FILE
#ifndef ANDROID_DEFAULT_CODE
		file_pcm_decoder = fopen("sdcard/mcs_DecoderOutput.pcm", "ab");
#else
		file_pcm_decoder = fopen("mcs_DecoderOutput.pcm", "ab");
#endif
#endif
...
#ifndef ANDROID_DEFAULT_CODE
			file_pcm_encoder = fopen("sdcard/mcs_EncoderInput.pcm", "ab");
		
#else
			file_pcm_encoder = fopen("mcs_EncoderInput.pcm", "ab");
#endif
...
#ifdef MCS_DUMP_ENCODEDATA_TO_FILE

#ifndef ANDROID_DEFAULT_CODE
		FILE *file_aacencoder = fopen("sdcard/mcs_EncoderOutput.aac", "ab");
		
#else
		FILE *file_aacencoder = fopen("mcs_EncoderOutput.aac", "ab");
#endif

	    fwrite(pEncOutBuffer.pTableBuffer[0], pEncOutBuffer.pTableBufferSize[0], 1,
	        file_aacencoder);

	    if( file_aacencoder )
	    {
	        fclose(file_aacencoder);
	        file_aacencoder = NULL;
	    }
#endif		
...
#ifndef ANDROID_DEFAULT_CODE
		pC->dAuDecCurrentCts += pC->m_audioAUDuration;	
#endif
    }
    else
    {
...
#ifndef ANDROID_DEFAULT_CODE            
		ALOGE("M4MCS_intGetInputClipProperties:videoRotationDegrees=%d", pC->pReaderVideoStream->videoRotationDegrees);
#endif		
    }
    else
    {
...
#ifndef ANDROID_DEFAULT_CODE
			pC->InputFileProperties.uiDecodedPcmSize *=2;//aac max sample per frame 2048
#endif
        }
#ifndef ANDROID_DEFAULT_CODE
		if( pC->InputFileProperties.VideoStreamType == M4VIDEOEDITING_kNoneVideo ){
			if(pC->InputFileProperties.uiSamplingFrequency > 48000 || pC->InputFileProperties.uiSamplingFrequency < 8000)
			{
				ALOGD("Video Editor support 8KHz ~ 48KHz only! nowe samplerate %d",pC->InputFileProperties.uiSamplingFrequency);
				pC->bUnsupportedAudioFound=M4OSA_TRUE;
				pC->InputFileProperties.AudioStreamType = M4VIDEOEDITING_kUnsupportedAudio;
			}
		}
#endif
    }
...
#ifndef ANDROID_DEFAULT_CODE
    ALOGI("duration v=%d, a=%d", pC->InputFileProperties.uiClipVideoDuration, 
            pC->InputFileProperties.uiClipAudioDuration);
#endif
...
M4OSA_ERR M4MCS_intCheckAndGetCodecProperties(
...
#ifndef ANDROID_DEFAULT_CODE
    ALOGD("M4MCS_intCheckAndGetCodecProperties :start");
#endif    
    ...
    * Allocate output buffer for the audio decoder */
	...
	if (M4DA_StreamTypeAudioAac == 	
				pC->pReaderAudioStream->m_basicProperties.m_streamType){	
		//soft aac decoder force stereo,and max sample per frame is 2048.		
		outputBuffer.m_bufferSize = 2048 * pC->pReaderAudioStream->m_byteSampleSize* 2;
	}//prevent size no enough,no must
#endif
...
#ifndef ANDROID_DEFAULT_CODE
		if (outputBuffer.m_dataAddress != NULL)
		{
			memset (outputBuffer.m_dataAddress, 0, outputBuffer.m_bufferSize*sizeof(short));
		}	
#endif		
    }
    ...
    if ( err == M4WAR_INFO_FORMAT_CHANGE ) {
        ...
        if (M4DA_StreamTypeAudioAac ==
            ...
            pC->AacProperties.aSampFreq =
                pC->pReaderAudioStream->m_samplingFrequency;
#ifndef ANDROID_DEFAULT_CODE
			M4_AACType iAacType2 = 0;
			pC->m_pAudioDecoder->m_pFctGetOptionAudioDec(pC->pAudioDecCtxt, 
				M4AD_kOptionID_StreamType,(M4OSA_DataOption) &iAacType2);
			switch( iAacType2 )
            {
                case M4_kAAC:
                    pC->AacProperties.aSBRPresent = 0;
                ...
                case M4_kAACplus:
                    ...
                    pC->AacProperties.aPSPresent = 0;
                    pC->AacProperties.aExtensionSampFreq =
                        pC->pReaderAudioStream->m_samplingFrequency; //TODO
                    break;
			 ...
			 ALOGD( "M4MCS_intGetInputClipProperties: AAC aSBRPresent=%d, aPSPresent=%d",
                        pC->AacProperties.aSBRPresent,
                        pC->AacProperties.aPSPresent);
#endif
            }
