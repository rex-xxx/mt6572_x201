...
#include <utils/Log.h>
#define LOG_TAG "M4VSS3GPP_EditAudio"
//#define DumpAudioPCMData
#endif
        ...
        case M4VSS3GPP_kEditAudioState_READ_WRITE:
            {
                ...
                * (the BZZZ effect occurs even with the second AU!) */
                    ...
                    < (pC->ewc.iSilenceFrameDuration + 1)) )
                {
                    ...
                    * Copy a silence AU to the output */
                    ...
                    M4OSA_TRACE1_1("A #### silence AU %d",pC->ewc.uiSilenceFrameSize);
                }
...
#ifndef ANDROID_DEFAULT_CODE     
					/* vend_jogi.lin reset 3gp reader to avoid AAC decoder read data overwriting 3gpp data */
					if (pC->pC1->isFirstFrame)
					{
						err = M4VSS3GPP_intResetReader(pC->pC1);
						if(( M4ERR_STATE == err ) || ( M4NO_ERROR == err))
						{
							pC->pC1->isFirstFrame = 0;
						}				
						else if( M4NO_ERROR != err )
						{
							M4OSA_TRACE1_1(
								"M4VSS3GPP_intEditStepAudio: READ_WRITE:\
								M4VSS3GPP_intResetReader returns 0x%x!",
								err);
							return err;
						}						
						

						if ((pC->pC1->isFirstFrame == 0) && ( M4NO_ERROR == err)) 
						{
			                err = M4VSS3GPP_intClipReadNextAudioFrame(pC->pC1);

			                M4OSA_TRACE2_3("init .... read  : cts  = %.0f + %.0f [ 0x%x ]",
                ...
                * This boolean is only used to fix the BZZ bug... */
                ...
                /**
                * Write the AU */
				if (pC->ewc.WriterAudioAU.size == 6)
				{
					ALOGI("[READ_WRITE] pAudioFramePtr 0x%x WriterAudioAU len6 0x%x,0x%x,0x%x,0x%x,0x%x,0x%x\n",
						(void *)pC->pC1->pAudioFramePtr,
						*((M4OSA_UInt8 *)pC->ewc.WriterAudioAU.dataAddress),
						*((M4OSA_UInt8 *)(pC->ewc.WriterAudioAU.dataAddress)+1),
						*((M4OSA_UInt8 *)(pC->ewc.WriterAudioAU.dataAddress)+2),
						*((M4OSA_UInt8 *)(pC->ewc.WriterAudioAU.dataAddress)+3),
						*((M4OSA_UInt8 *)(pC->ewc.WriterAudioAU.dataAddress)+4),
						*((M4OSA_UInt8 *)(pC->ewc.WriterAudioAU.dataAddress)+5));						
				}
...
#ifdef DumpAudioPCMData				

				FILE *fp3 = fopen("/sdcard/transition.pcm", "ab");
				if(fp3){
					fwrite( pEncInBuffer.pTableBuffer[0], 1,pEncInBuffer.pTableBufferSize[0], fp3);
					fclose(fp3);
				}
#endif
#ifdef M4VSS_SUPPORT_OMX_CODECS
