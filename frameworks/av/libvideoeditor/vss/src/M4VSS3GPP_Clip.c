...
#ifndef	ANDROID_DEFAULT_CODE
	pClipCtxt->bAuReadSeek = M4OSA_FALSE;
#endif
...
M4OSA_ERR M4VSS3GPP_intResetReader( M4VSS3GPP_ClipContext *pClipCtxt)
{
    M4OSA_ERR err;

	if (( pClipCtxt->pReaderContext != NULL) && ((M4_StreamHandler *)pClipCtxt->pAudioStream != NULL))
	{
		...
		{
			M4OSA_TRACE1_1(
				"M4VSS3GPP_intResetReader(): m_pReader->m_pFctReset(audio) returns 0x%x",
				err);
			return err;
		}
	}
    ...
    M4VSS3GPP_ClipContext *pClipCtxt )
{
    ...
    /* ---------------------------------- */
        ...
        == pClipCtxt->pSettings->ClipProperties.AudioStreamType) )
    {
		M4OSA_TRACE1_0("pAudioFramePtr AMR Path");
        ...
        else /* bAudioFrameAvailable */
        {
            ...
            * Go to the next AMR frame in the AU */
			...
			M4OSA_TRACE1_1("pAudioFramePtr + %d",pClipCtxt->uiAudioFrameSize);
            /**
        ...
        == pClipCtxt->pSettings->ClipProperties.AudioStreamType) )
    {
        ...
        if( M4NO_ERROR == err )
        {
            ...
            pClipCtxt->iAudioFrameCts =
                ...
                + 0.5);
			M4OSA_TRACE1_2("[M4VSS3GPP_intClipReadNextAudioFrame] : 0x%x uiAudioFrameSize=%d", pClipCtxt->pAudioFramePtr, pClipCtxt->uiAudioFrameSize);

			if (pClipCtxt->uiAudioFrameSize == 6)
			{
				M4OSA_TRACE1_5("[M4VSS3GPP_intClipReadNextAudioFrame] len6 0x%x,0x%x,0x%x,0x%x,0x%x\n",*((M4OSA_UInt8 *)(pClipCtxt->AudioAU.m_dataAddress)),*((M4OSA_UInt8 *)(pClipCtxt->AudioAU.m_dataAddress)+1),
					*((M4OSA_UInt8 *)(pClipCtxt->AudioAU.m_dataAddress)+2),*((M4OSA_UInt8 *)(pClipCtxt->AudioAU.m_dataAddress)+3),*((M4OSA_UInt8 *)(pClipCtxt->AudioAU.m_dataAddress)+4));					
			}
        ...
        else if( M4WAR_NO_MORE_AU == err )
        {
            ...
            * we return silence at the end of the stream. */
            ...
            M4OSA_TRACE1_1("a #### silence AU %d",pClipCtxt->uiAudioFrameSize);
    ...
    M4VSS3GPP_ClipContext *pClipCtxt )
{
    ...
    * Standard decoding mode */
    ...
    {
#ifndef ANDROID_DEFAULT_CODE
		if (pClipCtxt->AudioDecBufferOut.m_dataAddress != NULL)
		{
			memset (pClipCtxt->AudioDecBufferOut.m_dataAddress, 0, pClipCtxt->AudioDecBufferOut.m_bufferSize * sizeof(M4OSA_Int16));
		}	
#endif		
        /**
...
#ifndef	ANDROID_DEFAULT_CODE
	pClipCtxt->bAuReadSeek = M4OSA_TRUE;
#endif
    /**
    * Return with no error */
...
M4OSA_ERR M4VSS3GPP_intCheckAndGetCodecAacProperties(
    ...
    * Allocate output buffer for the audio decoder */
    ...
    if( outputBuffer.m_bufferSize > 0 ) {
		...
		{
			memset(outputBuffer.m_dataAddress, 0, outputBuffer.m_bufferSize * sizeof(M4OSA_Int16));
		}	
#endif			
    }
