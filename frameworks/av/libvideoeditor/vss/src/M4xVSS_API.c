            ...
            == M4VIDEOEDITING_kFileType_M4V )
        {
           ...
           if ((fileProperties.uiVideoProfile >
                ...
                (fileProperties.uiVideoLevel >
                     xVSS_context->pSettings->xVSS.outputVideoLevel) ||
#ifndef ANDROID_DEFAULT_CODE
                (fileProperties.VideoStreamType != xVSS_context->pSettings->xVSS.outputVideoFormat) ||
#endif
                (fileProperties.VideoStreamType == M4VIDEOEDITING_kMPEG4)) {
            ...
            if( videoIsDifferent == M4OSA_TRUE || audioIsDifferent == M4OSA_TRUE)
            {
                ...
                * End of the UTF conversion, use the converted file path*/
                ...
                memcpy((void *)pParams->pFileOut, (void *)pDecodedPath,
                    (length + 1)); /* Copy output file path */
#if 0     // tmp pop noise clarification
				if ( NULL != strstr(pParams->pFileOut, "vid0.3gp"))
				{
					for ( i = 0; i < xVSS_context->pSettings->uiClipNumber; i++ )
					{								
						if (xVSS_context->pSettings->pTransitionList[i] != M4OSA_NULL)
						{
							if (xVSS_context->pSettings->pTransitionList[i].AudioTransitionType != NULL)
						 	{
								M4OSA_TRACE1_1("Transition pop %d ",xVSS_context->pSettings->pTransitionList[i].AudioTransitionType);
						 	}
						}
					}
				}
...
#ifndef ANDROID_DEFAULT_CODE
    ALOGD("M4xVSS_Step state %d analyse %d editing %d", xVSS_context->m_state, xVSS_context->analyseStep, xVSS_context->editingStep);
#endif
...
#ifndef ANDROID_DEFAULT_CODE
                        // we will do rotation in MCS, no need to pass this value
                        rotationDegree = 0;
#endif
