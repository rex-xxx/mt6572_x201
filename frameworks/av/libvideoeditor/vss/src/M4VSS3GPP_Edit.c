...
#ifndef ANDROID_DEFAULT_CODE
        } else if (pC->pClipList[i].ClipProperties.videoRotationDegrees != 0) {
            // Demon, MCS doesn't do rotation, tell VSS to do it.
            ALOGI("rotation %d file still needs transcoding", pC->pClipList[i].ClipProperties.videoRotationDegrees);
#endif
        } else {
...
#ifndef ANDROID_DEFAULT_CODE
            ALOGI("change state in VideoEdit not here, Vstate = %d", pC->Vstate);
#else
            pC->Vstate = M4VSS3GPP_kEditVideoState_READ_WRITE;
#endif
        }
