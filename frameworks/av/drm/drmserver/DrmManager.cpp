...
int DrmManager::addUniqueId(bool isNative) {
    ALOGD("DrmManager::addUniqueId()");
    Mutex::Autolock _l(mLock);

    int temp = 0;
    bool foundUniqueId = false;
    const int size = mUniqueIdVector.size();
    ALOGD("DrmManager::addUniqueId() : server side try to find a unique id .size[%d]-------->",size);
    ...
    ALOGD("DrmManager::addUniqueId() : server side generate unique id [%d]", temp);
...
void DrmManager::removeUniqueId(int uniqueId) {
    Mutex::Autolock _l(mLock);
    ALOGD("DrmManager::removeUniqueId() : uniqueId [%d]----------------->", uniqueId);
