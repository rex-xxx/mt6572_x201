...
#ifdef  MTK_CAMERA_BSP_SUPPORT
        // To avoid release/new MediaPlayer when switching between main/sub sensor, and it will reduce the switch time.
        //releaseSound();  //should not releaseSound here, because ~CameaClient() will do this instead .
#endif
        return NULL;
    }
...
void CameraService::loadSound() {
    ...
    if (policy == SCHED_OTHER) {
        ...
        oriPriority = ::getpriority(PRIO_PROCESS, 0);
        LOG1("[CameraService::loadSound] tid(%d) ori_policy(SCHED_OTHER:%d) ori_priority(%d)", ::gettid(), policy, oriPriority);
        //we want to increase the thread priority for loadSound
        ::setpriority(PRIO_PROCESS, 0, PRIO_MIN);   //  Note: "priority" is nice value.
        LOG1("[CameraService::loadSound] tid(%d) set_policy(SCHED_OTHER:%d) set_priority(%d)",
                 ::gettid(), policy, PRIO_MIN);
    }
    else 
    {
        LOG1("[CameraService::loadSound] thread priroity is already RR"); 
    }
...
#ifdef MTK_CAMERA_BSP_SUPPORT
    // set the thread priority to original 
    if (policy == SCHED_OTHER) 
    {
        ::setpriority(PRIO_PROCESS, 0, oriPriority);   //  Note: "priority" is nice value.
         LOG1("[CameraService::loadSound] set thread to ori priority:%d", oriPriority); 
    }    
#endif 
}
