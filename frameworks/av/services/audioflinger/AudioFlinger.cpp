...
#ifdef AUDIO_HARDWARE_MODULE_ID
#undef AUDIO_HARDWARE_MODULE_ID
#endif
...
#undef ALOGV
#undef ALOGVV
#define ALOGV   MTK_ALOG_D
#define ALOGVV  MTK_ALOG_V
#else
...
//add by weiguo to dump pcm data
    ...
    static   const char * gaf_mixer_pcm = "/sdcard/mtklog/audio_dump/af_mixer_pcm.pcm";
    static   const char * gaf_mixer_propty = "af.mixer.pcm";

    static   const char * gaf_record_pcm = "/sdcard/mtklog/audio_dump/af_record.pcm";
    static   const char * gaf_record_propty = "af.record.dump.pcm";
#endif
#endif
...
{
    MTK_ALOG_V("MTK_downmix_to_mono_i16_from_stereo_i16");
    while (count--) {
        *dst++ = src[0];
        src += 2;
    }
...
{
    MTK_ALOG_V("SetAudioData par1 = %d,len = %d ",par1,len);
    AutoMutex lock(mHardwareLock);
    audio_hw_device_t *dev = mPrimaryHardwareDev->hwDevice();
    mHardwareStatus = AUDIO_HW_SET_PARAMETER;
...
{
    MTK_ALOG_V("GetAudioData par1 = %d,len = %d ",par1,len);
    AutoMutex lock(mHardwareLock);
    audio_hw_device_t *dev = mPrimaryHardwareDev->hwDevice();
    mHardwareStatus = AUDIO_HW_SET_PARAMETER;
...
{
    MTK_ALOG_V("SetAudioCommand par1 = %d,par2 = %d ",parameters1,parameters2);
    AutoMutex lock(mHardwareLock);
    audio_hw_device_t *dev = mPrimaryHardwareDev->hwDevice();
    mHardwareStatus = AUDIO_HW_SET_PARAMETER;
...
{
    MTK_ALOG_V("GetAudioCommand par1 = %d",parameters1);
    int value =0;
    AutoMutex lock(mHardwareLock);
    audio_hw_device_t *dev = mPrimaryHardwareDev->hwDevice();
    mHardwareStatus = AUDIO_HW_SET_PARAMETER;
...
{
    MTK_ALOG_V("+AudioFlinger::SetACFPreviewParameter!! ");
    AutoMutex lock(mHardwareLock);
    audio_hw_device_t *dev = mPrimaryHardwareDev->hwDevice();
    mHardwareStatus = AUDIO_HW_SET_PARAMETER;
    dev->SetACFPreviewParameter(dev,ptr,len);
    mHardwareStatus = AUDIO_HW_IDLE;
    MTK_ALOG_V("-SetACFPreviewParameter");
    return NO_ERROR;
}

status_t AudioFlinger::SetHCFPreviewParameter(void *ptr, size_t len)
{
    MTK_ALOG_V("+AudioFlinger::SetHCFPreviewParameter!! ");
    AutoMutex lock(mHardwareLock);
    audio_hw_device_t *dev = mPrimaryHardwareDev->hwDevice();
    mHardwareStatus = AUDIO_HW_SET_PARAMETER;
    dev->SetHCFPreviewParameter(dev,ptr,len);
    mHardwareStatus = AUDIO_HW_IDLE;
    MTK_ALOG_V("-SetHCFPreviewParameter");
    return NO_ERROR;
}
...
//
#ifdef HAVE_SRSAUDIOEFFECT
    if (access("/data/misc/srs_processing.cfg", F_OK) == -1)
    {
        MTK_ALOG_D("Create /data/misc/srs_processing.cfg");
	//check
        struct stat l_stat;
        int lstresult = lstat("/data/misc", &l_stat);
        MTK_ALOG_D("lstrresult=%d,mode_t=%u,uid_t=%u,gid_t=%u",lstresult, l_stat.st_mode, l_stat.st_uid,l_stat.st_gid);
	    if (lstresult == -1)
	    {
	        MTK_ALOG_E("lstat errro,errno=%d",errno);
	    }
        //
        FILE *pf1 = fopen("/data/misc/srs_processing.cfg", "wb");
        if (pf1  == NULL)
        {
           MTK_ALOG_E("can't open file:data/misc/srs_processing.cfg,error=%d",errno);
        }
        else
        {
            fclose(pf1);
            chmod("/data/misc/srs_processing.cfg", S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP|S_IROTH);
            MTK_ALOG_D("chmod /data/misc/srs_processing.cfg:rw-rw-r--");
        }
    }
#endif
//
...
/*
//Added by MTK80721 2012-09-09 Create Loundness
#ifdef HAVE_MTKLOUDNESS_EFFECT
        //if (streamType == AUDIO_STREAM_RING || streamType ==AUDIO_STREAM_ALARM)
        if ((thread->device() & AUDIO_DEVICE_OUT_SPEAKER) && eff_handle == 0)
        {
            int32_t priority = 0;
            effect_descriptor_t desc;
            int enabled = 0;
            int eStatus = 0;
            int loudness_sessionid = 0;
            MTK_ALOG_D("create loudness Effect+ ,sessionid=%d,streamtype=%d",*sessionId,streamType);
            //eStatus = EffectGetDescriptor(&peffect, &desc);
            eStatus = EffectGetDescriptor(SL_IID_MTKLOUDNESS, &desc);
            if (eStatus < 0)
            {
                MTK_ALOG_E("createEffect() error %d from EffectGetDescriptor", eStatus);
            }
            else
            {
                //eff_handle = thread->createEffect_l(NULL, NULL, priority, *sessionId, &desc, &enabled, &eStatus);
                eff_handle = thread->createEffect_l(NULL, NULL, priority, loudness_sessionid, &desc, &enabled, &eStatus);
                if (eff_handle != 0)
                {
                    MTK_ALOG_D("eff_handle=%d,enabled=%d",eff_handle.get(),enabled);
                    eff_handle->enable();
                }
            }
            MTK_ALOG_D("create loudness Effect- ");
        }
#endif
//-
*/
...
{
    MTK_ALOG_D("+setMode mode=%d",mode);
    status_t ret = initCheck();
    if (ret != NO_ERROR) {
        return ret;
    }
...
status_t AudioFlinger::setStreamMute(audio_stream_type_t stream, bool muted)
{
    ...
    if(stream == AUDIO_STREAM_MATV)
    {
        MTK_ALOG_D("setStreamMute MATV muted=%d",muted);
        char mute_vol[30];
        sprintf(mute_vol,"SetMatvMute=%d",muted);
        String8 Key = String8(mute_vol);
        //mPrimaryHardwareDev->set_parameters(mPrimaryHardwareDev,Key);
        audio_hw_device_t *dev = mPrimaryHardwareDev->hwDevice();
        dev->set_parameters(dev, Key);
    }
#endif
...
status_t AudioFlinger::setParameters(audio_io_handle_t ioHandle, const String8& keyValuePairs)
{
    ...
    if (ioHandle == 0) {
        ...
        MTK_ALOG_D("setParameters iohandle=0");
        return final_result;
    }
...
#ifdef MTK_AUDIO
			MTK_ALOG_W("Use Resampler with sampleRate  =	%d", sampleRate);
#else
...
void AudioFlinger::PlaybackThread::readOutputParameters()
{
    ...
    MTK_ALOG_V("readOutputParameters mSampleRate=%u,mChannelCount=%u,mFrameSize=%d,mFrameCount=%d",
           mSampleRate,mChannelCount,mFrameSize,mFrameCount);
...
#ifdef MTK_AUDIO
#ifdef DEBUG_AUDIO_PCM
	AudioDump::dump(gaf_mixer_pcm,mMixBuffer,mixBufferSize,gaf_mixer_propty);
#endif
#endif
...
void AudioFlinger::MixerThread::threadLoop_sleepTime()
{
    ...
    if (sleepTime == 0) {
        if (mMixerStatus == MIXER_TRACKS_ENABLED) {
            ...
            MTK_ALOG_D("threadLoop_sleepTime sleep time %u",sleepTime);
        ...
        MTK_ALOG_D("threadLoop_sleepTime write muted data to hw");
...
// check if prepare need to judge first track
#ifdef MTK_AUDIO
static bool MixerHardwareOutput(uint32_t mDevice)
{
    if (mDevice  == AUDIO_DEVICE_OUT_EARPIECE||
         mDevice  == AUDIO_DEVICE_OUT_SPEAKER||
         mDevice  == AUDIO_DEVICE_OUT_WIRED_HEADSET ||
         mDevice  == AUDIO_DEVICE_OUT_WIRED_HEADPHONE)
    {
        return true;
    }
    else
        ...
        Vector< sp<Track> > *tracksToRemove)
{
    ...
    for (size_t i=0 ; i<count ; i++) {
...
#endif
                ALOGW_IF(minFrames > cblk->frameCount,"minFrames(%u) > cblk->frameCount(%u)",minFrames,cblk->frameCount);
            }
        }
                ...
                !track->isPaused() && !track->isTerminated())
        {
            ...
            MTK_ALOG_V("track %d u=%08x, s=%08x [OK] on thread %p", name, cblk->user, cblk->server, this);
            mixedTracks++;
            ...
            if (track->isMuted() || track->isPausing() ||
                ...
                vl = vlr & 0xFFFF;
                vr = vlr >> 16;
                MTK_ALOG_V("typeVolume=%f,trackvolue1=%d,trackvoluer=%d,masterVolume=%f,name=%d",typeVolume, vl,vr,masterVolume,track->name());
            ...
            {
                if (track->auxEffectId() > 0)
                {
                    //disable aux effect
                    sp<EffectModule> effect = getEffect_l(AUDIO_SESSION_OUTPUT_MIX, track->auxEffectId());
                    if (effect != 0)
                    {
                        effect->clearbuf_l();
                    }
                ...
                /*
                //insert effect: bass boost/virtualizer
                if (track->isTerminated() && track->mainBuffer() != mMixBuffer)
		        {
		            sp<EffectChain> chain = getEffectChain_l(track->sessionId());
                    if (chain != 0)
                    {
            ...
            ALOGD("track %d u=%08x, s=%08x [NOT READY] on thread %p", name, cblk->user, cblk->server, this);
            if ((track->sharedBuffer() != 0) || track->isTerminated() ||
                ...
                {
                    // if track is stoped and has not enough data to satify above condition,remove it from active tracks
                    // For exampler, start track but do not write data to buffer and then stop track. 
                    // This track  can not be removed from activetracks.
                    if (track->isStopped() && track->framesReady()==0) 
                    {
                        mBytesWritten += mixBufferSize;
                    }
                }
                ...
                ALOGD("BUFFER TIMEOUT:   track->mUnderrunCount++;(%d) ",track->mUnderrunCount++);
                // No buffers for this track. Give it a few chances to
                // fill a buffer, then remove it from active list.
                if (--(track->mRetryCount) <= 0) {
                    ALOGW("BUFFER TIMEOUT: remove(%d) from active list on thread %p", name, this);
                    tracksToRemove->add(track);
                    // indicate to client process that the track was disabled because of underrun;
                    // it will then automatically call start() when data is available
...
void AudioFlinger::PlaybackThread::invalidateTracks(audio_stream_type_t streamType)
{
    ...
    for (size_t i = 0; i < size; i++) {
        ...
        if (t->streamType() == streamType) {
            ...
            {
                //disable aux effect
                sp<EffectModule> effect = getEffect_l(AUDIO_SESSION_OUTPUT_MIX, t->auxEffectId());
                if (effect != 0)
                {
                    MTK_ALOG_V("clear track audio aux effect temp buffer");
                    effect->clearbuf_l();
                }
            }
/*
            //insert effect: bass boost/virtualizer
            if (t->mainBuffer() != mMixBuffer)
		    {
		        sp<EffectChain> chain = getEffectChain_l(t->sessionId());
                if (chain != 0)
                {
                    //LOGD("clear track audio insert effect temp buffer");
                    chain->clearBuf();
                }
             }
*/
...
#ifdef HAVE_SRSAUDIOEFFECT
        // SRS Processing
        if (mOutDevice != AUDIO_DEVICE_NONE)
            POSTPRO_PATCH_JB_OUTPROC_PLAY_ROUTE(this, param, value);
        // SRS Processing
#endif
#endif
...
#ifdef MTK_AUDIO
    int frameCount = (4 * mNormalFrameCount * mSampleRate) / thread->sampleRate();
#else
        ...
        AudioBufferProvider::Buffer* buffer, int64_t pts)
{
    ...
    if (CC_LIKELY(framesReady)) {
        ...
        const int SIZE = 256;
        char fileName[SIZE];
        sprintf(fileName,"%s_%d_%p.pcm",gaf_track_pcm, name(),this);
        AudioDump::dump(fileName,buffer->raw,buffer->frameCount*cblk->frameSize,gaf_track_propty);
#endif
#endif
        return NO_ERROR;
    }
...
#ifdef MTK_AUDIO
        mMemoryDealer(new MemoryDealer(4096*1024, "AudioFlinger::Client")),
#else
...
AudioFlinger::TrackHandle::~TrackHandle() {
    ...
    MTK_ALOG_D("~TrackHandle trackName=%d",mTrack->name());
    mTrack->destroy();
}
...
status_t AudioFlinger::RecordThread::readyToRun()
{
    ...
    struct sched_param sched_p;
    sched_getparam(0, &sched_p);
    sched_p.sched_priority = RTPM_PRIO_AUDIO_RECORD;
    if(0 != sched_setscheduler(0, SCHED_RR, &sched_p))
    {
        MTK_ALOG_E("[%s] failed, errno: %d", __func__, errno);
    }
    ...
    while (!exitPending()) {
        ...
        if (mActiveTrack != 0) {
            ...
            if (CC_LIKELY(mActiveTrack->getNextBuffer(&buffer) == NO_ERROR)) {
...
#if defined(MTK_AUDIO) && defined(MTK_DOWNMIX_ENABLE)                                       
                ...
                if (mFramestoDrop == 0) {
#if defined(MTK_AUDIO) && defined(DEBUG_AUDIO_PCM)
                   AudioDump::dump(gaf_record_pcm, buffer.raw, (mActiveTrack->mCblk->frameSize)*(buffer.frameCount),gaf_record_propty);
#endif
                    mActiveTrack->releaseBuffer(&buffer);
                } else {
                    if (mFramestoDrop > 0) {
...
void AudioFlinger::purgeStaleEffects_l() {
    ...
    MTK_ALOG_D("-purging stale effects");
    return;
}
...
void AudioFlinger::ThreadBase::disconnectEffect(const sp<EffectModule>& effect,
    ...
    if (effect->removeHandle(handle) == 0) {
        if (!effect->isPinned() || unpinIfLast) {
            ...
            MTK_ALOG_D("disconnectEffect() unregisterEffect+");
            AudioSystem::unregisterEffect(effect->id());
            MTK_ALOG_D("disconnectEffect() unregisterEffect-");
        }
    }
...
#ifdef MTK_AUDIO
            status = NO_AUDIO_EFFECT;
            MTK_ALOG_E("Can't find aux effect,EffectId=%d,error=%d",EffectId,status);
#else
            MTK_ALOG_E("No AuxEffect for Effectid(%d)",EffectId);
            status = BAD_VALUE;
#endif
        }
    }
...
size_t AudioFlinger::EffectModule::disconnect(EffectHandle *handle, bool unpinIfLast)
{
    ...
    sp<EffectModule> keep(this);
    {
        //FIX CR: ALPS00275828
/*
#ifndef ANDROID_DEFATULT_CODE
        if (handle==keep->mHandles[0] && keep->mHandles.size() > 1)
        {
            sp<EffectHandle> h_next = keep->mHandles[1].promote();
            MTK_ALOG_D("increase a refcount for mHandles[1],%p",h_next.get());
            sp<ThreadBase> thread = mThread.promote();
            if (thread != 0) {
...
//Fix CR: ALPS00279471
#ifndef ANDROID_DEFATULT_CODE
    sp<EffectModule> kpModule(mEffect);
    if (kpModule != NULL)
    {
        kpModule->disconnect(this, unpinIfLast);

        if (mHasControl && mEnabled) {
            sp<ThreadBase> thread = kpModule->thread().promote();
            if (thread != 0) {
                thread->checkSuspendOnEffectEnabled(kpModule, false, kpModule->sessionId());
            }
        }
    }
    else
...
#ifdef HAVE_MTKLOUDNESS_EFFECT
// auxiliary effects and visualizer , mtkloudness are never suspended on output mix
    if ((mSessionId == AUDIO_SESSION_OUTPUT_MIX) &&
        (((desc.flags & EFFECT_FLAG_TYPE_MASK) == EFFECT_FLAG_TYPE_AUXILIARY) ||
         (memcmp(&desc.type, SL_IID_VISUALIZATION, sizeof(effect_uuid_t)) == 0) ||
         (memcmp(&desc.type, SL_IID_VOLUME, sizeof(effect_uuid_t)) == 0) ||
         (memcmp(&desc.type, SL_IID_MTKLOUDNESS, sizeof(effect_uuid_t)) == 0))) {
        return false;
    }
#else
