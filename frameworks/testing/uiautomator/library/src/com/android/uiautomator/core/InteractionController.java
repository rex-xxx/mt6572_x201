...
// google:            boolean foundEnd = event.getFromIndex() == 0 ||
// google:                    (event.getItemCount() - 1) == event.getToIndex();
// modified by mtk54039:
            boolean foundEnd = false;
            if(downY < upY){
                foundEnd = (event.getFromIndex() == 0);
            }else{
                foundEnd = ((event.getItemCount() - 1) == event.getToIndex());
            }
