...
public class PduComposer {
    ...
    private int appendHeader(int field) {
        switch (field) {
            ...
            case PduHeaders.EXPIRY:
                ...
                if (mForBackup) {
                    Xlog.e(LOG_TAG, "absolute token");
                    append(PduHeaders.VALUE_ABSOLUTE_TOKEN);
                } else {
                    Xlog.e(LOG_TAG, "relative token");
    ...
    private int makeMessageBody(int pduType) {
        ...
        for (int i = 0; i < partNum; i++) {
                ...
                if (null == name || name.length == 0) {
                    ...
                    if (null == name || name.length == 0) {
                         ...
                         if (name != null && name.length != 0){
                             Xlog.d(LOG_TAG, "makeMessageBody name 1= " + name.toString());
                         } else {
            ...
            /// @}
            if (name != null && name.length != 0){
                Xlog.d(LOG_TAG, "makeMessageBody name 2= " + name.toString());
            }
    ...
    protected static int checkAddressType(String address) {
        ...
        if (address.matches(REGEXP_IPV4_ADDRESS_TYPE)) {
            ...
            // Unknown address.
            Xlog.i(LOG_TAG, "checkAddressType PDU_UNKNOWN_ADDRESS_TYPE");
            return PDU_UNKNOWN_ADDRESS_TYPE;
        }
    }
