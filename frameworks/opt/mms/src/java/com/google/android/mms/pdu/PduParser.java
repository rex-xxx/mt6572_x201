...
public class PduParser {
    ...
    /// M:Code analyze 001, modified for useless @{
    ...
    public GenericPdu parse(){
        if (mPduDataStream == null) {
            Xlog.i(LOG_TAG, "Input parse stream is null");
            return null;
        }
        ...
        if (null == mHeaders) {
            ...
            Xlog.i(LOG_TAG, "Parse PduHeader Failed");
            return null;
        }
        ...
        /// M:Code analyze 003,parse mPduDataStream,if the unsigned integer is more than 32
        /// reconstruct retrieveConf @{
        mPduDataStream.mark(1);
        int count = parseUnsignedInt(mPduDataStream); 
        mPduDataStream.reset();
        if (PduHeaders.MESSAGE_TYPE_RETRIEVE_CONF == messageType && count >= TEXT_MIN) {
            byte[] contentType = mHeaders.getTextString(PduHeaders.CONTENT_TYPE);
            if (null == contentType) {
                Xlog.i(LOG_TAG, "Parse MESSAGE_TYPE_RETRIEVE_CONF Failed: content Type is null _0");
                return null;
            }
            ...
            if (!contentTypeStr.equals(ContentType.MULTIPART_MIXED)
                ...
                if (contentTypeStr.equals(ContentType.TEXT_PLAIN)) {
                    Xlog.i(LOG_TAG, "Content Type is text/plain");
                    ...
                    try {
                        ...
                        Xlog.i(LOG_TAG, "new RetrieveConf has exception");
                    }
                    if (retrieveConf == null) {
                        Xlog.i(LOG_TAG, "retrieveConf is null");
                    }
                    return retrieveConf;
                }
        ...
        if ((PduHeaders.MESSAGE_TYPE_SEND_REQ == messageType) ||
            ...
            if (null == mBody) {
                // Parse parts failed.
                Xlog.i(LOG_TAG, "Parse parts Failed");
                return null;
            }
        }
        ...
        switch (messageType) {
            ...
            case PduHeaders.MESSAGE_TYPE_RETRIEVE_CONF:
                ...
                if (null == contentType) {
                    Xlog.i(LOG_TAG, "Parse MESSAGE_TYPE_RETRIEVE_CONF Failed: content Type is null _1");
                    return null;
                }
                ...
                Xlog.i(LOG_TAG, "Parse MESSAGE_TYPE_RETRIEVE_CONF Failed: content Type is null _2");
                return null;
            ...
            switch (headerField) {
                case PduHeaders.MESSAGE_TYPE:
                {
                    ...
                    switch (messageType) {
                        ...
                        case PduHeaders.MESSAGE_TYPE_CANCEL_CONF:
                            Xlog.i(LOG_TAG, "PduParser: parseHeaders: We don't support these kind of messages now.");
                            return null;
                    }
                    try {
                ...
                case PduHeaders.MESSAGE_SIZE:
                {
                    try {
                        ...
                        if (headerField == PduHeaders.DATE) {
                            headers.setLongInteger(value, PduHeaders.DATE_SENT);
                            if (mForRestore == false) {
                                value = System.currentTimeMillis()/1000;//change date to local time
                            }
                        }
                        /// @}
    ...
    protected static PduBody parseParts(ByteArrayInputStream pduDataStream) {
        ...
        for (int i = 0 ; i < count ; i++) {
            ...
            if (startPos <= 0) {
                // Invalid part.
                Xlog.i(LOG_TAG, "PduParser: parseParts: Invalid part.");
                return null;
            }
            ...
            if (partHeaderLen > 0) {
                if (false == parsePartHeaders(pduDataStream, part, partHeaderLen)) {
                    ...
                    Xlog.i(LOG_TAG, "PduParser: parseParts: Parse part header faild.");
                    return null;
                }
            } else if (partHeaderLen < 0) {
                // Invalid length of content-type.
                Xlog.i(LOG_TAG, "PduParser: parseParts: Invalid length of content-type");
                return null;
            }
            ...
            if ((null == part.getContentLocation())
                ...
                Xlog.i(LOG_TAG, "PduParser: parseParts: Hasn't find ContentLocation,so generate one.");
            ...
            if (dataLength > 0) {
                ...
                /// M: Fix ALPS00362106, if server not send all data, treat it fail so we can retry
                ...
                case PduPart.P_CHARSET:
                    ...
                    if (((firstValue > TEXT_MIN) && (firstValue < TEXT_MAX)) ||
                        ...
                        try {
                            ...
                            Xlog.i(LOG_TAG, "Parse CharacterSets: charsetStr");
                        ...
                        if (map != null) {
                            Xlog.i(LOG_TAG, "Parse Well-known-charset: charset");
            ...
            if (header > TEXT_MAX) {
                ...
                switch (header) {
                    ...
                    case PduPart.P_CONTENT_ID:
                        ...
                        lastLen = length - (startPos - tempPos);
                        break;
                    /// M:Code analyze 007,add two new header fields,they are not support for google default @{
                    case PduPart.P_X_WAP_CONTENT_URI:
                        byte[] xWapContentUri = parseWapString(pduDataStream, TYPE_TEXT_STRING);
                        if (null != xWapContentUri) {
                        ...
                        break;
                    case PduPart.P_DATE:
                        long date = parseLongInteger(pduDataStream);
                        date = System.currentTimeMillis()/1000;//change date to local time
                        part.setDate(date);
                ...
                // Not assigned header.
                /// M:Code analyze 008,reset the location back to the marked location @{
                pduDataStream.reset();
                /// @}
                ...
                Xlog.i(LOG_TAG, "Got unknown header field: " + unAssignedHeader);
                Xlog.i(LOG_TAG, "Got unknown header tempValue: " + new String(tempValue));
                // Check the header whether it is "Content-Transfer-Encoding".
                if (true ==
                    PduPart.CONTENT_TRANSFER_ENCODING.equalsIgnoreCase(unAssignedHeader)) {
                    part.setContentTransferEncoding(tempValue);
                }

                /// M:Code analyze 009, Check the header whether it is "Content-Id",if yes,
                /// set it into map @{
                if (true ==
                    PduPart.CONTENT_ID.equalsIgnoreCase(unAssignedHeader)) {
                    if (null != tempValue) {
                ...
                /// M:Code analyze 010, Check the header whether it is "Content-Location",
                /// if yes,set it into map @{
                if (true ==
                    PduPart.CONTENT_LOCATION.equalsIgnoreCase(unAssignedHeader)) {
                    if (null != tempValue) {
                ...
                /// M:Code analyze 011,Check the header whether it is "Content-Disposition"
                /// if yes,set it into map @{
                if (true ==
                    PduPart.CONTENT_DISPOSITION.equalsIgnoreCase(unAssignedHeader)) {
                    if (null != tempValue) {
                ...
                /// M:Code analyze 012, Check the header whether it is "Name",if yes,
                /// set it into map @{
                if (true ==
                    PduPart.PARA_NAME.equalsIgnoreCase(unAssignedHeader)) {
                    if (null != tempValue) {
    ...
    protected static boolean checkMandatoryHeader(PduHeaders headers) {
        ...
        if (0 == mmsVersion) {
            ...
            Xlog.i(LOG_TAG, "Parse MandatoryHeader Failed: PDU hasn't mmsVersion");
            return false;
        }
        ...
        switch (messageType) {
            case PduHeaders.MESSAGE_TYPE_SEND_REQ:
                ...
                if (null == srContentType) {
                    Xlog.i(LOG_TAG, "Parse MandatoryHeader Failed: MESSAGE_TYPE_SEND_REQ PDU hasn't content type field");
                    return false;
                }
                ...
                if (null == srFrom) {
                    Xlog.i(LOG_TAG, "Parse MandatoryHeader Failed: MESSAGE_TYPE_SEND_REQ PDU hasn't From field");
                    return false;
                }
                ...
                if (null == srTransactionId) {
                    Xlog.i(LOG_TAG, "Parse MandatoryHeader Failed: MESSAGE_TYPE_SEND_REQ PDU hasn't Transaction-Id field");
                    return false;
                }
*/
                break;
            case PduHeaders.MESSAGE_TYPE_SEND_CONF:
                ...
                if (0 == scResponseStatus) {
                    Xlog.i(LOG_TAG, "Parse MandatoryHeader Failed: MESSAGE_TYPE_SEND_CONF PDU hasn't Response-Status field");
                    return false;
                }
                ...
                if (null == scTransactionId) {
                    Xlog.i(LOG_TAG, "Parse MandatoryHeader Failed: MESSAGE_TYPE_SEND_CONF PDU hasn't Transaction-Id field");
                    return false;
                }
            ...
            case PduHeaders.MESSAGE_TYPE_NOTIFICATION_IND:
                ...
                if (null == niContentLocation) {
                    Xlog.i(LOG_TAG, "Parse MandatoryHeader Failed: MESSAGE_TYPE_NOTIFICATION_IND PDU hasn't Content-Location field");
                    return false;
                }
                ...
                if (-1 == niExpiry) {
                    Xlog.i(LOG_TAG, "Parse MandatoryHeader Failed: MESSAGE_TYPE_NOTIFICATION_IND PDU hasn't Expiry field");
                    return false;
                }
                ...
                if (null == niMessageClass) {
                    Xlog.i(LOG_TAG, "Parse MandatoryHeader Failed: MESSAGE_TYPE_NOTIFICATION_IND PDU hasn't Message-Class field");
                    return false;
                }
                ...
                if (-1 == niMessageSize) {
                    Xlog.i(LOG_TAG, "Parse MandatoryHeader Failed: MESSAGE_TYPE_NOTIFICATION_IND PDU hasn't Message-Size field");
                    return false;
                }
                ...
                if (null == niTransactionId) {
                    Xlog.i(LOG_TAG, "Parse MandatoryHeader Failed: MESSAGE_TYPE_NOTIFICATION_IND PDU hasn't Transaction-Id field");
                    return false;
                }
            ...
            case PduHeaders.MESSAGE_TYPE_NOTIFYRESP_IND:
                ...
                if (0 == nriStatus) {
                    Xlog.i(LOG_TAG, "Parse MandatoryHeader Failed: MESSAGE_TYPE_NOTIFYRESP_IND PDU hasn't Status field");
                    return false;
                }
                ...
                if (null == nriTransactionId) {
                    Xlog.i(LOG_TAG, "Parse MandatoryHeader Failed: MESSAGE_TYPE_NOTIFYRESP_IND PDU hasn't Transaction-Id field");
                    return false;
                }
            ...
            case PduHeaders.MESSAGE_TYPE_RETRIEVE_CONF:
                ...
                if (null == rcContentType) {
                    Xlog.i(LOG_TAG, "Parse MandatoryHeader Failed: MESSAGE_TYPE_RETRIEVE_CONF PDU hasn't Content-Type field");
                    return false;
                }
                ...
                if (-1 == rcDate) {
                    Xlog.i(LOG_TAG, "Parse MandatoryHeader Failed: MESSAGE_TYPE_RETRIEVE_CONF PDU hasn't Date field");
                    return false;
                }
            ...
            case PduHeaders.MESSAGE_TYPE_DELIVERY_IND:
                ...
                if (-1 == diDate) {
                    Xlog.i(LOG_TAG, "Parse MandatoryHeader Failed: MESSAGE_TYPE_DELIVERY_IND PDU hasn't Date field");
                    return false;
                }
                ...
                if (null == diMessageId) {
                    Xlog.i(LOG_TAG, "Parse MandatoryHeader Failed: MESSAGE_TYPE_DELIVERY_IND PDU hasn't Message-Id field");
                    return false;
                }
                ...
                if (0 == diStatus) {
                    Xlog.i(LOG_TAG, "Parse MandatoryHeader Failed: MESSAGE_TYPE_DELIVERY_IND PDU hasn't Status field");
                    return false;
                }
                ...
                if (null == diTo) {
                    Xlog.i(LOG_TAG, "Parse MandatoryHeader Failed: MESSAGE_TYPE_DELIVERY_IND PDU hasn't To field");
                    return false;
                }
            ...
            case PduHeaders.MESSAGE_TYPE_ACKNOWLEDGE_IND:
                ...
                if (null == aiTransactionId) {
                    Xlog.i(LOG_TAG, "Parse MandatoryHeader Failed: MESSAGE_TYPE_ACKNOWLEDGE_IND PDU hasn't Transaction-Id field");
                    return false;
                }
            ...
            case PduHeaders.MESSAGE_TYPE_READ_ORIG_IND:
                ...
                if (-1 == roDate) {
                    Xlog.i(LOG_TAG, "Parse MandatoryHeader Failed: MESSAGE_TYPE_READ_ORIG_IND PDU hasn't Date field");
                    return false;
                }
                ...
                if (null == roFrom) {
                    Xlog.i(LOG_TAG, "Parse MandatoryHeader Failed: MESSAGE_TYPE_READ_ORIG_IND PDU hasn't From field");
                    return false;
                }
                ...
                if (null == roMessageId) {
                    Xlog.i(LOG_TAG, "Parse MandatoryHeader Failed: MESSAGE_TYPE_READ_ORIG_IND PDU hasn't Message-Id field");
                    return false;
                }
                ...
                if (0 == roReadStatus) {
                    Xlog.i(LOG_TAG, "Parse MandatoryHeader Failed: MESSAGE_TYPE_READ_ORIG_IND PDU hasn't Read-Status field");
                    return false;
                }
                ...
                if (null == roTo) {
                    Xlog.i(LOG_TAG, "Parse MandatoryHeader Failed: MESSAGE_TYPE_READ_ORIG_IND PDU hasn't To field");
                    return false;
                }
            ...
            case PduHeaders.MESSAGE_TYPE_READ_REC_IND:
                ...
                if (null == rrFrom) {
                    Xlog.i(LOG_TAG, "Parse MandatoryHeader Failed: MESSAGE_TYPE_READ_REC_IND PDU hasn't From field");
                    return false;
                }
                ...
                if (null == rrMessageId) {
                    Xlog.i(LOG_TAG, "Parse MandatoryHeader Failed: MESSAGE_TYPE_READ_REC_IND PDU hasn't Message-Id field");
                    return false;
                }
                ...
                if (0 == rrReadStatus) {
                    Xlog.i(LOG_TAG, "Parse MandatoryHeader Failed: MESSAGE_TYPE_READ_REC_IND PDU hasn't Read-Status field");
                    return false;
                }
                ...
                if (null == rrTo) {
                    Xlog.i(LOG_TAG, "Parse MandatoryHeader Failed: MESSAGE_TYPE_READ_REC_IND PDU hasn't To field");
                    return false;
                }
            ...
            default:
                ...
                Xlog.i(LOG_TAG, "Parse MandatoryHeader Failed: Parser doesn't support this message type in this version");
                return false;
        }
