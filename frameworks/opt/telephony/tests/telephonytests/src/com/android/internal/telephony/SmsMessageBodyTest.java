...
public class SmsMessageBodyTest extends AndroidTestCase {
    ...
    private static final byte[] sTestBytes = {0x01,0x02,0x03,0x04};
    ...
    @SmallTest
    public void testSubmitPdu() throws Exception {
        callSubmitPduMethods();
    }

    @SmallTest
    public void testNewFromFunctions() throws Exception {
        callNewFromMethods();
    }

    @SmallTest
    public void testDeliverPduWithLang() throws Exception {
        String scAddress="0912345678";
        String destinationAddress="0987654321";
        
        java.util.Date today = new java.util.Date();
        java.sql.Timestamp ts = new java.sql.Timestamp(today.getTime());
        long time=ts.getTime();
        
        com.android.internal.telephony.gsm.SmsMessage.DeliverPdu pduData = 
            com.android.internal.telephony.gsm.SmsMessage.getDeliverPduWithLang(scAddress, destinationAddress, 
                scAddress, sTestBytes, time, 1, 1);

        Log.d(TAG, "testDeliverPduWithLang, pduData="+pduData.toString());
        assertTrue(true);
    }
    
    private void callSubmitPduMethods () throws Exception
    {
        // deprecated GSM-specific method
        String scAddress="0912345678";
        String destinationAddress="0987654321";
        short destinationPort=9001;
        short originalPort=9002;

        com.android.internal.telephony.gsm.SmsMessage.SubmitPdu gsmPduData = com.android.internal.telephony.gsm.SmsMessage.getSubmitPdu(scAddress, destinationAddress, 
            destinationPort, originalPort, sTestBytes, false);

        Log.d(TAG, "callSubmitPduMethods, gsmPduData="+gsmPduData.toString());
        assertTrue(true);
        
        android.telephony.SmsMessage.SubmitPdu pduData = SmsMessage.getSubmitPdu(scAddress, destinationAddress, destinationPort, originalPort, sTestBytes, false);
        Log.d(TAG, "callSubmitPduMethods, pduData="+pduData.toString());
        assertTrue(true);

        gsmPduData = com.android.internal.telephony.gsm.SmsMessage.getSubmitPdu(scAddress, destinationAddress, 
                            scAddress, false, sTestBytes);
        Log.d(TAG, "callSubmitPduMethods, gsmPduData="+gsmPduData.toString());
        assertTrue(true);
        
        pduData = android.telephony.SmsMessage.getSubmitPdu(scAddress, destinationAddress, scAddress, false, sTestBytes);
        Log.d(TAG, "callSubmitPduMethods, pduData="+pduData.toString());
        assertTrue(true);
        
        pduData = android.telephony.SmsMessage.getSubmitPdu(scAddress, destinationAddress, scAddress, false);
        Log.d(TAG, "callSubmitPduMethods, pduData="+pduData.toString());
        assertTrue(true);

        gsmPduData = com.android.internal.telephony.gsm.SmsMessage.getSubmitPdu(scAddress, destinationAddress, 
                            scAddress, false, sTestBytes, SmsMessage.ENCODING_UNKNOWN, 1, 1);

        Log.d(TAG, "callSubmitPduMethods, gsmPduData="+gsmPduData.toString());
        assertTrue(true);

        gsmPduData = com.android.internal.telephony.gsm.SmsMessage.getSubmitPdu(scAddress, destinationAddress, 
                            scAddress, false, sTestBytes, SmsMessage.ENCODING_7BIT, 1, 1);

        Log.d(TAG, "callSubmitPduMethods, gsmPduData="+gsmPduData.toString());
        assertTrue(true);

        gsmPduData = com.android.internal.telephony.gsm.SmsMessage.getSubmitPdu(scAddress, destinationAddress, 
                            scAddress, false, sTestBytes, SmsMessage.ENCODING_16BIT, 1, 1);

        Log.d(TAG, "callSubmitPduMethods, gsmPduData="+gsmPduData.toString());
        assertTrue(true);       

        pduData = android.telephony.SmsMessage.getSubmitPdu(scAddress, destinationAddress, destinationPort, sTestBytes, false);

        Log.d(TAG, "callSubmitPduMethods, pduData="+pduData.toString());
        assertTrue(true);
        
        gsmPduData = com.android.internal.telephony.gsm.SmsMessage.getSubmitPdu(scAddress, destinationAddress, 
                            scAddress, destinationPort, true);

        Log.d(TAG, "callSubmitPduMethods, gsmPduData="+gsmPduData.toString());
        assertTrue(true);

        com.android.internal.telephony.gsm.SmsMessage msg = com.android.internal.telephony.gsm.SmsMessage.createFromPdu(sTestBytes);
        assertTrue(true);
        
        gsmPduData = com.android.internal.telephony.gsm.SmsMessage.getSubmitPdu(scAddress, destinationAddress, 
                            sTestBytes, sTestBytes, true);

        Log.d(TAG, "callSubmitPduMethods, gsmPduData="+gsmPduData.toString());
        assertTrue(true);

        gsmPduData = com.android.internal.telephony.gsm.SmsMessage.getSubmitPduWithLang(scAddress, destinationAddress, 
                            scAddress, true, sTestBytes, 1, 1);

        Log.d(TAG, "callSubmitPduMethods, gsmPduData="+gsmPduData.toString());
        assertTrue(true);

        gsmPduData = com.android.internal.telephony.gsm.SmsMessage.getSubmitPdu(scAddress, destinationAddress, 
                            scAddress, false, sTestBytes, SmsMessage.ENCODING_UNKNOWN, 1, 1, 0);

        Log.d(TAG, "callSubmitPduMethods, gsmPduData="+gsmPduData.toString());
        assertTrue(true);

        Log.d(TAG,"SimSmsTest starts");
        ISms sms = ISms.Stub.asInterface(ServiceManager.getService("isms"));
        assertNotNull(sms);

        SmsParameters meters = sms.getSmsParameters();
        if (meters!=null)
            {
                Log.d(TAG,"meters not null");
            }
        else
            {
                Log.d(TAG,"meters null");
            }
        assertTrue(true);
    }

    private void callNewFromMethods() throws Exception
    {
        String scAddress="0912345678";
            
        android.telephony.SmsMessage msg = android.telephony.SmsMessage.newFromCDS(scAddress);
        assertTrue(true);
            
        msg = android.telephony.SmsMessage.newFromCMTI(scAddress);
        /*
        String resultStr=msg.getServiceCenterAddress();
        resultStr=msg.getOriginatingAddress();
        resultStr=msg.getDisplayOriginatingAddress();
        resultStr=msg.getMessageBody();
        resultStr=msg.getDisplayMessageBody();
        resultStr=msg.getPseudoSubject();
        long resultLong=msg.getTimestampMillis();
        boolean resultBool=msg.isEmail();
        resultStr=msg.getEmailBody();
        resultStr=msg.getEmailFrom();
        int resultInt=msg.getProtocolIdentifier();
        resultBool=msg.isReplace();
        resultBool=msg.isMWIClearMessage();
        resultBool=msg.isMWISetMessage();
        resultBool=msg.isMwiDontStore();
        byte[] resultByte=msg.getPdu();
        resultInt=msg.getStatusOnIcc();
        resultInt=msg.getIndexOnSim();
        resultInt=msg.getIndexOnIcc();
        resultInt=msg.getStatus();
        resultBool=msg.isStatusReportMessage();
        resultBool=msg.isReplyPathPresent();
        resultStr=msg.getDestinationAddress();
        com.android.internal.telephony.SmsHeader resultHeader=msg.getUserDataHeader();
        resultByte=msg.getSmsc();
        resultByte=msg.getTpdu();
        */
        assertTrue(true);
        
        msg = android.telephony.SmsMessage.newFromCMT(sCharacterClasses);
        assertTrue(true);
        
        msg = android.telephony.SmsMessage.createFromEfRecord(1,sTestBytes, com.android.internal.telephony.SmsConstants.FORMAT_3GPP);
        assertTrue(true);
        
        ArrayList<String> resultArray = android.telephony.SmsMessage.fragmentText(scAddress, com.android.internal.telephony.SmsConstants.ENCODING_7BIT);
        resultArray = android.telephony.SmsMessage.fragmentText(scAddress, com.android.internal.telephony.SmsConstants.ENCODING_16BIT);
        resultArray = android.telephony.SmsMessage.fragmentText(scAddress);
        assertTrue(true);
    }
...
//                    Log.d(TAG, "Unicode only: skipping to next test string");
            ...
            int[] expectedValues)
    {
        ...
        ted = com.android.internal.telephony.gsm.SmsMessage.calculateLength(msgBody, use7bitOnly, SmsMessage.ENCODING_7BIT);
        ted = com.android.internal.telephony.gsm.SmsMessage.calculateLength(msgBody, use7bitOnly, SmsMessage.ENCODING_16BIT);
        assertTrue(true);
    }
