...
public class SmsMessage {
    ...
    */
    ...
    public static SubmitPdu getSubmitPdu(String scAddress, String destinationAddress,
        ...
        Log.d(LOG_TAG, "[xj android.telephony.SmsMessage getSubmitPdu");
        SubmitPduBase spb;
        int activePhone = TelephonyManager.getDefault().getCurrentPhoneType();
    ...
    public byte[] getSmsc() {
        if(FeatureOption.EVDO_DT_VIA_SUPPORT == true) {
            Log.d(LOG_TAG, "should call getSmsc sub");
            return null;
        } else {
            Log.d(LOG_TAG, "call getSmsc");
    ...
    public byte[] getTpdu() {
        if(FeatureOption.EVDO_DT_VIA_SUPPORT == true) {
            Log.d(LOG_TAG, "should call getTpdu sub");
            return null;
        } else {
            Log.d(LOG_TAG, "call getTpdu");
            byte[] pdu = this.getPdu();
            if(pdu == null) {
                Log.d(LOG_TAG, "pdu is null");
                return null;
    ...
    public static SmsMessage createFromEfRecord(int index, byte[] data, String format) {
        ...
        Log.d(LOG_TAG, "createFromEfRecord(): format " + format);
        if (FORMAT_3GPP2.equals(format)) {
            ...
            Log.e(LOG_TAG, "createFromEfRecord(): unsupported message format " + format);
            return null;
        }
