...
/// M: [mtk04070][111117][ALPS00093395]MTK added for printing log.
...
public class CdmaDataConnection extends DataConnection {
    ...
    protected void logd(String s) {
        if (FeatureOption.MTK_GEMINI_SUPPORT) {
            Log.d(LOG_TAG, "[CDC][simId" + phone.getMySimId()+ "]"+ s);
        } else {
            Log.d(LOG_TAG, "[CDC] " + s);
        }
    }
    ...
    protected void logi(String s) {
        if (FeatureOption.MTK_GEMINI_SUPPORT) {
            Log.i(LOG_TAG, "[CDC][simId" + phone.getMySimId()+ "]"+ s);
        } else {
            Log.i(LOG_TAG, "[CDC] " + s);
        }
    }
    ...
    protected void logw(String s) {
        if (FeatureOption.MTK_GEMINI_SUPPORT) {
            Log.w(LOG_TAG, "[CDC][simId" + phone.getMySimId()+ "]"+ s);
        } else {
            Log.w(LOG_TAG, "[CDC] " + s);
        }
    }
    ...
    protected void loge(String s) {
        if (FeatureOption.MTK_GEMINI_SUPPORT) {
            Log.e(LOG_TAG, "[CDC][simId" + phone.getMySimId()+ "]"+ s);
        } else {
            Log.e(LOG_TAG, "[CDC] " + s);
        }
    }
