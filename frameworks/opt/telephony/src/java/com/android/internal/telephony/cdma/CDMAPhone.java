...
public class CDMAPhone extends PhoneBase {
    ...
    /// M: Add for Gemini CDMAPhone initialization. @{
    private int mySimId;
    ...
    public CDMAPhone(Context context, CommandsInterface ci, PhoneNotifier notifier) {
		...
		this(context, ci, notifier, false, PhoneConstants.GEMINI_SIM_2);
		/*Set GSM as Phone1 and CDMA as Phone2 */
    }

    public CDMAPhone(Context context, CommandsInterface ci, PhoneNotifier notifier,
            boolean unitTestMode) {
			this(context, ci, notifier, unitTestMode, PhoneConstants.GEMINI_SIM_2);
	}
    ...
    protected void init(Context context, PhoneNotifier notifier) {
        ...
        /// M: Create VIA CdmaDataConnectionTracker for CDMA. @{
        if (FeatureOption.EVDO_DT_SUPPORT) {
           mDataConnectionTracker = new ViaCdmaDataConnectionTracker (this);
        }   
        /// @}
    ...
    public void handleMessage(Message msg) {
        ...
        switch(msg.what) {
            case EVENT_RADIO_AVAILABLE: {
                ...
                Log.d(LOG_TAG, "Event QUERY UIM INSERTED STATUS");
                mCM.queryUimInsertedStatus(obtainMessage(EVENT_QUERY_UIM_INSERTED_STATUS_DONE));
                /// @}
            }
            break;
		...
		//mCM.queryUimInsertedStatus(obtainMessage(EVENT_QUERY_UIM_INSERTED_STATUS_DONE));
    	    ...
    	    case EVENT_QUERY_UIM_INSERTED_STATUS_DONE:{
                ...
                if (ar.exception == null) {
                    ...
                    if (uimInsertedStatus == UIM_STATUS_NO_CARD_INSERTED) {
                        if(DBG)Log.d(LOG_TAG, "[CDMAPhone, EVENT_QUERY_UIM_INSERTED_STATUS_DONE, execute notifySimMissing]");
                        mCM.notifyUimInsertedStatus(uimInsertedStatus);
                        }
                    }
                    else {
                        if(DBG)Log.w(LOG_TAG, "[CDMAPhone, EVENT_QUERY_UIM_INSERTED_STATUS_DONE, exception]");
            ...
            case EVENT_GET_UIM_INSERT_STATUS_RETRY: {
                if(DBG)Log.d(LOG_TAG, "Event QUERY UIM INSERTED STATUS, retry....");
                mCM.queryUimInsertedStatus(obtainMessage(EVENT_QUERY_UIM_INSERTED_STATUS_DONE));
            }  
            break;
            //via support end
    ...
    boolean updateCurrentCarrierInProvider() {
        ...
        if (mIccRecords.get() != null) {
            try {
                ...
                ContentValues map = new ContentValues();
                map.put(Telephony.Carriers.NUMERIC, mIccRecords.get().getOperatorNumeric());
    ...
    public void registerForCrssSuppServiceNotification(Handler h, int what, Object obj) {
        Log.e(LOG_TAG, "method registerForCrssSuppServiceNotification is NOT supported in CDMA!");
    }

    public void unregisterForCrssSuppServiceNotification(Handler h) {
        Log.e(LOG_TAG, "method unregisterForCrssSuppServiceNotification is NOT supported in CDMA!");
    }
    ...
    public void getFacilityLock(String facility, String password, Message onComplete) {
        Log.e(LOG_TAG, "getFacilityLock: not possible in CDMA");
    }
    ...
    public void getVtCallForwardingOption(int commandInterfaceCFReason, Message onComplete) {
        Log.e(LOG_TAG, "getVtCallForwardingOption: not possible in CDMA");
    }
