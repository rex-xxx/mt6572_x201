...
public class SipPhone extends SipPhoneBase {
    ...
    public void switchHoldingAndActive() throws CallStateException {
        ...
        if ((foregroundCall.getState() == SipCall.State.DIALING)) {
        ...
        protected void setState(State newState) {
            if (state != newState) {
                ...
                if(newState == Call.State.INCOMING) {
                    Log.d(LOG_TAG, "Start the SIP phone ring");
                    notifySipCallRing();                   
                }else if(newState == Call.State.DISCONNECTED && connections.size() == 1) {
                   if(SipPhone.this.getRingingCall().isRinging()){
                      Log.d(LOG_TAG, "Start the SIP phone ring after the call is ended");
                      /// M: For solving ALPS00542520, @{
                      /* Change the state of ringing call from WAITING to INCOMING 
                         when there is no more foreground and background call */
                      if (ringingCall.getState() == Call.State.WAITING) {
                          ringingCall.state = Call.State.INCOMING;
                          Log.d(LOG_TAG, "[setState]Change WAITING to INCOMING.  New state = " + ringingCall.state);
                      }
                      /// @}
                      notifySipCallRing();                   
                    }
