...
public class VCardEntry {
    ...
    static {
        ...
        sImMap.put(VCardConstants.PROPERTY_X_CUSTOM_IM, Im.PROTOCOL_CUSTOM);
    }
...
//            }
            ...
            builder.withValue(StructuredPostal.NEIGHBORHOOD, mExtendedAddress);
...
//                		builder.append(PhoneNumberUtils.WAIT);
                    hasPauseOrWait = true;
                } else if (('0' <= ch && ch <= '9') || (i == 0 && ch == '+') || ch == ' ' || ch == PhoneNumberUtils.WAIT || ch == PhoneNumberUtils.PAUSE) {
                    builder.append(ch);
                }
            }
            if (!hasPauseOrWait) {
        ...
        if (propertyName.equals(VCardConstants.PROPERTY_VERSION)) {
            ...
            if (typeCollection != null) {
                for (final String typeStringOrg : typeCollection) {
                    ...
                    } else if (typeStringUpperCase.equals(VCardConstants.PARAM_ADR_EXTRA_TYPE_OTHER)) {
            ...
            if (typeCollection != null) {
                if (protocol == Im.PROTOCOL_CUSTOM && typeCollection.size() > 0) {
                	StringBuilder builder = new StringBuilder();
    				for (String typeString : typeCollection) {
    					builder.append(typeString);
    				}   
    				customProtocol = builder.toString();
                }
			}

            addIm(protocol, customProtocol, propValue, type, isPrimary);
