...
public class VCardComposer {
    ...
    private String createOneEntryInternal(final String contactId,
        ...
        try {
             ...
             *   CR ID: ALPS00283301
             *   Descriptions: The joined contacts' name would be changed 
             *                 after exporting to vCard file.
             */
            long nameRawContactId = -1;
            nameRawContactId = getNameRawContactId(mContentResolver, Contacts._ID + "=?", selectionArgs);
            /*
            ...
            while (entityIterator.hasNext()) {
                ...
                for (NamedContentValues namedContentValues : entity.getSubValues()) {
                    ...
                    if (key != null) {
                         ...
                         *                 after exporting to vCard file.
                         */
                        if (!key.equals(StructuredName.CONTENT_ITEM_TYPE)
                                || (key.equals(StructuredName.CONTENT_ITEM_TYPE) && (nameRawContactId == -1 || nameRawContactId == entity
                                        .getEntityValues().getAsLong(RawContacts._ID)))) {
                            /*
                             * New Feature by Mediatek Begin.
                             * M:AAS, set phone type to Mobile. TODO Easy Porting
                             */
                            String accountType = entity.getEntityValues().getAsString(RawContacts.ACCOUNT_TYPE);
                            if(isAasEnabled(accountType) && Phone.CONTENT_ITEM_TYPE.equals(key)){
                                contentValues.put(Phone.TYPE, Phone.TYPE_MOBILE);
                                Log.i(LOG_TAG, "AAS: set phone type to be Mobile.");
                            }
                            /*
    ...
    /** M:AAS */
    private boolean isAasEnabled(String accountType) {
        if("OP03".equals(SystemProperties.get("ro.operator.optr"))
                && "USIM Account".equals(accountType)) {
            return true;
        }
        return false;
    }
    ...
    private long getNameRawContactId(final ContentResolver resolver, final String selection,
            final String[] selectionArgs) {
        long nameRawContactId = -1;
        Cursor cursor = null;
        try {
            cursor = mContentResolver.query(Contacts.CONTENT_URI, new String[] {
                "name_raw_contact_id",
            }, selection, selectionArgs, null);
            if (cursor != null) {
                if (cursor.moveToNext()) {
                    nameRawContactId = cursor.getLong(0);
                }
            }
        } finally {
            if (cursor != null) {
