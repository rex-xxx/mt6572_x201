...
//                                    .replace(PhoneNumberUtils.WAIT, 'w');
                            ...
                            if (TextUtils.equals(numberWithControlSequence, actualPhoneNumber)) {
                                ...
                                for (int i = 0; i < length; i++) {
                                    ...
                                    if (Character.isDigit(ch) || ch == '+' || ch == 'p' 
                                    	|| ch == 'w' || ch == 'P' || ch == 'W' || ch == ' ' || ch == ',' || ch == ';') {
...
//                                        digitsOnlyBuilder.toString(), phoneFormat);
    ...
    private PostalStruct tryConstructPostalStruct(ContentValues contentValues) {
        ...
        if (!VCardUtils.areAllEmpty(rawAddressArray)) {
            ...
            String encodedNeighborhood = null;
...
//            }
            if (reallyUseQuotedPrintable) {
                ...
                /* Bug Fix by Mediatek Begin. ALPS00473356 */
            ...
            if (!TextUtils.isEmpty(encodedNeighborhood)) {
    ...
    public VCardBuilder appendIms(final List<ContentValues> contentValuesList) {
        if (contentValuesList != null) {
            for (ContentValues contentValues : contentValuesList) {
                ...
                if (Im.PROTOCOL_CUSTOM == protocolAsObject) {   
                	String tmep = VCardConstants.PROPERTY_X_CUSTOM_IM;
                	String customTypeName = contentValues.getAsString(Im.DATA6);                	
                	final List<String> parameterList = new ArrayList<String>();
                	if (customTypeName != null && !customTypeName.isEmpty()) {
                		parameterList.add(customTypeName);
                	}
                    appendLineWithCharsetAndQPDetection(tmep, parameterList, contentValues.getAsString(Im.DATA));
                	continue;
                }
    ...
    public void appendPostalLine(final int type, final String label,
        ...
        switch (type) {
            ...
            case StructuredPostal.TYPE_OTHER: {
            	parameterList.add(VCardConstants.PARAM_ADR_EXTRA_TYPE_OTHER);
                break;
            }
            default: {
    ...
    public void appendTelLine(final Integer typeAsInteger, final String label,
        ...
        switch (type) {
            ...
            case Phone.TYPE_CALLBACK: {
                parameterList.add(VCardConstants.PARAM_PHONE_EXTRA_TYPE_CALLBACK);
                break;
            }                        
            case Phone.TYPE_CAR: {
            ...
            case Phone.TYPE_COMPANY_MAIN: {
                ...
                parameterList.add(VCardConstants.PARAM_PHONE_EXTRA_TYPE_COMPANY_MAIN);
                isPrimary = true;
                break;
            }
            ...
            case Phone.TYPE_OTHER_FAX: {
                parameterList.add(VCardConstants.PARAM_PHONE_EXTRA_OTHER_FAX);
                break;
            }
            ...
            case Phone.TYPE_RADIO: {
            	parameterList.add(VCardConstants.PARAM_PHONE_EXTRA_TYPE_RADIO);
            	break;
            }
            case Phone.TYPE_TTY_TDD: {
            	parameterList.add(VCardConstants.PARAM_PHONE_EXTRA_TYPE_TTY_TDD);
            	break;
            }
            case Phone.TYPE_ASSISTANT: {
            	parameterList.add(VCardConstants.PARAM_PHONE_EXTRA_TYPE_ASSISTANT);
            	break;
            }
