...
public class MediaPlayer
{
    ...
    public void setDataSource(Context context, Uri uri, Map<String, String> headers)
        ...
        try {
            ...
            if (fd == null) {
                Xlog.e(TAG, "setDataSource: Null fd! uri=" + uri);
                return;
            }
            ...
            Xlog.e(TAG, "setDataSource: SecurityException! uri=" + uri, ex);
        } catch (IOException ex) {
            Xlog.e(TAG, "setDataSource: IOException! uri=" + uri);
        } finally {
            if (fd != null) {
                fd.close();
            }
        ...
        IOmaSettingHelper helper = MediatekClassFactory.createInstance(IOmaSettingHelper.class);
    ...
    public void addTimedTextSource(Context context, Uri uri, String mimeType)
        ...
        try {
            ...
            fd = resolver.openAssetFileDescriptor(uri, "r");
            if (fd == null) {
                Xlog.e(TAG, "addTimedTextSource: Null fd! uri=" + uri);
                return;
            }
            addTimedTextSource(fd.getFileDescriptor(), mimeType);
            return;
        } catch (SecurityException ex) {
            Xlog.e(TAG, "addTimedTextSource: SecurityException! uri=" + uri, ex);
        } catch (IOException ex) {
            Xlog.e(TAG, "addTimedTextSource: IOException! uri=" + uri);
        } finally {
            if (fd != null) {
                fd.close();
            }
    ...
    private class EventHandler extends Handler
    {
        ...
        public void handleMessage(Message msg) {
            ...
            case MEDIA_DURATION_UPDATE:
                Xlog.v(TAG, "Duration update (duration=" + msg.arg1 + ")");
            ...
            case MEDIA_PAUSE_COMPLETE:
                if (mOnInfoListener != null) {
                    if (msg.arg1 != 0) {
                        Xlog.e(TAG, "MEDIA_PAUSE_COMPLETE failed " + msg.arg1);
                    }
                    /// M: msg.arg1 == 0(OK of MediaPlayer.cpp)
                    mOnInfoListener.onInfo(mMediaPlayer, MEDIA_INFO_PAUSE_COMPLETED,
                            (msg.arg1 == 0 ? PAUSE_PLAY_SUCCEED : PAUSE_PLAY_FAILED));
                }
                break;
            case MEDIA_PLAY_COMPLETE:
                if (mOnInfoListener != null) {
                    if (msg.arg1 != 0) {
                        Xlog.e(TAG, "MEDIA_PLAY_COMPLETE failed " + msg.arg1);
                    }
                    mOnInfoListener.onInfo(mMediaPlayer, MEDIA_INFO_PLAY_COMPLETED,
                            (msg.arg1 == 0 ? PAUSE_PLAY_SUCCEED : PAUSE_PLAY_FAILED));
                }
                break;
            default:
                                            ...
                                            int what, int arg1, int arg2, Object obj)
    {
        ...
        if (mp == null) {
            Xlog.e(TAG, "postEventFromNative: Null mp! what=" + what + ", arg1=" + arg1 + ", arg2=" + arg2);
            return;
        }
    ...
    /**
     * M: Bad file, Decoder cannot decode this file.
     * 
     * @hide
     */
    ...
    /**
     * M: Not support current media type.
     * 
     * @hide
     */
    ...
    /**
     * M: Invalid link for phone to connect. (403 Forbidden)
     * 
     * @hide
     */
    ...
    /**
     * M: When the source is RTSP streaming file, 
     * metadata may be not right when OnPreparedListener is called.
     * In this case, media server will send this info to notify app that 
     * the metadata is filled rightly.
     * 
     * {@hide}
     */
    ...
    /** 
     * M: The media CAN be seeked now(seek table has been setup)
     * @see android.media.MediaPlayer.OnInfoListener
     * 
     * {@hide}
     */
    ...
    /**
     * M: This will be sent after connecting to the server,
     * means streaming player has got display data from server.
     * @see android.media.MediaPlayer.OnInfoListener
     * 
     * {@hide}
     */
    ...
    /**
     * M: Interface definition to notify application when a new(accurate) duration
     * is available from lower layers
     * 
     * @hide
     */
    ...
    /**
     * M: Register a callback to be invoked when a new duration is available from lower layers
    ...
    /**
     * M: Capture a CMMB picture.
     * 
     * @hide
     */
    public native void native_capture(String path);

    /**
     * M: Status constant indicating asyn pause or play succeed.
     * 
     * {@hide}
     */
    ...
    /**
     * M: Status constant indicating asyn pause or play failed.
     * 
     * {@hide}
     */
