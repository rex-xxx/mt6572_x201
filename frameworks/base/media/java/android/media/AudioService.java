...
import com.mediatek.common.audioprofile.*;
...
public class AudioService extends IAudioService.Stub implements OnFinished {
   ...
   /// M: Modify the max stream volume
    ...
    private final AudioSystem.ErrorCallback mAudioSystemCallback = new AudioSystem.ErrorCallback() {
        public void onError(int error) {
            ...
            case AudioSystem.AUDIO_STATUS_SERVER_DIED:
                Log.d(TAG," Enter AudioSystem.AUDIO_STATUS_SERVER_DIED callback:" + mMediaServerOk);
                if (mMediaServerOk) {
            ...
            case AudioSystem.AUDIO_STATUS_OK:
                Log.d(TAG," Enter AudioSystem.AUDIO_STATUS_OK callback:" + mMediaServerOk);
                if (!mMediaServerOk) {
    ...
    public AudioService(Context context) {
       ...
       // Intialized volume
        ...
        /// M: enable camera forced sound for CMCC @{
        try {
            IAudioServiceExt cameraExt = MediatekClassFactory
                    .createInstance(IAudioServiceExt.class);
            if (cameraExt != null) {
                cameraSoundForced = cameraExt.isCameraSoundForced();
                Log.d(TAG, "plugin cameraSoundForced: " + cameraSoundForced);
            }
        } catch (Exception e) {
            Log.e(TAG, "createInstance IAudioServiceExt fail", e);
        }
        /// @}
        ...
        /// M: Listen both sim card @{
        //mark google default and add both sim card listen
        ...
        // Register for phone state monitoring
            ...
            try {
                ...
                tr.listen("AudioService", mPhoneStateListener.getCallback(), PhoneStateListener.LISTEN_CALL_STATE, notifyNow);
                ITelephonyRegistry tr2 = ITelephonyRegistry.Stub.asInterface(ServiceManager.getService("telephony.registry2"));
                tr2.listen("AudioService", mPhoneStateListener.getCallback(), PhoneStateListener.LISTEN_CALL_STATE, notifyNow);
                if(PhoneConstants.GEMINI_SIM_NUM >= 3){
                    ITelephonyRegistry tr3 = ITelephonyRegistry.Stub.asInterface(ServiceManager.getService(
                            "telephony.registry3"));
                    tr3.listen("AudioService", mPhoneStateListener.getCallback(), PhoneStateListener.LISTEN_CALL_STATE, notifyNow);
                }
                if(PhoneConstants.GEMINI_SIM_NUM >= 4){      
                    ITelephonyRegistry tr4 = ITelephonyRegistry.Stub.asInterface(ServiceManager.getService(
                            "telephony.registry4"));
                    tr4.listen("AudioService", mPhoneStateListener.getCallback(), PhoneStateListener.LISTEN_CALL_STATE, notifyNow);
                }           
    ...
    public void adjustStreamVolume(int streamType, int direction, int flags) {
        ...
        if ((direction == AudioManager.ADJUST_RAISE) &&
            ...
            if ((streamTypeAlias == AudioSystem.STREAM_MUSIC) &&
                ...
                if (DEBUG_VOL) Log.d(TAG, "adjustStreamVolume: MuteCount > 0.");
    ...
    public void adjustMasterVolume(int steps, int flags) {
        ...
        if (DEBUG_VOL) Log.d(TAG, "adjustMasterVolume volume: " + volume + " steps: " + steps);
    ...
    public void setStreamSolo(int streamType, boolean state, IBinder cb) {
        if (DEBUG_VOL) Log.d(TAG, "setStreamSolo: StreamType = " + streamType + ", state = " + state);
    ...
    public void setStreamMute(int streamType, boolean state, IBinder cb) {
        if (DEBUG_VOL) Log.d(TAG, "setStreamMute: StreamType = " + streamType + ", state = " + state);
    ...
    public void setRingerMode(int ringerMode) {
        ...
        if (ringerMode != getRingerMode()) {
            ...
            broadcastRingerMode(ringerMode);
            /// M: Notify RingerMode changed to AudioProfile
    ...
    public void setMode(int mode, IBinder cb) {
        if (DEBUG_VOL) Log.d(TAG, "setMode: mode = " + mode);
	...
	/// M: Add synchronized to avoid monkey AudioService JE @ {
    ...
    private PhoneStateListener mPhoneStateListener = new PhoneStateListener() {
        ...
        public void onCallStateChanged(int state, String incomingNumber) {
            if (state == TelephonyManager.CALL_STATE_RINGING) {
				...
				/// M: Add for set PhoneMode.
				mAudioManager.setParameters ("PhoneMode=1");
				...
				/// M: Add for set PhoneMode.
		        mAudioManager.setParameters("PhoneMode=2");
                synchronized(mRingingLock) {
                    mIsRinging = false;
                }
				...
				/// M: Add for set PhoneMode.
		        mAudioManager.setParameters ("PhoneMode=0");
                synchronized(mRingingLock) {
                    mIsRinging = false;
                }
            }
        ...
        if (!mFocusStack.empty() && (mFocusStack.peek().mFocusDispatcher != null)) {
            if (canReassignAudioFocus()) {
                try {
                    ...
                    Log.i(TAG, "notifyTopOfAudioFocusStack: clientId = " + mFocusStack.peek().mClientId);
                } catch (RemoteException e) {
    ...
    public int requestAudioFocus(int mainStreamType, int focusChangeHint, IBinder cb,
        ...
        synchronized(mAudioFocusLock) {
            if (!canReassignAudioFocus()) {
            	Log.i(TAG, "Can not Reassign AudioFocus");
                return AudioManager.AUDIOFOCUS_REQUEST_FAILED;
            }
    ...
    private void handleConfigurationChanged(Context context) {
        try {
            ...
            try {
                ...
                if (cameraExt != null) {
                    cameraSoundForced = cameraExt.isCameraSoundForced();
                    Log.d(TAG, "handleConfigurationChanged() plugin cameraSoundForced: "
                            + cameraSoundForced);
                }
            } catch (Exception e) {
                Log.e(TAG, "handleConfigurationChanged() createInstance IAudioServiceExt fail", e);
            }
            /// @}
            synchronized (mSettingsLock) {
                synchronized (mCameraSoundForced) {
	...
	*/
     ...
     * M: enable HDMI Audio.
     */
    public boolean enableHdmiAudio(boolean enabled, IBinder cb){
        Log.e(TAG, "enableHDMIAudio("+enabled+")");
        String state  = null;
        boolean bRet = false;

        state = mAudioManager.getParameters(GET_HDMI_AUDIO_STATUS);
        if((enabled && state.equalsIgnoreCase(HDMI_AUDIO_STATUS_DISABLED))||(!enabled && state.equalsIgnoreCase(HDMI_AUDIO_STATUS_ENABLED))) {
//        if(true) {
            String cmdStr = enabled?SET_HDMI_AUDIO_ENABLED:SET_HDMI_AUDIO_DISABLED;
            Log.e(TAG, "Command string = "+cmdStr);
            mAudioManager.setParameters(cmdStr);
            state = mAudioManager.getParameters(GET_HDMI_AUDIO_STATUS);
            
            if((enabled && state.equalsIgnoreCase(HDMI_AUDIO_STATUS_ENABLED)) || (!enabled && state.equalsIgnoreCase(HDMI_AUDIO_STATUS_DISABLED))) {
                Log.d(TAG, "enableHDMIAudio(), set HDMI audio enable state succeed, audio state =\" " + state + "\"");
                bRet = true;
            } else {
                Log.e(TAG, "enableHDMIAudio(), set HDMI audio enable state failed, audio state = \"" + state + "\"");
            }
            
            if(enabled && cb!=null){
                EnableHDMIAudioDeathHandler hdl = new EnableHDMIAudioDeathHandler(cb);
                try {
                    cb.linkToDeath(hdl, 0);
                } catch (RemoteException e) {
                    // client has already died!
                    Log.e(TAG, "enableHDMIAudio("+enabled+") could not link to " + cb + " binder death.");
                    return false;
                }
            }
        }else{
            Log.e(TAG, "enableHDMIAudio(), Target HDMI audio state="+enabled+", but current state is "+state+", fail or not need to call audio driver");
        }
        return bRet;
    }
	
	  /**
	  * M: EnableHDMIAudioDeathHandler
	  */
    class EnableHDMIAudioDeathHandler implements IBinder.DeathRecipient{
        private IBinder mCb; // To be notified of client's death

        EnableHDMIAudioDeathHandler(IBinder cb) {
            mCb = cb;
        }

        public void binderDied() {
            Log.w(TAG, "EnableHDMIAudioDeathHandler::binderDied");
            enableHdmiAudio(false, mCb);
        }
  ...
  public boolean isFmOn() {
    ...
    public void listenRingerModeAndVolume(IAudioProfileListener callback, int event) {
        if (event != 0) {
            synchronized (mRecords) {
                ...
                if (event == AudioProfileListener.LISTEN_RINGERMODE_CHANGED) {
                    try {
                        ...
                        Log.e(TAG, "Dead object in listenRingerModeAndVolume, remove listener's callback!" + e);
                    }
                } else if (event == AudioProfileListener.LISTEN_RINGER_VOLUME_CHANGED) {
                    try {
                        record.mCallback.onRingerVolumeChanged(getStreamVolume(AudioManager.STREAM_RING), getStreamVolume(AudioManager.STREAM_RING), null);
                    } catch (RemoteException e) {
                        remove(record.mBinder);
                        Log.e(TAG, "Dead object in listenRingerModeAndVolume, remove listener's callback!" + e);
                    }
                    ...
                    try {
                        record.mCallback.onVibrateSettingChanged(AudioManager.VIBRATE_TYPE_NOTIFICATION, getVibrateSetting(AudioManager.VIBRATE_TYPE_NOTIFICATION));
                    } catch (RemoteException e) {
                        remove(record.mBinder);
                        Log.e(TAG, "Dead object in listenRingerModeAndVolume, remove listener's callback!" + e);
                    }
            ...
            Log.d(TAG, "listenRingerModeAndVolume with LISTEN_NONE, so remove this listener's callback!");
        }
    }
    /**
    ...
    private void notifyRingerModeChanged() {
        if(!mRecords.isEmpty()) {
            Log.d(TAG, "notifyRingerModeChanged: New ringermode = " + mRingerMode + ", clients = " + mRecords.size());
            synchronized (mRecords) {
                Iterator<Record> iterator = mRecords.iterator();
                while (iterator.hasNext()) {
                    Record record = (Record) iterator.next();
            ...
            synchronized (mRecords) {
                ...
                while (iterator.hasNext()) {
                    ...
                    if (record.mEvent == AudioProfileListener.LISTEN_VIBRATE_SETTING_CHANGED) {
                        try {
                            record.mCallback.onVibrateSettingChanged(vibrateType, getVibrateSetting(vibrateType));
                        } catch (RemoteException e) {
                            iterator.remove();
