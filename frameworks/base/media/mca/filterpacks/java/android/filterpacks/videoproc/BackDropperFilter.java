...
public class BackDropperFilter extends Filter {
    ...
    public void process(FilterContext context) {
        ...
        int saveOutput = SystemProperties.getInt("debug.bdf.output", 0);
		
        copyShaderProgram.process(video, mVideoInput);
		if ((saveOutput & FLAG_SAVE_VIDEOINPUT) != 0) mVideoInput.saveFrame("mVideoInput");
        copyShaderProgram.process(background, mBgInput);
		if ((saveOutput & FLAG_SAVE_BGINPUT) != 0) mBgInput.saveFrame("mBgInput");
		...
		if ((saveOutput & FLAG_SAVE_AUTOWB) != 0) mAutoWB.saveFrame("mAutoWB");
        ...
        if (mFrameCount <= mLearningDuration) {
            ...
            if (mFrameCount == mLearningDuration - mLearningVerifyDuration) {
				...
				if ((saveOutput & FLAG_SAVE_MASKVERIFY) != 0) mMaskVerify[outputIndex].saveFrame("mMaskVerify["+outputIndex+"]");
            ...
            if (mFrameCount == mLearningDuration) {
				...
				if ((saveOutput & FLAG_SAVE_MASKAVERAGE) != 0) mMaskAverage.saveFrame("mMaskAverage");
			...
			if ((saveOutput & FLAG_SAVE_OUTPUT) != 0) output.saveFrame("output");
		...
		{
			if ((saveOutput & FLAG_SAVE_BGMEAN) != 0) mBgMean[outputIndex].saveFrame("mBgMean["+outputIndex+"]");
			if ((saveOutput & FLAG_SAVE_BGVARIANCE) != 0) mBgVariance[outputIndex].saveFrame("mBgVariance["+outputIndex+"]");            
		}
