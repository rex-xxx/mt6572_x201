...
public abstract class Filter {
    ...
    protected final void pushOutput(String name, Frame frame) {
		...
		if (string.compareTo("all") == 0 || string.compareTo(mName) == 0)
		{
			...
			{
				wait3DReady();
				frame.saveFrame(mName);
			}
			else
				Log.d("Frame", "filter "+mName+" pushOutput");
		}		
    }
    ...
    final synchronized void performProcess(FilterContext context) {
		String process_in = SystemProperties.get("debug.effect.processin.name");

		if (process_in.compareTo(mName) == 0)
		{
			wait3DReady();
			Log.i(TAG, "filter "+mName+" process+ glReadPixels done");
		}

		String log_name = SystemProperties.get("debug.effect.log.process");

		if (log_name.compareTo("all") == 0)
		{
			Log.i(TAG, "filter "+mName+" performProcess +");
		}
		...
		String process_out = SystemProperties.get("debug.effect.processout.name");

		if (process_out.compareTo(mName) == 0)
		{
			wait3DReady();
			Log.i(TAG, "filter "+mName+" process- glReadPixels done");
		}
