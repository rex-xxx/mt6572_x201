...
#ifndef ANDROID_DEFAULT_CODE
#define STOPBGAUDIO   // support cancel stop background audio decode
#ifdef STOPBGAUDIO
typedef enum
{
	ManualMCSState_INITIALIZED,
	ManualMCSState_OPENED,
	ManualMCSState_RUNNING,
	ManualMCSState_STOPPING,
	ManualMCSState_FINISHED
}ManualMCSState;
#endif 
#endif
...
#ifdef STOPBGAUDIO	
	ManualMCSState					bgstate;
#endif
#ifndef ANDROID_DEFAULT_CODE
    Mutex                           mBigLock;
    Mutex                           mStopPreviewLock;
#endif
...
#ifndef ANDROID_DEFAULT_CODE
static M4OSA_ERR videoEditor_loadSettings(
#else
                ...
                M4VSS3GPP_ClipSettings*                       pSettings)
{
    ...
    videoEditJava_checkAndThrowIllegalStateException(&needToBeLoaded, pEnv,
                                             ...
                                             "not initialized");
#ifndef ANDROID_DEFAULT_CODE
    videoEditJava_onCheckFail(needToBeLoaded);
#endif
                              ...
                              jobject  thiz)
{
    ...
    videoEditJava_checkAndThrowIllegalStateException(&needToBeLoaded, pEnv,
                                             ...
                                             "not initialized");
#ifndef ANDROID_DEFAULT_CODE
    videoEditJava_onCheckFail2(needToBeLoaded, 0);
    Mutex::Autolock bigLock(pContext->mStopPreviewLock);
#endif
                                    ...
                                    jint surfaceHeight )
{
    ...
    VIDEOEDIT_LOG_FUNCTION(ANDROID_LOG_INFO,
        "VIDEO_EDITOR", "renderPreviewFrame surfaceWidth = %d",surfaceWidth);
    VIDEOEDIT_LOG_FUNCTION(ANDROID_LOG_INFO,
    ...
    if (pContext == NULL) {
        ALOGW("videoEditor_renderPreviewFrame Nothing to do, context is aleady NULL");
        return -1;
    }
    Mutex::Autolock bigLock(pContext->mBigLock);
    // Get the context again if other thread release pContext first
    pContext = (ManualEditContext*)videoEditClasses_getContext(&needToBeLoaded, pEnv, thiz);
    if ( (pContext == NULL) || (pContext->pEditSettings == NULL) ) {
        ALOGW("videoEditor_renderPreviewFrame Nothing to do, context or pEditSettings is aleady NULL");
        return -1;
    }
#endif
    VIDEOEDIT_LOG_FUNCTION(ANDROID_LOG_INFO,
    ...
    videoEditJava_checkAndThrowIllegalStateException(&needToBeLoaded, pEnv,
                                             ...
                                             "not initialized");
#ifndef ANDROID_DEFAULT_CODE
    videoEditJava_onCheckFail2(needToBeLoaded, -1);
#endif
    ...
    if (timeMs == 0) {
        ...
        for (i = 0; i < uiNumberOfClipsInStoryBoard; i++) {
#ifndef ANDROID_DEFAULT_CODE
            // Demon, should show the first frame of next clip, not last frame of previous clip
            // if timeMs exactly matches the duration of some clip
    ...
    /*+ Handle the image files here */
      if (pContext->pEditSettings->pClipList[iCurrentClipIndex]->FileType ==
        ...
        framesizeYuv = ((width+0xF)&~0xF) * ((height+0xF)&~0xF) * 3 >> 1;
#else
    ...
    VIDEOEDIT_LOG_FUNCTION(ANDROID_LOG_INFO, "VIDEO_EDITOR", "renderPreviewFrame done");
    return tnTimeMs;
}
...
#ifndef ANDROID_DEFAULT_CODE  // Morris Yang , for non 16x width video
    framesizeYuv = ((frameWidth+0xF)&~0xF) * ((frameHeight+0xF)&~0xF) * 3 >> 1;
#else
...
#ifdef STOPBGAUDIO
	if(pContext->bgstate != ManualMCSState_STOPPING){
		pContext->bgstate = ManualMCSState_INITIALIZED;
		ALOGV("videoEditor_generateAudio bgstate %d",pContext->bgstate);
	}
#endif
...
#ifdef STOPBGAUDIO
	if(pContext->bgstate != ManualMCSState_STOPPING){
		pContext->bgstate = ManualMCSState_OPENED;
		ALOGV("videoEditor_generateAudio change to bgstate %d",pContext->bgstate);
	}
#endif
...
#ifdef STOPBGAUDIO
	if(pContext->bgstate != ManualMCSState_STOPPING){
		pContext->bgstate = ManualMCSState_RUNNING;
		ALOGV("videoEditor_generateAudio change to bgstate %d",pContext->bgstate);
	}
#endif
...
#ifdef STOPBGAUDIO
	while ((result == M4NO_ERROR)&& (pContext->bgstate!=ManualMCSState_STOPPING) );
#else
    while (result == M4NO_ERROR);
#endif
    /*- PROGRESS CB */
#ifdef STOPBGAUDIO
	ALOGV("videoEditor_generateAudio change to bgstate %d  result %x",pContext->bgstate,result);
	if((result == M4NO_ERROR)&&(pContext->bgstate==ManualMCSState_STOPPING)){
		...
		pContext->bgstate = ManualMCSState_INITIALIZED;
		ALOGD("videoEditor_generateAudio stop return %d",result);
		return result;	
	}
#endif
...
#ifdef STOPBGAUDIO
	pContext->bgstate = ManualMCSState_FINISHED;
	ALOGV("videoEditor_generateAudio change to bgstate %d result %d",pContext->bgstate,result);
#endif
    return result;
}
                ...
                jobject                 audioSettingObject)
{
    ...
    if (needToBeLoaded == false) {
        j = 0;
#ifndef ANDROID_DEFAULT_CODE
        if (pContext == M4OSA_NULL || pContext->pEditSettings == M4OSA_NULL) {
            return;
        }
#endif
...
#ifndef ANDROID_DEFAULT_CODE
        pEnv->DeleteLocalRef(effectSettingsArray);
#endif
...
#ifndef ANDROID_DEFAULT_CODE
        pEnv->DeleteLocalRef(mEditClazz);
        pEnv->DeleteLocalRef(mEffectsClazz);
#endif
    }
    ...
    if (audioSettingObject != M4OSA_NULL) {
        ...
        if (regenerateAudio) {
...
#ifdef STOPBGAUDIO			
			ALOGD("Calling Generate Audio now");
#endif
			...
			if(M4NO_ERROR == result){
				ALOGD("videoEditor_populateSettings bgstate %d",pContext->bgstate);
				if(pContext->bgstate == ManualMCSState_FINISHED){
					pContext->bgstate = ManualMCSState_INITIALIZED;
				}else{
				    ALOGD("videoEditor_populateSettings bg audio no finished!");
					needToBeLoaded == false;
					remove((const char *)pContext->mAudioSettings->pPCMFilePath);
					goto videoEditor_populateSettings_cleanup;
				}
			}
#endif
...
#ifndef ANDROID_DEFAULT_CODE
        pEnv->DeleteLocalRef(audioSettingClazz);
#endif
...
#ifndef ANDROID_DEFAULT_CODE
    pEnv->DeleteLocalRef(mPreviewClipPropClazz);
    pEnv->DeleteLocalRef(engineClass);
#endif
    return;
}
                ...
                jboolean                loop)
{
    ...
    {
        M4VIDEOEDITING_ClipProperties tmppClipProperties;
        for(int i=0; i<pContext->pEditSettings->uiClipNumber; i++)
        {
            result = M4VSS3GPP_editAnalyseClip(pContext->pEditSettings->pClipList[i]->pFile, pContext->pEditSettings->pClipList[i]->FileType, 
                &tmppClipProperties, pContext->initParams.pFileReadPtr );
            if( (NULL!=pContext->mPreviewController->mOuterClipList)&&(NULL!=&(pContext->mPreviewController->mOuterClipList[i]->ClipProperties)))
            {
                if(pContext->pEditSettings->pClipList[i]->FileType == /*M4VIDEOEDITING_kFileType_JPG*/ M4VIDEOEDITING_kFileType_ARGB8888 )
                {
                    pContext->mPreviewController->mOuterClipList[i]->ClipProperties.uiClipAudioDuration = 
                        pContext->pEditSettings->pClipList[i]->ClipProperties.uiClipDuration;
                    pContext->mPreviewController->mOuterClipList[i]->ClipProperties.uiClipVideoDuration = 
                        pContext->pEditSettings->pClipList[i]->ClipProperties.uiClipDuration;
                }
                else
                {
                    pContext->mPreviewController->mOuterClipList[i]->ClipProperties.uiClipAudioDuration = tmppClipProperties.uiClipAudioDuration;
                    pContext->mPreviewController->mOuterClipList[i]->ClipProperties.uiClipVideoDuration = tmppClipProperties.uiClipVideoDuration;
                }
            }
            ALOGD("result %x, i = %d uiClipNumber = %d, a %d, v %d", result, i, pContext->pEditSettings->uiClipNumber,
                tmppClipProperties.uiClipAudioDuration, tmppClipProperties.uiClipVideoDuration);
        }
    }
#endif
                ...
                jstring                             file)
{
    ...
    videoEditJava_checkAndThrowIllegalStateException(&needToBeLoaded, pEnv,
                                             ...
                                             "not initialized");
#ifndef ANDROID_DEFAULT_CODE
    videoEditJava_onCheckFail2(needToBeLoaded, object);
#endif
...
#ifndef ANDROID_DEFAULT_CODE
        /*
         ALPS00613377 by ck.wang @20130502
         Move the timeMS ahead 0.5s to avoid decoding thumbnail at the end of video.
         Most of such cases happened when decoding the thumbnail of transition out clip.
         */
        if ((1 == noOfThumbnails) && (0 == duration) && (500 <= timeMS)) {
            timeMS -= 500;
        }
#endif
                ...
                jstring                             libraryPath)
{
    ...
    videoEditJava_checkAndThrowIllegalStateException(&initialized, pEnv,
                                             (M4OSA_NULL != pContext),
                                             "already initialized");
    VIDEOEDIT_LOG_API(ANDROID_LOG_INFO, "VIDEO_EDITOR", "initialized %d", initialized);
    ...
    if (initialized)
    {
        ...
        if (initialized)
        {
            ...
            // Log the result.
            VIDEOEDIT_LOG_RESULT(ANDROID_LOG_INFO, "VIDEO_EDITOR result: %s",
...
M4OSA_ERR videoEditor_processClip(
    ...
    videoEditJava_checkAndThrowIllegalStateException(&loaded, pEnv,
                                             ...
                                             "not initialized");
#ifndef ANDROID_DEFAULT_CODE
    videoEditJava_onCheckFail2(loaded, M4ERR_STATE);
#endif
...
videoEditor_generateClip(
    ...
    videoEditJava_checkAndThrowIllegalArgumentException(&loaded, pEnv,
                                                ...
                                                "settings is null");
#ifndef ANDROID_DEFAULT_CODE
    videoEditJava_onCheckFail2(loaded, M4ERR_PARAMETER);
#endif
...
#ifndef ANDROID_DEFAULT_CODE
    result = videoEditor_loadSettings(pEnv, thiz, settings);
    if (result != M4NO_ERROR) {
        return result; 
    }

    if (pContext->pEditSettings != NULL 
            && pContext->pEditSettings->xVSS.outputVideoFormat == M4VIDEOEDITING_kH264) {
        // for MediaFrameworkTest video editor stress test
        ALOGW("H264 export is unsupported now");
                ...
                jobject                             settings)
{
    ...
    videoEditJava_checkAndThrowIllegalArgumentException(&needToBeLoaded, pEnv,
                                                ...
                                                "settings is null");
#ifndef ANDROID_DEFAULT_CODE
    videoEditJava_onCheckFail2(needToBeLoaded, M4ERR_PARAMETER);
#endif
    ...
    videoEditJava_checkAndThrowIllegalStateException(&needToBeLoaded, pEnv,
                                             ...
                                             "not initialized");
#ifndef ANDROID_DEFAULT_CODE
    videoEditJava_onCheckFail2(needToBeLoaded, M4ERR_STATE);
#endif
                ...
                jobject                             thiz)
{
    ...
    ALOGD("videoEditor_stopEncoding START");
    ...
    // If context is not set, return (we consider release already happened)
    if ( pContext == NULL) {
        ALOGW("videoEditor_release Nothing to do, context is aleady NULL");
        return;
    }
    ...
    pContext = (ManualEditContext*)videoEditClasses_getContext(&stopped, pEnv, thiz);
    if ( (pContext == NULL) || (pContext->pEditSettings == NULL) ) {
        ALOGW("videoEditor_release Nothing to do, context or pEditSettings is aleady NULL");
        return;
    }

    // check if there is any incomplete output file
    if (NULL != pContext->pEditSettings->pOutputFile)
    {
        ALOGD ("@@ videoEditor_stopEncoding remove incomplete file (%s)(%d)",  (M4OSA_Char*)pContext->pEditSettings->pOutputFile, gettid());
        remove((const char *)pContext->pEditSettings->pOutputFile);
    }
#endif
    ...
    videoEditJava_checkAndThrowIllegalStateException(&stopped, pEnv,
                                             (M4OSA_NULL == pContext),
                                             "not initialized");
	pContext->bgstate = ManualMCSState_STOPPING;
...
#ifdef STOPBGAUDIO
	if (stopped) {
		ALOGD("videoEditor_stopEncoding pContext->bgstate %d",pContext->bgstate);
		if(pContext->bgstate != ManualMCSState_INITIALIZED){
	        // Set the state to initialized.
	        pContext->bgstate = ManualMCSState_INITIALIZED;			
		}else{
			pContext->state = state;
			ALOGD("videoEditor_stopEncoding BGAudio Stop!return pContext->state %d",pContext->state);
			return ;
		}
	}
#endif
    if (stopped) {
        ...
        if (pContext->state != ManualEditState_INITIALIZED)
        {
            ...
            result = M4xVSS_CloseCommand(pContext->engineContext);
            ALOGD("videoEditor_stopEncoding M4xVSS_CloseCommand() returned 0x%x",
                (unsigned int)result);
        }
...
#ifndef ANDROID_DEFAULT_CODE
    ALOGD("videoEditor_stopEncoding END");
#endif
}
                ...
                jobject                             thiz)
{
    ...
    Mutex::Autolock bigLock(pContext->mBigLock);
    VIDEOEDIT_LOG_FUNCTION(ANDROID_LOG_INFO,
                            "VIDEO_EDITOR","released = %d ",released);
#endif
    // Check if the context is valid (required because the context is dereferenced).
    if (released)
    {
		...
		if (pContext->bgstate != ManualMCSState_INITIALIZED){	
			pContext->bgstate = ManualMCSState_STOPPING;
			ALOGD("videoEditor_release ManualMCSState_STOPPING");
			Mutex::Autolock autoLock(pContext->mLock);
		}		
#endif
        // Reset the context.
    ...
    VIDEOEDIT_LOG_API(ANDROID_LOG_INFO, "VIDEO_EDITOR", "videoEditor_release()done");
}
                     ...
                     ManualEditContext* pContext)
{
    ...
    videoEditJava_checkAndThrowIllegalStateException(&threadStarted, pEnv,
                                             ...
                                             "not initialized");
#ifndef ANDROID_DEFAULT_CODE
    videoEditJava_onCheckFail2(threadStarted, M4ERR_STATE);
#endif
    ...
    *******************************************************************************/
    ...
    if ( err != M4NO_ERROR) {
        ...
        jniThrowException(pEnv, "java/io/IOException", "file size get option failed");
        //return -1;
#ifndef ANDROID_DEFAULT_CODE
		return err;
#endif
    ...
    *******************************************************************************/
    ...
    err = M4OSA_fileWriteData (outFileHandle, (M4OSA_MemAddr8)&samplesCountBigEndian,
        sizeof(M4OSA_UInt32) );
    if (err != M4NO_ERROR) {
        jniThrowException(pEnv, "java/io/IOException", "file write failed");
#ifndef ANDROID_DEFAULT_CODE
		goto out2;
#endif
    }
    ...
    if (numBytesToRead != 0) {
        //err = -1;
        jniThrowException(pEnv, "java/io/IOException", "numBytesToRead != 0 ; file write failed");
#ifndef ANDROID_DEFAULT_CODE
		goto out2;
#endif
    }
    ...
    if ( err != M4NO_ERROR) {
        jniThrowException(pEnv, "java/io/IOException", "file seek failed");
#ifndef ANDROID_DEFAULT_CODE			
			goto out2;
#endif		
    } else {
