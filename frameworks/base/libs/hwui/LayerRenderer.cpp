...
status_t LayerRenderer::prepareDirty(float left, float top, float right, float bottom,
        bool opaque) {
    LAYER_RENDERER_LOGD("Rendering into layer, fbo = %d, layer = %p", mLayer->getFbo(), mLayer);
...
Layer* LayerRenderer::createLayer(uint32_t width, uint32_t height, bool isOpaque) {
    ...
    LAYER_RENDERER_LOGD("Get layer, %dx%d fbo = %d, texture = %d, layer = %p",
            layer->getWidth(), layer->getHeight(), layer->getFbo(), layer->getTexture(), layer);
    ...
    if (status != GL_FRAMEBUFFER_COMPLETE) {
        ...
        ALOGW("Framebuffer attachment type = 0x%x, name = %d", type, name);
    }
...
void LayerRenderer::updateTextureLayer(Layer* layer, uint32_t width, uint32_t height,
    ...
    if (layer) {
        LAYER_RENDERER_LOGD("Updating layer, %dx%d fbo = %d, texture = %d, renderTarget = 0x%x",
                width, height, layer->getFbo(), layer->getTexture(), renderTarget);
...
void LayerRenderer::destroyLayer(Layer* layer) {
    if (layer) {
        LAYER_RENDERER_LOGD("Recycling layer, %dx%d fbo = %d, texture = %d, layer = %p",
                layer->getWidth(), layer->getHeight(), layer->getFbo(), layer->getTexture(), layer);
...
void LayerRenderer::destroyLayerDeferred(Layer* layer) {
    if (layer) {
        LAYER_RENDERER_LOGD("Deferring layer destruction, fbo = %d, texture = %d, layer = %p",
                layer->getFbo(), layer->getTexture(), layer);
