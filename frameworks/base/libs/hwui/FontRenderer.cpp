...
void FontRenderer::cacheBitmap(const SkGlyph& glyph, CachedGlyphInfo* cachedGlyph,
    ...
    *retOriginY = startY;
    ...
    if (g_HWUI_debug_dumpAlphaTexture)
        dumpAlphaTexture(stride, glyph.fHeight, bitmapBuffer, "Glyph");
#endif
#endif
...
#if defined(MTK_DEBUG_RENDERER)
#if MTK_DEBUG_FONT_TEXTURE
            if (g_HWUI_debug_dumpAlphaTexture) {
                char prefix[64];
                sprintf(prefix, "FontTexture_%d_%d", i, cacheTexture->getTextureId());
                dumpAlphaTexture(cacheTexture->getWidth(), cacheTexture->getHeight(), cacheTexture->getTexture(), prefix);
            }
#endif
#endif
        }
...
FontRenderer::DropShadow FontRenderer::renderDropShadow(SkPaint* paint, const char *text,
    ...
    /** Mediatek: Fix incorrect shadow for long text **/
    if (paddedWidth > (uint32_t)mMaxTextureSize) {
        ALOGW("Shadow width = %d exceed max texture size, truncate to %d",
            paddedWidth, mMaxTextureSize);
        paddedWidth = mMaxTextureSize;
    }
    if (paddedHeight > (uint32_t)mMaxTextureSize) {
        ALOGW("Shadow width = %d exceed max texture size, truncate to %d",
            paddedHeight, mMaxTextureSize);
        paddedHeight = mMaxTextureSize;
    }
