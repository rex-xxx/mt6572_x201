...
uint32_t PatchCache::countQuad(const int32_t* xDivs, const int32_t* yDivs,
        const uint32_t width, const uint32_t height,
        const float bitmapWidth, const float bitmapHeight) {
    uint32_t xCount = countSection(xDivs, width, bitmapWidth);
    uint32_t yCount = countSection(yDivs, height, bitmapHeight);
    return xCount * yCount;
}

uint32_t PatchCache::countSection(const int32_t* divs, const uint32_t divCount,
        const float bound) {
    uint32_t sectionCount = 0;
    if (divs[0] > 0.0f) {
        sectionCount++;
    }
    for (uint32_t i = 1; i < divCount; i++) {
        if ((divs[i] - divs[i - 1]) > 0.0f) {
            sectionCount++;
        }
    }
    if (divs[divCount - 1] < bound) {
        sectionCount++;
    }
    return sectionCount;
}
