...
void LayerCache::deleteLayer(Layer* layer) {
    if (layer) {
        LAYER_LOGD("Destroying layer %dx%d, fbo %d, layer %p", layer->getWidth(), layer->getHeight(),
                layer->getFbo(), layer);
...
Layer* LayerCache::get(const uint32_t width, const uint32_t height) {
    ...
    if (index >= 0) {
        ...
        LAYER_LOGD("Reusing layer %dx%d, layer = %p", layer->getWidth(), layer->getHeight(), layer);
    } else {
