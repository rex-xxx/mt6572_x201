...
void DisplayList::output(OpenGLRenderer& renderer, uint32_t level) {
    ...
    if (!Caches::getInstance().isDisplayListActive(this)) {
        ALOGE("%sThe displaylist has been destroyed!!", (char*) indent);
        return;
    }
#endif
        ...
        switch (op) {
            ...
            case DrawLayer: {
                ...
                if (!Caches::getInstance().isLayerActive(layer))
                    ALOGE("%sThe layer has been destroyed!!", (char*) indent);
#endif
            }
            break;
            case DrawBitmap: {
...
#if defined(MTK_DEBUG_RENDERER)
        if (op >= DisplayList::DrawDisplayList) { // is drawing operation
            dumpDraw(renderer.getWidth(), renderer.getHeight(), level);
        }
#endif
    }
...
#if defined(MTK_DEBUG_RENDERER)
    dumpDisplayList(renderer.getWidth(), renderer.getHeight(), level);
#endif
