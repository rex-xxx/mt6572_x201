...
void OpenGLRenderer::initProperties() {
    ...
    if (property_get(PROPERTY_DISABLE_SCISSOR_OPTIMIZATION, property, "true")) {
            ...
            while ((status = glGetError()) != GL_NO_ERROR) {
                ...
                switch (status) {
                    ...
                    case GL_INVALID_FRAMEBUFFER_OPERATION:
                        ALOGE("  GL_INVALID_FRAMEBUFFER_OPERATION");
                        ALOGE("Framebuffer status: 0x%x", glCheckFramebufferStatus(GL_FRAMEBUFFER));
                        break;
...
void OpenGLRenderer::setHWUIDebugLog(bool enable) {
    setDebugLog(enable);
}
...
void OpenGLRenderer::composeLayerRegion(Layer* layer, const Rect& rect) {
    ...
    if (CC_LIKELY(!layer->region.isEmpty())) {
        ...
        if (g_HWUI_debug_layers_as_regions)
...
status_t OpenGLRenderer::drawDisplayList(DisplayList* displayList,
    ...
    if (!Caches::getInstance().isDisplayListActive(displayList)) {
        ALOGE("ERROR: The displaylist %p (level=%d) has been destroyed!!", displayList, level + 1);
        if (!Caches::getInstance().dumpActiveDisplayLists(displayList)) {
            Caches::getInstance().recordInvalidDisplayList(displayList);
            return DrawGlInfo::kStatusDone;
        }
        ALOGW("WARNING: Found the displaylist %p in lists. Keep renderering.", displayList);
    }
#endif
    ...
    if (displayList && displayList->isRenderable()) {
        if ((mCaches.getDebugLevel() & kDebugDisplayLists) && level == 0) {
            outputDisplayList(displayList, level);
        }
...
#if MTK_DEBUG_ERROR_CHECK
    if (!Caches::getInstance().isLayerActive(layer)) {
        ALOGE("ERROR: The layer %p has been destroyed!!", layer);
        if (!Caches::getInstance().dumpActiveLayers(layer)) {
            return DrawGlInfo::kStatusDone;
        }
        ALOGW("WARNING: Found the layer %p in lists. Keep renderering.", layer);
    }
#endif
