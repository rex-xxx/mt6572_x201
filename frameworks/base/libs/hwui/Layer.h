...
struct Layer {
    ...
    inline void bindTexture() {
        if (texture.id) {
            LAYER_LOGD("Layer::bindTexture: glBindTexture(0x%x, %d)", renderTarget, texture.id);
    ...
    inline void generateTexture() {
        if (!texture.id) {
            ...
            LAYER_LOGD("Layer::generateTexture: glGenTextures(1, %d)", texture.id);
        }
    }

    inline void deleteTexture() {
        if (texture.id) {
            LAYER_LOGD("Layer::deleteTexture: glDeleteTextures(1, %d)", texture.id);
    ...
    inline void allocateTexture(GLenum format, GLenum storage) {
        LAYER_LOGD("Layer::allocateTexture: glTexImage2D(0x%x, 0, 0x%x, %d, %d, 0, 0x%x, 0x%x, NULL)",
                renderTarget, format, getWidth(), getHeight(), format, storage);
