...
///M:add For ThemeManager
#if MTK_THEMEMANAGER_APP
static const char* kDefaultThemePath = "/system/framework/framework-res.apk";
static char kThemeApkPath[100];
static const char* kISmsPath = "/data/app/com.hissage-";
#endif
namespace {
      ...
      mCacheMode(cacheMode), mCacheValid(false)
{
	...
	if (mThemePath == NULL) {
	    ALOGE("mThemePath allocate failed.");
	}
 #ifdef THEME_NO_BUILD
    ALOGV("THEME_NO_BUILD is set for host.");
 #else
    ALOGV("THEME_NO_BUILD is not set in device and parse here .mThemeManager =  %d.", mThemeManager);
...
{
	ALOGV("AssetManager destructor called here.");
#if MTK_THEMEMANAGER_APP 
	if (mThemePath != NULL) {
		delete mThemePath;
		mThemePath = NULL;
...
Asset* AssetManager::openNonAsset(void* cookie, const char* fileName, AccessMode mode)
{
    ...
    ALOGV("kThemeApkPath = '%s' \n", kThemeApkPath);
    mThemePath->path.setTo(kThemeApkPath);   
    ALOGV("mThemePath = '%s' \n", mThemePath->path.string());
#endif
    AutoMutex _l(mLock);
    ...
    if (mCacheMode != CACHE_OFF && !mCacheValid)
        loadFileNameCacheLocked();
#if MTK_THEMEMANAGER_APP
	ALOGV("openNonAsset MTK_THEMEMANAGER_APP feature option is opened.");
	if (which < mAssetPaths.size()) {
		ALOGV("Looking for non-asset '%s' in '%s',which = %d.\n", fileName,
				mThemePath->path.string(), which);
		Asset* pAsset = getAssetFromTheme(which, fileName, *mThemePath, mode);
		if (pAsset == NULL) {
			ALOGV("Asset not found from theme package.");
...
#else
	ALOGV("openNonAsset MTK_THEMEMANAGER_APP feature option is not opened.");
    if (which < mAssetPaths.size()) {
        ALOGV("Looking for non-asset '%s' in '%s'\n", fileName,
...
#if MTK_THEMEMANAGER_APP
/**
 * Change to upper case characters to lower case characters,
 * so it can be used as the resource name.
 */
char* AssetManager::getLowercaseApkName(char* str) {
	char *ptr1 = strrchr(str, '/');
	char *ptr2 = strrchr(str, '.');

	char* strRes = (char *)malloc(strlen(str) + 1);
	memset(strRes, 0x00, strlen(str) + 1);
	strncpy(strRes, ptr1, strlen(ptr1)-strlen(ptr2));

	for (unsigned int i = 0; i < strlen(strRes); i++) {
		if (*(strRes+i) >= 'A' && *(strRes+i) <= 'Z') {
			strRes[i] = *(strRes+i) + 0x20;
		} else if (*(strRes+i) == '-') {
			strRes[i] = '_';
		}
	}
	ALOGV("getLowercaseApkName: The strRes = %s, len = %d.\n", strRes, strlen(strRes));
	return strRes;
}

/**
 * Append the apk name as a prefix to the given resource name, return it
 * through outApkFileName, for example, if the apkName is "/system/app/Launcher2.apk",
 * and the filename is "a.png", the content of the outApkFileName would be "launcher2_a.png".
 */
void AssetManager::appendApkPath(char* apkName, const char* fileName,
		char* outApkFileName) {
	char *ptr1 = getLowercaseApkName(apkName);
    const char* ptr2 = strrchr(fileName, '/');

	strncpy(outApkFileName, fileName, strlen(fileName)-strlen(ptr2));
	strcat(outApkFileName, ptr1);
	strcat(outApkFileName, "_");
	strcat(outApkFileName, ptr2+1);

	free(ptr1);
    ALOGV("appendApkPath apkFileName Info '%s'\n", outApkFileName);
}
...
/**
 * Test the theme map function.
 */

void AssetManager::testThemeMap()
{
	bool ret1 = mThemeManager->isFileInMap("/system/framework/framework-res.apk", "activity_title_bar.9.png");
	bool ret2 = mThemeManager->isFileInMap("/system/app/Launcher2.apk", "a.png");
	bool ret3 = mThemeManager->isFileInMap("/system/app/Launcher2.apk", "b.png");
	bool ret4 = mThemeManager->isFileInMap("/system/app/Settings.apk", "a.png");
	bool ret5 = mThemeManager->isFileInMap("/system/app/Settings.apk", "b.png");
	bool ret6 = mThemeManager->isFileInMap("/system/app/asd.apk", "b.png");
	bool ret7 = mThemeManager->isFileInMap("/system/app/MtkBt.apk", "bt_share.png");
	bool ret8 = mThemeManager->isFileInMap("/system/app/SystemUI.apk", "statusbar_background.9.png");
	bool ret9 = mThemeManager->isFileInMap("/system/app/Contacts.apk", "phone_dial_num_0_blk.png");
	bool ret10 = mThemeManager->isFileInMap("/system/app/MtkWeatherWidget.apk", "wgt_weather_background.9.png");
	bool ret11 = mThemeManager->isFileInMap("/system/framework/framework-res.apk", "divider_vertical_dark.9.png");

	ALOGD("openNonAsset test:ret1 = %d, ret2 = %d, ret3 = %d, ret4 = %d, ret5 = %d, ret6 = %d,"
			"ret7 = %d, ret8 = %d, ret9 = %d, ret10 = %d, ret11 = %d.",
