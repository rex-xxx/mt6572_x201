        ...
        int light, int colorARGB, int flashMode, int onMS, int offMS, int brightnessMode)
{
    ...
    ALOGD("setLight_native: light=%d, colorARGB=0x%x, flashMode=%d, onMS=%d, offMS=%d, brightnessMode=%d",
	light, colorARGB, flashMode, onMS, offMS, brightnessMode);

#if defined(MTK_AAL_SUPPORT)
    if (light == LIGHT_INDEX_BACKLIGHT) {
        if (AALClient::getInstance().setBacklightColor(colorARGB & 0x00ffffff) == 0)
            return;
        ALOGW("Fail to set backlight from AAL service");
    }
#endif
