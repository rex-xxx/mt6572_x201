...
static void mtProf(int start)
{
    int fd_profsts,fd_memprof,fd_cputime;
    int ret;
    char buf[64];

    /* 1. Check if we need to trigger profiling*/
    fd_profsts = open("/proc/mtprof/status",O_RDWR);
    if (fd_profsts == -1){
        ALOG(LOG_INFO,"AP_PROF","unable to open /proc/mtprof/status entry");
        return;
    }
    read(fd_profsts, buf,1);
    if(buf[0] == '0'){
        //LOG(LOG_INFO,"AP_PROF"," mt prof is not set, status = 0");
	close(fd_profsts);
	return;
    }
    ...
    if (fd_cputime == -1){
        ALOG(LOG_INFO,"AP_PROF","unable to open /proc/mtprof/cputime entry");
        return;
    }
   
    if(1 == start){
        ALOG(LOG_INFO,"AP_PROF","Start MT profiling");
        write(fd_cputime,"1", 1);
    }
    else
    {
        write(fd_cputime,"0", 1);
        write(fd_profsts,"0", 1);
    }
...
void InputReader::loopOnce() {
    ...
    //ALOGD("loopOnce:: release lock 1" );
    size_t count = mEventHub->getEvents(timeoutMillis, mEventBuffer, EVENT_BUFFER_SIZE);

    { // acquire lock
        AutoMutex _l(mLock);
	//ALOGD("loopOnce:: acquire lock 2" );
...
void TouchInputMapper::dispatchTouches(nsecs_t when, uint32_t policyFlags) {
    ...
    if (currentIdBits == lastIdBits) {
        ...
        while (!upIdBits.isEmpty()) {
            ...
            Tracer::traceBegin(ATRACE_TAG_INPUT, "AppLaunch_dispatchPtr:Up");
            ALOGD("AP_PROF:AppLaunch_dispatchPtr:Up:%lld", when/1000000);
            dispatchMotion(when, policyFlags, mSource,
            ...
            Tracer::traceEnd(ATRACE_TAG_INPUT);
        }
        ...
        while (!downIdBits.isEmpty()) {
            ...
            if (dispatchedIdBits.count() == 1) {
                ...
                mDownTime = when;
                Tracer::traceBegin(ATRACE_TAG_INPUT, "AppLaunch_dispatchPtr:Down");
                ALOGD("AP_PROF:AppLaunch_dispatchPtr:Down:%lld", mDownTime/1000000);
                mtProf(1);
                Tracer::traceEnd(ATRACE_TAG_INPUT);
            }
