...
public class NetworkStatsServiceTest extends AndroidTestCase {
    ...
    ///M: EMMA
    private NetworkStatsService mService2;
...
//            IoUtils.deleteContents(mStatsDir);
            ...
            for (File file : mStatsDir.listFiles()) {
                Xlog.d(TAG, " setUp  file:" + file.getName());
                if(file.getName().equals("junit-report.xml") ||file.getName().equals("coverage.ec")  ){

                }else{
                    Xlog.d(TAG, " setUp delete file:" + file.getName());
                     IoUtils.deleteContents(file);
                }
            }
        }
        Xlog.d(TAG, " mStatsDir:" + mStatsDir);
        ...
        mService.setUseTrustedTime(true);

        ///M: EMMA
        mService2 = new  NetworkStatsService(mServiceContext,mNetManager, mAlarmManager);
        Field privateStringField = NetworkStatsService.class.getDeclaredField("LOGV");
        privateStringField.setAccessible(true);
        boolean fieldValue = (boolean) privateStringField.getBoolean(mService2);
 ...
 //       IoUtils.deleteContents(mStatsDir);
           ...
           for (File file : mStatsDir.listFiles()) {
                Xlog.d(TAG, " tearDown file:" + file.getName());
                if(file.getName().equals("junit-report.xml") ||file.getName().equals("coverage.ec")  ){
                    }else{
                    Xlog.d(TAG, " tearDown delete file:" + file.getName());
                   // file.delete();
                   IoUtils.deleteContents(file);
                }
           }
        ...
        mService.setUseTrustedTime(false);
    ...
    public void testStatsRebootPersist() throws Exception {
        ...
        ///M: EMMA
        mService.getNetworkTotalBytes(sTemplateWifi, Long.MIN_VALUE, Long.MAX_VALUE); 
        mService.forceUpdate();
        String[] iface = mService.getMobileIfaces();
        mService.advisePersistThreshold(2*1024*1024) ;
