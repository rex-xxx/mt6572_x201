...
public class InputMethodManagerService extends IInputMethodManager.Stub
    ...
    private static class MethodCallback extends IInputMethodCallback.Stub {
        ...
        /// M: notify Ims pid @{
        @Override
        public void notifyImsPid(int pid)
        {
            mImsPid = pid;
            Slog.v(TAG, "Current IMS pid="+mImsPid);   
        }
        /// notify Ims pid @}
    }
    ...
    private void showInputMethodMenuInternal(boolean showSubtypes) {
        ...
        synchronized (mMethodMap) {
            ...
            for (int i = 0; i < N; ++i) {
                ...
                if (mIms[i].getId().equals(lastInputMethodId)) {
                    ...
                    if ((subtypeId == NOT_A_SUBTYPE_ID)
                            ...
                            /// M: To make sure the switch dialog checked item display correctly;
                            ///    when system locale changed,the origin condition can not make
                            ///    the switch dialog dsplay the current selected ime correctly.@{
                            || item.mIsSystemLanguage) {
                            /// @}
                        checkedItem = i;
                    }
                }
            ...
            /// M: make the title font color adapte for the FeatureOption. @{
    ...
    private static class ImeSubtypeListItem implements Comparable<ImeSubtypeListItem> {
        ...
        /// M: use it in the showInputMethodMenuInternal. @{
    ...
    /// M: Nofity IMS to dump callstack @{
    public void notifyImsDump(boolean IsANRDump) 
    {
        try {
            if(IsANRDump)
                Process.sendSignal(mImsPid, 3);
            else
                Process.sendSignal(mImsPid, 16);
            
            Slog.e(TAG, "Dump IMS pid= "+ mImsPid);
        }
        catch (Exception e) {
            Slog.e(TAG, "Dump IMS exception", e);
        }   
    }
