...
final class DisplayPowerState {
    private static final String TAG = "PowerManagerDisplayState";
    ...
    public static final String IPO_BOOT1 = "android.intent.action.ACTION_BOOT_IPO";
    public static final String IPO_SHUTDWON1 = "android.intent.action.ACTION_SHUTDOWN_IPO";
    public static final String IPO_UNBLANK1 = "android.intent.action.ACTION_UNBLANK_IPO";
    private boolean mShutDownFlag = false;
    private int mDelay = 0;

    public DisplayPowerState(ElectronBeam electronBean,
            DisplayBlanker displayBlanker, LightsService.Light backlight, Context context) {
        ...
        filter.addAction(IPO_SHUTDWON1);
        filter.addAction(IPO_BOOT1);
        filter.addAction(IPO_UNBLANK1);
        mContext.registerReceiver(new BroadcastReceiver() {
            @Override
            public void onReceive(Context context, Intent intent) {
                if (IPO_SHUTDWON1.equals(intent.getAction())) {
                    if (DEBUG) {
                        Slog.d(TAG, "IPO_SHUTDWON1");
                    }
                    mShutDownFlag = true;
                } else if (IPO_BOOT1.equals(intent.getAction())){
                    if (DEBUG) {
                        Slog.d(TAG, "IPO_BOOT1");
                    }
                    mShutDownFlag = false;
                } else if (IPO_UNBLANK1.equals(intent.getAction())){
                    mDelay = intent.getIntExtra("delay", 0);
                    Slog.d(TAG, "mDelay = " + mDelay);
                    if (mDelay == 0) mDelay=150;
                }
            }
    ...
    public void updateElectronBeam() {
        mElectronBeam.updateToCurrentRotation();
    }
    ...
    private final class PhotonicModulator {
        ...
        //public class mTask extends Thread 
            ...
            public void run() {
                ...
                for (;;) {
                    ...
                    synchronized (mLock) {
                        ...
                        if (DEBUG) {
                            Slog.d(TAG, "mTask: on = " + on + ", mActualOn = " + mActualOn
                                    + ", backlight = " + backlight + ", mActualBacklight = " + mActualBacklight);
                        }						
                    ...
                    if (DEBUG) {
                        Slog.d(TAG, "mTask: on = " + on + ", onChanged = " + onChanged
                                + ", backlightChanged = " + backlightChanged + ", backlight = " + backlight);
                    }
                    if (onChanged && on) {
