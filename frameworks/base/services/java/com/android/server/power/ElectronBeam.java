...
final class ElectronBeam {
    ...
    /// MTK: for screen on/off display rotation @{
    public boolean updateToCurrentRotation() {
        if (mDisplayManager == null || mSurface == null) {
            Slog.e(TAG, "Can not update rotation!"); 
            return false;
        }

        Slog.d(TAG, "updateToCurrentRotation");

        DisplayInfo displayInfo = mDisplayManager.getDisplayInfo(Display.DEFAULT_DISPLAY);

        if (displayInfo.rotation == Surface.ROTATION_90
                || displayInfo.rotation == Surface.ROTATION_270) {
            mDisplayWidth = displayInfo.logicalHeight;
            mDisplayHeight = displayInfo.logicalWidth;
        } else {
            mDisplayWidth = displayInfo.logicalWidth;
            mDisplayHeight = displayInfo.logicalHeight;
        }
