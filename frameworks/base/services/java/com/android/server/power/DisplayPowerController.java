...
final class DisplayPowerController {
    private static final String TAG = "PowerManagerDisplayController";
    ...
    private static final int LIGHT_SENSOR_RATE_MILLIS = 100;    /// MTK: Auto backlight
    ...
    private static final long SHORT_TERM_AVERAGE_LIGHT_TIME_CONSTANT = 100; /// MTK: Auto backlight
    private static final long LONG_TERM_AVERAGE_LIGHT_TIME_CONSTANT = 500;  /// MTK: Auto backlight
    ...
    private static final long BRIGHTENING_LIGHT_DEBOUNCE = 300; /// MTK: Auto backlight
    private static final long DARKENING_LIGHT_DEBOUNCE = 500;   /// MTK: Auto backlight
    ...
    /// MTK: add for AAL support @{
    private static final int SCREEN_STATE_OFF = 0;    
    private static final int SCREEN_STATE_ON = 1;
    private static final int SCREEN_STATE_DIM = 2;
    private static native void nativeSetAALMode(int mode);
    private static native void nativeSetLightSensorValue(int value);
    private static native void nativeSetScreenBrightness(int brightness);    
    private static native void nativeSetScreenState(int state, int brightness);
    ///@}

    private long mLastEBoffTime;
    private Context mContext;
    private boolean mShutDownFlag_D = false;
    public static final String P_SHUTDWON = "android.intent.action.ACTION_PRE_SHUTDOWN";
    //public static final String IPO__BOOT = "android.intent.action.ACTION_BOOT_IPO";

    private ElectronBeam mElectronBeam;
    ...
    public DisplayPowerController(Looper looper, Context context, Notifier notifier,
        ...
        filter.addAction(P_SHUTDWON);
        mContext.registerReceiver(new BroadcastReceiver() {
            @Override
            public void onReceive(Context context, Intent intent) {
                if (DEBUG) {
                    Slog.d(TAG, "P_SHUTDWON.");
                }
                mShutDownFlag_D = true;
            }
    ...
    private void initialize() {
        mPowerState = new DisplayPowerState(
                ...
                mLights.getLight(LightsService.LIGHT_ID_BACKLIGHT), mContext);
    ...
    private void updatePowerState() {
        ...
        if (mProximitySensor != null) {
            if (DEBUG) {
                Slog.d(TAG, "mProximity = " + mProximity);
            }
        ...
        if (FeatureOption.MTK_AAL_SUPPORT) {
            ...
            if (DEBUG) {
                Slog.d(TAG, "updatePowerState: "
                        + "useAutoBrightness=" + mPowerRequest.useAutoBrightness                        
                        + ", useEcoBrightness=" + mPowerRequest.useEcoBrightness);
            }
            if (mPowerRequest.useAutoBrightness)
                nativeSetAALMode(1);
            else if (mPowerRequest.useEcoBrightness)
                nativeSetAALMode(2);
            else
                nativeSetAALMode(0);
        }
        ...
        if (!mScreenOffBecauseOfProximity) {
            if (wantScreenOn(mPowerRequest.screenState)) {
                ...
                if (!mElectronBeamOnAnimator.isStarted()) {
                 ...
                 //           mLastEBoffTime = SystemClock.uptimeMillis();
                            Slog.d(TAG, "mElectronBeamOffAnimator.start");
                        } else {
                            mElectronBeamOffAnimator.end();
                        }
                    } else {
                        //Slog.d(TAG, "ElectronBeam 666");
                    }
                }
            }
        }
        ...
        Slog.d(TAG, "updatePowerState: " 
                    + "mustNotify=" + mustNotify + ", mScreenOnWasBlocked=" + mScreenOnWasBlocked
                    + ", mElectronBeamOnAnimator=" + mElectronBeamOnAnimator.isStarted()
                    + ", mElectronBeamOffAnimator=" + mElectronBeamOffAnimator.isStarted()
                    + ", waitUntilClean=" + mPowerState.waitUntilClean(mCleanListener));
    ...
    private void setScreenOn(boolean on) {
        if (!mPowerState.isScreenOn() == on) {
            ...
            if (FeatureOption.MTK_AAL_SUPPORT) {
                if (on)
                    nativeSetScreenState(SCREEN_STATE_ON, clampScreenBrightness(mPowerRequest.screenBrightness));
                else
                    nativeSetScreenState(SCREEN_STATE_OFF, 0);
            }
            ///@}
        }
    }
    ...
    private void animateScreenBrightness(int target, int rate) {
        /// MTK: Auto backlight @{
        if (FeatureOption.MTK_AAL_SUPPORT) {
            nativeSetScreenBrightness(target);
        }
        ///@}
    ...
    private void handleProximitySensorEvent(long time, boolean positive) {
        ...
        if (DEBUG) {
            Slog.d(TAG, "mPendingProximity=" + mPendingProximity);
        }
    ...
    private void updateAmbientLux(long time) {
        ...
        if (mRecentShortTermAverageLux > brighteningLuxThreshold
            ...
            if (time >= debounceTime) {
                ...
                updateAutoBrightness(true);
                // MTK: Update the filters in case the light level does not change
        ...
        if (mRecentShortTermAverageLux < darkeningLuxThreshold
            ...
            if (time >= debounceTime) {
                ...
                updateAutoBrightness(true);
                // MTK: Update the filters in case the light level does not change
                if (mLastObservedLux < darkeningLuxThreshold) {
                    mHandler.sendEmptyMessageAtTime(MSG_LIGHT_SENSOR_DEBOUNCED,
                        time + SYNTHETIC_LIGHT_SENSOR_RATE_MILLIS);
                }            
            } else {
    ...
    private void updateAutoBrightness(boolean sendUpdate) {
        ...
        /// MTK: Auto backlight @{
        if (FeatureOption.MTK_AAL_SUPPORT) {
            nativeSetLightSensorValue((int)mAmbientLux);
        }
        ///@}
