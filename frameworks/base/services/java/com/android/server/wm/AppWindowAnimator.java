...
public class AppWindowAnimator {
     ...
     * A function to check the animation status of appWindowToken and child windows
     */
    boolean isAnimating() {
        if (WindowManagerService.localLOGV) Slog.v(TAG, "Check the animation for the app window token ");

        if (animation != null) {
            return true;
        }

        if (WindowManagerService.localLOGV) Slog.v(TAG, "Check the animation for the child windows ");

        int winSize = mAppToken.allAppWindows.size();
        for (int i = 0; i < winSize; i++) {
            WindowState win = mAppToken.allAppWindows.get(i);
            if (WindowManagerService.localLOGV) Slog.v(TAG, "Child # " + i + ": "
                + win + ", animating = " + win.mWinAnimator.mAnimating);
    ...
    public void setDummyAnimation() {
        ...
        /// M: [ALPS00419751] There will be black once when quit a module in settings
        /// M: [ALPS00427459] Press 'Add account',the screen will be flicker @{
        if (animation == sDummyAnimation) {
            return;
        }

        boolean existAnimation = isAnimating();
        ...
        if (existAnimation) {
            transformation.setAlpha(mAppToken.hiddenRequested ? 1 : 0);
        } else {
            transformation.setAlpha(mAppToken.reportedVisible ? 1 : 0);
        }
        /// @}
    ...
    boolean stepAnimationLocked(long currentTime, int dw, int dh) {
        ...
        Trace.traceBegin(Trace.TRACE_TAG_WINDOW_MANAGER, "animation done : " + mAppToken.toString());
        if (WindowManagerService.DEBUG_ANIM) Slog.v(
