...
class WindowStateAnimator {
    ...
    float mSurfaceAlpha;
    /// M:[ALPS00119099]
    boolean mSurfacePositionChanged;
    ...
    public void setAnimation(Animation anim) {
        if (WindowManagerService.localLOGV) Slog.v(TAG, "Setting animation in " + this + ": " + anim);
        mAnimating = false;
    ...
    boolean stepAnimationLocked(long currentTime) {
        ...
        if (WindowManagerService.DEBUG_LAYERS) Slog.v(TAG, "Stepping win " + this
        ...
        if (mDrawState == HAS_DRAWN
            ...
            if (WindowManagerService.DEBUG_STARTING_WINDOW) Slog.v(TAG, "Finish starting "
    ...
    void finishExit() {
        ...
        if (mSurface != null) {
            ...
            if (WindowManagerService.SHOW_TRANSACTIONS) WindowManagerService.logSurface(
    ...
    boolean finishDrawingLocked() {
        ...
        if (mDrawState == DRAW_PENDING) {
            if (WindowManagerService.DEBUG_SURFACE_TRACE || WindowManagerService.DEBUG_ANIM || WindowManagerService.SHOW_TRANSACTIONS || WindowManagerService.DEBUG_ORIENTATION)
    ...
    boolean commitFinishDrawingLocked(long currentTime) {
        ...
        if (WindowManagerService.DEBUG_SURFACE_TRACE || WindowManagerService.DEBUG_ANIM) {
    ...
    Surface createSurfaceLocked() {
        if (mSurface == null) {
            ...
            try {
                ...
                if (WindowManagerService.SHOW_TRANSACTIONS || WindowManagerService.SHOW_SURFACE_ALLOC) Slog.i(TAG,
                        "  CREATE SURFACE "
                        + mSurface + " IN SESSION "
                        + mSession.mSurfaceSession
            ...
            try {
                try {
                    ...
                    /// M: If someone sets the position again,
                    ///    it need to change mSurfacePositionChanged as true.
                    ///    When the change has been notified to ViewRoot, it can be clear as false.
                    mSurfacePositionChanged = false;
    ...
    void computeShownFrameLocked() {
        ...
        if (selfTransformation || attachedTransformation != null
            ...
            if ((WindowManagerService.DEBUG_SURFACE_TRACE || WindowManagerService.localLOGV)
    ...
    void updateSurfaceWindowCrop(final boolean recoveringMemory) {
        ...
        if ((w.mAttrs.flags & LayoutParams.FLAG_SCALED) != 0) {
            ...
            w.mSystemDecorRect.set(0, 0, w.mCompatFrame.width(),
                    w.mCompatFrame.height());
        ///M:[ALPS00462243] @{
        } else if(mIsWallpaper && !mWin.mWallpaperVisible){
            //If wallpaper isn't visible, it will not be cropped.
            w.mSystemDecorRect.set(w.mLastSystemDecorRect);
        /// @}
        } else {
        ...
        if (mSurfaceX != left || mSurfaceY != top) {
            try {
                ...
                if (!recoveringMemory) {
                    ...
                    /// M: [Klockwork]avoid possible NPE @{
                    if (mSurface == null)
                        return;
                    /// @}
                }
            ...
            /// M:[ALPS00119099]
            if (!mSurfacePositionChanged) {
                mSurfacePositionChanged = true;
            }
        }
    ...
    public void prepareSurfaceLocked(final boolean recoveringMemory) {
        ...
        if (mIsWallpaper && !mWin.mWallpaperVisible) {
            ...
            if (WindowManagerService.DEBUG_ANIM && isAnimating()) {
        ...
        mAnimator.mDisplayed |= displayed;
    ...
    void setWallpaperOffset(int left, int top) {
        /// M: ALPS00434925. When animating, it should not update the top
        /// position at mSurfaceX & mSurfaceY (Default Android Code) @{
//        mSurfaceX = left;
//        mSurfaceY = top;
        /// @}
        ...
        /// M: ALPS00434925. Without animating, it can update the top
        /// position at mSurfaceX & mSurfaceY @{
        mSurfaceX = left;
        mSurfaceY = top;
        /// @}
    ...
    boolean performShowLocked() {
        ...
        if (WindowManagerService.DEBUG_VISIBILITY || (WindowManagerService.DEBUG_STARTING_WINDOW &&
                mWin.mAttrs.type == WindowManager.LayoutParams.TYPE_APPLICATION_STARTING)) {
            RuntimeException e = null;
            if (!WindowManagerService.HIDE_STACK_CRAWLS) {
                e = new RuntimeException();
        ...
        if (mDrawState == READY_TO_SHOW && mWin.isReadyForDisplayIgnoringKeyguard()) {
            if (WindowManagerService.SHOW_TRANSACTIONS || WindowManagerService.DEBUG_ORIENTATION)
