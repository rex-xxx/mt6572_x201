...
public class LocationManagerService extends ILocationManager.Stub implements Runnable {
    ...
    private LocationProviderProxy mFusedLocationProvider;
    ...
    //MTK add timer to update location provider status
    private StatusUpdateTimer mProviderStatusCheckTimer = new StatusUpdateTimer();;
    //MTK end
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                if (ACTION_SHUTDOWN_IPO.equals(action)) {
                    ...
                    if(realProvider != null) {
                       ...
                       Log.d(TAG, "IPO boot up for location");
    ...
    private void loadProvidersLocked() {
        ...
        */
        ...
        if (networkProvider != null) {
            ...
            //Even though, bind is failed, we need also to add the Network Location Provider
            networkProvider = LocationProviderProxy.create(
                mContext,
                LocationManager.NETWORK_PROVIDER,
                NETWORK_LOCATION_SERVICE_ACTION,
                providerPackageNames, mLocationHandler, mCurrentUserId);
    ...
    private LocationProviderProxy loadFusedLocationProvider() {
        ...
        if (fusedLocationProvider != null) {
            ...
            mEnabledProviders.add(fusedLocationProvider.getName());
            mRealProviders.put(LocationManager.FUSED_PROVIDER, fusedLocationProvider);
            Log.d(TAG, "FusedLocationProvider is loaded");
        }
        return fusedLocationProvider;
    }
    
    private void unloadFusedLocationProvider() {
        
        if (mFusedLocationProvider != null) {
            Log.d(TAG, "Unload FUSED_PROVIDER");
            LocationProviderProxy.close(mFusedLocationProvider);
    ...
    // MTK add status update timer
    private final class StatusUpdateTimer extends CountDownTimer {
        // Interval is 30s
        private static final int LOCATION_PROVIDER_STATUS_CHECK_INTERVAL = 30000;

        // indication is timer started or not
        private boolean mTimerStarted = false;

        // mapping from provider name to all its status
        private final HashMap<String, Integer> mStatusByProvider =
                new HashMap<String, Integer>();

        public StatusUpdateTimer() {
            super(LOCATION_PROVIDER_STATUS_CHECK_INTERVAL, LOCATION_PROVIDER_STATUS_CHECK_INTERVAL);
            resetProviderStatus();
        }
        ...
        private void onFinishLocked() {

            Log.d(TAG, "status timer finished");
            HashSet<String> removedProvides = new HashSet<String>();
            for (String provider: mStatusByProvider.keySet()) {

                Integer s = mStatusByProvider.get(provider);
                if (s.intValue() == LocationProvider.AVAILABLE) {

                    LocationProviderInterface p = mProvidersByName.get(provider);
                    if (p == null ) {
                        removedProvides.add(provider);
                        Log.d(TAG, "removed provider: " + provider);
                        continue;
                    }
                    Bundle extras = new Bundle();
                    int status = p.getStatus(extras);
                    StatusUpdateTimer.this.updateProviderStatus(provider, status);
                    if (status != LocationProvider.AVAILABLE) {
                        for (Receiver aReceiver: mReceivers.values()) {
                            if (aReceiver.mUpdateRecords.containsKey(provider)) {
                                Log.d(TAG, "update status of provider " + s + " " + status + " " + mStatusByProvider.get(provider));
                                aReceiver.callStatusChangedLocked(provider, status, extras);
                            }
                        }
                    }
                }
            ...
            for (String provider: removedProvides) {
                mStatusByProvider.remove(provider);
            }

            if (!shouldStop()) {
                StatusUpdateTimer.this.start();
            } else {
                stopTimer();
            }
        }
        ...
        public void startTimer() {
            ...
            if (!mTimerStarted) {
                StatusUpdateTimer.this.start();
                mTimerStarted = true;
                Log.d(TAG, "start update status timer");
            }
        }
        
        // cancel the status update timer when no location receivers
        // and reset the all provider's status as temporarily unavailable
        private void stopTimer() {

            if (mTimerStarted) {

                StatusUpdateTimer.this.cancel();
                mTimerStarted = false;
                Log.d(TAG, "cancel status update timer");
                resetProviderStatus();
            }
        }

        private void resetProviderStatus() {
            mStatusByProvider.clear();
            for (String provider: mProvidersByName.keySet()) {
                mStatusByProvider.put(provider, new Integer(LocationProvider.TEMPORARILY_UNAVAILABLE));
            }
        }

        public void updateProviderStatus(String provider, int status) {
            if (mTimerStarted) {
                mStatusByProvider.put(provider, new Integer(status));
            }
        }
        ...
        private boolean shouldStop() {
            ...
            for (Receiver aReceiver: mReceivers.values()) {
                for (String provider: aReceiver.mUpdateRecords.keySet()) {
                    t.put(provider, new Integer(1));
                }
            }

            if ((t.isEmpty()) || ((t.size() == 1) && t.containsKey(LocationManager.PASSIVE_PROVIDER))) {
                return true;
            } else {
                return false;
            }
    ...
    private void applyRequirementsLocked(String provider) {
        ...
        //MTK add
        mProviderStatusCheckTimer.startTimer();
        //MTK end
    }
    ...
    private Receiver getReceiver(PendingIntent intent, int pid, int uid, String packageName) {
        Receiver receiver = null;
        synchronized (mLock) {
            receiver = mReceivers.get(intent);
            if (receiver == null) {
    ...
    public void requestLocationUpdates(LocationRequest request, ILocationListener listener,
        ...
        if (request.getProvider().equals(LocationManager.FUSED_PROVIDER) && mFusedLocationProvider == null) {
            mFusedLocationProvider = loadFusedLocationProvider();
        }
        try {
            synchronized (mLock) {
    ...
    private void requestLocationUpdatesLocked(LocationRequest request, Receiver receiver,
        ...
        Log.d(TAG, "request provider: "+ name + packageName);
        if (name == null) {
    ...
    private void removeUpdatesLocked(Receiver receiver) {
        ...
        ArrayList<UpdateRecord> records = mRecordsByProvider.get(LocationManager.FUSED_PROVIDER);
        if (records == null || records.size() == 0) {
            unloadFusedLocationProvider();
        }
    }
    ...
    private void handleLocationChangedLocked(Location location, boolean passive) {
        ...
        LocationProviderInterface p = mProvidersByName.get(provider);
        if (p == null) return;
        // MTK add start CR[ALPS00247273] [CTS4.0.3_r1]
        // testReplaceRealProvidersWithMocks - fail
        if (p instanceof MockProvider) {
            if (location.getExtras() != null && location.getExtras().getInt("Mock", 0) != 1) {
                Log.d(TAG, "location blocked " + location.toString());
                return;
            }
        }
        // MTK add end
       ...
       // Get latest status
        ...
        // MTK add update provider status of status check Timer
        mProviderStatusCheckTimer.updateProviderStatus(provider, status);
        // MTK end        
    ...
    // MTK add start For EPO
    public int getFileTime(long[] times) {
        if (times == null) {
            Slog.v(TAG, "getFileTime failed: paramer is null...");
            return -1;
        }

        LocationProviderInterface p = mProvidersByName.get("gps");
        if (p == null) {
            Slog.v(TAG, "LocationProvider is null...");
            return -1;
        }
        return p.getFileTime(times);
    }
