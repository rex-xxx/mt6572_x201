...
final class ActivityStack {
    ...
    static boolean localLOGV = ActivityManagerService.localLOGV;
    static boolean DEBUG_SWITCH = ActivityManagerService.DEBUG_SWITCH;
    static boolean DEBUG_PAUSE = ActivityManagerService.DEBUG_PAUSE;
    static boolean DEBUG_VISBILITY = ActivityManagerService.DEBUG_VISBILITY;
    static boolean DEBUG_USER_LEAVING = ActivityManagerService.DEBUG_USER_LEAVING;
    static boolean DEBUG_TRANSITION = ActivityManagerService.DEBUG_TRANSITION;
    static boolean DEBUG_RESULTS = ActivityManagerService.DEBUG_RESULTS;
    static boolean DEBUG_CONFIGURATION = ActivityManagerService.DEBUG_CONFIGURATION;
    static boolean DEBUG_TASKS = ActivityManagerService.DEBUG_TASKS;
    static boolean DEBUG_THERMAL = ActivityManagerService.DEBUG_THERMAL;

    static boolean DEBUG_CLEANUP = ActivityManagerService.DEBUG_CLEANUP;
    static boolean DEBUG_PROCESSES = ActivityManagerService.DEBUG_PROCESSES;
    ...
    static final int LAUNCH_TICK = 2000;
    /// @} 2012-04-15
    ...
    private boolean mIsPerfBoostEnable = false; /// M: for PerfBoost feature
    ...
    static final int HALT_RESUMING_TIMEOUT_MSG = ActivityManagerService.FIRST_ACTIVITY_STACK_MSG + 10;
    static final int RESTORE_RESUMING_MSG = ActivityManagerService.FIRST_ACTIVITY_STACK_MSG + 11;
    /// @}
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case HALT_RESUMING_TIMEOUT_MSG: {
                    ...
                    synchronized (mService) {
                        resumeTopActivityLocked(mAnimationPrev, mAnimationOptions);
                    }
...
//                    Slog.e(TAG,"No animation. Resume the top activity");
                    synchronized (mService) {
                        resumeTopActivityLocked(mAnimationPrev,mAnimationOptions);
                    }
                } break;
    ...
    final boolean realStartActivityLocked(ActivityRecord r,
        ...
        try {
            ...
            /// M: LTK @{
            if (FeatureOption.MTK_MEMORY_COMPRESSION_SUPPORT && !app.hasShownUi) {
                try {
                    IAmsPlus ap = MediatekClassFactory.createInstance(IAmsPlus.class);
                    ap.beforeScheduleLaunchActivity(convertLaunchRecord(r));
                } catch (Exception e) {
                    Log.e(TAG, "[LTK] Exception thrown during beforeScheduleLaunchActivity failed:", e);
                }
            }
            /// @}
        ...
        if (andResume) {
            ...
            r.stopped = false;
            r.waitingVisible = false; // M: It's to fix ALPS00141627
            mResumedActivity = r;
    ...
    /// M: LTK @{
    final IAmsPlusProcessRecord convertProcessRecord(final ProcessRecord pr) {
        if (pr == null)
            return null;
        return new IAmsPlusProcessRecord() {
            ...
            public int setKilledLTK(boolean kill) {
                pr.killedLTK = kill;
                return 0;
            }
            public boolean isLowMemory() {
                Trace.traceBegin(Trace.TRACE_TAG_ACTIVITY_MANAGER, "isLowMemory");
                ActivityManager.MemoryInfo outInfo = new ActivityManager.MemoryInfo();
                mService.getMemoryInfo(outInfo);
                Trace.traceEnd(Trace.TRACE_TAG_ACTIVITY_MANAGER);
                return outInfo.lowMemory;
            }
            public long getPauseAppMemUsage() {
                return pr.pauseAppMemUsage;
            }
            public long getPause3DUsage() {
                return pr.pause3DUsage;
            }
        };
    }

    final IAmsPlusLaunchRecord convertLaunchRecord(final ActivityRecord actRec) {
        return new IAmsPlusLaunchRecord() {
            public Intent getIntent() {
                return actRec.intent;
            }

            public boolean isLaunchingHomeActivity() {
                return actRec.isHomeActivity;
            }

            public IAmsPlusProcessRecord getLaunchedProcess() {
                return convertProcessRecord(actRec.app);
            }

            public IAmsPlusProcessRecord getWaitProcess() {
                if(actRec.resultTo == null)
                    return null;
                return convertProcessRecord(actRec.resultTo.app);
            }

            public ArrayList<IAmsPlusProcessRecord> getRecords() {
                ArrayList<IAmsPlusProcessRecord> list = new ArrayList<IAmsPlusProcessRecord>();
                synchronized (mService.mPidsSelfLocked) {
                    final int size = mService.mPidsSelfLocked.size();
                    Log.i(TAG, "[LTK] mService.mPidsSelfLocked size=" + size);
                    for (int i = 0; i < size; i++) {
                        final ProcessRecord proc = mService.mPidsSelfLocked.valueAt(i);
                        if (proc == null || proc.killedBackground) continue;
                        IAmsPlusProcessRecord newRec = convertProcessRecord(proc);
                        if (newRec == null) continue;
                        if (!proc.hasShownUi) continue; // skip non-activity processes
                        list.add(newRec);
                    }
                }
                return list;
    ...
    private int readFileInt(String path) {
        ...
        try {
            ...
            Slog.w(TAG, "Unable to read " + path);
            return 0;
        } finally {
            if (br != null) {
                try {
    ...
    private final void startSpecificActivityLocked(ActivityRecord r,
        ...
        /// M: ALPS00105328 Fix black screen when starting an activty with DeadObjectException after called realStartActivityLocked() @{
    ...
    private final void startPausingLocked(boolean userLeaving, boolean uiSleeping) {
        ...
        if (FeatureOption.MTK_MEMORY_COMPRESSION_SUPPORT) {
            if (prev.app != null) {
                prev.app.pauseAppMemUsage = Process.getRswapRssSum(prev.app.pid);
                prev.app.pause3DUsage = readFileInt("/proc/mali/memory_usage");
                Log.w(TAG, "[LTK] pid, app, 3d=" + prev.app.pid + ", " + prev.app.pauseAppMemUsage + ", " + prev.app.pause3DUsage);
            }
        }
        /// @} 2012-04-25

        /// M: Add for LCA launch time enhancement @{
        Trace.traceBegin(Trace.TRACE_TAG_ACTIVITY_MANAGER, "amScreenCapture");
        if (FeatureOption.MTK_LAUNCH_TIME_OPTIMIZE) {
        ...
        synchronized (mService) {
            ...
            if (index >= 0) {
                ...
                if (mPausingActivity == r) {
                    ...
                    if (FeatureOption.MTK_MEMORY_COMPRESSION_SUPPORT) {
                        try {
                            final ActivityRecord topRecord = topRunningActivityLocked(null);
                            if (topRecord != null && r.app != null) {
                                final boolean isTopApp = r.app == topRecord.app;
                                if (DEBUG_STATES) Slog.v(TAG, "LTK top isHome:" + topRecord.isHomeActivity);
                                if (topRecord.isHomeActivity && !isTopApp) {
                                    IAmsPlus ap = MediatekClassFactory.createInstance(IAmsPlus.class);
                                    ActivityRecord topAct = topRunningActivityLocked(mPausingActivity);
                                    ap.onActivityPauseTimeout(convertProcessRecord(r.app), convertLaunchRecord(topAct), timeout);
                                }
                            }
                        } catch (Exception e) {
                                Log.e(TAG, "[LTK] Exception thrown during onActivityPauseTimeout failed:", e);
                        }
                    }
                    /// @}
    ...
    final void ensureActivitiesVisibleLocked(ActivityRecord top,
        ...
        /// M: ALPS00431397. When halting an activity and activity is not in resumed state, it doesn't do any thing.
        synchronized (mHaltActivityLock) {
            if (mHaltActivity && top.state != ActivityState.RESUMED  && checkIfBlockResumeTop(top)) {
                Slog.d(TAG, "ACT-Because running the window animation, can't ensureActivitiesVisibleLocked");
                return;
            }
        }
    ...
    final boolean resumeTopActivityLocked(ActivityRecord prev, Bundle options) {
        ...
        synchronized (mHaltActivityLock) {
            if (mHaltActivity && checkIfBlockResumeTop(next)) {
        ...
        if ((mService.mSleeping || mService.mShuttingDown)
            ...
            mNoAnimActivities.clear();
            ActivityOptions.abort(options);
            Slog.w(TAG, "skip resumeTopActivityLocked, next = " + next);
            return false;
        }
        ...
        if (!mIsPerfBoostEnable) {
            ...
            PerfService.nativePerfBoostEnable(PerfService.SCN_APP_SWITCH);
                ...
                if (taskTop != null) {
                    ...
                    if ((launchFlags &
                        ...
                        if (((launchFlags&Intent.FLAG_ACTIVITY_SINGLE_TOP) != 0
                            ...
                            /// M: 2012-04-04 Log for debugging
                            if(!ActivityManagerService.IS_USER_BUILD)
                               Slog.d(TAG, "ACT-AM_NEW_INTENT " + r + taskTop.task);
                            /// @} 2012-04-04 Log for debugging
    ...
    final ActivityRecord activityIdleInternal(IBinder token, boolean fromTimeout,
        ...
        if (activityRemoved) {
            /// M: Fix google issue, add lock protection for resumeTopActivityLocked
            synchronized (mService) {
                resumeTopActivityLocked(null);
            }
        }
    ...
    final boolean requestFinishActivityLocked(IBinder token, int resultCode,
        ...
        if (reason.equals("app-request") &&
            ...
            if (index > 0) {
                ActivityRecord prevAct = mHistory.get(index-1);
                if (prevAct.app == r.app) {
                    ...
                    int topIndex = mHistory.size()-1;
                    ActivityRecord topAct = mHistory.get(topIndex);
    ...
    boolean removeHistoryRecordsForAppLocked(ProcessRecord app) {
        ...
        while (i > 0) {
            ...
            if (r.app == app) {
                /// M: LTK: add LTK kill condition @{
                boolean removeIfNoState = (!r.haveState && !r.stateNotNeeded) || r.finishing;
                if (FeatureOption.MTK_MEMORY_COMPRESSION_SUPPORT) {
                    removeIfNoState = removeIfNoState && !app.killedLTK;
                }
                /// @}
                if (removeIfNoState) {
            ...
            /** M:
                     *  remove the record from Activity Stack, ALPS00063234
                     *  since its hosting process already died @{
                     */
            else if ( null == r.app && r.finishing && !r.stateNotNeeded ) {
                Slog.v(TAG, "Removing this entry?!");

                boolean found = false;
                List<ActivityManager.RunningAppProcessInfo> runningList = mService.getRunningAppProcesses();
                for (ActivityManager.RunningAppProcessInfo p : runningList) {
                    if (p.processName.equals(r.processName)) {
                        found = true;
                        break;
                    }
                }
                if (!found) {
                    // make sure the hosting process already died, therefore,
                    // it's very safe to remvoe this record.
                    Slog.v(TAG, "Removing this entry!  haveState=" + r.haveState
                            + " stateNotNeeded=" + r.stateNotNeeded);
    ...
    boolean mHaltActivity = false;
    Object mHaltActivityLock = new Object();
    ActivityRecord mAnimationPrev;
    Bundle mAnimationOptions;
    /// M : The max duration to halt acvitiy is 3 seconds
    static final int MAX_TIMEOUT_HALT_ACTIVITY_RESUMING = 3000;
    ...
    */
    ...
    private boolean checkIfBlockResumeTop(ActivityRecord r) {
        if (r == null) {
            return false;
        }
