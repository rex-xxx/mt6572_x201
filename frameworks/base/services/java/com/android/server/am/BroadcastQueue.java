...
public class BroadcastQueue {
    ...
    private final void deliverToRegisteredReceiverLocked(BroadcastRecord r,
        ...
        if (!skip) {
            ...
            try {
                ...
                /// M: Broadcast enhanced logs for Framework Analyzer. @{
                if (!ActivityManagerService.IS_USER_BUILD && ordered) {
                    Slog.d(TAG, "BDC-Delivered broadcast: " + r.intent
                        + ", ordered=true"
                        + ", filter=" + filter
                        + ", receiver=" + r.receiver);
        ...
        if (! mPendingBroadcastTimeoutMessage) {
            ...
            if (!ActivityManagerService.IS_USER_BUILD) {
                Message msg2 = mService.mAnrHandler.obtainMessage(
                    ANRManager.START_MONITOR_BROADCAST_TIMEOUT_MSG, this);
    ...
    final void cancelBroadcastTimeoutLocked() {
        if (mPendingBroadcastTimeoutMessage) {
            ...
            /// dump the ANR process backtraces before ANR happens
            if (!ActivityManagerService.IS_USER_BUILD) {
                mService.mAnrHandler.removeMessages(ANRManager.START_MONITOR_BROADCAST_TIMEOUT_MSG, this);
            }
            /// @} 2012-07-17
        }
    }
    ...
    final void broadcastTimeoutLocked(boolean fromMsg) {
        ...
        if (fromMsg) {
            ...
            if (mService.isDidDexOpt(SystemClock.uptimeMillis() - mTimeoutPeriod)) {
