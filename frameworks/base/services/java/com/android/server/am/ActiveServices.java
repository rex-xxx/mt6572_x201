...
public class ActiveServices {
    ...
    private final void bumpServiceExecutingLocked(ServiceRecord r, String why) {
        ...
        if (r.executeNesting == 0 && r.app != null) {
            if (r.app.executingServices.size() == 0) {
                /// M: add debug service log @{
                if (DEBUG_SERVICE || DEBUG_SERVICE_EXECUTING) {
                    Log.i(TAG, "There is executingService!");
                }
                /// @}
                ...
                /// M: ANR debug enhancement:
                // dump the ANR process backtraces before ANR happens
    ...
    final void performServiceRestartLocked(ServiceRecord r) {
        ...
        if (FeatureOption.MTK_LCA_RAM_OPTIMIZE) {
            if (r.killedbyLMK) {
                if (!checkServiceRestartMemoryLocked(r)) {
                    return;
                }
            }
        }
    ...
    private void serviceDoneExecutingLocked(ServiceRecord r, boolean inStopping) {
        ...
        if (r.executeNesting <= 0 && r.app != null) {
            ...
            if (r.app.executingServices.size() == 0) {
                ...
                /// M: ANR debug enhancement:
                // remvoe the message since the service is already started
                if (!ActivityManagerService.IS_USER_BUILD) {
                    mAm.mAnrHandler.removeMessages(ANRManager.START_MONITOR_SERVICE_TIMEOUT_MSG, r.app);
                }
                /// @}
            }
            if (inStopping) {
    ...
    /**
     * M: LCA, accurate service restart
     * checkServiceRestartMemoryLocked
     */
    private boolean checkServiceRestartMemoryLocked(ServiceRecord sr) {
        boolean isRestart = false;

        // first level check: if there is launcher or background process
        for (int i = mAm.mLruProcesses.size() - 1 ; i >= 0 ; i--) {
            ProcessRecord r = mAm.mLruProcesses.get(i);
            if (r.thread != null && (r.curAdj == 6 || r.curAdj >=9)) {
                isRestart = true;
            }
        }
        ...
        if (!isRestart) {
            int slotCount = 0;
            for (int i = 0; i < mAm.freeMemProfile.length; i++) {
                if (mAm.freeMemProfile[i] != 0) {
                    avgFreeMem += mAm.freeMemProfile[i];
                    slotCount++;
                }
            }
            if (slotCount != 0) {
                avgFreeMem = avgFreeMem / slotCount;
            }

            if (avgFreeMem > mAm.mProcessList.getMemLevel(ProcessList.HIDDEN_APP_MAX_ADJ) ||
                avgFreeMem > mAm.mProcessList.getMemLevel(ProcessList.HOME_APP_ADJ) + sr.memUsage) {
                isRestart = true;
            }
            else {
                long delayTime = 0;
                sr.noMemCount++;
                if (sr.noMemCount < 6) {
                    delayTime = 10 * 1000 * sr.noMemCount;
                }
                else {
                    delayTime = 60 * 1000;
                }

                mAm.mHandler.postDelayed(sr.restarter, delayTime);
                Slog.e(TAG, "SVC-DelayRestart: Mem not enough to restart sr=" + sr.shortName + ", wait " + delayTime + "ms, free mem="
                         + avgFreeMem + " mem usage=" + sr.memUsage + " MIN ADJ level=" + mAm.mProcessList.getMemLevel(ProcessList.HIDDEN_APP_MIN_ADJ));
            }
        }

        if (isRestart) {
            if (avgFreeMem != 0) {
                Slog.e(TAG, "SVC-AccurateRestart: sr=" + sr.shortName + ", free mem="
                         + avgFreeMem + " mem usage=" + sr.memUsage + " MIN ADJ level=" + mAm.mProcessList.getMemLevel(ProcessList.HIDDEN_APP_MIN_ADJ));
            }
            else {
                Slog.e(TAG, "SVC-AccurateRestart: sr=" + sr.shortName);
            }
            sr.killedbyLMK = false;
            sr.noMemCount = 0;
        }
