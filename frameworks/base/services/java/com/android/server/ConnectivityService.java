        ...
        if (FeatureOption.MTK_GEMINI_SUPPORT) {
            ...
            if (enabled && (curSlotId == SimInfo.SLOT_NONE)) {
                try{
                    ...
                    for (int simId=PhoneConstants.GEMINI_SIM_1; simId<PhoneConstants.GEMINI_SIM_NUM; simId++) {
                        if (mITelephony.getSimState(simId) == TelephonyManager.SIM_STATE_READY) {
                            curSlotId = simId;
                            break;
                        }
                    }
				
                    if (curSlotId == SimInfo.SLOT_NONE) {
...
//                if (currentPriority >= mNetConfigs[checkType].mPriority) continue;
                ...
                if (!checkInfo.isConnectedOrConnecting() || checkTracker.isTeardownRequested()) {
                    ...
                    if (FeatureOption.MTK_GEMINI_SUPPORT
                        ...
                        if (slot >= PhoneConstants.GEMINI_SIM_1 && slot < PhoneConstants.GEMINI_SIM_NUM) {
   ...
   /**
    ...
    private void tryRestoreDefault() {
        ...
        if (!isDefaultConnected() && !isWifiConnected() && mFeatureUsers.isEmpty()) {
...
//                }
    ...
    public boolean getMobileDataEnabledGemini(int slotId) {
        ...
        return (dataEnabledSlotId == slotId);
    }
   ...
   private void handleMobileDataConnectionChange(int preSlotId, int slotId) {
                ...
                if(iTelephony!=null){
                    ...
                    if (subscriberId != null && mPolicyManager.isDataConnOverLimit(subscriberId)) {
                        ...
                        if (preSlotId != SimInfo.SLOT_NONE && 
                                preSimId == Settings.System.GPRS_CONNECTION_SIM_SETTING_NEVER) {
                            Xlog.d(MTK_TAG, "handleMobileDataConnectionChange previous SIM does not exist then set preSlotId to SLOT_NONE");
                            preSlotId = SimInfo.SLOT_NONE;
                        }
    ...
    private void sendSuspendedBroadcast(NetworkInfo info) {
     ...
     public boolean setMobileDataEnabledGemini(int slotId) {
         enforceChangePermission();
         log("setMobileDataEnabledGemini slotId" + slotId);
    ...
    public boolean setUsbInternetWithType(boolean enable, int system_type) {
        if (VDBG) log("setUsbInternet enable:" + enable +" system_type:"+system_type);
        NetworkStateTracker network = mNetTrackers[ConnectivityManager.TYPE_USB];
        mTethering.setUsbInternetEnable(enable, system_type);
        if (network != null) {
            if (enable) {
                NetworkInfo ni = network.getNetworkInfo();
    ...
    public boolean setUsbInternet(boolean enable) {
        ...
        return setUsbInternetWithType(enable, ConnectivityManager.USB_INTERNET_SYSTEM_DEFAULT);
    }
    /** @} */
    ...
    private boolean isDefaultConnected() {
        ...
        try {
            if (iTelephony != null) {
                ...
                if (FeatureOption.MTK_GEMINI_SUPPORT) {
                    state = iTelephony.getDataStateGemini(info.getSimId());
                } else {
                    state = iTelephony.getDataState();
                }
                isConnected = (state == TelephonyManager.DATA_CONNECTED || state == TelephonyManager.DATA_CONNECTING);
                if (DBG) Slog.d(TAG, "isDefaultConnected(): " + isConnected);
                return isConnected;
            }
        } catch (RemoteException e) {
            e.printStackTrace();
    ...
    private void setMtuSize(NetworkStateTracker nt){
        ...
        int networkRat = TelephonyManager.NETWORK_TYPE_UNKNOWN;
        int mtuSize = 0;
        int networkInfoType;
        ...
        networkInfoType = mNetworkInfo.getType();

        if(networkInfoType != ConnectivityManager.TYPE_MOBILE_MMS && networkInfoType != ConnectivityManager.TYPE_MOBILE){
           log("MTU is only applied to mobile/MMS");
           return;
        }

        String interfaceName = mLinkProperties.getInterfaceName();
        networkRat = mTelephonyManager.getNetworkType();
        log("configure mtu size of interface name " + interfaceName + " with network type " + networkRat);
        ...
        if (networkInfoType == ConnectivityManager.TYPE_MOBILE_MMS){
            if(networkRat < TelephonyManager.NETWORK_TYPE_UMTS){
        ...
        log("configure mtu size for " + mNetworkInfo.getType() + " with mtu size:" + mtuSize + ":" + mMtuSize);

        if(networkInfoType == ConnectivityManager.TYPE_MOBILE){
           String[] tetherInfacs = mTethering.getTetheredIfacePairs();
           if(tetherInfacs.length != 0){
                log("Tethreing is enabled");
                return;
           }           
           mMtuSize = mtuSize;
        }
