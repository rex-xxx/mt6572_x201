...
class TouchExplorer implements EventStreamTransformation {
    ...
    public void onMotionEvent(MotionEvent event, MotionEvent rawEvent, int policyFlags) {
        ...
        /// M: Action down event will be produced when event buffer overflowed
        /// during scrolling in the phone. According to driver's analysis, we need
        /// to work around here to avoid exception happen. @{
        if (event.getActionMasked() == MotionEvent.ACTION_DOWN &&
                (mCurrentState == STATE_DRAGGING || mCurrentState == STATE_DELEGATING)) {
            Slog.w(LOG_TAG, "Skip this event to avoid exception");
            return;
        }
        /// @}
