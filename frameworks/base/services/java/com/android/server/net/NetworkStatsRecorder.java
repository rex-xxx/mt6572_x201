...
public class NetworkStatsRecorder {
  ...
  /**
    ...
    /**
     * M: add argument : active simId  for GEMINI
     * Record any delta that occurred since last {@link NetworkStats} snapshot,
     * using the given {@link Map} to identify network interfaces. First
     * snapshot is considered bootstrap, and is not counted as delta.
     * @hide
     */
/*    public void recordSnapshotLocked(NetworkStats snapshot,
            Map<String, NetworkIdentitySet> ifaceIdent, long currentTimeMillis, long mDataActivedSimId) {
        final HashSet<String> unknownIfaces = Sets.newHashSet();
        ...
        final long end = currentTimeMillis;
        final long start = end - delta.getElapsedRealtime();        
//        final String IFACE_GEMINI = "duiface";

        NetworkStats.Entry entry = null;
        for (int i = 0; i < delta.size(); i++) {
            entry = delta.getValues(i, entry);

            ///M: modefy for GEMINI  & EVDO G+C @{
            String ifaceName = entry.iface;
            if(ifaceName==null) continue;


            final NetworkIdentitySet ident = ifaceIdent.get(ifaceName);
            if (ident == null) {
                Slog.v(TAG, "unknownIfaces ifaceName ="+ ifaceName);
                unknownIfaces.add(ifaceName);
                continue;
            }
           ///@}
...
*/
    ...
    public void maybePersistLocked(long currentTimeMillis) {
        ...
        if (pendingBytes >= mPersistThresholdBytes) {
            Slog.v(TAG, "maybePersistLocked pendingBytes ="+ pendingBytes);
