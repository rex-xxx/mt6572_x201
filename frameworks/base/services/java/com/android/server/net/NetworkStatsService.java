...
///M: add the following import 
import com.mediatek.common.featureoption.FeatureOption;
import static  android.net.NetworkIdentity.buildNetworkIdentityGemini;
    ...
    private INetworkManagementEventObserver mAlertObserver = new BaseNetworkObserver() {
        ...
        public void limitReached(String limitName, String iface) {
            ...
            if (LIMIT_GLOBAL_ALERT.equals(limitName)) {
                ...
                ///M:  modify flag 
                final int flags = FLAG_PERSIST_ALL;
    ...
    private void updateIfacesLocked() {
        ...
        for (NetworkState state : states) {
            ...
            if (state.networkInfo.isConnected()) {
                ...
                if (LOGV) Slog.v(TAG, iface + " is connected Nettype="+state.networkInfo.getType());
                if ( ( FeatureOption.EVDO_DT_SUPPORT || FeatureOption.MTK_DT_SUPPORT || FeatureOption.MTK_GEMINI_SUPPORT )
                    && isNetworkTypeMobile(state.networkInfo.getType()) ) {

                    NetworkIdentitySet ident = mActiveIfaces.get(iface);
                    if (ident == null) {
                        if (LOGV) Slog.v(TAG, "new netIdent for:" + iface);
                    ...
                    if(iTel == null){
                         Slog.w(TAG, "ITelephony is not ready");
                         continue;
                    }else{
                        int slotId = state.networkInfo.getSimId();
                        try{
                            String subscriberId = iTel.getSubscriberId(slotId);
                            ident.add(NetworkIdentity.buildNetworkIdentityGemini(mContext, state, subscriberId));
                        } catch (RemoteException e) {
                           // ignored; service lives in system_server
                        }
                    }
...
*/
     ...
     */
     //Log.wtf(TAG, "problem reading network stats", e);
       if (LOGV) Slog.v(TAG, "problem reading network stats");
