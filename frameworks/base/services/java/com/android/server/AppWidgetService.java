...
class AppWidgetService extends IAppWidgetService.Stub
{
    ...
    public void systemReady(boolean safeMode) {
        ...
        /// M: Register for IPO boot completed broadcast, so we can do same behavior
        /// with boot completed broadcast.@{
        IntentFilter ipoFilter = new IntentFilter();
        ipoFilter.addAction(ACTION_BOOT_IPO);
        /// @}
        mContext.registerReceiverAsUser(mBroadcastReceiver, UserHandle.ALL, ipoFilter, null, null); 
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (Intent.ACTION_BOOT_COMPLETED.equals(action)
              ...
              || ACTION_BOOT_IPO.equals(action)) {
                ...
                if (userId < 0) {
                    try {
                        userId = ActivityManagerNative.getDefault()
                                .getCurrentUser().id;
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
