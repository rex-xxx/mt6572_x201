...
public class LocationController extends BroadcastReceiver {
    ...
    public LocationController(Context context) {
        ...
        /// M: FOR ALPS00096196.
        filter.addAction("android.location.PRIVACY_OVERWRITE");
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        else if (action.equals("android.location.PRIVACY_OVERWRITE")) {
            Xlog.d(TAG, "[MtkAgpsManagerService] recv android.location.PRIVACY_OVERWRITE");
            mPrivacy = true;
            return;
        } else {
            if(mPrivacy) {
                Xlog.d(TAG, "[MtkAgpsManagerService] This session is privacy overwrite, don't show the GPS icon in status bar");
                mPrivacy = false;
                return;
            }
            /// M: FOR ALPS00096196. @}
