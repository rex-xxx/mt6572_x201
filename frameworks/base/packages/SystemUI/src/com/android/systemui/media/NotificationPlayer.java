        ...
        public void run() {
            ...
            synchronized(this) {
                ...
                try {
                    ...
                    ///M: for audio that is deleted from sd card. @{
                    try {
                        player.setDataSource(mCmd.context, mCmd.uri);
                    } catch (IOException e) {
                        mCmd.uri = Settings.System.DEFAULT_NOTIFICATION_URI;
                        player.setDataSource(mCmd.context, mCmd.uri);
                    }
                    //player.setDataSource(mCmd.context, mCmd.uri);
                    ///M: @}
                    ...
                    if ((mCmd.uri != null) && (mCmd.uri.getEncodedPath() != null)
                        ...
                        if (mCmd.looping) {
                            ...
                            audioManager.requestAudioFocus(mAudioFocusChangeListener, mCmd.stream,
                    ...
                    player.start();
                    ///M: add lock to aovid mPlyaer is released and result in illegalstate 
    ...
    private void startSound(Command cmd) {
        ...
        try {
            ...
            synchronized(mCompletionHandlingLock) {
                ///M: [ALPS00437442] @{
                // We have to abandonAudioFocus first before we quit the message loop.
                // Otherwise, the audiofocus state in AudioService may be in disorder.
                synchronized (mPlayerLock) {
                    if (mPlayer != null) {            
                        mPlayer.stop();
                        mPlayer.release();
