...
public class SettingsProvider extends ContentProvider {
    ...
    private class SettingsFileObserver extends FileObserver {
        ...
        public void onEvent(int event, String path) {
            /// M: fix JE ,when delete an added user in tablet , ALPS00436128 {@
            if (sKnownMutationsInFlight.get(mUserHandle) == null) {
               Log.d(TAG, "UserHandle " + mUserHandle + " null ");
               return;
            }
            /// @}
    ...
    private void establishDbTracking(int userHandle) {
        ...
        /// M: restore for application guide {@
        initDialogSequenceValue(userHandle);
        initOobeValue(userHandle);
        /// @}
    ...
    private void fullyPopulateCaches(final int userHandle) {
         if(userHandle == -1 || mOpenHelpers.get(userHandle) == null){
            Log.d(TAG, "userHandle  " + userHandle + "dbHelper " + mOpenHelpers.get(userHandle));
            return;
        }
            ...
            if (value == null) {
				...
				while ( (index > 0) && (!jumpWhileFlag) ) {
					...
					try {
						...
						Xlog.d(TAG,"create random again: " + newAndroidIdValue);
					} catch (NumberFormatException e) {
						Xlog.d(TAG, "NumberFormatException e ,  android_id " + newAndroidIdValue);
						jumpWhileFlag = true;
					}
				}
                ///  @}
    ...
    /// M: for power on sequence {@
    /**
     *  M:reset dialog sequence value
     */
    private boolean initDialogSequenceValue(int userHandle) {
        ...
        try {
            final String value = c.moveToNext() ? c.getString(0) : null;
            if (value != null) {
                Xlog.d(TAG, "reset DIALOG_SEQUENCE_SETTINGS to default");
                final ContentValues values = new ContentValues();
    ...
    public Bundle call(String method, String request, Bundle args) {
        ...
        Xlog.d(TAG,"call request for " + callingUser);
    ...
    private Bundle lookupValue(DatabaseHelper dbHelper, String table,
        ...
        synchronized (cache) {
            ...
            Xlog.d(TAG,"lookupValue table " + table + " cache contains Key " 
                    + key + " , value = " + value);
            if (value != null) {
                ...
                Xlog.d(TAG,"lookupValue table " + table + " cache.fullyMatchesDisk() " + key);
                return NULL_SETTING;
            }
        }
        Xlog.d(TAG,"lookupValue table " + table + " Not get value from cache " + key);
        SQLiteDatabase db = dbHelper.getReadableDatabase();
        Cursor cursor = null;
        try {
    ...
    private Uri insertForUser(Uri url, ContentValues initialValues, int desiredUserHandle) {
        ...
        Xlog.d(TAG, "insert(" + url + ") for user " + desiredUserHandle
                + " by " + callingUser);
        
        SqlArguments args = new SqlArguments(url);
        if (TABLE_FAVORITES.equals(args.table)) {
            Xlog.d(TAG, "table favorites , return null");
            return null;
        }
        ...
        if (Settings.Secure.LOCATION_PROVIDERS_ALLOWED.equals(name)) {
            if (!parseProviderList(url, initialValues)) {
                Xlog.d(TAG,"location_providers_allowed, and parseProviderList fail, return null");
                return null;
            }
    ...
    public int delete(Uri url, String where, String[] whereArgs) {
        ...
        mutationCount.decrementAndGet();
        Xlog.d(TAG, args.table + " delete " + args.where + " "
            ...
            if (soundUri != null) {
                ...
                if (isDrmAuthority || authority.equals(MediaStore.AUTHORITY)) {
                    ...
                    try {
                        ...
                        /// M: { @ get default ringtone uri
                        Uri defaultUri = getDefaultUri(context, ringtoneType);
                        Xlog.d(TAG, "openAssetFile: defaultUri = " + (defaultUri == null ? "null" : defaultUri.toString()));
                        if(defaultUri != null) {
                            try {
                                pfd = context.getContentResolver().openFileDescriptor(defaultUri, mode);
                                return new AssetFileDescriptor(pfd, 0, -1);
                            } catch (Exception e) {
                                e.printStackTrace();
                            }
   ...
   /// M : add by mtk , get the default ringtone uri
    private Uri getDefaultUri(Context context, int type) {
         ...
         if(type == RingtoneManager.TYPE_NOTIFICATION) {
             uriString = Settings.System.getString(context.getContentResolver(), AudioProfileManager.KEY_DEFAULT_NOTIFICATION);
         } else if(type == RingtoneManager.TYPE_RINGTONE) {
             uriString = Settings.System.getString(context.getContentResolver(), AudioProfileManager.KEY_DEFAULT_RINGTONE);
         } else if(type == RingtoneManager.TYPE_VIDEO_CALL) {
             uriString = Settings.System.getString(context.getContentResolver(), AudioProfileManager.KEY_DEFAULT_VIDEO_CALL);
         } else {
             Xlog.d(TAG, "getDefaultUri: unexpectedType = " + type);
             return null;
         }
    ...
    private static final class SettingsCache extends LruCache<String, Bundle> {
        ...
        public static boolean isRedundantSetValue(SettingsCache cache, String name, String value) {
            ...
            synchronized (cache) {
                ...
                Xlog.d(TAG,"redundant, old Value: " + String.valueOf(oldValue) 
