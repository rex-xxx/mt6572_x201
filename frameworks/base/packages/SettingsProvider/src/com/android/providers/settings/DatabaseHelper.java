...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    IDatabaseHelperExt ext;
    try {
            Log.d(TAG,"getDatabaseHelperPlgin() --- begin");
        ext = (IDatabaseHelperExt) PluginManager.createPluginObject(
                    context, IDatabaseHelperExt.class.getName());
            Log.d(TAG,"getDatabaseHelperPlgin() --- end: "+ext);
    } catch (Plugin.ObjectCreationException e) {
         Log.i(TAG, "Plugin ObjectCreationException");
         ext = new DefaultDatabaseHelperExt();
    }
    return ext;
    }
    /// @}
    ...
    private void loadSystemSettings(SQLiteDatabase db) {
        ...
        try {
            ...
            /// M: loading mtk add values in system table {@
            
            //set GENERAL profile as the default profile.
            loadStringSetting(stmt, AudioProfileManager.KEY_ACTIVE_PROFILE, R.string.def_active_profile);
            
            //set gemini default value
            if(!FeatureOption.MTK_GEMINI_SUPPORT){
                /* for consistent UI ,SIM Management for single sim project */              
                loadSetting(stmt, Settings.System.DUAL_SIM_MODE_SETTING,
    ...
    private void loadUISoundEffectsSettings(SQLiteStatement stmt) {
        ...
        /// M: loading the haptic feedback default value {@
        String hapticValue = mExt.getResBoolean(mContext,
                    Settings.System.HAPTIC_FEEDBACK_ENABLED,
                    R.bool.def_haptic_feedback);
            if (hapticValue != null) {
            loadSetting(stmt, Settings.System.HAPTIC_FEEDBACK_ENABLED, hapticValue);
            }else{
    ...
    private void loadSecureSettings(SQLiteDatabase db) {
        ...
        try {
            ...
            /// M: loading mtk secure table value {@                                 
            ...
            // plugin , loading the battery percentage default value 
            String batteryValue = mExt.getResBoolean(mContext,
                    Settings.Secure.BATTERY_PERCENTAGE,
                    R.bool.def_battery_percentage);
            if (batteryValue != null) {
                loadSetting(stmt, Settings.Secure.BATTERY_PERCENTAGE,
                        batteryValue);
            } else {
            ...
            //plugin , loading the install non market apps default value
            String locationValue = mExt.getResStr(mContext,
                    Settings.Secure.LOCATION_PROVIDERS_ALLOWED,
    ...
    private void loadGlobalSettings(SQLiteDatabase db) {
        ...
        try {
            ...
            /// M: loading default value by feature option set {@
            loadSetting(stmt, Settings.Global.MOBILE_DATA,
                    Settings.Secure.MOBILE_DATA_DEFAULT);
            /// @}
            ...
            /// M: load value in global ,MTK add begin {@
            ...
            loadBooleanSetting(stmt, Settings.Global.AUTO_TIME_GPS,
             ...
             //loading the auto time/zone default value and plugin set begin
            String autotimeValue = mExt.getResBoolean(mContext,
                    Settings.Global.AUTO_TIME, R.bool.def_auto_time);
            if (autotimeValue != null) {
                loadSetting(stmt, Settings.Global.AUTO_TIME, autotimeValue);
            } else {
                loadBooleanSetting(stmt, Settings.Global.AUTO_TIME,
                        R.bool.def_auto_time);// Sync time to NITZ
            }
            
            String autoTzoneValue = mExt.getResBoolean(mContext,
                    Settings.Global.AUTO_TIME_ZONE, R.bool.def_auto_time_zone);
            if (autoTzoneValue != null) {
            ...
            // loading the install non market apps default value 
            String nonMarketValue = mExt.getResBoolean(mContext,
                    Settings.Global.INSTALL_NON_MARKET_APPS,
                    R.bool.def_install_non_market_apps);
            if (nonMarketValue != null) {
