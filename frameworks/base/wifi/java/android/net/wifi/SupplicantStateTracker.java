...
class SupplicantStateTracker extends StateMachine {
    ...
    private boolean mForgetNetwork = false;
    private boolean mDoingWps = false;
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                case WifiMonitor.AUTHENTICATION_FAILURE_EVENT:
                    ...
                    Xlog.d(TAG, "AUTHENTICATION_FAILURE_EVENT, mAuthenticationFailuresCount:" + mAuthenticationFailuresCount);
                ...
                case WifiManager.CONNECT_NETWORK:
                    ...
                    mConnectNetworkId = (Integer)message.obj;
                    mForgetNetwork = false;
                    mAuthenticationFailuresCount = 0;
                    break;
                case WifiManager.FORGET_NETWORK:
                    if ((Integer)message.obj == mConnectNetworkId) {
                        mForgetNetwork = true;
                        mAuthenticationFailuresCount = 0;
                    }
                    break;
    ...
    class HandshakeState extends State {
         ...
         */
        private static final int MAX_SUPPLICANT_LOOP_ITERATIONS = 5;
                ...
                case WifiMonitor.SUPPLICANT_STATE_CHANGE_EVENT:
                    ...
                    if (SupplicantState.isHandshakeState(state)) {
                        if (mLoopDetectIndex >= state.ordinal()) {
                            mLoopDetectCount++;
                            Xlog.d(TAG, "mLoopDetectIndex:" + mLoopDetectIndex + ", state:" + 
                                    state.ordinal() + ", increase mLoopDetectCount:" + mLoopDetectCount);
                        }
    ...
    class CompletedState extends State {
        @Override
         public void enter() {
             ...
             Xlog.d(TAG, "Set scan result handling to scan only!");
             mWifiStateMachine.setScanOnly();
        }
        @Override
        public boolean processMessage(Message message) {
            if (DBG) Log.d(TAG, getName() + message.toString() + "\n");
