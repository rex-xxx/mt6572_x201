...
public class WifiP2pService extends IWifiP2pManager.Stub {
    ...
    /*M: fast connect scan time interval is 6s*/
    private static final int FAST_SCAN_INTERVAL_TIME_MS = 6 * 1000;
     ...
     * return the crendenttial and mac address of it self
     * @hide
     */
    public WifiP2pFastConnectInfo fastConnectAsGo(WifiP2pFastConnectInfo info) {
        try {
            Slog.d(TAG, "Fast Connect As GO in fastConnectIno: " + info);
            ...
            if (mThisDeviceAddress == null) {
                mThisDeviceAddress = WifiNative.getP2pDeviceAddress();
            }
            returnConnectInfo.deviceAddress = mThisDeviceAddress;
            ...
            Slog.d(TAG, "Fast Connect As GO return fastConnectIno: " + returnConnectInfo);
            Slog.d(TAG, "Fast Connect As GO Self fastConnectInfo: " + mFastConnectInfo);
            return returnConnectInfo;
        } catch (Exception e) {
            Slog.e(TAG, "fastConnectAsGo() exception: " + e);
            return null;
        }        
    }
    ...
    class P2pEnabledState extends State {
        ...
        public void enter() {
            ...
            if (mGoFastConnectEnaled)  {
            	setGoFastConnectInfo();
            	sendMessage(WifiP2pManager.DISCOVER_PEERS);
            	sendMessage(WifiP2pManager.FAST_CONNECT_AS_GO);
            }
            if (mGcFastConnectEnaled) {
            	setGcFastConnectInfo();
            	sendMessage(WifiP2pManager.DISCOVER_PEERS);
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                ...
                case WifiMonitor.P2P_DEVICE_FOUND_EVENT:
                    ...
                    logd("P2P_DEVICE_FOUND_EVENT Fast connect GC enableState:" + mGcFastConnectEnaled + " findTimeout:" +
                           mFoundGoTimeOut + " FoundGo:" + mFoundGo);
               ...
               case WifiP2pManager.FAST_DISCOVER_PEERS:
                    if (mGcFastConnectEnaled && !mFoundGoTimeOut && !mFoundGo) {
                        logd(getName() + " Send fast connect loop scan OK");
                        sendMessageDelayed(WifiP2pManager.FAST_DISCOVER_PEERS, FAST_SCAN_INTERVAL_TIME_MS);
                    } else {
                        logd(getName() + " fast connect loop scan stop");
                        break;
                    }
                    clearSupplicantServiceRequest();
                    if (mWifiNative.p2pFind(DISCOVER_TIMEOUT_S)) {
                        ...
                        logd(getName() + " Fast connect scan Fail");
                    }
                    break;
                default:
                    return NOT_HANDLED;
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                ...
                case WifiP2pManager.FAST_CONNECT_AS_GC: 
                    ...
                    logd("FAST_CONNECT_AS_GC mFoundGo=" + mFoundGo + " inMpeers=" +
                            mPeers.containsPeer(mFastConnectInfo.deviceAddress));
                    ...
                    if (mPeers.isGroupOwner(mFastConnectInfo.deviceAddress)) {
                        if (DBG) logd("Fast connect join Group");
                        mWifiNative.p2pReinvoke(mFastConnectInfo.networkId, mFastConnectInfo.deviceAddress);
                        transitionTo(mGroupNegotiationState);
                    } else {
                        if (DBG) logd("Fast connect as gc, but peer isn't go!");
                    }
                    transitionTo(mGroupNegotiationState);
                    break;
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                ...
                case WifiP2pManager.CANCEL_CONNECT:
                    ...
                    if (mWifiNative.p2pGroupRemove(mInterface)) {
                        handleGroupCreationFailure();
                        transitionTo(mInactiveState);
                        replyToMessage(message, WifiP2pManager.CANCEL_CONNECT_SUCCEEDED);
                    } else {
    ...
    class GroupCreatedState extends State {
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                ...
                case WifiMonitor.P2P_PEER_DISCONNECT_EVENT:
                    ...
                    if (DBG) loge(getName() + "I'm GC and has been disconnected by GO");
                    handleGroupRemoved();
                    transitionTo(mInactiveState);
                case WifiStateMachine.CMD_DISABLE_P2P_REQ:
                ...
                case WifiMonitor.P2P_INVITATION_RESULT_EVENT:
                    ...
                    if (true == inviteDone && 
                        ...
                        if (mPeers.remove(mPeers.get(mSavedPeerConfig.deviceAddress))) sendP2pPeersChangedBroadcast();
                    }
                    break;
                case WifiMonitor.P2P_PROV_DISC_PBC_REQ_EVENT:
                case WifiMonitor.P2P_PROV_DISC_ENTER_PIN_EVENT:
                ...
                case WifiMonitor.P2P_DEVICE_FOUND_EVENT:
                    ...
                    sendP2pPeersChangedBroadcast();
             	    break;         
                case WifiMonitor.SUP_DISCONNECTION_EVENT:
                    if (DBG) logd("Supplicant close unexpected, send fake Group Remove event");
        ...
        public void exit() {
            ...
            resetWifiP2pInfo();
            /*M: wifi direct fast connect*/
            setFastConnectInfoOnGroupTermination();
    ...
    private int connect(WifiP2pConfig config, boolean tryInvocation) {
        ...
        /* ALPS00605482: can't use p2pGroupAdd when peer is GO and had ever formed
         else if (join) {
    ...
    private void setFastConnectInfoOnGroupTermination() {
    ...
    public void setGoFastConnectInfo() {
    	int id = mWifiNative.addNetwork();
    	mFastConnectInfo.networkId = id;
    	logd("set fast connect as GO info: " + mFastConnectInfo);
    ...
    public void setGcFastConnectInfo() {
    	int id = mWifiNative.addNetwork();
    	mFastConnectInfo.networkId = id;
    	logd("set fast connect as GC info: " + mFastConnectInfo);
    	mWifiNative.setNetworkVariable(id, "ssid", "\"" + mFastConnectInfo.ssid + "\"");
    	mWifiNative.setNetworkVariable(id, "key_mgmt", "WPA-PSK");
    	mWifiNative.setNetworkVariable(id, "psk", "\"" + mFastConnectInfo.psk + "\"");
    	mWifiNative.setNetworkVariable(id, "proto", "RSN");
