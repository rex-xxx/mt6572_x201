...
class WifiConfigStore {
    ...
    WifiConfigStore(Context c, WifiNative wn) {
        mContext = c;
        mWifiNative = wn;
        mWifiExt = MediatekClassFactory.createInstance(IWifiFwkExt.class, mContext);
    }
    ...
    private NetworkUpdateResult addOrUpdateNetworkNative(WifiConfiguration config) {
        ...
        setVariables: {
            ...
            if (config.priority >= 0 && !mWifiNative.setNetworkVariable(
                        netId,
            ...
            if (FeatureOption.MTK_EAP_SIM_AKA) {
                if (config.imsi != null &&
                        !mWifiNative.setNetworkVariable(
                            netId,
                            WifiConfiguration.IMSI_VAR_NAME,
                            config.imsi)) {
                    Xlog.e(TAG, "failed to set imsi: " + config.imsi);
                    break setVariables;
                }
                if (config.simSlot != null &&
                        !mWifiNative.setNetworkVariable(
                            ...
                            config.simSlot)) {
                    Xlog.e(TAG, "failed to set simSlot: " + config.simSlot);
                    break setVariables;
                }
                if (config.pcsc != null &&
                        !mWifiNative.setNetworkVariable(
                            netId,
                            WifiConfiguration.PCSC_VAR_NAME,
                            config.pcsc)) {
                    Xlog.e(TAG, "failed to set pcsc: " + config.pcsc);
                    break setVariables;
                }
            }
    ...
    private void readNetworkVariables(WifiConfiguration config) {
        ...
        // M: Add for EAP-SIM begin
        if (FeatureOption.MTK_EAP_SIM_AKA) {
            value = mWifiNative.getNetworkVariable(netId, WifiConfiguration.IMSI_VAR_NAME);
            if (!TextUtils.isEmpty(value)) {
                config.imsi = value;
            } else {
                config.imsi = null;
            ...
            value = mWifiNative.getNetworkVariable(netId, WifiConfiguration.PCSC_VAR_NAME);
            if (!TextUtils.isEmpty(value)) {
                config.pcsc = value;
            } else {
                config.pcsc = null;
     ...
     * Set IP configuration for a given network id
     */
    void setIpConfiguration(int netId, LinkProperties linkProperties) {
        WifiConfiguration config = mConfiguredNetworks.get(netId);
        if (config != null) {
            // add old proxy details
            if(config.linkProperties != null) {
            ...
            Xlog.d(TAG, "No configuration for netId:" + netId);
        }
    }
