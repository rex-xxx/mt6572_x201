...
public class WifiMonitor {
    ...
    private static final int NO_CERTIFICATION = 9;  
   ...
   /* Network connection completed */
    ...
    /*M: Only GC will received this */
    public static final int P2P_PEER_DISCONNECT_EVENT            = BASE + 40;

    /* hostap events */
    public static final int AP_STA_DISCONNECTED_EVENT            = BASE + 41;
    public static final int AP_STA_CONNECTED_EVENT               = BASE + 42;

    public static final int WAPI_NO_CERTIFICATION_EVENT          = BASE + 51;
    ...
    private MonitorThread mCurrentThread;
    ...
    public void startMonitoring() {
        mStopped = false;
        mCurrentThread = new MonitorThread();
        mCurrentThread.start();
    }

    class MonitorThread extends Thread {
        public MonitorThread() {
            super("WifiMo-" + mWifiNative.mInterface);
        }
        ...
        public void run() {
            ...
            if (connectToSupplicant()) {
                ...
                if (!mWifiNative.mInterface.equals("ap0")) {
                    mStateMachine.sendMessage(SUP_DISCONNECTION_EVENT);
                }
                Log.d(TAG, "Exit because of connection failure for " + getName() + ", id:" + getId());
                return;
            }

            if (mStopped) {
                Log.d(TAG, "Need to stop, closeSupplicantConnection and exit! id:" + getId());
        ...
        private boolean connectToSupplicant() {
            ...
            while (!mStopped) {
                Log.d(TAG, "connectToSupplicant for " + getName() + ", id:" + getId() + ", connectTries:" + connectTries);
                mConnecting.set(true);
                if (mWifiNative.connectToSupplicant()) {
                    mConnecting.set(false);
                    return true;
                }
                if (!mStopped && connectTries++ < 5) {
                    nap(1);
                } else {
                    break;
                }
        ...
        private void handleP2pEvents(String dataString) {
            ...
            } else if (dataString.startsWith("CTRL-EVENT-DISCONNECTED")) {
                /*CTRL-EVENT-DISCONNECTED bssid=02:0c:e7:55:55:55 reason=2*/
                String[] tokens = dataString.split(" ");
                if (tokens.length != 3) return;
                String[] nameValue = tokens[1].split("=");
                if (nameValue.length != 2) return;
                mStateMachine.sendMessage(P2P_PEER_DISCONNECT_EVENT, nameValue[1]);
            }
        }
    ...
    public void stopMonitoring() {
        if (mCurrentThread != null) {
            mStopped = true;
            if (!mConnecting.get()) {
                Log.d(TAG, "stopMonitoring, closeSupplicantConnection for " + mCurrentThread.getName() + ", id:" + mCurrentThread.getId());
                mWifiNative.closeSupplicantConnection();
            } else {
                Log.d(TAG, "stopMonitoring, it's connecting to supplicant!");
            }
            try {
                mCurrentThread.join();
            } catch (InterruptedException ignore) {
            ...
            Log.d(TAG, "stopMonitoring, mCurrentThread is null!");
        }
    }
}
