...
public class WifiStateTracker implements NetworkStateTracker {
    ...
    public void startMonitoring(Context context, Handler target) {
        ...
        } else if (mNetType == ConnectivityManager.TYPE_WIFI_P2P) {
    ...
    private class WifiStateReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (intent.getAction().equals(WifiP2pManager.WIFI_P2P_CONNECTION_CHANGED_ACTION)) {
                ...
                SXlog.d(TAG, "New state:" + state + ", mLastState:" + mLastState);
                if (state == NetworkInfo.State.CONNECTED && mLinkProperties.getLinkAddresses().size() == 0) {
                    SXlog.d(TAG, "State is CONNECTED but there is no LinkAddress, ignore.");
                    return;
                } 
                if (mLastState == state &&
