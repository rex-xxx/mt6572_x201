...
public class View implements Drawable.Callback, KeyEvent.Callback,
    ...
    private int DBG_TIMEOUT_VALUE = 400;
    ...
    */
    ...
    protected void onFocusChanged(boolean gainFocus, int direction, Rect previouslyFocusedRect) {
        ...
        if (DBG_FOCUS) {
            Xlog.d(VIEW_LOG_TAG, "onFocusChanged: gainFocus = " + gainFocus + ",direction = "
    ...
    */
    ...
    public void onWindowFocusChanged(boolean hasWindowFocus) {
        InputMethodManager imm = InputMethodManager.peekInstance();
        if (DBG_FOCUS) {
            Xlog.d(VIEW_LOG_TAG, "onWindowFocusChanged: hasWindowFocus = " + hasWindowFocus
                    + ",imm = " + imm + ",this = " + this);
        }
   ...
   /**
    ...
    public void setLayerType(int layerType, Paint paint) {
        ...
        if (DBG_HWUI && isTargetPackage() && layerType == LAYER_TYPE_HARDWARE) {
            String logString = ">>>>>>>>>>>setLayerType: layerType = " + layerType + ",mLayerType = " + mLayerType
                    + ",mHardwareLayer = " + mHardwareLayer + ",this = " + this;
            if (mHardwareLayer != null && mHardwareLayer instanceof GLES20Layer) {
                logString += ", mLayer=0x" + Integer.toHexString(((GLES20Layer) mHardwareLayer).getLayer());
            }
            Xlog.d(VIEW_LOG_TAG, logString);
        }
    ...
    boolean destroyLayer(boolean valid) {
        if (mHardwareLayer != null) {
            if (DBG_HWUI && isTargetPackage()) {
                String logString = "<<<<<<<<<<<<destroyLayer: mHardwareLayer = " + mHardwareLayer + ",valid = " + valid
                        + ",mLayerType = " + mLayerType + ",mAttachInfo = " + mAttachInfo + ",this = " + this;
                if (mHardwareLayer instanceof GLES20Layer) {
                    logString += ", mLayer = 0x" + Integer.toHexString(((GLES20Layer) mHardwareLayer).getLayer());
                }
                if (mAttachInfo != null) {
                    logString += ",render = " + mAttachInfo.mHardwareRenderer;
                }
                Xlog.d(VIEW_LOG_TAG, logString, new Throwable("View destroyLayer"));
            }

            AttachInfo info = mAttachInfo;
            /// M: destroy layers even when the render is disabled if the force flag is set.
            if (info != null && info.mHardwareRenderer != null
                    && (info.mHardwareRenderer.isEnabled() || info.mHardwareRenderer.forceDestroyLayers())
                    && (valid || info.mHardwareRenderer.validate())) {
                if (DBG_HWUI && isTargetPackage()) {
                    Xlog.d(VIEW_LOG_TAG, "Run into really destory layers, this = " + this);
                }
    ...
    private DisplayList getDisplayList(DisplayList displayList, boolean isLayer) {
        ...
        if (DBG_HWUI && isTargetPackage()) {
            Xlog.v(VIEW_LOG_TAG, "getDisplayList: displayList = " + displayList + ",isLayer = " + isLayer
                    + ",mRecreateDisplayList = " + mRecreateDisplayList + ",this = " + this);
        }
        ...
        if (((mPrivateFlags & PFLAG_DRAWING_CACHE_VALID) == 0 ||
            ...
            if (DBG_HWUI && isTargetPackage()) {
                Xlog.v(VIEW_LOG_TAG, "getDisplayList middle: width = " + width + ",height = " + height + ",displayList = "
                        + displayList + ",layerType = " + getLayerType() + ",this = " + this);
            }
            ...
            try {
                ...
                if (!isLayer && layerType != LAYER_TYPE_NONE) {
                    if (DBG_HWUI && isTargetPackage()) {
                        Xlog.v(VIEW_LOG_TAG, "getDisplayList : layerType = " + layerType + ",this = " + this);
                    }
        ...
        if (!verticalEdges && !horizontalEdges) {
            ...
            if (!dirtyOpaque) {
                ...
                if (nowTime - logTime > DBG_TIMEOUT_VALUE) {
                    Xlog.d(VIEW_LOG_TAG, "[ANR Warning]onDraw time too long, this =" + this + "time =" + (nowTime - logTime));
                }
            }
    ...
    /**
     * M: debug option, whether the view is in target package, used to reduce useless logs.
     * 
     * @return
     */
    ...
    /**
     * M: This is used by viewgroups to clear child view's measured height previoulsy gained. For example,
     * in LinearLayout, if one child claims it has weight > 0 and height = 0, then it will be skipped in the first
     * children measurement round, but if after the first measurement round we get the 'delta' equals 0, then that 
     * child will never got measured again. So we must use this api to reset the previouly measured height value
     * to 0 in this case, otherwise the child view will layout and draw using previously measured height to update
     * For ALPS00442434
     * 
     * {@hide}
     */
    ...
    /**
     * M: This is used by viewgroups to clear child view's measured width previoulsy gained. For example,
     * in LinearLayout, if one child claims it has weight > 0 and width = 0, then it will be skipped in the first
     * children measurement round, but if after the first measurement round we get the 'delta' equals 0, then that 
     * child will never got measured again. So we must use this api to reset the previouly measured width value
     * to 0 in this case, otherwise the child view will layout and draw using previously measured height to update.
     * For ALPS00442434
     * 
     * {@hide}
