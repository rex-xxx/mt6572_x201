...
public abstract class HardwareRenderer {
    ...
    /// M: force destroy hardware layers in render, add only for trim memory level 60&80.
    ...
    /**
     * M: Switches on/off HWUI debug log.
     *
     * @param enable enable or disable log.
     */
    public static void setHWUIDebugLog(boolean enable) {
        nSetHWUIDebugLog(enable);
    }

    private static native void nSetHWUIDebugLog(boolean enable);
    ...
    /**
     * M: Indicates whether force destroy all hardware layers in current render.
     * 
     * @return True if force the hardware renderer to destroy layers, false otherwise.
     */
    boolean forceDestroyLayers() {
        if (DBG) {
            Xlog.v(LOG_TAG, "forceDestroyLayers: mForceDestroyLayers = " + mForceDestroyLayers + ",this = " + this);
        }
        return mForceDestroyLayers;
    }

    /**
     * M: Force destroy all hardware layers, only called when ViewRootImpl try to terminateHardwareResources by trim memory.
     * 
     * @param force True if force the hardware renderer to destroy layers, false otherwise.
     */
    void setForceDestroyLayers(final boolean force) {
        if (DBG) {
            Xlog.v(LOG_TAG, "setForceDestroyLayers: force = " + force + ",mForceDestroyLayers = " + mForceDestroyLayers
                    + ",this = " + this);
        }
        mForceDestroyLayers = force;
    }
    ...
    static abstract class GlRenderer extends HardwareRenderer {
        ...
        boolean initialize(Surface surface) throws Surface.OutOfResourcesException {
            if (isRequested() && !isEnabled()) {
                ...
                mDestroyed = false;
                if (DBG) {
                    Xlog.v(LOG_TAG, "Hardware render initialize: mGl = " + mGl + ",this = " + this);
                }
        ...
        void destroy(boolean full) {
            ...
            if (DBG) {
                Xlog.v(LOG_TAG, "Destroy hardware render: full = " + full + ",mCanvas = " + mCanvas + ",mDestroyed = "
                        + mDestroyed + ",this = " + this);
            }
    ...
    static class Gl20Renderer extends GlRenderer {
        ...
        static class Gl20RendererEglContext extends ManagedEGLContext {
            ...
            public void onTerminate(final EGLContext eglContext) {
                if (DBG) {
                    Xlog.v(LOG_TAG, "Gl20Renderer onTerminate : eglContext = " + eglContext + ",sEgl = " + sEgl + ",this = "
                            + this);
                }
        ...
        boolean safelyRun(Runnable action) {
            ...
            if (needsContext) {
                ...
                if (DBG) {
                    Xlog.d(LOG_TAG, "safelyRun cond 1 : action = " + action + ",managedContext = " + managedContext
                            + ",this = " + this);
                }
            ...
            if (DBG) {
                Xlog.d(LOG_TAG, "safelyRun cond 2 : action = " + action + ",needsContext = " + needsContext + ",this = "
                        + this);
            }
        ...
        void destroyLayers(final View view) {
            if (DBG) {
                Xlog.d(LOG_TAG, "GL20Render destroyLayers: view = " + view + ",isEnabled() = " + isEnabled() + ",this = "
                        + this);
            }
            if (view != null) {
        ...
        void destroyHardwareResources(final View view) {
            if (view != null) {
                safelyRun(new Runnable() {
                    ...
                    public void run() {
                        if (DBG) {
                            Xlog.d(LOG_TAG, "Safely run destroyHardwareResources: view = " + view + ",this = " + this);
                        }
        ...
        static void startTrimMemory(int level) {
            ...
            if (DBG) {
                Xlog.d(LOG_TAG, "Hardware render startTrimMemory: level = " + level);
            }
