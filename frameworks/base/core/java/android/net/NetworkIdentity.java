...
public class NetworkIdentity {
     ...
     * assuming that any mobile networks are using the current IMSI.
     */
    public static NetworkIdentity buildNetworkIdentityGemini(Context context, NetworkState state, long simId) {
        final int type = state.networkInfo.getType();
        final int subType = state.networkInfo.getSubtype();
        ...
        if (isNetworkTypeMobile(type)) {
            ...
            if (state.subscriberId != null) {
                ...
                ///M: prevent ITelephony is not ready
                if(iTel == null){
                     Slog.w(TAG, "ITelephony is not ready");
                }else{
                    try{
                        subscriberId = iTel.getSubscriberId(slotId);
