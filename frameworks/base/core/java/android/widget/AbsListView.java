...
public abstract class AbsListView extends AdapterView<ListAdapter> implements TextWatcher,
    ...
    /// M: Add a group of log filter for AbsListView @{
    ...
    /// M: Add a group of log filter flag for AbsListView, used to switch log on/off runtime @{
	private static final int DBG_MOTION_FLAG = 0x00000001;
	private static final int DBG_DATACHANGE_FLAG = 0x00000002;
	private static final int DBG_LAYOUT_FLAG = 0x00000004;
	private static final int DBG_RECYCLE_FLAG = 0x00000008;
	private static final int DBG_DRAW_FLAG = 0x00000010;
	private static final int DBG_FLAG = 0x00000020;
    /// @}

	/// M: The magnification of list over fling velocity.
    ...
    private void initAbsListView() {
        ...
        if (sDbgLayout) {
            Xlog.d(TAG, "initAbsListView here: this = " + this);
        }
    }
    ...
    private boolean contentFits() {
        ...
        if (sDbg) {
            Xlog.d(TAG, "contentFits:childCount = " + childCount + ",mItemCount = " + mItemCount
                    + ",first child top = " + getChildAt(0).getTop() + ",last child bottom = "
                    + getChildAt(childCount - 1).getBottom() + ",getHeight() = " + getHeight()
                    + ",mListPadding = " + mListPadding);
        }
    ...
    public Parcelable onSaveInstanceState() {
        ...
        if (sDbgLayout) {
            Xlog.d(TAG, "onSaveInstanceState: ss = " + ss + ",this = " + this);
        }
    ...
    public void onRestoreInstanceState(Parcelable state) {
        ...
        if (sDbgLayout) {
            Xlog.d(TAG, "onRestoreInstanceState: ss = " + ss + ",this = " + this);
        }
        requestLayout();
    }
    ...
    public void setFilterText(String filterText) {
        ...
        if (mTextFilterEnabled && !TextUtils.isEmpty(filterText)) {
            ...
            /** M: Do not call setSelection,  it will made the text show abnormal when the
             * filterText is too long, see CR 125819 for reference
    ...
    View obtainView(int position, boolean[] isScrap) {
        ...
        if (scrapView != null) {
            ...
            if (sDbgLayout) {
                Xlog.d(TAG, "obtainView: position = " + position + ",scrapView = " + scrapView
            ...
            if (sDbgLayout) {
                Xlog.d(TAG, "obtainView scrap view is null: position = " + position
                        + ",scrapView = " + scrapView + ",child = " + child + ",this = " + this);
            }
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (sDbgDraw) {
            Xlog.d(TAG, "dispatchDraw: mScrollY = " + mScrollY + ",clipToPadding = "
                    + clipToPadding + ",this = " + this);
        }
        if (clipToPadding) {
    ...
    protected void onAttachedToWindow() {
        ...
        if (mAdapter != null && mDataSetObserver == null) {
            ...
            /// M: [ALPS00594951] data changed, requestLayout here to make sure relayout again
            requestLayout();
        }
        mIsAttached = true;
    }
    ...
    public void onWindowFocusChanged(boolean hasWindowFocus) {
        ...
        if (!hasWindowFocus) {
            ...
            /// M: bug fix 402306, fast scroller's overlay drawable did not hide when cancel event comes @{
            if (mFastScroller != null) {
    ...
    final class CheckForTap implements Runnable {
        public void run() {
            if (mTouchMode == TOUCH_MODE_DOWN) {
                ...
                if (sDbg) {
                    Xlog.d(TAG, "CheckForTap:mFirstPosition = " + mFirstPosition
                            + ",mMotionPosition = " + mMotionPosition + ",child = " + child);
                }            
    ...
    private boolean startScrollIfNeeded(int y) {
        ...
        if (sDbgMotion) {
            Xlog.d(TAG, "startScrollIfNeeded:deltaY = " + deltaY + ",mScrollY = " + mScrollY
                    + ",mTouchSlop = " + mTouchSlop + ",mTouchMode = " + mTouchMode);
        }
        if (overscroll || distance > mTouchSlop) {
            ...
            /**
             * M: [ALPS00361026] unpressed all children when starting to scroll @{
             */                 
            /// Mark google default design                                    
    ...
    private void scrollIfNeeded(int y) {
        ...
        if (mTouchMode == TOUCH_MODE_SCROLL) {
            ...
            if (y != mLastY) {
                ...
                if (sDbgMotion) {
                    Xlog.d(TAG, "Touch scroll: incrementalDeltaY = " + incrementalDeltaY + ",mScrollY = " 
                            + mScrollY + ",atEdge = " + atEdge + ",motionIndex = " + motionIndex +
                            ",motionView = " + motionView + ",overscrollMode = " + getOverScrollMode());
                }
                if (motionView != null) {
            ...
            if (y != mLastY) {
                ...
                if (sDbgMotion) {
                    Xlog.d(TAG, "Touch over scroll: incrementalDeltaY = " + incrementalDeltaY
                            + ",mScrollY = " + mScrollY + ",newScroll = " + newScroll
                            + ",newDirection = " + newDirection + ",mDirection = " + mDirection
                            + ",y = " + y + ",mLastY = " + mLastY + ",mFirstPosition = " + mFirstPosition);
                }
                ...
                if (incrementalDeltaY != 0) {
                    if (sDbgMotion) {
                        Xlog.d(TAG, "Coming back to 'real' list scrolling: incrementalDeltaY = " + incrementalDeltaY 
                                + ",mScrollY = " + mScrollY + ",mFirstPosition = " + mFirstPosition);
                    }
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        /*
         * M: Abnormal case, intent to operate on non-exist data. Handle the case use touches on
         * non-exist view @{
         */
        if (getChildCount() == 0 || (mDataChanged && mFirstPosition > mItemCount)) {
            Xlog.w(TAG, "Intent to operate on non-exist data, childcount = " + getChildCount()
                    + ",mFirstPosition = " + mFirstPosition + ",adapter count = " + mItemCount
                    + ",action = " + action + ",mActivePointerId = " + mActivePointerId
                    + ",mScrollY = " + mScrollY + ",this = " + this);

            // Reset mActivePointerId if the current movement is a second pointer up.
        ...
        case MotionEvent.ACTION_DOWN: {
            if (sDbgMotion || sDbgDataChange) {
                Xlog.d(TAG, "Touch down: touch mode = " + mTouchMode + ",mScrollY = "
                        + mScrollY + ",y = " + ev.getY() + ",mFirstPosition = "
                        + mFirstPosition + ",mActivePointerId = " + mActivePointerId
                        + ",mDataChanged = " + mDataChanged + ",adatper size = " + mItemCount
                        + ",this = " + this);
            }
            switch (mTouchMode) {
            ...
            default: {
                ...
                if (sDbgMotion) {
            		Xlog.d(TAG, "Touch down: touch mode = " + mTouchMode + ",mMotionY = " + mMotionY + 
            				",mMotionPosition = " + mMotionPosition + ",mFirstPosition = " + mFirstPosition);
            	}
                break;
            }
            }
        ...
        case MotionEvent.ACTION_MOVE: {
            ...
            if (sDbgMotion || sDbgDataChange) {
                Xlog.d(TAG, "Touch move: touch mode = " + mTouchMode + ",mScrollY = "
                        + mScrollY + ",mLastY = " + mLastY + ",y = " + y
                        + ",mActivePointerId = " + mActivePointerId + ",pointerIndex = "
                        + pointerIndex + ",mMotionPosition = " + mMotionPosition
                        + ",mFirstPosition = " + mFirstPosition + ",mDataChanged = "
                        + mDataChanged + ",adatper size = " + mItemCount + ",this = " + this);
            }
        ...
        case MotionEvent.ACTION_UP: {
        	if (sDbgMotion || sDbgDataChange) {
        		Xlog.d(TAG, "Touch up: touch mode = " + mTouchMode + ",mScrollY = " + mScrollY + 
        				",mLastY = " + mLastY + ",mMotionPosition = " + mMotionPosition + 
        				",mFirstPosition = " + mFirstPosition + ",mDataChanged = "
                        + mDataChanged + ",adatper size = " + mItemCount + ",this = " + this);
        	}
            switch (mTouchMode) {
            ...
            case TOUCH_MODE_DONE_WAITING:
                ...
                if (child != null && !child.hasFocusable() && inList) {
                    ...
                    /**
                     * M: If user release the finger in the padding area, we need to reset the 
                     * pressed state of both the child view and the list itself. @{
                     */
                    Xlog.d(TAG, "Touch up out of list: mTouchMode = " + mTouchMode + ",pressed = " + isPressed());
                    if (mTouchMode != TOUCH_MODE_DOWN) {
                        setPressed(false);
                        if (child != null) {
                            child.setPressed(false);
            ...
            case TOUCH_MODE_SCROLL:
                ...
                if (childCount > 0) {
                    ...
                    if (mFirstPosition == 0 && firstChildTop >= contentTop &&
                        ...
                        Xlog.i(TAG, "Fling from scroll with initialVelocity = " + initialVelocity
                                + ",mActivePointerId = " + mActivePointerId
                                + ",mFirstPosition = " + mFirstPosition
                                + ",firstChildTop = " + firstChildTop + ",this = " + this);
            ...
            case TOUCH_MODE_REST: 
                /**
                 * M: When touch the list item and press the key, touch mode will change to TOUCH_MODE_REST,
                 * so reset child press state here @{
                 */
                final int lostPosition = mMotionPosition;
                final View lostChild = getChildAt(lostPosition - mFirstPosition);
                Xlog.d(TAG, "Touch cancel:mMotionPosition = " + mMotionPosition + ",mFirstPosition = " 
                        + mFirstPosition + ",lostChild = " + lostChild + ",this = " + this);
        ...
        case MotionEvent.ACTION_CANCEL: {
            if (sDbgMotion || sDbgDataChange) {
                Xlog.d(TAG, "Touch cancel: touch mode = " + mTouchMode + ",mScrollY = " + mScrollY + 
                        ",mActivePointerId = " + mActivePointerId + ",mMotionPosition = " + mMotionPosition + 
                        ",mFirstPosition = " + mFirstPosition + ",mDataChanged = "
                        + mDataChanged + ",adatper size = " + mItemCount + ",this = " + this);
            }
            switch (mTouchMode) {
            ...
            default:
                ...
                /// M: [ALPS00425497] change scroll state to idle when touch event canceled
                reportScrollStateChange(OnScrollListener.SCROLL_STATE_IDLE);
        ...
        case MotionEvent.ACTION_POINTER_UP: {
            /**
             * M:When user release a finger, check pressed child's state and reset it if needed. @{
             */
            final int lostMotionPosition = mMotionPosition;
            final View child = getChildAt(lostMotionPosition - mFirstPosition);
            ...
            if (sDbgDataChange) {
                Xlog.d(TAG, "Touch second pointer up, touch mode = " + mTouchMode
                        + ",mScrollY = " + mScrollY + ",mActivePointerId = " + mActivePointerId
                        + ",mFirstPosition = " + mFirstPosition + ",mDataChanged = "
                        + mDataChanged + ",adatper size = " + mItemCount + ",this = " + this);
            }
        ...
        case MotionEvent.ACTION_POINTER_DOWN: {
            ...
            if (sDbgMotion || sDbgDataChange) {
                Xlog.d(TAG, "Touch second pointer down, touch mode = " + mTouchMode
                        + ",mScrollY = " + mScrollY + ",mActivePointerId = " + mActivePointerId
                        + ",id = " + id + ",index = " + index + ",mFirstPosition = "
                        + mFirstPosition + ",mDataChanged = " + mDataChanged
                        + ",adatper size = " + mItemCount + ",this = " + this);
            }
            mMotionCorrection = 0;
            mActivePointerId = id;
            mMotionX = x;
    ...
    protected void onOverScrolled(int scrollX, int scrollY, boolean clampedX, boolean clampedY) {
        if (sDbgMotion) {
            Xlog.d(TAG, "onOverScrolled: mScrollY = " + mScrollY + ",scrollY = " + scrollY
                    + ",clampedY = " + clampedY + ",this = " + this);
        }
        if (mScrollY != scrollY) {
    ...
    public boolean onGenericMotionEvent(MotionEvent event) {
        if (sDbgMotion) {
            Xlog.d(TAG, "onGenericMotionEvent: mScrollY = " + mScrollY + ",event = " + event
                    + ",this = " + this);
        }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if (sDbgMotion) {
            Xlog.d(TAG, "onInterceptTouchEvent: action = " + action + ",x= " + ev.getX() + ",y = "
                    + ev.getY() + ",mTouchMode = " + mTouchMode + ",mFastScroller = "
                    + mFastScroller + ",this = " + this);
        }
        if (mPositionScroller != null) {
            mPositionScroller.stop();
        }
        ...
        case MotionEvent.ACTION_DOWN: {
            ...
            mVelocityTracker.addMovement(ev);
            if (sDbgMotion) {
                Xlog.d(TAG, "onInterceptTouchEvent touch down: motionPosition = "
                        + motionPosition + ",mMotionPosition = " + mMotionPosition + ",this = "
                        + this);
            }
    ...
    private void onSecondaryPointerUp(MotionEvent ev) {
        ...
        if (sDbgMotion) {
            Xlog.d(TAG, "onSecondaryPointerUp:pointerIndex = " + pointerIndex
                    + ",mActivePointerId = " + mActivePointerId + ",pointerId = " + pointerId);
        }        
    ...
    private class FlingRunnable implements Runnable {
        ...
        private final Runnable mCheckFlywheel = new Runnable() {
            public void run() {
                ...
                if (vt == null || activeId == INVALID_POINTER) {
                    Xlog.i(TAG, "Direct return : vt = " + vt + ",activeId = " + activeId
                            + ",mScrollY = " + mScrollY);
                    return;
                }
                ...
                if (sDbgMotion) {
                    Xlog.d(TAG, "Fly wheel: yvel = " + yvel + ",activeId = " + activeId
                            + ",mScrollY = " + mScrollY + ",mTouchMode = " + mTouchMode
                            + ",mMinimumVelocity = " + mMinimumVelocity + ",ListView = "
                            + AbsListView.this);
                }
        ...
        void start(int initialVelocity) {
            ...
            if (sDbgMotion) {
                Xlog.d(TAG, "start:touch mode = " + mTouchMode + ",mScrollY = " + mScrollY 
                        + ",initialVelocity = " + initialVelocity + ",mLastFlingY = " + mLastFlingY);
            }
        ...
        void startSpringback() {
            if (sDbgMotion) {
                Xlog.d(TAG, "startSpringback, touch mode = " + mTouchMode + ",mScrollY = " + mScrollY);
            }
        ...
        void startOverfling(int initialVelocity) {
            if (sDbgMotion) {
                Xlog.d(TAG, "startOverfling, touch mode = " + mTouchMode + ",mScrollY = "
                        + mScrollY + ",initialVelocity = " + initialVelocity
                        + ",mMinimumVelocity = " + mMinimumVelocity + ",getHeight() = "
                        + getHeight());
            }
            mScroller.setInterpolator(null);
        ...
        void edgeReached(int delta) {
            ...
            if (overscrollMode == OVER_SCROLL_ALWAYS ||
                ...
                if (sDbgMotion) {
                    Xlog.d(TAG, "edgeReached when fling, touch mode = " + mTouchMode + 
                            ",mScrollY = " + mScrollY + ",delta = " + delta + ",vel = " + vel);
                }
                if (delta > 0) {
        ...
        void startScroll(int distance, int duration, boolean linear) {
            ...
            mLastFlingY = initialY;
            if (sDbgMotion) {
                Xlog.d(TAG, "startScroll, touch mode = " + mTouchMode + ",mScrollY = " + mScrollY
                        + ",distance = " + distance + ",mLastFlingY = " + mLastFlingY);
            }
        ...
        void endFling() {
            if (sDbgMotion) {
                Xlog.d(TAG, "endFling: mScrollY = " + mScrollY + ",mTouchMode = " + mTouchMode,
                        new Throwable("endFling"));
            }
            mTouchMode = TOUCH_MODE_REST;
        ...
        void flywheelTouch() {
            if (sDbgMotion) {
                Xlog.d(TAG, "flywheelTouch: touch mode = " + mTouchMode + ",mScrollY = " + mScrollY
                        + ",this = " + AbsListView.this);
            }
        ...
        public void run() {
            if (sDbgDataChange) {
                Xlog.d(TAG, "FlingRunnable: touch mode = " + mTouchMode + ",mAdapter = " + mAdapter
                        + ",mFirstPosition = " + mFirstPosition + ",mDataChanged = " + mDataChanged
                        + ",adatper size = " + mItemCount + ",this = " + AbsListView.this);
            }
            switch (mTouchMode) {
            default:
                endFling();
            ...
            case TOUCH_MODE_FLING: {
                ...
                /** @} */
                if (sDbgMotion) {
                    Xlog.d(TAG, "FlingRunnable TOUCH_MODE_FLING: mScrollY = " + mScrollY
                            + ",atEnd = " + atEnd + ",delta = " + delta + ",more = " + more
                            + ",y = " + y + ",mLastFlingY = " + mLastFlingY);
                }
            ...
            case TOUCH_MODE_OVERFLING: {
                ...
                if (scroller.computeScrollOffset()) {
                    ...
                    if (overScrollBy(0, deltaY, 0, scrollY, 0, 0,
                        ...
                        if (sDbgMotion) {
                            Xlog.d(TAG, "FlingRunnable touch mode overfling: crossDown = "
                                    + crossDown + ",crossUp = " + crossUp + ",mScrollY = "
                                    + mScrollY + ",scrollY = " + scrollY + ",deltaY = " + deltaY);
                        }
                        ...
                        if (sDbgMotion) {
                            Xlog.d(TAG, "FlingRunnable overScrollBy return false: mScrollY = " + mScrollY 
                                    + ",old scrollY = " + scrollY + ",mScroller.getCurrVelocity() = " + mScroller.getCurrVelocity());
                        }
                        invalidate();
                        postOnAnimation(this);
                    }
                    ...
                    if (sDbgMotion) {
                        Xlog.d(TAG, "FlingRunnable overfling intend to endfling: mScrollY = "+ mScrollY);
                    }
                    endFling();
                }
                break;
    ...
    public void smoothScrollToPosition(int position) {
        if (sDbgMotion) {
            Xlog.d(TAG, "smoothScrollToPosition: position = " + position + ",mScrollY = "
                    + mScrollY + ",this = " + this);
        }
        if (mPositionScroller == null) {
    ...
    public void smoothScrollToPositionFromTop(int position, int offset, int duration) {
        if (sDbgMotion) {
            Xlog.d(TAG, "smoothScrollToPositionFromTop: position = " + position + ",offset = "
                    + offset + ",duration = " + duration + ",mScrollY = " + mScrollY + ",this = " + this);
        }
        if (mPositionScroller == null) {
            mPositionScroller = new PositionScroller();
        }
    ...
    public void smoothScrollToPositionFromTop(int position, int offset) {
        if (sDbgMotion) {
            Xlog.d(TAG, "smoothScrollToPositionFromTop: position = " + position + ",offset = "
                    + offset + ",mPositionScroller = " + mPositionScroller + ",mScrollY = "
                    + mScrollY + ",this = " + this);
        }
        if (mPositionScroller == null) {
            mPositionScroller = new PositionScroller();
    ...
    public void smoothScrollToPosition(int position, int boundPosition) {
        if (sDbgMotion) {
            Xlog.d(TAG, "smoothScrollToPosition: position = " + position + ",boundPosition = "
                    + boundPosition + ",mPositionScroller = " + mPositionScroller + ",mScrollY = "
                    + mScrollY + ",this = " + this);
        }        
        if (mPositionScroller == null) {
            mPositionScroller = new PositionScroller();
    ...
    void smoothScrollBy(int distance, int duration, boolean linear) {
        ...
        if (distance == 0 || mItemCount == 0 || childCount == 0 ||
            ...
            Xlog.i(TAG, "smoothScrollBy: mScrollY = " + mScrollY + ",distance = " + distance
                    + ",mItemCount = " + mItemCount + ",childCount = " + childCount);
            mFlingRunnable.endFling();
            if (mPositionScroller != null) {
                mPositionScroller.stop();
            }
    ...
    boolean trackMotionScroll(int deltaY, int incrementalDeltaY) {
        ...
        if (sDbgMotion) {
            Xlog.d(TAG, "trackMotionScroll: deltaY = " + deltaY + ",mScrollY = " + mScrollY
                    + ",incrementalDeltaY = " + incrementalDeltaY + ",firstTop = " + firstTop
                    + ",lastBottom = " + lastBottom + ",height = " + height + ",childCount = "
                    + childCount + ",spaceAbove = " + spaceAbove + ",this = " + this);
        }       
        if (deltaY < 0) {
    ...
    class RecycleBin {
        ...
        void clear() {
            if (mViewTypeCount == 1) {
                ...
                for (int i = 0; i < typeCount; i++) {
                    ...
                    if (sDbgRecycle) {
                        Xlog.d(TAG, "Recycle clear end: i = " + i + ",scrap size = " + scrap.size()
                                + ",active size = " + mActiveViews.length + ",this = " + this);
                    }
                }
            }
            if (mTransientStateViews != null) {
        ...
        void fillActiveViews(int childCount, int firstActivePosition) {
            ...
            if (sDbgRecycle) {
                Xlog.d(TAG, "fillActiveViews: childCount = " + childCount
                        + ",firstActivePosition = " + firstActivePosition + ",this = "
                        + AbsListView.this);
            }  
        ...
        View getActiveView(int position) {
            ...
            if (index >=0 && index < activeViews.length) {
                ...
                if (sDbgRecycle) {
                    Xlog.d(TAG, "getActiveView success: position = " + position + ",index = "
                            + index + ",match = " + match + ",this = " + AbsListView.this);
                }
                return match;
            }
            return null;
        ...
        View getScrapView(int position) {
            if (sDbgRecycle) {
                Xlog.d(TAG, "getScrapView: position = " + position + ",mViewTypeCount = "
                        + mViewTypeCount + ",mCurrentScrap = " + mCurrentScrap + ",this = " + this);
            }
            if (mViewTypeCount == 1) {
        ...
        void addScrapView(View scrap, int position) {
            ...
            if (sDbgRecycle) {
                Xlog.d(TAG, "addScrapView: scrap = " + scrap + ",position = " + position
                        + ",child width = " + scrap.getMeasuredWidth() + ",height = "
                        + scrap.getMeasuredHeight() + ",mViewTypeCount = " + mViewTypeCount
                        + ",viewType = " + viewType + ",this = " + this);
            }
        ...
        void scrapActiveViews() {
            ...
            if (sDbgRecycle) {
                Xlog.d(TAG, "scrapActiveViews: mActiveViews = " + mActiveViews + ",hasListener = "
                        + hasListener + ",mViewTypeCount = " + mViewTypeCount + ",mCurrentScrap = "
                        + mCurrentScrap + ",count = " + count + ",this = " + this);
            }
            for (int i = count - 1; i >= 0; i--) {
        ...
        private void pruneScrapViews() {
            ...
            if (sDbgRecycle) {
                Xlog.d(TAG, "pruneScrapViews: maxViews = " + maxViews + ",viewTypeCount = "
                        + viewTypeCount + ",mScrapViews = " + mScrapViews + ",this = " + this);
            }
            for (int i = 0; i < viewTypeCount; ++i) {
        ...
        void reclaimScrapViews(List<View> views) {
            if (sDbgRecycle) {
                Xlog.d(TAG, "reclaimScrapViews: views = " + views + ",mViewTypeCount = "
                        + mViewTypeCount + ",mCurrentScrap = " + mCurrentScrap + ",this = " + this);
            }
            if (mViewTypeCount == 1) {
    ...
    static View retrieveFromScrap(ArrayList<View> scrapViews, int position) {
        ...
        if (sDbgRecycle) {
            Xlog.d(TAG, "retrieveFromScrap: size = " + size + ",position = " + position
                    + ",scrapViews = " + scrapViews);
        }
        if (size > 0) {
    ...
    /**
     * M: Set size of the fast scroller pop up overlay drawable, you have to call
     * it after setFastScrollEnabled(true) or set the fastScrollEnabled in xml,
     * otherwise the mFastScroller would be null and the value can not be set.
     * 
     * @param width the width of the pop up overlay drawable, -1 means
     * using the default overlay size as its width.
     * @param height the height of the pop up overlay drawable, -1 means
     * using the default overlay size as its height.
     * 
     * @hide
     */
    public void setFastScrollerPopupSize(final int width, final int height) {
        if (mFastScroller != null) {
            mFastScroller.setOverlayDrawableSize(width, height);
        } else {
            Xlog.i(TAG, "Can not set pop up size since FastScroller is not enabled.");
        }
    }
    
    /**
     * M: Set the size of text drawing in the pop up.
     * 
     * @param paintSize the size of the text paint.
     * 
     * @hide
     */
    public void setFastScrollerPopupTextSize(final int paintSize) {
        if (mFastScroller != null) {
            mFastScroller.setTextPaintSize(paintSize);
        } else {
            Xlog.i(TAG, "Can not set pop up text size since FastScroller is not enabled.");
        }
    }
    
    /**
     * M: Set the color of text drawing in the pop up.
     * 
     * @param paintColor the color of the text paint.
     * 
     * @hide
     */
    public void setFastScrollerPopupTextColor(final int paintColor) {
        if (mFastScroller != null) {
            mFastScroller.setTextPaintColor(paintColor);
        } else {
            Xlog.i(TAG, "Can not set pop up text color since FastScroller is not enabled.");
        }
    }
    
    /**
     * M: Check whether ViewRoot logs is enabled.
     * 
     * @return true if the debug property is 1, else return false.
     */
    private static void checkAbsListViewlLogProperty() {
        String dumpString = SystemProperties.get(LOG_PROPERTY_NAME);
        if (dumpString != null) {
            if (dumpString.length() <= 0 || dumpString.length() > 6) {
                Xlog.d(TAG, "checkAbsListViewlLogProperty get invalid command");
                return;
            }
            int logFilter = 0;
            try {
                ...
                Xlog.w(TAG, "Invalid format of propery string: " + dumpString);
            }
            sDbgMotion = (logFilter & DBG_MOTION_FLAG) == DBG_MOTION_FLAG;
            sDbgDataChange = (logFilter & DBG_DATACHANGE_FLAG) == DBG_DATACHANGE_FLAG;
            sDbgLayout = (logFilter & DBG_LAYOUT_FLAG) == DBG_LAYOUT_FLAG;
            sDbgRecycle = (logFilter & DBG_RECYCLE_FLAG) == DBG_RECYCLE_FLAG;
            sDbgDraw = (logFilter & DBG_DRAW_FLAG) == DBG_DRAW_FLAG;
            sDbg = (logFilter & DBG_FLAG) == DBG_FLAG;
            Xlog.d(TAG, "checkAbsListViewlLogProperty debug filter: " +
                    "sDbgMotion=" + sDbgMotion +
            	    ", sDbgDataChange=" + sDbgDataChange +
            	    ", sDbgLayout=" + sDbgLayout +
            	    ", sDbgRecycle=" + sDbgRecycle +
            	    ", sDbgDraw=" + sDbgDraw +
            	    ", sDbg=" + sDbg);
        }
    }
