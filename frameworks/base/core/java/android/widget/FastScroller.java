...
class FastScroller {
        ...
        if (mState == STATE_DRAGGING && mDrawOverlay) {
            if (mOverlayPosition == OVERLAY_AT_THUMB) {
                ...
                switch (mPosition) {
                    ...
                    case View.SCROLLBAR_POSITION_RIGHT:
                        left = Math.max(0,
                                mThumbDrawable.getBounds().left - mThumbW - mOverlayWidth);
                        break;
                    case View.SCROLLBAR_POSITION_LEFT:
                        left = Math.min(mThumbDrawable.getBounds().right + mThumbW,
                                mList.getWidth() - mOverlayWidth);
                        break;
                }

                int top = Math.max(0,
                        Math.min(y + (mThumbH - mOverlayWidth) / 2, mList.getHeight() - mOverlayHeight));
            ...
            canvas.drawText(mSectionText, (int) (rectF.left + rectF.right) / 2 - hOff,
                    (int) (rectF.bottom + rectF.top) / 2 - (asent + descent) / 2 - vOff,
                    paint);
    ...
    private int getThumbPositionForListPosition(int firstVisibleItem, int visibleItemCount,
        ...
        /// M: We modify the result computation algorithm(default algorithm is:
        /// int result = (int) ((section + posWithinSection) / sectionCount * trackHeight);
        /// the problem is (section + posWithinSection) sometimes may be larger than selectionCount), 
        /// in this case, result may exceed the track bottom @{
        float selectionOffset = section + posWithinSection;
        if (selectionOffset > sectionCount) {
            selectionOffset = sectionCount;
        }
        /// @}
        int result = (int) (selectionOffset / sectionCount * trackHeight);
    ...
    /**
     * Set the size of the overlay drawable.
     * 
     * @param width  the width of the overlay drawable.
     * @param height  the height of the overlay drawable.
     */
    void setOverlayDrawableSize(final int width, final int height) {
        if (width == -1) {
            mOverlayWidth = mOverlaySize;
        } else {
            mOverlayWidth = width;
    ...
    /**
     * Set the size of the overlay text paint.
     * 
     * @param paintSize the size of the paint.
     */
    void setTextPaintSize(final int paintSize) {
        mPaint.setTextSize(paintSize);
    }
    
    /**
     * Set the color of the overlay text paint.
     * 
     * @param paintColor the color of the paint.
     */
    void setTextPaintColor(final int paintColor) {
    ...
    /**
     * M: Reset the draging state if fast scroller is in dragging state, AbsListView will call this api to
     * reset fast scroller when window focus change, which cause fast scroller's can not hide. ALPS00402306
     */
