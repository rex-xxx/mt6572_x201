...
public class StackView extends AdapterViewAnimator {
    ...
    public StackView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (FeatureOption.MTK_THEMEMANAGER_APP) {
            int themeColor = context.getResources().getThemeMainColor();
            if (themeColor != 0) {
                mResOutColor = themeColor;
                mClickColor = themeColor;
            }
        }
        ///M @}
        initStackView();
    ...
    void transformViewForTransition(int fromIndex, int toIndex, final View view, boolean animate) {
        if (DBG_SHOW) {
            Xlog.d(TAG, "transformViewForTransition: fromIndex = " + fromIndex + ",toIndex = "
    ...
    private void transformViewAtIndex(int index, final View view, boolean animate) {
        ...
        if (DBG_SHOW) {
            Xlog.d(TAG, "transformViewAtIndex 1: index = " + index + ",child index = "
        ...
        if (DBG_SHOW) {
            Xlog.d(TAG, "transformViewAtIndex 2: r = " + r + ",scale = " + scale
    ...
    void showOnly(int childIndex, boolean animate) {
        ...
        if (DBG) {
            Xlog.d(TAG, "showOnly: childIndex = " + childIndex + ",mCurrentWindowStart = "
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (DBG_SHOW) {
            Xlog.d(TAG, "StackFrame dispatchDraw: stackInvalidateRect = " + stackInvalidateRect
    ...
    private void onLayout() {
        ...
        if (DBG_SHOW) {
            Xlog.d(TAG, "onLayout: mFirstLayoutHappened = " + mFirstLayoutHappened
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if (DBG) {
            Log.d(TAG, "onInterceptTouchEvent end: action = " + action + ",x = " + ev.getX() + ",y = "
                    + ev.getY() + ",mSwipeGestureType = " + mSwipeGestureType);
        }
    ...
    private void beginGestureIfNeeded(float deltaY) {
        if ((int) Math.abs(deltaY) > mTouchSlop && mSwipeGestureType == GESTURE_NONE) {
            ...
            if (DBG_MOTION) {
                Xlog.d(TAG, "beginGestureIfNeeded: deltaY = " + deltaY + ",mSwipeGestureType = " + mSwipeGestureType
                        + ",swipeGestureType = " + swipeGestureType + ",stackMode = " + stackMode);
            }
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        if (DBG_MOTION) {
            Xlog.d(TAG, "onTouchEvent: action = " + action + ",x = " + newX + ",y = "
