...
public class TaskStackBuilder {
    ...
    public Intent[] getIntents() {
        ...
        /// M: Fix CMCC issues ALPS00436513
        /// If AP is MMS, we do not add the flag FLAG_ACTIVITY_TASK_ON_HOME. @{
        intents[0] = new Intent(mIntents.get(0));
        String processName = mSourceContext.getApplicationInfo().processName;

        intents[0].addFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TASK);
        if (!processName.equals("com.android.mms")) {
            intents[0].addFlags(Intent.FLAG_ACTIVITY_TASK_ON_HOME);
        }
        /// @}
