...
///M: ALPS00347366 timing issue, conn.unstableCount<0
...
public final class ActivityThread {
    ...
    /// M: 2012-04-04 Log for debugging
    public static boolean mIsUserBuild = false;
    /// M: ALPS00497111 [Volunteer free] LCA Memory Optimized
    static boolean is_trim = false;
    ...
    ///M: ALPS00347366 timing issue, conn.unstableCount<0
    final HashSet<ProviderRefCount> mRemoveProviders = new HashSet<ProviderRefCount>(); 
    ...
    private class ApplicationThread extends ApplicationThreadNative {
        ...
        public void setTrimMemoryFlagEx() {
            queueOrSendMessage(H.SET_TRIMMEM_FLAG_EX, null);
        }
        
        public void ResetSL() {
            queueOrSendMessage(H.RESET_SL, null);
        }
        /// @}
    ...
    private class H extends Handler {
        ...
        public static final int SET_TRIMMEM_FLAG_EX            = 143; /// M: ALPS00497111 [Volunteer free] LCA Memory Optimized
        public static final int RESET_SL            = 144;     /// M: ALPS00497111 [Volunteer free] LCA Memory Optimized
        String codeToString(int code) {
            ...
            if (DEBUG_MESSAGES || !mIsUserBuild) {
                switch (code) {
                    ...
                    case CONFIGURATION_CHANGED: return "ACT-CONFIGURATION_CHANGED";
                    ...
                    case SET_TRIMMEM_FLAG_EX: return "SET_TRIMMEM_FLAG_EX";  /// M: ALPS00497111 [Volunteer free] LCA Memory Optimized
                    case RESET_SL: return "RESET_SL";          /// M: ALPS00497111 [Volunteer free] LCA Memory Optimized
                }
            }
        ...
        boolean isDebuggableMessage(int code){
            switch (code) {
                 ...
                 case ACTIVITY_CONFIGURATION_CHANGED: return false;
                 ...
                 case SET_TRIMMEM_FLAG_EX: return false; /// M: ALPS00497111 [Volunteer free] LCA Memory Optimized
                 case RESET_SL: return false;     /// M: ALPS00497111 [Volunteer free] LCA Memory Optimized
                 default:
                    return false;
             }
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case UNSTABLE_PROVIDER_DIED:
                    ...
                    break;
                /// M: ALPS00497111 [Volunteer free] LCA Memory Optimized @{
                case SET_TRIMMEM_FLAG_EX:
                    Trace.traceBegin(Trace.TRACE_TAG_ACTIVITY_MANAGER, "setTrimMemoryFlagEx");
                    handlesetTrimMemoryFlagEx();
                    Trace.traceEnd(Trace.TRACE_TAG_ACTIVITY_MANAGER);
                    break;
                case RESET_SL:
                    Trace.traceBegin(Trace.TRACE_TAG_ACTIVITY_MANAGER, "reset_sl");
    ...
    public final LoadedApk getPackageInfo(ApplicationInfo ai, CompatibilityInfo compatInfo,
        ...
        boolean securityViolation = includeCode && ai.uid != 0
                && ai.uid != Process.SYSTEM_UID && (mBoundApplication != null
                        ...
                        : true);
        Slog.d(TAG, "getPackageInfo: securityViolation=" + securityViolation);
    ...
    final void handleTrimMemory(int level) {
        ...
        /// M: ALPS00497111 [Volunteer free] LCA Memory Optimized
        if(FeatureOption.MTK_LCA_RAM_OPTIMIZE && is_trim)
        {
            // Ask SQLite to free up as much memory as it can, mostly from its page caches.
            if (Process.myUid() != Process.SYSTEM_UID) {
                int sqliteReleased = SQLiteDatabase.releaseMemory();
            ...
            // Ask text layout engine to free also as much as possible
            Canvas.freeTextLayoutCaches();
            BinderInternal.forceGc("trimmem");
            is_trim = false;
        }
        /// @}
    }
    ...
    private void handleBindApplication(AppBindData data) {
        ...
        if (FeatureOption.MTK_LCA_RAM_OPTIMIZE) {
            try {
                ActivityManagerNative.getDefault().applicationBinded(Process.myPid());
            } catch (RemoteException ex) {
            }
        }
    }
        ...
        if (stable) {
            ...
            if (prc.stableCount == 1) {
                ...
                if (prc.removePending) {
                    ...
                    ///M: ALPS00347366 timing issue, conn.unstableCount<0
                    /// use mRemoveProviders to judge whether the message is executed, 
                    /// not remove message from handler directly in order to avoid timing issue. @{
                    if (mRemoveProviders.contains(prc)) {
                        mRemoveProviders.remove(prc);
                        unstableDelta = -1;
            ...
            if (prc.unstableCount == 1) {
                ...
                ///M: whether !prc.removePending or !mRemoveProviders.contains(prc), execute below action 
    ...
    final void completeRemoveProvider(ProviderRefCount prc) {
        synchronized (mProviderMap) {
            ///M: ALPS00347366 timing issue, when prc is not removed, the
            /// message just execute.
            if (mRemoveProviders.contains(prc)) {
                mRemoveProviders.remove(prc);
            } else {
                return ;
    ...
    private IActivityManager.ContentProviderHolder installProvider(Context context,
        ...
        Slog.d(TAG, "installProvider: context=" + context + "holder=" + holder + "noisy=" + noisy + "noReleaseNeeded=" + noReleaseNeeded + "stable=" + stable);
        if (holder == null || holder.provider == null) {
            ...
            Slog.d(TAG, "installProvider: context.getPackageName()=" + context.getPackageName() + "ai.packageName=" + ai.packageName);
    ...
    private final class QueryHistory {
        private  final boolean CURSOR_LEAK_DETECTER = Log.isLoggable("Cursor_Leak_Detecter", Log.VERBOSE);

        private Map<Integer,QueryHistoryRecord> recordMap = new HashMap<Integer,QueryHistoryRecord>();
        private Map<String,Integer> uriMap = new HashMap<String,Integer>();
         ...
         * @param cursorHashCode the hashcode of the CursorWrapperObjectInner Object
         * @hide
         */
        public boolean Add(String uri, Throwable stackTrace, int cursorHashCode) {
            synchronized(recordMap) {           
                ...
                if(recordMap.get(cursorHashCode) == null) {
                    QueryHistoryRecord qhr = new QueryHistoryRecord(uri, stackTrace);
                    recordMap.put(cursorHashCode,qhr);
                }

                if(CURSOR_LEAK_DETECTER) {
                    Log.v("CursorLeakDetecter", "Cursor Open:"+cursorHashCode+" Total Opened Cursor Count:"+recordMap.size()+".");
                }

                if(recordMap.size() == 50 || recordMap.size() == 60) {
                    Log.v("CursorLeakDetecter", "Total Opened Cursor Count:"+recordMap.size()+".");
                    Dump();

                    /// M: For cursor leak aee warning 
                    /// M: Only show red screen warning for resmon monitored applications @{
                    if (checkAeeWarningList()) { 
                    	try {
                            IExceptionLog exceptionLog = MediatekClassFactory.createInstance(IExceptionLog.class);
                            if (exceptionLog != null) {
                                //exceptionLog.handle("aee_warning Total Opened Cursor Count >=50", "", "");
                                exceptionLog.systemreport(exceptionLog.AEE_WARNING_JNI, "CursorLeakDetecter", "Total Opened Cursor Count:"+recordMap.size()+".", "/data/cursorleak/traces.txt");
                            }
                        }
                        catch (Exception e) {
        ...
        public void Remove(int cursorHashCode) {
            synchronized(recordMap) {
                QueryHistoryRecord qhr = recordMap.get(cursorHashCode);
                if(qhr == null || uriMap.get(qhr.mUri) == null) {
                    Log.e("CursorLeakDetecter", "bad request for cursor:"+cursorHashCode+".");
                    return;
                }else if(uriMap.get(qhr.mUri) == 1) {
                    uriMap.remove(qhr.mUri);
                    recordMap.remove(cursorHashCode);
                }else if(uriMap.get(qhr.mUri) > 1) {
                    uriMap.put(qhr.mUri,uriMap.get(qhr.mUri)-1);
                    recordMap.remove(cursorHashCode);
                }       

                if(CURSOR_LEAK_DETECTER) {
                    Log.v("CursorLeakDetecter", "Cursor Close:"+cursorHashCode+" Total Opened Cursor Count:"+recordMap.size()+".");
                }
            }
        }
        ...
        public void Dump() {
            ...
            while(iterator.hasNext()) {
                ...
                Log.v("CursorLeakDetecter", "CursorQueryHistory:"+qhr.mUri, qhr.mStackTrace);
            }
        }
    }
     ...
     * @hide
     */
    public boolean AddToQueryHistory(String uri, Throwable stackTrace, int cursorHashCode) {
        try{
            return mQueryHistory.Add(uri, stackTrace, cursorHashCode);
        }catch(Exception e){
            Log.e("CursorLeakDetecter", "AddToQueryHistory",e);
        }
        return true;
    }
     ...
     * @hide
     */
    public void RemoveFromQueryHistory(int cursorHashCode) {
        try {
            mQueryHistory.Remove(cursorHashCode);
        }catch(Exception e) {
            Log.e("CursorLeakDetecter", "AddToQueryHistory",e);
        }       
    }
