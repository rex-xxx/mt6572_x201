...
public class Resources {
    ...
    public Drawable getDrawable(int id) throws NotFoundException {
        //Theme manager +
        if (sPreloadedDrawables.size() == 0 && !mPreloading) {
            Log.e(TAG, "sPreloadedDrawables size is 0, reload preloaded resources.");
            loadPreloadedDrawableResources();
        }
        //Theme manager -
        synchronized (mTmpValue) {
            TypedValue value = mTmpValue;
    ...
    public ColorStateList getColorStateList(int id) throws NotFoundException {
        //Theme manager +
        if (sPreloadedColorStateLists.size() == 0 && !mPreloading) {
            Log.d(TAG, "sPreloadedDrawables size is 0, reload preloaded resources.");
            loadPreloadedColorResources();     
        }
        //Theme manager -
        synchronized (mTmpValue) {
            TypedValue value = mTmpValue;
    ...
    public void updateConfiguration(Configuration config,
        ...
        synchronized (mTmpValue) {
            ...
            //For theme manager
            boolean needClearPreloader = false;
            if (config != null) {
                ...
                if (mConfiguration.skin == null) {
                    mConfiguration.skin = Configuration.getSkin();
                }
                if (config.skin == null) {
                    config.skin = Configuration.getSkin();
                }
                if (config.skin == null || !mConfiguration.skin.equals(config.skin)) {
                    needClearPreloader = true;
                }
            ...
            if (needClearPreloader) {
                Log.d(TAG, "updateConfiguration from skin change, clear preloaded drawables.");
                sPreloadedDrawables.clear();
                sPreloadedColorStateLists.clear();
                mDrawableCache.clear();
                mColorDrawableCache.clear();
                Log.d("TAG", "updateConfiguration() mMtkColorCache.size = " + mMtkColorCache.size());
                mMtkColorCache.clear();
            }
   ...
   /**
     ...
     * @return the color value for the current theme, if the current theme is the
     * default theme, or the colorName is not present in the color.xml, return 0.
     * @hide
     * @internal
     */
    public int getThemeColor(String colorName) {
        ...
        // Add theme path to asset path to access it, if add asset path failed, return 0 directly.
    ...
    /**
     * Get highlight text color for current skin.
     * @hide
     * @internal
     */
    public int getThemeHighlightColor() {
        return getThemeColor("theme_highlight_color");
    }
    /**
     * Load preloaded drawable resources.
     */
    private void loadPreloadedDrawableResources() {
        try {
            sPreloaded = false;
            startPreloading();
            TypedArray ar = obtainTypedArray(com.android.internal.R.array.preloaded_drawables);
            int N = ar.length();
            for (int i = 0; i < N; i++) {
                int id = ar.getResourceId(i, 0);
                Log.v(TAG, "Preloading resource #" + Integer.toHexString(id) + "(" + ar.getString(i) + ")" );
                if (id != 0) {
                    Drawable dr = getDrawable(id);
                    if ((dr.getChangingConfigurations() & ~ActivityInfo.CONFIG_FONT_SCALE) != 0) {
                        Log.w(TAG, "Preloaded drawable resource #0x" + Integer.toHexString(id)
            ...
            Log.w(TAG, "Failure loading preloaded resources.", e);
        }
    }
    
    /**
     * Load preloaded state color resources.
     */
    private void loadPreloadedColorResources() {
        try {
            sPreloaded = false;
            startPreloading();
            TypedArray ar = obtainTypedArray(com.android.internal.R.array.preloaded_color_state_lists);
            int N = ar.length();
            N = ar.length();
            for (int i = 0; i < N; i++) {
                int id = ar.getResourceId(i, 0);
    ...
    /**
     * Clear all preloaded caches and reload them.
     * @hide
     * @internal
     */
    public void reloadPreloadedResources() {
        Log.d(TAG, "Reload all preloader drawables in Resources.java.");
        sPreloadedDrawables.clear();
        sPreloadedColorStateLists.clear();
        Log.d("TAG", "reloadPreloadedResources() mMtkColorCache.size = " + mMtkColorCache.size());
        mMtkColorCache.clear();
        loadPreloadedDrawableResources();
        loadPreloadedColorResources();
    }
    ...
    /**
     * Get theme preview bitmap of the specific theme.
     * 
     * @param pkgPath package path of the theme.
     * @return the theme preview bitmap of the theme.
     * @hide
     * @internal
     */
    public Bitmap getThemePreview(String pkgPath) {
        return getThemeImage(pkgPath, STR_TYPE_THEME_PREVIEW);
    }
    

    /**
     * Get theme preview bitmap of the specific theme.
     * @param themePackagePath
     * @param itemType
     * @return
     * @hide
     * @internal
    ...
    public Bitmap getThemeImage(String themePackagePath, String itemType) {
        ...
        if (themePackagePath != null && (themePackagePath.equals(DEFAULT_THEME_PATH) 
                || themePackagePath.equals(""))) {
            Log.i(TAG, "The current theme is the default theme, themePackagePath = " + themePackagePath);
            return null;
        }

        cookie = getAssets().addAssetPath(themePackagePath);   
        if (DEBUG_THEME) {
            Log.d(TAG, "getThemeImage: itemType = " + itemType + ",themePackagePath = " + 
                    themePackagePath + ",cookie = " + cookie);
        }
        ...
        if (DEBUG_THEME) {
            Log.d(TAG, "getThemeImage themeDrawablePath = " + themeDrawablePath);
        } 
    ...
    /**
     * Generate the resource path suffix depending on the current density.
     * 
     * @return resource path suffix
     */
    private String generateCurrentDensitySuffix() {
        int density = mMetrics.densityDpi;        
        Log.d(TAG, "ThemeManager density= " + density);
        ...
        Log.d(TAG, "ThemeManager densityString= " + densityString);
    ...
    private static final String STR_TYPE_THEME_PREVIEW = "previewtheme";

    private static final boolean DEBUG_THEME = false;
    
    //For getThemeColor add cache.
    private static HashMap<String,Integer> mMtkColorCache
