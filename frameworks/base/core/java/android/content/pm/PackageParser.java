...
public class PackageParser {
    ...
    */
    ...
    private static PackageLite parsePackageLite(Resources res, XmlPullParser parser,
        ...
        while ((type = parser.next()) != XmlPullParser.END_DOCUMENT
            ...
            if (parser.getDepth() == searchDepth) {
                ...
                /** M: [ALPS00407804][Need Patch][Volunteer Patch] (LCA project only) Overwrite the default install location from PREFER_INTERNAL to PREFER_EXTERNAL
                  *    [ALPS00417120][Rose][Auto Test][JBLCA][APPIOT][My Paper Plane 2][JE]happen JE on power on "android" logo screen
                  *    [ALPS00417275][Side Effect][Rose][6577JB][JBLCA][CTS][JE]The JE about system_server pops up continually when CTS is running.
                  *    [ALPS00412894][Rose][6577JB][JBLCA][JE][CTS Verifier 4.1_r2]The JE and "CTS Verifier has stopped" pops up when you tap "Force Lock"(Always) 
                  *    [ALPS00486870][ALPS.JB.LCA.MP.V1.12 Regression Test][Free test][3D APP] 3D Digital Weather Clock can't display in Widget tab.
                  *
                  * Related files: 
                  ...
                  * @{
                  **/
                if (com.mediatek.common.featureoption.FeatureOption.MTK_LCA_ROM_OPTIMIZE) {
                if ("application".equals(parser.getName())) {
                    switch (installLocation) {
                        ...
                        case PackageInfo.INSTALL_LOCATION_UNSPECIFIED:
                            if (checkDeviceAdmin(parser, attrs, outError)) {
    ...
    /** M: [ALPS00417120][Rose][Auto Test][JBLCA][APPIOT][My Paper Plane 2][JE]happen JE on power on "android" logo screen
      *    [ALPS00417275][Side Effect][Rose][6577JB][JBLCA][CTS][JE]The JE about system_server pops up continually when CTS is running.
      *    [ALPS00412894][Rose][6577JB][JBLCA][JE][CTS Verifier 4.1_r2]The JE and "CTS Verifier has stopped" pops up when you tap "Force Lock"(Always) @{
      **/
    static private boolean checkDeviceAdmin(XmlPullParser parser, AttributeSet attrs, String[] outError) throws IOException, XmlPullParserException {
        boolean ret = false;
        int type;
        final int searchDepth = parser.getDepth() + 1;
        while ((type = parser.next()) != XmlPullParser.END_DOCUMENT && 
    ...
    private boolean parseApplication(Package owner, Resources res,
         ...
         */
        if (com.mediatek.common.featureoption.FeatureOption. MTK_LCA_RAM_OPTIMIZE) {
            Slog.i(TAG, "[swRender] " + pkgName + ", hwui-ori" + hardwareAccelerated + ", sdk-check: " + (owner.applicationInfo.targetSdkVersion >= Build.VERSION_CODES.ICE_CREAM_SANDWICH));
            if (owner.applicationInfo.targetSdkVersion >= Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
                do {
                    if( pkgName.equals("android") || 
                        ...
                        Slog.i(TAG, "[swRender] " + pkgName + "skip, hwui-ori: " + hardwareAccelerated);
                        break;
                    }
                    ...
                    if ((flags & PARSE_IS_SYSTEM) != 0) {
                        hardwareAccelerated = sa.getBoolean(com.android.internal.R.styleable.AndroidManifestApplication_hardwareAccelerated, false);
                    }
                } while (false);
        }
            Slog.i(TAG, "[swRender] " + pkgName + ", system-app: " + ((flags&PARSE_IS_SYSTEM) != 0) + ", hwui-res: " + hardwareAccelerated);   
        }
        /** @} 2013-03-21 */
