...
public abstract class WallpaperService extends Service {
    ...
    /// M: fix a IPO related tablet issue ALPS00268509, judge TABLET build, if yes then register ACTION_BOOT_IPO. @{
    private static final String ACTION_BOOT_IPO = "android.intent.action.ACTION_PREBOOT_IPO";
    private static final String BUILD_CHARACTERISTICS = "ro.build.characteristics";
    ...
    /** M: judge wethear it's a TABLET build. */
    private boolean isTabletBuild() {
        return TABLET.equals(SystemProperties.get(BUILD_CHARACTERISTICS));
    }
    
    /**
     * M: to fix a Tablet IPO issue, a walk-round method to treate it as SCREEN_ON;
     * mReceiver was moved out Engine to fix a seldom JE. @{
     */
    private final BroadcastReceiver mReceiver = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            final String action = intent.getAction();
            final int size = mActiveEngines.size();
            boolean isScreenOn = false;
            if (Intent.ACTION_SCREEN_ON.equals(action)) {
                isScreenOn = true;
            } else if (Intent.ACTION_SCREEN_OFF.equals(action)) {
                isScreenOn = false;
            } else if (ACTION_BOOT_IPO.equals(action) && isTabletBuild()) {
                isScreenOn = true;
            }
            for (int i = 0; i < size; i++) {
                mActiveEngines.get(i).doScreenOnOff(isScreenOn);
            }
        }
    };
    /** @} */
    ...
    public class Engine {
        ...
        /**
         * M: the mReceiver that listen to ACTION_SCREEN_ON and ACTION_SCREEN_OFF was moved from 
         * Inner Class to Outer Class to fix a always JE, then register and unregister were done
         * in WallpaperService's life circle.
         */
        ...
        final BaseSurfaceHolder mSurfaceHolder = new BaseSurfaceHolder() {
            ...
            public void setFixedSize(int width, int height) {
                /// M: to support MTK Video Live Wallpaper.
        ...
        /// M: extract this method for moving ShutDown BroadCastReceiver out of the Inner Class. @{
        void doScreenOnOff(boolean on) {
            mScreenOn = on;
            reportVisibility();
        }
        /// @}
    ...
    /**
     * M: move Receiver from Inner Class to Outer Class to fix a always JE.
     * register and unregister it in Service's Life Circle. @{
     */
    @Override
