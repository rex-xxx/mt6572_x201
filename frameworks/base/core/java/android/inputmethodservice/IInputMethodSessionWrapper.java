...
class IInputMethodSessionWrapper extends IInputMethodSession.Stub
    ...
    public void executeMessage(Message msg) {
        if (mInputMethodSession == null) {
            /*
             * Add by Mediatek.
             * When active IME be changed or be killed,
             *     1. IMMS told IMS to finish the session then mInputMethodSession would be set as null.
             *     2. IMMS send message to IMM to inform the change and then IMM would update mInputMethodSession to new one.
             *   There are a time duration between step 1 and step2.
             *  It would cause one kind of error that IMM think that the session between IMS is valid and send the event to prevois IMS.
             *  But IMS already clear the session. Then error happens.
             *
             *  Mediatek add the workaround to avoid the error on the non-sync state.
             *  In the non-sync duration, we inform view root the key event is finished without IME processed.
             *
             *  Although all function call from IMM have such kind of error, we only fix the key event process first.
             *  If key event would be blocked by IME, it would cause ANR. But other function don't have such kind of issue.
             *
             */

            final int what = msg.what;
            if (what == DO_DISPATCH_KEY_EVENT || what == DO_DISPATCH_TRACKBALL_EVENT) {
                Log.d(TAG, "executeMessage dispatch event, mInputMethodSession == null");
                final SomeArgs args = (SomeArgs)msg.obj;
                InputMethodEventCallbackWrapper callback = new InputMethodEventCallbackWrapper((IInputMethodCallback)args.arg2);
                callback.finishedEvent(msg.arg1, false);
            }
