...
class WebCoreThreadWatchdog implements Runnable {
    ...
    /// M: Add XLOGTAG.
    static final String XLOGTAG = "webkit/webcorewatchdog";
    ...
    private void pauseWatchdog() {
        ...
        Xlog.d(XLOGTAG, "remove timed_out message: in pauseWatchdog");
    ...
    private void resumeWatchdog() {
        ...
        Xlog.d(XLOGTAG, "reset timed_out message: in resumeWatchdog");
    }
                ...
                public void handleMessage(Message msg) {
                    ...
                    case IS_ALIVE:
                        synchronized(WebCoreThreadWatchdog.class) {
                            ...
                            Xlog.d(XLOGTAG, "remove timed_out message: in createHandler->IS_ALIVE");
                            sendMessageDelayed(obtainMessage(TIMED_OUT), TIMEOUT_PERIOD);
                            Xlog.d(XLOGTAG, "reset timed_out message: in createHandler->IS_ALIVE");
                        ...
                        synchronized (WebCoreThreadWatchdog.class) {
                            ...
                            while (it.hasNext()) {
                                ...
                                if (activeView.getWindowToken() != null &&
                                    ...
                                    Xlog.d(XLOGTAG, "receive timed_out message: in createHandler->TIMED_OUT");
                            ...
                            if (!postedDialog) {
                                ...
                                Xlog.d(XLOGTAG, "reset timed_out message: in createHandler->TIMED_OUT");
                            }
                        }
                        break;
                    }
        ...
        synchronized (WebCoreThreadWatchdog.class) {
            if (!mPaused) {
                mWebCoreThreadHandler.obtainMessage(EventHub.HEARTBEAT,
                        mHandler.obtainMessage(IS_ALIVE)).sendToTarget();
                mHandler.sendMessageDelayed(mHandler.obtainMessage(TIMED_OUT), TIMEOUT_PERIOD);
                Xlog.d(XLOGTAG, "reset timed_out message: in createHandler thread run");
            }
        }
                            ...
                            new DialogInterface.OnClickListener() {
                                ...
                                public void onClick(DialogInterface dialog, int which) {
                                    ...
                                    mWatchdogHandler.sendMessageDelayed(
                                            ...
                                            SUBSEQUENT_TIMEOUT_PERIOD);
                                    Xlog.d(XLOGTAG, "reset timed_out message: " + 
                                            "in PageNotRespondingRunnable thread run1");
                                }
                            })
                    .setOnCancelListener(
                            new DialogInterface.OnCancelListener() {
                                ...
                                public void onCancel(DialogInterface dialog) {
                                    mWatchdogHandler.sendMessageDelayed(
                                            ...
                                            SUBSEQUENT_TIMEOUT_PERIOD);
                                    Xlog.d(XLOGTAG, "reset timed_out message: " + 
                                            "in PageNotRespondingRunnable thread run2");
                                }
                            })
