...
/** M: for log and exception handler */
...
class ByteArrayBuilder {
    ...
    /** M: xlog tag definition */
    private static final String XLOGTAG = "webkit/ByteArrayBuilder";
    ...
    public synchronized void append(byte[] array, int offset, int length) {
        /** M: add log */
        Xlog.d(XLOGTAG, "ByteArrayBuilder append length: " + length);
        while (length > 0) {
            Chunk c = null;
            if (mChunks.isEmpty()) {
    ...
    public synchronized int getByteSize() {
        ...
        /** M: add log */
        Xlog.d(XLOGTAG, "ByteArrayBuilder mChunks.size(): " + mChunks.size());
        return total;
    }

    public synchronized void clear() {
        /** M: add log */
        Xlog.d(XLOGTAG, "ByteArrayBuilder clear()");
        Chunk c = getFirstChunk();
        while (c != null) {
            /** M: add log */
            Xlog.d(XLOGTAG, "ByteArrayBuilder clear() chunk release:" + c);
            c.release();
            c = getFirstChunk();
        }
    }
    ...
    public static class Chunk {
        ...
        public Chunk(int length) {
            /** M: add log */
            Xlog.d(XLOGTAG, "ByteArrayBuilder Chun New length: " + length);
        ...
        public void release() {
            ...
            synchronized (sPool) {
                ...
                // be gc'd if needed.
                /** M: add log */
                Xlog.d(XLOGTAG, "ByteArrayBuilder Chun Relese this:" + this);
