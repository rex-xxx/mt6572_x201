...
public class WebSettingsClassic extends WebSettings {
    ...
    /// M: add XLOGTAG.
    static final String XLOGTAG = "WebSettingsClassic";
    ...
    private boolean         mUsesEncodingDetector;
    ...
    private NotificationState mNotificationState = NotificationState.ON;
    private String          mWebnotificationDatabasePath = "";
    private boolean         mWebnotificationEnabled = false;
    /// @ }
    ...
    /** M: auto-detector string */
    private static final String AUTO_DETECTOR = "auto-detector";
    ...
    WebSettingsClassic(Context context, WebViewClassic webview) {
        ...
        mContext = context;
        mWebView = webview;
        /** M: init default text encoding and encoding detector @{ */
        mDefaultTextEncoding = AUTO_DETECTOR;
        mUsesEncodingDetector = true;
        /** @} */
    ...
    public void setDoubleTapZoom(int doubleTapZoom) {
        if (mDoubleTapZoom != doubleTapZoom) {
            /// M: For browser settings, WebViewCore maybe destroyed early. @{
            if (mWebView.getSettings() == null) {
                Xlog.d(XLOGTAG, "setDoubleTapZoom mWebViewCore = null");
                return;
            }
            /// @}
    ...
    public void setDefaultZoom(ZoomDensity zoom) {
        if (mDefaultZoom != zoom) {
            /// M: For browser settings, WebViewCore maybe destroyed early. @{
            if (mWebView.getSettings() == null) {
                Xlog.d(XLOGTAG, "setDefaultZoom mWebViewCore = null");
                return;
            }
            /// @}
            mDefaultZoom = zoom;
    ...
    public synchronized void setDefaultTextEncodingName(String encoding) {
        if (encoding != null && !encoding.equals(mDefaultTextEncoding)) {
            ...
            /** M: set the encoding detector */
            mUsesEncodingDetector = mDefaultTextEncoding.equals(AUTO_DETECTOR);
            postSync();
        }
    }
    ...
    public void setNotificationState(NotificationState notificationState) {
        ...
        if (mNotificationState != notificationState) {
    ...
    @Override
    public void setWebNotificationDatabasePath(String path) {
        if (!FeatureOption.MTK_WEB_NOTIFICATION_SUPPORT) {
            return;
        }

        if (path != null && !path.equals(mWebnotificationDatabasePath)) {
            mWebnotificationDatabasePath = path;
            postSync();
        }
        return;
    }
    ...
    @Override
    public void setWebnotificationEnabled(boolean enable) {
        if (!FeatureOption.MTK_WEB_NOTIFICATION_SUPPORT) {
            return;
        }

        if (mWebnotificationEnabled != enable) {
            mWebnotificationEnabled = enable;
            postSync();
        }
        return;
    }
