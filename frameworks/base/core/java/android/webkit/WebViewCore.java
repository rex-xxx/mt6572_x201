...
/// M: HTML5: Network Information API
...
public final class WebViewCore {
    ...
    /// M: Add XLOGTAG.
    private static final String XLOGTAG = "webkit/webcore";
    ...
    /// M: Adjust scale. If web page isn't user scalable and initial scale is < 1.0f for mobile site.
    private float mAdjust = 1.0f;
    ...
    private void initialize() {
        ...
        if (FeatureOption.MTK_WEB_NOTIFICATION_SUPPORT) {
            NotificationPermissionsClassic.getInstance().createHandler();
        }
    ...
    private String openFileChooser(String acceptType, String capture) {
        ...
        if (uri != null) {
            ...
            /// M: change the content type URI to file type to make the file accessible in native @{
            if (uriString.startsWith("content://")) {
                int index = filePath.lastIndexOf("/");
                String newUriString = filePath.startsWith("file://") ? filePath : "file://" + filePath;
                String newFilePath = filePath.substring(index + 1);
                BrowserFrame.sJavaBridge.storeFilePathForContentUri(newFilePath, newUriString);
                return newUriString;
            } else {
    ...
    /// M: HTML5 web notification @ {
    protected void notificationManagerCancel(int id) {
        if (!FeatureOption.MTK_WEB_NOTIFICATION_SUPPORT) {
            return;
        }

        Xlog.d(LOGTAG, "notificationManagerCancel, id=" + id);
        mCallbackProxy.onNotificationManagerCancelPrompt(id);
    }

    protected void notificationManagerShow(String icon, String title, String body, int counter) {
        if (!FeatureOption.MTK_WEB_NOTIFICATION_SUPPORT) {
            return;
        }

        Xlog.d(LOGTAG, "notificationManagerShow"
                + " icon=" + icon + " title=" + title + " body=" + body + " counter=" + counter);

        mCallbackProxy.onNotificationManagerShowPrompt(icon, title, body, counter,
                new NotificationPermissions.Callback() {
            @Override
            ...
            public void invokeEvent(String eventName, int pointer) {
                InvokeEventparams localInvokeEventparams = new InvokeEventparams();
                localInvokeEventparams.mEventName = eventName;
                localInvokeEventparams.mPointer = pointer;
                sendMessage(EventHub.NOTIFICATION_RESPONSE_BACK, localInvokeEventparams);
            }

            @Override
            public void invokeNotificationID(int id, int counter) {
                InvokeNotifyID localInvokeNotifyID = new InvokeNotifyID();
                localInvokeNotifyID.mnotificationID = id;
                localInvokeNotifyID.mcounter = counter;
                sendMessage(EventHub.NOTIFICATION_ID_BACK, localInvokeNotifyID);
            }
        });
    }

    protected void notificationPermissionsHidePrompt() {
        if (!FeatureOption.MTK_WEB_NOTIFICATION_SUPPORT) {
            return;
        }

        Xlog.d(LOGTAG, "notificationPermissionsHidePrompt");
        mCallbackProxy.onNotificationPermissionsHidePrompt();
    }

    protected void notificationPermissionsShowPrompt(String url) {
        if (!FeatureOption.MTK_WEB_NOTIFICATION_SUPPORT) {
            return;
        }

        Xlog.d(LOGTAG, "notificationPermissionsShowPrompt" + " url=" + url);

        mCallbackProxy.onNotificationPermissionsShowPrompt(url, new NotificationPermissions.Callback() {
            @Override
            public void invoke(String origin, boolean allow) {
                NotificationPermissionsData localNotificationPermissionsData = new NotificationPermissionsData();
                localNotificationPermissionsData.mOrigin = origin;
                localNotificationPermissionsData.mAllow = allow;
                sendMessage(EventHub.NOTIFICATION_PERMISSIONS_PROVIDE, localNotificationPermissionsData);
            }
    ...
    */
    ...
    /// M: Dump memory usage
    private native void nativeDumpMemoryUsage(int nativeClass, boolean useFile);
            ...
            synchronized (WebViewCore.class) {
                sWebCoreHandler = new Handler() {
                    ...
                    public void handleMessage(Message msg) {
                        /** M: add log */
                        Xlog.d(XLOGTAG, "sWebCoreHandler.handleMessage() " + Integer.toString(msg.what) + " obj=" + msg.obj);
                        switch (msg.what) {
                            case INITIALIZE:
                        ...
                        /** M: add log */
                        Xlog.d(XLOGTAG, "sWebCoreHandler.handleMessage() done");
                    }
                };
    ...
    /// M : HTML5 web notificaiton @ {

    static class InvokeNotifyID {
        int mcounter;
        int mnotificationID;
    }

    static class NotificationPermissionsData {
        boolean mAllow;
        String mOrigin;
    }

    static class InvokeEventparams {
        String mEventName;
        int mPointer;
    }
    ...
    public class EventHub implements WebViewInputDispatcher.WebKitCallbacks {
        ...
        static final int DUMP_MEMORYUSAGE = 172;
        ...
        static final int PAGE_VISIBILITY_CHANGED = 230;

        /// M: A message to notify the webkit update the screen size, note this message is only used by Email application.
        static final int UPDATE_SCREEN_SIZE = 231;

        /// M: HTML5 web notification @ {
        static final int NOTIFICATION_ID_BACK = 232;
        static final int NOTIFICATION_PERMISSIONS_PROVIDE = 233;
        static final int NOTIFICATION_RESPONSE_BACK = 234;
        /// @ }

        /// M: DataList Support
        static final int SELECT_DATALIST_OPTION = 235;

        static final int SELECT_COLOR = 236;
            ...
            mHandler = new Handler() {
                ...
                public void handleMessage(Message msg) {
                    ...
                    switch (msg.what) {
                        ...
                        /// M: Dump memory usage @{
                        case DUMP_MEMORYUSAGE:
                            nativeDumpMemoryUsage(mNativeClass, msg.arg1 == 1);
                            break;
                        /// @}
                        ...
                        case SELECT_DATALIST_OPTION:
                            nativeSelectDataListOption(mNativeClass, msg.arg1);
                            break;
                        /// @}

                        /// M: HTML5 color element support @{
                        case SELECT_COLOR:
                            nativeSendColorPickerChoice(mNativeClass, msg.arg1);
                            break;
                        /// @}
                        case EXECUTE_JS:
                        ...
                        case INSERT_TEXT:
                            ...
                            break;
                        case SELECT_TEXT: {
                            /// M: msg.obj=null clear text selection. @ {
                            if (msg.obj == null) {
                        ...
                        case PAGE_VISIBILITY_CHANGED:
                             nativeSetPageVisibility(mNativeClass, msg.arg1);
                             break;
                        /// }@
                        
                        /// M: Do update the screen size. @{
                        case UPDATE_SCREEN_SIZE: 
                            nativeUpdateScreenSize(mNativeClass, msg.arg1, msg.arg2);
                            break;
                        /// }@

                        /// M: HTML5 web notification @ {
                        case NOTIFICATION_PERMISSIONS_PROVIDE:
                            NotificationPermissionsData localNotificationPermissionsData =
                                    (NotificationPermissionsData)msg.obj;
                            nativeNotificationPermissionsProvide(localNotificationPermissionsData.mOrigin,
                                    localNotificationPermissionsData.mAllow);
                            break;
                        case NOTIFICATION_RESPONSE_BACK:
                            InvokeEventparams localInvokeEventparams = (InvokeEventparams)msg.obj;
                            nativeNotificationResponseback(localInvokeEventparams.mEventName,
                                    localInvokeEventparams.mPointer);
                            break;
                        case NOTIFICATION_ID_BACK:
                            InvokeNotifyID localInvokeNotifyID = (InvokeNotifyID)msg.obj;
                            nativeNotificationIDback(localInvokeNotifyID.mnotificationID, localInvokeNotifyID.mcounter);
                            break;
                         /// @ }
                    }
                }
    ...
    private void viewSizeChanged(WebViewClassic.ViewSizeData data) {
        ...
        if (mWebViewClassic.mOrientationChange) {
            mWebViewClassic.mOrientationChange = false;
            mWebViewClassic.resumeTimers();
        }
        /// @}
    ...
    private int calculateWindowWidth(int viewWidth) {
        ...
        if (mSettings.getUseWideViewPort()) {
            if (mViewportWidth == -1) {
                ...
                // For mobile web site.
                /// M: re-calculate the window width if initial-scale has been set. @{
                if (mViewportInitialScale > 0 && mViewportInitialScale < 100 * mAdjust) {
                    width = Math.round(mWebViewClassic.getViewWidth() / (mViewportInitialScale * 0.01f));
                } else {
    ...
    private void webkitDraw() {
        ...
        Xlog.d(XLOGTAG, "webkitDraw::nativeRecordContent");
    ...
    /// M: DataList Support @{
    // called by JNI
    private void showDataListOptions(String[] array) {
        if (mWebViewClassic != null) {
            mWebViewClassic.showDataListOptions(array);
        }
    }
    /// @}
    ...
    // called by JNI
    private void requestDateTimePickers(String type, String value) {
        if (mWebViewClassic != null) {
            Message.obtain(mWebViewClassic.mPrivateHandler,
                    WebViewClassic.REQUEST_DATE_TIME_PICKER, DateTimePickerManager.getPickerId(type), 0, value)
                    .sendToTarget();
        }
    }
    /// @}
    ...
    /// M: HTML5 color element support @{
    // called by JNI
    private void openColorChooser(int color) {
        if (mWebViewClassic != null) {
            Message.obtain(mWebViewClassic.mPrivateHandler,
                    WebViewClassic.REQUEST_COLOR_PICKER, color, 0, null)
                    .sendToTarget();
        }
    }
    /// @}
    ...
    int getViewportInitialScale() {
        return mViewportInitialScale;
    }
    /// @}
    /** @} */
    ...
    /// M: Page Visibility API
    private native void nativeSetPageVisibility(int nativeClass, int newState);
    /// M: Update screen size API
    private native void nativeUpdateScreenSize(int nativeClass, int screenWidth, int screenHeight);

    //// M: HTML5 web notification @ {
    private native void nativeNotificationIDback(int id, int counter);
    private native void nativeNotificationPermissionsProvide(String origin, boolean allow);
    private native void nativeNotificationResponseback(String eventName, int pointer);
    /// @ }

    /// M: DataList Support
    private native void nativeSelectDataListOption(int nativeClass, int optionId);

    /// M: Color Input Support
    private native void nativeSendColorPickerChoice(int nativeClass, int color);
}