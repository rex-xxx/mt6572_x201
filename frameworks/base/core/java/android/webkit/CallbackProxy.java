...
class CallbackProxy extends Handler {
    ...
    private static final int NOTIFICATION_PERMISSIONS_SHOW_PROMPT = 145;
    private static final int NOTIFICATION_PERMISSIONS_HIDE_PROMPT = 146;
    private static final int NOTIFICATION_SEND_RESPONSE_BACK      = 147;
    private static final int NOTIFICATION_CANCEL_PROMPT           = 148;
    /// @ }
    ...
    private void setPreventWatchdogResume(boolean isPrevent) {
        if (mWebView instanceof WebViewClassic) {
            mWebView.setPreventWatchdogResume(isPrevent);
        }
    }
    ...
    public void handleMessage(Message msg) {
        ...
        Xlog.d(LOGTAG, "CallbackProxy#handleMessage, msg = " + msg);
        switch (msg.what) {
            case PAGE_STARTED:
                                ...
                                .setPositiveButton(R.string.ok,
                                        new DialogInterface.OnClickListener() {
                                            public void onClick(
                                                ...
                                                res.confirm();
                                                /// M: set the flag to prevent watchdog resume when JS has dialog
                                                setPreventWatchdogResume(false);
                                            }
                                        })
                                .setOnCancelListener(
            ...
            /// M: HTML5 web notification @ {
            case NOTIFICATION_PERMISSIONS_SHOW_PROMPT:
                if (mWebChromeClient != null) {
                    HashMap<String, Object> localHashMap = (HashMap<String, Object>)msg.obj;
                    String origin = (String)localHashMap.get("origin");
                    NotificationPermissions.Callback localCallback =
                            (NotificationPermissions.Callback)localHashMap.get("callback");
                    mWebChromeClient.onNotificationPermissionsShowPrompt(origin, localCallback);
                }
                break;

            case NOTIFICATION_PERMISSIONS_HIDE_PROMPT:
                if (mWebChromeClient != null) {
                    mWebChromeClient.onNotificationPermissionsHidePrompt();
                }
                break;

            case NOTIFICATION_SEND_RESPONSE_BACK:
                if (mWebChromeClient != null) {
                    HashMap<String, Object> localHashMap = (HashMap<String, Object>)msg.obj;
                    String iconUrl = (String)localHashMap.get("iconUrl");
                    String title = (String)localHashMap.get("titleStr");
                    String body = (String)localHashMap.get("bodyStr");
                    int counter = ((Integer)localHashMap.get("counter")).intValue();
                    NotificationPermissions.Callback localCallback =
                            (NotificationPermissions.Callback)localHashMap.get("callback");
                    mWebChromeClient.onNotificationManagerShowPrompt(iconUrl, title, body, counter, localCallback);
                }
                break;

            case NOTIFICATION_CANCEL_PROMPT:
                if (mWebChromeClient != null) {
                    int id = msg.getData().getInt("notificationID");
                    mWebChromeClient.onNotificationManagerCancelPrompt(id);
                }
                break;
            /// @ }
        }
    ...
    private String getJsDialogTitle(String url) {
        /**
         * M: Long dialog title can not be displayed completely.
         * We just display 'JavaScript' title. @ {
         */
        return mContext.getString(R.string.js_dialog_title_default);
        /**
        String title = url;
...
//            Looper.myQueue().setWaitCallback(null);
    ...
    public BrowserFrame createWindow(boolean dialog, boolean userGesture) {
        ...
        /// M: If messages blocked, don't send CREATE_WINDOW to UI,
        /// otherwise WebViewCoreThread will be blocked. @ {
        if (messagesBlocked()) {
            Log.d(LOGTAG, "message blocked, can't createWindow");
            return null;
        }
        /// @ }
    ...
    public void onNotificationManagerCancelPrompt(int id) {
        Xlog.d(LOGTAG, "onNotificationManagerCancelPrompt, id=" + id);
        if (mWebChromeClient == null) {
            return;
        }
        Message msg = obtainMessage(NOTIFICATION_CANCEL_PROMPT);
        msg.getData().putInt("notificationID", id);
        sendMessage(msg);
    }

    public void onNotificationManagerShowPrompt(String icon, String title, String body, int counter,
            NotificationPermissions.Callback callback) {
        Xlog.v(LOGTAG, "Inside CallbackProxy onNotificationManagerShowPrompt before send message");
        if (mWebChromeClient == null) {
            return;
        }
        Message msg = obtainMessage(NOTIFICATION_SEND_RESPONSE_BACK);
        HashMap<String, Object> localHashMap = new HashMap();
        localHashMap.put("iconUrl", icon);
        localHashMap.put("titleStr", title);
        localHashMap.put("bodyStr", body);
        localHashMap.put("counter", counter);
        localHashMap.put("callback", callback);
        msg.obj = localHashMap;
        sendMessage(msg);
    }
    ...
    public void onNotificationPermissionsHidePrompt() {
        ...
        sendMessage(obtainMessage(NOTIFICATION_PERMISSIONS_HIDE_PROMPT));
    }

    public void onNotificationPermissionsShowPrompt(String origin, NotificationPermissions.Callback callback) {
        if (mWebChromeClient == null){
            return;
        }
        Message msg = obtainMessage(NOTIFICATION_PERMISSIONS_SHOW_PROMPT);
        HashMap<String, Object> localHashMap = new HashMap();
        localHashMap.put("origin", origin);
        localHashMap.put("callback", callback);
        msg.obj = localHashMap;
        sendMessage(msg);
    }
    ...
    /**
     * M: Notify WebViewCoreThread to wake up, called by host application.
     */
    public synchronized void notifyWebViewCoreThread() {
        // WebView will destroy, block sending message to CallbackProxy.
        blockMessages();
        notify();
    }
}
