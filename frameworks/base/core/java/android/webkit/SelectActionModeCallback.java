...
class SelectActionModeCallback implements ActionMode.Callback {
    ...
    /// M: Logging tag
    private static final String LOGTAG = "SelectActionModeCallback";
    ...
    public boolean onActionItemClicked(ActionMode mode, MenuItem item) {
        switch(item.getItemId()) {
            ...
            case com.android.internal.R.id.websearch:
                ...
                /// M: binder transaction can transport 2MB, but browse UrlInputView's max length is 2048.
                ///    if length > 2000, truncate extra characters. @ {
                final int limit = 2000;
                String select = mWebView.getSelection();
                if (select != null && select.length() > 0 && select.length() > limit) {
                    Log.d(LOGTAG, "websearch truncate string. length=" + select.length());
                    select = select.substring(0, limit);
                }
                /// @ }
                i.putExtra(SearchManager.QUERY, select);
