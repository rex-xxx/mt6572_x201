...
class HTML5VideoViewProxy extends Handler
    ...
    /// M: Add XLOGTAG.
    private static final String XLOGTAG = "HTML5VideoViewProxy";
    ...
    /// M: mute/volume @{
    private static final int SET_VOLUME          = 150;
    /// @}
    ...
    /// M: use to tell exiting full screen is triggered by user 
    private boolean mIsExitFullScreenByBackKey;
    ...
    private static final class VideoPlayer {
        ...
        private static void setPlayerBuffering(boolean playerBuffering) {
            /// M: TEARDOWN message handled before BUFFERING_START message lead to mHTML5VideoView is null. @ {
    ...
    public boolean onError(MediaPlayer mp, int what, int extra) {
        Xlog.e(XLOGTAG, "mediaPlayer.onError(): " + what);
    ...
    public void dispatchOnPaused() {
        Xlog.d(XLOGTAG, "HTML5VideoViewProxy.dispatchOnPaused()");
    ...
    public void dispatchOnStopFullScreen(boolean stillPlaying) {
        ...
        // only keep playing when triggered by back key.
        /// M: also check mIsExitFullScreenByBackKey flag, to make sure if it is trigger back key
        msg.arg1 = (stillPlaying && mIsExitFullScreenByBackKey) ? 1 : 0;
        mWebCoreHandler.sendMessage(msg);
        /// M: reset flag since already dispatched.
        mIsExitFullScreenByBackKey = false;
    }
    ...
    private static final class PosterDownloader implements EventHandler {
        ...
        private void cleanup() {
            Xlog.d(XLOGTAG, "cleanup::mPosterBytes is: " + mPosterBytes + this);
            if (mPosterBytes != null) {
                try {
                    mPosterBytes.close();
    ...
    /// M: set volume and set muted, from webkit only @{
    /**
     *  Set the volume of the player
     *  @param volume The desired volume to set, from 0.0 to 1.0.
     */
    ...
    public void setVolume(float volume) {
        ...
        Message msg = obtainMessage(SET_VOLUME);
        msg.arg1 = (int) (volume * 100);
        sendMessage(msg);
    }

    /**
     *  Set muted or not for the player.
     *  @param muted Set the flag to true to mute the audio
     */
    public void setMuted(boolean muted) {
        int volume = muted ? 0 : (int) (mVolume * 100);
        mMuted = muted;
        Message msg = obtainMessage(SET_VOLUME);
        msg.arg1 = (int) (volume);
        sendMessage(msg);
    ...
    public boolean onInfo(MediaPlayer mp, int what, int extra) {
        Xlog.d(XLOGTAG, "mediaPlayer.onInfo(): " + what);    
    ...
    public boolean onKey(View v, int keyCode, KeyEvent event) {
        if (keyCode == KeyEvent.KEYCODE_BACK) {
            if (event.getAction() == KeyEvent.ACTION_DOWN) {
                ...
                /// M: to tell it is triggered by onPause or user press back key
                mIsExitFullScreenByBackKey = true;
                VideoPlayer.exitFullScreenVideo(this, mWebView);
                return true;
            }
        }
