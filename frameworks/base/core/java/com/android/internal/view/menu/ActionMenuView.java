...
public class ActionMenuView extends LinearLayout implements MenuBuilder.ItemInvoker, MenuView {
    ...
    public void onConfigurationChanged(Configuration newConfig) {
        ...
        /// M: When configuration changed, we re-inflate action menu item layout to update its padding.
        LayoutInflater inflater = LayoutInflater.from(getContext());
        ActionMenuItemView tempActionMenuItemView = (ActionMenuItemView) inflater
                .inflate(com.android.internal.R.layout.action_menu_item_layout,
                        this, false);

        if(tempActionMenuItemView != null) {
            final int childCount = getChildCount();
            for (int i = 0; i < childCount; i++) {
                final View child = getChildAt(i);
                final boolean isGeneratedItem = child instanceof ActionMenuItemView;
                if (isGeneratedItem) {
                    child.setPadding(tempActionMenuItemView.getPaddingLeft(),
                            tempActionMenuItemView.getPaddingTop(),
                            tempActionMenuItemView.getPaddingRight(),
                            tempActionMenuItemView.getPaddingBottom());
                }
            }
        }
    }
