...
status_t NativeInputEventReceiver::consumeEvents(JNIEnv* env,
	...
	int consumeCount = 0;   /// M: Fix ALPS00353045
    for (;;) {
        uint32_t seq;
        ...
        if (!skipCallbacks) {
            ...
            if (inputEventObj) {
                ...
                /// M: Fix ALPS00353045 the root cause is consumeBatch may create a lot of event object.
                /// It will let JNI local reference table overflow.
                /// We delete local reference in loop manually. (this is also a good JNI coding convention) @{
                env->DeleteLocalRef(inputEventObj);
                consumeCount++;
                if (consumeCount == 250) {
                    ALOGW("warning!! consumeCount will over 250");
                }
                /// @}
            } else {
