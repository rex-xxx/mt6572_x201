...
#ifdef __BTMTK__
extern int register_android_bluetooth_BluetoothSocketService(JNIEnv *env);
#ifdef __BTSIMULATOR__
...
extern int register_com_android_internal_app_ShutdownManager(JNIEnv *env);
...
int AndroidRuntime::startVm(JavaVM** pJavaVM, JNIEnv** pEnv)
{
    ...
    char stackTraceFileBuf2[PROPERTY_VALUE_MAX];
    char dexoptFlagsBuf[PROPERTY_VALUE_MAX];
    char enableAssertBuf[sizeof("-ea:")-1 + PROPERTY_VALUE_MAX];
    char jniOptsBuf[sizeof("-Xjniopts:")-1 + PROPERTY_VALUE_MAX];
    char jniTraceBuf[sizeof("-Xjnitrace:")-1 + PROPERTY_VALUE_MAX];
    ...
    property_get("dalvik.vm.mtk-stack-trace-file", stackTraceFileBuf2, "/data/anr/mtk_traces.txt");
    ...
    strcpy(jniTraceBuf, "-Xjnitrace:");
    property_get("dalvik.vm.jnitrace", jniTraceBuf+11, "");
    ALOGD("JNI trace buf :%s", jniTraceBuf);
    ...
    if (jniTraceBuf[11] != '\0')  {
        ALOGI("JNI Trace : %s", jniTraceBuf+11);
        opt.optionString = jniTraceBuf;
        mOptions.add(opt);
    }
    ...
    if (stackTraceFileBuf2[0] != '\0') {
        static const char* mstfOptName = "-Xmtkstacktracefile:";

        stackTraceFile2 = (char*) malloc(strlen(mstfOptName) +
            strlen(stackTraceFileBuf2) +1);
        strcpy(stackTraceFile2, mstfOptName);
        strcat(stackTraceFile2, stackTraceFileBuf2);
        opt.optionString = stackTraceFile2;
        mOptions.add(opt);
    }
...
static int javaAttachThread(const char* threadName, JNIEnv** pEnv)
{
    ...
    if (vm == NULL)  {
        ALOGE("NOTE: attach of thread '%s' failed : NULL\n", threadName);
        return JNI_ERR;
    }
    ...
    if (vm->functions->AttachCurrentThread == NULL)  {
        ALOGE("NOTE: attach of thread '%s' failed : FX NULL\n", threadName);
        return JNI_ERR;
    }
...
static int javaDetachThread(void)
{
    ...
    if (vm == NULL)  {
        ALOGE("ERROR: thread detach failed : NULL\n");
        return JNI_ERR;
    }

    if (vm->functions->DetachCurrentThread == NULL)  {
        ALOGE("ERROR: thread detach failed : FX NULL\n");
        return JNI_ERR;
    }
...
static const RegJNIRec gRegJNI[] = {
...
#ifdef __BTMTK__
    REG_JNI(register_android_bluetooth_BluetoothSocketService),
#ifdef __BTSIMULATOR__    
    REG_JNI(register_android_server_BluetoothFrameworkUT),
#endif
#endif
    ...
    REG_JNI(register_com_android_internal_app_ShutdownManager),
};
