...
static int parseIfaceStat(const char* iface, struct IfaceStat* stat) {
    ...
    while (fgets(buffer, 256, fp) != NULL) {
        ...
        //M: add for CR ALPS00539344, because Kernel enhance xtqtaguid to show localhost interface, we should filter it to pass CTS test
        if (  (!iface || !strcmp(iface, cur_iface))  &&  strcmp(cur_iface,"lo")!=0   ) {
