...
AaptGroupEntry::parseNamePart(const String8& part, int* axis, uint32_t* value)
{
    ...
    ///M:
    uint16_t negativeswdp;
    // negatvie smallest screen dp width
    if (getNegativeSmallestScreenWidthDpName(part.string(), &negativeswdp)) {
        *axis = AXIS_NEGATIVE_SMALLESTSCREENWIDTHDP;
        *value = negativeswdp;
        return 0;
    }
	///M: end
...
///M:
bool AaptGroupEntry::getNegativeSmallestScreenWidthDpName(const char* name, uint16_t* out)
{
    if (strcmp(name, kWildcardName) == 0) {
        if (out) {
            out = 0;
    ...
    if (out) {
        *out = (uint16_t)atoi(xName.string());
    }
