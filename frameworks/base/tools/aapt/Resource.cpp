...
static const char* image_overlaid_type[]= {".bmp", ".jpg", ".png", ".gif", ".BMP", ".JPG", ".PNG", ".GIF"};
///M: end
...
{
  size_t searchIndex = overlaidFlavorAndFiles.indexOfKey(groupEntry);
  bool ret = true;
  if(searchIndex < UNKNOWN_ERROR){
    ...
    while (overlay.get()) {
        ...
        if (index >= 0) {
            ...
            for (size_t overlayIndex=0; overlayIndex<overlayCount; overlayIndex++) {
                ...
                //for saving the new created group or flavor for the resType
                ...
                if(!newedFlavorAndFile.isEmpty()){
                    ...
                    for(size_t imgTypeIndex=0; imgTypeIndex < (sizeof(image_overlaid_type)/sizeof(char*)) ; imgTypeIndex++ ){
                        size_t rmBaseIndex = UNKNOWN_ERROR;
                        ...
                        if (rmBaseIndex < UNKNOWN_ERROR) {
                            ...
                            for(size_t newFlavorIndex=0; newFlavorIndex < newedFlavorAndFile.size() ; newFlavorIndex++ ){
                               size_t rmBaseFileIndex = rmBaseFiles->indexOfKey(newedFlavorAndFile.keyAt(newFlavorIndex));
                               if (rmBaseFileIndex < UNKNOWN_ERROR) {
