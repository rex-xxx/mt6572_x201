...
ResourceTable::ResourceTable(Bundle* bundle, const String16& assetsPackage)
    : mAssetsPackage(assetsPackage), mNextPackageId(1), mHaveAppPackage(false),
      mIsAppPackage(!bundle->getExtending()),mIsmtkCur(bundle->getmtkCur()),mIsmtkCurNum(bundle->getmtkCurNum()),
      mNumLocal(0),
      mBundle(bundle)
{
}
...
sp<ResourceTable::Package> ResourceTable::getPackage(const String16& package)
{
    ...
    if (p == NULL) {
        if (mIsAppPackage) {
			...
			///M:original resource ID just include 0x01&0x07,we and 0x02&0x03...0x09,@{
			if (mIsmtkCur){
                if(mIsmtkCurNum > 3){
				   p = new Package(package, mIsmtkCurNum);
				 } else {
