...
ResourceFilter::match(int axis, uint32_t value) const
{
    ...
    ssize_t index = mData.indexOfKey(axis);
    if (index < 0) {
        if (axis != AXIS_SMALLESTSCREENWIDTHDP){
            // android default logic
            // we didn't request anything on this axis so take everything
            return true;
        } else {
            ///M: 
            // if we are matching the swNsp,the negative request is considered            
            ssize_t n_index = mData.indexOfKey(AXIS_NEGATIVE_SMALLESTSCREENWIDTHDP);
            if (n_index < 0){
                //the -swNdp is not specified
                return true;
            } else {
                //the -swNdp is specified, return false if the value matches it                
                const SortedVector<uint32_t>& n_sv = mData.valueAt(n_index);
                return n_sv.indexOf(value) < 0;
            }
            ///M: end 
        }
