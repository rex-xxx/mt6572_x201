...
#if MTK_THEMEMANAGER_APP
    /* For ThemeManager feature. */
    MTKThemeManager* mThemeManager;   
    asset_path* mThemePath;
    char* getLowercaseApkName(char* str);
    void appendApkPath(char* apkName, const char* fileName, char* outApkFileName);
    void testThemeMap();
    Asset* getAssetFromTheme(int which, const char* fileName, asset_path themePath, AccessMode mode);
#endif
};
