...
public class KeyguardPatternView extends LinearLayout implements KeyguardSecurityView {
    ...
    private void displayDefaultSecurityMessage() {
        if (KeyguardUpdateMonitor.getInstance(mContext).getMaxBiometricUnlockAttemptsReached()) {
            ...
            if (mLockPatternUtils.usingBiometricWeak()) {
                ...
                mSecurityMessageDisplay.setMessage(com.mediatek.internal.R.string.voiceunlock_multiple_failures, true);
            }
        } else {
        ...
        public void onPatternDetected(List<LockPatternView.Cell> pattern) {
            if (mLockPatternUtils.checkPattern(pattern)) {
                ...
                if (pattern.size() >= LockPatternUtils.MIN_PATTERN_REGISTER_FAIL) {
                    ...
                    /// M: Do not use mFailedPatternAttemptsSinceLastTimeout to check unlock attemp time out,
                    /// for issue ALPS00431080
    ...
    public void onResume(int reason) {
        ...
        if (mLockPatternUtils.usingVoiceWeak() && mediaPlaying) {
            mSecurityMessageDisplay.setMessage(com.mediatek.internal.R.string.voice_unlock_media_playing, true);
        }
    }
