...
public class PhoneWindow extends Window implements MenuBuilder.Callback {
    ...
    private void openPanel(PanelFeatureState st, KeyEvent event) {
        ...
        /// M: Make sure there is at least one visible non-action item, or we will set not prepared and return
    ...
    private void installDecor() {
        ...
        if (mContentParent == null) {
            ...
            if (FeatureOption.MTK_THEMEMANAGER_APP){
                View titleDivider = findViewById(com.android.internal.R.id.titleDivider);
                int themeColor = getContext().getResources().getThemeMainColor();
                if (themeColor != 0) {    //default style highlight color
                    if (mTitleView != null) {
                        mTitleView.setTextColor(themeColor);
                    }
