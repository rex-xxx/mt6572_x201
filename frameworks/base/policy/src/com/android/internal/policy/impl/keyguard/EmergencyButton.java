...
public class EmergencyButton extends Button {
    ...
    KeyguardUpdateMonitorCallback mInfoCallback = new KeyguardUpdateMonitorCallback() {
        ...
        @Override
        public void onSimStateChangedGemini(State simState, int simId) {
            int phoneState = KeyguardUpdateMonitor.getInstance(mContext).getPhoneState();
            updateEmergencyCallButton(simState, phoneState);
        }
        ...
        public void onRefreshCarrierInfoGemini(CharSequence plmn, CharSequence spn, int simId) {
            ...
            if (KeyguardUtils.isValidSimId(simId)) {
                State simState = KeyguardUpdateMonitor.getInstance(mContext).getSimState(simId);
                updateEmergencyCallButton(simState, phoneState);
            }
        }
        /// M @}
    ...
    private void updateEmergencyCallButton(State simState, int phoneState) {
        ...
        boolean keyguardUtilShowEcc = mKeyguardUtilExt.shouldShowEmergencyBtnForVoiceOn();
        /// M:CTA new feature
    ...
    private boolean eccButtonShouldShow(){
        ...
        for (int i = PhoneConstants.GEMINI_SIM_1; i <= KeyguardUtils.getMaxSimId(); i++) {
            CharSequence mPlmn = KeyguardUpdateMonitor.getInstance(this.getContext()).getTelephonyPlmn(i);
            CharSequence mSpn = KeyguardUpdateMonitor.getInstance(this.getContext()).getTelephonySpn(i);
