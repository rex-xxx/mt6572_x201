...
public class SlidingChallengeLayout extends ViewGroup implements ChallengeLayout {
    ...
    /// M: Debug filter for plug-in layer
    private static final boolean DEBUG_PLUG_IN = false;

    private static final int FULL_SCREEN_CHALLENGE_HEIGHT = 400; //dp
    ...
    public void onAttachedToWindow() {
        ...
        if (DEBUG_PLUG_IN) {
            KeyguardUtils.xlogD(TAG, "onAttachedToWindow"
                    + ", mKeyguardLayerBackgroundView=" + mKeyguardLayerBackgroundView
                    + ", mKeyguardLayerBackground=" + mKeyguardLayerBackground
                    + ", mKeyguardLayerForegroundView=" + mKeyguardLayerForegroundView
                    + ", mKeyguardLayerForeground=" + mKeyguardLayerForeground
                    + ", this=" + this);
        }
        mHasLayout = false;
    }
    ...
    public void onDetachedFromWindow() {
        ...
        if (DEBUG_PLUG_IN) {
            KeyguardUtils.xlogD(TAG, "onDetachedFromWindow"
                    + ", mKeyguardLayerBackgroundView=" + mKeyguardLayerBackgroundView
                    + ", mKeyguardLayerBackground=" + mKeyguardLayerBackground
                    + ", mKeyguardLayerForegroundView=" + mKeyguardLayerForegroundView
                    + ", mKeyguardLayerForeground=" + mKeyguardLayerForeground
        ...
        /// M: Remove creating keyguard layer runnable @{
        removeCallbacks(mShowKeyguardLayerRunnable);
        /// @}
        mHasLayout = false;
        
        /// M: destory IKeyaugerLayer
        if (mKeyguardLayerBackground != null) {
            KeyguardUtils.xlogD(TAG, "onDetachedFromWindow mKeyguardLayerBackground destroy " + mKeyguardLayerBackground);
            mKeyguardLayerBackground.destroy();
            mKeyguardLayerBackground = null;
        }
        if (mKeyguardLayerForeground != null) {
            KeyguardUtils.xlogD(TAG, "onDetachedFromWindow mKeyguardLayerForeground destroy " + mKeyguardLayerForeground);
    ...
    protected void onMeasure(int widthSpec, int heightSpec) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (lp.childType == LayoutParams.CHILD_TYPE_CHALLENGE) {
                ...
                View glowPadView = child.findViewById(R.id.keyguard_selector_view);
                mHasGlowpad =  (glowPadView != null) && (glowPadView.getVisibility() == View.VISIBLE);
        ...
        if (mChallengeView != null && mChallengeView.getVisibility() != View.GONE) {
            ...
            if (root != null) {
                ...
                ///M: enlarge challenge height for full screen challenge style @{
                int maxChallengeHeight = lp.maxHeight - diff;
                View mAppWidgetContainer = (KeyguardWidgetPager) findViewById(R.id.app_widget_container);
                int fullScreenChallengeHeight = (int) (FULL_SCREEN_CHALLENGE_HEIGHT * mDisplayMetrics.density);
                if (mAppWidgetContainer != null && mAppWidgetContainer.getVisibility() != View.VISIBLE) {
                    if (maxChallengeHeight < Math.min(specSize, fullScreenChallengeHeight)) {
                        maxChallengeHeight = Math.min(specSize, fullScreenChallengeHeight);
                    }
                }
                // @}
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            /// M: Also layout IKeyguardLayerView @{
              else if (lp.childType == LayoutParams.CHILD_TYPE_KEYGUARD_LAYER_FOREGROUND) {
                  /// M: Always layout keyguard layer foreground view after challenge view
                  continue;
            } else if (lp.childType == LayoutParams.CHILD_TYPE_KEYGUARD_LAYER_BACKGROUND) {
        ...
        if (mKeyguardLayerForegroundLayout != null) {
            /// M: Make mKeyguardLayerForegroundLayout bottom aligne to mChallengeView
            mKeyguardLayerForegroundLayout.layout(l, mChallengeView.getBottom() - mKeyguardLayerForegroundLayout.getMeasuredHeight(), r, mChallengeView.getBottom());
        }
    ...
    private boolean moveChallengeTo(int bottom) {
        ...
        /// M: Also update KeyguardLayer's position
        updateKeyguardLayerPosition(bottom);
    ...
    public static class LayoutParams extends MarginLayoutParams {
        ...
        /// M: add type for IKeyguardLayers
    ...
    private Runnable mShowKeyguardLayerRunnable = new Runnable() {
        ...
        public void run() {
            ...
            if (mKeyguardLayerForegroundLayout != null) {
                mKeyguardLayerForegroundLayout.setVisibility(mShowKeyguardLayer ? View.VISIBLE : View.INVISIBLE);
            }
            if (mKeyguardLayerBackgroundLayout != null) {
                mKeyguardLayerBackgroundLayout.setVisibility(mShowKeyguardLayer ? View.VISIBLE : View.INVISIBLE);
            }
        }
    };
    ...
    public void onFinishInflate() {
        ...
        for (int i = 0; i < count; i++) {
            final View child = getChildAt(i);
            final LayoutParams lp = (LayoutParams) child.getLayoutParams();
            if (lp.childType == LayoutParams.CHILD_TYPE_KEYGUARD_LAYER_FOREGROUND) {
                mKeyguardLayerForegroundLayout = (FrameLayout)child;
            } else if (lp.childType == LayoutParams.CHILD_TYPE_KEYGUARD_LAYER_BACKGROUND) {
                mKeyguardLayerBackgroundLayout = (FrameLayout)child;
            } else if (lp.childType == LayoutParams.CHILD_TYPE_CHALLENGE) {
                if (mChallengeView != null) {
                    throw new IllegalStateException(
                            "There may only be one child with layout_isChallenge=\"true\"");
                }
    ...
    private void updateKeyguardLayerView(boolean show) {
        if (DEBUG_PLUG_IN) {
            KeyguardUtils.xlogD(TAG, "updateKeyguardLayerView show=" + show
                    + ", mKeyguardLayerBackgroundView=" + mKeyguardLayerBackgroundView
                    + ", mKeyguardLayerBackground=" + mKeyguardLayerBackground
                    + ", mKeyguardLayerForegroundView=" + mKeyguardLayerForegroundView
                    + ", mKeyguardLayerForeground=" + mKeyguardLayerForeground
        ...
        if (show && mKeyguardLayerBackgroundView == null && mKeyguardLayerForegroundView == null) {
            try{
                IPluginManager plugManager = MediatekClassFactory.createInstance(IPluginManager.class,
                        IPluginManager.CREATE,
                        mContext,
                        IKeyguardLayer.class.getName(),
                        null);
                int pluginCount = plugManager.getPluginCount();
                if (pluginCount != 0) {
                    IPlugin plugin;
                    IKeyguardLayer keyguardLayer;
                    View keyguardLayerView;
                    for (int i = 0; i < pluginCount; i++) {
                        plugin = plugManager.getPlugin(i);
                        keyguardLayer = (IKeyguardLayer)plugin.createObject();
                        keyguardLayerView = keyguardLayer.create();
                        KeyguardUtils.xlogD(TAG, "updateDataUsageView create plugin keyguardLayer=" + keyguardLayer + ", this=" + this);
                        if (keyguardLayer.getKeyguardLayerInfo() != null) {
                            mKeyguardLayerBackground = keyguardLayer;
                            mKeyguardLayerBackgroundView = keyguardLayerView;
                        } else {
                            mKeyguardLayerForeground = keyguardLayer;
                            mKeyguardLayerForegroundView = keyguardLayerView;
                        }
                    }
                }
                
                if (mKeyguardLayerForegroundLayout != null && mKeyguardLayerForegroundView != null) {
                    ViewGroup.LayoutParams layoutParam = new LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT);
                    mKeyguardLayerForegroundView.setLayoutParams(layoutParam);
                    mKeyguardLayerForegroundLayout.addView(mKeyguardLayerForegroundView);
                }
                
                if (mKeyguardLayerBackgroundLayout != null && mKeyguardLayerBackgroundView != null) {
                    ViewGroup.LayoutParams layoutParam = new LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT);
                    mKeyguardLayerBackgroundView.setLayoutParams(layoutParam);
    ...
    public void updateKeyguardLayerVisibility(boolean show) {
        mShowKeyguardLayer = show;
        removeCallbacks(mShowKeyguardLayerRunnable);
        post(mShowKeyguardLayerRunnable);
    }
    
    public void setTouchRecepientForKeyguardLayer(MediatekGlowPadView glowPadView) {
        if (mKeyguardLayerForegroundLayout != null && glowPadView != null) {
            glowPadView.setTouchRecepient(mKeyguardLayerForegroundLayout);
        }
    }
    
    private void updateKeyguardLayerPosition(int bottom) {
        if (mKeyguardLayerForegroundLayout != null && mChallengeView != null) {
            mKeyguardLayerForegroundLayout.layout(mKeyguardLayerForegroundLayout.getLeft(),
                    bottom -mKeyguardLayerForegroundLayout.getHeight(),
                    mKeyguardLayerForegroundLayout.getRight(), bottom);
            // Make sure bottom position is between 
            int minBottom = getLayoutBottom();
            int maxBottom = getLayoutBottom() + mChallengeView.getMeasuredHeight();
            int realBottom = Math.min(Math.max(minBottom, bottom), maxBottom);
            final float alpha = 1.0f - (realBottom - minBottom) * 1.0f / mChallengeView.getMeasuredHeight();
            if (mKeyguardLayerForegroundView != null) {
                mKeyguardLayerForegroundView.setAlpha(alpha);
            }
            if (DEBUG_PLUG_IN) {
                KeyguardUtils.xlogD(TAG, "updateKeyguardLayerPosition bottom=" + bottom + ", mKeyguardLayerForegroundLayout.getHeight()=" + mKeyguardLayerForegroundLayout.getHeight()
                        + ", mKeyguardLayerForegroundLayout.left=" + mKeyguardLayerForegroundLayout.getLeft() + ", mKeyguardLayerForegroundLayout.right=" + mKeyguardLayerForegroundLayout.getRight()
                        + ", mKeyguardLayerForegroundLayout.top=" + mKeyguardLayerForegroundLayout.getTop() + ", mKeyguardLayerForegroundLayout.bottom=" + mKeyguardLayerForegroundLayout.getBottom()
                        + ", alpha=" + alpha);
            }
        }
    }
    ...
    public void setClipChildrenForNewEventView(boolean clipChildren) {
        ...
        if (mChallengeView != null) {
            mChallengeView.setClipChildrenForNewEventView(clipChildren);
        }
    }
    /// @}
}
