...
public class KeyguardWidgetPager extends PagedView implements PagedView.PageSwitchListener,
    ...
    ///M: flag to specify StatusWidget can be completely shown. ex. HVGA resolution.
    private boolean mStatusWidgetIsCompletelyShown = true;

    /// M: [LCA] Disable widget and overscroll rotation features
    private boolean mDisableWidgetFeature = KeyguardUtils.isMediatekLCASupport() ? true : false;
    ...
    public void onPageSwitched(View newPage, int newPageIndex) {
        ...
        if (showingStatusWidget && mStatusWidgetIsCompletelyShown) {
    ...
    protected void screenScrolled(int screenCenter) {
        ...
        for (int i = 0; i < getChildCount(); i++) {
            ...
            if (v != null) {
                ...
                /// M: Overscroll rotation features are supported by non-LCA projects
                if (isOverScrollChild(i, scrollProgress) && PERFORM_OVERSCROLL_ROTATION
                    && !mDisableWidgetFeature) {
    ...
    void showOutlinesAndSidePages() {
        KeyguardUtils.xlogD(TAG, "showOutlinesAndSidePages");
        animateOutlinesAndSidePages(true);
    }

    void hideOutlinesAndSidePages() {
        KeyguardUtils.xlogD(TAG, "hideOutlinesAndSidePages");
    ...
    public void showInitialPageHints() {
        ...
        KeyguardUtils.xlogD(TAG, "showInitialPageHints childCount=" + count + ", mCurrentPage=" + mCurrentPage);
    ...
    void animateOutlinesAndSidePages(final boolean show, int duration) {
        KeyguardUtils.xlogD(TAG, "animateOutlinesAndSidePages show=" + show + ", duration=" + duration);
        if (mChildrenOutlineFadeAnimation != null) {
            KeyguardUtils.xlogD(TAG, "animateOutlinesAndSidePages mChildrenOutlineFadeAnimation != null, cancel it");
        ...
        mChildrenOutlineFadeAnimation.addListener(new AnimatorListenerAdapter() {
            ...
            public void onAnimationStart(Animator animation) {
                KeyguardUtils.xlogD(TAG, "animateOutlinesAndSidePages onAnimationStart show=" + show);
                if (show) {
                    enablePageContentLayers();
                }
            }
            ...
            public void onAnimationEnd(Animator animation) {
                KeyguardUtils.xlogD(TAG, "animateOutlinesAndSidePages onAnimationEnd show=" + show);
                if (!show) {
    ...
    public boolean onLongClick(View v) {
        ...
        /// M: LCA non widget projects don't support long click to reordering
        if (mDisableWidgetFeature) {
            return true;   // handled Long click to avoid go next checking to startRecordering()
        } else if (!isChallengeOverlapping && startReordering()) {
            return true;  
        }
        return false;
    }
    ...
    /// M: set mStatusWidgetIsCompletelyShown flag
    public void setStatusWidgetIsCompletelyShown(boolean isCompleteShown) {
        mStatusWidgetIsCompletelyShown = isCompleteShown;
    }
}
