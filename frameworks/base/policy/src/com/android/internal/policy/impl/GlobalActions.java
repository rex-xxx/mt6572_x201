...
class GlobalActions implements DialogInterface.OnDismissListener, DialogInterface.OnClickListener  {
    ...
    public GlobalActions(Context context, WindowManagerFuncs windowManagerFuncs) {
        ...
        if (FeatureOption.MTK_GEMINI_SUPPORT != true) {
            ...
            try {
                ...
                if (tr1 != null) {
                    tr1.listen("GlobalActions", mPhoneStateListener.getCallback(), PhoneStateListener.LISTEN_SERVICE_STATE, notifyNow);
                } else {
                    Log.e(TAG, "The registry is null.");
                }
                ITelephonyRegistry tr2 = ITelephonyRegistry.Stub.asInterface(ServiceManager.getService("telephony.registry2"));
                if (tr2 != null) {
                    tr2.listen("GlobalActions", mPhoneStateListener.getCallback(), PhoneStateListener.LISTEN_SERVICE_STATE, notifyNow);
                } else {
                    Log.e(TAG, "The registry2 is null.");
                }
                ...
                if(PhoneConstants.GEMINI_SIM_NUM >= 3) {
                    ...
                    if (tr3 != null) {
                        tr3.listen("GlobalActions", mPhoneStateListener.getCallback(), PhoneStateListener.LISTEN_SERVICE_STATE, notifyNow);
                    } else {
                        Log.e(TAG, "The registry3 is null.");
                    }
                }
                if(PhoneConstants.GEMINI_SIM_NUM >= 4) {
                    ...
                    if (tr4 != null) {
                        tr4.listen("GlobalActions", mPhoneStateListener.getCallback(), PhoneStateListener.LISTEN_SERVICE_STATE, notifyNow);
                    } else {
                        Log.e(TAG, "The registry4 is null.");
                    }
                }                
        ...
        mAirplaneModeOn = new ToggleAction(
            ...
            public boolean isEnabled() {
                if(KeyguardUpdateMonitor.sDualSimSetting == 0){
                    Log.e(TAG, "if user unselect the dual sim mode setting on phone starting, the airplane mode can not be set.");
                    return false; //if user has not yet selected any item in the dual_sim_mode_setting dialog on phone starting, the airplane mode should be disabled.
                }
                else {
    ...
    private void prepareDialog() {
        ...
        if (SHOW_SILENT_TOGGLE) {
            /// M:[ALPS00230716]
            Log.d(TAG, "prepareDialog -- registerReceiver mRingerModeReceiver");
    ...
    public void onDismiss(DialogInterface dialog) {
        /// M:[ALPS00230716]
        if (SHOW_SILENT_TOGGLE & mRingerModeReceiverRegistered) {
    ...
    PhoneStateListener mPhoneStateListener = new PhoneStateListener() {
        ...
        public void onServiceStateChanged(ServiceState serviceState) {
            ...
            Log.v(TAG, "Phone State = " + serviceState.getState()
                    + " gemini = " + FeatureOption.MTK_GEMINI_SUPPORT
