...
public class KeyguardHostView extends KeyguardViewBase {
    ...
    private int getWidgetPosition(int id) {
        /// check if mAppWidgetContainer is null
    ...
    protected void onFinishInflate() {
        ...
        if(KeyguardUpdateMonitor.isAlarmBoot()){
            containerId = com.mediatek.internal.R.id.app_widget_container;
            deleteId = com.mediatek.internal.R.id.keyguard_widget_pager_delete_target;
            paneId = com.mediatek.internal.R.id.multi_pane_challenge;
            fliperId = com.mediatek.internal.R.id.view_flipper;
        }
        /// @}
        ...
        //ChallengeLayout challenge = mSlidingChallengeLayout != null ? mSlidingChallengeLayout :
            //(ChallengeLayout) findViewById(paneId);
        ChallengeLayout challenge = mSlidingChallengeLayout != null ? mSlidingChallengeLayout :
    ...
    private KeyguardSecurityCallback mCallback = new KeyguardSecurityCallback() {
        ...
        public void reportFailedUnlockAttempt() {
            if (mCurrentSecuritySelection == SecurityMode.Biometric
                    || mCurrentSecuritySelection == SecurityMode.Voice) { ///M: add for voice unlock
    ...
    void showPrimarySecurityScreen(boolean turningOff) {
        ...
        ///M: here we force to show selector view
        /*if (isLockScreenPriorSimPin(securityMode)) {
            Log.v(TAG, "showPrimarySecurityScreen,  lockscreen is prior to simPIN");
        ...
        if (mSlidingChallengeLayout != null) {
            ...
            if (mCurrentKeyguardSecurityView instanceof KeyguardSimPinPukView 
                ...
                final int smallWidgetMinHeight =
                        res.getDimensionPixelSize(com.mediatek.internal.R.dimen.keyguard_small_widget_height_for_phone);
    ...
    /**
     * M: If the LockScreen has been showed firstly, the simUnlock would not show.
     * @return If the unlock mode is simPin and mLockscreen is not null or gone, return true.
     * Otherwise return false.
     */
    private boolean isLockScreenPriorSimPin(SecurityMode mode) {
        KeyguardUtils.xlogD(TAG, "isLockScreenPriorSimPin enter mode=" + mode);
        boolean bPinRequired = false;
        /// M: Support GeminiPlus
        if (mode == SecurityMode.SimPinPukMe1 || mode == SecurityMode.SimPinPukMe2
             || mode == SecurityMode.SimPinPukMe3 || mode == SecurityMode.SimPinPukMe4) {
            bPinRequired = true;
        }
        KeyguardSecurityView oldView = getSecurityView(mCurrentSecuritySelection);
        KeyguardUtils.xlogD(TAG, "isLockScreenPriorSimPin return");
        return bPinRequired && (oldView instanceof KeyguardSelectorView);
    }
    ...
    @Override
    public void reset() {
        showPrimarySecurityScreen(!mScreenOn);
        mIsVerifyUnlockOnly = false;
        /// M: We did a optimization in KeyguardViewManager's reest, so do not reset to default screen
    }
        ...
        if (view == null && layoutId != 0) {
            ...
            if (view instanceof KeyguardSimPinPukView) {
                ...
                if (securityMode == SecurityMode.SimPinPukMe1) {
                    pinPukView.setSimId(PhoneConstants.GEMINI_SIM_1);
                } else if (securityMode == SecurityMode.SimPinPukMe2) {
                    pinPukView.setSimId(PhoneConstants.GEMINI_SIM_2);
                } else if (securityMode == SecurityMode.SimPinPukMe3) {
                    pinPukView.setSimId(PhoneConstants.GEMINI_SIM_3);
                } else if (securityMode == SecurityMode.SimPinPukMe4) {
                    pinPukView.setSimId(PhoneConstants.GEMINI_SIM_4);
                }
            }
    ...
    private void showSecurityScreen(SecurityMode securityMode) {
        ...
        if (KeyguardUtils.isGemini()) {
            isSimOrAccount = isSimOrAccount || (securityMode == SecurityMode.SimPinPukMe1
        ...
        /// @}
        /// M: If dm / alarm boot lock is on, we should also hide widget @{
        boolean dmLocked = KeyguardUpdateMonitor.getInstance(getContext()).dmIsLocked();
        mAppWidgetContainer.setVisibility(
                ((isSimOrAccount && fullScreenEnabled) || dmLocked || KeyguardUpdateMonitor.isAlarmBoot()) ? View.GONE : View.VISIBLE);
        /// @}

        if (mSlidingChallengeLayout != null) {
            /// M: If dm lock is on or not AccountView, we should also set changelayout interactive
            mSlidingChallengeLayout.setChallengeInteractive((securityMode != SecurityMode.Account) || dmLocked);
        }
    ...
    private int getSecurityViewIdForMode(SecurityMode securityMode) {
        switch (securityMode) {
            ...
            case SimPinPukMe1: return com.mediatek.internal.R.id.keyguard_sim_pin_puk_view;
            case SimPinPukMe2: return com.mediatek.internal.R.id.keyguard_sim_pin_puk_view;
            /// M: Support GeminiPlus
            case SimPinPukMe3: return com.mediatek.internal.R.id.keyguard_sim_pin_puk_view;
            case SimPinPukMe4: return com.mediatek.internal.R.id.keyguard_sim_pin_puk_view;            
            /// @}

            /// M: power-off alarm @{
            case AlarmBoot: return com.mediatek.internal.R.id.power_off_alarm_view;
            /// @}
            ///M: add voice unlock view id
            case Voice: return R.id.zz_keyguard_voice_unlock_view;
        }
        return 0;
    }
    ...
    private void addDefaultStatusWidget(int index) {
        LayoutInflater inflater = LayoutInflater.from(mContext);
        mStatusWidget = inflater.inflate(R.layout.keyguard_status_view, null, true);
        mAppWidgetContainer.addWidget(mStatusWidget, index);
    }
    ...
    public boolean handleMenuKey() {
        ...
        if (shouldEnableMenuKey()) {
            ///M: not to use bouncer style for next view
    ...
    public void dismiss() {
        ...
        // default to use bouncer style to show next lock screen
