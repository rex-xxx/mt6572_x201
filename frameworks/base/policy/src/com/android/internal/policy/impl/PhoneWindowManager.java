...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    /// M: [ALPS00569504] Power off alarm screen display divide @{
    boolean mLayoutDisableStatusBar = false;
    /// @}
    ...
    /// M: [ALPS00411109] The statu bar disappear and a blank area displayed. @{
    private boolean mfindingTopFullscreenOpaque = false;
    /// @}
    ...
    public void beginLayoutLw(boolean isDefaultDisplay, int displayWidth, int displayHeight,
        ...
        /// M: [ALPS00569504] Power off alarm screen display divide @{
        mLayoutDisableStatusBar = (mStatusBar == null);

        /// M: Add for Alarm
        if (KeyguardUpdateMonitor.isAlarmBoot() || KeyguardViewMediator.mHideNavBar) {
            if (DEBUG_LAYOUT) Log.v(TAG, "Hiding status bar by for power-off alarm screen");
                ...
                if (mNavigationBarOnBottom) {
                    ...
                    if (navVisible) {
                        if (!KeyguardUpdateMonitor.isAlarmBoot()
                                && !KeyguardViewMediator.mHideNavBar) {
    ...
    public void layoutWindowLw(WindowState win, WindowManager.LayoutParams attrs,
            ...
            if ((fl & (FLAG_LAYOUT_IN_SCREEN | FLAG_FULLSCREEN | FLAG_LAYOUT_INSET_DECOR))
                ...
                if (DEBUG_LAYOUT)
                    Log.v(TAG, "layoutWindowLw(" + attrs.getTitle() 
                            + "): IN_SCREEN, INSET_DECOR, !FULLSCREEN, sim = #" + Integer.toHexString(adjust));
                ...
                if (attached != null) {
                    ...
                    /// M: [ALPS00569504] [ALPS00411109] [ALPS00435427] The statu bar disappear and a blank area displayed. @{
                    if (mfindingTopFullscreenOpaque && !mHasSystemNavBar && !mLayoutDisableStatusBar) {
                        if (cf.top < mUnrestrictedScreenTop + mStatusBarHeight) {
                            cf.top = mUnrestrictedScreenTop + mStatusBarHeight;
                        }

                        if (vf.top < mUnrestrictedScreenTop + mStatusBarHeight) {
                            vf.top = mUnrestrictedScreenTop + mStatusBarHeight;
                        }
                    }
                    /// @}
                }
                ...
                /// M: [ALPS00569504] [ALPS00435427] The statu bar disappear and a blank area displayed.
                ///    [ALPS00449990] The picture shakes when tap the screen in single view @{
                boolean nonFullScreenApplyStable =
                        (sysUiFl & View.SYSTEM_UI_FLAG_LAYOUT_STABLE) != 0 &&
                        (fl & FLAG_FULLSCREEN) == 0;
                if (mfindingTopFullscreenOpaque && !nonFullScreenApplyStable && !mHasSystemNavBar && !mLayoutDisableStatusBar) {
                    if (cf.top > mRestrictedScreenTop) {
                        cf.top = mRestrictedScreenTop;
                    }

                    if (vf.top > mRestrictedScreenTop) {
        ...
        /// M: [ALPS00411109] The statu bar disappear and a blank area displayed. @{
        if (mfindingTopFullscreenOpaque && win.isVisibleLw()) {
    ...
    public void beginPostLayoutPolicyLw(int displayWidth, int displayHeight) {
        ...
        /// M: [ALPS00445635] The idle screen will flash a short time when tap Emergency call @{
        //mHideLockScreen = false; /// Google original code
        /// @}
        ...
        if (mTopFullscreenOpaqueWindowState == null &&
            ...
            if (applyWindow
                ...
                if ((attrs.flags & FLAG_SHOW_WHEN_LOCKED) != 0) {
                    ...
                    /// M: [ALPS00445635] The idle screen will flash a short time when tap Emergency call @{
                    if (DEBUG_LAYOUT) Log.v(TAG, "Setting mHideLockScreen to false by win " + win);
                ...
                if ((attrs.flags & FLAG_DISMISS_KEYGUARD) != 0
                    ...
                    /// M: [ALPS00484759] Handle switch Keyguard secure mode when dismissing @{
                    final boolean isSecure = mKeyguardMediator.isSecure();
                    mDismissKeyguard = mWinDismissingKeyguard == win && mSecureDismissingKeyguard == isSecure ?
    ...
    public int finishPostLayoutPolicyLw() {
        ...
        if (mKeyguard != null) {
            if (localLOGV) Log.v(TAG, "finishPostLayoutPolicyLw: mHideKeyguard="
                    ...
                    + " mKeyguardMediator.isSecure()= " + mKeyguardMediator.isSecure());
                    /// @}
            if (mDismissKeyguard != DISMISS_KEYGUARD_NONE && !mKeyguardMediator.isSecure()) {
                ...
                /// @}
                /// M: [ALPS00082277] In order to enhanced the showing performance
                ///    of keyguard We disable the enter animation.
    ...
    public int interceptKeyBeforeQueueing(KeyEvent event, int policyFlags, boolean isScreenOn) {
        ...
        if (KeyEvent.KEYCODE_POWER == event.getKeyCode()
                && mKeyguardMediator.isAlarmUnlockScreen()) {
            return 0;
        }
        /// @}
        ...
        switch (keyCode) {
            ...
            case KeyEvent.KEYCODE_VOLUME_MUTE: {
                ...
                if (down) {
                    ...
                    if (telephonyService != null) {
                        try {
                            ...
                            if (telephonyService.isOffhook()
                                ...
                                /// M: [ALPS00462787] HFP,HFP Volume cannot change from DUT when LCD OFF with a call on-going @{
                                IAudioService audioService = getAudioService();
                    ...
                    /// M: [ALPS00270178] Adjust Volume key for FM at PhoneWindowManager.java
            ...
            if (screenOnListener != null) {
                mKeyguardMediator.onScreenTurnedOn(new KeyguardViewManager.ShowListener() {
                    ...
                    public void onShown(IBinder windowToken) {
                        /// M: [ALPS00428805] Avoid to output a temp screen if there is no keyguard when screen on. @{
                        if (windowToken == null) {
    ...
    /// M: [ALPS00428805] Avoid to output a temp screen if there is no keyguard when screen on. @{
    private void waitForTopFullscreenWindowDrawn(final ScreenOnListener screenOnListener) {
        IBinder windowToken = null;
        if (mTopFullscreenOpaqueWindowState != null) {
            windowToken = mTopFullscreenOpaqueWindowState.asBinder();
        }

        // Reuse waitForKeyguardWindowDrawn()
        waitForKeyguardWindowDrawn(windowToken, screenOnListener);
    }
    /// @}
    ...
    public void dump(String prefix, PrintWriter pw, String[] args) {
        ...
        if (args.length > 0  && args[0].contains("-d")) {
            ...
            } else if ("-d enable 16".equals(args[0])) {
                //DEBUG_FALLBACK= true;
            } else if ("-d enable 21".equals(args[0])) {
                // Wait for Keyguard to enable it
                //mKeyguardMediator.setDebugFilterStatus(true);
            ...
            } else if ("-d disable 16".equals(args[0])) {
                //DEBUG_FALLBACK= false;
            } else if ("-d disable 21".equals(args[0])) {
                // Wait for Keyguard to enable it
                //mKeyguardMediator.setDebugFilterStatus(false);
            }
        } else {
