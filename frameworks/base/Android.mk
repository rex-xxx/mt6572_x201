...
ifneq ($(PARTIAL_BUILD),true)
  ifeq (,$(DO_GET_ARTIFACTS))
    ifneq (,$(ARTIFACT.$(LOCAL_MODULE).JARS))
      $(full_target):out/target/common/obj/JAVA_LIBRARIES/gemini_dummy_intermediates/classes-full-debug.jar
    endif
  endif
endif
...
LOCAL_JAVA_LIBRARIES := bouncycastle core core-junit ext CustomProperties
LOCAL_JAVA_LIBRARIES += mediatek-common

LOCAL_MODULE := framework
LOCAL_MODULE_CLASS := JAVA_LIBRARIES

# Add by mtk01411 for MR1 (build frameworks/opt/telephony into frameworks.jar)
# Note that: LOCAL_AIDL_INCLUDES uses the rootdir as the starting path not frameworks/base
LOCAL_AIDL_INCLUDES += frameworks/opt/telephony/src/java
LOCAL_SRC_FILES += \
	../opt/telephony/src/java/com/android/internal/telephony/ISms.aidl \
  ../opt/telephony/src/java/com/android/internal/telephony/IIccPhoneBook.aidl \
  ../opt/telephony/src/java/com/android/internal/telephony/EventLogTags.logtags

LOCAL_SRC_FILES += $(call all-java-files-under, ../opt/telephony/src/java)

LOCAL_SECONDARY_JAVA_CLASSES += android/app/* android/view/* android/widget/*

# Use viatelecomjar
LOCAL_STATIC_JAVA_LIBRARIES := libNq viatelecomjar
...
ifneq (,$(DO_GET_ARTIFACTS))
  ifeq (,$(ARTIFACT.$(LOCAL_MODULE).JARS))
...
#This is only for BT_UT
ifeq ($(strip $(BT_UT)),yes)
   LOCAL_EMMA_COVERAGE_FILTER := @$(LOCAL_PATH)/bluetooth_emma_filter.txt
   FRAMEWORKS_BASE_UTDIRS := ../../mediatek/packages/apps/Bluetooth/tests/common/src/android/server
   FRAMEWORKS_BASE_A2DP_UTDIRS := ../../mediatek/packages/apps/Bluetooth/tests/profiles/a2dp/src/android/server
   FRAMEWORKS_BASE_HDP_UTDIRS := ../../mediatek/packages/apps/Bluetooth/tests/profiles/hdp/src/android/server
   LOCAL_SRC_FILES += $(call find-other-java-files,$(FRAMEWORKS_BASE_UTDIRS))
   LOCAL_SRC_FILES += $(call find-other-java-files,$(FRAMEWORKS_BASE_A2DP_UTDIRS))
   LOCAL_SRC_FILES += $(call find-other-java-files,$(FRAMEWORKS_BASE_HDP_UTDIRS))
endif
...
aidl_files += \
	$(LOCAL_PATH)/../../mediatek/frameworks-ext/base/core/java/android/bluetooth/BluetoothDevice.aidl \
	$(LOCAL_PATH)/../../mediatek/frameworks-ext/base/core/java/android/bluetooth/BluetoothHealthAppConfiguration.aidl
#else 
#aidl_files += \
...
# Build secondary-framework.jar
# ============================================================
include $(CLEAR_VARS)
LOCAL_SRC_FILES := $(call all-java-files-under, core/java/android/app)
LOCAL_SRC_FILES += $(call all-java-files-under, core/java/android/view)
LOCAL_SRC_FILES += $(call all-java-files-under, core/java/android/widget)
LOCAL_SRC_FILES += $(call all-java-files-under,  ../../mediatek/frameworks-ext/base/core/java/android/app/)
LOCAL_SRC_FILES += $(call all-java-files-under,  ../../mediatek/frameworks-ext/base/core/java/android/view/)
LOCAL_SRC_FILES += $(call all-java-files-under,  ../../mediatek/frameworks-ext/base/core/java/android/widget/)
...
LOCAL_NO_STANDARD_LIBRARIES := true
LOCAL_MODULE_TAGS := optional
LOCAL_MODULE := secondary-framework
